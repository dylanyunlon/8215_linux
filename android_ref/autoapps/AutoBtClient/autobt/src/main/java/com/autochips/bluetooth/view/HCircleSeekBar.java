...
public class HCircleSeekBar extends View {
    ...
    private float mDefShadowOffset = 2.0f;
    ...
    public HCircleSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HCircleSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(AttributeSet attrs, int defStyle) {
        ...
        mMaxProcess = a.getInt(R.styleable.CircleSeekBar_circle_max_process, 100);
        mCurProcess = a.getInt(R.styleable.CircleSeekBar_circle_cur_process, 0);
        if (mCurProcess > mMaxProcess) mCurProcess = mMaxProcess;
        mReachedColor = a.getColor(R.styleable.CircleSeekBar_circle_reached_color, 0xFF3AA1ff);
        mUnreachedColor = a.getColor(R.styleable.CircleSeekBar_circle_unreached_color,
                0x00000000);
        mUnreachedWidth = a.getDimension(R.styleable.CircleSeekBar_circle_unreached_width,
                4);
        isHasReachedCornerRound = a.getBoolean(R.styleable.CircleSeekBar_circle_reached_has_corner_round, true);
        mReachedWidth = a.getDimension(R.styleable.CircleSeekBar_circle_reached_width, mUnreachedWidth);
        mPointerColor = a.getColor(R.styleable.CircleSeekBar_circle_pointer_color, 0xFFFFFFFF);
        mPointerRadius = a.getDimension(R.styleable.CircleSeekBar_circle_pointer_radius, mReachedWidth / 2);
        isHasWheelShadow = a.getBoolean(R.styleable.CircleSeekBar_circle_has_wheel_shadow, false);
        if (isHasWheelShadow) {
            mWheelShadowRadius = a.getDimension(R.styleable.CircleSeekBar_circle_shadow_radius,
                    2);
        }
        isHasPointerShadow = a.getBoolean(R.styleable.CircleSeekBar_circle_has_pointer_shadow, false);
        if (isHasPointerShadow) {
            mPointerShadowRadius = a.getDimension(R.styleable.CircleSeekBar_circle_pointer_shadow_radius,
                    3);
        }
        isHasCache = a.getBoolean(R.styleable.CircleSeekBar_circle_has_cache, isHasWheelShadow);
        isCanTouch = a.getBoolean(R.styleable.CircleSeekBar_circle_can_touch, true);
        isScrollOneCircle = a.getBoolean(R.styleable.CircleSeekBar_circle_scroll_only_one_circle, false);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        //boolean isToch=isTouch(x, y);
        Log.d("CircleSeekBar", "----------onTouchEvent    isCanTouch=" + isCanTouch);

        // if (isCanTouch && (event.getAction() == MotionEvent.ACTION_MOVE || isTouch(x, y))) {
        if (isCanTouch && (event.getAction() == MotionEvent.ACTION_MOVE
            ...
            if (mChangListener != null && (event.getAction() & (MotionEvent.ACTION_UP)) > 0) {
                Log.d("CircleSeekBar", "----------ACTION_UP ACTION_CANCEL 2 -1");
                mChangListener.onStopTrackingTouch(this, mCurProcess);
            }

            if (mChangListener != null && event.getAction() == MotionEvent.ACTION_DOWN) {
                Log.d("CircleSeekBar", "----------ACTION_DOWN 5 -1");
                mChangListener.onStartTrackingTouch(this);
            }
    ...
    public interface OnSeekBarChangeListener {
        void onChanged(HCircleSeekBar seekbar, int curValue);

        void onStopTrackingTouch(HCircleSeekBar seekbar, int curValue);

        void onStartTrackingTouch(HCircleSeekBar seekBar);
    }
}
