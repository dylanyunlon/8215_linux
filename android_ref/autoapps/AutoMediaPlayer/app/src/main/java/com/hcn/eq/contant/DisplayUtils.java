...
public class DisplayUtils {
    ...
    public static void overridingPendingAnim(Activity activity, int inAnim, int outAnim) {
        ...
        LogUtil.d(TAG, "OverridePending:Activity=" + activity);
    ...
    public static Bitmap getBlurBmp(Context context, Bitmap srcBmp, float scale) {
        try {
            Bitmap dstBmp = Bitmap.createScaledBitmap(srcBmp, (int) (srcBmp.getWidth() * scale),
                    (int) (srcBmp.getHeight() * scale), false);

            Class<?> cScreenShotUtils = Class.forName("miui.util.ScreenshotUtils");
            Method mGetBlurBackground = cScreenShotUtils.getMethod("getBlurBackground",
    ...
    public static Bitmap getLockscreenBmp(Context context) {
        try {
            ...
            mGetLockWallpaperCache = cThemeResources.getDeclaredMethod("getLockWallpaperCache",
                    Context.class);
            BitmapDrawable backgound = (BitmapDrawable) mGetLockWallpaperCache.invoke(
                    cThemeResources, context);
