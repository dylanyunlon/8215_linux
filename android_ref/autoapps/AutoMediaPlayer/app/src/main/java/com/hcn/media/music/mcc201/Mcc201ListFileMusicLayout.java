...
public class Mcc201ListFileMusicLayout extends FrameLayoutEx
        implements OnItemClickListener, OnScrollListener,
        OnClickListener, OnTouchListener {

    private static final String TAG = Mcc201ListFileMusicLayout.class.getSimpleName();
    ...
    private final List<MusicFilesInfo> mListMusicFile = new ArrayList<MusicFilesInfo>();
    ...
    public Mcc201ListFileMusicLayout(Context context,
    ...
    private void initView() {
        LayoutInflater.from(mContext)
                .inflate(R.layout.mcc201_layout_folder, this, true);
    ...
    private void initData() {
        ...
        for (int i = 0; i < mAppData.mStorageDeviceList.size(); i++) {
            mapPath.putAll(mAppData.mStorageDeviceList.get(i).mPathListMap);
        }
    ...
    private void updatePlayIndex() {
        if (null != mAppData.mCurrentMediaInfo) {
            String parentRoute = Objects.requireNonNull(new File(
    ...
    public void setMediaEventListener(MediaEventPostbox listener) {
        ...
        if (null != mFolderListAdapter) {
            mFolderListAdapter.setMediaEventListener(listener);
        }
    }
    ...
    private void onChangeAllListView() {
        if (null != mLayoutCallback) {
            mLayoutCallback.onCallback(mStorageDeviceEx,
                    IMediaEvent.EVENT_GOTO_ALL_LIST_PAGE);
        }
    }
