...
/**
 * 系统原生 MediaPlayer 播放器
 * <p> 原生播放器局限于平台硬解码能力，兼容性差；
 * @author 65821
 */
public class SystemPlayerManager extends BasePlayerManager {
    private static final String TAG = "SystemPlayerManager";
        ...
        try {
            if (funModel.isCache() && cacheManager != null) {
                cacheManager.doCacheLogic(context, mediaPlayer,
                        funModel.getUrl(), funModel.getMapHeadData(), funModel.getCachePath());
            } else {
                mediaPlayer.setDataSource(context,
                        Uri.parse(funModel.getUrl()), funModel.getMapHeadData());
            }
            mediaPlayer.setLooping(funModel.isLooping());
            if (funModel.getSpeed() != 1 && funModel.getSpeed() > 0) {
                setSpeed(funModel.getSpeed());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        if (mediaPlayer != null && mediaPlayer.getInternalMediaPlayer() != null && mediaPlayer.isPlayable()) {
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    LogUtils.iTag(TAG, "not support setSpeed");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
