package com.hcn.media_dummy.render.glrender;
...
public abstract class FunVideoGLViewBaseRender
    ...
    protected FunVideoGLRenderErrorListener mVideoGLRenderErrorListener;
    ...
    protected int loadShader(int shaderType, String source) {
        ...
        if (shader != 0) {
            ...
            GLES20.glGetShaderiv(shader, GLES20.GL_COMPILE_STATUS,
                    compiled, 0);
            if (compiled[0] == 0) {
                LogUtils.vTag(Config.TAG, "Could not compile shader " + shaderType + ":");
                LogUtils.vTag(Config.TAG, GLES20.glGetShaderInfoLog(shader));
    ...
    protected int createProgram(String vertexSource, String fragmentSource) {
        ...
        if (program != 0) {
            ...
            if (linkStatus[0] != GLES20.GL_TRUE) {
                LogUtils.vTag(Config.TAG, "Could not link program: ");
                LogUtils.vTag(Config.TAG, GLES20.glGetProgramInfoLog(program));
    ...
    protected void checkGlError(final String op) {
        ...
        if ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
            LogUtils.eTag(Config.TAG, op + ": glError " + error);

            mHandler.post(() -> {
                if (mVideoGLRenderErrorListener != null) {
                    mVideoGLRenderErrorListener.onError(
                            FunVideoGLViewBaseRender.this,
    ...
    public void setGLSurfaceListener(GLSurfaceListener onSurfaceListener) {
        mGLSurfaceListener = onSurfaceListener;
    }
    ...
    /**
     * 设置滤镜效果
     * @param shaderEffect 着色器
     */
    public void setEffect(FunVideoGLView.ShaderInterface shaderEffect) {
    }

    public FunVideoGLView.ShaderInterface getEffect() {
        return null;
    }
    ...
    public void setVideoGLRenderErrorListener(FunVideoGLRenderErrorListener videoGLRenderErrorListener) {
        mVideoGLRenderErrorListener = videoGLRenderErrorListener;
    }
}