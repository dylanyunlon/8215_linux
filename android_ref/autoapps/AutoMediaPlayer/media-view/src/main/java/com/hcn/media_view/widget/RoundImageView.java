...
public class RoundImageView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    private boolean isCircular; // 是否显示为圆形，如果为圆形则设置的corner无效
    ...
    public RoundImageView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < ta.getIndexCount(); i++) {
            ...
            } else if (attr == R.styleable.RoundImageView_is_circular) {
                isCircular = ta.getBoolean(attr, isCircular);
            } else if (attr == R.styleable.RoundImageView_external_border_width) {
                borderWidth = ta.getDimensionPixelSize(attr, borderWidth);
            } else if (attr == R.styleable.RoundImageView_external_border_color) {
    ...
    public void isCircle(boolean isCircular) {
