...
public class MediaPlayer {
    ...
    public static final String[] SUB_TYPES =
            {".srt", ".ssa", ".smi", ".txt", ".sub", ".ass", ".webvtt"};
     ...
     * </p>
     *
     * @return  MediaPlayer will try to use hardware accelerated decoder if true
     */
    ...
    private static void postEventFromNative(Object mediaplayer_ref, int what, int arg1, int arg2,
                                            Object obj) {
    ...
    private TrackInfo[] getInbandTrackInfo(String encoding) {
        if (mInbandTracks == null) {
            ...
            for (int i = 0; i < size; i++) {
                SparseArray<MediaFormat> sparseArray = parseTrackInfo(trackSparse.valueAt(i),
                        encoding);
    ...
    private void surfaceRender() {
        synchronized (this) {
            if (mLocalSurface == null || !mLocalSurface.isValid() || mBitmap == null
                    || mByteBuffer == null) {
                return;
            }
        ...
        private void onBufferingUpdate(Message msg) {
            ...
            if (percent >= 100 && mInBuffering) {
                ...
                if (mOnInfoListener != null) {
                    ...
                    mOnInfoListener.onInfo(mMediaPlayer, MediaPlayer.MEDIA_INFO_BUFFERING_END,
                            percent);
                }
            }
        }
            ...
            switch (msg.what) {
                ...
                case MEDIA_SET_VIDEO_SIZE:
                    if (mOnVideoSizeChangedListener != null) {
                        mOnVideoSizeChangedListener.onVideoSizeChanged(mMediaPlayer, msg.arg1,
                                msg.arg2);
                    }
                    return;
                case MEDIA_ERROR:
                    ...
                    if (mOnErrorListener != null) {
                        error_was_handled = mOnErrorListener.onError(mMediaPlayer, msg.arg1,
                                msg.arg2);
                    }
                ...
                case MEDIA_TIMED_TEXT:
                    ...
                    if (mData.getInt(MEDIA_SUBTITLE_TYPE) == SUBTITLE_TEXT) {
                        ...
                        if (mOnTimedTextListener != null) {
                            mOnTimedTextListener.onTimedText(
                                    mData.getString(MEDIA_SUBTITLE_STRING));
                        }
                    } else if (mData.getInt(MEDIA_SUBTITLE_TYPE) == SUBTITLE_BITMAP) {
                        Log.i("Subtitle : bitmap");
                        if (mOnTimedTextListener != null) {
                            mOnTimedTextListener.onTimedTextUpdate(
                                    mData.getByteArray(MEDIA_SUBTITLE_BYTES), msg.arg1, msg.arg2);
                        }
                    }
                    return;
                case MEDIA_CACHING_UPDATE:
                    if (mOnCachingUpdateListener != null) {
                        ...
                        if (cacheType == CACHE_TYPE_NOT_AVAILABLE) {
                            ...
                            mOnCachingUpdateListener.onCachingSpeed(mMediaPlayer,
                                    msg.getData().getInt(MEDIA_CACHING_INFO));
