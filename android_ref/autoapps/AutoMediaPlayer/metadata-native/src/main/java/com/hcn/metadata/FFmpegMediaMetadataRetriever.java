...
/**
 * 基于 ffmpeg 的媒体元数据检索类
 * <p> 这是一个开源库（致敬 William Seemann），由于作者不再维护，这里由我们自己根据需求编译更新裁剪到最新版本。
 *
 * @Author youwj
 * @Create 2021/5/14 20:15
 */
public class FFmpegMediaMetadataRetriever {
    private final static String TAG = "FFmpegMediaMetadataRetriever";

    /**
     * 用户定义的位图配置，位图配置描述了如何存储像素；
     * 这会影响质量（颜色深度）以及显示透明/半透明颜色的能力。
     */
    ...
    /**
     * Sets the data source (file pathname) to use. Call this method before the rest of the methods in this class. This
    ...
    /**
     * Sets the data source (URI) to use. Call this method before the rest of the methods in this class. This method may
     * be time-consuming.
     *
    ...
    /**
     * Sets the data source (FileDescriptor) to use.  It is the caller's responsibility to close the file descriptor. It
     * is safe to do so as soon as this call returns. Call this method before the rest of the methods in this class.
     ...
     * @param length the length in bytes of the data to be played. It must be non-negative.
    ...
    /**
     * Sets the data source as a content Uri. Call this method before the rest of the methods in this class. This method
     ...
     * <p>
     * The keyCode currently supported is listed below as METADATA_XXX constants. With any other value, it returns a
     * null pointer.
     *
     * @param key One of the constants listed below at the end of the class.
     * @return The meta data value associate with the given keyCode on success; null on failure.
     */
    public native String extractMetadata(String key);

    /**
     * Call this method after setDataSource(). This method retrieves the meta data value associated with the keyCode
     * from a specific chapter, if available.
     * <p>
     * The keyCode currently supported is listed below as METADATA_XXX constants. With any other value, it returns a
     * null pointer.
     *
     ...
     * <p>
     * //* @param update_only controls whether the full set of available //* metadata is returned or just the set that
     * changed since the //* last call. See {@see #METADATA_UPDATE_ONLY} and {@see //* #METADATA_ALL}. //* //* @param
     * apply_filter if true only metadata that matches the //* filter is returned. See {@see #APPLY_METADATA_FILTER} and
     * {@see //* #BYPASS_METADATA_FILTER}. //*
     *
     ...
     */
    private native HashMap<String, String> native_getMetadata(boolean update_only,
                                                              boolean apply_filter,
                                                              HashMap<String, String> reply);

    /**
     * Call this method after setDataSource(). This method finds a representative frame close to the given time position
     * by considering the given option if possible, and returns it as a bitmap. This is useful for generating a
     * thumbnail for an input data source or just obtain and display a frame at the given time position.
     *
     * @param timeUs The time position where the frame will be retrieved. When retrieving the frame at the given time
     * position, there is no guarantee that the data source has a frame located at the position. When this happens, a
     * frame nearby will be returned. If timeUs is negative, time position and option will ignored, and any frame that
     * the implementation considers as representative may be returned.
     * @param option a hint on how the frame is found. Use {@link #OPTION_PREVIOUS_SYNC} if one wants to retrieve a sync
     * frame that has a timestamp earlier than or the same as timeUs. Use {@link #OPTION_NEXT_SYNC} if one wants to
     * retrieve a sync frame that has a timestamp later than or the same as timeUs. Use {@link #OPTION_CLOSEST_SYNC} if
     * one wants to retrieve a sync frame that has a timestamp closest to or the same as timeUs. Use {@link
     * #OPTION_CLOSEST} if one wants to retrieve a frame that may or may not be a sync frame but is closest to or the
     * same as timeUs. {@link #OPTION_CLOSEST} often has larger performance overhead compared to the other options if
     * there is no sync frame located at timeUs.
    ...
    /**
     * Call this method after setDataSource(). This method finds a representative frame close to the given time position
     * if possible, and returns it as a bitmap. This is useful for generating a thumbnail for an input data source. Call
     * this method if one does not care how the frame is found as long as it is close to the given time; otherwise,
     * please call {@link #getFrameAtTime(long, int)}.
     *
     * @param timeUs The time position where the frame will be retrieved. When retrieving the frame at the given time
     * position, there is no guarentee that the data source has a frame located at the position. When this happens, a
     * frame nearby will be returned. If timeUs is negative, time position and option will ignored, and any frame that
     * the implementation considers as representative may be returned.
     * @return A Bitmap containing a representative video frame, which can be null, if such a frame cannot be retrieved.
    ...
    /**
     * Call this method after setDataSource(). This method finds a representative frame at any time position if
     * possible, and returns it as a bitmap. This is useful for generating a thumbnail for an input data source. Call
     * this method if one does not care about where the frame is located; otherwise, please call {@link
     * #getFrameAtTime(long)} or {@link #getFrameAtTime(long, int)}
     *
     * @return A Bitmap containing a representative video frame, which can be null, if such a frame cannot be retrieved.
     ...
     * if possible, and returns it as a bitmap. This is useful for generating a thumbnail for an input data source. Call
     * this method if one does not care how the frame is found as long as it is close to the given time; otherwise,
     * please call {@link #getScaledFrameAtTime(long, int, int, int)}.
     *
     * @param timeUs The time position where the frame will be retrieved. When retrieving the frame at the given time
     * position, there is no guarentee that the data source has a frame located at the position. When this happens, a
     * frame nearby will be returned. If timeUs is negative, time position and option will ignored, and any frame that
    ...
    /**
     * Call this method after setDataSource(). This method finds the optional graphic or album/cover art associated
     * associated with the data source. If there are more than one pictures, (any) one of them is returned.
     *
    ...
    /**
     * Sets the {@link Surface} to be used as the sink for the video portion of the media. This is similar to
     * {@link MediaPlayer#setDisplay(SurfaceHolder)}, but does not support {@link MediaPlayer#setScreenOnWhilePlaying(boolean)}.
     * Setting a Surface will un-set any Surface or SurfaceHolder that was previously set. A null surface will
     * result in only the audio track being played.
     * <p>
     * If the Surface sends frames to a {@link SurfaceTexture}, the timestamps returned from {@link
     * SurfaceTexture#getTimestamp()} will have an unspecified zero point.  These timestamps cannot be directly compared
     * between different media sources, different instances of the same media source, or multiple runs of the same
     * program.  The timestamp is normally monotonically increasing and is unaffected by time-of-day adjustments, but it
     * is reset when the position is set.
     *
    ...
    /**
     * This option is used with {@link #getFrameAtTime(long, int)} to retrieve a sync (or key) frame associated with a
     * data source that is located right before or at the given time.
     *
     * @see #getFrameAtTime(long, int)
     */
    ...
    /**
     * This option is used with {@link #getFrameAtTime(long, int)} to retrieve a sync (or key) frame associated with a
     * data source that is located right after or at the given time.
     *
     * @see #getFrameAtTime(long, int)
     */
    ...
    /**
     * This option is used with {@link #getFrameAtTime(long, int)} to retrieve a sync (or key) frame associated with a
     * data source that is located closest to (in time) or at the given time.
     *
     * @see #getFrameAtTime(long, int)
     */
    ...
    /**
     * This option is used with {@link #getFrameAtTime(long, int)} to retrieve a frame (not necessarily a key frame)
    ...
    /**
     * The metadata key to retrieve the main creator of the set/album, if different from artist. e.g. "Various Artists"
    ...
    /**
     * The metadata key to retrieve the artist who performed the work, if different from artist. E.g for "Also sprach
     * Zarathustra", artist would be "Richard Strauss" and performer "London Philharmonic Orchestra".
     */
    ...
    /**
     * This key retrieves the video rotation angle in degrees, if available. The video rotation angle may be 0, 90, 180,
     * or 270 degrees.
     */
    ...
    /**
     * Class to hold the media's metadata.  Metadata are used for human consumption and can be embedded in the media
     * (e.g shoutcast) or available from an external source. The source can be local (e.g thumbnail stored in the DB) or
     * remote.
     * <p>
     * Metadata is like a Bundle. It is sparse and each key can occur at most once. The key is an integer and the value
     ...
     * <p>
     * //@hide //@deprecated Use {@link MediaMetadata}.
     */
    public static class Metadata {
        ...
        /**
         * Check a parcel containing metadata is well formed. The header is checked as well as the individual records
         * format. However, the data inside the record is not checked because we do lazy access (we check/unmarshall
         * only data the user asks for.)
         * <p>
         ...
         * </pre>
         *
         * @param metadata With the serialized data. Metadata keeps a reference on it to access it later on. The caller
         * should not modify the parcel after this call (and not call recycle on it.)
        ...
        public int getInt(final String key) {
            ...
            return Integer.parseInt(Objects.requireNonNull(mParcel.get(key)));
        }
        ...
        public boolean getBoolean(final String key) {
            ...
            return Integer.parseInt(Objects.requireNonNull(mParcel.get(key))) == 1;
        }
        ...
        public long getLong(final String key) {
            ...
            return Long.parseLong(Objects.requireNonNull(mParcel.get(key)));
        }
        ...
        public double getDouble(final String key) {
            ...
            return Double.parseDouble(Objects.requireNonNull(mParcel.get(key)));
        }
        ...
        public byte[] getByteArray(final String key) {
            ...
            return Objects.requireNonNull(mParcel.get(key)).getBytes();
        }
        ...
        public Date getDate(final String key) {
            ...
            final long timeSinceEpoch = Long.parseLong(Objects.requireNonNull(mParcel.get(key)));
