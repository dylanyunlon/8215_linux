...
public class RoundKnobSeekBar extends View {
    ...
    private static final int DEFAULT_MAX_VALUE = 410;               // 默认最大数值
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mArcColors = getColorsByArrayResId(context, R.array.round_knob_seek_bar_arc_colors_default); // 获取 Arc 颜色数组
        ...
        initKnobScrollValue(true);
        int progress = ta.getInt(R.styleable.round_knob_seek_bar_attr_arc_progress, mMinValue);
        setProgress(progress);
        mDrawEdge = ta.getBoolean(R.styleable.round_knob_seek_bar_attr_arc_draw_edge, false);;
    ...
    //根据AM/FM来计算数值
    public void initKnobScrollValue(boolean isFM) {
        if (isFM) {
            mMaxValue = (FMDataControl.mRadioParameters.FmMax - FMDataControl.mRadioParameters.FmMin) / FMDataControl.mRadioParameters.FmStep;
        } else {
            mMaxValue =  (FMDataControl.mRadioParameters.AmMax - FMDataControl.mRadioParameters.AmMin)  / FMDataControl.mRadioParameters.AmStep;
        }
    ...
    private void initData() {
        ...
        DEFAULT_EDGE_LENGTH = (int) getResources().getDimension(SkinUtils.getId(R.dimen.round_knob_seek_bar_width));
    }
