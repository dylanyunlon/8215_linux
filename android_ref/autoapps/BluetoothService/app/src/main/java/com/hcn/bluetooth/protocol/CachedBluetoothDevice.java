...
public class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    private boolean mUnpairAfterDisconnect;
    ...
    void onProfileStateChanged(LocalBluetoothProfile profile, int newProfileState) {
        ...
        /// M:Add for A2DP Sink.
        if (isA2dpSinkSupport(mLocalAdapter) &&
        ...
        if (newProfileState == BluetoothProfile.STATE_CONNECTED) {
            ...
            if (mLocalNapRoleConnected && profile instanceof PanProfile &&
                    ((PanProfile) profile).isLocalRoleNap(mDevice)) {
            ...
            if (profile instanceof HeadsetClientProfile) {
                if (isUnpairAfterDisconnect()) {
                    setUnpairAfterDisconnect(false);
                    unpair();
                }
            }
        }
    ...
    public void disconnect() {
        Log.d(TAG,"disconnect all connected profiles");
        for (LocalBluetoothProfile profile : mProfiles) {
            Log.d(TAG,"profile == "+profile + " state: " + profile.getConnectionStatus(mDevice));
            //if(profile.getConnectionStatus(mDevice) == BluetoothProfile.STATE_CONNECTED)
                disconnect(profile);
        }
    ...
    public boolean disconnect(LocalBluetoothProfile profile) {
    ...
    synchronized boolean connectInt(LocalBluetoothProfile profile) {
        ...
        if (profile.getConnectionStatus(mDevice) == BluetoothProfile.STATE_CONNECTED) {
            Log.d(TAG, "connectInt: already connected " + profile.toString());
            return true;
        }
        Log.d(TAG, "connectInt, mDevice = " + mDevice);
    ...
    public void connectProfileName(String profileName) {
		...
		if (profileName.equals(res.getString(R.string.bluetooth_profile_a2dp))){
            ...
            mHeadsetClientProfile.setPreferred(mDevice, true);
            connectProfile(mHeadsetClientProfile);
        } else if (profileName.equals(res.getString(R.string.bluetooth_profile_pan_NAP))) {
            PanProfile panProfile = mProfileManager.getPanProfile();
            panProfile.setPreferred(mDevice, true);
            connectProfile(panProfile);
        }
    }
    ...
    public void setConnectAfterPairing(boolean auto_connect){
        mConnectAfterPairing=auto_connect;
    }
    public boolean isUnpairAfterDisconnect() {
        return mUnpairAfterDisconnect;
    }
    public void setUnpairAfterDisconnect(boolean auto_unpair){
        mUnpairAfterDisconnect=auto_unpair;
    }
