...
static void getAppName(pid_t pid, char *buff)
{
    char filename[20] = {0};
    FILE *f = NULL;
    ...
    // 针对 kugou 这个特别处理
    if (!strcmp(buff, "com.kugou.android.support")) {
        strcpy(buff, "com.kugou.android");
    }
...
static char sMediaList[2 * 1024];

static void loadMediaPackages(void)
{
    const char *FILELIST = "/data/readonly/media_packages.txt";

    int fd = open(FILELIST, O_RDONLY);
    if (fd > 0) {
        read(fd, sMediaList, sizeof(sMediaList));
        close(fd);
    } else {
        sMediaList[0] = 0;
    }
...
static const char * const sMediaPackages = 
    "com.edog.car,"
    "cn.kuwo.kwmusiccar,"
    ...
    "com.ximalaya.ting.android.dingwei:playe,"
    "com.zoulou.dab,"//DAB hardstone
    "com.hcn.audioinputsource"//AUX Audio.
    ...
    "com.zjinnova.zlink,"
    "/system/bin/LinkSDKServer"
    "/system/bin/gocsdk"
    "/system/bin/blink"
    "com.ss.android.ugc.aweme.lite";

static const char * const sNaviPackages = 
    ...
    "com.skype.rover,"
    "com.tencent.mm.title.navi,"
    "com.gp.dvr,com.fvision.camera"
    "com.txznet.nav,"
    "com.google.android.apps,"
    "com.google.android.apps.maps,"
    "com.navngo.igo.javaclient," // IGO
    "com.nng.igo.primong.igoworld,"
    "com.basarsoft.igonextgen.javaclient,"   // IGO
    "com.mireo.dontpanic.inf,"   // REIS
    "ru.yandex.yandexnavi,"      // yandex
    "com.here.app.maps,"         // HERE WE GO
    "com.sygic.aura,"            // sygic
    "cityguide.probki.net,"      // cityguide
    "com.waze,"                  // waze
    "com.autonavi.amapauto,"
    "com.autonavi.amapautolite,"
    "com.papago.M11SGMY";

static void sendAudioTrackStatus(int duration, int session, const char * const name, String16 state)
{
    if (!strcmp("system_server", name)) {
        ALOGE("sendAudioTrackStatus[intercept 'system_server' track event], status %s", String8(state).string());
        return;
    }

    sp<IServiceManager> sm = defaultServiceManager();
    sp<IBinder> binder = sm->getService(String16("sourceinfo"));
    if (binder != NULL)
    {
        Parcel data;
        data.writeInterfaceToken(String16("android.sourceservice.ISourceInfo"));
        data.writeString16(String16(name));
        data.writeInt32(session);
        data.writeInt32(duration);
        data.writeString16(state);
        status_t ret = binder->transact(IBinder::FIRST_CALL_TRANSACTION + 400, data, NULL, IBinder::FLAG_ONEWAY);
        (void) ret;
    }
}
        ...
        audio_port_handle_t selectedDeviceId)
{
    ...
    // @Tag: HCN2024_T
    bool isMediaServer = true;
    mIsMusic = false;
    mMusicTotalDuration = 0;
    mSendAtDuration = 0;
    mEnableCustomization = property_get_bool("sys.debug.audio_track_en", true);
    ...
    if (pAttributes == NULL) {
        ...
        mOriginalStreamType = AudioSystem::attributesToStreamType(*pAttributes);
        ALOGD("%s(): Building AudioTrack with attributes:"
                " usage = %d, content = %d, flags = 0x%x, tags = [%s]",
                __func__,
                 mAttributes.usage, mAttributes.content_type, mAttributes.flags, mAttributes.tags);
    }
...
// @ Begin HCN2019
    ...
    if (mEnableCustomization)
    {
        getAppName(myPid, mProcessName);
        loadMediaPackages();
        ...
        // 判断这个是从哪里调用，只有不是 mediaserver 自己调用才发送状态到 SourceService
        if (strcmp("/system/bin/mediaserver", mProcessName)) {
            isMediaServer = false;
            bool naviStreamEnable = property_get_bool("sys.debug.navi_stream_en", false);
            if (naviStreamEnable) {
                char value[PROPERTY_VALUE_MAX];
                property_get("sys.navi.packages", value, "");
                if (strstr(sNaviPackages, mProcessName) || strstr(value, mProcessName) || strstr(mProcessName, "cld.navi")) {
                    mOriginalStreamType = AUDIO_STREAM_NOTIFICATION;
                }
            }

            if ((strstr(sMediaList, mProcessName) || strstr(sMediaPackages, mProcessName)) && (mStreamType == AUDIO_STREAM_MUSIC)) {
                mIsMusic = true;

                //腾讯爱趣听 进入就创建audiotrack但是不请求焦点，导致其他常驻应用不会暂停播放，结果就是腾讯爱趣听被杀掉
                //所有这里不在发送状态给SourceService
                //sendAudioTrackStatus(899900, mSessionId, mProcessName, String16(""));
            }
        }
    }
...
//////////////////////////////////
    ...
    if (mEnableCustomization)
    {
        if (!isMediaServer && !mIsMusic) {
            if ((strstr(sMediaList, mProcessName) 
                    || strstr(sMediaPackages, mProcessName)) && (mStreamType == AUDIO_STREAM_MUSIC)) {
                mIsMusic = true;
            }
        }
    }
    ALOGI("[HCN]%s(): mProcessName=%s, streamType = %d mSessionId=%d" , __func__, mProcessName, mStreamType, mSessionId);
...
status_t AudioTrack::start()
{
    ...
    // @Tag: HCN2024_T
    if (mIsMusic) {
        sendAudioTrackStatus(9990000, mSessionId, mProcessName, String16("start"));
    }
...
void AudioTrack::stop()
{
    ...
    // @Tag: HCN2024_T
    if (mIsMusic) {
        sendAudioTrackStatus(9990000, mSessionId, mProcessName, String16("stopped"));
    }
...
void AudioTrack::pause()
{
    ...
    // @Tag: HCN2024_T
    if (mIsMusic) {
        sendAudioTrackStatus(9990000, mSessionId, mProcessName, String16("paused"));
    }
...
// @Tag: HCN2024_T
static bool isSoundable(const char * buffer, int32_t userSize)
{
    bool muted = 1;

    if (userSize > 64) {
        muted &= (buffer[10] == 0) && (buffer[11] == 0);
        muted &= (buffer[56] == 0) && (buffer[57] == 0);
    }

    if (userSize > 256) {
        muted &= (buffer[170] == 0) && (buffer[171] == 0);
        muted &= (buffer[181] == 0) && (buffer[182] == 0);
    }

    if (userSize > 1000) {
        muted &= (buffer[810] == 0) && (buffer[811] == 0);
        muted &= (buffer[681] == 0) && (buffer[682] == 0);
    }

    if (userSize > 2000) {
        muted &= (buffer[1910] == 0) && (buffer[1911] == 0);
        muted &= (buffer[1481] == 0) && (buffer[1482] == 0);
    }
    return !muted;
}
...
// @ Begin HCN2019
    ...
    if (mEnableCustomization)
    {
        // 待机时不允许播放声音，否则待机失败
        char value[PROPERTY_VALUE_MAX];
        property_get("sys.running.phase", value, "");
        if (strncmp(value, "suspend", 7) == 0) {
            usleep(1000 * 100);
            return userSize;
        }

        // 当前不是蓝牙音乐，但是蓝牙音频在播放，果断拒绝之。。。
        /* property_get("sys.sourcing.package", value, "");
        if ((mStreamType == AUDIO_STREAM_MUSIC) && strcmp(value, "com.autochips.bluetooth") && !strcmp("com.android.bluetooth", mProcessName)) {
            usleep(2 * 1000);
            return userSize;
        } */
        ...
        property_get("sys.sourcing.huaweinavi", value1, "0");

        // 当前不是华为音乐焦点, 但是华为在播放，果断拒绝之。主要是屏蔽按键音，按键音不会请求焦点
        if ((strcmp(value, "com.huawei.dmsdpdevice") != 0)
             && !strcmp("com.huawei.dmsdpdevice", mProcessName)
             && mStreamType == AUDIO_STREAM_MUSIC 
             && strcmp("1", value1)) {
            usleep(2 * 1000);
            ALOGE("AudioTrack::write return by com.huawei.dmsdpdevice");
            return userSize;
        }

        if (strcmp("/system/bin/mediaserver", mProcessName)) {
            int duration = (userSize * 1000) / mSampleRate / mChannelCount / (mFormat == AUDIO_FORMAT_PCM_8_BIT ? 1:2);
            mMusicTotalDuration += duration;

            // mIsMusic |= (musicTotalDuration > 12000);
            bool sound = isSoundable((const char *) buffer, userSize);

            if (sound && ((mSendAtDuration == 0) || ((mMusicTotalDuration - mSendAtDuration) > 800))) {
                mSendAtDuration = mMusicTotalDuration;
                sendAudioTrackStatus(mIsMusic ? duration + 9900000 : duration, mSessionId, mProcessName, String16("write"));
            }
        }
    }
...
nsecs_t AudioTrack::processAudioBuffer()
{
    ...
    if (mLoopCount > 0) {
        ...
        bufferPosition = mStaticProxy->getBufferPosition();
    ...
    if (mTransfer != TRANSFER_CALLBACK && mTransfer != TRANSFER_SYNC_NOTIF_CALLBACK) {
        // @ Begin HCN2022
        // 使用共享内存写入方式的进程（例如：Yandex 导航）

        if (mEnableCustomization)
        {
            if (mTransfer == TRANSFER_SHARED) {
                // 判断这个是从哪里调用，只有不是 mediaserver 自己调用才发送状态到 SourceService
                if (ns > 0 && strcmp("/system/bin/mediaserver", mProcessName)) {
                    int duration = (int)(ns / 1000 / 1000);
                    mMusicTotalDuration += duration;

                    if ((mSendAtDuration == 0) || ((mMusicTotalDuration - mSendAtDuration) > 1000)) {
                        mSendAtDuration = mMusicTotalDuration;
                        sendAudioTrackStatus(mIsMusic ? duration + 7900000 : duration, mSessionId, mProcessName, String16("write"));
                    }
                }
            }
        }
    ...
    // 标识通过该接口传过来的数据是否有为有效的音频数据
    // 之所以加上这个标识是为了修复后台播放音乐或者视频的时候，使用chrome浏览网页会出现无声的情况。
    // 出现无声的原因：chrome内部会调用该接口传一些无效数据，该方法会将音频信息发到carservice，carservice接受到这个消息后
    // 会识别成混音，就会执行混音的压低声音的流程。
    bool soundable = true;
    ...
    while (mRemainingFrames > 0) {
        ...
        //检查音频数据的有效性
        soundable = isSoundable((const char *) audioBuffer.raw, audioBuffer.size());
...
// @ Begin HCN2019
    ...
    {
        // 判断这个是从哪里调用，只有不是 mediaserver 自己调用才发送状态到 SourceService
        if (soundable && strcmp("/system/bin/mediaserver", mProcessName)) {
            int duration = (totalWrittenSize * 1000) / mSampleRate / mChannelCount / (mFormat == AUDIO_FORMAT_PCM_8_BIT ? 1:2);
            mMusicTotalDuration += duration;

            // mIsMusic |= (mMusicTotalDuration > 12000);

            if ((mSendAtDuration == 0) || ((mMusicTotalDuration - mSendAtDuration) > 800)) {
                mSendAtDuration = mMusicTotalDuration;
                sendAudioTrackStatus(mIsMusic ? duration + 7900000 : duration, mSessionId, mProcessName, String16("write"));
            }
        }
