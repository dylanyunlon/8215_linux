...
// @Tag: HCN2024_T
void AudioFlingerClientAdapter::setSessionVolume(audio_session_t session, int percentage) {
    [&]() -> status_t {
        int32_t sessionAidl = VALUE_OR_RETURN_STATUS(
                legacy2aidl_audio_session_t_int32_t(session));
        int32_t percentageAidl = VALUE_OR_RETURN_STATUS(convertReinterpret<int32_t>(percentage));
        return statusTFromBinderStatus(
                mDelegate->setSessionVolume(sessionAidl, percentageAidl));
    }();
    // Failure is ignored.
}

void AudioFlingerClientAdapter::setSessionFadeout(audio_session_t session, int duration) {
    [&]() -> status_t {
        int32_t sessionAidl = VALUE_OR_RETURN_STATUS(
                legacy2aidl_audio_session_t_int32_t(session));
        int32_t durationAidl = VALUE_OR_RETURN_STATUS(convertReinterpret<int32_t>(duration));
        return statusTFromBinderStatus(
                mDelegate->setSessionFadeout(sessionAidl, durationAidl));
    }();
    // Failure is ignored.
}

void AudioFlingerClientAdapter::setStreamsMute(int32_t mask) {
    [&]() -> status_t {
        int32_t maskAidl = VALUE_OR_RETURN_STATUS(convertReinterpret<int32_t>(mask));
        return statusTFromBinderStatus(
                mDelegate->setStreamsMute(maskAidl));
    }();
    // Failure is ignored.
}
...
// @Tag: HCN2024_T
Status AudioFlingerServerAdapter::setSessionVolume(int32_t session, int32_t percentage) {
    audio_session_t sessionLegacy = VALUE_OR_RETURN_BINDER(
            aidl2legacy_int32_t_audio_session_t(session));
    int percentageLegacy = VALUE_OR_RETURN_BINDER(convertReinterpret<int>(percentage));

    mDelegate->setSessionVolume(sessionLegacy, percentageLegacy);
    return Status::ok();
}

Status AudioFlingerServerAdapter::setSessionFadeout(int32_t session, int32_t duration) {
    audio_session_t sessionLegacy = VALUE_OR_RETURN_BINDER(
            aidl2legacy_int32_t_audio_session_t(session));
    int durationLegacy = VALUE_OR_RETURN_BINDER(convertReinterpret<int>(duration));

    mDelegate->setSessionFadeout(sessionLegacy, durationLegacy);
    return Status::ok();
}

Status AudioFlingerServerAdapter::setStreamsMute(int32_t mask) {
    int maskLegacy = VALUE_OR_RETURN_BINDER(convertReinterpret<int>(mask));

    mDelegate->setStreamsMute(maskLegacy);
    return Status::ok();
}
