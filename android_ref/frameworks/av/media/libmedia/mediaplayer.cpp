...
static char process_name[40] = {0};
static char stringPath[PATH_MAX] = {0};
static bool donotReport = false;

static void getAppName(pid_t pid)
{
    char filename[20] = {0};
    FILE *f = NULL;
...
static int readSymLink(const char * const path, char *link, size_t max) {
    struct stat s;
    int length;
...
static void sendPlayerStatus(String16 state, int duration, int position, int session)
{
    if (donotReport)
        return;

    if (!strcmp("system_server", process_name)) {
        ALOGE("sendPlayerStatus[intercept 'system_server' play event], status %s", String8(state).string());
        return;
    }
    ...
    if (binder != NULL)
    {
        ...
        data.writeString16(String16(process_name));
        data.writeInt32(session);
        data.writeString16(String16(stringPath));
        data.writeString16(state);
        data.writeInt32(duration);
        data.writeInt32(position);
        status_t ret = binder->transact(IBinder::FIRST_CALL_TRANSACTION + 200, data, NULL, IBinder::FLAG_ONEWAY);
        (void) ret;
    } else {
        ALOGW("MediaPlayer sendPlayerStatus binder not found");
    }
}
...
// @ Begin HCN2019
    mDuration = -1;
    mEnableCustomization = property_get_bool("sys.debug.media_player_en", true);

    if (mEnableCustomization)
    {
        getAppName(getpid());
        ALOGI("HCN: streamType = %d | process_name = %s", mStreamType, process_name);
        bool naviStreamEnable = property_get_bool("sys.debug.navi_stream_en", false);
        if (naviStreamEnable) {
            char value[PROPERTY_VALUE_MAX];
            property_get("sys.navi.packages", value, "");
            if (strstr(sNaviPackages, process_name) || strstr(value, process_name) || strstr(process_name, "cld.navi")) {
...
// @ Begin HCN2019
            ...
            if (mEnableCustomization)
            {
                ALOGI("[%p] setDataSource, url:%s, audioSessionId:%d", this, url, mAudioSessionId);
                strcpy(stringPath, "url:");
                strcat(stringPath, url);
            }
...
// @ Begin HCN2019
        ...
        if (mEnableCustomization)
        {
            if (fd > 0) {
                ...
                stringPath[0] = '\0';
                readSymLink(path, stringPath, sizeof(stringPath));
                long flength = fileLength(fd);
                ALOGI("setDataSource: fd %d, path %s, length %ld", fd, stringPath, flength);
                if (!strcmp(stringPath, "/system/media/audio/ringtones/zero.mp3")) {
                    donotReport = true;
                }
            }
        }
...
// @ Begin HCN2019
            ...
            if (mEnableCustomization)
            {
                ...
                if (!strncmp(stringPath, "url:", 4)) {
                    mDuration = 0x7fffffff;
                }

                sendPlayerStatus(String16("start"), mDuration, msec, mAudioSessionId);
            }
...
status_t MediaPlayer::stop()
{
    ...
    if ( (mPlayer != 0) && ( mCurrentState & ( MEDIA_PLAYER_STARTED | MEDIA_PLAYER_PREPARED |
        ...
        if (ret != NO_ERROR) {
            ...
            if (mEnableCustomization)
            {
                sendPlayerStatus(String16("stopped"), mDuration, -1, mAudioSessionId);
            }
        }
        return ret;
    }
...
status_t MediaPlayer::pause()
{
    ...
    if ((mPlayer != 0) && (mCurrentState & MEDIA_PLAYER_STARTED)) {
        ...
        if (ret != NO_ERROR) {
            ...
            if (mEnableCustomization)
            {
                sendPlayerStatus(String16("paused"), mDuration, -1, mAudioSessionId);
            }
        }
        return ret;
    }
...
// @ Begin HCN2019
    ...
    if (mEnableCustomization)
    {
        if (status == NO_ERROR) {
            ...
            const int second = *msec / 1000;

            if (sPosition != second) {
                sPosition = second;
                if (mDuration != -1) {
                    String16 state("stopped");
                    switch (mCurrentState) {
                    case MEDIA_PLAYER_STARTED:
                        state = String16("start");
                        break;
                    case MEDIA_PLAYER_PAUSED:
                        state = String16("paused");
                        break;
                    ...
                    sendPlayerStatus(state, mDuration, *msec, mAudioSessionId);
                }
            }
        }
    }
