                                  ...
                                  , AudioRouting
{
    ...
    private String getProcessName(int pid) {
        ...
        try (FileInputStream fis = new FileInputStream(cmdlineFile)) {
            ...
            if (bytesRead > 0) {
                cmdline = new String(buffer, 0, bytesRead).replace('\0', ' ').trim();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    // 禁止修改 streamtype 、attributes 的进程名称
    private static String naviProNames = "com.google.android.apps.maps,"
                                         + SystemProperties.get("sys.forbid.navi.packages");
    ...
    public void setAudioStreamType(int streamtype) {
        String pName = getProcessName(android.os.Process.myPid());
        if (naviProNames.contains(pName)) {
            return;
        }
