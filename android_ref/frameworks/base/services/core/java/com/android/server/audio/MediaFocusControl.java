...
// @Tag: HCN2024_T
import com.android.car.audio.MediaFocusControlExt;
...
public class MediaFocusControl implements PlayerFocusEnforcer {
     ...
     * non-transient way.
     * 这里要设置false，安卓12+新增的焦点淡出功能，会焦点通知延迟2s发送，导致走淡出应用的app晚2s收到loss，导致释放释放晚了，切源逻辑出问题。
     * bug #71670 #71914 #71934 #73593 #75751
     */
    ...
    /** HCN: Extension Object */
    private MediaFocusControlExt mExt;
    protected MediaFocusControl(Context cntxt, PlayerFocusEnforcer pfe) {
        ...
        // HCN: Business Expansion
        mExt = new MediaFocusControlExt(cntxt);
    }
     ...
     * <pre>
     *    放弃当前音频焦点的所有者权利；
     *    e.g. Intent.ACTION_USER_SWITCHED（用户切换动作）
     * </pre>
     */
    ...
    private boolean isLockedFocusOwner(FocusRequester fr) {
        return (fr.hasSameClient(AudioSystem.IN_VOICE_COMM_FOCUS_ID) || fr.isLockedFocusOwner() ||
            mExt.isLockedFocusOwner(fr.getPackageName(), fr.getGainRequest(),
                    fr.getAudioAttributes().getContentType()));
    }
    ...
    void notifyExtPolicyFocusGrant_syncAf(AudioFocusInfo afi, int requestResult) {
        // @Tag: HCN2024_T
        mExt.notifySourceInfoAudioFocusState(afi, requestResult, 1);
    ...
    void notifyExtPolicyFocusLoss_syncAf(AudioFocusInfo afi, boolean wasDispatched) {
        // @Tag: HCN2024_T
        mExt.notifySourceInfoAudioFocusState(afi, AudioManager.AUDIOFOCUS_REQUEST_GRANTED, 0);
        for (IAudioPolicyCallback pcb : mFocusFollowers) {
            try {
                // oneway
    ...
    protected int requestAudioFocus(@NonNull AudioAttributes aa, int focusChangeHint, IBinder cb,
        ...
        synchronized(mAudioFocusLock) {
            ...
            // @Tag: HCN2024_T
            focusChangeHint = mExt.requestAudioFocus_Hook(callingPackageName, focusChangeHint);
            ...
            if (!canReassignAudioFocus()
                    || !mExt.canReassignAudioFocus(callingPackageName)) {
            ...
            // @Tag: HCN2024_T
            // 记录媒体播放器请求 audio focus 的包名；
            mExt.recordRequestAudioFocus(callingPackageName, focusChangeHint);

            // 当前请求焦点的客户端就在栈顶
            if (!mFocusStack.empty() && mFocusStack.peek().hasSameClient(clientId)) {
                // 如果请求焦点状态标记也没变化（重复请求什么也不用干）
