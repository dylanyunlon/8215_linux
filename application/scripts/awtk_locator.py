...
def init(ARGUMENTS = None):
    ...
    if LINUX_FB:
        awtk_from_env = os.environ.get("awtk_linux_fb_dir")
        if not awtk_from_env:
            # 独立构建：AWTK_ROOT 即 awtk-linux-fb 目录，未由父进程注入 awtk_linux_fb_dir 时兜底
            awtk_from_env = AWTK_ROOT
        AWTK_SCRIPTS_ROOT = os.path.join(awtk_from_env, '../awtk/scripts')
    else:
