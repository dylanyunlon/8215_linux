...
unsigned int irt_dma_base = 0xf0046000;
unsigned int irtdmairq = 75;
struct clk* irtdmaclk = 0xce493300;
...
irqreturn_t IRT_DMA_HwISR(int u2Vector, void *dev_id)
{
    ...
    //IRD_LOG(IRD_LOG_LVL_IRQ, "IRT_DMA_HwISR, u2Vector:%d, VECTOR_IRT_DMA:%d \r\n", u2Vector, VECTOR_IRT_DMA);
...
INT32 IRT_DMA_HwInit(void)
{
	...
	BOOL clk_on;
    IRT_APIEntry();
    IRD_LOG(IRD_LOG_LVL_ERR, "IRT_DMA_HwInit\r\n");
