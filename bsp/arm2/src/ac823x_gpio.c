...
int ac823x_gpio_inout_sel_reg(unsigned gpio, int dir)
{
	unsigned val, idx, offset;
	int ret = -1;
	if ((gpio == PIN_197_GPIO7) || (gpio == PIN_219_GPIO8) || (gpio == PIN_220_GPIO9)) {
		offset = (gpio == PIN_197_GPIO7) ? (3):(gpio - 215);
		CKGEN_WRITE32(0x240F4, (CKGEN_READ32(0x240F4) | (1<<offset))); //enable gpio
		val = CKGEN_READ32(0x240D4);
		val = (dir == OUTPUT) ? (val | (1U << offset)) :
			(val & ~(1U << offset));
		CKGEN_WRITE32(0x240D4, val); //set input/output
...
int ac823x_gpio_get_value_reg(unsigned gpio)
{
	unsigned val, idx, offset;

	if (valid_gpio(gpio)) {
		if ((gpio == PIN_197_GPIO7) || (gpio == PIN_219_GPIO8) || (gpio == PIN_220_GPIO9)) {
			val = CKGEN_READ32(0x240D0);
			offset = (gpio == PIN_197_GPIO7) ? (3):(gpio - 215);
...
int ac823x_gpio_set_value_reg(unsigned gpio, int value)
{
	unsigned val, idx, offset;
	...
	if (valid_gpio(gpio)) {
		if ((gpio == PIN_197_GPIO7) || (gpio == PIN_219_GPIO8) || (gpio == PIN_220_GPIO9)) {
			offset = (gpio == PIN_197_GPIO7) ? (3):(gpio - 215);
			val = CKGEN_READ32(0x240D8);
			val = (value == 1) ? (val | (1U << offset)) :
					(val & ~(1U << offset));
			CKGEN_WRITE32(0x240D8, val);
		}else if (gpio <= PIN_220_GPIO9) {
			offset = gpio_get_addroffset(gpio);
			idx = gpio_get_addridx(gpio);
...
int ac823x_gpio_direction_input_reg(unsigned gpio)
{
	int ret = -1;

	if (valid_gpio(gpio)) {
		ret = ac823x_gpio_inout_sel_reg(gpio, INPUT);
		if (ret == 0) {
			ret = ac823x_gpio_get_value_reg(gpio);
		}
...
int ac823x_gpio_direction_output_reg(unsigned gpio, int value)
{
	int ret = 0;

	if (valid_gpio(gpio)) {
		ret = ac823x_gpio_set_value_reg(gpio, value);
		ret |= ac823x_gpio_inout_sel_reg(gpio, OUTPUT);
