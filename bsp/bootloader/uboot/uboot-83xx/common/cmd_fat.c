...
int do_fat_fsload (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	extern g_u8RealDataSize;
	g_u8RealDataSize = size;
#endif
...
int do_fat_fsload_at (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	long size;
	unsigned long offset;
	unsigned long pos;
	...
	if (argc < 7) {
		printf ("usage: fatloadat <interface> <dev[:part]> <addr> <filename> [pos] [bytes]\n");
		return 1;
	}
	dev = (int)simple_strtoul (argv[2], &ep, 16);
	dev_desc=get_dev(argv[1],dev);
	...
	offset = simple_strtoul (argv[3], NULL, 16);
	pos = simple_strtoul (argv[5], NULL, 16);
	count = simple_strtoul (argv[6], NULL, 16);

	ret = file_fat_read_at(argv[4], pos, (unsigned char *) offset, count, &size);;
	if (ret < 0) {
...
U_BOOT_CMD(
	fatloadat,	7,	0,	do_fat_fsload_at,
	"load binary file from a dos filesystem",
	"<interface> <dev[:part]>  <addr> <filename> [pos] [bytes]\n"
	"    - load binary file 'filename' from 'dev' on 'interface'\n"
	"      to address 'addr' from dos filesystem"
);
