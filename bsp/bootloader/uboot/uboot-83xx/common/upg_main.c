...
#include <linux/mtd/ac83xx_part_oper.h>
...
INT32 i4SetUBIMountEnv(PARTITION_INFO_ENTRY partInfoTable[]) {
    ...
    char _tmp_buf[1024] = "";
    /* mtdparts */
    strcat(_bootargs_buf, "mtdparts=ac83xx_nand:");
    for (i = 0; (i < MAX_PARTITION_INFO_ENTRY_NUM) && 
                (partInfoTable[i].u4PartID != INVALID_PART_ID); i++) {
    ...
    i4Ret =  setenv("mtdids", "nand0=ac83xx_nand");
    if (-1 == i4Ret) {
...
INT32 findPITFromFinalBinaryImage(UINT8 *u4pImgStartAddr, UINT32 u4ImgSize) {
	...
	for (i = 0; i < u4ImgSize; i += PIT_SEARCH_STEP) {
		if ( (*(UINT32 *) (u4pImgStartAddr + i) == PITINDEXHEADSIG) &&
			...
			(*(UINT32 *) (u4pImgStartAddr + i + 8) == PITINDEXHEADSIG) &&
    ...
    //FOR PITIndex 
    u4PITHeadSigAddr += 16;
    for (i = 0; ((u4PITHeadSigAddr + i * 16) < u4PITTailSigAddr); i++) {
         UINT32 u4Nansize = *(UINT32 *) (u4PITHeadSigAddr + (i * 16) );
         UINT32 u4OffsetFromImag = *(UINT32 *) (u4PITHeadSigAddr + (i * 16)+ 4);
         UINT32 u4TableSize = *(UINT32 *) (u4PITHeadSigAddr + (i * 16) + 8);
         UINT32 u4ParTableTagId = *(UINT32 *) (u4PITHeadSigAddr + (i * 16) + 12);
 ...
 * Name: upgBEImge
 *
 * Description: Upgrade backend image such as ac83xx_linux_img_dbg.bin
 *
...
//
         ...
         (partInfoTable[i].u4PartID != INVALID_PART_ID); i++) 
    {
       if( ISUBIVOLUME(partInfoTable[i].u4PartInfo) != 4 )
       {
         if(ac83xx_init_mtd_table(partInfoTable[i].szName) != 0)	
         {
...
INT32 checkFEImgAndParts(u4ImgSize, u4FEPartSize, u4FEBakPartSize) {
	if (u4ImgSize > u4FEPartSize || u4ImgSize > u4FEBakPartSize) {
		UPG_LOG(UPG_LOG_ERROR, "[UPG_ERROR] FE image size[0x%x] > FE Part size[0x%x] or FE Bak Part size[0x%x]\n",
...
//	UINT32 *u4pImgStartAddr = 0;
   ...
   memcpy((void *) (partInfoTable),( void * )(partInfoTableFlash),sizeof(PARTITION_INFO_ENTRY)*(MAX_PARTITION_INFO_ENTRY_NUM-1));
   {
         ...
         (partInfoTableFlash[i].u4PartID != INVALID_PART_ID); i++) 
    {
       ...
       {
         if(ac83xx_init_mtd_table(partInfoTableFlash[i].szName) != 0)	
         {
