...
static disk_partition_t cur_part_info = {0};
 ...
 */
__u8 get_contents_vfatname_block[MAX_CLUSTSIZE]  __attribute__ ((aligned(ARCH_DMA_MINALIGN)));
 ...
 */
__u8 get_dentfromdir_block[MAX_CLUSTSIZE] __attribute__ ((aligned(ARCH_DMA_MINALIGN)));
//	__aligned(ARCH_DMA_MINALIGN);
...
static __u32 get_partition_addr()
{
    __u32 ret = -1;
    int i = 0;
    __u32 bytes_per_sector;
    __u8 *bs = malloc(512);

    bs = memalign(ARCH_DMA_MINALIGN, 512);
	if (bs == NULL) {
		debug("Error: allocating block\n");
		return ret;
	}

	if (disk_read(0, 1, bs) < 0) {
		debug("Error: reading block\n");
		return ret;
	}

    bytes_per_sector = fat_read16(bs,0xb);
    if ((bytes_per_sector != 0x200) && (bytes_per_sector != 0x400) && (bytes_per_sector != 0x800)) {
        debug("No MBR FAT Filesystem. Try to get DPT Info\n");
    	
        for(i = 0; i < 4; i++) {
            if(bs[0x1be + 0x10 * i] == 0x80 )
                break;
        }
    
        if (i == 4) {
            debug("no active partition\n");
            return ret;
        } else {            
            ret = fat_read32(bs,0x1c6 + 0x10 * i);           
        }
    }
    else {
        ret = 0;
...
read_bootsectandvi(boot_sector *bs, volume_info *volinfo, int *fatsize)
{
	...
	volume_info *vistart;
	int ret = 0;
    __u32 partition_addr = -1;
    __u32 read_start_block = 0;
    int i = 0;    
    ...
    if(cur_part_info.start == 0) {
        partition_addr = get_partition_addr();
        if (partition_addr > 0) {
            read_start_block = partition_addr;
        }
    }
    
	if (disk_read(read_start_block, 1, block) < 0) {
		debug("Error: reading block\n");
		goto fail;
	}
...
//__u8 do_fat_read_at_block[MAX_CLUSTSIZE] __attribute__ ((aligned(ARCH_DMA_MINALIGN)));
//	__aligned(ARCH_DMA_MINALIGN);
...
int do_fat_read_update_kerenl (const char *filename, void *buffer, unsigned long maxsize, int dols, const char * name)
{
	return -1;
}
