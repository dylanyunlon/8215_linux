...
unsigned long long simple_strtou64(const char *cp,char **endp,unsigned int base,unsigned long long *ret)
{
	unsigned long long result = 0,value;
    //printf("---> simple_strtoull one cp:%s <---\n",cp);
	if (*cp == '0') {
		cp++;
		if ((*cp == 'x') && isxdigit(cp[1])) {
			base = 16;
	...
	if (endp)
		*endp = (char *)cp;
	*ret = result;
	//printf("---> simple_strtoull one result: 0x%08X, 0x%08X <---\n", (u32)(result >> 32), (u32)(result & 0xFFFFFFFF));
	//printf("---> simple_strtoull one *ret: 0x%08X, 0x%08X <---\n", (u32)(*ret >> 32), (u32)(*ret & 0xFFFFFFFF));
	return result;
}
