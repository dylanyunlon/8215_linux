...
LINUX_MAKE_FLAGS = \
	...
	HAVE_AEE_FEATURE=$(HAVE_AEE_FEATURE) \
	LOAD_VISS_TO_DRAM=$(LOAD_VISS_TO_DRAM) \
	KEDUMP_MINI=$(KEDUMP_MINI) \
	RAM_CONSOLE=$(RAM_CONSOLE) \
	ATC_METAZONE_SUPPORT=$(ATC_METAZONE_SUPPORT) \
	ATC_LOGO_SUPPORT=$(ATC_LOGO_SUPPORT) \
	ATC_FS_LOW_STORAGE_PROTECTION_SUPPORT=$(ATC_FS_LOW_STORAGE_PROTECTION_SUPPORT) \
	ATC_FS_AUTO_RECOVERY_SUPPORT=$(ATC_FS_AUTO_RECOVERY_SUPPORT) \
	ATC_BACKCAR_SUPPORT=$(ATC_BACKCAR_SUPPORT) \
	ATC_CAMERA_SUPPORT=$(ATC_CAMERA_SUPPORT) \
	ATC_MACRO_VTS=$(ATC_MACRO_VTS) \
	ATC_MIPI1_SENSOR_N4_SUPPORT=$(ATC_MIPI1_SENSOR_N4_SUPPORT) \
	ATC_MIPI2_SENSOR_TP2854_SUPPORT=$(ATC_MIPI2_SENSOR_TP2854_SUPPORT) \
	ATC_BT656_SENSOR_TP2825_SUPPORT=$(ATC_BT656_SENSOR_TP2825_SUPPORT) \
	ATC_MIPI2_SENSOR_TW9992_SUPPORT=$(ATC_MIPI2_SENSOR_TW9992_SUPPORT) \
	ATC_SECURE_BOOT_AVB_SUPPORT=$(ATC_SECURE_BOOT_AVB_SUPPORT) \
	ATC_AVB_SWITCH_SLOT=$(ATC_AVB_SWITCH_SLOT) \
...
define LINUX_KCONFIG_FIXUP_CMDS
	...
	$(if $(BR2_PACKAGE_IPTABLES),
		...
		$(call KCONFIG_DISABLE_OPT,CONFIG_BRIDGE_NETFILTER,$(@D)/.config)
	...
	$(call KCONFIG_SET_OPT,CONFIG_ATC_WIFI_CHIP,"$(ATC_WIFI_CHIP)",$(@D)/.config)
	$(call KCONFIG_SET_OPT,CONFIG_ATC_WLAN_TRANSMISSION_MODE,"$(ATC_WLAN_TRANSMISSION_MODE)",$(@D)/.config)
endef
...
# Generate reserved memory dtsi
PARSER_DIR = $(TOPDIR)/../vendor/autochips/proprietary/tools/ParseReservedMemory
RESV_MEMO_FILE = $(LINUX_ARCH_PATH)/boot/dts/autochips/ac8x_reserved_memory_linux.xls
RESV_MEMO_DTSI = $(LINUX_ARCH_PATH)/boot/dts/autochips/ac8x_reserved_memory_linux.dtsi

DTB_PARSER_TOOL_DIR = $(TOPDIR)/../vendor/autochips/proprietary/tools/ParseDtbTool
TOOL_OUT_DIR = $(DTB_PARSER_TOOL_DIR)/TOOL_OUT
PARSER_DTB_TOOL = $(TOOL_OUT_DIR)/dtbtool
MERGED_DTB = $(TOOL_OUT_DIR)/mergd_dtb
DTB_TOOL_GCC_COMPILE = $(CC)

define LINUX_PASER_RESV_INFO
	python $(PARSER_DIR)/generate_rsv.py $(RESV_MEMO_FILE) $(RESV_MEMO_DTSI) $(@D)/.config 8 ac8x
endef

define LINUX_APPEND_RESV_INFO
	$(MAKE) -C $(DTB_PARSER_TOOL_DIR) GCC_COMPILE=$(DTB_TOOL_GCC_COMPILE) ; \
	(for dtb in $(LINUX_DTS_NAME); do \
		$(PARSER_DTB_TOOL) $(LINUX_ARCH_PATH)/boot/dts/$${dtb}.dtb $(TOOL_OUT_DIR)/tmp.dtb; \
		mv $(TOOL_OUT_DIR)/tmp.dtb $(LINUX_ARCH_PATH)/boot/dts/$${dtb}.dtb; \
	done) ;\
	$(MAKE) -C $(DTB_PARSER_TOOL_DIR) GCC_COMPILE=$(DTB_TOOL_GCC_COMPILE) clean
endef

TOOLPATH = $(TOPDIR)/../build/tools/
export KERNEL_BUILD_DIR = $(BUILD_DIR)/linux-local/
export KERNEL_EXT_LIBS = $(TOPDIR)/../source/kernel/klibs/
DEFCONFIGFILE = $(KERNEL_BUILD_DIR)/arch/arm/configs/ac83xx_defconfig
define CUSTOM_PREBUILD_BUILD_CMDS
	sed -i "s/ATC_DDR_SIZE/$(ATC_DDR_SIZE_HEX)/g" $(KERNEL_BUILD_DIR)/arch/arm/boot/dts/ac83xx.dts
	sync $(KERNEL_BUILD_DIR)/arch/arm/boot/dts/ac83xx.dts
	python3 ${TOOLPATH}/gen_rsv.py ${TOPDIR}/../out/config/rsv.dtsi ${TOPDIR}/../out/config "0x${ATC_DDR_SIZE_HEX}"
	cp ${TOPDIR}/../out/config/rsv.dtsi ${KERNEL_BUILD_DIR}/arch/arm/boot/dts/
	$(KERNEL_BUILD_DIR)/scripts/gen_atc_project.sh $(DEFCONFIGFILE) $(KERNEL_BUILD_DIR) $(PROJECT_NAME) $(PLATFORM_NAME) "linux" "dizzy" "atc" "ac8317" "evb"
	cp $(KERNEL_EXT_LIBS)/  $(KERNEL_BUILD_DIR)/../ -arf
	cd "$(KERNEL_BUILD_DIR)/../klibs/libs" &&  $(MAKE) PROJECT=linux DEVICE=ac83xx OS=linux
endef
...
define LINUX_BUILD_CMDS
	...
	$(CUSTOM_PREBUILD_BUILD_CMDS)
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) V=1 -C $(@D) $(LINUX_TARGET_NAME) -j1 
	@if grep -q "CONFIG_MODULES=y" $(@D)/.config; then \
		$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) modules ; \
	fi
	$(LINUX_BUILD_DTB)
endef
#$(LINUX_PASER_RESV_INFO)
#$(LINUX_APPEND_RESV_INFO)
#$(LINUX_APPEND_DTB)
