...
define IPTABLES_REMOVE_UNUSED_LIBS
( \
    cd $(TARGET_DIR)/usr/lib/xtables/; \
    if ls lib* >/dev/null 2>&1; then \
        mkdir tmp/; \
        mv lib* tmp/; \
        cd tmp/; \
        mv libxt_standard.so ../; \
        mv libipt_MASQUERADE.so ../; \
        mv libip6t_MASQUERADE.so ../; \
        mv libxt_conntrack.so ../; \
        mv libxt_state.so ../; \
        cd ../; \
        rm -rf tmp/; \
    fi; \
...
IPTABLES_POST_INSTALL_TARGET_HOOKS += IPTABLES_REMOVE_UNUSED_LIBS
