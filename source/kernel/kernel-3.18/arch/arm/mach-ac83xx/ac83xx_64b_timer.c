...
// Private functions
//==================================================
bool _fg_is_timer_enabled(unsigned int u4TimerId)
{
    /* error check */
...
int _i_bim_reset_64b_timer(unsigned int u4TimerId)
{
    T64b_REG* t64bReg;
...
int _i_bim_get_64b_timer(unsigned int u4TimerId, T64b_RAW_TIME_T* pRawTime)
{
    T64b_REG* t64bReg;
...
int i_hal_t64b_raw_to_time(const T64b_RAW_TIME_T* pRawTime, HAL_TIME_T* pTime)
{
  unsigned long long u8Cycles;
    ...
    pTime->u4Seconds = (unsigned int)(u8Cycles / (unsigned long long)_u4SysClock);
    pTime->u4Micros = (unsigned int)(((u8Cycles % (unsigned long long)_u4SysClock) * 1000000L) / _u4SysClock);
...
{
    unsigned long long u8New;
    unsigned long long u8Old;
    unsigned long long u8Result;
    ...
    u8New = ((unsigned long long)pT1->u4H << 32) | (pT1->u4L);
    u8Old = ((unsigned long long)pT0->u4H << 32) | (pT0->u4L);
...
{
    unsigned long long u8T0;
    unsigned long long u8T1;

	u8T0 = ((unsigned long long)pT0->u4H << 32) | (pT0->u4L);
	u8T1 = ((unsigned long long)pT1->u4H << 32) | (pT1->u4L);
