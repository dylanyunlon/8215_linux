...
typedef struct
{
  unsigned int    u4L;                    //Number of timer interrupts from startup
  unsigned int    u4H;                   //System cycles from last timer interrupt
} T64b_RAW_TIME_T;

typedef struct
{
  unsigned int    u4Seconds;                  //Number of seconds from startup
  unsigned int    u4Micros;                   //Remainder in microsecond
} HAL_TIME_T;
