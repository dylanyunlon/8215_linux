...
#endif
#define AUDIO_IN_MEM_PA           (AUDIO_FIFO_MEM_PA + AUDIO_FIFO_MEM_SIZE)

#define AUDIO_PCM_MEM_SIZE         (1024*512UL)     // 256K audio  Buffer
#define AUDIO_PCM_MEM_PA           (AUDIO_IN_MEM_PA + AUDIO_IN_MEM_SIZE)

#define MULTIMEDIA_MEM_SIZE         (1024*1024*80UL)//(1024*1024*46UL)  //management by OS
#define MULTIMEDIA_MEM_PA           (AUDIO_PCM_MEM_PA + AUDIO_PCM_MEM_SIZE)

#define EXTERNAL_DISPLAY_FB_MEM_SIZE   ( 0x000000 )
...
#define FRAMEBUFFER_MEM_SIZE        (1024*1024*12UL)
...
#define IMAGE_RESIZE_MEM_PA         (SD_DMA_MEM_PA + SD_DMA_MEM_SIZE)

#define YC_BUFFER_MEM_SIZE          (1024*1024*0UL) // 3M for Y C
#define YC_BUFFER_PA                (IMAGE_RESIZE_MEM_PA + IMAGE_RESIZE_MEM_SIZE)

#if 1
#define TESTBACKWARD_MEM_SIZE         (0UL)  //test
#define TESTBACKWARD_MEM_PA           (YC_BUFFER_PA + YC_BUFFER_MEM_SIZE)

#define TESTFORWARD_MEM_SIZE         (0UL)  //management by OS
#define TESTFORWARD_MEM_PA           (TESTBACKWARD_MEM_PA + TESTBACKWARD_MEM_SIZE)
#endif

#define WCH_MEM_TVD_SIZE          (0x00300000) // 5*720*576*3/2
#define WCH_MEM_TVD_PA            (TESTFORWARD_MEM_PA + TESTFORWARD_MEM_SIZE)
...
#define TRUSTZONE_RESERVED_MEM_SIZE    (1024*1024*2UL)
#define TRUSTZONE_RESERVED_MEM_PA      (ARM2_RESERVED_MEM_PA + ARM2_RESERVED_MEM_SIZE)

#define TZ_SHARED_DATA_RESERVED_MEM_SIZE    (1024*1024*1UL)
#define TZ_SHARED_DATA_RESERVED_MEM_PA      (TRUSTZONE_RESERVED_MEM_PA + TRUSTZONE_RESERVED_MEM_SIZE)

#define RESV_MEM_IO_VIRT_START      0xF0000000

#define RESV_MEM_IO_SIZE          (   DVP_BUFFER_MEM_SIZE   \
                                      + AUDIO_DSP_MEM_SIZE    \
                                      + AUDIO_FIFO_MEM_SIZE   \
                                      + AUDIO_IN_MEM_SIZE    \
                                      + AUDIO_PCM_MEM_SIZE   \
                                      ...
                                      + TRUSTZONE_RESERVED_MEM_SIZE\
                                      +TZ_SHARED_DATA_RESERVED_MEM_SIZE)
...
#define REVERSE_MEM_SIZE            (AUDIO_FIFO_MEM_SIZE   \
                                      + AUDIO_IN_MEM_SIZE    \
                                      + AUDIO_PCM_MEM_SIZE   \
                                      + MULTIMEDIA_MEM_SIZE   )  // Multimedia used reserved memory size(AFIFO+MultimediaSize)
