...
#define MVG_CFG_VCC_DROP_TIME     50000   /* 50ms */
...
#define MVG_DELAYED_PWR_RESET                      _IOW('V', 157, int)
#define MVG_RANDOM_DELAYED_PWR_RESET               _IOW('V', 158, int)
#define MVG_DELAYED_PWR_RESET_BACKGROUND           _IOW('V', 159, int)
#define MVG_RANDOM_DELAYED_PWR_RESET_BACKGROUND    _IOW('V', 160, int)
#define MVG_ENABLE_SW_PWR_LOSS_CONFIG              _IOW('V', 161, int)
#define MVG_ENABLE_HW_PWR_LOSS_CONFIG              _IOW('V', 162, int)
...
#define PL_TIME_RAND_WITHOUT_CHECK_ADDR(time, time_max) do { time=0; \
		if ((time_max)&&(mvg_trigger())) { \
			get_random_bytes(&time, sizeof(u32)); \
			if (mvg_get_method()==MVG_METHOD_HW_CUT) { \
				time = time % (time_max+MVG_CFG_VCC_DROP_TIME) + 1; \
...
#define PL_TIME_RAND_WITHOUT_CHECK_ADDR(time, time_max)
