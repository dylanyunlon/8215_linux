...
//Todo: CKGEN interface was deprecated.
#define G3D_CLK_MASK             (7U << 24)
#define G3D_CLK_OFFSET           24
#define G3D_CLK_27M              0x0
#define G3D_CLK_APLL2            0x1    //294
#define G3D_CLK_APLL1            0x2    //270
#define G3D_CLK_USB_CLK_D2       0x3    //240
#define G3D_CLK_SYSPLL_CLK_D3    0x4    //216
#define G3D_CLK_ARMPLL_CLK_D4    0x5    //200
#define G3D_CLK_DMPLL_CLK_D2     0x6    //160
#define G3D_CLK_SYSPLL_CLK_D6    0x7    //108

#undef CKGEN_READ32(REG)
#define CKGEN_READ32(REG)         __raw_readl(__io(CKGEN_BASE_VA + REG))

#undef CKGEN_WRITE32(REG, VAL)
#define CKGEN_WRITE32(REG, VAL)   __raw_writel(VAL, __io(CKGEN_BASE_VA + REG))

#define SPM_READ32(REG)            __raw_readl(__io(SPM_BASE_VA + REG))

#define SPM_WRITE32(VAL, REG)      \
	do {  \
		__raw_writel(VAL, __io(SPM_BASE_VA + REG));  \
		mb();  \
	} while(0);
...
{
	uint32_t val =0;
	pr_info("[SPM] spm_power_mali_all:%d %d\n", sw_hw_sel, on_off);
	...
	if (on_off) {
		...
		//Todo: Add mali clock select here is not suitable, we should do something
		//select clock
		val = CKGEN_READ32(0x0014);
		val = val & (~G3D_CLK_MASK);
		val = val | (G3D_CLK_APLL2 << G3D_CLK_OFFSET);
		CKGEN_WRITE32(0x0014, val);
		//
	} else {
		//power off
		if (sw_hw_sel) {
...
void spm_poweron_vdec(void)
{
	...
	while (SPM_READ32(0x94) != 0x0) {
		...
		if (cnt > VDEC_RDREG_TIMES) {
			pr_info("[SPM] ++spm_poweron_vdec failed after try morn than %d times\n", VDEC_RDREG_TIMES);
			break;
		}
	}
}
...
void spm_powerdown_vdec(void)
{
	...
	while (SPM_READ32(0x94) != 0xFFFFFFFF) {
		cnt++;
		if (cnt > VDEC_RDREG_TIMES) {
			pr_info("[SPM] ++spm_powerdown_vdec failed after try morn than %d times\n", VDEC_RDREG_TIMES);
			break;
		}
	}
}
