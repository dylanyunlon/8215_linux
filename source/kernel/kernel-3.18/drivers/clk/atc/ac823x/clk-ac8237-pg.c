...
static void __init mt_scpsys_init(struct device_node *node)
{
	...
	if (!infracfg_reg || !spm_reg) {
		pr_err("clk-pg-ac8237: missing reg\n");
		return;
	}
...
CLK_OF_DECLARE(mtk_pg_regs, "mediatek,ac8237-scpsys", mt_scpsys_init);
...
MODULE_DESCRIPTION("clk driver for AC823X");
MODULE_LICENSE("GPL");
