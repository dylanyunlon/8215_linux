...
#if !defined(ATC_CCF_BRINGUP)
#define ATC_CCF_BRINGUP	0
#endif
...
static int spm_mtcmos_ctrl_mdsys1(int state)
{
	...
	if (state == STA_POWER_DOWN) {
...
#else
		pr_debug("[CCF] MD power on by SPM\n");
		...
		while (spm_read(SPM_PCM_PASR_DPD_3)) {
			...
			if (count > 1000) {
				pr_debug("[CCF] MD power on: SPM no response\n");
				pr_debug("[CCF] PCM_IM_PTR : 0x%x (%u)\n", spm_read(SPM_PCM_IM_PTR),
...
static void __init mt_scpsys_init(struct device_node *node)
{
	...
	if (!infracfg_reg || !spm_reg) {
		pr_err("[CCF] clk-pg-ac8317: missing reg\n");
		return;
	}
...
CLK_OF_DECLARE(mtk_pg_regs, "mediatek,ac8317-scpsys", mt_scpsys_init);
...
static int test_pg_dump_regs(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < NR_SYSS; i++) {
		...
		seq_printf(s, "[CCF] %10s: [0x%p]: 0x%08x\n", syss[i].name,
...
static void dump_pg_state(const char *clkname, struct seq_file *s)
{
	...
	if (IS_ERR_OR_NULL(c)) {
		seq_printf(s, "[CCF] [%17s: NULL]\n", clkname);
		return;
	}

	seq_printf(s, "[CCF] [%17s: %3s, %3d, %3d, %10ld, %17s]\n",
		   __clk_get_name(c),
...
static int test_pg_1(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_clks); i++) {
		...
		if (IS_ERR_OR_NULL(g_clks[i].clk)) {
			seq_printf(s, "[CCF] clk_get(%s): NULL\n",
				   g_clks[i].name);
			continue;
		}

		clk_prepare_enable(g_clks[i].clk);
		seq_printf(s, "[CCF] clk_prepare_enable(%s)\n",
...
static int test_pg_2(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_clks); i++) {
		if (IS_ERR_OR_NULL(g_clks[i].clk)) {
			seq_printf(s, "[CCF] (%s).clk: NULL\n",
				   g_clks[i].name);
			continue;
		}

		seq_printf(s, "[CCF] clk_disable_unprepare(%s)\n",
			   __clk_get_name(g_clks[i].clk));
...
MODULE_DESCRIPTION("clk driver for AC83XX");
MODULE_LICENSE("GPL");
