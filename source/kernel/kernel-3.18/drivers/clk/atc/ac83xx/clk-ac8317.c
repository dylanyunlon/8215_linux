...
#include "clk-ac8317-pll.h"

#include <dt-bindings/clock/ac8317-clk.h>
...
#define vpll_ttl_d8		"vpll_ttl_d8"
#define vpll_ttl_d10	"vpll_ttl_d10"
#define vpll_ttl_d15	"vpll_ttl_d15"
...
#define aud_dvd_sel		"aud_dvd_sel"
...
#define img_dramc_usb				"img_dramc_usb"
...
/* DVD */
#define dvd_mvdo					"dvd_mvdo"
#define dvd_dgo						"dvd_dgo"
#define dvd_osd						"dvd_osd"
#define dvd_gra						"dvd_gra"
#define dvd_bim						"dvd_bim"
#define dvd_turbo32					"dvd_turbo32"
#define dvd_vdec					"dvd_vdec"
#define dvd_parser					"dvd_parser"
#define dvd_rambuf					"dvd_rambuf"
#define dvd_pt110					"dvd_pt110"
#define dvd_rs232					"dvd_rs232"
#define dvd_cddvd					"dvd_cddvd"
#define dvd_audio					"dvd_audio"
#define dvd_servo_misc				"dvd_servo_misc"
#define dvd_rambuf_apctrl_tbus3		"dvd_rambuf_apctrl_tbus3"
#define dvd_rambuf_apctrl_tbus4		"dvd_rambuf_apctrl_tbus4"
#define dvd_rambuf_apctrl_tbus5		"dvd_rambuf_apctrl_tbus5"
#define dvd_mfg_top_pwr_wrap		"dvd_mfg_top_pwr_wrap"
...
static void __init init_clk_divider(void __iomem *top_base, struct clk_onecell_data *clk_data)
{
	...
	for (i = 0; i < ARRAY_SIZE(sub_divs); i++) {
		...
		if (IS_ERR(clk)) {
			pr_err("[CCF] Failed to register clk %s: %ld\n", divider->name, PTR_ERR(clk));
			continue;
		}
		...
		if (ret) {
			pr_err("[CCF] Failed to register clkdev %s\n", divider->name);
		}
...
static struct atc_fixed_factor top_divs[] __initdata = {
	...
	FACTOR(TOP_VPLL_TTL_D9, vpll_ttl_d8, vpll_ttl, 1, 9),
	FACTOR(TOP_VPLL_TTL_D12, vpll_ttl_d10, vpll_ttl, 1, 12),
	FACTOR(TOP_VPLL_TTL_D18, vpll_ttl_d15, vpll_ttl, 1, 18),
...
static const char *aud_dvd_parents[] __initconst = {
	clk27m_ck,
	aclk_k7,
	aclk_in,
	i2s_out2_mclk
...
static struct atc_mux top_muxes[] __initdata = {
	...
	MUX(TOP_MUX_CLK_AUD_DVD, aud_dvd_sel, aud_dvd_parents, 0x0008, 8, 2, INVALID_MUX_GATE_BIT),
...
static struct atc_gate img_dramc_clks[] __initdata = {
	...
	GATE(IMG_DRAMC_CLK_OSD_RESZ, img_dramc_osd_resz, resz_sel, img_dramc_cg_regs, 5, 1, 5, 1),
	...
	GATE(IMG_DRAMC_CLK_DEMUX_TS0, img_dramc_demux_ts0, ts0_sel, img_dramc_cg_regs, 8, 0, 7, 1),
	GATE(IMG_DRAMC_CLK_DEMUX_TS1, img_dramc_demux_ts1, ts1_sel, img_dramc_cg_regs, 9, 0, 7, 1),
	GATE(IMG_DRAMC_CLK_DEMUX_27M, img_dramc_demux_27m, demux_sel, img_dramc_cg_regs, 11, 0, 7, 1),
	GATE(IMG_DRAMC_CLK_NFI, img_dramc_nfi, nflash_sel, img_dramc_cg_regs, 12, 1, 12, 1),
	GATE(IMG_DRAMC_CLK_USB, img_dramc_usb, usb_27m_sel, img_dramc_cg_regs, 13, 1, 13, 1),
	GATE(IMG_DRAMC_CLK_IRT_DMA_WRAPPER, img_dramc_irt_dma_wrapper, demux_sel, img_dramc_cg_regs, 14, 1, 14, 1),
	GATE(IMG_DRAMC_CLK_ARM9, img_dramc_arm9, armpll2, img_dramc_cg_regs, 15, 1, INVALID_MUX_GATE_BIT, 0),
};
...
/* gate dvd */
static struct atc_gate_regs dvd_cg_regs = {
	.onoff_ofs = 0x00AC,
	.rst_ofs = 0x00C8,
};

static struct atc_gate dvd_clks[] __initdata = {
	GATE(DVD_CLK_MVDO, dvd_mvdo, aud_dvd_sel, dvd_cg_regs, 0, 1, 0, 1),
	GATE(DVD_CLK_DGO, dvd_dgo, aud_dvd_sel, dvd_cg_regs, 2, 1, 2, 1),
	GATE(DVD_CLK_OSD, dvd_osd, aud_dvd_sel, dvd_cg_regs, 4, 1, 4, 1),
	GATE(DVD_CLK_GRA, dvd_gra, aud_dvd_sel, dvd_cg_regs, 5, 1, 5, 1),
	GATE(DVD_CLK_BIM, dvd_bim, aud_dvd_sel, dvd_cg_regs, 6, 1, 6, 1),
	GATE(DVD_CLK_TURBO32, dvd_turbo32, aud_dvd_sel, dvd_cg_regs, 7, 1, 7, 1),
	GATE(DVD_CLK_VDEC, dvd_vdec, aud_dvd_sel, dvd_cg_regs, 8, 1, 8, 1),
	GATE(DVD_CLK_PARSER, dvd_parser, aud_dvd_sel, dvd_cg_regs, 9, 1, 9, 1),
	GATE(DVD_CLK_RAMBUF, dvd_rambuf, aud_dvd_sel, dvd_cg_regs, 10, 1, 10, 1),
	GATE(DVD_CLK_PT110, dvd_pt110, aud_dvd_sel, dvd_cg_regs, 11, 1, 11, 1),
	GATE(DVD_CLK_RS232, dvd_rs232, aud_dvd_sel, dvd_cg_regs, 12, 1, 12, 1),
	GATE(DVD_CLK_CDDVD, dvd_cddvd, aud_dvd_sel, dvd_cg_regs, 13, 1, 13, 1),
	GATE(DVD_CLK_AUDIO, dvd_audio, aud_dvd_sel, dvd_cg_regs, 14, 1, 14, 1),
	GATE(DVD_CLK_SERVO_MISC, dvd_servo_misc, aud_dvd_sel, dvd_cg_regs, 15, 1, 15, 1),
	GATE(DVD_CLK_RAMBUF_APCTRL_TBUS3, dvd_rambuf_apctrl_tbus3, aud_dvd_sel, dvd_cg_regs, 16, 1, 16, 0),
	GATE(DVD_CLK_RAMBUF_APCTRL_TBUS4, dvd_rambuf_apctrl_tbus4, aud_dvd_sel, dvd_cg_regs, 17, 1, 17, 0),
	GATE(DVD_CLK_RAMBUF_APCTRL_TBUS5, dvd_rambuf_apctrl_tbus5, aud_dvd_sel, dvd_cg_regs, 18, 1, 18, 0),
	GATE(DVD_CLK_MFG_TOP_PWR_WRAP, dvd_mfg_top_pwr_wrap, aud_dvd_sel, dvd_cg_regs, 31, 0, 31, 1),
};

static void __init init_clk_dvd_sys(void __iomem *dvd_base, struct clk_onecell_data *clk_data)
{
	init_clk_gates(dvd_base, dvd_clks, ARRAY_SIZE(dvd_clks), clk_data);
}
...
static struct atc_gate lvds_clks[] __initdata = {
	...
	GATE(LVDS_CLK_TP_TOP1, lvds_tp_top1, tp_sel, lvds_cg_regs, 2, 1, 1, 0),
	GATE(LVDS_CLK_TP_TOP2, lvds_tp_top2, tp_sel, lvds_cg_regs, 3, 1, 1, 0),
...
static void __init ac8317_topckgen_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	if (!base) {
		pr_err("[CCF] ioremap topckgen failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc topckgen clock failed\n");
		return;
	}
	...
	if (r) {
		pr_err("[CCF] could not register clock provide\n");
	}
}
CLK_OF_DECLARE(atc_topckgen, "atc,ac8317-topckgen", ac8317_topckgen_init);

static void __init ac8317_topselect_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	if (!base) {
		pr_err("[CCF] ioremap topselect failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc topselect clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_topselect, "atc,ac8317-topselect", ac8317_topselect_init);


static void __init ac8317_divider_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	if (!base) {
		pr_err("[CCF] ioremap divider failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc divider clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_sub_divider, "atc,ac8317-divider", ac8317_divider_init);

static void __init ac8317_apmixedsys_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	if (!base) {
		pr_err("[CCF] ioremap apmixedsys failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc apmixedsys clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_apmixedsys, "atc,ac8317-apmixedsys", ac8317_apmixedsys_init);
...
static void __init atc_vdecsys_init(struct device_node *node)
{
	...
	if (!base) {
		pr_err("[CCF] ioremap vdecsys failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc vdecsys clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_vdecsys, "atc,ac8317-vdecsys", atc_vdecsys_init);

static void __init ac8317_img_dramc_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	if (!base) {
		pr_err("[CCF] ioremap img dramc failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc img dramc clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_infrasys, "atc,ac8317-imgdramcsys", ac8317_img_dramc_init);
...
static void __init atc_audioperisys_init(struct device_node *node)
{
	...
	if (!base) {
		pr_err("[CCF] ioremap audioperisys failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc audioperisys clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_audioperisys, "atc,ac8317-audioperisys", atc_audioperisys_init);

static void __init atc_dvdsys_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	if (!base) {
		pr_err("[CCF] ioremap dvdsys failed\n");
		return;
	}

	clk_data = alloc_clk_data(DVD_NR_CLK);

	if (clk_data == NULL) {
		pr_err("[CCF] alloc dvdsys clock failed\n");
		return;
	}

	init_clk_dvd_sys(base, clk_data);
...
CLK_OF_DECLARE(atc_dvdsys, "atc,ac8317-dvdsys", atc_dvdsys_init);
...
static void __init atc_lvdssys_init(struct device_node *node)
{
	...
	if (!base) {
		pr_err("[CCF] ioremap lvdssys failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc lvdssys clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_lvdssys, "atc,ac8317-lvdssys", atc_lvdssys_init);
...
static void __init atc_vdoutsys_init(struct device_node *node)
{
	...
	if (!base) {
		pr_err("[CCF] ioremap vdoutsys failed\n");
		return;
	}
	...
	if (clk_data == NULL) {
		pr_err("[CCF] alloc vdoutsys clock failed\n");
		return;
	}
...
CLK_OF_DECLARE(atc_vdoutsys, "atc,ac8317-vdoutsys", atc_vdoutsys_init);
