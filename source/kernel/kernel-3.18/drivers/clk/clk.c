...
struct clk *clk_get_parent_by_name(struct clk *clk, const char *name)
{
	int i = 0;

	if (!clk || !(clk->parents) || !name) {
		return NULL;
	}

	if (clk->num_parents > 0) {
		for (i = 0; i < clk->num_parents; i++) {
			if (clk->parents[i] != NULL && clk->parents[i]->name != NULL) {
				if (!strcmp(clk->parents[i]->name, name)) {
					pr_debug("clk_get_parent_by_name: got index [%d] with [%s] ", i, name);
...
EXPORT_SYMBOL_GPL(clk_get_parent_by_name);
