	...
	for (i = 0; i < all_freq_table->table_size; i++) {
		...
		for_each_possible_cpu(cpu) {
			...
			if (policy == NULL)
				continue;
/*
  * Just for ac8317/ac8237, one cluster in the sample freqence.
  1, because policy-> always 0, so, per_cpu var all_cpufreq_stats just one for cpu0;
  2, If we disable cpu0, then the policy->cpu update to cpu 1, so get the all_stat is NULL, then return N/A
  3, 8317/8237 just use one cluster, and all core in the cluster must at the same frequence, so we get all_stat always from cpu0 per_cpu var.
  4, If we use two cluster(such as 8237 8-core enable), we must abjust it.
*/
			all_stat = per_cpu(all_cpufreq_stats, 0);//policy->cpu);
