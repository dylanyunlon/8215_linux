...
#define GPIO_INT_STA_REG(idx)    GPIO_823X_READ32(REG_RW_INT_STA_OFFSET+(4*(idx)))
#define GPIO_INT_STA_WRITE(idx, val)    \
	GPIO_823X_WRITE32((REG_RW_INT_STA_OFFSET+(4*(idx))), (val))

#define GPIO_INT_STA_REG1()  GPIO_823X_READ32(REG_RW_INT_STA_OFFSET+(4*0))
#define GPIO_INT_STA_REG2()  GPIO_823X_READ32(REG_RW_INT_STA_OFFSET+(4*1))
#define GPIO_INT_STA_REG3()  GPIO_823X_READ32(REG_RW_INT_STA_OFFSET+(4*2))

#define GPIO_INT_STA_CLN1()    \
	GPIO_823X_WRITE32((REG_RW_INT_STA_OFFSET+(4*(0))), (0))
#define GPIO_INT_STA_CLN2()    \
	GPIO_823X_WRITE32((REG_RW_INT_STA_OFFSET+(4*(1))), (0))
#define GPIO_INT_STA_CLN3()    \
	GPIO_823X_WRITE32((REG_RW_INT_STA_OFFSET+(4*(2))), (0))

#define GPIO_INT_GROUP_REG()    GPIO_823X_READ32(REG_RW_GPIO_INT_GROUP_OFFSET)
#define GPIO_INT_GROUP_WRITE(val)    \
	GPIO_823X_WRITE32(REG_RW_GPIO_INT_GROUP_OFFSET, (val))

#define GPIO_INT_EN_REG(idx)    GPIO_823X_READ32(REG_RW_INT_EN_OFFSET+(4*(idx)))
#define GPIO_INT_EN_WRITE(idx, val)    \
	GPIO_823X_WRITE32((REG_RW_INT_EN_OFFSET+(4*(idx))), (val))

#define GPIO_INT_ED2_REG(idx)    GPIO_823X_READ32(REG_RW_INT_ED2_OFFSET+(4*(idx)))
#define GPIO_INT_ED2_WRITE(idx, val)    \
	GPIO_823X_WRITE32((REG_RW_INT_ED2_OFFSET+(4*(idx))), (val))

#define GPIO_INT_LEV_REG(idx)    GPIO_823X_READ32(REG_RW_INT_LEV_OFFSET+(4*(idx)))
#define GPIO_INT_LEV_WRITE(idx, val)    \
	GPIO_823X_WRITE32((REG_RW_INT_LEV_OFFSET+(4*(idx))), (val))

#define GPIO_INT_POL_REG(idx)    GPIO_823X_READ32(REG_RW_INT_POL_OFFSET+(4*(idx)))
#define GPIO_INT_POL_WRITE(idx, val)    \
	GPIO_823X_WRITE32((REG_RW_INT_POL_OFFSET+(4*(idx))), (val))
		...
		unsigned gpio, int value)
{
	...
	spin_lock_irqsave(&gpio_lock, flags);
	val = atc_set_gpio_out(gpio, value);
	spin_unlock_irqrestore(&gpio_lock, flags);
		...
		unsigned gpio, int value)
{
	...
	pr_info("ac83xx_gpio_direction_output is called  gpio = %d value = %d\n",
				gpio, value);
...
{
	return bsp_pinset(funcsel, i4Func);
}
EXPORT_SYMBOL(BSP_PinSet);

int BSP_PinGet(int funcsel)
{
	return bsp_pinget(funcsel);
}
EXPORT_SYMBOL(BSP_PinGet);
...
int gpio_int_clear(int gpionum)
{
	unsigned  offset, idx, val;
	if (gpionum >= TOTAL_GPIO_NUM) {
		return -1;
	}
	offset = gpio_get_addroffset((_au1IntSel[gpionum][2]));
	idx = gpio_get_addridx((_au1IntSel[gpionum][2]));
...
int set_gpio_of_group_irq(unsigned int gpio)
{
	...
	atc_set_gpio_pinmux(gpio, PINMUX_LEVEL_GPIO_END_FLAG);
...
static int atc_gpio_remove(struct platform_device *pdev)
{
	struct ac83xx_gpio *dev;
...
static const struct of_device_id gpio_of_match[] = {
	{.compatible = "autochips,ac823x-gpio",},
	{},
};
MODULE_DEVICE_TABLE(of, gpio_of_match);

static int atc_gpio_probe(struct platform_device *pdev)
{
	...
	pr_info("******************ac823x_gpio_probe start************\n");
	...
	if (node) {
        ...
        pr_err("[GPIO]can't find compatible node\n");
	}

	gpio_irq_num = irq_of_parse_and_map(node, 0);
	if (gpio_irq_num ==0) {
        pr_err("[GPIO]can't find gpio irq\n");
	}else {
        pr_info("[GPIO]gpio_irq_num=%d\n", gpio_irq_num);
	}
	...
	pr_info("******************ac823x_gpio_probe successful************\n");
	#if 0     //for test GPIO pinmux and gpio output
	i = 30;
	pr_info("###before 0x68=%x\n", GPIO_823X_READ32(0x68));
	atc_set_gpio_pinmux(124, PWM2_SEL);
	pr_info("###after 0x68=%x\n", GPIO_823X_READ32(0x68));
	atc_set_gpio_pinmux(134, PINMUX_LEVEL_GPIO_END_FLAG);
	atc_set_gpio_dir(134, OUTPUT);
	pr_info("0x84=%x\n", GPIO_823X_READ32(0x84));
	pr_info("0xF0=%x\n", GPIO_823X_READ32(0xF0));
	pr_info("###after 0x84=%x\n", GPIO_823X_READ32(0x84));
	while(i--)
	{
		atc_set_gpio_out(134, 0);
		msleep(1);
		atc_set_gpio_out(134, 1);
		msleep(1);
	}
	#endif
...
err:
	kfree(dev);
	pr_info("******************ac823x_gpio_probe fail************\n");
	return ret;
}

static struct platform_driver gpio_driver = {
	.probe = atc_gpio_probe,
	.remove = atc_gpio_remove,
	.shutdown = NULL,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
