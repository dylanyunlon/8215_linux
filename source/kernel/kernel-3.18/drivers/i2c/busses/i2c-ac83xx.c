...
struct clk* clk_ac8317_i2c[2];
struct clk* clk_ac8317_i2c_select[2];
struct clk* clk_ac8317_i2c_parent[2];
...
static int ac83xx_i2c_init(struct ac83xx_i2c_dev *dev)
{
	...
	I2C_DBG(KERN_ERR " ac83xx_i2c_init\n");
	...
	if (dev->id == 0) {    /* master0 set pinmux and clock*/
		...
		if (IS_ERR(pins_i2c0_st3)) {
			I2C_ERR("lookup pins_i2c0_st3 failed!");
		}
		ret = pinctrl_select_state(pinctrl_i2c0, pins_i2c0_st3);
		if (ret) {
			I2C_ERR("select pins_i2c0_st3 failed!");
		}
		clk_ac8317_i2c_parent[0] = clk_get(NULL,"clk27m_ck");
		ret = clk_set_parent(clk_ac8317_i2c[0],clk_ac8317_i2c_parent[0]);
		clk_ac8317_i2c_parent[0] = clk_get_parent(clk_ac8317_i2c[0]);
		if(clk_ac8317_i2c_parent[0] == NULL)   {
			I2C_ERR("get i2c0 clk parent error!  ;%d \n",ret);
		}
		...
		clk_prepare(clk_ac8317_i2c_select[0]);
		clk_enable(clk_ac8317_i2c_select[0]); 
		//u4Tmp = SIF_IO_READ32(SIF_CLOCK);
		//pr_err("i2c clk is :%d",u4Tmp);
		/* enable sif master0 */
	...
	if (dev->id == 1) {    /* master1 set pinmux and clock */
		...
		if (IS_ERR(pins_i2c1_st3)) {
			I2C_ERR("lookup pins_i2c1_st3 failed!");
		}
		ret = pinctrl_select_state(pinctrl_i2c1, pins_i2c1_st3);
		if (ret) {
			I2C_ERR("select pins_i2c1_st3 failed!");
		}

		clk_ac8317_i2c_parent[1] = clk_get(NULL,"clk27m_ck");
		ret = clk_set_parent(clk_ac8317_i2c[1],clk_ac8317_i2c_parent[1]);
		clk_ac8317_i2c_parent[1] = clk_get_parent(clk_ac8317_i2c[1]);
		if(clk_ac8317_i2c_parent[1] == NULL)   {
			I2C_ERR("get i2c1 clk parent error!  ;%d \n",ret);
			return 0;
		}
		...
		clk_prepare(clk_ac8317_i2c_select[1]);
		clk_enable(clk_ac8317_i2c_select[1]);   //

		//u4Tmp = SIF_IO_READ32(SIF_CLOCK);
		//pr_err("i2c clk1 is :%d",u4Tmp);
...
static int SifMRead(u32 ucDev, u8 *pucValue, u32 u4Count, u32 NoRDAck)
{
	...
	u4Ack = SIFM_ACK_READ();
	if (u4Ack != 0x1) {
		I2C_DBG("MASTER0 READ ACK FAILURE\n");
		return -EIO;
	}
...
static int SifMWrite(u32 ucDev, const u8 *pucValue, u32 u4Count)
{
	...
	u4Ack = SIFM_ACK_READ();
	if (u4Ack != 0x1) {
		I2C_DBG("MASTER0 WRITE ACK FAILURE\n");
		return -EIO;
	}
				...
				struct i2c_msg *msg, int stop)
{
	...
	if (msg->len == 0)
		return -EINVAL;
	if (dev == NULL) {
		I2C_ERR("i2c_dev is null, get_adapdate fail\n");
		return -EIO;
	}
	if (dev->suspended)
		return -EIO;
		...
		struct i2c_msg msgs[], int num)
{
	...
	if (dev == NULL) {
		I2C_ERR("dev is null, i2c_get_adapdata fail\n");
		return -EIO;
	}
...
static int ac83xx_i2c_probe(struct platform_device *pdev)
{
	...
	pdev->id = i2c_nr++;
	if(i2c_nr == 1) {
		clk_ac8317_i2c[0] = devm_clk_get(&pdev->dev, "i2c-mux0");
		clk_ac8317_i2c_select[0] = devm_clk_get(&pdev->dev, "i2c-select0");
		if((clk_ac8317_i2c[0] == NULL) || (clk_ac8317_i2c_select[0] == NULL))  {
			I2C_ERR("get i2c clk0 %d error! \n",pdev->id);
			return -1;
		}
	}
	else {
		clk_ac8317_i2c[1] = devm_clk_get(&pdev->dev, "i2c-mux1");
		clk_ac8317_i2c_select[1] = devm_clk_get(&pdev->dev, "i2c-select1");
		if((clk_ac8317_i2c[1] == NULL) || (clk_ac8317_i2c_select[1] == NULL))  {
			I2C_ERR("get i2c clk0 %d error! \n",pdev->id);
			return -1;
		}
	}
...
static int ac83xx_i2c_suspend(struct device *dev)
{
	...
	struct ac83xx_i2c_dev *i2c = platform_get_drvdata(pdev);
	if (i2c == NULL) {
		I2C_ERR("i2c_dev is null, get_drvdate fail\n");
		return -EIO;
	}
	i2c->suspended = 1;
	if(pdev->id == 0)  {
		clk_disable(clk_ac8317_i2c_select[0]);
    	clk_unprepare(clk_ac8317_i2c_select[0]);
	}
	if(pdev->id == 1)   {
		clk_disable(clk_ac8317_i2c_select[1]);
    	clk_unprepare(clk_ac8317_i2c_select[1]);
	}
	I2C_DBG("I2c suspend\n");
