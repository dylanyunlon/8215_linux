...
bool AuxADCInitKeypad(void)
{
    ...
    printk("---cgxreg-AuxADCInitKeypad-----enter------\n");
    {
        tmp = IO_READ32(IO_BASE_VA, 0XCC);
        //printk("---cgxreg-1--addr 0XCC = %x\n", tmp);
        tmp |= 0X00000002;
        IO_WRITE32(IO_BASE_VA, 0XCC, tmp);
        tmp = IO_READ32(IO_BASE_VA, 0XCC);
        //printk("---cgxreg-2--addr 0XCC = %x\n", tmp);

        tmp = IO_READ32(IO_BASE_VA, 0XB0);
        //printk("---cgxreg-1--addr 0XB0 = %x\n", tmp);
        tmp |= 0X0000000E;
        IO_WRITE32(IO_BASE_VA, 0XB0, tmp);
        tmp = IO_READ32(IO_BASE_VA, 0XB0);
        //printk("---cgxreg-2--addr 0XB0 = %x\n", tmp);

        tmp = IO_READ32(IO_BASE_VA, 0x6f0);//0X6A0 3363//0x6f0 3365
        //printk("---cgxreg-1--addr 0x6f0 = %x\n", tmp);
        tmp = tmp & (0xfffdffff);
        IO_WRITE32(IO_BASE_VA, 0x6f0, tmp);//0X6A0 3363//0x6f0 3365
        //tmp = IO_READ32(IO_BASE_VA, 0x6f0);
        //printk("---cgxreg-2--addr 0x6f0 = %x\n", tmp);

    }
  
	tmp = ADC_READ32(AUXADC_PDN_CON);
    //printk("---cgxreg-1--addr AUXADC_PDN_CON = %x\n", tmp);
	ADC_WRITE32(AUXADC_PDN_CON, 0);
	tmp = ADC_READ32(AUXADC_PDN_CON);
    //printk("---cgxreg-2--addr AUXADC_PDN_CON = %x\n", tmp);
    
	tmp = ADC_READ32(AUXADC_MISC);
    //printk("---cgxreg-1--addr AUXADC_MISC = %x\n", tmp);
	tmp |= 0x200;
	ADC_WRITE32(AUXADC_MISC, tmp); //enable  ECC
  	tmp = ADC_READ32(AUXADC_MISC);
    //printk("---cgxreg-2--addr AUXADC_MISC = %x\n", tmp);

    //tmp = ADC_READ32(AUXADC_ECC);//cgx
    //printk("---cgxreg-1--addr AUXADC_ECC = %x\n", tmp);
    tmp = 0x1118;
 ...
 */ 
    printk("---cgxreg-AuxADCInitKeypad-----end------\n");
    return true;
}
...
u32  ac823x_knob_init(void)
{
    ...
    pr_debug("ac823x_knob_init  START !\r\n");
    ...
    pr_debug("ac823x_knob_init  DONE !\r\n");
