...
struct semaphore g_aud_volume_lock;
...
bool fgAdspSetMediaType(AUD_MEDIA_TYPE ptAudMediaType)
{
    ...
    {
        LOG(LOG_ADSP_ERR, TEXT("Media type para error.\n"));
        up(&g_aud_media_lock);
        return FALSE;
    }
...
bool fgAudDeconlyGetBuff(AUD_DECONLY_GET_BUF *pGetBuf)
{
    ...
    u32 u4BaseAddr,u4BankAddr,u4ChannelSize;
    u32 u4UserAddr = pGetBuf->u4BufAddr;
    u32 u4InputSampleRate = 0;
    u32 u4InputChannelNum = 0;
    ...
    if(DspGetDeconlyReady())
    {
        ...
        {
            extern void vDeconlyDumpDataForDbg(u32 u4Addr, u32 u4Size);
            vDeconlyDumpDataForDbg((u32)pu1DeconlyBufAddr,eDeconlyGBuf.u4BufLen);
        }
        #endif

        u4TotalSizeByBytes += eDeconlyGBuf.u4BufLen;
        u4TotalSizeCnt++;
        LOG(LOG_FEATURE, TEXT("Packet(%d):BufAddr(0x%08x), BufLen(0x%08x)Total(0x%08x), Bank(0x%08x), PtsH(0x%08x)L(0x%08x)\n"), u4TotalSizeCnt,  \
                                         eDeconlyGBuf.u4BufAddr, eDeconlyGBuf.u4BufLen, u4TotalSizeByBytes, u4TotalSendBank,  \
...
bool AudGetSpectrumData(u8* pAddr, u32 u4Size, u32 u4scaleMode)
{
    ...
    msleep(20);  // for cts gapless test
    return s4Ret;
}
...
void vAdspChannelVolumeGainCtrl(AUD_CH_T eChannel, u32 u4Value)
{
    ...
    AUD_VERIFY(eChannel <= AUD_CH_ALL);
...
void vAdspSrcVolumeCtrl(AUD_MEDIA_SOURCE_TYPE_T eSrc, AUD_MEDIA_OUT_TYPE_T eType, u32 u4Value)
{
    ...
    DspCfgSetSrcVolumeGain(eSrc, eType);
}
