...
#define AUD_VER_MINOR       04           //minor version, Large feature tens add 1 and clear unit,
                                         //other feature and add new function need add 1.
#define AUD_VER_REV         13           //version number, you should add 1 when check in aud
#define AUD_VER_CHANGELIST  150218       //the latest change list of  aud

//code check in time
#define AUD_VER_MONTH  TEXT("09")
#define AUD_VER_DAY    TEXT("14")
#define AUD_VER_YEAR   TEXT("2018")
...
do { \
    if (!(x)) \
        pr_err("[AUD] Err: AUD_ASSERT failed: %s, %d!\r\n", __FILE__, __LINE__); \
} while (0)
...
do { \
    if (!(x))   \
        pr_err("[AUD] Err: AUD_VERIFY failed: %s, %d!\r\n", __FILE__, __LINE__); \
} while (0)
