...
void vAudGpsMix_SemaphoreDelete()
{
    ...
    {
        //AUD_VERIFY(x_sema_delete(_GpsMixSemaUopComDone) == OSR_OK);
    }
}
...
void vAudGpsMixMsgqDelete(void)
{
     if(_fgAudGpsMixDrvInitiated == FALSE)
     {
          AUD_VERIFY(x_msg_q_delete(_AudGpsMixCmdQueue) == OSR_OK);
     }
}
...
static void _GpsMixDrvThreadInit(void)
{
    ...
    if (!_fgAudDrvGpsMixThreadInit)
    {
        ...
        AUD_VERIFY((x_msg_q_create(&_AudGpsMixCmdQueue, s_wname, sizeof(u32), AUD_GPSMIX_CMD_QUEUE_SIZE)) == OSR_OK);
...
bool AudGpsMix_DrvCmd(AUD_DRV_GPS_MIX_CMD_T eGpsMixCmd)
{
    ...
    AUD_VERIFY((x_msg_q_send(_AudGpsMixCmdQueue, &u4Msg, sizeof(u32), 1)) == OSR_OK);
    return TRUE;
}
...
static void _AudGpsMixWaitEvent(u32 * pu4Event)
{
    ...
    do
    {
        AUD_VERIFY(x_msg_q_receive(&u2MsgIdx, &u4Event, &zMsgSize,
...
static s32 _AudGpsMixDrvThread(void* pvArg)
{
    ...
    while (_fgAudDrvGpsMixThreadInit)
    {
        ...
        case AUD_GPS_MIX_DRV_PAUSING:
            ...
            AUD_VERIFY(u1AsvGpsMixPauseCmd());
        ...
        case AUD_GPS_MIX_DRV_STARTING:
            ...
            AUD_VERIFY(u1AsvGpsMixStartCmd());
        ...
        case AUD_GPS_MIX_DRV_RESUMING:
            ...
            AUD_VERIFY(u1AsvGpsMixResumeCmd());
