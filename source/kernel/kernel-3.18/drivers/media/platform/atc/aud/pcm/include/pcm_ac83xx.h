...
typedef struct {
	substream_data subStr;

	/* Private data for record stream*/
	s32                refDataThread_exit;
	s32                refDataThread_wq_flag;
	struct task_struct *refDataThread_task;
	wait_queue_head_t  refDataThread_wq;
	PVirtualMicIn prMic;
	bool isReseted;
	//u32 mState;
} atc_ref_stream;
...
extern s32 ref_wq_flag;
	
extern wait_queue_head_t  ref_wq;
