...
s32 MicIn_SetGain(u32 u4MicGain)
{
	if(u4MicGain <= 63U) {
		...
		PCM_INFO(LOG_TAG, "MicIn_SetGain: gain(%d) is not between 0 and 63, so set fail!\r\n",
		       (u32)u4MicGain);
		return INVALIDPRAM;
	}

	PCM_INFO(LOG_TAG, "MicIn_SetGain: %d (%d dB) \r\n",
...
{
	PCM_INFO(LOG_TAG, "MicIn_GetGain: %d (%d dB) \r\n",
		 (u32)g_rMic.m_rMicCfg.u4MicGain, (s32)(g_rMic.m_rMicCfg.u4MicGain - 14U));
...
s32 MicIn_Start(void *owner)
{
	...
	AUD_ASSERT(owner);
	PCM_DEBUG(LOG_TAG, "MicIn_Start: owner(0x%x).\r\n", (u32)owner);
	...
	PCM_INFO(LOG_TAG, "MicIn_Start: end.\r\n");
...
bool MicIn_EnableDataFromFile(bool fgEnable)
{
	...
	if(STATE_STARTED != g_rMic.m_u4State) {
		if(g_rMic.m_fgDataFromFile != fgEnable) {
			PCM_INFO(LOG_TAG, "MicIn_EnableDataFromFile: fgEnable(%d)\r\n",
			       (u32)fgEnable);
