...
START:
        ...
        if (u4Code != WAIT_OBJECT_0) {
            PCM_ERROR(LOG_TAG, "StreamProc_Thread: Wait StreamProc Event Failed.\r\n");
            return NORESOURCE;
        }
        PCM_DEBUG(LOG_TAG, "StreamProc_Thread: Receive StreamProc Event\r\n");
        if (kthread_should_stop()) {
            PCM_ERROR(LOG_TAG, "StreamProc_Thread: kthread_should_stop\r\n");
            break;
        }
        while (OSR_OK == x_msg_q_receive(&u2MsgIdx, &rStrmProcMsg, &zMsgSize,
            &g_hStrmProcMsgQ, 1, X_MSGQ_OPTION_NOWAIT)) {
            PCM_DEBUG(LOG_TAG, "StreamProc_Thread: chip->m_u4StartCount = %d\r\n", (s32)snd_chip->m_u4StartCount);
            ...
            if (STREAM_PROC_MSG_START == rStrmProcMsg.u4MsgID) {
                ...
                PCM_DEBUG(LOG_TAG, "StreamProc_Thread: OState to stopped\r\n");
                if ((STATE_STARTED == snd_chip->m_pbu4State) && (snd_chip->m_u4StartCount)) {
                    AUD_ASSERT(snd_chip->m_u4StartCount);
                    snd_chip->m_u4StartCount--;
                    if (!snd_chip->m_u4StartCount) {
                        PCM_DEBUG(LOG_TAG, "StreamProc_Thread: Stop with");
                        PCM_DEBUG(LOG_TAG, "chip->m_u4StartCount = %d\r\n", (u32)snd_chip->m_u4StartCount);
                        DspMixOut_Stop();
                        snd_chip->m_pbu4State = STATE_STOPPED;
                    }
                }
            } else {
                PCM_ERROR(LOG_TAG, "StreamProc_Thread: StreamProc MsgID(%d), start(%d).\r\n",
...
static s32 DspMix_Thread(void *data)
{
	...
	PCM_DEBUG(LOG_TAG, "DspMix_Thread: Start\r\n");
	while (true) {

		code = x_event_wait_for_objects(1, &g_hDspMixEvent, false, u4Time);

		if (kthread_should_stop()) {
			PCM_ERROR(LOG_TAG, "DspMix_Thread: kthread_should_stop\r\n");
			break;
		}
		...
		if (code == WAIT_OBJECT_0) {
			...
			PCM_ERROR(LOG_TAG, "DspMix_Thread: Error for WaitForMultipleObjects\r\n");
			break;
		}
	}
	PCM_ERROR(LOG_TAG, "DspMix_Thread: DspMix_Thread End\r\n");
...
s32 DspMixOut_Init(void)
{
	...
	PCM_DEBUG(LOG_TAG, "DspMixOut_Init: m_u4State change to STATE_UNINIT.\r\n");

	if (STATE_UNINIT == m_prDspMixOut.m_u4State) {
		DspMixOut_initBuf();

		m_prDspMixOut.m_u4State = STATE_INITED;
		PCM_DEBUG(LOG_TAG, "DspMixOut_Init: m_u4State change to STATE_INITED.\r\n");
		...
		if (!g_hStrmProcEvent) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Init: Create g_hStrmProcEvent Failed\r\n");
			err = NORESOURCE;
			goto ERROR;
		}
		...
		if (!g_hDspMixEvent) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Init: Create g_hDspMixEvent Failed\r\n");
			err = NORESOURCE;
			goto ERROR;
		}
		...
		if (IS_ERR(g_hDspMixThread_task)) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Init: Create g_hDspMixThread_task ERR!\r\n");
		...
		if (IS_ERR(g_hStreamProcThread_task)) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Init: Create g_hStreamProcThread_task ERR!\r\n");
		...
		if (OSR_OK != x_msg_q_create(&g_hStrmProcMsgQ, TEXT("StreamProc_MSGQ"), sizeof(StrmProc_MSG_T), 40)) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Init: Failed to create StreamProc msg queue\r\n");
			goto ERROR;
		}

		PCM_DEBUG(LOG_TAG, "DspMixOut_Init: Success!\r\n");
	}
...
ERROR:
	...
	if (g_hDspMixThread_task) {
		...
		g_hDspMixThread_task = NULL;
		PCM_ERROR(LOG_TAG, "DspMixOut_Init: g_hDspMixThread_task stop\r\n");
	}

	if (g_hStreamProcThread_task) {
		kthread_stop(g_hStreamProcThread_task);
		g_hStreamProcThread_task = NULL;
		PCM_ERROR(LOG_TAG, "DspMixOut_Init: g_hStreamProcThread_task stop\r\n");
	}

	if (0 != g_hStrmProcMsgQ) {
		if (OSR_OK != x_msg_q_delete(g_hStrmProcMsgQ)) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Init: Failed to delete StreamProc msg queue\r\n");
		}
		g_hStrmProcMsgQ = 0;
	}
...
u32 DspMixOut_UnInit(void)
{
	...
	if ((STATE_INITED == m_prDspMixOut.m_u4State) || (STATE_STOPPED == m_prDspMixOut.m_u4State)) {
		m_prDspMixOut.m_u4State = STATE_UNINIT;
		PCM_ERROR(LOG_TAG, "DspMixOut_UnInit: m_u4State change to STATE_UNINIT.\r\n");
	}
	...
	if (g_hDspMixThread_task) {
		kthread_stop(g_hDspMixThread_task);
		g_hDspMixThread_task = NULL;
		PCM_DEBUG(LOG_TAG, "DspMixOut_UnInit: g_hDspMixThread_task stop\r\n");
	}

	if (g_hStreamProcThread_task) {
		kthread_stop(g_hStreamProcThread_task);
		g_hStreamProcThread_task = NULL;
		PCM_ERROR(LOG_TAG, "DspMixOut_UnInit: g_hStreamProcThread_task stop\r\n");
	}

	if (0 != g_hStrmProcMsgQ) {
		if (OSR_OK != x_msg_q_delete(g_hStrmProcMsgQ)) {
			PCM_ERROR(LOG_TAG, "DspMixOut_UnInit: Failed to delete StreamProc msg queue\r\n");
		}
		g_hStrmProcMsgQ = 0;
	}
...
{
	if ((STATE_INITED == m_prDspMixOut.m_u4State) || (STATE_STOPPED == m_prDspMixOut.m_u4State)) {
		PCM_DEBUG(LOG_TAG, "DspMixOut_Setup: Start\r\n");
		DspMixOut_setBuffer();
		...
		PCM_DEBUG(LOG_TAG, "DspMixOut_Setup: Success!\r\n");
	}
}
...
s32 DspMixOut_Start(void)
{
	...
	if ((STATE_STOPPED == m_prDspMixOut.m_u4State) && (!m_prDspMixOut.m_fgHibernated)) {
		...
		if (code != WAIT_OBJECT_0) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Start: Wait DSP Start Failed.\r\n");
			return NORESOURCE;
		}
		PCM_DEBUG(LOG_TAG, "DspMixOut_Start: DSP Started %dms\r\n", (s32)(1000 * jiffies / HZ));
		m_prDspMixOut.m_u4RP = 0;
		m_prDspMixOut.m_u4State = STATE_STARTED;
		PCM_DEBUG(LOG_TAG, "DspMixOut_Start: m_u4State change to STATE_STARTED.\r\n");
		...
		PCM_ERROR(LOG_TAG, "DspMixOut_Start: State(%d) error!\r\n", (s32)m_prDspMixOut.m_u4State);
		return INVALIDSTATE;
	}
...
s32 DspMixOut_Stop(void)
{
	...
	if (STATE_STARTED == m_prDspMixOut.m_u4State) {
		...
		if (code != WAIT_OBJECT_0) {
			PCM_ERROR(LOG_TAG, "DspMixOut_Stop: Wait DSP Stop Failed.\r\n");
			return NORESOURCE;
		}

		m_prDspMixOut.m_u4State = STATE_STOPPED;
		PCM_DEBUG(LOG_TAG, "DspMixOut_Stop: m_u4State change to STATE_STOPPED.\r\n");
		m_prDspMixOut.m_u4WP = 0;
		m_prDspMixOut.m_u4RP = 0;
		x_memset((void *)m_prDspMixOut.m_u4VirAddr, 0, m_prDspMixOut.m_u4MemSize << 1);
	} else {
		PCM_ERROR(LOG_TAG, "DspMixOut_Stop: DspMixOut is not started!\r\n");
		return INVALIDSTATE;
	}
...
u32 DspMixOut_initOutputCh(void)
{
	...
	DspMixOut_SetOutputCh(AUD_NORMAL_MODE, DSP_MIX_FRONT_LR_CH);
	DspMixOut_SetOutputCh(AUD_BT_CALL_MODE, DSP_MIX_FRONT_LR_CH);
	DspMixOut_SwitchOutPut(AUD_NORMAL_MODE);
...
u32 DspMixOut_SetOutputCh(AUD_OUTPUT_MODE eAudOutPutMode, u32 u4Ch)
{
	...
	PCM_DEBUG(LOG_TAG, "DspMixOut_SetOutputCh: Set dsp mix to ch(0x%x)\r\n", (u32)u4Ch);
...
u32 DspMixOut_SwitchOutPut(AUD_OUTPUT_MODE eAudOutPutMode)
{
	m_prDspMixOut.m_eCurUsedMode = eAudOutPutMode;
        ...
        {
            m_prDspMixOut.m_rDspMixCfg[m_prDspMixOut.m_eCurUsedMode].Union.u4DspMixCh910 = 1;
        }
	PCM_DEBUG(LOG_TAG, "DspMixOut_SwitchOutPut: Switch to ch(0x%x)\r\n",
...
s32 DspMixOut_initBuf(void)
{
	...
	if (!m_prDspMixOut.m_u4VirAddr) {
		PCM_ERROR(LOG_TAG, "DspMixOut_initBuf: Failed to malloc DspMix AOUT Buffer.\r\n");
		return NORESOURCE;
	}
	x_memset((void *)m_prDspMixOut.m_u4VirAddr, 0, m_prDspMixOut.m_u4MemSize);
	m_prDspMixOut.m_u4MemSize >>= 1;
	PCM_DEBUG(LOG_TAG, "DspMixOut_initBuf: SA(0x%x) EA(0x%x) MemSize(%d)\r\n",
		(u32)rBufInfo.u4CommBufSA, (u32)rBufInfo.u4CommBufEA,
...
s32 DspMixOut_GetBuffer(AUD_DATA_BUF_T *prBuffer)
{
	if (prBuffer) {
		...
		PCM_ERROR(LOG_TAG, "DspMixOut_GetBuffer: prBuffer is NULL!\r\n");
		return INVALIDPRAM;
	}
...
u32	DspMixOut_InterruptThread(void)
{
    ...
    else if((snd_chip->m_u4pcmMode ==PCM_MODE_VOIP) && (snd_chip->m_u4speechMode == 0) && (STATE_STOPPED == m_prDspMixOut.m_u4State))
    {
        ContinueCopyDataToRefWhenDlStop();
        return DSPMIX_WAIT_TIMEOUT;
    }
