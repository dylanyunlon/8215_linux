...
#define DEBUG_SDCARD_PRE  "/sdcard"
...
s32 SpeechEnhance_Init(void)
{
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_Init: Enter\r\n");
	...
	if (!g_prSpeechEnhance.m_hCompleted) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_Init: Create m_hCompleted Event error\r\n");
		return NORESOURCE;
	}
	...
	if (!prShareMem) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_Init: malloc prShareMem err.\r\n");
		return NORESOURCE;
	}
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_Init: rFrame(%x) u4MaxFrame(%d)\r\n",
		(u32)prShareMem->rFrame, (s32)prShareMem->u4MaxFrame);
	...
	msleep(1000);
#if ARM1_SPEECH_INTERACT_WITH_ARM2
	SpeechSendMessage((u32)BT_SET_HW_RESOURCE, (u32)SHARE_MEM((u32)prShareMem,
		g_prSpeechEnhance.m_u4PhyAddr), sizeof(BT_SHARE_MEM_EX_T), g_prSpeechEnhance.m_u4HwSemaphore);
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_Init: Wait for ARM2 HW Resource Start\r\n");
	SpeechEnhance_SpeechWaitCompleted((u32)BT_SET_HW_RESOURCE, (u32)INFINITE);
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_Init: Wait for ARM2 HW Resource Finish\r\n");
#endif
	g_prSpeechEnhance.m_u4State = STATE_INITED;
...
static void SpeechEnhance_InitParamFromRegistry(void)
{
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: AEC Param = ");
	for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", (s32)g_prSpeechEnhance.m_prShareMem->rSphParam.enhance_pars[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");

	memcpy(&g_prSpeechEnhance.m_prShareMem->rAecRxParam, aec_com_rx, sizeof(AEC_COM_RX_struct));
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: aec_com_rx Param = ");
	for (u4Idx = 0; u4Idx < AEC_COM_RX; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", g_prSpeechEnhance.m_prShareMem->rAecRxParam.enhance_pars[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");

	memcpy(&g_prSpeechEnhance.m_prShareMem->rAecTxParam, aec_com_tx, sizeof(AEC_COM_RX_struct));
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: aec_com_tx Param = ");
	for (u4Idx = 0; u4Idx < AEC_COM_TX; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", g_prSpeechEnhance.m_prShareMem->rAecTxParam.enhance_pars[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");

	memcpy(&g_prSpeechEnhance.m_prShareMem->rSphParam2, &Sph_Enh_ctrl2, sizeof(SPH_ENH_08K_ctrl_struct));
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: AEC Param2 =");
	for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", (s32)g_prSpeechEnhance.m_prShareMem->rSphParam2.enhance_pars[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");

	memcpy(&g_prSpeechEnhance.m_prShareMem->rDmnrParam, ABF_cal_data, sizeof(DMNR_PARAM_T));
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: DMNR Param =");
	for (u4Idx = 0; u4Idx < DMNR_PARAM_NUM; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", (s32)g_prSpeechEnhance.m_prShareMem->rDmnrParam.dmnrParm[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");

	memcpy(g_prSpeechEnhance.m_prShareMem->Sph_Enh_ctrl_16k, Sph_Enh_ctrl_16k, sizeof(Sph_Enh_ctrl_16k));
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: 16k AEC Param =");
	for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", (s32)g_prSpeechEnhance.m_prShareMem->Sph_Enh_ctrl_16k[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");

	memcpy(g_prSpeechEnhance.m_prShareMem->DMNR_cal_data_16k, DMNR_cal_data_16k, sizeof(DMNR_cal_data_16k));
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: 16k DMNR_cal_data Param =");
	for (u4Idx = 0; u4Idx < DMNR_PARAM_NUM_16K; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", (s32)g_prSpeechEnhance.m_prShareMem->DMNR_cal_data_16k[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");

	memcpy(g_prSpeechEnhance.m_prShareMem->Compen_filter_16k, Compen_filter_16k, sizeof(Compen_filter_16k));
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitParamFromRegistry: 16k COMPEN_FILTER Param =");
	for (u4Idx = 0; u4Idx < COMPEN_FILTER_16K; u4Idx++) {
		if (0 == (u4Idx % 10U)) {
			PCM_DEBUG(LOG_TAG, "\r\n");
		}
		PCM_DEBUG(LOG_TAG, "%d, ", (s32)g_prSpeechEnhance.m_prShareMem->Compen_filter_16k[u4Idx]);
	}
	PCM_DEBUG(LOG_TAG, "\r\n");
}
...
u32 SpeechEnhance_SpeechHandleAECMsg(u32 u4MsgID, u32 u4Param1, u32 u4Param2, u32 u4Param3)
{
	...
	case BT_MSG_COMPLETED:
		...
		x_set_event(g_prSpeechEnhance.m_hCompleted);
		break;
...
static bool SpeechEnhance_SpeechWaitAECState(u32 u4State)
{
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_SpeechWaitAECState: u4State(%x)\r\n", (u32)u4State);

	WaitAECStateTimeOutCnt = 0;
	while (u4State != g_prSpeechEnhance.m_u4AECState) {
		WaitAECStateTimeOutCnt++;
		if (WaitAECStateTimeOutCnt >= 500U) {
			PCM_ERROR(LOG_TAG, "SpeechWaitAECState: Wait ARM2 state timeout error!\r\n");
			break;
		}
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SpeechWaitAECState: m_u4AECState(%x)\r\n",
...
static bool SpeechEnhance_SpeechWaitCompleted(u32 u4Event, u32 u4TimeOut)
{
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_SpeechWaitCompleted: u4Event(%x)\r\n", (u32)u4Event);
	do {

		u32 code = x_event_wait_for_objects(1, &g_prSpeechEnhance.m_hCompleted, FALSE, u4TimeOut);

		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SpeechWaitCompleted: return(%x)\r\n",
			(u32)code);
...
bool SpeechEnhance_PostProcess(u32 u4Index)
{
	...
	if (u4Index != g_prSpeechEnhance.m_u4LastReadIdx) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_PostProcess: u4Index(%d)!=LastReadIdx(%d)\r\n",
	...
	if ((u4CurTime - _u4PTime) > PRINT_INTERVAL) {
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_PostProcess: (%06d ms)\r\n", (s32)u4CurTime);
		PCM_DEBUG(LOG_TAG, "DL: Count(%d) Max(%dms) Avg(%dms). UL Count(%d) Max(%dms) Avg(%dms)\r\n",
	...
	if (prFrame->u4Opt & FRAME_OPT_DL) {
		...
		SpeechDev_EventInform((u32)EVT_DL_FRAME_FINISH, (u32)prFrame);
	} else {
		u32 u4Size = 0;
		...
		SpeechDev_EventInform((u32)EVT_UL_FRAME_FINISH, (u32)prFrame);
		if (g_prSpeechEnhance.m_fgDbgEnable && fgInitFileSuccess) {
			u4Size = (u32)MAX_DBG_FILE_BUFFER -
...
{
	if (!IS_FREE_FRAME) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_GetFreeFrame: No Free Frame. ");
		PCM_ERROR(LOG_TAG, "L-Idx(%d) R-Idx(%d) W-Idx(%d)\r\n",
...
s32 SpeechEnhance_GetEnhanceParam(PCM_SPEECH_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_GetEnhanceParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
	...
	PCM_INFO(LOG_TAG, "SpeechEnhance_GetEnhanceParam: AEC Param = \r\n");
	for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
		PCM_INFO(LOG_TAG, "%d \r\n", (s32)g_prSpeechEnhance.m_prShareMem->rSphParam.enhance_pars[u4Idx]);
	}
	PCM_INFO(LOG_TAG, "rSphParam.error_flag = %d\r\n",
		(s32)g_prSpeechEnhance.m_prShareMem->rSphParam.error_flag);

	PCM_INFO(LOG_TAG, "SpeechEnhance_GetEnhanceParam: AEC Param2 = \r\n");
	for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
		PCM_INFO(LOG_TAG, "%d \r\n", (s32)g_prSpeechEnhance.m_prShareMem->rSphParam2.enhance_pars[u4Idx]);
	}
	PCM_INFO(LOG_TAG, "rSphParam2.error_flag = %d\r\n",
...
s32 SpeechEnhance_SetEnhanceParam(const PCM_SPEECH_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetEnhanceParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
	...
	if (g_prSpeechEnhance.m_prShareMem) {
		...
		PCM_INFO(LOG_TAG, "SpeechEnhance_SetEnhanceParam: AEC Param = \r\n");
		for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
			PCM_INFO(LOG_TAG, "%d \r\n", (s32)g_prSpeechEnhance.m_prShareMem->rSphParam.enhance_pars[u4Idx]);
		}
		PCM_INFO(LOG_TAG, "rSphParam.error_flag = %d\r\n",
		...
		PCM_INFO(LOG_TAG, "SpeechEnhance_SetEnhanceParam: AEC Param2 = \r\n");
		for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
			PCM_INFO(LOG_TAG, "%d \r\n", (s32)g_prSpeechEnhance.m_prShareMem->rSphParam2.enhance_pars[u4Idx]);
		}
		PCM_INFO(LOG_TAG, "rSphParam2.error_flag = %d\r\n",
...
s32 SpeechEnhance_GetEnhance16KParam(PCM_SPEECH_16K_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_GetEnhance16KParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
	...
	PCM_INFO(LOG_TAG, "SpeechEnhance_GetEnhance16KParam: AEC 16K Param = \r\n");
	for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
		PCM_INFO(LOG_TAG, "%d \r\n", (s32)prSpeechConf->enhance_16k_pars[u4Idx]);
	}
...
s32 SpeechEnhance_SetEnhance16KParam(PCM_SPEECH_16K_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetEnhance16KParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
	...
	if (g_prSpeechEnhance.m_prShareMem) {
		...
		PCM_INFO(LOG_TAG, "SpeechEnhance_SetEnhance16KParam: AEC 16K Param = \r\n");
		for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
			PCM_INFO(LOG_TAG, "%d \r\n", (s32)prSpeechConf->enhance_16k_pars[u4Idx]);
		}
	} else {
		PCM_INFO(LOG_TAG, "SpeechEnhance_SetEnhance16KParam:Init AEC 16K Param = \r\n");
		for (u4Idx = 0; u4Idx < AEC_NDC_PARAM_NUM; u4Idx++) {
			Sph_Enh_ctrl_16k[u4Idx] = prSpeechConf->enhance_16k_pars[u4Idx];
			PCM_INFO(LOG_TAG, "%d \r\n", (s32)Sph_Enh_ctrl_16k[u4Idx]);
		}
	}
...
s32 SpeechEnhance_SetDmnrParam(const PCM_SPEECH_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetDmnrParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}

	if (g_prSpeechEnhance.m_prShareMem) {
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetDmnrParam: DMNR Param =");
		for (u4Idx = 0; u4Idx < DMNR_PARAM_NUM; u4Idx++) {
		...
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetDmnrParam:Init DMNR Param =");
		for (u4Idx = 0; u4Idx < DMNR_PARAM_NUM; u4Idx++) {
			ABF_cal_data[u4Idx] = prSpeechConf->ABF_cal_data[u4Idx];
			if (0 == (u4Idx % 10U)) {
				PCM_DEBUG(LOG_TAG, "\r\n");
			}
			PCM_DEBUG(LOG_TAG, "%d, ", (s32)ABF_cal_data[u4Idx]);
		}
		PCM_DEBUG(LOG_TAG, "\r\n");
	}
...
u32 SpeechEnhance_EnableDmnr(bool fgEnable)
{
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_EnableDmnr: fgEnable(%d)", (s32)g_prSpeechEnhance.m_rSphOpt.fgDMNR);
...
s32 SpeechEnhance_SetComRxParam(const PCM_SPEECH_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetComRxParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}

	if (g_prSpeechEnhance.m_prShareMem) {
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetComRxParam: Param = ");
		for (u4Idx = 0; u4Idx < AEC_COM_RX; u4Idx++) {
		...
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetComRxParam:Init Param = ");
		for (u4Idx = 0; u4Idx < AEC_COM_RX; u4Idx++) {
			aec_com_rx[u4Idx] = prSpeechConf->aec_com_rx[u4Idx];
			if (0 == (u4Idx % 10U)) {
				PCM_DEBUG(LOG_TAG, "\r\n");
			}
			PCM_DEBUG(LOG_TAG, "%d, ", aec_com_rx[u4Idx]);
		}
		PCM_DEBUG(LOG_TAG, "\r\n");
	}
	return NOERR;
...
s32 SpeechEnhance_SetComTxParam(const PCM_SPEECH_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetComTxParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}

	if (g_prSpeechEnhance.m_prShareMem) {
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetComTxParam: Param = ");
		for (u4Idx = 0; u4Idx < AEC_COM_TX; u4Idx++) {
		...
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetComTxParam:Init Param = ");
		for (u4Idx = 0; u4Idx < AEC_COM_TX; u4Idx++) {
			aec_com_tx[u4Idx] = prSpeechConf->aec_com_tx[u4Idx];
			if (0 == (u4Idx % 10U)) {
				PCM_DEBUG(LOG_TAG, "\r\n");
			}
			PCM_DEBUG(LOG_TAG, "%d, ", aec_com_tx[u4Idx]);
		}
		PCM_DEBUG(LOG_TAG, "\r\n");
	}
...
s32 SpeechEnhance_SetDmnr16kParam(const PCM_SPEECH_16K_CONF *prSpeechConf)
{
	...
	if (NULL == prSpeechConf) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetDmnr16kParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
	if (g_prSpeechEnhance.m_prShareMem) {
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetDmnr16kParam: 16k DMNR_cal_data Param =");
		for (u4Idx = 0; u4Idx < DMNR_PARAM_NUM_16K; u4Idx++) {
		...
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetDmnr16kParam:Init 16k DMNR_cal_data Param =");
		for (u4Idx = 0; u4Idx < DMNR_PARAM_NUM_16K; u4Idx++) {
			DMNR_cal_data_16k[u4Idx] = prSpeechConf->dmnr_16k_pars[u4Idx];
			if (0 == (u4Idx % 10U)) {
				PCM_DEBUG(LOG_TAG, "\r\n");
			}
			PCM_DEBUG(LOG_TAG, "%d, ", (s32)DMNR_cal_data_16k[u4Idx]);
		}
		PCM_DEBUG(LOG_TAG, "\r\n");
	}
...
s32 SpeechEnhance_SetFilter16kParam(const signed short *pData, u32 u4DataLen)
{
	...
	if (NULL == pData) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetFilter16kParam: pData error!\r\n");
		return INVALIDPRAM;
	}
	if (u4DataLen != sizeof(g_prSpeechEnhance.m_prShareMem->Compen_filter_16k)) {
		PCM_ERROR(LOG_TAG, "SpeechEnhance_SetFilter16kParam: size error!\r\n");
		return INVALIDPRAM;
	}

	if (g_prSpeechEnhance.m_prShareMem) {
		memcpy(g_prSpeechEnhance.m_prShareMem->Compen_filter_16k, pData, u4DataLen);
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetFilter16kParam: 16k COMPEN_FILTER Param =");
		for (u4Idx = 0; u4Idx < COMPEN_FILTER_16K; u4Idx++) {
			if (0 == (u4Idx % 10U)) {
				PCM_DEBUG(LOG_TAG, "\r\n");
			}
		...
		PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetFilter16kParam:Init 16k COMPEN_FILTER Param =");
		for (u4Idx = 0; u4Idx < COMPEN_FILTER_16K; u4Idx++) {
			if (0 == (u4Idx % 10U)) {
				PCM_DEBUG(LOG_TAG, "\r\n");
			}
			PCM_DEBUG(LOG_TAG, "%d, ", (s32)Compen_filter_16k[u4Idx]);
		}
		PCM_DEBUG(LOG_TAG, "\r\n");
	}
...
u32 SpeechEnhance_SetARM2SpeechLog(u32 u4logLevel)
{
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_SetARM2SpeechLog: u4Log = 0x%02x\r\n",
...
u32 SpeechEnhance_EnablePLC(bool fgEnable)
{
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_EnablePLC: m_fgPLC = %d\r\n",
...
u32 SpeechEnhance_EnableDump(bool fgEnable)
{
	...
	PCM_INFO(LOG_TAG, "SpeechEnhance_EnableDump: fgEnable(%d)\r\n\r\n",
...
void WriteWaveFileHeader(const char *filename, const WaveHeader *prWaveHdr)
{
	...
	if (IS_ERR(fd)) {
		PCM_ERROR(LOG_TAG, "WriteWaveFileHeader: filp_open err\r\n");
		set_fs(fs);
		return;
	}
	...
	if (sizeof(WaveHeader) != u4Size) {
		PCM_ERROR(LOG_TAG, "WriteWaveFileHeader: vfs write1 err %i\r\n", u4Size);
	}
	filp_close(fd, NULL);
	set_fs(fs);
}
...
static s32 BtDbg_File_Thread(void *data)
{
	...
	while (1) {
		...
		if (IS_ERR(fd)) {
			PCM_ERROR(LOG_TAG, "BtDbg_File_Thread: filp_open err.filename(%s).\r\n", BtDbg_wavbuf->szFileName);
			set_fs(fs);
			continue;
		}
        ...
        if ((BtDbg_wavbuf->writed_len + BtDbg_wavbuf->data_Len) > MAX_DBG_FILE_BUFFER)
        {
			...
			u4Size = vfs_write(fd, (BtDbg_wavbuf->pwave_buf + BtDbg_wavbuf->writed_len),
				u4Sizetmp, &t_cur_pos);
			if (u4Size < 0) {
				PCM_ERROR(LOG_TAG, "BtDbg_File_Thread: vfs write1 err %i\r\n", u4Size);
				filp_close(fd, NULL);
				set_fs(fs);
			...
			if (u4Size < 0) {
				PCM_ERROR(LOG_TAG, "BtDbg_File_Thread: vfs write2 err %i\r\n", u4Size);
				filp_close(fd, NULL);
				set_fs(fs);
                up(&(g_rSpeechDbgSema[id]));
				continue;
        ...
        else
        {
            ...
            if (BtDbg_wavbuf->pwave_buf != NULL)
            {
                ...
                if (u4Size < 0)
                {
                    PCM_ERROR(LOG_TAG, "BtDbg_File_Thread: vfs write3 err %i\r\n",u4Size);
				    filp_close(fd, NULL);
				    set_fs(fs);
                    up(&(g_rSpeechDbgSema[id]));
				    continue;
...
static s32 SpeechEnhance_InitDebugWaveFiles(void)
{
	...
	PCM_DEBUG(LOG_TAG, "SpeechEnhance_InitDebugWaveFiles: Create BtDbgFile(%s)\r\n",
	...
	for (u4Idx = 0; u4Idx < DBG_FILE_NUM; u4Idx++) {
		...
		if (IS_ERR(Thread_task)) {
			PCM_ERROR(LOG_TAG, "InitDebugWaveFiles: Create BtDbg_File_Thread %d ERR!\r\n", u4Idx);
