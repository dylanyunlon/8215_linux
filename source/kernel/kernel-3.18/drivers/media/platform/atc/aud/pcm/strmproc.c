...
static struct snd_pcm_substream *cap_substream = NULL;
...
static s32 BtOutput_MapGain(s32 StreamGain, s32 Channel)
{
	...
	if ((StreamGain <= MUTE_VOLUME) || (DeviceGain <= MUTE_VOLUME)) {
		PCM_DEBUG(LOG_TAG, "BtOutput_MapGain: MUTE VOLUME\r\n");
		dwGainMultiplier = 0;
	} else {
	...
	PCM_DEBUG(LOG_TAG, "BtOutput_MapGain dwGainMultiplier(0x%x) \r\n", dwGainMultiplier);
...
static void ProcessInput_user(StreamProcess *strmProc);
static void ProcessOutput_user(StreamProcess *strmProc);
...
void StreamProcess_Init(StreamProcess *strmProc)
{
	...
	PCM_DEBUG(LOG_TAG, "StreamProcess_Init: end\r\n");
}
...
s32 StrmProcDSPMixCh(u32 u4DspMixCh)
{
	...
	if ((((u32)0x0) == (u4DspMixCh & 0x3FU)) || /*  Mix to NULL */
		...
		PCM_ERROR(LOG_TAG, "StrmProcDSPMix: Dsp Can not mix to Ch(0x%x)\r\n",
			(u32)u4DspMixCh);
		return INVALIDPRAM;
	}

	if (u4DspMixCh & 0x3U) {
		u4MixCh |= DSP_MIX_FRONT_LR_CH;
	}
	if (u4DspMixCh & 0xCU) {
		u4MixCh |= DSP_MIX_SURROUND_CH;
	}
	if (u4DspMixCh & 0x10U) {
	...
	PCM_DEBUG(LOG_TAG, "StrmProcDSPMix: u4DspMixCh=0x%x, u4MixCh=0x%x\r\n",
		(u32)u4DspMixCh, (u32)u4MixCh);
	DspMixOut_SetOutputCh(AUD_NORMAL_MODE, u4MixCh);
	PCM_DEBUG(LOG_TAG, "StrmProcDSPMix: m_fxpGain[0]=0x%x, m_fxpGain[1]=0x%x\r\n",
		m_fxpGain[0], m_fxpGain[1]);
	PCM_DEBUG(LOG_TAG, "StrmProcDSPMix: m_fxpBTGain[0]=0x%x, m_fxpBTGain[1]=0x%x\r\n",
...
static s32 StrmProcStart(u32 u4StrmProcIdx)
{
	...
	if (OSR_OK != x_msg_q_send(g_hStrmProcMsgQ, &rStrmProcMsg, sizeof(StrmProc_MSG_T), 1)) {
		PCM_ERROR(LOG_TAG, "StrmProcStart: Send StrmProc Start msg fail!\r\n");
		return NORESOURCE;
	}
	...
	PCM_DEBUG(LOG_TAG, "StrmProcStart: Start StrmProc.\r\n");
...
s32 StrmProcStop(u32 u4StrmProcIdx)
{
	...
	rStrmProcMsg.u4StrmProcID = u4StrmProcIdx;
	if (OSR_OK != x_msg_q_send(g_hStrmProcMsgQ, &rStrmProcMsg, sizeof(StrmProc_MSG_T), 1)) {
		PCM_ERROR(LOG_TAG, "StrmProcStop: Send StrmProc Stop msg fail!\r\n");
		return NORESOURCE;
	}

	x_event_set(g_hStrmProcEvent);

	PCM_DEBUG(LOG_TAG, "StrmProcStop: Stop StrmProc.\r\n");
...
s32 PrepareStream(struct snd_pcm_substream *substream)
{
	...
	if ((!substream) || (!substream->runtime) || (!substream->runtime->private_data)) {
		AUD_ASSERT(substream);
		PCM_ERROR(LOG_TAG, "PrepareStream: Parameter Error.\r\n");
		return INVALIDPRAM;
	}
	...
	PCM_DEBUG(LOG_TAG, "PrepareStream: successfully\r\n");
...
s32 AttachStream(struct snd_pcm_substream *substream, StreamProcess *strmProc)
{
	...
	PCM_DEBUG(LOG_TAG, "AttachStream: IFS(%d), OFS(%d), IBW(%d), OBW(%d), Channels(%d)\r\n",
	...
	if (NOERR != AsrcMgr_AllocASRC(&rAsrFmt, substrm_data->IsBtSpeech, &u4Idx)) {
		PCM_ERROR(LOG_TAG, "AttachStream: AsrcMgr_AllocASRC err.\r\n");
		return NORESOURCE;
	}
	substrm_data->m_u4Idx = u4Idx;
	strmProc->m_u4Idx = u4Idx;
	strmProc->m_prStream = substream;
	PCM_DEBUG(LOG_TAG, "AttachStream: successfully\r\n");
...
s32 DetachStream(struct snd_pcm_substream *substream)
{
	...
	if (strmProc->m_prStream != substream) {
		PCM_ERROR(LOG_TAG, "DetachStream: m_prStream(0x%x) != substream(0x%x)\r\n",
	...
	while (STATE_STARTED == strmProc->m_u4OState) {
		...
		Sleep(1);
		if (5000U == u4Count) {
			PCM_ERROR(LOG_TAG, "DetachStreamProc: Stream(%d) is in running state!\r\n",
				(s32)strmProc->m_u4Idx);
			return INVALIDSTATE;
		}
	}

	PCM_DEBUG(LOG_TAG, "QK DetachStreamProc: Start to detach.\r\n");
	...
	PCM_DEBUG(LOG_TAG, "QK DetachStream: end.\r\n");
	...
	case EVT_UPDATE_PLAYED_SAMPLE:
		if (STATE_STARTED == strmProc->m_u4OState) {
			if (0 == strmProc->m_rOutFmt.u4FS) {
				PCM_ERROR(LOG_TAG, "EventInform: error u4SampleRate = 0\r\n");
				return INVALIDPRAM;
			}
			...
			ProcessOutput_user(strmProc);
			ProcessInput_user(strmProc);
			#else
...
s32 DataAvailableInform(struct snd_pcm_substream *substream)
{
	...
	if (strmProc->m_prStream != substream) {
		PCM_ERROR(LOG_TAG, "DataAvailableInform: strmProc->m_prStream != substream\r\n");
		return INVALIDPRAM;
	}
...
static void ProcessInput(StreamProcess *strmProc)
{
	...
	if (NULL == strmProc) {
		PCM_ERROR(LOG_TAG, "ProcessInput: strmProc is NULL!\r\n");
		return;
	}

	substream = strmProc->m_prStream;
	if (NULL == substream) {
		PCM_ERROR(LOG_TAG, "ProcessInput: substream is NULL!\r\n");
		return;
	}

	runtime = substream->runtime;
	if (NULL == runtime) {
		PCM_ERROR(LOG_TAG, "ProcessInput: runtime is NULL!\r\n");
		return;
	}

	substrm_data = runtime->private_data;
	if (NULL == substrm_data) {
		PCM_ERROR(LOG_TAG, "ProcessInput: substrm_data is NULL!\r\n");
		return;
	}
	...
	if (rInBuf.u4DataSz >= (rInBuf.u4ChBufSz >> 1)) {
		PCM_ERROR(LOG_TAG, "ProcessInput: rInBuf.u4DataSize = %d, rInBuf.u4BufSize/2 = %d\r\n",
	...
	if (u4AvaiBytes < strmProc->m_u4MiniBytes) {
		if (u4AvaiBytes && (!strmProc->m_u4OutLen)) {
			...
			if ((rInBuf.u4ChBufSz - rInBuf.u4DataSz) <= ASRC_BUFFER_RESERVE_SIZE) {
				...
				PCM_ERROR(LOG_TAG, "ProcessInput: discard data < 16 samples.\r\n");
			}
		}
		return;
	}
	...
	if (u4OldOff != rInBuf.u4DataOff) {
		...
		if (strmProc->m_u4ProcInTime > 5) {
			PCM_DEBUG(LOG_TAG, "ProcessInput: cost %d ms.\r\n", (s32)strmProc->m_u4ProcInTime);
		}
	}
}
...
static void ProcessInput_user(StreamProcess *strmProc)
{
	...
	if (NULL == strmProc) {
		PCM_ERROR(LOG_TAG, "ProcessInput_user: strmProc is NULL!\r\n");
		return;
	}

	substream = strmProc->m_prStream;
	if (NULL == substream) {
		PCM_ERROR(LOG_TAG, "ProcessInput_user: substream is NULL!\r\n");
		return;
	}

	runtime = substream->runtime;
	if (NULL == runtime) {
		PCM_ERROR(LOG_TAG, "ProcessInput_user: runtime is NULL!\r\n");
		return;
	}

	substrm_data = runtime->private_data;
	if (NULL == substrm_data) {
		PCM_ERROR(LOG_TAG, "ProcessInput_user: substrm_data is NULL!\r\n");
		return;
	}
	...
	if (rInBuf.u4DataSz >= (rInBuf.u4ChBufSz >> 1)) {
		PCM_ERROR(LOG_TAG, "ProcessInput_user: rInBuf.u4DataSize = %d, rInBuf.u4BufSize/2 = %d\r\n",
			(s32)rInBuf.u4DataSz, (s32)(rInBuf.u4ChBufSz >> 1));
		return;
	}
	...
	if (u4AvaiBytes < strmProc->m_u4MiniBytes) {
		if (u4AvaiBytes && (!strmProc->m_u4OutLen)) {
			...
			if ((rInBuf.u4ChBufSz - rInBuf.u4DataSz) <= ASRC_BUFFER_RESERVE_SIZE) {
				...
				GetCurrentData(runtime, &rStrmData);
				PCM_ERROR(LOG_TAG, "ProcessInput_user: discard data < 16 samples.\r\n");
			}
		}
		return;
	}
...
/*#if (!NO_GAIN_CTRL)
	...
	if (u4OldOff != rInBuf.u4DataOff) {
		...
		strmProc->m_u4ProcInTime = (u32)1000 * (u32)jiffies / (u32)HZ - u4STime;
		if (strmProc->m_u4ProcInTime > 5) {
			PCM_DEBUG(LOG_TAG, "ProcessInput_user: cost %d ms.\r\n", (s32)strmProc->m_u4ProcInTime);
		}
	}
}
...
u32 SetCapSubStream(struct snd_pcm_substream *substream){
	...
	cap_substream = substream;
    up(&g_prSpeechDev.m_refLock);

    if(cap_substream == NULL && snd_chip->m_u4SampleRate == 0)
    {
        pr_debug("Note:SetRefSubStream:NULL enter\n");
        StreamProcess *prTmp = NULL;
        mutex_lock(&snd_chip->m_HeadLock);
    	prTmp = snd_chip->m_prSPHead;
    	while (prTmp) {
            pr_debug("Note:prTmp->m_u4BufState = BUF_STATE_NO_FILL\n");
    		prTmp->m_u4BufState = BUF_STATE_NO_FILL;
...
void CopyDataToRef(PWAVE_DATA_BUF_T prAsrcOut, u32 u4TranLen,StreamProcess *strmProc,substream_data *DLsubstrm_data)
{
	...
	u32 hw_pos = 0;
	u32 hw_bytes = 0;
	s8 *m_lpCurrData = NULL;
	u32 i, j, u4RP, u4WP = 0;
    u32 au4TranLen[2]={0}, au4CopySz[2]={0};
	u32 re_pos = 0;
	int dma_freeSize = 0;
	...
	if(snd_chip->m_u4SampleRate == SAMPLE_RATE_8000 || snd_chip->m_u4SampleRate == SAMPLE_RATE_16000){
		if(DLsubstrm_data->IsBtSpeech == 1)
		{
			if(substrm_data->m_eState != SNDRV_PCM_TRIGGER_START || strmProc->m_u4BufState != BUF_STATE_HAS_DATA){
                up(&g_prSpeechDev.m_refLock);
				return NOERR;
	}
	...
	if((!m_lpCurrData)) {
				pr_debug("CopyDataToRef:m_lpCurrData null,substrm_data->m_eState:%d\n",substrm_data->m_eState);
                up(&g_prSpeechDev.m_refLock);
				return NOERR;
	}
...
void OutputDataCopy(StreamProcess *strmProc, PWAVE_DATA_BUF_T prAsrcOut, u32 u4TranLen)
{
	u32 u4MuteChn = 0;
	u32 u4LGain = 0;
	u32 u4RGain = 0;
	...
	for (i = 0; (i < 2U) && (au4TranLen[i] > 0); i++) {
		...
		for (j = 0; (j < 2U) && (au4CopySz[j] > 0); j++) {
			...
			while (au4CopySz[j]) {
				i4Data = (s32)(*pi2DstL + (*pi2SrcL * (s16)u4LGain));
				pi2SrcL++;
				*pi2DstL = Sat_16BIT(i4Data);
				pi2DstL++;

				i4Data = (s32)(*pi2DstR + (*pi2SrcR * (s16)u4RGain));
				pi2SrcR++;
...
void OutputDataCopy_user(StreamProcess *strmProc, PWAVE_DATA_BUF_T prAsrcOut, u32 u4TranLen,substream_data *substrm_data)
{
	u32 u4MuteChn = 0;
	u32 u4LGain = 0;
	u32 u4RGain = 0;
	u32 dwLGain, dwRGain, dwGainShift = 0;
	u32 i, j, u4RP, u4WP = 0;
       u32 au4TranLen[2] = {0}, au4CopySz[2] = {0};
	s32 i4Data = 0;
	MIXER_DATA_T rStrmData;
    ...
    if (SetBtSPHOnly && (1 == substrm_data->IsBtSpeech)) {
        ...
        if ((newGainL != oldGainL) || (newGainR != oldGainR)) {
            ...
            if (0 == stepGainL) {
                ...
                needSmoothL = 1;
                pr_debug("needSmoothL ===oldGainL:%d\n",oldGainL);
            }
...
static void ProcessOutput(StreamProcess *strmProc)
{
	...
	if (NULL == strmProc) {
		PCM_ERROR(LOG_TAG, "ProcessOutput: strmProc is NULL!\r\n");
		return;
	}

	substream = strmProc->m_prStream;
	if (NULL == substream) {
		PCM_ERROR(LOG_TAG, "ProcessOutput: substream is NULL!\r\n");
		return;
	}

	runtime = substream->runtime;
	if (NULL == runtime) {
		PCM_ERROR(LOG_TAG, "ProcessOutput: runtime is NULL!\r\n");
		return;
	}

	substrm_data = runtime->private_data;
	if (NULL == substrm_data) {
		PCM_ERROR(LOG_TAG, "ProcessOutput: substrm_data is NULL!\r\n");
		return;
	}
	...
	if (u4TranLen) {
		...
		if (!strmProc->m_u4OutLen) {
			...
			if (BUF_STATE_NO_FILL != strmProc->m_u4BufState) // Ingore first times.
			{
				if (!strmProc->m_u4UnderrunCount)
				{
					...
					PCM_INFO(LOG_TAG, "stream(%d) bt(%d) underrun!!! (%d) ms\r\n",
		...
		else
		{
			if (strmProc->m_u4UnderrunCount)
			{
				...
				PCM_INFO(LOG_TAG, "stream(%d) bt(%d) underrun count(%d) (%d) ms\r\n",
		...
		OutputDataCopy(strmProc, &rAsrcOut, u4TranLen);
	...
	if (u4STime > 10U) {
		PCM_DEBUG(LOG_TAG, "ProcessOutput: cost %d ms. \r\n", (s32)u4STime);
	}
...
static void ProcessOutput_user(StreamProcess *strmProc)
{
	...
	if (NULL == strmProc) {
		PCM_ERROR(LOG_TAG, "ProcessOutput_user: strmProc is NULL!\r\n");
		return;
	}

	substream = strmProc->m_prStream;
	if (NULL == substream) {
		PCM_ERROR(LOG_TAG, "ProcessOutput_user: substream is NULL!\r\n");
		return;
	}

	runtime = substream->runtime;
	if (NULL == runtime) {
		PCM_ERROR(LOG_TAG, "ProcessOutput_user: runtime is NULL!\r\n");
		return;
	}

	substrm_data = runtime->private_data;
	if (NULL == substrm_data) {
		PCM_ERROR(LOG_TAG, "ProcessOutput_user: substrm_data is NULL!\r\n");
		return;
	}
	...
	if (u4TranLen) {
		...
		if (!strmProc->m_u4OutLen) {
            if(ref_substream !=NULL&&cap_substream!=NULL && snd_chip->m_u4SampleRate != SAMPLE_RATE_8000&& snd_chip->m_u4SampleRate != SAMPLE_RATE_16000){
				if (BUF_STATE_NO_FILL == strmProc->m_u4BufState)
                {
                    strmProc->m_u4OutWP = DspMixOut_GetNextRP(&strmProc->m_u4OutLen);
                }
            ...
            pr_debug("QK strmProc->m_u4OutWP:%d\n",strmProc->m_u4OutWP);
			if (BUF_STATE_NO_FILL != strmProc->m_u4BufState) // Ingore first times.
			{
				if (!strmProc->m_u4UnderrunCount)
				{
		...
		if(ref_substream != NULL)
		{
			...
			if( substrm_data->IsBtSpeech == 1 || substrm_data->IsVoIP== 1)
			{
		...
		OutputDataCopy_user(strmProc, &rAsrcOut, u4TranLen,substrm_data);
		DspMixOut_UpdateWP(strmProc->m_u4OutWP);
	...
	if (strmProc->m_u4OutLen) {
		 if (BUF_STATE_NO_FILL == strmProc->m_u4BufState) {
			if (substrm_data->IsBtSpeech) {
				SpeechDev_SynchronizationEx_user();
                strmProc->m_u4BufState = BUF_STATE_HAS_DATA;
			}
			else
            {
                if(snd_chip->m_u4pcmMode == PCM_MODE_VOIP)
                {
                    down(&g_prSpeechDev.m_micLock);
				    if(ref_substream !=NULL&&cap_substream!=NULL){
					    if(snd_chip->m_u4SampleRate != SAMPLE_RATE_8000&& snd_chip->m_u4SampleRate != SAMPLE_RATE_16000){
						    struct snd_pcm_runtime *runtime = cap_substream->runtime;
						    atc_ref_stream *cap_stream = NULL;
    						substream_data *substrm_data_1 = runtime->private_data;
    						cap_stream = (atc_ref_stream *) substrm_data_1;
    						if(cap_stream->isReseted ==false && cap_stream->prMic != NULL && substrm_data_1->m_eState == SNDRV_PCM_TRIGGER_START ){
							    //strmProc->m_u4OutWP =DspMixOut_GetTRP();
							    cap_stream->isReseted =true;
							    cap_stream->prMic->UpdateRP(cap_stream->prMic, cap_stream->prMic->GetWP(cap_stream->prMic));
							    cap_stream->refDataThread_wq_flag = 1;
							    wake_up_interruptible(&cap_stream->refDataThread_wq);
                                strmProc->m_u4BufState = BUF_STATE_HAS_DATA;
							    pr_debug("QK wake up the mic thread\n");
						    }
                            else
                            {
                                strmProc->m_u4BufState = BUF_STATE_NO_FILL;
                            }
					    }
				    }
                    else
	...
	u4STime = (u32)1000 * (u32)jiffies / (u32)HZ - u4STime;
	if (u4STime > 10U) {
		PCM_DEBUG(LOG_TAG, "ProcessOutput_user: cost %d ms. \r\n", (s32)u4STime);
	}
...
void ContinueCopyDataToRefWhenDlStop(void)
{
    if(ref_substream !=NULL)
    {
        ...
        if(substrm_data->m_eState == SNDRV_PCM_TRIGGER_START)
        {
            hw_pos = (u32)frames_to_bytes(runtime, (snd_pcm_sframes_t)(substrm_data->last_ptr - substrm_data->hw_Base));
        	m_lpCurrData = (s8 *)(substrm_data->dma_start + hw_pos);
            if(m_lpCurrData != NULL)
            {
                dma_freeSize = (runtime->status->hw_ptr > runtime->control->appl_ptr)?(runtime->status->hw_ptr - runtime->control->appl_ptr):(substrm_data->buffer_size-runtime->control->appl_ptr+runtime->status->hw_ptr);
                if(dma_freeSize > 1920)dma_freeSize = 1920;
                s16 *pi2Dst = (s16 *)(substrm_data->dma_start + hw_pos);
                while(dma_freeSize)
                {
                    *pi2Dst = 0;
                    pi2Dst++;
