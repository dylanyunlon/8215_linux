...
#define ADCLOG_ERR(exp, ...)                 AUDLOG(_u4AdcLog & ALOG_ERR,  T("[AUD][ADC]"), exp, ##__VA_ARGS__)
#define ADCLOG_WARN(exp, ...)                AUDLOG(_u4AdcLog & ALOG_WARN, T("[AUD][ADC]"), exp, ##__VA_ARGS__)
#define ADCLOG_INFO(exp, ...)                AUDLOG(_u4AdcLog & ALOG_INFO, T("[AUD][ADC]"), exp, ##__VA_ARGS__)
#define ADCLOG_CLI(exp, ...)                 AUDLOG(_u4AdcLog & ALOG_CLI,  T("[AUD][ADC][CLI]"), exp, ##__VA_ARGS__)
#define ADCLOG_DBG(exp, ...)                 AUDLOG(_u4AdcLog & ALOG_DBG,  T("[AUD][ADC]"), exp, ##__VA_ARGS__)

#define ADCLOG_TEST(exp, ...)                AUDLOG(_u4AdcLog & ALOG_TEST, T("[AUD][ADC][TEST]"), exp, ##__VA_ARGS__)

#define ADCLOG_ERR_DBG(err, exp, ...)    \
    if (err){                       \
        ADCLOG_ERR(exp, ##__VA_ARGS__);             \
    } else { \
        ADCLOG_DBG(exp, ##__VA_ARGS__);             \
    }
