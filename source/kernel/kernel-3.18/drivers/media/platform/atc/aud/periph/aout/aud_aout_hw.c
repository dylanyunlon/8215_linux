...
static void AoutHw_InitCfg(void * pThis, void * pCfg)
{
    ...
    AoutHw_SetNsadr(prThis, prBuf->u4Chn, (prBuf->u4PhySddr & 0xFF), prBuf->u4ChBufSz);
  #endif
...
static void AoutHw_SetDataSrc(void * pThis, AUD_DAC_TYPE_T eDacType, bool fgAdcBypasMode)
{
    ...
    else if (AUDID_AOUT1 == prThis->eAoutId)
    {
        ...
        if (fgAdcBypasMode)
        {
            AoutHw_SetAoutAsDataPin(prThis, eDacType, ASDATA_PIN1, ePinSrc);
        }else {
            AoutHw_SetAoutAsDataPin(prThis, eDacType, ASDATA_PIN1, ASDATA_RLRR);
        }
    }
}
