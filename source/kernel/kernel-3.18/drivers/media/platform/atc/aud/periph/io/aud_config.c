...
bool AudAout_NewHalClass(AUD_AOUT_DEVID eAoutID)
{
    ...
    AUD_ASSERT((eAoutID == AUDID_AOUT1) ||(eAoutID == AUDID_AOUT2));
...
void AudAout_Init(AUD_CLOCK_T eAoutClkId, AUDIO_SAMPLING_T eSample)
{
    ...
    AUD_ASSERT(FALSE != AudAout_NewHalClass(eAoutId));
    AUD_ASSERT(FALSE != AudAout_StartHalClass(eAoutId, eSample));
}
...
bool AudAout_DacTypeSet(AUD_DAC_TYPE_SEL_T *pDacType)
{
    ...
    AUD_ASSERT((pDacType->eOut == AUD_FRONT) || (pDacType->eOut == AUD_REAR));
