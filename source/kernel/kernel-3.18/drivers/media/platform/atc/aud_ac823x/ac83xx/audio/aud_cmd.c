...
#define AUD_CLI_DECONLY_DUMP            (AUD_CLI_MEMORY_GRP    + 11)
...
typedef struct _tagAudDumpFifo
{
    ...
    uintptr_t(*u4GetFIFOSAdr)(void* pUsr);
    uintptr_t(*u4GetFIFOEAdr)(void* pUsr);
    uintptr_t(*u4GetWPtr)(void* pUsr);
} AUD_DUMP_FIFO;
...
extern uintptr_t u4AudHalGetDSPDataPageStartAddr(u32 u4PageId);
...
static s32 vAudDumpFifoTask(void* pvArg)
{
    ...
    u32 dwWriteSize = 0;  
    uintptr_t u4SAdr = pDump->u4GetFIFOSAdr(pDump->pUsr);
    uintptr_t u4EAdr = pDump->u4GetFIFOEAdr(pDump->pUsr);
    uintptr_t u4FifoSize = u4EAdr - u4SAdr;
    uintptr_t u4WPtr = pDump->u4GetFIFOSAdr(pDump->pUsr);//dump read pointer
    uintptr_t u4CWPtr = 0;//real afifo write pointer
    ...
    while(*bRunFlag)
    {
        ...
        if (u4CWPtr != u4WPtr)
        {
            ...
            LOG(LOG_CTRLF,TEXT("[AudCliCmd]DUMPFIFO u4WPtr = 0x%lx,u4CWPtr = 0x%lx,u4Size = 0x%x,\r\n"),
                         u4WPtr, u4CWPtr, u4Size);

            i4Ret = i4DumpFifoFsWriteFile(pDump->hFs,(void *)u4WPtr,(u32)u4Size,&dwWriteSize,NULL);
...
{
    bool* bRunFlag = NULL;
    uintptr_t u4Size[AUD_AOUT_CHNUM] = {0};
    u32 u4TotalSz[AUD_AOUT_CHNUM] = {0};
    u32 dwWriteSize[AUD_AOUT_CHNUM] = {0};  
    uintptr_t u4SAdr[AUD_AOUT_CHNUM] = {0};
    uintptr_t u4EAdr[AUD_AOUT_CHNUM] = {0};
    uintptr_t u4FifoSize[AUD_AOUT_CHNUM] = {0};
    uintptr_t u4WPtr[AUD_AOUT_CHNUM] = {0};
    uintptr_t u4CWPtr[AUD_AOUT_CHNUM] = {0};//real afifo write pointer
    u32 u4Ind =0;
    ...
    while(*bRunFlag)
    {
        ...
        { 
            if (u4CWPtr[u4Ind] != u4WPtr[u4Ind])
            {
                ...
                i4DumpFifoFsWriteFile(g_hAudDumAoutFs[u4Ind],(void *)(u4WPtr[u4Ind]),(u32)u4Size[u4Ind],&(dwWriteSize[u4Ind]),NULL);
...
/************************************************************************************************
 ...
 Modify Data:
                       2011.3.7     modify format according coding standard
*************************************************************************************************/
static uintptr_t u4AudDUMPAFIFOGetFIFOSAdr(void* pUsr)
{
    uintptr_t u4DecId = (uintptr_t)pUsr;
...
/************************************************************************************************
 ...
 Modify Data:
                       2011.3.7     modify format according coding standard
*************************************************************************************************/
static uintptr_t u4AudDUMPAFIFOGetFIFOEAdr(void* pUsr)
{
    uintptr_t u4DecId = (uintptr_t)pUsr;
...
/************************************************************************************************
 ...
 2011.3.7     modify format according coding standard
*************************************************************************************************/
static uintptr_t u4AudDUMPAFIFOGetWPtr(void* pUsr)
{
    uintptr_t u4DecId = (uintptr_t)pUsr;
...
/************************************************************************************************
 ...
 Modify Data:
*************************************************************************************************/
static uintptr_t u4AudDUMPAOUTGetFIFOSAdr(void* pUsr)
{
    uintptr_t u4CmdId = (uintptr_t)pUsr;
    u32 u4AoutId =  (u4CmdId>>4) & 0x0F;
    u32 u4ChId = u4CmdId & 0x0F;
    uintptr_t u4BaseAddr;
    u32 u4Ch1Size, u4Ch1SAdr;
    uintptr_t u4SAdr = 0;
...
*************************************************************************************************/
static uintptr_t u4AudDUMPAOUTGetFIFOEAdr(void* pUsr)
{
    uintptr_t u4CmdId = (uintptr_t)pUsr;
    u32 u4AoutId =(u4CmdId>>4) & 0x0F;
    u32 u4ChId = u4CmdId & 0x0F;
...
/************************************************************************************************
 ...
 Modify Data:
*************************************************************************************************/
static uintptr_t u4AudDUMPAOUTGetRefPtr(void* pUsr)
{
    uintptr_t u4CmdId = (uintptr_t)pUsr;
    u32 u4AoutId = (u4CmdId>>4) & 0x0F;
    u32 u4ChId = u4CmdId & 0x0F;
    uintptr_t u4BaseAddr;
    u32 u4NSAdrId, u4NSAdr;
    uintptr_t u4RPtr = 0;
...
void AudCliDumpAfifo(u32 u4Cmdid,const s8 ** filename)
{
    ...
    if(9 == u4Cmdid)
    {
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] MAP m_afifo_info u4AfifoWPtr= 0x%lx \r\n"), u4AudEsmGetWritePtr(pContext));
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] MAP m_afifo_info u4AfifoRPtr= 0x%lx \r\n"), u4AudEsmGetReadPtr(pContext));
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] u4AfifoWPtrForApp=0x%lx\r\n"), pContext->u4AfifoWPtrForApp);
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] AFIFO HW wptr 0x5088= %lx \r\n"), u4AudDUMPAFIFOGetWPtr((void*)PRI_DEC));
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] AFIFO RD rptr 0x50B0= %lx \r\n"), u4AudHalGetBufRPtr(PRI_DEC, DSP_AFIFO));
       return;
    }
    ...
    if(10 == u4Cmdid)
    {
       ...
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] MAP m_afifo_info u4AfifoRPtr= 0x%lx \r\n"), u4AudEsmGetReadPtr(pContext));
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] u4AfifoWPtrForApp=0x%lx\r\n"), pContext->u4AfifoWPtrForApp);
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] AFIFO HW wptr = %lx \r\n"), u4AudDUMPAFIFOGetWPtr((void*)SEC_DEC));
       LOG(LOG_CTRLF, TEXT("[AudCliCmd] AFIFO RD rptr = %lx \r\n"), u4AudHalGetBufRPtr(SEC_DEC, DSP_AFIFO)); ;
       return;
    }
    ...
    if(11 == u4Cmdid)
    {
       ...
       LOG(LOG_CTRLF, TEXT("MAP m_afifo_info u4AfifoWPtr= 0x%lx \r\n"), u4AudEsmGetWritePtr(pContext));
       LOG(LOG_CTRLF, TEXT("MAP m_afifo_info u4AfifoRPtr= 0x%lx \r\n"), u4AudEsmGetReadPtr(pContext));
       LOG(LOG_CTRLF, TEXT("u4AfifoWPtrForApp=0x%lx\r\n"), pContext->u4AfifoWPtrForApp);
       LOG(LOG_CTRLF, TEXT("AFIFO HW wptr = %lx \r\n"), u4AudDUMPAFIFOGetWPtr((void*)TER_DEC));
       LOG(LOG_CTRLF, TEXT("AFIFO RD rptr = %lx \r\n"), u4AudHalGetBufRPtr(TER_DEC, DSP_AFIFO));
       LOG(LOG_CTRLF, TEXT("SA = %lx, EA = %lx \r\n"),
    ...
    else if ((1 == u4Cmdid) || (2 == u4Cmdid) || (3 == u4Cmdid))
    {
        ...
        LOG(LOG_CTRLF, TEXT("[AudCliCmd] AFIFO Start Addr = 0x%lx \r\n"), u4AudDUMPAFIFOGetFIFOSAdr((void*)u4Cmdid));
        LOG(LOG_CTRLF, TEXT("[AudCliCmd] AFIFO End Addr = 0x%lx \r\n"), u4AudDUMPAFIFOGetFIFOEAdr((void*)u4Cmdid));
        LOG(LOG_CTRLF, TEXT("[AudCliCmd] AFIFO wptr = 0x%lx \r\n"), u4AudDUMPAFIFOGetWPtr((void*)u4Cmdid));
...
void AudCliDumpAout(u32 u4Cmdid,const s8 ** filename)
{
    ...
    else
    {
        ...
        LOG(LOG_CTRLF, TEXT("[AudCliCmd] filename: %s \r\n"), filename);
        LOG(LOG_CTRLF, TEXT("[AudCliCmd] AOUT u4Cmdid = 0x%lx \r\n"), u4Cmdid);
        LOG(LOG_CTRLF, TEXT("[AudCliCmd] AOUT Start Addr = %lx \r\n"), u4AudDUMPAOUTGetFIFOSAdr((void*)u4Cmdid));
        LOG(LOG_CTRLF, TEXT("[AudCliCmd] AOUT End Addr = %lx \r\n"), u4AudDUMPAOUTGetFIFOEAdr((void*)u4Cmdid));
...
static s32 vAudDumpFifoTask(void* pvArg)
{
    ...
    uintptr_t u4SAdr, u4EAdr, u4WPtr, u4CWPtr;
    uintptr_t u4FifoSize, u4Size;
    u32 u4TotalSz = 0;
    AUD_DUMP_FIFO *pDump = &tAudDumpFifo;
    ...
    while(bRunFlag)
    {
        ...
        if(u4CWPtr != u4WPtr)
        {
            ...
            LOG(LOG_CTRLF,TEXT("*****[_AudCliCmd] DUMPFIFO u4WPtr = 0x%lx,u4CWPtr = 0x%x,u4FifoSize = 0x%x,u4Size = 0x%x,u4SAdr = 0x%lx,u4EAdr = 0x%lx\r\n"),
                          (uintptr_t)u4WPtr, (uintptr_t)u4CWPtr, (u32)u4FifoSize, (u32)u4Size, (uintptr_t)u4SAdr, (uintptr_t)u4EAdr);
...
static s32 vAudDumpAoutTask(void* pvArg)
{
    bool i4Ret;
    bool bRunFlag = TRUE;
    uintptr_t u4SAdr[AUD_AOUT_CHNUM], u4EAdr[AUD_AOUT_CHNUM], u4WPtr[AUD_AOUT_CHNUM];
    uintptr_t u4CWPtr[AUD_AOUT_CHNUM];
    uintptr_t u4FifoSize[AUD_AOUT_CHNUM], u4Size[AUD_AOUT_CHNUM];
    u32 u4TotalSz[AUD_AOUT_CHNUM] = {0};
    s32 u4Ind;
    ...
    while(bRunFlag)
    {
        ...
        for(u4Ind = 0; u4Ind <AUD_AOUT_CHNUM; u4Ind++)
        {
            if(u4CWPtr[u4Ind] != u4WPtr[u4Ind])
            {
                ...
                LOG(LOG_CTRLF,TEXT("zjian2*****[_AudCliCmd] DUMPFIFO u4WPtr = 0x%lx,u4CWPtr = 0x%lx,u4FifoSize = 0x%x,u4Size = 0x%x,u4SAdr = 0x%lx,u4EAdr = 0x%lx\r\n"),
                              u4WPtr[u4Ind], u4CWPtr[u4Ind], (u32)u4FifoSize[u4Ind], (u32)u4Size[u4Ind], u4SAdr[u4Ind], u4EAdr[u4Ind]);

                i4Ret = i4DumpFifoFsWriteFile(szFilePath_dump[u4Ind], (const void *)u4WPtr[u4Ind], (u32)u4Size[u4Ind]);
                if(i4Ret)
                {
 ...
 *************************************************************************************************/
static u64 u4AudDUMPAOUTGetFIFOEAdr(void* pUsr)
{
    uintptr_t u4CmdId = (uintptr_t)pUsr;
    u32 u4AoutId =(u4CmdId>>4) & 0x0F;
    u32 u4ChId = u4CmdId & 0x0F;
...
void AudCliDumpAfifo(u32 u4Cmdid,const s8 ** filename)
{
    ...
    if(9 == u4Cmdid)
    {
       LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] MAP m_afifo_info u4AfifoWPtr= 0x%lx \r\n"), (uintptr_t)u4AudEsmGetWritePtr(pContext));
       LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] MAP m_afifo_info u4AfifoRPtr= 0x%lx \r\n"), (uintptr_t)u4AudEsmGetReadPtr(pContext));
       LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] u4AfifoWPtrForApp=0x%lx\r\n"), (uintptr_t)pContext->u4AfifoWPtrForApp);
       LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] AFIFO HW wptr 0x5088= %lx \r\n"), (uintptr_t)u4AudDUMPAFIFOGetWPtr((void*)PRI_DEC));
       LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] AFIFO RD rptr 0x50B0= %lx \r\n"), (uintptr_t)u4AudHalGetBufRPtr(PRI_DEC, DSP_AFIFO));
       return;
    }
    ...
    else if ((1 == u4Cmdid) || (2 == u4Cmdid) || (3 == u4Cmdid))
    {
        ...
        LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] AFIFO Start_Addr = %lx \r\n"), (uintptr_t)u4AudDUMPAFIFOGetFIFOSAdr((void*)u4Cmdid));
        LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] AFIFO End_Addr = %lx \r\n"), (uintptr_t)u4AudDUMPAFIFOGetFIFOEAdr((void*)u4Cmdid));
        LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] AFIFO wptr = %lx \r\n"), (uintptr_t)u4AudDUMPAFIFOGetWPtr((void*)u4Cmdid));
...
void AudCliEsmState(u32 arg1, u32 arg2)
{
    ...
    if(arg2 & 0x1)
    {
        LOG(LOG_CLI, TEXT(">>>u4AfifoSA: 0x%lx\n"), pContext->u4AfifoSA);
        LOG(LOG_CLI, TEXT(">>>u4AfifoEA: 0x%lx\n"), pContext->u4AfifoEA);
    }
    if(arg2 & 0x2)
    {
        LOG(LOG_CLI, TEXT(">>>u4AfifoRPtr: 0x%lx\n"), pContext->u4AfifoRPtr);
        LOG(LOG_CLI, TEXT(">>>u4AfifoWPtr: 0x%lx\n"), pContext->u4AfifoWPtr);
        LOG(LOG_CLI, TEXT(">>>u4AfifoWPtrForApp: 0x%lx\n"), pContext->u4AfifoWPtrForApp);
    }
    ...
    {
        LOG(LOG_CLI, TEXT(">>>u4PTSQRPtr: 0x%lx\n"), pContext->u4PTSQRPtr);
        LOG(LOG_CLI, TEXT(">>>u4PTSQWPtr: 0x%lx\n"), pContext->u4PTSQWPtr);
        LOG(LOG_CLI, TEXT(">>>u4PTSQStrAddr: 0x%lx\n"), pContext->u4PTSQStrAddr);
        LOG(LOG_CLI, TEXT(">>>u4PTSQEndAddr: 0x%lx\n"), pContext->u4PTSQEndAddr);
        
        LOG(LOG_CLI, TEXT(">>>u4IBCQRPtr: 0x%lx\n\n"), pContext->u4IBCQRPtr);
        LOG(LOG_CLI, TEXT(">>>u4IBCQWPtr: 0x%lx\n"), pContext->u4IBCQWPtr);
        LOG(LOG_CLI, TEXT(">>>u4IBCQStrAddr: 0x%lx\n"), pContext->u4IBCQStrAddr);
        LOG(LOG_CLI, TEXT(">>>u4IBCQEndAddr: 0x%lx\n\n"), pContext->u4IBCQEndAddr);
             
        LOG(LOG_CLI, TEXT(">>>eType: 0x%x\n"), pContext->eType);
        LOG(LOG_CLI, TEXT(">>>u4Handle: 0x%x\n\n"), pContext->u4Handle);

        LOG(LOG_CLI, TEXT(">>>u4LastUpdatedRptr: 0x%lx\n"), pContext->u4LastUpdatedRptr);
...
void AudCliDumpDspDram(u32 arg1,u32 arg2,u32 arg3,const s8 ** filename)
{
    ...
    sprintf(szFilePath,"%s%s", "//", (s8*)filename);
...
/************************************************************************************************
 Function Name: AudCliDeconlyDump
 Function Description:Read comm dram value
 input para:arg1: SrcAddr,arg2:Length
 output para:NULL
 Author:fei.zhu
 ...
 Modify Data:
                       2011.3.7     modify format according coding standard
*************************************************************************************************/
#define AUD_CLI_DECONLY_DUMP_STR()    LOG(LOG_CLI, \
    TEXT("[CLI]%d: Deconly Dump Data [flag][filename] \n"), AUD_CLI_DECONLY_DUMP)
static bool fgDeconlyDumpFlag = FALSE;
static u32 u4DeconlyDumpSize = 0;
static u32 u4DeconlyDumpCnt = 0;
static s8 szDeconlyFilePath[64];
void AudCliDeconlyDump(u32 arg1,const s8 ** filename)
{   
    if((ARG_INIT_VAL == arg1)||(filename == NULL)||(*filename == NULL))
    {
        AUD_CLI_DECONLY_DUMP_STR();
        return;
    }

    sprintf(szDeconlyFilePath,"%s%s", "//", (s8*)filename);

    if(arg1 == 0)
    {        
        fgDeconlyDumpFlag = FALSE;
        LOG(LOG_CTRLF, TEXT("*****[_Aud Cli Cmd] Deconly Dump End \r\n"));
    }
    else
    {
        LOG(LOG_CTRLF, TEXT("*****[_Aud Cli Cmd] Deconly Dump Start \r\n"));
        fgDeconlyDumpFlag = TRUE;
    }
...
void vDeconlyDumpDataForDbg(uintptr_t u4Addr, u32 u4Size)
{
    if(!fgDeconlyDumpFlag)
        return;
        
    if(!i4DumpFifoFsWriteFile(szDeconlyFilePath, (const void *)u4Addr, u4Size))
    {
        LOG(LOG_CTRLF, TEXT("*****[_AudCliCmd] Deconly current time write file failed! *****\r\n"));
    } 

    u4DeconlyDumpSize += u4Size;

    if((u4DeconlyDumpSize>>20) > u4DeconlyDumpCnt)
    {
        LOG(LOG_CTRLF, TEXT("*****Deconly dump size: %d MB *****\r\n"), (u4DeconlyDumpSize>>20));
        u4DeconlyDumpCnt++;
    }
}
...
void AudCliShowAudWorkingBuf(u32 arg1,u32 arg2)
{
    if(0 == arg1)
    {
        uintptr_t u4BufTmp = (uintptr_t)g_ucAdspWorkingBuffer;
...
void  AudCliCmdUopSRCVolume(u32 arg1,u32 arg2)
{ 
    if(ARG_INIT_VAL == arg1 || ARG_INIT_VAL == arg2)
    {
        AUD_CLI_MODIFY_VOL_STR();
    ...
    {
        AUD_SRC_VOL_CTL sAudioVolume;

        sAudioVolume.eMediaSrc = (AUD_MEDIA_SOURCE_TYPE_T)arg1%(AUD_MEDIA_SOURCE_DVP + 1);
        sAudioVolume.eMediaOut = (AUD_MEDIA_OUT_TYPE_T)arg1/(AUD_MEDIA_SOURCE_DVP + 1);
        sAudioVolume.u4Vol = arg2;
        AudSetSrcVolGain(&sAudioVolume);
    }
}
...
void AudCli_MemorySvc(u32 u4Type, u32 arg1, u32 arg2, u32 arg3, const s8 **pfilename)
{
    ...
    case AUD_CLI_AUD_WORKINGBUF:
        ...
        break;
    case AUD_CLI_DECONLY_DUMP:
        AudCliDeconlyDump(arg1,pfilename);
        break;
...
void  AudSetCliCmd(AUD_DEC_CLI_TYPE eAudCli,u32 arg1,u32 arg2,u32 arg3,u32 arg4,const s8 **pfilename)
{
    ...
    case AUD_DEC_CLI_V_SRC:
        AudCliCmdUopSRCVolume(arg1,arg2);
        break;
