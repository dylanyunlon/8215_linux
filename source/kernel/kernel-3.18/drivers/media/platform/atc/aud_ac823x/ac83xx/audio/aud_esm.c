...
static struct semaphore g_rAUListSema[(TER_DEC + 1)];
...
static uintptr_t g_u4SkipDataAddr = 0xFFFFFFFF;
...
static uintptr_t _hAudEsmEvntGrp;
...
static void AudEsmSetPtsToQueue(u8 u1DecId,uintptr_t u4PtsAddr, u32 u4PtsVal);
...
*   Return    :
****************************************************************************/
uintptr_t u4AudEsmGetWritePtr(AUD_ESM_CONTEXT_T *pContext)
{
    //update AFIFO Read Pointer to ESM
    uintptr_t u4CurWritePtr = pContext->u4AfifoSA + m_aud_afifo_info->posInfo[(u8)pContext->u2AudDrvCompId].ptrAfifoWPtr;
...
*   Return    :
****************************************************************************/
uintptr_t u4AudEsmGetReadPtr(AUD_ESM_CONTEXT_T *pContext)
{
    //update AFIFO Read Pointer to ESM
    uintptr_t u4CurReadPtr = pContext->u4AfifoSA + m_aud_afifo_info->posInfo[(u8)pContext->u2AudDrvCompId].ptrAfifoRPtr;
...
static s32 i4AudEsm_DealDataAU(u8 u1DecId,AUD_ESM_CONTEXT_T *pContext, AU_AUDIO *pAu)
{
    ...
    else
    {
        ...
        LOG(LOG_FAIL, TEXT("i4AudEsm_WriteAU, pAu->ptrSAddr:0x%lx, pAu->ptrEAddr:0x%lx\n"),
            pAu->ptrSAddr, pAu->ptrEAddr);
        LOG(LOG_FAIL, TEXT("i4AudEsm_WriteAU, pContext->u4AfifoSA:0x%lx, pContext->u4AfifoEA:0x%lx\n"), 
...
static void AudEsm_GetLin1Au(void)
{
    ...
    uintptr_t u4Wptr = Aud_Linein_GetWPtr();
    if (u4Wptr != 0) 
    {
...
s32 i4AudEsm_TaskCreate(void)
{
    ...
    for(u1Ind = PRI_DEC; u1Ind <=TER_DEC; u1Ind++)
    {
        ...
        sema_init(&g_rAUListSema[u1Ind], 1);
    }
    ...
    //g_hAudSetCmd[PRI_DEC] = CreateEvent(NULL, FALSE, FALSE, TEXT("AUDSETCMD"));
    //g_hAudSetCmd[SEC_DEC] = CreateEvent(NULL, FALSE, FALSE, TEXT("AUDDEC2SETCMD"));
    //g_hAudSetCmd[TER_DEC] = CreateEvent(NULL, FALSE, FALSE, TEXT("AUDDEC3SETCMD"));
    return AUD_OK;
}
...
{
    s32 i4Ret = AUD_OK;
    uintptr_t u4Start = MT3360_AFIFO_VA;
    uintptr_t u4End = MT3360_AFIFO_VA + AUD_AFIFO_TOTAL_SIZE;
    ...
    LOG(LOG_DATAF, TEXT("Set AFifo SA = 0x%lX, EA = 0x%lX, for decoder [%d]\n"),
...
#ifndef __linux__
    pContext = &g_rAudEsmContext[PRI_DEC];
    if (NULL == pContext->hMap)
    {
        pContext->hMap = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE | SEC_COMMIT,
            0, sizeof(AFIFO_POSINFO_T), L"AFIFO0");
        if (pContext->hMap != NULL)
        {
            m_aud_afifo_info = (AFIFO_POSINFO_T *)MapViewOfFile(pContext->hMap, FILE_MAP_ALL_ACCESS, 0, 0, 0);
            if(NULL == m_aud_afifo_info)
            {
                CloseHandle(pContext->hMap);
                pContext->hMap = NULL;
                LOG(LOG_FAIL, TEXT("********aud_esm map view fail*******\n"));
                return (-1);
            }
        }
    }
    InitializeCriticalSection(&esm_send_au_lock);
#else
...
static void vAudEsm_StopInit(u16 u2DecId)
{
    ...
    LOG(LOG_FEATURE, TEXT("Reset dec[%d] afifo write pointer 0x%lx.\n"), u2DecId, u4Write);
...
bool AudEsm_SendBufferInfo(u8 u1DecId, AUD_SEND_BUF_INFO *pBufInfo, bool fgUserSpace)
{
    AUD_ESM_CONTEXT_T *pEsmContext = &g_rAudEsmContext[u1DecId];
    AU_AUDIO au;
    uintptr_t u4Tmp;
    uintptr_t u4WPtr = pEsmContext->u4AfifoWPtrForApp;
    if (0 == pBufInfo->ptrBufAddr || 0 == pBufInfo->u4BufLen
        || (u4AudEsm_GetSpareBufLen(pEsmContext) <= pBufInfo->u4BufLen))
    {
       // LOG(LOG_FAIL, TEXT("AudEsm_SendBufferInfo fail, BufAddr 0x%x, BufLen 0x%x, AFIFOSpareBufLen 0x%x\n"),
          //pBufInfo->ptrBufAddr, pBufInfo->u4BufLen, u4AudEsm_GetSpareBufLen(pEsmContext));
        //LOG(LOG_FAIL, _T("AudEsm_SendBufferInfo fail.\r\n"));
        return FALSE;
    }

    au.eAuType = AU_DATA;

    if(TRUE == fgUserSpace) //user space memory need access
    { 
        if (!access_ok(VERIFY_READ, (void __user *)pBufInfo->ptrBufAddr, pBufInfo->u4BufLen))
        {
            LOG(LOG_FAIL, _T("SendBufferInfo access_ok fail.\r\n"));
            return FALSE;
        }
    }
    ...
    {
        int u4Idx;
        memcpy_toio((void *)u4WPtr, (void *)pBufInfo->ptrBufAddr, pBufInfo->u4BufLen);
    ...
    { 
        ...
        memcpy_toio((void *)u4WPtr, (void *)pBufInfo->ptrBufAddr, u4Tmp);
        pBufInfo->u4BufLen -= u4Tmp;
        if (pBufInfo->u4BufLen)
            memcpy_toio((void *)pEsmContext->u4AfifoSA, (u8 *)pBufInfo->ptrBufAddr + u4Tmp, pBufInfo->u4BufLen);
...
{
    uintptr_t u4SA, u4EA, u4AfifoSA, u4AfifoEA;
...
uintptr_t u4AudEsm_GetSpareBufLen(AUD_ESM_CONTEXT_T *pContext)
{
    uintptr_t u4Ret;
...
s32 i4AudEsm_SendBuffer(u8 u1DecId,void *buf, u32 u4Size)
{
    AUD_ESM_CONTEXT_T *pContext = &g_rAudEsmContext[u1DecId];
    AU_AUDIO au;
    uintptr_t u4WPtr = pContext->u4AfifoWPtrForApp;
    ...
    u4WPtr = (uintptr_t)buf;
    if (u4Size <= pContext->u4AfifoEA - u4WPtr)
    {
        au.ptrSAddr = (uintptr_t)buf - pContext->u4AfifoSA;
        au.ptrEAddr = u4Size + au.ptrSAddr;
    }
    else
    {
        au.ptrSAddr = (uintptr_t)buf - pContext->u4AfifoSA;
        au.ptrEAddr = u4Size + (uintptr_t)buf - pContext->u4AfifoEA;
    }
...
#ifndef __linux__
    void *  hMultiDev = INVALID_HANDLE_VALUE;
    hMultiDev = (void *)filp_open(L"MUT1:", O_RDWR | O_CREAT, 0);
    if (INVALID_HANDLE_VALUE != hMultiDev)
    {
       if (!DeviceIoControl(hMultiDev, IOCTL_MISCDEV_HWCLOCK_GETTIME, 
                         NULL, 0, (void*)&u8Time, sizeof(u64), NULL, NULL))
       {
           LOG(LOG_FAIL, TEXT("DeviceIoControl, IOCTL_MISCDEV_HWCLOCK_GETTIME Failed.\n"));
       }

    }
    CloseHandle(hMultiDev);
    hMultiDev = INVALID_HANDLE_VALUE;
#else
...
static void AudEsmSetPtsToQueue(u8 u1DecId,uintptr_t u4PtsAddr, u32 u4PtsVal)
{
    uintptr_t u4FifoBase = u4AudHalGetAFIFOBaseAddr(u1DecId);//b1000000
