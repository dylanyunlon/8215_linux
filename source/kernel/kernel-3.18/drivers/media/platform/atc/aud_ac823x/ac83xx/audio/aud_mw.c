...
void AudSetSrcVolGain(AUD_SRC_VOL_CTL * prSrcVol)
{
    ...
    if (g_rSrcVolume[rAudSrc][rAudOutType] != prSrcVol->u4Vol)
    {
    ...
    {
        LOG(LOG_FAIL, TEXT("g_rSrcVolume[%d][%d] equal to prSrcVol->u4Vol, the value is %x!\r\n"),
...
void AudSetDecInfo(u8 u1DecId, AUD_DEC_AUD_INFO_T * pv_set_info)
{
    ...
    {
        LOG(LOG_CTRLF, _T("SEC_DEC Set Decoder Info.\n"));
