...
bool AudSmixSetMwCtrl(AUD_MEDIA_TYPE* pType)
{
    ...
    LOG(LOG_INFO, _T("%s command type(%d), Source(%d), Output(%d).\r\n"),
    ...
    if((AUD_SMIX_STARTED == eState&& AUD_MEDIA_ON == pType->eMediaCtrl)||
        ...
        LOG(LOG_INFO, _T("%s state is %d, ctrl cmd is %d.\r\n"),__func__, eState, pType->eMediaCtrl);
        up(&pContext->Cmdlock);
        return TRUE;
    }
...
static inline s32 AudSmixDataCpy(uintptr_t pDst, u16* pSrc, u32 u4Len)
{
    u32 u4Data    = 0;
    u32 u4SampIdx = 0;
    u32 u4SrcIdx  = 0;
    uintptr_t pDst_l    = pDst;
    uintptr_t pDst_r    = pDst + RISC_SW_MIX_CH_SIZE;
    ...
    for(u4SampIdx = 0; u4SampIdx < u4Len; u4SampIdx++){
        ...
        *((volatile u32 *)(g_u4DspDramBuf[6]+(pDst_l<<2))) = u4Data;
        __flush_dcache_area(g_u4DspDramBuf[6] + (pDst_l<<2), u4Len * 2);
        pDst_l++;
        ...
        *((volatile u32 *)(g_u4DspDramBuf[6]+(pDst_r<<2))) = u4Data;
        __flush_dcache_area(g_u4DspDramBuf[6] + (pDst_r<<2), u4Len * 2);
        pDst_r++;
    }
