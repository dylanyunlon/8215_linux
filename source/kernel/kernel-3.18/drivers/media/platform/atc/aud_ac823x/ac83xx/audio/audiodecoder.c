...
bool ADE_Deinit(uintptr_t dwContext)
{
...
uintptr_t ADE_Open(uintptr_t dwContext, u32 dwAccessMode, u32 dwShareMode)
{
    ...
    return (uintptr_t)pContext;
}

bool ADE_Close(uintptr_t dwContext)
{
...
{
    LOG(LOG_CTRLF, TEXT("adec_vma_open, virt 0x%lx, phys 0x%lx.\n"),
                    (uintptr_t)vma->vm_start, (uintptr_t)(vma->vm_pgoff << PAGE_SHIFT));
}
...
s32 ADE_Mmap(uintptr_t dwContext, struct vm_area_struct *vma)
{
    uintptr_t length = 0;
...
u32 ADE_Read(uintptr_t context, void *pBuffer, u32 dwCount)
{
    AUDIO_BUF_INFO info;
    s32 i4Ret;
...
u32 ADE_Write(uintptr_t context, void *pBuffer, u32 dwCount)
{
    AU_AUDIO au;
    AUD_DRV_CONTEXT *pContext = (AUD_DRV_CONTEXT *)context;
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIO_GET_SRC_VOLUME:
        ...
        if (NULL != pInBuffer && NULL != pOutBuffer)
        {
			...
			LOG(LOG_FEATURE, TEXT("AudGetSrcVolGain  volume = 0x%x\n"), *(u32*)pOutBuffer);
        }
        else
        {
            LOG(LOG_FAIL, TEXT(" *** get src volume parameter error *****\n"));
            fgRet = FALSE;
        }
        break;
    ...
    else
    {
        ...
        case IOCTL_AUDIO_GET_CURRENTPTS:
            if(pOutBuffer != NULL)
            {
                ...
                LOG(LOG_DECINFO, TEXT("***********IOCTL_AUDIO_GET_CURRENTPTS,u8DecReadyPTS=%lx,u8TargetPTS=%lx***********\n"),
                    (u64)pAudSyncInfo->u8DecReadyPTS, (u64)pAudSyncInfo->u8TargetPTS);
            }
            else
            {
        ...
        case IOCTL_AUDIO_SET_AUDIO_DEC_INFO:
            ...
            if(pInBuffer != NULL)
            {
                ...
                AudCfg_RestoreAoutRegs();
                if (prPbInfo->eAudDecFmt == AUD_DEC_FMT_AC3 || prPbInfo->eAudDecFmt == AUD_DEC_FMT_DTS)
                {
                    prPbInfo->eAudDecFmt = AUD_DEC_FMT_PCM;
                    prPbInfo->prInfo->e_aud_fmt = AUD_DEC_FMT_PCM;
                    AudSetDecPlayBackInfo(pContext, prPbInfo);
                    fgRet = false;
                    LOG(LOG_CTRLF, TEXT("AC3/DTS unsupportted, PCM format will be set\n"));
                    break;
                }
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIO_SEND_BUFFER:
        ...
        else
        {
            ...
            #endif
            {
                fgRet = AudEsm_SendBufferInfo(pContext->u1DecId, (AUD_SEND_BUF_INFO *)pInBuffer, TRUE);
                if (FALSE == fgRet) {
    ...
    case IOCTL_AUDIO_SEND_BUFFER_KERNEL:
        ...
        else
        {
            ...
            #endif
            {
                fgRet = AudEsm_SendBufferInfo(pContext->u1DecId, (AUD_SEND_BUF_INFO *)pInBuffer, FALSE);
                if (FALSE == fgRet)
                    u4ErrType = AUD_ADSP_BUF_FULL;
            }
        }
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIO_SET_MEDIA_TYPE:
        ...
        if (NULL != pInBuffer)
        {
            ...
            {
                LOG(LOG_CTRLF, TEXT("Set audio media type error\n"));
                fgRet = FALSE;
            }
        }
        else
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIN_COPY_FROM_USER:
        ...
        if (NULL != pInBuffer && inSize == sizeof(AUD_USER_INFO))
        {
            ...
            LOG(LOG_CTRLF, TEXT("pContext->u4AfifoWPtr 0x%lx, pContext->u4AfifoSA 0x%lx, prUser_Info->puser 0x%x, size 0x%x\n"),
                (uintptr_t)pESMContext->u4AfifoWPtr, (uintptr_t)pESMContext->u4AfifoSA, (u32)prUser_Info->puser, (u32)prUser_Info->buf_size);
...
bool ADE_IOControl(uintptr_t context, u32 code, u8 *pInBuffer, u32 inSize,
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    bool fgRet = TRUE;
    AUD_DRV_CONTEXT *pContext = (AUD_DRV_CONTEXT *)context;
