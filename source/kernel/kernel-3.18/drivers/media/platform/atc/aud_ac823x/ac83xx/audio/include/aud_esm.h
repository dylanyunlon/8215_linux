...
typedef struct _AUD_ESM_CONTEXT_T
{
    ...
    uintptr_t u4LastUpdatedRptr;       // last Rptr updated to ESM
    u32 u4LastIteratedAUIdx;     // last AU index updated to DSP
    u32 u4Read;

    uintptr_t u4AfifoSA,u4AfifoEA;
    uintptr_t u4AfifoRPtr, u4AfifoWPtr, u4AfifoWPtrForApp;  //got from dsp/dspctrl, reset when Stop and Init

    uintptr_t u4PTSQRPtr, u4IBCQRPtr;          //got from dsp/dspctrl, reset when Stop and Init
    uintptr_t u4PTSQWPtr, u4IBCQWPtr;          //got from ESM
    uintptr_t u4PTSQStrAddr, u4IBCQStrAddr;    //from dspctrl after memory partitioned
    uintptr_t u4PTSQEndAddr, u4IBCQEndAddr;    //from dspctrl after memory partitioned
...
extern uintptr_t u4AudEsmGetWritePtr(AUD_ESM_CONTEXT_T *pContext);
extern uintptr_t u4AudEsmGetReadPtr(AUD_ESM_CONTEXT_T *pContext);
...
extern bool  AudEsm_SendBufferInfo(u8 u1DecId, AUD_SEND_BUF_INFO *pBufInfo, bool fgUserSpace);
...
extern uintptr_t u4AudEsm_GetSpareBufLen(AUD_ESM_CONTEXT_T *pContext);
