...
void  AudmhlBufInit(void)
{
    ...
    VERIFY(((g_rAudmhlInBuf.u4Buf_EA -g_rAudmhlInBuf.u4Buf_SA)%(AUDIO_IN_INT_PERIOD*HDMI_INT_TIME_SLOT))==0);
    // Set Multiple Line In buffer  , Start and End Address MUST BE block address.
    VERIFY((g_rAudmhlInBuf.u4Buf_SA & 0xFF)==0);
    VERIFY((g_rAudmhlInBuf.u4Buf_EA & 0xFF)==0);
    // SPDIF In Buffer MUST BE  times of parser's slot.
    VERIFY(((g_rAudmhlInBuf.u4Buf_EA -g_rAudmhlInBuf.u4Buf_SA)%(AUDIO_IN_INT_PERIOD*INT_TIME_SLOT))==0);
    // SPDIF Buffer MUST BE times of stereo PCM (2*3 Bytes) to avoid buffer write through error.
    VERIFY(((g_rAudmhlInBuf.u4Buf_EA -g_rAudmhlInBuf.u4Buf_SA)%6)==0);
...
Return      : 
************************************************************************/
uintptr_t AudmhlGetAudInBufSA(void)
{
...
Return      : 
************************************************************************/
uintptr_t AudmhlGetAudInBufEA(void)
{
...
Return      : 
************************************************************************/
uintptr_t AudmhlGetAudInBufWP(void)
{
...
Return      : 
************************************************************************/
uintptr_t AudmhlGetAudInBufRP(void)
{
...
Return      : 
************************************************************************/
uintptr_t AudmhlBufDataSize(void)
{
...
void AudmhlUpAudInBufWPtrByMLinHW(void)
{
    ...
    if(g_rAudmhlInBuf.u4Buf_W > g_rAudmhlInBuf.u4Buf_EA)
    {
        LOG(LOG_MHL, _T("[Overflow] g_rAudmhlInBuf.u4Buf_W = 0x%lx. \n"), g_rAudmhlInBuf.u4Buf_W);
        LOG(LOG_MHL, _T("[Overflow] g_rAudmhlInBuf.u4Buf_SA = 0x%lx. \n"), g_rAudmhlInBuf.u4Buf_SA);
        LOG(LOG_MHL, _T("[Overflow] g_rAudmhlInBuf.u4Buf_EA = 0x%lx. \n"), g_rAudmhlInBuf.u4Buf_EA);
        g_rAudmhlInBuf.u4Buf_W = g_rAudmhlInBuf.u4Buf_SA;
    }
...
{    
    VERIFY(AudmhlGetAudInType()!= AUDIN_NONE);
    ...
    LOG(LOG_MHL, _T("[AUDIN_MULTI] g_rMlinParams.u4BufPhyAdr = 0x%lx. \n"), g_rMlinParams.u4BufPhyAdr);
...
s32 AudmhlGetAudInSBuf(SEND_BUFFER *prSendBuffer)
{
    ...
    if (AudmhlBufDataSize() > u4SlotSize)
    {
        if((u4MhlSzjCnt % 99) == 0)
        {
             //LOG(LOG_MHL, _T("RdPt:%x, WtPt:%lx, ID:0x%lx, ofstt:%lx \n"),
             // g_rAudmhlInBuf.u4Buf_R,g_rAudmhlInBuf.u4Buf_W, g_u4PsrSessionID, (u64)g_u8PSRSrcOffset);
        }
        u4MhlSzjCnt++;
