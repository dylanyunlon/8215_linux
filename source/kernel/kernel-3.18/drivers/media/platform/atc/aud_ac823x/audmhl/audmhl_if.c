...
s32 AudmhlInit(const AUDMHL_SET_T *prAudinSetInfo)
{
    ...
    if (g_fgAudmhlInited == FALSE)
    {
        ...
        VERIFY((i4Ret = AudmhlDRVInit(prAudinSetInfo->eAudmhlInType)) > 0);
...
void AudmhlUnInit(AUDMHL_IN_TYPE u1AudinType)
{
    ...
    if (g_fgAudmhlInited == TRUE)
    {
        ...
        VERIFY((i4Ret = AudmhlDRVUnInit(u1AudinType)) >= 0);

        if (g_hAudmhlThread)
        {
            //CloseHandle((void *)g_hAudmhlThread);
            //kthread_stop(g_hAudmhlThread);
        ...
        #ifndef __linux__
        if(NULL != g_prBufPointer)
        {
            UnmapViewOfFile(g_prBufPointer);
            g_prBufPointer = NULL;
        }
        if(NULL != g_hMapFile)
        {
...
/************************************************************************
Function    : s32 AudmhlCreateMsgQ(uintptr_t* pMsgHandle, const s8* pName,
                        size_t zMsgSize, u16 u4MsgCount)
Description : create msg queue
Parameter   : None
Return      : None
**********************************************************************/
s32 AudmhlCreateMsgQ(uintptr_t* pMsgHandle, const s8* pName,
                        size_t zMsgSize, u16 u4MsgCount, bool fgRead)
{
    ...
    *pMsgHandle = (uintptr_t)hTmpQueue;
...
s32 AudmhlDeleteMsgQ(uintptr_t* MsgHandle)
{
