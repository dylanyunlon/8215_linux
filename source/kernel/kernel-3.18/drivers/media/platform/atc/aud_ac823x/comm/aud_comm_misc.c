...
void AudMisc_BufferData_Read(uintptr_t u4SAdr, u32 u4Size)
{  
    u32 i;
    u8 *pbBuf;
    pbBuf = (u8 *)u4SAdr;

    AUDLOG_NO_PREFIX(1, (T("[Read Buffer Data] SAdr(0x%lx) Size(%d)>>>>>>>>>>>>>>> \r\n"),  (uintptr_t)u4SAdr, (s32)u4Size));
    for (i = 0; i < u4Size; i++)
    {
        if(i % 16 == 0) {
            AUDLOG_NO_PREFIX(1, (T("\n0x%lX:  "), (uintptr_t)pbBuf));
        } 
        AUDLOG_NO_PREFIX(1, (T("%2X "), *pbBuf));  
        pbBuf++;
    }
...
void AudMisc_BufferData_Draw(uintptr_t u4SAdr, u32 u4Size, u32 u4BW, u32 u4CutBit, u32 u4CutSpace)
{  
    u32 i, j;
    u8 *pbBuf = (u8 *)u4SAdr;
    ...
    AUDLOG_NO_PREFIX(1, (T("[Draw Buffer Data] SAdr(0x%lx) Size(%d)>>>>>>>>>>>>>>> \r\n"),  (uintptr_t)u4SAdr, (s32)u4Size));
    for (i = 0; i < u4Size; i += u4ByteNum)
    {
        ...
        AUDLOG_NO_PREFIX(1, (T("0x%8x"), (u32)u4Value));      
        while(u4SpaceLoop--)
        {      
            AUDLOG_NO_PREFIX(1, (T(" ")));
        }
