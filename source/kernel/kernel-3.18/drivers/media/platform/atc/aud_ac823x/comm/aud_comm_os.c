...
void AudOS_IRQ_Clear(u32 u4Vect)
{
    ...
    BIM_ClearIrq(u4Vect);
#else
    mt33xx_mask_ack_bim_irq(u4Vect); 
#endif 
}
...
bool AudOS_ISR_UnReg(u32 u4Vect)
{
    ...
    {
        fgRet = FALSE;
        COMMLOG_ERR((T("Failed to UnRegister IRQ(%d).\r\n"), (s32)u4Vect));
    }
    #else   
...
{
    uintptr_t u4VirAddr = 0;
    #ifndef __linux__
    u4VirAddr = (uintptr_t)AllocPhysMem(u4Size, PAGE_READWRITE, u4Alignment, 0, (uintptr_t *)(pu4PhyAddr));
    #else
    //u4VirAddr = (uintptr_t)x_mem_alloc_ret_phy_addr(u4Size, pu4PhyAddr);
    u4VirAddr = (uintptr_t)kmalloc(u4Size, GFP_KERNEL);
...
void AudOS_BkBuf_Free(PAUD_BACKUP_BUF_T prBkBuf)
{
    ...
    {
        COMMLOG_INFO((T("Free BkBuf (0x%lx, 0x%lx, 0x%x) \n"),
                (uintptr_t)(prBkBuf->u4VirAddr), (uintptr_t)(prBkBuf->u4PhyAddr), (u32)(prBkBuf->u4MemSize)));
...
void AudOS_BkBuf_Alloc(PAUD_BACKUP_BUF_T prBkBuf, u32 u4Size)
{
    if (u4Size) 
    {
        ...
        COMMLOG_INFO((T("Alloc BkBuf: BkBuf(0x%lx, 0x%lx, 0x%lx) \n"),
            (uintptr_t)(prBkBuf->u4VirAddr), (uintptr_t)(prBkBuf->u4PhyAddr), (u32)(prBkBuf->u4MemSize)));
    }   
}
...
{
    COMMLOG_INFO((T("Reset BkBuf: (0x%lx, 0x%lx, 0x%lx) \n"),
            (uintptr_t)(prBkBuf->u4VirAddr), (uintptr_t)(prBkBuf->u4PhyAddr), (u32)(prBkBuf->u4MemSize)));
    prBkBuf->u4VirAddr = 0;   
}
...
uintptr_t AudOS_BkBuf_Request(PAUD_BACKUP_BUF_T prBkBuf, u32 u4Size)
{
    ...
    if (prBkBuf->u4UsedSize + u4Size > prBkBuf->u4MemSize) {
        COMMLOG_INFO((T("No free backup memory!!! (0x%lx + 0x%lx > 0x%x)\n"),
