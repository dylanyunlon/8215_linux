...
extern uintptr_t g_u4Dec1Afifo[]; //0:start,1:end
extern uintptr_t g_u4Dec4Afifo[];
extern uintptr_t g_u4Dec5Afifo[];
extern u8 *g_ucAdspWorkingBuffer;

// 1MB alignment Bank Address
#define AFIFOBKADDR     (AFIFO_PHYSICAL(DspGetAfifoSA()) & 0xFFF00000)

static uintptr_t s_u4AFIFOBaseAddr = 0;

uintptr_t DspGetAfifoSA(void)
{
   return g_u4Dec1Afifo[0];
}


uintptr_t u4AudHalGetAFIFOBankForInit(void)
{
    uintptr_t u4MinAddr = DspGetAfifoSA();
    s_u4AFIFOBaseAddr = u4MinAddr;
	
    return ((AFIFO_PHYSICAL(u4MinAddr) & 0xFFF00000) >> 20);
}
...
/***************************************************************************
    ...
    Return    :
***************************************************************************/
void vAudHalSetBufStartAddr(u8 u1DecId, u8 u1BufType, uintptr_t u4Addr)
{
...
/***************************************************************************
    ...
    Return    :
***************************************************************************/
void vAudHalSetBufEndAddr(u8 u1DecId, u8 u1BufType, uintptr_t u4Addr)
{
	u32 u4RegisterTemp = 0;
...
/***************************************************************************
    ...
    Return    :
***************************************************************************/
void vAudHalSetBufWPtr(u8 u1DecId, u8 u1BufType, uintptr_t u4WPtrVal)
{
    if (u1DecId == PRI_DEC)
    {
        //Primary Audio
        if(DSP_AFIFO == u1BufType)
        {
            ...
            {
                LOG(LOG_FAIL, TEXT("Pri wptr out of range: wptr = 0x%lx,SA = 0x%lx, EA = 0x%lx.\n"),
    ...
    else if (SEC_DEC == u1DecId)
    {
        if(DSP_AFIFO == u1BufType)
        {
            ...
            {
                LOG(LOG_FAIL, TEXT("2nd wptr out of range: wptr = 0x%lx,SA = 0x%lx, EA = 0x%lx.\n"),
    ...
    { 
        if(DSP_AFIFO == u1BufType)
        {
            ...
            {
                LOG(LOG_DATAF, TEXT("3rd wptr out of range: wptr = 0x%lx,SA = 0x%lx, EA = 0x%lx.\n"),
...
/***************************************************************************
    ...
    Return    :
***************************************************************************/
uintptr_t  u4AudHalGetBufRPtr(u8 u1DecId, u8 u1BufType)
{
    uintptr_t u4BufRPtr = 0;
    if (u1DecId == PRI_DEC)
    {
        if (DSP_AFIFO == u1BufType)
        {   //Read pointer
...
uintptr_t u4AudHalGetAFIFOWPtr(u32 u4DecId)
{
    ...
    {
    case PRI_DEC:
        u4WPtr = (uintptr_t)AUDREG_READ(REGENV_SW_BS0_PPNT);
        u4WPtr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;

    case SEC_DEC:
        u4WPtr = (uintptr_t)AUDREG_READ(REGENV_SW_BS1_PPNT);
        u4WPtr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;

    case TER_DEC:
        u4WPtr = (uintptr_t)AUDREG_READ(REGENV_SW_BS2_PPNT);
        u4WPtr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;
...
uintptr_t u4AudHalGetAFIFOStartAddr(u32 u4DecId)
{
    ...
    {
    case PRI_DEC:
        u4SAddr = (uintptr_t)AUDREG_READ(REGENV_SW_BS0_SBLK) & 0x00FFFFFF;
        u4SAddr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;

    case SEC_DEC:
        u4SAddr = (uintptr_t)AUDREG_READ(REGENV_SW_BS1_SBLK) & 0x00FFFFFF;
        u4SAddr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;

    case TER_DEC:
        u4SAddr = (uintptr_t)AUDREG_READ(REGENV_SW_BS2_SBLK) & 0x00FFFFFF;
        u4SAddr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;
...
uintptr_t u4AudHalGetAFIFOEndAddr(u32 u4DecId)
{
    ...
    {
    case PRI_DEC:
        u4EAddr = (uintptr_t)AUDREG_READ(REGENV_SW_BS0_EBLK) & 0x00FFFFFF;
        u4EAddr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;
                
    case SEC_DEC:
        u4EAddr = (uintptr_t)AUDREG_READ(REGENV_SW_BS1_EBLK) & 0x00FFFFFF;
        u4EAddr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;
    case TER_DEC:
        u4EAddr = (uintptr_t)AUDREG_READ(REGENV_SW_BS2_EBLK) & 0x00FFFFFF;
        u4EAddr += u4AudHalGetAFIFOBaseAddr(u4DecId);
        break;
