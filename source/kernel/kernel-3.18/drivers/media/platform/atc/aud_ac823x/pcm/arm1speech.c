...
#define SPH_MSG_QUEUE_ITEM_SIZE	32//4*sizeof(uintptr_t)//16U


static struct semaphore  g_rSpeechSema;

static uintptr_t g_hAECMsgQ;
static uintptr_t g_hBTMsgQ;
...
static void SpeechHandleMsg(uintptr_t u4MsgID, uintptr_t u4P1, uintptr_t u4P2, uintptr_t u4P3)
{
...
static s32 AEC_IST(void *data)
{
	...
	void* ptr_tmp = kzalloc(SPH_MSG_QUEUE_ITEM_SIZE, GFP_KERNEL);

	pr_debug("[PCM D]AEC_IST: start\r\n");
	while (!fgISTExit) {
		s32  i4_res = 0;
		u16 ui2_index = 0;
		u32 z_msg_size = SPH_MSG_QUEUE_ITEM_SIZE;
		uintptr_t *phHandle = (uintptr_t *)&g_hAECMsgQ;
		u32 u4Param[4] = {0};
		memset(ptr_tmp,0,SPH_MSG_QUEUE_ITEM_SIZE);
		if (phHandle == NULL) {
			pr_err("[PCM ERR]AEC_IST_tasklet_process: g_hAECMsgQ is NULL\r\n");
			return false;
		}
		i4_res = x_msg_q_receive(&ui2_index, (void*)ptr_tmp, &z_msg_size, phHandle, 1U, X_MSGQ_OPTION_WAIT);
		if (i4_res == (s32)OSR_OK) {
			if (SPH_MSG_QUEUE_ITEM_SIZE != z_msg_size) {
				pr_err("[PCM ERR]AEC_IST: Read AEC Msg size(%d)\r\n", z_msg_size);
			}
			//SpeechHandleMsg(u4Param[0], u4Param[1], u4Param[2], u4Param[3]);
			//pr_debug("QK u4Param[0]:0x%x,u4Param[0]:%d\n",*((uintptr_t*)ptr_tmp),*(((uintptr_t*)ptr_tmp)+1));
			SpeechHandleMsg(*((uintptr_t*)ptr_tmp), *(((uintptr_t*)ptr_tmp)+1), *(((uintptr_t*)ptr_tmp)+2), *(((uintptr_t*)ptr_tmp)+3));
		} else {
			pr_err("[PCM ERR]AEC_IST: Read AEC Message Failure %d\r\n", (s32)i4_res);
		}
	}
	pr_debug("[PCM D]AEC_IST: AEC IST Exit. fgISTExit(%d)\r\n", (s32)fgISTExit);
...
static bool AEC_IST_Init(void)
{
	if (x_msg_q_create(&g_hAECMsgQ, (const char *)AEC_MSGQ_NAME,
		...
		pr_err("[PCM ERR]AEC_IST_Init: Failed to create AEC msg queue\r\n");
		return false;
	}

	g_hAECIST = kthread_create(AEC_IST, (void *)NULL, "AEC_IST");
	if (IS_ERR(g_hAECIST)) {
		pr_err("[PCM ERR]AEC_IST_Init: Create AEC_IST_Thread ERR %ld!\r\n", PTR_ERR(g_hAECIST));
		g_hAECIST = NULL;
		return false;
	}
...
static s32 ARM2_EMU_Thread(void *data)
{
	...
	while (true) {
		...
		u16 ui2_index = 0;
		u32 z_msg_size = SPH_MSG_QUEUE_ITEM_SIZE;
		uintptr_t *phHandle = (uintptr_t *)&g_hBTMsgQ;
		uintptr_t u4Param[4] = {0};
		memset(ptr_tmp,0,SPH_MSG_QUEUE_ITEM_SIZE);
		if (phHandle == NULL) {
			pr_err("[PCM ERR]ARM2_EMU_Thread: _hBTMsgQ is NULL!\r\n");
			return false;
		}
		i4_res = x_msg_q_receive(&ui2_index, (void*)ptr_tmp, &z_msg_size, phHandle, 1U, X_MSGQ_OPTION_WAIT);
		if (i4_res == (s32)OSR_OK) {
			if (SPH_MSG_QUEUE_ITEM_SIZE != z_msg_size) {
				pr_err("[PCM ERR]ARM2_EMU_Thread: Read BT Msg size(%d)\r\n", z_msg_size);
			}
			SpeechCB(*((uintptr_t*)ptr_tmp), *(((uintptr_t*)ptr_tmp)+1), *(((uintptr_t*)ptr_tmp)+2), *(((uintptr_t*)ptr_tmp)+3));
			//SpeechCB(u4Param[0], u4Param[1], u4Param[2], u4Param[3]);
			SpeechStateMachine();
		} else {
			pr_err("[PCM ERR]ARM2_EMU_Thread: Read BT Message Failure %d\r\n", (s32)i4_res);
		}
	}
	pr_debug("[PCM]ARM2_EMU_Thread: end\r\n");
...
static bool ARM2EmulationInit(void)
{
	if (!g_fgArm2Inited) {
		...
		if (x_msg_q_create(&g_hBTMsgQ, (const char *)BT_MSGQ_NAME,
			SPH_MSG_QUEUE_ITEM_SIZE, SPH_MSG_QUEUE_ITEM_NUM) != OSR_OK) {
			pr_err("[PCM ERR]ARM2EmulationInit: Failed to create BT msg queue\r\n");
			return false;
		}
		g_hArm2Thread = kthread_create(ARM2_EMU_Thread, (void *)NULL, "ARM2_EMU_Thread");
		if (IS_ERR(g_hArm2Thread)) {
			pr_err("[PCM ERR]ARM2EmulationInit: Create ARM2_EMU_Thread ERR %ld!\r\n",
...
bool SpeechSendMessage(uintptr_t u4Msg, uintptr_t u4P1, u32 u4P2, u32 u4P3)
{
	...
	if (g_fgArm2Inited) {
		if (x_msg_q_send(g_hBTMsgQ, dwParam, SPH_MSG_QUEUE_ITEM_SIZE, 1U) != OSR_OK) {
			pr_err("[PCM ERR]SpeechSendMessage: Failed to send BT Msg\r\n");
		}
		return true;
	}
...
bool AECSendMessage(uintptr_t u4Msg, uintptr_t u4P1, uintptr_t u4P2, uintptr_t u4P3)
{
	...
	if (x_msg_q_send(g_hAECMsgQ, dwParam, SPH_MSG_QUEUE_ITEM_SIZE, 1U) != OSR_OK) {
		pr_err("[PCM ERR]SpeechSendMessage: Failed to send AEC Msg\r\n");
	}
