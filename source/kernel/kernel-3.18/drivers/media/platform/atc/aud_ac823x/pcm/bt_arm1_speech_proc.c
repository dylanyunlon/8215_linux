...
static BT_SHARE_MEM_EX_T	 *g_prShareMem;
...
#define CONFIG_DRV_AUD_AC83XX 0;
...
//#if CONFIG_DRV_AUD_AC83XX
	...
	if (NULL == piAECMemPtr) {
		pr_err("[PCM]AECInit: malloc piAECMemPtr error!\r\n");
		return AEC_RESULT_FAILED;
	}
	pr_debug("[PCM]AECInit: SAdr(0x%x) size(%d)\r\n",
...
static bool SpeechProcess(u32 u4Opt)
{
	...
	BTMemCopy(_pi2SMUL, &g_Sph_Enh_ctrl_16k.PCM_buffer[0], g_prShareMem->u4FrameByte);
	...
	if (_prFrame->u4Opt & DATA_REQ_POST_NDC) {
		...
		BTMemCopy(_pi2SMDLDelay, &g_Sph_Enh_ctrl_16k.PCM_buffer[960], g_prShareMem->u4FrameByte);
	}

	/* Copy final data to DL Buffer */
	//BTMemCopy(_pi2SMDL, &g_Sph_Enh_ctrl_16k.EPL_buffer[2880], g_prShareMem->u4FrameByte);
	BTMemCopy(_pi2SMDL, &g_Sph_Enh_ctrl_16k.PCM_buffer[640], g_prShareMem->u4FrameByte);
...
/*==========================================
  ...
  ==========================================*/
static u32 BTSetHWResource(uintptr_t u4PhyAddr, uintptr_t u4Size, uintptr_t u4HwSema)
{
	...
	if ((BT_STATE_SCO == _u4State) || (BT_STATE_UNINIT == _u4State)) {
		...
		g_prShareMem = (BT_SHARE_MEM_EX_T *)ARM1PHY2ARM2UCV(u4PhyAddr);
		_u4HwSemaphore = u4HwSema;
		if (BT_STATE_INIT == _u4State) {
			_u4State = BT_STATE_IDLE;
			g_prShareMem->u4AECState = _u4State;
			pr_debug("[PCM]BTSetHWResource: Enter IDLE State. g_prShareMem(0x%lx),g_prShareMem->u4MaxFrame:%d\r\n",
				(uintptr_t)g_prShareMem,g_prShareMem->u4MaxFrame);
		}
		pr_debug("QK [PCM]BTSetHWResource: ->AECSendMessage \r\n");
		AECSendMessage((uintptr_t)BT_MSG_COMPLETED, (uintptr_t)BT_SET_HW_RESOURCE, _u4State, (uintptr_t)0);
		u4Ret = BT_SUCCESS;
	}
...
static u32 BTEnterSCO(bool fgEnter, u32 u4Request)
{
	...
	if ((8000 != g_prShareMem->u4SampleRate) && (16000 != g_prShareMem->u4SampleRate)) {
		pr_err("[PCM]BTEnterSCO: Sample rate(%d) error!\r\n",
	...
	if (fgEnter) {
		...
		if (8000 == g_prShareMem->u4SampleRate) {
			...
			pr_debug("[PCM]BTEnterSCO: AEC Parameters Request(0x%x)\r\n",
				u4Request);
			for (i = 0; i < AEC_NDC_PARAM_NUM; i++) {
				if (0 == (i % 10U)) {
					pr_debug("\r\n");
				}
				pr_debug("%d, ", (s32)Sph_Enh_ctrl.enhance_pars[i]);
			}
			pr_debug("\r\n");
			...
			if (u4Request & FRAME_OPT_AEC) {
				...
				u4Return = AECInit();
				pr_debug("[PCM]BTEnterSCO: AEC Init. Return(0x%x)\r\n", u4Return);
			}

			if (u4Request & FRAME_OPT_NDC) {
				u4Return = NDCInit();
				pr_debug("[PCM]BTEnterSCO: NDCInit. Return(0x%x)\r\n", u4Return);
			}
		} else {
			...
			pr_debug("[PCM]BTEnterSCO: QK11 check 48 -----AEC 16k Param =\r\n");
			for (i = 0; i < AEC_NDC_PARAM_NUM; i++) {
				if (0 == (i % 10)) {
					pr_debug("\r\n");
				}
				pr_debug("%d, ", (s32)g_Sph_Enh_ctrl_16k.enhance_pars[i]);
			}
			pr_debug("[PCM]BTEnterSCO: end QK11 check 48 -----AEC 16k Param =\r\n");


			BTMemCopy(g_Sph_Enh_ctrl_16k.common_pars, g_prShareMem->Sph_Enh_ctrl_comm_16k,
					sizeof(g_Sph_Enh_ctrl_16k.common_pars));

			pr_debug("[PCM]BTEnterSCO: QK11 check 12 -----AEC 16k common Param =\r\n");
			for (i = 0; i < COMMOM_PARAM_NUM; i++) {
				if (0 == (i % 10)) {
			pr_debug("\r\n");
				}
				pr_debug("%d, ", (s32)g_Sph_Enh_ctrl_16k.common_pars[i]);
			}
			pr_debug("[PCM]BTEnterSCO: end QK11 check 12 -----AEC 16k common Param =\r\n");
			...
			pr_debug("[PCM]BTEnterSCO: DMNR_cal_data 16k Param =\r\n");
			for (i = 0; i < DMNR_PARAM_NUM_16K; i++) {
				if (0 == (i % 10U)) {
					pr_debug("\r\n");
				}
				pr_debug("%d, ", (s32)g_Sph_Enh_ctrl_16k.DMNR_cal_data[i]);
			}
			pr_debug("\r\n");

			BTMemCopy(g_Sph_Enh_ctrl_16k.Compen_filter, g_prShareMem->Compen_filter_16k,
					sizeof(g_Sph_Enh_ctrl_16k.Compen_filter));
			pr_debug("[PCM]BTEnterSCO: COMPEN_FILTER 16k Param =\r\n");
			for (i = 0; i < COMPEN_FILTER_16K; i++) {
				if (0 == (i % 10U)) {
					pr_debug("\r\n");
				}
				pr_debug("%d, ", (s32)g_Sph_Enh_ctrl_16k.Compen_filter[i]);
			}
			pr_debug("\r\n");
			iMemSz = ENH_API_Get_Memory(&g_Sph_Enh_ctrl_16k);
			piMemPtr = (s32 *)BT_Malloc(iMemSz);
			if (NULL == piMemPtr) {
				pr_err("[PCM]BTEnterSCO: malloc piMemPtr error!\r\n");
				return BT_FAILURE;
			}
			pr_debug("[PCM]BTEnterSCO: SAdr(0x%x) size(%d)\r\n",
		...
		TimeStatInit();
		pr_debug("[PCM]BTEnterSCO: g_prShareMem(0x%x) MaxFrame(%d)\r\n",
			(u32)g_prShareMem, (s32)g_prShareMem->u4MaxFrame);
		pr_debug("[PCM]BTEnterSCO: ReadIdx(%d) WriteIdx(%d)\r\n",
			(s32)g_prShareMem->u4ReadIdx, (s32)g_prShareMem->u4WriteIdx);
		_u4State = BT_STATE_SCO;
		pr_debug("[PCM]BTEnterSCO: Change state from IDLE to SCO.\r\n");
	} else {
		if (8000 == g_prShareMem->u4SampleRate) {
		...
		_u4State = BT_STATE_IDLE;
		BT_MemoryUninit();
		pr_debug("[PCM]BTEnterSCO: Change state from SCO to IDLE.\r\n");
	}
...
static u32 BTHandleMsg(uintptr_t u4MsgID, uintptr_t u4P1, uintptr_t u4P2, uintptr_t u4P3)
{
	u32 u4Ret = BT_SUCCESS;
...
u32 SpeechStateMachine(void)
{
	...
	if (((u4Temp - _u4DbgMiniSecond) > 30000) && (BT_STATE_SCO ==  _u4State)) {
		pr_debug("[PCM]SpeechStateMachine: (%d ms)\r\n"), (s32)u4Temp);
...
u32 SpeechCB(uintptr_t u4MsgID, uintptr_t u4P1, uintptr_t u4P2, uintptr_t u4P3)
{
	...
	if (((uintptr_t)BT_SCO_AUDIO_CONTROL) == (u4MsgID & ((uintptr_t)0xFFFF))) {
