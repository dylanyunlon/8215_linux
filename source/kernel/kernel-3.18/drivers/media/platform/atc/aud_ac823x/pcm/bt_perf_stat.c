...
bool TimeStatUnInit(void)
{
	...
	u4Temp = (u32) (_u8HighPerFrequency / 1000U);
	if (0 == u4Temp) {
		pr_err("[PCM]TimeStatUnInit: error dwTemp = 0\r\n");
		return false;
	}
	for (i = 0; i < STAT_IDX_MAX; i++) {
		_prStatItem[i].u4OverLimit /= u4Temp;
...
bool TimeStatOutput(void)
{
	...
	pr_debug("[PCM]TimeStatOutput Start >>>>>>>>>>>>>>>>>>>>>>>\r\n");
	pr_debug("[PCM]The frequency of the high performance clock: %d ticks/Sec\r\n",
	...
	dUsPerFrequency = _u8HighPerFrequency;
	dUsPerFrequency = MICROSECOND / dUsPerFrequency;

	pr_debug("Name,  Max(us),  Avg(us),  Min(us),  Total(us),  Count, OverCount\r\n");
	for (i = 0; i < STAT_IDX_MAX; i++) {
	...
	pr_debug("[PCM]TimeStatOutput End <<<<<<<<<<<<<<<<<<<<<<<<<<<\r\n");
