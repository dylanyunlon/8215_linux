...
static s32 PCMDev_Thread(void *data)
{
	...
	pr_debug("[PCM]PCMDev_Thread: end!\r\n");

	return NOERR;
}

static s32 PCMDev_Thread_UL(void *data)
{
	u32 u4Ret = 0;

	pr_debug("[PCM]PCMDev_Thread_UL: start!\r\n");
	
	while (1) {
		g_rBtPcm.PCMDev_Thread_UL_wq_flag = 0;
		u4Ret = wait_event_interruptible(g_rBtPcm.PCMDev_Thread_UL_wq,
			g_rBtPcm.PCMDev_Thread_UL_wq_flag);
		...
		while(STATE_STARTED == g_rBtPcm.m_u4State){
			//msleep(1);
			SpeechDev_ULByPass(1920);
			msleep(5);
		}
	}
	pr_debug("[PCM]PCMDev_Thread_UL: end!\r\n");
...
s32 BtPCMHw_Init(void)
{
	...
	if (IS_ERR(g_rBtPcm.PCMDev_Thread_task)) {
		pr_err("[PCM ERR]BtPCMHw_Init: Create PCMDev_Thread_task ERR!\r\n");
	...
	else
	{
		...
		ret = sched_setscheduler_nocheck(g_rBtPcm.PCMDev_Thread_task, SCHED_RR, &param);
		ASSERT(ret == 0);
	}
	...
	/*g_rBtPcm.PCMDev_Thread_UL_task = kthread_create(PCMDev_Thread_UL, NULL, "PCMDev_Thread");
	if (IS_ERR(g_rBtPcm.PCMDev_Thread_UL_task)) {
		pr_err("[PCM ERR]BtPCMHw_Init: Create PCMDev_Thread_task ERR!\r\n");
		i4Ret = (s32)PTR_ERR(g_rBtPcm.PCMDev_Thread_UL_task);
		g_rBtPcm.PCMDev_Thread_UL_task = NULL;
		return i4Ret;
	}
	else
	{
		...
		param.sched_priority = to_sched_priority(ADSPTASK_THREAD_PRIORITY);
		ret = sched_setscheduler_nocheck(g_rBtPcm.PCMDev_Thread_UL_task, SCHED_RR, &param);
		ASSERT(ret == 0);
	}
	init_waitqueue_head(&g_rBtPcm.PCMDev_Thread_UL_wq);
	g_rBtPcm.PCMDev_Thread_UL_wq_flag = 0;
    wake_up_process(g_rBtPcm.PCMDev_Thread_UL_task);*/
	...
	if (g_rBtPcm.m_prPcmHal) {
		...
		pr_debug("[PCM]BtPCMHw_Init: RX Buf: 0x%lx, 0x%lx, 0x%x; TX Buf: 0x%lx, 0x%lx, 0x%x\r\n",
			(uintptr_t)g_rBtPcm.m_rRxBuf.u4PhySddr, (uintptr_t)g_rBtPcm.m_rRxBuf.u4VirSAdr,
			(u32)g_rBtPcm.m_rRxBuf.u4ChBufSz, (uintptr_t)g_rBtPcm.m_rTxBuf.u4PhySddr,
			(uintptr_t)g_rBtPcm.m_rTxBuf.u4VirSAdr, (u32)g_rBtPcm.m_rTxBuf.u4ChBufSz);
	} else {
		pr_err("[PCM ERR]BtPCMHw_Init: Can not alloc PCM Hal! \r\n");
		i4Ret = NORESOURCE;
		return i4Ret;
	}
...
u32 BtPCMHw_UnInit(void)
{
	...
	if (g_rBtPcm.PCMDev_Thread_task) {
		...
		g_rBtPcm.PCMDev_Thread_task = NULL;
		pr_debug("[PCM]BtPCMHw_UnInit: PCMDev_Thread stop!\r\n");
	}

	/*g_rBtPcm.PCMDev_Thread_UL_wq_flag = 1;
	wake_up_interruptible(&g_rBtPcm.PCMDev_Thread_UL_wq);
	if (g_rBtPcm.PCMDev_Thread_UL_task) {
		kthread_stop(g_rBtPcm.PCMDev_Thread_UL_task);
		g_rBtPcm.PCMDev_Thread_UL_task = NULL;
		pr_debug("[PCM]BtPCMHw_UnInit: PCMDev_Thread stop!\r\n");
	}*/
...
s32 BtPCMHw_Start(u32 u4SampleRate)
{
	...
	if ((STATE_INITED == g_rBtPcm.m_u4State) || (STATE_STOPPED == g_rBtPcm.m_u4State)) {
		...
		i4Ret = INVALIDSTATE;
		pr_err("[PCM ERR]BtPCMHw_Start: m_u4State error with UNINIT\r\n");
	} else {
		pr_err("[PCM ERR]BtPCMHw_Start: m_u4State error with STARTED\r\n");
	}
...
s32 BtPCMHw_Stop(void)
{
	...
	if (STATE_STARTED == g_rBtPcm.m_u4State) {
		...
		i4Ret = INVALIDSTATE;
		pr_err("[PCM ERR]BtPCMHw_Stop: m_u4State(%d) error\r\n", g_rBtPcm.m_u4State);
	}
...
{
	pr_debug("[PCM]BtPCMHw_EnableLoopback: fgEnable(%d) \r\n", (u32)fgEnable);
...
u32 BtPCMHw_FillULData(void *pvData, u32 u4DataSize)
{
	...
	if (u4DataSize & 0xFU) {
		pr_err("[PCM ERR]BtPCMHw_FillULData: Fill data size isn't 16 bytes alignment\r\n");
	}
	...
	memcpy_toio((void *)(g_rBtPcm.m_rTxBuf.u4VirSAdr + g_rBtPcm.m_u4ULWP), pvData, u4Size);
...
void BtPCMHw_SetDataFromFile(bool fgDataFromFile)
{
	...
	pr_debug("[PCM]BtPCMHw_SetDataFromFile: fgDataFromFile(%d)\r\n",
...
bool BtPCMHw_EnableDataFromFile(bool fgEnable)
{
	...
	if (STATE_STARTED != g_rBtPcm.m_u4State) {
		if (g_rBtPcm.m_fgDataFromFile != fgEnable) {
			pr_debug("[PCM]BtPCMHw_EnableDataFromFile: fgEnable(%d) \r\n", (s32)fgEnable);
			if (fgEnable) {
...
s32 DLDataFile_Load(void)
{
	...
	if (IS_ERR(g_DLFile.m_pfDL)) {
		pr_err("[PCM ERR]DLDataFile_Load: filp_open err(%d)!\r\n", (u32)g_DLFile.m_pfDL);
		set_fs(fs);
		return NORESOURCE;
	}
	...
	if (u4size < 0) {
		pr_err("[PCM ERR]DLDataFile_Load: vfs_read err(%i)!\r\n", u4size);
...
s32 DLDataFile_Init(void)
{
	...
	g_DLFile.m_rBuf.u4VirSAdr = (uintptr_t)kmalloc(g_DLFile.m_rBuf.u4ChBufSz * g_DLFile.m_rBuf.u4Chn, GFP_KERNEL);
	if (0 == g_DLFile.m_rBuf.u4VirSAdr) {
		pr_err("[PCM ERR]DLDataFile_Init: kmalloc buffer error!\r\n");
		return NORESOURCE;
	}
...
{
	if (!g_DLFile.m_rBuf.u4VirSAdr) {
		pr_err("[PCM ERR]DLDataFile_Read: u4VirSAdr is 0!\r\n");
		return NORESOURCE;
	}
