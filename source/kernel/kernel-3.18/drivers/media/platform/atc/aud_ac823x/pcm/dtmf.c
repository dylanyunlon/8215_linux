...
static void Dtmf_FindMaxDataIdx(void)
{
	...
	if (LogRange()) {
		pr_debug("[PCM]Dtmf_FindMaxDataIdx: Magnitude(Cnt: %d)\r\n",
			(s32)g_rDtmf.m_u4DbgCnt);
		for (i = 0; i < (u32)DTMF_FREQ_SIZE; i++)
			pr_debug("[PCM]Dtmf_FindMaxDataIdx: %d:\t%d/100\t%d/100\r\n",
...
static bool Dtmf_MaxDataEnough(void)
{
	if ((g_rDtmf.m_adlLowMag[g_rDtmf.m_u4LowIdx] < g_rDtmf.m_dlLowThreshold) ||
		...
		if (LogRange()) {
			pr_debug("[PCM]Dtmf_MaxDataEnough: Magnitude not great enough!\r\n");
		}
		return false;
	}
...
static bool Dtmf_CheckNoNoise(void)
{
	...
	if (u4NoiseCheckCnt > 2U) {
		if (LogRange()) {
			pr_debug("[PCM]Dtmf_CheckNoNoise: Noise too great!!!\r\n");
		}
		return false;
	}
...
{
	pr_debug("[PCM]Dtmf_GetNewDtmfSignal: (%d: %d, %d, %d, %d -- %d)!!!!\r\n",
...
static void Dtmf_AnalyseDtmfSignal(u32 u4NotifyMsg)
{
	if (g_rDtmf.m_u4Signal1 == u4NotifyMsg) {
		...
		if (g_rDtmf.m_u4ValidCnt1 < g_rDtmf.m_u4ValidCnt2) {
			pr_debug("[PCM]Dtmf_AnalyseDtmfSignal: 1(%d: %d, %d, %d, %d -- %d)\r\n",
				(s32)g_rDtmf.m_u4DbgCnt, (s32)g_rDtmf.m_u4ValidCnt1, (s32)g_rDtmf.m_u4ValidCnt2,
				(s32)g_rDtmf.m_u4BadValidCnt, (s32)g_rDtmf.m_u4InValidCnt, (s32)u4NotifyMsg);
			g_rDtmf.m_u4Signal1 = u4NotifyMsg;
			g_rDtmf.m_u4ValidCnt1 = 1;
		} else {
			pr_debug("[PCM]Dtmf_AnalyseDtmfSignal: 2(%d: %d, %d, %d, %d -- %d)\r\n",
				(s32)g_rDtmf.m_u4DbgCnt, (s32)g_rDtmf.m_u4ValidCnt1, (s32)g_rDtmf.m_u4ValidCnt2,
				(s32)g_rDtmf.m_u4BadValidCnt, (s32)g_rDtmf.m_u4InValidCnt, (s32)u4NotifyMsg);
...
static void Dtmf_NotifyDtmfSignal(u32 u4NotifyMsg)
{
	...
	pr_debug("[PCM]Dtmf_NotifyDtmfSignal: NotifyMsg(%c)\r\n", (s32)u4NotifyMsg);
	/*68031 DA must to do
...
static void Dtmf_AnalyseInput(void)
{
	...
	if ((Dtmf_MaxDataEnough()) && (Dtmf_CheckNoNoise())) {
		...
		if (g_rDtmf.m_u4DetectState != ST_INVALID) {
			...
			if (g_rDtmf.m_u4InValidCnt >= g_rDtmf.m_u4InValidLeast) {
				pr_debug("[PCM]Dtmf_AnalyseInput:");
				pr_debug("InValid(%d: %d, %d, %d, %d --%d)\r\n",
					(s32)g_rDtmf.m_u4DbgCnt, (s32)g_rDtmf.m_u4ValidCnt1, (s32)g_rDtmf.m_u4ValidCnt2,
	...
	if (g_rDtmf.m_u4DetectState != ST_INVALID) {
		if (g_rDtmf.m_u4ValidCnt1 + g_rDtmf.m_u4ValidCnt2 + g_rDtmf.m_u4BadValidCnt > g_rDtmf.m_u4MaxLen) {
			pr_debug("[PCM]Dtmf_AnalyseInput: Over Max Len(%d: %d, %d, %d, %d -- %d)\r\n",
				(s32)g_rDtmf.m_u4DbgCnt, (s32)g_rDtmf.m_u4ValidCnt1, (s32)g_rDtmf.m_u4ValidCnt2,
...
void Dtmf_SetThreshold(double dlLowThreshold, double dlHighThreshold)
{
	...
	pr_debug("[PCM]Dtmf_SetThreshold: Threshold(Low: %d/100, High: %d/100)\r\n",
...
void Dtmf_SetNoiseRatio(u32 u4NoiseRatio)
{
	...
	pr_debug("[PCM]Dtmf_SetNoiseRatio: NoiseRatio(%d/100)\r\n", (s32)u4NoiseRatio);
}
...
void Dtmf_SetValidTime(u32 u4ValidTime, u32 u4Scale)
{
	...
	pr_debug("[PCM]Dtmf_SetValidTime: ValidLeast(%d, %d/100 -- %d)\r\n",
...
void Dtmf_SetInValidTime(u32 u4InValidTime, u32 u4Scale)
{
	...
	pr_debug("[PCM]Dtmf_SetInValidTime: InValidLeast(%d, %d/100 -- %d)\r\n",
...
void Dtmf_SetNewSamples(u32 u4NewSamples)
{
	...
	pr_debug("[PCM]Dtmf_SetNewSamples: u4NewSamples(%d)\r\n", (s32)g_rDtmf.m_u4NewSamples);
}
...
void Dtmf_SetMaxScale(u32 u4MaxScale)
{
	...
	pr_debug("[PCM]Dtmf_SetMaxScale: Max Len(%d, %d/100 -- %d)\r\n",
...
void Dtmf_SetLogRange(u32 u4Start, u32 u4End)
{
	...
	pr_debug("[PCM]Dtmf_SetLogRange: (%d -- %d) \r\n",
...
{
	pr_debug("[PCM][Dtmf]============Dtmf Paramters============\r\n");
	pr_debug("Threshold: low(%d/100), high(%d/100)\r\n",
		(u32)(g_rDtmf.m_dlLowThreshold * 100.0), (u32)(g_rDtmf.m_dlHighThreshold * 100.0));
	pr_debug("Noise Ratio: %d/100 \r\n", (u32)(g_rDtmf.m_dlNoiseRatio * 100.0));
	pr_debug("Valid Time: %d, %d/100 \r\n",
		(s32)g_rDtmf.m_u4ValidTime, (u32)(g_rDtmf.m_dlValidScale * 100.0));
	pr_debug("InValid Time: %d, %d/100 \r\n",
		(s32)g_rDtmf.m_u4InValidTime, (u32)(g_rDtmf.m_dlInValidScale * 100.0));
	pr_debug("New Samples: %d	\r\n", (s32)(g_rDtmf.m_u4NewSamples));
	pr_debug("Max Scale: %d/100 \r\n", (u32)(g_rDtmf.m_dlMaxSacle * 100.0));
	pr_debug("Least cnt: valid(%d), max(%d), invalid(%d)\n",
		(s32)g_rDtmf.m_u4ValidLeast, (s32)g_rDtmf.m_u4MaxLen, (s32)g_rDtmf.m_u4InValidLeast);
	pr_debug("[PCM][Dtmf]============Dtmf Paramters end=========\r\n");
}
...
s32 Dtmf_Init(void)
{
	...
	pr_debug("[PCM]Dtmf_Init: m_ai2DtmfBuf(0x%x).\r\n",
...
s32 Dtmf_EnableDtmfFunc(u32 u4DtmfCtrlType, u32 u4NotifyId)
{
	if (g_rDtmf.m_u4DtmfCtrlType != u4DtmfCtrlType) {
		...
		pr_debug("[PCM]Dtmf_EnableDtmfFunc: %d, NotifyId: 0x%x\r\n",
...
static void DtmfInfoSender_Notify(void)
{
	...
	pr_debug("[PCM]DtmfInfoSender_Notify: NotifyId(0x%x), Cnt(%d, %d)\r\n",
...
static void DtmfInfoSender_ShowDataInfo(u32 u4Cnt)
{
	...
	while (NULL != prBuf) {
		pr_debug("[PCM]DtmfInfoSender_ShowDataInfo:[Proc(%d)]0x%x ->\r\n",
...
{
	pr_debug("[PCM]DtmfInfoSender_Init: NotifyId(0x%x)\r\n", (u32)u4NotifyId);
...
{
	pr_debug("[PCM]DtmfInfoSender_UnInit: Delete\r\n");
	g_DtmfInfoSender.m_prBufList = NULL;
...
s32 DtmfInfoSender_GetNewData(u32 u4SAdr, u32 u4Size)
{
	...
	if (u4Size % g_DtmfInfoSender.m_u4ProcLen) {
		pr_err("[PCM]DtmfInfoSender_GetNewData: The size(%d)of new buf is error! \r\n", (s32)u4Size);
		i4Ret = NORESOURCE;
	} else {
...
s32 DtmfTest_AnalyseFiles(u32 u4NotifyId, u32 u4Range)
{
	...
	pr_debug("[PCM]DtmfTest_AnalyseFiles: u4NotifyId(%d) u4Range(0x%x) \r\n",
	...
	for (u4Idx = u4Start; u4Idx <= u4End; u4Idx++) {
		...
		pr_debug("[PCM]DtmfTest_AnalyseFiles: %s\r\n", szFileName);
		fs = get_fs();
		set_fs(KERNEL_DS);
		m_pfDL = filp_open(szFileName, O_RDONLY, 0);
		if (IS_ERR(m_pfDL)) {
			pr_err("[PCM]DtmfTest_AnalyseFiles: filp_open err(%d)!\r\n", (u32)m_pfDL);
			set_fs(fs);
			return NORESOURCE;
		}
		...
		if (NULL == pi2Buf) {
			pr_err("[PCM]DtmfTest_AnalyseFiles: kmalloc err!\r\n");
		...
		if (u4ReadSize < 0) {
			pr_err("[PCM]DtmfTest_AnalyseFiles: vfs_read err(%i)!\r\n", u4ReadSize);
			filp_close(m_pfDL, NULL);
			set_fs(fs);
			kfree(pi2Buf);
			pi2Buf = NULL;
