...
{
	pr_debug("[PCM D] Capture_NdcEnable: %d -> %d .\r\n", (int)_fgCaptureDncEn, (int)fgEnable);  
...
static u32 TransferData(struct snd_pcm_substream  *substream)
{
	...
	if(NOERR != capture_stream->prMic->GetBuffer(capture_stream->prMic, &rMicBuf)) {
		pr_err("[PCM ERR]TransferData: Get Mic In output buffer error\r\n");
		return 0;
	}
	...
	while((substrm_data->m_u4MicRP != u4RP) && m_lpCurrData) {
		...
		case PCM_TYPE_M8:
			...
			memcpy_fromio(m_lpCurrData,rMicBuf.u4Buf1 + substrm_data->m_u4MicRP,1U);
		...
		case PCM_TYPE_S8:
			...
			*m_lpCurrData = *m_lpCurrData + 128;
			...
			memcpy_fromio(m_lpCurrData,rMicBuf.u4Buf2 + substrm_data->m_u4MicRP,1U);
			*m_lpCurrData = *m_lpCurrData + 128;
			m_lpCurrData++;
			u4size = 2U;
			break;
		...
		case PCM_TYPE_M16:
			...
			memcpy_fromio(m_lpCurrData,rMicBuf.u4Buf1 + substrm_data->m_u4MicRP,2U);
			m_lpCurrData += 2;
			u4size = 2U;
			break;
		...
		case PCM_TYPE_S16:
			...
			memcpy_fromio(m_lpCurrData,rMicBuf.u4Buf2 + substrm_data->m_u4MicRP,2U);
			m_lpCurrData += 2;
...
s32 CopyMicDataThread(void *data)
{
	...
	if(NULL == substrm_data) {
		pr_err("[PCM ERR]CopyMicDataThread: substrm_data is NULL!\r\n");
		goto ERROR;
	}
	substream = (struct snd_pcm_substream *)substrm_data->substream;
	
	if(NULL == substream) {
		pr_err("[PCM ERR]CopyMicDataThread: substream is NULL!\r\n");
		goto ERROR;
	}
	...
	if(NULL == runtime) {
		pr_err("[PCM ERR]CopyMicDataThread: runtime is NULL!\r\n");
		goto ERROR;
	}

	capture_stream = (atc_capture_stream *) substrm_data;
	pr_info("[PCM]CopyMicDataThread: start capture(0x%x) \r\n", (u32)capture_stream);
	if (_fgCaptureDncEn)
		type = VMT_NDC;
	...
	while(!capture_stream->CopyMicDataThread_exit) {
		...
		if(NOERR != capture_stream->prMic->Start(capture_stream->prMic)) {
			pr_err("[PCM ERR] Failed to start Virtual Mic In.\r\n");
			goto ERROR;
		}
		...
		if(NOERR != capture_stream->prMic->Stop(capture_stream->prMic)) {
			pr_err("[PCM ERR] Failed to stop Virtual Mic In.\r\n");
		}
	...
	pr_info("[PCM]CopyMicDataThread: end capture(0x%x) \r\n", (u32)capture_stream);
...
ERROR:
	up(&substrm_data->rThreadExitSema);
	pr_err("[PCM ERR]CopyMicDataThread: end (Error) capture(0x%x) \r\n", (u32)capture_stream);
