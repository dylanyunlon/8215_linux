...
s32 MicIn_Init(void)
{
	...
	if(!g_rMic.m_prMicHal) {
		pr_err("[PCM ERR]MicIn_Init: Create MIC Hal failed!\r\n");
		i4Ret = NORESOURCE;
	} else {
		if(g_rExtMicConf.u4ExtMicEn) {
			...
			pr_debug("[PCM]MicIn_Init: Use External Mic PinMux(%d) FS_Idx(%d) BW(%d)\r\n",
				 (u32)g_rMic.m_rMicCfg.eI2sPin, (u32)g_rMic.m_rMicCfg.eFs,
				 g_rMic.m_rMicCfg.u4SrcBitNum);
		} else {
			g_rMic.m_rMicCfg.eSrc = INT_MICIN;
			g_rMic.m_rMicCfg.eFs = FS_8K;
			pr_debug("[PCM]MicIn_Init: Use Internal Mic FS_Idx(%d) BW(%d)\r\n",
...
void MicIn_SetPrimaryMic(u32 u4Index)
{
	...
	pr_debug("[PCM]MicIn_SetPrimaryMic: Set mic%d as primary mic!\r\n",
...
void MicIn_SetExtMicCfg(EXT_MIC_CONF rExtMicConf)
{
	...
	pr_debug("[PCM]MicIn_SetExtMicCfg: En(%d) PinMux(%d) FS(%d) BW(%d)\r\n",
...
s32 MicIn_SetGain(u32 u4MicGain)
{
	if(u4MicGain <= 63U) {
		...
		pr_err("[PCM ERR]MicIn_SetGain: gain(%d) is not between 0 and 63, so set fail!\r\n",
		       (u32)u4MicGain);
		return INVALIDPRAM;
	}

	pr_debug("[PCM]MicIn_SetGain: %d (%d dB) \r\n",
...
{
	pr_debug("[PCM]MicIn_GetGain: %d (%d dB) \r\n",
		 (u32)g_rMic.m_rMicCfg.u4MicGain, (s32)(g_rMic.m_rMicCfg.u4MicGain - 14U));
...
s32 MicIn_Start(void *owner)
{
	...
	ASSERT(owner);
	pr_info("[PCM]MicIn_Start: owner(0x%x).\r\n", (u32)owner);
    ...
    {
        pr_err("[PCM ERR]MicInHal_Start: No free MicIn array!\r\n");
	...
	if (!g_rMic.m_u4StartTimes)
	{
		...
		if((STATE_INITED == g_rMic.m_u4State) || (STATE_STOPPED == g_rMic.m_u4State))  {
			...
			pr_debug("[PCM]MicIn_Start: INVALIDSTATE\r\n");
			mutex_unlock(&g_rMic.m_MicLock);
			return INVALIDSTATE;
		}
	}
	g_rMic.m_u4StartTimes ++;
	pr_info("[PCM]MicIn_Start: end.\r\n");
...
s32 MicIn_Stop(void *owner)
{
	...
	pr_debug("[PCM]MicIn_Stop owner(0x%x) StartTimes(%d).\r\n", (u32)owner, g_rMic.m_u4StartTimes);
	if (!g_rMic.m_u4StartTimes)
		return (INVALIDSTATE);
    ...
    if (MICIN_MAX_NUM == u4Idx)
    {
        pr_err("[PCM ERR]MicIn_Stop: can't find owner!!!\r\n");
        return (INVALIDPRAM);
    }
	...
	if(!g_rMic.m_u4StartTimes) {
		if(STATE_STARTED == g_rMic.m_u4State) {
			...
			pr_debug("[PCM]MicIn_Stop: Stop MIC IN.\r\n");
		} else {
			pr_debug("[PCM]MicIn_Stop: INVALIDSTATE\r\n");
			i4Ret = INVALIDSTATE;
		}
	}
...
void MicIn_InitBuffer(void)
{
	...
	pr_debug("[PCM]MicIn_InitBuffer: MicIn Buf: 0x%lx, 0x%lx, %d, %d ,BufPhyAdr:0x%lx \r\n",
		 (uintptr_t)g_rMic.m_rBuf.u4Buf1, (uintptr_t)g_rMic.m_rBuf.u4Buf2,
		 (u32)g_rMic.m_rBuf.u4ChBufSz, (u32)g_rMic.m_rBuf.u4Chn,g_rMic.m_rMicCfg.u4BufPhyAdr);
}
...
void MicIn_SetDataFromFile(bool fgDataFromFile)
{
	...
	pr_debug("[PCM]MicIn_SetDataFromFile: fgDataFromFile(%d)\r\n",
...
bool MicIn_EnableDataFromFile(bool fgEnable)
{
	...
	if(STATE_STARTED != g_rMic.m_u4State) {
		if(g_rMic.m_fgDataFromFile != fgEnable) {
			pr_err("[PCM ERR]MicIn_EnableDataFromFile: fgEnable(%d)\r\n",
			       (u32)fgEnable);
...
s32 ULDataFile_Init(void)
{
	...
	if(!g_ULFile.m_rBuf.u4Buf1) {
		...
		if(0 == g_ULFile.m_rBuf.u4Buf1) {
			pr_err("[PCM ERR]ULDataFile_Init: kmalloc buffer error!\r\n");
			return NORESOURCE;
		}
...
s32 ULDataFile_SetRP(u32 u4RP)
{
	if(u4RP == g_ULFile.m_u4WP) {
		...
		if(!g_ULFile.m_rBuf.u4Buf1) {
			pr_err("[PCM ERR]ULDataFile_SetRP: u4Buf1 is 0!\r\n");
			return NORESOURCE;
		}
		...
		if(IS_ERR(g_ULFile.m_pfUL)) {
			pr_err("[PCM ERR]ULDataFile_SetRP: filp_open err(%d)!\r\n", (u32)g_ULFile.m_pfUL);
			set_fs(fs);
			return NORESOURCE;
		}
		...
		if(u4size < 0) {
			pr_err("[PCM ERR]ULDataFile_SetRP: vfs_read err(%i)!\r\n", u4size);
		...
		if(IS_ERR(g_ULFile.m_pfUL2)) {
			pr_err("[PCM ERR]ULDataFile_SetRP: filp_open UL2 err(%d)!\r\n", (u32)g_ULFile.m_pfUL2);
			set_fs(fs);
			return NORESOURCE;
		}
		...
		if(u4size < 0) {
			pr_err("[PCM ERR]ULDataFile_SetRP: vfs_read UL2 err(%i)!\r\n", u4size);
