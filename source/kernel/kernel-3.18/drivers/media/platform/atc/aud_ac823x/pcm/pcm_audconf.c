...
#define SPEECH_DELAY_SAMPLE			1100U
#define SPEECH_16K_DELAY_SAMPLE		1960U
#define SPEECH_MIC_GAIN				34U  /* 40-14=26dB */ //24~26~28
...
static uWord32 sph_enh_ctrl_16k[AEC_NDC_PARAM_NUM] = {
	96,		/* AEC NLP */
	224,		/* AEC control word */
	16388,		/* AEC Echo suppression */
	31,			/* NDC UL control word */
	57067,		/* NDC NR */
	799,			/* NDC DL control word */
	400,		/* NDC calibration */
	64,		/* Digital Gain */
	80,		/* NDC NR */
	4325,		/* NDC NR aggressive mode */
...
static uWord32 sph_enh_comm_16k[COMMOM_PARAM_NUM] = {
	0,
	55997,
	31000,
	32767,
...
s32 pcm_audconf_set(void)
{
	...
	for (u4Idx = 0; u4Idx < COMMOM_PARAM_NUM; u4Idx++) {
		rSpeech16kConf.enhance_16k_comm_pars[u4Idx] = sph_enh_comm_16k[u4Idx];
	}
...
s32 pcm_audconf_ioctl(u32 context, u32 code, void *pBuffer, u32 dwCount)
{
	...
	pr_debug("[PCM]pcm_audconf_ioctl: cmd = 0x%02x\r\n", (code - SET_SPH_DELAY) >> 2);
	if (NULL == pBuffer) {
		pr_err("[PCM]pcm_audconf_ioctl: pBuffer is NULL\r\n");
		return -1;
	}

	switch (code) {
	case SET_SPH_DELAY:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_SPH_DELAY\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_SPH_DELAY count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_SPH_DELAY:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_SPH_DELAY\r\n");
		pr_debug("[PCM]pcm_audconf_ioctl: 8K speech has delayed %d samples\r\n",
			(s32)SpeechDev_GetDLDelay());
		*((u32 *)pBuffer) = SpeechDev_GetDLDelay();
		break;

	case SET_SPH_16K_DELAY:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_SPH_16K_DELAY\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_SPH_16K_DELAY count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_SPH_16K_DELAY:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_SPH_16K_DELAY\r\n");
		pr_debug("[PCM]pcm_audconf_ioctl: 16K speech has delayed %d samples\r\n",
			(s32)SpeechDev_GetDL16KDelay());
		*((u32 *)pBuffer) = SpeechDev_GetDL16KDelay();
		break;

	case SET_DEVICE_SPH_GAIN:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_DEVICE_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_DEVICE_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_DEVICE_SPH_GAIN:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_DEVICE_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			pr_err("[PCM]pcm_audconf_ioctl: GET_DEVICE_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_BT_SPH_GAIN:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_BT_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_BT_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_BT_SPH_GAIN:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_BT_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			pr_err("[PCM]pcm_audconf_ioctl: GET_BT_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_MIC_MUTE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_MIC_MUTE\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_MIC_MUTE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_SPH_MIC_GAIN:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_SPH_MIC_GAIN\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_SPH_MIC_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_SPH_MIC_GAIN:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_SPH_MIC_GAIN\r\n");
		*((u32 *)pBuffer) = MicIn_GetGain();
		break;

	case SET_DMNR_PARAM:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_DMNR_PARAM\r\n");
		break;

	case SET_AEC_NDC_PARAM:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_AEC_NDC_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_AEC_NDC_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_AEC_NDC_PARAM:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_AEC_NDC_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: GET_AEC_NDC_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_AEC_NDC_16K_PARAM:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_AEC_NDC_16K_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_16K_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_AEC_NDC_16K_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_AEC_NDC_16K_PARAM:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_AEC_NDC_16K_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_16K_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: GET_AEC_NDC_16K_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case BT_SCO_ENABLE:
		pr_debug("[PCM]pcm_audconf_ioctl: BT_SCO_ENABLE\r\n");

		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: BT_SCO_ENABLE count err!\r\n");
			return INVALIDPRAM;
		}
#if 0		
		if (SpeechDev_IsSCOEnable() != true) {
			pr_debug("[PCM]pcm_audconf_ioctl: BT_SCO TO ENABLED\r\n");
			SpeechDev_EnableSCO(true, *((u32 *)pBuffer));
		} else {
			pr_debug("[PCM]pcm_audconf_ioctl: BT_SCO has ENABLED\r\n");
		}
#endif		
	...
	case BT_SCO_DISABLE:
		pr_debug("[PCM]pcm_audconf_ioctl: BT_SCO_DISABLE\r\n");
        SpeechDev_Enable(FALSE, TRUE, 8000);
#if 0		
		if (SpeechDev_IsSCOEnable() != false) {
			pr_debug("[PCM]pcm_audconf_ioctl: BT_SCO TO DISABLE\r\n");
			SpeechDev_EnableSCO(false, 8000);
		} else {
			pr_debug("[PCM]pcm_audconf_ioctl: BT_SCO has DISABLE\r\n");
		}
#endif		
		break;

	case SET_DL_USE_FILE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_DL_USE_FILE\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_DL_USE_FILE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_UL_USE_FILE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_UL_USE_FILE\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_UL_USE_FILE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_BT_SPH_PLC:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_BT_SPH_PLC\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_BT_SPH_PLC count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_DSP_MIX_CH:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_DSP_MIX_CH\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_DSP_MIX_CH count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_BT_SPH_DUMP:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_BT_SPH_DUMP\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_BT_SPH_DUMP count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PRIMARY_MIC:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PRIMARY_MIC\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PRIMARY_MIC count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_LOG:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_LOG\r\n");
		if (dwCount < sizeof(u32)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_LOG count err!\r\n");
			return INVALIDPRAM;
		}
		break;
	case SET_CAPTURE_NDC_ENABLE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_CAPTURE_NDC_ENABLE\r\n");
		Capture_NdcEnable(*((bool *)pBuffer));
		break;

#if (ENABLE_DTMF_FUNCTION)
	case SET_PCM_DTMF_CTRL:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_CTRL\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_CTRL count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_THRESHOLD:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_THRESHOLD\r\n");
		if (dwCount < sizeof(PCM_DTMF_THRESHOLD)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_THRESHOLD count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_NOISE_RATIO:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_NOISE_RATIO\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_NOISE_RATIO count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_VALID_TIME:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_VALID_TIME\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_VALID_TIME count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_INVALID_TIME:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_INVALID_TIME\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: DTMF_INVALID_TIME count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_NEW_SAMPLES:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_NEW_SAMPLES\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_NEW_SAMPLES count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_MAX_SCALE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_MAX_SCALE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_MAX_SCALE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_INFO_SENDER_EN:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_INFO_SENDER_EN\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: DTMF_INFO_SENDER_EN count err!\r\n");
			return INVALIDPRAM;
		}
		pr_debug("[PCM][68031]pcm_audconf_ioctl: SENDER_EN (%d, %d)\r\n",
	...
	case SET_PCM_DTMF_INFO_SENDER_WRITE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_INFO_SENDER_WRITE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: DTMF_INFO_SENDER_WRITE count err!\r\n");
			return INVALIDPRAM;
		}
		if (((PCM_DTMF_CONF *)pBuffer)->u4Param1)
	...
	case GET_PCM_DTMF_PARAM:
		pr_debug("[PCM]pcm_audconf_ioctl: GET_PCM_DTMF_PARAM\r\n");
		Dtmf_LogParams();
		break;

	case SET_PCM_DTMF_LOG_RANGE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_LOG_RANGE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_LOG_RANGE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_TEST_USE_FILE:
		pr_debug("[PCM]pcm_audconf_ioctl: SET_PCM_DTMF_TEST_USE_FILE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			pr_err("[PCM]pcm_audconf_ioctl: DTMF_TEST_USE_FILE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	default:
		i4Ret = -1;
		pr_debug("[PCM]pcm_audconf_ioctl: UNKNOWN CMD, cmd=%04x, u4Ret=%d\r\n", code, i4Ret);
		break;
	}
