...
{
    pr_debug("[PCM D]SpeechDev_Enable: En(%d) TrueSph(%d) Fs(%d) (%d %d)\r\n",
    ...
    if (fgEnable)
    {
    	...
    	{
			pr_warn("[PCM WAR]SCO has been enabled already!!!\r\n");
			return;
    	}
    ...
    else
    {
    	if (fgTrueSph && !g_prSpeechDev.m_fgTrueSphEn)
    	{
			pr_warn("[PCM WAR]SCO is disalbed!!!\r\n");
			return;
    	}
    	if (!fgTrueSph && !g_prSpeechDev.m_u4NDCForCapture)
    	{
			pr_warn("[PCM WAR]SCO for capture is disalbed!!!\r\n");
			return;
    	}
    }
...
s32 SpeechDev_EnableBT(bool fgEnable,u32 u4SampleRate){
	...
	if ((STATE_INITED == g_prSpeechDev.m_u4State) && fgEnable) {
		pr_debug("[PCM]SpeechDev_EnableSCO: SampleRate(%d)\r\n", (s32)u4SampleRate);
...
s32 SpeechDev_EnableSCO(bool fgEnable, u32 u4SampleRate)
{
	...
	pr_debug("[PCM D]SpeechDev_EnableSCO: EnableSCO(%d) start\r\n", (s32)fgEnable);
	if ((STATE_INITED == g_prSpeechDev.m_u4State) && fgEnable) {
		pr_debug("[PCM]SpeechDev_EnableSCO: SampleRate(%d)\r\n", (s32)u4SampleRate);
        g_u4DLSilenceFrame = 0;
		g_u4SpeechPrePbSize = (u32)SPEECH_PRE_PB_SIZE * u4SampleRate / 8000U;
		...
		if (u4SampleRate == 8000U) {
			pr_debug("[PCM D]SpeechDev_EnableSCO: Speech delay %d samples\r\n",
	...
	pr_debug("[PCM D]SpeechDev_EnableSCO(%d) end\r\n", fgEnable);
...
void SpeechDev_SubstreamToTx(void)
{
	...
	for(i = 0;(i < 2) &&(transforSrc[i] > 0);i++){
		if ((g_rBtPcm.m_u4ULWP + transforSrc[i]) > g_rBtPcm.m_rTxBuf.u4ChBufSz) {
			...
			memcpy_toio((void *)(g_rBtPcm.m_rTxBuf.u4VirSAdr + g_rBtPcm.m_u4ULWP), substrm_data->dma_start + u4Len, transforDest[0]);
			memcpy_toio((void *)(g_rBtPcm.m_rTxBuf.u4VirSAdr), substrm_data->dma_start + u4Len + transforDest[0], transforDest[1]);
			__flush_dcache_area((void*)(g_rBtPcm.m_rTxBuf.u4VirSAdr + g_rBtPcm.m_u4ULWP), transforDest[0]);
			__flush_dcache_area((void*)(g_rBtPcm.m_rTxBuf.u4VirSAdr), transforDest[1]);
		...
		else
        {
		    ...
		    memcpy_toio((void *)(g_rBtPcm.m_rTxBuf.u4VirSAdr + g_rBtPcm.m_u4ULWP), substrm_data->dma_start + u4Len, transforSrc[i]);
            __flush_dcache_area((void*)(g_rBtPcm.m_rTxBuf.u4VirSAdr + g_rBtPcm.m_u4ULWP), transforSrc[i]);
...
u32 SpeechDev_EventInform(u32 u4Event, uintptr_t u4Param)
{
	...
	case EVT_SPH_SYNC:
		if (STATE_WAITFOR_SYNC == g_prSpeechDev.m_u4State) {
			pr_debug("[PCM]SpeechDev_EventInform: STATE_WAITFOR_SYNC event.\r\n");
			SpeechDev_Synchronization();
		}
		break;
...
s32 SpeechDev_Init(void)
{
	...
	if (!g_prSpeechDev.m_ai2DLRingBuffer) {
		pr_err("[PCM ERR]SpeechDev_Init: get DLRingBuffer failed\r\n");
		return NORESOURCE;
	}
	memset(g_prSpeechDev.m_ai2DLRingBuffer, 0, DL_REF_BUF_SIZE);
	pr_debug("[PCM]SpeechDev_Init: m_ai2DLRingBuffer(0x%lx).\r\n",
		(uintptr_t)g_prSpeechDev.m_ai2DLRingBuffer);
	...
	pr_debug("[PCM]SpeechDev_Init: g_pBTDmaArea(0x%lx).\r\n", (uintptr_t)g_pBTDmaArea);
	...
	pr_debug("[PCM]SpeechDev_Init: Success.\r\n");
...
void SpeechDev_SynchronizationEx(void)
{
	if (STATE_WAITFOR_SYNC == g_prSpeechDev.m_u4State) {
		...
		memset_io((void *)(g_rBtPcm.m_rTxBuf.u4VirSAdr),0,g_rBtPcm.m_rTxBuf.u4ChBufSz);
		memset_io((void *)(g_rBtPcm.m_rRxBuf.u4VirSAdr),0,g_rBtPcm.m_rRxBuf.u4ChBufSz);
		__flush_dcache_area((void *)(g_rBtPcm.m_rTxBuf.u4VirSAdr), g_rBtPcm.m_rTxBuf.u4ChBufSz);
		__flush_dcache_area((void *)(g_rBtPcm.m_rRxBuf.u4VirSAdr), g_rBtPcm.m_rRxBuf.u4ChBufSz);

		BtPCMHw_Start(g_prSpeechEnhance.m_prShareMem->u4SampleRate);
		pr_debug("[PCM]SynchronizationEx: %dms,m_u4MICRP:%d,g_prSpeechEnhance.m_prShareMem->u4SampleRate:%d\r\n", (s32)(1000 * jiffies / HZ),g_prSpeechDev.m_u4MICRP,g_prSpeechEnhance.m_prShareMem->u4SampleRate);
		//mutex_unlock(&g_prSpeechDev.m_SpeechLock);
	}
...
void SpeechDev_Synchronization(void)
{
	...
	pr_debug("[PCM]SpeechDev_Synchronization: Copy Bt speech data to appl_buffer Start\r\n");
	...
	pr_debug("[PCM]SpeechDev_Synchronization: end %dms\r\n", (s32)(1000 * jiffies / HZ));
}
...
static void SpeechDev_ULPostProcess(SPEECH_FRAME_T *prFrame)
{
	...
	if (u4Len < g_prSpeechEnhance.m_prShareMem->u4FrameByte) {
		pr_err("[PCM ERR]SpeechDev_ULPostProcess: Lost UL Data. u4Len: %d \r\n", (s32)u4Len);
	} else {
...
int SpeechDev_ULByPass(u32 dataLen)
{
    ...
    
	{
		...
		switch(substrm_data->m_SampleType) {
			...
			case PCM_TYPE_M16:
				if(g_prSpeechDev.m_u4MICRP + u4UlLen <= g_prSpeechDev.m_rMicBuf.u4ChBufSz){
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf1 + g_prSpeechDev.m_u4MICRP), u4UlLen);
				}else{
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf1 + g_prSpeechDev.m_u4MICRP), g_prSpeechDev.m_rMicBuf.u4ChBufSz - g_prSpeechDev.m_u4MICRP);
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf1), u4UlLen - g_prSpeechDev.m_rMicBuf.u4ChBufSz + g_prSpeechDev.m_u4MICRP);
				}
				break;
			case PCM_TYPE_S8:
			case PCM_TYPE_S16:
				...
				if(g_prSpeechDev.m_u4MICRP + u4UlLen <= g_prSpeechDev.m_rMicBuf.u4ChBufSz){
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf1 + g_prSpeechDev.m_u4MICRP ), u4UlLen);
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf2 + g_prSpeechDev.m_u4MICRP ), u4UlLen);
				}else{
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf1 + g_prSpeechDev.m_u4MICRP), g_prSpeechDev.m_rMicBuf.u4ChBufSz - g_prSpeechDev.m_u4MICRP);
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf1), u4UlLen - g_prSpeechDev.m_rMicBuf.u4ChBufSz + g_prSpeechDev.m_u4MICRP);

					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf2 + g_prSpeechDev.m_u4MICRP), g_prSpeechDev.m_rMicBuf.u4ChBufSz - g_prSpeechDev.m_u4MICRP);
					__flush_dcache_area((void*)(g_prSpeechDev.m_rMicBuf.u4Buf2), u4UlLen - g_prSpeechDev.m_rMicBuf.u4ChBufSz + g_prSpeechDev.m_u4MICRP);
				}
				break;
			default:
				return 0;
...
static void SpeechDev_DLByPass(void)
{
...

	{
		...
		if(u4DlLen2 < g_prSpeechEnhance.m_prShareMem->u4FrameByte)return;
		u4Len = g_prSpeechDev.m_u4DLRP;
		if(g_prSpeechDev.m_DropDLCnt < PCMRX_SILENCE_FRAME || snd_chip->m_u4BtMute == 1){
			...
			if ((g_prSpeechDev.m_u4DLRP + g_prSpeechEnhance.m_prShareMem->u4FrameByte) > g_prSpeechDev.m_rDLBuf.u4ChBufSz) {
				memset_io((void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + g_prSpeechDev.m_u4DLRP),0,g_prSpeechDev.m_rDLBuf.u4ChBufSz - g_prSpeechDev.m_u4DLRP);
				memset_io((void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr),0,g_prSpeechEnhance.m_prShareMem->u4FrameByte - g_prSpeechDev.m_rDLBuf.u4ChBufSz + g_prSpeechDev.m_u4DLRP);
				__flush_dcache_area((void*)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + g_prSpeechDev.m_u4DLRP), g_prSpeechDev.m_rDLBuf.u4ChBufSz - g_prSpeechDev.m_u4DLRP);
				__flush_dcache_area((void*)(g_prSpeechDev.m_rDLBuf.u4VirSAdr), g_prSpeechEnhance.m_prShareMem->u4FrameByte - g_prSpeechDev.m_rDLBuf.u4ChBufSz + g_prSpeechDev.m_u4DLRP);
			} else {
				memset_io((void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + g_prSpeechDev.m_u4DLRP),0,g_prSpeechEnhance.m_prShareMem->u4FrameByte);
				__flush_dcache_area((void*)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + g_prSpeechDev.m_u4DLRP), g_prSpeechEnhance.m_prShareMem->u4FrameByte);
			}
		}

        /*if(snd_chip->m_u4BtMute == 1)
        {
		...
		for(i = 0;(i < 2) &&(transforSrc[i] > 0);i++){
			if ((app_ptr_tmp + transforSrc[i]) >= btsubstrm_data->buffer_size) {
				...
				transforDest[1] = transforSrc[i] - transforDest[0];
				__flush_dcache_area((void*)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + u4Len), transforDest[0]);
				__flush_dcache_area((void*)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + u4Len + transforDest[0]), transforDest[1]);
                if(snd_chip->m_u4pcmDump)
                {
				...
				memcpy_fromio((btsubstrm_data->dma_start + app_ptr_tmp), (void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + u4Len),transforDest[0]);
				memcpy_fromio((btsubstrm_data->dma_start), (void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + u4Len + transforDest[0]),transforDest[1]);
			} 
			else{
				__flush_dcache_area((void*)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + u4Len), transforDest[i]);
                if(snd_chip->m_u4pcmDump)
                {
                    if (!IS_ERR(PCM_dump_rx))
                    {
				...
				memcpy_fromio((btsubstrm_data->dma_start + app_ptr_tmp), (void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + u4Len),transforSrc[i]);
			}
			u4Len = 0;
		}
		step_appptr(btruntime, g_prSpeechEnhance.m_prShareMem->u4FrameByte);
...
static void SpeechDev_DLPostProcess(const SPEECH_FRAME_T *prFrame)
{
	...
	if (!prFrame) {
		pr_err("[PCM ERR]SpeechDev_DLPostProcess: Error prFrame is NULL\r\n");
		return;
	}
...
static void SpeechDev_PreProcess(void)
{
	...
	while (fgProcessed && u4MaxLoop) {
		...
		if (u4UlLen >= (g_prSpeechDev.m_rMicBuf.u4ChBufSz - g_prSpeechEnhance.m_prShareMem->u4FrameByte)) {
			pr_err("[PCM ERR] UL Buffer is overflow. RP(%d), Size(%d) (%dms)\r\n",
		...
		if (8000 == g_prSpeechEnhance.m_prShareMem->u4SampleRate) {
			...
			if (u4DlLen >=
				...
				pr_err("[PCM ERR] DL Buffer is overflow. RP(%d), WP(%d) (%dms) \r\n",
			...
			if (u4DlLen2 >=
				(g_prSpeechDev.m_rDLBuf.u4ChBufSz - g_prSpeechEnhance.m_prShareMem->u4FrameByte)) {
				u4DlWP = BtPCMHw_GetDLWP();
				pr_err("[PCM ERR] 16K DL Buffer is overflow. ");
				pr_err("RP(%d), WP(%d) (%dms) \r\n", (s32)g_prSpeechDev.m_u4DLRP,
...
static void SpeechDev_ReadDLData(SPEECH_FRAME_T *prFrame)
{
	...
	if ((g_prSpeechDev.m_u4DLRP + g_prSpeechEnhance.m_prShareMem->u4FrameByte) > g_prSpeechDev.m_rDLBuf.u4ChBufSz) {
		...
		x_memcpy((void *)(&prFrame->DLBuf[u4Len >> 1]),
			...
			g_prSpeechEnhance.m_prShareMem->u4FrameByte - u4Len);*/
	//QK note
		memcpy_fromio((void *)prFrame->DLBuf,
			(void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + g_prSpeechDev.m_u4DLRP), u4Len);
		memcpy_fromio((void *)(&prFrame->DLBuf[u4Len >> 1]),
			(void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr),
			g_prSpeechEnhance.m_prShareMem->u4FrameByte - u4Len);
		...
		//x_memcpy((void *)prFrame->DLBuf, (void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + g_prSpeechDev.m_u4DLRP),
			//g_prSpeechEnhance.m_prShareMem->u4FrameByte);
		memcpy_fromio((void *)prFrame->DLBuf, (void *)(g_prSpeechDev.m_rDLBuf.u4VirSAdr + g_prSpeechDev.m_u4DLRP),
			g_prSpeechEnhance.m_prShareMem->u4FrameByte);
		g_prSpeechDev.m_u4DLRP += g_prSpeechEnhance.m_prShareMem->u4FrameByte;
		if (g_prSpeechDev.m_u4DLRP >= g_prSpeechDev.m_rDLBuf.u4ChBufSz) {
			g_prSpeechDev.m_u4DLRP -= g_prSpeechDev.m_rDLBuf.u4ChBufSz;
...
u32 SpeechDev_EnableULMute(bool fgEnable)
{
	...
	pr_debug("[PCM]SpeechDev_EnableULMute: fgEnable = %d\r\n", (s32)fgEnable);
...
s32 SpeechDev_GetSCOVolume(u32 *pu4LVolume, u32 *pu4RVolume)
{
	if ((NULL == pu4LVolume) || (NULL == pu4RVolume)) {
		pr_err("[PCM ERR]SpeechDev_GetSCOVolume: pu4LVolume or pu4RVolume is NULL!\r\n");
		return INVALIDPRAM;
	}
	*pu4LVolume = (g_prSpeechDev.m_u4SCOVolume & 0xFF00U) >> 8;
	*pu4RVolume = g_prSpeechDev.m_u4SCOVolume >> 24;
	pr_debug("[PCM]SpeechDev_GetSCOVolume: u4LVolume = %d, u4RVolume = %d\r\n",
...
s32 SpeechDev_SetSCOVolume(u32 u4LVolume, u32 u4RVolume)
{
	...
	pr_debug("[PCM]SpeechDev_SetSCOVolume: u4LVolume = %d, u4RVolume = %d\r\n",
	...
	pr_debug("[PCM]SpeechDev_SetSCOVolume: m_u4SCOVolume = 0x%08x\r\n",
...
{
	if ((NULL == pu4LVolume) || (NULL == pu4RVolume)) {
		pr_err("[PCM ERR]SpeechDev_GetDevVolume: pu4LVolume or pu4RVolume is NULL!\r\n");
		return INVALIDPRAM;
	}
	*pu4LVolume = (g_prSpeechDev.m_u4DevVolume & 0xFF00U) >> 8;
	*pu4RVolume = g_prSpeechDev.m_u4DevVolume >> 24;
	pr_debug("[PCM]SpeechDev_GetDevVolume: u4LVolume = %d, u4RVolume = %d\r\n",
		(s32)*pu4LVolume, (s32)*pu4RVolume);
...
s32 SpeechDev_SetDevVolume(u32 u4LVolume, u32 u4RVolume)
{
	u4LVolume = (u4LVolume > 255U) ? 255U : u4LVolume;
	u4RVolume = (u4RVolume > 255U) ? 255U : u4RVolume;
	pr_debug("[PCM]SpeechDev_SetDevVolume: u4LVolume = %d, u4RVolume = %d\r\n",
		(s32)u4LVolume, (s32)u4RVolume);
	g_prSpeechDev.m_u4DevVolume = (u4RVolume << 24) | (u4LVolume << 8);
	DeviceSPH_SetGain(g_prSpeechDev.m_u4DevVolume);
	pr_debug("[PCM]SpeechDev_SetDevVolume: m_u4DevVolume = 0x%08x\r\n",
...
s32 SpeechDev_GetEnhanceParam(PCM_SPEECH_CONF *prSpeechConf)
{
	if (NULL == prSpeechConf) {
		pr_err("[PCM ERR]SpeechDev_GetEnhanceParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	if (NULL == prSpeechConf) {
		pr_err("[PCM ERR]SpeechDev_SetEnhanceParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	if (NULL == prSpeechConf) {
		pr_err("[PCM ERR]SpeechDev_Get16KEnhanceParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	if (NULL == prSpeechConf) {
		pr_err("[PCM ERR]SpeechDev_SetEnhance16KParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	if (NULL == prSpeechConf) {
		pr_err("[PCM ERR]SpeechDev_SetDmnrParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	if (NULL == prSpeechConf) {
		pr_err("[PCM ERR]SpeechDev_SetComRxParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	if (NULL == prSpeechConf) {
		pr_err("[PCM ERR]SpeechDev_SetComTxParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	if (NULL == pData) {
		pr_err("[PCM ERR]SpeechDev_SetFilter16kParam: prSpeechConf error!\r\n");
		return INVALIDPRAM;
	}
...
{
	pr_debug("[PCM]SpeechDev_SetDLDelay: Set Delay %d Samples\r\n", (s32)u4Samples);
	if (u4Samples < DL_ENHANCE_SAMPLE) {
		pr_err("[PCM ERR]SpeechDev_SetDLDelay: Delay should be larger than %d\r\n",
			DL_ENHANCE_SAMPLE - 1);
		return INVALIDPRAM;
	}
...
{
	pr_debug("[PCM]SpeechDev_SetDL16KDelay: Set Delay %d Samples\r\n", (s32)u4Samples);
	if (u4Samples < DL_16K_ENHANCE_SAMPLE) {
		pr_err("[PCM ERR]SpeechDev_SetDL16KDelay: Delay should be larger than %d\r\n",
