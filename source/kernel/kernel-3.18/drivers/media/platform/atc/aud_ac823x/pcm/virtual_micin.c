...
static s32 CaptureStartAsrc(VirtualMicIn *pVMicIn)
{
	...
	pr_debug("[PCM] Capture ASRC Cfg: IFS(%d), OFS(%d), IBW(%d), OBW(%d), CH(%d)\r\n",
	...
	if(pVMicIn->u4AsrcIdx >= ASRC_CHSET_NUM) {
		if(NOERR != AsrcMgr_AllocASRC(&pVMicIn->rAsrcFmt, false, &pVMicIn->u4AsrcIdx)) {
			pr_err("[PCM ERR]CaptureStartAsrc: Alloc asrc err.\r\n");
			return (NORESOURCE);
		}
	}
...
void NdcVirtualMicIn_FsChange(u32 u4NewFs)
{
	...
	{
		pr_debug("[PCM]NdcVirtualMicIn_FsChange. VMIc is not started!\r\n");
		return NOERR;
	}
...
static int NdcVirtualMicInit(void)
{
    if (!_fgNdcBufInited)
    {
		...
		_rNdcDataBuf.u4Buf1 = (uintptr_t)vmalloc(_rNdcDataBuf.u4ChBufSz);
		if (0 == _rNdcDataBuf.u4Buf1)
		{
		    pr_err("[PCM ERR]CaptureNdcInit: vmalloc _rNdcDataBuf.u4Buf1 err!\r\n");
		    return (NORESOURCE);
		}
		_rNdcDataBuf.u4Buf2 = (uintptr_t)vmalloc(_rNdcDataBuf.u4ChBufSz);
		if (0 == _rNdcDataBuf.u4Buf2)
		{
		    pr_err("[PCM ERR]CaptureNdcInit: vmalloc _rNdcDataBuf.u4Buf2 err!\r\n");
		    return (NORESOURCE);
		}
    }
...
static void CopyMicDataToAsrcIn(VirtualMicIn *pVMicIn)
{
	...
	if(NOERR != Asrc_GetIBuf(pVMicIn->u4AsrcIdx, &rAsrcIn)) {
		pr_err("[PCM ERR]CopyMicDataToAsrcIn: Get ASRC input buffer error\r\n");
		return;
	}
	...
	{
		pr_debug("[PCM D] CopyMicDataToAsrcIn. MIC R(%d) W(%d) bz(%d), ASRC off(%d) sz(%d) bz(%d)\r\n",
...
s32 VirtualMic_ResetRP(void * pThis)
{
	...
	if (STATE_STARTED != pVMicIn->u4State )
	{
		pr_debug("[PCM]VirtualMic_ReseetRP. VMIc is not started!\r\n");
		return NOERR;
	}
...
s32 VirtualMic_Start(void * pThis)
{
    ASSERT(pThis);
	PVirtualMicIn pVMicIn = (PVirtualMicIn)pThis;
	pr_info("[PCM INFO]VirtualMic_Start\r\n");
	if (STATE_STARTED == pVMicIn->u4State )
	{
		pr_warn("[PCM INFO]VirtualMic_Start. It has been started!\r\n");
		return NOERR;
	}
	else if (STATE_UNINIT == pVMicIn->u4State)
	{
		pr_err("[PCM ERR]VirtualMic_Start error (invalid state)!\r\n");
		return (INVALIDSTATE);
	}
	else
	{
		...
		{
			pr_err("[PCM ERR] Failed to start MIC IN!\r\n");
			return (NORESOURCE);
		}
...
s32 VirtualMic_Stop(void * pThis)
{
    ASSERT(pThis);
	PVirtualMicIn pVMicIn = (PVirtualMicIn)pThis;
	pr_info("[PCM INFO]VirtualMic_Stop\r\n");
	if (STATE_STARTED != pVMicIn->u4State )
	{
		return (NOERR);
	}
...
s32 NdcVirtualMic_Start(void * pThis)
{
	...
	if (STATE_STARTED == pVMicIn->u4State )
	{
		pr_warn("[PCM INFO]NdcVirtualMic_Start. It has been started!\r\n");
		return NOERR;
	}
	else if (STATE_UNINIT == pVMicIn->u4State)
	{
		pr_err("[PCM ERR]NdcVirtualMic_Start error (invalid state)!\r\n");
		return (INVALIDSTATE);
	}
	else
	{
...
s32 NdcVirtualMic_Stop(void * pThis)
{
    ASSERT(pThis);
	PVirtualMicIn pVMicIn = (PVirtualMicIn)pThis;
	pr_info("[PCM INFO]NdcVirtualMic_Stop\r\n");
	if (STATE_STARTED != pVMicIn->u4State )
	{
		return (NOERR);
	}
    ...
    if (!_u4NdcCaptureTimes)
    {
		pr_warn("[PCM] NdcVirtualMic_Stop Mic is not started!!!\r\n");
		return (NOERR);
    }
    _u4NdcCaptureTimes --;
...
u32 VirtualMic_GetBuffer(void * pThis, WAVE_DATA_BUF_T *prBuffer)
{
	...
	else
	{
		...
		{
			pr_err("[PCM ERR]VirtualMic_GetBuffer:Get ASRC(%d) output buffer error\r\n", pVMicIn->u4AsrcIdx);
			return (INVALIDSTATE);
		}
...
s32 VirtualMic_Setup(void * pThis, u32 u4Fs)
{
	...
	if (STATE_STARTED == pVMicIn->u4State )
	{
		pr_err("[PCM ERR]VirtualMic_Setup error (invalid state)!\r\n");
		return INVALIDSTATE;
	}
	else
	{
	...
	pr_info("[PCM]VirtualMic_Setup. Fs(%d)!\r\n", pVMicIn->u4Fs);
	return NOERR;
}
...
s32 CreateVirtualMicIn(VIRTUAL_MIC_TYPE type, PVirtualMicIn *ppVMicIn)
{
	...
	{
		pr_err("[PCM ERR]CreateVirtualMicIn NDC Virtual is in used!!!\r\n");
		return (NORESOURCE);
		
	}
	*ppVMicIn = vmalloc(sizeof(VirtualMicIn));
	if(0 == *ppVMicIn) {
		pr_err("[PCM ERR]CreateVirtualMicIn: vmalloc failed!\r\n");
		return (NORESOURCE);
	}
	...
	pr_info("[PCM]CreateVirtualMicIn. this(0x%x)!\r\n", (u32)*ppVMicIn);
	return NOERR;
}
...
{
    ASSERT(pVMicIn);
	pr_info("[PCM INFO]DeleteVirtualMicIn. this(0x%x)!\r\n", (u32)pVMicIn);
