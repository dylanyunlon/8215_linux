...
typedef struct
{
    ...
    uintptr_t u4BufPhyAdr; //if allocate by hal layer, set to '0'
    u32 u4BufSz; 
...
#define AOUTLOG_ERR(exp)                AUDLOG(_u4AoutLog & ALOG_ERR,  (T("<***AUD_AOUT_ERR***>")), exp)
#define AOUTLOG_WARN(exp)               AUDLOG(_u4AoutLog & ALOG_WARN, (T("<AUD_AOUT_WARN>")), exp)
#define AOUTLOG_INFO(exp)               AUDLOG(_u4AoutLog & ALOG_INFO, (T("[AUD_AOUT]")), exp)
#define AOUTLOG_CLI(exp)                AUDLOG(_u4AoutLog & ALOG_CLI,  (T("[AUD_AOUT_CLI]")), exp)
#define AOUTLOG_DBG(exp)                AUDLOG(_u4AoutLog & ALOG_DBG,  (T("[AUD_AOUT]")), exp)

#define AOUTLOG_TEST(exp)               AUDLOG(_u4AoutLog & ALOG_TEST, (T("[AUD_AOUT_TEST]")), exp)
