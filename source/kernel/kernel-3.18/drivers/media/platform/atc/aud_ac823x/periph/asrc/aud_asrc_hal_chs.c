...
static void AsrcChs_BufferInit(PASRC_CHS_CLS prThis)
{
    ...
    pr_debug("QK [Chs(%d-%d)] Buf init: IN(0x%lx, 0x%lx, 0x%lx, 0x%x) OUT(0x%lx, 0x%lx, 0x%lx, 0x%x\r\n",
        prThis->prMgr->eType, (u32)(prThis->u4Idx),
        (uintptr_t)(prBuf->u4ISAdr), (uintptr_t)(prBuf->u4IVirSAdr[0]), (uintptr_t)(prBuf->u4IVirSAdr[1]), (u32)(prBuf->u4IChSize),
        (uintptr_t)(prBuf->u4OSAdr), (uintptr_t)(prBuf->u4OVirSAdr[0]), (uintptr_t)(prBuf->u4OVirSAdr[1]), (u32)(prBuf->u4OChSize));

    pr_debug("QK [Chs] OutRP(0x%x).\r\n", 
...
static u32 AsrcChs_SetIWP(void * pThis, u32 u4WP)
{
    ...
    prHw->SetIWP(prHw, prThis->u4Idx, (u32)(prThis->rBuf.u4ISAdr + u4WP));
...
static u32 AsrcChs_SetORP(void * pThis, u32 u4RP)
{
    ...
    prHw->SetORP(prHw, prThis->u4Idx, (u32)(prThis->rBuf.u4OSAdr + u4RP));
...
static void AsrcChs_LogAttribute(void * pThis)
{
    ...
    ASRCLOG_INFO((T("         ChBuf: SAdr(0x%x, 0x%x) IVir(0x%lx, 0x%lx) OVir(0x%lx, 0x%lx)..\r\n"),
        (uintptr_t)(prBuf->u4ISAdr), (uintptr_t)(prBuf->u4OSAdr), (uintptr_t)(prBuf->u4IVirSAdr[0]),
        (uintptr_t)(prBuf->u4IVirSAdr[1]), (uintptr_t)(prBuf->u4OVirSAdr[0]), (uintptr_t)(prBuf->u4OVirSAdr[1])));
}
