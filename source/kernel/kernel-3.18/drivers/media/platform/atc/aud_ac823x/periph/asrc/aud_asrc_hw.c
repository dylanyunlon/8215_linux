...
{   
    ...
    volatile uintptr_t IBufSAdr;
    volatile u32 IBufSize;
    volatile uintptr_t OBufSAdr;
    volatile u32 OBufSize;
    
    volatile uintptr_t IBufRp[6];
    volatile uintptr_t IBufWp[6];
    volatile uintptr_t OBufWp[6];
    volatile uintptr_t OBufRp[6];
    ...
    volatile uintptr_t IIRCramAddr;
...
static bool AsrcHw_SetIBufAddr(void * pThis, uintptr_t u4Addr, u32 u4ChBufSize)
{
    PASRC_HW_CLS prThis = (PASRC_HW_CLS)pThis;
    bool fgRet = TRUE;
    //uintptr_t u4BankAddr = (u4Addr >> 20);
	uintptr_t u4BankAddr = ((u4Addr&0xfff00000) >> 20);
    uintptr_t u4StartAddr = (u4Addr & 0xFFFFF);
    ...
    {    
    	pr_debug("QK AsrcHw_SetIBufAddr bank:0x%x,u4StartAddr:0x%x,u4ChBufSize:%d\n",u4BankAddr,u4StartAddr,u4ChBufSize);
    ...
    ASRCLOG_ERR_DBG(!fgRet, (T("[HW(%d)] IBufAddr: Bank(0x%lx) Start(0x%lx) ChSize(0x%x) \n"),
            prThis->eType, (uintptr_t)u4BankAddr, (uintptr_t)u4StartAddr, (u32)u4ChBufSize));
    
    return (fgRet);
}


static bool AsrcHw_SetOBufAddr(void * pThis, uintptr_t u4Addr, u32 u4ChBufSize)
{
    PASRC_HW_CLS prThis = (PASRC_HW_CLS)pThis;
    bool fgRet = TRUE;
    uintptr_t u4BankAddr = ((u4Addr&0xfff00000) >> 20);
    ...
    ASRCLOG_ERR_DBG(!fgRet, (T("[HW(%d)] OBufAddr: Bank(0x%x) Start(0x%lx) ChSize(0x%lx) \n"),
            prThis->eType, (uintptr_t)u4BankAddr, (uintptr_t)u4StartAddr, (u32)u4ChBufSize));
