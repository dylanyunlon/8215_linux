...
static u32 Aud_APLLCalibration(APLL_DOMAIN eAPLLDomain)
{
    ...
    do
    {
        ...
        {
            AudUtil_Delayus(200);
            if (IO_READ32(CKGEN_VIRT_N, MONITOR_PLLGP_STATUS) & BIT_AD_RGS_APLL294_VCOCAL_FAIL)
            {
                failN++;
                CKGEN_SETBITS(u4PwdReg, BIT_RG_APLL_PWD);
                kfail = 1;
            ...
            else if (IO_READ32(CKGEN_VIRT_N, MONITOR_PLLGP_STATUS) & BIT_AD_RGS_APLL294_VCOCAL_CPLT)
            {
                kfail = 0;
                break;
            }
