...
void AudAout_SetFRAoutMode(u32 u4Data);
...
REAR_AOUT_TYPE g_eRearOut = AOUT_DEFAULT;
...
void AudCfg_RestoreAoutRegs(void)
{
    ...
    if (prAout->IsArmCtrl(prAout))
    {
        ...
        prExtCfg->eOutPath = AOUT_FS;
        prAout->SetAoutPath(prAout, AOUT_FS);
        AudAout_SetFRAoutMode(g_eRearOut);
    }
}
...
bool AudAout_NewHalClass(AUD_AOUT_DEVID eAoutID)
{
    ...
    if(NULL == AudAout_GetHalClass(eAoutID))
    {
        if(AUDID_AOUT1 == eAoutID)
        {
            ...
            {
                LOG(LOG_DAC,_T("Audio New Aout1 Obj success, g_prAudAout1 0x%lx\r\n"), (uintptr_t)g_prAudAout1);
            }
        }
        else if(AUDID_AOUT2 == eAoutID)
        {
            ...
            {
                LOG(LOG_DAC,_T("Audio New Aout2 Obj success, g_prAudAout2 0x%lx\r\n"), (uintptr_t)g_prAudAout2);
            }
        }
        else
        {
...
void AudAout_DeInit(AUD_CLOCK_T eAoutClkId)
{
    if(AUD_CLK_AOUT1 == eAoutClkId)
    {
        if(NULL != g_prAudAout1)
        {
            LOG(LOG_CTRLF, _T("AudAout_DeInit eAoutClkId(%d), g_prAudAout1 0x%lx, g_prAudAout1->Delete 0x%lx\r\n\r\n"),
                eAoutClkId, (uintptr_t)g_prAudAout1, (uintptr_t)(g_prAudAout1->Delete));
    ...
    else if(AUD_CLK_AOUT2 == eAoutClkId)
    {
        if(NULL != g_prAudAout2)
        {
            LOG(LOG_CTRLF, _T("AudAout_DeInit eAoutClkId(%d), g_prAudAout2 0x%lx, g_prAudAout2->Delete 0x%lx\r\n\r\n"),
                eAoutClkId, (uintptr_t)g_prAudAout2, (uintptr_t)(g_prAudAout2->Delete));
...
void AudAout_SetFRAoutMode(u32 u4Data)
{
    PAOUT_HAL_CLS_PUB prOut = NULL;

    LOG(LOG_CTRLF, _T("Set rear Aout mode is %d.\r\n"), u4Data);
    switch(u4Data)
    {
    case AOUT_DEFAULT:
        g_eRearOut = AOUT_DEFAULT;
        prOut = AudAout_GetHalClass(AUDID_AOUT2);
        prOut->SetAoutPath(prOut, AOUT_RS);
        break;

    case AOUT_FRMR:
        g_eRearOut = AOUT_FRMR;
        prOut = AudAout_GetHalClass(AUDID_AOUT1);
        prOut->SetAoutPath(prOut, AOUT_RS);
        DspMixOut_SwitchOutPut(AUD_NORMAL_MODE);
        break;

        default:
            LOG(LOG_CTRLF, _T("Set rear out mode error.\r\n"));
            break;
    }
    return;
}
