...
static u32 LinHal_AllocBuf(PLIN_HAL_CLS prThis)
{
    ...
    LINLOG_INFO((T("LIN(%d) BUF INFO:  buf phy adr : 0x%lx, buf vir adr : 0x%lx \r\n"),
                                    prThis->eLinId, (uintptr_t)(prCfg->rBuf.u4PhySddr), (uintptr_t)(prCfg->rBuf.u4VirSAdr)));
...
#else
    eLinId = (VECTOR_SPDIF == u2Vector) ? AUDID_LIN1 : AUDID_LIN2;
...
#else
    u2Vector = (AUDID_LIN1 == prThis->eLinId) ? VECTOR_SPDIF : VECTOR_LIN2_RCINT;
