...
typedef struct
{
    ...
    uintptr_t u4BufPhyAdr; //if mic in buffer allocate by up layer,set this value; allocate by hal layer, set to '0'
    u32 u4BufSz; 
...
#define MICLOG_ERR(exp)                AUDLOG(_u4MicLog & ALOG_ERR,  (T("<***AUD_MIC_ERR***>")), exp)
#define MICLOG_WARN(exp)               AUDLOG(_u4MicLog & ALOG_WARN, (T("<AUD_MIC_WARN>")), exp)
#define MICLOG_INFO(exp)               AUDLOG(_u4MicLog & ALOG_INFO, (T("[AUD_MIC]")), exp)
#define MICLOG_CLI(exp)                AUDLOG(_u4MicLog & ALOG_CLI,  (T("[AUD_MIC_CLI]")), exp)
#define MICLOG_DBG(exp)                AUDLOG(_u4MicLog & ALOG_DBG,  (T("[AUD_MIC]")), exp)

#define MICLOG_TEST(exp)               AUDLOG(_u4MicLog & ALOG_TEST, (T("[AUD_MIC_TEST]")), exp)
