...
typedef struct
{
    ...
    uintptr_t u4BufPhyAdr; //set '0', if need mlne hal layer to allocate memory
    u32 u4BufSz; 
...
#define MLINLOG_ERR(exp)                AUDLOG(_u4MlinLog & ALOG_ERR,  (T("<***AUD_MLIN_ERR***>")), exp)
#define MLINLOG_WARN(exp)               AUDLOG(_u4MlinLog & ALOG_WARN, (T("<AUD_MLIN_WARN>")), exp)
#define MLINLOG_INFO(exp)               AUDLOG(_u4MlinLog & ALOG_INFO, (T("[AUD_MLIN]")), exp)
#define MLINLOG_CLI(exp)                AUDLOG(_u4MlinLog & ALOG_CLI,  (T("[AUD_MLIN_CLI]")), exp)
#define MLINLOG_DBG(exp)                AUDLOG(_u4MlinLog & ALOG_DBG,  (T("[AUD_MLIN]")), exp)

#define MLINLOG_TEST(exp)               AUDLOG(_u4MlinLog & ALOG_TEST, (T("[AUD_MLIN_TEST]")), exp)
