...
{
    AUDREG_BITS_W(REGENV_PCMTX_INTR_SAMPLE_NUM,
...
static void PcmHw_InitCfg(void * pThis, void * pCfg)
{
    ...
    uintptr_t u4BufSadr, u4TxSadr, u4TxEadr, u4RxSadr, u4RxEadr;
    uintptr_t u4TxBlkAdr, u4TxBankAdr, u4RxBlkAdr, u4RxBankAdr;
	...
	u4BufSadr = prCfg->rTxBuf.u4PhySddr;
    u4TxBankAdr = (u4BufSadr & 0xfff00000) >> 20;
    u4TxBlkAdr = 0;//(u4BufSadr & 0x0fff00) >> 8;
    u4TxSadr = (u4BufSadr & 0xffff0) >> 4;
    u4TxEadr = u4TxSadr +  (prCfg->rTxBuf.u4ChBufSz >> 4) - 1;

    u4BufSadr = prCfg->rRxBuf.u4PhySddr;
    u4RxBankAdr = (u4BufSadr & 0xfff00000) >> 20;
    u4RxBlkAdr = 0;//s(u4BufSadr & 0x0fff00) >> 8;
    u4RxSadr = (u4BufSadr & 0xffff0) >> 4;
    u4RxEadr = u4RxSadr +  (prCfg->rRxBuf.u4ChBufSz >> 4) - 1;
