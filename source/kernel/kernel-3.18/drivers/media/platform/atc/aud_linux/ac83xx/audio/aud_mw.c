...
static void AudSetHDMIInPcmInfo(AUD_DEC_AUD_INFO_T * pv_set_info)
{
    ...
    LOG(0, TEXT("[AUD_MW]HDMI LPCM setting.\n"));
...
{
    if(prContext->u1Output == AUD_FRONT)
    {
        prContext->ePlayType = AUD_OUT_MEDIA_NONE;
        fgAdspSetFrontAoutMediaType(prContext->ePlayType);
    }
    else if(prContext->u1Output == AUD_REAR)
    {
        prContext->ePlayType = AUD_OUT_MEDIA_NONE;
        fgAdspSetRearAoutMediaType(prContext->ePlayType);
    }
    else if(prContext->u1Output == AUD_FRONT_REAR)
    {
        prContext->ePlayType = AUD_OUT_MEDIA_NONE;
        fgAdspSetFrontAoutMediaType(prContext->ePlayType);
        fgAdspSetRearAoutMediaType(prContext->ePlayType);
    }
	prContext->u1Output= AUD_FRONT_REAR;
