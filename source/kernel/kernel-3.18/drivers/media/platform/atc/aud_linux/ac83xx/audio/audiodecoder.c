...
u32 ADE_Init(s8 *pszContext)
{
    ...
    LOG(LOG_OTHER, TEXT("[ADE_Init]ADE_Init... \n"));
...
bool ADE_Close(u32 dwContext)
{
    ...
    LOG(LOG_OTHER, TEXT("[ADE_Close]ADE_Close u1DecId:0x%x\n"), (u32)pContext->u1DecId);

    if ((PRI_DEC == pContext->u1DecId || SEC_DEC == pContext->u1DecId) && pContext->fgEnPlay)
    {
        ...
        if (PRI_DEC == pContext->u1DecId) {
             if(AUD_OUT_MEDIA_USB == u1AdspGetFrontAoutType())//play usb
             {
                 LOG(LOG_DUALCTRL, TEXT("[ADE_Close]clear front mediatype. \n"));
                 fgAdspSetFrontAoutMediaType(AUD_OUT_MEDIA_NONE);
                 AudSetDecContext(AUD_OUT_MEDIA_NONE, pContext, AUD_FRONT);
             }
        } else if (SEC_DEC == pContext->u1DecId) {
             if (pContext->fgAvinHwOn == TRUE) {
                 LOG(LOG_CTRLF, TEXT("[ADE_Close_StopDec] SEC_DEC, AvinHwOn will Hw Off!\r\n"));
                 //close audio in hw
                 AUDIN_SET_ONOFF rLinInCmd;
                 rLinInCmd.lMode = LIN_DRAM_FRONT;
                 rLinInCmd.fgAudInOnOff = LIN_OFF;
                 rLinInCmd.eLineINGroupSel = AUD_AADC_LINEIN_INPUT_GROUP1;
                 Aud_Linein_SetCtrl(&rLinInCmd, pContext->u1DecId);
             }
             //clear media type
             if ((pContext->ePlayType == AUD_OUT_MEDIA_LINE_IN) && (u1AdspGetFrontAoutType() == AUD_OUT_MEDIA_LINE_IN)) {
                 LOG(LOG_CTRLF, TEXT("[ADE_Close_StopDec] Media type is %d, will Clear!\r\n"), pContext->ePlayType);
                 fgAdspSetFrontAoutMediaType(AUD_OUT_MEDIA_NONE);
                 pContext->u1Output = (UINT8)AUD_OUT_MAX;
                 pContext->ePlayType = AUD_OUT_MEDIA_NONE;
                 pContext->fgEnPlay = FALSE;
                 pContext->u1DecId = (UINT8)MAX_AUDDRV_NUM;
             }

        }

        if(AUD_OUT_MEDIA_USB == u1AdspGetRearAoutType())//play usb
        {
            LOG(LOG_DUALCTRL, TEXT("[ADE_Close]clear rear mediatype. \n"));
            fgAdspSetRearAoutMediaType(AUD_OUT_MEDIA_NONE);
            AudSetDecContext(AUD_OUT_MEDIA_NONE, pContext, AUD_FRONT);
        }
    }
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIO_FEATURE_SUPPORT:
        ...
        if ((NULL != pInBuffer) && (NULL != pOutBuffer))
        {
            ...
            *pOutBuffer = (u8)fgAudFeatureSupport(u4feature);
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIO_SEND_BUFFER:
        ...
        else
        {
            ...
            #endif
            {
                fgRet = AudEsm_SendBufferInfo(pContext->u1DecId, (AUD_SEND_BUF_INFO *)pInBuffer);
                if (FALSE == fgRet)
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIO_GET_FRONT_TYPE:
        ...
        if (pOutBuffer != NULL && outSize == sizeof(u8))
        {
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIN_SET_ONOFF:
        ...
        if (NULL != pInBuffer)
        {
            ...
            if (TRUE == Aud_Linein_SetCtrl(prLinInCmd, pContext->u1DecId)) {
                if (prLinInCmd->fgAudInOnOff == LIN_ON) {
                    pContext->fgAvinHwOn = TRUE;
                } else {
                    pContext->fgAvinHwOn = FALSE;
                }
            }
                  ...
                  u8 *pOutBuffer, u32 outSize, u32 *pOutSize)
{
    ...
    case IOCTL_AUDIN_COPY_FROM_USER:
        ...
        if (NULL != pInBuffer && inSize == sizeof(AUD_USER_INFO))
        {
            ...
            LOG(LOG_IO, TEXT("pContext->u4AfifoWPtr 0x%x, pContext->u4AfifoSA 0x%x, prUser_Info->puser 0x%x, size 0x%x\n"),
            ...
            LOG(LOG_IO, TEXT("***********IOCTL_AUDIN_COPY_FROM_USER success success***********\n"));
        }
        else
        {
