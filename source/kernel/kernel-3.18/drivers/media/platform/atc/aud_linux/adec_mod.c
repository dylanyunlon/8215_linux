...
static s32 audiodecoder_probe(struct platform_device *pdev)
{
	...
	phys_addr_t afifom_base, afifom_size;
...
#if 0 // afifo dynamic change to static
...
#else
	result = get_static_reserved_memory("afifo", &afifom_base, &afifom_size);
	if (0 != result)
		LOG(LOG_FAIL, "node not exist or not a static reserved memory node\n");

	LOG(LOG_INFO, "get afifo reserve memory : base 0x%x, size 0x%x\r\n", afifom_base, afifom_size);
#endif
	...
	adec_dev->afifom_base = afifom_base; //adec_mem->base;
	adec_dev->afifom_size = afifom_size; //adec_mem->size;
	...
	if (result != 0) {
	    LOG(LOG_FAIL, "audio decoder probe fail because of misc_register, error = %d\r\n\n", result);
...
static s32 __init adec_init(void)
{
    ...
    if (!node)
    {
        LOG(LOG_INFO, "audio decoder init fail in find dts compatible node!!\r\r\n");
        result = -ENOMEM;
        goto err_node;
    }
