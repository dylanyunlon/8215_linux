...
{
	PCM_DEBUG(LOG_TAG, "Asrc_UnInit: Index is %d!\r\n", (u32)u4ChIdx);
	if ((asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->Delete)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_UnInit: NULL Pointer, Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
{
	PCM_DEBUG(LOG_TAG, "Asrc_Setup: Index is %d. Chs(0x%x)!\r\n", (u32)u4ChIdx, (u32)pvAsrcChs);
	if (pvAsrcChs) {
		...
		x_memset((void *)(asrc_list[u4ChIdx].m_rIBuf.u4Buf1), 0, rInBuf.u4ChBufSz);
		x_memset((void *)(asrc_list[u4ChIdx].m_rIBuf.u4Buf2), 0, rInBuf.u4ChBufSz);
		...
		x_memset((void *)(asrc_list[u4ChIdx].m_rOBuf.u4Buf1), 0, rOutBuf.u4ChBufSz);
		x_memset((void *)(asrc_list[u4ChIdx].m_rOBuf.u4Buf2), 0, rOutBuf.u4ChBufSz);
...
s32 Asrc_Start(u32 u4ChIdx)
{
	if ((asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.Start)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_Start: NULL Pointer, Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
s32 Asrc_Stop(u32 u4ChIdx)
{
	if ((asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.Stop)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_Stop: NULL Pointer, Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}

	asrc_list[u4ChIdx].m_u4IWP = Asrc_GetIRP(u4ChIdx);
	asrc_list[u4ChIdx].m_u4ORP = Asrc_GetOWP(u4ChIdx);
...
s32 Asrc_SetIWP(u32 u4ChIdx, u32 u4WP)
{
	if ((asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.SetIWP)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_SetIWP: NULL Pointer, Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
s32 Asrc_SetORP(u32 u4ChIdx, u32 u4RP)
{
	...
	if ((asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.SetORP)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_SetORP: NULL Pointer, Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
s32 Asrc_GetIDataSz(u32 u4ChIdx, u32 *pu4Size)
{
	...
	if (pu4Size && (asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.GetIRP)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_GetIDataSz: NULL Pointer, Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
s32 Asrc_GetODataSz(u32 u4ChIdx, u32 *pu4Size)
{
	...
	if (pu4Size && (asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.GetOWP)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_GetODataSz: NULL Pointer, Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
s32 Asrc_GetIBuf(u32 u4ChIdx, PWAVE_DATA_BUF_T prBuf)
{
	...
	if ((!asrc_list[u4ChIdx].m_prAsrcChs) || (!asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.GetIRP)) {
		PCM_ERROR(LOG_TAG, "Asrc_GetIBuf: NULL Pointer Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
s32 Asrc_GetOBuf(u32 u4ChIdx, PWAVE_DATA_BUF_T prBuf)
{
	if ((asrc_list[u4ChIdx].m_prAsrcChs) && (asrc_list[u4ChIdx].m_prAsrcChs->rHwIf.GetOWP)) {
		...
		PCM_ERROR(LOG_TAG, "Asrc_GetOBuf: NULL Pointer Index is %d!\r\n", (u32)u4ChIdx);
		return INVALIDPRAM;
	}
...
s32 AsrcMgr_Init(void)
{
	...
	if (g_rAsrcMgr.m_prAsrcMgr) {
		...
		PCM_ERROR(LOG_TAG, "AsrcMgr_Init: New Asrc Mgr Error!\r\n");
		return NORESOURCE;
	}
...
s32 AsrcMgr_AllocASRC(PASRC_CHS_FMT_T prChCfg, bool fgSpeech, u32 *pu4Idx)
{
	...
	if ((prChCfg->u4OFS < 8000) || (prChCfg->u4OFS > 48000) ||
		...
		(prChCfg->u4Chn > 2)) {
		PCM_ERROR(LOG_TAG, "AsrcMgr_AllocASRC: Fmt error: OFS(%d) IFS(%d) Chn(%d)\r\n",
			(u32)prChCfg->u4OFS, (u32)prChCfg->u4IFS, (u32)prChCfg->u4Chn);
		return INVALIDPRAM;
	}

	if (fgSpeech) {
		PCM_INFO(LOG_TAG, "AsrcMgr_AllocASRC: Modify FixPalette sample rate to %d\r\n",
	...
	if (ASRC_CHSET_NUM == u4Idx) {
		PCM_ERROR(LOG_TAG, "AsrcMgr_AllocASRC: No free ASRC\r\n");
		return NORESOURCE;
	}
	...
	if (pvChs) {
		...
		PCM_ERROR(LOG_TAG, "AsrcMgr_AllocASRC: AsrcMgr No free ASRC\r\n");
		return NORESOURCE;
	}
	PCM_DEBUG(LOG_TAG, "AsrcMgr_AllocASRC index(%d) pvChs(0x%x) \r\n", u4Idx, (u32)pvChs);
