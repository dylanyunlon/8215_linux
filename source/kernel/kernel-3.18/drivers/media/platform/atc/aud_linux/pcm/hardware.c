...
void Out_InterruptThread(u32 u4UsedBytes)
{
	...
	if (STATE_STARTED != snd_chip->m_pbu4State) {
		PCM_ERROR(LOG_TAG, "Out_InterruptThread: m_pbu4State = %d != STATE_STARTED\r\n",
			(s32)snd_chip->m_pbu4State);
		return;
	}
	if ((u4CurTime - _u4OutProc) >= (snd_chip->m_u4IntrTime << (u32)1))
		PCM_INFO(LOG_TAG, "Out_InterruptThread: Process Interval(%d ms).\r\n",
	...
	if ((u4CurTime - _u4OutProc) >= (snd_chip->m_u4IntrTime >> (u32)1))
		PCM_INFO(LOG_TAG, "Out_InterruptThread: Process time(%d ms).\r\n",
			(s32)(u4CurTime - _u4OutProc));
	_u4OutProc = u4CurTime;
}
...
s32 HardWare_Init(ac_83xx *chip)
{
	...
	PCM_DEBUG(LOG_TAG, "HardWare_Init: Start\r\n");
	if (chip->m_Intialized) {
		PCM_ERROR(LOG_TAG, "HardWare_Init: HardWare is Initialized\r\n");
		return INVALIDSTATE;
	}
	...
	if (chip->m_capu4State == STATE_UNINIT) {
		err = MicIn_Init();
		if (err < 0) {
			PCM_ERROR(LOG_TAG, "HardWare_Init: MicIn_Init fail\r\n");
			MicIn_UnInit();
			return NORESOURCE;
		}
	...
	err = SpeechDev_Init();
	if (err < 0) {
		PCM_ERROR(LOG_TAG, "HardWare_Init: SpeechDev_Init fail\r\n");
		SpeechDev_UnInit();
		return NORESOURCE;
	}
	...
	if (chip->m_pbu4State == STATE_UNINIT) {
		err = DspMixOut_Init();
		if (err < 0) {
			PCM_ERROR(LOG_TAG, "HardWare_Init: DspMixOut_Init fail\r\n");
			DspMixOut_UnInit();
			return NORESOURCE;
		}
	...
	if (AsrcMgr_Init() < 0) {
		PCM_ERROR(LOG_TAG, "HardWare_Init: AsrcMgr_Init fail\r\n");
		AsrcMgr_UnInit();
		return NORESOURCE;
	}
	...
	PCM_DEBUG(LOG_TAG, "HardWare_Init: success!\r\n");
...
s32 GetStreamProcess(struct snd_pcm_substream  *substream)
{
	...
	if (prTmp) {
		PCM_DEBUG(LOG_TAG, "GetStreamProcess: return 0x%x.\r\n", (u32)prTmp);
	...
	if (NOERR != AttachStream(substream, prTmp)) {
		PCM_ERROR(LOG_TAG, "GetStreamProcess: AttachStream fail\r\n");
		kfree(prTmp);
	...
	PCM_DEBUG(LOG_TAG, "GetStreamProcess: Success with index(%d)\r\n",
		(u32)prTmp->m_u4Idx);
