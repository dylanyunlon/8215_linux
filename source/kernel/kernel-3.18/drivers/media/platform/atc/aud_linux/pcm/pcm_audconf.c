...
#define SPEECH_MIC_GAIN				40U  /* 40-14=26dB */
...
s32 pcm_audconf_set(void)
{
    ...
    g_EnhanceRecordDelaySample = -156;//rPCMConf.ENH_REC_DELAY_SAMPLE;//cgx todo
    g_NormalRecordWithEnhance = 0;//rPCMConf.ENH_REC_NORMAL_EN;//cgx todo
...
s32 pcm_audconf_ioctl(u32 context, u32 code, void *pBuffer, u32 dwCount)
{
	...
	PCM_DEBUG(LOG_TAG, "cmd = 0x%02x\r\n", (code - SET_SPH_DELAY) >> 2);
	if (NULL == pBuffer) {
		PCM_ERROR(LOG_TAG, "pBuffer is NULL\r\n");
		return -1;
	}

	switch (code) {
	case SET_SPH_DELAY:
		PCM_DEBUG(LOG_TAG, "SET_SPH_DELAY\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_SPH_DELAY count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_SPH_DELAY:
		PCM_DEBUG(LOG_TAG, "GET_SPH_DELAY\r\n");
		PCM_DEBUG(LOG_TAG, "8K speech has delayed %d samples\r\n",
			(s32)SpeechDev_GetDLDelay());
		*((u32 *)pBuffer) = SpeechDev_GetDLDelay();
		break;

	case SET_SPH_16K_DELAY:
		PCM_DEBUG(LOG_TAG, "SET_SPH_16K_DELAY\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_SPH_16K_DELAY count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_SPH_16K_DELAY:
		PCM_DEBUG(LOG_TAG, "GET_SPH_16K_DELAY\r\n");
		PCM_DEBUG(LOG_TAG, "16K speech has delayed %d samples\r\n",
			(s32)SpeechDev_GetDL16KDelay());
		*((u32 *)pBuffer) = SpeechDev_GetDL16KDelay();
		break;

	case SET_DEVICE_SPH_GAIN:
		PCM_DEBUG(LOG_TAG, "SET_DEVICE_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			PCM_ERROR(LOG_TAG, "SET_DEVICE_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_DEVICE_SPH_GAIN:
		PCM_DEBUG(LOG_TAG, "GET_DEVICE_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			PCM_ERROR(LOG_TAG, "GET_DEVICE_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_BT_SPH_GAIN:
		PCM_DEBUG(LOG_TAG, "SET_BT_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			PCM_ERROR(LOG_TAG, "SET_BT_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_BT_SPH_GAIN:
		PCM_DEBUG(LOG_TAG, "GET_BT_SPH_GAIN\r\n");
		if (dwCount < sizeof(PCM_VOLUME)) {
			PCM_ERROR(LOG_TAG, "GET_BT_SPH_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_MIC_MUTE:
		PCM_DEBUG(LOG_TAG, "SET_MIC_MUTE\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_MIC_MUTE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_SPH_MIC_GAIN:
		PCM_DEBUG(LOG_TAG, "SET_SPH_MIC_GAIN\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_SPH_MIC_GAIN count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_SPH_MIC_GAIN:
		PCM_DEBUG(LOG_TAG, "GET_SPH_MIC_GAIN\r\n");
		*((u32 *)pBuffer) = MicIn_GetGain();
		break;

	case SET_DMNR_PARAM:
		PCM_DEBUG(LOG_TAG, "SET_DMNR_PARAM\r\n");
		break;

	case SET_AEC_NDC_PARAM:
		PCM_DEBUG(LOG_TAG, "SET_AEC_NDC_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_AEC_NDC_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_AEC_NDC_PARAM:
		PCM_DEBUG(LOG_TAG, "GET_AEC_NDC_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_CONF)) {
			PCM_ERROR(LOG_TAG, "GET_AEC_NDC_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_AEC_NDC_16K_PARAM:
		PCM_DEBUG(LOG_TAG, "SET_AEC_NDC_16K_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_16K_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_AEC_NDC_16K_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case GET_AEC_NDC_16K_PARAM:
		PCM_DEBUG(LOG_TAG, "GET_AEC_NDC_16K_PARAM\r\n");
		if (dwCount < sizeof(PCM_SPEECH_16K_CONF)) {
			PCM_ERROR(LOG_TAG, "GET_AEC_NDC_16K_PARAM count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case BT_SCO_ENABLE:
		...
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "BT_SCO_ENABLE Parameter count err!\r\n");
			return INVALIDPRAM;
		}
		PCM_INFO(LOG_TAG, "BT_SCO_ENABLE SampleRate(%d)\r\n", *((u32 *)pBuffer));
#if 0		
		if (SpeechDev_IsSCOEnable() != true) {
			PCM_DEBUG(LOG_TAG, "BT_SCO TO ENABLED\r\n");
			SpeechDev_EnableSCO(true, *((u32 *)pBuffer));
		} else {
			PCM_DEBUG(LOG_TAG, "BT_SCO has ENABLED\r\n");
		}
#endif		
        SpeechDev_Enable(TRUE, TRUE, *((u32 *)pBuffer));
		PCM_INFO(LOG_TAG, "BT_SCO_ENABLE End\r\n");
		break;

	case BT_SCO_DISABLE:
		PCM_INFO(LOG_TAG, "BT_SCO_DISABLE\r\n");
        SpeechDev_Enable(FALSE, TRUE, 8000);
#if 0		
		if (SpeechDev_IsSCOEnable() != false) {
			PCM_DEBUG(LOG_TAG, "BT_SCO TO DISABLE\r\n");
			SpeechDev_EnableSCO(false, 8000);
		} else {
			PCM_DEBUG(LOG_TAG, "BT_SCO has DISABLE\r\n");
		}
#endif		
		PCM_INFO(LOG_TAG, "BT_SCO_DISABLE End\r\n");
		break;

	case SET_DL_USE_FILE:
		PCM_INFO(LOG_TAG, "SET_DL_USE_FILE\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_DL_USE_FILE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_UL_USE_FILE:
		PCM_INFO(LOG_TAG, "SET_UL_USE_FILE\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_UL_USE_FILE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_BT_SPH_PLC:
		PCM_INFO(LOG_TAG, "SET_BT_SPH_PLC\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_BT_SPH_PLC count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_DSP_MIX_CH:
		PCM_INFO(LOG_TAG, "SET_DSP_MIX_CH\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_DSP_MIX_CH count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_BT_SPH_DUMP:
		PCM_DEBUG(LOG_TAG, "SET_BT_SPH_DUMP\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_BT_SPH_DUMP count err!\r\n");
			return INVALIDPRAM;
		}
		SpeechDev_EnableDump(*((bool *)pBuffer));
        g_EnhRecordDump = *(BOOL *)pBuffer;
        g_ForceEnableEnhance = *(BOOL *)pBuffer;
		break;

	case SET_PRIMARY_MIC:
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_PRIMARY_MIC count err!\r\n");
			return INVALIDPRAM;
		}
		PCM_INFO(LOG_TAG, "SET_PRIMARY_MIC (%d)\r\n", *(u32 *)pBuffer);
		MicIn_SetPrimaryMic(*((u32 *)pBuffer));
		break;

	case SET_PCM_LOG:
		PCM_DEBUG(LOG_TAG, "SET_PCM_LOG\r\n");
		if (dwCount < sizeof(u32)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_LOG count err!\r\n");
			return INVALIDPRAM;
		}
		break;
	case SET_CAPTURE_NDC_ENABLE:
		PCM_INFO(LOG_TAG, "SET_CAPTURE_NDC_ENABLE\r\n");
		Capture_NdcEnable(*((bool *)pBuffer));
		break;

#if (ENABLE_DTMF_FUNCTION)
	case SET_PCM_DTMF_CTRL:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_CTRL\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_DTMF_CTRL count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_THRESHOLD:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_THRESHOLD\r\n");
		if (dwCount < sizeof(PCM_DTMF_THRESHOLD)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_DTMF_THRESHOLD count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_NOISE_RATIO:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_NOISE_RATIO\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_DTMF_NOISE_RATIO count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_VALID_TIME:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_VALID_TIME\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_DTMF_VALID_TIME count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_INVALID_TIME:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_INVALID_TIME\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "DTMF_INVALID_TIME count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_NEW_SAMPLES:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_NEW_SAMPLES\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_DTMF_NEW_SAMPLES count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_MAX_SCALE:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_MAX_SCALE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_DTMF_MAX_SCALE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_INFO_SENDER_EN:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_INFO_SENDER_EN\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "DTMF_INFO_SENDER_EN count err!\r\n");
			return INVALIDPRAM;
		}
		PCM_DEBUG(LOG_TAG, "[68031]SENDER_EN (%d, %d)\r\n",
	...
	case SET_PCM_DTMF_INFO_SENDER_WRITE:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_INFO_SENDER_WRITE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "DTMF_INFO_SENDER_WRITE count err!\r\n");
			return INVALIDPRAM;
		}
		if (((PCM_DTMF_CONF *)pBuffer)->u4Param1)
	...
	case GET_PCM_DTMF_PARAM:
		PCM_DEBUG(LOG_TAG, "GET_PCM_DTMF_PARAM\r\n");
		Dtmf_LogParams();
		break;

	case SET_PCM_DTMF_LOG_RANGE:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_LOG_RANGE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "SET_PCM_DTMF_LOG_RANGE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	case SET_PCM_DTMF_TEST_USE_FILE:
		PCM_DEBUG(LOG_TAG, "SET_PCM_DTMF_TEST_USE_FILE\r\n");
		if (dwCount < sizeof(PCM_DTMF_CONF)) {
			PCM_ERROR(LOG_TAG, "DTMF_TEST_USE_FILE count err!\r\n");
			return INVALIDPRAM;
		}
	...
	default:
		i4Ret = -1;
		PCM_DEBUG(LOG_TAG, "UNKNOWN CMD, cmd=%04x, u4Ret=%d\r\n", code, i4Ret);
		break;
	}
