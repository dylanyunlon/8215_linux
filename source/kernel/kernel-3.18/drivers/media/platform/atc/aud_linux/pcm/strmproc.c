...
#define SAT_8_BIT(a)				(s16)((a) > 0x7f ? 0x7f : (a) < -128 ? -128 : (a))
...
static u32 m_dwGain = (u32)MAX_GAIN;
static u32 m_dwBTGain = (u32)MAX_GAIN;
...
#else
	m_fxpBTGain[0] = (m_dwBTGain & 0xFFFFU);
	m_fxpBTGain[1] = (m_dwBTGain >> 16);
	m_dwBTGainShift = 16U;
#endif
...
#else
	m_fxpGain[0] = ((m_dwGain & 0xFF00U) >> 8) * 256U;
	m_fxpGain[1] = (m_dwGain >> 24) * 256U;
	m_dwGainShift = 16U;
#endif
...
static void NotifyPeriodElapsed(StreamProcess *strmProc, u32 frames);
...
u32 StateChangeInform(const struct snd_pcm_substream *substream)
{
	...
	if (SNDRV_PCM_TRIGGER_START == substrm_data->m_eState) {
		if ((STATE_INITED == strmProc->m_u4State) ||
		        (STATE_STOPPED == strmProc->m_u4State)) {
		...
		if (0/*STREAM_SAMPLING_RATE_16K == strmProc->m_rInFmt.u4FS*/) {//The volume of 16K is too low when there is no setting from userspace.
			m_fade[0].fade_gain[0] = 0;
			m_fade[0].fade_gain[1] = 0;
		} else {
			m_fade[1].fade_gain[0] = 0;
			m_fade[1].fade_gain[1] = 0;
		}
	} else {
...
s32 DetachStream(struct snd_pcm_substream *substream)
{
	...
	PCM_DEBUG(LOG_TAG, "DetachStreamProc: Start to detach.\r\n");
	...
	PCM_DEBUG(LOG_TAG, "DetachStream: end.\r\n");
...
s32 EventInform(u32 u4Event, u32 u4Param, StreamProcess *strmProc)
{
	...
	case EVT_UPDATE_PLAYED_SAMPLE:
		if (STATE_STARTED == strmProc->m_u4OState) {
			...
			NotifyPeriodElapsed(strmProc, u4Tmp);
		}
		break;
...
static void StrmFadePrepare(virout_t *this, u32 left_gain, u32 right_gain)
{
	if (this)
	{
		this->gain[0] = left_gain;
		this->gain[1] = right_gain;
		if (left_gain > this->fade_gain[0]) {
			this->fade_state = FADE_IN;
			this->fade_step[0] = (left_gain - this->fade_gain[0]) / OUTHW_FADE_SAMPLES;
			this->fade_step[1] = (right_gain - this->fade_gain[1]) / OUTHW_FADE_SAMPLES;
		} else {
			this->fade_state = FADE_OUT;
			this->fade_step[0] = (this->fade_gain[0] - left_gain) / OUTHW_FADE_SAMPLES;
			this->fade_step[1] = (this->fade_gain[0] - right_gain) / OUTHW_FADE_SAMPLES;
		}
	}
}

static void StrmFadeProc(virout_t *this, buf_trans_t *trans)
{
	if (this && trans && trans->size)
	{
		//Accuracy will be lost to some extent
		if (trans->bitsPerSample == 8) {
			u32 samples = trans->size;
			s32 data = 0;
			u8 *src = (u8 *)trans->src[0];

			while (samples) {
				data = *src - 128;
				data = (data * this->fade_gain[0]) >> trans->vol_shift;
				*src = (SAT_8_BIT(data) + 128);
				src++;

				if (trans->channels == 2) {
					data = *src - 128;
					data = (data * this->fade_gain[1]) >> trans->vol_shift;
					*src = (SAT_8_BIT(data) + 128);
					src++;
				}

				if (this->fade_state == FADE_IN) {
				if((this->fade_gain[0] + this->fade_step[0]) <= this->gain[0]){
						this->fade_gain[0] += this->fade_step[0];
				}
				if((this->fade_gain[1] + this->fade_step[1]) <= this->gain[1]){
						this->fade_gain[1] += this->fade_step[1];
				}
				if (this->fade_gain[0] >= this->gain[0]) {
						this->fade_state = FADE_DONE;
					} else if (this->fade_step[0] == 0) {
						this->fade_gain[0] ++;
						this->fade_gain[1] ++;
				}
				} else if (this->fade_state == FADE_OUT) {
				if(((this->fade_gain[0] - this->fade_step[0]) >= this->gain[0]) && ((this->fade_gain[0] - this->fade_step[0]) >= 0)){
						this->fade_gain[0] -= this->fade_step[0];
				    }
				if(((this->fade_gain[1] - this->fade_step[1]) >= this->gain[1]) && ((this->fade_gain[1] - this->fade_step[1]) >= 0)){
						this->fade_gain[1] -= this->fade_step[1];
				    }
				if (this->fade_gain[0] <= this->gain[0]) {
						this->fade_state = FADE_DONE;
					} else if((this->fade_step[0] == 0) && (this->fade_gain[0] > 0)) {
						this->fade_gain[0] --;
						this->fade_gain[1] --;
					}
				}
...
static void ProcessInput(StreamProcess *strmProc)
{
	...
	buf_trans_t buf_trans;
	virout_t *m_fade_proc;
	...
	if (0/*STREAM_SAMPLING_RATE_16K == strmProc->m_rInFmt.u4FS*/) {//The volume of 16K is too low when there is no setting from userspace.
		...
		StrmFadePrepare(&m_fade[0], dwLGain, dwRGain);
		m_fade_proc = &m_fade[0];
	} else {
		dwLGain = m_fxpGain[0];
		dwRGain = m_fxpGain[1];
		dwGainShift = m_dwGainShift;
		StrmFadePrepare(&m_fade[1], dwLGain, dwRGain);
		m_fade_proc = &m_fade[1];
	}
	...
	while (u4AvaiBytes) {
		...
		if (rStrmData.u4BitsPerSample == 8) {
			...
			buf_trans.channels = rStrmData.u4Chn;
			buf_trans.size = u4Size;
			buf_trans.src[0] = pbData;
			buf_trans.vol_shift = dwGainShift;
			buf_trans.bitsPerSample = rStrmData.u4BitsPerSample;
			StrmFadeProc(m_fade_proc, &buf_trans);
...
static void NotifyPeriodElapsed(StreamProcess *strmProc, u32 frames)
{
	...
	step_pb_real_hwptr(runtime, frames);
