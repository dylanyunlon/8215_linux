	       ...
	       VOID *lpOutBuf, DWORD OutBufSize , DWORD *lpBytesReturned)
{
	...
	context = (DWORD)Audiofilp->private_data;
...
{
	AUDIO_DECODER *pADec;
	AUD_OUT_MEDIA_TYPE_T eFrontSrcType;
	eFrontSrcType = AUD_OUT_MEDIA_NONE;

#ifndef ATC_AOSP_ENHANCEMENT_AUDIO
	AUD_OUT_MEDIA_TYPE_T eSrcType;
	eSrcType = AUD_OUT_MEDIA_NONE;
#else
	...
	/*AUD_OUT_MEDIA_TYPE_T eRearSrcType  = AUD_OUT_MEDIA_NONE;*/


	if (NULL == pADec) {
		pr_info("[AVIN][hdmi_adec]%s param pADec is null\r\n", __func__);
		return;
	}

	pADec->eAudCmdStatus = AUD_CMD_STATUS_IDLE;

	AudioIoCtl(pADec->AudioFilp, IOCTL_AUDIO_GET_FRONT_TYPE, NULL, 0, &eFrontSrcType, sizeof(UINT8), NULL);

#ifndef ATC_AOSP_ENHANCEMENT_AUDIO

	if (AUD_OUT_MEDIA_USB == eFrontSrcType) {
		AudioIoCtl(pADec->AudioFilp, IOCTL_AUDIO_SET_FRONT_MEDIA_TYPE,
			&eSrcType, sizeof(eSrcType), NULL, 0, NULL);
	}
#else
...
#endif
	/*
	AudioIoCtl(pADec->AudioFilp, IOCTL_AUDIO_GET_REAR_TYPE,  NULL, 0, &eRearSrcType,  sizeof(UINT8), NULL);
	if (AUD_OUT_MEDIA_USB == eRearSrcType)
	{
	AudioIoCtl(pADec->AudioFilp, IOCTL_AUDIO_SET_REAR_MEDIA_TYPE, &eSrcType, sizeof(eSrcType), NULL, 0, NULL);
	}*/
	...
	MM_FREE(pADec);/*need change, need free*/
	pADec = NULL;
...
#ifndef ATC_AOSP_ENHANCEMENT_AUDIO
			eSrcType = AUD_OUT_MEDIA_USB;

			if (!AudioIoCtl(pADec->AudioFilp, IOCTL_AUDIO_SET_FRONT_MEDIA_TYPE,
				&eSrcType, sizeof(eSrcType), NULL, 0, NULL)) {
...
BOOL ADec_SetInputBuf(HANDLE hInst, VOID *pvBuf, UINT32 u4BufSz, VOID *pvOutBuf, UINT32 u4OutBufSz)
{
	...
	if (!pADec->bIsOurDmx) {
		...
		rSendBufInfo.ptrBufAddr = (UINT32)pvBuf;
...
BOOL ADec_CloseAOut(HANDLE hInst, AUD_OUTPUT_T AOutPut)
{
	...
	case AUD_OUTPUT_FRONT:
		...
		if ((pADec->eAudCmdStatus != AUD_CMD_STATUS_STOP) &&
		    (eRearSrcType != AUD_OUT_MEDIA_USB)) {
			pr_info("[AVIN][hdmi_adec]%s close front type do nothing with status(%s) and rear src type(%d)\r\n", __func__,
				StatusStr[pADec->eAudCmdStatus], eFrontSrcType);
		} else {
#ifndef ATC_AOSP_ENHANCEMENT_AUDIO
			AudioIoCtl(pADec->AudioFilp, IOCTL_AUDIO_SET_FRONT_MEDIA_TYPE,
				&eSrcType, sizeof(eSrcType), NULL, 0, NULL);
	...
	case AUD_OUTPUT_REAR:
		...
		if ((pADec->eAudCmdStatus != AUD_CMD_STATUS_STOP) &&
		    (eFrontSrcType != AUD_OUT_MEDIA_USB)) {
			pr_info("[AVIN][hdmi_adec]%s close rear type do nothing with status(%s) and rear src type(%d)\r\n", __func__,
				StatusStr[pADec->eAudCmdStatus], eFrontSrcType);
		} else {
#ifndef ATC_AOSP_ENHANCEMENT_AUDIO
			AudioIoCtl(pADec->AudioFilp, IOCTL_AUDIO_SET_REAR_MEDIA_TYPE,
				&eSrcType, sizeof(eSrcType), NULL, 0, NULL);
#else
	...
	case AUD_OUTPUT_FRONT_REAR:
		if (pADec->eAudCmdStatus != AUD_CMD_STATUS_STOP) {
			pr_info("[AVIN][hdmi_adec]%s close rear type do nothing with status(%s)\r\n", __func__,
...
AUDIO_DECODER *ADec_CreateInstance(AVCODECID_T codec_type, UINT32 u4Flag)
{
	...
	if (audiofilp == INVALID_HANDLE_VALUE) {
		/*dwErr =errno;*/
		if (pADec != NULL) {
			pr_info("[AVIN][hdmi_adec]%s CreateFile fail\r\n", __func__);
			goto FAIL_EXIT;
		}
	}
