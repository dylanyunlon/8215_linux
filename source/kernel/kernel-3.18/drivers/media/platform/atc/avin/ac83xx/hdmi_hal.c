...
WCH_BUFF_INFO_T mWchBufferInfo_orien;
...
void getVideoRect(int cmd, int outRect[]){
    ...
    /*if (0 != hdmihandle) {
		...
		if ((DEVICE_MHL == device_type) && (SIGNAL_READY == mSigState)
			 /*&& (0 != hvsihandle)*/) {
			mCurrentMhlWchVdoInfo.u4YVaAddr = ioremap((unsigned long)mWchBufferInfo.u4YBuf[buffer_id], mWidth * mHeight);//mWchBufferInfo_orien.u4YBuf[buffer_id];//
			mCurrentMhlWchVdoInfo.u4CVaAddr = ioremap((unsigned long)mWchBufferInfo.u4CBuf[buffer_id], mWidth * mHeight/2);//mWchBufferInfo_orien.u4CBuf[buffer_id];//
...
void getVideoRect(int cmd, int outRect[]){
    ...
    /*if (0 != hdmihandle) {
		...
		if ((DEVICE_MHL == device_type) && (SIGNAL_READY == mSigState)
			 /*&& (0 != hvsihandle)*/) {
			mCurrentMhlWchVdoInfo.u4YVaAddr = mWchBufferInfo_orien.u4YBuf[buffer_id];
			mCurrentMhlWchVdoInfo.u4CVaAddr = mWchBufferInfo_orien.u4CBuf[buffer_id];
			mCurrentMhlWchVdoInfo.u4Height  = mHeight;
			mCurrentMhlWchVdoInfo.u4Width	= mWidth;
			mCurrentMhlWchVdoInfo.u4Mode = MODE_LINE;
			pr_info("[AVIN][hdmi_hal]%s GR_GetActiveRect before\r\n", __func__);
...
static void wch_buffer_get(u32 *bufindex)
{
	...
	data.ycaddr.y = mWchBufferInfo.u4YBuf[bufferId];
	data.ycaddr.c = mWchBufferInfo.u4CBuf[bufferId];
...
static void onVdoNoSignal(void)
{
	...
	if (WchIoControl(1, IOCTL_WCH_STOP, (UCHAR *)&mSrcAppId, sizeof(WCH_SRC_APP_ID_E), NULL, 0, NULL)) {
		pr_info("[AVIN][hdmi_hal]%s execute IOCTL_WCH_STOP failed!\r\n", __func__);
		return;
	}
...
static void onVdoSignal(void)
{
	...
	mWchCtl.tWchCfg.fgHSyncPolarity = 1;/*mhdmiVdoInfo.bHPol;*/
	mWchCtl.tWchCfg.fgVSyncPolarity = 0;/*mhdmiVdoInfo.bVPol;*/

	mWchCtl.tWchCfg.eInputSrc = DATA_SRC_HDMI;
	mWchCtl.tWchCfg.eInputFmt = DATA_FMT_YUV422;
	mWchCtl.tWchCfg.u4SrcWidth = mhdmiVdoInfo.u4Width;
	mWchCtl.tWchCfg.u4SrcHeight = mhdmiVdoInfo.u4Height;
	...
	mWchCtl.tWchCfg.u4DstWidth = mhdmiVdoInfo.u4Width;
	mWchCtl.tWchCfg.u4DstHeight = mhdmiVdoInfo.u4Height;
	mWchCtl.tWchCfg.eOutputFmt = DATA_FMT_YUV420;

	mWchCtl.tWchCfg.fgProgressive = !mhdmiVdoInfo.bInterlaced;
	pr_info("[AVIN][hdmi_hal]%s bInterlaced = %d\r\n", __func__, (int)mhdmiVdoInfo.bInterlaced);

	mWchCtl.tWchCfg.u1YSel = 0;/*may change cause of different hw*/
	mWchCtl.tWchCfg.u1USel = 4;/*above*/
	mWchCtl.tWchCfg.u1VSel = 4;/*above*/
	...
	mWchCtl.tWchCfg.eTiming = (WCH_TIMING_E)timing;
	mWidth = mhdmiVdoInfo.u4Width;
	mHeight = mhdmiVdoInfo.u4Height;
	mWchCtl.tWchCfg.GetWchBufIndx = wch_buffer_get;/*wch_buffer_done;*/
	...
	pr_info("[AVIN][hdmi_hal]%s config success!\r\n", __func__);
...
int hdmi_init_video(int index)
{
	...
	if (WchIoControl(1, IOCTL_WCH_OPEN, (UCHAR *)&mSrcAppId,
		sizeof(WCH_SRC_APP_ID_E), (UCHAR *)&mWchId, sizeof(int), NULL)) {

		pr_info("[AVIN][hdmi_hal]%s wch open fail!\r\n", __func__);
		/*mutex_unlock(&videolock);*/
		return -EINVAL;
	}
	pr_info("[AVIN][hdmi_hal]%s wch open success!\r\n", __func__);
...
int hdmi_stop_video(int index)
{
	...
	if (mSigState == SIGNAL_NONE || mSigState == SIGNAL_IDLE) {
	...
	if (wchStop == false || mSigState == SIGNAL_LOST || mSigState == SIGNAL_CONNECTING) {
		if(WchIoControl(1, IOCTL_WCH_STOP, (UCHAR *)&mSrcAppId, sizeof(WCH_SRC_APP_ID_E), NULL, 0, NULL)) {
			pr_info("[hdmi_hal]%s wch stop fail!\r\n", __func__);
			mutex_unlock(&videolock);
			return -1;
		}
		wchStop = true;
		pr_info("[hdmi_hal]%s wch stop success!\r\n", __func__);
		...
		if (mAudState == HDMI_AUD_STABLE) {
			...
			if (esm_info != NULL) {
				...
				if (!ret) {
					...
					ReleaseAudioOutputBuf((void*)esm_info);
				}
			}
			msleep(1);
