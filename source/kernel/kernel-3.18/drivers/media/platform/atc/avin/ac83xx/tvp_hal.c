...
struct v4l2_data g_tvp_data[TVP_MAX_VIDEO_DEVS];
...
static int tvp_i2c_read(uint8_t *reg, uint8_t *buffer, uint8_t count, uint8_t dataSize)
{
	...
	if ((NULL == reg) || (NULL == buffer) || (dataSize <= 0) || (dataSize > 2)) {
		AVIN_ERROR(LOG_TAG, "param reg/buffer/datasize(%d) error!\n", dataSize);
		return -1;
	}
	...
	for (idx = 0; idx < count; idx++) {
		...
		if (TVP_I2C_RETRY_TIME == retry) {
			AVIN_ERROR(LOG_TAG, "i2c_transfer(0x%x, %x) write error(%d) with idx(%d)!\n",
				msg.addr, msg.buf[0], err, idx);
			return err;
		}
		...
		if (TVP_I2C_RETRY_TIME == retry) {
			AVIN_ERROR(LOG_TAG, "i2c_transfer(0x%x, %x) read error(%d) with idx(%d)!\n",
				msg.addr, msg.buf[0], err, idx);
			return err;
		}
	}
...
static int tvp_i2c_write(uint8_t *reg, uint8_t *buffer, uint8_t count, uint8_t dataSize)
{
	...
	if ((NULL == reg) || (NULL == buffer) || (dataSize <= 0) || (dataSize > 2)) {
		AVIN_ERROR(LOG_TAG, "param dataSize(%d) error!\n", dataSize);
		return -1;
	}
...
static int tvp_download_patch(void)
{
	...
	patchAddr = (uint8_t*)g_pData;
	patchSize = sizeof(g_pData);

	ret = tvp_select_write(0xF);
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_select_write error with ret(%d)\n", ret);
		return ret;
	}
	...
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_i2c_write error with ret(%d)\n", ret);
		return ret;
	}
	...
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_i2c_read error with ret(%d)\n", ret);
		return (ret);
	}

	vbusStatus = regVal[0];
	if (vbusStatus & 0x2) {
		AVIN_INFO(LOG_TAG, "Patch is already running.\n");
		return ret;
	}
	else
	{
		AVIN_INFO(LOG_TAG, "Patch is downloading...\n");
	}
	...
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_vbus_write error with ret(%d)\n", ret);
		return ret;
	}
	...
	AVIN_INFO(LOG_TAG, "Patch is downloaded.  Soft reset\n");
...
static int tvp_check_chip(void)
{
	...
	if ((regVal[0] != ((TVP_CHIP_ID & 0xFF00) >> 8)) ||
		...
		AVIN_ERROR(LOG_TAG, "tvp_i2c_read error with regVal(0x%02x, 0x%02x)\n",
			regVal[0], regVal[1]);
		return -1;
	}
...
static int tvp_check_input(tvp_input_standard *pInputStandard)
{
	...
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_select_read error with ret(%d)\n", ret);
		return (ret);
	}
	...
	AVIN_ERROR(LOG_TAG, "dec1_std(%d) dec2_std(%d) dec3_std(%d) dec4_std(%d)\n",
			pInputStandard->dec1_std, pInputStandard->dec2_std,
...
int tvp_set_attribute(uint8_t chid, uint32_t control, int32_t value)
{
	...
	if (chid >= TVP_MAX_VIDEO_DEVS) {
		AVIN_ERROR(LOG_TAG, "param error with chid(%d)!\n", chid);
		return -1;
	}
	...
	default:
		AVIN_ERROR(LOG_TAG, "param control(%d) error\n", control);
		return -1;
	}
...
int tvp_get_attribute(uint8_t chid, uint32_t control, int32_t *pvalue)
{
	...
	if (NULL == pvalue) {
		AVIN_ERROR(LOG_TAG, "param pvalue is NULL!\n");
		return -1;
	}
	count = 1;
	datasize = 1;
...
int tvp_get_std(uint8_t chid, v4l2_std_id *pstd_type)
{
	...
	default:
		AVIN_ERROR(LOG_TAG, "get error standards(0x%02x)!\n", regVal);
...
int tvp_i2c_start(int ofmMode)
{
	...
	if (g_TvpStartCnt) {
		AVIN_DEBUG(LOG_TAG, "tvp has started!\n");
		g_TvpStartCnt++;
		return 0;
	}
	...
	ret = tvp_check_chip();
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_check_chip error with ret(%d)\n", ret);
		return ret;
	}

	ret = tvp_reset_ofm();
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_reset_ofm error with ret(%d)\n", ret);
		return ret;
	}
	...
	case TVP_VIDEO_DECODER_MODE_4CH_HALF_D1_PROG:
		AVIN_DEBUG(LOG_TAG, "TVP_VIDEO_DECODER_MODE_4CH_HALF_D1\n");
		B0 = 0xA2;
		B8 = 0x70;
		B9 = 0x10;
		break;
	...
	case TVP_VIDEO_DECODER_MODE_4CH_HALF_D1_PROG_CROP:
		AVIN_DEBUG(LOG_TAG, "TVP_VIDEO_DECODER_MODE_4CH_HALF_D1_CROP\n");
		B0 = 0xA2;
		B1 = 0x50;
		B8 = 0x80;
		B9 = 0x10;
	...
	default:
		AVIN_ERROR(LOG_TAG, "error with unsupported ofm mode(%d)\n", ret);
		return -1;
	}
...
int tvp_i2c_stop(void)
{
	...
	if (!g_TvpStartCnt) {
		AVIN_DEBUG(LOG_TAG, "tvp has not started!\n");
		return 0;
	}

	if (g_TvpStartCnt > 1) {
		AVIN_DEBUG(LOG_TAG, "tvp don't need to stop!\n");
		g_TvpStartCnt--;
		return 0;
	}
	...
	ret = tvp_soft_reset();
	if (ret) {
		AVIN_ERROR(LOG_TAG, "tvp_soft_reset error with ret(%d)\n", ret);
		return ret;
	}
...
static void tvp_get_wch_buf_idx(u32 *pu4BufIdx)
{
	...
	if (NULL == pu4BufIdx)
	{
		AVIN_ERROR(LOG_TAG, "param pu4BufIdx is NULL!\n");
		return;
	}
	...
	if (OSR_OK != i4Ret)
	{
		AVIN_ERROR(LOG_TAG, "x_msg_q_send fail(%d)!\n", i4Ret);
	}
}
...
static struct framebuf * tvp_get_next_valid_buf(uint32_t u4ChIdx, int32_t i4CurrIdx)
{
	...
	if ((NULL != pBuf) && (NULL != pBuf->data) && (FRAME_EMPTY == pBuf->status))
	{
		...
		if (!(g_u4LogCnt % 50)) {
			g_u4LogCnt = 0;
			AVIN_ERROR(LOG_TAG, "ch(%d) next buf(%d) is invalid with status(%d)!\n",
				u4ChIdx, i4NextIdx, pBuf->status);
		}
		return NULL;
	}
}
...
static int tvp_parser_thread(void *data)
{
	...
	if (g_tvp_data[0].pix.width) {
		...
		AVIN_ERROR(LOG_TAG, "tvp data width error!\n");
		return -1;
	}
	...
	while (TRUE) {
		...
		if (OSR_OK != i4Ret) {
			AVIN_ERROR(LOG_TAG, "x_msg_q_receive fail!\n");
			break;
		}
		if (0xFF == u4WchBufIdx) {
			break;
		...
		for (u4LineIdx = 0; u4LineIdx < TVP_SUPER_FRAME_HEIGHT_4CH_HALF_D1; u4LineIdx++) {
			...
			if (((uStartCode[3] & 0x80) != 0x80) || ((uStartCode[2] & 0x80) != 0x00) ||
				...
				//AVIN_ERROR(LOG_TAG, "Error startcode with SC3:SC1 = 0x%02x%02x%02x%02x\n",
					//uStartCode[3], uStartCode[2], uStartCode[1], uStartCode[0]);
				continue;
			}
			...
			if (true == fgStart[u4ChId]) {
				...
				if ((NULL != pBuf[u4ChId]) && (0 == u4Field) &&
					...
					if (i4Ret) {
						AVIN_DEBUG(LOG_TAG, "ch(%d) reuse buf idx(%d)!\n",
							u4ChId, g_tvp_data[u4ChId].cur_frames);
					} else {
	...
	AVIN_ERROR(LOG_TAG, "thread exit success!\n");
...
int tvp_wch_start(E_TVP_TYPE_T eTVPFmt)
{
	...
	if (g_WchStartCnt) {
		AVIN_DEBUG(LOG_TAG, "wch has started!\n");
		g_WchStartCnt++;
		return 0;
	}
	...
	if (IS_ERR(g_Thread_task))
	{
		...
		AVIN_ERROR(LOG_TAG, "kthread_create fail(%d)!\n", i4Ret);
		g_Thread_task = NULL;
		return i4Ret;
	}
	g_fgExitThread = false;
	...
	case TVP_TYPE_656_I_352_480:
		AVIN_DEBUG(LOG_TAG, "TVP_TYPE_656_I_352_480\n");
		g_rTvpWchCtrl.tWchCfg.fgVSyncPolarity = 0; // FALSE is LOW level present sync.
		g_rTvpWchCtrl.tWchCfg.fgHSyncPolarity = 0; // TRUE is High.
	...
	case TVP_TYPE_656_P_352_2100:
		AVIN_DEBUG(LOG_TAG, "TVP_TYPE_656_P_352_2100\n");
		g_rTvpWchCtrl.tWchCfg.fgVSyncPolarity = 0; // FALSE is LOW level present sync.
		g_rTvpWchCtrl.tWchCfg.fgHSyncPolarity = 0; // TRUE is High.
	...
	default:
		AVIN_ERROR(LOG_TAG, "Error TVP Fmt: %d\n", eTVPFmt);
		goto error;
	}
	...
	if (!g_rTvpWchBufInfo.tWchBuf.u4YBuf[0]) {
		...
		for (i4Idx = 0; i4Idx < WCH_BUF_MAX_CNT; i4Idx++) {
			AVIN_DEBUG(LOG_TAG, "Physic Y(%08x) C(%08x)\n",
			...
			AVIN_DEBUG(LOG_TAG, "virtual Y(%08x) C(%08x)\n",
				g_rTvpWchBufInfo.tWchBuf.u4YBuf[i4Idx], g_rTvpWchBufInfo.tWchBuf.u4CBuf[i4Idx]);
		}
	}
...
int tvp_wch_stop(uint32_t u4ChIdx)
{
	int i4Ret = 0;
	uint32_t u4WchBufIdx = 0xFF;

	mutex_lock(&g_WchLock);
	if (u4ChIdx >= TVP_MAX_VIDEO_DEVS) {
		AVIN_ERROR(LOG_TAG, "u4ChIdx(%d) error!\n", u4ChIdx);
		mutex_unlock(&g_WchLock);
		return -1;
	}
	...
	g_tvp_data[u4ChIdx].streaming = 0;
	g_tvp_data[u4ChIdx].memset_cnt = 0;
	g_tvp_data[u4ChIdx].cur_frames = 0;
...
int tvp_init(unsigned short addr)
{
	...
	if (NULL == pAdapter) {
		AVIN_ERROR(LOG_TAG, "i2c_get_adapter failed!\n");
		return -1;
	}
	...
	if (NULL == g_pClient) {
		AVIN_ERROR(LOG_TAG, "i2c_new_device failed!\n");
		return -1;
	}
	...
	if (OSR_OK != x_msg_q_create(&g_hSendBufMsgQ, "SendBuf_MSGQ", sizeof(uint32_t), 40)) {
		AVIN_ERROR(LOG_TAG, "x_msg_q_create fail!\n");
		if (g_pClient) {
...
int tvp_deinit(void)
{
	...
	if (NULL_HANDLE != g_hSendBufMsgQ) {
		if (OSR_OK != x_msg_q_delete(g_hSendBufMsgQ)) {
			AVIN_ERROR(LOG_TAG, "x_msg_q_delete fail!\n");
		}
