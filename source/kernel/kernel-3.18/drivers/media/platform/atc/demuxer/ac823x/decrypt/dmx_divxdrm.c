...
MRESULT DivxDRMGetInstLastError(void *pvInst)
{
	...
	if (!DivxDRMIsInstValid(prInst)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d failed for invalid DivxDRM Instance (hInst: 0x%p)\r\n"),
	...
	if (NULL == prInst->pvContext) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d failed for DivxDRM Inst(0x%p)'s context is NULL\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst);
		prInst->u4Ref--;
...
MRESULT DivxDRMInitSystem(DECRYPT_DIVXDRM_INST_T *prInst)
{
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s line %d -- DivxDRM Inst(0x%p)'s state : %d\r\n"),
	...
	if (NULL == prInst->pvContext) {
		...
		DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d -- DivxDRM Inst(0x%p)'s context is NULL\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst);
		...
		if (DRM_SUCCESS != eErrCode) {
			...
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
				...
				TEXT("hInst: 0x%p, eErrCode: %d, mrRet: 0x%x)\r\n"),
	...
	if (DRM_SUCCESS != eErrCode) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail in drmInitSystem ")
			TEXT("hInst: 0x%p, u4ContextSz: %d, eErrCode: %d)\r\n"),
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s (hInst: 0x%p) success\r\n"),
...
MRESULT DivxDRMReleaseInst(void *pvInst)
{
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s(hInst: 0x%p) success\r\n"),
		DMX_FUNC_NAME, (uintptr_t)pvInst);
...
bool	DivxDRMIsInstValid(DECRYPT_DIVXDRM_INST_T *prInst)
{
	...
	if (NULL == prInstTmp) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail for no corresponding ")
			TEXT("DivxDRM Instance handle(hInst: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst);
		DIVXDRMUNLOCK();
		return FALSE;
	}
	...
	u32				   u4Cnt)
{
	...
	for (u4Idx = 0; u4Idx < u4Cnt; u4Idx++) {
		...
		if (DRM_SUCCESS != eErrCode) {
			...
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
				...
				TEXT("hInst: 0x%p, u4Cnt: %d), mrRet: 0x%x\r\n"),
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s success, (hInst: 0x%p, u4Cnt: %d)\r\n"),
	...
	DECRYPT_DIVXDRM_HEADER_T		 *prHdrInfo)
{
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s enter, hInst: 0x%p\r\n"),
		DMX_FUNC_NAME, prInst);
	...
	if (DECRYPT_DIVXDRM_STATUS_SYSTEM_INITED != prInst->eStatus) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("for call InitPlayback, hInst: 0x%p\r\n"),
	...
	if (0 != mm_copy_from_user(prInst->rHeader.pu1DrmHdrBuf, prHdrInfo->pu1DrmHdrData, prHdrInfo->u4DrmHdrSz)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("-- prHdrInfo->pu1DrmHdrData:0x%p\r\n"),
			prHdrInfo->pu1DrmHdrData);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		...
		TEXT("pvContext: 0x%x, u4DrmHdrSz: %d, pu1DrmHdrData: 0x%p, hInst: 0x%p\r\n"),
	...
	if (DRM_SUCCESS != eErrCode) {
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("eResult: %d, mrRet: 0x%x, hInst: 0x%p\r\n"),
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s line %d -- Set DivxDRM Instance(0x%p)'s")
		TEXT("state to be %d\r\n"),
...
MRESULT DivxDRMCommitPlayback(DECRYPT_DIVXDRM_INST_T  *prInst, s32 i4DecryptId)
{
	...
	if (DECRYPT_DIVXDRM_STATUS_FILE_INITED != prInst->eStatus) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("Commit Playback, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s line %d -- Set DivxDRM Instance(0x%p)'s state to be %d\r\n"),
		DMX_FUNC_NAME, DMX_LINE_NO, prInst, prInst->eStatus);

	prInst->u4Ref--;

	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s success, hInst: 0x%p\r\n"),
		DMX_FUNC_NAME, prInst);
...
MRESULT DivxDRMFinalizePlayback(DECRYPT_DIVXDRM_INST_T	*prInst)
{
	...
	if ((DECRYPT_DIVXDRM_STATUS_SYSTEM_INITED != prInst->eStatus)  &&
		...
		DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d exit, decrypt state(%d), hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
	...
	if (DRM_SUCCESS != eResult) {
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("eResult: %d, mrRet: %d, hInst: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, eResult, mrRet, prInst);
	...
	DECRYPT_DIVXDRM_RENTAL_STATUS_T  *prRentalStatus)
{
	...
	if (DECRYPT_DIVXDRM_STATUS_FILE_INITED != prInst->eStatus) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("state(%d) to Commit Playback, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
	...
	if (DRM_SUCCESS != eErrCode) {
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail in drmQueryRentalStatus, ")
			TEXT("eErrCode: %d, mrRet: 0x%x, hInst: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, eErrCode, mrRet, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(mrRet);
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		...
		TEXT("UseLimitCnt: 0x%02x, UseCnt: 0x%02x, hInst: 0x%p\r\n"),
	...
	u8 *pu1Cgmsa)
{
	...
	if ((DECRYPT_DIVXDRM_STATUS_FILE_INITED  != prInst->eStatus) &&
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("to Query Cgmsa, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
	...
	u8  *pu1Acptb)
{
	...
	if ((DECRYPT_DIVXDRM_STATUS_FILE_INITED  != prInst->eStatus) &&
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail for err decrypt state(%d) ")
			TEXT("to Query Acptb, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
	...
	u8  *pu1DigitalProc)
{
	...
	if ((DECRYPT_DIVXDRM_STATUS_FILE_INITED  != prInst->eStatus) &&
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail for err decrypt state(%d)")
			TEXT("to Query Digital Proctection, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
	...
	if (DRM_SUCCESS != eResult) {
		mrRet = DivxDRMMapErrCode(eResult);
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
TEXT("[DECRYPT][DIVXDRM] %s line %d fail in drmQueryDigitalProtection, eResult: %d, mrRet: 0x%x, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, eResult, mrRet, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(mrRet);
	...
	u8  *pu1Ict)
{
	...
	if ((DECRYPT_DIVXDRM_STATUS_FILE_INITED  != prInst->eStatus) &&
		(DECRYPT_DIVXDRM_STATUS_FILE_PLAYING != prInst->eStatus)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
TEXT("[DECRYPT][DIVXDRM] %s line %d fail for err decrypt state(%d) to Query ICT, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
	...
	if (DRM_SUCCESS != eResult) {
		mrRet = DivxDRMMapErrCode(eResult);
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
TEXT("[DECRYPT][DIVXDRM] %s line %d fail in drmQueryIct, eResult: %d, mrRet: 0x%x, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, eResult, mrRet, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(mrRet);
	...
	u32	*pu4Cnt)
{
	...
	if (DECRYPT_DIVXDRM_STATUS_SYSTEM_UNINIT == prInst->eStatus) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
TEXT("[DECRYPT][DIVXDRM] %s line %d fail for err decrypt state(%d) to Get Random Sample Count, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
	...
	*pu4Cnt = drmGetRandomSampleCounter(prInst->pvContext);
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s success, Cnt : %d, hInst: 0x%p\r\n"),
	...
	DECRYPT_DIVXDRM_CODE_STRING_T *prCodeString)
{
	...
	if (DECRYPT_DIVXDRM_STATUS_SYSTEM_UNINIT == prInst->eStatus) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail for err decrypt state(%d) ")
			TEXT("to Query Registeration Code String, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
		...
		if (prCodeString->u4CodeStrSz > DRM_REGISTRATION_CODE_BYTES) {
			...
			if (0 != mm_copy_to_user(prCodeString->szCodeStr, szCodeStr, DRM_REGISTRATION_CODE_BYTES)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
					...
					TEXT(" -- prCodeString->szCodeStr:0x%p\r\n"),
	...
	prInst->u4Ref--;
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s success, hInst: 0x%p, RegCodeStr: %s\r\n"),
	...
	DECRYPT_DIVXDRM_CODE_STRING_T *prCodeString)
{
	...
	prInst->u4Ref--;
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s success, hInst: 0x%p, DeactiveCodeString: %s\r\n"),
		DMX_FUNC_NAME, prInst, prCodeString->szCodeStr);
	...
	DECRYPT_DIVXDRM_DECRYPT_DATA_T *prDecryptInfo)
{
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s -- FrameBuffer: 0x%p, Size: %d, hInst: 0x%p\r\n"),
		DMX_FUNC_NAME, (uintptr_t)(prDecryptInfo->pu1FrameData), prDecryptInfo->u4FrameDataSz, prInst);
	...
	if (DECRYPT_DIVXDRM_STATUS_FILE_PLAYING != prInst->eStatus) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail for err decrypt state(%d) ")
			TEXT("to call drmDecryptAudio, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
	...
	DECRYPT_DIVXDRM_DECRYPT_DATA_T *prDecryptInfo)
{
	...
	if (DECRYPT_DIVXDRM_STATUS_FILE_PLAYING != prInst->eStatus) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			TEXT("[DECRYPT][DIVXDRM] %s line %d fail for err decrypt state(%d) ")
			TEXT("to call drmDecryptVideo, hInst: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prInst->eStatus, prInst);
		prInst->u4Ref--;
		DIVXDRMINSTUNLOCK(prInst);
		MM_RETURN(RET_DMX_ERR_STATE);
...
#if DMX_PRINT_DECRYPT_KEY_LOG
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s -- Video, KeyIdx: 0x%04x, hInst: 0x%p\r\n"),
...
#if DMX_PRINT_DECRYPT_KEY_LOG
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s -- Audio, hInst: 0x%p\r\n"),
		DMX_FUNC_NAME, prInst);
#endif /*DMX_PRINT_DECRYPT_KEY_LOG*/
...
#if DMX_PRINT_DECRYPT_KEY_LOG
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
		TEXT("[DECRYPT][DIVXDRM] %s success, hInst: 0x%p, u2KeyLen: %d\r\n"),
		DMX_FUNC_NAME, prInst, prKeyInfo->u2KeyLen);
#endif /*DMX_PRINT_DECRYPT_KEY_LOG*/
...
MRESULT DivxDRMGetActivationStatus(void *pvActStatus, u32 u4StatusSz)
{
	...
	if (FAILED(hr)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%x, Sz: %d), Err: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO,
			pvMarshalledMem, (uintptr_t)(prActStatus->pu1UsrId),
		...
		if (NULL != pvMarshalledMem) {
			...
			if (FAILED(hr)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
					...
					TEXT("(DestMarshallMem: 0x%x, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO,
					pvMarshalledMem, (uintptr_t)(prActStatus->pu1UsrId),
					u4UsrIdBufSz, DMX_GET_LASTERR);
			}
	...
	if (NULL != pvMarshalledMem) {
		...
		if (FAILED(hr)) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
				TEXT("[DECRYPT][DIVXDRM] %s line %d fail in CeCloseCallerBuffer")
				TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO,
				pvMarshalledMem, (uintptr_t)(prActStatus->pu1UsrId),
				u4UsrIdBufSz, DMX_GET_LASTERR);
			MM_RETURN(RET_DMX_OS_OPERA_FAIL);
...
MRESULT DivxDRMGetVersion(void *pvVersionInfo, u32 u4InfoBufLen)
{
	...
	if (FAILED(hr)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO,
			pvMarshalledMem1, (uintptr_t)(prVersion->szCodeName),
	...
	if (FAILED(hr)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
			...
			TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO,
			pvMarshalledMem1, (uintptr_t)(prVersion->szOfficialName),
		...
		if (NULL != pvMarshalledMem2) {
			...
			if (FAILED(hr)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
					...
					TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO,
					pvMarshalledMem2, (uintptr_t)(prVersion->szOfficialName),
					u4Size2, DMX_GET_LASTERR);
			}
		}
#endif /*__linux__*/
	...
	case DIVXDRM_INIT_SYSTEM:
		{
			if (!DivxDRMIsInstValid(prInst)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
					...
					TEXT("DivxDRM Instance (hInst: 0x%p)\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prInst);
				MM_RETURN(RET_DMX_PARAM_WRONG);
			}
	...
	case DIVXDRM_SET_RANDOM_SAMPLE:
		{
			...
			DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
				...
				TEXT("pvBufIn: 0x%p, u4LenIn: %d\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, (uintptr_t)pvBufIn, u4LenIn);
	...
	case DIVXDRM_INIT_PLAYBACK:
		{
			...
			if (FAILED(hr)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
					...
					TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO,
					pvMarshalledMem, (uintptr_t)(prHdrInfo->pu1DrmHdrData),
	...
	case DIVXDRM_GET_REG_CODE_STR:
		{
			...
			if (FAILED(hr)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
					...
					TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO,
					pvMarshalledMem, (uintptr_t)(prCodeString->szCodeStr),
	...
	case DIVXDRM_GET_ACT_MESSAGE:
		{
			...
			if (FAILED(hr)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DECRYPT,
					...
					TEXT("(DestMarshallMem: 0x%p, SrcMem: 0x%p, Sz: %d), Err: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO,
					pvMarshalledMem, (uintptr_t)(prMsg->szMsgStr),
