...
PBBUF					*g_aprPbBuf[DMX_MAX_PBBUF_INST_CNT];  //done
...
MRESULT DmxInstCreateSema(DMX_INST_T *prDmxInst)
{
	...
	if (NULL != prDmxInst->hLock) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[SPT] %s fail for sema already exist, pvDmxInst: 0x%p\r\n"),
	...
	DMX_INST_T *prDmxInst)
{
	...
	for (u4Idxj = 0; u4Idxj < MAX_SPT_STM_CONNECTED; u4Idxj++) {
		if (NULL != prSpt->pvStmHandles[u4Idxj]) {
			...
			if (((void *)prSpt == prStm->pvSptHdl) &&
				((void *)prDmxInst == prStm->pvDmxInst)) {
				...
				case STREAM_SECTION:
					...
					if (DMX_FAILED(mrRet)) {
						DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
							TEXT("[DMX] %s line %d")
							TEXT("failedin StreamDisable, pvSptHdl: 0x%p, ")
							TEXT("mrRet: 0x%x.\r\n"),
			...
			if (((void *)prSpt == prStm->pvSptHdl) &&
				...
				case STREAM_SECTION:
					...
					if (DMX_FAILED(mrRet)) {
						DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
							TEXT("[DMX] %s line %d ")
							TEXT("failed in StreamSetFlush, pvSptHdl: 0x%p, ")
							TEXT("mrRet: 0x%x.\r\n"),
							DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
					}
					break;
			...
			if (((void *)prSpt == prStm->pvSptHdl) &&
				...
				case STREAM_SECTION:
					...
					if (DMX_FAILED(mrRet)) {
						DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
							TEXT("[DMX] %s line %d ")
							TEXT("failed in StreamDestroy, pvSptHdl: 0x%p, ")
							TEXT("mrRet: 0x%x.\r\n"),
							DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
					}
		...
		if ((NULL != prSpt) && (prSpt->fgCreated)) {
			...
			if (DMX_FAILED(mrRet)) {
				DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
					...
					TEXT("SplitterSetParserPause, pvSptHdl: 0x%p, mrRet: 0x%x.\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
			}
			...
			if (DMX_FAILED(mrRet)) {
				DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
					TEXT("[DMX] %s line %d failed in ")
					TEXT("SplitterSetParserOff, pvSptHdl: 0x%p, mrRet: 0x%x.\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
			}
			...
			if (DMX_FAILED(mrRet)) {
				DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
					TEXT("[DMX] %s line %d failed in ")
					TEXT("SplitterDisable, pvSptHdl: 0x%p, mrRet: 0x%x.\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
			}
			...
			if (DMX_FAILED(mrRet)) {
				DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
					TEXT("[DMX] %s line %d failed in ")
					TEXT("SplitterDestroy, pvSptHdl: 0x%p, mrRet: 0x%x.\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
			}
...
void *dmx_inst_create(void)
{
	...
	prDmxInst->u1DevID = 0;//DMX_INVALID_UINT8;
	...
	DmxLogT(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
		...
		DMX_FUNC_NAME, (void *)prDmxInst, g_rDmxInstList.u4Cnt);
		...
		if (NULL != prSpt) {
			...
			if (DMX_FAILED(mrRet)) {
				DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
					...
					TEXT("0x%x), dwContext: 0x%p, mrRet: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, (void *)prSpt, dwContext, mrRet);
			}
			prDmxInst->u4SptCnt--;
		}
	}
...
bool dmx_inst_release(void *dwContext)
{
	...
	DmxLogT(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
		TEXT("[DMX] %s enter, pvDmxInst: 0x%p\r\n"),
	...
	for (u4Idx = 0; u4Idx < prDmxInst->u4SptCnt; u4Idx++) {
		...
		if ((NULL != prSpt) &&
			(prSpt->fgCreated) &&
			(prSpt->pvDmxInst == (void *)prDmxInst)) {
			break;
		}
	}
...
MRESULT dmx_sptinst_create(void *dwContext, void **ppvSptHdl)
{
	...
	*ppvSptHdl = pvSptHdl;
	...
	DmxLogT(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
		TEXT("[DMX] %s success, pvDmxInst: 0x%p, its splitter ")
		TEXT("inst cnt: %d\r\n"),
...
MRESULT dmx_sptinst_destroy(void *dwContext, void *pvSptHdl)
{
	...
	if (prDmxInst->u4SptCnt <= 0) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT(" no splitter instance, dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, dwContext);
		DmxInstReleaseSema(prDmxInst);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	for (u4Idx = 0; u4Idx < MAX_SPT_INST_CNT_PER_DMX; u4Idx++) {
		prSpt = prDmxInst->parSpt[u4Idx];
		if (((void *)prSpt == pvSptHdl) &&
			 (prSpt->pvDmxInst == (void *)dwContext)) {
			break;
		}
	}
	...
	if (MAX_SPT_INST_CNT_PER_DMX <= u4Idx) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail for the dmx instance has")
			TEXT(" no corresponding splitter instance(pvSptHdl: 0x%p), ")
			TEXT("dwContext: 0x%p\r\n"),
	...
	DMX_PBBUF_CONFIG_INFO_T	*prPbbufCfgInfo)
{
	...
	if (prDmxInst->u4SptCnt <= 0) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail for the dmx instance has")
			TEXT(" no splitter instance, dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, (void *)dwContext);
		DmxInstReleaseSema(prDmxInst);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	if (MAX_SPT_INST_CNT_PER_DMX <= u4Idx) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT(" no corresponding splitter instance(pvSptHdl: 0x%p), ")
			TEXT("dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, (void *)pvSptHdl, (void *)dwContext);
		DmxInstReleaseSema(prDmxInst);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail in SplitterEnable(pvSptHdl: 0x%p), ")
			TEXT("dwContext: 0x%p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, (void *)pvSptHdl, (void *)dwContext, mrRet);
		MM_RETURN(mrRet);
	}

	DmxLogT(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
		TEXT("[DMX] %s success, pvDmxInst: 0x%p\r\n"),
		DMX_FUNC_NAME, dwContext);
...
bool dmx_spt_isenable(void *dwContext, void *pvSptHdl)
{
	...
	if (MAX_SPT_INST_CNT_PER_DMX <= u4Idx) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("corresponding splitter instance(pvSptHdl: 0x%p), ")
			TEXT("dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, dwContext);
		DmxInstReleaseSema(prDmxInst);
		return FALSE;
...
MRESULT DmxSetRate(void *dwContext, SPT_PARAM_SET_RATE *prSetRate)
{
	...
	for (u4Idx = 0; u4Idx < MAX_SPT_INST_CNT_PER_DMX; u4Idx++) {
		prSpt = prDmxInst->parSpt[u4Idx];
		if (((void *)prSpt == prSetRate->pvSptHdl) &&
			 (prSpt->pvDmxInst == (void *)dwContext)) {
			break;
		}
	}
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail in SplitterSetRate(pvSptHdl: 0x%p, ")
			TEXT("Rate: %d, fgDmaAud: %s), dwContext: 0x%p, mrRet: 0x%x\r\n"),
	...
	STM_PARAM_CREATE *prParam, void **ppvStm)
{
	...
	for (u4Idx = 0; u4Idx < MAX_SPT_INST_CNT_PER_DMX; u4Idx++) {
		prSpt = prDmxInst->parSpt[u4Idx];
		if (((void *)prSpt == prParam->pvSptHdl) &&
			 (prSpt->pvDmxInst == (void *)dwContext)) {
			break;
		}
	}
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail in StreamCreate(pvSptHdl: 0x%p, ")
	...
	DmxLogT(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
		TEXT("[DMX] %s success, pvDmxInst: 0x%p, its stream inst cnt: %d\r\n"),
...
MRESULT dmx_stminst_destroy(void *dwContext, void *pvSptHdl, void *pvStm)
{
	...
	if (prDmxInst->u4StmCnt <= 0) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail for the dmx instance has")
			TEXT(" no stream instance, dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, dwContext);
		DmxInstReleaseSema(prDmxInst);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	for (u4Idxj = 0; u4Idxj < MAX_STM_INST_CNT_PER_DMX; u4Idxj++) {
		...
		if (((void *)prStm == pvStm) &&
			 (prStm->pvDmxInst == (void *)dwContext)) {
			break;
		}
	}
	...
	if (MAX_STM_INST_CNT_PER_DMX <= u4Idxj) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail for the dmx instance has")
			TEXT(" no corresponding stream instance(pvSptHdl: 0x%p, pvStm: 0x%x), ")
			TEXT("dwContext: 0x%p\r\n"),
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("(pvSptHdl: 0x%p, pvStm: 0x%x), dwContext: 0x%p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pvStm, dwContext, mrRet);
		MM_RETURN(mrRet);
	}

	DmxLogT(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
		TEXT("[DMX] %s success, pvDmxInst: 0x%p, its stream ")
		TEXT("inst cnt: %d\r\n"),
		DMX_FUNC_NAME, dwContext, prDmxInst->u4StmCnt);
...
MRESULT dmx_stminst_enable(void *dwContext, void *pvStm)
{
	...
	if (MAX_STM_INST_CNT_PER_DMX <= u4Idxj) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail for the dmx instance has")
			TEXT(" no corresponding stream instance(pvStm: 0x%x), ")
			TEXT("dwContext: 0x%p\r\n"),
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("(pvStm: 0x%x), dwContext: 0x%p, mrRet: 0x%x\r\n"),
...
MRESULT dmx_stminst_setfifoinfo(void *dwContext, void *pvStm, u32 u4FifoSz)
{
	...
	mrRet = StreamSetFifoInfo(pvStm, u4FifoSz);
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail in StreamSetFifoInfo")
			TEXT("(pvStm: 0x%x, fifosz: 0x%08x), dwContext: 0x%p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvStm, u4FifoSz, dwContext, mrRet);
		MM_RETURN(mrRet);
	}
	...
	u32 *pu4FullNess)
{
	...
	if (prDmxInst->u4SptCnt <= 0) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("has no splitter instance, dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, dwContext);
		DmxInstReleaseSema(prDmxInst);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	if (MAX_SPT_INST_CNT_PER_DMX <= u4Idx) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail for the dmx instance ")
			TEXT("has no corresponding splitter instance(pvSptHdl: 0x%p), ")
			TEXT("dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, dwContext);
		DmxInstReleaseSema(prDmxInst);
		MM_RETURN(RET_DMX_UNEXPECT);
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("Fullness(pvSptHdl: 0x%p, SptDataType: %d), dwContext: 0x%p, ")
			TEXT("mrRet: 0x%x\r\n"),
...
MRESULT dmx_parse_on(void *dwContext, DMX_PSR_ON_PARAM_T *prParam)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("(pvSptHdl: 0x%p), dwContext: 0x%p, mrRet: 0x%x\r\n"),
...
MRESULT dmx_parse_pause(void *dwContext, void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("Pause(pvSptHdl: 0x%p), dwContext: 0x%p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, dwContext, mrRet);
		MM_RETURN(mrRet);
	}
	...
	SPLITTER_PTX_RSP_OFF_INFO_T *prParam)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail in SplitterSetParser")
			TEXT("RspOff(pvSptHdl: 0x%p, RspTxType: 0x%02x, RspTxMode: 0x%02x, ")
			TEXT("ucState: 0x%02x), dwContext: 0x%p, mrRet: 0x%x\r\n"),
	...
	SPLITTER_PTX_REBUFFER_RANGE_INFO_T *prParam)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("(pvSptHdl: 0x%p, PtsDelay: 0x%llx, RspStartPts: 0x%llx), ")
			TEXT("dwContext: 0x%p, mrRet: 0x%x\r\n"),
...
MRESULT dmx_rsp_on(void *dwContext, SPLITTER_PTX_RSP_ON_INFO_T *prParam)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("(pvSptHdl: 0x%p, fgRebuf: %s), dwContext: 0x%p, ")
			TEXT("mrRet: 0x%x\r\n"),
...
MRESULT dmx_cfa_settype(void *dwContext, CFA_PARAM_SET_TYPE *prParam)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("(pvSptHdl: 0x%p, CfaType: 0x%x), dwContext: 0x%p, ")
			TEXT("mrRet: 0x%x\r\n"),
...
MRESULT dmx_cfa_config(void *dwContext, void *pvSptHdl, void *pvConfigInfo, bool fgIsUserMem)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("Configure(pvSptHdl: 0x%p), dwContext: 0x%p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, dwContext, mrRet);
		MM_RETURN(mrRet);
	}
	...
	u32 u4CfaParameterSize)
{
	...
	if (MAX_SPT_INST_CNT_PER_DMX <= u4Idx) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			TEXT("[DMX] %s line %d fail for the dmx instance has")
			TEXT(" no corresponding splitter instance(pvSptHdl: 0x%p), dwContext: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, dwContext);
		DmxInstReleaseSema(prDmxInst);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_INST, DMX_MOD_INST_LOGLVL_DEFAULT,
			...
			TEXT("(pvSptHdl: 0x%p, u4CfaFID: 0x%x), dwContext: 0x%p, mrRet: 0x%x\r\n"),
	...
	void *pvSptHdl, u64 *pu8FileOfst)
{
	...
	mrRet = SplitterGetCfaPosition((void *)pvSptHdl, pu8FileOfst);
