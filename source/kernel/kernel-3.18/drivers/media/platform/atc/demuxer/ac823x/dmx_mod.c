...
#ifdef CONFIG_COMPAT
EXTERN long DMX_IOControl_Compat(struct file *file, unsigned int cmd, unsigned long arg);
...
long demuxer_ioctl_compat(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (NULL == file) {
		DMX_ASSERT(FALSE);
		pr_err("[DMX] %s fail for file is NULL, ioctl '%c', dir=%d, #%d (0x%08x)\n",
			DMX_FUNC_NAME, _IOC_TYPE(cmd), _IOC_DIR(cmd), _IOC_NR(cmd), cmd);
		return -EINVAL;
	}

	if (0 == arg) {
		pr_err("[DMX] %s fail for no arg, ioctl '%c', dir=%d, #%d (0x%08x)\n",
			DMX_FUNC_NAME, _IOC_TYPE(cmd), _IOC_DIR(cmd), _IOC_NR(cmd), cmd);
		return -EINVAL;
	}
	
	if (!file->f_op->unlocked_ioctl) {
		pr_err("[DMX] %s fail for no unlocked_ioctl, ioctl '%c', dir=%d, #%d (0x%08x)\n",
			DMX_FUNC_NAME, _IOC_TYPE(cmd), _IOC_DIR(cmd), _IOC_NR(cmd), cmd);
		return -ENOIOCTLCMD;
  }
	...
	if (_IOC_TYPE(cmd) != 'D') {
    pr_err("[DMX] %s line %d fail for invalid ioctl '%c', dir=%d, #%d (0x%08x)\n",
      DMX_FUNC_NAME, DMX_LINE_NO,
      _IOC_TYPE(cmd), _IOC_DIR(cmd), _IOC_NR(cmd), cmd);
		return -ENOIOCTLCMD;
  }

	ret = DMX_IOControl_Compat(file, cmd, arg);
...
int dmx_resume(struct device *dev)
{
	...
	if (DMX_FAILED(mrRet)) {
		pr_debug("[dmx] dmx_resume failed in SplitterSetPowerState(D0)!\r\n");
		return 0;
	}
...
static int demuxer_open(struct inode *inode, struct file *flip)
{
	...
	pvContext = DMX_Open();
	if (NULL == pvContext) {
		pr_debug("[DMX] %s fail in DMX_Open(0, 0, 0)!\r\n", DMX_FUNC_NAME);
		return -1;
	}

	flip->private_data = pvContext;
	pr_debug("[DMX] %s success, inst: %p!\r\n", DMX_FUNC_NAME, pvContext);
	return 0;
}
...
static int demuxer_release(struct inode *inode, struct file *file)
{
	...
	if (!DMX_Close(pvContext)) {
		DMX_ASSERT(FALSE);
		pr_debug("[DMX] %s fail in DMX_Close(context: %p)!\r\n",
			 DMX_FUNC_NAME, pvContext);
		return -1;
	}
...
long demuxer_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pvContext = file->private_data;
	if (_IOC_TYPE(cmd) == 'D') {
		if (!DMX_IOControl(pvContext, cmd, arg, TRUE))
			return -1;
	} else {
...
static irqreturn_t ac823x_dmx_ftup_interrupt(int irq, void *dev_id)
{
...
static irqreturn_t ac823x_dmx_ddi_interrupt(int irq, void *dev_id)
{
	struct platform_device *pdev = dev_id;
	struct dmx_dev_info *dmx_dev = platform_get_drvdata(pdev);
...
static int demuxer_probe(struct platform_device *pdev)
{
	...
	dmx_dev->sys_dram_regs = of_iomap(node, 5);
	if (!dmx_dev->sys_dram_regs) {
		pr_err("fail in unable to iomap dram registers!!\r\n");
		goto err_free_mem;
	}
	dmx_dev->sys_base_regs = of_iomap(node, 6);
	if (!dmx_dev->sys_base_regs) {
		pr_err("fail in unable to iomap sys base registers!!\r\n");
		goto err_free_mem;
	}

	pr_info("demuxer --> dmx_regs [0: 0x%p] [1: 0x%p] [2: 0x%p] [8: 0x%p] [ddi: 0x%p] !!\r\n",
		(dmx_dev->dmx_base0_regs),
		(dmx_dev->dmx_base1_regs),
		(dmx_dev->dmx_base2_regs),
		(dmx_dev->dmx_base8_regs),
		(dmx_dev->dmx_ddi_regs));

	dmx_dev->ftup_irq = irq_of_parse_and_map(node, 0);
	if (dmx_dev->ftup_irq == (unsigned int)(-1)) {
		pr_err("demuxer_probe fail in get dmx ftup irq id!\r\n");
		goto err_free_mem;
	}

	dmx_dev->ddi_irq = irq_of_parse_and_map(node, 1);
	if (dmx_dev->ddi_irq == (unsigned int)(-1)) {
		pr_err("demuxer_probe fail in get dmx ddi irq id!\r\n");
		goto err_free_mem;
	}
	pr_info("demuxer_probe get dmx ddi_irq:%d, ftup_irq:%d!\r\n",dmx_dev->ddi_irq, dmx_dev->ftup_irq);
	...
	result = request_irq(dmx_dev->ddi_irq, ac823x_dmx_ddi_interrupt,
	...
	result = request_irq(dmx_dev->ftup_irq, ac823x_dmx_ftup_interrupt,
			     0, DEMUXER_DEVNAME, pdev);
	if (result) {
...
MODULE_DESCRIPTION("ATC ac823x Demuxer Driver");
MODULE_LICENSE("GPL");
