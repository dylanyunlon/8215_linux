...
u32 DMX_Init(const char * pContext, u32 dwBusContext)
{
	...
	//DmxLogEnable(TRUE, DMX_LOG_DEBUG, -1, -1);
...
void * DMX_Open(void)
{
	void *pvInstHdl = (void *)dmx_inst_create();
...
bool DMX_Close(void *pvInst)
{
	...
	fgRet = dmx_inst_release((void *)pvInst);

	if (!fgRet) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			TEXT("[DMX] %s fail in dmx_inst_release(Context: %p)\r\n"),
			DMX_FUNC_NAME, pvInst);
	}
...
bool DMX_IOControl(void *pvInst, unsigned int cmd, unsigned long arg, bool fgIsUserMem)
{
	...
	case DMX_IOCTL_SPT_CREATE: {
		...
		mrRet = dmx_sptinst_create(pvInst, &pvHdl);
	...
	case DMX_IOCTL_SPT_DESTROY: {
		...
		mrRet = dmx_sptinst_destroy(pvInst, pvSptHdl);
	...
	case DMX_IOCTL_SPT_ENABLE: {
		...
		mrRet = dmx_sptinst_enable(pvInst, rParam.pvSptHdl,
	...
	case DMX_IOCTL_SPT_DISABLE: {
		...
		mrRet = dmx_sptinst_disable(pvInst, (HANDLE)pvSptHdl);
	...
	case DMX_IOCTL_SPT_IS_ENABLED: {
		...
		return dmx_spt_isenable(pvInst, (HANDLE)pvSptHdl);
	}
	...
	case DMX_IOCTL_SPT_SET_RATE: {
		...
		mrRet = DmxSetRate(pvInst, &rParam);
	...
	case DMX_IOCTL_STM_CREATE: {
		...
		mrRet = dmx_stminst_create(pvInst, &rStmParam, &pvHdl);
		if (DMX_FAILED(mrRet)) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
	...
	case DMX_IOCTL_STM_DESTROY: {
		...
		mrRet = dmx_stminst_destroy(pvInst, rParam.pvSptHdl, rParam.pvStmHdl);
	...
	case DMX_IOCTL_STM_SET_THRESHOLD: {
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			TEXT("DMX_IOCTL_STM_SET_THRESHOLD -- rParam.pvStm: 0x%p, u4Threshold: 0x%08x\r\n"),
			(void *)(rParam.pvStmHdl), rParam.u4Threshold);
	...
	case DMX_IOCTL_PSR_VFIFO_USAGE: {
		...
		mrRet = dmx_stminst_getfifofullness(pvInst, (HANDLE)pvSptHdl,
			SPT_DATA_V, &u4VFifo);
	...
	case DMX_IOCTL_PSR_AFIFO_USAGE: {
		...
		pvSptHdl = rParam.pvSptHdl;
		mrRet = dmx_stminst_getfifofullness(pvInst, (HANDLE)pvSptHdl, SPT_DATA_A, &u4AFifo);
	...
	case DMX_IOCTL_PSR_ON: {
		...
		mrRet = dmx_parse_on(pvInst, &rParam);
	...
	case DMX_IOCTL_PSR_OFF: {
		...
		mrRet = dmx_parse_off(pvInst, (HANDLE)pvSptHdl);
	...
	case DMX_IOCTL_PSR_PAUSE: {
		...
		mrRet = dmx_parse_pause(pvInst, (HANDLE)pvSptHdl);
	...
	case DMX_IOCTL_PSR_FILE_OFST: {
		...
		mrRet = dmx_sptinst_getpsrfileofst(pvInst, pvSptHdl, &u8FileOfst);
	...
	case DMX_IOCTL_RSP_ON: {
		...
		mrRet = dmx_rsp_on(pvInst, &rParam);
	...
	case DMX_IOCTL_RSP_OFF: {
		...
		mrRet = dmx_rsp_off(pvInst, &rParam);
	...
	case DMX_IOCTL_REBUFFER_RANGE: {
		...
		mrRet = dmx_rsp_rebuf(pvInst, &rParam);
	...
	case DMX_IOCTL_CFA_SET_TYPE: {
		...
		mrRet = dmx_cfa_settype(pvInst, &rParam);
	...
	case DMX_IOCTL_CFA_CONFIG: {
		...
		mrRet = dmx_cfa_config(pvInst, rParam.pvSptHdl, rParam.pvConfig, fgIsUserMem);
	...
	case DMX_IOCTL_CFA_SET_RANGE: {
		...
		mrRet = dmx_cfa_setrange(pvInst, rParam.pvSptHdl, rParam.pvRange, fgIsUserMem);
	...
	case DMX_IOCTL_CFA_SET_GEN: {
		...
		mrRet = dmx_cfa_setgeneral(pvInst, rParam.pvSptHdl,
	...
	case DMX_IOCTL_CFA_GET_POSI: {
		...
		pvSptHdl = rParam.pvSptHdl;
		mrRet = dmx_cfa_getpsrpos(pvInst,
	...
	case DMX_IOCTL_PBBUF_ALLOC_BUF: {
		...
		mrRet = dmx_pbbuf_allocbuf(pvInst, rParam.pvSptHdl,
			&rSendBuffer);
	...
	case DMX_IOCTL_PBBUF_CANCEL_BUF: {
		...
		mrRet = dmx_pbbuf_cancelalloc(pvInst, pvSptHdl);
	...
	case DMX_IOCTL_PBBUF_SEND_BUF: {
		...
		mrRet = dmx_pbbuf_sendbuf(pvInst, rBufParam.pvSptHdl,
	...
	case DMX_IOCTL_PBBUF_RELEASE_BUF: {
		...
		mrRet = dmx_pbbuf_releasebuf(pvInst, rParam.pvSptHdl, &rSendBuffer);
	...
	case DMX_IOCTL_PBBUF_NODATA: {
		...
		DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			TEXT("+++++++++++++++++ DMX_IOCTL_PBBUF_NODATA")
				TEXT(" (pvSptHdl: 0x%p, Status: %d) +++++++++++++\r\n"),
			rParam.pvSptHdl, rParam.u4Status);
		dmx_pbbuf_infonodata(pvInst, &rParam);
