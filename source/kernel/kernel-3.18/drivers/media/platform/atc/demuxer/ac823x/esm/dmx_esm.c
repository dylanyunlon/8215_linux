...
void ESM_DumpESMInfo(u32 u4Handle)
{
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
		...
		u4Handle);
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
		TEXT("[ESM] eType: %d(%s), u4FilterType: %d, u4FilterIdx: %d, pvSptHdl: 0x%lx\r\n"),
	...
	*ppvAUInfo = NULL;
	...
	case ES_V: {
			...
			*ppvAUInfo = (void *)(&(paAUTable[u4Value]));
			if (((uintptr_t)(*ppvAUInfo) + sizeof(AU_VPic) > g_arESMInst[u4Handle].prAUTable->ptrSa +
				g_arESMInst[u4Handle].prAUTable->u4Sz) ||
				((uintptr_t)(*ppvAUInfo) < g_arESMInst[u4Handle].prAUTable->ptrSa)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
	...
	case ES_A: {
			...
			*ppvAUInfo = (void *)(&(paAUTable[u4Value]));
			if (((uintptr_t)(*ppvAUInfo) + sizeof(AU_AUDIO) > g_arESMInst[u4Handle].prAUTable->ptrSa +
				g_arESMInst[u4Handle].prAUTable->u4Sz) ||
				((uintptr_t)(*ppvAUInfo) < g_arESMInst[u4Handle].prAUTable->ptrSa)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
	...
	case ES_SP: {
			...
			*ppvAUInfo = (void *)(&(paAUTable[u4Value]));
			if (((uintptr_t)(*ppvAUInfo) + sizeof(AU_SP) > g_arESMInst[u4Handle].prAUTable->ptrSa +
				g_arESMInst[u4Handle].prAUTable->u4Sz) ||
				((uintptr_t)(*ppvAUInfo) < g_arESMInst[u4Handle].prAUTable->ptrSa)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
					TEXT("[ESM] %s line %d(u4Handle: 0x%x) fail for AU's address error,")
	...
	case ES_SECTION: {
			...
			*ppvAUInfo = (void *)(&(paAUTable[u4Value]));
			if (((uintptr_t)(*ppvAUInfo) + sizeof(AU_SECTION) > g_arESMInst[u4Handle].prAUTable->ptrSa +
				g_arESMInst[u4Handle].prAUTable->u4Sz) ||
				((uintptr_t)(*ppvAUInfo) < g_arESMInst[u4Handle].prAUTable->ptrSa)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
					TEXT("[ESM] %s line %d(u4Handle: 0x%x) fail for AU's address error,")
...
MRESULT ESM_FifoSwitch(u32 u4Handle, bool fgUseSwFifo)
{
	...
	if (fgUseSwFifo) {
		if ((ESM_INVALID_ADDRESS == g_arESMInst[u4Handle].prSwFifo->ptrSa) && /* liang luo sw*/
			...
			uintptr_t ptrFifoSa = ESM_INVALID_ADDRESS;
		...
		DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
			TEXT("[ESM] %s line %d (u4Handle: 0x%x, Type: %s) --")
			TEXT(" Set use HwFifo(SA: 0x%p, EA: 0x%p, Sz: 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, u4Handle,
			ESM_TYPESTR(g_arESMInst[u4Handle].eType),
			g_arESMInst[u4Handle].prFifo->ptrSa,
			g_arESMInst[u4Handle].prFifo->ptrEa,
...
MRESULT ESM_FifoSetWrPtr(u32 u4Handle, uintptr_t ptrValue)
{
	...
	if ((ptrValue < g_arESMInst[u4Handle].prFifo->ptrSa) ||
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
			...
			TEXT(" ToSetWPtr(0x%p)exceed fifo size(Sa: 0x%p, Ea: 0x%p)\r\n"),
...
MRESULT ESM_FifoIncWrPtr(u32 u4Handle, uintptr_t ptrValue)
{
	uintptr_t ptrNewWrPtr = 0;
	...
	ptrNewWrPtr = g_arESMInst[u4Handle].prFifo->ptrWrPtr + ptrValue;

	if (ptrNewWrPtr >= g_arESMInst[u4Handle].prFifo->ptrEa) {
		ptrNewWrPtr -= (g_arESMInst[u4Handle].prFifo->ptrEa -
			g_arESMInst[u4Handle].prFifo->ptrSa);
	}

	if ((ptrNewWrPtr < g_arESMInst[u4Handle].prFifo->ptrSa) ||
		(ptrNewWrPtr >= g_arESMInst[u4Handle].prFifo->ptrEa)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
			TEXT("[ESM] %s line %d (u4Handle: 0x%x) fail for invalid param,")
			TEXT(" ptrNewWrPtr(0x%p) exceed fifo size(Sa: 0x%p, Ea: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, u4Handle, ptrNewWrPtr,
			g_arESMInst[u4Handle].prFifo->ptrSa,
			g_arESMInst[u4Handle].prFifo->ptrEa);
		#if DMX_DBG_ESM_INFO
		ESM_DumpESMInfo(u4Handle);
	...
	g_arESMInst[u4Handle].prFifo->ptrWrPtr = ptrNewWrPtr;
...
MRESULT ESM_FifoSetRdPtr(u32 u4Handle, uintptr_t ptrValue, bool fgFF)
{
	if (!g_fgESMInit) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
			TEXT("[ESM] %s line %d fail for ESM hasn't been initialized\r\n"),
	...
	if ((ptrValue < g_arESMInst[u4Handle].prFifo->ptrSa) ||
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
			TEXT("[ESM] %s(u4Handle: 0x%x) fail for invalid param,")
			TEXT(" ptrValue: 0x%p, ptrSa: 0x%p, ptrEa: 0x%p\r\n"),
			DMX_FUNC_NAME, u4Handle, ptrValue,
			g_arESMInst[u4Handle].prFifo->ptrSa,
			g_arESMInst[u4Handle].prFifo->ptrEa);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	...
	g_arESMInst[u4Handle].prFifo->ptrRdPtr = ptrValue;
...
MRESULT ESM_CheckAudDrvStatus(u32 u4Handle)
{
	...
	ptrRp = g_arESMInst[u4Handle].prFifo->ptrRdPtr;
	...
	if (ES_A == g_arESMInst[u4Handle].eType) {
		...
		if ((DMX_INVALID_UINT32 == g_arESMInst[u4Handle].u4StmCodec) ||
			...
			if (g_arESMInst[u4Handle].prFifo == g_arESMInst[u4Handle].prHwFifo) {
				...
				ptrRp2 = rAudPos.ptrAfifoRPtr +	g_arESMInst[u4Handle].prFifo->ptrSa;
				if ((ptrRp == ptrRp2) &&
					(ptrRp2 == g_arESMInst[u4Handle].prFifo->ptrSa)) {
					_fgAudDrvInClearStatus = FALSE;
					DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
						TEXT("[ESM] %s line %d -- Audio DSP RP==0 Now\r\n"),
						DMX_FUNC_NAME, DMX_LINE_NO);
...
#else
				if (ptrRp == g_arESMInst[u4Handle].prFifo->ptrSa) {
					_fgAudDrvInClearStatus = FALSE;
					DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
						TEXT("[ESM] %s line %d -- Audio DSP RP==0 Now\r\n"),
						DMX_FUNC_NAME, DMX_LINE_NO);
...
void _ESM_PrintAUChkSum_V(u32 u4Handle, u32 u4AuIdx)
{
	AU_VPic *paAUTable;
	PicInfo *prInfo;
	uintptr_t ptrTempStartIdx = 0;
	uintptr_t u4ChkSumData = 0;
	...
	if ((NULL != paAUTable) &&
		...
		if (prInfo) {
			...
			ptrTempStartIdx = prInfo->ptrSAddr;
			while (ptrTempStartIdx != prInfo->ptrEAddr) {
				ptrTempStartIdx++;
				if (ptrTempStartIdx > g_arESMInst[u4Handle].prFifo->ptrEa) {
					ptrTempStartIdx = ptrTempStartIdx -
						g_arESMInst[u4Handle].prFifo->ptrEa +
						g_arESMInst[u4Handle].prFifo->ptrSa;
				}
				u4ChkSumData += *((u8 *)(ptrTempStartIdx));
			}
			DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
	...
	if ((NULL != paAUTable) &&
		...
		if (paAUTable) {
			DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
				TEXT("[ESM] AU Idx: %d, type: data\r\n"),
				u4AuIdx);
			ptrTempStartIdx = paAUTable->ptrSAddr;
			while (ptrTempStartIdx != paAUTable->ptrEAddr) {
				ptrTempStartIdx++;
				if (ptrTempStartIdx > g_arESMInst[u4Handle].prFifo->ptrEa) {
					ptrTempStartIdx = ptrTempStartIdx -
						g_arESMInst[u4Handle].prFifo->ptrEa +
...
)
{
	...
	DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
		TEXT("[ESM] Fifo %d/%d (%d%%) Rp: 0x%x, Wp: 0x%x, Sa: 0x%p, Ea: 0x%p\r\n"),
...
)
{
	...
	if (AU_DATA == paAUTable[u4AuIdx].eAuType) {
		...
		if (prInfo) {
			...
			ptrTempStartIdx = prInfo->ptrSAddr + u4StartOffset;
			if (ptrTempStartIdx > g_arESMInst[u4Handle].prFifo->ptrEa) {
				ptrTempStartIdx = ptrTempStartIdx -
					g_arESMInst[u4Handle].prFifo->ptrEa +
					g_arESMInst[u4Handle].prFifo->ptrSa;
	...
	if (AU_DATA == paAUTable[u4AuIdx].eAuType) {
		...
		if (paAUTable) {
			...
			DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
				TEXT("[ESM] AU data info from offset %d, length %d----\r\n"),
				u4StartOffset, u4Length);
			ptrTempStartIdx = paAUTable->ptrSAddr + u4StartOffset;
			if (ptrTempStartIdx > g_arESMInst[u4Handle].prFifo->ptrEa) {
				ptrTempStartIdx = ptrTempStartIdx -
					g_arESMInst[u4Handle].prFifo->ptrEa +
					g_arESMInst[u4Handle].prFifo->ptrSa;
	...
	if (AU_DATA == paAUTable[u4AuIdx].eAuType) {
		...
		if (prInfo) {
			...
			DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_ESM,
				TEXT("[ESM] AU data info from offset %d, length %d----\r\n"),
				u4StartOffset, u4Length);
			ptrTempStartIdx = prInfo->ptrAddr + u4StartOffset;
			if (ptrTempStartIdx > g_arESMInst[u4Handle].prFifo->ptrEa) {
				ptrTempStartIdx = ptrTempStartIdx -
					g_arESMInst[u4Handle].prFifo->ptrEa +
					g_arESMInst[u4Handle].prFifo->ptrSa;
