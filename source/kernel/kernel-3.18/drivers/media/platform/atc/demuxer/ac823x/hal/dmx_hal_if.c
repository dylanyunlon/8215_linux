...
MRESULT DMX_HAL_DMXConfig(DMX_CMDCFG_INFO_T *prDMXCfgInfo)
{
	...
	if (PVR_MAX_MM_COM_Q_ITEM_NUM < prDMXCfgInfo->rPTXCmdInfo.u1CmdNum) {
		DMXLOG_ERROR(TEXT("[PVR] %s -- CmdNum = %d over limitation(%d)\r\n"),
			DMX_FUNC_NAME, prDMXCfgInfo->rPTXCmdInfo.u1CmdNum, PVR_MAX_MM_COM_Q_ITEM_NUM);
		DMX_ASSERT(FALSE);
	...
	if ((1 < prDMXCfgInfo->rPTXCmdInfo.u1CmdNum) &&
		...
		DMXLOG_ERROR(TEXT("[PVR] %s fail for no CmdQ entry, ")
			TEXT("but Cmd Cnt(%d) > 1 ++++++++++++++++\r\n"),
			DMX_FUNC_NAME, prDMXCfgInfo->rPTXCmdInfo.u1CmdNum);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	if (prDMXCfgInfo->u4SrcBufSize > 0xF0000000) {
		DMXLOG_ERROR(TEXT("[PVR] %s fail for Src Data Size(%d) is too large++++++++++++++++\r\n"),
			DMX_FUNC_NAME, prDMXCfgInfo->u4SrcBufSize);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	} else if (prDMXCfgInfo->u4SrcBufSize < 1) {
		DMXLOG_ERROR(TEXT("[PVR] %s fail for Src Data Size(%d) < 1++++++++++++++++\r\n"),
			DMX_FUNC_NAME, prDMXCfgInfo->u4SrcBufSize);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	if (g_fgDmxDmaTwice) {
		MMLOG_ERROR(LOG_MOD_DMX, TEXT("[PVR] %s line %d -- (twice dma), so disable dmx interrupt\r\n"),
		...
		mrRet = DMX_HAL_GetHdrDetStatus(prDMXCfgInfo->rPIDInfo.u4PIDIdx,
			arHdrDectAddr);
		if (DMX_FAILED(mrRet)) {
				MMLOG_ERROR(LOG_MOD_DMX, TEXT("[PVR] %s line %d fail in DMX_HAL_GetHdrDetStatus, mrRet: 0x%08x\r\n"),
...
REPEATDMA:
	...
	if (DMX_FAILED(mrRet)) {
		...
		DMXLOG_ERROR(TEXT("[PVR] %s line %d fail for PID Index(%d) -- u4DstFifo")
			TEXT("(Addr(0x%p), Size(0x%x), WPtr(0x%p), RPtr(0x%p), SPtr(0x%p))\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prDMXCfgInfo->rPIDInfo.u4PIDIdx,
...
u32 DMX_HAL_GetFIFOWPtr(u32 u4HWPIDIndex, DMX_HAL_FIFOType eFIFOType)
{
	u32 u4RegOffset;
	...
	return (u32)PID_S_W(u4HWPIDIndex, u4RegOffset);
}

u32 DMX_HAL_GetFIFORPtr(u32 u4HWPIDIndex, DMX_HAL_FIFOType eFIFOType)
{
	u32 u4RegOffset;
...
void DMX_HAL_SetFIFOWPtr(u32 u4HWPIDIndex, DMX_HAL_FIFOType eFIFOType, u32 u4CurWritePtr)
{
	u32 u4RegOffset;
	...
	PID_S_W(u4HWPIDIndex, u4RegOffset) = u4CurWritePtr;
	mb();

	PVR_LOG_DBG(TEXT("------- [PVR] %s Exit --------\r\n"), DMX_FUNC_NAME);
}

void DMX_HAL_SetFIFORPtr(u32 u4HWPIDIndex, DMX_HAL_FIFOType eFIFOType, u32 u4CurReadPtr)
{
	u32 u4RegOffset;
	...
	PID_S_W(u4HWPIDIndex, u4RegOffset) = u4CurReadPtr;
	mb();
...
MRESULT DMX_HAL_SetHdrDetStatus(u32 u4HWPIDIndex, u32 *pu4HdrBufInfo)
{
	...
	if (NULL != pu4HdrBufInfo) {
		#if DMX_CHECK_MEM_VALIBILITY
		uintptr_t ptrVidHdrBufSa = 0;
		uintptr_t ptrVidHdrBufEa = 0;
		uintptr_t ptrVidHdrBufWp = 0;
		uintptr_t ptrVidHdrBufRp = 0;
		...
		ptrVidHdrBufSa = DMX_NONCACHE_REG(pu4HdrBufInfo[0]);
		ptrVidHdrBufEa = DMX_NONCACHE_REG(pu4HdrBufInfo[1]);
		ptrVidHdrBufWp = DMX_NONCACHE_REG(pu4HdrBufInfo[2]);
		ptrVidHdrBufRp = DMX_NONCACHE_REG(pu4HdrBufInfo[3]);

		if (DMX_EMPTYSIZE(ptrVidHdrBufRp, ptrVidHdrBufWp, (ptrVidHdrBufEa - ptrVidHdrBufSa)) < 3 * sizeof(u32)) {
