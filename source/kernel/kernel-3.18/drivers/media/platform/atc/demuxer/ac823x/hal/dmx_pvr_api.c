...
bool _PVR_Fill_CommandQueue_Items(u32 u4ItemCount, PVR_CMDQ_ENTRY_T *prComQItem)
{
	...
	memset_io((void *)PVR_MM_CMDQ_ENTRY_BASE, 0, (PVR_MAX_MM_COM_Q_ITEM_NUM << 3));
	mb();
...
MRESULT _PVR_SetTxDstFIFO(PVR_PIDCFG_INFO_T *prPIDCfgInfo)
{
	...
	if (0 == DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoAddr)) {
		PVR_LOG_ERR(TEXT("[PVR] %s fail for PID Index(%d)'s ptrDstFifoAddr(0x%p) error\r\n"),
			DMX_FUNC_NAME, u2PIDFilterIdx, prPIDCfgInfo->ptrDstFifoAddr);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	if (0 == DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoSPtr)) {
		PVR_LOG_ERR(TEXT("[PVR] %s fail for PID Index(%d)'s ptrDstFifoSPtr(0x%p) error\r\n"),
			DMX_FUNC_NAME, u2PIDFilterIdx, prPIDCfgInfo->ptrDstFifoSPtr);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	if (0 == DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoWPtr)) {
		PVR_LOG_ERR(TEXT("[PVR] %s fail for PID Index(%d)'s ptrDstFifoWPtr(0x%p) error\r\n"),
			DMX_FUNC_NAME, u2PIDFilterIdx, prPIDCfgInfo->ptrDstFifoWPtr);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	if (0 == DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoRPtr)) {
		PVR_LOG_ERR(TEXT("[PVR] %s fail for PID Index(%d)'s ptrDstFifoRPtr(0x%p) error\r\n"),
	...
	PID_S_W(u2PIDFilterIdx, 5) = (prPIDCfgInfo->ptrDstFifoAddr) ?
		DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoAddr) : 0;
	mb();
	PID_S_W(u2PIDFilterIdx, 7) = (prPIDCfgInfo->ptrDstFifoSPtr) ?
		DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoSPtr) : 0;
	mb();
	PID_S_W(u2PIDFilterIdx, 8) = (prPIDCfgInfo->ptrDstFifoWPtr) ?
		DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoWPtr) : 0;
	mb();
	PID_S_W(u2PIDFilterIdx, 9) = (prPIDCfgInfo->ptrDstFifoRPtr) ?
		DMX_PHYSICAL_REG(prPIDCfgInfo->ptrDstFifoRPtr) : 0;
	mb();
	...
	if (ptrDstFifoEa > 0) {
#if DMX_CHECK_MEM_VALIBILITY
		if (0 == DMX_PHYSICAL_REG(ptrDstFifoEa)) {
...
#endif /* DMX_CHECK_MEM_VALIBILITY*/
		PID_S_W(u2PIDFilterIdx, 6) = (0 != ptrDstFifoEa) ? ((u32)DMX_PHYSICAL_REG(ptrDstFifoEa)) : 0;
		mb();
	} else {
