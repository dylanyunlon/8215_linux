...
bool _PVR_DDI_DeInit(void)
{
	...
	if (_rPvrDDI.hDDINotifyEG) {
		x_ev_group_set_event((uintptr_t)_rPvrDDI.hDDINotifyEG, PVR_DDI_NOTIFY_EV_BUF_EXIT, X_EV_OP_OR);
		x_ev_group_delete((uintptr_t)_rPvrDDI.hDDINotifyEG);
		_rPvrDDI.hDDINotifyEG = NULL;
	}
...
bool _PVR_DDI_Set(u32 u4Flags, const PVR_DDI_T *prDDI)
{
	...
	if (u4Flags & PVR_DDI_FLAG_SET_BUF) {
		...
		/* Buffer start*/
		DDI_WRITE32(DDI_REG_DMA_BUF_START, DMX_PHYSICAL_REG(prDDI->ptrBufAddr));
		/* Buffer end*/
		DDI_WRITE32(DDI_REG_DMA_BUF_END, DMX_PHYSICAL_REG(prDDI->ptrBufAddr) + prDDI->u4BufSize);
		/* Initial RP*/
		DDI_WRITE32(DDI_REG_DMA_RP_INIT, DMX_PHYSICAL_REG(prDDI->ptrBufAddr));
		/* Write pointer*/
		DDI_WRITE32(DDI_REG_DMA_WP, DMX_PHYSICAL_REG(prDDI->ptrBufAddr));
	}
...
bool _PVR_DDI_Get(u32 u4Flags, PVR_DDI_T *prDDI)
{
	...
	if (u4Flags & PVR_DDI_FLAG_DATA_SIZE) {
		uintptr_t ptrBufStart, ptrBufEnd, ptrBufSize, ptrRp, ptrWp;

		if (!_rPvrDDI.fgAllocBuf) {
			prDDI->u4DataSize = 0;
		} else {
			ptrBufStart = DMX_NONCACHE_REG(DDI_READ32(DDI_REG_DMA_BUF_START));
			ptrBufEnd   = DMX_NONCACHE_REG(DDI_READ32(DDI_REG_DMA_BUF_END));
			ptrBufSize = (ptrBufEnd - ptrBufStart);
			ptrWp = DMX_NONCACHE_REG(DDI_READ32(DDI_REG_DMA_WP));
			DMX_ASSERT((ptrBufStart <= ptrWp) && (ptrWp < ptrBufEnd));
			ptrRp = DMX_NONCACHE_REG(DDI_READ32(DDI_REG_DMA_RP));
			if (ptrRp == 0) {
				prDDI->u4DataSize = ptrWp - ptrBufStart;
			} else {
				DMX_ASSERT((ptrBufStart <= ptrRp) && (ptrRp < ptrBufEnd));
				prDDI->u4DataSize = (ptrWp >= ptrRp) ?
									((u32)(ptrWp - ptrRp)) :
									((u32)((ptrWp + ptrBufSize) - ptrRp));
			}
		}
	}
...
void _PVR_DDI_FixDMAEndAddr(u32 *pu4EA, u32 u4WPtr)
{
	if ((*pu4EA - u4WPtr) > DDI_BUF_ALIGNMENT)
		return;

	*pu4EA += DDI_BUF_ALIGNMENT;

	if (u4WPtr >= *pu4EA)
		_PVR_DDI_FixDMAEndAddr(pu4EA, u4WPtr);
}
						 ...
						 uintptr_t ptrSrcAddr, u32 u4Size)
{
	...
	u32 u4PhyBufRp, u4PhyBufWp, u4PhyBufSa, u4PhyBufEa;
	u8  u1SkipBytes;
	...
	u4PhyBufSa = DMX_PHYSICAL_REG(ptrBufferSa);

	DMX_ASSERT(u4PhyBufSa > 0x0270);
	u1SkipBytes = 0;
	if (!_PVR_IsAligned(u4PhyBufSa, DDI_BUF_ALIGNMENT))
		u4PhyBufSa = _PVR_Align_Dec(u4PhyBufSa, DDI_BUF_ALIGNMENT, &u1SkipBytes);

	DMX_ASSERT(_PVR_IsAligned(u4PhyBufSa, DDI_BUF_ALIGNMENT));
	...
	u4PhyBufEa = DMX_PHYSICAL_REG(ptrBufferEa);
	DMX_ASSERT(u4PhyBufEa > 0x0270);
	if (!_PVR_IsAligned(u4PhyBufEa, DDI_BUF_ALIGNMENT))
		u4PhyBufEa = _PVR_Align(u4PhyBufEa, DDI_BUF_ALIGNMENT);
	DMX_ASSERT(_PVR_IsAligned(u4PhyBufEa, DDI_BUF_ALIGNMENT));

	DMX_ASSERT(ptrSrcAddr > 0x0270);
	DMX_ASSERT(u4PhyBufEa > 0x0270);
	/* Buffer Read Address*/
	u4PhyBufRp = DMX_PHYSICAL_REG(ptrSrcAddr);

	if (!((u4PhyBufRp >= u4PhyBufSa) && (u4PhyBufRp < u4PhyBufEa))) {
		PVR_LOG_ERR(TEXT("[PVR] %s fail for invalid args (PhyBufStart: 0x%x, ")
			TEXT("PhyBufEnd: 0x%x, PhyBufRp: 0x%x, Size: 0x%x\r\n"),
			DMX_FUNC_NAME, u4PhyBufSa, u4PhyBufEa, u4PhyBufRp, u4Size);
		DMX_ASSERT(FALSE);
		return FALSE;
	}

	u4PhyBufWp = u4PhyBufRp + u4SrcDataSz;
	DMX_ASSERT(u4PhyBufWp > 0x0270);

	/* Modify Buffer End Address, if the EA-WP <= DDI_BUF_ALIGNEMENT*/
	_PVR_DDI_FixDMAEndAddr(&u4PhyBufEa, u4PhyBufWp);

	u4BufSize = u4PhyBufEa - u4PhyBufSa;

	PVR_LOG_DBG(TEXT("[PVR] %s -- ptrSrcAddr:%p,ptrBufferEa:%p, PhyBufStart: 0x%x, PhyBufEnd: 0x%x, PhyBufRp: 0x%x, Size: 0x%x\r\n"),
		DMX_FUNC_NAME, ptrSrcAddr, ptrBufferEa, u4PhyBufSa, u4PhyBufEa, u4PhyBufRp, u4Size);
	...
	_rPvrDDI.ptrWp = _rPvrDDI.ptrRp + (u32)(u4PhyBufWp - u4PhyBufRp);
	_rPvrDDI.eState = PVR_DDI_PLAY;

	/* Set DDI ring buffer*/
	DMX_ASSERT(u4PhyBufSa > 0x0270);
	DDI_WRITE32(DDI_REG_DMA_BUF_START, u4PhyBufSa);		/* Start pointer*/
	DMX_ASSERT(u4PhyBufEa > 0x0270);
	DDI_WRITE32(DDI_REG_DMA_BUF_END, u4PhyBufEa);		/* End pointer*/
	DMX_ASSERT(u4PhyBufRp > 0x0270);
...
bool _PVR_DDI_SetISREventHandle(HANDLE hEvent)
{
...
bool _PVR_DDI_InitISR(void)
{
	...
	if (!_rPvrDDI.fgDDIISRInited) {
		PVR_LOG_TRACE(TEXT("[PVR] %s -- enable dmx ftup interrupt!\r\n"),
			      DMX_FUNC_NAME);
#if DMX_SUPPORT_DEVICE_TREE
	  /*enable_irq(g_dmxdevinfo->ddi_irq); */
#endif /* DMX_SUPPORT_DEVICE_TREE*/
		UNUSED(pfnOldIsr);
...
bool _PVR_DDI_UninitISR(void)
{
	...
	if (_rPvrDDI.fgDDIISRInited) {
#if DMX_SUPPORT_DEVICE_TREE
		/* disable_irq(g_dmxdevinfo->ddi_irq); */
#endif /* DMX_SUPPORT_DEVICE_TREE*/
		UNUSED(pfnOldIsr);
...
{
	u32 u4Sa,u4Ea, u4Rp, u4Wp, u4RpInit;

	DMXLOG_TRACE(TEXT("[PVR] DDI Before DMA -- SrcBufSa: 0x%x, Sz: 0x%x,")
		TEXT(" Rp: 0x%x, Wp: 0x%x\r\n"),
		DMX_PHYSICAL_REG(_rPvrDDI.ptrBufAddr), _rPvrDDI.u4BufSize,
		DMX_PHYSICAL_REG(_rPvrDDI.ptrRp),
		DMX_PHYSICAL_REG(_rPvrDDI.ptrWp));
	u4Sa = DDI_READ32(DDI_REG_DMA_BUF_START);
	u4Ea = DDI_READ32(DDI_REG_DMA_BUF_END);
	u4RpInit = DDI_READ32(DDI_REG_DMA_RP_INIT);
