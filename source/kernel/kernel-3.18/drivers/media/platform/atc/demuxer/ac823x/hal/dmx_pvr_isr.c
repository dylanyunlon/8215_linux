...
{\
	if ((CHECKADDR) + ((READPTR) << 2) >= ((BUFFEREND)))	 {	\
		...
		(READADDR)	= (u32 *)(DMX_NONCACHE_REG(BUFFERSTART));   } \
}
...
RETRYVIDEO:
	...
	if (0 != u4HdrBufStart)	{
			void * pvHdrBufVSa = (void *)DMX_NONCACHE_REG(u4HdrBufStart);
			if (NULL == pvHdrBufVSa) {
				_PVR_Unlock();
				PVR_LOG_ERR(TEXT("[PVR] %s line %d fail for pvHdrBufVSa == NULL, u4HdrBufStart: 0x%08x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, u4HdrBufStart);
				MM_RETURN(RET_DMX_HW_ERROR);
			}
			__flush_dcache_area(pvHdrBufVSa, (u4HdrBufEnd - u4HdrBufStart));
	}
	...
	pu4PESData = (u32 *)DMX_NONCACHE_REG(u4EndAddr);
	smp_mb();
	...
	if (rFILTERIntStatus.u4DataEndAddr < rFILTERIntStatus.u4DataStartAddr){
			void * pvFlushVSa = (void *)DMX_NONCACHE_REG(rFILTERIntStatus.u4DataStartAddr);
			if (NULL != pvFlushVSa)	{
					__flush_dcache_area(pvFlushVSa, (u4FifoEA_Phy - rFILTERIntStatus.u4DataStartAddr));
			}
			pvFlushVSa = (void *)DMX_NONCACHE_REG(u4FifoSA_Phy);
			if (NULL != pvFlushVSa)	{
					__flush_dcache_area(pvFlushVSa, (rFILTERIntStatus.u4DataEndAddr - u4FifoSA_Phy));
			}
	}
	else if (rFILTERIntStatus.u4DataEndAddr > rFILTERIntStatus.u4DataStartAddr)	{
			void * pvFlushVSa = (void *)DMX_NONCACHE_REG(rFILTERIntStatus.u4DataStartAddr);
			if (NULL != pvFlushVSa)	{
					__flush_dcache_area(pvFlushVSa, (rFILTERIntStatus.u4DataEndAddr - rFILTERIntStatus.u4DataStartAddr));
			}
	}
	...
	if (rFILTERIntStatus.rHdrDectResult.u4PicInfoCount > 0) {
		...
		for (i = 0; i < rFILTERIntStatus.rHdrDectResult.u4PicInfoCount; i++, prPicsInfo++) {
			...
			smp_mb();
			prPicsInfo->u4PicPhyStartAddr = pu4PESData[u4ReadPtr];
			smp_mb();

			if ((prPicsInfo->u4PicPhyStartAddr < u4FifoSA_Phy) ||
				(prPicsInfo->u4PicPhyStartAddr >= u4FifoEA_Phy)) {
				...
				PVR_LOG_ERR(TEXT("[PVR] Assert PicInfo -- PicIdx(%d), PicPhyStartAddr(0x%08x)\r\n"),
					i, (prPicsInfo->u4PicPhyStartAddr));
			...
			PVR_LOG_DBG(TEXT("[PVR] %s line %d -- PicIdx(%d) u4PicInfo: %d, u4PicPhyStartAddr: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, i, u4PicInfo, prPicsInfo->u4PicPhyStartAddr);
			...
			smp_mb();
			prPicsInfo->u4PicPhyStartAddr += i1VCodeOffset;
			smp_mb();

			if (prPicsInfo->u4PicPhyStartAddr < u4FifoSA_Phy)
				prPicsInfo->u4PicPhyStartAddr = u4FifoEA_Phy - (u4FifoSA_Phy - prPicsInfo->u4PicPhyStartAddr);
			...
			smp_mb();
			prPicsInfo->ptrPicStartAddr = DMX_NONCACHE_REG(prPicsInfo->u4PicPhyStartAddr);
			smp_mb();
			if ((uintptr_t)0 == prPicsInfo->ptrPicStartAddr) {
...
RETRYAUDIO:
	...
	if ((u4HdrBufStart != 0) &&
		...
		if (0 != u4HdrBufStart) {
				void * pvFlushVSa = (void *)DMX_NONCACHE_REG(u4HdrBufStart);
				if (NULL != pvFlushVSa)
						__flush_dcache_area(pvFlushVSa, (u4HdrBufEnd - u4HdrBufStart));
		}
		...
		if (u4DataEndAddr < u4DataStartAddr) {
			void * pvFlushVSa = (void *)DMX_NONCACHE_REG(u4DataStartAddr);
			if (NULL != pvFlushVSa)
				__flush_dcache_area(pvFlushVSa, (u4FifoEA_Phy - u4DataStartAddr));

			pvFlushVSa = (void *)DMX_NONCACHE_REG(u4FifoSA_Phy);
			if (NULL != pvFlushVSa)
				__flush_dcache_area(pvFlushVSa, (u4DataEndAddr - u4FifoSA_Phy));

		} else if (u4DataEndAddr > u4DataStartAddr) {
			void * pvFlushVSa = (void *)DMX_NONCACHE_REG(u4DataStartAddr);
			if (NULL != pvFlushVSa)
				__flush_dcache_area(pvFlushVSa, (u4DataEndAddr - u4DataStartAddr));
...
MRESULT _PVR_MMIntrHandler(void *pvArg)
{
	...
	if (NULL == prIntStatus) {
		DMXLOG_ERROR(TEXT("[PVR] %s fail for invalid args\r\n"),
			DMX_FUNC_NAME);
