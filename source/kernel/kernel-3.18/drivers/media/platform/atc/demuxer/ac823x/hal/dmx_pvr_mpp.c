...
static bool _MPP_AllocBuf(const PVR_MPP_T *prMPP)
{
    uintptr_t ptrBufStart =  0, ptrBufEnd = 0;
		u32 u4PhyBufStart = 0, u4PhyBufEnd = 0;
    ...
    u4PhyBufStart = DMX_PHYSICAL_REG(ptrBufStart);
    u4PhyBufEnd   = DMX_PHYSICAL_REG(ptrBufEnd);
    ...
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_START, u4PhyBufStart);     // Buffer start
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_END, u4PhyBufEnd);         // Buffer end
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_RP, u4PhyBufStart);        // Read pointer
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_WP, u4PhyBufStart);        // Write pointer
    // The update of RP will be done by hardware when DMA is activiated.
    _PVR_Unlock();

    PVR_LOG_TRACE(TEXT("%s line %d -- MPP Src Buf's SA(0x%p), EA(0x%p), InitRP(0x%p), WP(0x%p), RP(0x%p)\n"),
        DMX_FUNC_NAME, DMX_LINE_NO,
        ptrBufStart, ptrBufEnd, 
        ptrBufStart, ptrBufStart, ptrBufStart);
...
bool _PVR_MPP_DeInit(void)
{
    ...
    {
        x_ev_group_set_event((uintptr_t)_rPvrMPP.hMPPNotifyEG, PVR_MPP_NOTIFY_EV_BUF_EXIT, X_EV_OP_OR);
        x_ev_group_delete((uintptr_t)_rPvrMPP.hMPPNotifyEG);
        _rPvrMPP.hMPPNotifyEG = NULL;
    }
...
bool _PVR_MPP_Set(u32 u4Flags, const DMX_PVRPLAY_STRUCT_T *prPVRPlay)
{
    ...
    if (u4Flags & PVRPLAY_FLAGS_BUFFER)
    {
        ...
        // Set playback buffer
        DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_START, u4PhyBufStart);
        DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_END, u4PhyBufEnd - 1);
        DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_RP, u4PhyBufStart);
        DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_WP, u4PhyBufStart);
    }
...
bool _PVR_MPP_Get(u32 u4Flags, DMX_PVRPLAY_STRUCT_T *prPVRPlay)
{
    ...
    if (u4Flags & PVRPLAY_FLAGS_BUFFER)
    {
        ...
        prPVRPlay->ptrRp = DMX_NONCACHE_REG(ptrRp);
        prPVRPlay->ptrWp = DMX_NONCACHE_REG(ptrWp);
        PVR_LOG_DBG("%s line %d -- MiniPVR Play Input Buffer's RP(0x%p), RP(0x%p)\r\n",
            DMX_FUNC_NAME, DMX_LINE_NO, DMX_NONCACHE_REG(ptrRp), DMX_NONCACHE_REG(ptrWp));
        _PVR_Unlock();
...
bool _PVR_MPP_GetBufPointer(DMX_PVRPLAY_BUFPTR_T *prPtr)
{
    ...
    _PVR_Lock();
    prPtr->ptrHwWp = DMX_NONCACHE_REG(DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_WP));
    prPtr->ptrHwRp = DMX_NONCACHE_REG(DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_RP));
...
bool _PVR_MPP_SetBuffer(uintptr_t ptrBufStart, uintptr_t ptrBufEnd, uintptr_t ptrWp, uintptr_t ptrRp)
{
    ...
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_START, DMX_PHYSICAL_REG(ptrBufStart));
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_END, DMX_PHYSICAL_REG(ptrBufEnd));
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_WP, DMX_PHYSICAL_REG(ptrWp));
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_RP, DMX_PHYSICAL_REG(ptrRp));
...
*/
//-----------------------------------------------------------------------------
bool _PVR_MPP_SetPacketSize(UINT16 u2PktSize, UINT16 u2PktOutSize)
{
    u32 u4Reg;
    bool fgEnabled = FALSE;
...
void _PVR_MPP_FixDMAEndAddr(u32 *pu4EA, u32 u4WPtr)
{
    if ((*pu4EA - u4WPtr) > DMX_PVR_BUF_ALIGNMENT)
    {
        return;
    }

    *pu4EA += DMX_PVR_BUF_ALIGNMENT;

    if (u4WPtr >= *pu4EA)
    {
        _PVR_MPP_FixDMAEndAddr(pu4EA, u4WPtr);
    }
}
	...
	PFN_MPP_NOTIFY pfnNotify)
{
	...
	mm_memset(&rPlay, 0, sizeof(rPlay));
...
{
    u32 u4PhyBufStart, u4PhyBufEnd, u4PhyRp, u4PhyWp;
    bool fgRet;
    ...
    u4PhyBufStart = DMX_PHYSICAL_REG(ptrBufferSa);
    u1SkipBytes = 0;
    if (!_PVR_IsAligned(u4PhyBufStart, DMX_PVR_BUF_ALIGNMENT))
    {
        u4PhyBufStart = _PVR_Align_Dec(u4PhyBufStart, DMX_PVR_BUF_ALIGNMENT, &u1SkipBytes);
    }
    if (!_PVR_IsAligned(u4PhyBufStart, DMX_PVR_BUF_ALIGNMENT))
    {
        DMX_ASSERT(0);
    ...
    u4PhyBufEnd = DMX_PHYSICAL_REG(ptrBufferEa);
    if (!_PVR_IsAligned(u4PhyBufEnd, DMX_PVR_BUF_ALIGNMENT))
    {
        u4PhyBufEnd = _PVR_Align(u4PhyBufEnd, DMX_PVR_BUF_ALIGNMENT);
    }
    if (!_PVR_IsAligned(u4PhyBufEnd, DMX_PVR_BUF_ALIGNMENT))
    {
        DMX_ASSERT(0);
    ...
    _rPvrMPP.ptrWp = _rPvrMPP.ptrRp + (u4PhyBufEnd - u4PhyBufStart);
    
    _rPvrMPP.u4ThresholdSize = 0;
	
	PVR_LOG_DBG(TEXT("[PVR] %s (PhyBufStart: 0x%x, PhyBufEnd: 0x%x, PhyBufRp: 0x%x, ptrPhyWp: 0x%x\r\n"),
            DMX_FUNC_NAME, u4PhyBufStart, u4PhyBufEnd, u4PhyRp, u4PhyWp);
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_START, u4PhyBufStart);
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_END, u4PhyBufEnd);
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_RP, u4PhyRp);
    DMXCMD_WRITE32(PVR_REG_PVR_PLAY_BUF_WP, u4PhyWp);
...
static bool _PVR_MPP_DoStop(DMX_PVRPLAY_PTR_T *prPtr)
{
    ...
    if (prPtr != NULL)
    {
        ...
        ptrRp = DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_RP);
        ptrWp = DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_WP);
        prPtr->ptrRp = DMX_NONCACHE_REG(ptrRp);
        prPtr->ptrWp = DMX_NONCACHE_REG(ptrWp);
        _PVR_Unlock();
    }
...
bool _PVR_MPP_FlushBuf(void)
{
    ...
    prPvrPlayStruct = &_rPvrMPP;
    ptrBufStart = prPvrPlayStruct->ptrBufStart;
    u4BufPhyStart = DMX_PHYSICAL_REG(ptrBufStart);
...
void _PVR_MPP_GetCounter(DMX_PVRPLAY_COUNTER_T *prCounter)
{
    ...
    if (prCounter != NULL)
    {
        ...
        ptrWp = DMX_NONCACHE_REG(DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_WP));
        ptrRp = DMX_NONCACHE_REG(DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_RP));
...
bool _PVR_MPP_SetISREventHandle(HANDLE hEvent)
{
...
bool _PVR_MPP_DumpInfo(void)
{
    ...
    PVR_LOG_ERR(TEXT("[PVR] MPP Before DMA -- SrcBufSa: 0x%x, Sz: 0x%x, Rp: 0x%x, Wp: 0x%x\r\n"),
        DMX_PHYSICAL_REG(_rPvrMPP.ptrBufStart), _rPvrMPP.u4BufSize,
        DMX_PHYSICAL_REG(_rPvrMPP.ptrRp),
        DMX_PHYSICAL_REG(_rPvrMPP.ptrWp));
    u4Sa = DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_START);
    u4Ea = DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_END);
    ptrRp = DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_RP);
    ptrWp = DMXCMD_READ32(PVR_REG_PVR_PLAY_BUF_WP);
    MMLOG_ERROR(LOG_MOD_DMX, TEXT("[PVR] MPP After DMA -- SrcBufSa: 0x%x, Ea: 0x%x, Rp: 0x%x, Wp: 0x%x\r\n"),
