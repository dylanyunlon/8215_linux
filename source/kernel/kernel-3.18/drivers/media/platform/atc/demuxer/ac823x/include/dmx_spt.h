...
typedef struct {
	...
	HANDLE   hSptTask;	/* /< pointer to splitter task handle. */
	HANDLE   hSptSemaphore;	/* /< pointer to splitter semaphore handle. */
	HANDLE   hSptEvent;	/* /< pointer to splitter event handle. */
...
#define SplitterGetSptTask(pvSptHdl)                                        \
	    ((NULL != (pvSptHdl)) ? &(((DMX_SPT_INST_T *)(pvSptHdl))->pvSptHdlTask) : NULL)
...
#define SplitterSetPtxHandle(pvSptHdl, ptrPsr)                                     \
	    (((DMX_SPT_INST_T *)(pvSptHdl))->pvPsrCC = (ptrPsr))
