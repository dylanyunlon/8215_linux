...
#define PSRHWRESQUELOCKINIT(mrRet)	do { \
	...
	if (DMX_FAILED(mrRet)) {							 \
		DMXLOG_ERROR(TEXT("[DECRYPT] %s fail in create semaphore, mrRet: 0x%x\r\n"), \
			DMX_FUNC_NAME, mrRet); \
	} \
} while (0)
...
static MRESULT PsrHwResQueueInit(void)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s fail in PSRHWRESQUELOCKINIT!!\r\n"),
			    DMX_FUNC_NAME);
		MM_RETURN(mrRet);
	}
	...
	if (NULL == g_rPsrMan.rPSRHWResQueInfo.ppvQueue) {
		...
		if (NULL == g_rPsrMan.rPSRHWResQueInfo.ppvQueue) {
			DMXLOG_ERROR(
				    TEXT("[PSR] %s fail in alloc memory -- HW res cnt: %d!!\r\n"),
				    DMX_FUNC_NAME, MAX_FILTER_COUNT);
		...
		DMXLOG_ERROR(
			    TEXT("[PSR] %s fail -- HW Resource queue already exist!!\r\n"),
			    DMX_FUNC_NAME);
		DMX_ASSERT(FALSE);
...
MRESULT PsrHwResQueueEnQueue(void *pvQueueNode)
{
	...
	if (NULL == prPsrHwResQueInfo->ppvQueue) {
		DMXLOG_ERROR(TEXT("[PSR] %s fail for HW Resource Queue hasn't been")
					  TEXT(" initialized!!\r\n"), DMX_FUNC_NAME);
		DMX_ASSERT(FALSE);
...
MRESULT PSR_CC_CBSplitter(PSR_CC *prPsrCC, PSR_CB_EVENT eEvent, void *pvData)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s fail in Splitter4PsrEvent, pvSptHdl: 0x%x,")
					  TEXT(" prPsrCC: 0x%x, mrRet: 0x%x\r\n"),
			    DMX_FUNC_NAME, prPsrCC->pvSptHdl, prPsrCC, mrRet);
	}
...
bool PsrWakeupOnePsrCC(u32 u4CheckTxState, void *pvDmxInst)
{
	...
	for (i = 0; i < DMX_MAX_SPT_INST_CNT; i++) {
		...
		if ((NULL ==  prSptInst) ||
			(NULL == prSptInst->pvDmxInst) ||
			(pvDmxInst != prSptInst->pvDmxInst))
			continue;
		if ((NULL != prSptInst->pvPsrCC)) {
			prPsrCC = (PSR_CC *) (prSptInst->pvPsrCC);
			DMXLOG_TRACE(TEXT("[PSR] %s line %d -- PsrCC's eTxState: %d\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prPsrCC->eTxState);

			if (TXS_WAIT_HW == prPsrCC->eTxState) {
				DMXLOG_TRACE(TEXT("[PSR] %s line %d -- Wake up ")
							  TEXT("PsrCC(0x%p), pvSptHdl(0x%p)\r\n"),
...
MRESULT ParserSetPowerState(DMX_PM_STATE ePowerState, void **ppvSptHdl)
{
	...
	case D2:
		...
		if ((D3 == eCurPowerState) || (D4 == eCurPowerState)) {
			if (PSR_HAL_IsHWResOccupied(DDI_PVR_DMA_PATH_ID) ||
				PSR_HAL_IsHWResOccupied(MINI_PVR_DMA_PATH_ID)) {
				DMXLOG_TRACE(TEXT("[PVR] %s line %d fail for invalid ")
							TEXT("state, HW should not be occupied\r\n"),
					    DMX_FUNC_NAME, DMX_LINE_NO);
			} else {
				...
				for (i = 0; i < DMX_DEV_CNT; i++){
					...
					if (DMX_FAILED(mrRet)) {
						DMXLOG_ERROR(
							    TEXT("[PSR] %s fail in PSR_HAL_SetVideoCodec,")
							     TEXT(" eVCodeC: %d, mrRet: 0x%x!\r\n"),
							    DMX_FUNC_NAME, eVideoCodec, mrRet);
					}
				}
				mrRet = PSR_SetAudioDataAftSuspend();
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(
						    TEXT("[PSR] %s fail in PSR_SetAudioDataAftSuspend,")
						     TEXT(" eVCodeC: %d, mrRet: 0x%x!\r\n"),
						    DMX_FUNC_NAME, eVideoCodec, mrRet);
				}
	...
	case D4:		/* Power Down */
		if ((D0 == eCurPowerState) || (D1 == eCurPowerState) || (D2 == eCurPowerState)) {
			...
			} else if (PSR_HAL_IsHWResOccupied(MINI_PVR_DMA_PATH_ID)) {
				HANDLE pvPsrCC = NULL;

				PSR_HAL_SetSuspend(MINI_PVR_DMA_PATH_ID, TRUE, &pvPsrCC);
				if ((NULL != pvPsrCC) && (NULL != ppvSptHdl)) {
					*ppvSptHdl = ((PSR_CC *) pvPsrCC)->pvSptHdl;
					DMXLOG_TRACE(
						    TEXT("[PSR] %s -- Demuxer HW is in occupied,")
