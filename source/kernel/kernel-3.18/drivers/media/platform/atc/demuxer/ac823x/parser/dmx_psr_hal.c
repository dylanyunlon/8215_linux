...
#define DMX_IS_WMV_VALID_HDR(PicType)				((u8)0xFF != (PicType))
const u8 _au1WMVHdrTable[8] = {
	0x01, 0x02, 0x1E, 0xF0, 0xFF, 0xF1, 0xF2, 0xF3
};

#define	DMX_IS_MPEG2_VALID_HDR(PicType)				((u8)0xFF != (PicType))
#define	DMX_IS_MPEG2_FRAME_HDR(PicType)				((u8)0xC0 == (PicType))
...
#define	DMX_IS_MPEG4_VALID_HDR(PicType)	(((u8)0xD1 != (PicType)) && ((u8)0xFF != (PicType)))
#define	DMX_IS_MPEG4_FRAME_HDR(PicType)				((u8)0xD0 == (PicType))
...
#define DMX_IS_H263_VALID_HDR(PicType)				((u8)0xFF != (PicType))
const u8 _au1H263HdrTable[6] = {
	0x18, 0x19, 0xFF, 0xFF, 0xFF, 0xFF
};

#define DMX_IS_H264_VALID_HDR(PicType)				((u8)0xFF != (PicType))
#define DMX_IS_H264_AVC_MVC_HDR(PicType)			((u8)0xE2 == (PicType))
...
#define DMX_IS_H265_RSVNVCL_41_44(x)	((((x)&0x5E) < (u8)0x5A) && ((u8)0x52 <= ((x)&0x5E)))
...
static MRESULT _PSR_SetPsrStructInfo(PSR_HALPT_INFO_T *prPTransInfo)
{
	...
	if ((DMX_HW_STATE_OCCUPIED != g_rPSRHalStruct[u1DevID].eState) &&
		...
		DMXLOG_ERROR(TEXT("[PSR] %s fail for PSR Struct is disable!\r\n"),
			    DMX_FUNC_NAME);
	...
	u32 u4Idx, bool fgError)
{
	...
	if ((NULL == prHdrDectResult) ||
		...
		DMXLOG_ERROR(TEXT("[PSR] %s fail for invalid args(u4Idx: %d)\r\n"),
			    DMX_FUNC_NAME, u4Idx);
		return;
	}
	...
	for (i = 0; i < u4HdrCnt; i++) {
		...
		case VC_MPEG2:
			...
			if (DMX_IS_MPEG2_VALID_HDR(u1PicHdrType)) {
				if (DMX_IS_MPEG2_FRAME_HDR(u1PicHdrType)) {
					...
					pbHdrData =
					    DMX_GET_MPEG2_FRAME_HDR_ADDR(prPsrStrHdrDetRst->ptrPicAddr[i]);
				...
				prHdrDetResult->ptrPicAddr[u4PicIndex] = prPsrStrHdrDetRst->ptrPicAddr[i];
				DMXLOG_DEBUG(TEXT("[PSR] line %d --> (%d) PicType: %u, PicAddr: 0x%p\r\n"),
							__LINE__, u4PicIndex, prHdrDetResult->u1PicType[u4PicIndex],
		...
		case VC_VC1:
			...
			if (DMX_IS_WMV_VALID_HDR(u1PicHdrType)) {
				if ((u8)0xF0 == u1PicHdrType)
					u1PicHdrType = PSR_HDR_WMV_SLICE;

				if ((u8)0xF1 == u1PicHdrType) {
					/*Entry Point Header */
	...
	if (DMX_INVALID_UINT8 == prDmxInst->u1DevID) {
		for (u1DevID = 0; u1DevID < DMX_DEV_CNT; u1DevID++){
			if ((!g_rPSRHalStruct[u1DevID].fgUsed) && ((DMX_HW_STATE_IDLE == g_rPSRHalStruct[u1DevID].eState) ||
				...
				MMLOG_DEBUG(LOG_MOD_DMX, TEXT("[PSR] %s line %d, get unused u1DevID:%d.(1)\r\n"),
		...
		MMLOG_DEBUG(LOG_MOD_DMX, TEXT("[PSR] %s line %d, cur used u1DevID:%d.(2)\r\n"),
	            			DMX_FUNC_NAME, DMX_LINE_NO, u1DevID);
...
void PSR_HAL_SetSuspend(u8 u1DevID, bool fgSuspend, void **ppvOwner)
{
	...
	if (DMX_HW_STATE_OCCUPIED == g_rPSRHalStruct[u1DevID].eState) {
		...
		DMXLOG_TRACE(TEXT("[PSR] %s -- Demuxer HW is in occupied, ")
					  TEXT("Set NEEDSUSPEND, PsrCC: 0x%p\r\n"),
...
MRESULT PSR_HAL_SetVideoCodec(u8 u1DevID, VCodeC eVCodeC, bool fgOperForce)
{
	...
	if (!DMX_HAL_SetVideoType(u1DevID, ePvrVideoType)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] % fail in DMX_HAL_SetVideoType, ePvrVideoType: %d\r\n"),
			    DMX_FUNC_NAME, ePvrVideoType);
		PSR_HAL_UNLOCK;
...
MRESULT PSR_HAL_PTransfer(u8 u1DevID, PSR_HALPT_INFO_T *prPTransInfo)
{
	...
	if (prPTransInfo->fgUseCmdQ) {
		DMX_CMDQ_TX_ENTRY_T *prTxEntry =
		    (DMX_CMDQ_TX_ENTRY_T *) (prPTransInfo->pvCmdQTxEntryBuffer);
...
MRESULT PSR_HAL_GetWPtr(u8 u1DevID, PSR_HAL_BitType eBitType, uintptr_t *pptrCurWPtr)
{
	...
	*pptrCurWPtr = (uintptr_t) (DMX_NONCACHE_REG(DMX_HAL_GetFIFOWPtr(u4PIDIdx, FFType_ES)));
...
MRESULT PSR_HAL_SetWPtr(u8 u1DevID, PSR_HAL_BitType eBitType, uintptr_t ptrCurWPtr)
{
	...
	u4PhyAddr = DMX_PHYSICAL_REG(ptrCurWPtr);
	if (0 == u4PhyAddr) {
		PSR_HAL_UNLOCK;
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	DMX_HAL_SetFIFOWPtr(u4PIDIdx, FFType_ES, u4PhyAddr);
	PSR_HAL_UNLOCK;
...
MRESULT PSR_HAL_GetRPtr(u8 u1DevID, PSR_HAL_BitType eBitType, uintptr_t *pptrCurRPtr)
{
	...
	*pptrCurRPtr = (uintptr_t) (DMX_NONCACHE_REG(DMX_HAL_GetFIFORPtr(u4PIDIdx, FFType_ES)));
...
MRESULT PSR_HAL_SetRPtr(u8 u1DevID, PSR_HAL_BitType eBitType, uintptr_t ptrCurReadPtr)
{
	...
	u4PhyAddr = DMX_PHYSICAL_REG(ptrCurReadPtr);
	if (0 == u4PhyAddr) {
		PSR_HAL_UNLOCK;
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	DMX_HAL_SetFIFORPtr(u4PIDIdx, FFType_ES, u4PhyAddr);
	PSR_HAL_UNLOCK;
...
MRESULT PSR_HAL_SetLastHdrDetStatus(u8 u1DevID, PSR_HAL_BitType eBitType, PSR_HDRDET_STATUS_T *prHdrDetStatus)
{
	...
	if (DDI_PVR_DMA_PATH_ID == u1DevID) {
		...
		PSR_HAL_UNLOCK;
		MMLOG_ERROR(LOG_MOD_DMX, TEXT("[PSR] %s, line %d, fail for invalid args(u1DevID:%d).\r\n"),
            DMX_FUNC_NAME, DMX_LINE_NO, u1DevID);
		MM_RETURN(RET_DMX_HW_ERROR);
	}
	PSR_HAL_UNLOCK;
...
MRESULT PSR_HAL_KeepHdrDetResult(u8 u1DevID, DMX_FTI_INTSTATUS_T *pStatus)
{
	...
	for (i = 0; i < u4HdrCnt; i++, prPicInfos++) {
		prPSRStruct->prHdrDetResult->u1PicType[i] = prPicInfos->u1PicType;
		prPSRStruct->prHdrDetResult->ptrPicAddr[i] = prPicInfos->ptrPicStartAddr;

		DMXLOG_DEBUG(TEXT("[PSR] %s -- (%d) --> PicType: %u, PicAddr: 0x%p\r\n"),
			    DMX_FUNC_NAME, i, prPicInfos->u1PicType, prPicInfos->ptrPicStartAddr);
	}
				...
				PSR_HDRDET_STATUS_T *prHdrDetStatus)
{
	...
	if (DDI_PVR_DMA_PATH_ID == prPsrFtr->ucHwDevId) {
		...
		case SPT_DATA_V:
			{
				/* Get the StartCode PictureType-Address Pairs information from the Parser Structure */
				mrRet = _PSR_GetPicHdrDetectStatus_SWKeepResult(prPsrFtr->ucHwDevId,
					prHdrDetResult);
				if (DMX_FAILED(mrRet)) {
					PSR_HAL_UNLOCK;
		...
		case SPT_DATA_V:
		    {
		        ...
		        if (DMX_FAILED(mrRet)) {
		            PSR_HAL_UNLOCK;
		            MMLOG_ERROR(LOG_MOD_DMX, TEXT("[PSR] %s line %d fail in _PSR_GetPicHdrDetectStatus_SWKeepResult, mrRet: 0x%x\r\n"),
		                DMX_FUNC_NAME, DMX_LINE_NO, mrRet);
		            MM_RETURN(mrRet);
		        }
		...
		PSR_HAL_UNLOCK;
		MMLOG_ERROR(LOG_MOD_DMX, TEXT("[PSR] %s line %d fail for device ID(%d) is invalid.\r\n"),
...
MRESULT PSR_HAL_ParsingIntData(void *pvHwData)
{
	...
	if (NULL == pvHwData) {
		DMXLOG_ERROR(TEXT("[PSR] %s fail for invalid arsgs\r\n"),
			    DMX_FUNC_NAME);
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_HAL_GlobalCB(void *pvData, void *pvUserPrivate)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in SplitterSendNfy")
					  TEXT("(pvSptHdl: 0x%x, DMX_SPT_NTY_TX_HW_CB)\r\n"),
		      ...
		      const void *pvNotifyTag)
{
	...
	if (SPT_DATA_V == prPsrFtr->eType) {
		...
		if (!PSR_IsNonHdrVideoType(prVFSD->eVCodeC)) {
			mrRet = PSR_HAL_KeepHdrDetResult(prPsrFtr->ucHwDevId, (DMX_FTI_INTSTATUS_T *) pvData);
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(
		...
		DMXLOG_DEBUG(TEXT("[PSR] %s success, (Video) PicCnt: %d\r\n"),
			    DMX_FUNC_NAME,
			    ((DMX_FTI_INTSTATUS_T *) pvData)->rHdrDectResult.u4PicInfoCount);
	} else {
...
DMX_PM_STATE PSR_HAL_GetPowerState(void)
{
	...
	default:
		DMXLOG_TRACE(
			  TEXT("[DMX] %s -- CurPowerState: UNKNOWN\r\n"), DMX_FUNC_NAME);
		break;
	}
