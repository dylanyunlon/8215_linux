	...
	u32 u4SlotSz, u32 u4HdrParamSz, u8 u1PbbufType)
{
	...
	if ((0 == u4SlotSz) || (u4BufTotalSz < 2 * u4SlotSz)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s fail for invalid args u4BufTotalSz")
			TEXT("0x%x, u4SlotSz: 0x%x !!\r\n"),
			DMX_FUNC_NAME, u4BufTotalSz, u4SlotSz);
...
//#if !DMX_NEW_PBBUF_MECHANISM
	if (!g_u4PbBufFlag) {
	  	if (DMX_MAX_PBBUF_INST_CNT <= prPbBuf->u4CompID) {
	  		DMXLOG_ERROR(
	  			TEXT("[PBBUF] %s fail for invalid Pbbuf's CompID(%d)!\r\n"),
	  	...
	  	if (NULL == prPbBuf->hTxData) {
	  		DMXLOG_ERROR(
	  			TEXT("[PBBUF] %s fail in x_event_open, Pbbuf's CompID(%d)!\r\n"),
	  			DMX_FUNC_NAME, prPbBuf->u4CompID);
	  		DMX_ASSERT(FALSE);
...
//#endif				/* !DMX_NEW_PBBUF_MECHANISM}*/
	...
	if (NULL == pcBufferSa) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s fail in alloc pbbuf (no mem)\r\n"),
			DMX_FUNC_NAME);
	...
	if (NULL == prPbBuf->pcMMRsvBufBase) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s fail in OSE_GetMMReservedMemStartAddr\r\n"),
			DMX_FUNC_NAME);
		MM_RETURN(RET_DMX_OS_OPERA_FAIL);
	}
	DMXLOG_DEBUG(TEXT("[PBBUF] %s line %d -- prPbBuf->pcMMRsvBufBase: %p\r\n"),
		DMX_FUNC_NAME, DMX_LINE_NO, prPbBuf->pcMMRsvBufBase);
	...
	for (u4Idx = 0; u4Idx < prPbBuf->u4SlotAmount; u4Idx++) {
		...
		DMXLOG_DEBUG(TEXT("[PBBUF] %s line %d -- slot(%p) idx(%d), pcBuffer: 0x%p, BufSz: 0x08x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prCurr, u4Idx,
			prCurr->pcBuffer, prCurr->u4BufferSize);
	...
	if (NULL != prPbBuf->pcBufSa) {
		...
		if (NULL != prPbBuf->pcUsrBufSa) {
			...
			if (!VirtualFreeEx
				...
				DMXLOG_ERROR(
					TEXT("[PBBUF] %s fail in VirtualFreeEx, Caller: 0x%x,")
		...
		if (prPbBuf->u4CompID > DMX_MAX_PBBUF_INST_CNT) {
			DMXLOG_ERROR(TEXT("[PBBUF] %s fail for Pbbuf Component")
...
MRESULT PBBUF_SelfBuf_GetAllocSlot(void *pvPbBuf, SEND_BUFFER *prSdBuf)
{
	...
	if (NULL == prPbBuf->pcUsrBufSa) {
		...
		DMXLOG_DEBUG(TEXT("[PBBUF] %s line %d -- prPbBuf->pcUsrBufSa: 0x%p, (0x%p - 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prPbBuf->pcUsrBufSa,
			prPbBuf->pcBufSa, prPbBuf->pcMMRsvBufBase);
		mrRet = E_DMX_OK;
		for (i = 0; i < prPbBuf->u4SlotAmount; i++) {
			prSlot = prPbBuf->prSLOT + i;
			prSlot->pcUsrBuffer = prPbBuf->pcUsrBufSa + i * prPbBuf->u4SlotSz;
			DMXLOG_DEBUG(TEXT("[PBBUF] %s line %d -- slot(%p), idx(%d), pcUsrBufSa(0x%p), pcBufSa(0x%p)\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSlot, i,
				prSlot->pcUsrBuffer, prPbBuf->pcBufSa);
		}
	}
	...
	if (NULL != prSlot) {
		...
		DMXLOG_DEBUG(TEXT("[PBBUF] -- %s line %d, slot(0x%p), pcBuffer(0x%p), Success\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSlot, prSdBuf->pcBuffer);
...
//#if DMX_NEW_PBBUF_MECHANISM
		...
		PBBUF_EXIT(prPbBuf);
  		i4Ret = x_ev_group_wait_event_timeout((uintptr_t)prPbBuf->hEvtGroup,
...
//#if DMX_NEW_PBBUF_MECHANISM
		if (g_u4PbBufFlag) {
  		...
  		if (NULL == prPbBuf->hEvtGroup) {
  			DMXLOG_ERROR(
  				TEXT
  				("[PBBUF] %s fail for prPbBuf(0x%x)'s hEvtGroup hasn't been create!\r\n"),
  		...
  		i4Ret = x_ev_group_set_event((uintptr_t)prPbBuf->hEvtGroup,
  			DMX_PBBUF_EV_STOP_ALLOC, X_EV_OP_OR);
  		if (OSR_OK != i4Ret) {
  			DMXLOG_ERROR(
  				TEXT
  				("[GAU] %s failed in prPbBuf(0x%x) Set Event EV_STOP_ALLOC!!!\r\n"),
  				DMX_FUNC_NAME, prPbBuf);
  			MM_RETURN(RET_DMX_OS_OPERA_FAIL);
  		}
		} else {
...
MRESULT PBBUF_SelfBuf_SendDataSlot(void *pvPbBuf, SEND_BUFFER *prSdBuf, bool *pfgExitSent)
{
	...
	if ((NULL == prSdBuf) || (NULL == prSdBuf->pvBuffer)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s fail for invalid args:")
			TEXT("(pvPbBuf: 0x%x, prSdBuf: 0x%x)\r\n"),
			DMX_FUNC_NAME, pvPbBuf, prSdBuf);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (NULL == prSlot) {
		DMXLOG_ERROR(
			TEXT("[PBBUF] %s fail for Can't find the slot whose handle")
			TEXT(" == 0x%p (pvSptHdl: 0x%p, prPbBuf: 0x%p)\r\n"),
			DMX_FUNC_NAME, prSdBuf->pvBuffer, pvPbBuf, prPbBuf);
		MM_RETURN(RET_DMX_NOT_FOUND);
	}

	if (NULL != prSlot->pcBuffer)
		__flush_dcache_area(prSlot->pcBuffer, prSlot->u4BufferSize);
	...
	if (PBBUF_SLOT_ALLOCATED != prSlot->eType) {
		...
		if (PBBUF_SLOT_NORMAL != prSdBuf->rHeader.eType) {
			...
			if (NULL != prSdBuf->rHeader.pvParam) {
				if (0 != mm_copy_from_user(prSlot->rHeader.pvParam,
	...
	DMXLOG_DEBUG(
		TEXT("[PBBUF] %s line %d exit, Success, SrcOfst: 0x%llx, DataSz: %d\r\n"),
...
MRESULT PBBUF_SelfBuf_ReleaseAllocSlot(void *pvPbBuf, SEND_BUFFER *prSdBuf)
{
	...
	if ((NULL == prSdBuf) || (NULL == prSdBuf->pvBuffer)) {
		DMXLOG_ERROR(
			TEXT
			("[PBBUF] %s fail for invalid args: (pvPbBuf: 0x%x, prSdBuf: 0x%x)\r\n"),
			DMX_FUNC_NAME, pvPbBuf, prSdBuf);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (NULL == prSlot) {
		DMXLOG_ERROR(
			TEXT("[PBBUF] %s fail for Can't find the slot whose handle == 0x%x ")
...
MRESULT PBBUF_SelfBuf_ReleaseNoUseSlot(void *pvPbBuf, DMX_READ_BUFFER *prRdBuf)
{
	...
	if (NULL == prSlot) {
		DMXLOG_ERROR(
			TEXT
			("[PBBUF] %s fail for can't find the reading slot whose handle = ")
					...
					bool *pfgExistUnCon)
{
	...
	if (NULL == prSlot) {
		DMXLOG_ERROR(
			TEXT("[PBBUF] %s fail for can't find the reading slot whose handle")
			TEXT(" = 0x%x, (pvPbBuf: 0x%x, prPbBuf: 0x%x)\r\n"),
			DMX_FUNC_NAME, prRdBuf->pvSlot, pvPbBuf, prPbBuf);
		MM_RETURN(RET_DMX_NOT_FOUND);
	}
