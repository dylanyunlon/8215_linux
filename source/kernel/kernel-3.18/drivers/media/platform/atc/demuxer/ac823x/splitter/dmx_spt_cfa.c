...
MRESULT SptCfaInit(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrInit, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
...
MRESULT SptCfaUninit(void *pvSptHdl)
{
	...
	if (NULL == pvSptHdl) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail for invalid args(pvSptHdl:0x%x)\r\n"),
	...
	if (NULL == prCfaInterface->pfmrUninit) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in pfmrUninit == NULL, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, pvSptHdl);
		MM_RETURN(RET_DMX_NO_IMPLEMENT);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrUninit, pvSptHdl: 0x%p, mrRet: 0x%x\r\n"),
...
MRESULT SptCfaSetInquirer(void *pvSptHdl, u32 u4InquirerTypes)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrSetInqTypes, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaConfigure(void *pvSptHdl, const void *pvCfaParameter, bool fgIsUserMem)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrConfigure, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaSetRange(void *pvSptHdl, const void *pvCfaRange, bool fgIsUserMem)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrSetRange, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaGetPosition(void *pvSptHdl, const void *pvCfaPosition)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrGetCurPos, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
			 ...
			 u32 u4CfaParameterSize)
{
	...
	if (DMX_FAILED(mrRet)) {
		if (!MM_IS_STATE_ERROR(mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in Cfa->pfmrGetGeneral,mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, mrRet, pvSptHdl);
		}
			 ...
			 u32 u4CfaParameterSize)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrSetGeneral, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaEnableStream(void *pvSptHdl, u32 u4CFAStmType, bool fgEnable)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrEnableStrm, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaSetTxDone(void *pvSptHdl, u64 u8TotalTxLen, bool fgRsp)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrTxDone, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaSetTurnOn(void *pvSptHdl)
{
	...
	DMXLOG_DEBUG(
		TEXT("[SPT] %s Cfa->pfmrTurnOn, pvSptHdl: 0x%p\r\n"),
		DMX_FUNC_NAME, mrRet, pvSptHdl);
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrTurnOn, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaSetJumpInfo(void *pvSptHdl, const void *pvJumpInfo)
{
	...
	if (NULL == pvJumpInfo) {
		DMXLOG_ERROR(TEXT("[SPT] %s failed for invalid arg\r\n"),
			DMX_FUNC_NAME);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrSetJumpInfo, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
		       ...
		       u32 u4CfaParameterSize)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrGetParam, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaSetAUInfo(void *pvSptHdl, const void *pvAUInf, const void *pvAUExtInf)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrFillAUInfo, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT SptCfaSetStmInfo(void *pvSptHdl, u32 u4StreamToSet, u32 u4StmUID)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrSetStrmInf, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
			 ...
			 const char *szParam)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrProcCliCmd, mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, mrRet, pvSptHdl);
		MM_RETURN(mrRet);
	}
...
#ifdef CONFIG_COMPAT
long SptCfaProcCompat(void *pvSptHdl, CFA_COMPAT_PROC_INFO_T *prInfo, bool *pfgIsUserMem)
{
	CfaIntf *prCfaInterface   = (CfaIntf *)SplitterGetCfaInterface(pvSptHdl);
	void	*pvCfaPrivateData = SplitterGetCfaPrivateData(pvSptHdl);
	long  ret = 0;
	...
	if (NULL == prCfaInterface->pfmrProcCompat)
		return -EINVAL;

	ret = prCfaInterface->pfmrProcCompat(prInfo, pfgIsUserMem);

	if (0 != ret) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in Cfa->pfmrProcCompat, ret: %ld, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, ret, pvSptHdl);
		return ret;
	}
...
#ifdef __linux__
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterPbb2Buf(u8Len: %lld, u8FileOfst:")
				TEXT(" %lld), mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, u8Len, u8FileOfst, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterPbb2Buf(u8Len: %I64d, u8FileOfst:")
...
#ifdef __linux__
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2Fifo(u8Len: %lld, PictureMode:")
				TEXT(" 0x%x, VidType: %d), mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				    DMX_FUNC_NAME, u8Len, rInf.u4TxPictureMode, eVidType, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2Fifo(u8Len: %I64d,PictureMode: ")
				TEXT(" 0x%x, VidType: %d),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, u8Len, rInf.u4TxPictureMode, eVidType, mrRet, pvSptHdl);
#endif /* #ifdef __linux__*/
		} else
...
#ifdef __linux__
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len: %lld,")
				TEXT("Offset: %lld),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, prVidInf->u8Len, prVidInf->u8FileOfst, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len: %I64d,")
...
#ifdef __linux__
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2Fifo(u8Len: %lld, u4TxUID:")
				TEXT(" 0x%x),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, u8Len, u4TxUID, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2Fifo(u8Len: %I64d, u4TxUID:")
				TEXT(" 0x%x), mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				    DMX_FUNC_NAME, u8Len, u4TxUID, mrRet, pvSptHdl);
#endif /* #ifdef __linux__*/
		} else
	...
	if (DMX_FAILED(mrRet)) {
		...
		if (!MM_IS_STATE_ERROR(mrRet)) {
			...
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len: %lld,")
				TEXT(" Offset: %lld),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, prAudInf->u8Len, prAudInf->u8FileOfst, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len: %I64d,")
				TEXT(" Offset: %I64d),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
...
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2Fifo(u8Len: %I64d,")
	...
	if (DMX_FAILED(mrRet)) {
		...
		if (!MM_IS_STATE_ERROR(mrRet)) {
			...
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len: %lld,")
				TEXT(" u4TxUID: 0x%x),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, prSectionInf->u8Len, rInf.u4TxUID, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len: %I64d,")
				TEXT(" u4TxUID: 0x%x),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
	...
	if (DMX_FAILED(mrRet)) {
		...
		if (!MM_IS_STATE_ERROR(mrRet)) {
			...
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2Fifo(u8Len: %lld, u4TxUID:")
				TEXT(" 0x%x), mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, u8Len, rInf.u4TxUID, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2Fifo(u8Len: %I64d, u4TxUID:")
				TEXT(" 0x%x),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
	...
	if (DMX_FAILED(mrRet)) {
		...
		if (!MM_IS_STATE_ERROR(mrRet)) {
			...
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len: %lld,")
				TEXT(" u4TxUID: 0x%x),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, prSubPicInf->u8Len, rInf.u4TxUID, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterBuf2FifoAUCtrl(u8Len:")
...
MRESULT Spt4CfaPbb2SectionFifoAUCtrl(void *pvSptHdl, CFA_SECTION_INFO_T *prSectionInf)
{
	...
	if (prSectionInf->fgUseCmdQ) {
		...
		if ((prSectionInf->fgUseCmdQ) &&
			...
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail for CmdQ Entry Cnt(%d) > MAXCNT(%d)\r\n"),
				DMX_FUNC_NAME, rInf.u2TxEntryCnt, DMX_MAX_TX_CNT_FOR_CMD_Q);
			DMX_ASSERT(FALSE);
...
#ifdef __linux__
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterPbb2Fifo(u8Len: %lld, Offset:")
				TEXT(" %lld),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, prSectionInf->u8Len,
				prSectionInf->u8FileOfst, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterPbb2Fifo(u8Len: %I64d, Offset:")
				TEXT(" %I64d),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, prSectionInf->u8Len,
				prSectionInf->u8FileOfst, mrRet, pvSptHdl);
#endif /* #ifdef __linux__*/
...
MRESULT Spt4CfaPbb2VFifoAUCtrl(void *pvSptHdl, CFA_VIDEO_INFO_T *prVidInf)
{
	...
	if (rInf.u4TxVideoCodec == CFA_VID_DIVX3) {
		if ((prVidInf->eTxMode != CFA_PTM_ONE_PIC_DX3_I) &&
			...
			DMXLOG_ERROR(
				TEXT("[Spt] %s fail for CFA_VID_DIVX3, eTxMode(0x%x) Err!\r\n"),
				DMX_FUNC_NAME, prVidInf->eTxMode);
			DMX_ASSERT(FALSE);
			MM_RETURN(RET_DMX_PARAM_WRONG);
		}
	}
		...
		if ((prVidInf->eTxMode != CFA_PTM_H263_SORENSON_I) &&
		    ...
		    (prVidInf->eTxMode != CFA_PTM_DUMMY)) {
			DMXLOG_ERROR(
				TEXT("[Spt] %s fail for CFA_VID_H263_SORENSON, eTxMode(0x%x) Err!\r\n"),
				DMX_FUNC_NAME, prVidInf->eTxMode);
			DMX_ASSERT(FALSE);
			MM_RETURN(RET_DMX_PARAM_WRONG);
		}
		...
		if ((prVidInf->eTxMode != CFA_PTM_MJPEG_I) &&
		    (prVidInf->eTxMode != CFA_PTM_DUMMY)) {
			DMXLOG_ERROR(
			TEXT("[Spt] %s fail for CFA_VID_MJPEG, eTxMode(0x%x) Err!\r\n"),
			DMX_FUNC_NAME, prVidInf->eTxMode);
			DMX_ASSERT(FALSE);
			MM_RETURN(RET_DMX_PARAM_WRONG);
		}
		...
		if ((prVidInf->eTxMode != CFA_PTM_ONE_PIC_VP6_I) &&
		    ...
		    (prVidInf->eTxMode != CFA_PTM_DUMMY)) {
			DMXLOG_ERROR(
				TEXT("[Spt] %s fail for CFA_VID_VP6, eTxMode(0x%x) Err!\r\n"),
				DMX_FUNC_NAME, prVidInf->eTxMode);
			DMX_ASSERT(FALSE);
			MM_RETURN(RET_DMX_PARAM_WRONG);
		}
		...
		if ((prVidInf->eTxMode != CFA_PTM_ONE_PIC_VP6_I) &&
		    ...
		    (prVidInf->eTxMode != CFA_PTM_DUMMY)) {
			DMXLOG_ERROR(
				TEXT("[Spt] %s fail for CFA_VID_VP6A, eTxMode(0x%x) Err!\r\n"),
				DMX_FUNC_NAME, prVidInf->eTxMode);
			DMX_ASSERT(FALSE);
			MM_RETURN(RET_DMX_PARAM_WRONG);
		}
		...
		if ((prVidInf->eTxMode != CFA_PTM_ONE_PIC_VP8_I) &&
		    ...
		    (prVidInf->eTxMode != CFA_PTM_DUMMY)) {
			DMXLOG_ERROR(
				TEXT("[Spt] %s fail for CFA_VID_VP8, eTxMode(0x%x) Err!\r\n"),
				DMX_FUNC_NAME, prVidInf->eTxMode);
			DMX_ASSERT(FALSE);
			MM_RETURN(RET_DMX_PARAM_WRONG);
		}
	...
	if ((prVidInf->fgUseCmdQ) &&
		...
		DMXLOG_ERROR(
			TEXT("[Spt] %s fail for CMDQ unsupport video codec(0x%x)!\r\n"),
			DMX_FUNC_NAME, prVidInf->eVidType);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if ((prVidInf->fgUseCmdQ) &&
		...
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail for CmdQ Entry Cnt(%d) > MAXCNT(%d)\r\n"),
			DMX_FUNC_NAME, rInf.u2TxEntryCnt,
	...
	if (prVidInf->fgUseCmdQ) {
		...
		if (rInf.fgInsertHdr) {
			if (rInf.u4InsertHdrLen > DMX_MAX_INST_BYTES_CNT) {
				DMXLOG_ERROR(
				TEXT("[SPT] %s fail for Insert Bytes Count(%d) should be <= %d\r\n"),
				DMX_FUNC_NAME, rInf.u4InsertHdrLen, DMX_MAX_INST_BYTES_CNT);
				DMX_ASSERT(FALSE);
				MM_RETURN(RET_DMX_OVER_LIMIT);
			}
		}
		...
		DmxLogD(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_INSTBS,
			TEXT("[SPT] %s -- Video InsertHdr(%s),HdrLen(%d), HdrBuf(0x%x)\r\n"),
		...
		if (!MM_IS_STATE_ERROR(mrRet)) {
			...
			DMXLOG_ERROR(
				...
				TEXT(" %lld, CMDQ: %d),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				    DMX_FUNC_NAME, prVidInf->u8Len, prVidInf->u8FileOfst,
...
MRESULT Spt4CfaPbb2AFifoAUCtrl(void *pvSptHdl, CFA_AUDIO_INFO_T *prAudInf)
{
	...
	if (DMX_FAILED(mrRet)) {
		...
		if (!MM_IS_STATE_ERROR(mrRet)) {
			...
			TEXT("[SPT] %s fail in SplitterPbb2Fifo(u8Len: %lld, Offset: ")
			TEXT("%lld, CMDQ: %d),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
			DMX_FUNC_NAME, prAudInf->u8Len, prAudInf->u8FileOfst,
			(prAudInf->fgUseCmdQ ? 1 : 0), mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterPbb2Fifo(u8Len: %I64d, ")
...
#ifdef __linux__
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterPbb2Fifo(u8Len: %lld, Offset: %lld)")
				TEXT(",mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				DMX_FUNC_NAME, prSubPicInf->u8Len, prSubPicInf->u8FileOfst,
				mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
				TEXT("[SPT] %s fail in SplitterPbb2Fifo(u8Len: %I64d,")
...
#ifdef __linux__
			DMXLOG_ERROR(
			TEXT("[SPT] %s fail in SplitterPbb2Fifo(u8Len: %lld, ")
			TEXT("Offset: %lld),mrRet: 0x%x, pvSptHdl: 0x%p\r\n"),
				    DMX_FUNC_NAME, u8Len, u8FileOfst, mrRet, pvSptHdl);
#else
			DMXLOG_ERROR(
			...
			if ((NULL != prSptInst)  &&
				...
				if (!(prPsrCC2->fgCfaPrsEnd)) {
					...
					case TXS_WAIT_VFIFO_PTS_THRESHOLD: {
						...
						PSR_CC_CBSplitter((void *)prPsrCC2, E_WAKEUP_ME, NULL);
						PSR_HAL_UNLOCK;
						MM_RETURN(RET_DMX_OK);
					}
					break;
...
static MRESULT Spt4CfaClearStmData(void *hStm, u32 u4SptDataType)
{
