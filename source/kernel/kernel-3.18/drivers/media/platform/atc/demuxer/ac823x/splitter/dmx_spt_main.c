...
MRESULT SptUninitInsts(void)
{
	...
	for (i = 0; i < DMX_MAX_SPT_INST_CNT; i++) {
		...
		if (DMX_FAILED(mrRet)) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
				TEXT("[SPT] %s -- fail in delete pvSptHdl(0x%p) CmdQ, mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, prSpt, mrRet);
			break;
		}
	}
...
MRESULT SptCreateInst(void *pvDmxInst, void **ppvHandle)
{
	...
	*ppvHandle = NULL;
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
			TEXT("[SPT] %s fail in SplitterCreateSema(pvSptHdl:0x%p, mrRet:0x%x)\r\n"),
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
			TEXT("[SPT] %s fail in SplitterCreateEvent(pvSptHdl:0x%p, mrRet:0x%x)\r\n"),
			DMX_FUNC_NAME, pvSptHdl, mrRet);
		goto ERRSPTCREATEINST;
	}
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
			TEXT("[SPT] %s fail in SplitterCreateCmdQ(pvSptHdl:0x%p, mrRet:0x%x)\r\n"),
			DMX_FUNC_NAME, pvSptHdl, mrRet);
		goto ERRSPTCREATEINST;
	}
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
			TEXT("[SPT] %s fail in SplitterCreateTask(pvSptHdl:0x%p, mrRet:0x%x)\r\n"),
			DMX_FUNC_NAME, pvSptHdl, mrRet);
		goto ERRSPTCREATEINST;
	}
	...
	mrRet = SplitterCreatePsr(pvDmxInst, pvSptHdl);
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
			TEXT("[SPT] %s fail in SplitterCreatePsr(pvSptHdl:0x%p, mrRet:0x%x)\r\n"),
			DMX_FUNC_NAME, pvSptHdl, mrRet);
		goto ERRSPTCREATEINST;
	}
	...
	if (DMX_FAILED(mrRet) || (NULL == prSpt->pvPBBuf)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			TEXT("[SPT] %s fail in PBBUF_Connect, pvSptHdl: 0x%p\r\n"),
...
MRESULT SplitterSetAudioMaxDuration(void *pvSptHdl, u8 ucAudSec)
{
	...
	DMXLOG_DEBUG(TEXT("[SPT] %s -- ucAudSec: 0x%x, pvSptHdl: 0x%p \r\n"),
...
{
	void *pvStm = GetStreamByType(prSpt, prSpt->u4PtxToStreamType);
	MRESULT mrRet  = RET_DMX_OK;

	if ((NULL != pvStm) && (!StreamIsEnabled(pvStm) || (SplitterRspIsEnabled(prSpt) &&
	...
	bool fgNeedRspLog)
{
	...
	if (NULL == prSpt) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, TEXT("[SPT] %s fail for invalid args (pvSptHdl: 0x%p)\r\n"),
			DMX_FUNC_NAME, pvSptHdl);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
		TEXT("[SPT] %s fail for Splitter has already started PTX data(pvSptHdl: 0x%p)\r\n"),
		DMX_FUNC_NAME, pvSptHdl);
...
MRESULT SplitterAddStmHandle(void *pvSptHdl, void *pvStm)
{
	...
	if ((NULL == prSpt) || (NULL == pvStm)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, 
			TEXT("[SPT] %s failed for invalid args(pvSptHdl: 0x%p, pvStm: 0x%p)!\r\n"),
			DMX_FUNC_NAME, pvSptHdl, pvStm);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	for (i = 0; i < MAX_SPT_STM_CONNECTED; i++) {
		if (prSpt->pvStmHandles[i] == pvStm)
			MM_RETURN(RET_DMX_ALREADY_EXIST);
	}

	if (prSpt->u4StmHandleNs < MAX_SPT_STM_CONNECTED) {
		prSpt->pvStmHandles[prSpt->u4StmHandleNs++] = pvStm;
		((DMX_STM_INST_T *)pvStm)->pvSptHdl = pvSptHdl;
...
MRESULT SplitterDelStmHandle(void *pvSptHdl, void *pvStm)
{
	...
	if (!((NULL != pvSptHdl) && (NULL != pvStm))) {
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	if (0 == prSpt->u4StmHandleNs) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, TEXT("[SPT] %s line %d failed for Splitter(pvSptHdl:")
	...
	DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT, TEXT("[SPT] %s line %d failed for can't find the steam")
		TEXT("(pvStm: 0x%p) in (pvSptHdl: 0x%p)!\r\n"),
		DMX_FUNC_NAME, DMX_LINE_NO, pvStm, pvSptHdl);
	...
	case RET_DMX_NEED_JUMP:
		{
			...
			if ((NULL != pvSptHdl) && ((CFA_TYPE_AUDIN == SplitterGetCfaType(pvSptHdl)) ||
