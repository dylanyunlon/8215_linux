...
#else
#define DMX_DUMP_FILE_PATH_1	  L"\\Storage Card1\\"
#define DMX_DUMP_FILE_PATH_2	  L"\\SDMMC\\"
#endif
...
)
{
	...
	DmxLogI(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
		...
		TEXT("pfile: 0x%p, curPos: %lld, f_pos: %lld\r\n"),
		__func__, __LINE__, lpFileName,
		pfile, t_cur_pos, pfile->f_pos);
...
)
{
	...
	while (dwWriteTotalSz < nNumberOfBytesToWrite) {
		...
		t_cur_pos = vfs_llseek(pfile, 0, SEEK_END);
		...
		if (u4Size < 0) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
				TEXT("hFile: 0x%p, Dmx vfs_write err, ")
				TEXT("ret = %i, i_size: %lld, t_cur_pos: %lld, f_pos: %lld\r\n"),
				hFile, u4Size, pinode->i_size, t_cur_pos, pfile->f_pos);
	...
	E_DMX_CLI_STM_TYPE_T eCliStmType, char *wszStmType)
{
	...
	g_rDmxDumpMan.arDmxStms[eCliStmType].ptrFifoDataEa = 0;
...
void DmxDumpASample(AU_AUDIO *prAudAU, uintptr_t ptrAFifoVSa,
	uintptr_t ptrAFifoVEa, u32 u4TxUID, bool fgRsping)
{
	u32	dwWriteBytes = 0;
	...
	if (0 == g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa)
		g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa = ptrAFifoVSa;

	if (!fgRsping) {
		if (g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa < prAudAU->ptrEAddr) {
			u32	dwLowSize1 = 0, dwLowSize2 = 0;
			u32	dwHighSize1 = 0, dwHighSize2 = 0;
			...
			if (!WriteFile(g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].hFifoFile,
				(void *)(g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa),
				(prAudAU->ptrEAddr - g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa),
				&dwWriteBytes, NULL)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			...
			sprintf(g_rDmxDumpMan.szDumpInfo,
				...
				(u32)dwHighSize1, (u32)dwLowSize1,
				(u32)dwHighSize2, (u32)dwLowSize2,
				(s64)prAudAU->rAUInfo.rInfo.u8Pts, prAudAU->ptrSAddr, prAudAU->ptrEAddr);
			...
			if (!WriteFile(g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].hFifoFile,
				(void *)(g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa),
				(ptrAFifoVEa - g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa),
					&dwWriteBytes, NULL)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			...
			if (!WriteFile(g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].hFifoFile,
				(void *)ptrAFifoVSa,
				(prAudAU->ptrEAddr - ptrAFifoVSa), &dwWriteBytes, NULL)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
					TEXT("[DUMP] %s line %d failed in write AFIFO ")
					TEXT("File 2.2, err: %d!\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, DMX_GET_LASTERR);
		...
		g_rDmxDumpMan.arDmxStms[DMX_CLI_STM_AUD].ptrFifoDataEa = prAudAU->ptrEAddr;
...
void DmxDumpSPSample(AU_SP *prSPAU, uintptr_t ptrSPFifoVSa,
...
void DmxDumpPbbufData(u64 u8FileOfst, void *pvBuf, u64 u8Len)
{
	...
	if (NULL != pvBuf) {
		...
		sprintf(g_rDmxDumpMan.szDumpInfo,
			"Pbbuf -- FileOfst: 0x%llx, Len: 0x%llx, PbbufSlotHandle: 0x%p\r\n",
			u8FileOfst, u8Len, pvBuf);
	...
	RSP_HDR_MEM_LIST *prHdrMemList, bool fgAdd)
{
	...
	sprintf(g_rDmxDumpMan.szDumpInfo,
		"%s -- NodeAddr: 0x%x, Addr: %p, Size: %d, Prev: 0x%p, Next: 0x%p, Head: 0x%p, Tail: 0x%p, Rp: %p, Wp: %p, Sa: %p, Ea: %p\r\n",
		(fgAdd ? "ADD" : "Remove"),
		prNode, prNode->pvAddr, prNode->u4Size,
		(void *)(prNode->prPrev), (void *)(prNode->prNext),
		(void *)(prHdrMemList->prHead), (void *)(prHdrMemList->prTail),
...
MRESULT DmxDumpPsrCCInfo(void *pvPsrCC)
{
	...
	if (NULL == pvPsrCC) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			TEXT("[CLI] %s fail for invalid param --> pvPsrCC == NULL\r\n"),
			DMX_FUNC_NAME);
		MM_RETURN(RET_DMX_OK);
	}
	...
	DmxLogI(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
		TEXT("CLI] fgUseCmdQ: %d, CmdEntryBufSa: 0x%p, CmdCnt: %d, ")
		TEXT("u2CurTxRngSIdx: %d, u2CurTxRngEIdx: %d\r\n"),
		prPsrCC->fgUseCmdQ, prPsrCC->pvCmdQTxEntryBuffer, prPsrCC->u2TxEntryCnt,
	...
	for (u4Idx = 0; u4Idx < MAX_CACHE_PBBUF; u4Idx++) {
		...
		DmxLogI(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			...
			TEXT(" PlayOfst: 0x%x, PlaySz: 0x%x, pvSlot: 0x%x\r\n"),
			u4Idx,
...
MRESULT DmxDumpPsrFilterInfo(void *pvPsrFtr)
{
	...
	if (NULL == pvPsrFtr) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
			TEXT("[CLI] %s fail for invalid param --> pvPsrFtr == NULL\r\n"),
			DMX_FUNC_NAME);
		MM_RETURN(RET_DMX_OK);
	}
	...
	if ((prPsrFtr->u4Flag) & FF_USED) {
		if ((SPT_DATA_V == prPsrFtr->eType)  ||
			...
			DmxLogI(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
				...
				TEXT("+++++++++++++++++++++++++++++++++++++\r\n"));
			DmxLogI(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
				TEXT("[CLI] DmxInst(0x%x), PsrCC(0x%x), PsrFilter(0x%x)'s")
	...
	DMX_DUMP_FLOW_OPER_INFO_T *prOperInfo)
{
	...
	if (DMX_OPER_PSR_ON == eOperType) {
		...
		sprintf(g_rDmxDumpMan.szDumpInfo, "[0] PSR_ON--> pvSptHdl: 0x%p\r\n",
			(prOperInfo->pvSptHdl));
		...
		sprintf(g_rDmxDumpMan.szDumpInfo, "[%d] PSR_PAUSE--> pvSptHdl: 0x%p\r\n",
		...
		sprintf(g_rDmxDumpMan.szDumpInfo, "[%d] PSR_OFF--> pvSptHdl: 0x%p\r\n",
			(s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
			(prOperInfo->pvSptHdl));
		...
		sprintf(g_rDmxDumpMan.szDumpInfo,
			"[%d] SyncPbbuf--> pvSptHdl: 0x%p, Ofst: 0x%llx, Len: 0x%llx\r\n",
			(s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
			(prOperInfo->pvSptHdl), prOperInfo->unFlow.rSync.u8FileOfst,
		...
		sprintf(g_rDmxDumpMan.szDumpInfo,
			"[%d] SyncPbbufEx--> pvSptHdl: 0x%p, Ofst: 0x%llx, Len: 0x%llx\r\n",
			(s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
			(prOperInfo->pvSptHdl), prOperInfo->unFlow.rSync.u8FileOfst,
			prOperInfo->unFlow.rSync.u8Len);
		...
		if (NULL == prOperInfo->unFlow.rDma.pvBuf) {
			sprintf(g_rDmxDumpMan.szDumpInfo,
				"[%d] SW_START_DMA--> pvSptHdl: 0x%p, StmType: %s, (PBBUF), Ofst: 0x%llx, Len: 0x%llx\r\n",
				 (s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
				 (prOperInfo->pvSptHdl),
			...
			sprintf(g_rDmxDumpMan.szDumpInfo,
				"[%d] SW_START_DMA--> pvSptHdl: 0x%p, StmType: %s, (BUF), Ofst: 0, Len: 0x%llx\r\n",
				 (s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
				 (prOperInfo->pvSptHdl),
				DMX_SPTDATATYPE_STR(prOperInfo->unFlow.rDma.u4StmType),
		...
		if (0 < prOperInfo->unFlow.rDecryptSetting.u4DecryptLen) {
			sprintf(g_rDmxDumpMan.szDumpInfo,
				"[%d] SET_DECRYPT--> pvSptHdl: 0x%p, TURN_ON, (Type: %s), StOfst: 0x%llx, Len: 0x%x, FrameKeyIdx: 0x%02x\r\n",
				(s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
				(prOperInfo->pvSptHdl),
			...
			sprintf(g_rDmxDumpMan.szDumpInfo,
				"[%d] SET_DECRYPT--> pvSptHdl: 0x%p, TURN_OFF\r\n",
		...
		sprintf(g_rDmxDumpMan.szDumpInfo,
			"[%d] COMPOSE_DECRYPT--> pvSptHdl: 0x%p, StmType: %s, (StOfst: 0x%llx, Len: 0x%x), CurSlot(StOfst: 0x%llx, DataSz: 0x%x), Pos: %d\r\n",
			 (s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
			 (prOperInfo->pvSptHdl),
		...
		sprintf(g_rDmxDumpMan.szDumpInfo,
			"[%d] DECRYPT--> pvSptHdl: 0x%p, StmType: %s, Ofst: 0x%llx, Len: 0x%x\r\n",
			(s32)((u32)GetTickCount() - g_rDmxDumpMan.rFlowInfo.u4TickCount),
			(prOperInfo->pvSptHdl),
		...
		if (NULL == prOperInfo->unFlow.rDma.pvBuf) {
			sprintf(g_rDmxDumpMan.szDumpInfo,
				"[%d] HW_PTX--> pvSptHdl: 0x%p, StmType: %s, (PBBUF), Ofst: 0x%llx, Len: 0x%llx\r\n",
				(s32)((u32)GetTickCount() -
				g_rDmxDumpMan.rFlowInfo.u4TickCount),
				(prOperInfo->pvSptHdl),
				DMX_SPTDATATYPE_STR(prOperInfo->unFlow.rDma.u4StmType),
			...
			sprintf(g_rDmxDumpMan.szDumpInfo,
				"[%d] HW_PTX--> pvSptHdl: 0x%p, StmType: %s, (BUF), Ofst: 0, Len: 0x%llx\r\n",
				(s32)((u32)GetTickCount() -
				g_rDmxDumpMan.rFlowInfo.u4TickCount),
				(prOperInfo->pvSptHdl),
				DMX_SPTDATATYPE_STR(prOperInfo->unFlow.rDma.u4StmType),
		...
		sprintf(g_rDmxDumpMan.szDumpInfo,
			"[%d] FillAU-> pvSptHdl: 0x%p, StmType: %s, AUIdx: %d\r\n",
			(s32)((u32)GetTickCount() -
				g_rDmxDumpMan.rFlowInfo.u4TickCount),
			(prOperInfo->pvSptHdl),
		...
		sprintf(g_rDmxDumpMan.szDumpInfo,
			"[%d] GETAU-> pvSptHdl: 0x%p, StmType: %s, AUIdx: %d\r\n",
			(s32)((u32)GetTickCount() -
				g_rDmxDumpMan.rFlowInfo.u4TickCount),
			(prOperInfo->pvSptHdl),
...
void DmxDumpData(HANDLE hFile, uintptr_t ptrFifoRP,
	uintptr_t ptrFifoWP, uintptr_t ptrFifoSa, uintptr_t ptrFifoEa)
{
	u32	dwWriteBytes = 0;
	...
	if (ptrFifoRP < ptrFifoWP) {
		if (!WriteFile(hFile, (void *)(ptrFifoRP),
			(ptrFifoWP - ptrFifoRP), &dwWriteBytes, NULL)) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
		...
		if (!WriteFile(hFile, (void *)(ptrFifoRP),
			(ptrFifoEa - ptrFifoRP), &dwWriteBytes, NULL)) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
				TEXT("[DUMP] %s line %d failed in write FIFO File, err: %d!\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, DMX_GET_LASTERR);
		}
		if (!WriteFile(hFile, (void *)ptrFifoSa,
			(ptrFifoWP - ptrFifoSa), &dwWriteBytes, NULL)) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
				TEXT("[DUMP] %s line %d failed in write FIFO File, err: %d!\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, DMX_GET_LASTERR);
		}
