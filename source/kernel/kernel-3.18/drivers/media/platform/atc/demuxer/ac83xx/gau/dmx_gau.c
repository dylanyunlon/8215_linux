...
static MRESULT JudgeThreshold(void);
	...
	ESM_IO_BUF_INFO *pEsmIOBufInfo)
{
	...
	if (MAX_GAU_INSTANCE_CNT <= u4Handle) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU,
		TEXT("[GAU] %s line %d fail for uintptr_t(%d) > MAX_GAU_INSTANCE_CNT(%d)\r\n"),
...
void GAU_Init(void)
{
	...
	for (u4Idx = 0; u4Idx < MAX_GAU_INSTANCE_CNT; ++u4Idx) {
		...
		g_rDmxGauManager.arGAUInstance[u4Idx].hAUInEG = NULL_HANDLE;
...
void GAU_func_CB(ES_CBEVENT eEvent, void *pvData, void *pvPrivate)
{
	...
	case CBE_AU_IN: {
			...
			if (OSR_OK != x_ev_group_set_event(ins->hAUInEG, GAU_EV_AU_IN, X_EV_OP_OR)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_DEFAULT,
					TEXT(
...
g_aszSptDataTypeName[ins->u4StmType] : TEXT("UNKNOWN")));
			...
			if (OSR_OK != x_ev_group_set_event(ins->hAUInEG, GAU_EV_FIFO_FLUSH, X_EV_OP_OR)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_DEFAULT,
					TEXT(
"[GAU] %s(u4Handle: 0x%x) fail in Set Event GAU_EV_REACH_THRESHOLD!!!\r\n"
),
...
MRESULT GAU_Connect(GAU_CONNECT_PARAM_T *prParam)
{
	...
	if (OSR_OK !=  x_ev_group_create(&(g_rDmxGauManager.arGAUInstance[u4Idx].hAUInEG), tbuf, 0)) {
#else
	...
	if (0 == g_rDmxGauManager.arGAUInstance[u4Idx].rSelfFifoInfo.ptrFifoVSa) {
		...
		if (u4SelfFifoSz > 0) {
			...
			DmxLogD(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_DEFAULT,
				TEXT("[GAU] %s (%s) FifoVSa: 0x%x, FifoPSa: 0x%x, FifoPEa: 0x%x, FifoSz: 0x%x\r\n"),
				DMX_FUNC_NAME,
				(((prParam->u4StmType) < MAX_SPT_DATA_TYPE_CNT) ?
				g_aszSptDataTypeName[prParam->u4StmType] : TEXT("UNKNOWN")),
				g_rDmxGauManager.arGAUInstance[u4Idx].rSelfFifoInfo.ptrFifoVSa,
...
CONNECTERR:
	...
	if (NULL_HANDLE != g_rDmxGauManager.arGAUInstance[u4Idx].hAUInEG)
		x_ev_group_delete(g_rDmxGauManager.arGAUInstance[u4Idx].hAUInEG);
	g_rDmxGauManager.arGAUInstance[u4Idx].hAUInEG = NULL_HANDLE;
...
MRESULT GAU_Disconnect(u32 u4Handle)
{
	...
	if ((NULL_HANDLE != g_rDmxGauManager.arGAUInstance[u4Handle].hAUInEG) &&
		(OSR_OK != x_ev_group_delete(g_rDmxGauManager.arGAUInstance[u4Handle].hAUInEG))) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_DEFAULT,
...
g_aszSptDataTypeName[g_rDmxGauManager.arGAUInstance[u4Handle].u4StmType] : TEXT("UNKNOWN")),
	...
	g_rDmxGauManager.arGAUInstance[u4Handle].hAUInEG = NULL_HANDLE;
...
"[GAU] %s line %d fail in ESM_AUTableGetTotalCount(u4Handle: 0x%x), mrRet: 0x%x\r\n"
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU_V,
			...
			DMX_FUNC_NAME, DMX_LINE_NO);
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU_V,
			TEXT("[GAU] %s line %d FATAL ERROR -- ptrSAddr == 0x%x, u4VType == 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO,
			((AU_VPic *)pvAU)->rAUInfo.rInfo.ptrSAddr,
			((AU_VPic *)pvAU)->rAUInfo.rInfo.u4VType);
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU_V,
			TEXT(
"[GAU] %s line %d FATAL ERROR -- FifoSa == 0x%x, FifoEa == 0x%x, FifoRp == 0x%x, FifoWp == 0x%x\r\n"
),
			DMX_FUNC_NAME, DMX_LINE_NO,
			ptrFifoSa, ptrFifoEa, ptrFifoRp, ptrFifoWp);
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU_V,
			TEXT(
"[GAU] %s line %d FATAL ERROR -- AUTable TotalCnt == 0x%x, WrIdx == 0x%x, RdIdx == 0x%x\r\n"
),
			DMX_FUNC_NAME, DMX_LINE_NO,
			u4AUTableTotalCnt, u4AUTableWrIdx, u4AUTableRdIdx);
	}

	if ((pEsmIOBufInfo->rAU.rVPicAU.rAUInfo.rInfo.ptrSAddr < pEsmIOBufInfo->ptrFifoSPAddr) ||
		(pEsmIOBufInfo->rAU.rVPicAU.rAUInfo.rInfo.ptrSAddr > pEsmIOBufInfo->ptrFifoEPAddr) ||
		(pEsmIOBufInfo->rAU.rVPicAU.rAUInfo.rInfo.ptrEAddr < pEsmIOBufInfo->ptrFifoSPAddr) ||
		(pEsmIOBufInfo->rAU.rVPicAU.rAUInfo.rInfo.ptrEAddr > pEsmIOBufInfo->ptrFifoEPAddr)) {
...
"[GAU] %s line %d fail in ESM_AUTableGetTotalCount(u4Handle: 0x%x), mrRet: 0x%x\r\n"
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU_V,
			TEXT(
"[GAU] %s line %d FATAL ERROR -- u4VType (0x%x), AU's SA(0x%x) or EA(0x%x) beyond Fifo\r\n"
),
			DMX_FUNC_NAME, DMX_LINE_NO,
			((AU_VPic *)pvAU)->rAUInfo.rInfo.u4VType,
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU_V,
			TEXT("[GAU] %s line %d -- FifoSa == 0x%x, FifoEa == 0x%x, FifoRp == 0x%x, FifoWp == 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO,
			ptrFifoSa, ptrFifoEa, ptrFifoRp, ptrFifoWp);
		DmxLogE(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU_V,
	...
	rSelfFifoInfo.ptrFifoVSa));
				...
				#ifdef __linux__
				DMX_NewHwAlignMemory(u4FifoSz, ((u32)1 << 12U), g_rDmxGauManager.arGAUInstance[u4Handle].
	rSelfFifoInfo.ptrFifoVSa);
				#else
...
(void *)(g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoVSa));
				...
				DmxLogD(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU,
					TEXT(
"[GAU] %s (Video) Realloc -- FifoVSa: 0x%x, FifoPSa: 0x%x, FifoPEa: 0x%x, FifoSz: 0x%x\r\n"
),
					DMX_FUNC_NAME,
			...
			if (0 == g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrUserVirSA) {
				...
				DmxLogD(DMX_MOD_OTH, DMX_MOD_GAU_LOGLVL_GETAU,
					TEXT(
					"[GAU] %s (SP/ST) 4 -- FifoVSa: 0x%x, FifoPSa: 0x%x, FifoPEa: 0x%x, FifoSz: 0x%x, UVSa: 0x%x\r\n"
					),
					DMX_FUNC_NAME,
					g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoVSa,
					g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoPSa,
			...
			dmx_memcpy((u8 *)(g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoVSa + u4SzToEnd),
				(u8 *)((uintptr_t)pEsmIOBufInfo->ptrFifoSPAddr), u4DataSz - u4SzToEnd);
			...
			pEsmIOBufInfo->ptrFifoSVAddr = pEsmIOBufInfo->ptrFifoSPAddr;
			pEsmIOBufInfo->ptrFifoEVAddr = pEsmIOBufInfo->ptrFifoEPAddr;
	...
	void			*pvAU)
{
	...
	if ((pEsmIOBufInfo->rAU.rAudioAU.ptrSAddr < pEsmIOBufInfo->ptrFifoSPAddr) ||
		(pEsmIOBufInfo->rAU.rAudioAU.ptrEAddr > pEsmIOBufInfo->ptrFifoEPAddr)) {
		DmxLogE(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU_A,
			TEXT("[GAU] line: %d, Fail for invalid address, ptrSAddr:0x%p, ptrEAddr:0x%p, AFIFO(SA:0x%llx, EA:0x%llx).\r\n"),
			DMX_LINE_NO,
			pEsmIOBufInfo->rAU.rAudioAU.ptrSAddr, pEsmIOBufInfo->rAU.rAudioAU.ptrEAddr,
			pEsmIOBufInfo->ptrFifoSPAddr, pEsmIOBufInfo->ptrFifoEPAddr);
		pEsmIOBufInfo->u4Status = GAU_E_TIMEOUT;
		mrRet = GAU_Q_ReleaseTailInUseAU(&(g_rDmxGauManager.arGAUInstance[u4Handle].rGetAUQueue));
		if (RET_DMX_OK != mrRet)
		{
			DmxLogE(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU_A,
				TEXT("[GAU] Fail for release tail au in use au list.\r\n"));
			MM_RETURN(mrRet);
		}
		...
		if (pEsmIOBufInfo->rAU.rAudioAU.ptrSAddr > pEsmIOBufInfo->rAU.rAudioAU.ptrEAddr) {
			...
			if (u4DataSz > g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.u4FifoSz) {
				...
				DmxLogD(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU,
					TEXT(
					"[GAU] %s (Audio) Realloc -- FifoVSa: 0x%x, FifoPSa: 0x%x, FifoPEa: 0x%x, FifoSz: 0x%x\r\n"
					),
					DMX_FUNC_NAME,
					g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoVSa,
					g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoPSa,
	...
	void			*pvAU)
{
	...
	if (0 == g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrUserVirSA) {
		...
		DmxLogD(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU,
			TEXT("[GAU] %s (SP/ST) 4 -- FifoVSa: 0x%x, FifoPSa: 0x%x, FifoPEa: 0x%x, ")
			TEXT("FifoSz: 0x%x, UVSa: 0x%x\r\n"),
			DMX_FUNC_NAME,
			g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoVSa,
			g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoPSa,
			g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrFifoPEa,
	...
	pEsmIOBufInfo->rAU.rSPStruct.ptrUserVirSA =
		g_rDmxGauManager.arGAUInstance[u4Handle].rSelfFifoInfo.ptrUserVirSA;
	DmxLogD(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU,
		TEXT("[GAU] %s (SP), u4VirSa: 0x%x, UserVirSA: 0x%x, AUSz: 0x%x\r\n"),
	...
	void			*pvAU)
{
	...
	if (ptrSAddr >= ptrEAddr) {
		...
		u4SzToEnd =
			(u32)(pEsmIOBufInfo->ptrFifoEPAddr - pSPAU->rAUInfo.rInfo.ptrAddr);
	...
	void			*pvAU)
{
	...
	pEsmIOBufInfo->ptrFifoSPAddr = (u32)(DMX_PHYSICAL(pEsmIOBufInfo->ptrFifoSPAddr));
	pEsmIOBufInfo->ptrFifoEPAddr = (u32)(DMX_PHYSICAL(pEsmIOBufInfo->ptrFifoEPAddr));
...
MRESULT GAU_GetSectionAUEx(u32 u4Handle, void *pvIOBuf, u32 u4BuffSize)
{
	...
	if (!(g_rDmxGauManager.arGAUInstance[u4Handle].fgGetAUEnable)) {
		x_ev_group_wait_event_timeout(
			g_rDmxGauManager.arGAUInstance[u4Handle].hAUInEG,
	...
	if ((u4AvailCount < (u32)1) && (!(g_rDmxGauManager.arGAUInstance[u4Handle].fgEOS))) {
		...
		i4WaitResult =	x_ev_group_wait_event_timeout(g_rDmxGauManager.arGAUInstance[u4Handle].hAUInEG,
			(GAU_EV_AU_IN		  |
			 GAU_EV_FIFO_FLUSH	  |
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU_SEC,
			TEXT(
				"[GAU] %s line %d (u4Handle: 0x%x) fail in ESM_FifoSetRdPtr, u4AuEa: 0x%x, eType: 0x%x, u4RdIdx: 0x%x, i4Ret: 0x%x\r\n"
			),
			DMX_FUNC_NAME, DMX_LINE_NO, u4Handle, u4AuEa, eType, u4RdIdx, mrRet);
		return 0;
	}
...
MRESULT GAU_GetAU(u32 u4Handle, void *pvIOBuf)
{
	...
	u32	u4RdIdx = ESM_INVALID_INDEX;
	MRESULT mrRet	= RET_DMX_OK;
	uintptr_t ptrFifoSPAddr = 0;
	uintptr_t ptrFifoEPAddr = 0;
	...
	while (TRUE) {
		...
		pEsmIOBufInfo->ptrFifoSPAddr = ptrFifoSPAddr;
		...
		pEsmIOBufInfo->ptrFifoEPAddr = ptrFifoEPAddr;
		if ((pEsmIOBufInfo->ptrFifoEPAddr < 0x20000000) ||
			(pEsmIOBufInfo->ptrFifoSPAddr < 0x20000000)) {
			DmxLogE(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU,
...
#if DMX_DISABLE_GET_REALAU
		{
			...
			if (ES_V == eType) {
				u4AuEa = ((AU_VPic *)pvAU)->rAUInfo.rInfo.ptrEAddr;
			} else if (ES_A == eType) {
				u4AuEa = ((AU_AUDIO *)pvAU)->ptrEAddr;
			} else if (ES_SP == eType) {
				if (NULL == pEsmIOBufInfo) {
					DmxLogE(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU,
						TEXT(
				...
				if (pEsmIOBufInfo->ptrFifoEPAddr > pEsmIOBufInfo->ptrFifoSPAddr) {
					...
					pEsmIOBufInfo->rAU.rSPStruct.ptrAUEndAddrOfFifo =
						pEsmIOBufInfo->ptrFifoSPAddr
							+ pEsmIOBufInfo->rAU.rSPStruct.ptrAUEndAddrOfFifo;
					u4AuEa = pEsmIOBufInfo->rAU.rSPStruct.ptrAUEndAddrOfFifo;
				} else {
					DmxLogE(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_GETAU,
						TEXT(
							"[GAU] %s line %d (Section) is fail for SP's FifoEA(0x%x) <= FifoSA(0x%x)\r\n"
						),
						DMX_FUNC_NAME, DMX_LINE_NO,
						pEsmIOBufInfo->ptrFifoEPAddr,
						pEsmIOBufInfo->ptrFifoSPAddr);
...
MRESULT GAU_ReleaseAU(u32 u4Handle, void *pvIOBuf, bool fgFF)
{
	...
	while (NULL != prElem) {
		...
		if (ES_V == eType) {
			...
			if (0 == g_rDmxGauManager.arGAUInstance[u4Handle].u8DecSendBufMask) {
				u4AuEa = ((AU_AUDIO *)pvAU)->ptrEAddr;
				ptrQueAUAddrSa = ((AU_AUDIO*)pvAU)->ptrSAddr;
				ptrRelAUAddrSa = prEsmIOBufInfo->rAU.rAudioAU.ptrSAddr + prEsmIOBufInfo->ptrFifoSVAddr;
			} else {
				u4AuEa = ((AU_AUDIO *)pvAU)->ptrEAddr;
				ptrQueAUAddrSa = ((AU_AUDIO*)pvAU)->ptrSAddr;
			...
			#else //CONFIG_ANDROID_ENABLE
			u4AuEa = ((AU_AUDIO *)pvAU)->ptrEAddr;
			ptrQueAUAddrSa = ((AU_AUDIO*)pvAU)->ptrSAddr;
			ptrRelAUAddrSa = prEsmIOBufInfo->rAU.rAudioAU.ptrSAddr + prEsmIOBufInfo->ptrFifoSVAddr;
			#endif //CONFIG_ANDROID_ENABLE
			...
			u4AuEa = prEsmIOBufInfo->rAU.rSPStruct.ptrAUEndAddrOfFifo;
			...
			u4AuEa = prEsmIOBufInfo->rAU.rSectionAU.ptrEAddr;
		...
		if (ptrRelAUAddrSa != ptrQueAUAddrSa) {
			DmxLogD(DMX_MOD_GAU, DMX_MOD_GAU_LOGLVL_RELAU,
				TEXT("[GAU] %s line %d (u4Handle: 0x%x) u4RdIdx(%d) is invalid in ESM_AUTableGetRdIdx\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, u4Handle, u4RdIdx);
		} else {
			break;
		}
...
MRESULT GAU_SetEvent(u32 u4Handle, EV_GRP_EVENT_T u4SetEvent)
{
	...
	if (x_ev_group_set_event(g_rDmxGauManager.arGAUInstance[u4Handle].hAUInEG,
...
MRESULT GAU_Enable(u32 u4Handle, bool fgEnable)
{
	...
	if (g_rDmxGauManager.arGAUInstance[u4Handle].fgGetAUEnable != fgEnable) {
		if (!fgEnable) {
			...
			if (OSR_OK != x_ev_group_set_event(g_rDmxGauManager.arGAUInstance[u4Handle].hAUInEG,
...
void GAU_DisableThreshold(void)
{
	if (!g_rDmxGauManager.fgReachThreshold) {
		...
		for (u4Idx = 0; u4Idx < MAX_GAU_INSTANCE_CNT; u4Idx++) {
			...
			if (OSR_OK != x_ev_group_set_event(g_rDmxGauManager.arGAUInstance[u4Idx].hAUInEG,
