...
#define	DDI_READ32(off)					IO_READ32(g_dmxdevinfo->dmx_ddi_regs, (off))
#define	DDI_WRITE32(off, val)			do {\
	IO_WRITE32(g_dmxdevinfo->dmx_ddi_regs, (off), (val)); mb();\
} while (0)
...
EXTERN void _PVR_DDI_FixDMAEndAddr(uintptr_t *pptrEA, uintptr_t ptrWPtr);
EXTERN bool _PVR_DDI_InitISR(void);
EXTERN bool _PVR_DDI_UninitISR(void);
EXTERN void _PVR_DDI_IrqHandler(u16 u2Vector);

EXTERN bool _PVR_DDI_SetISREventHandle(HANDLE_T hEvent);
