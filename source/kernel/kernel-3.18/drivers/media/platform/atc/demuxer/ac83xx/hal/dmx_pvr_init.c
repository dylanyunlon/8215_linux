...
VOID _PVR_DisableDramLocalArbiter(void)
{
    ...
    u4DramReg = IO_READ32(IO_BASE_VA + 0x52004, 0);
    u4DramReg &= ~(1 << 25);
    IO_WRITE32(IO_BASE_VA + 0x52004, 0, u4DramReg);
    u4DramReg = IO_READ32(IO_BASE_VA + 0x52104, 0);
    u4DramReg &= ~(1 << 31);
    IO_WRITE32(IO_BASE_VA + 0x52104, 0, u4DramReg);
...
void _PVR_EnableDramLocalArbiter(void)
{
    ...
    #if PVR_DBG_LOCAL_ARBITER
    u4DramReg1 = IO_READ32(IO_BASE_VA + 0x52004, 0);
    u4DramReg2 = IO_READ32(IO_BASE_VA + 0x52104, 0);
    
    u4DramReg = IO_READ32(IO_BASE_VA + 0x6130, 0);
    u4DramReg &= ~(1 << 0);
    IO_WRITE32(IO_BASE_VA + 0x6130, 0, u4DramReg);
    u4DramReg |= (1 << 0);
    IO_WRITE32(IO_BASE_VA + 0x6130, 0, u4DramReg);
...
void _PVR_DumpDramKeyRegs(bool fgFirstClear)
{
    ...
    u4DramReg = IO_READ32(IO_BASE_VA + 0x7008, 0);
    PVR_LOG_INFO(TEXT("[PVR] %s line %d -- 0x7008=0x%08x\r\n"),
            DMX_FUNC_NAME, DMX_LINE_NO, u4DramReg);
    
    u46134Reg = IO_READ32(IO_BASE_VA + 0x6134, 0);
    u46154Reg = IO_READ32(IO_BASE_VA + 0x6154, 0);
    u46158Reg = IO_READ32(IO_BASE_VA + 0x6158, 0);
    u4615cReg = IO_READ32(IO_BASE_VA + 0x615c, 0);
    ...
    u452004Reg = IO_READ32(IO_BASE_VA + 0x52004, 0);
    u452004Reg &= ~(0x1F << 16);
    u452004Reg |= (0x8 << 16);
    IO_WRITE32(IO_BASE_VA + 0x52004, 0, u452004Reg);
    
    u452018Reg1 = IO_READ32(IO_BASE_VA + 0x52018, 0);
    u452018Reg2 = IO_READ32(IO_BASE_VA + 0x52018, 0);
    ...
    u452104Reg = IO_READ32(IO_BASE_VA + 0x52104, 0);
    u452104Reg &= ~(0x1F << 16);
    u452104Reg |= (0x8 << 16);
    IO_WRITE32(IO_BASE_VA + 0x52104, 0, u452004Reg);

    u452018Reg1 = IO_READ32(IO_BASE_VA + 0x52118, 0);
    u452018Reg2 = IO_READ32(IO_BASE_VA + 0x52118, 0);
    ...
    u45201cReg1 = IO_READ32(IO_BASE_VA + 0x5201c, 0);
    u45201cReg2 = IO_READ32(IO_BASE_VA + 0x5201c, 0);
    
    PVR_LOG_INFO(TEXT("[PVR] %s -- 0x5201c: 0x%08x, 0x%08x\r\n"),
            DMX_FUNC_NAME, u45201cReg1, u45201cReg2);
    
    u45201cReg1 = IO_READ32(IO_BASE_VA + 0x5211c, 0);
    u45201cReg2 = IO_READ32(IO_BASE_VA + 0x5211c, 0);
...
void _PVR_CkgenInit(bool fgOn, PVR_INPUT_TYPE_T eInputType)
{
	...
	if (fgOn) {
		...
		PVR_LOG_INFO(TEXT("Reg[0x54]: 0x%08x, Reg[0xA0]: 0x%08x, Reg[0x0C]:")
			     ...
			     CKGEN_READ32(0x54), CKGEN_READ32(0xA0), CKGEN_READ32(0x0C),
			     CKGEN_READ32(0x30), CKGEN_READ32(0xBC));
...
#if DMX_SUPPORT_DEVICE_TREE
		clk_disable_unprepare(g_dmxdevinfo->dmx_27m_gateclk);
		clk_disable_unprepare(g_dmxdevinfo->dmx_ts0_gateclk);
		clk_disable_unprepare(g_dmxdevinfo->dmx_ts1_gateclk);
...
static void _DmxInitFVR(void)
{
	...
	memset((void *)&(FVR_GBL_ARY_W(0, 0)), 0, FVR_GBL_SIZE * 4);
	memset((void *)&(FVR_GBL_ARY_W(1, 0)), 0, FVR_GBL_SIZE * 4);

	/*reset DMX record hardware common region 124byte */
	memset((void *)FVR_GBL_COMMON_REGION, 0, FVR_GBL_COMMON_SIZE * 4);

	/* Reset the Record PID data structure region */
	for (i = 0; i < FVR_NUM_PID_INDEX; i++) {
		FVR_PID_INDEX_TABLE(i) = 0;
		memset((void *)&(FVR_PER_PID_S(i)), 0, FVR_PID_SIZE * 4);
	}
...
static bool _DmxInitMultimedia(void)
{
	...
	mm_memset((void *)(PVR_DMEM_BASE), 0, PVR_DMEM_MAX_LEN);

	/*---------------------------------------------------------*/
	/* Clear Play back global region */
	/*---------------------------------------------------------*/
	mm_memset((void *)PLAYBACK_GBL_BASE, 0, PLAYBACK_GBL_SIZE * 4);
	...
	for (i = 0; i < PVR_NUM_PID_INDEX; i++) {
		PID_INDEX_TABLE(i) = 0;
		memset((void *)&(PID_S(i)), 0, PVR_DMEM_ENTRY_LEN * 4);
	...
	memset((void *)PVR_SECTION_CONTROL, 0, 4 * PVR_NUM_FILTER_INDEX);
	mb();
	memset((void *)PVR_SECTION_POSNEG, 0, 8 * PVR_NUM_FILTER_INDEX);
	mb();
	memset((void *)PVR_SECTION_PATTERN, 0, 8 * PVR_NUM_FILTER_INDEX);
	mb();
	memset((void *)PVR_SECTION_MASK, 0, 8 * PVR_NUM_FILTER_INDEX);
	mb();

	PVR_LOG_DBG(TEXT("[DMX][HW] %s -- Clear One-Byte Section filters\r\n"), DMX_FUNC_NAME);
	/* One-Byte Section Filter Patten-Mask Region */
	memset((void *)PVR_ONEBYTE_FILTER_BASE, 0, 2 * PVR_NUM_ONEBYTE_FILTER_INDEX);
	smp_mb();
...
static bool _DmxDeInitMultimedia(void)
{
	...
	for (i = 0;i < DMX_DEV_CNT; i++){
		if (!_PVR_SetVideoType(i, PVR_VIDEO_UNKNOWN)){
			PVR_LOG_ERR(TEXT("[DMX][HW] %s -- fail in _PVR_SetVideoType\r\n"), DMX_FUNC_NAME);
			FUNC_EXIT;
			return FALSE;
		}
	}
	...
	for (i = 0; i < PVR_NUM_PID_INDEX; i++) {
		...
		memset((void *)prPidStruct, 0, sizeof(PVR_PID_STRUCT_T));
	}
	smp_mb();
...
MRESULT _PVR_InitAllocMem(void)
{
	...
	memset((void *) (_rDmxPvrInfo.prPvrPidStructs), 0,
	...
	memset((void *) (_rDmxPvrInfo.prPicInfos), 0,
...
MRESULT _PVR_Init(PVR_INPUT_TYPE_T eInputType)
{
    ...
    for (i = 0; i < PVR_NUM_PID_INDEX; i++)
    {
        PVR_PID_STRUCT_T* prPidStruct;
        prPidStruct = _PVR_GetPidStruct(i);
        x_memset((void*)prPidStruct, 0, sizeof(PVR_PID_STRUCT_T));
    }
    smp_mb();
