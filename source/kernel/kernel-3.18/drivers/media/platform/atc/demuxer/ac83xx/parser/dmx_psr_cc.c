	...
	case TXS_TX_JUMP: {
			...
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d fail in ")
					TEXT("SplitterChangeState(pvSptHdl: 0x%p, RUNING, JUMP)\r\n"),
...
MRESULT PSR_CC_Create(void *pvDmxInst, void *pvSptHdl, void *pvPBBuf, void **ppvPsrCC)
{
	...
	if (NULL == ppvPsrCC) {
		DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for invalid args (pvSptHdl: 0x%p,")
			TEXT(" pvPBBuf: 0x%p, ppvPsrCC: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pvPBBuf, ppvPsrCC);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	*ppvPsrCC = NULL;
	...
	if (MAX_PSR_CC_CNT == u4Idx) {
		DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for no unused PSR CC ")
			TEXT("Instance(pvSptHdl: 0x%p)\r\n"), DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PSR_UNLOCK;
	...
	DMX_NewMemory((sizeof(DMX_CMDQ_TX_ENTRY_T) * DMX_MAX_TX_CNT_FOR_CMD_Q),
		prPsrCC->u4CmdQTxEntryBuffer);
	#else
	DMX_NewMemory((sizeof(DMX_CMDQ_TX_ENTRY_T) * DMX_MAX_TX_CNT_FOR_CMD_Q),
		(void *)(prPsrCC->u4CmdQTxEntryBuffer));
	#endif /* #ifdef __linux__*/
	if (0 == prPsrCC->u4CmdQTxEntryBuffer) {
		DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail in dmx_mem_alloc(NO MEM),")
			TEXT(" (pvSptHdl: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PSR_UNLOCK;
	...
	prPsrCC->u8FileEndOffset        = DMX_INVALID_UINT64;
	...
	if (DMX_FAILED(mrRet)) {
		...
		prPsrCC->u4Flag &= ~CCF_USED;
		#ifdef __linux__
		DMX_FreeMemory(prPsrCC->u4CmdQTxEntryBuffer);
		#else
		DMX_FreeMemory((void *)(prPsrCC->u4CmdQTxEntryBuffer));
		#endif /* #ifdef __linux__*/
		PSR_UNLOCK;
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_CC_Destroy(void *pvPsrCC)
{
	...
	if (prPsrCC->u4CmdQTxEntryBuffer != 0) {
		#ifdef __linux__
		DMX_FreeMemory(prPsrCC->u4CmdQTxEntryBuffer);
		#else
		DMX_FreeMemory((void *)(prPsrCC->u4CmdQTxEntryBuffer));
		#endif /* #ifdef __linux__*/
		prPsrCC->u4CmdQTxEntryBuffer = 0;
	}
...
MRESULT PSR_CC_ResumeTx(PSR_CC *prPsrCC)
{
	...
	if (prPsrFtr->eType != SPT_DATA_BUF) {
		...
		if (NULL == prDMASD) {
			DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for PSR_DMASD has't been")
				TEXT(" allocated\r\n"), DMX_FUNC_NAME, DMX_LINE_NO);
			PSR_CC_UNLOCK(prPsrCC->rLock);
			MM_RETURN(RET_DMX_NO_INIT);
		}
...
MRESULT PSR_CC_GetCurrentOffset(PSR_CC *prPsrCC, u64 *pu8Offset)
{
	...
	if (NULL == pu8Offset) {
		DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for invalid args,")
			TEXT(" (prPsrCC: 0x%x, pu8Offset: 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prPsrCC, pu8Offset);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
...
MRESULT PSR_CC_AttachFilter(PSR_CC *prPsrCC, void *hPsrFtr)
{
	...
	for (u4Idx = 0; u4Idx < MAX_PSR_FILTER_PER_CC; u4Idx++) {
		if (hPsrFtr == prPsrCC->apvFtr[u4Idx]) {
			DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for the PsrFtr:0x%x ")
				TEXT("already exists\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, hPsrFtr);
			PSR_CC_UNLOCK(prPsrCC->rLock);
	...
	for (u4Idx = 0; u4Idx < MAX_PSR_FILTER_PER_CC; u4Idx++) {
		if (NULL == prPsrCC->apvFtr[u4Idx]) {
			...
			if (prPsrCC->u4PsrFtrCnt > MAX_PSR_FILTER_PER_CC) {
				DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for PsrCC(0x%x)'s ")
					TEXT("existed filter Count > Limit(%d)\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prPsrCC, MAX_PSR_FILTER_PER_CC);
				PSR_CC_UNLOCK(prPsrCC->rLock);
				MM_RETURN(RET_DMX_OVER_LIMIT);
			}
			break;
	...
	if (MAX_PSR_FILTER_PER_CC == u4Idx) {
		DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for can't find the free ")
	...
	for (u4Idx = 0; u4Idx < MAX_PSR_FILTER_PER_CC; u4Idx++) {
		if (hPsrFtr == prPsrCC->apvFtr[u4Idx]) {
			...
			if (0 == prPsrCC->u4PsrFtrCnt) {
				DMXLOG_ERROR(TEXT("[PSR_CC] %s line %d, fail for PsrCC(0x%x)'s existed")
...
MRESULT PSR_CC_GetWMVParsingMode(PSR_CC *prPsrCC, u8 *pucWMVParsingMode)
{
	...
	*pucWMVParsingMode = 0;
	...
	if (SPT_DATA_V == prPsrFtr->eType) {
		...
		DMXLOG_ERROR(TEXT("[PSR] %s line %d, fail for PsrFtr's Type(%d) is not Video\r\n"),
...
void PSR_CC_DumpCurrentState(PSR_CC *prPsrCC)
{
	...
	if (NULL == prPsrCC) {
		DMXLOG_ERROR(TEXT("[CLI] %s line %d, fail for invalid param --> prPsrCC")
			TEXT(" == NULL\r\n"), DMX_FUNC_NAME, DMX_LINE_NO);
		return;
	}
...
MRESULT PSR_CC_Reset4NonConPbbufSlot(PSR_CC *prPsrCC)
{
	...
	if (NULL == prPsrCC) {
		DMXLOG_ERROR(TEXT("[CLI] %s line %d, fail for invalid param --> prPsrCC == NULL\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if ((0 != (prPsrCC->u4Flag & CCF_CPS_ON)) &&
		...
		if (DECRYPT_COMPLETE != prPsrCC->rDecryptMan.eStatus) {
			if (DECRYPT_ACCROSS_SLOTS == prPsrCC->rDecryptMan.rHWParam.rDRM.eInSlotType) {
				...
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(TEXT("[PSR] %s line %d, fail in calculate hw ")
