	...
	*pfgFull = FALSE;
	...
	if (NULL == prPsrCC) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_FIFOFULL,
			TEXT("[PSR] %s fail for PsrFtr(0x%p)'s pvPsrCC == NULL\r\n"),
				...
				for (i = 0; i < DMX_MAX_SPT_INST_CNT; i++) {
					prPsrCC2 = (PSR_CC *) ((NULL !=
								g_rSptMan.aprSptInst[i]) ?
										 ((g_rSptMan.aprSptInst[i])->
								pvPsrCC) : NULL);
					if ((NULL == prPsrCC2) || (prPsrCC2->pvDmxInst != prDmxInst))
						continue;
...
bool PSR_Filter_ESICBCheckPsrEnd(PSR_FILTER *prPsrFtr)
{
	...
	for (i = 0; i < DMX_MAX_SPT_INST_CNT; i++) {
		...
		if ((NULL == prSptInst) || (prSptInst->pvDmxInst != prDmxInst))
			continue;
...
MRESULT PSR_AFilter_UpdateESIInfo(PSR_FILTER *prPsrFtr)
{
	...
	fgAUCtrlByDefault = !(prPsrFtr->fgAUCtrlByEnd || prPsrFtr->fgAUCtrlByLen || (prPsrCC->fgUseCmdQ && prPsrCC->fgAUByCmdQEnd));
	...
	if (prPsrFtr->u4Flag & FF_TX_PBBUF) {
		...
		if (((prPsrCC->u8TxStartOffset + prPsrCC->u8TxLen) !=
				(prPsrCC->u8TxCurrOffset + u8TxSize)) && (!fgAUCtrlByDefault)) {
				...
				if (0 == prAudAU->ptrSAddr) {
					...
					DmxLogD(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_COMPAU_A,
						TEXT("[PSR] %s line %d -- ByCmdQ  Set prAudAU(AUIdx: %d)")
						 TEXT("->ptrSAddr = u4CurStartWp(0x%p)\r\n"),
...
MRESULT PSR_SectionFilter_UpdateESIInfo(PSR_FILTER *prPsrFtr)
{
	...
	/*fix klocwork bug */
	if (NULL == prSectionAU) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d, prSectionAU = NULL Failed!\r\n"),DMX_FUNC_NAME, DMX_LINE_NO);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	AU_VPic *prVPicAU, uintptr_t ptrCurFIFOWp, u32 u4PicIdx)
{
	...
	ptrPicAddr = prVFSD->rPicDetResult.ptrPicAddr[u4PicIdx];

	/* Calculate offset */
	if (ptrPicAddr >= ptrCurFIFOWp) {
		i4CurPicAdvLen = (prVFSD->rPicDetResult.ptrPicAddr[u4PicIdx] -
					ptrCurFIFOWp);
	} else if ((VC_H264 == prVFSD->eVCodeC || VC_H265 == prVFSD->eVCodeC) 
				&& (ptrPicAddr >= ptrCurFIFOWp - 3)){
		/*H.264, H.265 codec may insert 0x000001*/
		i4CurPicAdvLen = ((prVFSD->rPicDetResult.ptrPicAddr[u4PicIdx] +
					 3) - ptrCurFIFOWp);
	} else if ((VC_VC1 == prVFSD->eVCodeC) && (ptrPicAddr >= ptrCurFIFOWp - 4)) {
		/*VC1 codec insert 4bytes(0x000001xx)*/
		i4CurPicAdvLen = ((prVFSD->rPicDetResult.ptrPicAddr[u4PicIdx] +
					 4) - ptrCurFIFOWp);
	} else {
		i4CurPicAdvLen = ((prVFSD->rPicDetResult.ptrPicAddr[u4PicIdx] +
	...
	if ((prPsrCC->fgUseCmdQ) &&
		...
		if (0 == prPsrCC->u4CmdQTxEntryBuffer) {
			DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_COMPAU_V,
				TEXT("[PSR] %s line %d update MP2 ESI, ")
				TEXT("cmd q buffer is null, fatal error!!\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO);
			DMX_ASSERT(FALSE);
			MM_RETURN(RET_DMX_UNEXPECT);
		}
	...
	uintptr_t ptrHWCurWp, uintptr_t ptrCurFIFOWp)
{
	...
	while (u4Idx < prVFSD->rPicDetResult.u1PicInfoCount) {
		...
		DmxLogD(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_COMPAU_V,
			TEXT("MPEG2: Create AU(Idx:%d) -- VType: 0x%x, SA: 0x%x")
...
MRESULT PSR_VFilter_UpdateESIInfo_H264(PSR_FILTER *prPsrFtr,
	u32 u4CurAUWrIdx, AU_VPic *prVPicAU,
	u32 ptrHWCurWp, u32 ptrCurFIFOWp)
{
	PSR_AU rAU;
	PSR_CC *prPsrCC = NULL;
	PSR_VFSD *prVFSD = NULL;
	...
	while (u4Idx < prVFSD->rPicDetResult.u1PicInfoCount) {
		...
		case PSR_HDR_AVC_NONIDR:{
				...
				if ((u32) pb >= prPsrFtr->ptrESFifoEa)
	...
	while (u4Idx < prVFSD->rPicDetResult.u1PicInfoCount) {
		...
		case PSR_HDR_WMV_SeqHdr:
			if (!prVFSD->fgHasSetPics) {
				...
				if ((u32) pb9 >= prPsrFtr->ptrESFifoEa)
		...
		case PSR_HDR_WMV_FRAME:
			if (!prVFSD->fgHasSetPics) {
				...
				if ((u32) pb4 >= prPsrFtr->ptrESFifoEa)
		...
		case PSR_HDR_WMV_SLICE:{
				...
				if ((u32) ucBuf >= prPsrFtr->ptrESFifoEa)
