...
static void PSR_Filter_InitHALStatus(PSR_FILTER *prPsrFtr, PSR_HDRDET_STATUS_T *prHeaderDetect)
{
	...
	case SPT_DATA_V:
		{
			u32 u4PhysMem = DMX_PHYSICAL(prPsrFtr->ptrHdrBufAddr);	/* 32K */
...
MRESULT PSR_Filter_SetVDummyInfo(PSR_FILTER *prPsrFtr, bool fgDummyAUEnd, bool fgDummyCmdAU)
{
	...
	if (SPT_DATA_V != prPsrFtr->eType) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d, fail for PsrFilter(0x%x)'s Type(%d) != Video!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr, prPsrFtr->eType);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (NULL == prVFSD) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d, fail for PSR_VFSD hasn't been allocated!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		DMX_ASSERT(FALSE);
...
MRESULT PSR_Filter_AddDummyAU(PSR_FILTER *prPsrFtr, bool fgDummyAUEnd, bool fgDummyCmdAU)
{
	...
	if (NULL == prPsrCC) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d, fail for PsrFtr(0x%x)'s pvPsrCC == NULL\r\n"),
	...
	if (prPsrFtr->u4StmType != SPT_DATA_V) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d, fail for PsrFtr(0x%x)'s StmType(%d) != SPT_DATA_V\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr, prPsrFtr->eType);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (NULL == prVFSD) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d, fail for prPsrFtr(0x%x)'s prVFSD == NULL\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d, fail in PSR_Filter_IsAUTableFull, PsrCC's")
			  ...
			  void **ppvHandle)
{
	...
	if ((NULL == ppvHandle) || (NULL == g_rPsrMan.aprPsrFtrs[0])) {
		DMXLOG_ERROR(TEXT("[STM] %s line %d failed for invalid args, (u4PsrFtrType: ")
					  TEXT("0x%x, u4PsrFtlId: 0x%x, ppvHandle: 0x%x\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, u4PsrFtrType, u4PsrFtrId, ppvHandle);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	*ppvHandle = NULL;
	...
	if (MAX_FILTER_COUNT == u4Idx) {
		DMXLOG_ERROR(TEXT("[STM] %s line %d failed for no unsed PSR_FILTER, ")
					  TEXT
					  ("(u4PsrFtrType: 0x%x, u4PsrFtlId: 0x%x, ppvHandle: 0x%x\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, u4PsrFtrType, u4PsrFtrId, ppvHandle);
	...
	g_rPsrMan.aprPsrFtrs[u4Idx]->fgDST = FALSE;
	...
	g_rPsrMan.aprPsrFtrs[u4Idx]->u8FileEndOffset = DMX_INVALID_UINT64;
	...
	case SPT_DATA_V:
		{
			...
			if (NULL == prVFSD) {
				DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for PsrFtr(0x%x)'s ")
							  TEXT
			...
			DMXLOG_DEBUG(
				    TEXT
				    ("[PSR] %s line %d -- VideoHdrBufAddr: 0x%08x, PhyMem: 0x%08x\r\n"),
				    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->ptrHdrBufAddr,
				    DMX_PHYSICAL(prPsrFtr->ptrHdrBufAddr));
		}
		break;
	case SPT_DATA_A:
		{
			...
			if (NULL == prAFSD) {
				DMXLOG_ERROR(
					    TEXT("[PSR] %s line %d, fail for PsrFtr(0x%x)'s PSR_AUDFSD ")
			...
			DMXLOG_DEBUG(
				    TEXT("%s line %d -- AudHdrBufAddr: 0x%08x, PhyMem: 0x%08x\r\n"),
				    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->ptrHdrBufAddr,
				    DMX_PHYSICAL(prPsrFtr->ptrHdrBufAddr));
			prPsrFtr->u8HdrPTS = INVALID_TIMESTAMP;
			prPsrFtr->u8LastPTS = INVALID_TIMESTAMP;
	...
	case SPT_DATA_SECTION:
		{
			...
			DMXLOG_DEBUG(
				    TEXT
				    ("%s line %d -- SP or Section HdrBufAddr: 0x%08x, PhyMem: 0x%08x\r\n"),
				    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->ptrHdrBufAddr,
				    DMX_PHYSICAL(prPsrFtr->ptrHdrBufAddr));
			prPsrFtr->u8HdrPTS = INVALID_TIMESTAMP;
			prPsrFtr->u8LastPTS = INVALID_TIMESTAMP;
...
MRESULT PSR_Filter_SetVCodeC(PSR_FILTER *prPsrFtr)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			    TEXT
			    ("[PSR] %s line %d fail in PSR_HAL_SetVideoCodec, eVCodeC: %d, mrRet: 0x%x!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prVFSD->eVCodeC, mrRet);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_Filter_TxToGround(PSR_FILTER *prPsrFtr)
{
	...
	if (NULL == prPsrCC) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for PsrFtr(0x%x)'s PsrCC == NULL!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
...
MRESULT PSR_Filter_TriggerHALGTx(PSR_FILTER *prPsrFtr)
{
...

	{
		...
		if (prPsrCC->u8TxCurrOffset >= prPsrCC->u8TxStartOffset) {
			ptrTxDstSa =
			    (u32) (prPsrCC->u8TxCurrOffset - prPsrCC->u8TxStartOffset) +
...
MRESULT PSR_Filter_TriggerHALPTx(PSR_FILTER *prPsrFtr)
{
	...
	default:
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for invalid prPsrFtr->eType: %d\r\n"),
	...
	if (prPsrCC->fgUseCmdQ) {
		if (0 == prPsrCC->u4CmdQTxEntryBuffer) {
		...
		rPTInfo.u4CmdQTxEntryBuffer = prPsrCC->u4CmdQTxEntryBuffer;
		...
		if (SPT_DATA_V == prPsrFtr->eType) {
			if ((rPTInfo.u2EntryWrIdx == prPsrCC->u2TxEntryCnt - 1) &&
				...
				DMX_CMDQ_TX_ENTRY_T *prTxEntry =
				    (DMX_CMDQ_TX_ENTRY_T *) (prPsrCC->u4CmdQTxEntryBuffer);
				u32 u4EndDataOfst = 0;
				u32 u4CmdIdx = 0;
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in PSR_HAL_PTransfer, ccHdl:%x, ")
					  TEXT("St:0x%x, TxSt:0x%x, Flag:0x%x\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrCC, prPsrCC->eState, prPsrCC->eTxState,
...
MRESULT PSR_Filter_TxPbbuf(PSR_FILTER *prPsrFtr, u64 u8Offset)
{
	...
	if (TXS_WAIT_PBBUF == prPsrCC->eTxState) {
		...
		if ((DMX_INVALID_UINT64 != prPsrCC->u8FileEndOffset) && (u8Offset >= prPsrCC->u8FileEndOffset)) {
			prPsrCC->eState = CCS_INIT;
			PSR_CC_SetTxSt(prPsrCC, TXS_TX_OK);
			PSR_CC_CBSplitter(prPsrCC, E_TX_DONE, NULL);
			MM_RETURN(RET_DMX_OK);
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for in ")
						  TEXT
						  ("PSR_CC_CheckPBBuf, pvPsrCC: 0x%x, mrRet: 0x%x!\r\n"),
...
MRESULT PSR_Filter_Tx4HdrParsing(PSR_FILTER *prPsrFtr)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for in PSR_CC")
					  TEXT("_CheckPBBuf, pvPsrCC: 0x%x, mrRet: 0x%x!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrCC, mrRet);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
	...
	if (PSR_CC_IsOffsetInPbbuf(prPsrCC, u4PbbufIdx, (u8Offset + u4Len - 1))) {
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in ")
						  TEXT
						  ("PSR_CC_GetWaitTxBufSa, mrRet: 0x%x, PsrCC: 0x%x!!\r\n"),
		...
		mrRet = PSR_CC_GetWaitTxBufSa(prPsrCC, &ptrPbbufSa);
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in ")
						  TEXT
						  ("PSR_CC_GetWaitTxBufSa, mrRet: 0x%x, PsrCC: 0x%x!!\r\n"),
		...
		if (prPsrCC->arPBBuf[u4PbbufIdx2].pcPlayBuffer !=
			...
			dmx_memcpy((void *) prDMASD->ptrPrivMemSa, (void *) ptrPbbufSa, u4CpyLen);
	...
	if (NULL != prDMASD->pu4TgtHdrPrsSa)
		*(prDMASD->pu4TgtHdrPrsSa) = ptrTgtSa;
...
MRESULT PSR_Filter_IRQ_Proc(PSR_FILTER *prPsrFtr)
{
	...
	if (0 == (prPsrFtr->u4Flag & FF_ENABLE)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for PsrFilter is disable\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
...
MRESULT PSR_Filter_SetType(PSR_FILTER *prPsrFtr, E_SPT_DATA_TYPE_T eType, u64 u8DecSendBufMask)
{
	...
	prPsrCC = (PSR_CC *) (prPsrFtr->pvPsrCC);
	if (NULL == prPsrCC) {
		DMXLOG_ERROR(TEXT("[PSR] % line %d fail for no corresponding PsrCC\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	if (SPT_DATA_BUF == eType) {
		...
		if (NULL == prPsrFtr->pvFilterSpecific) {
			DMXLOG_ERROR(
				    TEXT("[PSR] % line %d fail in alloc PSR_DMASD (no Memory) \r\n"),
				    DMX_FUNC_NAME, DMX_LINE_NO);
			MM_RETURN(RET_DMX_NO_MEM);
		}
	...
	case SPT_DATA_V:
		if (NULL == prPsrFtr->pvFilterSpecific) {
			...
			if (NULL == prPsrFtr->pvFilterSpecific) {
				DMXLOG_ERROR(
					    TEXT("[PSR] % line %d fail in alloc PSR_VFSD (no Memory) \r\n"),
					    DMX_FUNC_NAME, DMX_LINE_NO);
				MM_RETURN(RET_DMX_NO_MEM);
			}
	...
	case SPT_DATA_A:
		if (NULL == prPsrFtr->pvFilterSpecific) {
			...
			if (NULL == prPsrFtr->pvFilterSpecific) {
				DMXLOG_ERROR(
					    TEXT
					    ("[PSR] % line %d fail in alloc PSR_AUDFSD (no Memory) \r\n"),
					    DMX_FUNC_NAME, DMX_LINE_NO);
				MM_RETURN(RET_DMX_NO_MEM);
			}
	...
	case SPT_DATA_SECTION:
		if (NULL == prPsrFtr->pvFilterSpecific) {
			...
			if (NULL == prPsrFtr->pvFilterSpecific) {
				DMXLOG_ERROR(
					    TEXT
					    ("[PSR] % line %d fail in alloc PSR_NORMALFSD (no Memory) \r\n"),
					    DMX_FUNC_NAME, DMX_LINE_NO);
				MM_RETURN(RET_DMX_NO_MEM);
			}
	...
	if (DMX_INVALID_UINT32 != prPsrFtr->u4ESIH) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for prPsrFtr->u4ESIH has already created\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
	...
	default:
		DMXLOG_ERROR(TEXT("[Psr] %s line %d failed for invalid ESType: 0x%x\r\n"),
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in ESM_SetESType \r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_Filter_IsFifoFull(PSR_FILTER *prPsrFtr, bool *pfgFull, u64 u8TxLen)
{
	...
	*pfgFull = FALSE;
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail in PSR_Filter_IsESBufFull, mrRet: 0x%x\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, mrRet);
		MM_RETURN(mrRet);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail in PSR_Filter_IsAUTableFull, mrRet: 0x%x\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, mrRet);
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_Filter_SwitchFifo(PSR_FILTER *prPsrFtr)
{
	...
	case SPT_DATA_V:
		{
			...
			if (NULL == prVFSD) {
				DMXLOG_ERROR(
					    TEXT
					    ("[PSR] %s line %d fail for PsrFtr(Video)'s prVFSD == NULL\r\n"),
					    DMX_FUNC_NAME, DMX_LINE_NO);
				MM_RETURN(RET_DMX_PARAM_WRONG);
			}
	...
	case SPT_DATA_A:
		{
			...
			if (NULL == prAFSD) {
				DMXLOG_ERROR(
					    TEXT
					    ("[PSR] %s line %d fail for PsrFtr(Audio)'s prAFSD == NULL\r\n"),
					    DMX_FUNC_NAME, DMX_LINE_NO);
				MM_RETURN(RET_DMX_PARAM_WRONG);
			}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in ESM_FifoSwitch(ESIH: 0x%x)\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->u4ESIH);
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_Filter_SetCodeC(PSR_FILTER *prPsrFtr, u32 u4Codec)
{
	...
	if (NULL == prPsrFtr) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for invalid args(u4Codec: %d)\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, u4Codec);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
					  ...
					  PSR_AUCtrlInfo *prAUCtrlInfo)
{
	...
	if ((NULL == prPsrFtr) || (NULL == prAUCtrlInfo)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for PsrFilter or prAUCtrlInfo is NULL!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	if (0 == prAUCtrlInfo->u8Len) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d -- error for tx len is 0!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if ((CCS_INIT == prPsrCC->eState) ||
		...
		if (prPsrCC->fgUseCmdQ) {
			...
			dmx_memcpy((void *) prPsrCC->u4CmdQTxEntryBuffer,
				...
				if (PSR_IsNonHdrVideoType(prVFSD->eVCodeC)
					...
					DMXLOG_DEBUG(
						    TEXT("[PSR] %s line %d -- CreateAU=TRUE ")
						     TEXT
						     ("prVFSD->u4VType: 0x%x, prAUCtrlInfo->u4Vtype: 0x%x!\r\n"),
					 ...
					 u64 u8Len, EXT_INFO_T *prExtInf)
{
	...
	if ((NULL == prPsrFtr) || (NULL == prExtInf)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for PsrFilter or prExtInf is NULL!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if ((0 == u8Len) && (!prExtInf->fgAUEnd)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for u8Len is 0 but fgAUEnd is FALSE!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	/* check CC status and Tx state */
	...
	if ((CCS_INIT == prPsrCC->eState) ||
		...
		if (prExtInf->fgCreateAU) {
			...
			if (SPT_DATA_A == prPsrFtr->eType) {
				/* only can turn on in 1st unit */
				prPsrFtr->fgDST = prExtInf->fgDST;
			}
			...
			/* Jump HW fifo wpidx + 5 au  for SACD_DST reserved space */
			if (prPsrFtr->fgDST) {
				...
				DMX_ASSERT(prPsrFtr->ucHwDevId == 0);
				/* get sacd au start address */
				mrRet = ESM_FifoGetWrPtr(prPsrFtr->u4ESIH, &(prPsrFtr->ptrSacdAuSa));
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in ")
					  TEXT("ESM_FifoGetWrPtr(u4Handle: 0x%x), mrRet: 0x%x\r\n"),
				    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->u4ESIH,
				    mrRet);
					PSR_CC_UNLOCK(prPsrCC->rLock);
					MM_RETURN(mrRet);
				}
				mrRet = ESM_FifoIncWrPtr(prPsrFtr->u4ESIH, 5);
				if (DMX_FAILED(mrRet)) {
					PSR_CC_UNLOCK(prPsrCC->rLock);
					MM_RETURN(mrRet);
				}
				...
				mrRet = PSR_HAL_SetWPtr(prPsrFtr->ucHwDevId, BitType_Audio, ptrESICurWp);
				if (DMX_FAILED(mrRet)) {
					PSR_CC_UNLOCK(prPsrCC->rLock);
					MM_RETURN(mrRet);
				}
	...
	void *pvSrcSa, PSR_AUCtrlInfo *prAUCtrlInfo)
{
	...
	if (0 == u4Len) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for u8Len is 0!!\r\n"), DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
...
MRESULT PSR_Filter_DMAPBBuf4HdrParsing(PSR_FILTER *prPsrFtr, u64 u8Offset,
				       u32 u4Len, u32 *pptrTgtSa, u32 *pu4AvailSz)
{
	MRESULT mrRet = RET_DMX_OK;
	PSR_CC *prPsrCC;
	...
	if ((NULL == pptrTgtSa) || (NULL == prPsrFtr)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for pptrTgtSa is 0 or prPsrFtr is NULL!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	*pptrTgtSa = 0;
	...
	if (SPT_DATA_BUF != prPsrFtr->eType) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for PsrFilter's eType(%d) is not BUF!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->eType);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (0 == (prPsrFtr->u4Flag & FF_ENABLE)) {
		DMXLOG_ERROR(
			    TEXT("[PSR] %s line %d fail for PsrFilter's Flag not include FF_ENABLE!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_ERR_STATE);
	}
	...
	if ((CCS_INIT == prPsrCC->eState) || (CCS_TX == prPsrCC->eState)) {
		...
		prDMASD->pu4TgtHdrPrsSa = pptrTgtSa;
...
MRESULT PSR_Filter_PreChkForCmdQTx(PSR_FILTER *prPsrFtr, u32 u4FifoSpace, u32 u4CurMaxTxLen)
{
	...
	if ((!prPsrCC->fgCurTotalCmdQTxStarted) && (prTxEntry->u4TxOfst > 0)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for First Entry's TxOfst Error!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	prTxEntry =
	    ((DMX_CMDQ_TX_ENTRY_T *) (prPsrCC->u4CmdQTxEntryBuffer)) + prCmdQTxInf->u2CurTxRngSIdx;
    /**
	...
	**/
	...
	for (u2Idx = prCmdQTxInf->u2CurTxRngEIdx + 1; u2Idx < prPsrCC->u2TxEntryCnt; u2Idx++) {
		prTxEntry = ((DMX_CMDQ_TX_ENTRY_T *) (prPsrCC->u4CmdQTxEntryBuffer)) + u2Idx;
		/**
...

	{
		...
		for (u2Idx = prCmdQTxInf->u2CurTxRngSIdx + 1; u2Idx < prCmdQTxInf->u2CurTxRngEIdx;
		     u2Idx++) {
			prTxEntry =
			    ((DMX_CMDQ_TX_ENTRY_T *) (prPsrCC->u4CmdQTxEntryBuffer)) + u2Idx;
...
MRESULT PSR_Filter_TxDecideA(PSR_FILTER *prPsrFtr, PSR_CC *prPsrCC)
{
	...
	mrRet = ESM_AUTableGetAvailCount(prPsrFtr->u4ESIH, &u4AvailAUCnt);
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in ESM_AUTableGetAvailCount")
					  TEXT("(u4Handle: 0x%x), mrRet: 0x%x\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->u4ESIH, mrRet);
	...
	u4AUTableUsage = u4TotalAUCnt - u4AvailAUCnt;
		...
		if (prPsrCC->u4Flag & CCF_FIFOHOLD) {
			if ((SplitterRspIsEnabled(prPsrCC->pvSptHdl) &&
				...
				if ((u4FifoUsage <= PURE_AUDIO_A_FIFO_USAGE_MIN) ||
					(u4AUTableUsage > PURE_AUDIO_A_AU_AVAIL_MAX)) {
					DmxLogD(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_FIFOFULL,
...
MRESULT PSR_Filter_TXDecide(PSR_FILTER *prPsrFtr)
{
	...
	if (prPsrFtr->u4Flag & FF_TX_PBBUF) {
			...
			if (!MM_IS_STATE_ERROR(mrRet)) {
				DMXLOG_ERROR(
					    TEXT("[PSR] %s line %d fail in PSR_CC_Get")
					     TEXT("WaitTxBufSa, mrRet: 0x%x, PsrCC: 0x%x!!\r\n"),
					    DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prPsrCC);
				DMX_ASSERT(FALSE);
			}
			MM_RETURN(mrRet);
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in PSR_CC_Get")
						  TEXT
						  ("WaitTxBufSa, mrRet: 0x%x, PsrCC: 0x%x!!\r\n"),
				    DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prPsrCC);
			DMX_ASSERT(FALSE);
			MM_RETURN(mrRet);
		}
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in PSR_CC_Get u8TxCurrLen, mrRet: 0x%x, PsrCC: 0x%x\r\n")
				    DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prPsrCC);
			DMX_ASSERT(FALSE);
			MM_RETURN(mrRet);
		}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in PSR_CC_GetWait")
					  TEXT("TxBufSa, mrRet: 0x%x, PsrCC: 0x%x!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prPsrCC);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in PSR_CC_GetWait")
					  TEXT("TxBufSize, mrRet: 0x%x, PsrCC: 0x%x!!\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prPsrCC);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail in PSR_CC_Calc")
					  TEXT("HWDecryptOfst(PsrCC: 0x%x), mrRet: 0x%x\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrCC, mrRet);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_Filter_MoveFifoWp2AUIdx(PSR_FILTER *prPsrFtr, u32 u4AUIdx)
{
	...
	if (ESM_INVALID_INDEX == u4AUIdx) {
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for invalid AUIdx: %d\r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, u4AUIdx);
		MM_RETURN(mrRet);
	}
...
MRESULT PSR_Filter_Reset(PSR_FILTER *prPsrFtr)
{
	...
	if (0 != (u4Flag & FF_USED)) {
		...
		DMXLOG_ERROR(TEXT("[PSR] %s line %d -- prPsrFtr(eType: %d,")
					  TEXT(" flag: 0x%x) is an unused parser filter \r\n"),
			    DMX_FUNC_NAME, DMX_LINE_NO, prPsrFtr->eType, u4Flag);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
