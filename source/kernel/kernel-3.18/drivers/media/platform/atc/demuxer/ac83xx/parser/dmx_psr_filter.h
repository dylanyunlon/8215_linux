...
#define PURE_AUDIO_A_AU_AVAIL_MAX                60
...
typedef struct _PSR_DMA_SPECIFIC {
	...
	u32 *pu4TgtHdrPrsSa;	/* /< Hdr Parsing target memory start address */
...
typedef struct _PSR_FILTER {
	...
	* lengthmatch(e.g. Total TX Len(u8CurAULen)>=u8TotalAULen) */
	...
	bool fgDST;		/* /< SACD specific type, need to save space in the fifo */
	...
	u64 u8DecSendBufMask;	/* /< Send Buffer Dec Mask */
...
MRESULT PSR_Filter_DMAPBBuf4HdrParsing(PSR_FILTER *pPsrFtr, u64 u8Offset,
				       u32 u4Len, u32 *pu4TgtSa, u32 *pu4AvailSz);
