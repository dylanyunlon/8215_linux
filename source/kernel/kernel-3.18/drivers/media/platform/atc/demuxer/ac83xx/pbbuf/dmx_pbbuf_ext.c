...
int PBBUF_ReceiveDataThread(void *pv_arg)
{
	...
	while (fgRet) {
		...
		i4Ret =
			x_msg_q_receive(&u2Index, &rSendBuf, &z_msg_size, &(prExtInfo->hMsgQueue), 1,
					X_MSGQ_OPTION_WAIT);
		if (OSR_OK != i4Ret) {
			DMXLOG_ERROR(
		...
		i4Ret = x_msg_q_num_msgs(prExtInfo->hMsgQueue, &u2AvailMsgCnt);
...
MRESULT PBBUF_ExtBuf_ExtSendData(void *pvDmxTag, SEND_BUFFER *prSendBuffer)
{
	...
	if (NULL == prSendBuffer) {
		DMXLOG_ERROR(
			TEXT
			("[PBBUF] %s line %d fail for invalid args (ptrPbBuf: 0x%x, prSdBuf: 0x%x)\r\n"),
	...
	if ((NULL != prExtInfo) && (NULL_HANDLE != prExtInfo->hMsgQueue)) {
		i4Ret = x_msg_q_send(prExtInfo->hMsgQueue, prSendBuffer, sizeof(SEND_BUFFER), 1);
		if (OSR_TOO_MANY == i4Ret) {
			DMXLOG_ERROR(
				TEXT("[PBBUF] %s line %d fail in x_msg_q_send(ptrPbBuf:")
			...
			DMXLOG_ERROR(
				TEXT
				("[PBBUF] %s line %d fail in x_msg_q_send(ptrPbBuf: 0x%x, prSdBuf: 0x%x)\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prPbBuf, prSendBuffer);
			MM_RETURN(RET_DMX_OS_OPERA_FAIL);
		}
        else
				...
				u32 u4SlotSz, u32 u4HdrParamSz, u8 u1PbbufType)
{
	...
	case SPT_PBUFF_AUDIN:
		...
		if (NULL == prExtInfo) {
			DMXLOG_ERROR(
				TEXT("[PBBUF] %s line %d fail in alloc pvExtInfo (no mem)\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO);
	...
	case SPT_PBUFF_AUDIN:
		{
			...
			HANDLE_T hMsgQueue = NULL_HANDLE;
			s32 i4Ret = OSR_OK;

			i4Ret = x_msg_q_create(&hMsgQueue, TEXT("AudInDmxMsgQueue"),
		 sizeof(SEND_BUFFER), prPbBuf->u4SlotAmount);
			...
			prExtInfo->hThread = (HANDLE_T) prtaskstruct;
	...
	case SPT_PBUFF_AUDIN:
		{
			...
			if (NULL_HANDLE != prExtInfo->hMsgQueue) {
				i4Ret = x_msg_q_delete(prExtInfo->hMsgQueue);
				if (OSR_OK != i4Ret)
					DMXLOG_ERROR(
					...
					DMXLOG_DEBUG(
						...
						DMX_FUNC_NAME, DMX_LINE_NO, prPbBuf,
						prPbBuf->u4CompID);
				prExtInfo->hMsgQueue = NULL_HANDLE;
			}
...
MRESULT PBBUF_ExtBuf_ReleaseNoUseSlot(void *pvPbBuf, DMX_READ_BUFFER *prRdBuf)
{
	...
	if ((NULL == prRdBuf) || (NULL == prRdBuf->pvSlot)) {
		DMXLOG_ERROR(
			TEXT("[PBBUF] %s line %d fail for invalid args (pvPbBuf: 0x%p,")
			TEXT(" prRdBuf: 0x%p)\r\n"),
	...
	if (NULL == prSlot) {
		DMXLOG_ERROR(
			...
			TEXT("whose handle = 0x%p, (pvPbBuf: 0x%p, prPbBuf: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prRdBuf->pvSlot, pvPbBuf, prPbBuf);
		MM_RETURN(RET_DMX_NOT_FOUND);
	}

	u4SlotDataEa = (u32) (prSlot->pcPlayBuffer) + prSlot->u4PlaySize;
	if ((u4SlotDataEa < (u32) (prPbBuf->pcBufSa)) ||
		(u4SlotDataEa > (u32) (prPbBuf->pcBufSa) + prPbBuf->u4TotalSz)) {
		DMXLOG_ERROR(
			TEXT("[PBBUF] %s line %d fail for this slot's end address(0x%x)")
			TEXT(" exceed the pbbuf range[0x%x, 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, u4SlotDataEa, (u32) (prPbBuf->pcBufSa),
			((u32) (prPbBuf->pcBufSa) + prPbBuf->u4TotalSz));
		MM_RETURN(RET_DMX_UNEXPECT);
	}

	if (u4SlotDataEa == (u32) (prPbBuf->pcBufSa) + prPbBuf->u4TotalSz)
		u4SlotDataEa = (u32) (prPbBuf->pcBufSa);
...
#endif				/* HDMI_AUDIN_USE_SESSIONID*/
		{
			prExtInfo->rAudInCbsInfo.pvUpdateRpCb(u4SlotDataEa);
		}
	}
#endif				/*CONFIG_DRV_HDMI_RX*/
	 ...
	 bool *pfgExistUnCon)
{
	...
	if (NULL == prSlot) {
		DMXLOG_ERROR(
			TEXT("[PBBUF] %s line %d fail for can't find the reading slot whose ")
			TEXT("handle = 0x%p, (pvPbBuf: 0x%p, prPbBuf: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prRdBuf->pvSlot, pvPbBuf, prPbBuf);
		MM_RETURN(RET_DMX_NOT_FOUND);
	}
...
#if  CONFIG_DRV_HDMI_RX
MRESULT PBBUF_ExtBuf_GetAudInParsingInfo(void *pvPBBuf, AUDIN_PARSING_INFO_T *prAudinPsringInfo)
{
	PBBUF *prPbBuf = (PBBUF *) pvPBBuf;
	PBBUF_AUDIN_EXTINFO_T *prExtInfo = NULL;
