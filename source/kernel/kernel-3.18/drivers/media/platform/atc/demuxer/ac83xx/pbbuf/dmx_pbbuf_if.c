 ...
 *+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 */
MRESULT PBBUF_DumpSlotData(void *pvSlot, char *wszPreFix)
{
	SLOT *prSlot = (SLOT *)pvSlot;
...
MRESULT PBBUF_Init(void)
{
	...
	for (i = 0; i < DMX_MAX_PBBUF_INST_CNT; i++) {
		...
		if (NULL == g_aprPbBuf[i]) {
			DMXLOG_ERROR(
				TEXT("[PBBUF] %s line %d fail in alloc pbbuf instance(no mem, i: %d)\r\n"),
...
//#if DMX_NEW_PBBUF_MECHANISM
		if (g_u4PbBufFlag) {
  		if (NULL_HANDLE != g_aprPbBuf[u4Idx]->hEvtGroup) {
  			DMX_ASSERT(OSR_OK == x_ev_group_delete(g_aprPbBuf[u4Idx]->hEvtGroup));
  			g_aprPbBuf[u4Idx]->hEvtGroup = NULL_HANDLE;
  		}
		}
...
MRESULT PBBUF_Connect(void *pvSptHdl, void *pvPsrHdl, u32 *pu4Idx, void **ppvPbbuf)
{
	PBBUF  *prPbBuf = NULL;
	MRESULT  mrRet = RET_DMX_OK;
	u32 i = 0;
	...
	if (!g_fgPbBufInit) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail for g_fgPbbufInit is FALSE")
	...
	if (NULL == ppvPbbuf) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail for invalid args(phPbbuf: %p, ")
			TEXT("ptrPsr: 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, ppvPbbuf, pvPsrHdl);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	smp_mb();
	...
	for (i = 0; i < DMX_MAX_PBBUF_INST_CNT; i++) {
		...
		if ((NULL != prPbBuf) && !prPbBuf->fgConnected) {
			...
			if (NULL == prPbBuf->hSemaphore) {
				...
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in create semaphore")
						TEXT("(i: %d, mrRet: 0x%x)\r\n"),
						DMX_FUNC_NAME, DMX_LINE_NO, i, mrRet);
					DMX_ASSERT(FALSE);
					MM_RETURN(mrRet);
				}
			}
			...
			if (g_u4PbBufFlag) {
				DMXLOG_TRACE(TEXT("[PBBUF]-------USING NEW PBBUF MECHANISM--------\r\n"));
	  			if (NULL_HANDLE == prPbBuf->hEvtGroup) {
	  				...
	  				i4Ret = x_ev_group_create(&(prPbBuf->hEvtGroup),
	  					(const char *)szNameBuf, DMX_PBBUF_EV_INITIAL);
	  				if (OSR_OK != i4Ret) {
	  					DMXLOG_ERROR(
	  						TEXT("[PBBUF] %s line %d fail in x_ev_group_create(i: %d, i4Ret: 0x%x)\r\n"),
	  						DMX_FUNC_NAME, DMX_LINE_NO, i, i4Ret);
			...
			*ppvPbbuf = (void *)prPbBuf;

			DMXLOG_DEBUG(TEXT("[PBBUF] %s line %d success, ")
				TEXT("(phPbbuf: 0x%x, *pu4Idx: 0x%x, pvSptHdl: 0x%x, ptrPsr: 0x%x)\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, ppvPbbuf, i, pvSptHdl, pvPsrHdl);

			MM_RETURN(RET_DMX_OK);
		}
	}

	DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail for no unused Pbbuf instance")
		TEXT("(phPbbuf: 0x%x, *pu4Idx: 0x%x, ptrPsr: 0x%x)\r\n"),
		DMX_FUNC_NAME, DMX_LINE_NO, ppvPbbuf, i, pvPsrHdl);
...
MRESULT PBBUF_Disconnect(void *pvSptHdl)
{
	...
	if (NULL == prPbBuf) {
		DMXLOG_TRACE(
			TEXT("[PBBUF] %s line %d success, pvSptHdl(0x%p) han't connect to any pbbuf\r\n"),
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[PBBUF] %s line %d fail in PBBUF_Disable (pvSptHdl: 0x%p, mrRet: 0x%x)!\r\n"),
...
//#if DMX_NEW_PBBUF_MECHANISM
	if (g_u4PbBufFlag) {
		if (NULL_HANDLE != prPbBuf->hEvtGroup) {
			DMX_ASSERT(OSR_OK == x_ev_group_delete(prPbBuf->hEvtGroup));
			prPbBuf->hEvtGroup = NULL_HANDLE;
		}
	}
//#endif /* DMX_NEW_PBBUF_MECHANISM*/
	smp_mb();
	...
	DMXLOG_DEBUG(TEXT("[PBBUF] %s line %d success, pvSptHdl(0x%p)\r\n"),
		DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
	...
	u32 u4HdrParamSz)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufInitBuffer, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_GetAllocSlot(void *pvSptHdl, SEND_BUFFER *prSdBuf)
{
	...
	if (DMX_FAILED(mrRet) && (!MM_IS_STATE_ERROR(mrRet))) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufGetAllocSlot, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_CancelAllocSlot(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufCancelAllocSlot, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
/**********************************************************************/
/* MRESULT PBBUF_SendAllocSlot(void *pvSptHdl, SEND_BUFFER *prSdBuf, bool *pfgExitSent)*/
...
MRESULT PBBUF_SendDataSlot(void *pvSptHdl, SEND_BUFFER *prSdBuf, bool *pfgExitSent)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufSendDataSlot, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_ReleaseAllocSlot(void *pvSptHdl, SEND_BUFFER *prSdBuf)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufReleaseAllocSlot, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_CleanAllSlots(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufCleanAllSlots, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_GetAvailDataSlot(void *pvSptHdl, DMX_READ_BUFFER *prRdBuf)
{
	...
	if (DMX_FAILED(mrRet) && (!MM_IS_STATE_ERROR(mrRet))) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufGetAvailDataSlot, pvSptHdl(0x%p), mrRet(0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_ReleaseNoUseSlot(void *pvSptHdl, DMX_READ_BUFFER *prRdBuf)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufReleaseNoUseSlot, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_CancelReadSlot(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufCancelReadSlot, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
...
MRESULT PBBUF_GetAudInParsingInfo(void *pvSptHdl, AUDIN_PARSING_INFO_T *prAudinPsringInfo)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[PBBUF] %s line %d fail in rPbBufInterfaces")
			TEXT(".pmrPbBufGetAudInParsingInfo, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		PBBUF_EXIT(prPbBuf);
		MM_RETURN(mrRet);
	}
