...
MRESULT SplitterDestroy(void *pvSptHdl)
{
	...
	if (prSpt->fgCreated) {
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d -- fail in delete prSpt(0x%p) task, mrRet: 0x%x\r\n"),
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d -- fail in delete prSpt(0x%p) event, mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
		} else {
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d -- fail in delete prSpt(0x%p) semaphore, mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
		} else {
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d -- fail in delete prSpt(0x%p) CmdQ, mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
		} else {
...
MRESULT SplitterEnable(
	...
	if ((NULL == pvSptHdl) ||
		(NULL == prPbbufCfgInfo)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for invalid args(pvSptHdl: %p, prPbbufCfgInfo: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, prPbbufCfgInfo);
	...
	if (SplitterIsEnable(pvSptHdl)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for pvSptHdl(0x%p) is already enabled.\r\n"),
	...
	if (!SplitterIsTaskRunning(pvSptHdl)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for pvSptHdl(0x%p)'s task thread has already exited.\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		SPLITTER_UNLOCK;
...
MRESULT SplitterDisable(void *pvSptHdl)
{
	...
	if (SPLITTER_STATE_IDLE != u4SptState) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for Can not do disable op ")
			TEXT("under non-idle(%d) state, pvSptHdl(0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, u4SptState, pvSptHdl);
		SPLITTER_UNLOCK;
		MM_RETURN(RET_DMX_ERR_STATE);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SplitterSetDisable, mrRet: 0x%x.\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, mrRet);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterSetRate(void *pvSptHdl, s32 i4Rate, bool fgDmaAud)
{
	...
	if (NULL == pvSptHdl) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args(pvSptHdl: %p, Rate: %d).\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, i4Rate);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
...
MRESULT SplitterSetLastMem(void *pvSptHdl, bool fgLastMem)
{
	...
	if (NULL == pvSptHdl) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args(pvSptHdl: %p, ")
			TEXT("fgLastMem: %d).\r\n"),
...
MRESULT SplitterSetCfaType(void *pvSptHdl, u32 u4CfaType)
{
	...
	if (NULL == pvSptHdl) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args (pvSptHdl: %p, ")
			TEXT("u4CfaType: 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, u4CfaType);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SptCfaUninit, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
	...
	if (NULL == pvCfaInterface) {
		...
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for pvCfaInterface == NULL,")
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SptCfaInit pvSptHdl: %p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterSetCfaConfigure(void *pvSptHdl, void *pvCfaParameter, bool fgIsUserMem)
{
	...
	if ((NULL == pvSptHdl) ||
		...
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args (pvSptHdl: %p, ")
			TEXT("CfaParam: 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pvCfaParameter);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	if (!SplitterIsEnable(pvSptHdl)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for Splitter is disable, pvSptHdl: %p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		MM_RETURN(RET_DMX_ERR_STATE);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SptCfaConfigure, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterSetCfaRange(void *pvSptHdl, void *pvCfaRange, bool fgIsUserMem)
{
	...
	if ((NULL == pvSptHdl) ||
		(NULL == pvCfaRange)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for invalid args (pvSptHdl: %p, pvCfaRange: 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pvCfaRange);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SptSetCfaRange, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
...
MRESULT SplitterSetCfaInquire(void *pvSptHdl, u32 u4InquirerTypes)
{
	...
	if (NULL == pvSptHdl) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args(pvSptHdl: %p,")
			...
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, u4InquirerTypes);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SptCfaSetInquirer, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterGetCfaPosition(void *pvSptHdl, void *pvCfaPosition)
{
	...
	if ((NULL == pvSptHdl) || (NULL == pvCfaPosition)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args(pvSptHdl: %p,")
			...
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pvCfaPosition);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SptCfaGetPosition, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
						  ...
						  u32 u4CfaParameterSize)
{
	...
	if ((NULL == pvSptHdl) ||
		...
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args(pvSptHdl: %p,")
			...
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, u4CfaFID, pvCfaParameter);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet) && (!MM_IS_STATE_ERROR(mrRet))) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SptCfaGetGeneral, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterSetCfaGeneral(void *pvSptHdl, u32 u4CfaFID, void *pvCfaParameter,
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SptCfaSetGeneral, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterGetStmFifoFullness(
	...
	u32 u4SptDataType,
	u32 *pu4FullNess) {
	void	*pvStmHdl = NULL, *pvPsrFtr = NULL;
	MRESULT mrRet = RET_DMX_OK;

	if ((NULL == pu4FullNess) ||
		(NULL == pvSptHdl)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for invalid args, pvSptHdl: %p, pu4FullNess: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pu4FullNess);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	if ((SPT_DATA_V != u4SptDataType) &&
		(SPT_DATA_A != u4SptDataType)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for unsupport sptdatatype(%d)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, u4SptDataType);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}

	if (!SplitterIsEnable(pvSptHdl)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for Splitter is disable, pvSptHdl: %p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		MM_RETURN(RET_DMX_ERR_STATE);
	}
	...
	pvStmHdl = GetStreamByType(pvSptHdl, u4SptDataType);
	if ((NULL == pvStmHdl) ||
		(!StreamIsEnabled(pvStmHdl))) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for no Designated Stm or Stm ")
			TEXT("is disabled (pvSptHdl: %p, pvStmHdl: 0x%p, SptDataType: 0x%x)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pvStmHdl, u4SptDataType);
		SPLITTER_UNLOCK;
		MM_RETURN(RET_DMX_NOT_FOUND);
	}

	pvPsrFtr = GetPsrFtrFromStm(pvStmHdl);

	mrRet = PSR_Filter_GetFifoAvailSize(pvPsrFtr, pu4FullNess);
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in PSR_Filter_GetFifoAvailSize")
			TEXT(" (pvSptHdl: %p, SptDataType:0x%x, pvPsrFtr: 0x%p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, u4SptDataType, pvPsrFtr);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
	SPLITTER_UNLOCK;
...
MRESULT SplitterSetParserOn(DMX_PSR_ON_PARAM_T *prParam)
{
	...
	if (NULL == prParam) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args\r\n"),
	...
	if (NULL == pvSptHdl) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for invalid args, pvSptHdl: %p, prStmsCnt: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, prStmsCnt);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterSendCmd(TURN_ON),")
	...
	if (DMX_FAILED(rCmd.mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in exceed cmd TURN_ON, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, rCmd.mrRet);
		DMX_ASSERT(FALSE);
		SplitterSetEOSForError(pvSptHdl, RET_DMX_OS_OPERA_FAIL);
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterSetParserOff(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in SplitterSendCmd(RSP_OFF), pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
	...
	if (DMX_FAILED(rCmd.mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail in exceed cmd RSP_OFF, pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, rCmd.mrRet);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	}
...
//#endif
	...
	mrRet = SplitterSendCmd(pvSptHdl, &rCmd);
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterSendCmd(DMX_SPT_")
	...
	if (DMX_FAILED(rCmd.mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in exceed cmd DMX_SPT_CMD_PAUSE")
			TEXT(", pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, rCmd.mrRet);
		DMX_ASSERT(FALSE);
		MM_RETURN(mrRet);
	...
	DMXLOG_TRACE(TEXT("[SPT] %s success, exit (pvSptHdl: %p, State:0x%x)\r\n"),
		DMX_FUNC_NAME, pvSptHdl, SplitterGetState(pvSptHdl));
...
MRESULT SplitterGetFileOfst(void *pvSptHdl, u64 *pu8FileOfst)
{
	...
	if ((NULL == pvSptHdl) ||
		...
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid args (pvSptHdl: %p,")
			...
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, pu8FileOfst);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d failed in PSR_CC_GetCurrentOffset(pvSptHdl: %p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		SPLITTER_UNLOCK;
		MM_RETURN(mrRet);
	}
	  ...
	  u8  ucState)
{
	...
	if (NULL == pu1RspTxRet) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for invalid args, pu1RspTxRet == NULL\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (NULL == prPsrCC) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for Splitter(0x%p) has no PSR_CC\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		*pu1RspTxRet = 0;  /* unsupport resplitter*/
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	*pu1RspTxRet = 1; /* support resplitter*/
	...
	case (u8)SPT_DATA_A: {
			...
			if (MM_IS_FFRW_PLAY(SplitterGetPlayRate(pvSptHdl))) {
				...
				if (NULL != GetStreamByTypes(prSpt->pvDmxInst, SPT_DATA_V)) {
					...
					if (DMX_FAILED(mrRet)) {
						DMXLOG_ERROR(TEXT("[SPT] %s exit, line %d, Can't to switch ")
			...
			if (NULL == GetStreamByTypes(prSpt->pvDmxInst, SPT_DATA_V)) {
				*pu1RspTxRet = 0; /* unsupport resplitter*/
				DMXLOG_ERROR(TEXT("[SPT] %s exit, line %d, unsupport to do switch")
	...
	default:
		*pu1RspTxRet = 0; /* unsupport resplitter*/
		DMXLOG_ERROR(TEXT("[SPT] %s exit, line %d, unsupport to do switch")
			...
			DMX_FUNC_NAME, DMX_LINE_NO, ucRspTxType);
		SPLITTER_UNLOCK;
		MM_RETURN(RET_DMX_OK);
	}
	...
	mrRet = SplitterSendCmd(pvSptHdl, &rCmd);
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterSendCmd(RSP_OFF),")
			TEXT(" pvSptHdl: %p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
	...
	if (DMX_FAILED(rCmd.mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in exceed cmd RSP_OFF, pvSptHdl:")
	...
	bool   *fgRebuff)
{
	...
	if (!SplitterIsEnable(pvSptHdl)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for Splitter(pvSptHdl: %p) is disable\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		MM_RETURN(RET_DMX_ERR_STATE);
	}
	...
	if (!SplitterRspIsEnabled(pvSptHdl)) {
		...
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for Resplitter is disabled, (pvSptHdl: %p)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		SPLITTER_UNLOCK;
		MM_RETURN(RET_DMX_ERR_STATE);
	}
	...
	if ((SPT_DATA_A != SplitterGetRspTxType(pvSptHdl)) &&
		...
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d -- unsupport to switch, SplitterGetRspTxType(pvSptHdl) = ")
			TEXT("0x%x (not Audio/SP), pvSptHdl: %p .\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, SplitterGetRspTxType(pvSptHdl), pvSptHdl);
		SPLITTER_UNLOCK;
		MM_RETURN(RET_DMX_UNEXPECT);
	}
	...
	if (SPT_DATA_SP == SplitterGetRspTxType(pvSptHdl)) {
		if (MM_IS_FFRW_PLAY(SplitterGetPlayRate(pvSptHdl))) {
			...
			if (NULL != GetStreamByType(pvSptHdl, SPT_DATA_V)) {
				DMXLOG_ERROR(
					TEXT("[SPT] %s line %d fail for unsupport to switch SP while Video & ")
					TEXT("SP are transferred in the Same Spt(0x%p) inst in FF/RW.\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
				SPLITTER_UNLOCK;
				MM_RETURN(RET_DMX_UNEXPECT);
			} else {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for Separated SP insts")
		...
		if (MM_IS_FFRW_PLAY(SplitterGetPlayRate(pvSptHdl))) {
			if (NULL == GetStreamByTypes(prSpt->pvDmxInst, SPT_DATA_V)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for unsupport to switch")
...
MRESULT SplitterSetParserRspOn(void *pvSptHdl, bool fgRebuf)
{
	...
	if (NULL == prSpt) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for invalid args, (pvSptHdl: %p, fgRebuf: %d)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, (fgRebuf ? 1 : 0));
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (!SplitterIsEnable(pvSptHdl)) {
		...
		DMXLOG_ERROR(
		TEXT("[SPT] %s line %d fail for Splitter is disable (pvSptHdl: %p, fgRebuf: %d)\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, (fgRebuf ? 1 : 0));
		SplitterSetEOSForError(pvSptHdl, RET_DMX_UNEXPECT);
		MM_RETURN(RET_DMX_ERR_STATE);
	}
	...
	if (SPT_DATA_SP == SplitterGetRspTxType(pvSptHdl)) {
		if (MM_IS_FFRW_PLAY(SplitterGetPlayRate(pvSptHdl))) {
			...
			if (NULL != GetStreamByType(pvSptHdl, SPT_DATA_V)) {
				...
				DMXLOG_ERROR(
					TEXT("[SPT] %s line %d fail for separated SP insts should do stop->start")
		...
		if (MM_IS_FFRW_PLAY(SplitterGetPlayRate(pvSptHdl))) {
			if (NULL == GetStreamByTypes(prSpt->pvDmxInst, SPT_DATA_V)) {
				DMXLOG_ERROR(
					TEXT("[SPT] %s line %d fail for unsupport to switch Audio for Pure ")
	...
	if (!SplitterIsTaskRunning(pvSptHdl)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for pvSptHdl(0x%x)'s task thread has already exited.\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl);
		SPLITTER_UNLOCK;
		MM_RETURN(RET_DMX_UNEXPECT);
	}
...
MRESULT SplitterGetStmEmptyInfo(PSR_FILTER *prPsrFilter, bool *pfgEmpty)
{
	...
	if (SPT_DATA_A == prPsrFilter->eType) {
		...
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d %s ----------> Fifo Total Size: %d, Fifo Data Size: %d\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, 
			((prPsrFilter->eType < MAX_SPT_DATA_TYPE_CNT) ?
			g_aszSptDataTypeName[prPsrFilter->eType] : TEXT("UNKNOWN")),
	...
	DMXLOG_ERROR(
		TEXT("[SPT] %s line %d %s ----------> Total AU Count: %d, Avail AU Count: %d\r\n"),
		DMX_FUNC_NAME, DMX_LINE_NO,
		((prPsrFilter->eType < MAX_SPT_DATA_TYPE_CNT) ?
		g_aszSptDataTypeName[prPsrFilter->eType] : TEXT("UNKNOWN")),
	...
	SPT_PARAM_FIFO_USAGE_OUTPUT *prOutInfo)
{
	...
	if ((NULL != prVStm) &&
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d fail in SplitterGetStmEmptyInfo(Video, ESIH: 0x%x),")
				TEXT(" mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO,prPsrFilter->u4ESIH, mrRet);
		}
	}
	...
	if ((NULL != prAStm) &&
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterGetStmEmptyInfo")
	...
	if (fgVideoEmpty &&
		...
		if ((NULL != prVStm) && (RET_DMX_OK != mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d fail in GAU_SetThreshold, strmtype: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prVStm->u4StmType);
			SPLITTER_UNLOCK;
			MM_RETURN(mrRet);
		}
		if ((NULL != prAStm) && (RET_DMX_OK != mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d fail in GAU_SetThreshold, strmtype: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prAStm->u4StmType);
			SPLITTER_UNLOCK;
			MM_RETURN(mrRet);
		}
		if ((NULL != prSctStm) && (RET_DMX_OK != mrRet)) {
			DMXLOG_ERROR(
				TEXT("[SPT] %s line %d fail in GAU_SetThreshold, strmtype: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSctStm->u4StmType);
			SPLITTER_UNLOCK;
			MM_RETURN(mrRet);
		}
	}
...
MRESULT SplitterSetPowerState(DMX_PM_STATE PowerState)
{
		...
		if (VALID_DX(PowerState)) {
			...
			case D2:
				...
				if ((D3 == eCurPowerState) ||
					...
					SPLITTER_UNLOCK;
					DMXLOG_TRACE(TEXT("[SPT] %s exit -- no inuse dmx instance, dmx hw hasn't been power up, so don't need to set power up/down.\r\n"),
						DMX_FUNC_NAME);
					MM_RETURN(RET_DMX_OK);
				} else {
					SPLITTER_UNLOCK;
					DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for invalid powerstate to set(%d)\r\n"),
						DMX_FUNC_NAME, DMX_LINE_NO, PowerState);
					MM_RETURN(RET_DMX_OK);
				}
				break;
	...
	if ((RET_DMX_SUSPEND_NEED_WAIT == mrRet) && (NULL != pvSptHdl)) {
		DMXLOG_TRACE(
			...
			TEXT("SUSPEND_OK\r\n"),
			DMX_FUNC_NAME, pvSptHdl);
		mrRet = SplitterGetUsrEvent(pvSptHdl, SPLITTER_UEV_SUSPEND_OK, &eEvts, DMX_SUSPEND_WAIT_MAXTIME);
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(
				TEXT("[PVR] %s line %d fail in Get Usr Event: SPLITTER_UEV_SUSPEND_OK\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO);
			SPLITTER_UNLOCK;
			SplitterSetEOSForError(pvSptHdl, mrRet);
			MM_RETURN(mrRet);
...
DMX_PM_STATE SplitterGetPowerState(void)
{
	...
	if (!fgInit) {
        DMXLOG_ERROR(TEXT("[SPT] %s line %d exit, splitter hasn't been inited.\r\n"),
            DMX_FUNC_NAME, DMX_LINE_NO);
        eState = D4;
    } else {
