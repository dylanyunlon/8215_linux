...
MRESULT SplitterCreateTask(DMX_SPT_INST_T *prSpt, u32 u4SptIdx)
{
	...
	if (NULL != (void *)(prSpt->hSptTask))
		prSpt->hSptTask = (HANDLE_T)NULL;

	u4Handle = (u32)kthread_create(SplitterForeverLoop, prSpt, szNameBuf);
	if ((struct task_struct *)u4Handle == ERR_PTR(-ENOMEM)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in kthread_create")
			TEXT("(szNameBuf(%s), u4SptIdx(%d)), prSpt:0x%p, Err: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, szNameBuf, u4SptIdx, prSpt, u4Handle);
		prSpt->fgExitThread = TRUE;
		prSpt->hSptTask = (HANDLE_T)NULL;
		MM_RETURN(RET_DMX_NO_MEM);
	} else if (IS_ERR((struct task_struct *)u4Handle)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in kthread_create")
			TEXT("(szNameBuf(%s), u4SptIdx(%d)), prSpt:0x%p, Err: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, szNameBuf, u4SptIdx, prSpt, u4Handle);
		prSpt->fgExitThread = TRUE;
		...
		ret = sched_setscheduler_nocheck((struct task_struct *)u4Handle,
			SCHED_RR, &param);
		DMX_ASSERT(ret == 0);
	...
	wake_up_process((struct task_struct *)u4Handle);

	prSpt->hSptTask = (HANDLE_T)u4Handle;
...
MRESULT SplitterDeleteTask(DMX_SPT_INST_T *prSpt)
{
	...
	DMXLOG_DEBUG(TEXT("[SPT] %s enter, prSpt:0x%p\r\n"),
		DMX_FUNC_NAME, prSpt);
...
MRESULT SplitterCreateSema(DMX_SPT_INST_T *prSpt)
{
	...
	mrRet = dmx_sema_create(&(prSpt->u4SptSemaphore), DMX_SEMA_TYPE_BINARY,
...
MRESULT SplitterDeleteSema(DMX_SPT_INST_T *prSpt)
{
	...
	if (NULL != prSpt->u4SptSemaphore) {
		mrRet = dmx_sema_delete(prSpt->u4SptSemaphore);
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[SPT] %s fail in delete semaphore, ")
				TEXT("prSpt:0x%x, mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, prSpt, mrRet);
			MM_RETURN(mrRet);
		}

		prSpt->u4SptSemaphore = NULL;
	}
...
MRESULT SplitterLockSema(DMX_SPT_INST_T *prSpt)
{
	...
	mrRet = dmx_sema_lock(prSpt->u4SptSemaphore, DMX_SEMA_OPTION_WAIT);
	if (DMX_SUCCEED(mrRet))
		MM_RETURN(RET_DMX_OK);
...
MRESULT SplitterReleaseSema(DMX_SPT_INST_T *prSpt)
{
	...
	mrRet = dmx_sema_unlock(prSpt->u4SptSemaphore);
	if (DMX_SUCCEED(mrRet))
		MM_RETURN(RET_DMX_OK);
...
MRESULT SplitterCreateEvent(DMX_SPT_INST_T *prSpt, u32 u4Idx)
{
	...
	i4Ret = x_ev_group_create(&(prSpt->u4SptEvent),
	...
	if (0 != prSpt->u4UsrEvent) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail for sema already exist, prSpt: 0x%p\r\n"),
			DMX_FUNC_NAME, prSpt);
		x_ev_group_delete(prSpt->u4SptEvent);
		prSpt->u4SptEvent = 0;
		MM_RETURN(RET_DMX_ALREADY_EXIST);
	}
	...
	i4Ret = x_ev_group_create((uintptr_t *)&(prSpt->u4UsrEvent),
		(const char *)szNameBuf,
	...
	DMXLOG_DEBUG(
		TEXT("[SPT] %s -- prSpt:0x%p, u4UsrEvent: 0x%x, u4SptEvent: 0x%08x\r\n"),
		DMX_FUNC_NAME, prSpt, prSpt->u4UsrEvent, prSpt->u4SptEvent);
...
MRESULT SplitterDeleteEvent(DMX_SPT_INST_T *prSpt)
{
	...
	if (0 != prSpt->u4SptEvent) {
		i4Ret = x_ev_group_delete(prSpt->u4SptEvent);
		if (OSR_OK != i4Ret) {
	...
	if (0 != prSpt->u4UsrEvent) {
		i4Ret = x_ev_group_delete(prSpt->u4UsrEvent);
		if (OSR_OK != i4Ret) {
			DMXLOG_ERROR(TEXT("[SPT] %s fail in x_ev_group_delete ")
	...
	u32		   u4WaitTimeout)
{
	...
	i4Ret = x_ev_group_wait_event_timeout(prSpt->u4SptEvent,
				u4WaitOnEvent, pu4ReceiveEvent,
				X_EV_OP_OR_CONSUME, u4WaitTimeout);
	if (OSR_OK == i4Ret) {
		DMXLOG_DEBUG(
			TEXT("[SPT] %s -- prSpt:0x%p wait event:0x%x success!\r\n"),
...
MRESULT SplitterSetEvent(DMX_SPT_INST_T *prSpt, EV_GRP_EVENT_T u4SetEvent)
{
	...
	i4Ret = x_ev_group_set_event(prSpt->u4SptEvent, u4SetEvent, X_EV_OP_OR);
	if (OSR_OK == i4Ret) {
		DMXLOG_DEBUG(
			TEXT("[SPT] %s -- prSpt:0x%p set event:0x%x success!\r\n"),
	...
	u32		   u4WaitTimeout)
{
	...
	i4Ret = x_ev_group_wait_event_timeout(prSpt->u4UsrEvent,
				u4WaitOnEvent, pu4ReceiveEvent,
				X_EV_OP_OR_CONSUME, u4WaitTimeout);
	if (OSR_OK == i4Ret) {
		DMXLOG_DEBUG(
...
MRESULT SplitterSetUsrEvent(DMX_SPT_INST_T *prSpt, EV_GRP_EVENT_T u4SetEvent)
{
	...
	i4Ret = x_ev_group_set_event(prSpt->u4UsrEvent, u4SetEvent, X_EV_OP_OR);
	if (OSR_OK == i4Ret) {
		DMXLOG_DEBUG(
...
static MRESULT SplitterIdlePsrOn(DMX_SPT_INST_T *prSpt)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(
			TEXT("[SPT] %s fail in SplitterSetCfaPsrEnd(prSpt: 0x%p, FALSE)!\r\n"),
			DMX_FUNC_NAME, prSpt);
		MM_RETURN(mrRet);
	}
	...
	DMXLOG_DEBUG(TEXT("[SPT] %s line %d  -- SptCfaSetTurnOn(prSpt: ")
		TEXT("0x%p, cfaType: %d)!\r\n"),
	...
	case SPLITTER_STATE_IDLE:
		{
			...
			DMXLOG_TRACE(TEXT("[SPT] %s line %d -- CMD_PTX_PAUSE(prSpt")
				...
				if (((DMX_FAILED(mrRet) && (!MM_IS_STATE_ERROR(mrRet))) ||
					...
					if (DMX_FAILED(mrRet)) {
						DMXLOG_ERROR(TEXT("[SPT] %s fail in Splitter")
							TEXT("ChangeState(RUNING, PAUSE), prSpt: 0x%p, mrRet:")
							TEXT(" 0x%x\r\n"),
							DMX_FUNC_NAME, prSpt, mrRet);
						SplitterSetEOSForError((void *)pvSptHdl, mrRet);
						prCmd->mrRet = mrRet;
						*pfgReply = TRUE;
						return;
					}
	...
	*pfgReply = FALSE;
	...
	case SPLITTER_STATE_RUNING:
		...
		default:
			DMXLOG_ERROR(TEXT("[SPT] %s fail for Error Splitter State")
				TEXT(" to do PsrOff (prSpt: 0x%p, SptState: 0x%x,  SptTxState: 0x%x)!\r\n"),
				DMX_FUNC_NAME, prSpt, prSpt->eSptState, prSpt->eSptTxState);
			prCmd->mrRet = RET_DMX_ERR_STATE;
			*pfgReply = TRUE;
			break;
	...
	default:
		DMXLOG_ERROR(TEXT("[SPT] %s fail for Error Splitter State to")
			TEXT(" do PsrOff (prSpt: 0x%p, SptState: 0x%x,  SptTxState: 0x%x)!\r\n"),
			DMX_FUNC_NAME, prSpt, prSpt->eSptState, prSpt->eSptTxState);
		prCmd->mrRet = RET_DMX_ERR_STATE;
		*pfgReply = TRUE;
		break;
	...
	DMX_CMD_INFO_T *prCmd, bool *pfgReply)
{
	...
	*pfgReply = FALSE;
	...
	case SPLITTER_STATE_RUNING:
		{
			...
			default:
				DMXLOG_ERROR(TEXT("[SPT] %s fail for Error Splitter ")
					TEXT("State to do PsrOff (prSpt: 0x%p, State: 0x%x)!\r\n"),
					DMX_FUNC_NAME, prSpt, eState);
				DMX_ASSERT(FALSE);
				prCmd->mrRet = RET_DMX_ERR_STATE;
				*pfgReply = TRUE;
				break;
	...
	default:
		DMXLOG_ERROR(TEXT("[SPT] %s fail for Error Splitter State to")
			TEXT(" do PsrOff (prSpt: 0x%p, State: 0x%x)!\r\n"),
			DMX_FUNC_NAME, prSpt, eState);
		DMX_ASSERT(FALSE);
		prCmd->mrRet = RET_DMX_ERR_STATE;
		*pfgReply = TRUE;
		...
		if (DMX_FAILED(mrRet)) {
			...
			*pfgReply = TRUE;
			DMXLOG_ERROR(TEXT("[SPT] %s fail in SplitterChangeState")
				TEXT("(RUNING, PAUSE), prSpt: 0x%p, mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, prSpt, mrRet);
			return;
		}
	...
	case SPLITTER_STATE_RUNING:
		...
		case SPLITTER_TX_STATE_TXING:
			if (!SplitterRspIsEnabled(prSpt)) {
				...
				SplitterSetReResplitter(prSpt, FALSE);
				*pu1RspTxRet = 0;
				prCmd->mrRet = RET_DMX_OK;
				*pfgReply = TRUE;
	...
	DMX_CMD_INFO_T *prCmd, bool *pfgReply)
{
	...
	case SPLITTER_STATE_RUNING:
		if (SPLITTER_TX_STATE_RSPOFF != prSpt->eSptTxState) {
			...
			DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for error splitter")
				TEXT(" state to do Rebuffer(prSpt: 0x%p, State: %d, eTxState: %d).\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prSpt->eSptState,
				prSpt->eSptTxState);
			prCmdOutBuf->fgNeedRebuf = FALSE;
	...
	default:
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for error splitter state")
			TEXT(" to do Rebuffer(prSpt: 0x%p, State: %d, eTxState: %d).\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prSpt->eSptState,
			prSpt->eSptTxState);
		prCmdOutBuf->fgNeedRebuf = FALSE;
		PSR_CC_GetCurPbbufStartOffset(prSpt->pvPsrCC, &(prCmdOutBuf->u8PbbStartOffset));
	...
	SplitterSetRspStartOffset(prSpt, DMX_INVALID_UINT64);
	SplitterSetRspOffsetDelta(prSpt, 0);

	switch (prSpt->ucRspMode) {/*PTS mode is default*/
	case SPLITTER_PTX_RSP_BY_PTS:
		{
			SplitterSetRspStartPts(prSpt, prCmdInBuf->u8RspStartPts);

			SplitterSetRspPtsDelta(prSpt, prCmdInBuf->u8RspDelta);

			SplitterSetRspStartOffset(prSpt, prCmdInBuf->u8RspStartOffset);
			...
			mrRet = SplitterRspGetRspRangeByPts(prSpt, prCmdInBuf->u8RspDelta,
				&(prCmdOutBuf->u8RspStartOffset), &(prCmdOutBuf->u8PbbStartOffset));
		}
		break;
	case SPLITTER_PTX_RSP_BY_OFFSET:
		{
			...
			SplitterSetRspOffsetDelta(prSpt, prCmdInBuf->u8RspDelta);

			/* Get the Rsp Entry whose start Pts is little smaller than the designated Playing*/
			/* PTS(u8RspStartPts + u8RspPtsDelta)*/
			/* Get its fileoffset of the Rsp entry*/
			mrRet = SplitterRspGetRspRangeByOffset(prSpt, prCmdInBuf->u8RspDelta,
				&(prCmdOutBuf->u8RspStartOffset), &(prCmdOutBuf->u8PbbStartOffset));
		}
		break;
	default:
		...
		DMXLOG_ERROR(TEXT("[SPT] %s fail for unsupport Resplitter Mode")
			TEXT("(prSpt: 0x%p, RspMode: %d).\r\n"),
			DMX_FUNC_NAME, prSpt, prSpt->ucRspMode);
		prCmd->mrRet = RET_DMX_PARAM_WRONG;
		SplitterSetEOSForError((void *)prSpt, prCmd->mrRet);
		*pfgReply = TRUE;
		return;
	}
	...
	DMX_CMD_INFO_T *prCmd, bool *pfgReply)
{
	...
	prCfaInterface	 = (CfaIntf *)SplitterGetCfaInterface(prSpt);
	...
	if (NULL != prCfaInterface->pfmrRebuf) {
		mrRet = prCfaInterface->pfmrRebuf(prSpt, fgRebuf, pvCfaPrivateData);
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(
...
static MRESULT SplitterStmEnable(DMX_STM_INST_T *prStm, bool fgEnable)
{
	...
	if (fgEnable) {
		...
		case SPT_DATA_V:
			...
			/*g_rDmxCliMan.fgDmxPrintGetAULog[DMX_CLI_STM_VID] = TRUE;*/
			break;
		case SPT_DATA_A:
	...
	DMX_CMD_INFO_T *prCmd, bool *pfgReply)
{
	...
	if ((NULL == prCmd->pvInBuf) ||
		...
		DMXLOG_ERROR(
			TEXT("[SPT] %s line %d fail for invalid params, prSpt: 0x%p!\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
		prCmd->mrRet = RET_DMX_PARAM_WRONG;
		*pfgReply = TRUE;
		return;
	...
	DMXLOG_DEBUG(
		TEXT("[SPT] %s line %d -- Process %s Stream %d(%s), prSpt: 0x%p!\r\n"),
	...
	*pfgReply = FALSE;
	...
	case SPLITTER_STATE_RUNING:
		...
		case SPLITTER_TX_STATE_TXING:
			{
				...
				if (NULL != prPsrCC) {
					...
					DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for Stream's")
						TEXT(" PsrCC ==NULL, prSpt: 0x%p, StmType: %d!\r\n"),
						DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prStm->u4StmType);
		...
		default:
			DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for Error Splitter")
				TEXT(" State to do PsrOff (prSpt: 0x%p, SptState: 0x%x,	SptTxState")
				TEXT(": 0x%x)!\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prSpt->eSptState, prSpt->eSptTxState);
			prCmd->mrRet = RET_DMX_ERR_STATE;
			*pfgReply = TRUE;
	...
	default:
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail for Error Splitter ")
			TEXT("State to do PsrOff (prSpt: 0x%p, SptState: 0x%x,  SptTxState: 0x%x)!\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prSpt->eSptState, prSpt->eSptTxState);
		prCmd->mrRet = RET_DMX_ERR_STATE;
		*pfgReply = TRUE;
		return;
...
static DMX_CMD_INFO_T *SplitterGetCmd(DMX_SPT_INST_T *prSpt)
{
	...
	DMXLOG_DEBUG(TEXT("[SPT] %s line %d  -- Get Cmd(eType: %d, Name:")
		TEXT(" %s, RefCnt: %d), CmdRdIdx: %d, prSpt: 0x%p\r\n"),
...
MRESULT SplitterSendCmd(DMX_SPT_INST_T *prSpt, DMX_CMD_INFO_T *prCmd)
{
	...
	prPsrCC = (PSR_CC *)(prSpt->pvPsrCC);
	if (NULL != prPsrCC) {
		DMXLOG_DEBUG(TEXT("[SPT] %s line %d  -- prSpt: 0x%p, SptState:")
			TEXT(" %d, SptTxState: %d, PsrCC's eState: 0x%x, eTxState: 0x%x) Before")
			TEXT(" SendCmd\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prSpt->eSptState, prSpt->eSptTxState,
			prPsrCC->eState, prPsrCC->eTxState);
	} else {
		DMXLOG_DEBUG(TEXT("[SPT] %s line %d  -- prSpt: 0x%p, SptState")
			TEXT(": %d, SptTxState: %d, PsrCC==NULL) Before SendCmd\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prSpt->eSptState, prSpt->eSptTxState);
	...
	if (!prCmd->fgASync) {
		...
		if (DMX_FAILED(mrRet)) {
			...
			DMXLOG_DEBUG(
				TEXT("[SPT] %s line %d	-- Spt(0x%p) Get User Event(0x%x)\r\n"),
...
MRESULT SplitterProcPtxDoneNfyInRunningState(DMX_SPT_INST_T *prSpt)
{
	...
	if (SplitterRspIsEnabled(prSpt) &&
		...
		if (SplitterRspIsEnabled(prSpt) &&
			...
			DMXLOG_DEBUG(
				TEXT("[SPT] %s -- SPLITTER_EV_PTX_DONE, Rsping, prSpt: 0x%p\r\n"),
				DMX_FUNC_NAME, prSpt);
...
MRESULT SplitterProcPtxDoneNfy(DMX_SPT_INST_T *prSpt)
{
	...
	case SPLITTER_STATE_IDLE:
		DMXLOG_DEBUG(
			TEXT("[SPT] %s line %d -- prSpt: 0x%p Previous TX_DONE in IDLE\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
		break;
	case SPLITTER_STATE_RUNING:
...
MRESULT SplitterProcPtxJumpNfy(DMX_SPT_INST_T *prSpt)
{
	...
	case SPLITTER_STATE_RUNING:
		...
		if (CFA_TYPE_AUDIN == SplitterGetCfaType(prSpt)) {
			if (MM_IS_NORMAL_PLAY(prSpt->i4Rate)) {
				DMXLOG_TRACE(
					TEXT("[SPT] %s line %d -- Jump while normal play, prSpt: 0x%p\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
			} else {
				DMXLOG_ERROR(
			...
			if (!MM_IS_FFRW_PLAY(prSpt->i4Rate)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d failed for FR/BR")
					TEXT(" rate = %d, so should not JUMP, prSpt: 0x%p\r\n"),
		...
		DMXLOG_DEBUG(
			TEXT("[SPT] %s line %d -- NEED DO JUMP FLOW, prSpt: 0x%p.\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
		...
		if (NULL != prPsrCC) {
			...
			if (CCS_PAUSE == prPsrCC->eState) {
				DMXLOG_TRACE(TEXT("[SPT] %s line %d -- Jump while PsrCC")
					TEXT(" enter to Pause State, prSpt: 0x%x.\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
				PSR_CC_UNLOCK(prPsrCC->rLock);
				MM_RETURN(RET_DMX_OK);
			}
			...
			if ((NULL != prPsrCC) &&
				...
				DMXLOG_TRACE(TEXT("[SPT] %s line %d -- Jump while ")
					TEXT("PsrCC enter to Pause State, prSpt: 0x%p.\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
				PSR_CC_UNLOCK(prPsrCC->rLock);
				MM_RETURN(RET_DMX_OK);
			}
			...
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in Splitter")
					TEXT("ChangeState(RUNING, TX_CHECK), prSpt: 0x%p, mrRet: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
				PSR_CC_UNLOCK(prPsrCC->rLock);
				SplitterSetEOSForError((void *)prSpt, mrRet);
				MM_RETURN(mrRet);
			...
			DMXLOG_TRACE(
				TEXT("[SPT] %s line %d -- SptCfaSetJumpInfo, prSpt: 0x%p.\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
			...
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SptCfaSet")
					TEXT("JumpInfo (mrRet:0x%x, prSpt:0x%p).\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prSpt);
				SplitterSetEOSForError((void *)prSpt, mrRet);
				PSR_CC_UNLOCK(prPsrCC->rLock);
				MM_RETURN(mrRet);
			...
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in PSR_CC_")
					TEXT("Reset4NonConPbbufSlot (mrRet:0x%x, prSpt:0x%p).\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prSpt);
				SplitterSetEOSForError((void *)prSpt, mrRet);
				PSR_CC_UNLOCK(prPsrCC->rLock);
				MM_RETURN(mrRet);
			...
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in PBBUF_")
					TEXT("GetFstSentSlotHeader (mrRet:0x%x, prSpt:0x%p).\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prSpt);
				SplitterSetEOSForError((void *)prSpt, mrRet);
				MM_RETURN(mrRet);
			}
	...
	default:
		DMXLOG_ERROR(TEXT("[PSR] %s line %d fail for error SptState")
			TEXT("(%s, %s) to do JUMP, prSpt: 0x%p\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO,
			((prSpt->eSptState <= SPLITTER_STATE_RUNING) ?
				g_awszDmxSptStatus[prSpt->eSptState] : TEXT("UNKNOWN")),
			((prSpt->eSptTxState <= SPLITTER_TX_STATE_ERROR) ?
...
static MRESULT SplitterMainLoop(DMX_SPT_INST_T *prSpt)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] -------- %s line %d -- Got Events ")
			TEXT("Failed (mrRet:0x%x, prSpt:0x%x). --------\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prSpt);
		SplitterSetEOSForError((void *)prSpt, mrRet);
	}
	...
	if (0 != (SPLITTER_EV_SPT_CMD_IN & u8Events)) {
		DMXLOG_DEBUG(TEXT("[SPT] -------- %s line %d -- Got SPLITTER")
			TEXT("_EV_SPT_CMD_IN (prSpt:0x%x). --------\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
		...
		if (DMX_FAILED(mrRet)) {
			if (MM_IS_STATE_ERROR(mrRet)) {
				...
				DMXLOG_ERROR(TEXT("[SPT] %s line %d failed in Splitter")
					TEXT("ProcCmd, (SPT_CMD_IN, mrRet:0x%x, prSpt:0x%x).\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prSpt);
				SplitterSetEOSForError((void *)prSpt, mrRet);
			}
		}
	...
	if (0 != (SPLITTER_ALL_NFY_EVS & u8Events)) {
		...
		if (DMX_FAILED(mrRet)) {
			if (MM_IS_STATE_ERROR(mrRet)) {
				...
				DMXLOG_ERROR(TEXT("[SPT] %s line %d failed in Splitter")
					TEXT("ProcNfy, (mrRet:0x%x, prSpt:0x%x).\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, mrRet, prSpt);
				SplitterSetEOSForError((void *)prSpt, mrRet);
			}
		}
...
#endif
{
	...
	DMXLOG_TRACE(TEXT("[SPT] *********Enter Splitter Thread******* ")
		TEXT("(Thread ID:0x%x, prSpt: 0x%p).\r\n"),
		dwThreadID, prSpt);
	#else
	DMXLOG_TRACE(TEXT("[SPT] *********Enter Splitter Thread******* ")
		TEXT("(prSpt: 0x%p).\r\n"),
	...
	DMXLOG_DEBUG(TEXT("[SPT] Exit Main thread loop process (prSpt: 0x%p), ")
	...
	DMXLOG_DEBUG(TEXT("[SPT] get kthead_should_stop flag, ")
		TEXT("now *********Exit Splitter Thread******* (prSpt: 0x%x)\r\n"),
		prSpt);
