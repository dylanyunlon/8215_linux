...
/*/////////////////////////////////////////////////////////////////////////////*/
static bool SplitterFifoIsThreshold(uintptr_t ptrStm)
{
	MRESULT mrRet = RET_DMX_OK;
	u32	u4FifoDataSize = 0;
	u32	u4FifoTh = DMX_INVALID_UINT32;

	if (!StreamIsEnabled(ptrStm))
		return FALSE;

	/* Check fifo threshold if arrived.*/
	mrRet = PSR_Filter_GetFifoAvailSize(((DMX_STM_INST_T *)ptrStm)->pvPsrFtr, &u4FifoDataSize);
	if (DMX_FAILED(mrRet))
		return FALSE;

	u4FifoTh = StreamGetFifoThreshold(ptrStm);
	if (0 == u4FifoTh)
		return TRUE;
...
MRESULT SplitterPbb2Buf(void *pvSptHdl, DMAInfo *prDMA)
{
	...
	pvStmHdl = GetStreamByType(pvSptHdl, SPT_DATA_BUF);

	/* Set the TX info into splitter*/
	mrRet = SplitterSetPtxData(pvSptHdl, &rInf, pvStmHdl, TRUE);
	if (DMX_FAILED(mrRet)) {
	...
	mrRet = PSR_Filter_DMAPBBuf4HdrParsing(((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr,
								   prDMA->u8FromFileOfst,
								   (u32)prDMA->u8TxLen,
								   (u32 *)prDMA->pucToAddress,
								   prDMA->pu4AvailSize);
...
MRESULT SplitterBuf2Fifo(void *pvSptHdl, DMX_SPT_DMA2FIFO_INFO_T *prInf)
{
	...
	SplitterSendNfy(prSpt, DMX_SPT_NTY_TX_END);
	mrRet = SplitterChangeState(prSpt, SPLITTER_STATE_RUNING, SPLITTER_TX_STATE_TXING);
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterChangeState")
			TEXT("(RUNING, TXING), prSpt: 0x%p, mrRet: 0x%x\r\n"),
	...
	pvStmHdl = GetStreamByType(prSpt, prInf->u4TxStreamType);

	mrRet = SplitterSetPtxData(prSpt, prInf, pvStmHdl, FALSE);
	if (DMX_FAILED(mrRet)) {
	...
	if (!SplitterRspIsEnabled(prSpt)) {
		if ((SPT_DATA_A == prInf->u4TxStreamType) ||
			...
			u32 u4StmUID = GetStmUIDByType(prSpt, prInf->u4TxStreamType);
			...
			if (prInf->u4TxUID != u4StmUID) /* not the stream used in current playing*/ {
				...
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in ")
						TEXT("SplitterChangeState(RUNING, TXING), prSpt: 0x%p,")
						TEXT(" mrRet: 0x%x\r\n"),
						DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
					MM_RETURN(mrRet);
				}
	...
	if (SplitterRspIsLoging(prSpt)) {
#if DMX_RSP_SUPPORT_PURE_AUDIO
		if (SPT_DATA_A == prInf->u4TxStreamType) {
			SplitterSetPtxNotBusy(prSpt);

			mrRet = SplitterSetPtxData(prSpt, prInf, pvStmHdl, TRUE);
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterSetPtxData")
					TEXT(" (prSpt 0x%p, mrRet: 0x%x)!\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
		...
		if ((SPT_DATA_A == prInf->u4TxStreamType) &&
			...
			mrRet = SplitterSetPtxData(pvSptHdl, prInf, pvStmHdl, TRUE);
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterSetPtxData")
	...
	if (NULL == pvStmHdl) {
		/*We Do not real issue Parser Tx in this case*/
		DMXLOG_DEBUG(TEXT("[SPT] %s exit for prSpt(0x%p) has no stream")
			TEXT(" whose type is %d!\r\n"),
			DMX_FUNC_NAME, prSpt, prInf->u4TxStreamType);
	...
	if (SPT_DATA_V == prInf->u4TxStreamType) {
		mrRet = PSR_Filter_SetCodeC(((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr,
									 prInf->u4TxVideoCodec);
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[SPT] %s fail in PSR_Filter_SetCodeC ")
				TEXT("(prSpt 0x%p, Codec %d), mrRet: 0x%x!\r\n"),
				DMX_FUNC_NAME, prSpt, prInf->u4TxVideoCodec, mrRet);
			MM_RETURN(mrRet);
		}
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[SPT] %s fail in PSR_Filter_SetCodeC ")
				TEXT("(prSpt 0x%p, AudioCodec %d), mrRet: 0x%x!\r\n"),
				DMX_FUNC_NAME, prSpt, prInf->u4TxAudioCodec, mrRet);
			MM_RETURN(mrRet);
		}
	}

	mrRet = PSR_Filter_TxMemory2Fifo(((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr,
...
MRESULT SplitterBuf2FifoAUCtrl(void *pvSptHdl, DMX_SPT_DMA2FIFO_INFO_T *prInf)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterChangeState")
			TEXT("ING, TXING), prSpt: 0x%p, mrRet: 0x%x\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
		MM_RETURN(mrRet);
	}
	MM_RETURN(RET_DMX_OK);
	...
	pvStmHdl = GetStreamByType(pvSptHdl, prInf->u4TxStreamType);

	mrRet = SplitterSetPtxData(pvSptHdl, prInf, pvStmHdl, FALSE);
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterSetPtxData")
			TEXT(" (pvSptHdl 0x%p, mrRet: 0x%x)!\r\n"),
			DMX_FUNC_NAME, DMX_LINE_NO, pvSptHdl, mrRet);
	...
	if ((SplitterRspIsLoging(prSpt) ||
		...
		((SPT_DATA_A == prInf->u4TxStreamType) ||
		 (SPT_DATA_SP == prInf->u4TxStreamType))) {
		if (prInf->u4TxUID != ((DMX_STM_INST_T *)pvStmHdl)->u4StmUID) {
			...
			/* Just a check for CFA UID set */
			DMXLOG_ERROR(TEXT("%s line %d -- prSpt(0x%p) -- prInf->")
				TEXT("u4TxUID(0x%x) != u4StmUID(0x%x)\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt,
				prInf->u4TxUID, ((DMX_STM_INST_T *)pvStmHdl)->u4StmUID);
			...
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in Splitter")
					TEXT("ChangeState(RUNING, TXING), prSpt: 0x%p, mrRet: 0x%x\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
				MM_RETURN(mrRet);
			}
	...
	if (SPT_DATA_A == prInf->u4TxStreamType) {
		...
		if (prInf->fgDummyUnit) {
			mrRet = PSR_Filter_AddDummyAU(((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr,
						TRUE, FALSE);
			if (DMX_FAILED(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in PSR_Filter")
					TEXT("_AddDummyAU, prSpt 0x%p\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prSpt);
				MM_RETURN(mrRet);
			}
		...
		if (NULL != ((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr) {
			bool fgFifoFull = FALSE;
			PSR_FILTER *prPsrFtr = (PSR_FILTER *)(((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr);
		...
		mrRet = PSR_Filter_TxMemory2FifoWithAUCtrl(
			((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr, prInf->pvFromAddress, &rAUCtrlInfo);
		if (DMX_FAILED(mrRet)) {
			if (!MM_IS_STATE_ERROR(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s line %d fail prSpt 0x%p in")
...
MRESULT SplitterPbb2Fifo(void *pvSptHdl, DMX_SPT_DMA2FIFO_INFO_T *prInf)
{
	...
	mrRet = PSR_CC_GetCurPbbufStartOffset(prSpt->pvPsrCC,
										  &u8CurPbbufStartOffset);
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s fail in PSR_CC_GetCurPbbufStartOffset")
			TEXT(" (prSpt 0x%p, mrRet: 0x%x)!\r\n"),
	...
	if (prInf->u8FromFileOfst < u8CurPbbufStartOffset) {
		...
		PSR_CC_AbortTx(prSpt->pvPsrCC);
		MM_RETURN(RET_DMX_UNEXPECT);
	}

	if (DMX_INVALID_UINT64 != prSpt->u8FileEndOffset) {
		if (prInf->u8FromFileOfst >= prSpt->u8FileEndOffset) {
			DmxLogT(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_DEFAULT,
				TEXT("[SPT] %s u8FromFileOfst(%lld) >= u8FileEndOffset(%lld),should send EOS, u8TxLen:%lld.\r\n"),
	            DMX_FUNC_NAME, prInf->u8FromFileOfst, prSpt->u8FileEndOffset, prInf->u8TxLen);
			mrRet = SplitterPsrSetEOS(pvSptHdl, GAU_E_EOS);
	...
	if (NULL == pvStmHdl) {
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in SplitterChang")
				TEXT("eState(RUNING, TXING), prSpt: 0x%p, mrRet: 0x%x\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
			MM_RETURN(mrRet);
		}
	...
	pvPsrFtr = ((DMX_STM_INST_T *)pvStmHdl)->pvPsrFtr;
	/*fix Klocwork*/
	if (NULL == pvPsrFtr) {
		/*We Do not real issue Parser Tx in this case*/
		DMXLOG_ERROR(TEXT("[SPT] %s exit for prSpt(0x%p) has no Psr ")
	...
	u4StmUID = GetStmUIDByType(prSpt, prInf->u4TxStreamType);
	...
	if (prInf->u4TxUID != u4StmUID) {
		...
		case SPT_DATA_A: {
				...
				mrRet = SplitterSetPsrAuTable(prSpt, (void *)&rAuData);
				if (DMX_FAILED(mrRet)) {
				...
				DMXLOG_DEBUG(TEXT("[SPT] %s line %d -- (u8TxUID(%d) !=")
					TEXT(" StmUID(%d))Set SPLITTER_EV_PTX_DONE, prSpt: 0x%p\r\n"),
					DMX_FUNC_NAME, DMX_LINE_NO, prInf->u4TxUID, u4StmUID, prSpt);
				SplitterSendNfy(prSpt, DMX_SPT_NTY_TX_END);
				mrRet = SplitterChangeState(prSpt, SPLITTER_STATE_RUNING, SPLITTER_TX_STATE_TXING);
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in ")
		...
		case SPT_DATA_SP: {
				...
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(TEXT("[SPT] %s fail in SplitterSet")
						TEXT("PsrAuTable (prSpt 0x%p, mrRet: 0x%x)!\r\n"),
						DMX_FUNC_NAME, prSpt, mrRet);
					MM_RETURN(mrRet);
				}
				...
				if (DMX_FAILED(mrRet)) {
					DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in Splitte")
						TEXT("rChangeState(RUNING, TXING), prSpt: 0x%p, mrRet: 0x%x\r\n"),
						DMX_FUNC_NAME, DMX_LINE_NO, prSpt, mrRet);
					MM_RETURN(mrRet);
				}
	...
	if (prInf->fgDummyUnit) {
		DMXLOG_DEBUG(TEXT("[SPT] %s -- PSR_Filter_AddDummyAU, prSpt: 0x%x\r\n"),
			DMX_FUNC_NAME, prSpt);
		...
		if (DMX_FAILED(mrRet)) {
			DMXLOG_ERROR(TEXT("[SPT] % line %d fail in PSR_Filter_AddDummyAU")
				TEXT(" (prSpt 0x%p, fgDummyAUEnd: %d)!\r\n"),
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt,
	...
	if (prInf->fgAUByEnd) {
		...
		if ((VC_UNKNOW == prInf->u4TxVideoCodec) &&
			...
			if (AUD_DRV_FMT_SACD == u4Codec)
				rEx.fgDST = prInf->rExInf.rSACD.fgDST;
		} else { /*/ Video*/
		...
		if (DMX_FAILED(mrRet)) {
			if (!MM_IS_STATE_ERROR(mrRet)) {
				DMXLOG_ERROR(TEXT("[SPT] %s fail in PSR_Filter_TxPBBuf")
					TEXT("2FifoWithAUEnd(prSpt: 0x%p, mrRet: 0x%x)!\r\n"),
					DMX_FUNC_NAME, prSpt, mrRet);
			}
			MM_RETURN(mrRet);
		}
		...
		DMXLOG_DEBUG(
			TEXT("[SPT] %s -- PSR_Filter_TxPBBuf2FifoWithAUCtrl, prSpt: 0x%p\r\n"),
			DMX_FUNC_NAME, prSpt);
		...
		if (DMX_FAILED(mrRet)) {
			if (!MM_IS_STATE_ERROR(mrRet)) {
				DMXLOG_ERROR(
					TEXT("[SPT] %s fail in PSR_Filter_TxPBBuf2FifoWithAUCtrl(prSpt:")
					TEXT(" 0x%p, mrRet: 0x%x)!\r\n"),
					DMX_FUNC_NAME, prSpt, mrRet);
			}
			MM_RETURN(mrRet);
...
MRESULT SplitterPsrSetEOS(void *pvSptHdl, u32 u4Status)
{
	...
	if ((GAU_E_ERRCHUNK == u4Status) ||
		...
		DMXLOG_TRACE(TEXT("[SPT] %s line %d -- Spt(0x%p)'s Repeat ")
			...
			DMX_FUNC_NAME, DMX_LINE_NO, prSpt, prSpt->u4RepeatErrChkCnt);
		...
		if (prSpt->u4RepeatErrChkCnt >= DMX_MAX_ERRCHUNK_CNT) {
			...
			DMXLOG_ERROR(TEXT("[SPT] %s line %d -- Spt(0x%p)'s Repeat ")
				...
				DMX_FUNC_NAME, DMX_LINE_NO, prSpt, DMX_MAX_ERRCHUNK_CNT);
		}
	}
...
MRESULT SplitterPsrTurnDivxDRM(void *pvSptHdl, CFA_DIVXDRM_INFO_T *prDivxDRMInf)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s fail in PSR_CC_EnableDivxDRMDecrypt")
			TEXT(" (prSpt 0x%p, pvPsrCC: 0x%x), mrRet: 0x%x!\r\n"),
			DMX_FUNC_NAME, prSpt, prSpt->pvPsrCC, mrRet);
		MM_RETURN(mrRet);
	}
...
MRESULT SplitterPsrAudInIsRaw(void *pvSptHdl, bool *pfgIsRawAud)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s fail in PSR_CC_IsAudinRaw (prSpt ")
...
MRESULT SplitterPsrGetAudInParsingInfo(void *pvSptHdl, AUDIN_PARSING_INFO_T *prPsrInfo)
{
	...
	if (DMX_FAILED(mrRet)) {
		DMXLOG_ERROR(TEXT("[SPT] %s line %d fail in PSR_CC_GetAudIn")
			TEXT("ParsingInfo (prSpt 0x%p), mrRet: 0x%x!\r\n"),
			DMX_FUNC_NAME, prSpt, mrRet);
		MM_RETURN(mrRet);
	}
...
{
	DMX_STM_INST_T	  *prStm	= (DMX_STM_INST_T *)pvStmHdl;
