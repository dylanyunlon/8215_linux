...
MRESULT StreamCreate(void *pvDmxInst, STM_PARAM_CREATE *prParam, void **ppvHandle)
{
	...
	if ((NULL == prParam) ||
		...
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s failed for invalid args, (prParam: 0x%p, ppvHandle: 0x%p)!\r\n"),
	...
	*ppvHandle = NULL;
	...
	if (NULL == prSpt) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			...
			TEXT("prParam: 0x%p, pHandle: 0x%p)!\r\n"),
			DMX_FUNC_NAME, prParam, ppvHandle);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
...
MRESULT StreamDisable(void  *pvStm)
{
	...
	if (NULL == pvSptHdl) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s exit, ptrStm's pvSptHdl == NULL, (pvStm: 0x%p)!\r\n"),
			DMX_FUNC_NAME, pvStm);
		MM_RETURN(RET_DMX_OK);
	}
...
MRESULT StreamDestroy(void *pvSptHdl, void *pvStm)
{
	...
	if ((NULL == pvSptHdl) ||
		(NULL == prStm)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s failed for invalid args, (prStm: 0x%p, pvSptHdl: 0x%p)!\r\n"),
			DMX_FUNC_NAME, prStm, pvSptHdl);
		DMX_ASSERT(FALSE);
		MM_RETURN(RET_DMX_PARAM_WRONG);
	}
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s failed in GAU_Disconnect, (prStm: 0x%p, pvSptHdl: 0x%p)!\r\n"),
			DMX_FUNC_NAME, prStm, pvSptHdl);
		STREAM_UNLOCK(pvSptHdl);
		MM_RETURN(mrRet);
	}
		...
		if (NULL != prSpt->pvPsrCC) {
			...
			if (DMX_FAILED(mrRet)) {
				DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
					...
					TEXT("pvPsrCC: 0x%x, pvPsrFtr: 0x%x)!\r\n"),
...
MRESULT StmDisconnectPsr(void *pvSptHdl)
{
	...
	if (NULL == ppvStmHandles) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s fail in SplitterGetStmHandles, pvSptHdl: 0x%x \r\n"),
...
MRESULT StreamConnectSptByHandle(void *pvStm, void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s fail in SplitterAddStmHandle, pvStm: 0x%p Spth: 0x%p\r\n"),
...
MRESULT StreamDisconnectSptByHandle(void *pvStm, void *pvSptHdl)
{
	...
	mrRet = PSR_CC_DetachFilter(pvPsr, prStm->pvPsrFtr);
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s fail in PSR_CC_DetachFilter, pvSptHdl: 0x%p \r\n"),
			DMX_FUNC_NAME, pvSptHdl);
...
MRESULT StreamEnable(void *pvStm)
{
	...
	if (NULL == pvSptHdl) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s fail for ptrStm's pvSptHdl == NULL, (pvStm: 0x%p)!\r\n"),
			DMX_FUNC_NAME, pvStm);
		MM_RETURN(RET_DMX_UNEXPECT);
	}
...
MRESULT StreamSetFifoInfo(void *pvStm, u32 u4FifoSz)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s fail in PSR_Filter_SetType, pvStm : 0x%p, u4FifoSz: 0x%x\r\n"),
...
MRESULT StreamGetCodec(void *pvStm, u32 *pu4Codec)
{
	...
	*pu4Codec = DMX_INVALID_UINT32;
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[STM] %s fail in PSR_Filter_GetCodeC(hPsr: 0x%p, ..)!\r\n"),
...
/* and is the stream of the designated splitter*/
/****************************************************/
u32 GetStreamCntFromType(void *pvDmxInst, void *pvStmHandle)
{
	DMX_STM_INST_T *prStm = (DMX_STM_INST_T *)pvStmHandle;
	u32	u4Ftr4SptNo = 0;
	u32	i;
	u32	u4StreamType;
	void *pvSptHdl;
...
MRESULT CreateDmaStm(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[SPT] %s fail in PSR_Filter_Enable: pvStm: 0x%x, StmType: 0x%x!\r\n"),
			DMX_FUNC_NAME, pvStm, prStm->u4StmType);
		STREAM_UNLOCK(pvSptHdl);
		MM_RETURN(mrRet);
	}
...
MRESULT CreateGrdStm(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[SPT] %s fail in PSR_Filter_Enable: prStm: 0x%p, StmType: 0x%x!\r\n"),
...
MRESULT DeleteGrdStm(void *pvSptHdl)
{
	...
	if (DMX_FAILED(mrRet)) {
		DmxLogE(DMX_MOD_OTH, DMX_MOD_OTH_LOGLVL_STM,
			TEXT("[SPT] %s fail in PSR_Filter_Disable: prStm: 0x%p, StmType: 0x%x!\r\n"),
			DMX_FUNC_NAME, prStm, prStm->u4StmType);
		STREAM_UNLOCK(pvSptHdl);
		MM_RETURN(mrRet);
	}
