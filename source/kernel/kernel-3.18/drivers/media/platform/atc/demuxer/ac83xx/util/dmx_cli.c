...
static MRESULT DmxCliDumpStmFifoInfo(void *pvSptHdl, u32 u4SptDataType)
{
	...
	if ((NULL != hStm) &&
		...
		if (NULL != pPsr->pvPsrCC) {
			...
			DMXLOG_TRACE(
				TEXT("[CLI] %s ----------> DmxInst: 0x%p, SptHandle: 0x%p PsrCC(0x%x)'s eTxState: %d,")
...
MRESULT DmxCliDumpFifoInfo(u32 u4StmTypeID)
{
	...
	for (u4Idx = 0; u4Idx < DMX_MAX_SPT_INST_CNT; u4Idx++) {
		...
		switch (u4StmTypeID) {
		case 1:
			mrRet = DmxCliDumpStmFifoInfo((void *)(g_rSptMan.aprSptInst[u4Idx]),
				SPT_DATA_V);
			break;
		case 2:
			mrRet = DmxCliDumpStmFifoInfo((void *)(g_rSptMan.aprSptInst[u4Idx]),
		...
		case 4:
			...
			break;
		case ((u32)(-1)): /* ALl */
			DmxCliDumpStmFifoInfo((void *)(g_rSptMan.aprSptInst[u4Idx]),
				SPT_DATA_V);
			DmxCliDumpStmFifoInfo((void *)(g_rSptMan.aprSptInst[u4Idx]),
				SPT_DATA_A);
			DmxCliDumpStmFifoInfo((void *)(g_rSptMan.aprSptInst[u4Idx]),
	...
	for (u4Idx = 0; u4Idx < DMX_MAX_SPT_INST_CNT; u4Idx++) {
		...
		if (fgDump) {
			if (!DmxCreateDumpRspFile(g_rSptMan.aprSptInst[u4Idx]->u4SptCompId)) {
				DMXLOG_ERROR(
					TEXT("[CLI] %s fail in DmxCreateDumpRspFile(SptId: %d, SptHandle: 0x%p)\r\n"),
