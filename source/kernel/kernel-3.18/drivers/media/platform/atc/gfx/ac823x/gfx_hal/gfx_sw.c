...
static uint32_t _au4GfxSwReg[GFX_HAL_HW_INST_NUM][GREG_FILE_SIZE];

static uint32_t _u4GfxSwActionCount;
...
static const INT32 _i4sAlphaInvTbl[] =
{
    ...
    static uint32_t _u4YCbCr2Rgb_RedErrorCount = 0;
    static uint32_t _u4YCbCr2Rgb_GreenErrorCount = 0;
    static uint32_t _u4YCbCr2Rgb_BlueErrorCount = 0;
#endif

#if (CONFIG_DRV_LINUX)
#if 0//(CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8560)
uint32_t VIRADDR(const MI_DIF_FIELD_T *prGfxReg, uint32_t u4Addr)
{
    uint32_t u4Rst;
...
static uint32_t _u4RGB565Alpha = 0;
static uint32_t _u4UseRGB565Alpha = 0;
uint32_t g_u4I2DAlcom = 0; /* Just for I2D + Alcom + XXXX(Mirror/Flip/Stretch) */
//---------------------------------------------------------------------------
// Static functions
//---------------------------------------------------------------------------
 ...
 */
//-------------------------------------------------------------------------
static uint32_t _GfxSwColorExpansion(uint32_t u4Color, uint32_t u4ColorMode)
{
...
typedef struct _GFX_CLIP_PARA_T{
    ...
    uint32_t u4DstX;
    uint32_t u4DstY;
    uint32_t u4ClipTopEn;
    uint32_t u4ClipTop;
    uint32_t u4ClipBotEn;
    uint32_t u4ClipBot;
    uint32_t u4ClipLeftEn;
    uint32_t u4ClipLeft;
    uint32_t u4ClipRightEn;
    uint32_t u4ClipRight;
}GFX_CLIP_PARA_T, *PGFX_CLIP_PARA_T;

static BOOL  fgGfxWithinClip(uint32_t u4CurX, uint32_t u4CurY, GFX_CLIP_PARA_T *prClipPara)
{
...
INT32 GFX_SwBitBlt_Clip(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    ...
    uint32_t u4ColchgEn, uint32_t u4KeynotEn, uint32_t u4Color, GFX_CLIP_PARA_T *prClipPara);
...
static INT32 _GfxSwRectFill(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    uint32_t *pu4Color;
    uint32_t u4Width, u4Height, u4DstPitch, u4Color, u4DstCM;
    uint32_t x, y;
    ...
    ui4Temp    = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_DST_BSAD);
...
static INT32 _GfxSwDrawObliqueLine(const MI_DIF_FIELD_T *prsGfxReg)
{
    UINT8 *pu1DstBase, *pu1Write; //, *pu1DstLine;
    uint32_t *pu4Color;
    //uint32_t u4Width, u4Height;
    uint32_t u4DstPitch, u4Color, u4DstCM;
    uint32_t  u4SrcX, u4SrcY, u4DstX, u4DstY;
    //uint32_t x, y;
    INT32 ai4RectColor[4] = {0};
    uint32_t  u4Bpp;
...
INT32 GFX_SwBitBlt_R2Y(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
    uint32_t u4Height, uint32_t u4ColorMin, uint32_t u4ColorMax, uint32_t u4TransEn,
    uint32_t u4ColchgEn, uint32_t u4KeynotEn, uint32_t u4Color,uint32_t u4R2YY2REnable);
//-------------------------------------------------------------------------
/** _GfxSwBitBlt
 *
 */
...
static INT32 _GfxSwBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    uint32_t u4SrcPitch, u4DstPitch;
    uint32_t u4SrcCM, u4DstCM;
    uint32_t u4Color;
    uint32_t u4Width, u4Height;
    uint32_t u4TransEn, u4ColchgEn, u4KeynotEn;
    uint32_t u4DstFlip = 0;
    uint32_t u4DstMirror = 0;
    uint32_t u4SrcFlip = 0;
    uint32_t u4SrcMirror = 0;
    uint32_t u4R2YY2REnable = 0;
    uint32_t u4Flip = 0;
    uint32_t u4Mirror = 0;
    uint32_t u4SrcFlag=0;
    uint32_t u4DstFlag=0;
    uint32_t ui4Temp = 0;
    
    uint32_t u4SrcColorMin;
    uint32_t u4SrcColorMax;
    uint32_t u4DstColorMin;
    uint32_t u4DstColorMax;
    
    uint32_t u4DisSrcKey;
    uint32_t u4DisDstKey;
    uint32_t u4SrcKeyIn;
    uint32_t u4DstKeyIn;
    uint32_t u4Alpha;
    // Clip Opt
    ...
    //for warning
    ui4Temp = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_SRC_BSAD);
...
static INT32 _GfxSwBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    uint32_t u4SrcPitch, u4DstPitch;
    uint32_t u4SrcCM, u4DstCM;
    uint32_t u4Color, u4ColorMin, u4ColorMax;
    uint32_t u4Width, u4Height;
    uint32_t u4TransEn, u4ColchgEn, u4KeynotEn;
    uint32_t u4DstFlip = 0;
    uint32_t u4DstMirror = 0;
...
static INT32 _Gfx24BppSwBitBlt(const MI_DIF_FIELD_T *prsGfx24BppReg)
{
    ...
    ui4Temp = VIRADDR(prsGfx24BppReg, (uint32_t)prsGfx24BppReg->fg_SRC_BSAD_24BPP);
    pu1SrcBase = (UINT8 *)ui4Temp;
    ...
    ui4Temp = 0;
    //for warning
    ui4Temp = VIRADDR(prsGfx24BppReg, (uint32_t)prsGfx24BppReg->fg_DST_BSAD);
    pu1DstBase = (UINT8 *)ui4Temp;
...
{
    uint32_t *pu4Src, *pu4Dst;
    uint32_t u4NDW;
    uint32_t ui4Temp = 0;

    //pu4Src = (uint32_t *)(prsGfxReg->fg_SRC_BSAD);
    //pu4Dst = (uint32_t *)(prsGfxReg->fg_DST_BSAD);

    ui4Temp = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_SRC_BSAD);
    pu4Src = (uint32_t *)ui4Temp;

    ui4Temp = 0;
    ui4Temp = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_DST_BSAD);
    pu4Dst = (uint32_t *)ui4Temp;
...
static INT32 _GfxMsAlphaBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    UINT8 *pu1SrcBase, *pu1DstBase;
	uint32_t u4SrcPitch, u4DstPitch;
	uint32_t u4DstCM, u4Alpha, u4SrcPremult;
	uint32_t u4Width, u4Height;
	uint32_t ui4Temp;
...
static INT32 _GfxAlphaComposition(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    uint32_t u4SrcPitch, u4DstPitch;
    uint32_t u4DstCM, u4Alpha;
    uint32_t u4Width, u4Height, u4Alcom;
    uint32_t ui4Temp = 0;
...
static INT32 _GfxYCrCb2Rgb(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    ui4Temp = 0;
    ui4Temp = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_SRCCBCR_BSAD);
...
static INT32  _GfxSwRopBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    uint32_t u4SrcCM, u4DstCM;
    uint32_t u4Width, u4Height;
    uint32_t u4RopOpCode, u4nowr, u4cmpflag, u4JavaXorClr;
    uint32_t ui4Temp = 0;
    uint32_t u4ColorRepEn;
    /* 8555 or newer IC, and 8550 ECO IC, use the following variables */
...
static INT32  _GfxSwInx2DirBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
  ...
  UINT8 *pu1PalBase;
    uint32_t u4SrcPitch, u4DstPitch;
    uint32_t u4CharCM, u4DstCM;
    uint32_t u4Width, u4Height;
  uint32_t u4LnStByeAl, u4MsbLeft;
...
static INT32  _GfxSwH2VLine(const MI_DIF_FIELD_T *prsGfxReg)
{
  ...
  uint32_t u4DstPitch;
  uint32_t u4Alpha;
    uint32_t u4SrcWidth, u4SrcHeight, u4StrDstWidth, u4StrDstHeight;
    uint32_t u4wise=0;
    uint32_t ui4Temp = 0;
...
static INT32  _GfxSwPgigDecode(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    ui4Temp = 0;
    ui4Temp = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_PAL_BSAD);
...
static uint32_t _u4IsTableExist = (uint32_t)FALSE;
...
INT32 GFX_SwAlphaCompositionLoop(const GFX_ACLM_DATA_T *prsData)
{
    ...
    uint32_t u4SrcPitch, u4DstPitch;
    uint32_t u4SrcCM, u4DstCM;
    uint32_t u4RectSrc, u4RectColor;
    ...
    uint32_t u4PreColorize, u4ColorRepEn, u4JavaXorColor;
    ...
    if (_u4IsTableExist == (uint32_t)FALSE)
    {
    ...
    for (h = 0; h < height; h++)
    {
        for (w = 0; w < width; w++)
        {
            ...
            if (u4RectSrc)
            {
                ...
                pixel_s[3] = (uint32_t)(ai4RectColor[0]);
                pixel_s[2] = (uint32_t)(ai4RectColor[1]);
                pixel_s[1] = (uint32_t)(ai4RectColor[2]);
                pixel_s[0] = (uint32_t)(ai4RectColor[3]);
            }
            else
            {
                // Src pixel values
                GFX_SwGetColorComponent_5381(&pu1Read, u4SrcCM, ai4SrcColor);
                pixel_s[3] = (uint32_t)(ai4SrcColor[0]);
                pixel_s[2] = (uint32_t)(ai4SrcColor[1]);
                pixel_s[1] = (uint32_t)(ai4SrcColor[2]);
                pixel_s[0] = (uint32_t)(ai4SrcColor[3]);
            ...
            pixel_d[3] = (uint32_t)(ai4DstColor[0]);
            pixel_d[2] = (uint32_t)(ai4DstColor[1]);
            pixel_d[1] = (uint32_t)(ai4DstColor[2]);
            pixel_d[0] = (uint32_t)(ai4DstColor[3]);
...
INT32 GFX_SwAlphaCompositionLoop2Src(const GFX_ACLM_DATA_T *prsData)
{
    ...
    uint32_t u4SrcPitch, u4DstPitch, u4Src2Pitch;
    uint32_t u4SrcCM, u4DstCM;
    uint32_t u4RectSrc, u4RectColor;
    INT32 ai4SrcColor[COMP_NUM];
    INT32 ai4DstColor[COMP_NUM];
    ...
    for (h = 0; h < height; h++)
    {
        for (w = 0; w < width; w++)
        {
            ...
            if (u4RectSrc)
            {
                ...
                pixel_d[3] = (uint32_t)(ai4RectColor[0]);
                pixel_d[2] = (uint32_t)(ai4RectColor[1]);
                pixel_d[1] = (uint32_t)(ai4RectColor[2]);
                pixel_d[0] = (uint32_t)(ai4RectColor[3]);
            }
            else
            {
                // Src pixel values
...
static INT32 _GfxSwAlphaCompositionLoop(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    // get source & destination info
    u4Temp = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_SRC_BSAD);
    rData.pu1SrcBase = (UINT8 *)u4Temp;

    rData.u4SrcPitch = prsGfxReg->fg_SRC_PITCH << 4;    // 128bit aligned
    u4Temp = VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_DST_BSAD);
...
static INT32 _GfxSwAlphaCompositionLoop(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    rData.pu12ndSrcBase  = (UINT8 *)VIRADDR(prsGfxReg, (uint32_t)prsGfxReg->fg_SRCCBCR_BSAD);
 ...
 */
//-------------------------------------------------------------------------
void GFX_SwSetIdx2DirComp(UINT8 **ppu1DestFb, uint32_t u4DstCM,
    const UINT8 *pu1PaleBase, uint32_t u4PaleIdx)
{
    uint32_t *pu4PaleTable;
    UINT16 *pu2PaleTable;
    uint32_t u4Color;
    UINT16 u2Color;
    ...
    case CM_ARGB8888_DIRECT32:
        pu4PaleTable = (uint32_t *)pu1PaleBase;
        u4Color = pu4PaleTable[u4PaleIdx];
        *((uint32_t *)*ppu1DestFb) = u4Color;
        *ppu1DestFb += 4;
        break;
 ...
 */
//-------------------------------------------------------------------------
uint32_t GFX_SwGetPaleIdx(uint32_t u4CharCM, uint32_t u4ShiftBit, UINT8 u1SrcByte)
{
    uint32_t u4Mask;
    uint32_t u4PaleIdx;
    ...
    u4PaleIdx = (((uint32_t)u1SrcByte >> u4ShiftBit) & u4Mask);
...
INT32 GFX_SwIndexToDirectBitBlt(const GFX_IDX2DIR_DATA_T *prsData)
{
    ...
    uint32_t u4CharCM, u4DstCM;
    uint32_t u4Width, u4Height;
    uint32_t u4ByteAlign, u4MsbLeft;
    uint32_t i, y, g, sht;
    UINT8 *pu1DstLine= NULL;//*pu1SrcLine = NULL,
    UINT8 *pu1Read, *pu1Write;
    uint32_t u4BitNumPerByte, u4BitNumPerPix;
    uint32_t u4PixPerByte, u4PixGroup, u4Remainder;
    uint32_t u4PaleIdx;
    UINT8 u1SrcByte;
 ...
 */
//-------------------------------------------------------------------------
INT32 GFX_SwGetRegBase(uint32_t u4GfxHwId, uint32_t **ppu4RegBase)
{
    if ((ppu4RegBase != NULL) && (*ppu4RegBase != NULL))
    {
        *ppu4RegBase = (uint32_t *)&_au4GfxSwReg[u4GfxHwId][0];
        return (INT32)E_GFX_OK;
    }
 ...
 */
//-------------------------------------------------------------------------
uint32_t GFX_SwGetOpCount(void)
{
...
//-------------------------------------------------------------------------
/*lint -save -e740 -e826 */
INT32 GFX_SwAction(uint32_t u4GfxHwId)
{
...
void GFX_SwSetColorComponent(UINT8 **ppu1DestFb, uint32_t u4CM,
...
//-------------------------------------------------------------------------
/*lint -save -e703 -e704 -e826 -e961 */
void GFX_SwSetColorComponentEx(UINT8 **ppu1DestFb, uint32_t u4CM, const INT32 *pi4sColorComponent)
{
    INT32  i4Q, i4W, i4E, i4R;
    uint32_t u4Color;
    UINT16 u2Color;
...
//-------------------------------------------------------------------------
/*lint -save -e703 -e704 -e826 -e961 */
void GFX_SwSetColorComponent_Mirror(UINT8 **ppu1DestFb, uint32_t u4CM,
    const INT32 *pi4sColorComponent)
{
    INT32  i4Q, i4W, i4E, i4R;
    uint32_t u4Color;
...
//-------------------------------------------------------------------------
/*lint -save -e704 -e826 -e961 */
void GFX_SwGetColorComponent(UINT8 **ppu1DestFb, uint32_t u4CM,
...
//-------------------------------------------------------------------------
/*lint -save -e704 -e826 -e961 */
void GFX_SwGetColorComponentEx(UINT8 **ppu1DestFb, uint32_t u4CM, INT32 *pi4ColorComponent)
{
    // get color components from frame buffer, advance frame buffer point for 1 pixel
    INT32  i4Q, i4W, i4E, i4R;
    uint32_t u4Color;
...
//-------------------------------------------------------------------------
/*lint -save -e704 -e826 -e961 */
void GFX_SwGetColorComponent_Mirror(UINT8 **ppu1DestFb, uint32_t u4CM,
    INT32 *pi4ColorComponent)
{
    // get color components from frame buffer, advance frame buffer point for 1 pixel
    INT32  i4Q, i4W, i4E, i4R;
...
//-------------------------------------------------------------------------
/*lint -save -e704 -e826 -e961 */
void GFX_SwGetColorComponent_5381(UINT8 **ppu1DestFb, uint32_t u4CM,
    INT32 *pi4ColorComponent)
{
    // get color components from frame buffer, advance frame buffer point for 1 pixel
    INT32  i4Q, i4W, i4E, i4R;
...
//-------------------------------------------------------------------------
/*lint -save -e740 -e613 */
INT32 GFX_SwBitBlt(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
   ...
   // BIM_WAIT_WALE();
    ...
    for (y = 0; y < u4Height; y++)
    {
        ...
        for (x = 0; x < u4Width; x++)
        {
            ...
            if ((u4SrcCM == u4DstCM) && ((uint32_t)CM_RGB_CLUT8 == u4SrcCM))
            {
                if (u4TransEn)
                {
            ...
            else if((u4SrcCM == u4DstCM) && ((uint32_t)CM_RGB565_DIRECT16 == u4SrcCM))// 32bpp and 16bpp
            {
                if (u4TransEn)
                {
                    if (!u4DisDstKey && !u4DisSrcKey)//Dst Color Key
                    {
                        if (u4DstKeyIn && u4SrcKeyIn) //u4DstKeyIn==1 && u4SrcKeyIn==1
                        {
                            ...
                            {
                            // keep dst color
                                if ((uint32_t)CM_ARGB8888_DIRECT32 == u4DstCM)
                                {
                                    pu1Write += 4;
                                }
                                else if (((uint32_t)CM_ARGB4444_DIRECT16 == u4DstCM) ||
                                         ((uint32_t)CM_ARGB1555_DIRECT16 == u4DstCM) ||
                                         ((uint32_t)CM_RGB565_DIRECT16   == u4DstCM))
                                {
                                    pu1Write += 2;
                                }
                                else    // for lint happy
...
//-------------------------------------------------------------------------
/*lint -save -e740 -e613 */
INT32 GFX_24Bpp_SwBitBlt(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
    uint32_t u4Height, uint32_t u4ColorMin, uint32_t u4ColorMax, uint32_t u4TransEn,
    uint32_t u4ColchgEn, uint32_t u4SrcKeyNotIn, uint32_t u4Color, uint32_t u4GAlphaEn, uint32_t u4GAlpha)
{
    INT32 ai4ColorCompMin[4] = {0};
    INT32 ai4ColorCompMax[4] = {0};
    INT32 ai4ColorComp[4] = {0};
    ...
    UINT8 *pu1SrcLine, *pu1DstLine;
    UINT8 *pu1Read, *pu1Write;
    //, uint32_t u4GAlphaEn, uint32_t u4GAlpha
    // protection
    if (u4TransEn && u4ColchgEn)
    {
        return -(INT32)E_GFX_INV_ARG;
 ...
 */
//-------------------------------------------------------------------------
INT32 GFX_SwBitBlt_NewMethod(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
    uint32_t u4Height, uint32_t u4ColorMin, uint32_t u4ColorMax, uint32_t u4TransEn,
    uint32_t u4ColchgEn, uint32_t u4KeynotEn, uint32_t u4Color)
{
                ...
                if (u4TransEn)
                {
                    if (u4KeynotEn)
                    {
                            ...
                            (ai4ColorComp[3] <= ai4ColorCompMax[3]))
                        {
                            ...
                            {
                                pu1Write += ((uint32_t)CM_ARGB8888_DIRECT32 == u4SrcCM) ? 4 : 2;
                            }
                        }
                        else
                        {
...
//-------------------------------------------------------------------------
/*lint -save -e740 -e613 */
INT32 GFX_SwBitBlt_DstFlip(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
    uint32_t u4Height, uint32_t u4ColorMin, uint32_t u4ColorMax, uint32_t u4TransEn,
    uint32_t u4ColchgEn, uint32_t u4KeynotEn, uint32_t u4Color, uint32_t u4SrcFlag, uint32_t u4DstFlag )
{
    INT32 ai4ColorCompMin[4] = {0};
    INT32 ai4ColorCompMax[4] = {0};
    INT32 ai4ColorComp[4] = {0};
...
/*lint -save -e740 -e613 */
#if 0
INT32 GFX_SwBitBlt_DstMirr(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
    uint32_t u4Height, uint32_t u4ColorMin, uint32_t u4ColorMax, uint32_t u4TransEn,
    uint32_t u4ColchgEn, uint32_t u4KeynotEn, uint32_t u4Color)
{
...
//-------------------------------------------------------------------------
/*lint -save -e740 -e613 */
INT32 GFX_SwBitBlt_SrcFlip(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
    uint32_t u4Height, uint32_t u4ColorMin, uint32_t u4ColorMax, uint32_t u4TransEn,
    uint32_t u4ColchgEn, uint32_t u4KeynotEn, uint32_t u4Color)
{
...
//-------------------------------------------------------------------------
/*lint -save -e740 -e613 */
INT32 GFX_SwBitBlt_SrcMirr(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4SrcPitch,
    uint32_t u4DstPitch, uint32_t u4SrcCM, uint32_t u4DstCM, uint32_t u4Width,
    uint32_t u4Height, uint32_t u4ColorMin, uint32_t u4ColorMax, uint32_t u4TransEn,
    uint32_t u4ColchgEn, uint32_t u4KeynotEn, uint32_t u4Color)
{
    ...
    uint32_t u4Width, uint32_t u4Height)
{
    ...
    uint32_t u4DstPW;                 // dst pixel width
    uint32_t i, j;
    ...
    if (u4SrcCM != (uint32_t)CM_RGB_CLUT8)
    {
        return -(INT32)E_GFX_INV_ARG;
    }
    //HalFlushDCache();
...
{
    uint32_t u4Alpha, u4Fs, u4Fd, x, y;
    UINT8 *pu1SrcLine, *pu1DstLine;
    UINT8 *pu1Read, *pu1Write, *pu1Dst;
    ...
    case 2:     // Pass 2
        for (y = 0; y < u4Height; y++)
        {
            for (x = 0; x < u4Width; x++)
            {
                ...
                case E_AC_DST_IN:   // 1
                    u4Fs = 0;
                    u4Fd = (uint32_t)(ai4SrcColor[0]);
                    break;

                case E_AC_DST_OUT:  // 2
                    u4Fs = 0;
                    u4Fd = (uint32_t)(256 - ai4SrcColor[0]);
                    break;

                case E_AC_DST_OVER: // 3
                    u4Fs = (uint32_t)(256 - ai4DstColor[0]);
                    u4Fd = 256;
                    break;
    ...
    case 3:     // Pass 3
        for (y = 0; y < u4Height; y++)
        {
            for (x = 0; x < u4Width; x++)
            {
                ...
                u4Alpha = (uint32_t)ai4SrcColor[0];
                for (i = 1; i < 4; i++)
                {
                    if (u4Alpha)
                    {
    ...
    uint32_t u4SrcAlphaCheck, uint32_t u4ColorRepEn)
{
    ...
    UINT8 *pu1Read, *pu1Write;
    uint32_t x, y, i;
    uint32_t u4ByteNum = 0, u4PiexlLg = 0;
    uint32_t u4BreakFlag = 0;
...
//add for h2vline
INT32 GFX_SwH2VLine(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, uint32_t u4DstPitch,
                    ...
                    uint32_t u4StrDstWidth, uint32_t u4StrDstHeight, uint32_t u4Alpha, uint32_t u4wise)
{
    ...
    volatile uint32_t u4ByteNum, u4PiexlLg;
...
INT32 GFX_SwYCbCr420LinearScan(const GFX_YCBCR2RGB_DATA_T *prsData)
{
    ...
    uint32_t u4LumaPitch, u4ChromaPitch, u4DstPitch;
    uint32_t u4DstCM, i, j;
    uint32_t u4Width, u4Height;
    INT32 ai4Argb8888[4];
    ...
    // choose one equation
    if ((uint32_t)E_VSTD_BT601 == prsData->u4VideoStd)
    {
        if ((uint32_t)E_VSYS_VID == prsData->u4VideoSys)
        {
            pfnEquation = GFX_SwYCbCr601toArgbVideoEqu;
        }
        else if ((uint32_t)E_VSYS_COMP == prsData->u4VideoSys)
        {
    ...
    else if ((uint32_t)E_VSTD_BT709 == prsData->u4VideoStd)
    {
        if ((uint32_t)E_VSYS_VID == prsData->u4VideoSys)
        {
...
INT32 GFX_SwYCbCr2RGB(const GFX_YCBCR2RGB_DATA_T *prsData)
{
    ...
    if (prsData->u4YCFormat >= (uint32_t)E_YCFMT_422LINEAR)
    {
        // MT5371 does NOT support 422 Linear Scan
        return -(INT32)E_GFX_INV_ARG;
    }

    if (prsData->u4YCFormat == (uint32_t)E_YCFMT_420LINEAR)
    {
        GFX_UNUSED_RET(GFX_SwYCbCr420LinearScan(prsData))
    }
    else if (prsData->u4YCFormat == (uint32_t)E_YCFMT_420MB)
    {
        ;
    }
    else            // for lint happy
    ...
    uint32_t u4SrcPremult, uint32_t u4Width, uint32_t u4Height)
{
    UINT8 *pu1SrcLine, *pu1DstLine;
	UINT8 *pu1Read, *pu1Write, *pu1Dst;
	uint32_t u4Alpha0, u4Alpha1, x, y;
...
{
    uint32_t p, u4Color;
    uint32_t u4Byte0, u4Byte1, u4Byte2, u4Byte3;
    uint32_t *pu4DstPtr, *pu4SrcPtr;
    ...
    case CM_ARGB8888_DIRECT32:
        pu4DstPtr = ((uint32_t *)pu1DstAddr);
        pu4SrcPtr = ((uint32_t *)pu1SrcAddr);
...
INT32 GFX_SwGradientFill(const GFX_GRADIENT_DATA_T *prsData)
{
    UINT8 *pu1Write;
    uint32_t *pu4Color;
    uint32_t u4XInc, u4YInc;
    uint32_t u4Width, u4Height;
    uint32_t u4DstCM, u4RectColor, u4GradMode;
    INT32 i4ColorComponent[4], i4TempColor[4];
    FLOAT f4DeltaX[4], f4DeltaY[4], f4Temp[4];
    FLOAT f4TempColor[4];
    uint32_t i, j, xinc = 0, yinc = 0;
   ...
   // BIM_WAIT_WALE();
    for (j = 0; j < u4Height; j++)
    {
        ...
        for (i = 0; i < u4Width; i++)
        {
            ...
            if (u4GradMode & (uint32_t)E_GRAD_VER)
            {
            ...
            if (u4GradMode & (uint32_t)E_GRAD_HOR)
            {
...
{
#if 0
    uint32_t  u4Val, u4SrcIdx, u4Line, i;
...
INT32 GFX_SwPremultipliedCvnBitblt(const GFX_ACLM_DATA_T *prsData)
{
    ...
    uint32_t u4NonPre2PremultipliedEn, u4Pre2NonPremultipliedEn;
    int x, y;
    int byte_index;
    pixel **src;
    pixel **dst;
