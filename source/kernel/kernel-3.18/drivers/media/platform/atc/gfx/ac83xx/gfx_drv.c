...
static int atc_gfx_ioctl( struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	UINT32 u4DstPitch,u4DstX,u4DstY,u4Width,u4Height;
	static struct timeval start,end;
	#endif
	
	mutex_lock(&gfx_mutex);

	//pr_debug("current pid is 0x%08X",current->pid);
	...
	switch(cmd)
	{
		...
		case GFX_IOC_FILLRECT:
			...
			gfx_paras.dstBuffer.u4Addr = (uint32_t)((UINT32)pu1DstAddr & 0x3fffffff);
...
{
	/*struct gfx_device *gfxdev;
	int result;
	
	gfxdev = kzalloc(sizeof(struct gfx_device), GFP_KERNEL);
	if (gfxdev == NULL) {
		dev_err(&pdev->dev, "[gfx]: gfx_probe: malloc device failed\n");
		return -ENOMEM;
	}

	platform_set_drvdata(pdev, gfxdev);
 
    gfxdev->cdev.name = "gfx";
    gfxdev->cdev.minor = MISC_DYNAMIC_MINOR;
    gfxdev->cdev.fops = &gfx_fops;
  	#ifdef CONFIG_PM
	gfxdev->dev = &(pdev->dev);
	#endif
	gfxdev->cdev.parent = &(pdev->dev);

    result = misc_register(&(gfxdev->cdev));

    if ( result == 0 )
    {
        pr_info( "gfx init successes\n");
    } 
	else 
    {
        pr_err( "gfx misc device register error\n");
        kfree(gfxdev);
		return result;
    }
	g_gfxdev = gfxdev;
	#ifdef CONFIG_PM
	pm_runtime_enable(g_gfxdev->dev);
	#endif
    //call gfx HW init function */
	int result;
   ...
   gfx_addr= of_iomap(nd, 0);
	...
	gfx_base = (unsigned int)gfx_addr;
	printk(KERN_ERR "[GFX]:get gfx base addr %x\n",gfx_base);
