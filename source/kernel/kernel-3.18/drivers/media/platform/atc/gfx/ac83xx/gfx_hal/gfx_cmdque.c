...
#endif
#ifdef GFX_QUE_REG
static UINT32 GfxReg[80];
//static UINT32 GfxRegOld[80];
#endif
...
typedef struct _GFX_CMDQUE_VAR
{
   ...
   // for sw mode use
   UINT32 u4GfxSwFlushCount;
   UINT32 u4GfxSwIntCount ;
...
volatile UINT32 _u4GfxCmdQueBuf;
volatile UINT32 _u4GfxRegistBuf;
#endif
#else
#if 1//(CONFIG_DRV_VERIFY_SUPPORT)
UINT32 _u4GfxCmdQueBuf[4096];
UINT32 _u4GfxRegistBuf[8];
#endif
#endif
...
//static UINT32 _u4GfxHwFlushCount = 0;
...
// for sw mode use
//static UINT32 _u4GfxSwFlushCount = 0;
//static UINT32 _u4GfxSwIntCount = 0;
 ...
 */
//-------------------------------------------------------------------------
static void _GfxSwRealIsr(UINT32 u4GfxHwId)
{
 ...
 */
//-------------------------------------------------------------------------
static void _GfxSwQueAction(UINT32 u4GfxHwId)
{
    INT32 i4Ret;
    INT32 i4Element; 
    INT32 i4CurIdx;
    ...
    pu4RegBase = (UINT32 *)(Gfx_CmdQueVar[u4GfxHwId].prRegBase);
 ...
 */
//-------------------------------------------------------------------------
UINT32 _GFX_GetFlushCount(UINT32 u4GfxHwId)
{
    return (UINT32)Gfx_CmdQueVar[u4GfxHwId].u4GfxHwFlushCount;
}
...
INT32 GFX_CmdQueInit(void)
{
    ...
    for(i4 = 0;i4<GFX_HAL_HW_INST_NUM;i4++)
    {
        if ((UINT32)E_GFX_SW_MOD == GFX_DifGetData(i4)->u4GfxMode)
        {
    ...
    if (_i4GfxCmdqueBufExist == (INT32)FALSE)
    {
        ...
        {
            u4Size  = (UINT32)(Gfx_CmdQueVar[i4].i4GfxCqCapacity * GFX_ONE_CMD_SIZE);
 ...
 */
//-------------------------------------------------------------------------
INT32 GFX_CmdQueUninit(UINT32 u4GfxHwId)
{
    INT32 i4Ret;
    UINT32 u4GfxRegBase;

    // sw mode
    if ((UINT32)E_GFX_SW_MOD == GFX_DifGetData(u4GfxHwId)->u4GfxMode)
    {
...
VOID  GFX_CmdQueSetFlushAllFlag(UINT32 u4GfxHwId, UINT32 ui4FlushAllFlag)
{
 ...
 */
//-------------------------------------------------------------------------
INT32 GFX_CmdQueReset(UINT32 u4GfxHwId)
{
    ...
    default:
     ...
     _rGfxCmdQue[u4GfxHwId].pu8QueTop     = (volatile UINT64 *)
                                ((UINT32)Gfx_CmdQueVar[u4GfxHwId].pu8GfxCmdqueBuf);
    ...
    if ((UINT32)E_GFX_SW_MOD == GFX_DifGetData(u4GfxHwId)->u4GfxMode)
    {
        ...
        Gfx_CmdQueVar[u4GfxHwId].prRegBase->rField.fg_DRAMQ_BSAD = ((UINT32)_rGfxCmdQue[u4GfxHwId].pu8QueTop)&0x3fffffff;
    ...
    if ((UINT32)E_GFX_HW_8520_MOD == GFX_DifGetData(u4GfxHwId)->u4GfxMode)
    {
        ...
        u4Value = ((UINT32)_rGfxCmdQue[u4GfxHwId].i4CqSizeCfg << GREG_SHT_CYC_SIZE) |
        			((UINT32)_rGfxCmdQue[u4GfxHwId].pu8QueTop & 0x3FFFFFFF);
                  //PHYSICAL(((UINT32)_rGfxCmdQue[u4GfxHwId].pu8QueTop));
        ...
        u4Value = GFX_READ32(u4GfxHwId, GFX_REG_G_CONFIG);
        u4Value = (u4Value) |
                  ((UINT32)_rGfxCmdQue[u4GfxHwId].i4ShortCmdque << GREG_SHT_SRAM_LP);
        GFX_WRITE32(u4GfxHwId, GFX_REG_G_CONFIG, u4Value);
        GFX_MB();
    }
...
#ifdef GFX_QUE_REG
static UINT32  gaui4MoniterQueue[1024*10];
static UINT32  gui4MoniterQueueCount = 0;
static UINT32  gui4MoniterQueueCountPrev = 0;
 ...
 */
//-------------------------------------------------------------------------
INT32 GFX_CmdQuePushBack(UINT32 u4GfxHwId, UINT32 u4Reg, UINT32 u4Val)
{
  ...
  // GFX_LockCmdque(u4GfxHwId);
    ...
    // calculate GFX register address
     u4Reg = ( (unsigned int)gfx_base | ((u4Reg << 2) & 0xfff));
...
#if 1//(CONFIG_DRV_VERIFY_SUPPORT)
     if (((UINT32)E_GFX_SW_MOD == GFX_DifGetData(u4GfxHwId)->u4GfxMode) &&
         ((u4Reg == (UINT32)0xFD004010) && ((u4Val & GREG_MSK_FIRE) == GREG_MSK_FIRE)))
     {
         i4Ret = GFX_CmdQueAction(u4GfxHwId);
         if (i4Ret)
         {
 ...
 */
//-------------------------------------------------------------------------
void GFX_CmdQueDbgInfo(UINT32 u4GfxHwId)
{
    LOG(5, "\ngfx cmdq dump - begin\n");
 ...
 */
//-------------------------------------------------------------------------
void GFX_CmdQueSetCqCapacity(UINT32 u4GfxHwId, INT32 i4Capacity)
{
...
#ifdef GFX_QUE_REG
static UINT32 gui4QueueSize = 0;
static UINT32 gui4PreExt=0;
#endif

INT32 GFX_CmdQueCheckSize(UINT32 u4GfxHwId)
{
...
INT32 GFX_CmdQueAction(UINT32 u4GfxHwId)
{
    ...
    if ((UINT32)E_GFX_HW_8520_MOD == GFX_DifGetData(u4GfxHwId)->u4GfxMode)
    {
       ...
       //     RleFlushFlag=2;
        ...
        //HalFlushDCache();
        //BSP_FlushDCacheRange((UINT32)(_rGfxCmdQue[u4GfxHwId].pu8QueTop + _rGfxCmdQue[u4GfxHwId].i4ReadIndex), _rGfxCmdQue[u4GfxHwId].i4QueSize*GFX_ONE_CMD_SIZE);
        /*if (!_rGfxCmdQue[u4GfxHwId].bNeedFlushAll)
        {
            BSP_FlushDCacheRange((UINT32)(_rGfxCmdQue[u4GfxHwId].pu8QueTop), _rGfxCmdQue[u4GfxHwId].i4QueCapacity*GFX_ONE_CMD_SIZE);
        }
        else
        {
            HalFlushInvalidateDCache();
      ...
      //  GFX_SAVE_FLUSH_TIME();         // for debug use
        ...
        // 1 CMD size = 8 bytes
        //u4CmdQueLen = (UINT32)(_rGfxCmdQue[u4GfxHwId].i4QueSize * GFX_ONE_CMD_SIZE);
        ...
        u4CmdQueLen = (UINT32)(_rGfxCmdQue[u4GfxHwId].i4QueSize * GFX_ONE_CMD_SIZE) | 0x10000000;
#ifdef GFX_PERFORMANCE_TEST
#include "x_timer.h"
...
#if 0//(CONFIG_DRV_VERIFY_SUPPORT)
    {
        UINT32 u4i;
        UINT32 *u4GfxCmqTop;
        u4GfxCmqTop = (UINT32 *)_rGfxCmdQue[u4GfxHwId].pu8QueTop;
...
{
    UINT32  * pui4_data;
    UINT32  * pui4_data_old;
    UINT32  ui4_idx;

    pui4_data = (UINT32 *)(&_rGfxCmdQue.pu8QueTop[_rGfxCmdQue.i4PrevIndex]);
    pui4_data_old = (UINT32 *)(&_rGfxCmdQue.pu8QueTop[gui4PreExt]);
    LOG(0, "\n\n************* Queue info excuted **********\n");
...
{
    UINT32  ui4_idx;
    UINT32  * pui4_reg_addr = NULL;

    LOG(0, "\n\n************* Register info **********\n");

    pui4_reg_addr = (UINT32 * )0x70004000;
 ...
 */
//-------------------------------------------------------------------------
INT32 GFX_RiscPushBack(UINT32 u4GfxHwId, UINT32 u4Reg, UINT32 u4Val)
{
    INT32 i4Ret = (INT32)E_GFX_OK;
    UINT32 u4RegOffset;
    ...
    else if ((u4RegOffset == (UINT32)GFX_REG_G_MODE)&& ((u4Val & GREG_MSK_FIRE) == GREG_MSK_FIRE)) 
    {
      //  GFX_INC_DRV_FLUSH_COUNT(u4GfxHwId);  // for debug use
