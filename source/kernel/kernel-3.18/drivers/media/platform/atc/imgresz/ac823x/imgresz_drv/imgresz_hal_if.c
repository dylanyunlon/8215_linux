...
struct clk *clk_ac8317_imgr_parent1 = NULL;
struct clk *clk_ac8317_imgr_parent2 = NULL;

extern IMGRESZ_HAL_IMG_INFO_T SrcBuf;
extern volatile int scale_done;
...
typedef struct {
	...
	unsigned long u4TempBufSa;                               /*/ The start address of temporary buffer for partial mode.*/
...
IMGRESZ_HAL_INFO_T _rImgReszHalInfo[HW_IMGRESZ_NUM] = {{0}, {0} ,{ 0 } , { 0 } };  /* HAL local information for each HW.*/
...
void vImgreszSetMMU(u32 u4ImgResizerID, bool fg2way)
{
	...
	if (u4ImgResizerID == 0) {
		/*vIIOMMU_Init();*/
		IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ, 0x0030010A);
		/*reset table*/
		/*IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ, 0x0400000A);*/
		/*IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ, 0x0700000A);*/
		/*IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ, 0x0400000A);*/
		/*enable mmu*/
		u4RegVal = 0xC00000FE | (0x1 << 0);
		IOMMU_WRITE32(REG_RW_IOMMU_CFG0, IOMMU_RESZ, u4RegVal);
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG0, IOMMU_RESZ);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG0 is %x . \r\n", u4RegVal);

		IOMMU_WRITE32(REG_RW_IOMMU_CFG1, IOMMU_RESZ, u4HalGetTTB1());
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG1, IOMMU_RESZ);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG1 is %x . \r\n", u4RegVal);

                if(fg2way)
		        IOMMU_WRITE32(REG_RW_IOMMU_CFG2, IOMMU_RESZ, 0x00130011);
                else
                        IOMMU_WRITE32(REG_RW_IOMMU_CFG2, IOMMU_RESZ, 0x00130010);
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG2, IOMMU_RESZ);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG2 is %x . \r\n", u4RegVal);

		IOMMU_WRITE32(REG_RW_IOMMU_CFG3, IOMMU_RESZ, 0x00000000);
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG3, IOMMU_RESZ);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG3 is %x . \r\n", u4RegVal);
	} else if (u4ImgResizerID == 1) {
		/*vIIOMMU_Init();*/
		IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ1, 0x0020010A);
		/*reset table*/
		/*IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ, 0x0400000A);*/
		/*IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ, 0x0700000A);*/
		/*IOMMU_WRITE32(REG_RW_IOMMU_CFG4, IOMMU_RESZ, 0x0400000A);*/
		/*enable mmu*/
		u4RegVal = 0xC00000FE | (0x1 << 0);
		IOMMU_WRITE32(REG_RW_IOMMU_CFG0, IOMMU_RESZ1, u4RegVal);
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG0, IOMMU_RESZ1);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG0 is %x . \r\n", u4RegVal);

		IOMMU_WRITE32(REG_RW_IOMMU_CFG1, IOMMU_RESZ1, u4HalGetTTB1());
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG1, IOMMU_RESZ1);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG1 is %x . \r\n", u4RegVal);

		IOMMU_WRITE32(REG_RW_IOMMU_CFG2, IOMMU_RESZ1, 0x00130011);
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG2, IOMMU_RESZ1);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG2 is %x . \r\n", u4RegVal);

		IOMMU_WRITE32(REG_RW_IOMMU_CFG3, IOMMU_RESZ1, 0x00000000);
		u4RegVal = IOMMU_READ32(REG_RW_IOMMU_CFG3, IOMMU_RESZ1);
		IMGR_LOG(IMGR_LOG_LVL_DBG, "MMU CFG3 is %x . \r\n", u4RegVal);
	}
}
#endif
...
#define vWriteReg(dAddr, dVal)  (*(volatile u32 *)(IO_BASE_VA + dAddr) = dVal)
#define u4ReadReg(dAddr)         (*(volatile u32 *)(IO_BASE_VA + dAddr))

void vHwImgReszClk(void)
{
	int ret = 0;

	clk_ac8317_imgr_parent1 = clk_get(NULL, "syspll_d2");
	IMGR_LOG(IMGR_LOG_LVL_DBG, "got parent name [%s]  ", clk_ac8317_imgr_parent1->name);

	clk_ac8317_imgr_parent2 = clk_get(NULL, "syspll_d2");
	IMGR_LOG(IMGR_LOG_LVL_DBG, "got parent name [%s]  ", clk_ac8317_imgr_parent2->name);

	ret = clk_set_parent(clk_ac8317_imgr_top1, clk_ac8317_imgr_parent1);
	IMGR_LOG(IMGR_LOG_LVL_DBG, "ret is %d  ", ret);
	ret = clk_set_parent(clk_ac8317_imgr_top2, clk_ac8317_imgr_parent2);
	IMGR_LOG(IMGR_LOG_LVL_DBG, "ret is %d  ", ret);

	clk_ac8317_imgr_parent1 = clk_get_parent(clk_ac8317_imgr_top1);
	IMGR_LOG(IMGR_LOG_LVL_DBG, "regot parent clk, name [%s]  ", clk_ac8317_imgr_parent1->name);
	clk_ac8317_imgr_parent2 = clk_get_parent(clk_ac8317_imgr_top2);
	IMGR_LOG(IMGR_LOG_LVL_DBG, "regot parent clk, name [%s]  ", clk_ac8317_imgr_parent2->name);
}


irqreturn_t vImgResz_HAL_ISR(int vector_id, void *dev_id)
{
	u32 u4ImgReszID = 0;

	if (vector_id == imgr1irq) {
		u4ImgReszID = 1;
	} else if (vector_id == imgr0irq) {
		u4ImgReszID = 0;
		scale_done = 2;
	} else if (vector_id == imgr2irq) {
		u4ImgReszID = 2;
	} else if (vector_id == imgr3irq) {
		u4ImgReszID = 3;
	}
	//IMGR_LOG(IMGR_LOG_LVL_ERR, "enter imgresz ISR,HW is %d \n", u4ImgReszID);

	/*RETAILMSG(1,(TEXT("GET IMGRESZ IRQ = %d!!\r\n"),vector_id));*/
	...
	mt33xx_mask_ack_bim_irq(vector_id);
	return IRQ_HANDLED;
}
...
#endif
	if (request_irq(imgr0irq, vImgResz_HAL_ISR, 0, "ISR_IMGR0", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "request_irq %d error\r\n", imgr0irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	if (request_irq(imgr1irq, vImgResz_HAL_ISR, 0, "ISR_IMGR1", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "request_irq %d error\r\n", imgr1irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	if (request_irq(imgr2irq, vImgResz_HAL_ISR, 0, "ISR_IMGR2", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "request_irq %d error\r\n", imgr1irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	if (request_irq(imgr3irq, vImgResz_HAL_ISR, 0, "ISR_IMGR3", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "request_irq %d error\r\n", imgr1irq);
		return E_IMGRESZ_HAL_FAIL;
	}
...
#endif
	if (request_irq(imgr0irq, NULL, 0, "ISR_IMGR0", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "de-request_irq %d error\r\n", imgr0irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	if (request_irq(imgr1irq, NULL, 0, "ISR_IMGR1", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "de-request_irq %d error\r\n", imgr1irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	if (request_irq(imgr2irq, NULL, 0, "ISR_IMGR2", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "de-request_irq %d error\r\n", imgr0irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	if (request_irq(imgr3irq, NULL, 0, "ISR_IMGR3", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "de-request_irq %d error\r\n", imgr1irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	return S_IMGRESZ_HAL_OK;
...
)
{
	...
	{
		clk_prepare_enable(clk_ac8317_imgr1);
	}
	if(u4ImgReszID == 2)
		/*CKGEN_AgtOnClk(e_CLK_OSD_RESZ);*/
	{
		clk_prepare_enable(clk_ac8317_imgr2);
	}
	if(u4ImgReszID == 3)
		/*CKGEN_AgtOnClk(e_CLK_OSD_RESZ);*/
	{
		clk_prepare_enable(clk_ac8317_imgr3);
	}
	...
	if (fgMMUTable) {
                bool fg2way = true;
                if(_arImgreszInst[u4ImgReszID].eImgReszScaleMd == IMGRESZ_DRV_JPEG_PIC_SCALE||
					((SrcBuf.rBufferFormat.eBufferMainFormat==IMGRESZ_HAL_IMG_BUF_MAIN_FORMAT_Y_C_BUFFER) && (u4ImgReszID == 0)))
                        fg2way = false;
...
/*		clk_disable_unprepare(clk_ac8317_imgr0);
	...
	if(u4ImgReszID == 1)
		/*CKGEN_AgtOnClk(e_CLK_OSD_RESZ);*/
	{
		clk_disable_unprepare(clk_ac8317_imgr1);
	}
	if(u4ImgReszID == 2)
		/*CKGEN_AgtOnClk(e_CLK_OSD_RESZ);*/
	{
		clk_disable_unprepare(clk_ac8317_imgr2);
	}
	if(u4ImgReszID == 3)
		/*CKGEN_AgtOnClk(e_CLK_OSD_RESZ);*/
	{
		clk_disable_unprepare(clk_ac8317_imgr3);
	}
...
s32 i4ImgResz_HAL_Set_Partial_Mode_Info(
	...
	unsigned long u4TempBufSa      /*/< [IN] The start address of temporary buffer for partial mode.*/
...
)
{
	...
	x_memcpy(&(_rImgReszHalInfo[u4ImgReszID].rSrcRowBufInfo),
...
)
{
	...
	IMGR_LOG(IMGR_LOG_LVL_DBG," enter imgresz hal resize\n");
	...
	if ((prImgReszHalInfo->eResizeMode == IMGRESZ_HAL_RESIZE_MODE_FRAME) ||
		...
		if (prImgReszHalInfo->rSrcImgInfo.rBufferFormat.eBufferMainFormat ==
			...
			i4HwImgReszSetColorPalletTable(u4ImgReszID,
				(unsigned long)prImgReszHalInfo->rSrcImgInfo.rBufferFormat.prColorPallet);
		}
	...
	IMGR_LOG(IMGR_LOG_LVL_DBG," exit from imgresz hal resize\n");
