...
static VOID RemovePaVaFromMapTable(unsigned long u4Pa);
static u32 PA_TO_VA(unsigned long u4Pa);
...
VOID RemovePaVaFromMapTable(unsigned long u4Pa)
{
	u32 i;
...
u32 PA_TO_VA(unsigned long u4Pa)
{
	u32 j;
...
u32 VA_TO_PA(unsigned long u4Va)
{
	u32 j;
...
void *x_alloc_aligned_nc_mem(u32 u4Size, u32 u4Align)
{
	...
	AddPaVatoMapTable(pa, (unsigned long) u4Va, PAGE_ALIGN(u4Size));
...
void  x_free_aligned_nc_mem(void *pUser)
{
	...
	pvVa = (VOID *) PA_TO_VA((unsigned long)pUser);

#ifdef __linux__

	for (i = 0; i < _u4MapCnt; i++) {
		if (_aMapTable[i].u4Pa == (unsigned long)pUser) {
