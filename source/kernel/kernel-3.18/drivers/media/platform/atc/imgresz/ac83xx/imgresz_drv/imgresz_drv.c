...
extern struct reserved_mem *TempLine_Reserved;  /* added by mtk68119 to store the base address of reserved memory*/
...
static void *Dst;
int dst_is_phy=0;
IMGRESZ_DRV_DST_COLOR_MODE dst_color_mode;
...
s32 ImgReszHwInstNotifyCallback(s32 i4State, void *pvPrivData)
{
	...
	i4Ret = x_ev_group_set_event(_arImgreszHwInst[(u32)pvPrivData].hEventHandle,
...
bool ImgreszSetParam(struct file * filp,IMGRESZ_MW_PARAM * pParam)
{
	...
	dst_color_mode = pParam->DstColorMode;
	
	if ((pParam->SrcColorMode!=IMGRESZ_DRV_INPUT_COL_MD_420_BLK) ||
		((pParam->DstColorMode!=IMGRESZ_DRV_OUTPUT_COL_MD_RGB_565)&&(pParam->DstColorMode!=IMGRESZ_DRV_OUTPUT_COL_MD_ARGB_8888))) {
	...
	if (pParam->DstIsVirtual==false) {
		dst_is_phy = 1;
	}
	IMGR_LOG(IMGR_LOG_LVL_DBG,"dst_is_phy is %d \n",dst_is_phy);
	if (NeedToStop==true) {
	...
	IMGR_LOG(IMGR_LOG_LVL_DBG,"exit from imgresz driver set param \n");
	return true;
}
bool ImgreszScaleFire(struct file * filp)
{
	...
	if(dst_is_phy == 0)
		...
		DstBuf.u4BufSA1=(__u32)TempLine_Reserved->base;
	...
	IMGR_LOG(IMGR_LOG_LVL_DBG,"first end,begin second \n");
	...
	if(dst_color_mode == IMGRESZ_DRV_OUTPUT_COL_MD_ARGB_8888) {
	DstBuf.rBufferFormat.eARGBBufferFormat= IMGRESZ_HAL_ARGB_BUFFER_FORMAT_8888;
	DstBuf.u4BufWidth = DstBuf.u4BufWidth * 2;
	}
	...
	memset(&DstBuf,0,sizeof(IMGRESZ_HAL_IMG_INFO_T));
	i4ImgResz_Drv_ReleaseTicket(&ticket);
	IMGR_LOG(IMGR_LOG_LVL_DBG,"exit from imgresz driver scale fire \n");
		      ...
		      VOID  *prDestImgInfo)
{
	...
	u4TempLineBufSa = (u32)x_alloc_aligned_nc_mem(prDst->u4BufWidth * 4 * 3, 16);
...
void vImgReszHwInstResz(u32 u4HwId, IMGRESZ_INST_T *prImgReszInst)
{
	...
	if (prImgReszInst->eImgReszScaleMd == IMGRESZ_DRV_JPEG_PIC_SCALE) { /* Jpeg picture mode*/
       ...
       /*added by mtk68119 to set the value of TempLineBufSa*/
		...
		prImgReszInst->u4TempLineBufSa = (u32)TempLine_Reserved->base;
