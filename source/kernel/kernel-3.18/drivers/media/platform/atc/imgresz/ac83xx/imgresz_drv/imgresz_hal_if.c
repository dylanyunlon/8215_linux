...
struct clk *clk_ac8317_imgr_parent = NULL;
extern IMGRESZ_HAL_IMG_INFO_T SrcBuf;
extern int scale_done;
...
void vHwImgReszClk(void)
{
	...
	clk_ac8317_imgr_parent = clk_get(NULL, "syspll_d2");
	IMGR_LOG(IMGR_LOG_LVL_DBG, "got parent name [%s]  ", clk_ac8317_imgr_parent->name);

	ret = clk_set_parent(clk_ac8317_imgr, clk_ac8317_imgr_parent);
	IMGR_LOG(IMGR_LOG_LVL_DBG, "ret is %d  ", ret);

	clk_ac8317_imgr_parent = clk_get_parent(clk_ac8317_imgr);
	IMGR_LOG(IMGR_LOG_LVL_DBG, "regot parent clk, name [%s]  ", clk_ac8317_imgr_parent->name);
}
...
irqreturn_t vImgResz_HAL_ISR(int vector_id, void *dev_id)
{
	...
	ac83xx_mask_ack_bim_irq(vector_id);
	return IRQ_HANDLED;
}
...
)
{
	...
	if (request_irq(imgr1irq, vImgResz_HAL_ISR, 0, "ISR_IMGR0", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "request_irq %d error\r\n", imgr1irq);
		return E_IMGRESZ_HAL_FAIL;
	}
...
)
{
	...
	if (request_irq(imgr1irq, NULL, 0, "ISR_IMGR0", NULL) != OSR_OK) {
		IMGR_LOG(IMGR_LOG_LVL_ERR, "de-request_irq %d error\r\n", imgr1irq);
		return E_IMGRESZ_HAL_FAIL;
	}
	return S_IMGRESZ_HAL_OK;
