	...
	*time_period = time_now - *start_time;
	...
	if (accumulated_work_time_gpu == 0 && work_start_time_gpu == 0) {
		...
		*need_add_timer = MALI_FALSE;
		...
		MALI_DEBUG_PRINT(1, ("last_utilization_gpu = %d \n", last_utilization_gpu));
		MALI_DEBUG_PRINT(1, ("last_utilization_gp = %d \n", last_utilization_gp));
		MALI_DEBUG_PRINT(1, ("last_utilization_pp = %d \n", last_utilization_pp));
