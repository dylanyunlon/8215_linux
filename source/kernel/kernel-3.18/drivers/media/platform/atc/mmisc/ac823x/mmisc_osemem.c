...
#define OSE_HOUSEKEEPING_BUF_VA_START  (((uintptr_t)MULTIMEDIA_BUF_VA_START) + MULTIMEDIA_BUF_SIZE - OSE_HOUSEKEEPING_BUF_SIZE)
...
static OSE_DYNAMIC_NODE *g_DynamicMemNodeList = NULL;
...
{
	memset(pNode, 0, sizeof(OSE_MEM_NODE));
}
...
bool OSE_Init(void)
{
	...
	u32 u4MaxNodeCnt = 0;
	OSE_MEM_NODE *prNode = NULL;
	void *mm_reserved_vir = MULTIMEDIA_BUF_VA_START;

	pr_info("OSE_Init enter\r\n");
	...
	pr_info("OSE_Init u4MaxNodeCnt: %d, pOSEMemList: %p, pOSENodeList: %p\r\n",
		u4MaxNodeCnt, pOSEMemList, pOSENodeList);
	init_MUTEX(&g_hOSESema);
	down(&g_hOSESema);

	pr_info("OSE_Init memset pOSEMemList: %p\r\n", pOSEMemList);
	memset((void *)pOSEMemList, 0, sizeof(OSE_MEM_LIST));
	pr_info("OSE_Init memset pOSENodeList: %p\r\n", pOSENodeList);
	memset((void *)pOSENodeList, 0, (OSE_HOUSEKEEPING_BUF_SIZE - sizeof(OSE_MEM_LIST)));

  prNode = OSE_GET_NODE(pOSENodeList, u4MaxNodeCnt);
	pr_info("OSE_Init prNode: %p\r\n", prNode);
	pOSEMemList->head = pOSEMemList->tail = prNode;
...
#endif
		pr_info("OSE_Init pvVirAddr: %p, u4Size: 0x%08x\r\n",
			pOSEMemList->head->pvVirAddr, pOSEMemList->head->u4Size);
	}

	g_DynamicMemNodeList = NULL;

	up(&g_hOSESema);
	pr_info("OSE_Init success\r\n");
...
void *OSE_MemAllocCustom_R(OSE_MEM_REGION eMemRegion, u32 u4Size, u32 u4Align,
	...
	if (NULL == pNode) {
		...
		*pptrPhyAddr = (uintptr_t) NULL;
		...
		memcpy_toio((void *)pNode->pvVirAddr, g_guard, sizeof(g_guard));
		memcpy_toio((void *)(pNode->pvActualVirAddr - sizeof(g_guard)), g_guard,
			  sizeof(g_guard));
		memcpy_toio((void *)(pNode->pvActualVirAddr + pNode->u4ActualSize), g_guard,
			  sizeof(g_guard));
...
void *OSE_MemAllocCustom_S(OSE_MEM_REGION eMemRegion, u32 u4Size, u32 u4Align,
	...
	if (NULL == pDynamicNode) {
		*pptrPhyAddr = (uintptr_t) 0;
		...
		pDynamicNode->u4HwPhyAddr = mm_cpuphy_to_hwphy(pDynamicNode->ptrPhyAddr);
		...
		if (NULL != pDynamicNode->pvVirAddr) {
			...
			pDynamicNode->u4ActualHwPhyAddr =
			    pDynamicNode->u4HwPhyAddr + NEW_ALIGN_VALUE(u4Align);

			*pptrPhyAddr = pDynamicNode->ptrActualPhyAddr;
			pvVirAddr = pDynamicNode->pvActualVirAddr;

			memcpy_toio(pDynamicNode->pvVirAddr, g_guard, sizeof(g_guard));
			memcpy_toio((void *)((uintptr_t) (pDynamicNode->pvActualVirAddr) -
...
u32 OSE_VAToHwPA(void *pvVirAddr)
{
	OSE_DYNAMIC_NODE *pDynamicNode = g_DynamicMemNodeList;
...
#endif
#ifndef BSP_NO_MEMMAN
				return pDynamicNode->u4ActualHwPhyAddr;
#else
				return pDynamicNode->u4HwPhyAddr;
#endif
			} else {
				pDynamicNode = pDynamicNode->next;
			}
	...
	return (u32)(g_rsvmem_info->phys_addr +
			((uintptr_t) pvVirAddr - (uintptr_t) (g_rsvmem_info->virt_addr)));
}
EXPORT_SYMBOL(OSE_VAToHwPA);
...
void *OSE_PAToVA(uintptr_t ptrPhy)
{
	...
	if ((ptrPhy > (uintptr_t) OSE_HOUSEKEEPING_BUF_PA_START)
		...
		return (void *)(g_rsvmem_info->virt_addr +
				((uintptr_t) ptrPhy - (uintptr_t) (g_rsvmem_info->phys_addr)));
}
EXPORT_SYMBOL(OSE_PAToVA);

void *OSE_HwPAToVA(u32 u4HwPhy)
{
	OSE_DYNAMIC_NODE *pDynamicNode = g_DynamicMemNodeList;

	if ((u4HwPhy > mm_cpuphy_to_hwphy(OSE_HOUSEKEEPING_BUF_PA_START))
	    || (u4HwPhy < mm_cpuphy_to_hwphy(MULTIMEDIA_BUF_PA_START))) {
		while (NULL != pDynamicNode) {
#ifndef BSP_NO_MEMMAN
			if (pDynamicNode->u4ActualHwPhyAddr == u4HwPhy) {
#else
			if (pDynamicNode->u4HwPhyAddr == u4HwPhy) {
#endif
#ifndef BSP_NO_MEMMAN
				return pDynamicNode->pvActualVirAddr;
#else
	...
	return (void *)(g_rsvmem_info->virt_addr +
			((u32) u4HwPhy - (u32) (g_rsvmem_info->phys_addr)));
}
EXPORT_SYMBOL(OSE_HwPAToVA);
...
static void *MM_GetReservedMemSAddr(void)
{
	...
	pr_debug("MM_GetReservedMemSAddr return 0x%p\r\n",
		 (void *)(g_rsvmem_info->virt_addr));
...
static u32 MM_GetReservedMemSize(void)
{
	...
	pr_debug("MM_GetReservedMemSize return 0x%08x\r\n",
		  g_rsvmem_info->size);
