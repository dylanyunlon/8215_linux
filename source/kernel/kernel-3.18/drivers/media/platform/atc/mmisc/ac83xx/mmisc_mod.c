	...
	case IOCTL_MMISC_RELEASE_HW_MEM:
		{
			...
			void *pvMMResrVirAddr = NULL;
			__u32 u4HwResrMemSz = OSE_GetMMReservedMemSize();
			...
			if ((uintptr_t)pvHwVirAddr >= (uintptr_t)pvMMResrVirAddr + (uintptr_t)u4HwResrMemSz) {
				pr_err("%s line %d fail for invalid hw virtual memory addr: 0x%p, [%p, %p], arg: 0x%p, err: %d\r\n",
							 __func__, __LINE__, pvHwVirAddr, pvMMResrVirAddr,
							 (void *)((uintptr_t)pvMMResrVirAddr + (uintptr_t)u4HwResrMemSz),
							 arg, ret);
				return -EINVAL;
			}
	...
	case IOCTL_MMISC_MEMDBG_RMNODE:
		{
			...
			if (!MM_MemDbg_RemoveNode(rRmNodeInfo.pvAddr, &(rRmNodeInfo.pvOutAddr))) {
				pr_err
				    ("%s line %d fail in mm memdbg insert node, remove node: %p, err: %d\r\n",
			...
			if (0 != ret) {
				pr_err
				    ("%s line %d fail in copy to user, arg: %p, err: %d\r\n",
				     __func__, __LINE__, arg, ret);
				return -EACCES;
			}
		}
...
static int mmiscdev_probe(struct platform_device *pdev)
{
	...
	mmisc_dev->reserved_mem_info.virt_addr = ioremap(mmisc_reserved_mem->base,
