...
// 通用USB摄像头扩展控制 - 移除SONIX特定定义

// 通用扩展单元控制基础ID
#define V4L2_CID_BASE_EXTCTR_GENERIC			0x0A0c4501
#define V4L2_CID_BASE_GENERIC					V4L2_CID_BASE_EXTCTR_GENERIC
#define V4L2_CID_GENERIC_CTRL					V4L2_CID_BASE_GENERIC+1
#define V4L2_CID_LAST_EXTCTR_GENERIC			V4L2_CID_GENERIC_CTRL

// 通用控制选择器 - 移除SONIX特定定义，支持通用USB摄像头
#define XU_GENERIC_CTRL      					0x01

// 通用USB摄像头不需要SONIX特定的扩展单元控制定义
...
void uvc_ctrl_cleanup_device(struct uvc_device *dev)
{
	...
	list_for_each_entry(entity, &dev->entities, list) {
		...
		entity->controls = NULL;  /* Important: Set to NULL after free */
		entity->ncontrols = 0;
	}
}
...
{
	// 对于通用USB摄像头，不再进行SONIX特定的芯片检测
	// 直接设置为通用设备并返回成功
	DbgPrint(My_TRACE_XU,"uvc_xu_ctrll_ReadChip: Generic USB camera detected\n");
	
	*chip = CHIP_NONE;  // 表示通用设备，不是特定的SONIX芯片
	dev->valid_device = 1;  // 标记为有效设备
	
	DbgPrint(My_TRACE_XU,"uvc_xu_ctrll_ReadChip: Generic camera initialized successfully\n");
	return 0;
}
