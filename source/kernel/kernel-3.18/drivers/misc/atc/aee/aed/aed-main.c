...
#define AEE_IO_BASE	(0xF0000000)
#elif defined(CONFIG_ATC_PLATFORM_ac823x)
#define AEE_IO_BASE	(0x10000000)
#else
...
static int aed_ee_mmap(struct file *fp, struct vm_area_struct *vma)
{
        ...
        if (remap_pfn_range(vma,
                        vma->vm_start,
                        __phys_to_pfn(AEE_IO_BASE) + vma->vm_pgoff,
...
#if defined(CONFIG_MTK_AEE_AED) && defined(CONFIG_DAL_ATC)
	if (down_interruptible(&aed_dal_sem) < 0) {
...
#ifdef CONFIG_SYS_CHECK_ATC
int syscheck_gpio_reserve(int pin, int dir,
									const char *name)
{
	int ret;
    ...
    GPIO_MultiFun_Set(pin, PINMUX_LEVEL_GPIO_END_FLAG);

	ret = gpio_request(pin, name);
	if (ret) {
		LOGD("Failed to allocat pin %d (%s)\n", pin, name);
		return ret;
	}

	ret = gpio_direction_output(pin, dir);
	if(ret) {
		LOGD("Failed to set direction for pin %d (%s)\n", pin, name);
		gpio_free(pin);
		return ret;
	}
	
  gpio_set_value(pin, 0);
	LOGD("Set Low Level for pin %d (%s).\n", pin);
	
	ret = gpio_export(pin, 0);
	if(ret) {
		LOGD("Failed to export pin %d (%s)\n", pin, name);
		gpio_free(pin);
		return ret;
	}
...
EXPORT_SYMBOL(syscheck_gpio_reserve);
#endif /*CONFIG_SYS_CHECK_ATC.	*/
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("ATC AED Driver");
