...
const ipanic_dt_op_t ipanic_dt_ops[] = {
	...
	{"SYS_KERNEL_LOG", __LOG_BUF_LEN, ipanic_klog_buffer}, // 0.25M
	{"SYS_WDT_LOG", WDT_LOG_LEN, ipanic_klog_buffer},      // 0.25M
	{"SYS_WQ_LOG", 0, NULL},
	{"reserved", 0, NULL},
	{"reserved", 0, NULL},
	{"PROC_CUR_TSK", sizeof(struct aee_process_info), ipanic_current_task_info}, // ~3.5K
	{"_exp_detail.txt", OOPS_LOG_LEN, ipanic_klog_buffer},  // 0.25M
	{"SYS_MINI_RDUMP", MRDUMP_MINI_BUF_SIZE, NULL}, /* 8 */ // 1.25M+4K
#ifdef CONFIG_MTK_MMPROFILE_SUPPORT
	{"SYS_MMPROFILE", IPANIC_MMPROFILE_LIMIT, ipanic_mmprofile}, // 2.2M
#else
	{"SYS_MMPROFILE", 0, NULL},
#endif


#ifdef AEE_FOR_ANDROID
	{"SYS_MAIN_LOG_RAW", __MAIN_BUF_SIZE, ipanic_alog_buffer},      // 256K
	{"SYS_SYSTEM_LOG_RAW", __SYSTEM_BUF_SIZE, ipanic_alog_buffer},  // 256K
	{"SYS_EVENTS_LOG_RAW", __EVENTS_BUF_SIZE, ipanic_alog_buffer},  // 256K
	{"SYS_RADIO_LOG_RAW", __RADIO_BUF_SIZE, ipanic_alog_buffer},    // 256K
	{"SYS_LAST_LOG", LAST_LOG_LEN, ipanic_klog_buffer},             // 256K
	{"SYS_ATF_LOG", ATF_LOG_SIZE, ipanic_atflog_buffer},            // 32K
#else
	/*
	 * Andriod log is deleted here.
	 * @11/04/2016 Joshua Wang.
	 */

	{"SYS_MAIN_LOG_RAW", 0, NULL},
	{"SYS_SYSTEM_LOG_RAW", 0, NULL},
	{"SYS_EVENTS_LOG_RAW", 0, NULL},
	{"SYS_RADIO_LOG_RAW", 0, NULL},
	{"SYS_LAST_LOG", LAST_LOG_LEN, NULL},   //256K
	{"SYS_ATF_LOG", 0, NULL},
#endif /* AEE_FOR_ANDROID. */
...
inline int ipanic_func_write(fn_next next, void *data, int off, int total, int encrypt)
{
	...
	if (!iheader) {
		LOGE("[aee]ipanic header invalide.");
		return -ENXIO;
	}
...
int ipanic_data_to_sd(int dt, void *data)
{
	...
	errno = ipanic_header_to_sd(dheader);
	return errno;
}

static bool ipanic_mrdump_mini(AEE_REBOOT_MODE reboot_mode, const char *msg, ...)
{
	int ret;
	...
	LOGE("[aee]ipanic mrdump mini entry."); // for identify last kmsg info in CrashCollector.
	...
	if (ipanic_hdr) {
		...
		LOGE("[aee]ipanic mrdump mini Failed to get Header.");
		return false;
	}
	return true;
}
...
int ipanic(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	if (!ipanic_mrdump_mini(AEE_REBOOT_MODE_KERNEL_PANIC, "kernel PANIC")) {
		LOGE("[aee]ipanic mrdump mini Failed.");
		goto ERROR;
	}
	if (!ipanic_data_is_valid(IPANIC_DT_KERNEL_LOG)) {
		...
		if (IS_ERR(ERR_PTR(errno))) {
			if (errno == -1)
				aee_nested_printf("$");
			if (errno == -ENXIO) {
				LOGE("[aee]ipanic data to sd Failed.");
				goto ERROR;
			}
		}
	}
...
void ipanic_recursive_ke(struct pt_regs *regs, struct pt_regs *excp_regs, int cpu)
{
	...
	flush_cache_all();
	if (!ipanic_mrdump_mini(AEE_REBOOT_MODE_NESTED_EXCEPTION, "Nested Panic")) {
		LOGE("[aee]ipanic mrdump mini Failed.");
		return ;
	}
...
struct ipanic_header *ipanic_header(void)
{
	...
	/*
	 * Keep the old expdb layout. @04/07/2016.
	 */
#if 0
	...
	for (i = IPANIC_DT_HEADER + 1; i < IPANIC_DT_RESERVED31; i++) {
		...
		if (ipanic_dt_active(i) && ipanic_dt_ops[i].size) {
			...
			if (iheader->partsize < (dheader->offset + dheader->total)) {
				LOGW("skip %s[%x@%x>%x]\n", dheader->name, dheader->total, dheader->offset, iheader->partsize);
...
#ifdef CONFIG_MTD_NAND_ATC
extern int check_sdcard_present(void);
	if (0 == check_sdcard_present())
		return NOTIFY_DONE;
#endif /* CONFIG_MTD_NAND_ATC */
	aee_disable_api();
