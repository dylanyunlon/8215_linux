	...
	case 32:
		{
			...
			u4Size &= 0xFFFFFFFC;
			u4Addr &= 0xFFFFFFFC;
			u4VirAddr = CLIMEMRSV_PHY_TO_VIRT(u4Addr);
			pr_err(TEXT("\r\n Read  %d DWORDs from address 0x%x"),
					u4Size >> 2, u4Addr);
			for (u4Loop = 0; u4Loop < u4Size; u4Loop += 4) {
				...
				pr_err(TEXT("  0X%08x"),
					__raw_readl((const volatile void *)(u4VirAddr + u4Loop)));
			}
			pr_err(TEXT("\r\n"));
		}
		break;
	case 16:
		{
			...
			for (u4Loop = 0; u4Loop < u4Size; u4Loop += 2) {
				if (!(u4Loop % 16))
					pr_err(TEXT("\r\n 0x%08x: "), u4Addr + u4Loop);
				pr_err(TEXT("  0X%04x"), __raw_readw((const volatile void *)(u4VirAddr + u4Loop)));
			}
			pr_err(TEXT("\r\n"));
		}
		break;
	default:
		{
			...
			for (u4Loop = 0; u4Loop < u4Size; u4Loop++) {
				if (!(u4Loop % 16))
					pr_err(TEXT("\r\n 0x%08x: "), u4Addr + u4Loop);
				pr_err(TEXT("  0X%02x: "), __raw_readb((const volatile void *)(u4VirAddr + u4Loop)));
			}
			pr_err(TEXT("\r\n"));
		}
		break;
	...
	case 32:
		{
			...
			u4Addr &= 0xFFFFFFFC;
			u4VirAddr = CLIMEMRSV_PHY_TO_VIRT(u4Addr);
			pr_err(TEXT("\r\n Write %d DWORDs from address 0x%x"), u4Size >> 2, u4Addr);
	...
	case 16:
		{
			...
			u4Addr &= 0xFFFFFFFE;
			u4VirAddr = CLIMEMRSV_PHY_TO_VIRT(u4Addr);
			pr_err(TEXT("\r\n Write %d DWORDs from address 0x%x"), u4Size >> 1, u4Addr);
