...
#define UART_READ32_(REG)            __raw_readl(__io(RS232_BASE_VA + REG))
#define UART_WRITE32_(VAL, REG)      __raw_writel(VAL, __io(RS232_BASE_VA + REG))
...
#define CKGEN_READ32_(REG)            __raw_readl(__io(CKGEN_BASE_VA + REG))
#define CKGEN_WRITE32_(VAL, REG)      __raw_writel(VAL, __io(CKGEN_BASE_VA + REG))
...
static int __init drvcli_init(VOID)
{
	...
	ret = register_chrdev(0, "drvcli", &cli_fops);
