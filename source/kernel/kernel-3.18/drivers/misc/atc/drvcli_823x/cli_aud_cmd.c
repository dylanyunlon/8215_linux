...
u32 g_u4Value = 0;
PCM_SPEECH_CONF         g_rSpeech_conf = {{0},0,{0},0,{0},{0},{0}};
PCM_SPEECH_16K_CONF     g_rSpeech_16k_conf = {{0},{0}};
...
static s32 _CLI_EnableBTSCO(s32 i4Argc, const s8 **szArgv)
{
	CLI_PCM_CHECK_DRV_HANDLE();

	if (i4Argc < 2) {
		pr_info(TEXT("drv.aud.pcm.ensco <0:Disable 1:Enable> [samplerate:8000/1600]\r\n"));
		return 0;
	}
	if (0 == _ttoi(szArgv[1])) {
		tCliCfg.eAudCliType = BT_SCO_DISABLE;
	} else {
		tCliCfg.eAudCliType = BT_SCO_ENABLE;
	}
    g_u4Value = 8000;
    if (i4Argc >= 3)
	{
			    g_u4Value = _ttoi(szArgv[2]);
    }
...
static s32 _CLI_EnableDLFromFile(s32 i4Argc, const s8 **szArgv)
{
	CLI_PCM_CHECK_DRV_HANDLE();

	if (i4Argc < 2) {
		pr_info(TEXT("drv.aud.pcm.endl <0:Disable 1:Enable>\r\n"));
		return 0;
	}

	tCliCfg.eAudCliType = SET_DL_USE_FILE;
...
static s32 _CLI_EnableULFromFile(s32 i4Argc, const s8 **szArgv)
{
	CLI_PCM_CHECK_DRV_HANDLE();

	if (i4Argc < 2) {
		pr_info(TEXT("drv.aud.pcm.enul <0:Disable 1:Enable>\r\n"));
		return 0;
	}

	tCliCfg.eAudCliType = SET_UL_USE_FILE;
	g_u4Value = _ttoi(szArgv[1]);
	tCliCfg.u4arg1 = (UINT32)(&g_u4Value);
	tCliCfg.u4arg2 = sizeof(g_u4Value);
...
static s32 _CLI_EnableBTSphPLC(s32 i4Argc, const s8 **szArgv)
{
	CLI_PCM_CHECK_DRV_HANDLE();

	if (i4Argc < 2) {
		pr_info(TEXT("drv.aud.pcm.enplc <0:Disable 1:Enable>\r\n"));
		return 0;
	}

	tCliCfg.eAudCliType = SET_BT_SPH_PLC;
	g_u4Value = _ttoi(szArgv[1]);
	tCliCfg.u4arg1 = (UINT32)(&g_u4Value);
	tCliCfg.u4arg2 = sizeof(g_u4Value);
...
static s32 _CLI_EnableNdcCapture(s32 i4Argc, const s8 **szArgv)
{
	CLI_PCM_CHECK_DRV_HANDLE();

	if (i4Argc < 2) {
		pr_info(TEXT("drv.aud.pcm.ennc <0:Disable 1:Enable>\r\n"));
		return 0;
	}

	tCliCfg.eAudCliType = SET_CAPTURE_NDC_ENABLE;
	g_u4Value = _ttoi(szArgv[1]);
	tCliCfg.u4arg1 = (UINT32)(&g_u4Value);
	tCliCfg.u4arg2 = sizeof(g_u4Value);
...
static s32 _CLI_EnableBTSphDump(s32 i4Argc, const s8 **szArgv)
{
	CLI_PCM_CHECK_DRV_HANDLE();

	if (i4Argc < 2) {
		pr_info(TEXT("drv.aud.pcm.btdump <0:Disable 1:Enable>\r\n"));
		return 0;
	}

	tCliCfg.eAudCliType = SET_BT_SPH_DUMP;
	g_u4Value = _ttoi(szArgv[1]);
	tCliCfg.u4arg1 = (UINT32)(&g_u4Value);
	tCliCfg.u4arg2 = sizeof(g_u4Value);
...
static s32 _CLI_SpeechDelay(s32 i4Argc, const s8 **szArgv)
{
	...
	if (i4Argc < 2) {
		tCliCfg.eAudCliType = GET_SPH_DELAY;
		g_u4Value = 0;
	} else {
		tCliCfg.eAudCliType = SET_SPH_DELAY;
		g_u4Value = _ttoi(szArgv[1]);
	}
	tCliCfg.u4arg1 = (UINT32)(&g_u4Value);
	tCliCfg.u4arg2 = sizeof(g_u4Value);
...
static s32 _CLI_Speech16KDelay(s32 i4Argc, const s8 **szArgv)
{
	CLI_PCM_CHECK_DRV_HANDLE();

	if (i4Argc < 2) {
		tCliCfg.eAudCliType = GET_SPH_16K_DELAY;
		g_u4Value = 0;
	} else {
		tCliCfg.eAudCliType = SET_SPH_16K_DELAY;
		g_u4Value = _ttoi(szArgv[1]);
	}
	tCliCfg.u4arg1 = (UINT32)(&g_u4Value);
	tCliCfg.u4arg2 = sizeof(g_u4Value);
...
static s32 _CLI_EnhanceParam(s32 i4Argc, const s8 **szArgv)
{
    u32 u4Idx;
	s32 i4Value;
	...
	if (i4Argc < 2) {
		tCliCfg.eAudCliType = GET_AEC_NDC_PARAM;
		tCliCfg.u4arg1 = (UINT32)(&g_rSpeech_conf);
		tCliCfg.u4arg2 = sizeof(g_rSpeech_conf);
		CLI_PCM_IOCTL();
		return 0; /* Get enhance param return */
	}
	if (i4Argc != 4) {
		pr_err(TEXT("drv.aud.pcm.enhance: param number != 4\r\n"));
		return 0;
	}
	if ((_ttoi(szArgv[1]) != 1) && (_ttoi(szArgv[1]) != 2)) {
		pr_err(TEXT("drv.aud.pcm.enhance: param1 must between 1 and 2\r\n"));
		return 0;
	}
	if ((_ttoi(szArgv[2]) > 28) || (_ttoi(szArgv[2]) < 0)) {
		pr_err(TEXT("drv.aud.pcm.enhance:  param2 must between 0 and 28\r\n"));
		return 0;
	}
	u4Idx = (u32)_ttoi(szArgv[2]);
	i4Value = (s32)_ttoi(szArgv[3]);
	...
	if (1 == _ttoi(szArgv[1])) {
		if (28 == u4Idx) {
			g_rSpeech_conf.error_flag = i4Value;
		} else {
			g_rSpeech_conf.enhance_pars[u4Idx] = i4Value;
		}
	} else {
		if (28 == u4Idx) {
			g_rSpeech_conf.error_flag2 = i4Value;
		} else {
			g_rSpeech_conf.enhance_pars2[u4Idx] = i4Value;
		}
	}

	tCliCfg.eAudCliType = SET_AEC_NDC_PARAM;
	tCliCfg.u4arg1 = (UINT32)(&g_rSpeech_conf);
	tCliCfg.u4arg2 = sizeof(g_rSpeech_conf);
...
static s32 _CLI_Enhance16KParam(s32 i4Argc, const s8 **szArgv)
{
    u32 u4Idx;
    s32 i4Value;
	...
	if (i4Argc < 2) {
		tCliCfg.eAudCliType = GET_AEC_NDC_16K_PARAM;
		tCliCfg.u4arg1 = (UINT32)(&g_rSpeech_16k_conf);
		tCliCfg.u4arg2 = sizeof(g_rSpeech_16k_conf);
		CLI_PCM_IOCTL();
		return 0; /* Get enhance param return */
	}
	if (i4Argc != 4) {
		pr_err(TEXT("drv.aud.pcm.enhance16: param number != 4\r\n"));
		return 0;
	}
	if ((_ttoi(szArgv[1]) != 1) && (_ttoi(szArgv[1]) != 2)) {
		pr_err(TEXT("drv.aud.pcm.enhance16: param1 must between 1 and 2\r\n"));
		return 0;
	}
	if ((_ttoi(szArgv[2]) > 27) || (_ttoi(szArgv[2]) < 0)) {
		pr_err(TEXT("drv.aud.pcm.enhance16:  param2 must between 0 and 27\r\n"));
		return 0;
	}
	u4Idx = (u32)_ttoi(szArgv[2]);
	i4Value = (s32)_ttoi(szArgv[3]);
	...
	CLI_PCM_IOCTL();
	if (1 == _ttoi(szArgv[1])) {
		g_rSpeech_16k_conf.enhance_16k_pars[u4Idx] = i4Value;
	} else {
		g_rSpeech_16k_conf.enhance_16k_pars[u4Idx] = i4Value;
	}

	tCliCfg.eAudCliType = SET_AEC_NDC_16K_PARAM;
	tCliCfg.u4arg1 = (UINT32)(&g_rSpeech_16k_conf);
	tCliCfg.u4arg2 = sizeof(g_rSpeech_16k_conf);
...
CLI_EXEC_T _arPCMCmdTbl[] = {
	{TEXT("Enable BT SCO"), TEXT("ensco"), _CLI_EnableBTSCO,
		NULL, TEXT("Enable BT SCO"), CLI_GUEST},
	{TEXT("Enable NDC Capture"), TEXT("ennc"), _CLI_EnableNdcCapture,
		NULL, TEXT("Enable NDC Capture"), CLI_GUEST},
	{TEXT("Enable DL From File"), TEXT("endl"), _CLI_EnableDLFromFile,
		NULL, TEXT("Enable DL from File"), CLI_GUEST},
	{TEXT("Enable UL From File"), TEXT("enul"), _CLI_EnableULFromFile,
		NULL, TEXT("Enable UL from File"), CLI_GUEST},
	{TEXT("Set BT Speech PLC"), TEXT("enplc"), _CLI_EnableBTSphPLC,
		NULL, TEXT("Set BT Speech PLC"), CLI_GUEST},
	{TEXT("Set BT Speech Dump"), TEXT("btdump"), _CLI_EnableBTSphDump,
		NULL, TEXT("Set BT Speech Dump"), CLI_GUEST},
	{TEXT("Set/Get BT Speech Delay"), TEXT("spdelay"), _CLI_SpeechDelay,
		NULL, TEXT("Set/Get BT Speech Delay"), CLI_GUEST},
	{TEXT("Set/Get 16K Speech Delay"), TEXT("spdelay16"),
		_CLI_Speech16KDelay,
		NULL, TEXT("Set/Get 16K Speech Delay"), CLI_GUEST},
	...
	{TEXT("Set/Get Enhance Param"), TEXT("enhance"), _CLI_EnhanceParam,
		NULL, TEXT("Set/Get Enhance Param"), CLI_GUEST},
	{TEXT("Set/Get Enhance 16K Param"), TEXT("enhance16"),
		_CLI_Enhance16KParam,
		NULL, TEXT("Set/Get Enhance 16K Param"), CLI_GUEST},
