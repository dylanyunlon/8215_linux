...
static int __init gator_module_init(void)
{
    ...
    pr_err("gator: gator_module_init Log Point AAA\n");
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0)
    ...
    pr_err("gator: gator_module_init Log Point BBB\n");

    if (gatorfs_register())
        return -1;

    pr_err("gator: gator_module_init Log Point CCC\n");
    if (gator_init()) {
        gatorfs_unregister();
        return -1;
    }

    pr_err("gator: gator_module_init Log Point DDD\n");
    ...
    pr_err("gator: gator_module_init Log Point EEE\n");
    return 0;
}
