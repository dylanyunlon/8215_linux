...
*
* Implementation of the 6630 GPS driver.
*
...
#endif
#define GPS_DBG_NONE(fmt, arg...)    do {} while (0)
#define GPS_DBG_FUNC(fmt, arg...)    pr_debug(KERN_INFO PFX "%s: " fmt, __FUNCTION__ ,##arg)
#define GPS_ERR(fmt, arg...)         pr_err(KERN_ERR PFX "%s: " fmt, __FUNCTION__ ,##arg)
#define GPS_WARN(fmt, arg...)        pr_warning(KERN_WARNING PFX "%s: " fmt, __FUNCTION__ ,##arg)
#define GPS_NOTICE(fmt, arg...)      pr_notice(KERN_NOTICE PFX "%s: " fmt, __FUNCTION__ ,##arg)
#define GPS_INFO(fmt, arg...)        pr_info(KERN_INFO PFX "%s: " fmt, __FUNCTION__ ,##arg)
#define GPS_TRC_FUNC(f)              pr_debug(PFX "<%s>\n", __FUNCTION__);
#define GPS_TRC_VERBOSE(fmt, arg...) pr_debug(PFX fmt, ##arg)

#define PFX "[GPS]"
#define GPS_DBG GPS_DBG_FUNC
#define GPS_TRC GPS_TRC_FUNC
...
static inline int mt3326_gps_set_suspend(struct gps_drv_obj *obj, unsigned char suspend)
{
    ...
    if (obj->suspend != suspend) {
        ...
        if(GPS_PWR_SUSPEND == (unsigned int)suspend) {
            //pr_info("[GPS], notify suspend\n");
            sysfs_notify(obj->kobj, NULL, "suspend");
        }
        else if(GPS_PWR_RESUME == (unsigned int)suspend) {
            //pr_info("[GPS], notify resume\n");
            sysfs_notify(obj->kobj, NULL, "resume");
        }
    }
    obj->suspend = suspend;
...
static int __init mt3326_gps_mod_init(void)
{
    int ret = 0;
    GPS_TRC();
    GPS_DBG("[6630][Kernel][new mt3326_gps_mod_init]\n");
    ...
    pr_info("[GPS] new mt3326_gps_mod_init, ret:[%d]..\n", ret);
...
static void __exit mt3326_gps_mod_exit(void)
{
    ...
    platform_device_unregister(&mt3326_device_gps);
}
