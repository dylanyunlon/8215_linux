...
struct msdc_hw {
    ...
    u32            flags;            /* hardware capability flags */
    unsigned long  data_pins;        /* data pins */
    unsigned long  data_offset;      /* data address offset */
	u8             tuning_method;    /* tuning method select */
	...
	unsigned int cmdrtactr_sdr50; /* command response turn around counter, sdr 50 mode*/
	unsigned int wdatcrctactr_sdr50; /* write data crc turn around counter, sdr 50 mode*/
	unsigned int intdatlatcksel_sdr50; /* internal data latch CK select, sdr 50 mode*/
	unsigned int cmdrtactr_sdr104; /* command response turn around counter, sdr 104 mode*/
	unsigned int wdatcrctactr_sdr104; /* write data crc turn around counter, sdr 104 mode*/
	unsigned int intdatlatcksel_sdr104; /* internal data latch CK select, sdr 104 mode*/
	
	u8             host_function;	 /* define host function*/
	bool		   boot;			 /* define boot host*/ 
	u8			   cd_level;		 /* card detection level*/	
	u8             cd_method;		/* card detect method: 0- Interrupt mode, 1- GPIO polling mode, 2- Interrupt with GPIO together*/
