...
typedef union _HDMI_RX_Audio_InfoFrame {
    struct {
        ...
        u8 Ver;
        u8 Len;
        u8 AudioChannelCount:3;//HDMI_RX_AUDIO_CHANNEL_COUNT_T
        u8 RSVD1:1;
        u8 AudioCodingType:4; //see HDMI_RX_AUDIO_CODING_TYPE_T
        u8 SampleSize:2;//HDMI_RX_SAMPLE_SIZE_TYPE_T
        u8 SampleFreq:3;//HDMI_RX_SAMPLE_FREQ_TYPE_T
        u8 Rsvd2:3;
        u8 FmtCoding;//no use
        u8 SpeakerPlacement;//HDMI_RX_SPEAKER_ALLOCATE_T
        u8 Rsvd3:3;
        u8 LevelShiftValue:4;//HDMI_RX_AUD_LEVEL_TYPE_T
        u8 DM_INH:1;// 1:Down mix Prohibites, 0:Permitted down mix
    } info;
    struct {
        u8 AUD_HB[3];
        u8 AUD_DB[10];//Original raw audio inforframe data (for other module it sould not refer to it)
    } pktbyte;
...
typedef   struct  _HDMI_RX_IN_AUDIO_INFO_T
{
  ...
  u8 u1HBRAudio; //  1: Means HBR audio in, 0:SD audio in
  u8 u1DSDAudio;////  1: Means DSD audio in, 0:Non DSD audio in
  u8 u1RawSDAudio; // 1: AC3, DTS SD raw bit-stream in, 0:PCM or HBR audio in
  u8 u1PCMMultiCh; // 1: PCM Multi channel in ,  0: PCM 2 channel in
                      // PS: When u1RawSDAudio is 0 and u1HBRAudio is 0, this parameter  
                      //will be valid, otherwise don\A1\A6t care it
  u8 u1FsDec; // by IEC937 and IEC 958 Spec,
                //0x00:44.1khz, 0x02:48khz, 0x03:32khz, 0x08:88.2khz
                //0x0A:96khz, 0x0c:176khz, 0x0E:192khz, 0x09:768khz(HBR)
  u8 u1I2sChanValid; //bit0: I2S Channel 0  Valid
                        //bit1: I2S Channel 1  Valid  
                        //bit2: I2S Channel 2  Valid  
                        //bit3: I2S Channel 3  Valid  
                        
  u8 u1I2sCh0Sel; // I2S channel 0 mapping: see HDMI_I2S_IN_CH_MAP_T
  u8 u1I2sCh1Sel; // I2S channel 1 mapping: see HDMI_I2S_IN_CH_MAP_T
  u8 u1I2sCh2Sel; // I2S channel 2 mapping: see HDMI_I2S_IN_CH_MAP_T
  u8 u1I2sCh3Sel; // I2S channel 3 mapping: see HDMI_I2S_IN_CH_MAP_T
