...
static phys_addr_t g_ram_addr = 0;
static phys_addr_t g_ram_size = 0;
/*
...
static int ram_console_check_header(struct ram_console_buffer *buffer)
{
	...
	if (!buffer || (buffer->sz_buffer != ram_console_buffer->sz_buffer) || buffer->off_pl > buffer->sz_buffer
		...
		pr_err("ram_console: illegal header.");
		/*
		for (i = 0; i < 16; i++)
...
static int __init ram_console_early_init(void)
{
	...
	if (of_scan_flat_dt(dt_get_ram_console, &sram)) {
		if (sram.start == 0) {
			sram.start = g_ram_addr;
			sram.size = g_ram_size;
		}
...
#else
	bufp = (struct ram_console_buffer *)g_ram_addr;
	buffer_size = g_ram_size;
#endif

#elif defined(CONFIG_MTK_RAM_CONSOLE_USING_DRAM)
#ifdef CONFIG_ARCH_AC83XX
	bufp = remap_lowmem(g_ram_addr, g_ram_size);
#else
	bufp = ioremap(g_ram_addr, g_ram_size);
#endif /* CONFIG_ARCH_AC83XX */
	ram_console_buffer_pa = (struct ram_console_buffer *)g_ram_addr;
	if (bufp == NULL) {
...
int ram_console_binary_reserve_memory(struct reserved_mem *rmem, unsigned long node, const char *uname)
{
	pr_alert("[memblock]%s: 0x%llx - 0x%llx (0x%llx)\n", uname, (unsigned long long)rmem->base,
		 (unsigned long long)rmem->base + (unsigned long long)rmem->size, (unsigned long long)rmem->size);
	return 0;
}
RESERVEDMEM_OF_DECLARE(reserve_memory_ram_console, "ram_console-reserve-memory", ram_console_binary_reserve_memory);

int ram_console_reserve_memory(struct reserved_mem *rmem, unsigned long node, const char *uname)
{
	pr_alert("[memblock]%s: 0x%llx - 0x%llx (0x%llx)\n", uname, (unsigned long long)rmem->base,
		 (unsigned long long)rmem->base + (unsigned long long)rmem->size, (unsigned long long)rmem->size);
	g_ram_addr = rmem->base;
	g_ram_size = rmem->size;
	return 0;
}
RESERVEDMEM_OF_DECLARE(reserve_mem_ram_console, "atc-console_rsv", ram_console_reserve_memory);
...
void aee_rr_show_last_pc(struct seq_file *m)
{
	...
	/*
	 * MemLeak Checked by Lyon.
	 * 03/06/2017 JoshuaWang
	 */
#if 0
...
phys_addr_t ram_console_base_get(void)
{
	return g_ram_addr;
}
EXPORT_SYMBOL(ram_console_base_get);

phys_addr_t ram_console_size_get(void)
{
	return g_ram_size;
}
EXPORT_SYMBOL(ram_console_size_get);
