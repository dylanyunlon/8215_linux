...
void msdc_dump_register_debug(struct seq_file *m, u32 id, void __iomem *base)
{
	...
	pr_register(m, OFFSET_SDC_CMD_STS,              SDC_CMD_STS);
...
static void msdc_set_field(struct seq_file *m,void __iomem *address, unsigned int start_bit,
	unsigned int len, unsigned int value)
{
	...
	if (start_bit > 31 || start_bit < 0 || len > 32 || len <= 0) {
		...
		seq_printf(m,"[SD_Debug]invalid reg field range or length\n");
	} else {
...
static void msdc_get_field(struct seq_file *m,void __iomem *address, unsigned int start_bit,
	unsigned int len, unsigned int value)
{
	unsigned long field;
...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	case SD_TOOL_REG_ACCESS:
		...
		if (id >= HOST_MAX_NUM || id < 0 || atc_msdc_host[id] == NULL)
			goto invalid_host_id;
	...
	case SD_TOOL_SET_DRIVING:
		...
		host = atc_msdc_host[id];
		if (p1 == 1) {
			if ((unsigned char)p3 > 0x3F || (unsigned char)p4 > 0x3F ||
					(unsigned char)p5 > 0x3F || (unsigned char)p6 > 0x3F ||
					(unsigned char)p7 > 0x3F) {
				seq_puts(m, "[SD_Debug]Some drving value was invalid(invalid:0~0x3F)\n");
				break;
			}
			if (id == 0) {
				...
				msdc_init_pad(host);
				seq_printf(m, "[SD_Debug] SD0 set driving: clk_drv=%d, cmd_drv=%d, dat_drv=%d\n",
				host->hw->clk_drv, host->hw->cmd_drv,
				host->hw->dat_drv);
				...
				host->hw->dat_drv = (unsigned char)p5;
				msdc_init_pad(host);
				seq_printf(m, "[SD_Debug] SD1 set driving: clk_drv=%d, cmd_drv=%d, dat_drv=%d\n",
				host->hw->clk_drv, host->hw->cmd_drv,
				host->hw->dat_drv);
				...
				host->hw->dat_drv = (unsigned char)p5;
				msdc_init_pad(host);
				seq_printf(m, "[SD_Debug] SDIO set driving: clk_drv=%d, cmd_drv=%d, dat_drv=%d\n",
				host->hw->clk_drv, host->hw->cmd_drv,
				host->hw->dat_drv);
...
static int msdc_debug_proc_read_FT_show(struct seq_file *m, void *data)
{
	...
	case 1:
		if (atc_msdc_host[1]) {
			base = atc_msdc_host[1]->base;
			msdc_id = 1;
		}
		break;
	case 2:
		if (atc_msdc_host[2]) {
			base = atc_msdc_host[2]->base;
			msdc_id = 2;
		}
		break;
	case 3:
		if (atc_msdc_host[3]) {
			base = atc_msdc_host[3]->base;
			msdc_id = 3;
		}
		break;
	default:
...
#if defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT)
	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 0 && atc_msdc_host[0])
		base = atc_msdc_host[0]->base;
	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 1 && atc_msdc_host[1])
		base = atc_msdc_host[1]->base;
	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 2 && atc_msdc_host[2])
		base = atc_msdc_host[2]->base;
	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 3 && atc_msdc_host[3])
		base = atc_msdc_host[3]->base;
#else
	return -1;
#endif
...
extern int card_dump_func_write(unsigned char *buf, unsigned int len,
	unsigned long long offset, int dev);
extern int msdc_detect_change(unsigned int slot, u32 enable, u32 type);

#ifdef CONFIG_ATC_PRJ_ac823x_adas
extern void msdc_change_clksrc(struct msdc_host *host,unsigned char hs200_mode,unsigned char clksrc);

#endif
extern void msdc_change_sd_clksrc(struct msdc_host *host,unsigned char clksrc);
#if MSDC_DEBUG_ENABLE
int msdc_sw_reset_debug=0;
#endif
	...
	size_t count, loff_t *data)
{
	...
	int ret;
	int cmd, p1, p2;
	char buff[512]="123sdf456789dandan liu i love you end \r\n";
	u64 base;
	u32 status = 0;
	...
	if (3  == sscanf(cmd_buf, "%x %x %x", &cmd, &p1, &p2)) {
		...
		case 0:
			...
			MSDC_SET_FIELD(pd_reset_en_cfg3, MSDC_3_SWRST, 1);
				MSDC_SET_FIELD(PAD_EN0, MT6630_PMU_EN_CFG, 1);
			MSDC_SET_FIELD(PAD_EN1, MT6630_SYS_RST_CFG, 1);

			MSDC_SET_FIELD(GPIO_OUT0, MT6630_PMU_EN_CFG, 1);
			mdelay(1000);
			MSDC_SET_FIELD(GPIO_OUT1, MT6630_SYS_RST_CFG, 0);
			mdelay(1000);
			MSDC_SET_FIELD(GPIO_OUT1, MT6630_SYS_RST_CFG, 1);
			msdc_detect_change(3,p1,0);
#endif
			break;
		...
		case 8:
			...
			break;
		case 9:
			//card_dump_func_write(buff, 512*10,0,0);
			simulate_insert_enable=p1;
			printk("simulate insert %s \n",((p1 == 0) ? "disable" : "enable"));
			break;
		case 0x10:
			#ifdef SIMULATE_CARD_INSERT
			...
			if((atc_msdc_host[p1]->hw->cd_method & MSDC_CD_METHOD_INTERRUPT)||
				(atc_msdc_host[p1]->hw->cd_method & MSDC_CD_METHOD_INT_POL))
				tasklet_hi_schedule(&atc_msdc_host[p1]->card_tasklet);
			#endif
			break;
		#ifdef CONFIG_ATC_PRJ_ac823x_adas
		case 0x11:
			msdc_change_clksrc(atc_msdc_host[0],p1,p2);
			break;
		#endif
		case 0x12:/*change the clock source*/
			msdc_change_sd_clksrc(atc_msdc_host[p1],p2);
			break;
		case 0x13:/*get device status*/
			mmc_claim_host(atc_msdc_host[p1]->mmc);
			msdc_get_card_status(atc_msdc_host[p1]->mmc, atc_msdc_host[p1], &status);
			mmc_release_host(atc_msdc_host[p1]->mmc);
			msdc_dump_card_status(atc_msdc_host[p1],status);
			break;
		case 0x14:/*send stop cmd*/
			mmc_claim_host(atc_msdc_host[p1]->mmc);
			msdc_send_stop(atc_msdc_host[p1]);
			mmc_release_host(atc_msdc_host[p1]->mmc);
			break;
	#if MSDC_DEBUG_ENABLE
		case 0x15:
			/*enable/disable sw reset function for debug*/
			msdc_sw_reset_debug=p1;
			break;
	#endif
		}
	}
...
int msdc_debug_proc_init(void)
{
	...
	if (prEntry) {
		pr_info("[%s]: create /proc/msdc_debug\n", __func__);
	...
	if (prEntry)
		pr_info("[%s]: create /proc/msdc_help\n", __func__);
	else
	...
	if (prEntry)
		pr_info("[%s]: create /proc/msdc_DVT\n", __func__);
	else
	...
	if (tune) {
		...
		pr_info("[%s]: create /proc/msdc_tune\n", __func__);
	} else {
	...
	if (tune_flag)
		pr_info("[%s]: create /proc/msdc_tune_flag\n", __func__);
	else
	...
	if (voltage_flag) {
		...
		pr_info("[%s]: create /proc/msdc_voltage_flag\n",
			__func__);
	} else {
