...
u32 msdc_v33_18_pin[]={0,MSDC_V33_18_SW0,MSDC_V33_18_SW1,MSDC_V33_18_SW2};
#if 1
#include "../../../../gpio/gpio_ac823x_pinmux.h"
#define PINMUX_LEVEL_GPIO_END_FLAG 0xFF
#define PIN_17_BIT    (1<<17)
#define PIN_18_BIT    (1<<18)

void card_detect_gpio_config(struct msdc_host *host)
{
	int ret;	
	int pullud,driving;
	/*Config the gpio to pull up*/
	if(host->id==1){
		GPIO_823X_WRITE32(0x400,GPIO_823X_READ32(0x400)|PIN_17_BIT);
	}
	else if(host->id==2){
		GPIO_823X_WRITE32(0x400,GPIO_823X_READ32(0x400)|PIN_18_BIT);
	}
	else{
		return ;
	}
	/*config the pinmux*/
	ret=atc_set_gpio_pinmux(host->hw->cd_gpio, PINMUX_LEVEL_GPIO_END_FLAG);	
	if(ret)
		MSDC_LOG(ERR,"set gpio pinmux fail ");
	/*config the direction to input*/
	ret=atc_set_gpio_dir(host->hw->cd_gpio, INPUT);
	if(ret)
		MSDC_LOG(ERR,"set gpio dir fail ");
}
...
void msdc_gpio_config_pinmux(struct msdc_host *host)
{
	...
	/*config the pinmux*/
	if(host->id==1)
		ret=atc_set_gpio_pinmux(host->hw->cd_gpio, 168);	
	else if(host->id==2)
		ret=atc_set_gpio_pinmux(host->hw->cd_gpio, 172);
	if(ret)
		MSDC_LOG(ERR,"set gpio pinmux fail ");
	/*config the direction to input*/
	ret=atc_set_gpio_dir(host->hw->cd_gpio, INPUT);
...
{
	int ret;
	u32 gpio_pin=msdc_v33_18_pin[host->id];
	 MSDC_LOG_NORMAL(pr_debug,"%d start to do sd power switch status(%d)",host->id,on);
	/*config the pinmux*/
	ret=atc_set_gpio_pinmux(gpio_pin, PINMUX_LEVEL_GPIO_END_FLAG);	
	if(ret)
		MSDC_LOG(ERR,"set gpio pinmux fail ");
	/*config the direction to input*/
	ret=atc_set_gpio_dir(gpio_pin, OUTPUT);
	if(ret)
		MSDC_LOG(ERR,"set gpio dir fail ");
	if(on)
...
void msdc_sdio_power_switch(struct msdc_host *host, u32 on)
{
	...
	u32 gpio_pin=msdc_v33_18_pin[host->id];
	 MSDC_LOG_NORMAL(pr_debug,"%d start to do sdio power switch status(%d)",host->id,on);
	/*config the pinmux*/
	ret=atc_set_gpio_pinmux(gpio_pin, PINMUX_LEVEL_GPIO_END_FLAG);	
	if(ret)
		MSDC_LOG(ERR,"set gpio pinmux fail ");
	...
	ret=atc_set_gpio_dir(gpio_pin, OUTPUT);
	if(ret)
		MSDC_LOG(ERR,"host(%d)set gpio dir fail ");
	if(on)
		__gpio_set_value(gpio_pin,1);
	else
		__gpio_set_value(gpio_pin,0);
...
static int msdc_get_pinctl_settings(struct msdc_host *host)
{
	...
	of_property_read_u8(pins_cmd_node, "drive-strength",
			&host->hw->cmd_drv);
	of_property_read_u8(pins_cmd_node, "slew_rate",
			&host->hw->cmd_sr);
	of_property_read_u8(pins_cmd_node, "resistor",
			&host->hw->cmd_res);
	
	pins_dat_node = of_get_child_by_name(pinctl_node, "pins_dat");
	of_property_read_u8(pins_dat_node, "drive-strength",
			&host->hw->dat_drv);
	of_property_read_u8(pins_dat_node, "slew_rate",
			&host->hw->dat_sr);
	of_property_read_u8(pins_dat_node, "resistor",
			&host->hw->dat_res);

	pins_clk_node = of_get_child_by_name(pinctl_node, "pins_clk");
	of_property_read_u8(pins_clk_node, "drive-strength",
			&host->hw->clk_drv);
	of_property_read_u8(pins_clk_node, "slew_rate",
			&host->hw->clk_sr);
	of_property_read_u8(pins_clk_node, "resistor",
			&host->hw->clk_res);
	...
	of_property_read_u8(pins_ds_node, "slew_rate",
			&host->hw->ds_sr);
	of_property_read_u8(pins_ds_node, "resistor",
			&host->hw->ds_res);
#if 0
	/* SDR104 for SDcard */
...
static int msdc_get_rigister_settings(struct msdc_host *host)
{
	...
	if (register_setting_node) {
		...
		MSDC_LOG(ERR,"register_setting is not found in DT.\n");
		return 1;
	}
	return 0;
}
...
int msdc_of_parse(struct mmc_host *mmc)
{
	...
	else if (0 == strcmp(np->name, "MSDC3")) {
		host->id = 3;
		mmc->index = 3;
	}
	else {
		MSDC_LOG(ERR,"parse DT for host index error. np-name: %s", np->name);
		return 1;
	}
...
#else
	of_property_read_u32_index(np, "interrupts", 1, &host->irq);
	//printk("the host(%d) irq is %d \n ",host->id,host->irq);

#endif
	/* get clk_src */
	if (of_property_read_u8(np, "clk_src", &host->hw->clk_src)) {
		MSDC_LOG(ERR,"error: clk_src isn't found in device tree.");
		/*need to use default clk_src */
	}
	
	/* get cd_method */
	if (of_property_read_u8(np, "cd_method", &host->hw->cd_method)) {
		MSDC_LOG(ERR,"cd_method isn't found in DT");
	}

	of_property_read_u32(np, "clk_max", &host->hw->clk_max);

	of_property_read_u32(np, "clk_tx", &host->clk_tx);
	...
	* property data isn't large enough.*/
	...
	if (of_property_read_u8(np, "host_function", &host->hw->host_function))
		MSDC_LOG(ERR,"host_function isn't found in device tree");
	...
	if (of_property_read_u8(np, "cd_level", &host->hw->cd_level))
		MSDC_LOG(ERR,"cd_level isn't found in device tree");
#if 1
	/*get cd_gpio*/
	if(of_property_read_u32(np, "cd_gpio", &host->hw->cd_gpio))
		MSDC_LOG(ERR,"cd_gpio isn't found in device tree");

#else
	of_property_read_u32_index(np, "cd-gpios", 1, &host->hw->cd_gpio);
	pr_err("[MSDC%d] cd-gpio = %d\n", host->id, host->hw->cd_gpio);
...
u32 hclks_msdc50_0_hclk[] = {PLLCLK_27M, PLLCLK_98M, PLLCLK_120M, PLLCLK_135M,
					PLLCLK_162M, PLLCLK_270M, PLLCLK_200M, PLLCLK_147M};
/* msdc0 clock source reference value is 400M */
u32 hclks_msdc50[] = {PLLCLK_27M, PLLCLK_400M, PLLCLK_200M, PLLCLK_162M,
			PLLCLK_120M, PLLCLK_108M, PLLCLK_54M, PLLCLK_48M,PLLCLK_189M,
			PLLCLK_147M, PLLCLK_324M, PLLCLK_135M, PLLCLK_294M,PLLCLK_100M,
			PLLCLK_13_5M,PLLCLK_6_65M};
/* msdc1 clock source reference value is 200M */
u32 hclks_msdc30[] = {PLLCLK_27M,PLLCLK_200M,PLLCLK_202M, PLLCLK_162M,
			PLLCLK_120M, PLLCLK_108M, PLLCLK_54M, PLLCLK_48M,PLLCLK_189M,
			PLLCLK_147M, PLLCLK_74M, PLLCLK_135M, PLLCLK_81M,PLLCLK_100M,
			PLLCLK_13_5M,PLLCLK_6_65M};
	...
	struct msdc_host *host)
{
	...
	if (IS_ERR(host->clock_control)) {
		MSDC_LOG(ERR,"can not get clock control");
		return 1;
	}
	if (clk_prepare(host->clock_control)) {
		MSDC_LOG(ERR,"can not prepare clock control");
		return 1;
	}
	return 0;
}
...
void msdc_select_clksrc(struct msdc_host *host, int clksrc)
{
	...
	hclks = msdc_get_hclks(host->id);
	 ...
	 MSDC_LOG_NORMAL(pr_info,"[%s]: msdc%d change clk_src from %dKHz to %d:%dKHz", __func__,
...
void msdc_dump_clock_sts(struct msdc_host *host)
{
	...
	if (topckgen_reg_base) {
		/* CLK_CFG_3 control msdc clock source PLL */
		 MSDC_LOG_NORMAL(pr_info," CLK_CFG_3 register address is 0x%p",
			topckgen_reg_base + MSDC_CLK_CFG_3_OFFSET);
		 MSDC_LOG_NORMAL(pr_info," bit[9~8]=01b,     bit[15]=0b");
		 MSDC_LOG_NORMAL(pr_info," bit[19~16]=0001b, bit[23]=0b");
		 MSDC_LOG_NORMAL(pr_info," bit[26~24]=0010b, bit[31]=0b");
		 MSDC_LOG_NORMAL(pr_info," Read value is       0x%x\n",
			MSDC_READ32(topckgen_reg_base + MSDC_CLK_CFG_3_OFFSET));
	}
	if (apmixed_reg_base) {
		/* bit0 is enables PLL, 0: disable 1: enable */
		 MSDC_LOG_NORMAL(pr_info," MSDCPLL_CON0_OFFSET register address is 0x%p",
			apmixed_reg_base + MSDCPLL_CON0_OFFSET);
		 MSDC_LOG_NORMAL(pr_info," bit[0]=1b");
		 MSDC_LOG_NORMAL(pr_info," Read value is       0x%x",
			MSDC_READ32(apmixed_reg_base + MSDCPLL_CON0_OFFSET));

		 MSDC_LOG_NORMAL(pr_info," MSDCPLL_CON1_OFFSET register address is 0x%p",
			apmixed_reg_base + MSDCPLL_CON1_OFFSET);
		 MSDC_LOG_NORMAL(pr_info," Read value is       0x%x",
			MSDC_READ32(apmixed_reg_base + MSDCPLL_CON1_OFFSET));

		 MSDC_LOG_NORMAL(pr_info," MSDCPLL_CON2_OFFSET register address is 0x%p",
			apmixed_reg_base + MSDCPLL_CON2_OFFSET);
		 MSDC_LOG_NORMAL(pr_info," Read value is       0x%x",
			MSDC_READ32(apmixed_reg_base + MSDCPLL_CON2_OFFSET));

		 MSDC_LOG_NORMAL(pr_info," MSDCPLL_PWR_CON0 register address is 0x%p",
