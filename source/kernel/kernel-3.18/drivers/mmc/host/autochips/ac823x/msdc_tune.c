...
extern int mmc_reinit(struct mmc_host *host, u32 ocr,struct mmc_card *oldcard);


static u32 emmc_switch_hs200(struct msdc_host *host)
{
    ...
    card = mmc->card;
    if (card == NULL) {
        pr_err("mmc->card is NULL");
        return 1;        
    }
    
   	pr_info("Power cycle start");
	...
	host_err = host->error;
	dma_xfer=host->dma_xfer;
	tune_flag=host->tuning_in_progress;
	
    spin_unlock(&host->lock);
    ret = mmc_reinit(mmc, mmc->card->ocr, card);
    spin_lock(&host->lock);
	host->mrq = mrq;
	host->dma_xfer=dma_xfer;	
    host->sw_timeout = sw_timeout;
	host->error = host_err;
	host->tuning_in_progress=tune_flag;

	pr_info("Power cycle ret(0x%x)", ret);  
    return ret;
}

extern int sd_reinit(struct mmc_host *host, u32 ocr,struct mmc_card *oldcard);
extern void msdc_init_hw(struct msdc_host *host);

static u32 sd_switch_high_speed(struct msdc_host *host)
{
    struct mmc_host *mmc = host->mmc;
    struct mmc_card *card;
	struct mmc_request *mrq;
	pr_info("msdc%d reinit card to high speed mode \n",host->id);
...
int sdcard_reset_tuning(struct mmc_host *mmc)
{
	...
	if (mmc->card->sw_caps.sd3_bus_mode & SD_MODE_UHS_SDR104) {
		...
		return sd_switch_high_speed(host);
	}
	...
	msdc_clk_stable(host, mode, div, 1);
	host->sclk = (div == 0) ? host->hclk / 4 : host->hclk / (4 * div);
	pr_info("msdc%d: HS mode lower frequence to %d Mhz\n",
		host->id, host->sclk / 1000000);
	return 0;

}

/* SDIO will change speed mode and power reset
 * UHS_SDR104 --> UHS_DDR50 --> UHS_SDR50 --> UHS_SDR25
 *now just lower freq
 */
int sdio_reset_tuning(struct mmc_host *mmc)
{
	struct msdc_host *host = mmc_priv(mmc);
	int ret = 0;
	u32 div = 0;
	...
	msdc_set_mclk(host,mmc->ios.timing,host->sclk/2);
...
void msdc_change_clksrc(struct msdc_host *host,unsigned char hs200_mode,unsigned char clksrc)
{
	struct mmc_host *mmc = host->mmc;
    struct mmc_card *card;
	struct mmc_request *mrq;
	...
	mmc_claim_host(mmc);
	MSDC_LOG_NORMAL(pr_info,"%d change clksrc from %dMhz to %dMhz",host->id,
		hclks[host->hw->clk_src]/1000000,hclks[clksrc]/1000000);
	...
	if((hs200_mode)&&(mmc->card->mmc_avail_type & EXT_CSD_CARD_TYPE_HS400)) 
		mmc->card->mmc_avail_type &= ~EXT_CSD_CARD_TYPE_HS400;
    // clock should set to 260K 
    mmc->ios.clock = HOST_MIN_MCLK;  
    mmc->ios.bus_width = MMC_BUS_WIDTH_1;
...
#endif
void msdc_change_sd_clksrc(struct msdc_host *host,unsigned char clksrc)
{
	u32 *hclks = msdc_get_hclks(host->id);
	MSDC_LOG_NORMAL(pr_info,"%d change clksrc from %dMhz to %dMhz",host->id,
		hclks[host->hw->clk_src]/1000000,hclks[clksrc]/1000000);
