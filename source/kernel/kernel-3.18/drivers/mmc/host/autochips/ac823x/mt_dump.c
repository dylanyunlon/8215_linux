...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
	...
	if (host->id == 0 && atc_msdc_host[0])
		msdc_set_driving_by_id(host->id, atc_msdc_host[0]->hw, 0);
	else  if (host->id == 1 && atc_msdc_host[1]->hw)
		msdc_set_driving_by_id(host->id, atc_msdc_host[1]->hw, 0);
...
#ifdef MTK_MSDC_USE_CACHE
	struct msdc_host *host2 = atc_msdc_host[host->index];
...
static int simp_mmc_init(int card_type, bool boot)
{
	...
	cur_msdc_host->id = id;
	if (NULL == atc_msdc_host[id])
		goto out;

#ifdef CONFIG_OF
	cur_msdc_host->base = (atc_msdc_host[id])->base;
	...
	unsigned int offset, unsigned int dev)
{
	...
	mmc_claim_host(atc_msdc_host[BOOT_STORAGE_ID]->mmc);
	if (emmc_init == 0) {
...
/*	do {
	...
	if (ret == 0)
		ret = SIMP_SUCCESS;
	mmc_release_host(atc_msdc_host[BOOT_STORAGE_ID]->mmc);
	return ret;

}
#endif /* end of CONFIG_MMC_ATC */
...
static int get_emmc_dump_info(void)
{
	struct mmc_part_info *part_info=NULL;	

	/* find the expdb partition in emmc */	
	if(!(part_info=mmc_find_part_by_name("expdb"))) {		
		pr_err("get partition expdb info fail \n");		
		return 1;	
	}
	
	partition_ready_flag = 1;	
	lp_start_sect=part_info->start_sect;	
	lp_nr_sects=part_info->nr_sects;	
	MTK_DUMP_PR_DBG("expdb partition start 0x%08x, size 0x%08x \n",lp_start_sect<<9,lp_nr_sects<<9);
	return 0;
}
...
static int  get_emmc_dump_info(void)
{
	struct hd_struct *hd_info=NULL;	

	/* find the expdb partition in emmc */	
	if(!(hd_info=mmc_find_part_by_name("expdb"))) {		
		pr_err("get partition expdb info fail \n");		
		return 1;	
	}
...
#ifdef CONFIG_MMC_ATC
		if((!partition_ready_flag)&&(get_emmc_dump_info()))
			return 1;
