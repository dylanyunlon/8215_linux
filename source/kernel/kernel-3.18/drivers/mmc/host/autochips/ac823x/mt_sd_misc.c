...
static int simple_sd_ioctl_sd30_mode_switch(struct msdc_ioctl *msdc_ctl)
{
	...
	if (atc_msdc_host[msdc_ctl->host_num] == NULL) {
		pr_err("host%d is not config\n", msdc_ctl->host_num);
		return -EINVAL;
	}
...
static int __init simple_sd_init(void)
{
	...
	pr_debug(DRV_NAME_MISC ": AutoChips simple SD/MMC Card Driver\n");
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("simple AutoChips SD/MMC Card Driver");
MODULE_AUTHOR("dandan.liu <dandan.liu@autochips.com>");
