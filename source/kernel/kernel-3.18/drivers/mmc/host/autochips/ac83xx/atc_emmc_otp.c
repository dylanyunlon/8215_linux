...
{
	return atc_msdc_host[EMMC_HOST_NUM];
}
...
unsigned int emmc_get_wp_size(void)
{
	...
	if (0 == sg_wp_size) {
		...
		MSDC_LOG_NORMAL(pr_info, "%s:%d mmc_host is : 0x%p\n", __func__, __LINE__, host_ctl->mmc);
		/* pr_info("[%s:%d]claim host done!", __func__, __LINE__);
		pr_info("claim_status = %d, claim_cnt = %d, claimer = 0x%x, current = 0x%x\n",
...
unsigned int emmc_otp_start(void)
{
	...
	l_offset = 0;		/* msdc_get_offset(); */
	l_reserve = 0;		/* msdc_get_reserve(); */
	...
	MSDC_LOG_NORMAL(pr_info, "find OTP start address is 0x%x\n", l_addr);
...
#if EMMC_OTP_DEBUG
	MSDC_LOG_NORMAL(pr_info, "EMMC_OTP: start MSDC_SINGLE_READ_WRITE!\n");
#endif
...
{
	MSDC_LOG_NORMAL(pr_info, "%s:(MAJOR)%d:(MINOR)%d\n", __func__, MAJOR(inode->i_rdev), MINOR(inode->i_rdev));
			 ...
			 unsigned int length, unsigned int *status)
{
	...
	MSDC_LOG_NORMAL(pr_info, "%s: %s (0x%x) length:(%d bytes) !\n", __func__, access_type ? "WRITE" : "READ",
		offset, length);
	...
	do {
		...
		if (FS_EMMC_OTP_READ == access_type) {
			...
			MSDC_LOG_NORMAL(pr_info, "%s: Read Access of page (%d)\n", __func__, blkno);

			Status = g_emmc_otp_func.read(blkno, p_D_Buff);
			*status = Status;

			if (OTP_SUCCESS != Status) {
				MSDC_LOG_NORMAL(pr_info, "%s: Read status (%d)\n", __func__, Status);
				break;
			}
			...
			*status = Status;
			...
			if (OTP_SUCCESS != Status) {
				MSDC_LOG_NORMAL(pr_info, "%s: read before write, Read status (%d) blkno (0x%x)\n",
			...
			*status = Status;
			...
			if (OTP_SUCCESS != Status) {
				MSDC_LOG_NORMAL(pr_info, "%s: Write status (%d)\n", __func__, Status);
				break;
			}
		} else {
			MSDC_LOG_NORMAL(pr_info, "%s: Error, not either read nor write operations !\n", __func__);
			break;
		}
		...
		length -= AccessLength;
		MSDC_LOG_NORMAL(pr_info, "%s: Remaining %s (%d) !\n", __func__, access_type ? "WRITE" : "READ", length);
...
static long mt_otp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case EMMC_OTP_GET_LENGTH:
		MSDC_LOG_NORMAL(pr_info, "OTP IOCTL: EMMC_OTP_GET_LENGTH\n");

		if (('i' == (otpctl.status & 0xf)) && ('c' == ((otpctl.status >> 8) & 0xf))
		    && ('c' == ((otpctl.status >> 16) & 0xf))
		    && ('c' == ((otpctl.status >> 24) & 0xf))) {
		...
		MSDC_LOG_NORMAL(pr_info, "OTP IOCTL: The Length is %d\n", otpctl.QLength);
		break;

	case EMMC_OTP_READ:
		MSDC_LOG_NORMAL(pr_info, "OTP IOCTL: EMMC_OTP_READ Offset(0x%x), Length(0x%x)\n", otpctl.Offset,
			otpctl.Length);
		...
		if (copy_to_user(otpctl.BufferPtr, pbuf, (sizeof(char) * otpctl.Length))) {
			MSDC_LOG_NORMAL(pr_info, "EMMC_OTP IOCTL: Copy to user buffer Error !\n");
			goto error;
		}
	...
	case EMMC_OTP_WRITE:
		MSDC_LOG_NORMAL(pr_info, "OTP IOCTL: EMMC_OTP_WRITE Offset(0x%x), Length(0x%x)\n", otpctl.Offset,
			otpctl.Length);

		if (copy_from_user(pbuf, otpctl.BufferPtr, (sizeof(char) * otpctl.Length))) {
			MSDC_LOG_NORMAL(pr_info, "EMMC_OTP IOCTL: Copy from user buffer Error !\n");
			goto error;
		}
...
static const struct miscdevice emmc_otp_dev = {
...
static int emmc_otp_probe(struct platform_device *pdev)
{
	...
	MSDC_LOG_NORMAL(pr_info, "in emmc otp probe function\n");
...
static int __init emmc_otp_init(void)
{
	...
	MSDC_LOG_NORMAL(pr_info, "Autochips EMMC OTP misc driver init\n");
	...
	if (entry == NULL) {
		MSDC_LOG_NORMAL(pr_info, "emmc OTP: unable to create /proc entry\n");
		return -ENOMEM;
	}
	...
	MSDC_LOG_NORMAL(pr_info, "OTP: register EMMC OTP device ...\n");
	err = misc_register(&emmc_otp_dev);

	if (unlikely(err)) {
		MSDC_LOG_NORMAL(pr_info, "OTP: failed to register EMMC OTP device!\n");
		return err;
	}
...
{
	MSDC_LOG_NORMAL(pr_info, "Autochips EMMC OTP misc driver exit\n");
