#include <linux/version.h>
#include <linux/kernel.h>
#include <linux/sched.h>
#include <linux/kthread.h>
#include <linux/delay.h>
#include <linux/module.h>
#include <linux/init.h>
#include <linux/proc_fs.h>
#include <linux/string.h>
#include <linux/uaccess.h>
#include <linux/mmc/host.h>

#include <mach/board.h>

#include <asm/io.h>
#include <linux/mmc/mmc.h>
#include <linux/mmc/card.h>
#include <linux/scatterlist.h>

#include "atc_msdc_dbg.h"
#include <linux/seq_file.h>
#include <linux/device.h>


/*	sd card simulate insert test interface */
#define SIMULATE_INSERT_DISABLE			(0)
#define SIMULATE_INSERT_ENABLE			(1)
#define MMC_HOST_SLOT0					(0)
#define MMC_HOST_SLOT1					(1)
#define MMC_HOST_SLOT2					(2)
int simulate_insert_slot0 = SIMULATE_INSERT_DISABLE;
int simulate_insert_slot1 = SIMULATE_INSERT_DISABLE;
int simulate_insert_slot2 = SIMULATE_INSERT_DISABLE;
int simulate_insert_enable = SIMULATE_INSERT_DISABLE;

#ifdef MTK_IO_PERFORMANCE_DEBUG
unsigned int g_mtk_mmc_perf_dbg = 0;
unsigned int g_mtk_mmc_dbg_range = 0;
unsigned int g_dbg_range_start = 0;
unsigned int g_dbg_range_end = 0;
unsigned int g_mtk_mmc_dbg_flag = 0;
unsigned int g_dbg_req_count = 0;
unsigned int g_dbg_raw_count = 0;
unsigned int g_dbg_write_count = 0;
unsigned int g_dbg_raw_count_old = 0;
unsigned int g_mtk_mmc_clear = 0;
int g_check_read_write = 0;
int g_i = 0;
unsigned long long g_req_buf[4000][30] = { {0} };
unsigned long long g_req_write_buf[4000][30] = { {0} };
unsigned long long g_req_write_count[4000] = { 0 };

unsigned long long g_mmcqd_buf[400][300] = { {0} };

char *g_time_mark[] = {
	"--start fetch request",
	"--end fetch request",
	"--start dma map this request",
	"--end dma map this request",
	"--start request",
	"--DMA start",
	"--DMA transfer done",
	"--start dma unmap request",
	"--end dma unmap request",
	"--end of request",
};

char *g_time_mark_vfs_write[] = { "--in vfs_write",
				  "--before generic_segment_checks",
				  "--after generic_segment_checks",
				  "--after vfs_check_frozen",
				  "--after generic_write_checks",
				  "--after file_remove_suid",
				  "--after file_update_time",
				  "--after generic_file_direct_write",
				  "--after generic_file_buffered_write",
				  "--after filemap_write_and_wait_range",
				  "--after invalidate_mapping_pages",
				  "--after 2nd generic_file_buffered_write",
				  "--before generic_write_sync",
				  "--after generic_write_sync",
				  "--out vfs_write"
				};

#endif

/* for get transfer time with each trunk size, default not open */
#ifdef MTK_MMC_PERFORMANCE_TEST
unsigned int g_mtk_mmc_perf_test = 0;
#endif

typedef enum {
	MODE_NULL = 0,
	SDHC_HIGHSPEED,		/* 0x1 Host supports HS mode */
	UHS_SDR12,		/* 0x2 Host supports UHS SDR12 mode */
	UHS_SDR25,		/* 0x3 Host supports UHS SDR25 mode */
	UHS_SDR50,		/* 0x4 Host supports UHS SDR50 mode */
	UHS_SDR104,		/* 0x5 Host supports UHS SDR104 mode */
	UHS_DDR50,		/* 0x6 Host supports UHS DDR50 mode */

	DRIVER_TYPE_A,		/* 0x7 Host supports Driver Type A */
	DRIVER_TYPE_B,		/* 0x8 Host supports Driver Type B */
	DRIVER_TYPE_C,		/* 0x9 Host supports Driver Type C */
	DRIVER_TYPE_D,		/* 0xA Host supports Driver Type D */

	MAX_CURRENT_200,	/* 0xB Host max current limit is 200mA */
	MAX_CURRENT_400,	/* 0xC Host max current limit is 400mA */
	MAX_CURRENT_600,	/* 0xD Host max current limit is 600mA */
	MAX_CURRENT_800,	/* 0xE Host max current limit is 800mA */

	SDXC_NO_POWER_CONTROL,	/*0xF   Host not supports >150mA current at 3.3V /3.0V/1.8V */
	SDXC_POWER_CONTROL,	/*0x10 Host supports >150mA current at 3.3V /3.0V/1.8V */
} HOST_CAPS;

static char cmd_buf[256];


#if (3 == HOST_MAX_NUM)
/* for debug zone */
unsigned int msdc_debug_zone[HOST_MAX_NUM] = {
	DBG_EVT_ERR | DBG_EVT_IRQ | DBG_EVT_TUNE | DBG_EVT_DUMP,
	DBG_EVT_ERR | DBG_EVT_IRQ | DBG_EVT_TUNE | DBG_EVT_DUMP,
	DBG_EVT_ERR | DBG_EVT_IRQ | DBG_EVT_TUNE | DBG_EVT_DUMP,
};

/* mode select */
u32 dma_size[HOST_MAX_NUM] = {
	512,
	512,
	512,
};

#if 1
msdc_mode drv_mode[HOST_MAX_NUM] = {
	MODE_SIZE_DEP,		/* using DMA or not depend on the size */
	MODE_SIZE_DEP,
	MODE_SIZE_DEP,
};
#else
msdc_mode drv_mode[HOST_MAX_NUM] = {
	MODE_PIO,		/* using DMA or not depend on the size */
	MODE_PIO,
	MODE_PIO,
};
#endif

unsigned char msdc_clock_src[HOST_MAX_NUM] = {
	0,
	0,
	0,
};

u32 msdc_host_mode[HOST_MAX_NUM] = {
	0,
	0,
	0,
};

u32 msdc_host_mode2[HOST_MAX_NUM] = {
	0,
	0,
	0,
};
#endif				/* end of HOST_MAX_NUM */


drv_mod msdc_drv_mode[HOST_MAX_NUM];
int sdio_cd_result = 1;


/* for driver profile */
#define TICKS_ONE_MS  (13000)
u32 gpt_enable = 0;
u32 sdio_pro_enable = 0;	/* make sure gpt is enabled */
static unsigned long long sdio_pro_time = 30U;	/* no more than 30s */
static unsigned long long sdio_profiling_start;
struct sdio_profile sdio_perfomance = { 0 };

u32 sdio_enable_tune = 0;
u32 sdio_iocon_dspl = 0;
u32 sdio_iocon_w_dspl = 0;
u32 sdio_iocon_rspl = 0;
u32 sdio_pad_tune_rrdly = 0;
u32 sdio_pad_tune_rdly = 0;
u32 sdio_pad_tune_wrdly = 0;
u32 sdio_dat_rd_dly0_0 = 0;
u32 sdio_dat_rd_dly0_1 = 0;
u32 sdio_dat_rd_dly0_2 = 0;
u32 sdio_dat_rd_dly0_3 = 0;
u32 sdio_dat_rd_dly1_0 = 0;
u32 sdio_dat_rd_dly1_1 = 0;
u32 sdio_dat_rd_dly1_2 = 0;
u32 sdio_dat_rd_dly1_3 = 0;
u32 sdio_clk_drv = 0;
u32 sdio_cmd_drv = 0;
u32 sdio_data_drv = 0;
u32 sdio_tune_flag = 0;

static void msdc_dump_reg(unsigned int base)
{
	u32 id = 0;

	switch (base) {
	case MSDC_0_BASE:
		id = 0;
		break;

	case MSDC_1_BASE:
		id = 1U;
		break;

	case MSDC_2_BASE:
		id = 2U;
		break;

	default:
		break;
	}

	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[00] MSDC_CFG       = 0x%.8x", id, MSDC_READ32(base + 0x00));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[04] MSDC_IOCON     = 0x%.8x", id, MSDC_READ32(base + 0x04));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[08] MSDC_PS        = 0x%.8x", id, MSDC_READ32(base + 0x08));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[0C] MSDC_INT       = 0x%.8x", id, MSDC_READ32(base + 0x0C));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[10] MSDC_INTEN     = 0x%.8x", id, MSDC_READ32(base + 0x10));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[14] MSDC_FIFOCS    = 0x%.8x", id, MSDC_READ32(base + 0x14));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[30] SDC_CFG        = 0x%.8x", id, MSDC_READ32(base + 0x30));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[34] SDC_CMD        = 0x%.8x", id, MSDC_READ32(base + 0x34));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[38] SDC_ARG        = 0x%.8x", id, MSDC_READ32(base + 0x38));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[3C] SDC_STS        = 0x%.8x", id, MSDC_READ32(base + 0x3C));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[40] SDC_RESP0      = 0x%.8x", id, MSDC_READ32(base + 0x40));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[44] SDC_RESP1      = 0x%.8x", id, MSDC_READ32(base + 0x44));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[48] SDC_RESP2      = 0x%.8x", id, MSDC_READ32(base + 0x48));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[4C] SDC_RESP3      = 0x%.8x", id, MSDC_READ32(base + 0x4C));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[50] SDC_BLK_NUM    = 0x%.8x", id, MSDC_READ32(base + 0x50));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[58] SDC_CSTS       = 0x%.8x", id, MSDC_READ32(base + 0x58));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[5C] SDC_CSTS_EN    = 0x%.8x", id, MSDC_READ32(base + 0x5C));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[60] SDC_DATCRC_STS = 0x%.8x", id, MSDC_READ32(base + 0x60));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[70] EMMC_CFG0      = 0x%.8x", id, MSDC_READ32(base + 0x70));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[74] EMMC_CFG1      = 0x%.8x", id, MSDC_READ32(base + 0x74));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[78] EMMC_STS       = 0x%.8x", id, MSDC_READ32(base + 0x78));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[7C] EMMC_IOCON     = 0x%.8x", id, MSDC_READ32(base + 0x7C));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[80] SD_ACMD_RESP   = 0x%.8x", id, MSDC_READ32(base + 0x80));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[84] SD_ACMD19_TRG  = 0x%.8x", id, MSDC_READ32(base + 0x84));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[88] SD_ACMD19_STS  = 0x%.8x", id, MSDC_READ32(base + 0x88));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[90] DMA_SA         = 0x%.8x", id, MSDC_READ32(base + 0x90));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[94] DMA_CA         = 0x%.8x", id, MSDC_READ32(base + 0x94));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[98] DMA_CTRL       = 0x%.8x", id, MSDC_READ32(base + 0x98));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[9C] DMA_CFG        = 0x%.8x", id, MSDC_READ32(base + 0x9C));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[A0] SW_DBG_SEL     = 0x%.8x", id, MSDC_READ32(base + 0xA0));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[A4] SW_DBG_OUT     = 0x%.8x", id, MSDC_READ32(base + 0xA4));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[B0] PATCH_BIT0     = 0x%.8x", id, MSDC_READ32(base + 0xB0));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[B4] PATCH_BIT1     = 0x%.8x", id, MSDC_READ32(base + 0xB4));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[EC] PAD_TUNE       = 0x%.8x", id, MSDC_READ32(base + 0xEC));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[F0] DAT_RD_DLY0    = 0x%.8x", id, MSDC_READ32(base + 0xF0));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[F4] DAT_RD_DLY1    = 0x%.8x", id, MSDC_READ32(base + 0xF4));
	MSDC_LOG_NORMAL(pr_info,"[%d]Reg[F8] HW_DBG_SEL     = 0x%.8x", id, MSDC_READ32(base + 0xF8));
	MSDC_LOG_NORMAL(pr_info,"[%d]Rg[100] MAIN_VER       = 0x%.8x", id, MSDC_READ32(base + 0x100));
	MSDC_LOG_NORMAL(pr_info,"[%d]Rg[104] ECO_VER        = 0x%.8x", id, MSDC_READ32(base + 0x104));
}

static void msdc_set_field(unsigned int address, unsigned int start_bit, unsigned int len,
			   unsigned int value)
{
	unsigned long field;

	if ((start_bit > 31U) || (len > 32U) || (len == 0)) {
		MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]reg filed beyoned (0~31) or length beyoned (1~32)");
	} else {
		field = ((1 << len) - 1) << start_bit;
		value &= (1 << len) - 1;
		MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Original:0x%x (0x%x)", address, MSDC_READ32(address));
		MSDC_SET_FIELD(address, field, value);
		MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Modified:0x%x (0x%x)", address, MSDC_READ32(address));
	}
}

static void msdc_get_field(unsigned int address, unsigned int start_bit, unsigned int len,
			   unsigned int value)
{
	unsigned long field;

	if ((start_bit > 31U) || (len > 32U) || (len == 0)) {
		MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]reg filed beyoned (0~31) or length beyoned (1~32)");
	} else {
		field = ((1 << len) - 1) << start_bit;
		MSDC_GET_FIELD(address, field, value);
		MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Reg:0x%x start_bit(%d)len(%d)(0x%x)", address,
			start_bit, len, value);
	}
}

static void msdc_init_gpt(void)
{
#if 0
	GPT_CONFIG config;

	config.num = GPT6;
	config.mode = GPT_FREE_RUN;
	config.clkSrc = GPT_CLK_SRC_SYS;
	config.clkDiv = GPT_CLK_DIV_1;	/* 13MHz GPT6 */

	if (GPT_Config(config) == FALSE) {
		return;
	}

	GPT_Start(GPT6);
#endif
}

u32 msdc_time_calc(u32 old_L32, u32 old_H32, u32 new_L32, u32 new_H32)
{
	u32 ret = 0;

	if (new_H32 == old_H32) {
		ret = new_L32 - old_L32;
	} else if (new_H32 == (old_H32 + 1)) {
		if (new_L32 > old_L32) {
			MSDC_LOG_NORMAL(pr_info,"msdc old_L<0x%x> new_L<0x%x>", old_L32, new_L32);
		}

		ret = (0xffffffffU - old_L32);
		ret += new_L32;
	} else {
		MSDC_LOG_NORMAL(pr_info,"msdc old_H<0x%x> new_H<0x%x>", old_H32, new_H32);
	}

	return ret;
}

void msdc_sdio_profile(struct sdio_profile *result)
{
	struct cmd_profile *cmd;
	u32 i;

	MSDC_LOG_NORMAL(pr_info,"[sdio] === performance dump ===");
	MSDC_LOG_NORMAL(pr_info,"[sdio] === total execute tick<%d> time<%dms> Tx<%dB> Rx<%dB>",
		result->total_tc, result->total_tc / TICKS_ONE_MS,
		result->total_tx_bytes, result->total_rx_bytes);

	/* CMD52 Dump */
	cmd = &result->cmd52_rx;
	MSDC_LOG_NORMAL(pr_info,"[sdio] === CMD52 Rx <%d>times tick<%d> Max<%d> Min<%d> Aver<%d>", cmd->count,
		cmd->tot_tc, cmd->max_tc, cmd->min_tc, cmd->tot_tc / cmd->count);
	cmd = &result->cmd52_tx;
	MSDC_LOG_NORMAL(pr_info,"[sdio] === CMD52 Tx <%d>times tick<%d> Max<%d> Min<%d> Aver<%d>", cmd->count,
		cmd->tot_tc, cmd->max_tc, cmd->min_tc, cmd->tot_tc / cmd->count);

	/* CMD53 Rx bytes + block mode */
	for (i = 0; i < 512; i++) {
		cmd = &result->cmd53_rx_byte[i];

		if (cmd->count) {
			MSDC_LOG_NORMAL(pr_info,"[sdio]<%6d><%3dB>_Rx_<%9d><%9d><%6d><%6d>_<%9dB><%2dM>", cmd->count,
				i, cmd->tot_tc, cmd->max_tc, cmd->min_tc, cmd->tot_tc / cmd->count,
				cmd->tot_bytes, (cmd->tot_bytes / 10) * 13 / (cmd->tot_tc / 10));
		}
	}

	for (i = 0; i < 100; i++) {
		cmd = &result->cmd53_rx_blk[i];

		if (cmd->count) {
			MSDC_LOG_NORMAL(pr_info,"[sdio]<%6d><%3d>B_Rx_<%9d><%9d><%6d><%6d>_<%9dB><%2dM>", cmd->count,
				i, cmd->tot_tc, cmd->max_tc, cmd->min_tc, cmd->tot_tc / cmd->count,
				cmd->tot_bytes, (cmd->tot_bytes / 10) * 13 / (cmd->tot_tc / 10));
		}
	}

	/* CMD53 Tx bytes + block mode */
	for (i = 0; i < 512; i++) {
		cmd = &result->cmd53_tx_byte[i];

		if (cmd->count) {
			MSDC_LOG_NORMAL(pr_info,"[sdio]<%6d><%3dB>_Tx_<%9d><%9d><%6d><%6d>_<%9dB><%2dM>", cmd->count,
				i, cmd->tot_tc, cmd->max_tc, cmd->min_tc, cmd->tot_tc / cmd->count,
				cmd->tot_bytes, (cmd->tot_bytes / 10U) * 13U / (cmd->tot_tc / 10U));
		}
	}

	for (i = 0; i < 100; i++) {
		cmd = &result->cmd53_tx_blk[i];

		if (cmd->count) {
			MSDC_LOG_NORMAL(pr_info,"[sdio]<%6d><%3d>B_Tx_<%9d><%9d><%6d><%6d>_<%9dB><%2dM>", cmd->count,
				i, cmd->tot_tc, cmd->max_tc, cmd->min_tc, cmd->tot_tc / cmd->count,
				cmd->tot_bytes, (cmd->tot_bytes / 10U) * 13U / (cmd->tot_tc / 10U));
		}
	}

	MSDC_LOG_NORMAL(pr_info,"[sdio] === performance dump done ===");
}

/* ========= sdio command table =========== */
void msdc_performance(u32 opcode, u32 sizes, u32 bRx, u32 ticks)
{
	struct sdio_profile *result = &sdio_perfomance;
	struct cmd_profile *cmd;
	u32 block;
	long long endtime;

	if (sdio_pro_enable == 0) {
		return;
	}

	if (opcode == 52U) {
		cmd = bRx ? &result->cmd52_rx : &result->cmd52_tx;
	} else if (opcode == 53U) {
		if (sizes < 512U) {
			cmd = bRx ? &result->cmd53_rx_byte[sizes] : &result->cmd53_tx_byte[sizes];
		} else {
			block = sizes / 512U;

			if (block >= 99U) {
				MSDC_LOG_NORMAL(pr_info,"cmd53 error blocks");

				while (1)
					;
			}

			cmd = bRx ? &result->cmd53_rx_blk[block] : &result->cmd53_tx_blk[block];
		}
	} else {
		return;
	}

	/* update the members */
	if (ticks > cmd->max_tc) {
		cmd->max_tc = ticks;
	}

	if ((cmd->min_tc == 0) || (ticks < cmd->min_tc)) {
		cmd->min_tc = ticks;
	}

	cmd->tot_tc += ticks;
	cmd->tot_bytes += sizes;
	cmd->count++;

	if (bRx) {
		result->total_rx_bytes += sizes;
	} else {
		result->total_tx_bytes += sizes;
	}

	result->total_tc += ticks;
#if 0

	/* dump when total_tc > 30s */
	if (result->total_tc >= sdio_pro_time * TICKS_ONE_MS * 1000) {
		msdc_sdio_profile(result);
		memset(result, 0, sizeof(struct sdio_profile));
	}

#endif



	endtime = sched_clock();

	if ((endtime - sdio_profiling_start) >= sdio_pro_time * 1000000000U) {
		msdc_sdio_profile(result);
		memset(result, 0, sizeof(struct sdio_profile));
		sdio_profiling_start = endtime;
	}


}

#define COMPARE_ADDRESS_MMC   0x402000U
#define COMPARE_ADDRESS_SD    0x2000U
#define COMPARE_ADDRESS_SDIO  0x0
#define COMPARE_ADDRESS_SD_COMBO  0x2000U

#define MSDC_MULTI_BUF_LEN  (4U*1024U)

static DEFINE_MUTEX(sd_lock);
static DEFINE_MUTEX(emmc_lock);

u32 msdc_multi_wbuf_sd[MSDC_MULTI_BUF_LEN + 1024U];
u32 msdc_multi_rbuf_sd[MSDC_MULTI_BUF_LEN + 1024U];

u32 msdc_multi_wbuf_emmc[MSDC_MULTI_BUF_LEN + 1024U];
u32 msdc_multi_rbuf_emmc[MSDC_MULTI_BUF_LEN + 1024U];

static int sd_multi_rw_compare_slave(int host_num, int read, uint address)
{
#ifdef MTK_EMMC_SUPPORT
	char l_buf[512];
#endif

	struct scatterlist msdc_sg;
	struct mmc_data msdc_data;
	struct mmc_command msdc_cmd;
	struct mmc_command msdc_stop;

#ifdef MTK_MSDC_USE_CMD23
	struct mmc_command msdc_sbc;
#endif

	struct mmc_request msdc_mrq;
	struct msdc_host *host_ctl;
	/* struct msdc_host *host = atc_msdc_host[host_num]; */
	int result = 0, forIndex = 0;
	u8 *wPtr;
	u8 wData[16] = {
		0x67, 0x45, 0x23, 0x01,
		0xef, 0xcd, 0xab, 0x89,
		0xce, 0x8a, 0x46, 0x02,
		0xde, 0x9b, 0x57, 0x13
	};

	host_ctl = atc_msdc_host[host_num];

	if ((host_ctl == NULL) || (host_ctl->mmc == NULL) || (host_ctl->mmc->card == NULL)) {
		MSDC_LOG_NORMAL(pr_info," there is no card initialized in host[%d]", host_num);
		return -1;
	}

	if (!IS_CARD_PRESENT(host_ctl)) {
		MSDC_LOG_NORMAL(pr_info,"  [%s]: card is removed!", __func__);
		return -1;
	}

	mmc_claim_host(host_ctl->mmc);

#ifdef MTK_EMMC_SUPPORT

	if (host_ctl->hw->host_function == MSDC_EMMC) {
		mmc_send_ext_csd(host_ctl->mmc->card, l_buf);

		/* make sure access partition is user data area */
		if (0 != (l_buf[179] & 0x7)) {
			/* set back to access user area */
			MSDC_LOG_NORMAL(pr_info,"set back to user area");
			l_buf[179] &= ~0x7;
			l_buf[179] |= 0x0;

			mmc_switch(host_ctl->mmc->card, 0, 179, l_buf[179], 1000);
		}
	}

#endif

	memset(&msdc_data, 0, sizeof(msdc_data)); /* MISRA Try*/
	memset((&msdc_mrq), 0, sizeof(struct mmc_request));
	memset(&msdc_cmd, 0, sizeof(struct mmc_command));
	memset(&msdc_stop, 0, sizeof(struct mmc_command));

#ifdef MTK_MSDC_USE_CMD23
	memset(&msdc_sbc, 0, sizeof(struct mmc_command));
#endif

	msdc_mrq.cmd = &msdc_cmd;
	msdc_mrq.data = &msdc_data;

	if (read) {
		/* init read command */
		msdc_data.flags = MMC_DATA_READ;
		msdc_cmd.opcode = MMC_READ_MULTIPLE_BLOCK;
		msdc_data.blocks = sizeof(msdc_multi_rbuf_sd) / 512;
		wPtr = (u8 *) msdc_multi_rbuf_sd;

		/* init read buffer */
		for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN * 4; forIndex++) {
			*(wPtr + forIndex) = 0x0;
		}

		/* for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN;forIndex++) */
		/* pr_info("R_buffer[0x%x]\n",msdc_multi_rbuf_sd[forIndex]); */
	} else {
		/* init write command */
		msdc_data.flags = MMC_DATA_WRITE;
		msdc_cmd.opcode = MMC_WRITE_MULTIPLE_BLOCK;
		msdc_data.blocks = sizeof(msdc_multi_wbuf_sd) / 512;
		/* init write buffer */
		wPtr = (u8 *) msdc_multi_wbuf_sd;

		for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN * 4; forIndex++) {
			*(wPtr + forIndex) = wData[forIndex % 16];
		}

		/* for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN;forIndex++) */
		/* pr_info("W_buffer[0x%x]\n",msdc_multi_wbuf_sd[forIndex]); */
	}

	msdc_cmd.arg = address;

	BUG_ON(!host_ctl->mmc->card);

#ifdef MTK_MSDC_USE_CMD23

	if ((mmc_card_mmc(host_ctl->mmc->card)
	     || (mmc_card_sd(host_ctl->mmc->card)
		 && (host_ctl->mmc->card->scr.cmds & SD_SCR_CMD23_SUPPORT)))
	    && (!(host_ctl->mmc->card->quirks & MMC_QUIRK_BLK_NO_CMD23))) {
		msdc_mrq.sbc = &msdc_sbc;
		msdc_mrq.sbc->opcode = MMC_SET_BLOCK_COUNT;
		msdc_mrq.sbc->arg = msdc_data.blocks;
		msdc_mrq.sbc->flags = MMC_RSP_R1 | MMC_CMD_AC;
	}

#endif

	if (!mmc_card_blockaddr(host_ctl->mmc->card)) {
		/* pr_info("this device use byte address!!\n"); */
		msdc_cmd.arg <<= 9;
	}

	msdc_cmd.flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;

	msdc_stop.opcode = MMC_STOP_TRANSMISSION;
	msdc_stop.arg = 0;
	msdc_stop.flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;

	msdc_data.stop = &msdc_stop;
	msdc_data.blksz = 512;
	msdc_data.sg = &msdc_sg;
	msdc_data.sg_len = 1;

	if (read) {
		sg_init_one(&msdc_sg, msdc_multi_rbuf_sd, sizeof(msdc_multi_rbuf_sd));
	} else {
		sg_init_one(&msdc_sg, msdc_multi_wbuf_sd, sizeof(msdc_multi_wbuf_sd));
	}

	mmc_set_data_timeout(&msdc_data, host_ctl->mmc->card);
	mmc_wait_for_req(host_ctl->mmc, &msdc_mrq);

	if (read) {
		for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN; forIndex++) {
			/* pr_info("index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]\t\n",
				forIndex, msdc_multi_wbuf_sd[forIndex], msdc_multi_rbuf_sd[forIndex]); */

			if (msdc_multi_wbuf_sd[forIndex] != msdc_multi_rbuf_sd[forIndex]) {
				MSDC_LOG_NORMAL(pr_info,"index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]\tfailed",
					forIndex, msdc_multi_wbuf_sd[forIndex],
					msdc_multi_rbuf_sd[forIndex]);
				result = -1;
			}
		}

		/*if(result == 0)
		   pr_info("pid[%d][%s]: data compare successed!!\n", current->pid, __func__);
		   else
		   pr_info("pid[%d][%s]: data compare failed!!\n", current->pid, __func__); */
	}

	mmc_release_host(host_ctl->mmc);

	if (msdc_cmd.error) {
		result = msdc_cmd.error;
	}

	if (msdc_data.error) {
		result = msdc_data.error;
	} else {
		result = 0;
	}

	return result;
}

static int sd_multi_rw_compare(int host_num, uint address, int count)
{
	int i = 0, j = 0;
	int error = 0;

	for (i = 0; i < count; i++) {
		/* pr_info("============ cpu[%d] pid[%d]: start the %d time compare ============\n", */
		/* task_cpu(current), current->pid, i); */

		mutex_lock(&sd_lock);
		error = sd_multi_rw_compare_slave(host_num, 0, address);	/* write */

		if (error) {
			MSDC_LOG_NORMAL(pr_info,"%s: failed to write data, error=%d", __func__, error);
			mutex_unlock(&sd_lock);
			break;
		}

		for (j = 0; j < 1; j++) {
			error = sd_multi_rw_compare_slave(host_num, 1, address);	/* read */

			if (error) {
				MSDC_LOG_NORMAL(pr_info,"%s: failed to read data, error=%d", __func__, error);
				break;
			}
		}

		if (error)
			MSDC_LOG_NORMAL(pr_info,
			"============ cpu[%d] pid[%d]: FAILED the %d time compare ============",
			 task_cpu(current), current->pid, i);
		else
			MSDC_LOG_NORMAL(pr_info,
			"============ cpu[%d] pid[%d]: FINISH the %d time compare ============",
			 task_cpu(current), current->pid, i);

		mutex_unlock(&sd_lock);
	}

	if (i == count) {
		MSDC_LOG_NORMAL(pr_info,"pid[%d]: successed to compare data within %d times", current->pid, count);
	}

	return error;
}

static int emmc_multi_rw_compare_slave(int host_num, int read, uint address)
{
#ifdef MTK_EMMC_SUPPORT
	char l_buf[512];
#endif
	struct scatterlist msdc_sg;
	struct mmc_data msdc_data;
	struct mmc_command msdc_cmd;
	struct mmc_command msdc_stop;

#ifdef MTK_MSDC_USE_CMD23
	struct mmc_command msdc_sbc;
#endif

	struct mmc_request msdc_mrq;
	struct msdc_host *host_ctl;
	/* struct msdc_host *host = atc_msdc_host[host_num]; */
	int result = 0, forIndex = 0;
	u8 *wPtr;
	u8 wData[16] = {
		0x67, 0x45, 0x23, 0x01,
		0xef, 0xcd, 0xab, 0x89,
		0xce, 0x8a, 0x46, 0x02,
		0xde, 0x9b, 0x57, 0x13
	};

	host_ctl = atc_msdc_host[host_num];

	if ((host_ctl == NULL) || (host_ctl->mmc == NULL) || (host_ctl->mmc->card == NULL)) {
		MSDC_LOG_NORMAL(pr_info," there is no card initialized in host[%d]", host_num);
		return -1;
	}

	if (!IS_CARD_PRESENT(host_ctl)) {
		MSDC_LOG_NORMAL(pr_info,"  %s: card is removed!", __func__);
		return -1;
	}

	mmc_claim_host(host_ctl->mmc);

#ifdef MTK_EMMC_SUPPORT

	if (host_ctl->hw->host_function == MSDC_EMMC) {
		mmc_send_ext_csd(host_ctl->mmc->card, l_buf);

		/* make sure access partition is user data area */
		if (0 != (l_buf[179] & 0x7)) {
			/* set back to access user area */
			MSDC_LOG_NORMAL(pr_info,"set back to user area");
			l_buf[179] &= ~0x7;
			l_buf[179] |= 0x0;

			mmc_switch(host_ctl->mmc->card, 0, 179, l_buf[179], 1000);
		}
	}

#endif

	memset(&msdc_data, 0, sizeof(struct mmc_data));
	memset(&msdc_mrq, 0, sizeof(struct mmc_request));
	memset(&msdc_cmd, 0, sizeof(struct mmc_command));
	memset(&msdc_stop, 0, sizeof(struct mmc_command));

#ifdef MTK_MSDC_USE_CMD23
	memset(&msdc_sbc, 0, sizeof(struct mmc_command));
#endif

	msdc_mrq.cmd = &msdc_cmd;
	msdc_mrq.data = &msdc_data;

	if (read) {
		/* init read command */
		msdc_data.flags = MMC_DATA_READ;
		msdc_cmd.opcode = MMC_READ_MULTIPLE_BLOCK;
		msdc_data.blocks = sizeof(msdc_multi_rbuf_emmc) / 512;
		wPtr = (u8 *) msdc_multi_rbuf_emmc;

		/* init read buffer */
		for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN * 4; forIndex++) {
			*(wPtr + forIndex) = 0x0;
		}

		/* for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN;forIndex++) */
		/* pr_info("R_buffer[0x%x]\n",msdc_multi_rbuf_emmc[forIndex]); */
	} else {
		/* init write command */
		msdc_data.flags = MMC_DATA_WRITE;
		msdc_cmd.opcode = MMC_WRITE_MULTIPLE_BLOCK;
		msdc_data.blocks = sizeof(msdc_multi_wbuf_emmc) / 512;
		/* init write buffer */
		wPtr = (u8 *) msdc_multi_wbuf_emmc;

		for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN * 4; forIndex++) {
			*(wPtr + forIndex) = wData[forIndex % 16];
		}

		/* for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN;forIndex++) */
		/* pr_info("W_buffer[0x%x]\n",msdc_multi_wbuf_emmc[forIndex]); */
	}

	msdc_cmd.arg = address;

	BUG_ON(!host_ctl->mmc->card);
#ifdef MTK_MSDC_USE_CMD23

	if ((mmc_card_mmc(host_ctl->mmc->card)
	     || (mmc_card_sd(host_ctl->mmc->card)
		 && (host_ctl->mmc->card->scr.cmds & SD_SCR_CMD23_SUPPORT)))
	    && ((host_ctl->mmc->card->quirks & MMC_QUIRK_BLK_NO_CMD23) == 0)) {
		msdc_mrq.sbc = &msdc_sbc;
		msdc_mrq.sbc->opcode = MMC_SET_BLOCK_COUNT;
		msdc_mrq.sbc->arg = msdc_data.blocks;
		msdc_mrq.sbc->flags = MMC_RSP_R1 | MMC_CMD_AC;
	}

#endif

	if (!mmc_card_blockaddr(host_ctl->mmc->card)) {
		/* pr_info("this device use byte address!!\n"); */
		msdc_cmd.arg <<= 9;
	}

	msdc_cmd.flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;

	msdc_stop.opcode = MMC_STOP_TRANSMISSION;
	msdc_stop.arg = 0;
	msdc_stop.flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;

	msdc_data.stop = &msdc_stop;
	msdc_data.blksz = 512;
	msdc_data.sg = &msdc_sg;
	msdc_data.sg_len = 1;

	if (read) {
		sg_init_one(&msdc_sg, msdc_multi_rbuf_emmc, sizeof(msdc_multi_rbuf_emmc));
	} else {
		sg_init_one(&msdc_sg, msdc_multi_wbuf_emmc, sizeof(msdc_multi_wbuf_emmc));
	}

	mmc_set_data_timeout(&msdc_data, host_ctl->mmc->card);
	mmc_wait_for_req(host_ctl->mmc, &msdc_mrq);

	if (read) {
		for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN; forIndex++) {
			/* pr_info("index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]\t\n",
			forIndex, msdc_multi_wbuf_emmc[forIndex], msdc_multi_rbuf_emmc[forIndex]); */

			if (msdc_multi_wbuf_emmc[forIndex] != msdc_multi_rbuf_emmc[forIndex]) {
				MSDC_LOG_NORMAL(pr_info,"index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]\tfailed",
					forIndex, msdc_multi_wbuf_emmc[forIndex],
					msdc_multi_rbuf_emmc[forIndex]);
				result = -1;
			}
		}

		/*if(result == 0)
		   pr_info("pid[%d][%s]: data compare successed!!\n", current->pid, __func__);
		   else
		   pr_info("pid[%d][%s]: data compare failed!!\n", current->pid, __func__); */
	}

	mmc_release_host(host_ctl->mmc);

	if (msdc_cmd.error) {
		result = msdc_cmd.error;
	}

	if (msdc_data.error) {
		result = msdc_data.error;
	} else {
		result = 0;
	}

	return result;
}

static int emmc_multi_rw_compare(int host_num, uint address, int count)
{
	int i = 0, j = 0;
	int error = 0;

	for (i = 0; i < count; i++) {
		/* pr_info("============ cpu[%d] pid[%d]: start the %d time compare ============\n",
		task_cpu(current), current->pid, i); */

		mutex_lock(&emmc_lock);
		error = emmc_multi_rw_compare_slave(host_num, 0, address);	/* write */

		if (error) {
			MSDC_LOG_NORMAL(pr_info,"%s: failed to write data, error=%d", __func__, error);
			mutex_unlock(&emmc_lock);
			break;
		}

		for (j = 0; j < 1; j++) {
			error = emmc_multi_rw_compare_slave(host_num, 1, address);	/* read */

			if (error) {
				MSDC_LOG_NORMAL(pr_info,"%s: failed to read data, error=%d", __func__, error);
				break;
			}
		}

		if (error)
			MSDC_LOG_NORMAL(pr_info,
			"============ cpu[%d] pid[%d]: FAILED the %d time compare ============",
			 task_cpu(current), current->pid, i);
		else
			MSDC_LOG_NORMAL(pr_info,
			"============ cpu[%d] pid[%d]: FINISH the %d time compare ============",
			 task_cpu(current), current->pid, i);

		mutex_unlock(&emmc_lock);
	}

	if (i == count) {
		MSDC_LOG_NORMAL(pr_info, "pid[%d]: successed to compare data within %d times", current->pid, count);
	}

	return error;
}



#define MAX_THREAD_NUM_FOR_SMP 20

/* make the test can run on 4GB card */
static uint smp_address_on_sd[MAX_THREAD_NUM_FOR_SMP] = {
	0x2000,
	0x80000,
	0x100000,
	0x180000,
	0x200000,		/* 1GB */
	0x202000,
	0x280000,
	0x300000,
	0x380000,
	0x400000,		/* 2GB */
	0x402000,
	0x480000,
	0x500000,
	0x580000,
	0x600000,
	0x602000,		/* 3GB */
	0x660000,		/* the real total size of 4GB sd card is below 4GB */
	0x680000,
	0x6a0000,
	0x6b0000,
};

/* cause the system run on the emmc storage,
 * so do not to access the first 2GB region */
static uint smp_address_on_mmc[MAX_THREAD_NUM_FOR_SMP] = {
	0x402000,
	0x410000,
	0x520000,
	0x530000,
	0x640000,
	0x452000,
	0x460000,
	0x470000,
	0x480000,
	0x490000,
	0x4a2000,
	0x4b0000,
	0x5c0000,
	0x5d0000,
	0x6e0000,
	0x602000,
	0x660000,		/* the real total size of 4GB sd card is below 4GB */
	0x680000,
	0x6a0000,
	0x6b0000,
};

static uint smp_address_on_sd_combo[MAX_THREAD_NUM_FOR_SMP] = {
	0x2000,
	0x20000,
	0x200000,
	0x2000000,
	0x2200000,
	0x2400000,
	0x2800000,
	0x2c00000,
	0x4000000,
	0x4200000,
	0x4400000,
	0x4800000,
	0x4c00000,
	0x8000000,
	0x8200000,
	0x8400000,
	0x8800000,
	0x8c00000,
	0xc000000,
	0xc200000
};

struct write_read_data {
	int host_id;		/* the target host you want to do SMP test on. */
	uint start_address;	/* where you want to do write/read of the memory card */
	int count;		/* how many times you want to do read after write bit by bit comparison */
};

static struct write_read_data wr_data[HOST_MAX_NUM][MAX_THREAD_NUM_FOR_SMP];
/*
 * 2012-03-25
 * the SMP thread function
 * do read after write the memory card, and bit by bit comparison
 */
static int write_read_thread(void *ptr)
{
	struct write_read_data *data = (struct write_read_data *)ptr;

	if (1 == data->host_id) {
		MSDC_LOG_NORMAL(pr_info,"sd thread");
		sd_multi_rw_compare(data->host_id, data->start_address, data->count);
	} else if (0 == data->host_id) {
		MSDC_LOG_NORMAL(pr_info,"emmc thread");
		emmc_multi_rw_compare(data->host_id, data->start_address, data->count);
	} else {
		MSDC_LOG_NORMAL(pr_info,"write_read_thread:host id error.");
	}

	return 0;
}

/*
 * 2012-03-25
 * function:         do SMP test on the same one MSDC host
 * thread_num:       the number of thread you want to trigger on this host.
 * host_id:          the target host you want to do SMP test on.
 * count:            how many times you want to do read after write bit by bit comparison in each thread.
 * multi_address:    whether do read/write the same/different address of the memory card in each thread.
 */
static int smp_test_on_one_host(int thread_num, int host_id, int count, int multi_address)
{
	int i = 0, ret = 0;
	char thread_name[128];
	struct msdc_host *host_ctl;

	MSDC_LOG_NORMAL(pr_info,"============================[%s] start ================================\n\n",
		__func__);
	MSDC_LOG_NORMAL(pr_info," host %d run %d thread, each thread run %d RW comparison", host_id, thread_num,
		count);

	if ((host_id >= HOST_MAX_NUM) || (host_id < 0)) {
		MSDC_LOG_NORMAL(pr_info," bad host id: %d", host_id);
		ret = -1;
		goto out;
	}

	if (thread_num > MAX_THREAD_NUM_FOR_SMP) {	/* && (multi_address != 0)) */
		MSDC_LOG_NORMAL(pr_info," too much thread for SMP test, thread_num=%d", thread_num);
		ret = -1;
		goto out;
	}

	host_ctl = atc_msdc_host[host_id];

	if ((host_ctl == NULL) || (host_ctl->mmc == NULL) || (host_ctl->mmc->card == NULL)) {
		MSDC_LOG_NORMAL(pr_info,"there is no card initialized in host[%d]", host_id);
		ret = -1;
		goto out;
	}


	for (i = 0; i < thread_num; i++) {
		switch (host_ctl->mmc->card->type) {
		case MMC_TYPE_MMC:
			if (!multi_address) {
				wr_data[host_id][i].start_address = COMPARE_ADDRESS_MMC;
			} else {
				wr_data[host_id][i].start_address = smp_address_on_mmc[i];
			}

			if (i == 0) {
				MSDC_LOG_NORMAL(pr_info,"[%d], MMC:", host_id);
			}

			break;

		case MMC_TYPE_SD:
			if (!multi_address) {
				wr_data[host_id][i].start_address = COMPARE_ADDRESS_SD;
			} else {
				wr_data[host_id][i].start_address = smp_address_on_sd[i];
			}

			if (i == 0) {
				MSDC_LOG_NORMAL(pr_info,"[%d], SD:", host_id);
			}

			break;

		case MMC_TYPE_SDIO:
			if (i == 0) {
				MSDC_LOG_NORMAL(pr_info,"[%d], SDIO:", host_id);
				MSDC_LOG_NORMAL(pr_info,"   please manually trigger wifi application instead of ");
				MSDC_LOG_NORMAL(pr_info,"write/read something on SDIO card");
			}

			ret = -1;
			goto out;

		case MMC_TYPE_SD_COMBO:
			if (!multi_address) {
				wr_data[host_id][i].start_address = COMPARE_ADDRESS_SD_COMBO;
			} else {
				wr_data[host_id][i].start_address = smp_address_on_sd_combo[i];
			}

			if (i == 0) {
				MSDC_LOG_NORMAL(pr_info," [%d], SD_COMBO:", host_id);
			}

			break;

		default:
			if (i == 0) {
				MSDC_LOG_NORMAL(pr_info," [%d], cannot recognize this card", host_id);
			}

			ret = -1;
			goto out;
		}

		wr_data[host_id][i].host_id = host_id;
		wr_data[host_id][i].count = count;
		sprintf(thread_name, "msdc_H%d_T%d", host_id, i);
		kthread_run(write_read_thread, &wr_data[host_id][i], thread_name);
		MSDC_LOG_NORMAL(pr_info,"   start thread: %s, at address 0x%x", thread_name,
			wr_data[host_id][i].start_address);
	}

out:
	MSDC_LOG_NORMAL(pr_info,"========================[%s] end ======================\n\n", __func__);
	return ret;
}

/*
 * 2012-03-25
 * function:         do SMP test on all MSDC hosts
 * thread_num:       the number of thread you want to trigger on this host.
 * count:            how many times you want to do read after write bit by bit comparison in each thread.
 * multi_address:    whether do read/write the same/different address of the memory card in each thread.
 */
static int smp_test_on_all_host(int thread_num, int count, int multi_address)
{
	int i = 0;
	int j = 0;
	int ret = 0;
	char thread_name[128];
	struct msdc_host *host_ctl;

	MSDC_LOG_NORMAL(pr_info,"=====================[%s] start =====================\n\n", __func__);
	MSDC_LOG_NORMAL(pr_info," each host run %d thread, each thread run %d RW comparison", thread_num, count);

	if (thread_num > MAX_THREAD_NUM_FOR_SMP) {	/* && (multi_address != 0)) */
		MSDC_LOG_NORMAL(pr_info," too much thread for SMP test, thread_num=%d", thread_num);
		ret = -1;
		goto out;
	}

	for (i = 0; i < HOST_MAX_NUM; i++) {
		host_ctl = atc_msdc_host[i];

		if ((!host_ctl) || (!host_ctl->mmc) || (!host_ctl->mmc->card)) {
			MSDC_LOG_NORMAL(pr_info,"[%d], no card is initialized", i);
			continue;
		}

		if (host_ctl->mmc->card->type == MMC_TYPE_SDIO) {
			MSDC_LOG_NORMAL(pr_info,"[%d], SDIO, please manually trigger wifi application instead of ", i);
			MSDC_LOG_NORMAL(pr_info,"write/read something on SDIO card");
			continue;
		}

		for (j = 0; j < thread_num; j++) {
			wr_data[i][j].host_id = i;
			wr_data[i][j].count = count;

			switch (host_ctl->mmc->card->type) {
			case MMC_TYPE_MMC:
				if (!multi_address) {
					wr_data[i][j].start_address = COMPARE_ADDRESS_MMC;
				} else {
					wr_data[i][j].start_address = smp_address_on_mmc[i];
				}

				if (j == 0) {
					MSDC_LOG_NORMAL(pr_info,"[%d], MMC: ", i);
				}

				break;

			case MMC_TYPE_SD:
				if (!multi_address) {
					wr_data[i][j].start_address = COMPARE_ADDRESS_SD;
				} else {
					wr_data[i][j].start_address = smp_address_on_sd[i];
				}

				if (j == 0) {
					MSDC_LOG_NORMAL(pr_info,"[%d], SD:", i);
				}

				break;

			case MMC_TYPE_SDIO:
				if (j == 0) {
					MSDC_LOG_NORMAL(pr_info,"[%d], SDIO:", i);
					MSDC_LOG_NORMAL(pr_info,"   please manually trigger wifi application instead of ");
					MSDC_LOG_NORMAL(pr_info,"write/read something on SDIO card");
				}

				ret = -1;
				goto out;

			case MMC_TYPE_SD_COMBO:
				if (!multi_address) {
					wr_data[i][j].start_address = COMPARE_ADDRESS_SD_COMBO;
				} else {
					wr_data[i][j].start_address = smp_address_on_sd_combo[i];
				}

				if (j == 0) {
					MSDC_LOG_NORMAL(pr_info,"[%d], SD_COMBO:", i);
				}

				break;

			default:
				if (j == 0) {
					MSDC_LOG_NORMAL(pr_info,"[%d], cannot recognize this card", i);
				}

				ret = -1;
				goto out;
			}

			sprintf(thread_name, "msdc_H%d_T%d", i, j);
			kthread_run(write_read_thread, &wr_data[i][j], thread_name);
			MSDC_LOG_NORMAL(pr_info," start thread: %s, at address: 0x%x", thread_name,
				wr_data[i][j].start_address);
		}
	}

out:
	MSDC_LOG_NORMAL(pr_info,"=======================[%s] end =========================\n\n", __func__);
	return ret;
}


static int msdc_help_proc_show(struct seq_file *m, void *v)
{
	seq_puts(m, "\n====================[msdc_help]=====================\n");

	seq_printf(m, "\n   LOG control:           echo %x [host_id] [debug_zone] > msdc_debug\n",
		   SD_TOOL_ZONE);
	seq_puts(m,
		 "          [debug_zone]       DMA:0x1,  CMD:0x2,  RSP:0x4,   INT:0x8,   CFG:0x10,  FUC:0x20,\n");
	seq_puts(m,
		 "                OPS:0x40, FIO:0x80, WRN:0x100, PWR:0x200, CLK:0x400, RW:0x1000, NRW:0x2000\n");
	seq_puts(m, "\n   DMA mode:\n");
	seq_printf(m,
		   "          set DMA mode:      echo %x 0 [host_id] [dma_mode] [dma_size] > msdc_debug\n",
		   SD_TOOL_DMA_SIZE);
	seq_printf(m, "          get DMA mode:      echo %x 1 [host_id] > msdc_debug\n",
		   SD_TOOL_DMA_SIZE);
	seq_puts(m, "          [dma_mode]       0:PIO, 1:DMA, 2:SIZE_DEP\n");
	seq_puts(m,
		 "          [dma_size]       valid for SIZE_DEP mode, the min size can trigger the DMA mode\n");
	seq_printf(m, "\n   SDIO profile:          echo %x [enable] [time] > msdc_debug\n",
		   SD_TOOL_SDIO_PROFILE);
	seq_puts(m, "\n   CLOCK control:\n");
	seq_printf(m, "          set clk src:       echo %x 0 [host_id] [clk_src] > msdc_debug\n",
		   SD_TOOL_CLK_SRC_SELECT);
	seq_printf(m, "          get clk src:       echo %x 1 [host_id] > msdc_debug\n",
		   SD_TOOL_CLK_SRC_SELECT);
	seq_printf(m,
		   "          [clk_src]        0:SYS PLL(26Mhz), 1:3G PLL(197Mhz), 2:AUD PLL(208Mhz)\n");
	seq_puts(m, "\n   REGISTER control:\n");
	seq_printf(m,
		   "        write register:    echo %x 0 [host_id] [register_offset] [value] > msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(m,
		   "        read register:     echo %x 1 [host_id] [register_offset] > msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(m,
		   "  write mask:  echo %x 2 [host_id] [register_offset] [start_bit] [len] [value] > msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(m,
		   "    read mask:    echo %x 3 [host_id] [register_offset] [start_bit] [len] > msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(m, "          dump all:          echo %x 4 [host_id]> msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_puts(m, "\n   DRVING control:\n");
	seq_printf(m,
		   "          set driving:       echo %x 0 [host_id] [clk_drv] [cmd_drv] [dat_drv] > msdc_debug\n",
		   SD_TOOL_SET_DRIVING);
	seq_printf(m, "          get driving:       echo %x 1 [host_id] > msdc_debug\n",
		   SD_TOOL_SET_DRIVING);
	seq_puts(m, "\n   DESENSE control:\n");
	seq_printf(m, "          write register:    echo %x 0 [value] > msdc_debug\n",
		   SD_TOOL_DESENSE);
	seq_printf(m, "        read register:     echo %x 1 > msdc_debug\n", SD_TOOL_DESENSE);
	seq_printf(m,
		   "        write mask:        echo %x 2 [start_bit] [len] [value] > msdc_debug\n",
		   SD_TOOL_DESENSE);
	seq_printf(m, "          read mask:         echo %x 3 [start_bit] [len] > msdc_debug\n",
		   SD_TOOL_DESENSE);
	seq_printf(m,
		   "\n   RW_COMPARE test:       echo %x [host_id] [compare_count] > msdc_debug\n",
		   RW_BIT_BY_BIT_COMPARE);
	seq_puts(m,
		 "        [compare_count]    how many time you want to \"write=>read=>compare\"\n");

	seq_printf(m, "\n   SMP_ON_ONE_HOST test:  echo %x ", SMP_TEST_ON_ONE_HOST);
	seq_puts(m, "[host_id] [thread_num] [compare_count] [multi_address] > msdc_debug\n");

	seq_puts(m,
		 "        [thread_num]       how many R/W comparision thread you want to run at host_id\n");
	seq_puts(m,
		 "        [compare_count]    how many time you want to \"write=>read=>compare\" in each thread\n");
	seq_puts(m,
		 "        [multi_address]    whether read/write different address in each thread, 0:No, 1:Yes\n");
	seq_printf(m,
		   "\n   SMP_ON_ALL_HOST test:  echo %x [thread_num] [compare_count] [multi_address] > msdc_debug\n",
		   SMP_TEST_ON_ALL_HOST);
	seq_puts(m,
		 "        [thread_num]       how many R/W comparision thread you want to run at each host\n");
	seq_puts(m,
		 "        [compare_count]    how many time you want to \"write=>read=>compare\" in each thread\n");
	seq_puts(m,
		 "        [multi_address]    whether read/write different address in each thread, 0:No, 1:Yes\n");
	seq_puts(m, "\n   SPEED_MODE control:\n");
	seq_printf(m,
		   "        set speed mode:    echo %x 0 [host_id] [speed_mode]", SD_TOOL_MSDC_HOST_MODE);
	seq_puts(m, "[driver_type] [max_current] [power_control] > msdc_debug\n");

	seq_printf(m, "        get speed mode:    echo %x 1 [host_id]\n",
		   SD_TOOL_MSDC_HOST_MODE);
	seq_puts(m,
		 "          [speed_mode]       0:N/A,  1:HS,  2:SDR12,  3:SDR25,  4:SDR:50,  5:SDR104,  6:DDR\n");
	seq_puts(m,
		 "          [driver_type]      0:N/A,  7:type A,  8:type B,  9:type C,  a:type D\n");
	seq_puts(m,
		 "          [max_current]      0:N/A,  b:200mA,   c:400mA,   d:600mA,   e:800mA\n");
	seq_puts(m, "          [power_control]    0:N/A,  f:disable, 10:enable\n");
	seq_printf(m, "\n   DMA viloation:         echo %x [host_id] [ops]> msdc_debug\n",
		   SD_TOOL_DMA_STATUS);
	seq_puts(m,
		 "          [ops]              0:get latest dma address,  1:start violation test\n");
	seq_puts(m, "\n   NOTE: All input data is Hex number!\n");

	seq_puts(m, "\n======================================================\n\n");

	return 0;
}

/*
 * for AHB read / write debug
 * return DMA status.
 */
int msdc_get_dma_status(int host_id)
{
	int result = -1;

	if ((host_id < 0) || (host_id >= HOST_MAX_NUM)) {
		MSDC_LOG_NORMAL(pr_info,"%s: failed to get dma status, bad host_id %d", __func__, host_id);
		return result;
	}

	if (msdc_latest_transfer_mode[host_id] == TRAN_MOD_DMA) {
		switch (msdc_latest_operation_type[host_id]) {
		case OPER_TYPE_READ:
			result = 1;	/* DMA read */
			break;

		case OPER_TYPE_WRITE:
			result = 2;	/* DMA write */
			break;

		default:
			break;
		}
	} else if (msdc_latest_transfer_mode[host_id] == TRAN_MOD_PIO) {
		result = 0;	/* PIO mode */
	} else {
		MSDC_LOG_NORMAL(pr_info,"%s: latest transfer mode error: %d", __func__,
			        msdc_latest_transfer_mode[host_id]);
	}

	return result;
}
EXPORT_SYMBOL(msdc_get_dma_status);

struct dma_addr *msdc_get_dma_address(int host_id)
{
	bd_t *bd;
	int i = 0;
	int mode = -1;
	struct msdc_host *host;
	u32 base;

	if ((host_id < 0) || (host_id >= HOST_MAX_NUM)) {
		MSDC_LOG_NORMAL(pr_info,"%s: failed to get dma status, bad host_id %d", __func__, host_id);
		return NULL;
	}

	if (!atc_msdc_host[host_id]) {
		MSDC_LOG_NORMAL(pr_info,"%s: failed to get dma status, msdc%d is not exist", __func__, host_id);
		return NULL;
	}

	host = atc_msdc_host[host_id];
	base = host->base;
	/* spin_lock(&host->lock); */
	MSDC_GET_FIELD(MSDC_DMA_CTRL, MSDC_DMA_CTRL_MODE, mode);

	if (mode == 1) {
		MSDC_LOG_NORMAL(pr_info,"Desc.DMA");
		bd = host->dma.bd;
		i = 0;

		while (i < MAX_BD_PER_GPD) {
			msdc_latest_dma_address[i].start_address = (u32) bd[i].ptr;
			msdc_latest_dma_address[i].size = bd[i].buflen;
			msdc_latest_dma_address[i].end = bd[i].eol;

			if (i > 0) {
				msdc_latest_dma_address[i - 1].next = &msdc_latest_dma_address[i];
			}

			if (bd[i].eol) {
				break;
			}

			i++;
		}
	} else if (mode == 0) {
		MSDC_LOG_NORMAL(pr_info,"Basic DMA");
		msdc_latest_dma_address[i].start_address = MSDC_READ32(MSDC_DMA_SA);
		MSDC_GET_FIELD(MSDC_DMA_CTRL, MSDC_DMA_CTRL_XFERSZ,
			       msdc_latest_dma_address[i].size);
		msdc_latest_dma_address[i].end = 1;
	}

	/* spin_unlock(&host->lock); */

	return msdc_latest_dma_address;

}
EXPORT_SYMBOL(msdc_get_dma_address);


static u32 msdc_slot;
#define SET_TUNE_READ_PARAM			(0)
#define SET_TUNE_WRITE_PARAM		(1)
static u32 msdc_tune_param_ops;	/* 0 - read, 1 - write */

/* ========== driver proc interface =========== */
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	u32 base = 0;

	struct msdc_host *host;

	host = atc_msdc_host[msdc_slot];
	base = host->base;

	/* show_tuning_params(0); */

	seq_puts(m, "\n=========================================================\n");

	seq_puts(m, "Index<0> + Id + Zone\n");
	seq_puts(m,
		 "-> PWR<9> WRN<8> | FIO<7> OPS<6> FUN<5> CFG<4> | INT<3> RSP<2> CMD<1> DMA<0>\n");
	seq_puts(m, "-> echo 0 3 0x3ff >msdc_bebug -> host[3] debug zone set to 0x3ff\n");
	seq_printf(m, "-> MSDC[0] Zone: 0x%.8x\n", msdc_debug_zone[0]);
	seq_printf(m, "-> MSDC[1] Zone: 0x%.8x\n", msdc_debug_zone[1]);
	seq_printf(m, "-> MSDC[2] Zone: 0x%.8x\n", msdc_debug_zone[2]);

	seq_puts(m, "Index<1> + ID:4|Mode:4 + DMA_SIZE\n");
	seq_puts(m, "-> 0)PIO 1)DMA 2)SIZE\n");
	seq_puts(m, "-> echo 1 22 0x200 >msdc_bebug -> host[2] size mode, dma when >= 512\n");
	seq_printf(m, "-> MSDC[0] mode<%d> size<%d>\n", drv_mode[0], dma_size[0]);
	seq_printf(m, "-> MSDC[1] mode<%d> size<%d>\n", drv_mode[1], dma_size[1]);
	seq_printf(m, "-> MSDC[2] mode<%d> size<%d>\n", drv_mode[2], dma_size[2]);

	seq_puts(m, "Index<2> + SDIO_PROFILE + TIME\n");
	seq_puts(m, "-> echo 3 1 0x1E >msdc_bebug -> enable sdio_profile, 30s\n");
	seq_printf(m, "-> SDIO_PROFILE<%d> TIME<%llu s>\n", sdio_pro_enable, sdio_pro_time);
	seq_printf(m, "-> Clokc SRC selection Host[0]<%d>\n", msdc_clock_src[0]);
	seq_printf(m, "-> Clokc SRC selection Host[1]<%d>\n", msdc_clock_src[1]);
	seq_printf(m, "-> Clokc SRC selection Host[2]<%d>\n", msdc_clock_src[2]);

	if (host->hw->tuning_method == TUNE_ALL_DAT_LINE) {
		seq_printf(m, "===== MSDC%d TUNE_ALL_DAT_LINE, Read Params =====\n", msdc_slot);
		seq_printf(m,
			   "host->hw->read_dat_latch_ck_sel [DMSDC%d_READ_DAT_LATCH_CK_SEL] = 0x%08X\n",
			   msdc_slot, host->hw->read_dat_latch_ck_sel);
		seq_printf(m,
			   "host->hw->read_ckgen_delay_sel  [DMSDC%d_READ_CKGEN_DELAY_SEL]  = 0x%08X\n",
			   msdc_slot, host->hw->read_ckgen_delay_sel);
		seq_printf(m,
			   "host->hw->read_sample_edge      [DMSDC%d_READ_SAMPL_EDGE]       = 0x%08X\n",
			   msdc_slot, host->hw->read_sample_edge);
		seq_printf(m,
			   "host->hw->read_pad_delay        [DMSDC%d_READ_PAD_DELAY]        = 0x%08X\n",
			   msdc_slot, host->hw->read_pad_delay);
		seq_printf(m, "host->hw->read_patch_bit1_wrdat_crcs = 0x%08X\n",
			   host->hw->read_patch_bit1_wrdat_crcs);
		seq_printf(m, "host->hw->read_patch_bit1_cmdrsp_ta = 0x%08X\n",
			   host->hw->read_patch_bit1_cmdrsp_ta);


		seq_printf(m, "===== MSDC%d TUNE_ALL_DAT_LINE, Write Params =====\n", msdc_slot);
		seq_printf(m,
			   "host->hw->write_dat_latch_ck_sel [DMSDC%d_WRITE_DAT_LATCH_CK_SEL] = 0x%08X\n",
			   msdc_slot, host->hw->write_dat_latch_ck_sel);
		seq_printf(m,
			   "host->hw->write_ckgen_delay_sel  [DMSDC%d_WRITE_CKGEN_DELAY_SEL]  = 0x%08X\n",
			   msdc_slot, host->hw->write_ckgen_delay_sel);
		seq_printf(m,
			   "host->hw->write_pad_delay        [DMSDC%d_WRITE_PAD_DELAY]        = 0x%08X\n",
			   msdc_slot, host->hw->write_pad_delay);
		seq_printf(m,
			   "host->hw->write_internal_delay   [DMSDC%d_WRITE_INTERNAL_DELAY]   = 0x%08X\n",
			   msdc_slot, host->hw->write_internal_delay);
		seq_printf(m,
			   "host->hw->write_sample_edge      [DMSDC%d_WRITE_SAMPLE_EDGE]      = 0x%08X\n",
			   msdc_slot, host->hw->write_sample_edge);
	} else {
		seq_printf(m, "===== MSDC%d TUNE_EACH_DAT_LINE, Read Params =====\n", msdc_slot);
		seq_printf(m,
			   "host->hw->read_ckgen_delay_sel  [DMSDC%d_READ_CKGEN_DELAY_SEL]  = 0x%08X\n",
			   msdc_slot, host->hw->read_ckgen_delay_sel);
		seq_printf(m,
			   "host->hw->read_dat_latch_ck_sel [DMSDC%d_READ_DAT_LATCH_CK_SEL] = 0x%08X\n",
			   msdc_slot, host->hw->read_dat_latch_ck_sel);
		seq_printf(m,
			   "host->hw->read_sample_edge      [DMSDC%d_READ_SAMPL_EDGE]       = 0x%08X\n",
			   msdc_slot, host->hw->read_sample_edge);
		seq_printf(m, "host->hw->read_rxdly0 = 0x%08X\n", host->hw->read_rxdly0);
		seq_printf(m, "host->hw->read_rxdly1 = 0x%08X\n", host->hw->read_rxdly1);

		seq_printf(m, "host->hw->write_patch_bit1_wrdat_crcs = 0x%08X\n",
			   host->hw->write_patch_bit1_wrdat_crcs);
		seq_printf(m, "host->hw->write_patch_bit1_cmdrsp_ta = 0x%08X\n",
			   host->hw->write_patch_bit1_cmdrsp_ta);

		seq_printf(m, "===== MSDC%d TUNE_EACH_DAT_LINE, Write Params =====\n", msdc_slot);
		seq_printf(m,
			   "host->hw->write_sample_edge [DMSDC%d_WRITE_SAMPL_EDGE] = 0x%08X\n",
			   msdc_slot, host->hw->write_sample_edge);
		seq_printf(m, "host->hw->write_patch_bit1_wrdat_crcs = 0x%08X\n",
			   host->hw->write_patch_bit1_wrdat_crcs);
		seq_printf(m, "host->hw->write_rxdly0 = 0x%08X\n", host->hw->write_rxdly0);
	}

	seq_printf(m, "MSDC_PATCH_BIT0 = 0x%08X\n", MSDC_READ32(MSDC_PATCH_BIT0));
	seq_printf(m, "MSDC_PATCH_BIT1 = 0x%08X\n", MSDC_READ32(MSDC_PATCH_BIT1));
	seq_printf(m, "MSDC_PAD_TUNE   = 0x%08X\n", MSDC_READ32(MSDC_PAD_TUNE));

	seq_puts(m, "=========================================================\n\n");

	return 0;
}

static int msdc_debug_proc_write(struct file *file, const char *buf, size_t count,
				 loff_t *data)
{
	int ret;

	int cmd, p1, p2, p3, p4, p5, p6;
	int id, zone;
	int mode, size;
	int thread_num, compare_count, multi_address;
	unsigned int base;
	unsigned int offset = 0;
	unsigned int reg_value;
	HOST_CAPS spd_mode = MODE_NULL;
	HOST_CAPS drv_type = MODE_NULL;
	HOST_CAPS current_limit = MODE_NULL;
	HOST_CAPS pw_cr = MODE_NULL;
	struct msdc_host *host = NULL;
	struct dma_addr *dma_address, *p_dma_address;
	int dma_status = -1;

	host = atc_msdc_host[msdc_slot];

	if (count == 0) {
		return -1;
	}

	if (count > 255) {
		count = 255;
	}

	ret = copy_from_user(cmd_buf, buf, count);

	if (ret < 0) {
		return -1;
	}

	cmd_buf[count] = '\0';
	MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc write: %s", cmd_buf);

	ret = sscanf(cmd_buf, "%x %x %x %x %x %x %x", &cmd, &p1, &p2, &p3, &p4, &p5, &p6);

	/*  */
	/* Set MSDC Log Debug Zone, Format: 0 (SD_TOOL_ZONE) Slot (0/1/2/3) Debug_Zone */
	/*  */
	if (cmd == SD_TOOL_ZONE) {
		id = p1;
		zone = p2;
		MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id<%d> zone<0x%.8x>", id, zone);

		if ((id >= 0) && (id <= HOST_MAX_NUM - 1)) {
			msdc_debug_zone[id] = zone;
		} else if (id == HOST_MAX_NUM) {
			msdc_debug_zone[0] = msdc_debug_zone[1] = msdc_debug_zone[2] = zone;
		} else {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when set debug zone");
		}
	}
	/*  */
	/* Before Set Tune Parameters, use this cmd to select parameters for Read or Write */
	/*  */
	else if (cmd == MSDC_TUNE_PARAM_OPS) {
		MSDC_LOG_NORMAL(pr_info,"[MSDC_TUNE_PARAM_OPS] change tune param ops from %s to %s",
			msdc_tune_param_ops ? "read" : "write", p1 ? "read" : "write");
		msdc_tune_param_ops = p1;
	}
	/*  */
	/* Set Read or Write Parameters */
	/*  */
	else if (cmd == HOST_TUNE_PARAMS) {
		MSDC_LOG_NORMAL(pr_info,"[HOST_TUNE_PARAMS] msdc%d set %s params", msdc_slot,
			msdc_tune_param_ops ? "read" : "write");

		if (host->hw->tuning_method == TUNE_ALL_DAT_LINE) {
			if (msdc_tune_param_ops == SET_TUNE_READ_PARAM) {
				host->hw->read_dat_latch_ck_sel = p1;
				host->hw->read_ckgen_delay_sel = p2;
				host->hw->read_sample_edge = p3;
				host->hw->read_pad_delay = p4;
				host->hw->read_patch_bit1_wrdat_crcs = p5;
				host->hw->read_patch_bit1_cmdrsp_ta = p6;
			} else {
				host->hw->write_dat_latch_ck_sel = p1;
				host->hw->write_ckgen_delay_sel = p2;
				host->hw->write_pad_delay = p3;
				host->hw->write_internal_delay = p4;
				host->hw->write_sample_edge = p5;
			}
		} else {
			if (msdc_tune_param_ops == SET_TUNE_READ_PARAM) {
				host->hw->read_ckgen_delay_sel = p1;
				host->hw->read_dat_latch_ck_sel = p2;
				host->hw->read_sample_edge = p3;
				host->hw->read_rxdly0 = p4;
				host->hw->read_rxdly1 = p5;
			} else {
				host->hw->write_sample_edge = p1;
				host->hw->write_patch_bit1_wrdat_crcs = p2;
				host->hw->write_rxdly0 = p3;
			}
		}
	}
	/*  */
	/* Switch Tune method, Tune all data lines or tune each data line */
	/*  */
	else if (cmd == HOST_TUNE_METHOD) {
		MSDC_LOG_NORMAL(pr_info,"[HOST_TUNE_METHOD] msdc%d change tune method from %d to %d", msdc_slot,
			host->hw->tuning_method, p1);
		host->hw->tuning_method = p1;
	}
	/*  */
	/* Select commands is applied to which MSDC host */
	/*  */
	else if (cmd == MSDC_SET_CURRENT_HOST) {
		MSDC_LOG_NORMAL(pr_info,"[MSDC_SET_CURRENT_HOST] set msdc host_id from %d to %d", msdc_slot, p1);
		msdc_slot = p1;
	}
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_DMA_SIZE) {
		id = p2;
		mode = p3;
		size = p4;

		if ((id >= 0) && (id <= HOST_MAX_NUM - 1)) {
			if (p1 == 0) {
				drv_mode[id] = mode;
				dma_size[id] = size;
			} else {
				MSDC_LOG_NORMAL(pr_info,"-> MSDC[%d] mode<%d> size<%d>", id, drv_mode[id],
					dma_size[id]);
			}
		} else {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when select mode");
		}
	}
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_SDIO_PROFILE) {
		if (p1 == 1) {	/* enable profile */
			if (gpt_enable == 0) {
				msdc_init_gpt();
				gpt_enable = 1;
			}

			sdio_pro_enable = 1;

			if (p2 == 0) {
				p2 = 1;
			}

			if (p2 >= 30) {
				p2 = 30;
			}

			sdio_pro_time = p2;
		} else if (p1 == 0) {
			/* todo */
			sdio_pro_enable = 0;
		} else {
			MSDC_LOG_NORMAL(pr_info,"parameter p1 is invalid");
		}
	}
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_CLK_SRC_SELECT) {
		id = p2;

		if ((id >= 0) && (id < HOST_MAX_NUM)) {
			if (p1 == 0) {
				if (p3 >= 0 && p3 < CLK_SRC_MAX_NUM) {
					msdc_clock_src[id] = p3;
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc%d's pll source changed to %d",
					 		id, msdc_clock_src[id]);
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]to enable the above settings,");
					MSDC_LOG_NORMAL(pr_info,"please suspend and resume the device again");
				} else
					MSDC_LOG_NORMAL(pr_info,
					"[MSDC_DBG] invalide clock src id:%d, check /proc/msdc_help", p3);

			} else if (p1 == 1) {
				switch (id) {
				case 0:
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc%d's pll source is %d", id,
						msdc_clock_src[id]);
					break;

				case 1:
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc%d's pll source is %d", id,
						msdc_clock_src[id]);
					break;

				case 2:
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc%d's pll source is %d", id,
						msdc_clock_src[id]);
					break;

				case 3:
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc%d's pll source is %d", id,
						msdc_clock_src[id]);
					break;
				}
			} else {
				MSDC_LOG_NORMAL(pr_info,"[%s] p1 is invalid", __func__);
			}
		} else {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when select clock source");
		}
	}
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_REG_ACCESS) {
		id = p2;

		if (id >= HOST_MAX_NUM || id < 0) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when modify msdc reg");
		} else {
			offset = (unsigned int)p3;

			if (offset > 0x104U) {
				MSDC_LOG_NORMAL(pr_info, "[MSDC_DBG]register offset error when modify msdc reg");
				return count;
			}

			if (id == 0) {
				base = MSDC_0_BASE;
			}

			if (id == 1) {
				base = MSDC_1_BASE;
			}

			if (id == 2) {
				base = MSDC_2_BASE;
			}

			if (((offset == 0x18U) || (offset == 0x1CU)) && (p1 != 4U)) {
				MSDC_LOG_NORMAL(pr_info, "[MSDC_DBG]Err: Accessing TXDATA and RXDATA is forbidden");
				return count;
			}

			if (p1 == 0) {
				reg_value = p4;

				if ((offset == 0xE0U) || (offset == 0xE4U) || (offset == 0xE8U)) {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Err: Bypass PAD_CTL");
				} else {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG][MSDC Reg]Original:0x%x+0x%x (0x%x)",
					 base, offset, MSDC_READ32(base + offset));
					MSDC_WRITE32(base + offset, reg_value);
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG][MSDC Reg]Modified:0x%x+0x%x (0x%x)",
					 base, offset, MSDC_READ32(base + offset));
				}
			} else if (p1 == 1)
				if ((offset == 0xE0U) || (offset == 0xE4U) || (offset == 0xE8U)) {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG][MSDC Reg]Reg:0x%x+0x%x (0x%x)",
						base, offset, MSDC_READ32(base + offset));
				} else {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG][MSDC Reg]Reg:0x%x+0x%x (0x%x)",
						base, offset, MSDC_READ32(base + offset));
				}
			else if (p1 == 2)
				if ((offset == 0xE0U) || (offset == 0xE4U) || (offset == 0xE8U)) {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Err: Bypass PAD_CTL");
				} else {
					msdc_set_field(base + offset, p4, p5, p6);
				}

			else if (p1 == 3)
				if ((offset == 0xE0U) || (offset == 0xE4U) || (offset == 0xE8U)) {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Err: Bypass PAD_CTL");
				} else {
					msdc_get_field(base + offset, p4, p5, p6);
				}
			else if (p1 == 4) {
				msdc_dump_reg(base);
			}
		}
	}

	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_SET_DRIVING) {
		id = p2;

		if ((id >= HOST_MAX_NUM) || (id < 0)) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when modify msdc driving");
		} else {
			host = atc_msdc_host[id];

			if (p1 == 0) {
				if (((unsigned char)p3 > 0x3FU) || ((unsigned char)p4 > 0x3FU)
				    || ((unsigned char)p5 > 0x3FU)) {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Some drving value was not right(correct:0~7)");
				} else {
#ifndef FPGA_PLATFORM

					if (p6 == 0x33) {
						host->hw->clk_drv = (unsigned char)p3;
						host->hw->cmd_drv = (unsigned char)p4;
						host->hw->dat_drv = (unsigned char)p5;
						msdc_set_driving(host, host->hw, 0);
					} else if (p6 == 0x18) {
						host->hw->clk_drv_sd_18 = (unsigned char)p3;
						host->hw->cmd_drv_sd_18 = (unsigned char)p4;
						host->hw->dat_drv_sd_18 = (unsigned char)p5;
						msdc_set_driving(host, host->hw, 1);
					}

#endif
					MSDC_LOG_NORMAL(pr_info, "[MSDC_SET_DRIVING]clk_drv=%d, cmd_drv=%d, dat_drv=%d",
					 p3, p4, p5);
				}
			} else if (p1 == 1) {
				/* pr_info("[MSDC_DBG]msdc%d: clk_drv=%d, cmd_drv=%d, dat_drv=%d\n",
				id, msdc_drv_mode[id].clk_drv, msdc_drv_mode[id].cmd_drv, msdc_drv_mode[id].dat_drv); */

				MSDC_LOG_NORMAL(pr_info,"[MSDC_GET_DRIVING]msdc%d: clk_drv=0x%02X, cmd_drv=0x%02X, dat_drv=0x%02X",
				 id, host->hw->clk_drv, host->hw->cmd_drv, host->hw->dat_drv);
			}
		}
	}

	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_ENABLE_SLEW_RATE) {
		id = p1;

		if ((id >= HOST_MAX_NUM) || (id < 0)) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when modify msdc sr");
		} else {
			host = atc_msdc_host[id];

			if (((unsigned char)p2 > 1U) || ((unsigned char)p3 > 1U) || ((unsigned char)p4 > 1U))
				MSDC_LOG_NORMAL(pr_info, "[MSDC_DBG]Some sr value was not right(correct:0(disable),1(enable))");
			else {
#ifndef FPGA_PLATFORM
				msdc_set_sr(host, p2, p3, p4);
#endif
				MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]clk_sr=%d, cmd_sr=%d, dat_sr=%d", p2,
					p3, p4);
			}
		}
	}
	
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_SET_RDTDSEL) {
		id = p1;

		if ((id >= HOST_MAX_NUM) || (id < 0)) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when modify msdc sr");
		} else {
			host = atc_msdc_host[id];

			if ((p2 != 0) && (p2 != 1))
				MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Some rd/td seletec was not right(set rd:1,set td:0)");
			else if (((p2 == 0) && ((unsigned char)p3 > 0xFU)) || 
				((p2 == 1) && ((unsigned char)p3 > 0x3FU))) {
				MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Some rd/td value was not right(rd:0~0x3F,td:0~0xF)");
			} else {
#ifndef FPGA_PLATFORM
				msdc_set_rdtdsel_dbg(host, p2, p3);
#endif
				MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]rd/td=%d, value=%d", p2, p3);
			}
		}
	}
	
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_ENABLE_SMT) {
		id = p1;
		host = atc_msdc_host[id];

		if ((id >= HOST_MAX_NUM) || (id < 0)) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when enable/disable msdc smt");
		} else {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]smt=%d", p2);
		}
	}
	
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_SET_TUNE) {
		id = p1;

		if ((id >= HOST_MAX_NUM) || (id < 0)) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when modify msdc sr");
		} else {
			host = atc_msdc_host[id];

			if ((p2 != 0) && (p2 != 1))
				MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]Some read/write cmd was not right(read:1,set:0)");
			else if (p2 == 1) {
				;
			} else if (p2 == 0) {
				if ((unsigned char)p3 > 0xFU)
					MSDC_LOG_NORMAL(pr_info, "[MSDC_DBG]Some tune value was not right(0~0xF");
				else {
					MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]set tune value=%d", p3);
				}
			}
		}
	}
	/*  */
	/*  */
	/*  */
	else if (cmd == RW_BIT_BY_BIT_COMPARE) {
		id = p1;
		compare_count = p2;

		if ((id >= HOST_MAX_NUM) || (id < 0)) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]: bad host id: %d", id);
			return 0;
		}

		if (compare_count < 0) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]: bad compare count: %d", compare_count);
			return 0;
		}

		if (id == 0) {	/* for msdc0 */
#ifdef MTK_EMMC_SUPPORT
			/* test the address 0 of eMMC card, since there a little memory. */
			sd_multi_rw_compare(0, COMPARE_ADDRESS_MMC, compare_count);
#else
			/* test a larger address of SD card */
			sd_multi_rw_compare(0, COMPARE_ADDRESS_SD, compare_count);
#endif
		} else {
			sd_multi_rw_compare(id, COMPARE_ADDRESS_SD, compare_count);
		}

	} else if (cmd == SMP_TEST_ON_ONE_HOST) {
		id = p1;
		thread_num = p2;
		compare_count = p3;
		multi_address = p4;
		smp_test_on_one_host(thread_num, id, compare_count, multi_address);
	} else if (cmd == SMP_TEST_ON_ALL_HOST) {
		thread_num = p1;
		compare_count = p2;
		multi_address = p3;
		smp_test_on_all_host(thread_num, compare_count, multi_address);
	}
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_MSDC_HOST_MODE) {
		id = p2;

		if ((id >= HOST_MAX_NUM) || (id < 0)) {
			MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc host_id error when modify msdc host mode");
		} else {
			if (p1 == 0) {
				if ((p3 <= UHS_DDR50) && (p3 >= SDHC_HIGHSPEED)) {
					spd_mode = p3;
				}

				if ((p4 <= DRIVER_TYPE_D) && (p4 >= DRIVER_TYPE_A)) {
					drv_type = p4;
				}

				if ((p5 <= MAX_CURRENT_800) && (p5 >= MAX_CURRENT_200)) {
					current_limit = p5;
				}

				if ((p6 <= SDXC_POWER_CONTROL) && (p6 >= SDXC_NO_POWER_CONTROL)) {
					pw_cr = p6;
				}

				if (spd_mode != 0) {
					switch (spd_mode) {
					case SDHC_HIGHSPEED:
						msdc_host_mode[id] |=
							MMC_CAP_MMC_HIGHSPEED | MMC_CAP_SD_HIGHSPEED;
						msdc_host_mode[id] &=
							(~MMC_CAP_UHS_SDR12) & (~MMC_CAP_UHS_SDR25) &
							(~MMC_CAP_UHS_SDR50) & (~MMC_CAP_UHS_DDR50) &
							(~MMC_CAP_1_8V_DDR) & (~MMC_CAP_UHS_SDR104);
						msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support Highspeed");
						break;

					case UHS_SDR12:
						msdc_host_mode[id] |= MMC_CAP_UHS_SDR12;
						msdc_host_mode[id] &=
							(~MMC_CAP_UHS_SDR25) & (~MMC_CAP_UHS_SDR50) &
							(~MMC_CAP_UHS_DDR50) & (~MMC_CAP_1_8V_DDR) &
							(~MMC_CAP_UHS_SDR104);
						msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support UHS-SDR12");
						break;

					case UHS_SDR25:
						msdc_host_mode[id] |=
							MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25;
						msdc_host_mode[id] &=
							(~MMC_CAP_UHS_SDR50) & (~MMC_CAP_UHS_DDR50) &
							(~MMC_CAP_1_8V_DDR) & (~MMC_CAP_UHS_SDR104);
						msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support UHS-SDR25");
						break;

					case UHS_SDR50:
						msdc_host_mode[id] |=
							MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |
							MMC_CAP_UHS_SDR50;
						msdc_host_mode[id] &=
							(~MMC_CAP_UHS_DDR50) & (~MMC_CAP_1_8V_DDR) &
							(~MMC_CAP_UHS_SDR104);
						msdc_host_mode2[id] &= ~MMC_CAP2_HS200_1_8V_SDR;
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support UHS-SDR50");
						break;

					case UHS_SDR104:
						msdc_host_mode[id] |=
							MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |
							MMC_CAP_UHS_SDR50 | MMC_CAP_UHS_SDR104;
						msdc_host_mode2[id] |= MMC_CAP2_HS200_1_8V_SDR;
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support UHS-SDR104");
						break;

					case UHS_DDR50:
						msdc_host_mode[id] |=
							MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |
							MMC_CAP_UHS_DDR50 | MMC_CAP_1_8V_DDR;
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support UHS-DDR50");
						break;

					default:
						break;
					}
				}

				if (drv_type != 0) {
					switch (drv_type) {
					case DRIVER_TYPE_A:
						msdc_host_mode[id] |= MMC_CAP_DRIVER_TYPE_A;
						msdc_host_mode[id] &=
							(~MMC_CAP_DRIVER_TYPE_C) &
							(~MMC_CAP_DRIVER_TYPE_D);
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support DRIVING TYPE A");
						break;

					case DRIVER_TYPE_B:
						msdc_host_mode[id] &=
							(~MMC_CAP_DRIVER_TYPE_A) &
							(~MMC_CAP_DRIVER_TYPE_C) &
							(~MMC_CAP_DRIVER_TYPE_D);
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support DRIVING TYPE B");
						break;

					case DRIVER_TYPE_C:
						msdc_host_mode[id] |= MMC_CAP_DRIVER_TYPE_C;
						msdc_host_mode[id] &=
							(~MMC_CAP_DRIVER_TYPE_A) &
							(~MMC_CAP_DRIVER_TYPE_D);
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support DRIVING TYPE C");
						break;

					case DRIVER_TYPE_D:
						msdc_host_mode[id] |= MMC_CAP_DRIVER_TYPE_D;
						msdc_host_mode[id] &=
							(~MMC_CAP_DRIVER_TYPE_A) &
							(~MMC_CAP_DRIVER_TYPE_C);
						MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]host will support DRIVING TYPE D");
						break;

					default:
						break;
					}
				}

				if (current_limit != 0) {
#if 0				/* cause MMC_CAP_MAX??? and MMC_CAP_SET??? removed from linux3.6 */

					switch (current_limit) {
					case MAX_CURRENT_200:
						msdc_host_mode[id] |= MMC_CAP_MAX_CURRENT_200;
						msdc_host_mode[id] &=
							(~MMC_CAP_MAX_CURRENT_400) &
							(~MMC_CAP_MAX_CURRENT_600) &
							(~MMC_CAP_MAX_CURRENT_800);
						pr_info
						("[MSDC_DBG]host will support MAX_CURRENT_200\n");
						break;

					case MAX_CURRENT_400:
						msdc_host_mode[id] |=
							MMC_CAP_MAX_CURRENT_200 |
							MMC_CAP_MAX_CURRENT_400;
						msdc_host_mode[id] &=
							(~MMC_CAP_MAX_CURRENT_600) &
							(~MMC_CAP_MAX_CURRENT_800);
						pr_info
						("[MSDC_DBG]host will support MAX_CURRENT_400\n");
						break;

					case MAX_CURRENT_600:
						msdc_host_mode[id] |=
							MMC_CAP_MAX_CURRENT_200 |
							MMC_CAP_MAX_CURRENT_400 |
							MMC_CAP_MAX_CURRENT_600;
						msdc_host_mode[id] &= (~MMC_CAP_MAX_CURRENT_800);
						pr_info
						("[MSDC_DBG]host will support MAX_CURRENT_600\n");
						break;

					case MAX_CURRENT_800:
						msdc_host_mode[id] |=
							MMC_CAP_MAX_CURRENT_200 |
							MMC_CAP_MAX_CURRENT_400 |
							MMC_CAP_MAX_CURRENT_600 |
							MMC_CAP_MAX_CURRENT_800;
						pr_info
						("[MSDC_DBG]host will support MAX_CURRENT_800\n");
						break;

					default:
						break;
					}

#endif
				}

				if (pw_cr != 0)
#if 0
					switch (pw_cr) {
					case SDXC_NO_POWER_CONTROL:
						msdc_host_mode[id] &=
							(~MMC_CAP_SET_XPC_330) & (~MMC_CAP_SET_XPC_300)
							& (~MMC_CAP_SET_XPC_180);
						pr_info
						("[MSDC_DBG]host will not support SDXC power control\n");
						break;

					case SDXC_POWER_CONTROL:
						msdc_host_mode[id] |=
							MMC_CAP_SET_XPC_330 | MMC_CAP_SET_XPC_300 |
							MMC_CAP_SET_XPC_180;
						pr_info
						("[MSDC_DBG]host will support SDXC power control\n");
						break;

					default:
						break;
					}

#endif
				MSDC_LOG_NORMAL(pr_info,"[MSDC]to enable the above settings, please suspend and resume the again\n");
			} else {
				MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc[%d] supports:\n", id);
				{
					pr_info("[MSDC][MSDC_DBG]      speed mode: ");

					if ((msdc_host_mode[id] & MMC_CAP_MMC_HIGHSPEED)
					    || (msdc_host_mode[id] & MMC_CAP_SD_HIGHSPEED)) {
						pr_info("HS, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_UHS_SDR12) {
						pr_info("SDR12, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_UHS_SDR25) {
						pr_info("SDR25, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_UHS_SDR50) {
						pr_info("SDR50, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_UHS_SDR104) {
						pr_info("SDR104, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_UHS_DDR50) {
						pr_info("DDR50 ");
					}

					if (!
					    (msdc_host_mode[id] &
					     (MMC_CAP_MMC_HIGHSPEED | MMC_CAP_SD_HIGHSPEED |
					      MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |
					      MMC_CAP_UHS_SDR50 | MMC_CAP_UHS_SDR104 |
					      MMC_CAP_UHS_DDR50))) {
						pr_info("N/A");
					}

					pr_info("\n");
				}
				{
					pr_info("[MSDC][MSDC_DBG]      driver_type: ");

					if (msdc_host_mode[id] & MMC_CAP_DRIVER_TYPE_A) {
						pr_info("A, ");
					}

					pr_info("B, ");

					if (msdc_host_mode[id] & MMC_CAP_DRIVER_TYPE_C) {
						pr_info("C, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_DRIVER_TYPE_D) {
						pr_info("D, ");
					}

					pr_info("\n");
				}
				{
#if 0
					pr_info("[MSDC_DBG]      current limit: ");

					if (msdc_host_mode[id] & MMC_CAP_MAX_CURRENT_200) {
						pr_info("200mA, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_MAX_CURRENT_400) {
						pr_info("400mA, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_MAX_CURRENT_600) {
						pr_info("600mA, ");
					}

					if (msdc_host_mode[id] & MMC_CAP_MAX_CURRENT_800) {
						pr_info("800mA, ");
					}

					if (!
					    (msdc_host_mode[id] &
					     (MMC_CAP_MAX_CURRENT_200 | MMC_CAP_MAX_CURRENT_400 |
					      MMC_CAP_MAX_CURRENT_600 | MMC_CAP_MAX_CURRENT_800))) {
						pr_info("N/A");
					}

					pr_info("\n");
#endif
				}
				{
#if 0
					pr_info("[MSDC_DBG]      power control: ");

					if (msdc_host_mode[id] & MMC_CAP_SET_XPC_330) {
						pr_info("3.3v ");
					}

					if (msdc_host_mode[id] & MMC_CAP_SET_XPC_300) {
						pr_info("3v ");
					}

					if (msdc_host_mode[id] & MMC_CAP_SET_XPC_180) {
						pr_info("1.8v ");
					}

					if (!
					    (msdc_host_mode[id] &
					     (MMC_CAP_SET_XPC_330 | MMC_CAP_SET_XPC_300 |
					      MMC_CAP_SET_XPC_180))) {
						pr_info("N/A");
					}

					pr_info("\n");
#endif
				}
			}
		}
	}
	/*  */
	/*  */
	/*  */
	else if (cmd == SD_TOOL_DMA_STATUS) {
		id = p1;

		if (p2 == 0) {
			dma_status = msdc_get_dma_status(id);
			pr_info("[MSDC]>>>> msdc%d: dma_status=%d, ", id, dma_status);

			if (dma_status == 0) {
				pr_info("DMA mode is disabled Now\n");
			} else if (dma_status == 1) {
				pr_info("Write data from SD to DRAM within DMA mode\n");
			} else if (dma_status == 2) {
				pr_info("Write data from DRAM to SD within DMA mode\n");
			} else if (dma_status == -1) {
				pr_info("No data transaction or the device is not present until now\n");
			}

			if (dma_status > 0) {
				dma_address = msdc_get_dma_address(id);

				if (!dma_address) {
					MSDC_LOG_NORMAL(pr_info,">>>> msdc%d: BD count=0", id);
					return -1;
				}

				MSDC_LOG_NORMAL(pr_info,">>>> msdc%d:", id);
				p_dma_address = dma_address;

				while (p_dma_address) {
					MSDC_LOG_NORMAL(pr_info,">>>>     addr=0x%x, size=%d",
						p_dma_address->start_address,
						p_dma_address->size);

					if (p_dma_address->end) {
						break;
					}

					p_dma_address = p_dma_address->next;
				}
			}
		} else if (p2 == 1) {
			MSDC_LOG_NORMAL(pr_info,">>>> msdc%d: start dma violation test", id);
			g_dma_debug[id] = 1;
			sd_multi_rw_compare(id, COMPARE_ADDRESS_SD, 3);
		}
	}

	/*  */
	/*  */
	/*  */
	else if (cmd == MMC_REGISTER_READ) {
		MSDC_LOG_NORMAL(pr_info,"p1 = 0x%x", p1);

		/* get the regster value, must know some register can't be read */
		MSDC_LOG_NORMAL(pr_info,"Regiser: 0x%x = 0x%08X\n", p1, MSDC_READ32(p1));
	}

	/*  */
	/*  */
	/*  */
	else if (cmd == MMC_REGISTER_WRITE) {
		MSDC_LOG_NORMAL(pr_info,"Addr = 0x%x, Value = 0x%08X\n", p1, p2);

		/* write the regster value, must know some register can't be write */
		MSDC_WRITE32(p1, p2);
		MSDC_LOG_NORMAL(pr_info,"Regiser: 0x%x = 0x%08X\n", p1, MSDC_READ32(p1));
	}

	/*  */
	/*  */
	/*  */
#ifdef MTK_IO_PERFORMANCE_DEBUG
	else if (cmd == MMC_PERF_DEBUG) {
		/* 1 enable; 0 disable */
		g_mtk_mmc_perf_dbg = p1;

		g_mtk_mmc_dbg_range = p2;

		if (2 == g_mtk_mmc_dbg_range) {
			g_dbg_range_start = p3;
			g_dbg_range_end = p3 + p4;
			g_check_read_write = p5;
		}

		MSDC_LOG_NORMAL(pr_info,"g_mtk_mmc_perf_dbg = 0x%x, g_mtk_mmc_dbg_range = 0x%x, start = 0x%x, end = 0x%x\n",
		 g_mtk_mmc_perf_dbg, g_mtk_mmc_dbg_range, g_dbg_range_start, g_dbg_range_end);
	} else if (cmd == MMC_PERF_DEBUG_PRINT) {
		int i, j, k, num = 0;

		if (p1 == 0) {
			g_mtk_mmc_clear = 0;
			return count;
		}

		MSDC_LOG_NORMAL(pr_info,"msdc g_dbg_req_count<%d>\n", g_dbg_req_count);

		for (i = 1; i <= g_dbg_req_count; i++) {
			/* */
			MSDC_LOG_NORMAL(pr_info,"anslysis: %s 0x%x %d block, PGh %d\n",
				(g_check_read_write == 18 ? "read" : "write"),
				(unsigned int)g_mmcqd_buf[i][298], (unsigned int)g_mmcqd_buf[i][299],
				(unsigned int)(g_mmcqd_buf[i][297] * 2));

			if (g_check_read_write == 18) {
				for (j = 1; j <= g_mmcqd_buf[i][296] * 2; j++) {
					MSDC_LOG_NORMAL(pr_info,"page %d:\n", num + 1);

					for (k = 0; k < 5; k++) {
						MSDC_LOG_NORMAL(pr_info,"%d %llu\n", k, g_req_buf[num][k]);
					}

					num += 1;
				}
			}

			MSDC_LOG_NORMAL(pr_info,"-------------------------------------------\n");

			for (j = 0; j < sizeof(g_time_mark) / sizeof(char *); j++) {
				MSDC_LOG_NORMAL(pr_info,"%d. %llu %s\n", j, g_mmcqd_buf[i][j],
					g_time_mark[j]);
			}

			MSDC_LOG_NORMAL(pr_info,"===========================================\n");
		}

		if (g_check_read_write == 25) {
			MSDC_LOG_NORMAL(pr_info,"msdc g_dbg_write_count<%d>\n", g_dbg_write_count);

			for (i = 1; i <= g_dbg_write_count; i++) {
				MSDC_LOG_NORMAL(pr_info,"********************************************\n");
				MSDC_LOG_NORMAL(pr_info,"write count: %llu\n", g_req_write_count[i]);

				for (j = 0; j < sizeof(g_time_mark_vfs_write) / sizeof(char *); j++)
					MSDC_LOG_NORMAL(pr_info,"%d. %llu %s\n", j, g_req_write_buf[i][j],
						g_time_mark_vfs_write[j]);
			}

			MSDC_LOG_NORMAL(pr_info,"********************************************\n");
		}

		g_mtk_mmc_clear = 0;
	}

#endif
	/*  */
	/*  */
	/*  */
#ifdef MTK_MMC_PERFORMANCE_TEST
	else if (cmd == MMC_PERF_TEST) {
		/* 1 enable; 0 disable */
		g_mtk_mmc_perf_test = p1;
	}

#endif
	else if (cmd == MMC_WP_OPERATION) {
		if(p2 == 0x55) {
			if(p1 == 1) {
				if(!mmc_clear_wp_and_save())
					MSDC_LOG_NORMAL(pr_info, "[WP] clear wp success\n");
				else
					MSDC_LOG_NORMAL(pr_info, "[WP] clear wp fail\n");
			}
			else if(p1 == 2) {
				if(!mmc_restore_wp())
					MSDC_LOG_NORMAL(pr_info, "[WP] restore wp success\n");
				else
					MSDC_LOG_NORMAL(pr_info, "[WP] restore wp fail\n");
			}
		}
	}
	return count;
}

static int msdc_tune_flag_proc_read_show(struct seq_file *m, void *data)
{
	seq_printf(m, "0x%X\n", sdio_tune_flag);
	return 0;
}

static int msdc_debug_proc_read_FT_show(struct seq_file *m, void *data)
{
	unsigned int base;
	unsigned char cmd_edge;
	unsigned char data_edge;
	unsigned char clk_drv1, clk_drv2, cmd_drv1, cmd_drv2, dat_drv1, dat_drv2;
	u32 cur_rxdly0;
	u8 u8_dat0, u8_dat1, u8_dat2, u8_dat3;
	u8 u8_wdat, u8_cmddat;
	u8 u8_DDLSEL;

#if defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT)

	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 0) {
		base = MSDC_0_BASE;
		msdc_id = 0;
	} else if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 1) {
		base = MSDC_1_BASE;
		msdc_id = 1;
	} else if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 2) {
		base = MSDC_2_BASE;
		msdc_id = 2;
	}

#else
	seq_puts(m, "\n=========================================\n");
	seq_puts(m, "There is no WCN SDIO SLOT.\n");
	seq_puts(m, "=========================================\n\n");
	return 0;
#endif

	MSDC_GET_FIELD((base + 0x04), MSDC_IOCON_RSPL, cmd_edge);
	MSDC_GET_FIELD((base + 0x04), MSDC_IOCON_DSPL, data_edge);

	/* Max Xia Marked, for AC8317, SMT/Driving are not tuning in those registers */
	/* MSDC_GET_FIELD((base + 0xe0), MSDC_PAD_CTL0_CLKDRVN, clk_drv1); */
	/* MSDC_GET_FIELD((base + 0xe0), MSDC_PAD_CTL0_CLKDRVP, clk_drv2); */

	/* MSDC_GET_FIELD((base + 0xe4), MSDC_PAD_CTL1_CMDDRVN, cmd_drv1); */
	/* MSDC_GET_FIELD((base + 0xe4), MSDC_PAD_CTL1_CMDDRVP, cmd_drv2); */

	/* MSDC_GET_FIELD((base + 0xe8), MSDC_PAD_CTL2_DATDRVN, dat_drv1); */
	/* MSDC_GET_FIELD((base + 0xe8), MSDC_PAD_CTL2_DATDRVP, dat_drv2); */

	MSDC_GET_FIELD(MSDC_IOCON, MSDC_IOCON_DDLSEL, u8_DDLSEL);
	cur_rxdly0 = MSDC_READ32(MSDC_DAT_RDDLY0);

	if (MSDC_READ32(MSDC_ECO_VER) >= 4) {
		u8_dat0 = (cur_rxdly0 >> 24) & 0x1F;
		u8_dat1 = (cur_rxdly0 >> 16) & 0x1F;
		u8_dat2 = (cur_rxdly0 >> 8) & 0x1F;
		u8_dat3 = (cur_rxdly0 >> 0) & 0x1F;
	} else {
		u8_dat0 = (cur_rxdly0 >> 0) & 0x1F;
		u8_dat1 = (cur_rxdly0 >> 8) & 0x1F;
		u8_dat2 = (cur_rxdly0 >> 16) & 0x1F;
		u8_dat3 = (cur_rxdly0 >> 24) & 0x1F;
	}

	MSDC_GET_FIELD((base + 0xec), MSDC_PAD_TUNE_DATWRDLY, u8_wdat);
	MSDC_GET_FIELD((base + 0xec), MSDC_PAD_TUNE_CMDRRDLY, u8_cmddat);

	seq_puts(m, "\n=========================================\n");

#if defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT)
	seq_printf(m, "(1) WCN SDIO SLOT is at msdc<%d>\n", CONFIG_MTK_WCN_CMB_SDIO_SLOT);
#endif

	seq_puts(m, "-----------------------------------------\n");
	seq_puts(m, "(2) clk settings\n");
	seq_puts(m, "mt6589 only using internal clock\n");

	seq_puts(m, "-----------------------------------------\n");
	seq_puts(m, "(3) settings of driving current\n");

	if ((clk_drv1 == clk_drv2) && (cmd_drv1 == cmd_drv2) && (dat_drv1 == dat_drv2) &&
	    (clk_drv2 == cmd_drv1) && (cmd_drv2 == dat_drv1)) {
		seq_printf(m, "driving current is <%d>\n", clk_drv1);
	} else
		seq_printf(m, "clk_drv1<%d>  clk_drv2<%d>  cmd_drv1<%d>  cmd_drv2<%d>  dat_drv1<%d>  dat_drv2<%d>\n",
			   clk_drv1, clk_drv2, cmd_drv1, cmd_drv2, dat_drv1, dat_drv2);

	seq_puts(m, "-----------------------------------------\n");
	seq_puts(m, "(4) edge settings\n");

	if (cmd_edge) {
		seq_puts(m, "cmd_edge is falling\n");
	} else {
		seq_puts(m, "cmd_edge is rising\n");
	}

	if (data_edge) {
		seq_puts(m, "data_edge is falling\n");
	} else {
		seq_puts(m, "data_edge is rising\n");
	}

	seq_puts(m, "-----------------------------------------\n");
	seq_puts(m, "(5) data delay info\n");
	seq_printf(m, "Read (MSDC_DAT_RDDLY0) is <0x%x> and (MSDC_IOCON_DDLSEL) is <0x%x>\n",
		   cur_rxdly0, u8_DDLSEL);
	seq_printf(m, "data0<0x%x>  data1<0x%x>  data2<0x%x>  data3<0x%x>\n",
		   u8_dat0, u8_dat1, u8_dat2, u8_dat3);
	seq_printf(m, "Write is <0x%x>\n", u8_wdat);
	seq_printf(m, "Cmd is <0x%x>\n", u8_cmddat);
	seq_puts(m, "=========================================\n\n");

	return 0;
}

static int msdc_debug_proc_write_FT(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
	int ret;

	int i_case = 0, i_par1 = -1, i_par2 = -1, i_clk = 0, i_driving = 0, i_edge = 0, i_data =
			0, i_delay = 0;
	u32 cur_rxdly0;
	u8 u8_dat0, u8_dat1, u8_dat2, u8_dat3;
	unsigned int base;

	if (count == 0) {
		return -1;
	}

	if (count > 255) {
		count = 255;
	}

	ret = copy_from_user(cmd_buf, buf, count);

	if (ret < 0) {
		return -1;
	}

	cmd_buf[count] = '\0';
	MSDC_LOG_NORMAL(pr_info,"[MSDC_DBG]msdc Write %s\n", cmd_buf);

	ret = sscanf(cmd_buf, "%d %d %d ", &i_case, &i_par1, &i_par2);

	if (i_par2 == -1) {
		return -1;
	}

	MSDC_LOG_NORMAL(pr_info,"i_case=%d i_par1=%d i_par2=%d\n", i_case, i_par1, i_par2);

#if defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT)

	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 0) {
		base = MSDC_0_BASE;
	}

	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 1) {
		base = MSDC_1_BASE;
	}

	if (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 2) {
		base = MSDC_2_BASE;
	}

#else
	return -1;
#endif

	if (i_case == 1) {	/* set clk */
		if (!((i_par1 == 0) || (i_par1 == 1))) {
			return -1;
		}

		i_clk = i_par1;

		/* MSDC_SET_FIELD(MSDC_PATCH_BIT0, MSDC_PATCH_BIT_CKGEN_CK, i_clk); */

		MSDC_LOG_NORMAL(pr_info,"i_clk=%d\n", i_clk);
	} else if (i_case == 2) {	/* set driving current */
		if (!((i_par1 >= 0) && (i_par1 <= 7))) {
			return -1;
		}

		i_driving = i_par1;

		/* Max Xia Marked, for AC8317, SMT/Driving are not tuning in those registers */
		/* MSDC_SET_FIELD((base + 0xe0), MSDC_PAD_CTL0_CLKDRVN, i_driving); */
		/* MSDC_SET_FIELD((base + 0xe0), MSDC_PAD_CTL0_CLKDRVP, i_driving); */

		/* MSDC_SET_FIELD((base + 0xe4), MSDC_PAD_CTL1_CMDDRVN, i_driving); */
		/* MSDC_SET_FIELD((base + 0xe4), MSDC_PAD_CTL1_CMDDRVP, i_driving); */

		/* MSDC_SET_FIELD((base + 0xe8), MSDC_PAD_CTL2_DATDRVN, i_driving); */
		/* MSDC_SET_FIELD((base + 0xe8), MSDC_PAD_CTL2_DATDRVP, i_driving); */

		MSDC_LOG_NORMAL(pr_info,"i_driving=%d\n", i_driving);
	} else if (i_case == 3) {	/* set data delay */
		if (!((i_par1 >= 0) && (i_par1 <= 3))) {
			return -1;
		}

		if (!((i_par2 >= 0) && (i_par2 <= 31))) {
			return -1;
		}

		i_data = i_par1;
		i_delay = i_par2;

		cur_rxdly0 = MSDC_READ32(MSDC_DAT_RDDLY0);

		if (MSDC_READ32(MSDC_ECO_VER) >= 4) {
			u8_dat0 = (cur_rxdly0 >> 24) & 0x1F;
			u8_dat1 = (cur_rxdly0 >> 16) & 0x1F;
			u8_dat2 = (cur_rxdly0 >> 8) & 0x1F;
			u8_dat3 = (cur_rxdly0 >> 0) & 0x1F;
		} else {
			u8_dat0 = (cur_rxdly0 >> 0) & 0x1F;
			u8_dat1 = (cur_rxdly0 >> 8) & 0x1F;
			u8_dat2 = (cur_rxdly0 >> 16) & 0x1F;
			u8_dat3 = (cur_rxdly0 >> 24) & 0x1F;
		}

		if (i_data == 0) {
			u8_dat0 = i_delay;
		} else if (i_data == 1) {
			u8_dat1 = i_delay;
		} else if (i_data == 2) {
			u8_dat2 = i_delay;
		} else if (i_data == 3) {
			u8_dat3 = i_delay;
		} else if (i_data == 4) { /* write data */
			MSDC_SET_FIELD((base + 0xec), MSDC_PAD_TUNE_DATWRDLY, i_delay);
		} else if (i_data == 5) { /* cmd data */
			MSDC_SET_FIELD((base + 0xec), MSDC_PAD_TUNE_CMDRRDLY, i_delay);
		} else {
			return -1;
		}

		if (MSDC_READ32(MSDC_ECO_VER) >= 4) {
			cur_rxdly0 = ((u8_dat0 & 0x1F) << 24) | ((u8_dat1 & 0x1F) << 16) |
				     ((u8_dat2 & 0x1F) << 8) | ((u8_dat3 & 0x1F) << 0);
		} else {
			cur_rxdly0 = ((u8_dat3 & 0x1F) << 24) | ((u8_dat2 & 0x1F) << 16) |
				     ((u8_dat1 & 0x1F) << 8) | ((u8_dat0 & 0x1F) << 0);
		}

		MSDC_SET_FIELD(MSDC_IOCON, MSDC_IOCON_DDLSEL, 1);
		MSDC_WRITE32(MSDC_DAT_RDDLY0, cur_rxdly0);

		MSDC_LOG_NORMAL(pr_info,"i_data=%d i_delay=%d\n", i_data, i_delay);
	} else if (i_case == 4) {	/* set edge */
		if (!((i_par1 == 0) || (i_par1 == 1))) {
			return -1;
		}

		i_edge = i_par1;

		MSDC_SET_FIELD((base + 0x04), MSDC_IOCON_RSPL, i_edge);
		MSDC_SET_FIELD((base + 0x04), MSDC_IOCON_DSPL, i_edge);

		MSDC_LOG_NORMAL(pr_info,"i_edge=%d\n", i_edge);
	} else {
		return -1;
	}

	return 1;
}

static int msdc_tune_proc_read_show(struct seq_file *m, void *data)
{
	seq_puts(m, "\n=========================================\n");
	seq_printf(m, "sdio_enable_tune: 0x%.8x\n", sdio_enable_tune);
	seq_printf(m, "sdio_iocon_dspl: 0x%.8x\n", sdio_iocon_dspl);
	seq_printf(m, "sdio_iocon_w_dspl: 0x%.8x\n", sdio_iocon_w_dspl);
	seq_printf(m, "sdio_iocon_rspl: 0x%.8x\n", sdio_iocon_rspl);
	seq_printf(m, "sdio_pad_tune_rrdly: 0x%.8x\n", sdio_pad_tune_rrdly);
	seq_printf(m, "sdio_pad_tune_rdly: 0x%.8x\n", sdio_pad_tune_rdly);
	seq_printf(m, "sdio_pad_tune_wrdly: 0x%.8x\n", sdio_pad_tune_wrdly);
	seq_printf(m, "sdio_dat_rd_dly0_0: 0x%.8x\n", sdio_dat_rd_dly0_0);
	seq_printf(m, "sdio_dat_rd_dly0_1: 0x%.8x\n", sdio_dat_rd_dly0_1);
	seq_printf(m, "sdio_dat_rd_dly0_2: 0x%.8x\n", sdio_dat_rd_dly0_2);
	seq_printf(m, "sdio_dat_rd_dly0_3: 0x%.8x\n", sdio_dat_rd_dly0_3);
	seq_printf(m, "sdio_dat_rd_dly1_0: 0x%.8x\n", sdio_dat_rd_dly1_0);
	seq_printf(m, "sdio_dat_rd_dly1_1: 0x%.8x\n", sdio_dat_rd_dly1_1);
	seq_printf(m, "sdio_dat_rd_dly1_2: 0x%.8x\n", sdio_dat_rd_dly1_2);
	seq_printf(m, "sdio_dat_rd_dly1_3: 0x%.8x\n", sdio_dat_rd_dly1_3);
	seq_printf(m, "sdio_clk_drv: 0x%.8x\n", sdio_clk_drv);
	seq_printf(m, "sdio_cmd_drv: 0x%.8x\n", sdio_cmd_drv);
	seq_printf(m, "sdio_data_drv: 0x%.8x\n", sdio_data_drv);
	seq_printf(m, "sdio_tune_flag: 0x%.8x\n", sdio_tune_flag);
	seq_puts(m, "=========================================\n\n");

	return 0;
}

static int msdc_tune_proc_write(struct file *file, const char *buf, size_t count, loff_t *data)
{
	int ret;
	int cmd, p1, p2;

	if (count == 0) {
		return -1;
	}

	if (count > 255) {
		count = 255;
	}

	ret = copy_from_user(cmd_buf, buf, count);

	if (ret < 0) {
		return -1;
	}

	cmd_buf[count] = '\0';
	MSDC_LOG_NORMAL(pr_info,"msdc write: %s\n", cmd_buf);

	if (3 == sscanf(cmd_buf, "%x %x %x", &cmd, &p1, &p2)) {
		switch (cmd) {
		case 0:
			if (p1 && p2) {
				sdio_enable_tune = 1;
			} else {
				sdio_enable_tune = 0;
			}

			break;

		case 1:	/* Cmd and Data latch edge */
			sdio_iocon_rspl = p1 & 0x1;
			sdio_iocon_dspl = p2 & 0x1;
			break;

		case 2:	/* Cmd Pad/Async */
			sdio_pad_tune_rrdly = (p1 & 0x1F);
			sdio_pad_tune_rdly = (p2 & 0x1F);
			break;

		case 3:
			sdio_dat_rd_dly0_0 = (p1 & 0x1F);
			sdio_dat_rd_dly0_1 = (p2 & 0x1F);
			break;

		case 4:
			sdio_dat_rd_dly0_2 = (p1 & 0x1F);
			sdio_dat_rd_dly0_3 = (p2 & 0x1F);
			break;

		case 5:	/* Write data edge/delay */
			sdio_iocon_w_dspl = p1 & 0x1;
			sdio_pad_tune_wrdly = (p2 & 0x1F);
			break;

		case 6:
			sdio_dat_rd_dly1_2 = (p1 & 0x1F);
			sdio_dat_rd_dly1_3 = (p2 & 0x1F);
			break;

		case 7:
			sdio_clk_drv = (p1 & 0x7);
			break;

		case 8:
			sdio_cmd_drv = (p1 & 0x7);
			sdio_data_drv = (p2 & 0x7);
			break;
		case 9:
			simulate_insert_enable = p1;
			MSDC_LOG_NORMAL(pr_info, "simulate insert %s\n", (p1 == 0)?"disable":"enable");
			break;
		case 0x10:
			if(simulate_insert_enable) {
				if(MMC_HOST_SLOT0==p1)
					simulate_insert_slot0=p2;
				else if(MMC_HOST_SLOT1==p1)
					simulate_insert_slot1=p2;
				else if(MMC_HOST_SLOT2==p1)
					simulate_insert_slot2=p2;
				else
					pr_err("!!!error host index: %d \n",p1);

				tasklet_hi_schedule(&atc_msdc_host[p1]->card_tasklet);
			}
			break;

		}
	}

	return count;
}

static int msdc_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, msdc_debug_proc_show, inode->i_private);
}

static const struct file_operations msdc_proc_fops = {
	.open = msdc_proc_open,
	.write = msdc_debug_proc_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int msdc_help_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, msdc_help_proc_show, inode->i_private);
}

static const struct file_operations msdc_help_fops = {
	.open = msdc_help_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int msdc_FT_open(struct inode *inode, struct file *file)
{
	return single_open(file, msdc_debug_proc_read_FT_show, inode->i_private);
}

static const struct file_operations msdc_FT_fops = {
	.open = msdc_FT_open,
	.write = msdc_debug_proc_write_FT,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int msdc_tune_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, msdc_tune_proc_read_show, inode->i_private);
}

static const struct file_operations msdc_tune_fops = {
	.open = msdc_tune_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = msdc_tune_proc_write,
};

static int msdc_tune_flag_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, msdc_tune_flag_proc_read_show, inode->i_private);
}

static const struct file_operations msdc_tune_flag_fops = {
	.open = msdc_tune_flag_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

int msdc_debug_proc_init(void)
{
	struct proc_dir_entry *prEntry;
	struct proc_dir_entry *tune;
	struct proc_dir_entry *tune_flag;
	struct msdc_host *host = NULL;

	/* proc/msdc_debug node */
	prEntry = proc_create("msdc_debug", 0660, NULL, &msdc_proc_fops);

	if (!prEntry) {
		MSDC_LOG(ERR,"failed to create /proc/msdc_debug");
	}

	/* proc/msdc_help node */
	prEntry = proc_create("msdc_help", 0660, NULL, &msdc_help_fops);

	if (!prEntry) {
		MSDC_LOG(ERR,"failed to create /proc/msdc_help");
	}

	/* proc/msdc_FT node */
	prEntry = proc_create("msdc_FT", 0660, NULL, &msdc_FT_fops);

	if (!prEntry) {
		MSDC_LOG(ERR,"failed to create /proc/msdc_FT");
	}

	memset(msdc_drv_mode, 0, sizeof(msdc_drv_mode));

	/* proc/msdc_tune node */
	tune = proc_create("msdc_tune", 0660, NULL, &msdc_tune_fops);

	if (!tune) {
		MSDC_LOG(ERR,"failed to create /proc/msdc_tune");
	}

	/* proc/msdc_tune_flag node */
	tune_flag = proc_create("msdc_tune_flag", 0660, NULL, &msdc_tune_flag_fops);

	if (!tune_flag) {
		MSDC_LOG(ERR,"failed to create /proc/msdc_tune_flag");
	}

	return 0;
}
EXPORT_SYMBOL_GPL(msdc_debug_proc_init);

int sd_simulate_detect_gpio(struct msdc_host *host) {
	if (simulate_insert_enable) {
		if (host->id == MMC_HOST_SLOT0) {
			return simulate_insert_slot0;
		} else if (host->id == MMC_HOST_SLOT1) {
			return simulate_insert_slot1;
		} else if (host->id == MMC_HOST_SLOT2) {
			return simulate_insert_slot2;
		}
		MSDC_LOG(ERR, "!!!error host index: %d", host->id);
	}

	return -1;
}
