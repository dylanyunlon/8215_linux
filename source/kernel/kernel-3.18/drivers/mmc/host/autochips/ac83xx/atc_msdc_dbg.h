#ifndef __ATC_MSDC_DEUBG__
#define __ATC_MSDC_DEUBG__

#include "atc_msdc.h"
#include "x_os.h"

// define Base Address here for msdc debug
#define MSDC_0_BASE			(0xFD00B000UL)
#define MSDC_1_BASE			(0xFD021000UL)
#define MSDC_2_BASE			(0xFD00A000UL)
...
typedef enum {
    ...
    HOST_TUNE_PARAMS		= 21,
    HOST_TUNE_METHOD		= 22,
    MSDC_SET_CURRENT_HOST	= 23,
    MSDC_TUNE_PARAM_OPS		= 24,
    MMC_WP_OPERATION            = 25,
} msdc_dbg;	
...
#define DBG_EVT_DMA         (1U << 0)  /* DMA related event */
#define DBG_EVT_CMD         (1U << 1)  /* MSDC CMD related event */
#define DBG_EVT_RSP         (1U << 2)  /* MSDC CMD RSP related event */
#define DBG_EVT_INT         (1U << 3)  /* MSDC INT event */
#define DBG_EVT_CFG         (1U << 4)  /* MSDC CFG event */
#define DBG_EVT_FUC         (1U << 5)  /* Function event */
#define DBG_EVT_OPS         (1U << 6)  /* Read/Write operation event */
#define DBG_EVT_FIO         (1U << 7)  /* FIFO operation event */
#define DBG_EVT_WARN        (1U << 8)  /* Warning event */
#define DBG_EVT_PWR         (1U << 9)  /* Power event */
#define DBG_EVT_CLK         (1U << 10) /* Trace clock gate/ungate operation*/
//==============================================================================
#define DBG_EVT_RW          (1U << 12) /* Trace the Read/Write Command */
#define DBG_EVT_NRW         (1U << 13) /* Trace other Command */

#define DBG_EVT_ERR			(1U << 15)
#define DBG_EVT_IRQ			(1U << 16)
#define DBG_EVT_INFO		(1U << 17)
#define DBG_EVT_TRACE		(1U << 18)
#define DBG_EVT_TUNE		(1U << 19)
#define DBG_EVT_DUMP		(1U << 20)
#define DBG_EVT_INIT		(1U << 21)
...
extern unsigned int msdc_debug_zone[HOST_MAX_NUM];
#define TAG 	"[MSDC]"

#define MSDC_LOG(evt, fmt, args...) 									\
	do {												\
		if (host) {										\
			if ((DBG_EVT_##evt) & msdc_debug_zone[host->id]) { 				\
				pr_err(TAG"[%d]%s(): ---> "fmt" <--- @%s:%d\n", 			\
					host->id, __FUNCTION__, ##args, FILE_ONLY, __LINE__); 		\
			}										\
		}else{											\
			pr_err(TAG"%s(): ---> "fmt" <---  @%s:%d\n", 					\
				__FUNCTION__, ##args, FILE_ONLY, __LINE__); 				\
		}											\
	} while(0)

#define MSDC_LOG_NORMAL(pr_level, fmt, args...)					\
	do {									\
		pr_level(TAG""fmt"\n", ##args);			\
	} while(0)
