...
static int msdc_pio_abort_warning(struct msdc_host *host, struct mmc_data *data, unsigned long tmo)
	{
		...
		if (time_after(jiffies, tmo)) {
			...
			//MSDC_LOG(ERR, "XXX PIO Data Timeout: CMD<%d>", host->mrq->cmd->opcode);
...
static int msdc_pio_abort(struct msdc_host *host, struct mmc_data *data, unsigned long tmo)
	{
		...
		if (time_after(jiffies, tmo)) {
			data->error = (unsigned int)-ETIMEDOUT;
			//MSDC_LOG(ERR, "XXX PIO Data SW Timeout: CMD<%d>", host->mrq->cmd->opcode);
			//msdc_dump_info(host->id);		   
			ret = 1;		
		}
...
static int msdc_pio_read(struct msdc_host *host, struct mmc_data *data)
	{
		...
		//MSDC_CLR_BITS(MSDC_INTEN, wints);    
		//if(data->error) MSDC_LOG(ERR, "read pio data->error<%d> left<%d> size<%d>", data->error, left,size);
		return data->error;
	}
...
static int msdc_pio_write(struct msdc_host* host, struct mmc_data *data)
	{
		...
		//if(data->error) MSDC_LOG(ERR, "write pio data->error<%d>", data->error);
...
int autok_start_rw(struct msdc_host *host, u8 *value, unsigned size, unsigned blocks, bool write)
{
    ...
    if (msdc_txfifocnt() || msdc_rxfifocnt()) {
        MSDC_LOG_NORMAL(pr_info, "[%d][%s]register abnormal,please check!\n",  host->id, __func__);
        msdc_reset_hw(host->id);
    }
...
int autok_io_rw_extended(struct msdc_host *host, unsigned int u4Addr, unsigned int u4Func, void *pBuffer, unsigned int u4Len, bool write)
{
    ...
    {
        MSDC_LOG_NORMAL(pr_err, "[%s][ERR] pBuffer = %p, host = %p\n", __func__, pBuffer, host);
        return -1;
    }
    
    if( u4Len < 4 )
    {
        MSDC_LOG_NORMAL(pr_err, "%s[ERR] u4Len = %d\n", __func__, u4Len);
        return -1;
    }
...
int autok_io_rw_direct(struct msdc_host *host, unsigned int u4Addr, unsigned int u4Func, void *pBuffer, unsigned int u4Len, bool write)
{
     ...
     */
    if (msdc_txfifocnt() || msdc_rxfifocnt()) {
        MSDC_LOG_NORMAL(pr_err, "[%d][%s] register abnormal,please check!\n", host->id, __func__);
        msdc_reset_hw(host->id);
    }
...
int msdc_autok_read(struct msdc_host *host, unsigned int u4Addr, unsigned int u4Func, void *pBuffer, unsigned int u4Len, unsigned int u4Cmd)
{
    ...
    if((pBuffer==NULL) || (host==NULL))
    {
        MSDC_LOG_NORMAL(pr_err,"%s pBuffer = %p, host = %p\n", __func__, pBuffer, host);
        return -1;
    }
    ...
    {
        MSDC_LOG_NORMAL(pr_err, "%s u4Cmd = %d, u4Len = %d\n", __func__, u4Cmd, u4Len);
        return -1;
    }
    ...
    {
        MSDC_LOG_NORMAL(pr_err, "%s Doesn't support u4Cmd = %d\n", __func__, u4Cmd);
        ret = -1;
    }
...
int msdc_autok_adjust_param(struct msdc_host *host, enum AUTOK_PARAM param, u32 *value, int rw)
{
    ...
    switch (param)
    {
        case CMD_EDGE:
            ...
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for CMD_EDGE is out of range, it should be [0~1]\n", __func__, *value);
                return -1;
            }
        ...
        case RDATA_EDGE:
            if((rw == AUTOK_WRITE) && (*value > 1))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for RDATA_EDGE is out of range, it should be [0~1]\n", __func__, *value);
                return -1;
            }
            reg = (ulong)(MSDC_IOCON);
        ...
        case WDATA_EDGE:
            if((rw == AUTOK_WRITE) && (*value > 1))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for WDATA_EDGE is out of range, it should be [0~1]\n", __func__, *value);
                return -1;
            }
            reg = (ulong)(MSDC_IOCON);
...
#if 1
        case CLK_DRV:
            if((rw == AUTOK_WRITE) && (*value > 0x3f))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for CLK_DRV is out of range, it should be [0~0x3f]\n", __func__, *value);
                return -1;
            }
                
            if(host->id != 1)
            {
                MSDC_LOG_NORMAL(pr_err, "%s MSDC%d doesn't support AUTO K\n", __func__, host->id);
                return -1;
            }

            //reg = (ulong)(MSDC2_GPIO_CLK_BASE);
           // field = (u32)(GPIO_MSDC1_MSDC2_DRVN);
            reg = (u32)(PAD_CFG_SD1_CLK);
            field = (u32)(PAD_CFG_DRV_MASK);
            break;
        case CMD_DRV:
            if((rw == AUTOK_WRITE) && (*value > 0x3f))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for CMD_DRV is out of range, it should be [0~0x3f]\n", __func__, *value);
                return -1;
            }
            
            if(host->id !=1 )
            {
                MSDC_LOG_NORMAL(pr_err, "%s MSDC%d doesn't support on AUTO K\n", __func__, host->id);
                return -1;
            }

            //reg = (ulong)(MSDC2_GPIO_CMD_BASE);
            //field = (u32)(GPIO_MSDC1_MSDC2_DRVN);
            reg = (u32)(PAD_CFG_SD1_CMD);
            field = (u32)(PAD_CFG_DRV_MASK);
            break;
        case DAT_DRV:
            if((rw == AUTOK_WRITE) && (*value > 0x3f))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DAT_DRV is out of range, it should be [0~0x3f]\n", __func__, *value);
                return -1;
            }
        ...
        case DAT0_RD_DLY:
            ...
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DAT0_RD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case DAT1_RD_DLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DAT1_RD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case DAT2_RD_DLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DAT2_RD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case DAT3_RD_DLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DAT3_RD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case DAT_WRD_DLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DAT_WRD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case DAT_RD_DLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DAT_RD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case CMD_RESP_RD_DLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for CMD_RESP_RD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case CMD_RD_DLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for CMD_RD_DLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case DATA_DLYLINE_SEL:
            if((rw == AUTOK_WRITE) && (*value > 1))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for DATA_DLYLINE_SEL is out of range, it should be [0~1]\n", __func__, *value);
                return -1;
            }
        ...
        case READ_DATA_SMPL_SEL:
            if((rw == AUTOK_WRITE) && (*value > 1))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for READ_DATA_SMPL_SEL is out of range, it should be [0~1]\n", __func__, *value);
                return -1;
            }
        ...
        case WRITE_DATA_SMPL_SEL:
            if((rw == AUTOK_WRITE) && (*value > 1))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for WRITE_DATA_SMPL_SEL is out of range, it should be [0~1]\n", __func__, *value);
                return -1;
            }
        ...
        case INT_DAT_LATCH_CK:
            ...
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for INT_DAT_LATCH_CK is out of range, it should be [0~7]\n", __func__, *value);
                return -1;
            }
        ...
        case CKGEN_MSDC_DLY_SEL:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for CKGEN_MSDC_DLY_SEL is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
        ...
        case CMD_RSP_TA_CNTR:
            if((rw == AUTOK_WRITE) && (*value > 7))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for CMD_RSP_TA_CNTR is out of range, it should be [0~7]\n", __func__, *value);
                return -1;
            }
        ...
        case WRDAT_CRCS_TA_CNTR:
            if((rw == AUTOK_WRITE) && (*value > 7))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for WRDAT_CRCS_TA_CNTR is out of range, it should be [0~7]\n", __func__, *value);
                return -1;
            }
        ...
        case PAD_CLK_TXDLY:
            if((rw == AUTOK_WRITE) && (*value > 31))
            {
                MSDC_LOG_NORMAL(pr_err, "%s Input value(%d) for PAD_CLK_TXDLY is out of range, it should be [0~31]\n", __func__, *value);
                return -1;
            }
            ...
            break;
        default:
            MSDC_LOG_NORMAL(pr_err, "%s Value of [enum AUTOK_PARAM param] is wrong\n", __func__);
            return -1;
    }
    ...
    if(rw == AUTOK_READ)
		...
		if(param == DAT_DRV){
			sdr_set_field(PAD_CFG_SD1_DAT0, PAD_CFG_DRV_MASK, *value);
			sdr_set_field(PAD_CFG_SD1_DAT1, PAD_CFG_DRV_MASK, *value);
			sdr_set_field(PAD_CFG_SD1_DAT2, PAD_CFG_DRV_MASK, *value);
			sdr_set_field(PAD_CFG_SD1_DAT3, PAD_CFG_DRV_MASK, *value);
		}
    ...
    {
        MSDC_LOG_NORMAL(pr_err, "%s Value of [int rw] is wrong\n", __func__);
        return -1;
    }
...
static E_RESULT_TYPE autok_read_test(struct msdc_host *host)
{
    ...
    for(i=0; i<TUNING_DATA_NO; i++) {
        ...
        if(memcmp(g_test_read_pattern, &g_test_write_pattern[i*TUNING_TEST_TIME], 4*TUNING_TEST_TIME) != 0) {
            ...
            MSDC_LOG_NORMAL(pr_err, "%s E_RESULT_CMP_ERR\n", __func__);
            goto end;
        }
    }
end:
