...
#ifndef ATC_SDIO30_ONLINE_TUNING_SUPPORT
#define DMA_ON 0
#define DMA_OFF 1
#endif
...
void autok_claim_host(struct msdc_host *host)
{
    ...
    MSDC_LOG_NORMAL(pr_info, "%s msdc%d host claimed\n", __func__, host->id);
}

void autok_release_host(struct msdc_host *host)
{
    mmc_release_host(host->mmc);
    MSDC_LOG_NORMAL(pr_info, "%s msdc%d host released\n", __func__, host->id);
}
...
int autok_mmap_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
    ...
    if (!dev->data) {
        MSDC_LOG_NORMAL(pr_err, "no data\n");
        return 0;    
    }
    if (offset >= dev->size)
        return 0;
...
//    preempt_disable();
 ...
 //   msdc_ungate_clock(host);
    ...
    if(stage == 1) {
        ...
        if(res){
            MSDC_LOG_NORMAL(pr_err, "%s Auto-K stage 1 fail, res = %d, set msdc parameter settings stored in nvram to 0\n", __func__, res);
        ...
        if(doStg2){
            ...
            if(res){
                MSDC_LOG_NORMAL(pr_info, "%s Auto-K stage 2 fail, res = %d, downgrade SDIO freq to 50MHz\n", __func__, res);
        ...
        MSDC_LOG_NORMAL(pr_info, "%s stage %d doesn't support in auto-K\n", __func__, stage);
        //return -EFAULT;
    }
    do_gettimeofday(&t1);
    if(dma == DMA_ON)
...
int send_autok_uevent(char *text, struct msdc_host *host)
{
    ...
    kfree(what_buf);
    if(err < 0)
        MSDC_LOG_NORMAL(pr_info, "%s kobject_uevent_env error = %d\n", __func__, err);
...
//    BOOTMODE btmod;
    ...
    if (1/*(btmod!=META_BOOT) && (btmod!=FACTORY_BOOT) && (btmod!=ATE_FACTORY_BOOT)*/){
        ...
        #endif
#ifdef ATC_SDIO30_ONLINE_TUNING_SUPPORT    
        atomic_set(&host->ot_work.ot_disable, 1);
#endif  // ATC_SDIO30_ONLINE_TUNING_SUPPORT
                        ...
                        const char *buf, size_t count)
{
    ...
    if (id >= HOST_MAX_NUM) {
        MSDC_LOG_NORMAL(pr_info, "%s id<%d> is bigger than HOST_MAX_NUM<%d>\n", __func__, id, HOST_MAX_NUM);
        return count;
    }
    ...
    switch(select){
        ...
        case PARAMS:
            ...
            MSDC_LOG_NORMAL(pr_err, "[AUTOKD] Enter Store Autok");
            MSDC_LOG_NORMAL(pr_err, "[AUTOKD] p_single_autok[%d].vol_count=%d", id, p_single_autok[id].vol_count);
            MSDC_LOG_NORMAL(pr_err, "[AUTOKD] p_single_autok[%d].param_count=%d", id, p_single_autok[id].param_count);
            for(i=0; i<p_single_autok[id].vol_count; i++){
                MSDC_LOG_NORMAL(pr_err, "[AUTOKD] p_single_autok[%d].vol_list[%d]=%d", id, i, p_single_autok[id].vol_list[i]);
            }
            for(i=0; i<p_single_autok[id].vol_count; i++){
                for(j=0; j<p_single_autok[id].param_count; j++)
                    MSDC_LOG_NORMAL(pr_err, "[AUTOKD] p_single_autok[%d].ai_data[%d][%d]=%d", id, i, j, p_single_autok[id].ai_data[i][j].data.sel);
            }
...
{    
    ...
    if (id >= HOST_MAX_NUM) {
        MSDC_LOG_NORMAL(pr_err, "%s id<%d> is bigger than HOST_MAX_NUM<%d>\n", __func__, id, HOST_MAX_NUM);
        return count;
    }
...
#else	
			len = snprintf(data_buf, 32, "disable autok %d", 1);
#endif
            break;
        case PARAM_COUNT:
...
static int fake_sdio_device(void *data)
{
    //struct msdc_host *host = atc_msdc_host[3];
    struct msdc_host *host = atc_msdc_host[1];
    wait_sdio_autok_ready(host->mmc);
    return 0;
}
...
{
    int retval = 0;
    MSDC_LOG_NORMAL(pr_info, "autok_init \n");
    autok_log_entry = debugfs_create_file("autok_log", 0660, NULL, NULL, &autok_log_fops);
    i_gid_write(autok_log_entry->d_inode, 1000);
    //autok_log_entry->d_inode->i_gid=1000;
    autok_log_info = (char*)kzalloc(LOG_SIZE, GFP_KERNEL);
...
MODULE_DESCRIPTION("ATC SDIO Auto-K Proc");
MODULE_LICENSE("GPL");
