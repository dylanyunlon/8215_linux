...
/*added by dandan liu */
int writesb_test(struct msdc_host *host,unsigned int u4Addr, unsigned int u4Func)
{
	int ret;
	...
	for(i=0;i<15;i++)
    	if(!(ret = sdio_writesb(sdioFunc, u4Addr, buff, 1024)))
			break;
	return ret;
}

int readsb_test(struct msdc_host *host,unsigned int u4Addr, unsigned int u4Func)
{
	int ret;
	struct sdio_func *sdioFunc;
	u8 buff[1536];
	...
	sdioFunc = host->mmc->card->sdio_func[u4Func - 1];
	for(i=0;i<15;i++)
    	if(!(ret = sdio_readsb(sdioFunc, buff,u4Addr , 1024)))
			break;
	return ret;
...
static E_RESULT_TYPE autok_write_test(struct msdc_host *host, unsigned int test_data[], int test_len)
{
...
#if 0
	if(writesb_test(host, SDIO_IP_WTMDR, LTE_MODEM_FUNC)){
			res = E_RESULT_DATA_ERR;
            goto end;
	}

	if(readsb_test(host, SDIO_IP_WTMDR, LTE_MODEM_FUNC)){
			res = E_RESULT_DATA_ERR;
            goto end;
	}
#endif
 ...
 E_RESULT_TYPE autok_read_test(struct msdc_host *host, unsigned int test_data[], int test_len)
{
    int i;
    E_RESULT_TYPE res = E_RESULT_PASS;
    unsigned char *data;
...
module_exit(ettagent_exit);
MODULE_AUTHOR("ATC Inc.");
MODULE_DESCRIPTION("ATC SDIO ETT Agent Proc");
MODULE_LICENSE("GPL");
