...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
int aicwf_dbg_level = AIC_LOG_LEVEL_DEFAULT;
module_param(aicwf_dbg_level, int, 0660);
EXPORT_SYMBOL(aicwf_dbg_level);
#else
...
static int aic_log_level_callback(int type)
{
	int ret = 0;

	AICWFDBG(LOGINFO, "set log level %d\n", type);
	switch (type) {
	case ATC_COMBO_WLAN_LOG_LEVEL_INFO:
		aicwf_dbg_level = AIC_LOG_LEVEL_INFO;
		break;
	case ATC_COMBO_WLAN_LOG_LEVEL_DEBUG:
		aicwf_dbg_level = AIC_LOG_LEVEL_DEBUG;
		break;
	case ATC_COMBO_WLAN_LOG_LEVEL_LOUD:
		aicwf_dbg_level = AIC_LOG_LEVEL_LOUD;
		break;
	default:
		AICWFDBG(LOGERROR, "unknown log level %d\n", type);
		ret = -EINVAL;
		break;
	}
...
static int __init aicbsp_init(void)
{
	...
	atc_combo_wlan_log_level_register_callback(
			ATC_COMBO_WLAN_LOG_LEVEL_INFO, aic_log_level_callback);
#endif
