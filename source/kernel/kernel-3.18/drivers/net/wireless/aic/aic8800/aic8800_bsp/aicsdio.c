...
int sdio_clk = FEATURE_SDIO_CLOCK_V3;
int sdio_f1 = 0x40;
int sdio_f8 = 0x00;
int sdio_f0 = 0x00;
int sdio_f3 = 0x10;

module_param(sdio_clk, int, 0660);
...
int aicbsp_set_subsys(int subsys, int state)
{
	...
	if (cur_power_state != pre_power_state) {
		...
		if (cur_power_state) {
			...
			pr_info("%s BT wake default to SLEEP\n", __func__);
#endif
#endif
	...
	const struct sdio_device_id *id)
{
	...
	if(sdiodev->chipid == PRODUCT_ID_AIC8800DC || sdiodev->chipid == PRODUCT_ID_AIC8800DW){
		...
		pr_info("the device is PRODUCT_ID_AIC8800DC \n");
	}
...
int aicbsp_sdio_init(void)
{
	...
	if (down_timeout(&aicbsp_probe_semaphore, msecs_to_jiffies(2000)) != 0){
		pr_info("%s aicbsp_sdio_probe fail\n", __func__);
		return -1;
	}
