...
static __inline__ int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	if (skb_tailroom(skb) < data_len) {
		pr_info("skb_tailroom() failed in add SID tag!\n");
		return -1;
	}
...
#ifdef BR_SUPPORT_DEBUG
#ifdef CL_IPV6_PASS
						panic_pr_info("NAT25 Expire H(%02d) M:%02x%02x%02x%02x%02x%02x N:%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
							"%02x%02x%02x%02x%02x%02x\n",
							     i,
							     f->macAddr[0],
							     f->macAddr[1],
...
int nat25_db_handle(struct rwnx_vif *vif, struct sk_buff *skb, int method)
{
...
#if 0
	pr_info("nat25_db_handle: DA=%02x%02x%02x%02x%02x%02x SA=%02x%02x%02x%02x%02x%02x\n",
			     skb->data[0],
			     skb->data[1],
			     skb->data[2],
			     skb->data[3],
	...
	if (protocol == __constant_htons(ETH_P_IP)) {
		...
		if (((unsigned char *)(iph) + (iph->ihl << 2)) >= (skb->data + ETH_HLEN + skb->len)) {
			pr_info("NAT25: malformed IP packet !\n");
			return -1;
		}
...
#endif
			{
				...
				if (!__nat25_db_network_lookup_and_replace(vif, skb, networkAddr)) {
					if (*((unsigned char *)&iph->daddr + 3) == 0xff) {
						...
						pr_info("NAT25: Set DA as boardcast\n");
	...
	/*---------------------------------------------------*/
		...
		if (arp->ar_pro != __constant_htons(ETH_P_IP)) {
			pr_info("NAT25: arp protocol unknown (%4x)!\n", htons(arp->ar_pro));
			return -1;
		}
	...
	/*---------------------------------------------------*/
		...
		if (ea != NULL) {
			...
			if (ea->hw_len != ETH_ALEN || ea->pa_len != AARP_PA_ALEN) {
				pr_info("NAT25: Appletalk AARP Sanity check fail!\n");
				return -1;
			}
	...
	/*---------------------------------------------------*/
		...
		case NAT25_INSERT:
			if (ph->sid == 0) {	/* Discovery phase according to tag */
					...
					if (vif->ethBrExtInfo.addPPPoETag) {
						...
						if (pOldTag) { /* if SID existed, copy old value and delete it */
							...
							if (old_tag_len + TAG_HDR_LEN + MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN > sizeof(tag_buf)) {
								pr_info("SID tag length too long!\n");
								return -1;
							}
							...
							if (skb_pull_and_merge(skb, (unsigned char *)pOldTag, TAG_HDR_LEN + old_tag_len) < 0) {
								pr_info("call skb_pull_and_merge() failed in PADI/R packet!\n");
								return -1;
							}
						...
						*pMagic = htons(MAGIC_CODE);
						...
						if (vif->pppoe_connection_in_progress &&
							...
							pr_info("Discard PPPoE packet due to another PPPoE connection is in progress!\n");
							return -2;
						}
				...
				pr_info("NAT25: Insert PPPoE, insert session packet to %s\n", skb->dev->name);
		...
		case NAT25_LOOKUP:
			if (ph->code == PADO_CODE || ph->code == PADS_CODE) {
				if (vif->ethBrExtInfo.addPPPoETag) {
					...
					if (ptr == 0) {
						pr_info("Fail to find PTT_RELAY_SID in FADO!\n");
						return -1;
					}
					...
					if ((tagType != ntohs(PTT_RELAY_SID)) || (tagLen < (MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN))) {
						pr_info("Invalid PTT_RELAY_SID tag length [%d]!\n", tagLen);
						return -1;
					}

					pMagic = (unsigned short *)tag->tag_data;
					if (ntohs(*pMagic) != MAGIC_CODE) {
						pr_info("Can't find MAGIC_CODE in %s packet!\n",
					...
					if (skb_pull_and_merge(skb, ptr + offset, TAG_HDR_LEN + MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN - offset) < 0) {
						pr_info("call skb_pull_and_merge() failed in PADO packet!\n");
						return -1;
					}
					...
					if (!vif->pppoe_connection_in_progress) {
						pr_info("Discard PPPoE packet due to no connection in progresss!\n");
						return -1;
					}
	...
	/*---------------------------------------------------*/
		...
		if (sizeof(*iph) >= (skb->len - ETH_HLEN)) {
			pr_info("NAT25: malformed IPv6 packet !\n");
			return -1;
		}
		...
		case NAT25_INSERT: {
			pr_info("NAT25: Insert IP, SA=%4x:%4x:%4x:%4x:%4x:%4x:%4x:%4x,"
		...
		case NAT25_LOOKUP:
			pr_info("NAT25: Lookup IP, SA=%4x:%4x:%4x:%4x:%4x:%4x:%4x:%4x,"
				 " DA=%4x:%4x:%4x:%4x:%4x:%4x:%4x:%4x\n",
				iph->saddr.s6_addr16[0], iph->saddr.s6_addr16[1], iph->saddr.s6_addr16[2], iph->saddr.s6_addr16[3],
				iph->saddr.s6_addr16[4], iph->saddr.s6_addr16[5], iph->saddr.s6_addr16[6], iph->saddr.s6_addr16[7],
				iph->daddr.s6_addr16[0], iph->daddr.s6_addr16[1], iph->daddr.s6_addr16[2], iph->daddr.s6_addr16[3],
