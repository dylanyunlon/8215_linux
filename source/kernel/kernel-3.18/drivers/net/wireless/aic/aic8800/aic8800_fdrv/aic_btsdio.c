                ...
                switch(subevent_code){
                    ...
                    case HCI_LE_EXTENDED_ADVERTISING_REPORT_EVT:
                    {
                        if(aic_queue_cnt() > (QUEUE_SIZE-490)){
                            pr_info("more adv report bypass\n");
                            ret = 1;
                        }
                    }
                    break;
...
static void print_event(struct sk_buff *skb)
{
    ...
    case HCI_EV_REMOTE_HOST_FEATURES:
        ...
        break;
    default:
        pr_info("unknow event");
        break;
    }
	pr_info("\n");
#if 0
    pr_info("%02x,len:%d,", *opcode,len);
    for (icount = 2; (icount < wlength) && (icount < 24); icount++)
        pr_info("%02x ", *(opcode+icount));
    pr_info("\n");
#endif
#endif
}
        ...
        loff_t *pos_p)
{
	...
	if(err<0){
		pr_info("%s rwnx_sdio_bt_send_req error %d",__func__,err);
	}
