...
/*	if (!capable(CAP_NET_ADMIN)) {
	...
	if ((priv_cmd.total_len > PRIVATE_COMMAND_MAX_LEN) || (priv_cmd.total_len < 0)) {
		pr_info("%s: buf length invalid:%d\n", __FUNCTION__, priv_cmd.total_len);
		ret = -EINVAL;
		goto exit;
	}
	...
	AICWFDBG(LOGINFO, "buf_size=%d\n", buf_size);
		...
		if(!strncasecmp(command, CMD_SET_COUNTRY, strlen(CMD_SET_COUNTRY)) &&
			...
			if (!country || strlen(country) < RWNX_COUNTRY_CODE_LEN) {
				pr_info("%s: invalid country code\n", __func__);
				ret = -EINVAL;
				goto exit;
			}
#if 0
			for(index = 0; index < reg_regdb_size; index++){
				...
				if((ret = regulatory_set_wiphy_regd(vif->rwnx_hw->wiphy, regdomain))){
					pr_info("regulatory_set_wiphy_regd fail\n");
				}else{
					pr_info("regulatory_set_wiphy_regd ok\n");
				}
			}
#endif
	...
	if (bytes_written >= 0) {
		...
		if (copy_to_user(priv_cmd.buf, command, bytes_written)) {
			pr_info("%s: failed to copy data to user buffer\n", __FUNCTION__);
			ret = -EFAULT;
		}
	} else {
		/* Propagate the error */
...
int handle_custom_msg(char *command, u32 cmd_len)
{
	...
	if (cust_msg_cfm->status) {
		pr_info("cfm status: %x", cust_msg_cfm->status);
	}
	return bytes_read;
}
...
/*	if (!capable(CAP_NET_ADMIN)) {
	...
	/* outputs */
	pr_info("%s: Devipc custom msg \"%s\" on %s\n", __FUNCTION__, command, ifr->ifr_name);
	pr_info("cmd = %x\n", cmd);
	pr_info("buf_size=%d\n", buf_size);
