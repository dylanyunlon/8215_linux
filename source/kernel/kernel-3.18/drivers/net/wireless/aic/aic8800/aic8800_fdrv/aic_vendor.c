	...
	const void *data, int len)
{
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		case MKEEP_ALIVE_ATTRIBUTE_IP_PKT:
			if (!ip_pkt_len) {
				ret = -EINVAL;
				pr_info("ip packet length is 0\n");
				goto exit;
			}
			ip_pkt = (u8 *)kzalloc(ip_pkt_len, kflags);
			if (ip_pkt == NULL) {
				ret = -ENOMEM;
				pr_info("Failed to allocate mem for ip packet\n");
				goto exit;
			}
	...
	ret = aic_dev_start_mkeep_alive(rwnx_hw, rwnx_vif, mkeep_alive_id, ip_pkt, ip_pkt_len, src_mac,
		dst_mac, period_msec);
	if (ret < 0) {
		pr_info("start_mkeep_alive is failed ret: %d\n", ret);
	}
	...
	const void *data, int len)
{
	...
	if (ret < 0) {
		pr_info("stop_mkeep_alive is failed ret: %d\n", ret);
	}
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		case ANDR_WIFI_ATTRIBUTE_COUNTRY:
			...
			if ((ret = regulatory_set_wiphy_regd(rwnx_hw->wiphy, regdomain))) {
				...
				pr_err("regulatory_set_wiphy_regd fail %d\n", ret);
#else
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		case APF_ATTRIBUTE_PROGRAM:
			if (mProgramLen > AIC_APF_MEM_SIZE || mProgramLen == 0) {
				pr_info("%s: apf program size invalid: %d (should > 0 and <= %d)\n",
