			       ...
			       unsigned char *buf, unsigned int plen)
{
	...
	//pr_info("drop:%d win_scale:%d",drop,win_scale);
	if (!drop && (0 == win_scale))
		return 0;
	...
	if (index >= 0) {
		...
		if (drop > 0 && atomic_read(&ack_m->enable)) {
			...
			{	
				drop = 2;
				pr_info("%d %d %d",win_scale,win,(ack_m->ack_winsize * SIZE_KB));
			}
