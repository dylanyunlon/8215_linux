...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
static void __maybe_unused atc_wlan_dump_thread(void)
{
	static DEFINE_RATELIMIT_STATE(_rs, 5 * HZ, 1);
	struct task_struct *tsk = NULL;
	struct task_struct *bustx_thread1 = NULL;
	struct task_struct *busrx_thread1 = NULL;
	struct task_struct *bustx_thread2 = NULL;
	struct task_struct *busrx_thread2 = NULL;
	...
	atc_combo_dump_task_by_name("aicwf_pwrctl");
#endif
#if 0
	atc_combo_dump_task_by_name("aicwf_bustx_thread");
	atc_combo_dump_task_by_name("aicwf_busrx_thread");
#else
	rcu_read_lock();
	for_each_process(tsk) { //-V568
		if (!strncmp(tsk->comm, "aicwf_bustx_thread", TASK_COMM_LEN - 1)) {
			if (!bustx_thread1) {
				bustx_thread1 = tsk;
			} else {
				bustx_thread2 = tsk;
			}
		} else if (!strncmp(tsk->comm, "aicwf_busrx_thread", TASK_COMM_LEN - 1)) {
			if (!busrx_thread1) {
				busrx_thread1 = tsk;
			} else {
				busrx_thread2 = tsk;
	...
	pr_info("show debug info of 'aicwf_bustx_thread' 1\n");
	atc_combo_dump_single_task(bustx_thread1);

	pr_info("show debug info of 'aicwf_bustx_thread' 2\n");
	atc_combo_dump_single_task(bustx_thread2);

	pr_info("show debug info of 'aicwf_busrx_thread' 1\n");
	atc_combo_dump_single_task(busrx_thread1);

	pr_info("show debug info of 'aicwf_busrx_thread' 2\n");
	atc_combo_dump_single_task(busrx_thread2);
#endif
#ifdef CONFIG_OOB
	atc_combo_dump_task_by_name("aicwf_busirq_thread");
#endif
}
...
static void aic8800_start_system_reset_flow(struct aic_sdio_dev *aic)
{
    ...
    pr_err("wlan error reset flow.\n");
    pr_err("send event.\n");
	ret=kobject_uevent_env(&aic->dev->kobj, KOBJ_CHANGE,envp);
	if(!ret)
	   pr_err("wlan error event send.\n");
}
#endif
...
static int cmd_mgr_llind(struct rwnx_cmd_mgr *cmd_mgr, struct rwnx_cmd *cmd)
{
	...
	if (!acked) {
		pr_crit("Error: acked cmd not found\n");
	} else {
		cmd->flags &= ~RWNX_CMD_FLAG_WAIT_ACK;
		if (RWNX_CMD_WAIT_COMPLETE(cmd->flags))
			cmd_complete(cmd_mgr, cmd);
...
void cmd_mgr_task_process(struct work_struct *work)
{
	...
	while (1) {
		...
		if (next) {
			...
			//pr_info("cmd_process, cmd->id=%d, tkn=%d\n",next->reqid, next->tkn);
			//rwnx_ipc_msg_push(rwnx_hw, next, RWNX_CMD_A2EMSG_LEN(next->a2e_msg));
#ifdef AICWF_SDIO_SUPPORT
