					...
					size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%x %x %x" , &oper, &addr, &val ) > 0)
		...
		ret = rwnx_send_dbg_mem_read_req(priv, addr, &mem_read_cfm);
		pr_info("before write : [0x%x] = [0x%x]\n", mem_read_cfm.memaddr, mem_read_cfm.memdata);
		ret = rwnx_send_dbg_mem_block_write_req(priv, addr, 4, &val);
		ret = rwnx_send_dbg_mem_read_req(priv, addr, &mem_read_cfm);
		pr_info("after write : [0x%x] = [0x%x]\n", mem_read_cfm.memaddr, mem_read_cfm.memdata);
    	}
			...
			size_t count, loff_t *ppos)
{
	...
	if(ret > 15) {
		pr_info("param error > 15\n");
	} else {
		switch(hwconfig_id)
		    {
		    case 0:
			if(ret != 5) {
			    pr_info("param error  != 5\n");
			    break;}
			ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id, addr, NULL);
			pr_info("ACS_TXOP_REQ bk:0x%x be:0x%x vi:0x%x vo:0x%x\n",addr[0],  addr[1], addr[2], addr[3]);
			break;
		    case 1:
			if(ret != 15) {
			    pr_info("param error  != 15\n");
			...
			addr[12] = ~addr[12] + 1;
			ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id, addr, NULL);
			pr_info("CHANNEL_ACCESS_REQ edca:%x,%x,%x,%x, vif:%x, retry_cnt:%x, rts:%x, long_nav:%x, cfe:%x, rc_retry_cnt:%x:%x:%x ccademod_th %x, remove_1m2m %x\n",
                                addr[0],  addr[1], addr[2], addr[3], addr[4], addr[5], addr[6], addr[7], addr[8], addr[9], addr[10], addr[11], addr[12], addr[13]);
			break;
		    case 2:
			if(ret != 7) {
		            pr_info("param error  != 7\n");
			    break;}
			ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id, addr, NULL);
			pr_info("MAC_TIMESCALE_REQ sifsA:%x,sifsB:%x,slot:%x,ofdm_delay:%x,long_delay:%x,short_delay:%x\n",
                                addr[0],  addr[1], addr[2], addr[3], addr[4], addr[5]);
			break;
		    case 3:
                        if(ret != 6) {
		            pr_info("param error  != 6\n");
			    break;}
			addr[1] = ~addr[1] + 1;
			addr[2] = ~addr[2] + 1;
			addr[3] = ~addr[3] + 1;
			addr[4] = ~addr[4] + 1;
			ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id, addr, NULL);
			pr_info("CCA_THRESHOLD_REQ auto_cca:%d, cca20p_rise:%d cca20s_rise:%d cca20p_fail:%d cca20s_fail:%d\n",
                                addr[0],  addr[1], addr[2], addr[3], addr[4]);
			break;
            case 4: // BWMODE_REQ
                if (ret != 2) {
                    pr_info("param error != 2\n");
                } else {
                    ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id, addr, NULL);
                    pr_info("BWMODE_REQ md=%d\n", addr[0]);
                }
            break;
            case 5: // CHIP_TEMP_GET_REQ
                if (ret != 1) {
                    pr_info("param error != 1\n");
                } else {
                    ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id, addr, addr_out);
                    pr_info("CHIP_TEMP_GET_REQ degree=%d\n", addr_out[0]);
                }
            break;
			case 6://AP_PS_LEVEL_SET_REQ
				if (ret != 2) {
					...
					ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id, addr, NULL);
					pr_info("AP_PS_LEVEL_SET_REQ lvl=%d\n", addr[0]);
				}
			break;
            default:
			pr_info("param error\n");
		...
		if(ret) {
		    pr_info("rwnx_send_vendor_hwconfig_req fail: %x\n", ret);
		}
	}
            ...
            size_t count, loff_t *ppos)
{
    ...
    if (ret > 3) {
        ...
        switch (swconfig_id)
        {
            case 0: // BCN_CFG_REQ
                if (ret != 2) {
                    ...
                    pr_info("BCN_CFG_REQ set_en=%d, get_en=%d\n", addr[0], addr_out[0]);
                }
            break;
            ...
            case 1: // TEMP_COMP_SET_REQ
                if (ret != 3) {
                    ...
                    ret = rwnx_send_vendor_swconfig_req(priv, swconfig_id, addr, addr_out);
                    pr_info("TEMP_COMP_SET_REQ set_en=%d, tmr=%dms, get_st=%d\n",
            ...
            case 2: // TEMP_COMP_GET_REQ
                if (ret != 1) {
                    ...
                    ret = rwnx_send_vendor_swconfig_req(priv, swconfig_id, addr, addr_out);
                    pr_info("TEMP_COMP_GET_REQ get_st=%d, degree=%d\n", addr_out[0], addr_out[1]);
                }
            break;
            ...
            case 3: // EXT_FLAGS_SET_REQ
                if (ret != 2) {
                    ...
                    ret = rwnx_send_vendor_swconfig_req(priv, swconfig_id, addr, addr_out);
                    pr_info("EXT_FLAGS_SET_REQ set ext_flags=0x%x, get ext_flags=0x%x\n",
                        addr[0], addr_out[0]);
                }
            break;
            ...
            case 4: // EXT_FLAGS_GET_REQ
                if (ret != 1) {
                    ...
                    ret = rwnx_send_vendor_swconfig_req(priv, swconfig_id, addr, addr_out);
                    pr_info("EXT_FLAGS_GET_REQ get ext_flags=0x%x\n", addr_out[0]);
                }
            break;
            ...
            case 5: // EXT_FLAGS_MASK_SET_REQ
                if (ret != 3) {
                    ...
                    ret = rwnx_send_vendor_swconfig_req(priv, swconfig_id, addr, addr_out);
                    pr_info("EXT_FLAGS_MASK_SET_REQ set ext_flags mask=0x%x, val=0x%x, get ext_flags=0x%x\n",
                        addr[0], addr[1], addr_out[0]);
                }
            break;
        ...
        if (ret) {
            pr_info("rwnx_send_vendor_swconfig_req fail: %x\n", ret);
        }
    }
            ...
            size_t count, loff_t *ppos)
{
    ...
    if ((ret > 3) || (ret < 1)) {
        ...
        pr_info("set_roc: start=%d, freq=%d\n", roc_start, chan_freq);
        if (roc_start) {
            if (chan_freq <= 2484) {
            ...
            if (ret) {
                pr_info("rwnx_send_roc fail: %d\n", ret);
            } else {
                pr_info("roc_cfm: opcode=%x, st=%d, idx=%d\n", roc_cfm.op_code, roc_cfm.status, roc_cfm.chan_ctxt_index);
            }
        } else {
            ret = rwnx_send_cancel_roc(priv);
            if (ret) {
                pr_info("rwnx_send_cancel_roc fail: %d\n", ret);
            }
        }
    }
	 ...
	 union rwnx_rate_ctrl_info *r_cfg, int *ru_size)
{
    ...
    switch(formatmod){
        ...
        default:
            pr_info("Don't have the formatmod");
    }
}
