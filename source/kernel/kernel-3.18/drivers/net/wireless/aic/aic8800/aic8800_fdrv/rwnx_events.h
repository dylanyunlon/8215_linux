...
TRACE_EVENT(
	...
	TP_pr_info("vif=%d", __entry->vif_idx)
);
...
TRACE_EVENT(
	...
	TP_pr_info("%s pkt_ready_up=%d skb=%p", __print_txq(__entry->txq_idx),
...
DECLARE_EVENT_CLASS(
	...
	TP_pr_info("%s", __print_hwq(__entry->hwq_idx))
);
...
DECLARE_EVENT_CLASS(
	...
	TP_pr_info("%s", __print_txq(__entry->txq_idx))
);
...
TRACE_EVENT(
	...
	#ifdef CONFIG_RWNX_FULLMAC
	TP_pr_info("%s txq_credits=%d, len=%d, retry_len=%d, push_limit=%d",
			  __print_txq(__entry->txq_idx), __entry->credit,
			  __entry->len, __entry->len_retry, __entry->limit)
	#else
	TP_pr_info("%s txq_credits=%d, len=%d, retry_len=%d",
			  __print_txq(__entry->txq_idx), __entry->credit,
...
DECLARE_EVENT_CLASS(
	...
	TP_pr_info("%s reason=%s status=%s",
...
#ifdef CONFIG_RWNX_FULLMAC
	TP_pr_info("%s skb=%p (len=%d) hw_queue=%s cred_txq=%d cred_hwq=%d %s flag=%s %s%s%s",
...
#else
	TP_pr_info("%s skb=%p (len=%d) hw_queue=%s cred_txq=%d cred_hwq=%d %s flag=%x (%s) sn=%d %s",
			  __print_txq(__entry->tx_queue), __entry->skb, __entry->len,
...
TRACE_EVENT(
	...
	TP_pr_info("%s skb=%p retry=%d txq_credits=%d queue_len=%d (retry = %d)",
...
TRACE_EVENT(
	...
	TP_pr_info("%s mac80211_queue_len=%d", __print_txq(__entry->txq_idx), __entry->q_len)
);
...
TRACE_EVENT(
	...
	TP_pr_info("%s %u pkt have been dropped by codel in mac80211 txq",
...
TRACE_EVENT(
	...
	TP_pr_info("hw_queue=%s hw_credits=%s",
...
DECLARE_EVENT_CLASS(
	...
	TP_pr_info("%s", __print_sta(__entry->idx))
);
...
TRACE_EVENT(
	...
	TP_pr_info("%s skb=%p hw_queue=%s, hw_credits=%s, txq_credits=%d (+%d)"
#ifdef CONFIG_RWNX_FULLMAC
			  " sn=%u ampdu=%d"
#ifdef CONFIG_RWNX_SPLIT_TX_BUF
			  " amsdu=%u"
...
TRACE_EVENT(
	...
	TP_pr_info("%s txq_credits=%d (%+d)", __print_txq(__entry->txq_idx),
			  __entry->sw_credit, __entry->sw_credit_up)
)
...
DECLARE_EVENT_CLASS(
	...
	TP_pr_info("%s [PS] ready=%d sp=%d [UAPSD] ready=%d sp=%d",
...
TRACE_EVENT(
	...
	TP_pr_info("%s %s%s traffic available ", __print_sta(__entry->sta_idx),
...
TRACE_EVENT(
	...
	TP_pr_info("%s %s traffic request %d pkt (ready=%d, sp=%d)",
...
TRACE_EVENT(
	...
	TP_pr_info("%s skb=%p %s nb_subframe=%d, len=%u",
			  __print_txq(__entry->txq_idx), __entry->skb,
...
TRACE_EVENT(
	...
	TP_pr_info("Group-id = %d", __entry->group_id)
);
...
TRACE_EVENT(
	...
	TP_pr_info("[Sta %d] Group-id = %d", __entry->sta_idx, __entry->group_id)
);
...
TRACE_EVENT(
	...
	TP_pr_info("%s: group=%d pos=%d", __print_txq(__entry->txq_idx),
...
DECLARE_EVENT_CLASS(
	...
	TP_pr_info("Mpath(%d): target=%pM next_hop=STA-%d",
...
TRACE_EVENT(
	...
	TP_pr_info("%s: PRI=%.5d LEN=%.3d FOM=%.2d%% freq=%dMHz ",
...
TRACE_EVENT(
	...
	TP_pr_info("%s: region=%s type=%d freq=%dMHz (pri=%dus)",
			  __print_symbolic(__entry->chain,
							   {RWNX_RADAR_RIU, "RIU"},
							   {RWNX_RADAR_FCU, "FCU"}),
...
TRACE_EVENT(
	...
	TP_pr_info("%s: %s radar detection %s",
			   __print_symbolic(__entry->chain,
							   {RWNX_RADAR_RIU, "RIU"},
							   {RWNX_RADAR_FCU, "FCU"}),
...
DECLARE_EVENT_CLASS(
	...
	TP_pr_info("%s (%d - %d)", RWNX_ID2STR(__entry->id),
