...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	vif->sta.external_auth_time = jiffies;
#endif
	...
	/* if(check_fwstate(pmlmepriv, WIFI_STATION_STATE|WIFI_ADHOC_STATE) == _TRUE) */
	{
		...
		#endif
		{
			...
			if (br_netdev) {
				...
				dev_put(br_netdev);
				pr_info(FUNC_NDEV_FMT" bind bridge dev "NDEV_FMT"("MAC_FMT")\n"
					, FUNC_NDEV_ARG(netdev), NDEV_ARG(br_netdev), MAC_ARG(br_netdev->dev_addr));
			} else {
				pr_info(FUNC_NDEV_FMT" can't get bridge dev by name \"%s\"\n"
...
#ifndef CONFIG_USE_P2P0
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			schedule_delayed_work(&rwnx_hw->p2p_alive_timer, msecs_to_jiffies(P2P_ALIVE_TIME_MS));
#else
											   ...
											   struct vif_params *params)
{
	...
	AICWFDBG(LOGINFO, "interface add: %02x:%02x:%02x:%02x:%02x:%02x\n", vif->wdev.address[0], vif->wdev.address[1], \
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
static void aicwf_p2p_alive_timeout(struct work_struct *work)
#else
...
#endif
{
	...
	u8_l p2p = 0;
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	rwnx_hw = container_of(work, struct rwnx_hw, p2p_alive_timer.work);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		AICWFDBG(LOGDEBUG, "%s rwnx_vif[%d] timer_count %d\n", __func__, rwnx_vif->vif_index,
	...
	if (rwnx_vif->up) {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		AICWFDBG(LOGINFO, "%s remove rwnx_vif[%d]\n", __func__, rwnx_vif->vif_index);
#endif
		...
		AICWFDBG(LOGINFO, "%s rwnx_vif[%d] is down\n", __func__, rwnx_vif->vif_index);
	}
#endif
}
											   ...
											   struct vif_params *params)
{
	...
	pr_info("rwnx_virtual_interface_add: %d, %s\n", type, name);
	...
	if (vif_idx < 0) {
		pr_info("virtual_interface_add %s fail\n", name);
		return NULL;
	}

	vif = kzalloc(sizeof(struct rwnx_vif), GFP_KERNEL);
	if (unlikely(!vif)) {
		pr_info("Could not allocate wireless device\n");
		return NULL;
	}
	wdev = &vif->wdev;
	...
	pr_info("rwnx_virtual_interface_add, ifname=%s, wdev=%p, vif_idx=%d\n", name, wdev, vif_idx);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		INIT_DELAYED_WORK(&rwnx_hw->p2p_alive_timer, aicwf_p2p_alive_timeout);
#else
		#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	...
	pr_info("p2p dev addr=%02x:%02x:%02x:%02x:%02x:%02x\n", vif->wdev.address[0], vif->wdev.address[1], \
		vif->wdev.address[2], vif->wdev.address[3], vif->wdev.address[4], vif->wdev.address[5]);
									  ...
									  struct vif_params *params)
{
	...
	RWNX_DBG(RWNX_FN_ENTRY_STR);
	pr_info("change_if: %d to %d, %d, %d", vif->wdev.iftype, type, NL80211_IFTYPE_P2P_CLIENT, NL80211_IFTYPE_STATION);
	...
	/* Abort scan request on the vif */
	    ...
	    if (ret) {
		pr_info("remove_if fail\n");
		return ret;
	    }
	    vif->rwnx_hw->vif_table[vif->vif_index] = NULL;
	    pr_info("change_if from %d \n", vif->vif_index);
	    ...
	    if (add_if_cfm.status != 0) {
		pr_info("add if status fail\n");
		return -EIO;
	    }

	    pr_info("change_if to %d \n", add_if_cfm.inst_nbr);
	    /* Save the index retrieved from LMAC */
...
static void rwnx_cfgp2p_stop_p2p_device(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	if (rwnx_vif == rwnx_hw->p2p_dev_vif) {
		rwnx_hw->is_p2p_alive = 0;
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		cancel_delayed_work_sync(&rwnx_hw->p2p_alive_timer);
#else
	...
	pr_info("Exit. P2P interface stopped\n");
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		if (time_after(jiffies, rwnx_vif->sta.external_auth_time
					+ msecs_to_jiffies(EXTERNAL_AUTH_TIMEOUT_MSEC))) {
			AICWFDBG(LOGERROR, "external auth timeout %lu ms\n",
					jiffies_to_msecs(jiffies - rwnx_vif->sta.external_auth_time));
			rwnx_external_auth_disable(rwnx_vif);
			rwnx_send_sm_external_auth_required_rsp(rwnx_hw, rwnx_vif, WLAN_STATUS_AUTH_TIMEOUT);
		} else {
			return -EBUSY;
		}
#else
...
)
{
	...
	do {
		...
		if(found) {
			...
			if (rwnx_vif->wdev.iftype == NL80211_IFTYPE_AP || rwnx_vif->wdev.iftype == NL80211_IFTYPE_P2P_GO) {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				pr_info("atc_cfg80211_del_sta mac %pM reason_code %u\n",
						cur->mac_addr, cur->reason_code);
				atc_cfg80211_del_sta(rwnx_vif->ndev, cur->mac_addr, GFP_KERNEL, cur->reason_code);
#else
...
void apm_staloss_work_process(struct work_struct *work)
{
    ...
    pr_info("apm vif idx=%d, found=%d, mac addr=%pM\n", rwnx_hw->apm_vif_idx, found, mac);
								...
								unsigned int duration, u64 *cookie, bool mgmt_roc_flag)
{
	...
	if (rwnx_hw->roc_elem) {
		msleep(2);
		if (rwnx_hw->roc_elem) {
			pr_info("remain_on_channel fail\n");
			return -EBUSY;
		}
	}
...
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0) */
{
	...
	if (!channel) {
		pr_info("mgmt_tx fail since channel\n");
		return -EINVAL;
	}
	...
	if (rwnx_hw->roc_elem) {
		...
		if ((rwnx_hw->roc_elem->chan->center_freq != channel->center_freq)
			...
			pr_info("mgmt rx chan invalid: %d, %d", rwnx_hw->roc_elem->chan->center_freq, channel->center_freq);
			return -EINVAL;
		}
	} else {
		u64 cookie;
...

{
	...
	if ((action_code == WLAN_TDLS_SETUP_REQUEST) &&
		...
		(rwnx_vif->tdls_status == TDLS_LINK_ACTIVE)) {
		pr_info("%s: only one TDLS link is supported!\n", __func__);
		return -ENOTSUPP;
	}

	if ((action_code == WLAN_TDLS_DISCOVERY_REQUEST) &&
		(rwnx_hw->mod_params->ps_on)) {
		pr_info("%s: discovery request is not supported when "
	...
	default:
		pr_info("%s: Unknown TDLS mgmt/action frame %pM\n",
				__func__, peer);
		ret = -EOPNOTSUPP;
		break;
	}
	...
	enum nl80211_tdls_operation oper)
{
	...
	if (!rwnx_vif->sta.tdls_sta) {
		pr_info("%s: TDLS station %pM does not exist\n", __func__, peer);
		return -ENOLINK;
	}
									  ...
									  struct cfg80211_chan_def *chandef)
{
	...
	if ((!rwnx_sta) || (memcmp(addr, rwnx_sta->mac_addr, ETH_ALEN))) {
		pr_info("%s: TDLS station %pM doesn't exist\n", __func__, addr);
		return -ENOLINK;
	}

	if (!rwnx_sta->tdls.chsw_allowed) {
		pr_info("%s: TDLS station %pM does not support TDLS channel switch\n", __func__, addr);
		return -ENOTSUPP;
	}
	...
	if (!cfm.status) {
		...
		pr_info("%s: TDLS channel switch already enabled and only one is supported\n", __func__);
		return -EALREADY;
	}
}
...
int rwnx_get_custom_mac_addr(u8_l *mac_addr_efuse)
{
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	u32 metaIndex = METAZONE_WIFI;
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
	u32 metaIndex = 0x10026;
#else
	u32 metaIndex = 0x10001;
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	MetaZone_ReadBinary(metaIndex, mac_addr, ETH_ALEN);
	pr_info("read mac from metazone:%pM\n", mac_addr);
	if (!is_valid_ether_addr(mac_addr)) {
		...
		mac_addr[2] = 0x22;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
		MetaZone_WriteBinary(metaIndex, mac_addr, ETH_ALEN);
#else
		MetaZone_SpecWriteBinary(metaIndex, mac_addr,
				ETH_ALEN, MTZ_WRITE_FLAG_FACTORYREST);
#endif
		MetaZone_Flush(0);
		pr_info("ATC random mac:%pM\n", mac_addr);
	}
	memcpy(mac_addr_efuse, mac_addr, ETH_ALEN);
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
extern int atc_wlan_power_on(int on);
#endif

static int __init rwnx_mod_init(void)
{
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	int ret = atc_wlan_power_on(1);
	if (ret) {
		AICWFDBG(LOGERROR, "atc_wlan_power_on failed %d\n", ret);
		return ret;
	}
#endif
	RWNX_DBG(RWNX_FN_ENTRY_STR);
...
#ifdef AICWF_PCIE_SUPPORT
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	ret = rwnx_platform_register_drv();
	if (ret) {
		AICWFDBG(LOGERROR, "rwnx_platform_register_drv failed %d\n", ret);
		atc_wlan_power_on(0);
	}
	return ret;
#else
