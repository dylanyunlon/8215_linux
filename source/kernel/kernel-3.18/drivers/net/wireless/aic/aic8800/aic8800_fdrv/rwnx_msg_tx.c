...
//		flags = GFP_KERNEL;
	...
	//pr_info("rwnx_msg_zalloc size=%d  id=%d\n",msg->param_len,msg->id);
						 ...
						 int reqcfm, lmac_msg_id_t reqid, void *cfm)
{
	...
	if (!test_bit(RWNX_DEV_STARTED, &rwnx_hw->drv_flags) &&
		...
		pr_crit("%s: bypassing (RWNX_DEV_RESTARTING set) 0x%02x\n",
			   __func__, reqid);
		kfree(msg);
		return -EBUSY;
	}
		...
		pr_crit("%s: bypassing (restart must have failed)\n", __func__);
		kfree(msg);
		return -EBUSY;
	}
#endif
	...
	if (cfm != NULL) {
		do {
			...
			if(!empty) {
				if(in_softirq()) {
        				pr_info("in_softirq:check cmdqueue empty\n");
					mdelay(10);
				}
				else {
        				pr_info("check cmdqueue empty\n");
					msleep(50);
				}
             		}
...
//	RWNX_DBG(RWNX_FN_ENTRY_STR);
    pr_info("%s (%d)%s reqcfm:%d in_irq:%d in_softirq:%d in_atomic:%d\n",
    __func__, reqid, RWNX_ID2STR(reqid), reqcfm, (int)in_irq(), (int)in_softirq(), (int)in_atomic());
...
int rwnx_send_rf_calib_req(struct rwnx_hw *rwnx_hw, struct mm_set_rf_calib_cfm *cfm)
{
	...
	{		
		...
		if (xtal_cap.enable) {
			pr_info("user xtal cap: %d, cap_fine: %d\n", xtal_cap.xtal_cap, xtal_cap.xtal_cap_fine);
			rf_calib_req->xtal_cap = xtal_cap.xtal_cap;
			rf_calib_req->xtal_cap_fine = xtal_cap.xtal_cap_fine;
		} else {
			rf_calib_req->xtal_cap = 0;
...
int rwnx_send_vendor_hwconfig_req(struct rwnx_hw *rwnx_hw, uint32_t hwconfig_id, int32_t *param, int32_t *param_out)
{
	...
	switch (hwconfig_id)
	{
		...
		pr_info("set_acs_txop_req: be: %x,bk: %x,vi: %x,vo: %x\n",
		...
		pr_info("set_mac_timescale_req:sifsA_time: %x, sifsB_time: %x, slot_time: %x, rx_startdelay ofdm:%x long %x short %x\n",
		...
		pr_info("cca_threshold_req: auto_cca_en:%d\ncca20p_rise_th = %d\ncca20s_rise_th = %d\ncca20p_fall_th = %d\ncca20s_fall_th = %d\n",
	...
	case CHIP_TEMP_GET_REQ:
                ...
                if (!error) {
                        ...
                        pr_info("get_chip_temp degree=%d\n", cfm.chip_temp_cfm.degree);
                } else {
                        pr_info("get_chip_temp err=%d\n", error);
                        }
                }
		break;
        ...
        case CUSTOMIZED_FREQ_REQ:
                ...
                pr_info("customized_freq_req: raw_freq[ %d %d %d %d ], map_freq[ %d %d %d %d]\n",
...
int rwnx_send_vendor_swconfig_req(struct rwnx_hw *rwnx_hw, uint32_t swconfig_id, int32_t *param_in, int32_t *param_out)
{
    ...
    switch (swconfig_id)
    {
        ...
        case TEMP_COMP_SET_REQ:
            ...
            pr_info("temp_comp_set_req: en=%d, tmr=%x\n",
        ...
        case EXT_FLAGS_SET_REQ:
            ...
            pr_info("ext_flags_set_req: flags=%x\n",
        ...
        case EXT_FLAGS_MASK_SET_REQ:
            ...
            pr_info("ext_flags_mask_set_req: flags mask=0x%x, val=0x%x\n",
							 ...
							 struct sm_connect_cfm *cfm)
{
	...
	pr_info("%s drv_vif_index:%d connect to %s(%d) channel:%d auth_type:%d\n",
		__func__,
	...
	for(index = 0; index < ie_req->add_ie_len; index++){
		if(ie_req->ie[index] == vendor_extension_id[0]){
			...
			if(ie_req->ie[index] == vendor_extension_id[1] &&
				...
				pr_info("%s find vendor_extension_subelement,index:%d len:%d\n", __func__, index, ie_req->ie[index]);
				break;
			}
		}
	}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			static DEFINE_RATELIMIT_STATE(_rs, 5 * HZ, 2);
			if (__ratelimit(&_rs)) {
							 ...
							 struct mesh_start_cfm *cfm)
{
	...
	if (setup->ie_len) {
		...
		if (dma_mapping_error(rwnx_hw->dev, req->ie_addr)) {
			pr_crit("%s - DMA Mapping error on additional IEs\n", __func__);
