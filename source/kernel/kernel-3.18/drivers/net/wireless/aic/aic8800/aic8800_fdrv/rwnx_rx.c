						 ...
						 struct sk_buff *skb,  struct hw_rxhdr *hw_rxhdr)
{
    ...
    if (ieee80211_is_assoc_req(mgmt->frame_control) && rwnx_vif->wdev.iftype == NL80211_IFTYPE_AP) {
		...
		#endif
		#endif
		pr_info("ASSOC_REQ: sta_idx %d MAC %pM\n", rwnx_vif->ap.aic_index, mgmt->sa);
...
int reord_single_frame_ind(struct aicwf_rx_priv *rx_priv, struct recv_msdu *prframe)
{
	...
	if(!prframe->forward) {
		//pr_info("single: %d not forward: drop\n", prframe->seq_num);
		dev_kfree_skb(skb);
		prframe->pkt = NULL;
	...
	if (!hw_rxhdr->hwvect.ga_frame) {
		if (((skb->data[0] & 0x0C) == 0) && (skb->data[1] & 0x40) == 0x40) { //protect management frame
			...
			(hw_rxhdr->hwvect.decr_status == RWNX_RX_HD_DECR_GCMP256)) {
				...
				pr_info("unsupport decr_status:%d\n", hw_rxhdr->hwvect.decr_status);
			}
		}
	}
}
    ...
    //if (is_amsdu) 
    {
        ...
        while (skb->len > 16) {
            ...
            if (skb->len > (sublen+14))
                ...
                pr_info("accroding to amsdu: this will not happen\n");
                break;
            }
            //pr_info("sublen = %d, %x, %x, %x, %x\n", sublen,skb->data[0], skb->data[1], skb->data[12], skb->data[13]);
#if 1
            sub_skb = __dev_alloc_skb(sublen - 6 + 12, GFP_ATOMIC);
            if(!sub_skb){
                pr_info("sub_skb alloc fail:%d\n", sublen);
                break;
            }
            ...
            rwnx_vif = rwnx_rx_get_vif(rwnx_hw, vif_idx);
            if (!rwnx_vif) {
                pr_info("Frame received but no active vif (%d)", vif_idx);
		...
		if ((skb->data[0] & 0x0f) == 0x08) {
			...
			if (b_4addr) {
				if ((skb->data[1] & 0x3) != 0x3) {
					pr_info("aicwf: 4addr DS error, to_from ds:%d\n", skb->data[1] & 0x3);
				...
				pr_info("aicwf 4addr: da: %pM, sa: %pM\n", ra, ta);
			} else {
				//pr_info("aicwf: to_from ds:%d\n", skb->data[1] & 0x3);
...
//Check NETGEAR R7000 router's AMSDU packet format for compliance. end
				...
				while (skb->len > 16) {
					...
					rwnx_vif = rwnx_rx_get_vif(rwnx_hw, hw_rxhdr->flags_vif_idx);
					if (!rwnx_vif) {
						pr_info("Frame received but no active vif (%d)", hw_rxhdr->flags_vif_idx);
						dev_kfree_skb(sub_skb);
						break;
					}
			...
			if (!hw_rxhdr->flags_need_reord && ((frame_ctrl & MAC_FCTRL_MOREFRAG) || frag_num)) {
				...
				if (frame_ctrl & MAC_FCTRL_MOREFRAG) {
					...
					//pr_info("rx frag: sn=%d, fn=%d, skb->len=%d\n", seq_num, frag_num, skb->len);
					if (defrag_info) {
						is_frag = 1;
		...
		if (hw_rxhdr->flags_is_80211_mpdu) {
			...
			if (hw_rxhdr->flags_sta_idx != RWNX_INVALID_STA) {
				...
				if (hw_rxhdr->flags_is_4addr && !rwnx_vif->use_4addr) {
					pr_info("aicwf: 4addr flag error\n");
#ifdef CONFIG_GKI
