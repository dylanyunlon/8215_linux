...
void rwnx_tx_push(struct rwnx_hw *rwnx_hw, struct rwnx_txhdr *txhdr, int flags)
{
	...
	if (((sw_txhdr->desc.host.flags & TXU_CNTRL_MGMT) && \
		...
		pr_info("push need cfm flags 0x%x\n", sw_txhdr->desc.host.flags);
		sw_txhdr->need_cfm = 1;
	...
	/* if(check_fwstate(pmlmepriv, WIFI_STATION_STATE|WIFI_ADHOC_STATE) == _TRUE) */
	{
			...
			/* if (!priv->pmib->ethBrExtInfo.nat25_disable)		 */
		{
			...
			if (do_nat25) {
				...
				if (res < 0) {
					pr_info("TX DROP: skb_linearize fail!\n");
					/* goto free_and_stop; */
					return -1;
				}
				...
				if (res < 0) {
					if (res == -2) {
						/* priv->ext_stats.tx_drops++; */
						pr_info("TX DROP: nat25_db_handle fail!\n");
						/* goto free_and_stop; */
						return -1;
		...
		if (memcmp(skb->data + MACADDRLEN, vif->ndev->dev_addr, MACADDRLEN)) {
			/* priv->ext_stats.tx_drops++; */
			pr_info("TX DROP: untransformed frame SA:%02X%02X%02X%02X%02X%02X!\n",
...
int rwnx_txdatacfm(void *pthis, void *host_id)
{
	...
	if (sw_txhdr->desc.host.flags & TXU_CNTRL_MGMT) {
		pr_info("done=%d retry_required=%d sw_retry_required=%d acknowledged=%d\n",
