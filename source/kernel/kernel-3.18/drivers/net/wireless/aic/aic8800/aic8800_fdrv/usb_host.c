				  ...
				  void *pthis)
{
	...
	//pr_info("[Gaofx]rwnx_init_aic pthis %p !\n", pthis);
...
void aicwf_usb_host_tx_cfm_handler(struct usb_host_env_tag *env, u32 *data)
{
	...
	pr_info("%s enter \n", __func__);
	//pr_info("sdio_host_tx_cfm_handler, %d, 0x%08x\n", queue_idx, data[1]);
	...
	//while (1)
	{
		...
		if (host_id == 0) {
			...
			pr_info("ERROR:No more confirmations\n");
			return;
			//break;
		}
		// set the cfm status
		...
		//pr_info("sdio_host_tx_cfm_handler, 0x%p\n", env->pthis);
