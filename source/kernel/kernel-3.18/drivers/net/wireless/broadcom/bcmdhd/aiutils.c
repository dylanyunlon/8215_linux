...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	if (sii->osh == NULL) {
		SI_ERROR(("%s: ai_corereg sii->osh is null\n", __FUNCTION__));
		dump_stack();
		return -1;
	}
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	SPINWAIT((status = R_REG(sii->osh, &ai->resetstatus)), 300);
#else // PVS V562
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		SPINWAIT((status = R_REG(sii->osh, &ai->resetstatus)), 10000);
#else // PVS V562
...
_ai_core_reset(si_t *sih, uint32 bits, uint32 resetbits)
{
	...
	/* ensure there are no pending backplane operations */
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	SPINWAIT((dummy = R_REG(sii->osh, &ai->resetstatus)), SPINWAIT_TIME_US);
#else // PVS V562
	...
	/* ensure there are no pending backplane operations */
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	SPINWAIT(R_REG(sii->osh, &ai->resetstatus), SPINWAIT_TIME_US);
#else // PVS V562
...
ai_force_clocks(si_t *sih, uint clock_state)
{
	...
	/* ensure there are no pending backplane operations */
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	SPINWAIT(R_REG(sii->osh, &ai->resetstatus), 300);
#else // PVS V562
