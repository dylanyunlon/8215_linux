...
uint sd_msglevel = SDH_ERROR_VAL | SDH_INFO_VAL;
#else
...
#define CHECK_IN_ATOMIC() do { \
	if (unlikely(in_atomic())) { \
		sd_err(("maybe scheduling while atomic, " \
...
bool sdioh_io_blocked = false;

static int _sdioh_claim_host_timeout(struct sdio_func *func);
static int _sdioh_err_handler(unsigned long start_time, int err);
...
sdioh_sdmmc_card_enablefuncs(sdioh_info_t *sd)
{
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	start_time = jiffies;
	err_ret = _sdioh_claim_host_timeout(sd->func[1]);
	if (!err_ret) {
#else
	...
	_sdioh_err_handler(start_time, err_ret);
#endif
	if (err_ret) {
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	start_time = jiffies;
	err_ret = _sdioh_claim_host_timeout(sd->func[2]);
	if (!err_ret) {
#else
...
sdioh_detach(osl_t *osh, sdioh_info_t *sd)
{
	...
	if (sd) {
		...
		if (sd->func[2]) {
			...
			int ret = _sdioh_claim_host_timeout(sd->func[2]);
			if (!ret) {
				ret = sdio_disable_func(sd->func[2]);
				sdio_release_host(sd->func[2]);
			}
			_sdioh_err_handler(start_time, ret);
#else
			sdio_claim_host(sd->func[2]);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			unsigned long start_time = jiffies;
			int ret = _sdioh_claim_host_timeout(sd->func[1]);
			if (!ret) {
				ret = sdio_disable_func(sd->func[1]);
				sdio_release_host(sd->func[1]);
			}
			_sdioh_err_handler(start_time, ret);
#else
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
extern SDIOH_API_RC _sdioh_enable_func_intr(sdioh_info_t *sd)
#else
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	err = _sdioh_claim_host_timeout(sd->func[0]);
	if (err) {
		sd_err(("_sdioh_claim_host_timeout\n"));
		return SDIOH_API_RC_FAIL;
	}
#else
...
extern SDIOH_API_RC sdioh_enable_func_intr(sdioh_info_t *sd)
{
	...
	do {
		...
		if (sdioh_io_blocked) {
			sd_err(("sdioh_io_blocked\n"));
			ret = -EIO;
			break;
		}
		retry++;
		ret = _sdioh_enable_func_intr(sd);
		_sdioh_err_handler(start_time, ret);
	} while (ret && retry < SDIO_RETRY_TIMES);

	return ret ? SDIOH_API_RC_FAIL : SDIOH_API_RC_SUCCESS;
}
#endif // CONFIG_ATC_AOSP_ENHANCEMENT

#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
extern SDIOH_API_RC _sdioh_disable_func_intr(sdioh_info_t *sd)
#else
...
extern SDIOH_API_RC sdioh_disable_func_intr(sdioh_info_t *sd)
{
	...
	do {
		...
		retry++;
		ret = _sdioh_disable_func_intr(sd);
		_sdioh_err_handler(start_time, ret);
	} while (ret && retry < SDIO_RETRY_TIMES);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		start_time = jiffies;
		ret = _sdioh_claim_host_timeout(sd->func[2]);
		if (!ret) {
			...
			_sdioh_err_handler(start_time, ret);
			sd_err(("_sdioh_claim_host_timeout ret %d", ret));
			return SDIOH_API_RC_FAIL;
		}
#else
		sdio_claim_host(sd->func[2]);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		start_time = jiffies;
		ret = _sdioh_claim_host_timeout(sd->func[1]);
		if (!ret) {
...
#elif defined(HW_OOB)
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	ret = sdioh_enable_func_intr(sd);
	if (ret) {
		sd_err(("sdioh_enable_func_intr ret %d", ret));
		return SDIOH_API_RC_FAIL;
	}
#else
...
#elif defined(HW_OOB)
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	ret = sdioh_disable_func_intr(sd);
	if (ret) {
		sd_err(("sdioh_disable_func_intr ret %d", ret));
	}
	ret2 |= ret;
#else
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		start_time = jiffies;
		bcmerror = _sdioh_claim_host_timeout(si->func[func]);
		if (!bcmerror) {
			bcmerror = sdio_set_block_size(si->func[func], blksize);
			sdio_release_host(si->func[func]);
		}
		_sdioh_err_handler(start_time, bcmerror);
		if (bcmerror) {
			sd_err(("Failed to set F%d blocksize to %d(%d)\n",
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
static int _sdioh_request_byte(sdioh_info_t *sd,
		uint rw, uint func, uint regaddr, uint8 *byte)
#else
extern SDIOH_API_RC sdioh_request_byte(sdioh_info_t *sd,
		uint rw, uint func, uint regaddr, uint8 *byte)
#endif
{
	int err_ret = 0;
	...
	if(rw) { /* CMD52 Write */
		if (func == 0) {
			...
			if (regaddr == SDIOD_CCCR_IOEN) {
				...
				do {
				if (sd->func[3]) {
					sd_debug(("bcmsdh_sdmmc F3: *byte 0x%x\n", *byte));

					if (*byte & SDIO_FUNC_ENABLE_3) {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
						err_ret = _sdioh_claim_host_timeout(sd->func[3]);
						if (err_ret) {
							sd_err(("_sdioh_claim_host_timeout %d\n", err_ret));
							break;
						}
#else
						...
						/* Enable Function 3 */
						sd_debug(("bcmsdh_sdmmc F3: enable F3 fn %p\n",
						sd->func[3]));
...
#endif
						/* Disable Function 3 */
						sd_debug(("bcmsdh_sdmmc F3: disable F3 fn %p\n",
						sd->func[3]));
			...
			/* to allow abort command through F1 */
				...
				while (sdio_abort_retry--) {
					if (sd->func[func]) {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
						err_ret = _sdioh_claim_host_timeout(sd->func[func]);
						if (err_ret) {
							sd_err(("_sdioh_claim_host_timeout %d\n", err_ret));
							continue;
						}
...
/* _sdioh_err_handler logic
 *
 * 1. host rw continuous timeout > SDIO_RW_TIMEOUT_MSEC
 *    SDIO_HOST_TMO_TRG_THRESHOLD times
 *    -> trigger reset
 *
 * 2. sdio continuous error SDIO_ERR_TRG_THRESHOLD times
 *    -> trigger reset
 *
 * 3. sdio cumulantive error SDIO_CUMU_ERR_TRG_THRESHOLD times
 *    -> trigger reset
 *
 * 4. sdio error interval > SDIO_CUMU_ERR_RESET_TIME_MSEC
 *    -> reset sdio cumulantive error count
 */
#define SDIO_RW_TIMEOUT_MSEC    2000
#define SDIO_CUMU_ERR_RESET_TIME_MSEC   5000

#define SDIO_HOST_TMO_TRG_THRESHOLD     5
#define SDIO_CONT_ERR_TRG_THRESHOLD     10
#define SDIO_CONT_ERR_TA_THRESHOLD      20
#define SDIO_CUMU_ERR_TRG_THRESHOLD     20
#define SDIO_CUMU_ERR_TA_THRESHOLD      40

static int _sdioh_err_handler(unsigned long start_time, int err)
{
	// sdio continuous error count
	static atomic_t cont_err_cnt = ATOMIC_INIT(0);
	// sdio host continuous timeout count
	static atomic_t host_timeout_cnt = ATOMIC_INIT(0);
	// sdio cumulantive ok count
	static atomic_t cumu_ok_cnt = ATOMIC_INIT(0);
	// sdio cumulantive error count
	static atomic_t cumu_err_cnt = ATOMIC_INIT(0);

	static unsigned long prev_err_time = INITIAL_JIFFIES;

	unsigned long timeout = start_time
		+ msecs_to_jiffies(SDIO_RW_TIMEOUT_MSEC);

	if (time_is_before_jiffies(timeout)) {
		atomic_inc(&host_timeout_cnt);
		sd_err(("sdio rw time %u ms > %u ms, "
			"host_timeout_cnt %d err %d\n",
			jiffies_to_msecs(jiffies - start_time),
			SDIO_RW_TIMEOUT_MSEC,
			atomic_read(&host_timeout_cnt), err));
		if (atomic_read(&host_timeout_cnt) == 1) {
			//dump_stack();
			atc_combo_dump_cpu_task_simple();
		}
	}
	if (!err) {
		atomic_inc(&cumu_ok_cnt);
		if (atomic_xchg(&cont_err_cnt, 0)) {
			sd_info(("reset cont_err_cnt\n"));
			dump_stack();
		}
		if (atomic_xchg(&host_timeout_cnt, 0)) {
			sd_info(("reset host_timeout_cnt\n"));
			dump_stack();
		}
		return 0;
	}
	if (time_is_before_jiffies(prev_err_time
			+ msecs_to_jiffies(SDIO_CUMU_ERR_RESET_TIME_MSEC))) {
		if (atomic_xchg(&cumu_ok_cnt, 0)) {
			sd_info(("reset cumu_ok_cnt\n"));
		}
		if (atomic_xchg(&cumu_err_cnt, 0)) {
			sd_info(("reset cumu_err_cnt\n"));
		}
	}
	prev_err_time = jiffies;
	atomic_inc(&cumu_err_cnt);
	atomic_inc(&cont_err_cnt);
	sd_err(("sdio rw err %d cumu_ok_cnt %d cumu_err_cnt %d "
		"cont_err_cnt %d host_timeout_cnt %d\n",
		err, atomic_read(&cumu_ok_cnt), atomic_read(&cumu_err_cnt),
		atomic_read(&cont_err_cnt), atomic_read(&host_timeout_cnt)));
	if (atomic_read(&cont_err_cnt) == 1) {
		dump_stack();
	}

	if (atomic_read(&host_timeout_cnt) == SDIO_HOST_TMO_TRG_THRESHOLD) {
		sd_err(("sdio host -ETIMEDOUT continually, trigger wlan reset\n"));
		sdioh_io_blocked = true;
		dhd_schedule_reset(NULL);
		return 0;
	}
	if (atomic_read(&cont_err_cnt) == SDIO_CONT_ERR_TRG_THRESHOLD) {
		sd_err(("sdio error continually, trigger wlan reset\n"));
		sdioh_io_blocked = true;
		dhd_schedule_reset(NULL);
		return 0;
	} else if (atomic_read(&cont_err_cnt) >= SDIO_CONT_ERR_TA_THRESHOLD) {
		atomic_set(&cont_err_cnt, 0);
		sd_info(("turnaround reset cont_err_cnt\n"));
	}
	if (atomic_read(&cumu_err_cnt) == SDIO_CUMU_ERR_TRG_THRESHOLD) {
		sd_err(("sdio cumulative error count reach %d, trigger wlan reset\n",
				SDIO_CUMU_ERR_TRG_THRESHOLD));
		sdioh_io_blocked = true;
		dhd_schedule_reset(NULL);
		return 0;
	} else if (atomic_read(&cumu_err_cnt) >= SDIO_CUMU_ERR_TA_THRESHOLD) {
		atomic_set(&cumu_err_cnt, 0);
		sd_info(("turnaround reset cumu_err_cnt\n"));
	}
...
static int _sdioh_claim_host_timeout(struct sdio_func *func)
{
	return atc_combo_sdio_claim_host_timeout(func, SDIO_RW_TIMEOUT_MSEC);
}
		...
		uint rw, uint func, uint regaddr, uint8 *byte)
{
	...
	do {
		...
		retry++;
		ret = _sdioh_request_byte(sd, rw, func, regaddr, byte);
		if (ret) {
			sd_err(("func%u addr 0x%x %s fail %d retry %d\n",
				func, regaddr, rw ? "write" : "read", ret, retry));
		}
		_sdioh_err_handler(start_time, ret);
	} while (ret && retry < SDIO_RETRY_TIMES);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
static int _sdioh_request_word(sdioh_info_t *sd,
		uint cmd_type, uint rw, uint func, uint addr,
		uint32 *word, uint nbytes)
#else
extern SDIOH_API_RC sdioh_request_word(sdioh_info_t *sd,
		uint cmd_type, uint rw, uint func, uint addr,
		uint32 *word, uint nbytes)
#endif
{
	...
	int sdio_abort_retry = MMC_SDIO_ABORT_RETRY_LIMIT;
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	int err_ret2 = SDIOH_API_RC_FAIL;
#endif
#endif // endif
	...
	if (err_ret) {
#if defined(MMC_SDIO_ABORT)
		/* Any error on CMD53 transaction should abort
		 * that function using function 0. */
		while (sdio_abort_retry--) {
			if (sd->func[0]) {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				err_ret = _sdioh_claim_host_timeout(sd->func[0]);
				if (err_ret) {
					sd_err(("_sdioh_claim_host_timeout %d\n", err_ret));
					continue;
				}
...
#else
				sdio_writeb(sd->func[0], func, SDIOD_CCCR_IOABORT, &err_ret);
#endif
				sdio_release_host(sd->func[0]);
			}
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		...
		uint32 *word, uint nbytes)
{
	...
	do {
		...
		retry++;
		ret = _sdioh_request_word(sd, cmd_type,
				rw, func, addr, word, nbytes);
		if (ret) {
			sd_err(("func%u cmd_type %u addr 0x%x len %u "
				"%s fail %d retry %d\n",
				func, cmd_type, addr, nbytes,
				rw ? "write" : "read", ret, retry));
		}
		_sdioh_err_handler(start_time, ret);
	} while (ret && retry < SDIO_RETRY_TIMES);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		err_ret = _sdioh_claim_host_timeout(sdio_func);
		if (!err_ret) {
#else
 ...
 */
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
static SDIOH_API_RC _sdioh_request_buffer(sdioh_info_t *sd,
		uint pio_dma, uint fix_inc, uint write, uint func,
		uint addr, uint reg_width, uint buf_len,
		uint8 *buffer, void *pkt)
#else
extern SDIOH_API_RC sdioh_request_buffer(sdioh_info_t *sd,
		uint pio_dma, uint fix_inc, uint write, uint func,
		uint addr, uint reg_width, uint buf_len,
		uint8 *buffer, void *pkt)
#endif
...
{
	int retry = 0;
	SDIOH_API_RC ret = SDIOH_API_RC_FAIL;
	...
	do {
		...
		retry++;
		ret = _sdioh_request_buffer(sd, pio_dma, fix_inc, write,
				func, addr, reg_width, buf_len, buffer, pkt);
		if (!SDIOH_API_SUCCESS(ret)) {
			sd_err(("func%u %s %s addr 0x%x width %u len %u "
				"%s fail %d retry %d\n",
				func, pio_dma == SDIOH_DATA_DMA ? "dma" : "pio",
				fix_inc == SDIOH_DATA_INC ? "inc" : "fix",
				addr, reg_width, buf_len,
				write ? "write" : "read", ret, retry));
		}
		_sdioh_err_handler(start_time, ret);
	} while (!SDIOH_API_SUCCESS(ret) && retry < SDIO_RETRY_TIMES);
...
static void IRQHandler(struct sdio_func *func)
{
	...
	do {
		//unsigned long start_time = jiffies;
		retry++;
		ret = _sdioh_claim_host_timeout(sd->func[0]);
		//don't reset timeout counter here
		//_sdioh_err_handler(start_time, ret);
	} while (ret && retry < SDIO_RETRY_TIMES);
#else
	sdio_claim_host(sd->func[0]);
		...
		*/
		if ((ret = sdio_reset_comm(sd->func[0]->card))) {
			...
			if (sd->func[1]) {
				...
				_sdioh_err_handler(start_time, ret);
				if (ret) {
					sd_err(("set F1 blocksize failed %d\n", ret));
					return ret;
				}
#else
				/* Claim host controller */
			...
			if (sd->func[2]) {
				...
				_sdioh_err_handler(start_time, ret);
				if (ret) {
					sd_err(("set F2 blocksize %d failed %d\n", sd_f2_blocksize, ret));
					return ret;
				}
#else
				/* Claim host controller F2 */
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			ret = sdioh_sdmmc_card_enablefuncs(sd);
			if (ret) {
				sd_err(("sdioh_sdmmc_card_enablefuncs ret %d\n", ret));
				return ret;
			}
#else
			sdioh_sdmmc_card_enablefuncs(sd);
			...
			_sdioh_err_handler(start_time, ret);
			if (ret) {
				sd_err(("sdio_claim_irq failed %d\n", ret));
				return ret;
			}
#endif
			...
			ret = sdioh_enable_func_intr(sd);
			if (ret) {
				sd_err(("sdioh_enable_func_intr ret %d\n", ret));
				return ret;
			}
#else
			sdioh_enable_func_intr(sd);
...
sdioh_stop(sdioh_info_t *sd)
{
	...
	*/
	if (sd->func[0]) {
		...
		_sdioh_err_handler(start_time, ret);
		if (ret) {
			sd_err(("sdio_release_irq failed %d\n", ret));
		}
		ret2 |= ret;
#endif
#else /* defined(OOB_INTR_ONLY) */
		...
		ret = sdioh_disable_func_intr(sd);
		if (ret) {
			sd_err(("sdioh_disable_func_intr ret %d\n", ret));
			return ret;
		}
		ret2 |= ret;
#else
...
#endif // endif
	DHD_TRACE(("%s: Before change: sd clock rate is %u\n", __FUNCTION__, ios->clock));
	if (hz < host->f_min) {
