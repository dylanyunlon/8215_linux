...
#define SDIO_VENDOR_ID_BROADCOM2	0x021c
...
static int sdioh_probe(struct sdio_func *func)
{
	...
	if (adapter  != NULL)
		...
		sd_info(("can't find adapter info for this chip\n"));
                              ...
                              const struct sdio_device_id *id)
{
	...
	sd_info(("bcmsdh_sdmmc: %s Exit ret %d\n", __FUNCTION__, ret));
...
static const struct sdio_device_id bcmsdh_sdmmc_ids[] = {
	...
	{ SDIO_DEVICE(SDIO_VENDOR_ID_BROADCOM2, BCM1B7E_CHIP_ID) },
...
static bool sdio_driver_registered = false;

int bcmsdh_register_client_driver(void)
{
	int ret = 0;

	bcmsdh_sdmmc_probe_ret = 0;
	ret = sdio_register_driver(&bcmsdh_sdmmc_driver);
	// __driver_attach will drop probe fail
	if (!ret && bcmsdh_sdmmc_probe_ret) {
		sd_err(("bcmsdh_sdmmc_probe fail %d\n",
				bcmsdh_sdmmc_probe_ret));
		sdio_unregister_driver(&bcmsdh_sdmmc_driver);
		ret = bcmsdh_sdmmc_probe_ret;
	}
	sdio_driver_registered = !ret;
