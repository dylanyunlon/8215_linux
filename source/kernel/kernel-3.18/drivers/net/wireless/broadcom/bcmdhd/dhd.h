...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#define DHD_LOG_DUMP_TS_SEC_PER_MIN         60
#define DHD_LOG_DUMP_TS_FMT_YYMMDDHHMMSSMS  "%02d%02d%02d%02d%02d%02d%03d"
#else
...
#endif /* REVERSE_AIFSN */
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	bool proc_dhd_trace_created;
	bool proc_dhd_ecounters_created;
	bool proc_dhd_rtt_created;
#endif
} dhd_pub_t;
...
WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(dhd_workitem_context_t, dhd_get_dhd_workitem_context)
	...
	#define DHD_PM_RESUME_RETURN_ERROR(a)   do { \
			if (dhd_mmc_suspend) { \
				DHD_ERROR(("mmc is still in suspend state!!!\n")); \
				return a; \
			} \
		} while (0)
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#define DHD_RESET_TYPE_RESET        1
#define DHD_RESET_TYPE_DUMP_RESET   2
#define DHD_RESET_TYPE_QB_OFF       11
#define DHD_RESET_TYPE_QB_ON        12
extern void dhd_schedule_reset(dhd_pub_t *dhdp);
extern void dhd_schedule_dump_reset(dhd_pub_t *dhdp);
#else // ! CONFIG_ATC_AOSP_ENHANCEMENT
...
#define DHD_PCIE_CONFIG_SAVE(bus)	_pci_save_state(bus->dev)
#define DHD_PCIE_CONFIG_RESTORE(bus)	_pci_restore_state(bus->dev)
