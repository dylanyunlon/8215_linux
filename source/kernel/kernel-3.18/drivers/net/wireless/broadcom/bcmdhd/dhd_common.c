...
#ifdef USER_DEBUG
int dhd_msg_level = DEFAULT_DHD_LOG_LEVEL | DHD_IOVAR_MEM_VAL;
#else
int dhd_msg_level = DEFAULT_DHD_LOG_LEVEL;
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
const char dhd_version[] = "Wlan Driver version: " EPI_VERSION_STR;
#else
...
dhd_get_sssr_reg_info(dhd_pub_t *dhd)
{
	...
	if (ret < 0) {
		DHD_INFO(("%s: sssr_reg_info failed (error=%d)\n",
			__FUNCTION__, ret));
		return BCME_ERROR;
	}
...
dhd_sssr_dump_init(dhd_pub_t *dhd)
{
	...
	if (dhd_get_sssr_reg_info(dhd) != BCME_OK) {
		DHD_INFO(("%s: dhd_get_sssr_reg_info failed\n", __FUNCTION__));
		return BCME_ERROR;
	}
...
static void ioctl_dump(char *tag, int ret,
		dhd_ioctl_t *dhd_ioc, wl_ioctl_t *wl_ioc)
{
	wl_ioctl_t *ioc = NULL;
	char *buf = NULL;
	int val = 0;
	...
	ioc = dhd_ioc ? (wl_ioctl_t *)dhd_ioc : wl_ioc;
	if (!ioc) {
		DHD_ERROR(("null ioc\n"));
		return;
	}
	if (!ioc->buf) {
		buf = "(nul)";
		val = 0;
	} else if (bcm_isascii(*(char *)ioc->buf)
			&& (bcm_isalpha(*(char *)ioc->buf)
				|| '_' == *(char *)ioc->buf)) {
		buf = (char *)ioc->buf;
		val = *(int *)(ioc->buf + strlen(ioc->buf) + 1);
	} else {
		buf = "?";
		val = *(int *)ioc->buf;
	}
	DHD_INFO(("%s: cmd %u %s buf %s val 0x%x len %u "
		"used %d needed %d ret %d\n",
		tag, ioc->cmd, ioc->set ? "set" : "get",
		buf, val, ioc->len,
		ioc->used, ioc->needed, ret));

	if (!ioc->buf) {
		return;
	}
	n = 0;
	for (i = 0; i < 16 && i < ioc->len; i++) {
		n += sprintf(tmp + n, "%02x ", ((char *)ioc->buf)[i]);
	}
	n += sprintf(tmp + n, "  ");
	for (i = 0; i < 16 && i < ioc->len; i++) {
		c = ((char *)ioc->buf)[i];
		n += sprintf(tmp + n, "%c",
				bcm_isascii(c) && bcm_isprint(c) ? c : '.');
	}
	DHD_INFO(("%s: %s\n", tag, tmp));
...
int dhd_ioctl_cmd(dhd_pub_t *dhd_pub,
		int cmd, void *arg, int len, uint8 set)
{
	dhd_ioctl_t ioc;
	...
	return dhd_ioctl(dhd_pub, &ioc, arg, len);
}

int dhd_ioctl_get_intiovar(dhd_pub_t *dhd_pub,
		char *name, uint *pval, int cmd, uint8 set)
{
	char iovbuf[WLC_IOCTL_SMLEN] = {0};
	int ret = -1;

	if (bcm_mkiovar(name, NULL, 0, iovbuf, sizeof(iovbuf))) {
		ret = dhd_ioctl_cmd(dhd_pub, cmd,
				iovbuf, sizeof(iovbuf), set);
		if (!ret) {
			*pval = ltoh32(*(uint*)iovbuf); //-V1032
		} else {
			DHD_ERROR(("get int iovar %s fail %d\n", name, ret));
		}
	} else {
		DHD_ERROR(("mkiovar %s fail\n", name));
	}

	return ret;
}

int dhd_ioctl_set_intiovar(dhd_pub_t *dhd_pub,
		char *name, uint val, int cmd, uint8 set)
{
	char iovbuf[WLC_IOCTL_SMLEN] = {0};
	int ret = -1;
	int lval = htol32(val);
	...
	len = bcm_mkiovar(name, (char*)&lval, sizeof(lval),
			iovbuf, sizeof(iovbuf));
	if (len) {
		ret = dhd_ioctl_cmd(dhd_pub, cmd, iovbuf, len, set);
		if (ret) {
			DHD_ERROR(("set int iovar %s fail %d\n", name, ret));
		}
	} else {
		DHD_ERROR(("mkiovar %s fail\n", name));
	}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	ioctl_dump("dhd_wl_ioctl start", 0, NULL, ioc);
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				if (ioc->cmd == WLC_SCAN_RESULTS && buf) {
					struct wl_scan_results *bss_list =
						(struct wl_scan_results *)buf;
					wl_bss_info_t *bss = next_bss(bss_list, NULL);
					s32 i = 0;

					DHD_INFO(("WLC_SCAN_RESULTS\n"));
					for_each_bss(bss_list, bss, i) {
						DHD_INFO(("%d/%d %s("MACDBG") RSSI %d\n",
								i, bss_list->count,
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	ioctl_dump("dhd_wl_ioctl end", ret, NULL, ioc);
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	ioctl_dump("dhd_ioctl start", 0, ioc, NULL);
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	ioctl_dump("dhd_ioctl end", bcmerror, ioc, NULL);
#endif
	...
	void *raw_event_ptr, char *eventmask)
{
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		DHD_EVENT(("MACEVENT: %s, opcode:0x%d  ifidx:%d role:%s\n",
			event_name, ifevent->opcode, ifevent->ifidx,
			wl_if_role2str(ifevent->role)));
#else
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#ifdef WL_SAE_EXTERNAL_SME // cypress wpa3 patch
	case WLC_E_EXT_AUTH_REQ:
		{
			DHD_EVENT(("WLC_E_EXT_AUTH_REQ\n"));
			break;
		}
	case WLC_E_EXT_AUTH_FRAME_RX:
		{
			DHD_EVENT(("WLC_E_EXT_AUTH_FRAME_RX\n"));
			break;
		}
	case WLC_E_MGMT_FRAME_TXSTATUS:
		{
			DHD_EVENT(("WLC_E_MGMT_FRAME_TXSTATUS\n"));
			break;
		}
	case WLC_E_MGMT_FRAME_OFF_CHAN_COMPLETE:
		{
			DHD_EVENT(("WLC_E_MGMT_FRAME_OFF_CHAN_COMPLETE\n"));
			break;
		}
	...
	default:
		DHD_TRACE(("MACEVENT: %s %d, MAC %s, status %d, reason %d, auth %d\n",
	...
	if ((DHD_BYTES_ON() || (host_data == TRUE)) //-V560
	...
	wl_event_msg_t *event, void **data_ptr, void *raw_event)
{
	...
	case WLC_E_IF:
		...
		if (ifevent->ifidx > 0 && ifevent->ifidx < DHD_MAX_IFS) {
			...
			DHD_TRACE(("%s: Invalid ifidx %d for %s\n",
			   __FUNCTION__, ifevent->ifidx, event->ifname));
	...
	case WLC_E_LINK:
		...
		if (!flags) {
			DHD_INFO(("%s: Deleting all STA from assoc list and flowrings.\n",
				__FUNCTION__));
...
dhd_pktfilter_offload_enable(dhd_pub_t * dhd, char *arg, int enable, int master_mode)
{
	...
	pkt_filterp = (wl_pkt_filter_enable_t *)(buf + str_len + 1); //-V1032
...
dhd_arp_offload_enable(dhd_pub_t * dhd, int arp_enable)
{
	...
	if (arp_enable) {
		...
		if (retcode) {
			DHD_TRACE(("%s: fail to get version (maybe version 1:retcode = %d\n",
				__FUNCTION__, retcode));
			dhd->arp_version = 1;
		}
		else {
			DHD_TRACE(("%s: ARP Version= %x\n", __FUNCTION__, version));
...
dhd_ndo_enable(dhd_pub_t * dhd, int ndo_enable)
{
	...
	if (wl_cfgnan_is_dp_active(dhd_linux_get_primary_netdev(dhd))) {
		...
		DHD_TRACE(("Active NAN DP, skip NDO\n"));
		return 0;
	}
...
dhd_ndo_get_version(dhd_pub_t *dhdp)
{
	...
	if (retcode) {
		DHD_INFO(("%s: failed, retcode = %d\n", __FUNCTION__, retcode));
		/* ver iovar not supported. NDO version is 0 */
		ver = 0;
	} else {
		wl_nd_hostip_t *ndo_ver_ret = (wl_nd_hostip_t *)iovbuf; //-V1032
...
dhd_ndo_add_ip_with_type(dhd_pub_t *dhdp, char *ipv6addr, uint8 type, int idx)
{
	...
	if (retcode) {
		...
		if (retcode == BCME_NORESOURCE) {
			...
			DHD_TRACE(("%s: Host IP count exceed device capacity,"
...
dhd_sroam_set_mon(dhd_pub_t *dhd, bool set)
{
	...
	if (set && (dhd->op_mode &
		...
		DHD_TRACE((" Failed to set sroam %d, op_mode 0x%04x\n", set, dhd->op_mode));
		return ret;
	}

	if (!dhd->sroam_turn_on) {
		DHD_TRACE((" Failed to set sroam %d, sroam turn %d\n", set, dhd->sroam_turn_on));
		return ret;
	}
	...
	sroam->sroam_on = set;
	DHD_TRACE((" Silent roam monitor mode %s\n", set ? "On" : "Off"));
...
int dhd_keep_alive_onoff(dhd_pub_t *dhd)
{
	...
	buf[ sizeof(buf) - 1 ] = '\0';
	mkeep_alive_pktp = (wl_mkeep_alive_pkt_t *) (buf + str_len + 1); //-V1032
...
dhd_check_current_clm_data(dhd_pub_t *dhd)
{
	...
	cspec = (wl_country_t *)iovbuf; //-V1032
	if ((strncmp(cspec->ccode, WL_CCODE_NULL_COUNTRY, WLC_CNTRY_BUF_SZ)) == 0) {
		DHD_INFO(("%s: ----- This FW is not included CLM data -----\n",
			__FUNCTION__));
		return FALSE;
	} 
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	else {
		DHD_INFO(("%s: ----- This FW is included CLM data -----\n",
					__FUNCTION__));
		return TRUE;
	}
#else
...
dhd_apply_default_clm(dhd_pub_t *dhd, char *clm_path)
{
	...
	if ((len > 0) && (len < MAX_CLM_BUF_SIZE) && memblock) {
		...
		if (err) {
			...
			DHD_TRACE(("%s: CLM download succeeded \n", __FUNCTION__));
		}
	} else {
	...
	if (trailer->log_magic == LOGSTRS_MAGIC) {
		...
		*/
		if (trailer->version == 1) {
			...
			DHD_TRACE(("%s: logstr header version = %u\n",
			...
			DHD_TRACE(("%s: logstr header version = %u; flags = %x\n",
...
dhd_tput_test(dhd_pub_t *dhd, tput_test_t *tput_data)
{
	...
	if (tput_data->direction == TPUT_DIR_TX) {
		...
		do {
			...
			for (i = 0; (i < dhd->batch_tx_num_pkts) || (tx_stop_pkt); ++i) {
				...
				*crc = hton32(hndcrc32(pktdata + tput_pkt_hdr_size + 4,
				...
				if (err != BCME_OK) {
					DHD_TRACE(("%s: send pkt (id = %u) fails (err = %d) ! \n",
			...
			DHD_TRACE(("%s: TX done, wait for completion...\n", __FUNCTION__));
		...
		/* TPUT_DIR_RX */
		DHD_TRACE(("%s: waiting for RX completion... \n", __FUNCTION__));
	...
	DHD_TRACE(("%s: DONE. tput = %llu bps, time = %llu ms\n", __FUNCTION__,
...
dhd_tput_test_rx(dhd_pub_t *dhd, void *pkt)
{
	...
	if ((memcmp(tput_pkt->mac_ap, dhd->tput_data.mac_ap,
		...
		(memcmp(tput_pkt->mac_sta, dhd->tput_data.mac_sta,
			ETHER_ADDR_LEN) != 0)) {
		dhd->tput_data.pkts_bad++;
		DHD_TRACE(("%s: dropping tput pkt with id %u due to bad AP/STA mac !\n",
	...
	if (crc != ntoh32(tput_pkt->crc32)) {
		DHD_TRACE(("%s: dropping tput pkt with id %u due to bad CRC !\n",
			__FUNCTION__, ntoh32(tput_pkt->pkt_id)));
		dhd->tput_data.pkts_bad++;
		return;
	}
		...
		char *text_hdr, int buflen, uint32 sec_type)
{
	...
	if (dhdp->concise_dbg_buf) {
		...
		sec_hdr->length = 0;
		ret = dhd_export_debug_data((char *)sec_hdr, NULL, user_buf, //-V519
			sizeof(*sec_hdr), &pos);
		do {
		...
		void *evt_decode_data)
{
	...
	*/
	...
	if (dhdp->logtrace_pkt_sendup) {
		DHD_TRACE(("%s: send up event log, len %u bytes\n", __FUNCTION__,
...
copy_hang_info_ioctl_timeout(dhd_pub_t *dhd, int ifidx, wl_ioctl_t *ioc)
{
	...
	DHD_TRACE(("%s hang info len: %d data: %s\n",
