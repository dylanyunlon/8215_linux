...
#define DHD_PRSRV_MEM(args) do { \
	if (dhd_msg_level & DHD_EVENT_VAL) { \
		if (dhd_msg_level & DHD_PRSRV_MEM_VAL) { \
			LOGI args; \
		} \
		DHD_LOG_DUMP_WRITE_PRSRV("[%s] "LOG_TAG"[I]%s,%d: ", \
				dhd_log_dump_get_timestamp(), __func__, __LINE__); \
...
#define DHD_ERROR_EX(args) do { \
	if (dhd_msg_level & DHD_ERROR_VAL) { \
		LOGE args; \
		DHD_LOG_DUMP_WRITE_EX("[%s] "LOG_TAG"[I]%s,%d: ", \
				dhd_log_dump_get_timestamp(), __func__, __LINE__); \
...
#define DHD_MSGTRACE_LOG(args) do { \
	...
	DHD_LOG_DUMP_WRITE("[%s] "LOG_TAG"[T]%s,%d: ", \
			dhd_log_dump_get_timestamp(), __func__, __LINE__); \
	DHD_LOG_DUMP_WRITE args; \
} while (0)
