...
pkt_frag_t pkt_frag_info(osl_t *osh, void *p)
{
	...
	if (length < ETHER_HDR_LEN) {
		...
		DHD_TRACE(("%s: non-SNAP 802.3 frame\n", __FUNCTION__));
		return DHD_PKT_FRAG_NONE;
	}
	...
	if (ethertype == ETHER_TYPE_8021Q) {
		...
		if (pt + ETHER_TYPE_LEN > frame + length) {
			DHD_TRACE(("%s: short VLAN frame (%d)\n", __FUNCTION__, length));
			return DHD_PKT_FRAG_NONE;
		}
	...
	if (ethertype != ETHER_TYPE_IP) {
		DHD_TRACE(("%s: non-IP frame (ethertype 0x%x, length %d)\n",
	...
	if ((ipl < IPV4_OPTIONS_OFFSET) || (IP_VER(iph) != IP_VER_4)) {
		DHD_TRACE(("%s: short frame (%d) or non-IPv4\n", __FUNCTION__, ipl));
		return DHD_PKT_FRAG_NONE;
	}
