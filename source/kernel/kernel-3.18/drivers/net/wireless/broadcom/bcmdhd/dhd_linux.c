...
#else // CONFIG_ATC_AOSP_ENHANCEMENT
...
#include <../misc/atc/metazone/metazone.h>
#else
#include <../drivers/soc/autochips/include/metazone/metazone.h>
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
#include <generated/storage/metazone_index.h>
#endif
#endif

#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#define BCMDHD_PATCH_DATE       "2025/06/09"
#endif
...
#else
static DEFINE_MUTEX(dhd_module_exit_lock);

// dhd_module_exit in progress, abort dhd_module_reset
static atomic_t exit_in_progress = ATOMIC_INIT(0);

// _dhd_module_init/_dhd_module_exit state
static atomic_t dhd_module_state = ATOMIC_INIT(DHD_STATE_REMOVE_DONE);

int dhd_module_state_get(void)
{
	return atomic_read(&dhd_module_state);
}

void dhd_module_state_set(int state)
{
	int old = atomic_xchg(&dhd_module_state, state);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
    if (sta == DHD_STA_NULL || sta->idx == ID16_INVALID
            || dhdp->staid_allocator == NULL || dhdp->sta_pool == NULL) {
        return;
    }
#endif
		...
		if (value && dhd->in_suspend) {
				...
				if (dhd->ndo_enable) {
					if (!dhd->ndo_host_ip_overflow) {
						...
						DHD_TRACE(("%s: NDO disabled on suspend due to"
								"HW capacity\n", __FUNCTION__));
					}
				}
				...
				ret = dhd_iovar(dhd, 0, "lpas", (char *)&lpas, sizeof(lpas), NULL,
						0, TRUE);
				if (ret < 0) {
					DHD_INFO(("%s:lpas failed:%d\n", __FUNCTION__, ret));
				}
				ret = dhd_iovar(dhd, 0, "bcn_to_dly", (char *)&bcn_to_dly,
						sizeof(bcn_to_dly), NULL, 0, TRUE);
				if (ret < 0) {
					DHD_INFO(("%s:bcn_to_dly failed:%d\n", __FUNCTION__, ret));
				}
				ret = dhd_iovar(dhd, 0, "bcn_timeout", (char *)&bcn_timeout,
						sizeof(bcn_timeout), NULL, 0, TRUE);
				if (ret < 0) {
...
dhd_timeout_expired(dhd_timeout_t *tmo)
{
	...
	if ((!CAN_SLEEP()) || tmo->increment < tmo->tick) { //-V530
...
_dhd_set_mac_address(dhd_info_t *dhd, int ifidx, uint8 *addr)
{
	int ret = 0;
#ifdef DHD_NOTIFY_MAC_CHANGED
	DHD_INFO(("%s: close dev for mac changing\n", __func__));
	...
	dhd_allow_stop = TRUE;
	DHD_INFO(("%s: notify mac changed done\n", __func__));
#endif /* DHD_NOTIFY_MAC_CHANGED */
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	/********************************
	  mask goto done when swtich from device mac
	  to random mac casue wl_link_down
	  and set dhd->pub.up = 0
	 *********************************/
#else // CONFIG_ATC_AOSP_ENHANCEMENT
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		//DHD_INFO(("%s: MACID is overwritten %pM\n",	__FUNCTION__, ifp->mac_addr));
#else
...
#if defined(SOFTAP) || !defined(CONFIG_ATC_AOSP_ENHANCEMENT)
done:
#endif
	DHD_PERIM_UNLOCK(&dhd->pub);
	DHD_OS_WAKE_UNLOCK(&dhd->pub);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		// BCMPCIE: dump at the end of dhd_prot_txdata
#ifdef BCMSDIO
...
#ifdef WL_CFG80211_SET_TX_PARAMS
		DHD_TRACE(("PKTPRIO 0x%x\n", PKTPRIO(pktbuf)));
#endif
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	if (skb_headroom(skb) < dhd->pub.hdrlen + htsfdlystat_sz) {
		...
		DHD_TRACE(("%s: insufficient headroom\n",
		          dhd_ifname(&dhd->pub, ifidx)));
		dhd->pub.tx_realloc++;
	...
	if (WET_ENABLED(&dhd->pub) &&
		...
		DHD_TRACE(("%s:%s: wet send proc failed\n",
...
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT // PVS V547
	if (!dhdp) {
...
exit:
	...
	dhdp->logtrace_thr_ts.complete_time = OSL_LOCALTIME_NS(); //-V779
...
dhd_init_logtrace_process(dhd_info_t *dhd)
{
	...
	if (dhd->thr_logtrace_ctl.thr_pid < 0) {
		...
		DHD_INFO(("%s: thr_logtrace_ctl(%ld) succedded\n", __FUNCTION__,
			dhd->thr_logtrace_ctl.thr_pid));
	}
#else
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (WET_ENABLED(&dhd->pub) && (dhd_wet_recv_proc(dhd->pub.wet_info,
				pktbuf) < 0)) {
			DHD_TRACE(("%s:%s: wet recv proc failed\n",
		...
		if (in_interrupt()) {
			...
			if (dhd->rxthread_enabled) {
				...
				if (dhd_dpc_prio >= 0) {
					...
					/* This function may only be called from softirq
					 * context and interrupts should be enabled. */
					netif_receive_skb(skb);
				}
#else
				netif_receive_skb(skb);
...
dhd_toe_get(dhd_info_t *dhd, int ifidx, uint32 *toe_ol)
{
	...
	if (ret < 0) {
		...
		DHD_TRACE(("%s: could not get toe_ol: ret=%d\n", dhd_ifname(&dhd->pub, ifidx), ret));
		return ret;
	}
...
void dhd_set_scb_probe(dhd_pub_t *dhd)
{
	...
	ret = dhd_iovar(dhd, 0, "scb_probe", (char *)&scb_probe, sizeof(wl_scb_probe_t), NULL, 0,
			TRUE);
	if (ret < 0) {
		DHD_INFO(("%s: max_scb_probe setting failed\n", __FUNCTION__));
		return;
	}
}
...
int dhd_ioctl_process(dhd_pub_t *pub, int ifidx, dhd_ioctl_t *ioc, void *data_buf)
{
	...
	if (pub->busstate == DHD_BUS_DOWN || pub->busstate == DHD_BUS_LOAD) {
		if ((!pub->dongle_trap_occured) && allow_delay_fwdl) {
			int ret;
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			if (DHD_STATE_REMOVE_START == dhd_module_state_get()
					|| DHD_STATE_REMOVE_DONE == dhd_module_state_get()) {
#else
...
dhd_stop(struct net_device *net)
{
	...
	struct net_info *iter = NULL;
	struct net_info *next = NULL;
#elif defined(WL_STATIC_IF)
	struct bcm_cfg80211 *cfg = wl_get_cfg(net);
#endif /* WL_STATIC_IF */
#endif /* WL_CFG80211 */
	dhd_info_t *dhd = DHD_DEV_INFO(net);
	...
	DHD_INFO(("%s: ######### dhd_stop called for ifidx=%d #########\n",
			__FUNCTION__, ifidx));
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	...
	for_each_ndev(cfg, iter, next) {
		...
		DHD_INFO(("iter->ndev->ieee80211_ptr->iftype : %d\n",
				iter->ndev->ieee80211_ptr->iftype));
		if ((iter->ndev->ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_GO
				|| iter->ndev->ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_CLIENT)
				&& wl_get_drv_status(cfg, CONNECTED, iter->ndev)) {
			DHD_INFO(("skip ifdown while p2p connected\n"));
			skip_reset = true;
	...
	if (static_if_ndev_get_state(cfg, net) == NDEV_STATE_FW_IF_CREATED) {
		DHD_INFO(("static if operational. skip chip reset.\n"));
		skip_reset = true;
		wl_cfg80211_sta_ifdown(net);
		goto exit;
	}
...
#ifdef DHD_NOTIFY_MAC_CHANGED
	if (!dhd_allow_stop) {
		DHD_ERROR(("DHD_NOTIFY_MAC_CHANGED. skip chip reset.\n"));
		skip_reset = true;
#if defined(WL_CFG80211)
		wl_cfg80211_sta_ifdown(net);
#endif /* WL_CFG80211 */
	...
	if (vif_num > 0) {
		DHD_INFO(("virtual if operational. skip chip reset.\n"));
		skip_reset = true;
		wl_cfg80211_sta_ifdown(net);
		goto exit;
	}
	...
	if (ifidx == 0) {
		...
		if (ifp && ifp->net == dhd->rx_napi_netdev) {
			DHD_TRACE(("%s napi<%p> disabled ifp->net<%p,%s>\n",
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#ifdef BCMPCIE
//#define DHD_FW_LOG_ENABLE   1
#endif
#ifdef DHD_FW_LOG_ENABLE
static int dhd_fw_log_enable(void);
#endif
#endif
...
dhd_open(struct net_device *net)
{
	...
	if (ifidx == 0) {
		...
		if (dhd->rx_napi_netdev == NULL) {
			...
			DHD_TRACE(("%s napi<%p> enabled ifp->net<%p,%s>\n",
				__FUNCTION__, &dhd->rx_napi_struct, net, net->name));
			napi_enable(&dhd->rx_napi_struct);
			DHD_TRACE(("%s load balance init rx_napi_struct\n", __FUNCTION__));
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	if (DHD_STATE_PROBE_SUCCESS == dhd_module_state_get()) {
		ret = dhd_open(net);
	} else {
		DHD_ERROR(("dhd_module_state %d\n", dhd_module_state_get()));
		ret = -1;
	}
#else
	ret = dhd_open(net);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		if (!atc_combo_wlan_reset_is_triggered()
				&& DHD_STATE_PROBE_SUCCESS != dhd_module_state_get()) {
			dhd_schedule_reset(NULL);
		}
#endif
		return ret;
	}
	...
	uint8 *mac, uint8 bssidx, const char *dngl_name, int if_state)
{
	...
	DHD_TRACE(("[STATIC_IF] ifp ptr updated for ifidx:%d curidx:%d if_state:%d\n",
...
fail:
	...
	ifp = NULL;
#else // PVS V547
	if (ifp != NULL) { // PVS V547 always true
		if (ifp->net != NULL) { // PVS V547 always false
...
#endif /* WL_STATIC_IF */
		if (ifp->net != NULL) {
			DHD_INFO(("deleting interface '%s' idx %d\n", ifp->net->name, ifp->idx));
			...
			if (DHD_IF_IS_TX_ACTIVE(ifp)) {
				DHD_TRACE(("TX in progress. Wait for it to be complete."));
...
dhd_init_logstrs_array(osl_t *osh, dhd_event_log_t *temp)
{
	...
	if (IS_ERR(filep)) {
		DHD_INFO_HW4(("%s: Failed to open the file %s \n", __FUNCTION__, logstrs_path));
		goto fail;
	}
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	DHD_TRACE(("%s load balance init tx_compl_tasklet\n", __FUNCTION__));
#endif /* DHD_LB_TXC */
#if defined(DHD_LB_RXC)
	...
	DHD_TRACE(("%s load balance init rx_compl_tasklet\n", __FUNCTION__));
#endif /* DHD_LB_RXC */
	...
	DHD_TRACE(("%s load balance init rx_napi_queue\n", __FUNCTION__));
#endif /* DHD_LB_RXP */
	...
	DHD_TRACE(("%s load balance init tx_pend_queue\n", __FUNCTION__));
#endif /* DHD_LB_TXP */
...
int dhd_download_btfw(wlan_bt_handle_t handle, char* btfw_path)
{
	...
	if (dhd->pub.busstate == DHD_BUS_DATA && dhd_update_btfw_path(dhd, btfw_path)) {
		DHD_TRACE(("%s: download btfw from: %s\n", __FUNCTION__, dhd->btfw_path));
...
dhd_tdls_set_mode(dhd_pub_t *dhd, bool wfd_mode)
{
	...
	if (ret < 0) {
		DHD_INFO(("Disable tdls_auto_op failed. %d\n", ret));
		return ret;
	}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	uint32 metaIndex = METAZONE_WIFI;
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
	uint32 metaIndex = 0x10026;
#else
	uint32 metaIndex = 0x10001;
#endif
	char readData[ETHER_ADDR_LEN] = {0};
	uint32 metaSize = ETHER_ADDR_LEN;
#endif /* CONFIG_ATC_AOSP_ENHANCEMENT */
#ifdef DISABLE_11N
	uint32 nmode = 0;
#endif /* DISABLE_11N */
...
#define EVENT_LOG_RATE_HC_THRESHOLD	1000
	...
	if ((!op_mode && dhd_get_fw_mode(dhd->info) == DHD_FLAG_MFG_MODE) ||
		...
		DHD_TRACE(("%s : Set IOCTL response time.\n", __FUNCTION__));
	}
#ifdef BCMPCIE_OOB_HOST_WAKE
	...
	DHD_INFO(("read mac from metazone:"MACDBG"\n", MAC2STRDBG(readData)));
	if (ETHER_ISBCAST(readData) || ETHER_ISNULLADDR(readData)) {
		get_random_bytes(&dhd->mac.octet[3], 3);
		/* Adding ATC OUI */
		dhd->mac.octet[0] = 0x00;
		dhd->mac.octet[1] = 0x08;
		dhd->mac.octet[2] = 0x22;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
        MetaZone_WriteBinary(metaIndex, dhd->mac.octet, metaSize);
#else
		MetaZone_SpecWriteBinary(metaIndex, dhd->mac.octet,
			metaSize, MTZ_WRITE_FLAG_FACTORYREST);
#endif
		MetaZone_Flush(0);
		/*set ATC random mac to FW */
		DHD_INFO(("ATC random mac:"MACDBG"\n", MAC2STRDBG(dhd->mac.octet)));
		ret = dhd_iovar(dhd, 0, "cur_etheraddr",
				(char *)&(dhd->mac.octet),
				ETHER_ADDR_LEN, NULL, 0, TRUE);
		if (ret < 0) {
			DHD_ERROR(("can't set MAC address , error=%d\n", ret));
			ret = BCME_NOTUP;
			goto done;
		}
	}else {
		memcpy(dhd->mac.octet, readData, ETHER_ADDR_LEN);
		/*set ATC random mac to FW */
		ret = dhd_iovar(dhd, 0, "cur_etheraddr",
				(char *)&readData,
				ETHER_ADDR_LEN, NULL, 0, TRUE);
	...
	if (!ETHER_ISNULLADDR(dhd->mac.octet)) {
		DHD_INFO(("p2p debug:"MACDBG"\n", MAC2STRDBG(dhd_linux_get_primary_netdev(dhd)->perm_addr)));
	...
	if ((!op_mode && dhd_get_fw_mode(dhd->info) == DHD_FLAG_HOSTAP_MODE) ||
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_AP,
			...
			if (ret < 0)
				...
				DHD_TRACE(("dhd_preinit_ioctls: p2p_da_override succeeded\n"));
		}
#else
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_SCAN_PERIOD, roam_scan_period,
		...
		DHD_INFO(("%s: roam scan period set failed %d\n", __FUNCTION__, ret));
	if ((dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_DELTA, roam_delta,
		sizeof(roam_delta), TRUE, 0)) < 0)
		DHD_INFO(("%s: roam delta set failed %d\n", __FUNCTION__, ret));
	ret = dhd_iovar(dhd, 0, "fullroamperiod", (char *)&roam_fullscan_period,
			sizeof(roam_fullscan_period), NULL, 0, TRUE);
	if (ret < 0)
		DHD_INFO(("%s: roam fullscan period set failed %d\n", __FUNCTION__, ret));
#ifdef ROAM_AP_ENV_DETECTION
	...
	if (ap_fw_loaded == FALSE)
		...
		if (!(dhd->op_mode &
			...
			if ((res = dhd_keep_alive_onoff(dhd)) < 0)
				DHD_INFO(("%s set keeplive failed %d\n",
				__FUNCTION__, res));
		}
	}
	...
	ret = dhd_iovar(dhd, 0, "scancache", (char *)&scancache_enab, sizeof(scancache_enab), NULL,
			0, TRUE);
	if (ret < 0) {
		DHD_INFO(("%s Set scancache failed %d\n", __FUNCTION__, ret));
	}
	...
	if (ret < 0) {
		DHD_INFO(("%s: enable_memuse is failed ret=%d\n",
			__FUNCTION__, ret));
	} else {
		DHD_INFO(("%s: enable_memuse = %d\n",
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_FAKEFRAG, (char *)&frameburst,
		...
		DHD_TRACE(("%s frameburst not supported  %d\n", __FUNCTION__, ret));
	}
#ifdef DHD_SET_FW_HIGHSPEED
	/* Set ack_ratio */
	...
	ret = dhd_iovar(dhd, 0, "vht_features", (char *)&vht_features, sizeof(vht_features),
			NULL, 0, FALSE);
	if (ret < 0) {
		DHD_INFO(("%s vht_features get failed %d\n", __FUNCTION__, ret));
		vht_features = 0;
	} else {
...
#endif /* WL_CHAN_UTIL */
#if defined(WL_SAE) || defined(WL_SAE_EXTERNAL_SME)
...
#endif /* WL_SAE || WL_SAE_EXTERNAL_SME */
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT
	...
	if (ret < 0)
		...
		if ((ver_temp_buf = bcmstrstr(ptr, "Customization:")) == NULL) {
			...
			if ((ver_temp_buf = bcmstrtok(&ptr, "(\n", &tokenlim)) == NULL) {
				...
				DHD_TRACE(("[INFO]CLM/Blob version = %s\n", clm_version));
				if ((ver_temp_buf = bcmstrtok(&ptr, "\n", &tokenlim)) == NULL) {
					DHD_ERROR(("Couldn't find New line character\n"));
				} else {
					snprintf(clm_version,
					...
					DHD_TRACE(("[INFO]CLM/Blob/project version = %s\n",
						clm_version));
	...
	if (ret2 < 0) {
		DHD_INFO(("%s wl ampdu_hostreorder failed %d\n", __FUNCTION__, ret2));
	...
	if (dhd_iovar(dhd, 0, "wnm", (char *)&wnm_cap, sizeof(wnm_cap), NULL, 0, TRUE) < 0) {
		DHD_INFO(("failed to set WNM capabilities\n"));
	}
		...
		uint res_len, int set)
{
	...
	if (set) {
		...
		if (!res_buf || !res_len) {
			DHD_INFO(("%s: GET failed. resp_buf NULL or length 0.\n", __FUNCTION__));
			ret = BCME_BADARG;
			goto exit;
		}

		if (res_len < input_len) {
			DHD_TRACE(("%s: res_len(%d) < input_len(%d)\n", __FUNCTION__,
					res_len, input_len));
			buf = MALLOCZ(pub->osh, input_len);
			if (!buf) {
				DHD_ERROR(("%s: mem alloc failed\n", __FUNCTION__));
...
dhd_register_vif(dhd_pub_t *dhdp)
{
	...
	DHD_TRACE(("%s Virtual interface [%s]:\n", __FUNCTION__, viface_name));
	rtnl_lock();
	for (i = 0; i < vif_num; i++) {
...
static int dhd_log_level_callback(int type)
{
	int ret = 0;

	int tmp_dhd_msg_level_info = DEFAULT_DHD_LOG_LEVEL;
	int tmp_dhd_msg_level_debug = tmp_dhd_msg_level_info
		| DHD_WL_VAL | DHD_WL_VAL2 | DHD_IOVAR_MEM_VAL;
	int tmp_dhd_msg_level_loud = tmp_dhd_msg_level_debug
		| DHD_TRACE_VAL | DHD_MSGTRACE_VAL;

	u32 tmp_wl_dbg_level_info = DEFAULT_WL_LOG_LEVEL;
	u32 tmp_wl_dbg_level_debug = tmp_wl_dbg_level_info
		| WL_DBG_SCAN | WL_DBG_DBG;
	u32 tmp_wl_dbg_level_loud = tmp_wl_dbg_level_debug
		| WL_DBG_TRACE;

	DHD_INFO(("set log level %d\n", type));
	switch (type) {
	case ATC_COMBO_WLAN_LOG_LEVEL_INFO:
		dhd_msg_level = tmp_dhd_msg_level_info;
		wl_dbg_level = tmp_wl_dbg_level_info;
		break;
	case ATC_COMBO_WLAN_LOG_LEVEL_DEBUG:
		dhd_msg_level = tmp_dhd_msg_level_debug;
		wl_dbg_level = tmp_wl_dbg_level_debug;
		break;
	case ATC_COMBO_WLAN_LOG_LEVEL_LOUD:
		dhd_msg_level = tmp_dhd_msg_level_loud;
		wl_dbg_level = tmp_wl_dbg_level_loud;
		break;
	default:
		DHD_ERROR(("unknown log level %d\n", type));
		ret = -EINVAL;
		break;
	}
...
static void _dhd_module_exit(void)
{
	int ret = -1;

	if (DHD_STATE_REMOVE_DONE == dhd_module_state_get()) {
		DHD_INFO(("already removed\n"));
		return;
	}
	DHD_INFO(("in\n"));
	dhd_module_state_set(DHD_STATE_REMOVE_START);
	dhd_module_cleanup();
	unregister_reboot_notifier(&dhd_reboot_notifier);
	dhd_destroy_to_notifier_skt();
	if (fw_bak_path[0] != '\0') {
	...
	ret = atc_wlan_power_on(0);
	if (ret) {
		DHD_ERROR(("atc_wlan_power_on(0) fail %d\n", ret));
	}

	dhd_module_state_set(DHD_STATE_REMOVE_DONE);
	DHD_INFO(("out\n"));
}
...
static void __exit dhd_module_exit(void)
{
	...
	atc_combo_wlan_reset_register_callback(NULL);
	atomic_set(&exit_in_progress, 1);
	mutex_lock(&dhd_module_exit_lock);
	_dhd_module_exit();
	mutex_unlock(&dhd_module_exit_lock);
	DHD_INFO(("out\n"));
}
...
static int dhd_fw_log_enable(void)
{
	...
	DHD_INFO(("\n"));
	if (!g_dhd_pub) {
		DHD_ERROR(("NULL g_dhd_pub\n"));
		return BCME_ERROR;
	}
	// dhd -i wlan0 dconpoll 10
	ret = dhd_ioctl_set_intiovar(g_dhd_pub,
			"dconpoll", 10, DHD_SET_VAR, TRUE);
	if (ret) {
		DHD_ERROR(("set dconpoll fail %d\n", ret));
		return ret;
	}
	// dhd -i wlan0 wdtick 10
	ret = dhd_ioctl_set_intiovar(g_dhd_pub,
			"wdtick", 10, DHD_SET_VAR, TRUE);
	if (ret) {
		DHD_ERROR(("set wdtick fail %d\n", ret));
		return ret;
	}
...
#ifdef BCMPCIE
void dhd_isr_dump(void);

static int dhd_ioctl_timeout_dump(void)
{
	char buf[WLC_IOCTL_SMLEN] = {0};
	char tmp[WLC_IOCTL_SMLEN] = {0};
	int val = 0;
	int i = 0;
	int n = 0;
	int ret = 0;
	...
	// cat /proc/interrupts | grep dhd
	dhd_isr_dump();

	// dmesg -c | grep CONSOLE

	// dhd -i wlan0 cons mu
	DHD_INFO(("dhd -i wlan0 cons mu:\n"));
	strcpy(buf, "cons");
	strcpy(buf + strlen(buf) + 1, "mu");
	ret = dhd_ioctl_cmd(g_dhd_pub, DHD_SET_VAR,
			buf, sizeof(buf), TRUE);
	if (ret) {
		DHD_ERROR(("set cons fail %d\n", ret));
	}
	...
	// dhd -i wlan0 pciereg 0
	strcpy(buf, "pciereg");
	val = 0;
	memcpy(buf + strlen(buf) + 1, &val, sizeof(val));
	ret = dhd_ioctl_cmd(g_dhd_pub, DHD_GET_VAR,
			buf, sizeof(buf), FALSE);
	if (ret) {
		DHD_ERROR(("set pciereg fail %d\n", ret));
	} else {
		memcpy(&val, buf, sizeof(val));
		DHD_INFO(("dhd -i wlan0 pciereg 0: 0x%08x\n", val));
	}

	// dhd -i wlan0 membytes 0x160000 32
	strcpy(buf, "membytes");
	n = strlen(buf) + 1;
	...
	ret = dhd_ioctl_cmd(g_dhd_pub, DHD_GET_VAR,
			buf, sizeof(buf), FALSE);
	if (ret) {
		DHD_ERROR(("set membytes fail %d\n", ret));
	} else {
		memcpy(&val, buf, sizeof(val));
		DHD_INFO(("dhd -i wlan0 membytes 0x160000 32:\n"));
		n = 0;
		for (i = 0; i < 16; i++) {
			n += sprintf(tmp + n, "%02x ", buf[i]);
		}
		DHD_INFO(("00160000: %s\n", tmp));
		n = 0;
		for (i = 16; i < 32; i++) {
	...
	// wl ver
	strcpy(buf, "ver");
	ret = dhd_wl_ioctl_cmd(g_dhd_pub, WLC_GET_VAR,
			buf, sizeof(buf), FALSE, 0);
	if (ret) {
		DHD_ERROR(("wl ver fail %d\n", ret));
	} else {
		DHD_INFO(("wl ver: %s\n", buf));
	}

	// dhd -i wlan0 devreset 1
	DHD_INFO(("dhd -i wlan0 devreset 1\n"));
	ret = dhd_ioctl_set_intiovar(g_dhd_pub,
			"devreset", 1, DHD_SET_VAR, TRUE);
	if (ret) {
		DHD_ERROR(("devreset 1 fail %d\n", ret));
		return ret;
	}

	// dhd -i wlan0 devreset 0
	DHD_INFO(("dhd -i wlan0 devreset 0\n"));
	ret = dhd_ioctl_set_intiovar(g_dhd_pub,
			"devreset", 0, DHD_SET_VAR, TRUE);
	if (ret) {
		DHD_ERROR(("devreset 0 fail %d\n", ret));
		return ret;
	}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	if (DHD_STATE_PROBE_SUCCESS == dhd_module_state_get()) {
		DHD_INFO(("already probed\n"));
		return 0;
	}
	DHD_INFO(("%s in\n", __FUNCTION__));
	dhd_module_state_set(DHD_STATE_PROBE_START);

	err = atc_wlan_power_on(1);
	if (err) {
		DHD_ERROR(("atc_wlan_power_on(1) fail %d\n", err));
		dhd_module_state_set(DHD_STATE_PROBE_FAIL);
		return err;
	}
#else // ! CONFIG_ATC_AOSP_ENHANCEMENT
	DHD_INFO(("%s in\n", __FUNCTION__));
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	dhd_module_state_set(err ? DHD_STATE_PROBE_FAIL
			: DHD_STATE_PROBE_SUCCESS);
#endif
	DHD_INFO(("%s out ret %d\n", __FUNCTION__, err));
...
static int dhd_module_reset(int type);
...
#if defined(BCMSDIO)
	DHD_INFO(("[bcmdhd][sdio] patch_date %s\n", BCMDHD_PATCH_DATE));
#elif defined(BCMPCIE)
	DHD_INFO(("[bcmdhd][pcie] patch_date %s\n", BCMDHD_PATCH_DATE));
#else
	DHD_INFO(("[bcmdhd] patch_date %s\n", BCMDHD_PATCH_DATE));
#endif

	ret = _dhd_module_init();
	atc_combo_wlan_reset_register_callback(dhd_module_reset);
	if (ret) {
		DHD_ERROR(("_dhd_module_init fail %d\n", ret));
		dhd_schedule_reset(NULL);
		ret = 0; // always insmod ok, recovery on wlan_reset
	}
	atc_combo_wlan_log_level_register_callback(
			ATC_COMBO_WLAN_LOG_LEVEL_INFO,
			dhd_log_level_callback);
...
#ifdef BCMPCIE
#define PCIE_RESET_TIMEOUT_MSEC     2000 // normal time is about 150ms
#endif
...
static int dhd_module_reset(int type)
{
	int ret = -1;
#ifdef BCMPCIE
	bool is_pcie_fail_reset = false;
#endif

	DHD_INFO(("start type %d\n", type));
	mutex_lock(&dhd_module_exit_lock);

#ifdef BCMPCIE
	if (pcie_host_status_dump(0)) {
		int retry = PCIE_RESET_TIMEOUT_MSEC / 10;

		DHD_ERROR(("pcie host error %d, wait for pcie reset ...\n",
				pcie_host_status_dump(0)));
		is_pcie_fail_reset = true;
		while (retry-- > 0) {
			ret = pcie_host_status_dump(0);
			if (!ret) {
				break;
			}
			msleep(10);
		...
		if (ret) {
			DHD_ERROR(("pcie reset timeout %d\n", ret));
			pcie_host_status_dump(1);
			goto exit;
		}
	}
	if (type == DHD_RESET_TYPE_DUMP_RESET) {
		if (atomic_read(&exit_in_progress)) {
			DHD_ERROR(("exit_in_progress\n"));
			ret = -1;
			goto exit;
		}
		if (!is_pcie_fail_reset) {
			dhd_ioctl_timeout_dump();
		}
	}
#endif

	if (type == DHD_RESET_TYPE_QB_ON) {
		DHD_INFO(("DHD_RESET_TYPE_QB_ON\n"));
	} else {
		if (atomic_read(&exit_in_progress)) {
			DHD_ERROR(("exit_in_progress\n"));
			ret = -1;
...
#ifdef BCMPCIE
	if (type != DHD_RESET_TYPE_QB_OFF && type != DHD_RESET_TYPE_QB_ON) {
		int retry = PCIE_RESET_TIMEOUT_MSEC / 10;
		 ...
		 * power.up
		 * wait for host ready
		 * host detect LINK_DOWN_RESET_OUT
		 * pcie access hang
		 *
		 * optimize:
		 *
		 ...
		 * pcie access ok
		 *
		 * so we should wait for host down here
		 */
		while (retry-- > 0) {
			ret = pcie_host_status_dump(0);
			if (ret) {
		...
		if (!ret) {
			DHD_ERROR(("wait for host down timeout\n"));
			pcie_host_status_dump(1);
			goto exit;
		}

		// pcie host require device power off time < 500ms
		ret = atc_wlan_power_on(1);
		if (ret) {
			DHD_ERROR(("atc_wlan_power_on(1) fail %d\n", ret));
			goto exit;
		}
		retry = PCIE_RESET_TIMEOUT_MSEC / 10;
		while (retry-- > 0) {
			ret = pcie_host_status_dump(0);
			if (!ret) {
				break;
	...
	if (type == DHD_RESET_TYPE_QB_OFF) {
		DHD_INFO(("DHD_RESET_TYPE_QB_OFF\n"));
	} else {
		if (atomic_read(&exit_in_progress)) {
			DHD_ERROR(("exit_in_progress\n"));
			ret = -1;
...
dhd_os_ioctl_resp_wait(dhd_pub_t *pub, uint *condition)
{
	...
	if (unlikely(in_atomic())) {
		DHD_ERROR(("maybe scheduling while atomic, "
...
{
	uint8* buf;
	gfp_t flags = CAN_SLEEP() ? GFP_KERNEL: GFP_ATOMIC; //-V530
...

{
	...
	*/
	if (dhdp && dhdp->up) {
		if (dhdp->in_suspend) {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			if (!val || !dhdp->suspend_disable_flag)
#else // PVS V728
...
dhd_dev_ndo_update_inet6addr(struct net_device *dev)
{
	...
	if (cnt > dhdp->ndo_max_host_ip) {
		if (!dhdp->ndo_host_ip_overflow) {
			...
			/* Disable ND offload in FW */
			DHD_TRACE(("%s: Host IP overflow, disable NDO\n", __FUNCTION__));
			ret = dhd_ndo_enable(dhdp, FALSE);
		}
	...
	for (i = 0; i < cnt; i++) {
		DHD_TRACE(("%s: Remove DAD failed addr\n", __FUNCTION__));
             ...
             wlc_roam_exp_params_t *roam_param)
{
	...
	DHD_TRACE(("a_band_boost_thr %d a_band_penalty_thr %d\n",
	      roam_param->a_band_boost_threshold, roam_param->a_band_penalty_threshold));
	DHD_TRACE(("a_band_boost_factor %d a_band_penalty_factor %d cur_bssid_boost %d\n",
	      roam_param->a_band_boost_factor, roam_param->a_band_penalty_factor,
	      roam_param->cur_bssid_boost));
	DHD_TRACE(("alert_roam_trigger_thr %d a_band_max_boost %d\n",
...
dhd_dev_cfg_rand_mac_oui(struct net_device *dev, uint8 *oui)
{
	...
	if (ETHER_ISMULTI(oui)) {
		...
		DHD_INFO(("Random MAC OUI to be used - "MACOUIDBG"\n",
...
dhd_dev_stop_mkeep_alive(dhd_pub_t *dhd_pub, uint8 mkeep_alive_id)
{
	...
	if (res < 0) {
		...
		for (i = 0; i < mkeep_alive_pktp->len_bytes; i++) {
			DHD_TRACE(("%02x", mkeep_alive_pktp->data[i]));
		}
		DHD_TRACE(("\n"));
	}
	...
	if (dtoh32(mkeep_alive_pktp->period_msec != 0)) {
		DHD_TRACE(("stop mkeep_alive on ID %d\n", mkeep_alive_id));
		memset(&mkeep_alive_pkt, 0, sizeof(wl_mkeep_alive_pkt_t));
...
dhd_dev_apf_get_version(struct net_device *ndev, uint32 *version)
{
	...
	if (!FW_SUPPORTED(dhdp, apf)) {
		DHD_INFO(("%s: firmware doesn't support APF\n", __FUNCTION__));
...
int dhd_net_set_fw_path(struct net_device *dev, char *fw)
{
	...
	if (strstr(fw, "apsta") != NULL) {
		DHD_TRACE(("GOT APSTA FIRMWARE\n"));
		ap_fw_loaded = TRUE;
	} else {
		DHD_TRACE(("GOT STA FIRMWARE\n"));
		ap_fw_loaded = FALSE;
	}
#endif /* defined(OEM_ANDROID) && defined(SOFTAP) */
	return 0;
...
void dhd_event_wake_unlock(dhd_pub_t *pub) //-V524
{
	dhd_info_t *dhd = (dhd_info_t *)(pub->info);
...
{
	/**
	 * ATC_AOSP_ENHANCEMENT modify and cypress is:
	 * #if defined(CONFIG_PM_WAKELOCKS) || defined(CONFIG_HAS_WAKELOCK)
	 */
#ifdef CONFIG_HAS_WAKELOCK
	dhd_info_t *dhd = (dhd_info_t *)(pub->info);
...
dhd_os_check_wakelock_all(dhd_pub_t *pub)
{
	...
	int c, lock_active;
#endif /* CONFIG_PM_WAKELOCKS || CONFIG_HAS_WAKELOCK */
	// ATC_AOSP_ENHANCEMENT add this define
#if defined(CONFIG_HAS_WAKELOCK) || (defined(BCMSDIO) && (LINUX_VERSION_CODE > \
	KERNEL_VERSION(2, 6, 36)))
	dhd_info_t *dhd;
...
void dhd_set_version_info(dhd_pub_t *dhdp, char *fw)
{
	...
	i = snprintf(&info_string[i], sizeof(info_string) - i, //-V1001
...
#if defined(CONFIG_ATC_AOSP_ENHANCEMENT) \
	&& (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static const struct proc_ops dhd_dbg_state_ops = {
	.proc_read   = dhd_dbg_state_read,
	.proc_write	= dhd_debugfs_write,
	.proc_open   = dhd_dbg_state_open,
	.proc_lseek	= dhd_debugfs_lseek
};
#else
...
int custom_rps_map_set(struct netdev_rx_queue *queue, char *buf, size_t len)
{
	...
	DHD_TRACE(("%s : Done. mapping cpu nummber : %d\n", __FUNCTION__, map->len));
	return map->len;
}
...
void custom_rps_map_clear(struct netdev_rx_queue *queue)
{
	...
	if (map) {
		...
		kfree_rcu(map, rcu);
		DHD_TRACE(("%s : rps_cpus map clear.\n", __FUNCTION__));
	}
}
#endif // endif
...
dhd_create_to_notifier_skt(void)
{
	...
	DHD_TRACE(("nl_to socket created successfully...\n"));
	return 0;
}
...
{
	DHD_TRACE(("Destroying nl_to socket\n"));
...
dhd_log_dump_init(dhd_pub_t *dhd)
{
	...
	*/
	...
	DHD_INFO(("%s: kernel log buf size = %uKB; logdump_prsrv_tailsize = %uKB;"
...
#else
	DHD_INFO(("%s: logdump_prsrv_tailsize = %uKB \n",
...
#if defined(CONFIG_DHD_USE_STATIC_BUF) && defined(DHD_USE_STATIC_MEMDUMP)
	DHD_INFO(("%s : Try to allocate memory total(%d) special(%d)\n",
		...
		int binary_len, const char *fmt, ...)
{
	...
	if (type < 0 || type >= DLD_BUFFER_NUM) {
		DHD_TRACE(("%s: Unknown DHD_LOG_DUMP_BUF_TYPE(%d).\n",
			__FUNCTION__, type));
		return;
	}
...
int dhd_parse_filter_ie(dhd_pub_t *dhd, uint8 *buf)
{
	...
	DHD_TRACE(("total element count %d \n", element_count));
...
{
	if (!atc_combo_wlan_reset_is_triggered()) {
		atc_combo_wlan_reset_trigger(DHD_RESET_TYPE_RESET);
	}
}

void dhd_schedule_dump_reset(dhd_pub_t *dhdp)
{
	if (!atc_combo_wlan_reset_is_triggered()) {
		atc_combo_wlan_reset_trigger(DHD_RESET_TYPE_DUMP_RESET);
	}
}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				DHD_LOG_DUMP_TS_FMT_YYMMDDHHMMSSMS,
#else
...
dhd_dump_file_manage_enqueue(dhd_pub_t *dhd, char *dump_path, char *fname)
{
	...
	DHD_TRACE(("%s(): fname: %s dump_path: %s\n", __FUNCTION__, fname, dump_path));
	...
	DHD_TRACE(("%s(): fm_idx: %d fp_idx: %d path: %s\n",
