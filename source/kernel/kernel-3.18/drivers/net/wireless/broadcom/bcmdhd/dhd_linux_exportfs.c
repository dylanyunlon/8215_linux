...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			dhdp->proc_dhd_trace_created = true;
#endif
		}
	} else {
		DHD_INFO(("dbg_verbose_ring is NULL, /proc/dhd_trace not created\n"));
	}
#endif /* DEBUGABILITY */
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		dhdp->proc_dhd_ecounters_created = true;
#endif
	}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		dhdp->proc_dhd_rtt_created = true;
#endif
	}
...
#ifdef DEBUGABILITY
	if (dhdp->proc_dhd_trace_created) {
		dhdp->proc_dhd_trace_created = false;
		remove_proc_entry(PROCFS_DIR_TRACE, NULL);
	}
#endif /* DEBUGABILITY */

#ifdef EWP_ECNTRS_LOGGING
	if (dhdp->proc_dhd_ecounters_created) {
		dhdp->proc_dhd_ecounters_created = false;
		remove_proc_entry(PROCFS_DIR_ENCOUNTERS, NULL);
	}
#endif /* EWP_ECNTRS_LOGGING */

#ifdef EWP_RTT_LOGGING
	if (dhdp->proc_dhd_rtt_created) {
		dhdp->proc_dhd_rtt_created = false;
...
set_hang_privcmd_err(struct dhd_info *dev, const char *buf, size_t count)
{
	...
	DHD_TRACE(("%s: Set report HANG for private cmd error: %d\n",
