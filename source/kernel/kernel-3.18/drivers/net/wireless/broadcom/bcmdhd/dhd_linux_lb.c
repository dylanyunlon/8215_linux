...
dhd_cpumasks_init(dhd_info_t *dhd)
{
	...
	DHD_INFO(("%s CPU masks primary(big)=0x%x secondary(little)=0x%x\n", __FUNCTION__,
...
void dhd_select_cpu_candidacy(dhd_info_t *dhd)
{
	...
	DHD_TRACE(("%s After primary CPU check napi_cpu %d compl_cpu %d tx_cpu %d\n",
	...
	DHD_TRACE(("%s Available secondary cpus %d nr_cpu_ids %d\n",
	...
	DHD_TRACE(("%s After secondary CPU check napi_cpu %d compl_cpu %d tx_cpu %d\n",
		__FUNCTION__, napi_cpu, compl_cpu, tx_cpu));
...
#pragma GCC diagnostic pop
	...
	if (!dhd || !(dhd->dhd_state & DHD_ATTACH_STATE_LB_ATTACH_DONE)) {
		DHD_TRACE(("%s(): LB data is not initialized yet.\n",
			__FUNCTION__));
		return NOTIFY_BAD;
	}
...
dhd_napi_schedule_on(dhd_info_t *dhd, int on_cpu)
{
	...
	DHD_TRACE(("%s dhd<%p> napi<%p> on_cpu<%d>\n",
...
dhd_lb_rx_napi_dispatch(dhd_pub_t *dhdp)
{
	...
	if ((on_cpu == curr_cpu) || (!cpu_online(on_cpu))) {
#endif /* DHD_LB_IRQSET */
		DHD_TRACE(("%s : curr_cpu : %d, cpumask : 0x%lx\n", __FUNCTION__,
			curr_cpu, *cpumask_bits(dhd->cpumask_primary)));
		dhd_napi_schedule(dhd);
	} else {
		DHD_TRACE(("%s : schedule to curr_cpu : %d, cpumask : 0x%lx\n",
