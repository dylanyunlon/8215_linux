...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#define WIFI_DT_COMPAT_ENTRY    "Autochips,ac8x-ExtWiFi"
#endif
...
void* wifi_platform_prealloc(wifi_adapter_info_t *adapter, int section, unsigned long size)
{
	...
	if (plat_data->mem_prealloc) {
		...
		if (alloc_ptr) {
			DHD_TRACE(("success alloc section %d\n", section));
			if (size != 0L)
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	if (strcmp(dev_name(&pdev->dev), name) == 0) {
#else
...
static int wifi_ctrlfunc_register_drv(void)
{
	...
	if (!dts_enabled) {
		if (dev1 == NULL && dev2 == NULL) {
			DHD_INFO(("no wifi platform data, skip\n"));
			return -ENXIO;
		}
	}
	...
	if (dhd_wifi_platdata == NULL) {
		...
		if (dhd_download_fw_on_driverload) {
			...
			for (i = 0; i < dhd_wifi_platdata->num_adapters; i++) {
				...
				DHD_TRACE((" - irq %d [flags %d], firmware: %s, nvram: %s\n",
					adapter->irq_num, adapter->intr_flags, adapter->fw_path,
					adapter->nv_path));
				DHD_TRACE((" - bus type %d, bus num %d, slot num %d\n\n",
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		if (err != 0) {
			DHD_ERROR(("dhd_bus_register ret=%d\n", err));
			return err;
		}
#endif
		return err;
