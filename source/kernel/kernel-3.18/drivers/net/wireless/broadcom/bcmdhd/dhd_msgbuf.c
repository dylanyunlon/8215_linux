...
/* to avoid below timeout and KE at kasan version
 * dhd_prot_d2h_sync_seqnum
 * dhd_prot_d2h_sync_xorcsum
 */
#define PCIE_D2H_SYNC_WAIT_TRIES    (512U)
#define PCIE_D2H_SYNC_WAIT_TRIES_LONG (512U)
#define PCIE_D2H_SYNC_NUM_OF_STEPS  (10U)
#define PCIE_D2H_SYNC_DELAY         (200UL)	/* in terms of usecs */
#else
#define PCIE_D2H_SYNC_WAIT_TRIES    (512U)
#define PCIE_D2H_SYNC_WAIT_TRIES_LONG (5*512U)
...
#endif
	for (step = 1; step <= PCIE_D2H_SYNC_NUM_OF_STEPS; step++) {
		for (tries = 0; tries < PCIE_D2H_SYNC_WAIT_TRIES_LONG; tries++) {
			...
			total_tries = ((step-1) * PCIE_D2H_SYNC_WAIT_TRIES_LONG) + tries;
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	end_time = jiffies;
	if (step*tries >= PCIE_D2H_SYNC_NUM_OF_STEPS*PCIE_D2H_SYNC_WAIT_TRIES)
		DHD_ERROR(("total_ms = %u ms\n", jiffies_to_msecs(end_time - start_time)));
#endif
	prot->d2h_sync_wait_tot += tries;
	return msg->msg_type;
...
dhd_prot_d2h_sync_init(dhd_pub_t *dhd)
{
	...
	if (HWA_ACTIVE(dhd)) {
		...
		DHD_INFO(("%s: TXCPLT hwa_db_type:0x%x RXCPLT hwa_db_type:0x%x\n",
...
#endif /* EWP_EDL */
		DHD_INFO(("%s(): D2H sync mechanism is NONE \r\n", __FUNCTION__));
	}
}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	map->keys = (dhd_pktid_key_t *)MALLOCZ(osh, map_keys_sz);
#else
	...
	void *pkt, dhd_pkttype_t pkttype)
{
	...
	if ((int)(map->avail) <= 0) { /* no more pktids to allocate */
		map->failures++;
		DHD_TRACE(("%s:%d: failed, no free keys\n", __FUNCTION__, __LINE__));
		DHD_PKTID_UNLOCK(map->pktid_lock, flags);
...
dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	if ((ret = dhd_prot_ring_attach(dhd, &prot->h2dring_ctrl_subn, "h2dctrl",
	...
	if ((ret = dhd_prot_ring_attach(dhd, &prot->h2dring_rxp_subn, "h2drxp",
	...
	if ((ret = dhd_prot_ring_attach(dhd, &prot->d2hring_ctrl_cpln, "d2hctrl",
	...
	if ((ret = dhd_prot_ring_attach(dhd, &prot->d2hring_tx_cpln, "d2htxcpl",
	...
	if ((ret = dhd_prot_ring_attach(dhd, &prot->d2hring_rx_cpln, "d2hrxcpl",
	...
	if ((ret = dhd_dma_buf_alloc(dhd, &prot->retbuf,
			IOCT_RETBUF_SIZE)) != BCME_OK) {
		goto fail;
	}

	/* IOCTL request buffer */
	if ((ret = dhd_dma_buf_alloc(dhd, &prot->ioctbuf,
			IOCT_RETBUF_SIZE)) != BCME_OK) {
		goto fail;
	}

	/* Host TS request buffer one buffer for now */
	if ((ret = dhd_dma_buf_alloc(dhd, &prot->hostts_req_buf,
			CTRLSUB_HOSTTS_MEESAGE_SIZE)) != BCME_OK) {
		goto fail;
	}
...
#ifdef BCM_HOST_BUF
	if ((ret = dhd_dma_buf_alloc(dhd, &prot->d2h_dma_scratch_buf,
			ROUNDUP(DMA_D2H_SCRATCH_BUF_LEN, 16)
			+ DMA_HOST_BUFFER_LEN)) != BCME_OK) {
#else
	if ((ret = dhd_dma_buf_alloc(dhd, &prot->d2h_dma_scratch_buf,
			DMA_D2H_SCRATCH_BUF_LEN)) != BCME_OK) {
#endif /* BCM_HOST_BUF */
		goto fail;
	}

	/* scratch buffer bus throughput measurement */
	if ((ret = dhd_dma_buf_alloc(dhd, &prot->host_bus_throughput_buf,
			DHD_BUS_TPUT_BUF_LEN)) != BCME_OK) {
		goto fail;
	}
...
#if defined(DHD_LB_TXC)
	{
		...
		DHD_TRACE(("%s: created tx_compl_workq <%p,%d>\n",
...
#if defined(DHD_LB_RXC)
	{
		...
		DHD_TRACE(("%s: created rx_compl_workq <%p,%d>\n",
					__FUNCTION__, buffer, DHD_LB_WORKQ_SZ));
	}
#endif /* DHD_LB_RXC */

	/* Initialize trap buffer */
	if ((ret = dhd_dma_buf_alloc(dhd, &dhd->prot->fw_trap_buf,
					trap_buf_len)) != BCME_OK) {
...
dhd_set_host_cap(dhd_pub_t *dhd)
{
	...
	if (dhd->bus->api.fw_rev >= PCIE_SHARED_VERSION_6) {
		...
		if (dhdpcie_bus_get_pcie_hostready_supported(dhd->bus)) {
			DHD_TRACE(("Advertise Hostready Capability\n"));
		...
		if (dhd->hp2p_capable) {
			...
			DHD_INFO(("Enable HP2P in host cap\n"));
		} else {
			DHD_INFO(("HP2P not enabled in host cap\n"));
		}
#endif // endif
		...
		DHD_TRACE(("%s:Active Ver:%d, Host Ver:%d, FW Ver:%d\n",
			__FUNCTION__,
			prot->active_ipc_version, prot->host_ipc_version,
...
void dhd_prot_detach(dhd_pub_t *dhd)
{
	...
	/* Common MsgBuf Rings */
	if (prot->d2hring_rx_cpln.ring_lock) // dhd_prot_ring_attach ok
		dhd_prot_ring_detach(dhd, &prot->d2hring_rx_cpln);

	if (prot->d2hring_tx_cpln.ring_lock)
		dhd_prot_ring_detach(dhd, &prot->d2hring_tx_cpln);

	if (prot->d2hring_ctrl_cpln.ring_lock)
		dhd_prot_ring_detach(dhd, &prot->d2hring_ctrl_cpln);

	if (prot->h2dring_rxp_subn.ring_lock)
		dhd_prot_ring_detach(dhd, &prot->h2dring_rxp_subn);

	if (prot->h2dring_ctrl_subn.ring_lock)
...
dhd_lb_tx_compl_handler(unsigned long data)
{
	...
	while (1) {
		...
		DHD_TRACE(("%s: tx_compl_cons pkt<%p>\n", __FUNCTION__, pkt));
...
dhd_check_create_info_rings(dhd_pub_t *dhd)
{
	...
	if (prot->h2dring_info_subn == NULL) {
		...
		DHD_TRACE(("%s: about to create debug submit ring\n", __FUNCTION__));
	...
	if (prot->d2hring_info_cpln == NULL) {
		...
		*/
		...
		DHD_TRACE(("%s: about to create debug cpl ring\n", __FUNCTION__));
...
dhd_prot_init_info_rings(dhd_pub_t *dhd)
{
	...
	if ((prot->d2hring_info_cpln->inited) || (prot->d2hring_info_cpln->create_pending)) {
		DHD_TRACE(("Info completion ring was created!\n"));
		return ret;
	}
...
dhd_check_create_hp2p_rings(dhd_pub_t *dhd)
{
	...
	if (prot->d2hring_hp2p_txcpl == NULL) {
		...
		DHD_TRACE(("%s: about to create hp2p txcpl ring\n", __FUNCTION__));
	...
	if (prot->d2hring_hp2p_rxcpl == NULL) {
		...
		*/
		...
		DHD_TRACE(("%s: about to create hp2p rxcpl ring\n", __FUNCTION__));
...
dhd_prot_init_hp2p_rings(dhd_pub_t *dhd)
{
	...
	if ((prot->d2hring_hp2p_txcpl->inited) || (prot->d2hring_hp2p_txcpl->create_pending)) {
		DHD_TRACE(("hp2p tx completion ring was created!\n"));
		return ret;
	}
	...
	if ((prot->d2hring_hp2p_rxcpl->inited) || (prot->d2hring_hp2p_rxcpl->create_pending)) {
		DHD_TRACE(("hp2p rx completion ring was created!\n"));
		return ret;
	}
...
dhd_prot_init_edl_rings(dhd_pub_t *dhd)
{
	...
	if ((prot->d2hring_edl->inited) || (prot->d2hring_edl->create_pending)) {
		DHD_TRACE(("EDL completion ring was created!\n"));
		return ret;
	}
...
int dhd_sync_with_dongle(dhd_pub_t *dhd)
{
	...
	ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf), FALSE, 0);
	if (ret < 0) {
		DHD_INFO(("%s: GET RxBuf post FAILED, default to %d\n",
...
dhd_prot_rxbuf_post(dhd_pub_t *dhd, uint16 count, bool use_rsv_pktid)
{
	...
	if (msg_start == NULL) {
		DHD_TRACE(("%s:%d: Rxbufpost Msgbuf Not available\n", __FUNCTION__, __LINE__));
	...
	for (i = 0; i < alloced; i++) {
		...
		if (use_rsv_pktid == TRUE) {
			...
			if (elem_ix == BCM_RING_EMPTY) {
				DHD_TRACE(("%s rx_compl_cons ring is empty\n", __FUNCTION__));
...
alloc_pkt_id:
	...
	if (i < alloced) {
		...
		if (ring->wr == 0) {
			DHD_TRACE(("%s: flipping the phase now\n", ring->name));
...
dhd_prot_infobufpost(dhd_pub_t *dhd, msgbuf_ring_t *ring)
{
	...
	if (count <= 0) {
		DHD_TRACE(("%s: Cannot post more than max info resp buffers\n",
			__FUNCTION__));
		return 0;
	}
	...
	if (msg_start == NULL) {
		DHD_TRACE(("%s:%d: infobufpost Msgbuf Not available\n", __FUNCTION__, __LINE__));
		DHD_RING_UNLOCK(ring->ring_lock, flags);
		return -1;
	}
...
#if defined(DHD_PCIE_PKTID)
	if (rxbuf_post->cmn_hdr.request_id == DHD_PKTID_INVALID) { //-V547
		if (ring->wr == 0) {
			ring->wr = ring->max_items - 1;
		} else {
			if (ring->wr == 0) { //-V571 PVS V571 wrong condition ?
				ring->current_phase = ring->current_phase ? 0 :
					BCMPCIE_CMNHDR_PHASE_BIT_INIT;
			}
		}
...
dhd_msgbuf_rxbuf_post_ioctlresp_bufs(dhd_pub_t *dhd)
{
	...
	if (max_to_post <= 0) {
		DHD_TRACE(("%s: Cannot post more than max IOCTL resp buffers\n",
			__FUNCTION__));
		return;
	}
...
dhd_prot_process_edl_complete(dhd_pub_t *dhd, void *evt_decode_data)
{
	...
	DHD_TRACE(("%s: EDL work items [%u] available \n",
	...
	*/
	if (num_items <= DHD_EVENT_LOGTRACE_BOUND) {
		...
		if (wr != (volatile uint16)ring->wr) {
			...
			DHD_TRACE(("%s: new items [%u] avail in edl\n",
	...
	*/
	if (max_items_to_process < num_items) {
		DHD_TRACE(("%s: EDL bound hit / new items found, "
...
dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	uint32 pktlen;
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	uint8 *dump_pktdata = NULL;
	uint32 dump_pktlen = 0;
#endif
	uint32 pktid;
	uint8	prio;
	uint16 flowid = 0;
	...
	txdesc = (host_txbuf_post_t *)
		dhd_prot_alloc_ring_space(dhd, ring, 1, &alloced, FALSE);
	if (txdesc == NULL) {
		DHD_TRACE(("%s:%d: HTOD Msgbuf Not available TxCount = %d\n",
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	dhd_dump_pkt(dhd, ifidx, dump_pktdata, dump_pktlen, TRUE, NULL, NULL);
#endif
...
int dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t * ioc, void * buf, int len)
{
	...
	if (ret >= 0) {
		...
		DHD_TRACE(("%s: status ret value is %d \n", __FUNCTION__, ret));
	...
	uint16 nitems, uint16 * alloced, bool exactly_nitems)
{
	...
	if (ret_buf == NULL) {
		...
		if (ret_buf == NULL) {
			DHD_TRACE(("%s: Ring space not available  \n", ring->name));
			return NULL;
		}
	}
	...
	uint16 max_items, uint16 item_len, uint16 ringid)
{
	...
	DHD_TRACE(("RING_ATTACH : %s Max item %d len item %d total size %d "
	...
	uint16 ring_type, uint32 req_id)
{
	...
	DHD_INFO(("%s ringid: %d idx: %d max_h2d: %d\n", __FUNCTION__, d2h_ring->ring_id,
...
dhd_prot_flow_ring_create(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node)
{
	...
	if (dhd->hp2p_capable && !dhd->hp2p_ring_active &&
		...
		DHD_INFO(("%s: flow ring for HP2P tid = %d flowid = %d\n",
...
dhd_prot_process_h2d_ring_create_complete(dhd_pub_t *dhd, void *buf)
{
	...
	DHD_TRACE(("%s ring create Response status = %d ring %d, id 0x%04x\n", __FUNCTION__,
	...
	if (dhd->prot->h2dring_info_subn->create_req_id == ltoh32(resp->cmn_hdr.request_id) &&
		...
		DHD_INFO(("info ring create status for not pending submit ring\n"));
	}
...
dhd_prot_flow_ring_fastdelete(dhd_pub_t *dhd, uint16 flowid, uint16 rd_idx)
{
	...
	DHD_TRACE(("%s: FAST delete ring, flowid=%d, rd_idx=%d, wr_idx=%d\n",
...
dhd_prot_flow_ring_delete_response_process(dhd_pub_t *dhd, void *msg)
{
	...
	DHD_INFO(("%s: Flow Delete Response status = %d Flow %d\n", __FUNCTION__,
...
dhd_prot_flow_ring_flush(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node)
{
	...
	DHD_TRACE(("%s: Send Flow Flush Req\n", __FUNCTION__));
...
dhd_prot_flow_ring_flush_response_process(dhd_pub_t *dhd, void *msg)
{
	...
	DHD_TRACE(("%s: Flow Flush Response status = %d\n", __FUNCTION__,
...
dhd_msgbuf_ring_config_d2h_soft_doorbell(dhd_pub_t *dhd)
{
	...
	for (ring_idx = 0; ring_idx < d2h_rings; ring_idx++) {
		...
		DHD_TRACE(("%s: Soft doorbell haddr 0x%08x 0x%08x value 0x%08x\n",
...
{
	DHD_TRACE(("%s: Ring Config Response - status %d ringid %d\n",
...
copy_hang_info_trap(dhd_pub_t *dhd)
{
	...
	DHD_TRACE(("hang info haed cnt: %d len: %d data: %s\n",
	...
	if (dhd->hang_info_cnt < HANG_FIELD_CNT_MAX) {
		...
		DHD_TRACE(("hang info specific cnt: %d len: %d data: %s\n",
			dhd->hang_info_cnt, (int)strlen(dhd->hang_info), dhd->hang_info));
	}
#else
	if (dhd->hang_info_cnt < HANG_FIELD_CNT_MAX) {
		copy_hang_info_stack(dhd, dhd->hang_info, &bytes_written, &dhd->hang_info_cnt);
		DHD_TRACE(("hang info stack cnt: %d len: %d data: %s\n",
			dhd->hang_info_cnt, (int)strlen(dhd->hang_info), dhd->hang_info));
	}
	...
	if (dhd->hang_info_cnt < HANG_FIELD_CNT_MAX) {
		copy_hang_info_trap_t(dhd->hang_info, &tr, VENDOR_SEND_HANG_EXT_INFO_LEN, FALSE,
				&bytes_written, &dhd->hang_info_cnt, dhd->debug_dump_time_hang_str);
		DHD_TRACE(("hang info trap_t cnt: %d len: %d data: %s\n",
			dhd->hang_info_cnt, (int)strlen(dhd->hang_info), dhd->hang_info));
	}
...
dhd_prot_debug_info_print(dhd_pub_t *dhd)
{
	...
	if (dhd->bus->is_linkdown) {
		DHD_ERROR(("%s : PCIe is_linkdown\n", __FUNCTION__));
	} else if (!dhd_pcie_dump_int_regs(dhd)) {
		DHD_ERROR(("%s : PCIe link might be down\n", __FUNCTION__));
		dhd->bus->is_linkdown = TRUE;
	}
#else
...
#pragma GCC diagnostic pop
	...
	DHD_TRACE(("%s: pend_item = %d flowid = %d\n",
...
dhd_calc_hp2p_burst(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint16 flowid)
{
	...
	if (ring->pend_items_count == dhd->pkt_thresh) {
		...
		DHD_TRACE(("%s: cancel hrtimer for flowid = %d \n"
		...
		if (hp2p_info->hrtimer_init == FALSE) {
			...
			DHD_TRACE(("%s: start hrtimer for flowid = %d hp2_flowid = %d\n",
...
dhd_update_hp2p_txdesc(dhd_pub_t *dhd, host_txbuf_post_t *txdesc)
{
	...
	DHD_TRACE(("%s: metadata_high = 0x%x metadata_low = 0x%x exp_time = %x\n",
