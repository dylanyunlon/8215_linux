	...
	volatile char *regs, volatile char *tcm, void *pci_dev)
{
	...
	*/
	...
	do {
		...
		if ((ret = dhdpcie_sromotp_customvar(bus,
						&customvar1, &customvar2)) != BCME_OK) {
			DHD_ERROR(("%s: dhdpcie_sromotp_customvar failed\n", __FUNCTION__));
			break;
		}
		if (!customvar2) {
			DHD_INFO(("%s:customvar2 is not OTPed"
...
dhdpcie_bus_isr(dhd_bus_t *bus)
{
	...
	do {
		...
		if (!bus->intr_enabled) {
			DHD_TRACE(("%s, not ready to receive interrupts\n", __FUNCTION__));
			break;
		}
...
dhdpcie_set_pwr_state(dhd_bus_t *bus, uint state)
{
	...
	*/
	if (state != PCIECFGREG_PM_CSR_STATE_D0 &&
			cur_state <= PCIECFGREG_PM_CSR_STATE_D3_COLD && //-V560
			cur_state > state) {
...
dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	if (MULTIBP_ENAB(bus->sih) && (bus->sih->buscorerev >= 66)) { //-V560
		/*
...
dhd_bus_schedule_queue(struct dhd_bus  *bus, uint16 flow_id, bool txs)
{
	...
	if ((dhdp->dequeue_prec_map & (1 << flow_ring_node->flow_info.tid)) == 0) {
		DHD_TRACE(("%s: tid %d is not in precedence map. block scheduling\n",
...
dhd_bus_txdata(struct dhd_bus *bus, void *txp, uint8 ifidx)
{
	...
	{
		DHD_FLOWRING_UNLOCK(flow_ring_node->lock, flags);
		DHD_TRACE(("%s: Dropping pkt flowid %d, status %d active %d\n",
...
dhd_bus_perform_flr(dhd_bus_t *bus, bool force_fail)
{
	...
	if (!flr_capab) {
	       DHD_INFO(("Chip does not support FLR\n"));
	       return BCME_UNSUPPORTED;
	}
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_GVAL(IOV_MEMBYTES):
	{
		...
		DHD_TRACE(("%s: Request to %s %d bytes at address 0x%08x\n dsize %d ", __FUNCTION__,
		    ...
		    ((address > bus->orig_ramsize) || (address + size > bus->orig_ramsize)))
		{
			...
			if (!REMAP_ENAB(bus) && (address >= SOCDEVRAM_ARM_ADDR)) {
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				if (address + size > SOCDEVRAM_ARM_ADDR + devramsize) {
#else // PVS V560
				...
				DHD_TRACE(("%s: Request to %s %d bytes @ Mapped address 0x%08x\n",
...
dhd_bus_dump_dar_registers(struct dhd_bus *bus)
{
	...
	if (bus->sih->buscorerev < 24) {
		DHD_INFO(("%s: DAR not supported for corerev(%d) < 24\n",
...
dhdpcie_bus_download_state(dhd_bus_t *bus, bool enter)
{
	...
	if (MULTIBP_ENAB(bus->sih) && !do_flr) { //-V560
		dhd_bus_pcie_pwr_req(bus);
	}
...
void dhd_dump_intr_counters(dhd_pub_t *dhd, struct bcmstrbuf *strbuf)
{
...
#endif /* BCMPCIE_OOB_HOST_WAKE */
	bcm_bprintf(strbuf, "\ncurrent_time="SEC_USEC_FMT" isr_entry_time="SEC_USEC_FMT //-V567
		" isr_exit_time="SEC_USEC_FMT"\ndpc_sched_time="SEC_USEC_FMT
	...
	bcm_bprintf(strbuf, "\nlast_suspend_start_time="SEC_USEC_FMT" last_suspend_end_time=" //-V567
		...
		bcm_bprintf(strbuf, "logtrace_thread_entry_time="SEC_USEC_FMT //-V567
...
dhdpcie_bus_process_mailbox_intr(dhd_bus_t *bus, uint32 intstatus)
{
	...
	if ((bus->sih->buscorerev == 2) || (bus->sih->buscorerev == 6) ||
		...
		if ((bus->d2h_intr_method == PCIE_MSI) ||
			((bus->d2h_intr_method == PCIE_INTX) && (intstatus & bus->d2h_mb_mask))) { //-V560
#ifdef DHD_PCIE_NATIVE_RUNTIMEPM
...
dhdpcie_readshared_console(dhd_bus_t *bus)
{
	...
	if ((addr == 0) || (addr == bus->nvram_csm) || (addr < bus->dongle_ram_base) ||
		...
		DHD_INFO(("%s:PCIe shared addr (0x%08x) read took %u usec "
...
dhdpcie_chipmatch(uint16 vendor, uint16 device)
{
	...
	if (vendor == PCI_VENDOR_ID_BROADCOM || vendor == PCI_VENDOR_ID_CYPRESS) {
		DHD_INFO(("%s: Supporting vendor %x device %x\n", __FUNCTION__,
			vendor, device));
	} else {
...
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT // PVS V560
...
{
	if (bus->sih->buscorerev < 19) {
		DHD_TRACE(("%s: Unsupported CTO, buscorerev=%d\n",
			__FUNCTION__, bus->sih->buscorerev));
		return BCME_UNSUPPORTED;
	}
...
dhd_pcie_dump_wrapper_regs(dhd_pub_t *dhd)
{
	...
	if (oob_base) {
		...
		val = R_REG(dhd->osh, &reg->intstatus[1]); //-V641
		DHD_ERROR(("reg: addr:%p val:0x%x\n", reg, val));
