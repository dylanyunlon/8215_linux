...
int dhdpcie_bus_registered = FALSE;
#endif
...
dhd_bus_aspm_enable_dev(dhd_bus_t *bus, struct pci_dev *dev, bool enable)
{
	...
	linkctrl_after = dhdpcie_access_cap(dev, PCIE_CAP_ID_EXP, PCIE_CAP_LINKCTRL_OFFSET,
		FALSE, FALSE, 0);
	DHD_INFO(("%s: %s %s, linkctrl_before: 0x%x linkctrl_after: 0x%x\n",
...
dhd_bus_aspm_enable_rc_ep(dhd_bus_t *bus, bool enable)
{
	...
	if (enable) {
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		ret2 = dhd_bus_aspm_enable_dev(bus, bus->dev, enable);
#else // PVS V519
		...
		ret = dhd_bus_aspm_enable_dev(bus, bus->dev, enable);
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		ret2 = dhd_bus_aspm_enable_dev(bus, bus->rc_dev, enable);
#else // PVS V519
		ret = dhd_bus_aspm_enable_dev(bus, bus->rc_dev, enable);
#endif
	}
...
dhd_bus_l1ss_enable_dev(dhd_bus_t *bus, struct pci_dev *dev, bool enable)
{
	...
	l1ssctrl_after = dhdpcie_access_cap(dev, PCIE_EXTCAP_ID_L1SS,
		PCIE_EXTCAP_L1SS_CONTROL_OFFSET, TRUE, FALSE, 0);
	DHD_INFO(("%s: %s %s, l1ssctrl_before: 0x%x l1ssctrl_after: 0x%x\n",
		__FUNCTION__, device, (enable ? "ENABLE " : "DISABLE"),
...
dhdpcie_rc_config_read(dhd_bus_t *bus, uint offset)
{
	...
	if (ret) {
		DHD_ERROR(("pcie host error %d\n", ret));
		return val;
	}
#endif
	if (bus->rc_dev) {
		_pci_read_config_dword(bus->rc_dev, offset, &val);
		OSL_DELAY(100);
	} else {
		DHD_ERROR(("%s: RC %x:%x handle is NULL\n",
			__FUNCTION__, PCIE_RC_VENDOR_ID, PCIE_RC_DEVICE_ID));
...
#endif
	if (is_write) {
		_pci_write_config_dword(pdev, (cap_ptr + offset), writeval);
		ret = BCME_OK;
	} else {
		_pci_read_config_dword(pdev, (cap_ptr + offset), &readval);
		ret = readval;
	}
...
#pragma GCC diagnostic pop
	...
	if (pcidev->vendor == VENDOR_CYPRESS) {
		...
		DHD_TRACE(("Found Broadcom PCI device 0x%04x\n", pcidev->device));
	}
	*cnt += 1;
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	DHD_INFO(("PCI_PROBE: bus %d slot %d vendor %X device %X "
		"(good PCI location)\n", pdev->bus->number,
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
static void dhd_isr_clear(void);
#endif
...
int dhdpcie_init(struct pci_dev *pdev)
{
	...
	do {
		...
		adapter = dhd_wifi_platform_get_adapter(PCI_BUS, pdev->bus->number,
			PCI_SLOT(pdev->devfn));
		if (adapter != NULL)
			DHD_INFO(("%s: found adapter info '%s'\n", __FUNCTION__, adapter->name));
		else
			DHD_INFO(("%s: can't find adapter info for this chip\n", __FUNCTION__));
		osl_static_mem_init(osh, adapter);

		/* Set ACP coherence flag */
		if (OSL_ACP_WAR_ENAB() || OSL_ARCH_IS_COHERENT()) //-V560
		...
		if (bus->dev->bus) {
			...
			DHD_INFO(("%s: rc_dev from dev->bus->self (%x:%x) is %pK\n", __FUNCTION__,
		...
		DHD_INFO(("%s: rc_ep_aspm_cap: %d rc_ep_l1ss_cap: %d\n",
			__FUNCTION__, bus->rc_ep_aspm_cap, bus->rc_ep_l1ss_cap));
	...
	*/
		...
		if (bus->intr) {
			...
			bus->pollrate = 1;
			DHD_TRACE(("%s: PCIe interrupt function is NOT registered "
				"due to polling mode\n", __FUNCTION__));
		}
...
static atomic_t dhd_isr_count = ATOMIC_INIT(0);
static unsigned long dhd_isr_time = 0;

static void dhd_isr_clear(void)
{
	dhd_isr_time = 0;
	atomic_set(&dhd_isr_count, 0);
}

static void dhd_isr_tick(void)
{
	dhd_isr_time = jiffies;
	atomic_inc(&dhd_isr_count);
}

void dhd_isr_dump(void)
{
	DHD_INFO(("delta time %u ms, dhd_isr_count %u\n",
		jiffies_to_msecs(jiffies - dhd_isr_time),
		atomic_read(&dhd_isr_count)));
}
...
int
dhdpcie_stop_host_pcieclock(dhd_bus_t *bus) //-V524
{
	int ret = 0;
#ifdef CONFIG_ARCH_MSM
#ifdef SUPPORT_LINKDOWN_RECOVERY
...
#endif
		_pci_read_config_dword(bus->dev, PCI_CFG_VID, &vid);
...
bool dhd_runtime_bus_wake(dhd_bus_t *bus, bool wait, void *func_addr)
{
	...
	if (DHD_BUS_BUSY_CHECK_RPM_ALL(bus->dhd)) {
		...
		if (wait) {
			...
			DHD_TRACE(("%s: bus wakeup but no wait until resume done\n", __FUNCTION__));
		}
