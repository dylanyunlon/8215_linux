...
dhdsdio_readshared(dhd_bus_t *bus, sdpcm_shared_t *sh)
{
	...
	if ((CHIPID(bus->sih->chip) == BCM43430_CHIP_ID ||
		...
		DHD_ERROR(("dhdsdio_sr_cap(bus) return false, bus->srmemsize 0\n"));
#endif
	}
...
dhdsdio_isr(void *arg)
{
	...
	bcmsdh_info_t *sdh;
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#define DHD_BUS_DOWN_TRG_THRESHOLD      20
	static int dhd_bus_down_cnt = 0;
#endif
	...
	if (bus->dhd->busstate == DHD_BUS_DOWN) {
		dhd_bus_down_cnt++;
		if (dhd_bus_down_cnt == DHD_BUS_DOWN_TRG_THRESHOLD) {
			DHD_ERROR(("dhd_bus_down_cnt %d, trigger wlan reset\n",
				dhd_bus_down_cnt));
			dhd_bus_down_cnt = 0;
			sdioh_io_blocked = true;
			dhd_schedule_reset(NULL);
		} else {
			DHD_ERROR(("dhd_bus_down_cnt %d, we have nothing to do\n",
				dhd_bus_down_cnt));
		}
		return;
	} else {
                     ...
                     uint16 devid)
{
	...
	if (bus->chipidpresent) {
		DHD_INFO(("F1 signature read @0x%lx=0x%4x\n",
	...
	do {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		if (fn == 2) {
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_CHIP_CTRL_DATA, 0x80, &ret);
			if (ret < 0) {
				continue;
			}
		}
		...
		if (ret == BCME_NODEVICE) {
			...
			for (i = 0; i < READ_FRM_CNT_RETRIES; i++) {
				...
				int ret1 = -1;
				int ret2 = -1;
				hi = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_WFRAMEBCHI, &ret1);
				lo = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_WFRAMEBCLO, &ret2);
				bus->f1regdata += 2;
				if (ret1 == 0 && hi == 0 && ret2 == 0 && lo == 0) {
					break;
				}
#else
				uint8 hi, lo;
	...
	if (flag == TRUE) {
		...
		if (bus->dhd->dongle_reset) {
			...
			*/
			...
			DHD_TRACE(("%s called when dongle is not in reset\n",
				__FUNCTION__));
#if defined(OEM_ANDROID)
			DHD_TRACE(("Will call dhd_bus_start instead\n"));
