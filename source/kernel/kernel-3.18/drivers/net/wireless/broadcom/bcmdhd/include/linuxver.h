...
#define	PCI_SAVE_STATE(a, b)	_pci_save_state(a)
#define	PCI_RESTORE_STATE(a, b)	_pci_restore_state(a)
#else
#define	PCI_SAVE_STATE(a, b)	_pci_save_state(a, b)
#define	PCI_RESTORE_STATE(a, b)	_pci_restore_state(a, b)
#endif // endif

#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 4, 6))
static inline int
_pci_save_state(struct pci_dev *dev, u32 *buffer)
{
	int i;
	...
	if (buffer) {
		for (i = 0; i < 16; i++)
			_pci_read_config_dword(dev, i * 4, &buffer[i]);
	}
	return 0;
}

static inline int
_pci_restore_state(struct pci_dev *dev, u32 *buffer)
{
	int i;
#if defined(CONFIG_ATC_AOSP_ENHANCEMENT) && defined(BCMPCIE)
	int ret = pcie_host_status_dump(0);
	...
	if (buffer) {
		for (i = 0; i < 16; i++)
			_pci_write_config_dword(dev, i * 4, buffer[i]);
	}
	/*
	 ...
	 */
		...
		for (i = 0; i < 6; i ++)
			_pci_write_config_dword(dev,
			                       PCI_BASE_ADDRESS_0 + (i * 4),
			                       pci_resource_start(dev, i));
		_pci_write_config_byte(dev, PCI_INTERRUPT_LINE, dev->irq);
	}
	return 0;
}
...
{ \
	...
	(tsk_ctl)->up_cnt = 0; \
	(tsk_ctl)->p_task  = kthread_run(thread_func, tsk_ctl, name); \
