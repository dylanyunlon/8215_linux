...
#define ASSERT_TYPE_PANIC           0
#define ASSERT_TYPE_FALL_THROUGH    1
#define ASSERT_TYPE_LOG             3
#define ASSERT_TYPE_BUG             2

uint32 g_assert_type = ASSERT_TYPE_LOG;
...
osl_attach(void *pdev, uint bustype, bool pkttag)
{
	...
	flags = CAN_SLEEP() ? GFP_KERNEL: GFP_ATOMIC; //-V530
	...
	if ((secdma_addr != 0) && (secdma_size != 0)) {
		LOGI("linux_osl.c: Buffer info passed via module params, using it.\n");
		if (secdma_found == 0) {
			...
			LOGE("linux_osl.c secdma: secDMA instances %d\n", secdma_found);
			kfree(osh);
			return NULL;
		}
		osh->contig_base = (phys_addr_t)osh->contig_base_alloc;
		LOGI("linux_osl.c: secdma_cma_size = 0x%x\n", secdma_memsize);
		LOGI("linux_osl.c: secdma_cma_addr = 0x%x\n",
			(unsigned int)osh->contig_base_alloc);
		osh->stb_ext_params = SECDMA_MODULE_PARAMS;
	}
	else if (stbpriv_init(osh) == 0) {
		LOGI("linux_osl.c: stbpriv.txt found. Get buffer info.\n");
		if (secdma_found == 0) {
		...
		LOGE("linux_osl.c: secDMA no longer supports internal buffer allocation.\n");
		kfree(osh);
		return NULL;
	}
	secdma_found++;
...
osl_pci_read_config(osl_t *osh, uint offset, uint size)
{
...
#if defined(CONFIG_ATC_AOSP_ENHANCEMENT) && defined(BCMPCIE)
	int ret = pcie_host_status_dump(0);

	if (ret) {
		LOGE("pcie host error %d\n", ret);
		return 0;
	}
#endif
	...
	do {
		_pci_read_config_dword(osh->pdev, offset, &val);
		if (val != 0xffffffff)
			break;
	} while (retry--);
...
osl_pci_write_config(osl_t *osh, uint offset, uint size, uint val)
{
	...
	do {
		_pci_write_config_dword(osh->pdev, offset, val);
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		hwdev->dev.coherent_dma_mask = DMA_BIT_MASK(64);
		if(!hwdev->dev.dma_mask) {
			hwdev->dev.dma_mask = &hwdev->dev.coherent_dma_mask;
		}
...
#else /* ! STB_SOC_WIFI */
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	hwdev = (struct pci_dev *)(osh->pdev);
	//LOGD("pci_map_single::size=0x%x, dir=0x%x dev=%p\n",
	//  size, dir, hwdev);
	hwdev->dma_mask = DMA_BIT_MASK(64);
	ret = dma_set_mask_and_coherent(&(hwdev->dev), DMA_BIT_MASK(64));
	//LOGD("dma_set_mask_and_coherent::SET MASK=0x%llx, MASK=0x%llx,ret=%d\n",
	//  *(hwdev->dev.dma_mask), hwdev->dma_mask, ret);
#endif
...
#ifdef BCMASSERT_LOG
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	snprintf(tempbuf, sizeof(tempbuf), "\"%s\": file \"%s\", line %d\n",
		exp, basename, line);
#else
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	case ASSERT_TYPE_PANIC:
		//panic("%s", tempbuf);
		LOGE("[ASSERT_TYPE_PANIC] %s\n", tempbuf);
		dump_stack();
		break;
	case ASSERT_TYPE_FALL_THROUGH:
		LOGE("[ASSERT_TYPE_FALL_THROUGH] %s\n", tempbuf);
		dump_stack();
		break;
	case ASSERT_TYPE_LOG:
		LOGE("[ASSERT_TYPE_LOG] %s\n", tempbuf);
		dump_stack();
		break;
	case ASSERT_TYPE_BUG:
		LOGE("[ASSERT_TYPE_BUG] %s\n", tempbuf);
		//BUG();
		dump_stack();
		break;
	default:
...
{
	// WifiManager scan timestamp is uptime

#ifdef CONFIG_ATC_AOSP_ENHANCEMENT // uptime

#if KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE
	return local_clock() / NSEC_PER_USEC;
#else // 4.9
	struct timespec ts;
	uint64_t bootTime = 0;
...
inline void osl_pcie_rreg(osl_t *osh, ulong addr, volatile void *v, uint size)
{
	...
	switch (size) {
	case sizeof(uint8):
		*(volatile uint8*)v = _readb((volatile uint8*)(addr));
		break;
	case sizeof(uint16):
		*(volatile uint16*)v = _readw((volatile uint16*)(addr));
		break;
	case sizeof(uint32):
		*(volatile uint32*)v = _readl((volatile uint32*)(addr));
		break;
	case sizeof(uint64):
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		*(volatile uint64*)v = _readq((volatile uint64*)(addr));
#else
...
osl_timer_init(osl_t *osh, const char *name, void (*fn)(void *arg), void *arg)
{
	...
	if ((t = MALLOCZ(NULL, sizeof(osl_timer_t))) == NULL) {
		LOGE("osl_timer_init: out of memory, malloced %d bytes\n",
...
{
	if (likely(in_irq() || irqs_disabled())) { //-V530
