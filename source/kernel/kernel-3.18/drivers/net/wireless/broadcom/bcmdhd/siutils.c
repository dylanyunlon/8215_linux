...
#define si_is_link_log_enable() _si_is_link_log_enable(__LINE__)

static bool _si_is_link_log_enable(int line)
{
	static unsigned int log_count = 0;
	static int log_line_pre = 0;
	static int log_line = 0;

	log_line_pre = log_line;
	log_line = line;

	// reset log_count at a new log line
	if (log_line_pre != log_line) {
		log_count = 0;
	}
	log_count++;

	// enable log at the same line for 3 times
	if (log_count <= 3) {
		return true;
	} else {
		return false;
...
static bool si_is_link_ok(si_t *sih)
{
	si_info_t *sii = SI_INFO(sih);
	osl_t *osh = sii->osh;
	struct pci_dev *pdev = (struct pci_dev *)(osh->pdev);
	uint32_t value;
	int ret = 0;

	ret = pcie_host_status_dump(0);

	// plug out pcie card get here
	if (-ENODEV == ret) {
		if (si_is_link_log_enable()) {
			SI_ERROR(("%s,%d: link down\n",
						__func__, __LINE__));
			pcie_host_status_dump(1);
		}
		return false;

	// plug out and plug in pcie card get here
	} else if (-ENOLINK == ret) {
		if (si_is_link_log_enable()) {
			SI_ERROR(("%s,%d: LINK_DOWN_RESET_OUT\n",
						__func__, __LINE__));
			pcie_host_status_dump(1);
		}
		return false;
	...
	/* Update this value 0x30400004 from pcie owner if change wlan driver.
	 *
	 * OK value: 0x30400004
	 * If plug out or (out and in) pcie card, we will get
	 * Fail value: 0x00000004, and si_corereg RW will KE.
	 * Is it similar for power drop & power ripple & chip abnormal reset ?
	 */
	ret = _pci_read_config_dword(pdev, PCI_BASE_ADDRESS_0, &value);
	if (ret != PCIBIOS_SUCCESSFUL
			|| value != 0x30400004) { // chip reset ?
		if (si_is_link_log_enable()) {
			SI_ERROR(("%s,%d: LINK_DOWN_RESET_OUT\n",
						__func__, __LINE__));
		}
	...
	si_is_link_log_enable(); // update log_line

	return true;
}

#endif // CONFIG_ATC_AOSP_ENHANCEMENT && BCMPCIE

uint
si_corereg(si_t *sih, uint coreidx, uint regoff, uint mask, uint val)
{
#if defined(CONFIG_ATC_AOSP_ENHANCEMENT) && defined(BCMPCIE)
	if (!si_is_link_ok(sih)) {
		if (!atc_combo_wlan_reset_is_triggered()) {
			if (g_dhd_bus) {
				g_dhd_bus->is_linkdown = 1;
			}
...
si_corebist(si_t *sih)
{
	...
	/* Wait for bist done */
	SPINWAIT(((si_core_sflags(sih, 0, 0) & SISF_BIST_DONE) == 0), 100000); //-V562
