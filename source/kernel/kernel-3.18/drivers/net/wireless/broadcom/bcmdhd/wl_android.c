...
static int wl_android_set_suspendopt(struct net_device *dev, char *command)
{
	...
	if (ret_now != suspend_flag) {
		if (!(ret = net_os_set_suspend(dev, ret_now, 1))) {
			DHD_TRACE(("wl_android_set_suspendopt: Suspend Flag %d -> %d\n",
...
static int wl_android_set_suspendmode(struct net_device *dev, char *command)
{
	...
	if (!(ret = net_os_set_suspend(dev, suspend_flag, 0)))
		DHD_TRACE(("wl_android_set_suspendmode: Suspend Mode %d\n", suspend_flag));
	else
...
int wl_android_get_chanspec(struct net_device *dev, char *command, int total_len)
{
	...
	DHD_TRACE(("wl_android_get_80211_mode: return value of chanspec:%x\n", chanspec));

	channel = chanspec & WL_CHANSPEC_CHAN_MASK;
	band = chanspec & WL_CHANSPEC_BAND_MASK;
	bw = chanspec & WL_CHANSPEC_BW_MASK;

	DHD_TRACE(("wl_android_get_80211_mode: channel:%d band:%d bandwidth:%d\n",
		channel, band, bw));
	...
	DHD_TRACE(("wl_android_get_chanspec: command:%s EXIT\n", command));
	return bytes_written;
...
int wl_android_get_rsdb_mode(struct net_device *dev, char *command, int total_len)
{
	...
	DHD_TRACE(("wl_android_get_rsdb_mode: rsdb_mode:%d\n", rsdb_mode));
...
static int wl_android_set_csa(struct net_device *dev, char *command)
{
	...
	DHD_TRACE(("wl_android_set_csa: command:%s\n", command));
	...
	struct net_device *dev, char *command, int total_len)
{
	...
	if (error) {
		...
		DHD_TRACE(("%s: get full roam scan period %d\n", __func__, full_roam_scan_period));
	}
		...
		if (assoc_maclist->count) {
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				...
				for (j = 0; j < maclist->count; j++) {
					DHD_TRACE(("wl_android_set_ap_mac_list: associated="MACDBG
					"list = "MACDBG "\n",
					MAC2STRDBG(assoc_maclist->ea[i].octet),
...
int wl_android_wifi_on(struct net_device *dev)
{
	...
	if (!g_wifi_on) {
		...
		} while (retry-- > 0); //-V654 PVS V654 why always false ?
		if (ret != 0) {
...
wl_android_set_max_num_sta(struct net_device *dev, const char* string_num)
{
	...
	DHD_TRACE(("wl_android_set_max_num_sta : HAPD_MAX_NUM_STA = %d\n", max_assoc));
...
wl_android_set_hide_ssid(struct net_device *dev, const char* string_num)
{
	...
	DHD_TRACE(("wl_android_set_hide_ssid: HIDE_SSID = %d\n", hide_ssid));
	if (hide_ssid) {
		enable = 1;
	}
...
wl_android_sta_diassoc(struct net_device *dev, const char* straddr)
{
	...
	DHD_TRACE(("wl_android_sta_diassoc: deauth STA %s\n", straddr));
...
wl_android_set_lpc(struct net_device *dev, const char* string_num)
{
	...
	DHD_TRACE(("wl_android_set_lpc: HAPD_LPC_ENABLED = %d\n", lpc_enabled));
...
wl_android_set_ltecx(struct net_device *dev, const char* string_num)
{
	...
	DHD_TRACE(("wl_android_set_ltecx: LTECOEX 0x%x\n", chan_bitmap));
...
wl_android_rmc_set_leader(struct net_device *dev, const char* straddr)
{
	...
	DHD_TRACE(("wl_android_rmc_set_leader: Set new RMC leader %s\n", straddr));

	bzero(&rmc_entry, sizeof(wl_rmc_entry_t));
	if (!bcm_ether_atoe(straddr, &rmc_entry.addr)) {
		if (strlen(straddr) == 1 && bcm_atoi(straddr) == 0) {
			DHD_TRACE(("wl_android_rmc_set_leader: Set auto leader selection mode\n"));
			bzero(&rmc_entry, sizeof(wl_rmc_entry_t));
		} else {
...
wl_android_iolist_add(struct net_device *dev, struct list_head *head, struct io_cfg *config)
{
	...
	if (config->iovar) {
		...
		if (ret) {
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			DHD_ERROR(("wl_android_iolist_add: Failed to set (null) to %d\n",
						config->param));
#else // PVS V576
			DHD_ERROR(("wl_android_iolist_add: Failed to set %s to %d\n",
				config->iovar, config->param));
...
wl_android_set_miracast(struct net_device *dev, char *command)
{
	...
	DHD_TRACE(("wl_android_set_miracast: enter miracast mode %d\n", mode));
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	if (sscanf(extra, "%u", &period_msec) != 1) {
#else
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	DHD_ERROR(("wl_keep_alive_set: period_msec is %u\n", period_msec));
#else
...
wl_android_get_snr(struct net_device *dev, char *command, int total_len)
{
	...
	DHD_TRACE(("wl_android_get_snr: command result is %s\n", command));
	return bytes_written;
}
...
wl_handle_private_cmd(struct net_device *net, char *command, u32 cmd_len)
{
	...
	if (strnicmp(command, CMD_START, strlen(CMD_START)) == 0) {
		DHD_TRACE(("wl_handle_private_cmd, Received regular START command\n"));
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	bytes_written = (int)wl_cfg80211_set_transition_mode(net, transition_disabled);
