...
u32 wl_dbg_level = DEFAULT_WL_LOG_LEVEL;
#else
...
#if defined(WL_FILS) || defined(WL_OWE) // ATC fix unused warning
...
static s32
wl_cfg80211_set_txq_params(struct wiphy *wiphy, struct net_device *dev,
	struct ieee80211_txq_params *params);
#endif /* WL_CFG80211_SET_TX_PARAMS */
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#ifdef WL_SAE_EXTERNAL_SME
static s32 wl_notify_extauth_req_event(struct bcm_cfg80211 *cfg,
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data);
#endif /* WL_SAE_EXTERNAL_SME */
#endif
...
static void swap_key_to_BE(struct wl_wsec_key *key) //-V524
{
	...
	char const *name, u8 *mac_addr, s32 *ret_err)
{
	...
	*ret_err = BCME_OK;
	...
	if (timeout > 0 && !wl_get_p2p_status(cfg, IF_ADDING) && cfg->if_event_info.valid) {
		...
		WL_INFO((" virtual interface(%s) is "
...
wl_cfg80211_p2p_if_del(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	if (wl_get_mode_by_netdev(cfg, ndev) == WL_MODE_AP) {
		 ...
		 */
		if (err == 0) {
			WL_INFO(("Wait for Link Down event for GO !!!\n"));
		...
		if (wl_get_drv_status(cfg, DISCONNECTING, ndev)) {
			WL_INFO(("Wait for Link Down event for GC !\n"));
	...
	if (unlikely(err)) {
		...
		if (timeout > 0 && !wl_get_p2p_status(cfg, IF_DELETING) &&
			cfg->if_event_info.valid) {
			WL_INFO(("P2P IFDEL operation done\n"));
			err = BCME_OK;
		} else {
...
wl_get_vif_macaddr(struct bcm_cfg80211 *cfg, u16 wl_iftype, u8 *mac_addr)
{
	...
	struct ether_addr *p2p_dev_addr = cfg->p2p
...
wl_cfg80211_change_p2prole(struct wiphy *wiphy, struct net_device *ndev, enum nl80211_iftype type)
{
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	WL_INFORM_MEM(("Enter. current_role:%s new_role:%s\n",
		wl_iftype2str(ndev->ieee80211_ptr->iftype), wl_iftype2str(type)));
#else
	...
	(is_mp2p_supported <= 0) &&
#endif // endif
	 (cfg->p2p->p2p_go_count > 0)
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT // PVS V571
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		WL_INFORM_MEM(("IF_ADD ifidx:%d bssidx:%d role:%s\n",
			ifidx, bssidx, wl_if_role2str(role)));
#else
	...
	wl_iftype_t cfg_iftype, s32 del, u8 *addr)
{
	...
	if (ret == BCME_UNSUPPORTED) {
		...
		return ret;
	} else if ((ret == 0) && *((uint32 *)ioctl_buf) == WL_INTERFACE_CREATE_VER_3) { //-V1032
	...
	const char *name, bool rtnl_lock_reqd)
{
	...
	WL_INFORM_MEM(("Network Interface (%s) registered with host."
		" cfg_iftype:%d wl_role:%s mac:" MACDBG "\n",
		new_ndev->name, iface_type, wl_if_role2str(event->role),
...
#ifdef WL_SAE_EXTERNAL_SME
static s32
wl_cfg80211_event_external_auth(struct bcm_cfg80211 *cfg, struct net_device *ndev,
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	struct sk_buff *skb;
	gfp_t kflags;
	...
	WL_DBG(("ssid=%s. bssid=%02x:%02x:%02x:%02x:%02x:%02x. akm=%04x.\n",
		...
		ntohl(WLAN_AKM_SUITE_SAE)));

	if (nla_put_u32(skb, NL80211_ATTR_IFINDEX, ndev->ifindex) ||
	    nla_put_u32(skb, NL80211_ATTR_AKM_SUITES, ntohl(WLAN_AKM_SUITE_SAE)) ||
	    nla_put_u32(skb, NL80211_ATTR_EXTERNAL_AUTH_ACTION, NL80211_EXTERNAL_AUTH_START) ||
	    nla_put(skb, NL80211_ATTR_BSSID, ETH_ALEN, auth_req->peer_mac.octet) ||
	    nla_put(skb, NL80211_ATTR_SSID, auth_req->ssid_len, auth_req->ssid)) {
		WL_ERR(("add NL80211_ATTR_EXTERNAL_AUTH fail\n"));
		goto done;
	}
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	...
	err = wl_cfg80211_event_external_auth(cfg, ndev, cfgdev, e, data);
...
#ifdef WL_CFG80211_SET_TX_PARAMS
static s32 wl_cfg80211_set_txq_params(struct wiphy *wiphy,
		struct net_device *dev, struct ieee80211_txq_params *params)
{
	#define POW2MINUS1_ARRAY_CNT 16
	static uint32 pow2minus1_array[POW2MINUS1_ARRAY_CNT] = {
	...
	WL_INFO(("dev->ieee80211_ptr->iftype %d, "
		"params->ac %d, params->aifs %d, params->cwmin %d, "
		"params->cwmax %d, params->txop %d\n",
		dev->ieee80211_ptr->iftype, params->ac,
		params->aifs, params->cwmin, params->cwmax, params->txop));
	 ...
	 */
	err = wldev_iovar_getbuf(dev, iovar_name, NULL, 0,
			&acparam_cur, sizeof(acparam_cur), NULL);
	if (err < 0)
		return err;

	if (NL80211_AC_BE == params->ac)
		aci = AC_BE;
	else if (NL80211_AC_BK == params->ac)
		aci = AC_BK;
	else if (NL80211_AC_VI == params->ac)
		aci = AC_VI;
	else if (NL80211_AC_VO == params->ac)
		aci = AC_VO;
	else
		return -EINVAL;
	...
	if (params->aifs >= EDCF_AIFSN_MIN && params->aifs <= EDCF_AIFSN_MAX)
		acp->ACI =
				(acp->ACI & ~EDCF_AIFSN_MASK) |
				(params->aifs & EDCF_AIFSN_MASK);
	else {
		WL_ERR(("aifsn %d out of range (%d-%d)\n",
				params->aifs, EDCF_AIFSN_MIN, EDCF_AIFSN_MAX));
		return -EINVAL;
	}

	for (i = 0; i < POW2MINUS1_ARRAY_CNT; i++) {
		if (pow2minus1_array[i] == params->cwmax) {
			break;
		}
	}

	if (i < POW2MINUS1_ARRAY_CNT){
		ecwmax = i;
	}
	else {
		WL_ERR(("Maximum contention window "
	...
	if (ecwmax >= EDCF_ECW_MIN && ecwmax <= EDCF_ECW_MAX)
		...
		WL_ERR(("ecwmax %d out of range (%d-%d)\n",
		        params->cwmax, EDCF_ECW_MIN, EDCF_ECW_MAX));
		return -EINVAL;
	}

	for (i = 0; i < POW2MINUS1_ARRAY_CNT; i++) {
		if (pow2minus1_array[i] == params->cwmin) {
			break;
		}
	}
	...
	if (i < POW2MINUS1_ARRAY_CNT){
		...
		WL_ERR(("Minimum contention window "
			"[a value of the form 2^n-1 in the range [1..32767]\n"));
		return -EINVAL;
	}
	...
	if (ecwmin >= EDCF_ECW_MIN && ecwmin <= EDCF_ECW_MAX)
		...
		WL_ERR(("ecwmin %d out of range (%d-%d)\n",
		        params->cwmin, EDCF_ECW_MIN, EDCF_ECW_MAX));
		return -EINVAL;
	}

	WL_INFO(("aci %d, ecwmax %d, ecwmin %d\n", aci, ecwmax, ecwmin));

	if (params->txop >= EDCF_TXOP_MIN && params->txop <= EDCF_TXOP_MAX)
		acp->TXOP = params->txop;
	else {
		WL_ERR(("txop %d out of range (%d-%d)\n",
		        params->txop, EDCF_TXOP_MIN, EDCF_TXOP_MAX));
		return -EINVAL;
	}
	 ...
	 */
	err = wldev_iovar_setbuf(dev, iovar_name, acp,
			sizeof(edcf_acparam_t), smbuf, sizeof(smbuf), NULL);
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (dhdp->roam_env_detection) {
		...
		err = wldev_ioctl_set(dev, WLC_SET_ROAM_TRIGGER, roam_trigger,
			sizeof(roam_trigger));
		if (unlikely(err)) {
				WL_INFO((" failed to restore roam_trigger for auto env"
					" detection\n"));
			}
		}
	}
	...
	*/
	...
	err = wldev_iovar_setbuf_bsscfg(dev, "join", ext_join_params, join_params_size,
		cfg->ioctl_buf, WLC_IOCTL_MAXLEN, bssidx, &cfg->ioctl_buf_sync);
	if (cfg->rcc_enabled) {
		WL_INFO(("[%s] Connecting with " MACDBG " ssid \"%s\","
...
exit:
	...
	WL_INFO(("Clearing disconnect IEs \n"));
	...
	struct key_params *params)
{
	 ...
	 */
		if (params->cipher != WLAN_CIPHER_SUITE_PMK) //-V628
#endif /* defined(WLAN_CIPHER_SUITE_PMK) */
		return -EINVAL;
	}
...
#endif // endif
{
	...
	switch (wl_iftype) {
		...
		case WL_IF_TYPE_IBSS:
			if (cfg->roam_offload) {
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT // PVS V507
				struct ether_addr bssid;
#endif
				bzero(&bssid, sizeof(bssid));
...
#if ((LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 39)) || defined(WL_COMPAT_WIRELESS)) && \
	!defined(OEM_ANDROID) \
	&& defined(CONFIG_PM) // CONFIG_ATC_AOSP_ENHANCEMENT
	...
	*cookie = id;
	...
	if (wl_get_drv_status(cfg, SCANNING, ndev)) {
		...
		if (duration > LONG_LISTEN_TIME) {
			...
			_timer = &cfg->p2p->listen_timer; //-V595
...
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT // PVS V519
	*cookie = 0;
#endif
	...
	*cookie = id;
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			struct maclist *assoc_maclist = (struct maclist *)mac_buf; //-V1032
	...
	enum nl80211_channel_type channel_type)
{
	...
	WL_INFO(("netdev_ifidx(%d), chan_type(%d) target channel(%d) \n",
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#ifdef WL_SAE_EXTERNAL_SME
		if (((wpa_auth & WPA3_AUTH_SAE_PSK) == WPA3_AUTH_SAE_PSK) &&
			(((wsec & TKIP_ENABLED) == TKIP_ENABLED) ||
			((wsec & WEP_ENABLED) == WEP_ENABLED))) {
			return BCME_ERROR;
		}
#endif /* WL_SAE_EXTERNAL_SME */
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		WL_INFO(("WLC_SCB_AUTHORIZE sta_flags_mask not set\n"));
#else
...
#endif // endif
	{
		WL_INFO(("%s info->crypto.ciphers_pairwise[0] is not "
				"WLAN_CIPHER_SUITE_SMS4 \n", __FUNCTION__));
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#ifdef WL_CFG80211_SET_TX_PARAMS
	.set_txq_params = wl_cfg80211_set_txq_params,
#endif /* WL_CFG80211_SET_TX_PARAMS */
#endif
};
...
static s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, wl_bss_info_t *bi, bool update_ssid)
{
	...
	WL_DBG(("SSID \"%s\" rssi %d channel %d freq %d "
		"capability 0x04%x bssid %pM "
		"mgmt_type %d frame_len %d\n", tmp_buf,
		notif_bss_info->rssi, notif_bss_info->channel,
		freq, mgmt->u.beacon.capab_info, &bi->BSSID,
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		mgmt->u.probe_resp.timestamp = OSL_SYSUPTIME_US(); // uptime
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		if (!tlv_data) {
			WL_ERR(("%s: NULL tlv_data\n", __func__));
			goto free_evt_data;
		}
#endif

		/* ssid */
		tlv_data->tag = WIFI_TAG_SSID; //-V595
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (((event == WLC_E_ASSOC_IND) || (event == WLC_E_REASSOC_IND)) &&
		...
		WL_INFORM_MEM(("[%s] del sta event for "MACDBG "\n",
			ndev->name, MAC2STRDBG(e->addr.octet)));
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		// add for reason code when ap is disconnected from sta
		WL_INFORM_MEM(("reason:%d\n", reason));
		atc_cfg80211_del_sta(ndev, e->addr.octet, GFP_ATOMIC, reason);
#else // CONFIG_ATC_AOSP_ENHANCEMENT
		...
		if (wl_is_linkup(cfg, e, ndev)) {
			...
			if (!wl_get_drv_status(cfg, DISCONNECTING, ndev)) {
				...
				if (event == WLC_E_LINK &&
					...
					if (event == WLC_E_LINK) {
						if(wl_get_drv_status(cfg, CONNECTED, ndev)) {
							wl_clr_drv_status(cfg, CONNECTED, ndev);
							WL_INFORM_MEM(("wl_clr_drv_status CONNECTED\n"));
						}
					}
#endif
			...
			if (wl_is_linkdown(cfg, e)) {
				...
				if ((bssidx = wl_get_bssidx_by_wdev(cfg,
					...
					WL_INFO(("link down--clearing disconnect IEs\n"));
	...
	size_t *join_params_size, struct ieee80211_channel *chan)
{
	...
	if ((ch != 0) && (cfg && !cfg->rcc_enabled)) {
		...
		*join_params_size += WL_ASSOC_PARAMS_FIXED_SIZE +
		...
		join_params->params.chanspec_num = //-V570
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
			WL_DBG(("Event WLC_E_P2P_PROBREQ_MSG\n"));
#else // PVS V547
...
s32 wl_cfg80211_up(struct net_device *net)
{
	...
	if (init_roam_cache(cfg, ioctl_version) == 0) {
		...
		WL_INFORM_MEM(("Failed to enable RCC.\n"));
	}
#endif /* ROAM_CHANNEL_CACHE */
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#if !defined(DHD_SSW_P2P_USE_PERM_ADDRESS)
		memcpy(p2pdev_addr, &primary_mac, sizeof(*p2pdev_addr));
#endif /* !DHD_SSW_P2P_USE_PERM_ADDRESS */
#else // PVS V512
...
wl_cfg80211_dfs_ap_move(struct net_device *ndev, char *data, char *command, int total_len)
{
	...
	if (!*data) {
		...
		status = (struct wl_dfs_ap_move_status_v2 *)ioctl_buf; //-V1032
	...
	s32 bssidx, s32 pktflag, const u8 *vndr_ie, u32 vndr_ie_len)
{
	...
	if (vndr_ie_len > mgmt_ie_buf_len) {
		...
		if (vndr_ie && vndr_ie_len && curr_ie_buf) { //-V560
			ptr = curr_ie_buf;
		...
		*mgmt_ie_len = 0;
		...
		if (mgmt_ie_buf && parsed_ie_buf_len) { //-V560
			ptr = mgmt_ie_buf;
...
wl_cfg80211_ch_switch_notify(struct net_device *dev, uint16 chanspec, struct wiphy *wiphy)
{
	...
	WL_INFO(("Channel switch notification for freq: %d "
		"chanspec: 0x%x\n", freq, chanspec));
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT
	return;
#endif
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		if (BCME_UNSUPPORTED == err) {
			WL_INFO(("seting gtk_key_info BCME_UNSUPPORTED\n"));
		} else {
...
wl_wps_handle_connect_fail(struct net_device *ndev, u16 inst)
{
	...
	if (cur_state == WPS_STATE_REAUTH_WAIT) {
		cfg->wps_session[inst].state = WPS_STATE_DONE;
		WL_CFG_WPS_SYNC_UNLOCK(&cfg->wps_sync, flags);
		wl_clr_drv_status(cfg, CONNECTED, ndev); // might_sleep
		wps_done = true;
                ...
                const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_IND_DOS_STATUS)
	{
		...
		if ((status) == 1) {
			...
			WL_INFORM(("STATE_UNKNOWN %d\n", status));
		}

	}
	else {
		WL_INFORM(("unknown_event %u\n", event));
	}
