...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#define DEFAULT_WL_LOG_LEVEL (WL_DBG_ERR \
	| WL_DBG_P2P_ACTION | WL_DBG_INFO)
#endif
...
#ifdef DHD_LOG_DUMP
#define WL_ERR_KERN(args) do { \
	if (wl_dbg_level & WL_DBG_ERR) { \
		LOGE args; \
	} \
} while (0)

#define	WL_ERR_MEM(args) do { \
	if (wl_dbg_level & WL_DBG_ERR) { \
		DHD_LOG_DUMP_WRITE("[%s] "LOG_TAG"[E]%s,%d: ", \
				dhd_log_dump_get_timestamp(), __func__, __LINE__); \
		DHD_LOG_DUMP_WRITE args; \
	} \
} while (0)

#define	WL_ERR(args) do { \
	if (wl_dbg_level & WL_DBG_ERR) { \
		WL_ERR_KERN(args); \
		WL_ERR_MEM(args); \
	} \
...
#define	WL_ERR_EX(args)	do { \
	if (wl_dbg_level & WL_DBG_ERR) { \
		WL_ERR_KERN(args); \
		DHD_LOG_DUMP_WRITE_EX("[%s] "LOG_TAG"[E]%s,%d: ", \
				dhd_log_dump_get_timestamp(), __func__, __LINE__); \
		DHD_LOG_DUMP_WRITE_EX args; \
	} \
} while (0)

#define	WL_INFORM_MEM(args) do { \
	if (wl_dbg_level & WL_DBG_INFO) { \
		LOGI args; \
		DHD_LOG_DUMP_WRITE("[%s] "LOG_TAG"[I]%s,%d: ", \
				dhd_log_dump_get_timestamp(), __func__, __LINE__); \
		DHD_LOG_DUMP_WRITE args; \
	} \
} while (0)

#define	WL_MEM(args) do { \
	DHD_LOG_DUMP_WRITE("[%s] "LOG_TAG"[I]%s,%d: ", \
			dhd_log_dump_get_timestamp(), __func__, __LINE__); \
	DHD_LOG_DUMP_WRITE args;	\
} while (0)
...
#define	WL_INFORM(args) do { \
	if (wl_dbg_level & WL_DBG_INFO) { \
		LOGI args; \
		} \
} while (0)

#define WL_INFO(args) WL_INFORM(args)

#ifdef WL_SCAN
#undef WL_SCAN
#endif

#define	WL_SCAN(args) do { \
...
#define	WL_TRACE_HW4(args) do { \
	if (wl_dbg_level & WL_DBG_ERR) { \
		LOGT args; \
	} \
} while (0)

#else /* ! CUSTOMER_HW4_DEBUG */
#define	WL_TRACE_HW4			WL_TRACE
#endif /* CUSTOMER_HW4_DEBUG */

#if (WL_DBG_LEVEL > 0)

#define	WL_DBG(args) do { \
...
wl_delete_all_netinfo(struct bcm_cfg80211 *cfg)
{
	...
	BCM_LIST_FOR_EACH_ENTRY_SAFE(_net_info, next, &cfg->net_list, list) {
		...
		list_del(&_net_info->list);
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT // free at wl_free_wdev
		if (_net_info->wdev) {
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#define bcmcfg_to_wiphy(cfg) ((cfg && cfg->wdev) ? cfg->wdev->wiphy : NULL)
#define bcmcfg_to_prmry_ndev(cfg) ((cfg && cfg->wdev) ? cfg->wdev->netdev : NULL)
#define bcmcfg_to_prmry_wdev(cfg) (cfg ? cfg->wdev : NULL)
#define bcmcfg_to_p2p_wdev(cfg) (cfg ? cfg->p2p_wdev : NULL)
#define ndev_to_wl(n) (wdev_to_wl(n ? n->ieee80211_ptr : NULL))
#define ndev_to_wdev(ndev) (ndev ? ndev->ieee80211_ptr : NULL)
#define wdev_to_ndev(wdev) (wdev ? wdev->netdev : NULL)
#else
