...
#define	CFGP2P_ACTION(args) do { \
	if (wl_dbg_level & WL_DBG_P2P_ACTION) { \
		LOGA args; \
		DHD_LOG_DUMP_WRITE("[%s] "LOG_TAG"[A]%s,%d: ", \
				dhd_log_dump_get_timestamp(), __func__, __LINE__); \
		DHD_LOG_DUMP_WRITE args; \
	} \
} while (0)
...
#define	CFGP2P_DBG(args) do { \
	if (wl_dbg_level & WL_DBG_DBG) { \
		LOGD args; \
	} \
} while (0)

#define INIT_TIMER(timer, func, duration, extra_delay) do { \
