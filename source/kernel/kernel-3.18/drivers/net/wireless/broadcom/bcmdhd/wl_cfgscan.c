                      ...
                      uint8 ie_id, uint8 *data, uint8 data_len)
{
	...
	memcpy((uint8 *)&ie_setbuf
			->ie_buffer.ie_list[0].ie_data.data[0],
			data, data_len);
#else
	(void)memcpy_s((uchar *)&ie_setbuf
			->ie_buffer.ie_list[0].ie_data.data[0],
			sizeof(uint8), data, data_len);
#endif

#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	WL_DBG(("ie_setbuf id:[%x], data:[%x], len:[%x]\n", //-V557
		ie_setbuf->ie_buffer.ie_list[0].ie_data.id,
		ie_setbuf->ie_buffer.ie_list[0].ie_data.data[0],
		ie_setbuf->ie_buffer.ie_list[0].ie_data.len));
#endif
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
			...
			*/
			...
			if (wl_cfgp2p_is_p2p_specific_scan(cfg->scan_request)) {
				WL_INFO(("P2P assoc scan fast aborted.\n"));
		...
		if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
			...
			if (
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				p2p_is_scan(cfg) && cfg->scan_request &&
#else
				p2p_scan(cfg) && cfg->scan_request &&
#endif
	...
	struct cfg80211_scan_request *request)
{
	...
	if ((request->n_channels > 0) && chan_list) { //-V560
	...
	struct cfg80211_scan_request *request, uint16 action)
{
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#if 0
	// for wifi performance test
	if (wl_get_drv_status_all(cfg, CONNECTED)) {
		WL_INFO(("ignore scan during connected\n"));
		err = -EPERM;
		goto exit;
	}
#endif
	...
	if (!cfg->p2p_supported
		...
		MFREE(cfg->osh, params, params_size);
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	} else if (p2p_is_on(cfg) && p2p_is_scan(cfg)) { //-V560
#else
	} else if (p2p_is_on(cfg) && p2p_scan(cfg)) { //-V560
#endif
		/* P2P SCAN TRIGGER */
		s32 _freq = 0;
		n_nodfs = 0;
...
#endif /* P2P_SKIP_DFS */
				list = (wl_uint32_list_t *) chan_buf; //-V1032
	...
	if (request) {		/* scan bss */
		...
		if (p2p_ssid) {
			...
			if (cfg->p2p_supported) {
				...
				*/
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				if (p2p_is_scan(cfg) == false) { //-V547
#else
				if (p2p_scan(cfg) == false) { //-V547
#endif
	...
	bool aborted, bool fw_abort)
{
	...
	dhd_pub_t *dhdp = (dhd_pub_t *)(cfg->pub);
	u8 tmp_buf[8][IEEE80211_MAX_SSID_LEN + 1];
	...
	if (likely(cfg->scan_request)) {
		...
		WL_INFORM_MEM(("scanned AP num (%d)\n", bss_list->count));
		memset(&tmp_buf[0][0], 0, sizeof(tmp_buf));
		for_each_bss(bss_list, bi, i) {
			memcpy(&tmp_buf[j][0], bi->SSID, bi->SSID_len);
			tmp_buf[j][bi->SSID_len] = '\0';
			j++;
			if (j % 8 == 0) {
				j = 0;
				WL_INFORM_MEM(("%s; %s; %s; %s; %s; %s; %s; %s;\n",
					&tmp_buf[0][0], &tmp_buf[1][0], &tmp_buf[2][0],
					&tmp_buf[3][0], &tmp_buf[4][0], &tmp_buf[5][0],
					&tmp_buf[6][0], &tmp_buf[7][0]));
				memset(&tmp_buf[0][0], 0, sizeof(tmp_buf));
			}
		}
		if (j) {
	...
	const wl_event_msg_t *e, void *data)
{
...
#else // cypress patch
	/* scan abort process
	 * confirm to use non-null & correct ndev to read drv status.
	 * otherwise scan timeout happen and cause ioctl abnormal.
	 */
	ndev = cfgdev_to_wlc_ndev(cfgdev, cfg);
	/* P2P SCAN is coming from primary interface */
	if (wl_get_p2p_status(cfg, SCANNING)) {
	...
	/* Check drv status after ndev initialization */
	if (!ndev || !wl_get_drv_status(cfg, SCANNING, ndev)) {
		WL_ERR(("scan is not ready on ndev:%s, "
			"p2p scanning:%u, drv_status_all scanning:%u.\n",
			ndev ? ndev->name : "INVALID",
			(wl_get_p2p_status(cfg, SCANNING)),
			wl_get_drv_status_all(cfg, SCANNING)));
		/* Continue if scan timer pending to avoid timeout, Abnomal state!!! */
		if (!timer_pending(&cfg->scan_timeout)) {
			return err;
		}
	}
#endif
	mutex_lock(&cfg->scan_sync);
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT // cypress patch
	/* Clear p2p scanning status */
	if (p2p_is_on(cfg)) {
		wl_clr_p2p_status(cfg, SCANNING);
	}
#endif
                             ...
                             struct cfg80211_sched_scan_request *request)
{
	...
	if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID)) {
		...
		bzero(event_data, alloc_len); //-V512
...
static void wl_scan_timeout(unsigned long data)
{
	...
	/* CONFIG_ATC_AOSP_ENHANCEMENT
	 * for BCMSDIO, dhd_bus_mem_dump might sleep
	 * and can't call from timer.
	 */
#ifdef BCMPCIE
	if (dhdp->memdump_enabled) {
	...
	const wl_event_msg_t *e, void *data)
{
	...
	u32 tlv_len = 0;
	u32 payload_len = 0;
	u8 tmp_buf[DOT11_MAX_SSID_LEN + 1] = {0};
#else
	u32 alloc_len = 0, tlv_len = 0;
	u32 payload_len;
