...
#else // ! DHD_WAKE_STATUS
#ifndef CONFIG_ATC_AOSP_ENHANCEMENT // unused-function
	...
	struct wireless_dev *wdev, const void * data, int len)
{
	...
	if (cmd_data->status == WL_NAN_E_OK) { //-V547
...
static int
wl_cfgvendor_external_auth_handler(struct wiphy *wiphy,
       struct wireless_dev *wdev, const void  *data, int len)
{
	const struct nlattr *iter;
	int err = 0;
	...
	struct wl_auth_req_status auth_req;
	struct net_device *ndev = wdev->netdev;

    nla_for_each_attr(iter, data, len, rem) {
        type = nla_type(iter);
		WL_ERR(("external_auth: ittr type=%d. len=%d.\n", type, nla_len(iter)));

		if (type == NL80211_ATTR_STATUS_CODE) {
			if (nla_get_u16(iter) == WLAN_STATUS_SUCCESS) {
				auth_req.flags = WL_EXTAUTH_SUCCESS;
			} else {
				auth_req.flags = WL_EXTAUTH_FAIL;
			}

			WL_ERR(("NL80211_ATTR_STATUS_CODE: status=%d.\n", nla_get_u16(iter)));
		}
		if (type == NL80211_ATTR_SSID) {
			auth_req.ssid_len = nla_len(iter);
			memcpy(&auth_req.ssid, (u8 *)nla_data(iter), auth_req.ssid_len);
			WL_ERR(("NL80211_ATTR_SSID: ssid(%d)=%s.\n",
				nla_len(iter), (u8 *)nla_data(iter)));
		}
		if (type == NL80211_ATTR_PMKID) {
			memcpy(&auth_req.pmkid, (u8 *)nla_data(iter), nla_len(iter));
			WL_ERR(("NL80211_ATTR_PMKID: pmkid_len=%d.\n", nla_len(iter)));
		}
		if (type == NL80211_ATTR_BSSID) {
			u8 *ptr = nla_data(iter);
			memcpy(&auth_req.peer_mac.octet, (u8 *)nla_data(iter), ETHER_ADDR_LEN);
			WL_ERR(("NL80211_ATTR_BSSID: bssid=%02x:%02x:%02x:%02x:%02x:%02x.\n",
				*ptr, *(ptr+1), *(ptr+2), *(ptr+3), *(ptr+4), *(ptr+5)));
		}
  }

	err = wldev_iovar_setbuf(ndev, "auth_status", &auth_req, sizeof(auth_req),
			iovbuf, sizeof(iovbuf), NULL);
	if (unlikely(err))
	{
		WL_ERR(("set auth_status Error (%d)\n", err));
	}
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	radio = (wifi_radio_stat *)iovar_buf; //-V1032
	...
	if (total_len > WLC_IOCTL_MAXLEN) { //-V547
	...
	const void *data, int len)
{
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		switch (type) {
			...
			default:
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
				// not support MKEEP_ALIVE_ATTRIBUTE_ETHER_TYPE yet, ignore
				WL_INFO(("Unknown type: %d\n", type));
				break;
#else
				WL_ERR(("Unknown type: %d\n", type));
				ret = BCME_BADARG;
...
static const struct wiphy_vendor_command wl_vendor_cmds [] = {
...
#ifdef WL_SAE_EXTERNAL_SME
	{
		{
			.vendor_id = OUI_BRCM,
			.subcmd = BRCM_VENDOR_SUBCMD_EXTERNAL_AUTH
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = wl_cfgvendor_external_auth_handler
	},
#endif /* WL_SAE_EXTERNAL_SME */
#endif // CONFIG_ATC_AOSP_ENHANCEMENT
	{
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
#ifdef WL_SAE_EXTERNAL_SME
		{ OUI_BRCM, BRCM_VENDOR_EVENT_EXTERNAL_AUTH },
#endif
#endif
