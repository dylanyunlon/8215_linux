...
#define PFX_BTM                         "[CNN][STP_BTM-6630]"
...
#define STP_BTM_LOUD_FUNC(fmt, arg...) do { \
	if (gBtmDbgLevel >= STP_BTM_LOG_LOUD) \
		osal_dbg_print(PFX_BTM "[L]%s,%d: " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_BTM_DBG_FUNC(fmt, arg...) do { \
	if (gBtmDbgLevel >= STP_BTM_LOG_DBG) \
		osal_dbg_print(PFX_BTM "[D]%s,%d: " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_BTM_INFO_FUNC(fmt, arg...) do { \
	if (gBtmDbgLevel >= STP_BTM_LOG_INFO) \
		osal_print(PFX_BTM "[I]%s,%d: " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_BTM_WARN_FUNC(fmt, arg...) do { \
	if (gBtmDbgLevel >= STP_BTM_LOG_WARN) \
		osal_print(PFX_BTM "[W]%s,%d:WARN! " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_BTM_ERR_FUNC(fmt, arg...) do { \
	if (gBtmDbgLevel >= STP_BTM_LOG_ERR) \
		osal_print(PFX_BTM "[E]%s,%d:ERROR! " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_BTM_TRC_FUNC(f) do { \
	if (gBtmDbgLevel >= STP_BTM_LOG_DBG) \
		osal_dbg_print(PFX_BTM "[T]<%s> <%d>\n", __func__, __LINE__); \
} while (0)
...
static INT32 stp_btm_dump_send_retry_handler(PINT8 tmp, INT32 len)
{
	...
	while (rc) {
		...
		//if (nl_retry > 1000) { // 1000 is too big
		if (nl_retry > 3) {
			STP_BTM_ERR_FUNC("**dump send fails, and retry more than %d: %d.**\n",
                    nl_retry - 1, rc);
			return 2;
		}
...
#define NUM_FETCH_ENTRY 8
	...
	static STP_PACKET_T pkt;
	static UINT8 tmp[sizeof(pkt)];
	...
	do {
		index = 3;
		remain = stp_dbg_dmp_out_ex((PINT8)&pkt, &buf_len);
		if (buf_len > 0) {
			len = pkt.hdr.len;
			osal_memcpy(&tmp[index], &len, 2);
			index += 2;
			if (pkt.hdr.dbg_type == STP_DBG_FW_DMP) {
	...
	do {
		...
		if (buf_len > 0) {
			if (pkt.hdr.dbg_type == STP_DBG_FW_DMP) {
				...
				//retry = 0; // PVS: assigned values twice
			}
			retry = 0;
		} else {
			retry++;
...
static INT32 _stp_btm_handler(MTKSTP_BTM_T *stp_btm, P_STP_BTM_OP pStpOp)
{
	...
	case STP_OPID_BTM_RST:
		...
		STP_BTM_WARN_FUNC("[coredump] Flush dump data\n");
		wcn_core_dump_flush(0);
		STP_BTM_WARN_FUNC("[coredump] whole chip reset start!\n");
		if (stp_btm->wmt_notify) {
			stp_btm->wmt_notify(BTM_RST_OP);
			ret = 0;
		} else {
			STP_BTM_ERR_FUNC("[coredump] stp_btm->wmt_notify is NULL.");
			ret = -1;
		}
		STP_BTM_WARN_FUNC("[coredump] whole chip reset end!\n");
	...
	case STP_OPID_BTM_DBG_DUMP:
		...
		dump_sink = ((stp_btm->wmt_notify(BTM_DMP_OP) == MTK_WCN_BOOL_TRUE)
				? 0 : 1);
		dump_sink = ((stp_btm->wmt_notify(BTM_GET_AEE_SUPPORT_FLAG)
					== MTK_WCN_BOOL_TRUE)
				? 0 : 1);
	#else
		#ifdef CONFIG_WMT_USE_STP_DUMP
		...
		if (dump_sink == 1) {
			STP_BTM_ERR_FUNC("[coredump] dump to nl\n");
			ret = _stp_btm_put_dump_to_nl();
		} else if (dump_sink == 0) {
			STP_BTM_ERR_FUNC("[coredump] dump to aee\n");
			ret = _stp_btm_put_dump_to_aee();
		} else
			STP_BTM_ERR_FUNC("[coredump] unknown sink %d\n", dump_sink);
#else
	#ifdef CONFIG_WMT_USE_STP_DUMP
		STP_BTM_ERR_FUNC("[coredump] dump to nl\n");
		ret = _stp_btm_put_dump_to_nl();
	...
	case STP_OPID_BTM_DUMP_TIMEOUT:
		/* Flush dump data, and reset compressor */
		STP_BTM_INFO_FUNC("[coredump] Flush dump data\n");
		wcn_core_dump_flush(0);
...
static inline INT32 _stp_btm_notify_wmt_rst_wq(MTKSTP_BTM_T *stp_btm)
{
	...
	if (!pOp) {
		STP_BTM_ERR_FUNC("get_free_lxop fail\n");
		return -1;	/* break; */
	}
	pOp->op.opId = STP_OPID_BTM_RST;
	pOp->signal.timeoutValue = 0;

	STP_BTM_WARN_FUNC("OPID(%d) type(%d) start\n\n",
			pOp->op.opId, pOp->op.au4OpData[0]);

	bRet = _stp_btm_put_act_op(stp_btm, pOp);

	STP_BTM_WARN_FUNC("OPID(%d) type(%d) bRet(%d)\n\n",
...
static inline INT32 _stp_btm_do_fw_assert(MTKSTP_BTM_T *stp_btm)
{
	...
	/* send assert command */
	STP_BTM_WARN_FUNC("[coredump] trigger stp assert process\n");
