...
#define STP_DBG_FUNC(fmt, arg...)    do { \
	if (gStpDbgLvl >= STP_LOG_DBG) \
		osal_dbg_print(PFX "[D]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STP_INFO_FUNC(fmt, arg...)   do { \
	if (gStpDbgLvl >= STP_LOG_INFO) \
		osal_info_print(PFX "[I]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STP_WARN_FUNC(fmt, arg...)   do { \
	if (gStpDbgLvl >= STP_LOG_WARN) \
		osal_warn_print(PFX "[W]%s,%d:WARN! " fmt, __func__,__LINE__, ##arg); \
} while (0)

#define STP_ERR_FUNC(fmt, arg...)    do { \
	if (gStpDbgLvl >= STP_LOG_ERR) \
		osal_err_print(PFX "[E]%s,%d:ERROR! " fmt, __func__,__LINE__, ##arg); \
} while (0)

#define STP_TRC_FUNC(f)              do { \
	if (gStpDbgLvl >= STP_LOG_DBG) \
		osal_dbg_print(PFX "[T]<%s> <%d>\n", __func__, __LINE__); \
} while (0)

#define STP_TRACE_FUNC(fmt, arg...)  do { \
	if (gStpDbgLvl >= STP_LOG_DBG)\
		osal_dbg_print(PFX "[T]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)
...
typedef struct {
	...
	/*
	 * f_wmt_last_close_pre = 1;    // block /dev/stpX
	 * mtk_wcn_wmt_func_off         // turn off WIFI/BT/GPS/FM
	 * f_wmt_last_close = 1;        // block io
	 */
	UINT32 f_wmt_last_close_pre;
...
extern INT32 mtk_wcn_stp_set_wmt_last_close_pre(UINT32 value);
extern MTK_WCN_BOOL mtk_wcn_stp_is_wmt_last_close_pre(VOID);

extern INT32 mtk_wcn_stp_set_wmt_last_close(UINT32 value);
extern MTK_WCN_BOOL mtk_wcn_stp_is_wmt_last_close(VOID);

extern INT32 mtk_wcn_stp_wmt_evt_err_trg_assert(VOID);
extern INT32 mtk_wcn_stp_wmt_evt_err_trg_reset(VOID);
...
extern VOID mtk_wcn_stp_read_fw_cpupcrr_times(UINT32 times, UINT32 sleep);
