...
#define STP_PSM_LOUD_FUNC(fmt, arg...) do { \
	if (gPsmDbgLevel >= STP_PSM_LOG_LOUD)	\
		osal_dbg_print(PFX_PSM "[L]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STP_PSM_DBG_FUNC(fmt, arg...) do { \
	if (gPsmDbgLevel >= STP_PSM_LOG_DBG) \
		osal_dbg_print(PFX_PSM "[D]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STP_PSM_INFO_FUNC(fmt, arg...) do { \
	if (gPsmDbgLevel >= STP_PSM_LOG_INFO)	\
		osal_print(PFX_PSM "[I]%s,%d: " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_PSM_WARN_FUNC(fmt, arg...) do { \
	if (gPsmDbgLevel >= STP_PSM_LOG_WARN)	\
		osal_print(PFX_PSM "[W]%s,%d:WARN! " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_PSM_ERR_FUNC(fmt, arg...) do { \
	if (gPsmDbgLevel >= STP_PSM_LOG_ERR) \
		osal_print(PFX_PSM "[E]%s,%d:ERROR! " fmt, __func__, __LINE__, ##arg);	\
} while (0)

#define STP_PSM_TRC_FUNC(f) do { \
	if (gPsmDbgLevel >= STP_PSM_LOG_DBG) \
		osal_dbg_print(PFX_PSM "[T]<%s> <%d>\n", __func__, __LINE__); \
} while (0)
...
static INT32 _stp_psm_dump_active_q(P_OSAL_OP_Q pOpQ)
{
	...
	if (pOpQ == &stp_psm->rActiveOpQ) {
		...
		STP_PSM_DBG_FUNC("not active queue, dont dump\n");
...
static INT32 _stp_psm_wait_for_msg(PVOID pvData)
{
	...
	STP_PSM_DBG_FUNC("stp_psm->rActiveOpQ = %d\n",
...
static INT32 _stp_psm_wait_wmt_event(PVOID pvData)
{
	...
	STP_PSM_DBG_FUNC("stp_psm->flag= %ld\n", stp_psm->flag.data);
...
static inline INT32 _stp_psm_notify_stp(MTKSTP_PSM_T *stp_psm, const MTKSTP_PSM_ACTION_T action)
{
	...
	if (/* (_stp_psm_get_state(stp_psm) < STP_PSM_MAX_STATE) && */ // PVS: always true
...
static inline INT32 _stp_psm_notify_wmt(MTKSTP_PSM_T *stp_psm, const MTKSTP_PSM_ACTION_T action)
{
	...
	case ACT:
		...
		if (action == SLEEP) {
			...
			STP_PSM_ERR_FUNC("state = %d, flag = %lu\n",
					stp_psm->work_state, stp_psm->flag.data);
...
/* osal_unsleepable_lock_init(&stp_psm->flagSpinlock); */
	...
	osal_wake_lock_init(&stp_psm->wake_lock, "MT6630");
