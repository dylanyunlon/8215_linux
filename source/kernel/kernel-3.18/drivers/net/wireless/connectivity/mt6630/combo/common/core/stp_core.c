...
#define STP_WMT_LAST_CLOSE_PRE(x)       ((x).f_wmt_last_close_pre)
#define STP_SET_WMT_LAST_CLOSE_PRE(x, v) ((x).f_wmt_last_close_pre = (v))
...
VOID mtk_wcn_stp_ctx_lock_dump(VOID)
{
#if CFG_STP_CORE_CTX_SPIN_LOCK
#else
	struct mutex *lock = &stp_core_ctx.stp_mutex.lock;
	struct mutex_waiter *cur = NULL;
	struct task_struct *owner = lock->owner;
	unsigned long flags = 0;

	STP_INFO_FUNC("start\n");
	STP_INFO_FUNC("stp context lock owner %d:%s "
			"count %d is_locked %d\n",
			owner ? owner->pid : -1, owner ? owner->comm : "(null)",
			atomic_read(&lock->count), mutex_is_locked(lock));
	if (owner) {
		atc_combo_dump_single_task(owner);
	}
	STP_INFO_FUNC("dump wait_list:\n");
	spin_lock_irqsave(&lock->wait_lock, flags);
	list_for_each_entry(cur, &lock->wait_list, list) {
		atc_combo_dump_single_task(cur->task);
	}
...
static VOID stp_sdio_process_packet(VOID)
{
	static INT32 rx_fail_count[MTKSTP_MAX_TASK_NUM] = {0};
	MTK_WCN_BOOL is_function_active = 0;
	INT32 type = stp_core_ctx.parser.type;
	INT32 ret = -1;

	if ((type == BT_TASK_INDX) && STP_BT_STK_IS_BLUEZ(stp_core_ctx)) {
		INT32 b;
		/*Indicate packet to hci_stp */
		if (gStpDbgLvl >= STP_LOG_DBG) {
			stp_dump_data(stp_core_ctx.rx_buf,
					"indicate_to_bt_core", stp_core_ctx.rx_counter);
		}
		...
		is_function_active = ((*sys_check_function_status)(
					type, OP_FUNCTION_ACTIVE) == STATUS_FUNCTION_ACTIVE);

		/*check type and function if active? */
		if ((type < MTKSTP_MAX_TASK_NUM)
				&& (is_function_active == MTK_WCN_BOOL_TRUE)) {
			if (type == WMT_TASK_INDX) {
				ret = wmt_parser_data(stp_core_ctx.rx_buf,
						stp_core_ctx.rx_counter, type);
			} else {
				ret = stp_add_to_rx_queue(
							stp_core_ctx.rx_buf,
			...
			if (!ret) {
				if (rx_fail_count[type]) {
					STP_WARN_FUNC("func(%d) reset rx_fail_count(%d -> 0)\n",
							type, rx_fail_count[type]);
					rx_fail_count[type] = 0;
				}

			} else {

				rx_fail_count[type]++;
				STP_WARN_FUNC("func(%d) rx_fail_count(%d)\n",
						type, rx_fail_count[type]);

				if (rx_fail_count[type] <= (MTKSTP_RETRY_LIMIT / 2)) {

					STP_WARN_FUNC("call event_cb\n");
					sys_event_set(type);

				} else if (rx_fail_count[type] < MTKSTP_RETRY_LIMIT) {

					if (BT_TASK_INDX == type) { // BT not allow drop data
						STP_WARN_FUNC("call event_cb\n");
						sys_event_set(type);
					} else {
						STP_WARN_FUNC("clear rx queue\n");
						mtk_wcn_stp_flush_rx_queue(type);
					}

				} else { // merge from uart stp_process_packet()

					STP_ERR_FUNC("whole chip reset\n");
					...
					if (0 == mtk_wcn_stp_get_wmt_evt_err_trg_assert()) {
						...
						ret = mtk_wcn_stp_wmt_evt_err_trg_assert();
						if (ret) {
					...
					if (ret && (0 == mtk_wcn_stp_get_wmt_evt_err_trg_assert())) {
						osal_dbg_assert_aee("[MT6630]Ack Miss",
								"**STP Ack Miss**\n Ack Miss.\n");
						...
						if (STP_IS_ENABLE_RST(stp_core_ctx)) {
							...
							STP_INFO_FUNC("No to launch whole chip reset! "
...
#define MAX_CORE_DUMP_PACKETS_NUM    3000

UINT32 coredump_packets_counter = 0;
...
static VOID stp_sdio_trace32_dump(VOID)
{
	if (STP_IS_ENABLE_DBG(stp_core_ctx)
		...
		coredump_packets_counter++;
		if (coredump_packets_counter == 20) {
			STP_INFO_FUNC("only dump 20 packages from the begging\n");
		} else if (coredump_packets_counter < 20) {
			STP_INFO_FUNC("len %d type %d counter %d: %s\n",
					stp_core_ctx.rx_counter, stp_core_ctx.parser.type,
					coredump_packets_counter, stp_core_ctx.rx_buf);
		}

		if (coredump_packets_counter < MAX_CORE_DUMP_PACKETS_NUM) {
			if (0 != stp_core_ctx.rx_counter) {
				STP_SET_READY(stp_core_ctx, 0);
				mtk_wcn_stp_coredump_start_ctrl(1);
				stp_psm_set_sleep_disable(stp_core_ctx.psm);
			...
			if (0 == osal_strncmp("coredump end",
		...
		} else if (coredump_packets_counter == MAX_CORE_DUMP_PACKETS_NUM) {
				UINT8 *str = "coredump end\r\n";

				STP_ERR_FUNC("%d coredump packets received\n",
						coredump_packets_counter);
				STP_ERR_FUNC("issue fake coredump end\n");
				STP_SET_READY(stp_core_ctx, 0);
				mtk_wcn_stp_coredump_start_ctrl(1);
				stp_psm_set_sleep_disable(stp_core_ctx.psm);
				/*STP_DBG_FW_ASSERT */
				stp_dbg_log_pkt(g_mtkstp_dbg, STP_DBG_FW_DMP,
						STP_TASK_INDX, 0, 0, 0, 0, strlen(str) + 1, str);

				//coredump_packets_counter = 0;
				STP_ERR_FUNC("do whole chip reset\n");
				mtk_wcn_stp_set_wmt_evt_err_trg_assert(0);
				mtk_wcn_stp_coredump_start_ctrl(0);
				stp_psm_set_sleep_enable(stp_core_ctx.psm);
				stp_btm_reset_btm_wq(STP_BTM_CORE(stp_core_ctx));
		     ...
		     INT32 len)
{
	...
	if (STP_IS_ENABLE_DBG(stp_core_ctx)) {
		if (STP_IS_READY(stp_core_ctx) ||
				(!STP_IS_READY(stp_core_ctx) //-V560
				 && (type > WIFI_TASK_INDX)
...
VOID stp_do_tx_timeout(VOID)
{
	...
	//wmt_lib_read_fw_cpupcr_times(16, 10);
...
static MTK_WCN_BOOL stp_is_tx_res_available(UINT32 length)
{
	...
	if (roomLeft < length) {
		STP_ERR_FUNC("tx queue room shortage\n");
		return MTK_WCN_BOOL_FALSE;
	}
	return MTK_WCN_BOOL_TRUE;
}
	...
	while (parser_length < length) {
		...
		if (wmtsubtype == WMT_LTE_COEX_FLAG) {
			...
			if (fgRxOk == 0) {
				...
				STP_ERR_FUNC("fgRxOk %d length %u parser_length %u coredump %d\n",
						fgRxOk, length, parser_length,
...
#endif
	STP_INFO_FUNC("STP Not Ready, Dont do Sleep/Wakeup\n");
	return -1;
#endif
}
...
/* stp_dump_data(buffer, "rx queue", length); */
	...
	/* STP over SDIO */
		...
		while (i > 0) {
			...
			case MTKSTP_FW_MSG:
				if (coredump_packets_counter < 20) {
					STP_ERR_FUNC("Handle MTKSTP_FW_MSG\n");
				}
#if 1
				i_ret = -2;
				...
				if (STP_IS_READY(stp_core_ctx)) {
					/*
					if (mtk_wcn_stp_is_uart_mand_mode()
...
INT32 mtk_wcn_stp_dbg_dump_package(VOID)
{
	if (STP_NOT_ENABLE(stp_core_ctx))
		...
		if (0 == mtk_wcn_stp_coredump_start_get()) {
			...
			STP_INFO_FUNC("assert start flag is set, "
					"disable packet dump function\n");
		}
	}
...
static VOID stp_dmp_tx_rx(VOID);

INT32 mtk_wcn_stp_rx_timeout_dump(VOID)
{
	if (STP_NOT_ENABLE(stp_core_ctx))
		STP_INFO_FUNC("STP dbg mode is off\n");
	else {
...
INT32 mtk_wcn_stp_coredump_start_ctrl(UINT32 value)
{
	if (value != STP_FW_COREDUMP_FLAG(stp_core_ctx)) {
		STP_WARN_FUNC("[coredump] set f/w assert (%d)\n", value);
...
INT32 mtk_wcn_stp_set_wmt_last_close_pre(UINT32 value)
{
	STP_INFO_FUNC("set wmt_last_close_pre flag (%d)\n", value);
	STP_SET_WMT_LAST_CLOSE_PRE(stp_core_ctx, value);

	return 0;
}

MTK_WCN_BOOL mtk_wcn_stp_is_wmt_last_close_pre(VOID)
{
	return STP_WMT_LAST_CLOSE_PRE(stp_core_ctx)
...
{
	mtk_wcn_stp_set_wmt_last_close_pre(value);
	if (value) {
	...
	mtk_wcn_hif_sdio_set_wmt_last_close(value);

	return 0;
}

MTK_WCN_BOOL mtk_wcn_stp_is_wmt_last_close(VOID)
{
...
//	osal_ftrace_print("%s|S|T%d|L%d\n", __func__, type, length);
	...
	if (0 != STP_WMT_LAST_CLOSE(stp_core_ctx)) {
		STP_ERR_FUNC("WMT last close, should not have tx request!\n");
		//return length;
		return -EIO;
	}
	/* if(g_block_tx) */
...
*  VOID
*****************************************************************************/
#if 0 // Unknown Symbol g_stp_sdio_host_info
...
#else
static VOID (*mtk_wcn_stp_dump_fw_reg)(UINT32 times, UINT32 sleep);

VOID mtk_wcn_stp_dump_fw_reg_op_reg(
		VOID (*op)(UINT32 times, UINT32 sleep))
{
	mtk_wcn_stp_dump_fw_reg = op;
}
EXPORT_SYMBOL(mtk_wcn_stp_dump_fw_reg_op_reg);

VOID mtk_wcn_stp_dump_fw_reg_op_unreg(VOID)
{
	mtk_wcn_stp_dump_fw_reg = NULL;
}
EXPORT_SYMBOL(mtk_wcn_stp_dump_fw_reg_op_unreg);

VOID mtk_wcn_stp_read_fw_cpupcr_times(UINT32 times, UINT32 sleep)
{
#if WMT_SDIO_MODE
	if (mtk_wcn_stp_dump_fw_reg) {
		mtk_wcn_stp_dump_fw_reg(times, sleep);
	} else {
		STP_ERR_FUNC("op is not registered\n");
	}
#endif
}
#endif

static VOID (*_stp_dmp_tx_rx)(VOID);

VOID mtk_wcn_stp_dmp_tx_rx_op_reg(VOID (*op)(VOID))
{
	_stp_dmp_tx_rx = op;
}
EXPORT_SYMBOL(mtk_wcn_stp_dmp_tx_rx_op_reg);

VOID mtk_wcn_stp_dmp_tx_rx_op_unreg(VOID)
{
	_stp_dmp_tx_rx = NULL;
}
EXPORT_SYMBOL(mtk_wcn_stp_dmp_tx_rx_op_unreg);
...
INT32 mtk_wcn_stp_wmt_evt_err_trg_reset(VOID)
{
	if (STP_IS_ENABLE_RST(stp_core_ctx)) {
		return stp_btm_notify_wmt_rst_wq(
				STP_BTM_CORE(stp_core_ctx));
	} else {
		STP_INFO_FUNC("No to launch whole chip reset! "
				"for debugging purpose\n");
...
/*
 * Async trigger reset api, return non-zero on trigger reset failed.
 */
#ifndef MTK_WCN_WMT_STP_EXP_SYMBOL_ABSTRACT
INT32 mtk_wcn_stp_trg_reset(VOID)
#else
INT32 _mtk_wcn_stp_trg_reset(VOID)
#endif
{
	INT32 ret = -1;
	...
	if (STP_IS_ENABLE_RST(stp_core_ctx)) {
		...
		STP_ERR_FUNC("STP_NOT_ENABLE_RST\n");
		ret = -1;
	}
...
#ifndef MTK_WCN_WMT_STP_EXP_SYMBOL_ABSTRACT
EXPORT_SYMBOL(mtk_wcn_stp_trg_reset);
#endif

INT32 mtk_wcn_stp_coredump_timeout_handle(VOID)
{
	STP_WARN_FUNC("[coredump] start\n");
