...
static INT32 mtk_wcn_sys_event_set(UINT8 function_type)
{
	if ((function_type < MTKSTP_MAX_TASK_NUM) && (event_callback_tbl[function_type] != 0x0))
		...
		WMT_ERR_FUNC("[%s] STP set event fail. "
				"It seems the function is not active.\n", __func__);
	}
...
VOID mtk_wcn_stp_set_if_tx_type(ENUM_STP_TX_IF_TYPE stp_if_type)
{
	...
	WMT_INFO_FUNC("[%s] set STP_IF_TX to %s.\n", __func__,
			(STP_UART_IF_TX == stp_if_type) ? "UART"
			: ((STP_SDIO_IF_TX == stp_if_type) ? "SDIO" : "NULL"));
}
...
#endif
{
	if (STP_UART_IF_TX == stp_if)
		...
		WMT_ERR_FUNC("[%s] STP_IF_TX(%d) out of boundary.\n", __func__, stp_if);
		return -1;
	}
...
#endif
{
	if (type < MTKSTP_MAX_TASK_NUM) {
		...
		/*clear rx queue */
		WMT_INFO_FUNC("Flush type = %d Rx Queue\n", type);
...
INT32 stp_drv_init(VOID)
{
	...
	MTK_WCN_STP_EXP_CB_INFO stpExpCb = {
		...
		.stp_trg_reset_cb = _mtk_wcn_stp_trg_reset
	};
