...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_CORE-6630]"
...
OSAL_WAKE_LOCK ic6630_wakelock;
...
INT32 wmt_core_init(VOID)
{
	...
	osal_wake_lock_init(&ic6630_wakelock, "ic6630_wakelock");
...
INT32 wmt_core_deinit(VOID)
{
	...
	osal_memset(&gMtkWmtCtx, 0, osal_sizeof(gMtkWmtCtx));
	osal_wake_lock_deinit(&ic6630_wakelock);
	/* gMtkWmtCtx.p_ops is cleared to NULL */
...
INT32 wmt_core_rx(PUINT8 pBuf, UINT32 bufLen, PUINT32 readSize)
{
	...
	if (iRet) {
		...
		// -1: rx timeout, dump at wmt_ctrl_rx()
		if (-1 != iRet) {
...
/* iRet = (*kal_stp_tx)((PUINT8)&rWmtPktCmd, u4WmtCmdPduLen, &u4WrittenSize); */
		...
		if (iRet) {
			...
			// already dump at wmt_core_rx
			// mtk_wcn_stp_dbg_dump_package();
			//wmt_core_ctrl(WMT_CTRL_EVT_ERR_TRG_ASSERT, &ctrlPa1, &ctrlPa2);
			wmt_core_ctrl(WMT_CTRL_EVT_ERR_TRG_RESET, &ctrlPa1, &ctrlPa2);
			break;
		}
...
INT32 wmt_core_opid_handler(P_WMT_OP pWmtOp)
{
	...
	opId = pWmtOp->opId;
	WMT_DBG_FUNC("wmt_core_opid_handler\n");
...
static INT32 wmt_core_stp_init(VOID)
{
	...
	WMT_INFO_FUNC("wmt_core_stp_init!\n");
	wmt_conf_read_file();
...
{
	WMT_INFO_FUNC(
			"[%s] status(b:%d f:%d g:%d w:%d lpbk:%d coredump:%d "
			"wmt:%d ant:%d sd1:%d sd2:%d stp:%d)\n",
...
INT32 wmt_core_is_stp_mode_set(VOID)
{
	return !!(gMtkWmtCtx.wmtInfoBit & WMT_OP_HIF_BIT);
}
...
static INT32 opfunc_pwr_on(P_WMT_OP pWmtOp)
{
	...
	if (gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_WMT] != DRV_STS_POWER_OFF) {
		WMT_WARN_FUNC("WMT-CORE: already on, WMT DRV_STS_[0x%x]\n",
			     gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_WMT]);
		osal_assert(0);
...
#ifdef COM_DRV_SIMULATE_PWR_ON_TMO
	/* simulate bug error */
	static INT32 tmp = 3;	/* must be less than wifi native retry times */
	if (tmp-- > 0) {
		WMT_WARN_FUNC("[COM_driver-debug]simulate power on retry(%d)\n", 3-tmp);
		msleep(6000);	/* should not less than 4s */
		iRet = -10;
	}
#endif
	...
	if (iRet) {
		...
		if (iRet)
			WMT_ERR_FUNC("WMT-CORE: opfunc_pwr_off fail during pwr_on error\n");
...
static INT32 opfunc_pwr_off(P_WMT_OP pWmtOp)
{
	...
	if (iRet)
		...
		WMT_INFO_FUNC("HW_PWR_OFF ok\n");

	/*anyway, set to POWER_OFF state */
	gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_WMT] = DRV_STS_POWER_OFF;
	osal_wake_unlock(&ic6630_wakelock);
	WMT_INFO_FUNC("HW_PWR_OFF ok wakelock release\n");
...
static INT32 opfunc_func_on(P_WMT_OP pWmtOp)
{
	...
	WMT_INFO_FUNC("[BE FUNC ON] func(%d)\n", drvType);
	wmt_core_dump_func_state("BE FUNC ON");

	iRet = wmt_lib_func_onoff_lock_aquire();
	if (iRet) {
		WMT_ERR_FUNC("lock func_onoff_lock failed, iRet=%d\n", iRet);
		return iRet;
	}
	...
	if (WMTDRV_TYPE_COREDUMP < drvType) {
		...
		osal_assert(0);
		wmt_lib_func_onoff_lock_release();
		return -1;
	}

	/* Check abnormal state */
	if ((DRV_STS_POWER_OFF > gMtkWmtCtx.eDrvStatus[drvType]) //-V1016 always false
	...
	if (iRet) {
		...
		if ((DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_BT]) &&
			...
			WMT_WARN_FUNC("WMT-CORE:Func(%d) [POWER_OFF] and power down chip\n",
				      drvType);
	...
	WMT_INFO_FUNC("[AF FUNC ON] func(%d) ok\n", drvType);
...
static INT32 opfunc_func_off(P_WMT_OP pWmtOp)
{
	...
	WMT_INFO_FUNC("[BE FUNC OFF] func(%d)\n", drvType);
	wmt_core_dump_func_state("BE FUNC OFF");
	iRet = wmt_lib_func_onoff_lock_aquire();
	if (iRet) {
		WMT_ERR_FUNC("lock func_onoff_lock failed, iRet=%d\n", iRet);
		return iRet;
	...
	if ((DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_BT]) &&
	    ...
	    (DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_ANT]) &&
	    (DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_COREDUMP])) {
		WMT_INFO_FUNC("WMT-CORE:Func(%d) [POWER_OFF] and power down chip\n", drvType);
	...
	WMT_INFO_FUNC("[AF FUNC OFF] func(%d) iRet(%d)\n", drvType, iRet);

	return iRet;
}

MTK_WCN_BOOL wmt_core_is_func_on(UINT32 type)
{
	return (DRV_STS_FUNC_ON == gMtkWmtCtx.eDrvStatus[type])
		? MTK_WCN_BOOL_TRUE : MTK_WCN_BOOL_FALSE;
}
EXPORT_SYMBOL(wmt_core_is_func_on);

INT32 wmt_core_pwr_off_if_func_off(INT32 coredump_off)
{
	INT32 iRet = 0;
	...
	if (coredump_off)
		wmt_core_set_coredump_state(DRV_STS_POWER_OFF);
	...
	if ((DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_BT]) &&
			...
			(DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_ANT]) &&
			(DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_COREDUMP])) {
		WMT_INFO_FUNC("WMT-CORE: power down chip due to all func off\n");
		iRet = opfunc_pwr_off(NULL);
		if (iRet) {
			WMT_ERR_FUNC("WMT-CORE: opfunc_pwr_off fail(%d)\n", iRet);
		}
	}
...
static INT32 opfunc_cmd_test(P_WMT_OP pWmtOp)
{
	...
	if (cmdNo == 0) {
		...
		if ((cmdNoPa >= 0x0) && (cmdNoPa <= 0xf)) {
			...
			if (tstCmdSz > 5) //-V547 always true
				tstCmd[5] = cmdNoPa;
			...
			} else if (cmdNoPa >= 0x6 /* && cmdNoPa <= 0xf */) { //-V547 always true
	...
	if (cmdNo == 2) {
		...
		if (pRes != NULL && resBufRoom > 0 && tstEvtSz > 5) {
			pWmtOp->au4OpData[3] =
...
static INT32 opfunc_hw_rst(P_WMT_OP pWmtOp)
{
	...
	if (DRV_STS_POWER_OFF == gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_WMT]) {
		...
		if (iRet) {
			WMT_ERR_FUNC("WMT-CORE: WMT_CTRL_HW_PWR_ON fail iRet(%d)\n", iRet);
			/* If we fail at sdio initial, then we can try to trigger reset
			 * to recover this issue. So just go ahead to complete the chip
			 * reset flow.
			 */
			//wmt_lib_func_onoff_lock_release();
			//return -1;
	...
	ctrlPa1 = 0;
	ctrlPa2 = 0;
	iRet2 = wmt_core_ctrl(WMT_CTRL_STP_CLOSE, &ctrlPa1, &ctrlPa2);
	if (iRet2) {
...
MTK_WCN_BOOL wmt_core_trigger_stp_assert(VOID)
{
	...
	if ((NULL != pctx->p_ic_ops) && (NULL != pctx->p_ic_ops->trigger_stp_assert)) {
		...
		// for MTKSTP_FW_MSG missing and didn't start dmp_timer case
		core_dump_timer_start();
