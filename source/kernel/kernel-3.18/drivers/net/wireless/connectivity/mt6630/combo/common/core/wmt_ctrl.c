...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_CTRL-6630]"
...
static INT32 wmt_ctrl_evt_err_trg_reset(P_WMT_CTRL_DATA pWmtCtrlData);
...
static const WMT_CTRL_FUNC wmt_ctrl_func[] = {
	...
	[WMT_CTRL_EVT_ERR_TRG_RESET] = wmt_ctrl_evt_err_trg_reset,
...
INT32 wmt_ctrl(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	if ((WMT_CTRL_MAX <= ctrlId))
		/* || (NULL == pWmtCtrlData) PVS: always false */
...
INT32 wmt_ctrl_rx(P_WMT_CTRL_DATA pWmtCtrlData /*UINT8 *pBuff, UINT32 buffLen, UINT32 *readSize */)
{
	...
	while (readLen == 0) {	/* got nothing, wait for STP's signal */
		...
		if (0 == waitRet) {
			...
			mtk_wcn_hif_sdio_dump_irq_state();
			wmt_lib_read_fw_cpupcr();
			mtk_wcn_stp_rx_timeout_dump();
			return -1;
		} else if (waitRet < 0) {
...
INT32 wmt_ctrl_hw_pwr_off(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	WMT_DBG_FUNC("wmt_ctrl_hw_pwr_off\n");
...
INT32 wmt_ctrl_hw_pwr_on(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	WMT_DBG_FUNC("wmt_ctrl_hw_pwr_on\n");
	if (osal_test_and_set_bit(WMT_STAT_PWR, &pDev->state)) {
		...
		if (iret) osal_test_and_clear_bit(WMT_STAT_PWR, &pDev->state);
	}
...
/* we can't know it's sdio fail, wmt sw timeout, or other fail
 * from wmt_lib_cmb_rst -> wmt_lib_hw_rst return false.
 * And wmt_lib_hw_rst and wmt_ctrl_hw_rst is different thread.
 * So we set this atomic to indicate hw reset fail.
 */
atomic_t wmt_ctrl_hw_rst_fail = ATOMIC_INIT(0);

INT32 wmt_ctrl_hw_rst(P_WMT_CTRL_DATA pWmtCtrlData)
{
	INT32 ret = 0;

	WMT_DBG_FUNC("wmt_ctrl_hw_rst\n");

	ret = wmt_plat_pwr_ctrl(FUNC_RST);
	atomic_set(&wmt_ctrl_hw_rst_fail, !!ret);
...
{
	WMT_DBG_FUNC("wmt_ctrl_hw_state_show\n");
...
INT32 wmt_ctrl_host_baudrate_set(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	WMT_INFO_FUNC("baud(%d), flowctrl(%d)\n", u4Baudrate, u4FlowCtrl);

	if (osal_test_bit(WMT_STAT_STP_OPEN, &gDevWmt.state)) {
#if 0
		osal_snprintf(cmdStr, NAME_MAX, "baud_%d_%d", u4Baudrate, u4FlowCtrl);
#else // launcher will set u4FlowCtrl by stpUartConfig->fc
		osal_snprintf(cmdStr, NAME_MAX, "baud_%d_0", u4Baudrate);
#endif
		...
		if (osal_test_bit(statBit, &pDev->state)) {
			...
			while (retry-- > 0 && iRet != 0) {
				...
				if (HIF_SDIO_ERR_NOT_PROBED == iRet) {
					...
					continue;
				} else if (HIF_SDIO_ERR_ALRDY_ON == iRet) {
...
static INT32 wmt_ctrl_evt_err_trg_assert(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	WMT_ERR_FUNC("wmt-ctrl:drv_type(%d),reason(%d)\n", drv_type, reason);
...
static INT32 wmt_ctrl_evt_err_trg_reset(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	return mtk_wcn_stp_wmt_evt_err_trg_reset();
}
