...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_DEV-6630]"
...
//#endif
static INT32 wmt_dbg_block_func_ctrl(INT32 par1, INT32 par2, INT32 par3);
...
INT32 wmt_dbg_reg_read(INT32 par1, INT32 par2, INT32 par3)
{
	...
	WMT_WARN_FUNC("read combo chip register (0x%08x) with mask (0x%08x) %s, value = 0x%08x\n",
...
static MTK_WCN_BOOL func_ctrl_block_check = MTK_WCN_BOOL_FALSE;
static ENUM_WMTDRV_TYPE_T func_ctrl_block_type = WMTDRV_TYPE_BT;
static MTK_WCN_BOOL func_ctrl_block_onoff = MTK_WCN_BOOL_FALSE;
// { PKT_TYPE_CMD, OPCODE_FUNC_CTRL, LEN_LSB, LEN_MSB,
//   ENUM_WMTDRV_TYPE_T, ON }
static UINT8 func_ctrl_block_cmd[] = { 0x01, 0x06, 0x02, 0x00, 0x00, 0x00 };

MTK_WCN_BOOL wmt_dbg_is_func_ctrl_block(PUINT8 buf, UINT32 len)
{
	UINT32 i = 0;

	if (MTK_WCN_BOOL_FALSE == func_ctrl_block_check) {
		return MTK_WCN_BOOL_FALSE;
	}

	for (i = 0; i < len; i++) {
		if (buf[i] == func_ctrl_block_cmd[0]
				&& !memcmp(buf + i, func_ctrl_block_cmd,
					sizeof(func_ctrl_block_cmd))) {
			func_ctrl_block_check = MTK_WCN_BOOL_FALSE;
			WMT_WARN_FUNC("block %s %s !!!\n",
					gStpDbgType[func_ctrl_block_type],
					MTK_WCN_BOOL_TRUE == func_ctrl_block_onoff
						? "on" : "off");
...
EXPORT_SYMBOL(wmt_dbg_is_func_ctrl_block);

static INT32 wmt_dbg_block_func_ctrl(INT32 par1, INT32 par2, INT32 par3)
{
	func_ctrl_block_type = par2;
	func_ctrl_block_onoff = par3;
	func_ctrl_block_cmd[4] = func_ctrl_block_type;
	func_ctrl_block_cmd[5] = func_ctrl_block_onoff;
	func_ctrl_block_check = MTK_WCN_BOOL_TRUE;

	WMT_INFO_FUNC("to block %s %s ...\n",
			gStpDbgType[func_ctrl_block_type],
			MTK_WCN_BOOL_TRUE == func_ctrl_block_onoff ? "on" : "off");
