...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_EXP-6630]"
...
#if WMT_SDIO_MODE
		/*don't block system server/Init/Netd from longer than 5s,
		 * in case of ANR happens */
		pSignal->timeoutValue = 4000;
#else
		pSignal->timeoutValue = 4800;
#endif
	} else {
		pSignal->timeoutValue = (WMT_OPID_FUNC_ON == pOp->op.opId)
			? MAX_FUNC_ON_TIME : MAX_FUNC_OFF_TIME;
	}
...
#endif
{
	...
	while (!wmt_core_is_stp_mode_set() && waitCounter) {
		WMT_WARN_FUNC("WMT_OP_HIF_BIT not set, waiting, waitCounter:%d\n",
				waitCounter);
		osal_sleep_ms(1000);
		waitCounter--;
	}
	if (!wmt_core_is_stp_mode_set()) {
		WMT_ERR_FUNC("WMT_OP_HIF_BIT not set, type(%d) func on fail\n", type);
		return MTK_WCN_BOOL_FALSE;
	}
	...
	if (IS_ERR(fileAddr)) {
		...
		UINT32 wifi_efuse = 0x0;
		UINT32 wifi_ac_enable = 0x0;
		UINT32 wifi_D02H = 0x0;
		wmt_lib_efuse_rw(0,0x00000004,&wifi_efuse,0xffffffff);
		wmt_lib_reg_rw(0, 0x81070024, &wifi_ac_enable, 0xffffffff);
		wmt_lib_reg_rw(0, 0x81070020, &wifi_D02H, 0xffffffff);
		WMT_INFO_FUNC("read reg[0x81070024],value[0x%08x]\n",wifi_ac_enable);
		WMT_INFO_FUNC("read reg[0x81070020],value[0x%08x]\n",wifi_D02H);
		if (0 != wifi_ac_enable) {
			wifi_ac_enable = 5;
			WMT_INFO_FUNC("is not ac,need low clock!\n");
			fileAddr = filp_open("/data/misc/wifi/wificlock", O_CREAT|O_RDWR, S_IRWXU);
			if (IS_ERR(fileAddr)) {
				WMT_WARN_FUNC("create wificlock file fail\n");
			} else {
				WMT_INFO_FUNC( "create wificlock file Success\n");
				fileAddr->f_pos = 0;
				fileAddr->f_op->write(fileAddr, &wifi_ac_enable, sizeof(UINT32), &fileAddr->f_pos);
			}
			WMT_INFO_FUNC("is not ac,low clock is ready!\n");
		} else {
			WMT_INFO_FUNC("is ac,not need low clock!\n");
		}
	} else {
...
#endif
{
	...
	pSignal->timeoutValue = MAX_EACH_WMT_CMD;
	/*parameter fill */
	if ((WMTDSNS_FM_DISABLE <= eType) //-V1016
			/* && (WMTDSNS_FM_GPS_ENABLE >= eType) PVS: always true */) {
...
#endif
{
	...
	if (mtk_wcn_stp_is_wmt_last_close()) {
		WMT_ERR_FUNC("[coredump] type(%d) reason(%u)\n", type, reason);
		WMT_ERR_FUNC("[coredump] wmt closed, block assert\n");
		dump_stack();
		return MTK_WCN_BOOL_FALSE;
	}
