...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_IC-6630]"
...
static UINT8 WMT_SET_DAI_MODE_REG_CMD[] = { 0x01, 0x08, 0x28, 0x00	/*length */
	    ...
	    , 0x00, 0x00, 0x20, 0x22	/*value:0x22200000 for pcm data*/
	    , 0x00, 0x00, 0xf0, 0xff	/*mask:0xfff00000 */
	    , 0x70, 0x50, 0x02, 0x80	/*addr:0x80025070 */
	    , 0x02, 0x00, 0x00, 0x00	/*value:0x00000002 for pcm data*/
	    , 0x0f, 0x00, 0x00, 0x00	/*mask:0x0000000f */
	    , 0x00, 0x53, 0x02, 0x80	/*addr:0x80025300 */
	    , 0x00, 0x00, 0x00, 0x00	/*value:0x00000000 for pcm data*/
...
static INT32 mt6630_sw_init(P_WMT_HIF_CONF pWmtHifConf)
{
	...
	if (WMT_HIF_UART == pWmtHifConf->hifType) {
		...
		if (WMT_UART_MTK_SW_FC == pWmtHifConf->uartFcCtrl) {
			...
			WMT_SET_BAUD_CMD_X[8] = (UINT8) 0x40;	/* MTK SW flow control */
			WMT_QUERY_BAUD_EVT_X[9] = (UINT8) 0x40;	/* MTK SW flow control */
		...
		ctrlPa2 = pWmtHifConf->uartFcCtrl;
		WMT_INFO_FUNC("change baudrate(%lu) uartFcCtrl(%lu)\n", ctrlPa1, ctrlPa2);
...
INT32 mt6630_co_clock_ctrl(WMT_CO_CLOCK on)
{
	...
	if ((WMT_CO_CLOCK_DIS <= on) && (WMT_CO_CLOCK_MAX > on)) { //-V1016
		gCoClockEn = on;
	} else {
...
static INT32 mt6630_ver_check(VOID)
{
	...
	ctrlPa2 = (ULONG)(
			((UINT32)(p_info->eWmtHwVer & 0x0000FFFF) << 16)
			| (fw_ver & 0x0000FFFF));
...
static const WMT_IC_INFO_S *mt6630_find_wmt_ic_info(const UINT32 hw_ver)
{
	...
	WMT_INFO_FUNC("find no ic info for (0x%x) by full match!"
			" try major num match!\n", hw_ver);
...
static INT32 mt6630_crystal_triming_set(VOID)
{
	...
	ctrlData.au4CtrlData[0] = (size_t) "/etc/nvram/WIFI";
