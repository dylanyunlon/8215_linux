...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_LIB-6630]"
...
OSAL_SLEEPABLE_LOCK gOsSLock;
OSAL_WAKE_LOCK wmtWakeLock;
#endif
...
********************************************************************************
*/
MTK_WCN_BOOL wmt_lib_is_rst_on(VOID)
{
	if (osal_test_bit(WMT_STAT_RST_ON, &gDevWmt.state)) {
		return MTK_WCN_BOOL_TRUE;
	} else {
		return MTK_WCN_BOOL_FALSE;
	}
}
EXPORT_SYMBOL(wmt_lib_is_rst_on);

INT32 wmt_lib_func_onoff_lock_aquire(VOID)
{
	return osal_lock_sleepable_lock(&gDevWmt.func_onoff_lock);
}

VOID wmt_lib_func_onoff_lock_release(VOID)
{
	osal_unlock_sleepable_lock(&gDevWmt.func_onoff_lock);
}
...
INT32 wmt_lib_init(VOID)
{
	...
	#ifdef CFG_WMT_WAKELOCK_SUPPORT
	osal_wake_lock_init(&wmtWakeLock, "wmtFuncCtrl");
	...
	WMT_DBG_FUNC("Create mtk_wmtd thread");
	...
	osal_sleepable_lock_init(&pDevWmt->func_onoff_lock);
	...
	if (0 != gDevWmt.rWmtGenConf.cfgExist) {
		...
		WMT_DBG_FUNC("initialize platform resources\n");
		...
		WMT_INFO_FUNC("Will call wmt_plat_init(NULL)\n");
...
INT32 wmt_lib_deinit(VOID)
{
	...
	osal_wake_lock_deinit(&wmtWakeLock);
	WMT_DBG_FUNC("destroy wmtWakeLock\n");
	#endif
	WMT_ERR_FUNC("wmt_lib_deinit\n");
	/* stop->deinit->destroy */
	...
	osal_sleepable_lock_deinit(&pDevWmt->func_onoff_lock);
...
INT32 wmt_lib_set_hif(unsigned long hifconf)
{
	...
	if (STP_SDIO == val) {
		...
		stpMode = val & 0x3;
		uartFcCtrl = (val >> 2) & 0x3;
	}
	if (STP_UART_FULL == stpMode) {
		pHif->hifType = WMT_HIF_UART;
		pHif->uartFcCtrl = uartFcCtrl;
		val = (hifconf >> 8);
...
static P_OSAL_OP wmt_lib_peek_op(P_OSAL_OP_Q pOpQ)
{
	P_OSAL_OP pOp;
	INT32 iRet;
...
P_OSAL_OP wmt_lib_get_free_op(VOID)
{
	...
	P_OSAL_OP pOp = wmt_lib_get_op(&gDevWmt.rFreeOpQ);
...
static MTK_WCN_BOOL wmt_lib_is_urgent_op(P_OSAL_OP pOp)
{
	if (!pOp) {
		return MTK_WCN_BOOL_FALSE;
	}
	if (WMT_OPID_HW_RST == pOp->op.opId
			|| WMT_OPID_SW_RST == pOp->op.opId
			|| WMT_OPID_TRIGGER_STP_ASSERT == pOp->op.opId
			|| WMT_OPID_CMD_TEST == pOp->op.opId) {
		return MTK_WCN_BOOL_TRUE;
	} else {
		return MTK_WCN_BOOL_FALSE;
	}
...
VOID wmt_lib_dump_all_thread(VOID);
...
MTK_WCN_BOOL wmt_lib_put_act_op(P_OSAL_OP pOp)
{
	...
	do {
		if (/*!pWmtDev || PVS: always false */ !pOp) {
		...
		/* put to active Q */
		pPeekOp = wmt_lib_peek_op(&pWmtDev->rActiveOpQ);
		if (wmt_lib_is_urgent_op(pOp)) {
			if (!pPeekOp) {
				WMT_WARN_FUNC("enqueue urgent opId %d\n", pOp->op.opId);
			...
			} else if (wmt_lib_is_urgent_op(pPeekOp)) {
				WMT_WARN_FUNC("urgent opId %d in rActiveOpQ, enqueue urgent opId %d\n",
						pPeekOp->op.opId, pOp->op.opId);
				bRet = wmt_lib_put_op(&pWmtDev->rActiveOpQ, pOp);
				if (MTK_WCN_BOOL_FALSE == bRet) {
					WMT_WARN_FUNC("put to active queue fail\n");
					bCleanup = MTK_WCN_BOOL_TRUE;
				...
				// block normal opId before enqueue urgent opId for quick assert
				while (pPeekOp) {
					pPeekOp = wmt_lib_get_op(&pWmtDev->rActiveOpQ);
					if (!pPeekOp) {
						break;
					}
					WMT_WARN_FUNC("block normal opId %d in rActiveOpQ "
							"before enqueue urgent opId %d\n",
							pPeekOp->op.opId, pOp->op.opId);
					if (osal_op_is_wait_for_signal(pPeekOp)) {
						osal_op_raise_signal(pPeekOp, -1);
					} else {
						/* put Op back to freeQ */
						wmt_lib_put_op(&pWmtDev->rFreeOpQ, pPeekOp);
					}
					pPeekOp = wmt_lib_peek_op(&pWmtDev->rActiveOpQ);
				}
			}
		} else {
			if (pPeekOp && wmt_lib_is_urgent_op(pPeekOp)) {
				bCleanup = MTK_WCN_BOOL_TRUE;
				WMT_WARN_FUNC("urgent opId %d in rActiveOpQ, block opId %d\n",
						pPeekOp->op.opId, pOp->op.opId);
				break;
			} else {
				// enqueue normal opId after normal opId
				bRet = wmt_lib_put_op(&pWmtDev->rActiveOpQ, pOp);
				if (MTK_WCN_BOOL_FALSE == bRet) {
					WMT_WARN_FUNC("put to active queue fail\n");
					bCleanup = MTK_WCN_BOOL_TRUE;
...
//			osal_thread_show_stack(pThread);
			wmt_lib_dump_all_thread();
			mtk_wcn_stp_ctx_lock_dump();
		} else {
			if (pOp->result)
...
MTK_WCN_BOOL wmt_cdev_rstmsg_snd(ENUM_WMTRSTMSG_TYPE_T msg)
{
	...
	for (i = 0; i <= WMTDRV_TYPE_ANT; i++) {
		WMT_WARN_FUNC("will send rst msg\n");
		osal_assert(pDevWmt);
		...
		if (NULL != pDevWmt) {
			...
			if (pDevWmt->rFdrvCb.fDrvRst[i]) {
				...
				pDevWmt->rFdrvCb.fDrvRst[i] (WMTDRV_TYPE_WMT, i,
						WMTMSG_TYPE_RESET, &msg, sizeof(ENUM_WMTRSTMSG_TYPE_T));
				WMT_WARN_FUNC("type = %s, msg sent\n", drv_name[i]);
			} else {
				WMT_WARN_FUNC("type = %s, unregistered\n", drv_name[i]);
			}
#if 0
		} else {
			WMT_WARN_FUNC("pDevWmt is null \n");
			return MTK_WCN_BOOL_FALSE;
		}
#endif
	}
	...
	/*
	 * turn func on at fDrvRst[i]() if necessary.
	 * turn off power if no func is on to release ic6630_wakelock.
	 */
	if (WMTRSTMSG_RESET_END == msg || WMTRSTMSG_RESET_END_FAIL == msg)
		wmt_core_pwr_off_if_func_off(1);

	return MTK_WCN_BOOL_TRUE;
}

VOID wmt_cdev_rstmsg_snd_wifi(ENUM_WMTRSTMSG_TYPE_T msg)
{
	P_DEV_WMT pDevWmt = &gDevWmt;
	...
	WMT_INFO_FUNC("wifi WMTRSTMSG %d\n", msg);
	pDevWmt->rFdrvCb.fDrvRst[WMTDRV_TYPE_WIFI](
			WMTDRV_TYPE_WMT, WMTDRV_TYPE_WIFI, WMTMSG_TYPE_RESET,
			&msg, sizeof(ENUM_WMTRSTMSG_TYPE_T));
}
EXPORT_SYMBOL(wmt_cdev_rstmsg_snd_wifi);
...
MTK_WCN_BOOL wmt_lib_hw_rst(VOID)
{
	...
	//pSignal->timeoutValue = MAX_GPIO_CTRL_TIME;
	pSignal->timeoutValue = MAX_HW_RESET_TIME;

	/* below event will cause wmt_lib_put_act_op()
	 * fail due to "block tx flag is set"
	 *
	 * stp_sdio_tx_rx_handling()
	 *   stp_sdio_do_own_clr(0) fail
	 *   _stp_sdio_issue_fake_coredump(
	 *       "ABT: STP-SDIO clear f/w own failed")
	 *     stp_dump
	 *       WMT_unlocked_ioctl() case 10:
	 *	       mtk_wcn_stp_coredump_start_ctrl(1)
	 *
	 * so we call mtk_wcn_stp_coredump_start_ctrl(0)
	 * before wmt_lib_put_act_op()
	 */
...
#define HW_RESET_FAIL_TIMES 3
	...
	while (retries > 0) {
		if (mtk_wcn_stp_is_wmt_last_close_pre()) {
			WMT_INFO_FUNC("[whole chip reset] wmt closed, break\n");
			bRet = MTK_WCN_BOOL_FALSE;
			break;
		}
		/* <4.1> reset combo hw */
		atomic_set(&wmt_ctrl_hw_rst_fail, 0);
		bRet = wmt_lib_hw_rst();
		if (bRet == MTK_WCN_BOOL_FALSE) {
			WMT_ERR_FUNC("[whole chip reset] fail at wmt_lib_hw_rst!\n");
			retries--;
			if (atomic_read(&wmt_ctrl_hw_rst_fail)
					&& (retries <= RETRYTIMES - HW_RESET_FAIL_TIMES)) {
				break;
			} else {
				continue;
			}
...
MTK_WCN_BOOL wmt_lib_msgcb_reg(ENUM_WMTDRV_TYPE_T eType, PF_WMT_CB pCb)
{
	...
	if (eType >= 0 && eType <= WMTDRV_TYPE_ANT) { //-V1016
		WMT_DBG_FUNC("reg ok!\n");
...
MTK_WCN_BOOL wmt_lib_msgcb_unreg(ENUM_WMTDRV_TYPE_T eType)
{
	...
	if (eType >= 0 && eType <= WMTDRV_TYPE_WIFI) { //-V1016
...
MTK_WCN_BOOL wmt_lib_is_stp_wmt_last_close_pre(VOID)
{
	return mtk_wcn_stp_is_wmt_last_close_pre();
}
EXPORT_SYMBOL(wmt_lib_is_stp_wmt_last_close_pre);
...
VOID wmt_lib_read_fw_cpupcr_times(UINT32 times, UINT32 sleep)
{
	mtk_wcn_stp_read_fw_cpupcr_times(times, sleep);
}
EXPORT_SYMBOL(wmt_lib_read_fw_cpupcr_times);

VOID wmt_lib_read_fw_cpupcr(VOID)
{
	mtk_wcn_stp_read_fw_cpupcr_times(5, 1);
}
EXPORT_SYMBOL(wmt_lib_read_fw_cpupcr);

VOID wmt_lib_dump_all_thread(VOID)
{
	WMT_INFO_FUNC("start\n");
	atc_combo_dump_task_by_name("mtk_wmtd");
	atc_combo_dump_task_by_name("stp_sdio_tx_rx");
	atc_combo_dump_task_by_name(PSM_THREAD_NAME);
	atc_combo_dump_task_by_name(BTM_THREAD_NAME);
	WMT_INFO_FUNC("end\n");
}
EXPORT_SYMBOL(wmt_lib_dump_all_thread);
