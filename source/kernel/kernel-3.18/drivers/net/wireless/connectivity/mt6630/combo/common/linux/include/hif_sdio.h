...
#define HIF_SDIO_LOUD_FUNC(fmt, arg...)	do { \
	if (gHifSdioDbgLvl >= HIF_SDIO_LOG_LOUD) \
		osal_dbg_print(DFT_TAG "[L]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define HIF_SDIO_DBG_FUNC(fmt, arg...) do { \
	if (gHifSdioDbgLvl >= HIF_SDIO_LOG_DBG) \
		osal_dbg_print(DFT_TAG "[D]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define HIF_SDIO_INFO_FUNC(fmt, arg...)	do { \
	if (gHifSdioDbgLvl >= HIF_SDIO_LOG_INFO) \
		osal_info_print(DFT_TAG "[I]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define HIF_SDIO_WARN_FUNC(fmt, arg...)	do { \
	if (gHifSdioDbgLvl >= HIF_SDIO_LOG_WARN) \
		osal_warn_print(DFT_TAG "[W]%s,%d:WARN! " fmt, \
				__func__, __LINE__, ##arg); \
} while (0)

#define HIF_SDIO_ERR_FUNC(fmt, arg...) do { \
	if (gHifSdioDbgLvl >= HIF_SDIO_LOG_ERR)	\
		osal_err_print(DFT_TAG "[E]%s,%d:ERROR! " fmt, \
				__func__, __LINE__, ##arg);	\
} while (0)
...
extern VOID mtk_wcn_hif_sdio_dump_irq_state(VOID);;

extern VOID mtk_wcn_hif_sdio_set_wmt_last_close(UINT32 value);

extern MTK_WCN_BOOL mtk_wcn_hif_sdio_is_wmt_last_close(VOID);
...
extern INT32 mtk_wcn_hif_sdio_read_chisr(
		MTK_WCN_HIF_SDIO_CLTCTX ctx, PUINT32 pval);

extern INT32 mtk_wcn_hif_sdio_peek_chisr(
		MTK_WCN_HIF_SDIO_CLTCTX ctx, PUINT32 pval);
