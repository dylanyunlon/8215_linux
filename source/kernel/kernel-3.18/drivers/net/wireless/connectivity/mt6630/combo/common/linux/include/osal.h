...
#if defined(CONFIG_WMT_SUPPORT_AEE) \
		&& ! defined(CONFIG_WMT_USE_STP_DUMP) \
		&& defined(WMT_PLAT_ALPS) && WMT_PLAT_ALPS
...
#if (KERNEL_VERSION(4, 9, 0) <= LINUX_VERSION_CODE)
/* <linux/device.h> will #include <linux/pm_wakeup.h> */
...
typedef struct _OSAL_TIMEOUT_TRACKER_ {
	...
	ULONG trig;
	UINT32 timeoutValue;
	MTK_WCN_BOOL isTimeout;
} OSAL_TIMEOUT_TRACKER, *P_OSAL_TIMEOUT_TRACKER;

typedef struct _OSAL_SIGNAL_ {
	struct completion comp;
	UINT32 timeoutValue;
	OSAL_TIMEOUT_TRACKER timeoutTracker;
...
#if 1 // if 0->1 if 1->0 need run PVS check
...
extern INT32 osal_usleep_range(ULONG min, ULONG max);
...
extern INT32 osal_wake_lock_init(P_OSAL_WAKE_LOCK plock, const char name[]);
...
#define osal_warn_print(fmt, arg...) osal_print(KERN_WARNING fmt, ##arg)
#define osal_info_print(fmt, arg...) osal_print(KERN_INFO fmt, ##arg)
