...
INT32 osal_dbg_assert(INT32 expr, const PINT8 file, INT32 line)
{
	if (!expr) {
		...
		pr_warn(DFT_TAG "toggle GPIO_ASSERT = %d\n", GPIO_ASSERT);
		udelay(10);
...
_osal_inline_ INT32 osal_thread_create(P_OSAL_THREAD pThread)
{
	pThread->pThread = kthread_create(pThread->pThreadFunc,
			pThread->pThreadData, "%s", pThread->threadName);
...
static VOID osal_timeout_tracker_start(
		P_OSAL_TIMEOUT_TRACKER timeoutTracker, UINT32 timeoutValue)
{
	timeoutTracker->handler = current;
	timeoutTracker->trigger = NULL;
	timeoutTracker->start = jiffies;
	timeoutTracker->end = 0;
	timeoutTracker->trig = 0;
	timeoutTracker->timeoutValue = timeoutValue;
	timeoutTracker->isTimeout = MTK_WCN_BOOL_FALSE;
}

static VOID osal_timeout_tracker_end(P_OSAL_TIMEOUT_TRACKER timeoutTracker)
{
	ULONG timeout = timeoutTracker->start
		+ msecs_to_jiffies(timeoutTracker->timeoutValue);

	timeoutTracker->end = jiffies;

	if (time_is_after_jiffies(timeout)) {
		return;
	}

	timeoutTracker->isTimeout = MTK_WCN_BOOL_TRUE;

	pr_err(DFT_TAG "%s: %p wait for signal/event timeout\n",
			__func__, timeoutTracker);

	if (timeoutTracker->trigger) {
		pr_err(DFT_TAG "%s wait/trigger/timeout: %u/%u/%u ms\n", __func__,
				jiffies_to_msecs(timeoutTracker->end - timeoutTracker->start),
				jiffies_to_msecs(timeoutTracker->trig - timeoutTracker->start),
				timeoutTracker->timeoutValue);
	} else {
		pr_err(DFT_TAG "%s wait/timeout: %u/%u ms, not be triggered\n",
				__func__,
				jiffies_to_msecs(timeoutTracker->end - timeoutTracker->start),
				timeoutTracker->timeoutValue);
	}
	...
	if (timeoutTracker->trigger) {
		pr_err(DFT_TAG "Trigger Task dump:\n");
		atc_combo_dump_single_task(timeoutTracker->trigger);
	}

	pr_err(DFT_TAG "Handler Task dump:\n");
	atc_combo_dump_single_task(timeoutTracker->handler);

	atc_combo_dump_cpu_task_simple();
}

static VOID osal_timeout_tracker_trig(P_OSAL_TIMEOUT_TRACKER timeoutTracker)
{
	timeoutTracker->trigger = current;
	timeoutTracker->trig = jiffies;

	if (!timeoutTracker->isTimeout) {
		return;
	}

	pr_err(DFT_TAG "%s: %p trigger after wait timeout\n",
			__func__, timeoutTracker);
...
_osal_inline_ INT32 osal_wait_for_signal_timeout(P_OSAL_SIGNAL pSignal)
{
	...
	osal_timeout_tracker_start(&pSignal->timeoutTracker, pSignal->timeoutValue);

	ret = wait_for_completion_timeout(&pSignal->comp,
			msecs_to_jiffies(pSignal->timeoutValue));

	osal_timeout_tracker_end(&pSignal->timeoutTracker);
...
_osal_inline_ INT32 osal_raise_signal(P_OSAL_SIGNAL pSignal)
{
	...
	if (pSignal->timeoutValue) {
		osal_timeout_tracker_trig(&pSignal->timeoutTracker);
	}
...
INT32 osal_wait_for_event_timeout(P_OSAL_EVENT pEvent, INT32(*condition) (PVOID), PVOID cond_pa)
{
	...
	osal_timeout_tracker_start(&pEvent->timeoutTracker, pEvent->timeoutValue);

	ret = wait_event_interruptible_timeout(pEvent->waitQueue,
			condition(cond_pa), msecs_to_jiffies(pEvent->timeoutValue));

	osal_timeout_tracker_end(&pEvent->timeoutTracker);
...
INT32 osal_trigger_event(P_OSAL_EVENT pEvent)
{
	...
	if (pEvent->timeoutValue) {
		osal_timeout_tracker_trig(&pEvent->timeoutTracker);
	}
						...
						UINT32 bitOffset)
{
	...
	if (0 == pEvent->timeoutValue) {
		return wait_event_interruptible(pEvent->waitQueue,
				test_bit(bitOffset, (unsigned long *)pState));
	}
...
INT32 _osal_fifo_init(OSAL_FIFO *pFifo, PUINT8 buf, UINT32 size)
{
	...
	if (!pFifo || pFifo->pFifoBody) {
		pr_err(DFT_TAG "pFifo must be !NULL, pFifo->pFifoBody must be NULL\n");
		if (!pFifo)
			pr_err(DFT_TAG "pFifo is NULL\n");
		else if (!pFifo->pFifoBody)
			pr_err(DFT_TAG "pFifo->pFifoBody is NULL\n");
		return -1;
	}
...
INT32 _osal_fifo_deinit(OSAL_FIFO *pFifo)
{
	...
	if (!pFifo || !pFifo->pFifoBody) {
		pr_warn(DFT_TAG "%s:pFifo = NULL or pFifo->pFifoBody = NULL, error\n", __func__);
		return -1;
	}
...
INT32 _osal_fifo_data_in(OSAL_FIFO *pFifo, const PVOID buf, UINT32 len)
{
	...
	if (buf && (len <= _osal_fifo_avail_size(pFifo)))
		ret = kfifo_in(fifo, buf, len);
	else {
		pr_warn(DFT_TAG "%s: kfifo_in, error, len = %d, _osal_fifo_avail_size = %d, buf=%p\n",
...
INT32 _osal_fifo_data_out(OSAL_FIFO *pFifo, PVOID buf, UINT32 len)
{
	...
	if (buf && (len <= _osal_fifo_len(pFifo)))
		ret = kfifo_out(fifo, buf, len);
	else {
		pr_warn(DFT_TAG "%s: kfifo_out, error, len = %d, osal_fifo_len = %d, buf=%p\n",
...
{
	if (!pFifo) {
		pr_warn(DFT_TAG "%s:pFifo = NULL, error\n", __func__);
		return -1;
	}
	...
	if (NULL != pFifo->pFifoBody) {
		pr_warn
		    (DFT_TAG "%s:Because pFifo room is avialable, we clear the room and allocate them again.\n",
		     __func__);
...
INT32 osal_wake_lock_init(P_OSAL_WAKE_LOCK pLock, const char name[])
{
	if (!pLock || !name)
		return -1;

	osal_strncpy((PINT8)(pLock->name), (const PINT8)name, MAX_WAKE_LOCK_NAME_LEN-1);
	pLock->name[MAX_WAKE_LOCK_NAME_LEN-1] = '\0';
...
INT32 osal_wake_lock_deinit(P_OSAL_WAKE_LOCK pLock)
{
	...
	if (pLock->init_flag == 1) {
		...
		pr_warn(DFT_TAG "%s: wake_lock is not initialized!\n", __func__);
#else
...
#if (KERNEL_VERSION(4, 9, 0) <= LINUX_VERSION_CODE)
	if (pLock->init_flag == 1 && pLock->wake_lock)
...
INT32 osal_printtimeofday(const PUINT8 prefix)
{
	...
	ret += osal_dbg_print(DFT_TAG "%s>sec=%d, usec=%d\n", prefix, sec, usec);
...
VOID osal_buffer_dump(const PUINT8 buf, const PUINT8 title, const UINT32 len, const UINT32 limit)
{
	...
	pr_warn(DFT_TAG "start of dump>[%s] len=%d, limit=%d,", title, len, limit);
	...
	for (k = 0; k < dump_len; k++) {
		if ((k != 0) && (k % 16 == 0))
			pr_warn(DFT_TAG "\n");
		pr_warn(DFT_TAG "0x%02x ", buf[k]);
	}
	pr_warn(DFT_TAG "<end of dump\n");
}
