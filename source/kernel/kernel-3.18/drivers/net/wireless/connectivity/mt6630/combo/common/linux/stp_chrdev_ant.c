...
static UINT32 gDbgLevel = ANT_LOG_WARN;

#define ANT_DBG_FUNC(fmt, arg...) do { \
	if (gDbgLevel >= ANT_LOG_DBG) \
		pr_info(PFX "[D]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define ANT_INFO_FUNC(fmt, arg...) do { \
	if (gDbgLevel >= ANT_LOG_INFO) \
		pr_info(PFX "[I]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define ANT_WARN_FUNC(fmt, arg...) do { \
	if (gDbgLevel >= ANT_LOG_WARN) \
		pr_warn(PFX "[W]%s,%d:WARN! " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define ANT_ERR_FUNC(fmt, arg...) do { \
	if (gDbgLevel >= ANT_LOG_ERR) \
...
static INT32 ANT_DownLoad_RAM_Code(unsigned long ver)
{
	...
	while ((i <= download_number)) {
		if (i < download_number) {
			...
			if ((__vfs_read(pPatchExtFile,
							(__force void __user *)pbPatchExtBin,
...
{
	ANT_INFO_FUNC("major %d minor %d (pid %d)\n",
