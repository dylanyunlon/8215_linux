...
static VOID bt_cdev_rst_cb(ENUM_WMTDRV_TYPE_T src, ENUM_WMTDRV_TYPE_T dst,
		ENUM_WMTMSG_TYPE_T type, PVOID buf, UINT32 sz)
{
	...
	if (sz <= sizeof(ENUM_WMTRSTMSG_TYPE_T)) {
		...
		BT_DBG_FUNC("src = %d, dst = %d, type = %d, "
				"buf = 0x%x sz = %d, max = %d\n", src,
...
ssize_t BT_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (wmt_lib_is_stp_wmt_last_close_pre()) {
		BT_ERR_FUNC("wmt closed, block this io\n");
		retval = -EIO;
		goto OUT;
	}
...
ssize_t BT_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	while (retval == 0) {	/* Got nothing, wait for STP's signal */
		...
		*/
		...
		BT_DBG_FUNC("wait_event 1\n");
		wait_event(BT_wq, flag != 0);
		BT_DBG_FUNC("wait_event 2\n");
		flag = 0;
		retval = mtk_wcn_stp_receive_data(i_buf, count, BT_TASK_INDX);
		BT_DBG_FUNC("mtk_wcn_stp_receive_data returns %d\n", retval);
	}
...
long BT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMBO_IOC_BT_HWVER:
		...
		BT_INFO_FUNC("HW version = %d, sizeof(hw_ver_sym) = %zd\n",
	...
	case COMBO_IOCTL_FW_ASSERT:
		...
		BT_ERR_FUNC("Host trigger FW assert......, reason:%lu\n", arg);
		bRet = mtk_wcn_wmt_assert(WMTDRV_TYPE_BT, arg);
		if (bRet == MTK_WCN_BOOL_TRUE) {
			BT_ERR_FUNC("Host trigger FW assert succeed\n");
			retval = 0;
		} else {
...
static int BT_open(struct inode *inode, struct file *file)
{
	...
	BT_INFO_FUNC("WMT func on BT OK!\n");
	rstflag = 0;
...
static int BT_init(void)
{
	...
	if (alloc_ret) {
		BT_ERR_FUNC("Failed to register char device\n");
		return alloc_ret;
	}
