...
#include "btm_core.h"
	...
	#define COMPRESSOR_EN   0
#else
	#define COMPRESSOR_EN   1
#endif

#define PFX_STP_DBG                      "[CNN][STPDbg-6630]"
...
VOID core_dump_timer_start(VOID)
{
	if (NULL == g_core_dump) {
		STP_DBG_ERR_FUNC("NULL g_core_dump\n");
		return;
	}

	osal_timer_start(&g_core_dump->dmp_timer, STP_CORE_DUMP_TIMEOUT);
}
...
#define L1_BUF_SIZE (32 * KBYTES)
...
#else
	#define L2_BUF_SIZE (4 * 512 * KBYTES)
#endif
	...
	wcn_compressor_reset(core_dmp->compressor, COMPRESSOR_EN, GZIP);
...
static INT32 wcn_core_dump_check_end(PUINT8 buf, INT32 len)
{
	if (strnstr(buf, "coredump end", len)) {
		if (0 == COMPRESSOR_EN) {
			STP_DBG_ERR_FUNC("[coredump] ignore this core dump end\n");
			return 0;
		} else {
			return 1;
		}
...
#define MAX_HEAD_LEN 512
	...
	switch (dmp->sm) {
	case CORE_DUMP_INIT:
		wcn_compressor_reset(dmp->compressor, COMPRESSOR_EN, GZIP);
...
#ifndef CONFIG_WMT_USE_STP_DUMP
...
#define INFO_HEAD ";COMBO_CONSYS FW CORE, "
	...
	if ((NULL != dmp->p_head)
		...
		STP_DBG_ERR_FUNC("<ASSERT> dmp->head_len:%d\n", dmp->head_len);
		STP_DBG_ERR_FUNC("<ASSERT> dmp->p_head:%s\n", dmp->p_head);
		...
		STP_DBG_ERR_FUNC("ABT dmp->head_len:%d\n", dmp->head_len);
		STP_DBG_ERR_FUNC("ABT dmp->p_head:%s\n", dmp->p_head);
...
P_WCN_COMPRESSOR_T wcn_compressor_init(PUINT8 name, INT32 L1_buf_sz, INT32 L2_buf_sz)
{
	...
	//if (compress->compress_type == GZIP) { // PVS: always true
...
fail:
	if (compress) {
		...
		//if (compress->worker) { // PVS: always true
			pstream = (z_stream *) compress->worker;
...
static VOID stp_dbg_dump_data(PUINT8 pBuf, PINT8 title, INT32 len)
{
	...
	n = osal_sprintf(str, "%s-len(%d): ", title, len);
	for (k = 0; k < len; k++) {
		if (n < (BUF_LEN - 32)) {
			n += osal_sprintf(str + n, "%02x ", pBuf[k]);
		} else {
			STP_DBG_WARN_FUNC("The data is too much\n");
			break;
		}
	}
	osal_sprintf(str + n, "--end\n");
	pr_warn(PFX_STP_DBG "%s", str);
}
...
static INT32 _stp_dbg_dmp_in(MTKSTP_DBG_T *stp_dbg, PINT8 buf, INT32 len)
{
	...
	//if (internalFlag) { // PVS: always true
		...
		if (0 != gStpDbgLogOut) {
			...
			STP_DBG_INFO_FUNC("STP-DBG:%d.%06ds, %s:pT%sn(%d)l(%d)s(%d)a(%d)\n", pHdr->sec,
...
INT32 stp_dbg_log_ctrl(UINT32 on)
{
	if (on != 0) {
		gStpDbgLogOut = 1;
		STP_DBG_INFO_FUNC("STP-DBG: enable pkt log dump out.\n");
	} else {
		gStpDbgLogOut = 0;
		STP_DBG_INFO_FUNC("STP-DBG: disable pkt log dump out.\n");
	}
	return 0;
}
...
INT32 stp_dbg_dmp_print(MTKSTP_DBG_T *stp_dbg)
{
	...
	do_gettimeofday(&now);
	STP_DBG_INFO_FUNC("=== start now %ld.%06ld jiffies %lu ===\n",
			now.tv_sec, now.tv_usec, jiffies);

	if (0 == spin_trylock_irqsave(&(stp_dbg->logsys->lock), flags)) {
		/* It is okay, because someone must have acquire this lock and
		 * it is dump print operation who are supposed to acquire this
		 * lock for much longer time */
		STP_DBG_WARN_FUNC("logsys log is locked by other, "
				"omit this dump request\n");
		return -1;
	}
	/* spin_lock_irqsave(&(stp_dbg->logsys->lock), flags); */
	/* Not to dequeue from loging system */
	inIndex = stp_dbg->logsys->in;
	dumpSize = stp_dbg->logsys->size > MAX_DMP_NUM
		? MAX_DMP_NUM : stp_dbg->logsys->size;
	outIndex = (inIndex + STP_DBG_LOG_ENTRY_NUM - 1) % STP_DBG_LOG_ENTRY_NUM;

	STP_DBG_INFO_FUNC("loged packet size %u dumpSize %u in %u out %u\n",
			stp_dbg->logsys->size, dumpSize, inIndex, outIndex);

	for (i = 0; i < dumpSize; i++) {
		pHdr = (STP_DBG_HDR_T *)stp_dbg->logsys->queue[outIndex].buffer;
		pBuf = (PINT8)(pHdr + 1);
		len = stp_dbg->logsys->queue[outIndex].len - sizeof(STP_DBG_HDR_T);

		pr_warn(PFX_STP_DBG "%d/%d: %s %s "
				"no %d len %d seq %d ack %d %d.%06ds %lu\n",
				i, dumpSize, pHdr->dir == PKT_DIR_TX ? "Tx" : "Rx",
				gStpDbgType[pHdr->type], pHdr->no, pHdr->len,
				pHdr->seq, pHdr->ack, pHdr->sec, pHdr->usec, pHdr->ts);

		stp_dbg_dump_data(pBuf, pHdr->dir == PKT_DIR_TX ? "Tx" : "Rx",
				len > STP_PKT_SZ ? STP_PKT_SZ : len);

		outIndex = (outIndex + STP_DBG_LOG_ENTRY_NUM - 1) % STP_DBG_LOG_ENTRY_NUM;
	}
	spin_unlock_irqrestore(&(stp_dbg->logsys->lock), flags);
	STP_DBG_INFO_FUNC("=== end ===\n");
...
static VOID stp_dbg_nl_init(VOID)
{
	...
	ret = genl_register_family_with_ops(&stp_dbg_gnl_family,
			stp_dbg_gnl_ops, ARRAY_SIZE(stp_dbg_gnl_ops));
#else
	ret = genl_register_family_with_ops(&stp_dbg_gnl_family,
			stp_dbg_gnl_ops);
#endif
	if (ret) {
		STP_DBG_ERR_FUNC("operation registration fail\n");
	}
}
...
static INT32 stp_dbg_nl_bind(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (num_bind_process < MAX_BIND_PROCESS) {
		...
		STP_DBG_ERR_FUNC("exceeding binding limit %d\n", MAX_BIND_PROCESS);
	}
...
{
	STP_DBG_ERR_FUNC("should not be invoked\n");
...
INT32 stp_dbg_nl_send(PINT8 aucMsg, UINT8 cmd, INT32 len)
{
	...
	if (num_bind_process == 0) {
		...
		STP_DBG_ERR_FUNC("the process is not invoked\n");
		return 0;
	}
...
unicast_retry:
	...
	if (!skb) {
		STP_DBG_ERR_FUNC("genlmsg_new fail...\n");
		return -1;
	}

	msg_head = genlmsg_put(skb, 0,
			stp_dbg_seqnum++, &stp_dbg_gnl_family, 0, cmd);
	if (msg_head == NULL) {
		nlmsg_free(skb);
		STP_DBG_DBG_FUNC("genlmsg_put fail...\n");
		return -1;
	}
	...
	if (rc != 0) {
		nlmsg_free(skb);
		STP_DBG_DBG_FUNC("nla_put_string fail...: %d\n", rc);
		return rc;
	}
	...
	/* sending message */
	rc = genlmsg_unicast(&init_net, skb, bind_pid[0]);
	if (rc != 0) {
		STP_DBG_ERR_FUNC("genlmsg_unicast fail(%d): (pid, num) = (%d, %d)\n",
				rc, bind_pid[0], num_bind_process);

		if (num_bind_process == 1)
			return rc;

		// update bind pid and try again
		last_bind_pid = bind_pid[0];    /* save last failed bind pid */
		for (i = 0; i < num_bind_process - 1; i++)
			bind_pid[i] = bind_pid[i+1];

		bind_pid[num_bind_process - 1] = last_bind_pid;
...
INT32 stp_dbg_aee_send(PUINT8 aucMsg, INT32 len, INT32 cmd)
{
	...
	//STP_DBG_ERR_FUNC("[coredump] enter\n");
	ret = wcn_core_dump_in(g_core_dump, aucMsg, len);
	if (ret == 1)
		wcn_core_dump_flush(0);
	//STP_DBG_ERR_FUNC("[coredump] end, ret(%d)\n", ret);
...
INT32 _stp_dbg_parser_assert_str(PINT8 str, ENUM_ASSERT_INFO_PARSER_TYPE type)
{
	...
	case STP_DBG_FW_TASK_ID:
		...
		if (NULL != pTemp){
			...
			STP_DBG_ERR_FUNC("pTemp is null \n");
			tempBuf[0] = '\0';
		}
	...
	case STP_DBG_FW_IRQ:
		...
		len = pTemp - pDtr;
		STP_DBG_ERR_FUNC("pdtr is %p ,ptemp is %p ,len is %d \n",pDtr,pTemp,len);
		if (NULL != pTemp){
			osal_memcpy(&tempBuf[0], pDtr, len);
			tempBuf[len] = '\0';
		} else {
