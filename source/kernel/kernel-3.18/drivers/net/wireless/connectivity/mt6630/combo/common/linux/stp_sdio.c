...
#define STP_SDIO_TX_RX_TMO_DBG 0
...
#undef DFT_TAG
#endif
#define DFT_TAG "[CNN][STP_SDIO-6630]"
...
#define STP_SDIO_RXDBG_MAX_DUMP (STP_SDIO_RXDBG_COUNT * 16) // 4096
#endif
...
static UINT32 stp_sdio_rxdbg_dump_idx(UINT32 idx);
static VOID stp_sdio_rxdbg_dump(VOID);
...
EXPORT_SYMBOL(g_stp_sdio_host_info);
...
static UINT32 stp_sdio_rxdbg_cnt;
...
#if STP_SDIO_TXDBG
static UINT32 stp_sdio_txdbg_cnt;
...
UINT32 gStpSdioDbgLvl = STPSDIO_LOG_DBG;
/*******************************************************************************
*                                 M A C R O S
********************************************************************************
*/
#define STPSDIO_LOUD_FUNC(fmt, arg...) do { \
	if (gStpSdioDbgLvl >= STPSDIO_LOG_LOUD) \
		pr_info(DFT_TAG "[L]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STPSDIO_DBG_FUNC(fmt, arg...) do { \
	if (gStpSdioDbgLvl >= STPSDIO_LOG_DBG) \
		pr_info(DFT_TAG "[D]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STPSDIO_HINT_FUNC(fmt, arg...) do { \
	if (gStpSdioDbgLvl >= STPSDIO_LOG_HINT) \
		pr_info(DFT_TAG "[H]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STPSDIO_INFO_FUNC(fmt, arg...) do { \
	if (gStpSdioDbgLvl >= STPSDIO_LOG_INFO) \
		pr_info(DFT_TAG "[I]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STPSDIO_WARN_FUNC(fmt, arg...) do { \
	if (gStpSdioDbgLvl >= STPSDIO_LOG_WARN) \
		pr_warn(DFT_TAG "[W]%s,%d:WARN! " fmt, __func__, __LINE__, ##arg); \
} while (0)

//-V:STPSDIO_ERR_FUNC:547
#define STPSDIO_ERR_FUNC(fmt, arg...) do { \
	if (gStpSdioDbgLvl >= STPSDIO_LOG_ERR) \
		pr_err(DFT_TAG "[E]%s,%d:ERROR! " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define STPSDIO_TRC_FUNC(f) do { \
	if (gStpSdioDbgLvl >= STPSDIO_LOG_DBG) \
		pr_info(DFT_TAG "[T]<%s> <%d>\n", __func__, __LINE__); \
} while (0)
   ...
   using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
	while (retry-- > 0) {
		...
		if (ret) {
			...
			if (wmt_lib_is_rst_on()) {
				STPSDIO_WARN_FUNC("whole chip resetting, break\n");
				retry = 0;
			}
			if (0 == retry)
				goto out;
	...
	do {
		ret = stp_sdio_rw_retry(HIF_TYPE_READL, STP_SDIO_RETRY_LIMIT, clt_ctx, CHLPCR,
				&chlcpr_value, 0);
		if (ret) {
			/* 4 <1.1> get CHLPCR Rx error handling */
		...
		if ((chlcpr_value & C_FW_COM_DRV_OWN) == C_FW_COM_DRV_OWN) {
			...
			STPSDIO_DBG_FUNC("firmware ownback is no polled, wait for (%d us) and retry\n",
				 delay_us);
			if(retry % 40 == 0) {
				mtk_wcn_hif_sdio_readl(clt_ctx, 0x0154, &write_value);
				//STPSDIO_INFO_FUNC("read SWPCDBGR(%x)\n", write_value);
				STPSDIO_DBG_FUNC("read SWPCDBGR(%x)\n", write_value);
			}
			udelay(delay_us);
		}
		if (0 == (retry - 1) % 40) {
			STPSDIO_ERR_FUNC("own back failed in %d us, "
					"something might goes wrong\n", 40*delay_us);
			// wmt_lib_read_fw_cpupcr(); // too loud
			if (wmt_lib_is_rst_on()) {
				STPSDIO_WARN_FUNC("whole chip resetting, break\n");
				retry = 0;
				break;
		...
		if (0 == (retry - 1)%200) {
			...
			// NOTE: here will cause wmt_dev_rx_timeout
...
INT32 stp_sdio_own_ctrl(SDIO_PS_OP op)
{
	...
	case OWN_SET:
		...
		//when sdio_cltctx is 0, gp_info has been set to 0,
		//wake_up maybe kernel panic
		if (gp_info->sdio_cltctx) {
		...
		{
		    STPSDIO_WARN_FUNC("stp_sdio_own_ctrl fail, gp_info->sdio_cltctx is null\n");
		}
		ret = 0;
		break;
...
extern VOID mtk_wcn_stp_dump_fw_reg_op_reg(
		VOID (*op)(UINT32 times, UINT32 sleep));
extern VOID mtk_wcn_stp_dump_fw_reg_op_unreg(VOID);

static VOID sdio_host_dumpregs(VOID)
{
#ifdef CONFIG_ARCH_AC8X
	extern void sdhci_sdio_dumpregs(void);

	sdhci_sdio_dumpregs();
#else
	STPSDIO_WARN_FUNC("no sdio host dump api\n");
#endif
}

static VOID stp_sdio_poll_cpupcr(UINT32 times, UINT32 sleep)
{
	MTK_WCN_HIF_SDIO_CLTCTX clt_ctx = g_stp_sdio_host_info.sdio_cltctx;
	UINT32 val = 0;
	INT32 i = 0;
	INT32 ret = 0;
	...
	STPSDIO_WARN_FUNC("======== start ========\n");

	for (i = 0; i < times; i++) {
		ret = mtk_wcn_hif_sdio_readl(clt_ctx, SWPCDBGR, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("%d/%d: read SWPCDBGR fail(%d)\n",
					i, times, ret);
			break;
		} else {
			STPSDIO_WARN_FUNC("%d/%d: read SWPCDBGR value(0x%x)\n",
					i, times, val);
		}
		osal_sleep_ms(sleep);
	}

	STPSDIO_WARN_FUNC("======== end ========\n");
}

static VOID _stp_sdio_dump_fw_reg(VOID)
{
	MTK_WCN_HIF_SDIO_CLTCTX clt_ctx = g_stp_sdio_host_info.sdio_cltctx;
	INT32 count = 3;
	...
	for (i = 0; i < count; i++) {
		...
		STPSDIO_WARN_FUNC("======== loop: %d/%d ========\n", i, count);

		ret = mtk_wcn_hif_sdio_readl(clt_ctx, CCIR, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CCIR      0x%08x ******\n", val);

		ret = mtk_wcn_hif_sdio_readl(clt_ctx, CHLPCR, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CHLPCR    0x%08x ******\n", val);

		ret = mtk_wcn_hif_sdio_readl(clt_ctx, CSDIOCSR, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CSDIOCSR  0x%08x ******\n", val);

		ret = mtk_wcn_hif_sdio_readl(clt_ctx, CHCR, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CHCR      0x%08x ******\n", val);

		ret = mtk_wcn_hif_sdio_peek_chisr(clt_ctx, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CHISR     0x%08x ******\n", val);

		ret = mtk_wcn_hif_sdio_readl(clt_ctx, CHIER, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CHIER     0x%08x ******\n", val);

		ret = mtk_wcn_hif_sdio_readl(clt_ctx, CTFSR, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CTFSR     0x%08x ******\n", val);

		ret = mtk_wcn_hif_sdio_readl(clt_ctx, CRPLR, &val);
		if (ret) {
			STPSDIO_ERR_FUNC("read reg fail(%d)\n", ret);
			break;
		}
		STPSDIO_WARN_FUNC("****** CRPLR     0x%08x ******\n", val);
...
static VOID stp_sdio_dump_fw_reg(UINT32 times, UINT32 sleep)
{
	STPSDIO_WARN_FUNC("======== start ========\n");

	dump_stack();
	sdio_host_dumpregs();
	stp_sdio_poll_cpupcr(times, sleep);
	_stp_sdio_dump_fw_reg();

	STPSDIO_WARN_FUNC("======== end ========\n");
}

extern VOID mtk_wcn_stp_dmp_tx_rx_op_reg(VOID (*op)(VOID));
extern VOID mtk_wcn_stp_dmp_tx_rx_op_unreg(VOID);
...
static VOID stp_sdio_tx_rx_handling(PVOID pData)
{
	...
	while (!osal_thread_should_stop(&pInfo->tx_rx_thread)) {
		...
		if ((0 != pInfo->irq_pending) && (1 == pInfo->awake_flag)) {
			...
			iRet = mtk_wcn_hif_sdio_read_chisr(clt_ctx, &chisr);
			if (iRet) {
				/* 4 <1.1> get CHISR Rx error handling */
				/* -EIO error handling at mtk_wcn_hif_sdio_read_chisr */
				if (-EIO == iRet) {
					pInfo->irq_pending = 0;
				}
				STPSDIO_ERR_FUNC("read CHISR failed(%d)\n", iRet);
			} else {
				pInfo->irq_pending = 0;
			}
			...
			//if (1 == pInfo->awake_flag) // PVS: always true
			...
			if (chisr & RX_DONE) {
				...
				if ((0 == pInfo->rx_pkt_len)
					...
					STPSDIO_ERR_FUNC("abnormal rx_pkt_len(%d) "
							"in CHISR(0x%08x) skip rx_worker\n",
					...
					if (pInfo->rx_pkt_len & 0x3) {
						STPSDIO_WARN_FUNC("rx len not 4 bytes, "
								"CHISR(0x%08x), rx len (%d).\n",
		...
		// osal_ftrace_print("%s|enable COM IRQ fail, iRet:%d\n", __func__, iRet);
			...
			if (iRet) {
				...
				if (chlcpr_value & C_FW_INT_EN_SET) {
					...
					/* INTR_STATUS CHECK */
					// iRet = mtk_wcn_hif_sdio_peek_chisr(clt_ctx, &chisr);
		...
		#else // ! CFG_WMT_PS_SUPPORT

			while_loop_counter = 0;
			STPSDIO_DBG_FUNC("ignore to set firmware own! (sleeping) ok\n");
			pInfo->awake_flag = 0;
			pInfo->sleep_flag = 0;
			mtk_wcn_hif_sdio_en_deep_sleep(clt_ctx);
			osal_raise_signal(&pInfo->isr_check_complete);

		#endif /* CFG_WMT_PS_SUPPORT */
		...
		if (1000 < while_loop_counter) {
			...
			STPSDIO_ERR_FUNC("stp_is_ready(%d) irq_pending(%d) "
					"tx_packet_num(%d) rx_pkt_len(%d)\n",
					mtk_wcn_stp_is_ready(), pInfo->irq_pending,
					pInfo->firmware_info.tx_packet_num, pInfo->rx_pkt_len);
			// NOTE: here will cause wmt_dev_rx_timeout
			// wmt_lib_read_fw_cpupcr();

			//osal_ftrace_print("%s|stp_is_ready(%d)irq_pending(%d)"
			//    "tx_packet_num(%d)rx_pkt_len(%d)\n",
	...
	while (0 < osal_signal_active_state(&pInfo->isr_check_complete)) {
		//May not have been implemented wait signal action, add signal action in while;
		osal_raise_signal(&pInfo->isr_check_complete);
	...
	/* Case 3: buffer is full (Aggregation) */
		...
		/* Case 3.2 Buffer is full */
			...
			spin_lock_irqsave(&gp_info->pkt_buf.rd_idx_lock,
					  gp_info->pkt_buf.rd_irq_flag);
			/* Check if the local buf is free enough */
			room = (gp_info->pkt_buf.wr_idx >= gp_info->pkt_buf.rd_idx) ? //-V547
			    (STP_SDIO_TX_BUF_CNT -
			     (gp_info->pkt_buf.wr_idx -
			      gp_info->pkt_buf.rd_idx)) : (gp_info->pkt_buf.rd_idx -
							   gp_info->pkt_buf.wr_idx);
		...
		if (p_info->firmware_info.tx_fifo_size >= four_byte_align_len) {
			...
			do {
				...
				stp_sdio_txdbg_buffer[idx].four_byte_align_len =
				    four_byte_align_len;
				stp_sdio_txdbg_buffer[idx].result = ret;
				/* store content */
			...
			if ((now.tv_sec - old.tv_sec) > TX_NO_ACK_TIMEOUT_ASSERT) {
				STPSDIO_INFO_FUNC("tx_fifo_size(%d), "
						"four_byte_align_len(%d), tx_packet_num(%d)\n",
						p_info->firmware_info.tx_fifo_size,
						four_byte_align_len,
						p_info->firmware_info.tx_packet_num);
				STPSDIO_INFO_FUNC("No ack trigger assert, "
						"tx %d seconds later\n",
...
MTK_WCN_BOOL wmt_dbg_is_func_ctrl_block(PUINT8 buf, UINT32 len);
...
//		osal_ftrace_print("%s|S|bus_txlen(%d)\n", __func__, four_byte_align_len);
		...
		if ((p_info->firmware_info.tx_fifo_size >= four_byte_align_len)
			...
			if (MTK_WCN_BOOL_TRUE
                    == wmt_dbg_is_func_ctrl_block(buf_tx, bus_txlen)) {
				STPSDIO_WARN_FUNC("func ctrl blocked !!!\n");
			} else {
				/* port write the packet to CTDR */
				ret = stp_sdio_rw_retry(HIF_TYPE_WRITE_BUF,
						STP_SDIO_RETRY_LIMIT, p_info->sdio_cltctx,
						CTDR, (PUINT32) buf_tx, bus_txlen);
				STPSDIO_DBG_FUNC("write to CTDR done\n");
			}
			...
			if ((now.tv_sec - old.tv_sec) > TX_NO_ACK_TIMEOUT_ASSERT) {
				...
				//osal_ftrace_print("%s|tx_fifo_size:%d, "
				//    four_byte_align_len:%d, tx_packet_num(%d)\n",
				//    __func__, p_info->firmware_info.tx_fifo_size,
				//    four_byte_align_len,
				//    p_info->firmware_info.tx_packet_num);
...
#if STP_SDIO_DBG_SUPPORT && STP_SDIO_RXDBG
	do {
		idx = stp_sdio_rxdbg_cnt++ & (STP_SDIO_RXDBG_COUNT - 1);
		/* skip clear buf */
...
#if STP_SDIO_TX_RX_TMO_DBG
		if (ret && (0 == p_info->rx_dbg_dump_flag)) {
			p_info->rx_dbg_dump_flag = 1;
			msleep(20); // to avoid log loss
			STPSDIO_ERR_FUNC("MTKSTP_FW_MSG received, "
					"call stp_sdio_rxdbg_dump_idx(%d)\n", idx);
			stp_sdio_rxdbg_dump_idx(idx);
		}
#endif
...
static INT32 stp_sdio_irq(const MTK_WCN_HIF_SDIO_CLTCTX clt_ctx)
{
	...
	ret = mtk_wcn_hif_sdio_read_chisr(clt_ctx, &chisr);
	if (ret) {
		/* 4 <1.1> get CHISR Rx error handling */
		/* -EIO error handling at mtk_wcn_hif_sdio_read_chisr */
		STPSDIO_ERR_FUNC("read CHISR failed(%d)\n", ret);
		if (-EIO != ret) {
			ret = mtk_wcn_hif_sdio_read_chisr(clt_ctx, &chisr);
			STPSDIO_ERR_FUNC("retry read CHISR ret %d val 0x%08x\n",
					ret, chisr);
		}
	}
...
stp_sdio_ownback_poll(const MTK_WCN_HIF_SDIO_CLTCTX clt_ctx, UINT32 retry, UINT32 delay_us)
{
	...
	do {
		...
		*/
		...
		STPSDIO_INFO_FUNC("start 111 ret=%d,chlpcr=0x%x\n", ret, chlpcr);
...
stp_sdio_probe(const MTK_WCN_HIF_SDIO_CLTCTX clt_ctx,
		const MTK_WCN_HIF_SDIO_FUNCINFO *sdio_func_infop)
{
	static INT32 probe_abnormal_cnt = 0;
	...
	if (g_stp_sdio_host_count) {
		probe_abnormal_cnt++;
		STPSDIO_ERR_FUNC("probed already g_stp_sdio_host_count %d probe_abnormal_cnt %d\n",
				g_stp_sdio_host_count, probe_abnormal_cnt);
		if (probe_abnormal_cnt < 3) {
			return -1;
		} else {
			STPSDIO_WARN_FUNC("reset g_stp_sdio_host_count\n");
			probe_abnormal_cnt = 0;
	...
	g_stp_sdio_host_info.rx_dbg_dump_flag = 0;
...
static UINT32 stp_sdio_rxdbg_dump_idx(UINT32 idx)
{
	...
	if (idx >= STP_SDIO_RXDBG_COUNT) {
		STPSDIO_ERR_FUNC("idx %d >= %lu\n", idx, STP_SDIO_RXDBG_COUNT);
		return 0;
	}
	pbuf = &stp_sdio_rxdbg_buffer[idx].rx_pkt_buf[0];
	bus_rxlen = stp_sdio_rxdbg_buffer[idx].bus_rxlen;
	ts = stp_sdio_rxdbg_buffer[idx].ts;

	pr_warn(DFT_TAG "stp_sdio_rxdbg_buffer idx(%u) bus_rxlen(0x%x, %u) ts(%lu)\n",
			idx, bus_rxlen, bus_rxlen, ts);

	for (k = 0, i = 0; i < bus_rxlen; i += 16) {
		if (i >= STP_SDIO_RXDBG_MAX_DUMP) {
			break;
		}
		pr_warn(DFT_TAG " %3x ["
...
static VOID stp_sdio_rxdbg_dump(VOID)
{
	...
	UINT32 i = 0;
	UINT32 idx = 0;
	UINT32 cnt = stp_sdio_rxdbg_cnt;
	UINT32 dump_len = 0;

	STPSDIO_INFO_FUNC("=== start jiffies %lu ===\n", jiffies);
	for (i = 0; i < STP_SDIO_RXDBG_COUNT; ++i) {
		idx = (cnt - 1 - i) & STP_SDIO_RXDBG_COUNT_MASK;
		dump_len += stp_sdio_rxdbg_dump_idx(idx);
		if (dump_len >= STP_SDIO_RXDBG_MAX_DUMP) {
			break;
		}
	}
	if (cnt != stp_sdio_rxdbg_cnt
			&& (i == STP_SDIO_RXDBG_COUNT
				|| (stp_sdio_rxdbg_cnt - cnt)
					> (STP_SDIO_RXDBG_COUNT - 1 - i))) {
		STPSDIO_WARN_FUNC("stp_sdio_rxdbg_cnt %x -> %x, "
				"%u new data on the end\n",
				cnt, stp_sdio_rxdbg_cnt, stp_sdio_rxdbg_cnt - cnt);
	}
	STPSDIO_INFO_FUNC("=== end dump_len %u ===\n", dump_len);
#endif
}
...
static VOID stp_sdio_txdbg_dump(VOID)
{
	...
	for (k = 0, i = 0; i < STP_SDIO_TXDBG_COUNT; ++i) {
		idx = (cnt - 1 - i) & STP_SDIO_TXDBG_COUNT_MASK;
		len = stp_sdio_txdbg_buffer[idx].bus_txlen;
		ts = stp_sdio_txdbg_buffer[idx].ts;
		ret = stp_sdio_txdbg_buffer[idx].result;
		STPSDIO_INFO_FUNC("stp_sdio_txdbg_buffer "
				"idx(%u) bus_txlen(0x%x, %u) result(%d) ts(%lu)\n",
				idx, len, len, ret, ts);
		if (0 == len) {
			STPSDIO_INFO_FUNC("idx(%u) 0 == len dump skip\n", idx);
			continue;
		}
		len = len > STP_SDIO_TXDBG_MAX_SIZE ? STP_SDIO_TXDBG_MAX_SIZE : len;

		for (j = 0; j < STP_SDIO_TX_ENTRY_SIZE && j < len; j += 16) {
			if (++k > STP_SDIO_TXDBG_COUNT) {
				break;
			}
	...
	if (cnt != stp_sdio_txdbg_cnt) {
		STPSDIO_WARN_FUNC("stp_sdio_txdbg_cnt %x -> %x, "
				"%u new data on the end\n",
				cnt, stp_sdio_txdbg_cnt, stp_sdio_txdbg_cnt - cnt);
	}

	STPSDIO_INFO_FUNC(
			"Dump tx info: pkt_num(%u) fifo(%u) "
			"pkt_list.rd(0x%x, %lu) pkt_list.wr(0x%x, %lu)\n",
	...
	for (i = 0; i < STP_SDIO_TX_PKT_LIST_SIZE; ++i) {
		...
		STPSDIO_INFO_FUNC(
				"tx_pkt_list idx(0x%x, %u) size(0x%x, %u), in_ts(%u), out_ts(%u)\n",
				(gp_info->tx_pkt_list.pkt_wr_cnt - 1 - i), idx,
				gp_info->tx_pkt_list.pkt_size_list[idx],
				gp_info->tx_pkt_list.pkt_size_list[idx],
				gp_info->tx_pkt_list.in_ts[idx],
				gp_info->tx_pkt_list.out_ts[idx]);
	}

#if STP_SDIO_NEW_TXRING
	STPSDIO_INFO_FUNC("\n\ndump pkt_buf.tx_buf: rd(%u) wr(%u) full(%u)\n",
			gp_info->pkt_buf.rd_cnt,
			gp_info->pkt_buf.wr_cnt,
			gp_info->pkt_buf.full_flag);
#else
	STPSDIO_INFO_FUNC("\n\ndump pkt_buf.tx_buf: rdi(%u) wri(%u) full(%u)\n",
			gp_info->pkt_buf.rd_idx,
			gp_info->pkt_buf.wr_idx,
			gp_info->pkt_buf.full_flag);
#endif

#if STP_SDIO_NEW_TXRING
	cnt = gp_info->pkt_buf.wr_cnt;
#else
	cnt = gp_info->pkt_buf.wr_idx;
#endif
	for (k = 0, i = 0; i < STP_SDIO_TX_BUF_CNT; ++i) {
#if STP_SDIO_NEW_TXRING
		idx = (cnt - 1 - i) & STP_SDIO_TX_BUF_CNT_MASK;
		len = gp_info->pkt_buf.tx_buf_sz[idx];
#else
		idx = (cnt - 1 - i + STP_SDIO_TX_BUF_CNT) % STP_SDIO_TX_BUF_CNT;
#if KMALLOC_UPDATE
...
#endif
		STPSDIO_INFO_FUNC("pkt_buf.tx_buf idx(%u) ts(%lu) len(%u)\n",
...
#if STP_SDIO_NEW_TXRING
	if (cnt != gp_info->pkt_buf.wr_cnt) {
		STPSDIO_WARN_FUNC("pkt_buf.wr_cnt %x -> %x, "
				"%u new data on the end\n",
				cnt, gp_info->pkt_buf.wr_cnt,
				gp_info->pkt_buf.wr_cnt - cnt);
	}
#else
	if (cnt != gp_info->pkt_buf.wr_idx) { //-V547
		STPSDIO_WARN_FUNC("pkt_buf.wr_idx %x -> %x, "
				"%u new data on the end\n",
				cnt, gp_info->pkt_buf.wr_idx,
				gp_info->pkt_buf.wr_idx - cnt);
	}
#endif

	STPSDIO_INFO_FUNC("=== end ===\n");
...
static VOID stp_sdio_dmp_tx_rx(VOID)
{
#if STP_SDIO_TXDBG
	stp_sdio_txdbg_dump();
#endif
...
#if STP_SDIO_DBG_SUPPORT
	mtk_wcn_stp_dmp_tx_rx_op_reg(stp_sdio_dmp_tx_rx);
#endif
...
static void stp_sdio_exit(void)
{
...
#if STP_SDIO_DBG_SUPPORT
	mtk_wcn_stp_dmp_tx_rx_op_unreg();
#endif
	...
	mtk_wcn_stp_dump_fw_reg_op_unreg();
