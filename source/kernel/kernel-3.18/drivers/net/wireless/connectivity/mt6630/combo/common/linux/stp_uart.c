...
static UINT32 gDbgLevel = UART_LOG_WARN;

#define UART_DBG_FUNC(fmt, arg...) do { \
	if (gDbgLevel >= UART_LOG_DBG)	\
		pr_info(PFX "[D]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define UART_INFO_FUNC(fmt, arg...)	do { \
	if (gDbgLevel >= UART_LOG_INFO)	\
		pr_info(PFX "[I]%s,%d: " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define UART_WARN_FUNC(fmt, arg...)	do { \
	if (gDbgLevel >= UART_LOG_WARN)	\
		pr_warn(PFX "[W]%s,%d:WARN! " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define UART_ERR_FUNC(fmt, arg...) do { \
	if (gDbgLevel >= UART_LOG_ERR)	\
		pr_err(PFX "[E]%s,%d:ERROR! " fmt, __func__, __LINE__, ##arg); \
} while (0)

#define UART_TRC_FUNC(f) do { \
	if (gDbgLevel >= UART_LOG_DBG)	\
		pr_info(PFX "[T]<%s> <%d>\n", __func__, __LINE__);	\
} while (0)
...
#define UART_TX_TIMEOUT_MSEC    1000
...
/* //    spin_lock_irqsave(&buf_lock, flags); */
	...
	if (len > 0 && len < MAX_PACKET_ALLOWED) {
		...
		if (len > 0 && len < MAX_PACKET_ALLOWED) {
			...
			rd_idx = ((rd_idx + written) % MTKSTP_BUFFER_SIZE);
			written_count += written;
		} else if (len < 0 || len >= MAX_PACKET_ALLOWED) { //-V560
...
static inline INT32 stp_uart_tx_wakeup(struct tty_struct *tty)
{
	...
	UART_DBG_FUNC("++\n");
	len = (wr_idx >= rd_idx)
		? (wr_idx - rd_idx) : (MTKSTP_BUFFER_SIZE - rd_idx);
	if (len == 0) {
		UART_WARN_FUNC("len 0\n");
		return 0;
	} else if (len < 0 || len >= MAX_PACKET_ALLOWED) {
		UART_ERR_FUNC("[%d:%s] len %d wr_idx %d rd_idx %d\n",
				current->pid, current->comm, len, wr_idx, rd_idx);
		return -1;
	}
	remainder = len;
	timeout = jiffies + msecs_to_jiffies(UART_TX_TIMEOUT_MSEC);
	while (remainder && time_is_after_jiffies(timeout)) {
		set_bit(TTY_DO_WRITE_WAKEUP, &tty->flags);
		/* ops->write is called by the kernel to write a series of
		 * characters to the tty device. The characters may come from
		 * user space or kernel space. This routine will return the
		 * number of characters actually accepted for writing.
		 */
		ret = tty->ops->write(tty, &tx_buf[rd_idx], remainder);
		if (ret < 0) {
			UART_ERR_FUNC("uart write fail %d\n", ret);
			return ret;
		} else if (ret == 0) {
			msleep(10); // wait for async tx
		} else if (ret == remainder) {
			written += ret;
			rd_idx = ((rd_idx + ret) % MTKSTP_BUFFER_SIZE);

			/* all data is accepted by UART driver, check again in case roll over */
			remainder = (wr_idx >= rd_idx)
				? (wr_idx - rd_idx) : (MTKSTP_BUFFER_SIZE - rd_idx);
			if (remainder == 0) {
				break;
			} else if (remainder >= MAX_PACKET_ALLOWED) {
				UART_ERR_FUNC("[%d:%s] len %d wr_idx %d rd_idx %d\n",
						current->pid, current->comm, remainder, wr_idx, rd_idx);
				return -1;
			} else {
				len += remainder;
			}
	...
	if (written != len) {
		UART_ERR_FUNC("uart write timeout - written %d != len %d "
				"wr_idx %d rd_idx %d\n", written, len, wr_idx, rd_idx);
	}
	UART_DBG_FUNC("--\n");
...
static INT32 stp_uart_fifo_init(VOID)
{
	...
	//if (0 == err) { // PVS: always true
		if (NULL != g_stp_uart_rx_fifo) {
			kfifo_reset(g_stp_uart_rx_fifo);
			UART_INFO_FUNC("stp_uart_fifo_init() success.\n");
		} else {
			err = -4;
			UART_ERR_FUNC
...
static int mtk_wcn_stp_uart_init(void)
{
	...
	if (!g_stp_uart_rx_work) {
		UART_ERR_FUNC("vmalloc work_struct(%zd) fail\n", sizeof(struct work_struct));
		err = -ENOMEM;
		goto init_err;
	}
