...
#define WIFI_DEV_MAJOR 155	/* 153 */

#define PFX                         "[CNN][WIFI-6630]"
...
INT32 wifi_reset_end(ENUM_RESET_STATUS_T status)
{
	...
	if (funoff_fail){
		funoff_fail = 0;
		up(&wr_mtx);
		WIFI_WARN_FUNC("[wifi-debug] funoff fail and return...\n");
		return 0;
	}
...
VOID wmt_reset_wifi_only(VOID)
{
	WIFI_WARN_FUNC("\n");
	// should turn off before fgIsResetting
	...
	wmt_cdev_rstmsg_snd_wifi(WMTRSTMSG_RESET_START);
	wmt_cdev_rstmsg_snd_wifi(WMTRSTMSG_RESET_END);
}
EXPORT_SYMBOL(wmt_reset_wifi_only);

static int WIFI_open(struct inode *inode, struct file *file)
{
	if (!strcmp(current->comm, "blkid")) {
		WIFI_INFO_FUNC("block blkid open\n");
		return -EPERM;
	}
...
ssize_t WIFI_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (0 == copy_from_user(local, buf, (count > sizeof(local)) ? sizeof(local) : count)) {
		...
		if (wmt_lib_is_stp_wmt_last_close_pre()) {
			WIFI_ERR_FUNC("wmt closed, block this io\n");
			retval = -EIO;
			goto done;
		}
		...
		if (local[0] == '0') {
			if (powered == 0
					&& (MTK_WCN_BOOL_FALSE
						== wmt_core_is_func_on(WMTDRV_TYPE_WIFI))) {
