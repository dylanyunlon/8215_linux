...
#define MTK_WMT_PATCH_DATE              "2024/09/12"
...
#define WMT_DEV_INIT_TO_MS (2 * 1000)
	...
	#define FSGID fsgid
#else
	#define FSUID fsuid.val
	#define FSGID fsgid.val
#endif
...
/*
 * Rootcause:
 * If wmt_dev_rx_event_cb() come before first
 * wmt_dev_rx_timeout(), it will get null gpRxEvent
 * and wmt_lib_flush_rx(), then wmt_dev_rx_timeout()
 * will timeout.
 *
 * Test Step:
 * 1. init gpRxEvent to NULL.
 * 2. Add 300ms delay at the begining of wmt_dev_rx_timeout()
 *    for the first time call.
 * 3. turn on WIFI and BT.
 * 4. reboot OT.
 * 5. wmt_dev_rx_timeout() will timeout randomly.
 *
 * Solution:
 * init gpRxEvent to &gDevWmt.rWmtRxWq.
 */
P_OSAL_EVENT gpRxEvent = &gDevWmt.rWmtRxWq;
...
static int wmt_probe_num;

#ifdef CONFIG_ARCH_AC823X
static const struct of_device_id mt6630_of_ids[] = {
	{.compatible = "Autochips,ac823x-CNNdynamic",},
	{}
};
...
static const struct of_device_id mt6630_of_ids[] = {
	{.compatible = "Autochips,ac8x-CNNdynamic",},
	{}
};
#else
...
/*
static void wmt_dev_release(struct device *dev)
{
	return;
}
*/
...
static struct platform_device wmt_device = {
	.name = WMT_DRIVER_NAME,
	.id = -1,
	.dev = {
		/* .release = &wmt_dev_release, */
	},
};

#ifndef CONFIG_ATC_OS_VERSION_JB2
#if defined(CONFIG_ARCH_AC8X)
struct _mt6630_gpio mt6630_gpio[PIN_NUM] = {
	{ "pwn-gpios", NULL, -1 },
	{ "rst-gpios", NULL, -1 }
};
#else
...
ssize_t wmt_aee_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	WMT_INFO_FUNC("count %zu pos %lld\n", count, *f_pos);
...
static INT32 wmt_dev_rx_is_triggered(PVOID pvData)
{
	return (0 != *(UINT32 *)pvData);
}
...
INT32 wmt_dev_rx_timeout(P_OSAL_EVENT pEvent)
{
	...
	if (0 != pEvent->timeoutValue) {
		#if 0
		ret = wait_event_interruptible_timeout(
				gpRxEvent->waitQueue, 0 != u4RxFlag,
				msecs_to_jiffies(pEvent->timeoutValue));
		#else
		ret = osal_wait_for_event_timeout(gpRxEvent,
				wmt_dev_rx_is_triggered, &u4RxFlag);
		#endif
	} else {
		#if 0
		ret = wait_event_interruptible(
				gpRxEvent->waitQueue, u4RxFlag != 0);
		#else
		ret = osal_wait_for_event(gpRxEvent,
				wmt_dev_rx_is_triggered, &u4RxFlag);
		#endif
	}
...
INT32 wmt_dev_read_file(PUINT8 pName, const PPUINT8 ppBufPtr, INT32 offset, INT32 padSzBuf)
{
	...
	*ppBufPtr = NULL;
	...
	do {
		...
		read_len = __vfs_read(fd, (__force void __user *)pBuf + padSzBuf, file_len, &fd->f_pos);
...
INT32 wmt_dev_patch_get(PUINT8 pPatchName, osal_firmware **ppPatch, INT32 padSzBuf)
{
	INT32 iRet = -1;
	INT32 i = 0;
	osal_firmware *pfw = NULL;

	if (!ppPatch) {
		WMT_ERR_FUNC("invalid ppPatch!\n");
		return -1;
	}
	if (*ppPatch) {
	...
	for (i = 0; i < 10; i++) {
		iRet = request_firmware((const struct firmware **)&pfw,
				pPatchName, NULL);
		if (iRet != -EAGAIN) {
			break;
		}
		if (mtk_wcn_stp_is_wmt_last_close_pre()) {
			WMT_INFO_FUNC("%d: load file(%s) failed(%d) "
					"when wmt closed, break.\n",
					i, pPatchName, iRet);
			break;
		}
		WMT_ERR_FUNC("%d: failed to open or read!(%s), retry again!\n",
				i, pPatchName);
		osal_sleep_ms(100);
	}

	if (iRet != 0) {
		WMT_ERR_FUNC("failed to open or read!(%s) iRet(%d)\n",
				pPatchName, iRet);
		return -1;
	} else {
		WMT_INFO_FUNC("load file %s ok!!\n", pPatchName);
		iRet = 0;
		*ppPatch = pfw;
	}
...
#define REFRESH_TIME    300	/* sec */
	...
	/* not query yet. */
	static INT32 temp_table[HISTORY_NUM] = { 99, 99, 99, 99, 99 };
...
unsigned int WMT_poll(struct file *filp, poll_table *wait)
{
	...
	WMT_DBG_FUNC("WMT_poll end,mask is %d!\n", mask);
	return mask;
}
...
long WMT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case WMT_IOCTL_SET_PATCH_NAME:	/* patch location */
		{
			WMT_INFO_FUNC("WMT_IOCTL_SET_PATCH_NAME\n");
			if (copy_from_user(pBuffer, (void *)arg, NAME_MAX)) {
				iRet = -EFAULT;
				break;
			}
			pBuffer[NAME_MAX] = '\0';
			WMT_INFO_FUNC("PATCH_NAME is %s\n", pBuffer);
			wmt_lib_set_patch_name(pBuffer);
			WMT_INFO_FUNC("gDevWmt.cPatchName is %s\n", gDevWmt.cPatchName);
		}
		break;

	case WMT_IOCTL_SET_STP_MODE:	/* stp/hif/fm mode */

		WMT_INFO_FUNC("WMT_IOCTL_SET_STP_MODE\n");
		/* set hif conf */
		do {
			P_OSAL_OP pOp;
			MTK_WCN_BOOL bRet;
	...
	case WMT_IOCTL_FUNC_ONOFF_CTRL:	/* test turn on/off func */
		...
		WMT_DBG_FUNC("WMT_IOCTL_FUNC_ONOFF_CTRL\n");
		do {
			MTK_WCN_BOOL bRet = MTK_WCN_BOOL_FALSE;

			if (arg & 0x80000000) {
				WMT_DBG_FUNC("mtk_wcn_wmt_func_on\n");
				bRet = mtk_wcn_wmt_func_on(arg & 0xF);
			} else {
				WMT_DBG_FUNC("mtk_wcn_wmt_func_off\n");
	...
	case WMT_IOCTL_LPBK_POWER_CTRL:
		WMT_DBG_FUNC("WMT_IOCTL_LPBK_POWER_CTRL\n");
	...
	case WMT_IOCTL_LPBK_TEST:
		WMT_DBG_FUNC("WMT_IOCTL_LPBK_TEST\n");
		do {
			P_OSAL_OP pOp;
			MTK_WCN_BOOL bRet;
			UINT32 u4Wait;
...
#define LOG_BUF_SZ 300
	...
	case WMT_IOCTL_GET_CHIP_INFO:
		{
			WMT_DBG_FUNC("WMT_IOCTL_GET_CHIP_INFO\n");
			if (0 == arg)
	...
	case WMT_IOCTL_SET_LAUNCHER_KILL:{
			WMT_DBG_FUNC("WMT_IOCTL_SET_LAUNCHER_KILL\n");
			if (1 == arg) {
				WMT_ERR_FUNC("launcher may be killed, block abnormal stp tx.\n");
	...
	case WMT_IOCTL_SET_PATCH_NUM:{
			WMT_DBG_FUNC("WMT_IOCTL_SET_PATCH_NUM\n");
			pAtchNum = arg;
	...
	case WMT_IOCTL_SET_PATCH_INFO:{
			...
			WMT_DBG_FUNC("WMT_IOCTL_SET_PATCH_INFO\n");
			if (!pPatchInfo) {
	...
	case WMT_IOCTL_PORT_NAME:{
			...
			WMT_DBG_FUNC("WMT_IOCTL_PORT_NAME\n");
	...
	case WMT_IOCTL_WMT_CFG_NAME:
		{
			...
			WMT_DBG_FUNC("WMT_IOCTL_WMT_CFG_NAME\n");
	...
	case WMT_IOCTL_WMT_QUERY_CHIPID:
		{
			WMT_DBG_FUNC("WMT_IOCTL_WMT_QUERY_CHIPID\n");
	...
	case WMT_IOCTL_WMT_TELL_CHIPID:
		{
			WMT_DBG_FUNC("WMT_IOCTL_WMT_TELL_CHIPID\n");
#if !(DELETE_HIF_SDIO_CHRDEV)
	...
	case WMT_IOCTL_WMT_COREDUMP_CTRL:
		{
			WMT_DBG_FUNC("WMT_IOCTL_WMT_COREDUMP_CTRL\n");
			mtk_wcn_stp_coredump_flag_ctrl(arg);
		}
		break;
	case WMT_IOCTL_WMT_STP_ASSERT_CTRL:
		{
			WMT_DBG_FUNC("WMT_IOCTL_WMT_STP_ASSERT_CTRL\n");
...
static int WMT_open(struct inode *inode, struct file *file)
{
	...
	if (!strcmp(current->comm, "blkid")) {
		WMT_INFO_FUNC("block blkid open\n");
		return -EPERM;
	}
...
#if !defined(CONFIG_ATC_OS_VERSION_JB2) \
	&& (defined(CONFIG_ARCH_AC83XX) \
			|| defined(CONFIG_ARCH_AC823X))

static int wmt_gpio_init(VOID)
{
	int ret = -1;
	/* init gpio */
	/* struct __mt6630_gpio mt6630_gpio; */
	/* get_gpio(&mt6630_gpio); */
	if (NULL == mt6630_gpio[PWN_PIN].desc) {
		WMT_INFO_FUNC("PWN_PIN desc is NULL\n");
		mt6630_gpio[PWN_PIN].desc =
		    __gpiod_get(&(wmt_device.dev), mt6630_gpio[PWN_PIN].name, GPIOD_ASIS);
		if (IS_ERR(mt6630_gpio[PWN_PIN].desc)) {
			pr_debug("[WMT_DEV:]can not get GPIO %s\n", mt6630_gpio[PWN_PIN].name);
			goto gpio_init_err;
		} else {
			WMT_INFO_FUNC("PWN_PIN Get success\n");
		}
	}
	if (NULL == mt6630_gpio[RST_PIN].desc) {

		WMT_INFO_FUNC("RST_PIN desc is NULL\n");
		mt6630_gpio[RST_PIN].desc =
		    __gpiod_get(&(wmt_device.dev), mt6630_gpio[RST_PIN].name, GPIOD_ASIS);
		if (IS_ERR(mt6630_gpio[RST_PIN].desc)) {
			WMT_WARN_FUNC("RST_PIN Get Fail\n");
			pr_debug("[WMT_DEV:]can not get GPIO %s\n", mt6630_gpio[RST_PIN].name);
			goto gpio_init_err;
		} else {
			WMT_INFO_FUNC("PWN_PIN Get success\n");
			ret = 0;
...
static int wmt_gpio_init(VOID)
{
	struct device_node *node = wmt_device.dev.of_node;
	int ret = 0;
	...
	if (!gpio_is_valid(mt6630_gpio[PWN_PIN].num)) {
		WMT_ERR_FUNC("[6630-gpio] get invalid PWN_PIN: %d\n",
	...
	if (ret) {
		WMT_ERR_FUNC("[6630-gpio] gpio_request PWN_PIN(%d) failed(%d)\n",
				mt6630_gpio[PWN_PIN].num, ret);
		return ret;
	}
	ret = gpio_direction_output(mt6630_gpio[PWN_PIN].num, 0);
	if (ret) {
		WMT_ERR_FUNC("[6630-gpio] gpio_direction_output "
				"PWN_PIN(%d) failed(%d)\n",
				mt6630_gpio[PWN_PIN].num, ret);
		return ret;
	}
	...
	if (!gpio_is_valid(mt6630_gpio[RST_PIN].num)) {
		WMT_ERR_FUNC("[6630-gpio] get invalid RST_PIN: %d\n",
	...
	if (ret) {
		WMT_ERR_FUNC("[6630-gpio] gpio_request RST_PIN(%d) failed(%d)\n",
	...
	WMT_INFO_FUNC("[6630-gpio] PWN_PIN: %d RST_PIN: %d\n",
			mt6630_gpio[PWN_PIN].num, mt6630_gpio[RST_PIN].num);

	WMT_INFO_FUNC("end ret(%d)\n", ret);
...
static int WMT_init(void)
{
	...
	WMT_INFO_FUNC("WMT Version= %s DATE=%s PATCH_DATE=%s\n",
			MTK_WMT_VERSION, MTK_WMT_DATE, MTK_WMT_PATCH_DATE);
	WMT_INFO_FUNC("COMBO Driver Version= %s\n", MTK_COMBO_DRIVER_VERSION);

	/* WMT_INFO_FUNC("The times call wmt_probe = %d\n",wmt_probe_num); */
	...
	if (ret) {
		WMT_ERR_FUNC("fail to register chrdev,ret is %d\n", ret);
		WMT_ERR_FUNC("[chrdev-region-fail]gWmtInitDone is %d\n", gWmtInitDone);
		return ret;
	}
	...
	if (IS_ERR(wmt_class)) {
		WMT_ERR_FUNC("class_create fails\n");
		goto error;
	}
	wmt_dev = device_create(wmt_class, NULL, devID, NULL, "stpwmt");
	if (IS_ERR(wmt_dev)) {

		WMT_ERR_FUNC("device_create fails\n");
		goto error;
	}

	WMT_INFO_FUNC("Will call wmt_lib_init");
	ret = wmt_lib_init();
	if (ret) {
...
#endif				/*CONFIG_MTK_COMBO_COMM_APO end */
	gWmtInitDone = 1;
	WMT_INFO_FUNC("gWmtInitDone is %d\n", gWmtInitDone);
	wake_up(&gWmtInitWq);
#ifdef CONFIG_MTK_COMBO_COMM_APO
	...
	WMT_INFO_FUNC("Will call wmt_gpio_init");
	ret = wmt_gpio_init();
	/*
	if (ret) {
		WMT_ERR_FUNC("wmt_gpio_init() fails (%d)\n", ret);
		goto error;
	}
	*/
#endif
...
static int WMT_exit(void)
{
	...
	WMT_INFO_FUNC("destroy func_on_off_wake_lock\n");
...
static int wmt_probe(struct platform_device *dev)
{
	memcpy(&wmt_device, dev, sizeof(struct platform_device));
	return WMT_init();
}

static int wmt_remove(struct platform_device *dev)
{
	return WMT_exit();
}
...
static struct platform_driver wmt_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
#ifndef CONFIG_ATC_OS_VERSION_JB2
		.of_match_table = mt6630_of_ids,
#endif
	},
};

static int __init wmt_drv_init(void)
{
	int ret = -1;

#ifdef CONFIG_ATC_OS_VERSION_JB2
	ret = platform_device_register(&wmt_device);
	if (ret) {
		WMT_ERR_FUNC("platform_device_register failed!\n");
		return ret;
	}
#endif
	ret = platform_driver_register(&wmt_driver);
	if (ret) {
		WMT_ERR_FUNC("platform_driver_register failed!\n");
		return ret;
	}

	WMT_INFO_FUNC("Init %s OK!\n", WMT_DRIVER_NAME);

	/* ret = os_driver_register(&wmt_driver); */
	/* WMT_INFO_FUNC("wmtdrv_mod_init finish\n"); */

	return ret;
}

static void __exit wmt_drv_exit(void)
{
	/* unload driver first */
	platform_driver_unregister(&wmt_driver);
	/* unload device */
	platform_device_unregister(&wmt_device);
}
...
module_init(wmt_drv_init);
module_exit(wmt_drv_exit);
#endif
