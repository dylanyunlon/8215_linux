...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_PLAT-6630]"
	...
	if (0 != mergeIfSupport) {
		if (CMB_STUB_AIF_CTRL_EN == ctrl) {
			...
			if (NULL != wmt_plat_audio_if_cb) {
				#if 0 // PVS: pinShare always false
				iRet +=
...
irqreturn_t wmt_plat_bgf_eirq_cb (int irq, void *data)
{
	...
	if (NULL != wmt_plat_bgf_irq_cb) {
		...
		spin_lock_irqsave(&mt6630_irq_lock, irqflags);
		disable_irq_nosync(69);
		spin_unlock_irqrestore(&mt6630_irq_lock, irqflags);
		(*(wmt_plat_bgf_irq_cb)) ();
...
INT32 wmt_plat_pwr_ctrl(ENUM_FUNC_STATE state)
{
	...
	WMT_INFO_FUNC("wmt_plat_pwr_ctrl,state is %d!\n", state);
	switch (state) {
	case FUNC_ON:
		...
		if (DEFAULT_PIN_ID != gpio_ctrl_info.gpio_ctrl_state[GPIO_COMBO_BGF_EINT_PIN].gpio_num) {
			if (PIN_STA_INIT == state) {
				...
				if (node) {
					...
					WMT_ERR_FUNC("can't find BGF eint compatible node\n");
...
{
#if 0 // ifdef GPIO_COMBO_6620_LDO_EN_PIN
	if (DEFAULT_PIN_ID == gpio_ctrl_info.gpio_ctrl_state[GPIO_COMBO_LDO_EN_PIN].gpio_num) {
...
#else
		WMT_INFO_FUNC("host gps lna pin not defined!!!\n");
		WMT_INFO_FUNC("if you donot use eighter AP or MT6306's pin as GPS_LNA\n");
		WMT_INFO_FUNC("please customize your own GPS_LNA related code here\n");
#endif
/* } */
...
INT32 wmt_plat_wake_lock_ctrl(ENUM_WL_OP opId)
{
	...
	extern OSAL_SLEEPABLE_LOCK gOsSLock;
	extern OSAL_WAKE_LOCK wmtWakeLock;
	INT32 ret = 0;
	...
	if (WL_OP_GET == opId && counter == 1) {
		...
		WMT_DBG_FUNC("WMT-PLAT: after wake_lock(%d), counter(%d)\n",
				osal_wake_lock_count(&wmtWakeLock), counter);

	} else if (WL_OP_PUT == opId && counter == 0) {
		osal_wake_unlock(&wmtWakeLock);
		WMT_DBG_FUNC("WMT-PLAT: after wake_unlock(%d), counter(%d)\n",
				osal_wake_lock_count(&wmtWakeLock), counter);
	} else {
		WMT_INFO_FUNC("WMT-PLAT: wakelock status(%d), counter(%d)\n",
				osal_wake_lock_count(&wmtWakeLock), counter);
	}
	return 0;
#else
...
{
	WMT_INFO_FUNC("set gCommIfType to %d\n", type);
	gCommIfType = type;
	return 0;
}
