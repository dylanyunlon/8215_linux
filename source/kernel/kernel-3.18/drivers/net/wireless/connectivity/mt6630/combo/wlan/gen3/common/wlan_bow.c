...
const BOW_CMD_T arBowCmdTable[] = {
	{(UINT_8)BOW_CMD_ID_GET_MAC_STATUS, bowCmdGetMacStatus},
	{(UINT_8)BOW_CMD_ID_SETUP_CONNECTION, bowCmdSetupConnection},
	{(UINT_8)BOW_CMD_ID_DESTROY_CONNECTION, bowCmdDestroyConnection},
	{(UINT_8)BOW_CMD_ID_SET_PTK, bowCmdSetPTK},
	{(UINT_8)BOW_CMD_ID_READ_RSSI, bowCmdReadRSSI},
	{(UINT_8)BOW_CMD_ID_READ_LINK_QUALITY, bowCmdReadLinkQuality},
	{(UINT_8)BOW_CMD_ID_SHORT_RANGE_MODE, bowCmdShortRangeMode},
	{(UINT_8)BOW_CMD_ID_GET_CHANNEL_LIST, bowCmdGetChannelList},
};
			  ...
			  IN UINT_32 u4SetQueryInfoLen, IN PUINT_8 pucInfoBuffer, IN UINT_8 ucSeqNumber)
{
	...
	prCmdInfo->u4InformationBufferLength = 0U;
...
WLAN_STATUS wlanbowHandleCommand(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	for (i = 0U; i < sizeof(arBowCmdTable) / sizeof(BOW_CMD_T); i++) {
...
WLAN_STATUS bowCmdGetMacStatus(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	UINT_8 ucNumOfChannel = 0U;	/* MAX_BOW_NUMBER_OF_CHANNEL; */
	...
	/* fill event header */
	prEvent = (P_AMPC_EVENT) kalMemAlloc(
		(UINT_32)(sizeof(AMPC_EVENT) + sizeof(BOW_MAC_STATUS)),
		VIR_MEM_TYPE);

	if (prEvent == NULL)
		return WLAN_STATUS_FAILURE;

	prEvent->rHeader.ucEventId = (UINT_8)BOW_EVENT_ID_MAC_STATUS;
	prEvent->rHeader.ucSeqNumber = prCmd->rHeader.ucSeqNumber;
	prEvent->rHeader.u2PayloadLength = (UINT_16)sizeof(BOW_MAC_STATUS);

	/* fill event body */
	prMacStatus = (P_BOW_MAC_STATUS) (prEvent->aucPayload);
	kalMemZero(prMacStatus, (UINT_32)sizeof(BOW_MAC_STATUS));

	/* 3 <2> Call CNM to decide if BOW available. */
	if (cnmBowIsPermitted(prAdapter) == TRUE)
	...
	if (cnmPreferredChannel(prAdapter, &eBand,
		&ucPrimaryChannel, &eBssSCO) == TRUE) {
		DBGLOG(BOW, EVENT, "bowCmdGetMacStatus, Get preferred channel.\n");

		prMacStatus->ucNumOfChannel = 1U;
		prMacStatus->arChannelList[0].ucChannelBand = (UINT_8)eBand;
		...
		if (ucNumOfChannel > 0U) {
			for (idx = 0U; idx < ucNumOfChannel; idx++) {
				prMacStatus->arChannelList[idx].ucChannelBand =
					(UINT_8)aucChannelList[idx].eBand;
...
WLAN_STATUS bowCmdSetupConnection(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	if (bowCheckBowTableIfVaild(prAdapter,
		prBowSetupConnection->aucPeerAddress) == TRUE) {
		wlanbowCmdEventSetStatus(prAdapter, prCmd, BOWCMD_STATUS_UNACCEPTED);
		return WLAN_STATUS_NOT_ACCEPTED;
	}
	...
	rCmdBtOverWifi.cMaxTxPower = (UINT_8)prBowSetupConnection->cMaxTxPower;

	if (prBowSetupConnection->ucChannelNum > 14U)
		rCmdBtOverWifi.ucChannelBand = (UINT_8)BAND_5G;
	else
		rCmdBtOverWifi.ucChannelBand = (UINT_8)BAND_2G4;
	...
	if (bowGetBowTableFreeEntry(prAdapter, &ucBowTableIdx) == FALSE) {
		wlanbowCmdEventSetStatus(prAdapter, prCmd, BOWCMD_STATUS_UNACCEPTED);
		return WLAN_STATUS_NOT_ACCEPTED;
	}
...
WLAN_STATUS bowCmdSetPTK(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	if (bowCheckBowTableIfVaild(prAdapter,
		prBowSetPTK->aucPeerAddress) == FALSE) {
		wlanbowCmdEventSetStatus(prAdapter, prCmd, BOWCMD_STATUS_UNACCEPTED);
	...
	rCmdKey.ucKeyId = 0U;
	rCmdKey.ucKeyLen = 16U;	/* AES = 128bit */
...
WLAN_STATUS bowCmdReadRSSI(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	/* parameter size check */
	if (prCmd->rHeader.u2PayloadLength != (UINT_16)sizeof(BOW_READ_RSSI))
		return WLAN_STATUS_INVALID_LENGTH;
...
WLAN_STATUS bowCmdShortRangeMode(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	/* parameter size check */
	if (prCmd->rHeader.u2PayloadLength !=
		(UINT_16)sizeof(BOW_SHORT_RANGE_MODE)) {
		wlanbowCmdEventSetStatus(prAdapter, prCmd, BOWCMD_STATUS_UNACCEPTED);
		return WLAN_STATUS_INVALID_LENGTH;
	}

	if (bowCheckBowTableIfVaild(prAdapter,
		prBowShortRangeMode->aucPeerAddress) == FALSE) {
		wlanbowCmdEventSetStatus(prAdapter, prCmd, BOWCMD_STATUS_UNACCEPTED);
		return WLAN_STATUS_NOT_ACCEPTED;
	}
	...
	rTxPwrParam.cTxPwr2G4Cck = (prBowShortRangeMode->cTxPower * 2);

	rTxPwrParam.cTxPwr2G4OFDM_BPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4OFDM_QPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4OFDM_16QAM = (prBowShortRangeMode->cTxPower * 2);

	rTxPwrParam.cTxPwr2G4OFDM_48Mbps = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4OFDM_54Mbps = (prBowShortRangeMode->cTxPower * 2);

	rTxPwrParam.cTxPwr2G4HT20_BPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT20_QPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT20_16QAM = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT20_MCS5 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT20_MCS6 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT20_MCS7 = (prBowShortRangeMode->cTxPower * 2);

	rTxPwrParam.cTxPwr2G4HT40_BPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT40_QPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT40_16QAM = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT40_MCS5 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT40_MCS6 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr2G4HT40_MCS7 = (prBowShortRangeMode->cTxPower * 2);

	rTxPwrParam.cTxPwr5GOFDM_BPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GOFDM_QPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GOFDM_16QAM = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GOFDM_48Mbps = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GOFDM_54Mbps = (prBowShortRangeMode->cTxPower * 2);

	rTxPwrParam.cTxPwr5GHT20_BPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT20_QPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT20_16QAM = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT20_MCS5 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT20_MCS6 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT20_MCS7 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT40_BPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT40_QPSK = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT40_16QAM = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT40_MCS5 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT40_MCS6 = (prBowShortRangeMode->cTxPower * 2);
	rTxPwrParam.cTxPwr5GHT40_MCS7 = (prBowShortRangeMode->cTxPower * 2);
...
VOID wlanbowCmdEventSetStatus(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd, IN UINT_8 ucEventBuf)
{
	...
	/* fill event header */
	prEvent = (P_AMPC_EVENT) kalMemAlloc(
		(UINT_32)(sizeof(AMPC_EVENT) + sizeof(BOW_COMMAND_STATUS)),
		VIR_MEM_TYPE);
	if (prEvent == NULL)
		return;
	prEvent->rHeader.ucEventId = (UINT_8)BOW_EVENT_ID_COMMAND_STATUS;
	prEvent->rHeader.ucSeqNumber = prCmd->rHeader.ucSeqNumber;
	prEvent->rHeader.u2PayloadLength = (UINT_16)sizeof(BOW_COMMAND_STATUS);

	/* fill event body */
	prBowCmdStatus = (P_BOW_COMMAND_STATUS) (&prEvent->aucPayload[0]);
	kalMemZero(prBowCmdStatus, (UINT_32)sizeof(BOW_COMMAND_STATUS));
...
VOID wlanbowCmdEventSetCommon(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	prBowCmdStatus->ucStatus = (UINT_8)BOWCMD_STATUS_SUCCESS;
...
VOID wlanbowCmdEventLinkConnected(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	/* fill event header */
	prEvent = (P_AMPC_EVENT) kalMemAlloc(
		(UINT_32)(sizeof(AMPC_EVENT) + sizeof(BOW_LINK_CONNECTED)),
		VIR_MEM_TYPE);
	if (prEvent == NULL)
		return;
	prEvent->rHeader.ucEventId = (UINT_8)BOW_EVENT_ID_LINK_CONNECTED;
	prEvent->rHeader.ucSeqNumber = (UINT_8) prCmdInfo->u4PrivateData;
	prEvent->rHeader.u2PayloadLength = (UINT_16)sizeof(BOW_LINK_CONNECTED);

	/* fill event body */
	prBowLinkConnected = (P_BOW_LINK_CONNECTED) (prEvent->aucPayload);
	kalMemZero(prBowLinkConnected, (UINT_32)sizeof(BOW_LINK_CONNECTED));
	prBowLinkConnected->rChannel.ucChannelNum = prBssInfo->ucPrimaryChannel;
	prBowLinkConnected->rChannel.ucChannelBand = (UINT_8)prBssInfo->eBand;
...
VOID wlanbowCmdEventLinkDisconnected(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	/* fill event header */
	prEvent = (P_AMPC_EVENT) kalMemAlloc(
		(UINT_32)(sizeof(AMPC_EVENT) + sizeof(BOW_LINK_DISCONNECTED)),
		VIR_MEM_TYPE);
	if (prEvent == NULL)
		return;
	prEvent->rHeader.ucEventId = (UINT_8)BOW_EVENT_ID_LINK_DISCONNECTED;
	if ((prCmdInfo->u4PrivateData != 0U))
		prEvent->rHeader.ucSeqNumber = (UINT_8) prCmdInfo->u4PrivateData;
	else
		prEvent->rHeader.ucSeqNumber = 0U;
	...
	kalMemZero(prBowLinkDisconnected,
		(UINT_32)sizeof(BOW_LINK_DISCONNECTED));
	prBowLinkDisconnected->ucReason = 0x0U;
...
VOID wlanbowCmdEventReadLinkQuality(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	/* fill event header */
	prEvent = (P_AMPC_EVENT) kalMemAlloc(
		(UINT_32)(sizeof(AMPC_EVENT) + sizeof(BOW_LINK_QUALITY)),
		VIR_MEM_TYPE);
	if (prEvent == NULL)
		return;
	prEvent->rHeader.ucEventId = (UINT_8)BOW_EVENT_ID_LINK_QUALITY;
	prEvent->rHeader.ucSeqNumber = (UINT_8) prCmdInfo->u4PrivateData;
	prEvent->rHeader.u2PayloadLength = (UINT_16)sizeof(BOW_LINK_QUALITY);

	/* fill event body */
	prBowLinkQuality = (P_BOW_LINK_QUALITY) (prEvent->aucPayload);
	kalMemZero(prBowLinkQuality, (UINT_32)sizeof(BOW_LINK_QUALITY));
...
VOID wlanbowCmdEventReadRssi(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prEvent == NULL)
		return;
	prEvent->rHeader.ucEventId = (UINT_8)BOW_EVENT_ID_RSSI;
	prEvent->rHeader.ucSeqNumber = (UINT_8) prCmdInfo->u4PrivateData;
	prEvent->rHeader.u2PayloadLength = (UINT_16)sizeof(BOW_RSSI);

	/* fill event body */
	prBowRssi = (P_BOW_RSSI) (prEvent->aucPayload);
	kalMemZero(prBowRssi, (UINT_32)sizeof(BOW_RSSI));
...
VOID wlanbowCmdTimeoutHandler(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo)
{
	...
	prBowCmdStatus->ucStatus = (UINT_8)BOWCMD_STATUS_TIMEOUT;
	/* timeout */
...
VOID bowStarting(IN P_ADAPTER_T prAdapter)
{
	...
	if (g_u4LinkCount == 1U) {
		...
		if (prBowFsmInfo->fgSupportQoS == TRUE) {
...
VOID bowAssignSsid(IN PUINT_8 pucSsid, IN PUINT_8 puOwnMacAddr)
{
	...
	for (i = 0U; i < 6U; i++) {
		pucSsid[(3U * i) + 3U] = 0x2DU;
		if ((*(puOwnMacAddr + i) >> 4U) < 0xAU)
			*(pucSsid + (3U * i) + 4U) =
				(*(puOwnMacAddr + i) >> 4U) + 0x30U;
		else
			*(pucSsid + (3U * i) + 4U) =
				(*(puOwnMacAddr + i) >> 4U) + 0x57U;

		if ((*(puOwnMacAddr + i) & 0x0FU) < 0xAU)
			pucSsid[(3U * i) + 5U] =
				(*(puOwnMacAddr + i) & 0x0FU) + 0x30U;
		else
			pucSsid[(3U * i) + 5U] =
				(*(puOwnMacAddr + i) & 0x0FU) + 0x57U;
	}
}
...
BOOLEAN bowValidateProbeReq(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, OUT PUINT_32 pu4ControlFlags)
{
	...
	if (prBssInfo->eCurrentOPMode == OP_MODE_BOW && g_u4Beaconing > 0U) {
...
VOID bowSendBeacon(IN P_ADAPTER_T prAdapter, IN ULONG ulParamPtr)
{
	...
	if ((g_u4Beaconing != 0U) && (g_u4LinkCount > 0U)
...
VOID bowResponderScan(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(BOW, EVENT, "BOW SCAN [REQ:%d]\n",
		prBowFsmInfo->ucSeqNumOfScanReq + 1U);

	prScanReqMsg = (P_MSG_SCN_SCAN_REQ) cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
		(UINT_32)sizeof(MSG_SCN_SCAN_REQ));

	if (prScanReqMsg == NULL) {
		ASSERT(NULL);	/* Can't trigger SCAN FSM */
		return;
	}
	...
	prScanReqMsg->u2IELen = 0U;

	/*Send scan message */
	mboxSendMsg(prAdapter, MBOX_ID_0, &prScanReqMsg->rMsgHdr,
		MSG_SEND_METHOD_BUF);
	...
	do {
		...
		if (1U == 1U) {
			...
			prScanCancel =
			    ...
			    RAM_TYPE_MSG, (UINT_32)sizeof(MSG_SCN_SCAN_CANCEL));
			if (prScanCancel == NULL) {
...
VOID bowResponderJoin(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc)
{
	...
	prStaRec->ucTxAuthAssocRetryLimit = (UINT_8)TX_AUTH_ASSOCI_RETRY_LIMIT;
	...
	prJoinReqMsg = (P_MSG_JOIN_REQ_T) cnmMemAlloc(prAdapter,
		RAM_TYPE_MSG, (UINT_32)sizeof(MSG_JOIN_REQ_T));
	if (prJoinReqMsg == NULL) {

		ASSERT(NULL);	/* Can't trigger SAA FSM */
		return;
	}
	...
	mboxSendMsg(prAdapter, MBOX_ID_0, &prJoinReqMsg->rMsgHdr,
		MSG_SEND_METHOD_BUF);
}
				...
				IN ENUM_PARAM_MEDIA_STATE_T eConnectionState, IN BOOLEAN fgDelayIndication)
{
	...
	if (!fgDelayIndication) {
		...
		ASSERT_BOOLEAN(eConnectionState
...
WLAN_STATUS bowRunEventAAAComplete(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	wlanoidSendSetQueryBowCmd(prAdapter,
				  (UINT_8)CMD_ID_CMD_BT_OVER_WIFI,
				  prBowFsmInfo->ucBssIndex,
				  TRUE, FALSE, wlanbowCmdEventLinkConnected,
				  wlanbowCmdTimeoutHandler, 0U, NULL, 0U);
...
WLAN_STATUS bowRunEventRxDeAuth(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prStaRec->ucStaState > STA_STATE_1) {
		...
		wlanoidSendSetQueryBowCmd(prAdapter,
			...
			prBowFsmInfo->ucBssIndex,
			TRUE,
			FALSE, wlanbowCmdEventLinkDisconnected,
			wlanbowCmdTimeoutHandler,
			0U, NULL, 0U);
...
VOID bowDisconnectLink(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	/*No one connected */
	if (g_u4LinkCount == 0U && g_u4Beaconing != 0U) {
	...
	*pu2StatusCode = STATUS_CODE_REQ_DECLINED;
	...
	while (EQUAL_MAC_ADDR(prAssocReqFrame->aucSrcAddr, prBowFsmInfo->aucPeerAddress)) {
		...
		if (prStaRec->u2DesiredNonHTRateSet == 0U) {
			...
			if (rateGetHighestRateIndexFromRateSet(
				...
				&ucHighestRateIndex) == TRUE)
				prStaRec->u2DesiredNonHTRateSet =
					(UINT_16)BIT(ucHighestRateIndex);
			else {
	...
	if (*pu2StatusCode != STATUS_CODE_SUCCESSFUL) {
		...
		if (CHECK_FOR_TIMEOUT(rCurrentTime, rLastRejectAssocTime,
			MSEC_TO_SYSTIME(5000U)) ||
			rLastRejectAssocTime == 0U) {
			fgReplyAssocResp = TRUE;
...
VOID bowRequestCh(IN P_ADAPTER_T prAdapter)
{
	...
	if (prBowFsmInfo->fgIsChannelGranted == FALSE) {
		...
		DBGLOG(BOW, EVENT, "BoW channel [REQUEST:%d], %d, %d.\n",
				    prBowFsmInfo->ucSeqNumOfChReq + 1U,
				    prBowFsmInfo->ucPrimaryChannel, prBowFsmInfo->eBand);

		prMsgChReq = (P_MSG_CH_REQ_T)
			cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
			(UINT_32)sizeof(MSG_CH_REQ_T));
		...
		prMsgChReq->eRfChannelWidth = CW_20_40MHZ;
		prMsgChReq->ucRfCenterFreqSeg1 = 0U;
		prMsgChReq->ucRfCenterFreqSeg2 = 0U;

		prBowFsmInfo->fgIsChannelRequested = TRUE;

		mboxSendMsg(prAdapter, MBOX_ID_0, &prMsgChReq->rMsgHdr,
			MSG_SEND_METHOD_BUF);
	}
}
...
VOID bowReleaseCh(IN P_ADAPTER_T prAdapter)
{
	...
	if (prBowFsmInfo->fgIsChannelGranted != FALSE || prBowFsmInfo->fgIsChannelRequested != FALSE) {
		...
		RAM_TYPE_MSG, (UINT_32)sizeof(MSG_CH_ABORT_T));
		if (prMsgChAbort == NULL) {
			ASSERT(NULL);	/* Can't release Channel to CNM */
			return;
		}
		...
		mboxSendMsg(prAdapter, MBOX_ID_0, &prMsgChAbort->rMsgHdr,
			MSG_SEND_METHOD_BUF);
	}
...
BOOLEAN bowNotifyAllLinkDisconnected(IN P_ADAPTER_T prAdapter)
{
	...
	kalMemZero(&rCmdInfo, (UINT_32)sizeof(CMD_INFO_T));

	while (ucBowTableIdx < (UINT_8)CFG_BOW_PHYSICAL_LINK_NUM) {
		if (arBowTable[ucBowTableIdx].fgIsValid == TRUE) {
...
BOOLEAN bowCheckBowTableIfVaild(IN P_ADAPTER_T prAdapter, IN UINT_8 aucPeerAddress[6])
{
	...
	for (idx = 0U; idx < CFG_BOW_PHYSICAL_LINK_NUM; idx++) {
		if (arBowTable[idx].fgIsValid == TRUE &&
...
BOOLEAN bowGetBowTableFreeEntry(IN P_ADAPTER_T prAdapter, OUT PUINT_8 pucBowTableIdx)
{
	...
	for (idx = 0U; idx < CFG_BOW_PHYSICAL_LINK_NUM; idx++) {
		if (arBowTable[idx].fgIsValid == FALSE) {
			DBGLOG(BOW, EVENT,
...
ENUM_BOW_DEVICE_STATE bowGetBowTableState(IN P_ADAPTER_T prAdapter, IN UINT_8 aucPeerAddress[6])
{
	...
	for (idx = 0U; idx < (UINT_8)CFG_BOW_PHYSICAL_LINK_NUM; idx++) {
		if (arBowTable[idx].fgIsValid == TRUE &&
			EQUAL_MAC_ADDR(arBowTable[idx].aucPeerAddress,
			aucPeerAddress)) {
			DBGLOG(BOW, EVENT,
...
BOOLEAN bowSetBowTableState(IN P_ADAPTER_T prAdapter, IN UINT_8 aucPeerAddress[6], IN ENUM_BOW_DEVICE_STATE eState)
{
	...
	if (bowGetBowTableEntryByPeerAddress(prAdapter,
		aucPeerAddress, &ucBowTableIdx) == TRUE) {
		KAL_SPIN_LOCK_DECLARATION();
		KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_BOW_TABLE);
