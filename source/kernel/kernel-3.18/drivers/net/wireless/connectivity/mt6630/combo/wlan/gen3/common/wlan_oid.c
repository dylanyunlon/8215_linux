...
static VOID SetTestChannel(UINT_8 *pucPrimaryChannel)
{
	if (*pucPrimaryChannel < 5U)
		*pucPrimaryChannel = 8U;
	else if (*pucPrimaryChannel > 10U)
		*pucPrimaryChannel = 3U;
}
#endif
				  ...
				  OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	/* Init. */
	for (u4NumItem = 0U; u4NumItem < PARAM_NETWORK_TYPE_NUM; u4NumItem++)
		eSupportedNetworks[u4NumItem] = PARAM_NETWORK_TYPE_FH;
	...
	*pu4QueryInfoLen =
	...
	kalMemCopy(&prSupported->eNetworkType[0],
		&eSupportedNetworks[0],
		u4NumItem * sizeof(ENUM_PARAM_NETWORK_TYPE_T));
			     ...
			     OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	if (u4QueryBufferLen < sizeof(ENUM_PARAM_NETWORK_TYPE_T)) {
		*pu4QueryInfoLen = (UINT_32)sizeof(ENUM_PARAM_NETWORK_TYPE_T);
			   ...
			   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (u4SetBufferLen < sizeof(ENUM_PARAM_NETWORK_TYPE_T)) {
		*pu4SetInfoLen = (UINT_32)sizeof(ENUM_PARAM_NETWORK_TYPE_T);
		return WLAN_STATUS_INVALID_LENGTH;
	}
		  ...
		  OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	ASSERT_BOOLEAN(u4QueryBufferLen >= MAC_ADDR_LEN);
	if (u4QueryBufferLen != 0U)
		ASSERT(pvQueryBuffer);
	ASSERT(pu4QueryInfoLen);

	if (kalGetMediaStateIndicated(prAdapter->prGlueInfo) == PARAM_MEDIA_STATE_CONNECTED)
		kalMemCopy((PUINT_8)pvQueryBuffer, (PUINT_8)
			&prAdapter->rWlanInfo.rCurrBssId.arMacAddress[0],
			MAC_ADDR_LEN);
	else if (prAdapter->rWifiVar.rConnSettings.eOPMode == NET_TYPE_IBSS) {
		PARAM_MAC_ADDRESS aucTemp;	/*!< BSSID */

		COPY_MAC_ADDR(&aucTemp[0],
			&prAdapter->rWlanInfo.rCurrBssId.arMacAddress[0]);
		aucTemp[0] &= (UINT_8)~BIT(0U);
		aucTemp[1] |= (UINT_8)BIT(1U);
		COPY_MAC_ADDR((PUINT_8)pvQueryBuffer, &aucTemp[0]);
	} else
		      ...
		      OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	if (u4BssidListExLen != 0U)
		u4BssidListExLen += 4U;	/* u4NumberOfItems. */
	else
		u4BssidListExLen = (UINT_32)sizeof(PARAM_BSSID_LIST_EX_T);
	...
	*pu4QueryInfoLen = u4BssidListExLen;
	...
	if (prAdapter->fgIsRadioOff == FALSE &&
		prAdapter->rWlanInfo.u4ScanResultNum > 0U) {
		/* fill up for each entry */
		for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
			prBssidEx = (P_PARAM_BSSID_EX_T) cp;

			/* copy structure */
			kalMemCopy((PUINT_8)prBssidEx, (PUINT_8)
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0U;
	...
	if (pvSetBuffer != NULL && u4SetBufferLen != 0U) {
		COPY_SSID((PUINT_8)&rSsid.aucSsid[0], rSsid.u4SsidLen,
			   ...
			   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (prAdapter->rAcpiState == ACPI_STATE_D3) {
		DBGLOG(OID, ERROR,
		       ...
		       prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
		return WLAN_STATUS_ADAPTER_NOT_READY;
	} else if (prAdapter->fgTestMode != FALSE) {
...
*/
/*----------------------------------------------------------------------------*/
static BOOLEAN wlanoidGetChannelInfo(IN P_ADAPTER_T prAdapter,
	IN PUINT_8 puPartialScanReq)
{
	...
	UINT_32 i = 0;
	UINT_32 j = 0;
	UINT_8 channel_num = 0U;
	UINT_8 channel_counts = 0U;
	...
	if ((scan_req_t->n_channels != 0)) {
		...
		channel_counts = (UINT_8)scan_req_t->n_channels;
		...
		while (j < channel_counts) {
			...
			channel_num = (UINT_8)nicFreq2ChannelNum(
				(UINT_32)channel_tmp->center_freq * 1000U);
	...
	if (i > 0U) {
		PartialScanChannel->ucChannelListNum = (UINT_8)i;
...
wlanoidSetBssid(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = MAC_ADDR_LEN;
	...
	for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
		if (EQUAL_MAC_ADDR((PUINT_8)
			&prAdapter->rWlanInfo.arScanResult[i].arMacAddress[0],
			(PUINT_8)pAddr))
			break;
	}
	...
	prAdapter->rWifiVar.rConnSettings.ucSSIDLen = 0U;
	prAdapter->rWifiVar.rConnSettings.aucSSID[0] = (UINT_8)'\0';
	COPY_MAC_ADDR(
		(PUINT_8) &prAdapter->rWifiVar.rConnSettings.aucBSSID[0],
		(PUINT_8)pAddr);

	if (EQUAL_MAC_ADDR(
		(PUINT_8)&prAdapter->rWlanInfo.rCurrBssId.arMacAddress[0],
		(PUINT_8)pAddr))
...
wlanoidSetSsid(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	UINT_32 i;
	INT_32 i4MaxRSSI = (INT_32)INT_MIN;
	P_MSG_AIS_ABORT_T prAisAbortMsg;
	...
	if (kalGetMediaStateIndicated(prGlueInfo) == PARAM_MEDIA_STATE_CONNECTED) {
		if (EQUAL_SSID(
			&prAdapter->rWlanInfo.rCurrBssId.rSsid.aucSsid[0],
			prAdapter->rWlanInfo.rCurrBssId.rSsid.u4SsidLen,
			&pParamSsid->aucSsid[0], pParamSsid->u4SsidLen)) {
			kalSetMediaStateIndicated(prGlueInfo, PARAM_MEDIA_STATE_TO_BE_INDICATED);
		} else
			kalIndicateStatusAndComplete(prGlueInfo, WLAN_STATUS_MEDIA_DISCONNECT, NULL, 0);
	}
	...
	for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
		...
		if (EQUAL_SSID((PUINT_8)aucSsid, (UINT_32)ucSsidLength,
			(PUINT_8) &pParamSsid->aucSsid[0],
	...
	if (prAdapter->rWifiVar.fgSupportWZCDisassociation != FALSE) {
		if (pParamSsid->u4SsidLen == ELEM_MAX_LEN_SSID) {
			fgIsValidSsid = FALSE;

			for (i = 0; i < ELEM_MAX_LEN_SSID; i++) {
				if (!((pParamSsid->aucSsid[i] > 0U)
				      && (pParamSsid->aucSsid[i] <= 0x1FU))) {
					fgIsValidSsid = TRUE;
					break;
				}
			}
	...
	/* Set Connection Request Issued Flag */
	if (fgIsValidSsid != FALSE) {
		prAdapter->rWifiVar.rConnSettings.fgIsConnReqIssued = TRUE;

		if (pParamSsid->u4SsidLen != 0U)
	...
	if (EQUAL_SSID(&prAdapter->rWlanInfo.rCurrBssId.rSsid.aucSsid[0],
			prAdapter->rWlanInfo.rCurrBssId.rSsid.u4SsidLen,
			&pParamSsid->aucSsid[0], pParamSsid->u4SsidLen)) {
		prAisAbortMsg->fgDelayIndication = TRUE;
	} else {
...
wlanoidSetConnect(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (pParamConn->pucBssid != NULL) {
		if (!EQUAL_MAC_ADDR((PUINT_8) &aucZeroMacAddr[0],
			pParamConn->pucBssid) &&
	...
	if (prAdapter->rWifiVar.fgSupportWZCDisassociation != FALSE) {
		if (pParamConn->u4SsidLen == ELEM_MAX_LEN_SSID) {
			...
			for (i = 0; i < ELEM_MAX_LEN_SSID; i++) {
				if (!((pParamConn->pucSsid[i] > 0U)
				      && (pParamConn->pucSsid[i] <= 0x1FU))) {
					fgIsValidSsid = TRUE;
					break;
				}
			}
	...
	if (fgEqualSsid == TRUE || fgEqualBssid == TRUE)
		prAisAbortMsg->fgDelayIndication = TRUE;
	else
		/* Update the information to CONNECTION_SETTINGS_T */
		prAisAbortMsg->fgDelayIndication = FALSE;
		 ...
		 OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_SSID_T);
			       ...
			       OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(ENUM_PARAM_OP_MODE_T);
...
/* UINT_8 i; */
	...
	*pu4SetInfoLen = (UINT_32)sizeof(ENUM_PARAM_OP_MODE_T);
...
#if CFG_SUPPORT_WAPI
	prAdapter->prGlueInfo->u2WapiAssocInfoIESz = 0U;
	...
	prAdapter->prGlueInfo->u2WSCAssocInfoIELen = 0U;
#endif
#endif
	...
	prAdapter->prAisBssInfo->ucTxDefaultKeyID = 0U;
	prAdapter->prAisBssInfo->ucCurrentGtkId = 0U;
		     ...
		     OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(ENUM_PARAM_AUTH_MODE_T);
		   ...
		   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(ENUM_PARAM_AUTH_MODE_T);
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(ENUM_PARAM_PRIVACY_FILTER_T);
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	ASSERT(pu4SetInfoLen);
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_RELOAD_DEFAULTS);
	...
	case ENUM_RELOAD_WEP_KEYS:
		...
		prAdapter->rWifiVar.rConnSettings.eEncStatus = ENUM_ENCRYPTION1_KEY_ABSENT;
		{
			...
			prCmdInfo->ucCID = (UINT_8)CMD_ID_ADD_REMOVE_KEY;
			prCmdInfo->fgSetQuery = TRUE;
			prCmdInfo->fgNeedResp = FALSE;
			prCmdInfo->fgDriverDomainMCR = FALSE;
			prCmdInfo->ucCmdSeqNum = ucCmdSeqNum;
			prCmdInfo->u4SetInfoLen =
				(UINT_32)sizeof(PARAM_REMOVE_KEY_T);
			...
			kalMemZero(prCmdKey, (UINT_32) sizeof(CMD_802_11_KEY));

			prCmdKey->ucAddRemove = 0U;	/* Remove */
			prCmdKey->ucKeyId = 0U;
			/* (UINT_8)(prRemovedKey->u4KeyIndex & 0x000000ff); */
			kalMemCopy((PUINT_8) &prCmdKey->aucPeerAddr[0],
				(PUINT_8) &aucBCAddr[0], MAC_ADDR_LEN);

			ASSERT_BOOLEAN(prCmdKey->ucKeyId < MAX_KEY_NUM);

			prCmdKey->ucKeyType = 0U;
	...
	wlanoidSetNetworkTypeInUse(prAdapter, &eNetworkType,
		(UINT_32)sizeof(eNetworkType), (PUINT_32)&u4Length);
...
wlanoidSetAddWep(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32) OFFSET_OF(PARAM_WEP_T, aucKeyMaterial);
	...
	if (u4SetBufferLen < OFFSET_OF(PARAM_WEP_T, aucKeyMaterial) + prNewWepKey->u4KeyLength) {
		...
		*pu4SetInfoLen =
			(UINT_32) OFFSET_OF(PARAM_WEP_T, aucKeyMaterial);
		return WLAN_STATUS_INVALID_DATA;
	}
	...
	if (prNewWepKey->u4KeyLength > 16U /* LEGACY_KEY_MAX_LEN */) {
		DBGLOG(OID, WARN,
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	/* Transmit key */
	if ((prNewWepKey->u4KeyIndex & IS_TRANSMIT_KEY) != 0U)
		prParamKey->u4KeyIndex |= (UINT_32)IS_TRANSMIT_KEY;

	/* Per client key */
	if ((prNewWepKey->u4KeyIndex & IS_UNICAST_KEY) != 0U)
		prParamKey->u4KeyIndex |= (UINT_32) IS_UNICAST_KEY;

	prParamKey->u4KeyLength = prNewWepKey->u4KeyLength;

	kalMemCopy((PUINT_8) &prParamKey->arBSSID[0],
		(PUINT_8) &aucBCAddr[0], MAC_ADDR_LEN);

	kalMemCopy(prParamKey->aucKeyMaterial, prNewWepKey->aucKeyMaterial, prNewWepKey->u4KeyLength);

	prParamKey->u4Length =
		(UINT_32) (OFFSET_OF(PARAM_KEY_T, aucKeyMaterial)
		+ prNewWepKey->u4KeyLength);

	rStatus = wlanoidSetAddKey(prAdapter, (PVOID) prParamKey,
		prParamKey->u4Length, &u4SetLen);
	if (rStatus != WLAN_STATUS_SUCCESS)
		DBGLOG(OID, TRACE, "wlanoidSetAddKey:%d\n", rStatus);
		    ...
		    IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_KEY_INDEX);
	...
	kalMemCopy((PUINT_8) &rRemoveKey.arBSSID[0],
		(PUINT_8) &aucBCAddr[0], MAC_ADDR_LEN);

	rStatus = wlanoidSetRemoveKey(prAdapter, (PVOID)&rRemoveKey,
		(UINT_32)sizeof(PARAM_REMOVE_KEY_T), &u4SetLen);
	if (rStatus != WLAN_STATUS_SUCCESS)
		DBGLOG(OID, TRACE, "wlanoidSetRemoveKey:%d\n", rStatus);
...
wlanoidSetAddKey(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	BOOL fgWpa3ReconnectWithOpenAuth = FALSE;
	uint32_t i = 0;
#endif
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	if ((prAdapter->rWifiVar.rConnSettings.eAuthMode == AUTH_MODE_OPEN) &&
		(rsnSearchAKMSuite(prAdapter, RSN_CIPHER_SUITE_SAE, &i) == TRUE)) {
		fgWpa3ReconnectWithOpenAuth = TRUE;
		DBGLOG(OID, TRACE, "Notice WPA3 reconnect with OPEN Auth\n");
	}
#endif
#if CFG_SUPPORT_RECONNECT_WPA3_WITH_PMKSA
	if ((fgWpa3ReconnectWithOpenAuth == TRUE) ||
	...
	prGlueInfo = prAdapter->prGlueInfo;
	prCmdInfo = cmdBufAllocateCmdInfo(prAdapter,
		(UINT_32)(CMD_HDR_SIZE + sizeof(CMD_802_11_KEY)));
	...
	prCmdInfo->u2InfoBufLen =
		(UINT_16)(CMD_HDR_SIZE + sizeof(CMD_802_11_KEY));
	prCmdInfo->pfCmdDoneHandler = nicCmdEventSetCommon;
	prCmdInfo->pfCmdTimeoutHandler = nicOidCmdTimeoutCommon;
	prCmdInfo->fgIsOid = TRUE;
	prCmdInfo->ucCID = (UINT_8)CMD_ID_ADD_REMOVE_KEY;
...
#endif
	if (prNewKey->u4KeyLength == 5U) {
		prCmdKey->ucAlgorithmId = CIPHER_SUITE_WEP40;
	} else if (prNewKey->u4KeyLength == 13U) {
		prCmdKey->ucAlgorithmId = CIPHER_SUITE_WEP104;
	} else if (prNewKey->u4KeyLength == 16U) {
	...
	} else if (prNewKey->u4KeyLength == 32U) {
...

	{
...
#endif
		if (prCmdKey->ucTxKey != 0U) {
			if (prStaRec != NULL) {
				if (prCmdKey->ucKeyType != 0U) {
					/* AIS RSN STA */
			...
			if ((prCmdKey->aucPeerAddr[0] &
				...
				prCmdKey->aucPeerAddr[4] &
				prCmdKey->aucPeerAddr[5]) == 0xFFU) {
				prCmdKey->ucWlanIndex = 255U; /* AIS WEP */
			} else {
				if (prStaRec != NULL) {
					/* AIS RSN Group key, addr is BSSID */
		...
		if (prCmdKey->ucWlanIndex < WTBL_SIZE) {
			...
			if (prCmdKey->ucTxKey) {
				if (prStaRec) {
					prWtblCfg[entry].fgRCA2 = 1U;
					prWtblCfg[entry].fgRV = 1U;
					prWtblCfg[entry].fgIKV = 0U;
					prWtblCfg[entry].fgRKV = 1U;
					if (fgOneTb3 == TRUE)
						prWtblCfg[entry].fgRCID = 0U;
					else
						prWtblCfg[entry].fgRCID = 1U;
					prWtblCfg[entry].ucKeyID = prCmdKey->ucKeyId;
					prWtblCfg[entry].fgRCA1 = 1U;
#if 0
					if (prCmdKey->ucIsAuthenticator)
						prWtblCfg[entry].fgEvenPN = 0;
					else
#endif
						prWtblCfg[entry].fgEvenPN = 1U;
					prWtblCfg[entry].ucMUARIdx = 0x00U;
				} else {
					prWtblCfg[entry].fgRCA2 = 1U;
					prWtblCfg[entry].fgRV = 1U;
					prWtblCfg[entry].fgIKV = 0U;
					...
					SetRCID(fgOneTb3,
						&prWtblCfg[entry].fgRCID);
					prWtblCfg[entry].ucKeyID =
						prCmdKey->ucKeyId;
					prWtblCfg[entry].fgRCA1 = 0U;
					prWtblCfg[entry].fgEvenPN = 1U;
				}
			} else {
				prWtblCfg[entry].fgRCA2 = 1U;
				...
				prWtblCfg[entry].ucKeyID = prCmdKey->ucKeyId;
				prWtblCfg[entry].fgRCA1 = 1U;
				prWtblCfg[entry].ucMUARIdx = 0x30U;
#if 0
				if (prCmdKey->ucIsAuthenticator)
					prWtblCfg[entry].fgEvenPN = 0U;
				else
#endif
					prWtblCfg[entry].fgEvenPN = 1U;
			}
		    ...
		    IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_REMOVE_KEY_T);
	...
	prCmdInfo->fgDriverDomainMCR = FALSE;
	prCmdInfo->ucCmdSeqNum = ucCmdSeqNum;
	prCmdInfo->u4SetInfoLen = (UINT_32)sizeof(PARAM_REMOVE_KEY_T);
	prCmdInfo->pvInformationBuffer = pvSetBuffer;
	prCmdInfo->u4InformationBufferLength = u4SetBufferLen;
	...
	kalMemZero((P_CMD_802_11_KEY)prCmdKey, sizeof(CMD_802_11_KEY));

	prCmdKey->ucAddRemove = 0U;	/* Remove */
	prCmdKey->ucKeyId = (UINT_8) (prRemovedKey->u4KeyIndex & 0x000000ffU);
...
#if CFG_SUPPORT_802_11W
	ASSERT_BOOLEAN(prCmdKey->ucKeyId < MAX_KEY_NUM + 2);
#else
	...
	if ((prRemovedKey->u4KeyIndex & IS_UNICAST_KEY) != 0U) {
		if (prStaRec != NULL) {
			prCmdKey->ucKeyType = 1U;
			prCmdKey->ucWlanIndex = prStaRec->ucWlanIndex;
...
{
	BOOLEAN fgTransmitKeyAvailable;
			   ...
			   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(ENUM_PARAM_ENCRYPTION_STATUS_T);
		       ...
		       OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen =
		(UINT_32)(4U * sizeof(UINT_32) +
		14U * sizeof(PARAM_AUTH_ENCRYPTION_T));
	...
	prCap->u4Version = 2U;	/* WPA2 */
	prCap->u4NoOfPMKIDs = CFG_MAX_PMKID_CACHE;
	prCap->u4NoOfAuthEncryptPairsSupported = 14U;
		  ...
		  OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32) (OFFSET_OF(PARAM_PMKID_T, arBSSIDInfo) +
	...
	for (i = 0U; i < prAisSpecBssInfo->u4PmkidCacheCount; i++) {
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanoidSetPmkid(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer,
			   ...
			   OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_RATES_EX);
	...
	kalMemCopy((PUINT_8) pvQueryBuffer,
		(PUINT_8) &eRate, sizeof(PARAM_RATES));
			 ...
			 OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_RATES_EX);
	...
	kalMemCopy((PUINT_8) pvQueryBuffer,
		(PUINT_8) &(prAdapter->rWlanInfo.eDesiredRates),
		sizeof(PARAM_RATES));
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_RATES);

	if (u4SetBufferLen < sizeof(PARAM_RATES))
		return WLAN_STATUS_INVALID_LENGTH;

	kalMemCopy((PUINT_8) &(prAdapter->rWlanInfo.eDesiredRates),
		(PUINT_8)pvSetBuffer, sizeof(PARAM_RATES));

	prAdapter->rWlanInfo.eLinkAttr.ucDesiredRateLen = PARAM_MAX_LEN_RATES;
	for (i = 0U; i < PARAM_MAX_LEN_RATES; i++)
		prAdapter->rWlanInfo.eLinkAttr.u2DesiredRate[i] = (UINT_16) (prAdapter->rWlanInfo.eDesiredRates[i]);

	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_LINK_ATTRIB,
				   TRUE,
				   FALSE,
				   TRUE,
				   nicCmdEventSetCommon,
				   nicOidCmdTimeoutCommon,
				   (UINT_32)sizeof(CMD_LINK_ATTRIB),
			 ...
			 OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	if (u4QueryBufferLen < sizeof(UINT_32)) {
		*pu4QueryInfoLen = (UINT_32)sizeof(UINT_32);
		return WLAN_STATUS_INVALID_LENGTH;
	}
		     ...
		     OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	kalMemCopy((PUINT_8) pvQueryBuffer,
		(PUINT_8) &prAdapter->aucMacAddress[0], 3U);
	*((PUINT_8) pvQueryBuffer + 3U) = 1U;
	*pu4QueryInfoLen = (UINT_32)sizeof(UINT_32);
		   ...
		   OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_RSSI_MONITOR_T);
	...
	kalMemCopy(&rRssi, (P_PARAM_RSSI_MONITOR_T)pvQueryBuffer,
		sizeof(PARAM_RSSI_MONITOR_T));
	if (rRssi.enable != FALSE) {
	...
	return wlanSendSetQueryCmd(prAdapter,
			(UINT_8)CMD_ID_RSSI_MONITOR,
			TRUE,
			FALSE,
			TRUE,
			nicCmdEventSetCommon,
			nicOidCmdTimeoutCommon,
			(UINT_32)sizeof(PARAM_RSSI_MONITOR_T),
		 ...
		 OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_RSSI);
	...
	if (kalGetMediaStateIndicated(prAdapter->prGlueInfo) == PARAM_MEDIA_STATE_DISCONNECTED) {
		...
		(rCurSysTime - prAdapter->rLinkQualityUpdateTime) <=
		...
		kalMemCopy((PARAM_RSSI *)pvQueryBuffer,
			&rRssi, sizeof(PARAM_RSSI));
		return WLAN_STATUS_SUCCESS;
	}
#ifdef LINUX
	return wlanSendSetQueryCmd(prAdapter,
		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_RSSI);
			   ...
			   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (u4SetBufferLen < sizeof(UINT_32)) {
		*pu4SetInfoLen = (UINT_32)sizeof(UINT_32);
		return WLAN_STATUS_INVALID_LENGTH;
	}
		     ...
		     IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	if (prAdapter->rAcpiState == ACPI_STATE_D3) {
		...
		*pu4QueryInfoLen = (UINT_32)sizeof(UINT_64);
		return WLAN_STATUS_INVALID_LENGTH;
	}
#if CFG_ENABLE_STATISTICS_BUFFERING
		       ...
		       IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_802_11_STATISTICS_STRUCT_T);
	...
	if (IsBufferedStatisticsUsable(prAdapter) == TRUE) {
		...
		*pu4QueryInfoLen =
			(UINT_32)sizeof(PARAM_802_11_STATISTICS_STRUCT_T);
			    ...
			    IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(ENUM_MEDIA_STREAM_MODE);
			  ...
			  IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(ENUM_MEDIA_STREAM_MODE);
	...
	if (eStreamMode == ENUM_MEDIA_STREAM_OFF)
		...
		prAdapter->rWlanInfo.eLinkAttr.ucMediaStreamMode = 1U;
			  ...
			  IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	COPY_MAC_ADDR((PUINT_8) pvQueryBuffer,
		(PUINT_8) &prAdapter->rWifiVar.aucPermanentAddress[0]);
	*pu4QueryInfoLen = MAC_ADDR_LEN;
			...
			IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	COPY_MAC_ADDR((PUINT_8) pvQueryBuffer,
		(PUINT_8) &prAdapter->rWifiVar.aucMacAddress[0]);
	*pu4QueryInfoLen = MAC_ADDR_LEN;
		      ...
		      IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(UINT_32);
	...
	if (kalGetMediaStateIndicated(prAdapter->prGlueInfo) != PARAM_MEDIA_STATE_CONNECTED) {
		...
		(rCurSysTime - prAdapter->rLinkRateUpdateTime) <=
		CFG_LINK_QUALITY_VALID_PERIOD) {
		*(PUINT_32) pvQueryBuffer =
			(UINT_32)prAdapter->rLinkQuality.u2LinkSpeed * 5000U;
		    ...
		    IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_MCR_RW_STRUCT_T);
...
#if CFG_SUPPORT_SWCR
	if ((prMcrRdInfo->u4McrOffset >> 16U) == 0x9F00U) {
		swCrReadWriteCmd(prAdapter,
			SWCR_READ,
			(UINT_16)(prMcrRdInfo->u4McrOffset & BITS(0U, 15U)),
	...
	if ((prMcrRdInfo->u4McrOffset & 0xFFFF0000U) != 0U) {
#if defined(MT6797)
		UINT32 val = 0x77777777U;
#endif
		/* fill command */
		rCmdAccessReg.u4Address = prMcrRdInfo->u4McrOffset;
		rCmdAccessReg.u4Data = 0U;
#if defined(MT6797)
		if ((prMcrRdInfo->u4McrOffset & 0xFFFF0000U) == 0x180f0000U) {
...
#endif
			return wlanSendSetQueryCmd(prAdapter,
					(UINT_8)CMD_ID_ACCESS_REG,
					FALSE,
					TRUE,
					TRUE,
					nicCmdEventQueryMcrRead,
					nicOidCmdTimeoutCommon,
					(UINT_32)sizeof(CMD_ACCESS_REG),
					(PUINT_8) &rCmdAccessReg,
					pvQueryBuffer,
					u4QueryBufferLen);
	} else {
		HAL_MCR_RD(prAdapter,
			(UINT_32) (prMcrRdInfo->u4McrOffset & BITS(2U, 31U)),
			&prMcrRdInfo->u4McrData);
		   ...
		   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_MCR_RW_STRUCT_T);
	...
	if (prMcrWrInfo->u4McrOffset == 0xFFFFFFFEU) {
	...
	else if (prMcrWrInfo->u4McrOffset == 0xFFFFFFFDU) {
	...
	/* 0xFFFFFFFFC for Control RF Band and SCO */
	else if (prMcrWrInfo->u4McrOffset == 0xFFFFFFFCU) {
		/* Band */
		if (prMcrWrInfo->u4McrData & 0x80000000U) {
			/* prBssInfo->eBand = BAND_5G; */
			/* prBssInfo->ucPrimaryChannel = 52U; */
		} else {
			prBssInfo->eBand = BAND_2G4;
			prBssInfo->ucPrimaryChannel = 8U;
		}

		/* Bandwidth */
		if (prMcrWrInfo->u4McrData & 0x00010000U) {
			prStaRec->u2HtCapInfo |= HT_CAP_INFO_SUP_CHNL_WIDTH;
			prStaRec->ucDesiredPhyTypeSet = PHY_TYPE_BIT_HT;

			if (prMcrWrInfo->u4McrData == 0x00010002U) {
				prBssInfo->eBssSCO = CHNL_EXT_SCB;	/* U20 */
				prBssInfo->ucPrimaryChannel += 2U;
			} else if (prMcrWrInfo->u4McrData == 0x00010001U) {
		...
		if (prMcrWrInfo->u4McrData & 0x00000000U) {
			prStaRec->u2HtCapInfo &= ~HT_CAP_INFO_SUP_CHNL_WIDTH;
	...
	else if (prMcrWrInfo->u4McrOffset == 0xFFFFFFFBU) {
		/* Enable HT Capability */
		if (prMcrWrInfo->u4McrData & 0x00000001U) {
	...
	else if (prMcrWrInfo->u4McrOffset == 0xFFFFFFF9U) {
		rCmdAccessReg.u4Address = prMcrWrInfo->u4McrOffset;
		rCmdAccessReg.u4Data = prMcrWrInfo->u4McrData;

		return wlanSendSetQueryCmd(prAdapter,
					   (UINT_8)CMD_ID_RANDOM_RX_RESET_DE,
					   TRUE,
					   FALSE,
					   TRUE,
					   nicCmdEventSetCommon,
	...
	else if (prMcrWrInfo->u4McrOffset == 0xFFFFFFF8U) {
		rCmdAccessReg.u4Address = prMcrWrInfo->u4McrOffset;
		rCmdAccessReg.u4Data = prMcrWrInfo->u4McrData;

		return wlanSendSetQueryCmd(prAdapter,
					   (UINT_8)CMD_ID_SAPP_EN,
					   TRUE,
					   FALSE,
					   TRUE,
					   nicCmdEventSetCommon,
	...
	else if (prMcrWrInfo->u4McrOffset == 0xFFFFFFF7U) {
		rCmdAccessReg.u4Address = prMcrWrInfo->u4McrOffset;
		rCmdAccessReg.u4Data = prMcrWrInfo->u4McrData;

		return wlanSendSetQueryCmd(prAdapter,
					   (UINT_8)CMD_ID_SAPP_DE,
					   TRUE,
					   FALSE,
					   TRUE,
					   nicCmdEventSetCommon,
	...
	if ((prMcrWrInfo->u4McrOffset & 0xFFFF0000U) != 0U) {
#if defined(MT6797)
		if ((prMcrWrInfo->u4McrOffset & 0xFFFF0000U) == 0x180f0000U) {

			writel(prMcrWrInfo->u4McrData, (volatile UINT_32 *)((*g_pHifRegBaseAddr) +
				(prMcrWrInfo->u4McrOffset & 0xffffU)));
		...
		/* 0x9000 - 0x9EFF reserved for FW */
#if CFG_SUPPORT_SWCR
		if ((prMcrWrInfo->u4McrOffset >> 16U) == 0x9F00U) {
			swCrReadWriteCmd(prAdapter,
				SWCR_WRITE,
				(UINT_16)(prMcrWrInfo->
				u4McrOffset & BITS(0U, 15U)),
		...
		if (prMcrWrInfo->u4McrOffset == 0x11111110U) {
			WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
			/* DbgPrint("Enter test mode\n"); */
			prAdapter->fgTestMode = TRUE;
			return rStatus;
		}
		if (prMcrWrInfo->u4McrOffset == 0x11111111U) {
			/* DbgPrint("nicpmSetAcpiPowerD3\n"); */

			if (nicpmSetAcpiPowerD3(prAdapter) == TRUE)
				kalDevSetPowerState(prAdapter->prGlueInfo,
					(UINT_32) ParamDeviceStateD3);
			return WLAN_STATUS_SUCCESS;
		}
		if (prMcrWrInfo->u4McrOffset == 0x11111112U) {
		...
		if (prMcrWrInfo->u4McrOffset == 0x22220000U) {
			/* read test mode */
			if (kalSetSdioTestPattern(
				prAdapter->prGlueInfo, TRUE, TRUE)
				== TRUE)
				return WLAN_STATUS_SUCCESS;
		...
		if (prMcrWrInfo->u4McrOffset == 0x22220001U) {
			/* write test mode */
			if (kalSetSdioTestPattern(
				prAdapter->prGlueInfo, TRUE, FALSE)
				== TRUE)
				return WLAN_STATUS_SUCCESS;
		...
		if (prMcrWrInfo->u4McrOffset == 0x22220002U) {
			/* leave from test mode */
			if (kalSetSdioTestPattern(
				prAdapter->prGlueInfo, FALSE, FALSE)
				== TRUE)
				return WLAN_STATUS_SUCCESS;
		...
		HAL_MCR_WR(prAdapter,
			(UINT_32)(prMcrWrInfo->u4McrOffset & BITS(2U, 31U)),
			prMcrWrInfo->u4McrData);
		       ...
		       IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T);
	...
	u2Id = (UINT_16) (prSwCtrlInfo->u4Id >> 16U);
	u2SubId = (UINT_16) (prSwCtrlInfo->u4Id & BITS(0U, 15U));
	u4Data = 0;
	...
	case 0xBABA:
		switch ((UINT_16)(u2SubId >> 8U) & BITS(0U, 7U)) {
		case 0x00:
		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T);
	...
	case 0x1001:
		if (u2SubId == 0x0U)
		...
		else if (u2SubId == 0x1U)
			prAdapter->fgDisBcnLostDetection = (BOOLEAN) u4Data;
		else if (u2SubId == 0x2U)
			prAdapter->rWifiVar.ucUapsd = (UINT_8) u4Data;
		else if (u2SubId == 0x3U) {
			prAdapter->u4UapsdAcBmp =
				(UINT32)(u4Data & BITS(0U, 15U));
			GET_BSS_INFO_BY_INDEX(prAdapter,
				u4Data >> 16U)->rPmProfSetupInfo.
				ucBmpDeliveryAC =
			...
			GET_BSS_INFO_BY_INDEX(prAdapter,
				...
				ucBmpTriggerAC =
				(UINT_8) prAdapter->u4UapsdAcBmp;
		} else if (u2SubId == 0x4U)
			prAdapter->fgDisStaAgingTimeoutDetection = (BOOLEAN) u4Data;
		else if (u2SubId == 0x5U)
			prAdapter->rWifiVar.rConnSettings.uc2G4BandwidthMode = (UINT_8) u4Data;
#if CFG_RX_BA_REORDERING_ENHANCEMENT
		else if (u2SubId == 0x6U)
			prAdapter->rWifiVar.fgEnableReportIndependentPkt = (BOOLEAN) u4Data;
#endif
		else if (u2SubId == 0x0100U) {
			if (u4Data == 2U)
				prAdapter->rWifiVar.ucRxGf =
					(UINT_8)FEATURE_DISABLED;
			else
				prAdapter->rWifiVar.ucRxGf =
					(UINT_8)FEATURE_ENABLED;
		} else if (u2SubId == 0x0101U)
			prAdapter->rWifiVar.ucRxShortGI = (UINT_8) u4Data;
		else if (u2SubId == 0x0103U) { /* AP Mode WMMPS */
			PARAM_CUSTOM_UAPSD_PARAM_STRUCT_T rUapsdParams;

			DBGLOG(OID, INFO, "ApUapsd 0x10010103 cmd received: %d\n", u4Data);
			if ((BOOLEAN) u4Data != FALSE) {
				prAdapter->rWifiVar.ucApUapsd = TRUE;
				rUapsdParams.fgEnAPSD = 1U;
				rUapsdParams.fgEnAPSD_AcBe = 1U;
				rUapsdParams.fgEnAPSD_AcBk = 1U;
				rUapsdParams.fgEnAPSD_AcVi = 1U;
				rUapsdParams.fgEnAPSD_AcVo = 1U;
				/* do not limit delivery pkt number */
				rUapsdParams.ucMaxSpLen = 0U;
			} else {
				prAdapter->rWifiVar.ucApUapsd = FALSE;
				rUapsdParams.fgEnAPSD = 0U;
				rUapsdParams.fgEnAPSD_AcBe = 0U;
				rUapsdParams.fgEnAPSD_AcBk = 0U;
				rUapsdParams.fgEnAPSD_AcVi = 0U;
				rUapsdParams.fgEnAPSD_AcVo = 0U;
				/* do not limit delivery pkt number */
				rUapsdParams.ucMaxSpLen = 0U;
			}
			if (nicSetUapsdParam(prAdapter, &rUapsdParams,
				NETWORK_TYPE_P2P) == WLAN_STATUS_SUCCESS)
				DBGLOG(OID, LOUD, "Set Uapsd Param Success\n");
		}
	...
	case 0x1002:
		...
		if (u2SubId == 0x1U) {
			...
			fgIsEnable = (u4Data & 0xffU) > 0 ? TRUE : FALSE;
			ucType = 0U;	/* ((u4Data>>4U) & 0xfU); */
			u4Timeout = ((u4Data >> 8U) & 0xffU);
	...
	case 0x2000:
		...
		if (u2SubId == 0x3U) {
			...
			if (authSendDeauthFrame(prAdapter,
				...
				NULL, 7, NULL) == WLAN_STATUS_SUCCESS) {
				DBGLOG(RSN, TRACE,
					"Send Deauth Frame Success\n");
			}
		}
		/* wext_set_mode */
		/*
		       ...
		       IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_CHIP_CONFIG_STRUCT_T);
	...
	rWlanStatus = wlanSendSetQueryCmd(prAdapter,
		(UINT_8)CMD_ID_CHIP_CONFIG, FALSE, TRUE, TRUE,
		/*nicCmdEventQuerySwCtrlRead, */
		nicCmdEventQueryChipConfig,
		nicOidCmdTimeoutCommon,
		(UINT_32)sizeof(CMD_CHIP_CONFIG_T),
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_CHIP_CONFIG_STRUCT_T);
...
wlanoidSetKeyCfg(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_KEY_CFG_STRUCT_T);
	...
	rWlanStatus = wlanCfgSet(prAdapter,
		prKeyCfgInfo->aucKey, prKeyCfgInfo->aucValue, 0);
		       ...
		       IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_EEPROM_RW_STRUCT_T);
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_ACCESS_EEPROM,
				   FALSE,
				   TRUE,
				   TRUE,
				   nicCmdEventQueryEepromRead,
				   nicOidCmdTimeoutCommon,
				   (UINT_32)sizeof(CMD_ACCESS_EEPROM),
		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_EEPROM_RW_STRUCT_T);
...
#if CFG_ENABLE_STATISTICS_BUFFERING
	if (IsBufferedStatisticsUsable(prAdapter) == TRUE) {
		if (u4QueryBufferLen == (UINT_32)sizeof(UINT_32)) {
			*pu4QueryInfoLen = (UINT_32)sizeof(UINT_32);
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	kalMemCopy((PUINT_8) &rCmdMacMcastAddr.arAddress[0],
		(PUINT_8) pvSetBuffer, u4SetBufferLen);
			      ...
			      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	do {
		 ...
		 */
		if ((u4NewPacketFilter &
			~(PARAM_PACKET_FILTER_SUPPORTED)) != 0U) {
			rStatus = WLAN_STATUS_NOT_SUPPORTED;
			break;
		}
#if DBG
...
{
	if (prAdapter->prGlueInfo->fgIsInSuspendMode != FALSE)
				 ...
				 IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_DEVICE_POWER_STATE);
			       ...
			       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_DEVICE_POWER_STATE);
			 ...
			 OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	if (u4QueryBufferLen < sizeof(PARAM_RTS_THRESHOLD)) {
		*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_RTS_THRESHOLD);
		return WLAN_STATUS_BUFFER_TOO_SHORT;
	}
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_RTS_THRESHOLD);
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0U;
	...
	prAisAbortMsg = (P_MSG_AIS_ABORT_T) cnmMemAlloc(prAdapter,
		RAM_TYPE_MSG, (UINT_8)sizeof(MSG_AIS_ABORT_T));
	if (!prAisAbortMsg) {
		ASSERT(NULL);
		return WLAN_STATUS_FAILURE;
	}
...
/* *(PPARAM_POWER_MODE) pvQueryBuffer = (PARAM_POWER_MODE)(prAdapter->rWlanInfo.ePowerSaveMode.ucPsProfile); */
		...
		*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_POWER_MODE);
		...
		if (prAdapter->fgEnCtiaPowerMode != FALSE) {
				   ...
				   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_POWER_MODE_T);
		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(UINT_32);
	...
	rCmdBasicConfig.ucNative80211 = 0U;	/* @FIXME: for Vista */

	if ((u4CSUMFlags & CSUM_OFFLOAD_EN_TX_TCP) != 0U)
		rCmdBasicConfig.rCsumOffload.u2TxChecksum |= (UINT_16)BIT(2U);

	if ((u4CSUMFlags & CSUM_OFFLOAD_EN_TX_UDP) != 0U)
		rCmdBasicConfig.rCsumOffload.u2TxChecksum |= (UINT_16)BIT(1U);

	if ((u4CSUMFlags & CSUM_OFFLOAD_EN_TX_IP) != 0U)
		rCmdBasicConfig.rCsumOffload.u2TxChecksum |= (UINT_16)BIT(0U);

	if ((u4CSUMFlags & CSUM_OFFLOAD_EN_RX_TCP) != 0U)
		rCmdBasicConfig.rCsumOffload.u2RxChecksum |= (UINT_16)BIT(2U);

	if ((u4CSUMFlags & CSUM_OFFLOAD_EN_RX_UDP) != 0U)
		rCmdBasicConfig.rCsumOffload.u2RxChecksum |= (UINT_16)BIT(1U);

	if ((u4CSUMFlags &
		(CSUM_OFFLOAD_EN_RX_IPv4 | CSUM_OFFLOAD_EN_RX_IPv6)) != 0U)
		rCmdBasicConfig.rCsumOffload.u2RxChecksum |= (UINT_16)BIT(0U);

	prAdapter->u4CSUMFlags = u4CSUMFlags;

	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_BASIC_CONFIG,
				   TRUE,
				   FALSE,
				   TRUE,
				   nicCmdEventSetCommon,
				   nicOidCmdTimeoutCommon,
				   (UINT_32)sizeof(CMD_BASIC_CONFIG_T),
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0U;
	...
	if (prAdapter->fgEnArpFilter != FALSE) {
	...
	*pu4SetInfoLen = 0U;
	...
	if (u4SetBufferLen == 0U) {
		if ((prAdapter->fgTestMode == FALSE)
			...
			rCmdTestCtrl.ucAction = 0U;	/* Switch mode */
			rCmdTestCtrl.u.u4OpMode = 1U;	/* RF test mode */

			rStatus = wlanSendSetQueryCmd(prAdapter,
					(UINT_8)CMD_ID_TEST_CTRL,
					TRUE,
					FALSE,
					TRUE,
					nicCmdEventEnterRfTest,
					nicOidCmdEnterRFTestTimeout,
					(UINT_32)sizeof(CMD_TEST_CTRL_T),
	...
	*pu4SetInfoLen = 0U;
	...
	if (u4SetBufferLen == 0U) {
		if (prAdapter->fgIcapMode == FALSE) {
			/* switch to RF Test mode */
			rCmdTestCtrl.ucAction = 0U;	/* Switch mode */
			rCmdTestCtrl.u.u4OpMode = 2U;	/* RF test mode */
	...
	*pu4SetInfoLen = 0U;
	...
	if (u4SetBufferLen == 0U) {
		if (prAdapter->fgTestMode == TRUE) {
			...
			rCmdTestCtrl.ucAction = 0U;	/* Switch mode */
			rCmdTestCtrl.u.u4OpMode = 0U;	/* normal mode */
			   ...
			   OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_MTK_WIFI_TEST_STRUCT_T);
			 ...
			 OUT PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_MTK_WIFI_TEST_STRUCT_T);
...
WLAN_STATUS rftestSetATInfo(IN P_ADAPTER_T prAdapter, UINT_32 u4FuncIndex, UINT_32 u4FuncData)
{
	...
	prGlueInfo = prAdapter->prGlueInfo;
	prCmdInfo = cmdBufAllocateCmdInfo(prAdapter,
		(UINT_32)(CMD_HDR_SIZE + sizeof(CMD_TEST_CTRL_T)));
	...
	prCmdInfo->u2InfoBufLen =
		(UINT_16)(CMD_HDR_SIZE + sizeof(CMD_TEST_CTRL_T));
	prCmdInfo->pfCmdDoneHandler = nicCmdEventSetCommon;
	prCmdInfo->pfCmdTimeoutHandler = nicOidCmdTimeoutCommon;
	prCmdInfo->fgIsOid = TRUE;
	prCmdInfo->ucCID = (UINT_8)CMD_ID_TEST_CTRL;
	prCmdInfo->fgSetQuery = TRUE;
	prCmdInfo->fgNeedResp = FALSE;
	prCmdInfo->fgDriverDomainMCR = FALSE;
	prCmdInfo->ucCmdSeqNum = ucCmdSeqNum;
	prCmdInfo->u4SetInfoLen = (UINT_32)sizeof(CMD_TEST_CTRL_T);
	...
	if ((u4FuncIndex == (UINT_32)RF_AT_FUNCID_COMMAND) &&
		(u4FuncData == (UINT_32)RF_AT_COMMAND_ICAP)) {
		g_bIcapEnable = TRUE;
		g_bCaptureDone = FALSE;
	}
	/* ICAP dump name Reset */
	if ((u4FuncIndex == (UINT_32)RF_AT_FUNCID_COMMAND) &&
		(u4FuncData == (UINT_32)RF_AT_COMMAND_RESET_DUMP_NAME))
		g_u2DumpIndex = 0U;
	/* insert into prCmdQueue */
	kalEnqueueCommand(prGlueInfo, &(prCmdInfo->rQueEntry));
		  ...
		  UINT_32 u4FuncIndex, UINT_32 u4FuncData, OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen)
{
	...
	if (u4FuncIndex == (UINT_32)RF_AT_FUNCID_FW_INFO) {
		/* driver implementation */
		prTestStatus = (P_EVENT_TEST_STATUS) pvQueryBuffer;

		prTestStatus->rATInfo.u4FuncData =
			(UINT_32)
			(((UINT_32)prAdapter->rVerInfo.u2FwProductID << 16U)
			| (prAdapter->rVerInfo.u2FwOwnVersion));
		u4QueryBufferLen = sizeof(EVENT_TEST_STATUS);

	} else if (u4FuncIndex == (UINT_32)RF_AT_FUNCID_DRV_INFO) {
		/* driver implementation */
		prTestStatus = (P_EVENT_TEST_STATUS) pvQueryBuffer;

		prTestStatus->rATInfo.u4FuncData = (UINT_32)CFG_DRV_OWN_VERSION;
		u4QueryBufferLen = sizeof(EVENT_TEST_STATUS);

	} else if (u4FuncIndex == (UINT_32)RF_AT_FUNCID_QUERY_ICAP_DUMP_FILE) {
		/* driver implementation */
		prTestStatus = (P_EVENT_TEST_STATUS) pvQueryBuffer;
		...
		pCmdTestCtrl = (P_CMD_TEST_CTRL_T) (prWifiCmd->aucBuffer);
		pCmdTestCtrl->ucAction = 2U;	/* Get ATInfo */
		pCmdTestCtrl->u.rRfATInfo.u4FuncIndex = u4FuncIndex;
		pCmdTestCtrl->u.rRfATInfo.u4FuncData = u4FuncData;
		    ...
		    PUINT_8 pucInfoBuffer, OUT PVOID pvSetQueryBuffer, IN UINT_32 u4SetQueryBufferLen)
{
	...
	if (u4SetQueryInfoLen > 0 && pucInfoBuffer != NULL)
		kalMemCopy((PUINT_8)&prWifiCmd->aucBuffer[0],
			(PUINT_8)pucInfoBuffer, u4SetQueryInfoLen);
	/* insert into prCmdQueue */
	kalEnqueueCommand(prGlueInfo, &(prCmdInfo->rQueEntry));
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	PUINT_8 cpPos;
	UINT_16 u2AuthSuiteCount = 0U;
	UINT_16 u2PairSuiteCount = 0U;
	UINT_32 u4AuthKeyMgtSuite = 0U;
	UINT_32 u4PairSuite = 0U;
	UINT_32 u4GroupSuite = 0U;
	UINT_16 u2IeLength = 0U;
	...
	if (!wextSrchDesiredWAPIIE((PUINT_8) pvSetBuffer,
		(INT_32)u4SetBufferLen, (PUINT_8 *) &prWapiInfo))
		return WLAN_STATUS_INVALID_LENGTH;

	if (!prWapiInfo || prWapiInfo->ucLength < 18)
		return WLAN_STATUS_INVALID_LENGTH;

	u2IeLength = (UINT_16)((UINT_16)prWapiInfo->ucLength + 2U);

	/* Skip Version check */
	cpPos = (PUINT_8) &prWapiInfo->u2AuthKeyMgtSuiteCount;

	WLAN_GET_FIELD_16(cpPos, &u2AuthSuiteCount);

	if (u2AuthSuiteCount > 1U)
		return WLAN_STATUS_INVALID_LENGTH;

	cpPos += 2U;
	WLAN_GET_FIELD_32(cpPos, &u4AuthKeyMgtSuite);

	DBGLOG(SEC, TRACE, "WAPI: Assoc Info auth mgt suite [%d]: %02x-%02x-%02x-%02x\n",
		u2AuthSuiteCount,
		(UCHAR) (u4AuthKeyMgtSuite & 0x000000FFU),
		(UCHAR) ((u4AuthKeyMgtSuite >> 8U) & 0x000000FFU),
		(UCHAR) ((u4AuthKeyMgtSuite >> 16U) & 0x000000FFU),
		(UCHAR) ((u4AuthKeyMgtSuite >> 24U) & 0x000000FFU));

	if (u4AuthKeyMgtSuite != WAPI_AKM_SUITE_802_1X && u4AuthKeyMgtSuite != WAPI_AKM_SUITE_PSK)
		ASSERT_BOOLEAN(FALSE);

	cpPos += 4U;
	WLAN_GET_FIELD_16(cpPos, &u2PairSuiteCount);
	if (u2PairSuiteCount > 1U)
		return WLAN_STATUS_INVALID_LENGTH;

	cpPos += 2U;
	WLAN_GET_FIELD_32(cpPos, &u4PairSuite);
	DBGLOG(SEC, TRACE, "WAPI: Assoc Info pairwise cipher suite [%d]: %02x-%02x-%02x-%02x\n",
		u2PairSuiteCount,
		(UCHAR) (u4PairSuite & 0x000000FFU),
		(UCHAR) ((u4PairSuite >> 8U) & 0x000000FFU),
		(UCHAR) ((u4PairSuite >> 16U) & 0x000000FFU),
		(UCHAR) ((u4PairSuite >> 24U) & 0x000000FFU));

	if (u4PairSuite != WAPI_CIPHER_SUITE_WPI)
		ASSERT_BOOLEAN(1 != 1);

	cpPos += 4U;
	WLAN_GET_FIELD_32(cpPos, &u4GroupSuite);
	DBGLOG(SEC, TRACE, "WAPI: Assoc Info group cipher suite : %02x-%02x-%02x-%02x\n",
		(UCHAR) (u4GroupSuite & 0x000000FFU),
...
#if (CFG_SUPPORT_WPA3 == 0)
	kalMemCopy((PUINT_8) prAdapter->prGlueInfo->aucWapiAssocInfoIEs,
		(PUINT_8) prWapiInfo, u2IeLength);
...
wlanoidSetWapiKey(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	DBGLOG_MEM8(OID, TRACE, (PUINT_8) pvSetBuffer, 560U);
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	/* Exception check */
	prNewKey->ucKeyID = (UINT_8)(prNewKey->ucKeyID & BIT(0U));
	...
	DBGLOG_MEM8(OID, TRACE, prNewKey->aucAddrIndex, 12U);
	prNewKey->u4LenWPIEK = 16U;

	DBGLOG_MEM8(OID, TRACE, (PUINT_8) prNewKey->aucWPIEK, (UINT_8) prNewKey->u4LenWPIEK);
	prNewKey->u4LenWPICK = 16U;
	...
	DBGLOG(OID, TRACE, "PN:\r\n");
	if (prNewKey->eKeyType == 0U) {
		prNewKey->aucPN[0] = 0x5cU;
		prNewKey->aucPN[1] = 0x36U;
		prNewKey->aucPN[2] = 0x5cU;
		prNewKey->aucPN[3] = 0x36U;
		prNewKey->aucPN[4] = 0x5cU;
		prNewKey->aucPN[5] = 0x36U;
		prNewKey->aucPN[6] = 0x5cU;
		prNewKey->aucPN[7] = 0x36U;
		prNewKey->aucPN[8] = 0x5cU;
		prNewKey->aucPN[9] = 0x36U;
		prNewKey->aucPN[10] = 0x5cU;
		prNewKey->aucPN[11] = 0x36U;
		prNewKey->aucPN[12] = 0x5cU;
		prNewKey->aucPN[13] = 0x36U;
		prNewKey->aucPN[14] = 0x5cU;
		prNewKey->aucPN[15] = 0x36U;
	}

	DBGLOG_MEM8(OID, TRACE, (PUINT_8) prNewKey->aucPN, 16U);

	prGlueInfo = prAdapter->prGlueInfo;

	prCmdInfo = cmdBufAllocateCmdInfo(prAdapter,
		(UINT_32)(CMD_HDR_SIZE + u4SetBufferLen));
	...
	if (prCmdKey->ucTxKey != 0U) {
		...
		if ((prCmdKey->aucPeerAddr[0] & prCmdKey->aucPeerAddr[1] &
			...
			== 0xFFU) {
			prCmdKey->ucWlanIndex = 255U;
			/* AIS WEP, should not have this case!! */
		} else {
			if (prStaRec != NULL) {
			   ...
			   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_PM_PACKET_PATTERN);
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_WMM_PS_TEST_STRUCT_T);
	...
	prPmProfSetupInfo->ucBmpDeliveryAC =
		(UINT_8)((rSetWmmPsTestParam.bmfgApsdEnAc >> 4U) &
		(UINT_8)BITS(0U, 3U));
	prPmProfSetupInfo->ucBmpTriggerAC =
		(UINT_8)(rSetWmmPsTestParam.bmfgApsdEnAc &
		(UINT_8)BITS(0U, 3U));

	u2CmdBufLen = (UINT_32)sizeof(CMD_SET_WMM_PS_TEST_STRUCT_T);
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,
				      (UINT_8) CMD_ID_SET_WMM_PS_TEST_PARMS,
				      TRUE,
				      FALSE,
				      TRUE,
				      nicCmdEventSetCommon,
...
wlanoidSetTxAmpdu(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(BOOLEAN);

	pfgEnable = (PBOOLEAN) pvSetBuffer;

	rTxAmpdu.fgEnable = *pfgEnable;

	u2CmdBufLen = (UINT_32)sizeof(CMD_TX_AMPDU_T);

	rStatus = wlanSendSetQueryCmd(prAdapter,
				      (UINT_8)CMD_ID_TX_AMPDU,
		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(BOOLEAN);
	...
	u2CmdBufLen = (UINT_32)sizeof(CMD_ADDBA_REJECT_T);

	rStatus = wlanSendSetQueryCmd(prAdapter,
				      (UINT_8)CMD_ID_ADDBA_REJECT,
		      ...
		      OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_MTK_SLT_TEST_STRUCT_T);
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_MTK_SLT_TEST_STRUCT_T);
	...
	case ENUM_MTK_SLT_FUNC_INITIAL:	/* Initialize */
		{
			...
			case 1:
				prSltInfo->ucChannel5G = 36U;
				break;
			case 6:
				prSltInfo->ucChannel5G = 52U;
				break;
			case 11:
				prSltInfo->ucChannel5G = 104U;
				break;
			case 16:
				prSltInfo->ucChannel2G4 = 14U;
				prSltInfo->ucChannel5G = 161U;
				break;
			default:
				ASSERT_BOOLEAN(FALSE);
			}
			...
			if (prBssDesc) {
				...
				prBssDesc->u2BeaconInterval = 100U;
				prBssDesc->u2ATIMWindow = 0U;
				prBssDesc->ucDTIMPeriod = 1U;

				prBssDesc->u2IELength = 0U;
	...
	case ENUM_MTK_SLT_FUNC_RATE_SET:	/* Update RF Settings. */
		if (prSltInfo->prPseudoStaRec == NULL) {
			...
			if ((prTRSetting->u4FixedRate & FIXED_BW_DL40) != 0U) {
				/* RF 40 */
				...
				/* U20/L20 Control. */
				switch (prTRSetting->u4FixedRate & 0xC000U) {
			...
			switch (prTRSetting->u4FixedRate & 0xFFU) {
			case RATE_OFDM_54M:
	...
	case ENUM_MTK_SLT_FUNC_LP_SET:	/* Reset LP Test Result. */
		{
			...
			case ENUM_MTK_LP_TEST_DUT:
				...
				prSltInfo->u4BeaconReceiveCnt = 0U;
		      ...
		      OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_NVRAM_RW_STRUCT_T);
	...
	if (prNvramRwInfo->ucEepromMethod == PARAM_EEPROM_READ_METHOD_READ) {
		fgStatus = kalCfgDataRead16(prAdapter->prGlueInfo,
			(UINT_32)prNvramRwInfo->ucEepromIndex << 1U,
		...
		prNvramRwInfo->u2EepromData = (UINT_16)CFG_FILE_WIFI_REC_SIZE;
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_NVRAM_RW_STRUCT_T);
	...
	fgStatus = kalCfgDataWrite16(prAdapter->prGlueInfo,
		(UINT_32)prNvramRwInfo->ucEepromIndex << 1,
		       ...
		       OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(ENUM_CFG_SRC_TYPE_T);
		       ...
		       OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(P_ENUM_EEPROM_TYPE_T);
		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	ASSERT(prAdapter);
	ASSERT(pvSetBuffer);
	ASSERT_BOOLEAN(u4SetBufferLen == 2U);
	...
	*pu4SetInfoLen = 2U;
	...
	prAdapter->rWifiVar.rConnSettings.u2CountryCode =
		(((UINT_16) pucCountry[0]) << 8U) | ((UINT_16) pucCountry[1]);
...
wlanoidSetBT(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PTA_IPC_T);
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,
			(UINT_8)CMD_ID_SET_BWCS,
			TRUE, FALSE, FALSE, NULL, NULL,
			(UINT_32)sizeof(PTA_IPC_T),
...
/* UINT_32 u4QueryBuffLen; */
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(PTA_IPC_T);
...
/* prPtaIpc->ucLen = (UINT_32)sizeof(prPtaIpc->u); */
...
wlanoidSetTxPower(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	prCmd = cnmMemAlloc(prAdapter, RAM_TYPE_BUF,
		(UINT_32)sizeof(SET_TXPWR_CTRL_T));
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,	/* prAdapter */
			(UINT_8)CMD_ID_SET_TXPWR_CTRL,	/* ucCID */
			TRUE,	/* fgSetQuery */
			FALSE,	/* fgNeedResp */
			TRUE,	/* fgIsOid */
			nicCmdEventSetCommon,
...
WLAN_STATUS wlanSendMemDumpCmd(IN P_ADAPTER_T prAdapter, IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen)
{
	...
	UINT_32 u4RemainLeng = 0U;
	UINT_32 u4CurAddr = 0U;
	UINT_8 ucFragNum = 0U;
	...
	ucFragNum = (UINT_8)(prMemDumpInfo->ucFragNum + 1U);
	...
	do {
		...
		rStatus = wlanSendSetQueryCmd(prAdapter,
					      (UINT_8)CMD_ID_DUMP_MEM,
					      FALSE,
					      TRUE,
					      TRUE,
					      nicCmdEventQueryMemDump,
					      nicOidCmdTimeoutCommon,
					      (UINT_32)sizeof(CMD_DUMP_MEM),
		    ...
		    IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(UINT_32);
	...
	prMemDumpInfo->u4Length = 0U;
	prMemDumpInfo->ucFragNum = 0U;
...
wlanoidSetP2pMode(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_P2P_SET_STRUCT_T);
	...
	if (prSetP2P->u4Enable != 0U) {
		p2pSetMode((prSetP2P->u4Mode == 1U) ? TRUE : FALSE);
		if (p2pLaunch(prAdapter->prGlueInfo) != FALSE)
			ASSERT_BOOLEAN(prAdapter->fgIsP2PRegistered);

		if (prAdapter->rWifiVar.ucApUapsd != 0U &&
			prSetP2P->u4Mode == 1U) {
			PARAM_CUSTOM_UAPSD_PARAM_STRUCT_T rUapsdParams;
			...
			rUapsdParams.fgEnAPSD_AcVi = 1U;
			rUapsdParams.fgEnAPSD_AcVo = 1U;
			/* default:0, Do not limit delivery pkt num */
			rUapsdParams.ucMaxSpLen = 0U;
			if (nicSetUapsdParam(prAdapter, &rUapsdParams,
				NETWORK_TYPE_P2P) == WLAN_STATUS_SUCCESS)
				DBGLOG(OID, LOUD, "Set Uapsd Param Success\n");
		...
		if (prAdapter->fgIsP2PRegistered != FALSE) {
			if (p2pRemove(prAdapter->prGlueInfo) == TRUE)
				DBGLOG(OID, LOUD, "p2p remove Success\n");
		}
	}
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	prGlueInfo = prAdapter->prGlueInfo;
	prCmdInfo = cmdBufAllocateCmdInfo(prAdapter,
		(UINT_32)(CMD_HDR_SIZE + sizeof(CMD_DEFAULT_KEY)));
	...
	prCmdInfo->pfCmdTimeoutHandler = nicOidCmdTimeoutCommon;
	prCmdInfo->fgIsOid = TRUE;
	prCmdInfo->ucCID = (UINT_8)CMD_ID_DEFAULT_KEY_ID;
	prCmdInfo->fgSetQuery = TRUE;
	prCmdInfo->fgNeedResp = FALSE;
	prCmdInfo->fgDriverDomainMCR = FALSE;
	prCmdInfo->ucCmdSeqNum = ucCmdSeqNum;
	...
	if (prDefaultKey->ucMulticast != 0U) {
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	kalMemCopy((PUINT_8) &rCmdContent,
		(PUINT_8) pvSetBuffer,
		u4SetBufferLen);

	rStatus = wlanSendSetQueryCmd(prAdapter,	/* prAdapter */
			(UINT_8)CMD_ID_SET_GTK_REKEY_DATA,	/* ucCID */
			TRUE,	/* fgSetQuery */
			FALSE,	/* fgNeedResp */
			TRUE,	/* fgIsOid */
			NULL,	/* pfCmdDoneHandler */
			NULL,	/* pfCmdTimeoutHandler */
			(UINT_32)sizeof(CMD_GTK_REKEY_DATA_T),
...
WLAN_STATUS wlanoidResetBAScoreboard(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen)
{
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,	/* prAdapter */
			(UINT_8)CMD_ID_RESET_BA_SCOREBOARD,	/* ucCID */
			TRUE,	/* fgSetQuery */
			FALSE,	/* fgNeedResp */
			TRUE,	/* fgIsOid */
			NULL,	/* pfCmdDoneHandler */
...
batchSetCmd(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4WritenLen)
{
	...
	*pu4WritenLen = 0U;
	...
	head = pvSetBuffer + kalStrLen(CMD_WLS_BATCHING) + 1U;
	...
	if (!kalStrnCmp(head, BATCHING_SET, kalStrLen(BATCHING_SET))) {
		...
		head += kalStrLen(BATCHING_SET) + 1U;
		 ...
		 * }
		 */
		rCmdBatchReq.ucChannelType = (UINT_8)SCAN_CHANNEL_SPECIFIED;
		rCmdBatchReq.ucChannelListNum = 0U;
		prRfChannelInfo = &rCmdBatchReq.arChannelList[0];
		p = head + kalStrLen(PARAM_CHANNEL) + 2U;	/* c_channel; */
		pcTemp = (char *)p;
		while ((p2 = kalStrSep(&pcTemp, ",")) != NULL) {
			...
			if (*p2 == '\0')
				continue;
			if (*p2 == 'A') {
				rCmdBatchReq.ucChannelType = (UINT_8)
					(rCmdBatchReq.ucChannelType ==
					SCAN_CHANNEL_2G4 ? SCAN_CHANNEL_FULL :
					SCAN_CHANNEL_5G);
			} else if (*p2 == 'B') {
				rCmdBatchReq.ucChannelType = (UINT_8)
					(rCmdBatchReq.ucChannelType ==
					SCAN_CHANNEL_5G ? SCAN_CHANNEL_FULL :
					SCAN_CHANNEL_2G4);
			} else {
				...
				prRfChannelInfo->ucBand = (UINT_8)
					(prRfChannelInfo->ucChannelNum < 15U ?
					BAND_2G4 : BAND_5G);

				rCmdBatchReq.ucChannelListNum++;
				if (rCmdBatchReq.ucChannelListNum >= 32U)
					break;
				prRfChannelInfo++;
			}
		}
		...
		rCmdBatchReq.u4Scanfreq = (UINT_32)scanfreq;
		rCmdBatchReq.ucMScan = (UINT_8)
			(mscan > CFG_BATCH_MAX_MSCAN ?
			CFG_BATCH_MAX_MSCAN : mscan);
		rCmdBatchReq.ucBestn = (UINT_8)bestn;
		rCmdBatchReq.ucRtt = (UINT_8)rtt;
		...
		rCmdBatchReq.ucSeqNum = 1U;
		rCmdBatchReq.ucNetTypeIndex =
			(UINT_8)KAL_NETWORK_TYPE_AIS_INDEX;
		rCmdBatchReq.ucCmd = (UINT_8)SCAN_BATCH_REQ_START;

		*pu4WritenLen = (UINT_32)
		...
		rCmdBatchReq.ucNetTypeIndex =
			(UINT_8)KAL_NETWORK_TYPE_AIS_INDEX;
		rCmdBatchReq.ucCmd = (UINT_8)SCAN_BATCH_REQ_STOP;
	} else {
		return WLAN_STATUS_FAILURE;
	}

	rStatus = wlanSendSetQueryCmd(prAdapter,
			(UINT_8)CMD_ID_SET_BATCH_REQ,
			TRUE, FALSE, TRUE, NULL, NULL,
			(UINT_32)sizeof(CMD_BATCH_REQ_T),
	    ...
	    OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(EVENT_BATCH_RESULT_T);

	rCmdBatchReq.ucSeqNum = 2U;
	rCmdBatchReq.ucCmd = (UINT_8)SCAN_BATCH_REQ_RESULT;
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (prCmdPscnAction != NULL) {
		DBGLOG(SCN, TRACE, "ucPscanAct=[%d]\n", prCmdPscnAction->ucPscanAct);
		if (prCmdPscnAction->ucPscanAct == (UINT_8)PSCAN_ACT_ENABLE) {
			prAdapter->rWifiVar.rScanInfo.fgGScnAction = TRUE;
			scnPSCNFsm(prAdapter, PSCN_SCANNING);
		} else if (prCmdPscnAction->ucPscanAct ==
			(UINT_8)PSCAN_ACT_DISABLE) {
			if (scnCombineParamsIntoPSCN(prAdapter, NULL,
				NULL, NULL, NULL, FALSE, FALSE, TRUE) == FALSE)
				return WLAN_STATUS_SUCCESS;

			if ((prAdapter->rWifiVar.rScanInfo.
				prPscnParam->fgNLOScnEnable == TRUE)
				|| (prAdapter->rWifiVar.rScanInfo.
				prPscnParam->fgBatchScnEnable == TRUE))
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (prCmdGscnConfigParam != NULL) {
		rCmdGscnConfig.u4BufferThreshold =
			prCmdGscnConfigParam->report_threshold_percent;
		rCmdGscnConfig.ucNumApPerScn =
			(UINT_8)prCmdGscnConfigParam->max_ap_per_scan;
		rCmdGscnConfig.u4NumScnToCache =
			prCmdGscnConfigParam->report_threshold_num_scans;
	}
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (prGetGscnScnResultParm != NULL) {
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (u4SetBufferLen < sizeof(PARAM_HS20_SET_BSSID_POOL)) {
		*pu4SetInfoLen = (UINT_32)sizeof(PARAM_HS20_SET_BSSID_POOL);
		return WLAN_STATUS_BUFFER_TOO_SHORT;
	}
...
wlanoidSetMonitor(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_MONITOR_SET_STRUCT_T);
	...
	rWlanStatus = wlanSendSetQueryCmd(prAdapter,
			(UINT_8)CMD_ID_SET_MONITOR,
			TRUE,
			FALSE,
			TRUE,
			nicCmdEventSetCommon,
			nicOidCmdTimeoutCommon,
			(UINT_32)sizeof(CMD_MONITOR_SET_INFO_T),
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_SET_SUSPEND_MODE,
				   TRUE,
				   FALSE,
				   TRUE,
				   nicCmdEventSetCommon,
				   nicOidCmdTimeoutCommon,
				   (UINT_32)sizeof(BOOLEAN),
				   (PUINT_8)&rSuspendCmd,
				   NULL,
				   0);
}
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = (UINT_32)sizeof(PARAM_PACKET_KEEPALIVE_T);
	...
	kalMemCopy((PUINT_8) prPacket,
		(PUINT_8) pvSetBuffer,
		sizeof(PARAM_PACKET_KEEPALIVE_T));

	DBGLOG(OID, INFO, "enable=%d, index=%d\r\n", prPacket->enable, prPacket->index);

	rStatus = wlanSendSetQueryCmd(prAdapter,
			(UINT_8)CMD_ID_WFC_KEEP_ALIVE,
			TRUE,
			FALSE,
			TRUE,
			nicCmdEventSetCommon,
			nicOidCmdTimeoutCommon,
			(UINT_32)sizeof(PARAM_PACKET_KEEPALIVE_T),
			(PUINT_8)prPacket, NULL, 0);
			   ...
			   IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	do {
		...
		if ((prAdapter == NULL) || (pu4QueryInfoLen == NULL))
			break;
		if ((pvQueryBuffer == NULL) || (u4QueryBufferLen == 0U))
			break;

		/* Get LTE safe channel list */
		rResult = wlanSendSetQueryCmd(prAdapter,
					      (UINT_8)CMD_ID_GET_LTE_CHN,
					      FALSE,
					      TRUE,
					      TRUE,
		...
		OUT PUINT_32 pu4SetInfoLen)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
		(UINT_8)CMD_ID_ROAMING_CONTROL,
		TRUE,
		FALSE,
		TRUE,
		nicCmdEventSetCommon,
		nicOidCmdTimeoutCommon,
		(UINT_32)sizeof(rRoamingCtrl),
			   ...
			   IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = (UINT_32)sizeof(struct _CMD_HEADER_T);

	if (u4QueryBufferLen < sizeof(struct _CMD_HEADER_T))
		return WLAN_STATUS_INVALID_LENGTH;

	kalMemCopy((struct _CMD_HEADER_T *)&cmdV1Header,
		(struct _CMD_HEADER_T *)prCmdV1Header,
		sizeof(struct _CMD_HEADER_T));
	rStatus = wlanSendSetQueryCmd(
			prAdapter,
			(UINT_8)CMD_ID_GET_SET_CUSTOMER_CFG,
			FALSE,
			TRUE,
			TRUE,
			nicCmdEventQueryCfgRead,
			nicOidCmdTimeoutCommon,
			(UINT_32)sizeof(struct _CMD_HEADER_T),
			(PUINT_8) &cmdV1Header,
			pvQueryBuffer,
			u4QueryBufferLen);
	return rStatus;
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	rTdlsPs.ucIsEnablePs = *(PUINT_8)pvSetBuffer - (UINT_8)'0';
	DBGLOG(OID, INFO, "enable tdls ps %d\n", rTdlsPs.ucIsEnablePs);
	wlanSendSetQueryCmd(prAdapter,
		(UINT_8)CMD_ID_TDLS_PS,
		TRUE,
		FALSE,
		FALSE,
		NULL,
		nicOidCmdTimeoutCommon,
		(UINT_32)sizeof(rTdlsPs),
		(PUINT_8)&rTdlsPs,
		NULL,
		0);
	return WLAN_STATUS_SUCCESS;
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	u4CurConPolicy = (UINT_32) prConnSettings->eConnectionPolicy;

	if (u4RoamingPoily == 1U) {
			...
			OUT uint32_t *pu4SetInfoLen)
{
	...
	P_MSG_SAA_EXTERNAL_AUTH_DONE_T prExternalAuthMsg = NULL;
	...
	prExternalAuthMsg = (P_MSG_SAA_EXTERNAL_AUTH_DONE_T)cnmMemAlloc(
			     prAdapter, RAM_TYPE_MSG,
			     sizeof(MSG_SAA_EXTERNAL_AUTH_DONE_T));
