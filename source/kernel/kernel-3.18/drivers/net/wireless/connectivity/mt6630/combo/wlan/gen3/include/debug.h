 ...
 */
#define DBG_CLASS_ERROR         BIT(0U)
#define DBG_CLASS_WARN          BIT(1U)
#define DBG_CLASS_STATE         BIT(2U)
#define DBG_CLASS_EVENT         BIT(3U)
#define DBG_CLASS_TRACE         BIT(4U)
#define DBG_CLASS_INFO          BIT(5U)
#define DBG_CLASS_LOUD          BIT(6U)
#define DBG_CLASS_TEMP          BIT(7U)
#define DBG_CLASS_MASK          BITS(0U, 7U)

#define DEFAULT_LOG_LEVEL \
	(DBG_CLASS_ERROR | DBG_CLASS_WARN \
	| DBG_CLASS_STATE |  DBG_CLASS_INFO)
...
#define DBGLOGLIMITED(_Module, _Class, _Fmt, ...) \
	do { \
		if ((aucDebugModule[DBG_##_Module##_IDX] & DBG_CLASS_##_Class) \
			== 0U) \
			break; \
		LOG_FUNC_LIMITED("%s:(" #_Module " " #_Class ") " _Fmt, \
...
#define ASSERT_BOOLEAN(_exp) \
	do { \
		if (!(_exp) && (fgIsBusAccessFailed == FALSE)) { \
			do {} while (1); \
		} \
	} while (0)
...
#define ASSERT_REPORT(_exp, _fmt) \
	do { \
		if (((_exp) == NULL) && !fgIsBusAccessFailed) { \
			LOG_FUNC("Assertion failed: %s:%d)\n",\
			__FILE__, __LINE__); \
			LOG_FUNC(_fmt); \
			kalBreakPoint(); \
