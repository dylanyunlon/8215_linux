...
#define SCN_RM_POLICY_EXCLUDE_CONNECTED     BIT(0U)
/* Remove the timeout one */
#define SCN_RM_POLICY_TIMEOUT               BIT(1U)
/* Remove the oldest one with hidden ssid */
#define SCN_RM_POLICY_OLDEST_HIDDEN         BIT(2U)
/* If there are more than half BSS which has the
 * same ssid as connection setting, remove the
 * weakest one from them
 * Else remove the weakest one.
 */
#define SCN_RM_POLICY_SMART_WEAKEST         BIT(3U)
/* Remove entire SCAN result */
#define SCN_RM_POLICY_ENTIRE                BIT(4U)
/* This is used by POLICY SMART WEAKEST,
 * If exceed this value, remove weakest BSS_DESC_T
 * with same SSID first in large network.
 */
#define SCN_BSS_DESC_SAME_SSID_THRESHOLD    20U

#define REMOVE_TIMEOUT_TWO_DAY     (60*60*24*2)

#if 1
#define SCN_BSS_DESC_REMOVE_TIMEOUT_SEC     30U
#define SCN_BSS_DESC_STALE_SEC				10U
...
#define SCN_NLO_NETWORK_CHANNEL_NUM         (4U)

#define SCAN_DONE_DIFFERENCE                3U
#define SCAN_DONE_TIMEOUT_THRESHOLD 3U
/*----------------------------------------------------------------------------*/
/* MSG_SCN_SCAN_REQ                                                           */
/*----------------------------------------------------------------------------*/
#define SCAN_REQ_SSID_WILDCARD              BIT(0U)
#define SCAN_REQ_SSID_P2P_WILDCARD          BIT(1U)
#define SCAN_REQ_SSID_SPECIFIED             BIT(2U)
...
#define SCN_AGPS_AP_LIST_MAX_NUM					32U
#endif

#define SCN_BSS_JOIN_FAIL_CNT_RESET_SEC				15U
#define SCN_BSS_JOIN_FAIL_RESET_STEP				2U

#if CFG_SUPPORT_BATCH_SCAN
/*----------------------------------------------------------------------------*/
/* SCAN_BATCH_REQ                                                             */
/*----------------------------------------------------------------------------*/
#define SCAN_BATCH_REQ_START                BIT(0U)
#define SCAN_BATCH_REQ_STOP                 BIT(1U)
#define SCAN_BATCH_REQ_RESULT               BIT(2U)
#endif

#define SCAN_NLO_CHECK_SSID_ONLY    0x00000001U
#define SCAN_NLO_DEFAULT_INTERVAL           30000U
/* PNO min period 30s, max period 300s */
#define SCAN_NLO_MIN_INTERVAL               30U
#define SCAN_NLO_MAX_INTERVAL               300U
#define SCN_BSS_JOIN_FAIL_THRESOLD          4U
...
VOID scanReportBss2Cfg80211(IN P_ADAPTER_T prAdapter,
	IN ENUM_BSS_TYPE_T eBSSType, IN P_BSS_DESC_T prSpecificBssDesc);
