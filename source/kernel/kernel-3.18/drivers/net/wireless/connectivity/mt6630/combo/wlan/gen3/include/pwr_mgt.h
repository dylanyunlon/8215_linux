...
#define ACQUIRE_POWER_CONTROL_FROM_PM(_prAdapter) \
	{ \
		if ((_prAdapter)->fgIsFwOwn != FALSE) { \
			if (nicpmSetDriverOwn(_prAdapter) == TRUE) \
				DBGLOG(INIT, TRACE, "Pm Set Driver Own\n"); \
		} \
...
#define RECLAIM_POWER_CONTROL_TO_PM(_prAdapter, _fgEnableGINT_in_IST) \
	{ \
		ASSERT_BOOLEAN((_prAdapter)->u4PwrCtrlBlockCnt != 0U); \
		/* Decrease Block to Enter Low Power Semaphore count */ \
		GLUE_DEC_REF_CNT((_prAdapter)->u4PwrCtrlBlockCnt); \
		if ((_prAdapter)->fgWiFiInSleepyState != FALSE \
			&& ((_prAdapter)->u4PwrCtrlBlockCnt == 0U)) { \
