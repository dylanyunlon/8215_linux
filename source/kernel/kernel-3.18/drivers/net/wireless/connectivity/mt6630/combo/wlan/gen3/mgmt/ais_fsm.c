...
static VOID aisFuncNotifyP2PDoChannelSwitch(IN P_ADAPTER_T prAdapter);
#endif
...
VOID aisFsmInit(IN P_ADAPTER_T prAdapter)
{
	...
	/*Autochips Add:reset ucScanTimeoutTimes value*/
...
VOID aisFsmStateInit_JOIN(IN P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc)
{
	...
	if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_DISCONNECTED) {
...
#endif
		default:
			ASSERT_BOOLEAN(!(prConnSettings->eAuthMode
				== AUTH_MODE_WPA_NONE));
			DBGLOG(AIS, ERROR,
		...
		ASSERT_BOOLEAN(prBssDesc->eBSSType == BSS_TYPE_INFRASTRUCTURE);
...
#if (CFG_SUPPORT_WPA3 == 1)
		if (prConnSettings->eAuthMode == AUTH_MODE_WPA3_SAE)
			prAisFsmInfo->ucAvailableAuthTypes = (UINT_8) AUTH_TYPE_SAE;
#endif
...
VOID aisFsmSteps(IN P_ADAPTER_T prAdapter, ENUM_AIS_STATE_T eNextState)
{
	...
	BOOLEAN fgIsWaitingForScanDone = FALSE;
	...
	do {
		...
		case AIS_STATE_IDLE:
			...
			cnmTimerStopTimer(prAdapter, &prAisFsmInfo->rScanDoneTimer);
			// Autochips Add
			fgIsWaitingForScanDone = timerPendingTimer(&prAisFsmInfo->rScanDoneTimer);
			// Add end
			...
			// Autochips Add
			if (fgIsWaitingForScanDone && AIS_STATE_SCAN == eNextState) {
				DBGLOG(AIS, INFO, "restart rScanDoneTimer\n");
...
#if CFG_SUPPORT_STA_AP_SCC
			aisFuncNotifyP2PDoChannelSwitch(prAdapter);
#endif
...
VOID aisFsmRunEventScanDone(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	ASSERT_BOOLEAN(prScanDoneMsg->ucBssIndex
...
VOID aisFsmRunEventAbort(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	DBGLOG(AIS, LOUD, "EVENT-ABORT: Current State %d, ucReasonOfDisconnect:%d\n",
...
VOID aisFsmRunEventFoundIBSSPeer(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	ASSERT_BOOLEAN(prAisIbssPeerFoundMsg->ucBssIndex
		== prAdapter->prAisBssInfo->ucBssIndex);
...
// Add end
	...
	// Autochips Add
	DBGLOG(AIS, STATE, "ucScanTimeoutTimes=%d\n", ucScanTimeoutTimes);
	// Add end
	ucScanTimeoutTimes++;
	if (ucScanTimeoutTimes > SCAN_DONE_TIMEOUT_TIMES_LIMIT) {
		DBGLOG(AIS, ERROR, "[Scan done timeout more than %d times!]\n", SCAN_DONE_TIMEOUT_TIMES_LIMIT);
		glSetRstReason(RST_SCAN_DONE_TIMEOUT);
...
VOID aisFsmRunEventJoinTimeout(IN P_ADAPTER_T prAdapter, ULONG ulParamPtr)
{
	...
	case AIS_STATE_JOIN:
		...
		if (prAisFsmInfo->prTargetBssDesc->ucJoinFailureCount < JOIN_MAX_RETRY_FAILURE_COUNT) {
			...
			eNextState = AIS_STATE_SEARCH;
			DBGLOG(AIS, INFO, "joinFailCount=%d and Retreat to AIS_STATE_SEARCH !\n",
			...
			eNextState = AIS_STATE_WAIT_FOR_NEXT_SCAN;
			DBGLOG(AIS, INFO, "retreat to AIS_STATE_WAIT_FOR_NEXT_SCAN"
					"state for next try!\n");
		} else
			if (prAisFsmInfo->rJoinReqTime != 0 &&
				!CHECK_FOR_TIMEOUT(rCurrentTime,
			...
			eNextState = AIS_STATE_JOIN_FAILURE;
			DBGLOG(AIS, INFO, "retreat to AIS_STATE_JOIN_FAILURE"
					"to terminate join operation!\n");
		}
...
VOID aisFsmScanRequest(IN P_ADAPTER_T prAdapter, IN P_PARAM_SSID_T prSsid, IN PUINT_8 pucIe, IN UINT_32 u4IeLength)
{
	...
	ASSERT(prAdapter);
	ASSERT_BOOLEAN(u4IeLength <= MAX_IE_LENGTH);
		     ...
		     IN UINT_8 ucSsidNum, IN P_PARAM_SSID_T prSsid, IN PUINT_8 pucIe, IN UINT_32 u4IeLength)
{
	...
	ASSERT(prAdapter);
	ASSERT_BOOLEAN(ucSsidNum <= SCN_SSID_MAX_NUM);
	ASSERT_BOOLEAN(u4IeLength <= MAX_IE_LENGTH);
...
VOID aisFsmRunEventChGrant(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (prAisFsmInfo->eCurrentState == AIS_STATE_REQ_CHANNEL_JOIN && prAisFsmInfo->ucSeqNumOfChReq == ucTokenID) {
		...
		/* Two cases can switch to join state:
		 * 1. There's an available PMKSA in wpa_supplicant
		 * 2. current state is disconnected. In this case, supplicant may not get a valid pmksa,
		 * so no pmkid will be passed to driver, so we no need to wait pmkid anyway.
		 */
		prEntry = rsnSearchPmkidEntry(prAdapter, prAisFsmInfo->prTargetBssDesc->aucBSSID,
				prAisBssInfo->ucBssIndex);
		if (prEntry)
...
VOID aisFsmRunEventMgmtFrameTx(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		ASSERT_BOOLEAN((prAdapter != NULL) && (prMsgHdr != NULL));
			      ...
			      IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	do {
		ASSERT_BOOLEAN((prAdapter != NULL) && (prMsduInfo != NULL));
		   ...
		   IN P_AIS_MGMT_TX_REQ_INFO_T prMgmtTxReqInfo, IN P_MSDU_INFO_T prMgmtTxMsdu, IN UINT_64 u8Cookie)
{
	...
	do {
		ASSERT_BOOLEAN((prAdapter != NULL)
			&& (prMgmtTxReqInfo != NULL));
		...
		ASSERT_BOOLEAN(prMgmtTxReqInfo->prMgmtTxMsdu == NULL);
...
VOID aisFuncValidateRxActionFrame(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		ASSERT_BOOLEAN((prAdapter != NULL) && (prSwRfb != NULL));
...
VOID aisRefreshFWKBlacklist(P_ADAPTER_T prAdapter)
{
	...
	LINK_FOR_EACH_ENTRY(prEntry, prBlackList,
		rLinkEntry, struct AIS_BLACKLIST_ITEM) {
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief This function will notify AP Mode to do channel switch
 * if AP mode channel not same with STA Mode.
 *
 * @param[in] prAdapter          Pointer to the Adapter structure.
 *
 * @retval none
...
static VOID aisFuncNotifyP2PDoChannelSwitch(IN P_ADAPTER_T prAdapter)
{
	P_MSG_HDR_T prP2pChannelSwitchMsg = NULL;
	P_BSS_INFO_T prAisBssInfo = (P_BSS_INFO_T) NULL;

		/*Check channel info*/
	prAisBssInfo = prAdapter->prAisBssInfo;
	if (prAdapter->rWifiVar.ucApChannel == prAisBssInfo->ucPrimaryChannel)
		return;

	prP2pChannelSwitchMsg = cnmMemAlloc(prAdapter,
		RAM_TYPE_MSG, sizeof(P_MSG_HDR_T));

	if (prP2pChannelSwitchMsg == NULL)
		ASSERT_BOOLEAN(FALSE);

	prP2pChannelSwitchMsg->eMsgId = MID_MNY_P2P_AP_CHANNEL_SWITCH;
	mboxSendMsg(prAdapter, MBOX_ID_0,
		prP2pChannelSwitchMsg, MSG_SEND_METHOD_BUF);
}
#endif
