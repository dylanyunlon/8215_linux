...
APPEND_VAR_IE_ENTRY_T txBcnIETable[] = {
...
#endif
	// Autochips Add
	, {(ELEM_HDR_LEN + ELEM_MAX_LEN_INTERWORKING), NULL, rlmRspGenerateInterworkingIE}	/* 107 */
	...
	// Autochips Add
	, {(ELEM_HDR_LEN + ELEM_MAX_LEN_APPLE_DEVICE), NULL, mqmGenerateAppleDeviceIE}	/* 221 */
#if CFG_ENABLE_WIFI_DIRECT
			   ...
			   IN ENUM_STA_TYPE_T eStaType, IN UINT_8 ucBssIndex, IN P_BSS_DESC_T prBssDesc)
{
	...
	if (ucNonHTPhyTypeSet) {
		...
		ASSERT_BOOLEAN(prStaRec->ucPhyTypeSet & PHY_TYPE_SET_802_11N);
			 ...
			 IN P_BSS_INFO_T prBssInfo, IN P_BSS_DESC_T prBssDesc, IN UINT_8 ucRCPI)
{
	...
	if (fgIsSameBSSID) {
		...
		if (!prStaRec) {
			...
			ASSERT_BOOLEAN((prStaRec->ucBssIndex
...
VOID ibssInitForAdHoc(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prBssInfo)
{
	...
	ASSERT(prBssInfo);
	ASSERT_BOOLEAN(prBssInfo->eCurrentOPMode == OP_MODE_IBSS);
...
VOID bssInitForAP(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prBssInfo, IN BOOLEAN fgIsRateUpdate)
{
	...
	DBGLOG(BSS, LOUD, "\n");
	/*Set BE CWmax default for 11n ap cert wmm case 4.2.22/23*/
	if (prAdapter->rWifiVar.ucCert11nMode == 1)
		auCWmaxLog2[0] = 10;

	ASSERT(prBssInfo);
	ASSERT_BOOLEAN((prBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT)
