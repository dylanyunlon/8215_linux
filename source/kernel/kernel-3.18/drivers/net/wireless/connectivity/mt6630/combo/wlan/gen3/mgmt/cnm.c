...
VOID cnmChMngrRequestPrivilege(P_ADAPTER_T prAdapter, P_MSG_HDR_T prMsgHdr)
{
	...
	ASSERT_BOOLEAN(prCmdBody->ucBssIndex <= MAX_BSS_INDEX);
...
VOID cnmChMngrHandleChEvent(P_ADAPTER_T prAdapter, P_WIFI_EVENT_T prEvent)
{
	...
	ASSERT_BOOLEAN(prEventBody->ucBssIndex <= MAX_BSS_INDEX);
	ASSERT_BOOLEAN(prEventBody->ucStatus == EVENT_CH_STATUS_GRANT);
