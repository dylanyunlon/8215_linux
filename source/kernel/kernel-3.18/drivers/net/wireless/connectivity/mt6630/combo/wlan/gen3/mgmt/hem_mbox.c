...
static PUINT_8 apucDebugMsg[] = {
	...
	(PUINT_8) DISP_STRING("MID_MNY_P2P_AP_CHANNEL_SWITCH"),
#endif
...
#endif
#if CFG_SUPPORT_STA_AP_SCC
	{MID_MNY_P2P_AP_CHANNEL_SWITCH, p2pRoleFsmRunEventSetAPModeChn},
#endif
...
#define MBOX_HNDL_MSG(prAdapter, prMsg) do { \
	...
	if (arMsgMapTable[prMsg->eMsgId].pfMsgHndl) { \
		...
		KAL_ACQUIRE_MUTEX2(prAdapter, MUTEX_P2P_ROLE_FSM_INFO); \
		arMsgMapTable[prMsg->eMsgId].pfMsgHndl(prAdapter, prMsg); \
		KAL_RELEASE_MUTEX2(prAdapter, MUTEX_P2P_ROLE_FSM_INFO); \
	} \
	else { \
...
VOID mboxInitMsgMap(VOID)
{
	...
	ASSERT_BOOLEAN((sizeof(arMsgMapTable)
		/ sizeof(MSG_HNDL_ENTRY_T)) == MID_TOTAL_NUM);
	...
	for (i = 0; i < MID_TOTAL_NUM; i++) {
		...
		ASSERT_BOOLEAN(idx < MID_TOTAL_NUM);
	...
	for (i = 0; i < MID_TOTAL_NUM; i++) {
		ASSERT_BOOLEAN(arMsgMapTable[i].eMsgId == (ENUM_MSG_ID_T) i);
...
VOID mboxSetup(IN P_ADAPTER_T prAdapter, IN ENUM_MBOX_ID_T eMboxId)
{
	...
	ASSERT_BOOLEAN(eMboxId < MBOX_ID_TOTAL_NUM);
	ASSERT(prAdapter);
