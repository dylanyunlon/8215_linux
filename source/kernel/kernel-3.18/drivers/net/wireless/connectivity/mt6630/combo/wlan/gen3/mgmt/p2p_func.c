	       ...
	       IN P_P2P_CONNECTION_REQ_INFO_T prP2pConnReqInfo, IN P_P2P_CHNL_REQ_INFO_T prP2pChnlReqInfo)
{
	do {
		...
		if (prBssInfo->eBand == BAND_5G) {
			...
			ASSERT_BOOLEAN(prP2pConnReqInfo->eConnRequest
...
WLAN_STATUS p2pFuncRoleToBssIdx(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx, OUT PUINT_8 pucBssIdx)
{
	...
	do {
		...
		if (!prAdapter->rWifiVar.aprP2pRoleFsmInfo[ucRoleIdx]) {
			DBGLOG(P2P, WARN, "invalid aprP2pRoleFsmInfo, ignore\n");
			rWlanStatus = WLAN_STATUS_FAILURE;
			break;
		}
		  ...
		  IN P_STA_RECORD_T prStaRec, IN BOOLEAN fgSendDeauth, IN UINT_16 u2ReasonCode)
{
	...
	do {
		...
		if (prP2pBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
			...
			wiphy = prGlueInfo->prP2PInfo->prDevHandler->ieee80211_ptr->wiphy;
#endif
#if 1 // Autochips modified
			atcKalP2PGOStationUpdate(prAdapter->prGlueInfo, prP2pRoleFsmInfo->ucRoleIndex, prStaRec, FALSE
					, u2ReasonCode);
#else
			kalP2PGOStationUpdate(prAdapter->prGlueInfo, prP2pRoleFsmInfo->ucRoleIndex, prStaRec, FALSE);
#endif // Autochips modified end
#if (CFG_NOTIFY_REASON_CODE == 1)
			mtk_cfg80211_vendor_event_sta_disconnect(wiphy,
				prGlueInfo->prP2PInfo->prDevHandler->ieee80211_ptr, prStaRec, u2ReasonCode);
#endif
		} else {
		...
		if (fgSendDeauth) {
#if (CFG_NOTIFY_REASON_CODE == 1) && (CFG_SUPPORT_SOFTAP_WPA3 == 1)
...
VOID p2pFuncValidateRxActionFrame(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		if (prAdapter->u4OsPacketFilter & PARAM_PACKET_FILTER_ACTION_FRAME) {
			...
			DBGLOG(P2P, LOUD,
				"do not indicate action. filter: 0x%x\n",
				prAdapter->u4OsPacketFilter);
			  ...
			  IN P_BSS_INFO_T prP2pBssInfo, IN PUINT_8 pucIEInfo, IN UINT_32 u4IELen)
{
	...
	DBGLOG(P2P, INFO, "p2pFuncParseBeaconContent\n");
	do {
		ASSERT_BREAK((prAdapter != NULL) && (prP2pBssInfo != NULL));
		...
		IE_FOR_EACH(pucIE, u4IELen, u2Offset) {
			...
			case ELEM_ID_OBSS_SCAN_PARAMS:	/* 74 *//* V */
				...
				break;
			case ELEM_ID_INTERWORKING:	/* 107 */ /* V */
				{
					DBGLOG(P2P, INFO, "ucId:%x, ucLength:%x, ucInDaConn:%x, "
						"ucVenueGroup:%x, ucVenueType:%x\n",
					Interworking_IE(pucIE)->ucId,
					Interworking_IE(pucIE)->ucLength,
					Interworking_IE(pucIE)->ucInDaConn,
					Interworking_IE(pucIE)->ucVenueGroup,
					Interworking_IE(pucIE)->ucVenueType);
					prP2pBssInfo->ucId =
							Interworking_IE(pucIE)->ucId;
					prP2pBssInfo->ucLength =
							Interworking_IE(pucIE)->ucLength;
					prP2pBssInfo->ucInDaConn =
							Interworking_IE(pucIE)->ucInDaConn;
					prP2pBssInfo->ucVenueGroup =
							Interworking_IE(pucIE)->ucVenueGroup;
					prP2pBssInfo->ucVenueType =
							Interworking_IE(pucIE)->ucVenueType;
				}
				break;
			   ...
			   IN P_P2P_SPECIFIC_BSS_INFO_T prP2pSpecificBssInfo, OUT BOOL *ucNewSecMode)
{
	do {
		...
		if (rsnParseCheckForWFAInfoElem(prAdapter, pucIE, &ucOuiType, &u2SubTypeVersion)) {
			...
			prP2pSpecificBssInfo->u2AttributeLen += IE_SIZE(pucIE);
			DBGLOG(P2P, INFO, "ucId:%x, ucLength:%x,"
				"aucOui:%x%x%x, ucOuiType:%x\n",
				Apple_Device_IE(pucIE)->ucId,
				Apple_Device_IE(pucIE)->ucLength,
				Apple_Device_IE(pucIE)->aucOui[0],
				Apple_Device_IE(pucIE)->aucOui[1],
				Apple_Device_IE(pucIE)->aucOui[2],
				Apple_Device_IE(pucIE)->ucOuiType);
				prP2pSpecificBssInfo->ucDeviceLength =
					Apple_Device_IE(pucIE)->ucLength;
				kalMemCopy(prP2pSpecificBssInfo->ucDeviceElement,
					Apple_Device_IE(pucIE)->ucElement,
			...
			IN P_P2P_CHNL_REQ_INFO_T prChnlReqInfo, IN P_P2P_SCAN_REQ_INFO_T prScanReqInfo)
{
	...
	do {
		...
		ASSERT_BOOLEAN(prConnReqInfo->eConnRequest
		    ...
		    IN PUINT_8 pucIE, IN UINT_16 u2IELength, OUT PPUINT_8 ppucAttriList, OUT PUINT_16 pu2AttriListLen)
{
	...
	if (pucAttriListStart) {
		...
		ASSERT_BOOLEAN(u2Offset == u2AttriListLen);
				    ...
				    IN PUINT_16 pu2Offset, IN PUINT_8 pucBuf, IN UINT_16 u2BufSize)
{
	...
	ASSERT_BOOLEAN(u2BufSize >= ((*pu2Offset) + (UINT_16) u4AttriLen));
...
UINT_32 wfdFuncCalculateWfdIELenForAssocRsp(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, IN P_STA_RECORD_T prStaRec)
{
	...
	if (IS_STA_P2P_TYPE(prStaRec) && (prWfdCfgSettings->ucWfdEnable > 0)) {
		...
		u2EstimatedExtraIELen = prAdapter->prGlueInfo->prP2PInfo->u2WFDIELen;
		ASSERT_BOOLEAN(u2EstimatedExtraIELen <= 400);
	}
