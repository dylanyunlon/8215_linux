...
static UINT_8 rlmObssChnlLevelIn2G4(P_BSS_INFO_T prBssInfo, UINT_8 ucPriChannel, ENUM_CHNL_EXT_T eExtend)
{
	...
	ASSERT_BOOLEAN(ucCenterChannel >= 1 && ucCenterChannel <= 14);
	...
	ASSERT_BOOLEAN(prBssInfo->auc2G_NonHtChnlList[0] <= CHNL_LIST_SZ_2G);
	...
	ASSERT_BOOLEAN(prBssInfo->auc2G_20mReqChnlList[0] <= CHNL_LIST_SZ_2G);
	...
	ASSERT_BOOLEAN(prBssInfo->auc2G_PriChnlList[0] <= CHNL_LIST_SZ_2G);
	...
	ASSERT_BOOLEAN(prBssInfo->auc2G_SecChnlList[0] <= CHNL_LIST_SZ_2G);
...
static UINT_8 rlmObssChnlLevelIn5G(P_BSS_INFO_T prBssInfo, UINT_8 ucPriChannel, ENUM_CHNL_EXT_T eExtend)
{
	...
	if (eExtend == CHNL_EXT_SCA)
		...
		return CHNL_LEVEL0;
	ASSERT_BOOLEAN(ucSecChannel >= 36);

	/* Check 5G primary channel list */
	ASSERT_BOOLEAN(prBssInfo->auc5G_PriChnlList[0] <= CHNL_LIST_SZ_5G);
	...
	ASSERT_BOOLEAN(prBssInfo->auc5G_NonHtChnlList[0] <= CHNL_LIST_SZ_5G);
	...
	ASSERT_BOOLEAN(prBssInfo->auc5G_SecChnlList[0] <= CHNL_LIST_SZ_5G);
...
VOID rlmObssScanExemptionRsp(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo, P_SW_RFB_T prSwRfb)
{
	...
	ASSERT_BOOLEAN((WLAN_MAC_HEADER_LEN + 5) <= PUBLIC_ACTION_MAX_LEN);
