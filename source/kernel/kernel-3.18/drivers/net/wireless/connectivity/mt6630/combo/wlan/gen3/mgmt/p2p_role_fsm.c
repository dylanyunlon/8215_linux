...
UINT_8 p2pRoleFsmInit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx)
{
	...
	do {
		...
		cnmTimerInitTimer(prAdapter,
			&(prP2pRoleFsmInfo->rP2pRoleFsmP2pChannelSwitchTimer),
			(PFN_MGMT_TIMEOUT_FUNC)p2pRoleFsmRunEventReSetAPModeChn,
...
VOID p2pRoleFsmUninit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx)
{
	...
	do {
		...
		KAL_ACQUIRE_MUTEX(prAdapter, MUTEX_P2P_ROLE_FSM_INFO);
		P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, ucRoleIdx) = NULL;

		if (prP2pRoleFsmInfo)
			kalMemFree(prP2pRoleFsmInfo, VIR_MEM_TYPE, sizeof(P2P_ROLE_FSM_INFO_T));
		KAL_RELEASE_MUTEX(prAdapter, MUTEX_P2P_ROLE_FSM_INFO);
			  ...
			  IN P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo, IN ENUM_P2P_ROLE_STATE_T eNextState)
{
	...
	if((!prAdapter)||(!prP2pRoleFsmInfo)) {
		DBGLOG(P2P, ERROR,"prAdapter or prP2pRoleFsmInfo is NULL!\n");
		return;
	}
	prP2pRoleBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, prP2pRoleFsmInfo->ucBssIndex);
	if (!prP2pRoleBssInfo) {
		DBGLOG(P2P, ERROR,"prP2pRoleBssInfo is NULL!\n");
		return;
	}
		...
		case OP_MODE_ACCESS_POINT:
			...
			if (authProcessRxDeauthFrame(prSwRfb,
				...
				if (rsnCheckBipKeyInstalled(prAdapter, prStaRec)) {
					...
					/* 4.3.3.1 send unprotected deauth reason 6/7 ,move to p2pFuncDisconnect() in*/
...
VOID p2pRoleFsmRunEventStartAP(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		if (p2pFuncIsAPMode(prAdapter->rWifiVar.prP2PConnSettings)) {
			...
			/* Modify for AP + STA work in scc mode*/
#if CFG_SUPPORT_STA_AP_SCC
			...
			prApSetting = &prAdapter->prGlueInfo
				->prP2PInfo->apsetting;
			prApSetting->startAP = TRUE;
			prApSetting->ucRoleIdx = prP2pStartAPMsg->ucRoleIdx;
#endif
...
VOID p2pRoleFsmRunEventScanRequest(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		ASSERT_BOOLEAN(prScanReqInfo->fgIsScanRequest == FALSE);
			    ...
			    IN P_MSG_HDR_T prMsgHdr, IN P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo)
{
	...
	do {
		//ASSERT_BREAK((prAdapter != NULL) && (prMsgHdr != NULL) && (prP2pRoleFsmInfo != NULL));
		if (prAdapter == NULL || prMsgHdr == NULL || prP2pRoleFsmInfo == NULL) {
			DBGLOG(P2P, WARN, "prAdapter,prMsgHdr or prP2pRoleFsmInfo NULL\n");
			return;
		}
		...
		prChnlReqInfo = &(prP2pRoleFsmInfo->rChnlReqInfo);
		if (prMsgChGrant == NULL || prChnlReqInfo == NULL) {
			DBGLOG(P2P, WARN, "prMsgChGrant or prChnlReqInfo NULL\n");
			return;
		}
		...
		if (ucTokenID == prChnlReqInfo->ucSeqNumOfChReq) {
			...
			ASSERT_BOOLEAN(!prChnlReqInfo->fgIsChannelRequested);
		}
...
p2pRoleFsmRunEventAAASuccess(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_BSS_INFO_T prP2pBssInfo)
{
	...
	do {
		...
		ASSERT_BOOLEAN(prP2pBssInfo->ucBssIndex < P2P_DEV_BSS_INDEX);
...
VOID p2pRoleFsmRunEventSwitchOPMode(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		ASSERT_BOOLEAN(prP2pRoleFsmInfo->ucBssIndex
			< P2P_DEV_BSS_INDEX);
			       ...
			       IN UINT_8 ucBssIdx, IN P_EVENT_UPDATE_NOA_PARAMS_T prEventUpdateNoaParam)
{
	...
	ASSERT_BOOLEAN(prP2pSpecificBssInfo->ucNoATimingCount
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief This function will reset the AP Mode channel.
 *
 * @param[in] prAdapter          Pointer to the Adapter structure.
 *
 * @return (none)
 */
/*----------------------------------------------------------------------------*/
VOID p2pRoleFsmRunEventReSetAPModeChn(
	IN P_ADAPTER_T prAdapter, IN ULONG ulParamPtr)
{
	P_BSS_INFO_T prAisBssInfo = (P_BSS_INFO_T) NULL;
	struct AP_PARAMS *prApSetting = NULL;
	P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo = (P_P2P_ROLE_FSM_INFO_T) NULL;
	...
	prP2pRoleFsmInfo = P2P_ROLE_INDEX_2_ROLE_FSM_INFO(
		prAdapter, prApSetting->ucRoleIdx);

	cnmTimerStopTimer(prAdapter,
		&(prP2pRoleFsmInfo->rP2pRoleFsmP2pChannelSwitchTimer));

	/*Set channel*/
	rRfChnlInfo.ucChannelNum = prAisBssInfo->ucPrimaryChannel;
	if (rRfChnlInfo.ucChannelNum <= 14)
		rRfChnlInfo.eBand = BAND_2G4;
	else
		rRfChnlInfo.eBand = BAND_5G;
	p2pFuncSetChannel(prAdapter, prApSetting->ucRoleIdx, &rRfChnlInfo);

	/*Restart AP Mode*/
	prP2pStartAPMsg = (P_MSG_P2P_START_AP_T) cnmMemAlloc(prAdapter,
	...
	/* Copy NO SSID. */
	prP2pStartAPMsg->eHiddenSsidType = prApSetting->HiddenSsidType;
	prP2pStartAPMsg->ucRoleIdx = prApSetting->ucRoleIdx;

	kalP2PSetRole(prAdapter->prGlueInfo, 2);
	COPY_SSID(prP2pStartAPMsg->aucSsid,
		prP2pStartAPMsg->u2SsidLen,
		prApSetting->Ssid, prApSetting->SsidLen);

	//p2pRoleFsmRunEventStartAP(prAdapter, (P_MSG_HDR_T) prP2pStartAPMsg);
 ...
 */
/*----------------------------------------------------------------------------*/
VOID p2pRoleFsmRunEventSetAPModeChn(
	IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	P_BSS_INFO_T prAisBssInfo = (P_BSS_INFO_T) NULL;
	struct AP_PARAMS *prApSetting = NULL;
	P_BSS_INFO_T prP2pBssInfo = (P_BSS_INFO_T) NULL;
	...
	/*Only work for AP Mode*/
	if (!p2pFuncIsAPMode(prAdapter->rWifiVar.prP2PConnSettings))
		return;

	/*Check start AP mode or not*/
	prApSetting = &prAdapter->prGlueInfo->prP2PInfo->apsetting;
	if (prApSetting->startAP == FALSE)
		return;
	...
	prP2pSwitchMode = cnmMemAlloc(prAdapter,
		RAM_TYPE_MSG, sizeof(MSG_P2P_SWITCH_OP_MODE_T));

	if (prP2pSwitchMode == NULL)
		return;

	prP2pSwitchMode->rMsgHdr.eMsgId = MID_MNY_P2P_STOP_AP;
	prP2pSwitchMode->ucRoleIdx = prApSetting->ucRoleIdx;
	//p2pRoleFsmRunEventStopAP(prAdapter, (P_MSG_HDR_T) prP2pSwitchMode);
	mboxSendMsg(prAdapter, MBOX_ID_0,
