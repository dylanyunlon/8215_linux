...
VOID secInit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	prAdapter->rMib.CfgCiphersTbl[0].CfgPWCipher = WPA_CIPHER_SUITE_WEP40;
	prAdapter->rMib.CfgCiphersTbl[1].CfgPWCipher = WPA_CIPHER_SUITE_TKIP;
	prAdapter->rMib.CfgCiphersTbl[2].CfgPWCipher = WPA_CIPHER_SUITE_CCMP;
	prAdapter->rMib.CfgCiphersTbl[3].CfgPWCipher = WPA_CIPHER_SUITE_WEP104;

	prAdapter->rMib.CfgCiphersTbl[4].CfgPWCipher = RSN_CIPHER_SUITE_WEP40;
	prAdapter->rMib.CfgCiphersTbl[5].CfgPWCipher = RSN_CIPHER_SUITE_TKIP;
	prAdapter->rMib.CfgCiphersTbl[6].CfgPWCipher = RSN_CIPHER_SUITE_CCMP;
	prAdapter->rMib.CfgCiphersTbl[7].CfgPWCipher = RSN_CIPHER_SUITE_WEP104;

	for (i = 0; i < (UINT_8)MAX_NUM_SUPPORTED_CIPHER_SUITES; i++)
		prAdapter->rMib.CfgCiphersTbl[i].CfgPWCipherEnabled = FALSE;
	...
	for (i = 0; i < (UINT_8)MAX_NUM_SUPPORTED_AKM_SUITES; i++) {
...
#if 1
	if ((prRxStatus->u2StatusFlag & (UINT_16)RXS_DW2_RX_CLASSERR_BITMAP) ==
		(UINT_16)RXS_DW2_RX_CLASSERR_VALUE) {
		...
		if (authSendDeauthFrame(prAdapter, NULL, NULL, prSwRfb,
				REASON_CODE_CLASS_3_ERR,
				(PFN_TX_DONE_HANDLER)NULL) !=
				WLAN_STATUS_SUCCESS) {
			DBGLOG(RSN, ERROR, "Send Deauth Error!\n");
		}
		return FALSE;
		/* } */
	}
...
VOID secSetCipherSuite(IN P_ADAPTER_T prAdapter, IN UINT_32 u4CipherSuitesFlags)
{
	...
	UINT_32 i;
	P_CFG_PW_CIPHERS_ENTRY prCipEntry;
	...
	if (u4CipherSuitesFlags == (UINT_32)CIPHER_FLAG_NONE) {
		/* Disable all the pairwise cipher suites. */
		for (i = 0; i < (UINT_32)MAX_NUM_SUPPORTED_CIPHER_SUITES; i++)
			prMib->CfgCiphersTbl[i].CfgPWCipherEnabled = FALSE;
	...
	for (i = 0; i < (UINT_32)MAX_NUM_SUPPORTED_CIPHER_SUITES; i++) {
		prCipEntry = &prMib->CfgCiphersTbl[i];

		switch (prCipEntry->CfgPWCipher) {
		case WPA_CIPHER_SUITE_WEP40:
			if ((u4CipherSuitesFlags &
				(UINT_32)CIPHER_FLAG_WEP40) != 0U)
				prCipEntry->CfgPWCipherEnabled = TRUE;
			else
				prCipEntry->CfgPWCipherEnabled = FALSE;
			break;
		...
		default:
			DBGLOG(RSN, TRACE,
				"Default Case in secSetCipherSuite\n");
			break;
		}
	}
...
VOID secClearPmkid(IN P_ADAPTER_T prAdapter)
{
	...
	kalMemZero((PVOID) prAisSpecBssInfo->arPmkidCandicate,
		sizeof(PMKID_CANDICATE_T) * (UINT_32)CFG_MAX_PMKID_CACHE);
	kalMemZero((PVOID) prAisSpecBssInfo->arPmkidCache,
		sizeof(PMKID_ENTRY_T) * (UINT_32)CFG_MAX_PMKID_CACHE);
}
...
BOOLEAN secEnabledInAis(IN P_ADAPTER_T prAdapter)
{
	...
	ASSERT_BOOLEAN(prAdapter->rWifiVar.rConnSettings.eEncStatus <
...
enum EAPOL_KEY_TYPE secGetEapolKeyType(PUINT_8 pucPacket)
{
	UINT_16 u2EthType = 0;
	UINT_16 u2KeyInfo = 0;
	enum EAPOL_KEY_TYPE enKeyType = EAPOL_KEY_NOT_KEY;
	...
	if (u2EthType != (UINT_16)ETH_P_1X)
		return EAPOL_KEY_NOT_KEY;

	u2KeyInfo = ((UINT_16)pucPacket[5U+ETHER_HEADER_LEN])<<8U |
				((UINT_16)pucPacket[6U+ETHER_HEADER_LEN]);
	...
	*/
	...
	switch (u2KeyInfo & 0x388U) {
	case 0x388:
		enKeyType = EAPOL_KEY_3_OF_4;
		break;
	...
	case 0x308: /* if BIT(9) in 3/4 is missing, it is hard to distinguish 4/4 and 2/4 */
		enKeyType = EAPOL_KEY_4_OF_4;
		break;
	case 0x108: /* BIT(8) | BIT(3) */
		enKeyType = EAPOL_KEY_2_OF_4;
		break;
	case 0x88: /* BIT(3) | BIT(7) */
		enKeyType = EAPOL_KEY_1_OF_4;
		break;
	case 0x180: /* don't check BIT(9) since some Authenticator may miss it */
		enKeyType = EAPOL_KEY_1_OF_2;
		break;
	case 0x100: /* don't check BIT(9) since some Authenticator may miss it */
		enKeyType = EAPOL_KEY_2_OF_2;
		break;
	default:
		DBGLOG(RSN, TRACE, "Default Case in secGetEapolKeyType\n");
		break;
	}
	return enKeyType;
}
...
#if CFG_SUPPORT_WAPI
			if (wlanQueryWapiMode(prAdapter) == TRUE)
				return FALSE;
#endif
		}
...
#if CFG_SUPPORT_802_11W
	if (prMsdu->ucPacketType == (UINT_8)TX_PACKET_TYPE_MGMT) {
...
UINT_8 secGetBmcWlanIndex(IN P_ADAPTER_T prAdapter, IN ENUM_NETWORK_TYPE_T eNetType, IN P_STA_RECORD_T prStaRec)
{
	...
	if (prAdapter->aprBssInfo[ucBssIndex]->fgTxBcKeyExist != FALSE) {
		if (prAdapter->aprBssInfo[ucBssIndex]->ucBMCWlanIndex ==
			(UINT_8)WTBL_RESERVED_ENTRY) {
			ucEntry = secPrivacySeekForBcEntry(prAdapter,
						ucBssIndex,
						prStaRec->aucMacAddr,
						prStaRec->ucIndex,
						CIPHER_SUITE_NONE,
	...
	ucEntry = secPrivacySeekForBcEntry(prAdapter, ucBssIndex,
				prStaRec->aucMacAddr, prStaRec->ucIndex,
				CIPHER_SUITE_NONE, 0xff, 0x0, 1U);
...
*/
/*----------------------------------------------------------------------------*/
BOOL secPrivacySeekForEntry(IN P_ADAPTER_T prAdapter,
		IN P_STA_RECORD_T prStaRec)
{
	...
	if (ucStartIDX > ucMaxIDX) {
		for (i = ucStartIDX; i > ucMaxIDX; i--) {
			if ((prWtbl[i].ucUsed != 0U) &&
				EQUAL_MAC_ADDR(&prStaRec->aucMacAddr[0],
				&prWtbl[i].aucMacAddr[0]) &&
				(prWtbl[i].ucPairwise != 0U)) {
				ucEntry = i;
				DBGLOG(RSN, TRACE,
	...
	if (ucEntry < (UINT_8)WTBL_SIZE) {

		prWtbl[ucEntry].ucUsed = 1;
		prWtbl[ucEntry].ucBssIndex = prStaRec->ucBssIndex;
		prWtbl[ucEntry].ucKeyId = 0;
		prWtbl[ucEntry].ucPairwise = 1;
		COPY_MAC_ADDR(prWtbl[ucEntry].aucMacAddr, prStaRec->aucMacAddr);
		prWtbl[ucEntry].ucStaIndex = prStaRec->ucIndex;

		prStaRec->ucWlanIndex = ucEntry;

		DBGLOG(RSN, INFO,
			"[Wlan index] BSS#%d keyid#%d P=%d" MACSTR
			" use WlanIndex#%d STAIdx=%d staType=%x\n",
			prStaRec->ucBssIndex, 0,
		...
		if (IS_AP_STA(prStaRec)) {
			prStaRec->ucBMCWlanIndex =
			secGetBmcWlanIndex(prAdapter,
					NETWORK_TYPE_AIS, prStaRec);
			ASSERT_BOOLEAN(prStaRec->ucBMCWlanIndex <
							(UINT_8)WTBL_SIZE);
		} else {
			/* DBGLOG(RSN, TRACE, ("AP or GO\n")); */
			/* For AP/GO, BC entry saved at BSS INFO */
			prStaRec->ucBMCWlanIndex = 255;
		}
#if DBG
...
VOID secPrivacyFreeSta(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	for (entry = 0; entry < (UINT_32)WTBL_SIZE; entry++) {
		if ((prWtbl[entry].ucUsed != 0U) &&
			(prStaRec->ucIndex == prWtbl[entry].ucStaIndex)) {
			secPrivacyFreeForEntry(prAdapter, (UINT_8)entry);
			 ...
			 IN UINT_8 ucAlg, IN UINT_8 ucKeyId, IN UINT_8 ucCurrentKeyId, IN UINT_8 ucTxRx)
{
	...
	if (ucAlg == (UINT_8)CIPHER_SUITE_WPI ||
		ucAlg == (UINT_8)CIPHER_SUITE_WEP40 ||
	    ucAlg == (UINT_8)CIPHER_SUITE_WEP104 ||
	    ucAlg == (UINT_8)CIPHER_SUITE_WEP128 ||
	    ucAlg == (UINT_8)CIPHER_SUITE_NONE) {
		fgCheckKeyId = FALSE;
	}
...
#endif
		if ((prWtbl[i].ucUsed != 0U) && (prWtbl[i].ucPairwise == 0U) &&
			(prWtbl[i].ucBssIndex == ucBssIndex) &&
			(EQUAL_MAC_ADDR(&prWtbl[i].aucMacAddr[0],
			&pucAddr[0]) || ((prBSSInfo != NULL) &&
			(prBSSInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT)))
		    ) {
			if (fgCheckKeyId == FALSE || prWtbl[i].ucKeyId == 0xffU
	...
	if (ucEntry < (UINT_8)WTBL_SIZE) {
		...
		prWtbl[ucEntry].ucPairwise = 0;
		kalMemCopy(&prWtbl[ucEntry].aucMacAddr[0],
			&pucAddr[0], MAC_ADDR_LEN);
	...
	ASSERT_BOOLEAN(ucEntry != (UINT_8)WTBL_ALLOC_FAIL);
...
UINT_8 secLookupStaRecIndexFromTA(P_ADAPTER_T prAdapter, PUINT_8 pucMacAddress)
{
	...
	for (i = 0; i < WTBL_SIZE; i++) {
		if (prWtbl[i].ucUsed != 0U) {
			if (EQUAL_MAC_ADDR(&pucMacAddress[0],
				&prWtbl[i].aucMacAddr[0]))
...
{
	enum EAPOL_KEY_TYPE enKeyType = (enum EAPOL_KEY_TYPE)ucEapolKeyType;
	...
	case EAPOL_KEY_3_OF_4:
		prBssInfo->ucKeyCmdAction = ucAction;
		if (ucAction == (UINT_8)SEC_QUEUE_KEY_COMMAND)
			prBssInfo->fgUnencryptedEapol = FALSE;
		/* If 3/4 is unencrypted,then 4/4 will be unencrypted */
		else
			prBssInfo->fgUnencryptedEapol = TRUE;
			DBGLOG(RSN, INFO,
				"ucKeyCmdAction is %d, prBssInfo->fgUnencryptedEapol is %d\n",
				prBssInfo->ucKeyCmdAction,
				prBssInfo->fgUnencryptedEapol);
		break;
	default:
		DBGLOG(RSN, TRACE, "Default Case in secSetKeyCmdAction\n");
		break;
	}
}
...
UINT_8 secGetBssIdxByNetType(P_ADAPTER_T prAdapter)
{
	...
	if ((fgP2pDevice == TRUE) && (fgAisDevice == TRUE)) {
