...
VOID rlmObssScanDone(P_ADAPTER_T prAdapter, P_MSG_HDR_T prMsgHdr)
{
	...
	if ((prBssInfo->eCurrentOPMode != OP_MODE_INFRASTRUCTURE) ||
	    ...
	    (prBssInfo->u2ObssScanInterval == 0U)) {
	 ...
	 */
	if ((prBssInfo->auc2G_20mReqChnlList[0] > 0U) ||
		(prBssInfo->auc2G_NonHtChnlList[0] > 0U)) {
		...
		if (prMsduInfo != NULL) {
			...
			prTxFrame->rBssCoexist.ucLength = 1U;
			prTxFrame->rBssCoexist.ucData =
				(prBssInfo->auc2G_20mReqChnlList[0] > 0U) ?
				(UINT_8)BSS_COEXIST_20M_REQ : 0U;

			u2PayloadLen = 2U + 3U;

			if (prBssInfo->auc2G_NonHtChnlList[0] > 0U) {
				...
				prTxFrame->rChnlReport.ucLength =
					prBssInfo->auc2G_NonHtChnlList[0] + 1U;
				prTxFrame->rChnlReport.ucRegulatoryClass = 81U;

				for (i = 0U; i < CHNL_LIST_SZ_2G &&
					i < prBssInfo->auc2G_NonHtChnlList[0];
					i++)
					prTxFrame->rChnlReport.aucChannelList[i] =
					prBssInfo->auc2G_NonHtChnlList[i + 1U];

				u2PayloadLen += (UINT_16)
					IE_SIZE(&prTxFrame->rChnlReport);
			}
			ASSERT_BOOLEAN((WLAN_MAC_HEADER_LEN + u2PayloadLen) <=
	...
	if (prBssInfo->u2ObssScanInterval > 0U) {
...
static VOID rlmObssScanTimeout(P_ADAPTER_T prAdapter, ULONG ulParamPtr)
{
	...
	/* WFD streaming */
		...
		if (prWfdCfgSettings->ucWfdEnable == TRUE) {
	...
	DBGLOG(RLM, INFO, "skip rlmObssTriggerScan\n");
}
...
VOID rlmObssTriggerScan(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo)
{
	...
	prScanReqMsg->ucSSIDType = (UINT_8)SCAN_REQ_SSID_WILDCARD;
