...
VOID roamingFsmInit(IN P_ADAPTER_T prAdapter)
{
	...
	prRoamingFsmInfo->rRoamingDiscoveryUpdateTime = 0U;
...
VOID roamingFsmUninit(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, LOUD,
		"->roamingFsmUninit(): Current Time = %u\n",
		kalGetTimeTick());
...
VOID roamingFsmSendCmd(IN P_ADAPTER_T prAdapter, IN P_CMD_ROAMING_TRANSIT_T prTransit)
{
	...
	DBGLOG(ROAMING, LOUD,
		"->roamingFsmSendCmd(): Current Time = %u\n",
		kalGetTimeTick());
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,
					(UINT_8)CMD_ID_ROAMING_TRANSIT,
					TRUE, FALSE, FALSE, NULL, NULL,
					(UINT_32)sizeof(CMD_ROAMING_TRANSIT_T),
					(PUINT_8) prTransit, NULL, 0);
...
VOID roamingFsmScanResultsUpdate(IN P_ADAPTER_T prAdapter)
{
	...
	if (prRoamingFsmInfo->fgIsEnableRoaming == FALSE)
		return;
...
static BOOLEAN roamingFsmIsNeedScan(IN P_ADAPTER_T prAdapter)
{
	...
	if (prBssDesc != NULL) {
		...
		if (prBssDesc->ucRCPI > 90U) {
			prAisBssInfo->ucRoamSkipTimes = 3U;
			prAisBssInfo->fgGoodRcpiArea = TRUE;
			prAisBssInfo->fgPoorRcpiArea = FALSE;
		} else {
			if (prAisBssInfo->fgGoodRcpiArea == TRUE) {
				prAisBssInfo->ucRoamSkipTimes--;
			} else if (prBssDesc->ucRCPI > 67U) {
				if (prAisBssInfo->fgPoorRcpiArea == FALSE) {
					prAisBssInfo->ucRoamSkipTimes = 2U;
		...
		if (prAisBssInfo->ucRoamSkipTimes == 0U) {
			prAisBssInfo->ucRoamSkipTimes = 3U;
			prAisBssInfo->fgPoorRcpiArea = FALSE;
			prAisBssInfo->fgGoodRcpiArea = FALSE;
...
VOID roamingFsmRunEventStart(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, EVENT,
		"EVENT-ROAMING START: Current Time = %u\n",
		kalGetTimeTick());
	...
	if (eNextState != prRoamingFsmInfo->eCurrentState) {
		rTransit.u2Event = (UINT_16)ROAMING_EVENT_START;
		rTransit.u2Data = (UINT_16)prAisBssInfo->ucBssIndex;
...
VOID roamingFsmRunEventDiscovery(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Param)
{
	...
	DBGLOG(ROAMING, EVENT,
		"EVENT-ROAMING DISCOVERY: Current Time = %u\n",
		kalGetTimeTick());
	...
	if (eNextState != prRoamingFsmInfo->eCurrentState) {
		...
		prBssDesc = scanSearchBssDescByBssid(prAdapter, prAisBssInfo->aucBSSID);
		if (prBssDesc != NULL) {
			prBssDesc->ucRCPI = (UINT_8) (u4Param & 0xffU);
...
VOID roamingFsmRunEventRoam(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, EVENT,
		"EVENT-ROAMING ROAM: Current Time = %u\n",
		kalGetTimeTick());
	...
	/* DISCOVERY -> ROAM */
	if (eNextState != prRoamingFsmInfo->eCurrentState) {
		rTransit.u2Event = (UINT_16)ROAMING_EVENT_ROAM;
...
VOID roamingFsmRunEventFail(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Reason)
{
	...
	/* ROAM -> DECISION */
	if (eNextState != prRoamingFsmInfo->eCurrentState) {
		rTransit.u2Event = (UINT_16)ROAMING_EVENT_FAIL;
		rTransit.u2Data = (UINT_16) (u4Reason & 0xffffU);
		roamingFsmSendCmd(prAdapter, (P_CMD_ROAMING_TRANSIT_T) & rTransit);
...
VOID roamingFsmRunEventAbort(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, TRACE,
		"EVENT-ROAMING ABORT: Current Time = %u\n",
		kalGetTimeTick());

	eNextState = ROAMING_STATE_IDLE;
	/* IDLE, DECISION, DISCOVERY, ROAM -> IDLE */
	if (eNextState != prRoamingFsmInfo->eCurrentState) {
		rTransit.u2Event = (UINT_16)ROAMING_EVENT_ABORT;
		roamingFsmSendCmd(prAdapter, (P_CMD_ROAMING_TRANSIT_T) & rTransit);
...
{
	DBGLOG(ROAMING, LOUD,
		"ROAMING Process Events: Current Time = %u\n",
		kalGetTimeTick());

	if (prTransit->u2Event == (UINT_16)ROAMING_EVENT_DISCOVERY)
