...
********************************************************************************
*/
#define REPLICATED_BEACON_TIME_THRESHOLD        (3000U)
#define REPLICATED_BEACON_FRESH_PERIOD          (10000U)
#define REPLICATED_BEACON_STRENGTH_THRESHOLD    (32U)
...
*/
#define BSS_FULL_SCORE                          100U
#define CHNL_BSS_NUM_THRESOLD                   100
#define BSS_STA_CNT_THRESOLD                    30U
#define SCORE_PER_AP                            1
#define ROAMING_NO_SWING_SCORE_STEP             10U
#define HARD_TO_CONNECT_RSSI_THRESOLD           -80

#define WEIGHT_IDX_CHNL_UTIL                    2
#define WEIGHT_IDX_SNR                          3
#define WEIGHT_IDX_RSSI                         3U
#define WEIGHT_IDX_SCN_MISS_CNT                 2U
#define WEIGHT_IDX_PROBE_RSP                    1U
#define WEIGHT_IDX_CLIENT_CNT                   3U
#define WEIGHT_IDX_AP_NUM                       2
#define WEIGHT_IDX_5G_BAND                      2U
#define WEIGHT_IDX_BAND_WIDTH                   1U
#define WEIGHT_IDX_STBC                         1U
#define WEIGHT_IDX_DEAUTH_LAST                  1U
#define WEIGHT_IDX_BLACK_LIST                   2U

static INT_32 rssiRangeHi = RSSI_HI_5GHZ;
static INT_32 rssiRangeMed = RSSI_MED_5GHZ;
static INT_32 rssiRangeLo = RSSI_LO_5GHZ;
static INT_8 pref5GhzHi = PREF_HI_5GHZ;
static INT_8 pref5GhzMed = PREF_MED_5GHZ;
static INT_8 pref5GhzLo = PREF_LO_5GHZ;
...
VOID scnInit(IN P_ADAPTER_T prAdapter)
{
	...
	ASSERT_BOOLEAN(((ULONG) pucBSSBuff -
		(ULONG)&prScanInfo->aucScanBuffer[0]) == SCN_MAX_BUFFER_SIZE);
	...
	ASSERT_BOOLEAN(((ULONG)pucRoamBSSBuff
		- (ULONG)&prScanInfo->aucScanRoamBuffer[0])
	...
	if (prScanInfo->prPscnParam == NULL) {
	...
	if (prScanInfo->prGscnFullResult == NULL) {
#if CFG_SUPPORT_SCN_PSCN
...
VOID scanAddToRoamBssDesc(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc)
{
	...
	if (prRoamBssDesc == NULL) {
		...
		do {
			...
			u4RemoveTime = u4RemoveTime / 2U;
		} while (u4RemoveTime > 0U);
...
#define SSID_ONLY_EXIST_ONE_AP      1U
				  ...
				  IN UINT_8 aucSrcAddr[], IN BOOLEAN fgCheckSsid, IN P_PARAM_SSID_T prSsid)
{
	...
	if (eBSSType == BSS_TYPE_IBSS) {
		...
		prBssDesc = scanSearchBssDescByTAAndSsid
			(prAdapter, aucSrcAddr, fgCheckSsid, prSsid);
		 ...
		 * CASE II	We have TA1(BSSID1), and get TA1
		 *	(BSSID1) again
		 *	-> Update TA1 entry's contain.
		 * CASE III	We have a SCAN result TA1(BSSID1),
		 *	and TA2(BSSID2).Sooner or later, TA2 merge
		 *	into TA1, we get TA2(BSSID1)
		 *	-> Remove TA2 first and then replace TA1 entry's
		 *	TA with TA2,Still have only one entry of BSSID.
		 * CASE IV	We have a SCAN result TA1(BSSID1),
		 *	and another TA2 also merge into BSSID1.
		 *	-> Replace TA1 entry's TA with TA2, Still have only
		 *	one entry.
		 * CASE V	New IBSS
...
VOID scanRemoveBssDescsByPolicy(IN P_ADAPTER_T prAdapter, IN UINT_32 u4RemovePolicy)
{
	...
	if ((u4RemovePolicy & SCN_RM_POLICY_TIMEOUT) != 0U) {
	...
	} else if ((u4RemovePolicy & SCN_RM_POLICY_OLDEST_HIDDEN) != 0U) {
		P_BSS_DESC_T prBssDescOldest = NULL;
		...
		LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
			...
			if (prBssDesc->fgIsHiddenSSID == FALSE)
				continue;

			if (prBssDescOldest == NULL) {	/* 1st element */
		...
		if (prBssDescOldest != NULL) {
			...
			if (prBssDescOldest->prBlack == NULL)
				prEntry = aisQueryBlackList
				(prAdapter, prBssDescOldest);
			if (prBssDescOldest->prBlack != NULL)
	...
	} else if ((u4RemovePolicy & SCN_RM_POLICY_SMART_WEAKEST) != 0U) {
		...
		LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
			...
			if ((prBssDesc->fgIsHiddenSSID == FALSE) &&
				...
				if (prBssDescWeakestSameSSID == NULL)
			...
			if (prBssDescWeakest == NULL) {	/* 1st element */
		...
		if ((u4SameSSIDCount >= SCN_BSS_DESC_SAME_SSID_THRESHOLD) &&
			(prBssDescWeakestSameSSID != NULL))
			prBssDescWeakest = prBssDescWeakestSameSSID;

		if (prBssDescWeakest != NULL) {

			/* DBGLOG(SCN, TRACE, ("Remove WEAKEST BSS DESC(%#x): MAC: "MACSTR", Update Time = %08lx\n", */
			/* prBssDescOldest, MAC2STR(prBssDescOldest->aucBSSID), prBssDescOldest->rUpdateTime)); */
			if (prBssDescWeakest->prBlack == NULL)
				prEntry = aisQueryBlackList
				(prAdapter, prBssDescWeakest);
			if (prBssDescWeakest->prBlack != NULL)
		...
		if ((u4RemovePolicy & SCN_RM_POLICY_ENTIRE) != 0U) {
			P_BSS_DESC_T prBSSDescNext;
			UINT_64 u8Current = kalGetBootTime();

			LINK_FOR_EACH_ENTRY_SAFE(prBssDesc,
				prBSSDescNext, prBSSDescList,
			...
			/* Don't remove the one currently we are connected. */
				...
				if (prBssDesc->prBlack != NULL)
					prBssDesc->prBlack
					->u8DisapperTime = u8Current;

			/* Remove this BSS Desc from the BSS Desc list */
				LINK_REMOVE_KNOWN_ENTRY
				(prBSSDescList, prBssDesc);
			/* Remove this BSS Desc from the Ess Desc List */
				if (LINK_ENTRY_IS_VALID
					(&prBssDesc->rLinkEntryEss))
					LINK_REMOVE_KNOWN_ENTRY
					(prEssList, &prBssDesc->rLinkEntryEss);
...
P_BSS_DESC_T scanAddToBssDesc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	P_WLAN_BEACON_FRAME_T prWlanBeaconFrame = NULL;
	P_IE_SSID_T prIeSsid = NULL;
	P_IE_SUPPORTED_RATE_T prIeSupportedRate = NULL;
	P_IE_EXT_SUPPORTED_RATE_T prIeExtSupportedRate = NULL;
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_SSID:
			...
			break;
		default:
			DBGLOG(SCN, TRACE, "Default in scanAddToBssDesc\n");
			break;
		}
	...
	if (prBssDesc == NULL) {
		...
		do {
			...
			(UINT_32)(SCN_RM_POLICY_EXCLUDE_CONNECTED |
		...
		if (prBssDesc->eBSSType != eBSSType) {
			...
			if (HAL_RX_STATUS_GET_CHNL_NUM(prSwRfb->prRxStatus) !=
	...
	if (fgIsNewBssDesc == FALSE && prBssDesc->fgIsConnecting == TRUE)
		return prBssDesc;
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_SUP_RATES:
			...
			if ((prIeSupportedRate == NULL) &&
				(IE_LEN(pucIE) <= (UINT_8)RATE_NUM_SW))
		...
		case ELEM_ID_RSN:
			if (rsnParseRsnIE(prAdapter,
				RSN_IE(pucIE), &prBssDesc->rRSNInfo)
				== TRUE) {
		...
		case ELEM_ID_HT_CAP:
		{
			...
			if (prBssDesc->fgMultiAnttenaAndSTBC == TRUE)
				break;
			for (; j < 4U; j++) {
				if (prHtCap->rSupMcsSet.aucRxMcsBitmask[j] > 0U)
					ucSpatial++;
			}
			prBssDesc->fgMultiAnttenaAndSTBC = (BOOLEAN)
				((ucSpatial > 1U) &&
				((prHtCap->u2HtCapInfo &
				HT_CAP_INFO_TX_STBC) != 0U));
			break;
		}
		case ELEM_ID_HT_OP:
		{
			...
			if (IE_LEN(pucIE) != ((UINT_8)sizeof(IE_HT_OP_T)
				- 2U))
				break;

			if ((((P_IE_HT_OP_T)pucIE)->ucInfo1 &
				HT_OP_INFO1_SCO) !=
				(UINT_16)CHNL_EXT_RES) {
				ucSco = ((P_IE_HT_OP_T)pucIE)->ucInfo1 &
		...
		case ELEM_ID_VHT_CAP:
		{
			...
			if (prBssDesc->fgMultiAnttenaAndSTBC == TRUE)
				break;
			for (; k < 8U; k++) {
				if ((u2TxMcsSet & BITS(2U*k, 2U*k+1U)) != 3U)
					ucSpatial++;
			}
			prBssDesc->fgMultiAnttenaAndSTBC = (BOOLEAN)
				((ucSpatial > 1U) && ((prVhtCap->u4VhtCapInfo &
				VHT_CAP_INFO_TX_STBC) != 0U));
			break;
		}
		case ELEM_ID_VHT_OP:
		{
			...
			if (IE_LEN(pucIE) != ((UINT_8)sizeof(IE_VHT_OP_T) - 2U))
				break;
			ucChannelWidth =
				((P_IE_VHT_OP_T) pucIE)->ucVhtOperation[0];
			if (ucChannelWidth == 0U)
				prBssDesc->eChannelWidth = CW_20_40MHZ;
			else if (ucChannelWidth == 1U)
				prBssDesc->eChannelWidth = CW_80MHZ;
			else if (ucChannelWidth == 2U)
				prBssDesc->eChannelWidth = CW_160MHZ;
			else {
				if (ucChannelWidth == 3U)
					prBssDesc->eChannelWidth = CW_80P80MHZ;
			}
			prBssDesc->ucCenterFreqS1 =
				((P_IE_VHT_OP_T) pucIE)->ucVhtOperation[1];
			prBssDesc->ucCenterFreqS2 =
				((P_IE_VHT_OP_T) pucIE)->ucVhtOperation[2];
		...
		case ELEM_ID_VENDOR:	/* ELEM_ID_P2P, ELEM_ID_WMM */
		{
			...
			if (prAdapter->fgIsP2PRegistered == TRUE) {
				if ((p2pFuncParseCheckForP2PInfoElem
					(prAdapter, pucIE, &ucOuiType) == TRUE)
	...
	/* 4 <4> Update information from HIF RX Header */
	{
		...
		if (prBssDesc->eBand == BAND_2G4) {
			...
			if (ucIeDsChannelNum >= 1U && ucIeDsChannelNum <= 14U) {
			...
			} else if (ucIeHtChannelNum >= 1U &&
				ucIeHtChannelNum <= 14U) {
				/* Receive Beacon/ProbeResp frame from adjacent channel. */
		...
		/* 5G Band */
			...
			if (ucIeHtChannelNum >= 1U && ucIeHtChannelNum < 200U) {
				/* Receive Beacon/ProbeResp frame from adjacent channel. */
				if ((ucIeHtChannelNum == ucHwChannelNum) || (ucRxRCPI > prBssDesc->ucRCPI))
					prBssDesc->ucRCPI = ucRxRCPI;
				/* trust channel information brought by IE */
	...
	if (rlmDomainIsValidRfSetting(prAdapter, prBssDesc->eBand,
		...
		prBssDesc->ucCenterFreqS2) == FALSE) {
		/*Dump IE Inforamtion */
		PUINT_8 pucDumpIE;
	...
	if (prBssDesc->eBand == BAND_2G4) {
		...
		if (prBssDesc->fgIsHTPresent == TRUE)
			prBssDesc->ucPhyTypeSet |= (UINT_8)PHY_TYPE_BIT_HT;

		/* if not 11n only */
		if ((prBssDesc->u2BSSBasicRateSet &
			RATE_SET_BIT_HT_PHY) == 0U) {
			/* check if support 11g */
			if (((prBssDesc->u2OperationalRateSet &
				RATE_SET_OFDM) != 0U) ||
				prBssDesc->fgIsERPPresent == TRUE)
				prBssDesc->ucPhyTypeSet |=
				(UINT_8)PHY_TYPE_BIT_ERP;
			...
			if ((prBssDesc->u2BSSBasicRateSet &
				...
				if ((prBssDesc->u2OperationalRateSet &
					RATE_SET_HR_DSSS) != 0U)
					prBssDesc->ucPhyTypeSet |=
					(UINT_8)PHY_TYPE_BIT_HR_DSSS;
			}
		}
	} else {		/* (prBssDesc->eBande == BAND_5G) */
		/* check if support 11n */
		if (prBssDesc->fgIsVHTPresent == TRUE)
			prBssDesc->ucPhyTypeSet |= (UINT_8)PHY_TYPE_BIT_VHT;
		...
		if ((prBssDesc->u2BSSBasicRateSet &
			...
			prBssDesc->ucPhyTypeSet |= (UINT_8)PHY_TYPE_BIT_OFDM;
	...
	if ((prWlanBeaconFrame->u2FrameCtrl & 0x50U) == 0x50U)
...
{
#define NUMBER_SSID_PER_LINE 16U
	...
	if ((u4ResultNum % NUMBER_SSID_PER_LINE) != 0U)
		u4ResultNum++;
...
WLAN_STATUS scanAddScanResult(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prBssDesc->eBand == BAND_2G4) {
		if (((prBssDesc->u2OperationalRateSet & RATE_SET_OFDM) != 0U)
		    || prBssDesc->fgIsERPPresent == TRUE) {
		...
		ASSERT_BOOLEAN(prBssDesc->eBand == BAND_5G);
	...
	rConfiguration.u4Length = (UINT_32)sizeof(PARAM_802_11_CONFIG_T);
	...
	if (prAdapter->rWifiVar.rScanInfo.fgNloScanning == TRUE &&
		...
		for (; j < SCN_SSID_MATCH_MAX_NUM; j++) {
			if (pprPendBssDesc[j] != NULL)
				continue;
			DBGLOG(SCN, INFO,
				"indicate bss[%pM] before wiphy resume, need to indicate again after wiphy resume\n",
				prBssDesc->aucBSSID);
			pprPendBssDesc[j] = prBssDesc;
			break;
		}
	}
...
static BOOLEAN scanCheckBssIsLegal(IN P_ADAPTER_T prAdapter,
	P_BSS_DESC_T prBssDesc)
{
...
WLAN_STATUS scanProcessBeaconAndProbeResp(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	WLAN_STATUS rTmpStatus = WLAN_STATUS_SUCCESS;
	P_BSS_INFO_T prAisBssInfo;
	...
	if (prBssDesc != NULL) {
		...
		if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED &&
			...
			if (rsnCheckSecurityModeChanged
				(prAdapter, prAisBssInfo, prBssDesc) == TRUE
#if CFG_SUPPORT_WAPI
				|| (prAdapter->rWifiVar.rConnSettings.fgWapiMode == TRUE &&
					wapiPerformPolicySelection
					(prAdapter, prBssDesc) == FALSE)
#endif
				) {
				...
				if (prConnSettings
					->fgSecModeChangeStartTimer == FALSE) {
				...
				if (prConnSettings
					->fgSecModeChangeStartTimer == TRUE) {
		...
		if (((prBssDesc->eBSSType == BSS_TYPE_INFRASTRUCTURE && prConnSettings->eOPMode != NET_TYPE_IBSS)
			...
			if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
				 ...
				 */
				if ((prAisBssInfo->ucDTIMPeriod == 0U) &&
				    EQUAL_MAC_ADDR(prBssDesc->aucBSSID, prAisBssInfo->aucBSSID) &&
		...
		if (prBssDesc->eBSSType == BSS_TYPE_INFRASTRUCTURE || prBssDesc->eBSSType == BSS_TYPE_IBSS) {
			...
			if (prConnSettings->fgIsScanReqIssued == TRUE ||
				prAdapter->rWifiVar.rScanInfo
				.fgNloScanning == TRUE
#if CFG_SUPPORT_SCN_PSCN
			|| prAdapter->rWifiVar.rScanInfo.fgPscnOngoing == TRUE
#endif
			) {
...
static INT_32 scanResultsAdjust5GPref(P_BSS_DESC_T prBssDesc)
{
	...
	if (prBssDesc->fgIsConnected == TRUE)
		rssi += ROAMING_NO_SWING_RCPI_STEP / 2;

	if (prBssDesc->eBand == BAND_5G || prBssDesc->fgIsConnected == TRUE)
...
P_BSS_DESC_T scanSearchBssDescByPolicy(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	P_BSS_DESC_T prBssDesc = NULL;
	P_BSS_DESC_T prPrimaryBssDesc = NULL;
	P_BSS_DESC_T prCandidateBssDesc = NULL;

	P_STA_RECORD_T prStaRec = NULL;
	P_STA_RECORD_T prPrimaryStaRec;
	P_STA_RECORD_T prCandidateStaRec = NULL;

	OS_SYSTIME rCurrentTime;

	/* The first one reach the check point will be our candidate */
	BOOLEAN fgIsFindFirst = FALSE;

	BOOLEAN fgIsFindBestRSSI = FALSE;
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
...
#if CFG_ENABLE_WIFI_DIRECT
#if CFG_SUPPORT_WFD
		if (prAdapter->rWifiVar.rWfdConfigureSettings.ucWfdEnable != 0U)
		...
		if (prBssInfo->eNetworkType == NETWORK_TYPE_AIS) {
			...
			if ((prConnSettings->fgIsConnByBssidIssued == TRUE) &&
				(prBssDesc->eBSSType ==
		...
		default:
			DBGLOG(SCN, TRACE,
				"Default in scanSearchBssDescByPolicy\n");
			break;
		}
		...
		if (prPrimaryBssDesc->eBSSType == BSS_TYPE_INFRASTRUCTURE) {
#if CFG_SUPPORT_WAPI
			if (prAdapter->rWifiVar
				.rConnSettings.fgWapiMode == TRUE) {
				if (wapiPerformPolicySelection
					(prAdapter, prPrimaryBssDesc) == TRUE) {
					fgIsFindFirst = TRUE;
				} else {
			...
			if (rsnPerformPolicySelection
				...
				if (prPrimaryBssDesc->ucEncLevel > 0U) {
					fgIsFindFirst = FALSE;
				}
			} else {
				/* Can't pass the Encryption Status Check, get next one */
		...
		if (prCandidateBssDesc == NULL) {
			...
			if (fgIsFindFirst == TRUE)
				break;
		} else {
			/* 4 <6D> Condition - Visible SSID win Hidden SSID. */
			if (prCandidateBssDesc->fgIsHiddenSSID == TRUE) {
				if (prPrimaryBssDesc->fgIsHiddenSSID == FALSE) {
					prCandidateBssDesc = prPrimaryBssDesc;	/* The non Hidden SSID win. */
					prCandidateStaRec = prPrimaryStaRec;
					continue;
				}
			} else {
				if (prPrimaryBssDesc->fgIsHiddenSSID == TRUE)
					continue;
			}

			/* 4 <6E> Condition - Choose the one with better RCPI(RSSI). */
			if (fgIsFindBestRSSI == TRUE) {
				/*
				...
				ASSERT_BOOLEAN(
				!(prCandidateBssDesc->fgIsConnected == TRUE
				&& prPrimaryBssDesc->fgIsConnected == TRUE));
				 ...
				 */
				if (prCandidateBssDesc->fgIsConnected == TRUE) {
...
VOID scanReportBss2Cfg80211(IN P_ADAPTER_T prAdapter, IN ENUM_BSS_TYPE_T eBSSType, IN P_BSS_DESC_T prSpecificBssDesc)
{
	...
	if (prSpecificBssDesc != NULL) {
		...
		LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
			...
			UINT_8 ucIdx = 0;
			struct ieee80211_mgmt *prMgmtFrame = (struct ieee80211_mgmt *)prBssDesc->aucRawBuf;

			if (prBssDesc->ucChannelNum <= 14U)
				ucIdx = prBssDesc->ucChannelNum - 1U;
			else if (prBssDesc->ucChannelNum >= 36U &&
				prBssDesc->ucChannelNum <= 64U)
				ucIdx = 14U +
				(prBssDesc->ucChannelNum - 36U) / 4U;
			else if (prBssDesc->ucChannelNum >= 100U &&
				prBssDesc->ucChannelNum <= 144U)
				ucIdx = 14U + 8U +
				(prBssDesc->ucChannelNum - 100U) / 4U;
			else {
				if (prBssDesc->ucChannelNum >= 149U)
					ucIdx = 14U + 8U + 12U +
					(prBssDesc->ucChannelNum - 149U) / 4U;
			}
			...
			if ((prBssDesc->eBSSType == eBSSType)
				...
				if (eBSSType == BSS_TYPE_INFRASTRUCTURE) {
					if (prBssDesc->u2RawLength != 0U) {
						kalIndicateBssInfo(prAdapter->prGlueInfo,
								   (PUINT_8) prBssDesc->aucRawBuf,
								   prBssDesc->u2RawLength,
								   prBssDesc->ucChannelNum,
								   RCPI_TO_dBm(prBssDesc->ucRCPI));
					DBGLOG(SCN, TRACE, "ktimestamp: %lu,BSSID: "MACSTR"  %d\n", prMgmtFrame->u.beacon.timestamp,
						MAC2STR(prBssDesc->aucBSSID), scan_bss_num++);
...
VOID scanReportScanResultToAgps(P_ADAPTER_T prAdapter)
{
	...
	fgResult = kalIndicateAgpsNotify(prAdapter,
		(UINT_8)AGPS_EVENT_WLAN_AP_LIST,
		(PUINT_8)prAgpsApList,
		(UINT_16)sizeof(AGPS_AP_LIST_T));
...
VOID scanGetCurrentEssChnlList(P_ADAPTER_T prAdapter)
{
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		if (prBssDesc->ucChannelNum > 214U)
			continue;
		/* Statistic AP num for each channel */
		if (aucChnlApNum[prBssDesc->ucChannelNum] < 255U)
		...
		if ((aucChnlBitMap[ucByteNum] & (UINT_8)BIT(ucBitNum)) != 0U)
			continue;
		aucChnlBitMap[ucByteNum] |= (UINT_8)BIT(ucBitNum);
...
#define CALCULATE_SCORE_BY_PROBE_RSP(prBssDesc) \
	((UINT_16)WEIGHT_IDX_PROBE_RSP * \
	(prBssDesc->fgSeenProbeResp == TRUE ? BSS_FULL_SCORE : 0U))

#define CALCULATE_SCORE_BY_MISS_CNT(prAdapter, prBssDesc) \
	((UINT_16)WEIGHT_IDX_SCN_MISS_CNT * \
	((UINT_16)(prAdapter->rWifiVar.rScanInfo.u4ScanUpdateIdx - \
	prBssDesc->u4UpdateIdx) > 3U ? 0U : (BSS_FULL_SCORE - \
	(UINT_16)(prAdapter->rWifiVar.rScanInfo.u4ScanUpdateIdx - \
	prBssDesc->u4UpdateIdx) * 25U)))

#define CALCULATE_SCORE_BY_BAND(prAdapter, prBssDesc, cRssi) \
	((UINT_16)WEIGHT_IDX_5G_BAND * \
	((prBssDesc->eBand == BAND_5G && prAdapter->fgEnable5GBand == TRUE && \
	cRssi > -70) ? BSS_FULL_SCORE : 0U))

#define CALCULATE_SCORE_BY_STBC(prAdapter, prBssDesc) \
	((UINT_16)WEIGHT_IDX_STBC * \
	((prBssDesc->fgMultiAnttenaAndSTBC == TRUE && \
	prAdapter->rWifiVar.ucRxStbc != 0U) ? BSS_FULL_SCORE:0U))

#define CALCULATE_SCORE_BY_DEAUTH(prBssDesc) \
	((UINT_16)WEIGHT_IDX_DEAUTH_LAST * \
	(prBssDesc->fgDeauthLastTime == TRUE ? 0U : BSS_FULL_SCORE))
...
static UINT_16 scanCalculateScoreByBandwidth(P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc)
{
	...
	if (prBssDesc->fgIsVHTPresent == TRUE &&
		...
		if (ucSta5GBW > ucStaBW)
			ucSta5GBW = ucStaBW;
		switch (ucSta5GBW) {
		case (UINT_8)MAX_BW_20MHZ:
		case (UINT_8)MAX_BW_40MHZ:
			eChannelWidth = CW_20_40MHZ;
			break;
		case (UINT_8)MAX_BW_80MHZ:
			eChannelWidth = CW_80MHZ;
			break;
		default:
			DBGLOG(SCN, TRACE,
				"Default in scanCalculateScoreByBandwidth\n");
			break;
		}
		...
		default:
			DBGLOG(SCN, TRACE,
				"Default in scanCalculateScoreByBandwidth-2\n");
			break;
		}
	} else if (prBssDesc->fgIsHTPresent == TRUE) {
		if (prBssDesc->eBand == BAND_2G4) {
			if (ucSta2GBW > ucStaBW)
				ucSta2GBW = ucStaBW;
			u2Score = (prBssDesc->eSco == CHNL_EXT_SCN ||
				ucSta2GBW == (UINT_8)MAX_BW_20MHZ) ? 40U:60U;
		} else {
			if (prBssDesc->eBand == BAND_5G) {
				if (ucSta5GBW > ucStaBW)
					ucSta5GBW = ucStaBW;
				u2Score = (prBssDesc->eSco == CHNL_EXT_SCN ||
				ucSta5GBW == (UINT_8)MAX_BW_20MHZ) ? 40U:60U;
			}
		}
	} else if ((prBssDesc->u2BSSBasicRateSet & RATE_SET_OFDM) != 0U)
		u2Score = 20;
	else
		u2Score = 10;
...
static UINT_16 scanCalculateScoreByClientCnt(P_BSS_DESC_T prBssDesc)
{
	...
	if (prBssDesc->fgExsitBssLoadIE == FALSE ||
		prBssDesc->u2StaCnt > BSS_STA_CNT_THRESOLD) {
		DBGLOG(SCN, TRACE, "exist bss load %d, sta cnt %d\n",
			prBssDesc->fgExsitBssLoadIE, prBssDesc->u2StaCnt);
		return 0;
	}
	u2Score = BSS_FULL_SCORE - prBssDesc->u2StaCnt * 3U;
...
{
	UINT_16 u2Score = 0;
	INT_32 cRssi = RCPI_TO_dBm(prBssDesc->ucRCPI);
	...
	if (cRssi >= -20)
		...
		u2Score = (UINT_16)(8 * ((cRssi + 69)/5) + 28);
...
static BOOLEAN scanSanityCheckBssDesc(P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc)
{
	...
	if (prAdapter->prAisBssInfo->fgDisConnReassoc == FALSE)
		...
		if (CHECK_FOR_TIMEOUT(kalGetTimeTick(), prBssDesc->rUpdateTime,
			...
			return FALSE;
#if CFG_SUPPORT_WAPI
	if (prAdapter->rWifiVar.rConnSettings.fgWapiMode == TRUE) {
		if (wapiPerformPolicySelection(prAdapter, prBssDesc) == FALSE)
			return FALSE;
	} else {
		if (rsnPerformPolicySelection(prAdapter, prBssDesc) == FALSE)
			return FALSE;
	}
#else
	if (rsnPerformPolicySelection(prAdapter, prBssDesc) == FALSE)
...
P_BSS_DESC_T scanSearchBssDescByScoreForAis(P_ADAPTER_T prAdapter)
{
	...
	INT_32 cRssi = -128;
	INT_32 cMaxRssi = HARD_TO_CONNECT_RSSI_THRESOLD;
	...
	if (prConnSettings->eConnectionPolicy != CONNECT_BY_BSSID) {
		LINK_FOR_EACH_ENTRY(prBssDesc, prEssLink, rLinkEntryEss, BSS_DESC_T) {
			if (fgSearchBlackList == FALSE) {
				...
				if (prBssDesc->prBlack) {
					if (prBssDesc->prBlack
							...
							== TRUE)
						DBGLOG(SCN, INFO,
							"%s(%pM) is in FWK blacklist, skip it\n",
							prBssDesc->aucSSID,
							prBssDesc->aucBSSID);
					continue;
				}
				...
				if (prBssDesc->prBlack->fgIsInFWKBlacklist
						== TRUE) {
					DBGLOG(SCN, INFO,
						"Although trying blacklist, %s(%pM) in FWK blklist, skip it\n",
						prBssDesc->aucSSID,
						prBssDesc->aucBSSID);
					continue;
				}
			...
			if (scanSanityCheckBssDesc(prAdapter, prBssDesc) == FALSE)
				continue;
...
try_again:
	LINK_FOR_EACH_ENTRY(prBssDesc, prEssLink, rLinkEntryEss, BSS_DESC_T) {
		if (prConnSettings->eConnectionPolicy == CONNECT_BY_BSSID) {
			...
			break;
		} else if (fgSearchBlackList == FALSE) {
			prBssDesc->prBlack = aisQueryBlackList(prAdapter, prBssDesc);
			if (prBssDesc->prBlack != NULL)
				continue;
		} else if (prBssDesc->prBlack == NULL)
			continue;
		else
			u2BlackListScore = WEIGHT_IDX_BLACK_LIST *
				aisCalculateBlackListScore(prAdapter, prBssDesc);
		...
		if (cRssi < HARD_TO_CONNECT_RSSI_THRESOLD) {
			if (prCandBssDescForLowRssi == NULL) {
			...
			} else if (prBssDesc->fgIsConnected == TRUE) {
				if (u2CandBssScoreForLowRssi > (u2ScoreTotal + ROAMING_NO_SWING_SCORE_STEP)) {
					prCandBssDescForLowRssi = prBssDesc;
					u2CandBssScoreForLowRssi = u2ScoreTotal;
				}
			} else {
				if (u2CandBssScoreForLowRssi < u2ScoreTotal) {
					prCandBssDescForLowRssi = prBssDesc;
					u2CandBssScoreForLowRssi = u2ScoreTotal;
				}
			}
		} else if (prCandBssDesc == NULL) {
			prCandBssDesc = prBssDesc;
			u2CandBssScore = u2ScoreTotal;
		} else if (prCandBssDesc->fgIsConnected == TRUE) {
			...
			if (u2CandBssScore < u2ScoreTotal) {
				prCandBssDesc = prBssDesc;
				u2CandBssScore = u2ScoreTotal;
			}
		}
	} /* end of LINK_FOR_EACH */
	if (prCandBssDesc != NULL) {
		if (prCandBssDesc->fgIsConnected == TRUE &&
			fgSearchBlackList == FALSE &&
			prEssLink->u4NumElem > 0U) {
	...
	if (fgSearchBlackList == FALSE && prEssLink->u4NumElem > 0U) {
		fgSearchBlackList = TRUE;
