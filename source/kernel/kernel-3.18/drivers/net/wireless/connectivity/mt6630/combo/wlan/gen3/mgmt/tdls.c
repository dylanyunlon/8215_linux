...
#define	TDLS_KEY_TIMEOUT_INTERVAL 43200U

#define TDLS_REASON_CODE_UNREACHABLE  25U
#define TDLS_REASON_CODE_UNSPECIFIED  26U

#define WLAN_REASON_TDLS_TEARDOWN_UNREACHABLE 25U
#define WLAN_REASON_TDLS_TEARDOWN_UNSPECIFIED 26U
...
*/
/*----------------------------------------------------------------------------*/
UINT_32 TdlsexLinkMgt(P_ADAPTER_T prAdapter, PVOID pvSetBuffer,
		UINT_32 u4SetBufferLen, PUINT_32 pu4SetInfoLen)
{
	...
	case (UINT_8)TDLS_FRM_ACTION_DISCOVERY_REQ:
		if (prStaRec == NULL)
			return 0;
	...
	case (UINT_8)TDLS_FRM_ACTION_SETUP_RSP:
	...
	case (UINT_8)TDLS_FRM_ACTION_DISCOVERY_RSP:
	...
	case (UINT_8)TDLS_FRM_ACTION_CONFIRM:
	...
	case (UINT_8)TDLS_FRM_ACTION_TEARDOWN:
...
UINT_32 TdlsFrameGeneralIeAppend(ADAPTER_T *prAdapter, STA_RECORD_T *prStaRec, UINT_8 *pPkt)
{
	...
	UINT_8 aucAllSupportedRates[RATE_NUM_SW] = { 0U };
	...
	rateGetDataRatesFromRateSet(u2SupportedRateSet, 0U,
	...
	SUPPORTED_CHANNELS_IE(pPkt)->ucLength = 2U;
	SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[0] = 1U;
	SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[1] = 13U;

	if (prAdapter->fgEnable5GBand == TRUE) {
		SUPPORTED_CHANNELS_IE(pPkt)->ucLength = 10U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[2] = 36U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[3] = 4U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[4] = 52U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[5] = 4U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[6] = 149U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[7] = 4U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[8] = 165U;
		SUPPORTED_CHANNELS_IE(pPkt)->ucChannelNum[9] = 1U;
	}
			   ...
			   UINT_8 ucDialogToken, UINT_16 u2StatusCode, UINT_8 *pAppendIe, UINT_32 AppendIeLen)
{
	...
	prMsduInfo = kalPacketAlloc(prGlueInfo, 1600U, &pPkt);
	if (prMsduInfo == NULL)
	...
	*(UINT_16 *) pPkt = htons(TDLS_FRM_PROT_TYPE);
	pPkt += 2U;
	u4PktLen += TDLS_FME_MAC_ADDR_LEN * 2U + 2U;
	...
	*pPkt = ucActionCode;
	...
	kalMemCopy(pPkt, &ReasonCode, 2U);
	pPkt += 2U;
	u4PktLen = u4PktLen + 2U;

	if (pAppendIe != NULL) {
		if ((ucActionCode != (UINT_8)TDLS_FRM_ACTION_TEARDOWN) ||
		    (prStaRec != NULL)) {
	...
	prMsduInfo->len = (UINT_32)u4PktLen;

	/* 5. send the data frame */
	rStatus = wlanHardStartXmit(prMsduInfo, prMsduInfo->dev);
			    ...
			    UINT_8 ucDialogToken, UINT_16 u2StatusCode, UINT_8 *pAppendIe, UINT_32 AppendIeLen)
{
	...
	*pPkt = ucDialogToken;
	...
	EXT_CAP_IE(pPkt)->ucLength = 5U;
/* 0320 !! */
	EXT_CAP_IE(pPkt)->aucCapabilities[0] = 0x00U;	/* bit0 ~ bit7 */
	EXT_CAP_IE(pPkt)->aucCapabilities[1] = 0x00U;	/* bit8 ~ bit15 */
	EXT_CAP_IE(pPkt)->aucCapabilities[2] = 0x00U;	/* bit16 ~ bit23 */
	EXT_CAP_IE(pPkt)->aucCapabilities[3] = 0x00U;	/* bit24 ~ bit31 */
	EXT_CAP_IE(pPkt)->aucCapabilities[4] = 0xFFU;	/* bit32 ~ bit39 */

	EXT_CAP_IE(pPkt)->aucCapabilities[3] |= (UINT_8)BIT((28U - 24U));
	EXT_CAP_IE(pPkt)->aucCapabilities[3] |= (UINT_8)BIT((30U - 24U));
	EXT_CAP_IE(pPkt)->aucCapabilities[4] |= (UINT_8)BIT((37U - 32U));
...
/* check */
	...
	TIMEOUT_INTERVAL_IE(pPkt)->ucLength = 5U;

	TIMEOUT_INTERVAL_IE(pPkt)->ucType = 2U;
	...
	if (ucActionCode != (UINT_8)TDLS_FRM_ACTION_TEARDOWN) {
		...
		BSS_20_40_COEXIST_IE(pPkt)->ucLength = 1U;
		BSS_20_40_COEXIST_IE(pPkt)->ucData = 0x01U;
		LR_TDLS_FME_FIELD_FILL(3U);
	}
			    ...
			    UINT_8 ucDialogToken, UINT_16 u2StatusCode, UINT_8 *pAppendIe, UINT_32 AppendIeLen)
{
	...
	*pPkt = ucDialogToken;
	...
	if ((prAdapter->rWifiVar.ucAvailablePhyTypeSet &
		PHY_TYPE_SET_802_11N) != 0U) {
		/* Add HT IE *//* try to reuse p2p path */
		u4IeLen = rlmFillHtCapIEByAdapter(prAdapter,
			prBssInfo, pPkt);
		pPkt += u4IeLen;
		u4PktLen += u4IeLen;
	}
				...
				UINT_8 ucDialogToken, UINT_16 u2StatusCode, UINT_8 *pAppendIe, UINT_32 AppendIeLen)
{
	...
	/* make up frame content */
	if (ucActionCode != (UINT_8)TDLS_FRM_ACTION_DISCOVERY_RSP) {
		...
		LR_TDLS_FME_FIELD_FILL(TDLS_FME_MAC_ADDR_LEN);
		*(UINT_16 *) pPkt = htons(TDLS_FRM_PROT_TYPE);
		LR_TDLS_FME_FIELD_FILL(2U);

		/* 2. payload type */
		*pPkt = TDLS_FRM_PAYLOAD_TYPE;
		LR_TDLS_FME_FIELD_FILL(1U);
		...
		*pPkt = TDLS_FRM_CATEGORY;
		...
		prHdr->u2SeqCtrl = 0U;
		LR_TDLS_FME_FIELD_FILL((UINT_32)sizeof(WLAN_MAC_HEADER_T));
	...
	*pPkt = ucActionCode;
	...
	/* 3. Frame Formation - Status Code */
	if (ucActionCode == (UINT_8)TDLS_FRM_ACTION_SETUP_RSP ||
		ucActionCode == (UINT_8)TDLS_FRM_ACTION_CONFIRM ||
		ucActionCode == (UINT_8)TDLS_FRM_ACTION_TEARDOWN) {
	...
	if (ucActionCode != (UINT_8)TDLS_FRM_ACTION_TEARDOWN) {
		 ...
		 * Timeout Interval
		 */
		if (ucActionCode != (UINT_8)TDLS_FRM_ACTION_CONFIRM) {
			 ...
			 */
			u4IeLen = TdlsFrameGeneralIeAppend
				(prAdapter, prStaRec, pPkt);
			LR_TDLS_FME_FIELD_FILL(u4IeLen);
			...
			EXT_CAP_IE(pPkt)->aucCapabilities[0] = 0x00U;
			EXT_CAP_IE(pPkt)->aucCapabilities[1] = 0x00U;
			EXT_CAP_IE(pPkt)->aucCapabilities[2] = 0x00U;
			EXT_CAP_IE(pPkt)->aucCapabilities[3] = 0x00U;
			EXT_CAP_IE(pPkt)->aucCapabilities[4] = 0x00U;
	...
	kalMemCopy(TDLS_LINK_IDENTIFIER_IE(pPkt)->aInitiator, pucInitiator, 6U);
	kalMemCopy(TDLS_LINK_IDENTIFIER_IE(pPkt)->aResponder, pucResponder, 6U);
	...
	if (ucActionCode != (UINT_8)TDLS_FRM_ACTION_DISCOVERY_RSP) {
		...
		prMsduInfo->len = (UINT_16)u4PktLen;

		rStatus = wlanHardStartXmit(prMsduInfo, prMsduInfo->dev);
	} else {
		prMsduInfoMgmt->ucPacketType = (UINT_8)TX_PACKET_TYPE_MGMT;
				...
				UINT_8 ucDialogToken, UINT_16 u2StatusCode, UINT_8 *pAppendIe, UINT_32 AppendIeLen)
{
	...
	*pPkt = ucActionCode;
	...
	kalMemCopy(TDLS_LINK_IDENTIFIER_IE(pPkt)->aBSSID,
		prBssInfo->aucBSSID, 6U);
	kalMemCopy(TDLS_LINK_IDENTIFIER_IE(pPkt)->aInitiator,
		pPeerMac, 6U);	/* prAdapter->rMyMacAddr */
	kalMemCopy(TDLS_LINK_IDENTIFIER_IE(pPkt)->aResponder,
		prAdapter->rMyMacAddr, 6U);	/* pPeerMac */
...
*/
/*----------------------------------------------------------------------------*/
VOID TdlsexEventHandle(P_GLUE_INFO_T prGlueInfo,
	UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	UINT_32 u4EventId;
	...
	if (u4EventId == (UINT_32)TDLS_HOST_EVENT_TEAR_DOWN) {
		DBGLOG(TDLS, INFO, "TDLS_HOST_EVENT_TEAR_DOWN\n");
		TdlsEventTearDown(prGlueInfo, prInBuf + 4U, u4InBufLen);
	}
}
...
VOID TdlsEventTearDown(GLUE_INFO_T *prGlueInfo, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	if (fgIsPtiTimeoutSkip == TRUE) {
		...
		if (u4TearDownSubId == (UINT_32)TDLS_HOST_EVENT_TD_PTI_TIMEOUT)
			return;
	}
	...
	if (u4TearDownSubId == (UINT_32)TDLS_HOST_EVENT_TD_AGE_TIMEOUT) {
...
VOID TdlsBssExtCapParse(P_STA_RECORD_T prStaRec, P_UINT_8 pucIE)
{
	...
	/* parse */
	pucIeExtCap = pucIE + 2U;
	pucIeExtCap += 4U;	/* shift to the byte we care about */

	if ((*pucIeExtCap & (UINT_8)BIT(38U - 32U)) != 0U)
		prStaRec->fgTdlsIsProhibited = TRUE;
	if (((*pucIeExtCap) & (UINT_8)BIT(39U - 32U)) != 0U)
...
TdlsSendChSwControlCmd(P_ADAPTER_T prAdapter, PVOID pvSetBuffer, UINT_32 u4SetBufferLen, PUINT_32 pu4SetInfoLen)
{
	...
	u4Status = wlanSendSetQueryCmd(prAdapter,
			    (UINT_8)CMD_ID_SET_TDLS_CH_SW,
			    TRUE,
			    FALSE, FALSE, NULL, NULL,
			    (UINT_32)sizeof(CMD_TDLS_CH_SW_T),
