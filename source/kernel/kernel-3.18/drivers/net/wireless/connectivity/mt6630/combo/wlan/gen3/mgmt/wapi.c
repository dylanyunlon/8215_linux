...
VOID wapiGenerateWAPIIE(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	if (prAdapter->prGlueInfo->u2WapiAssocInfoIESz != 0U) {
...
BOOLEAN wapiParseWapiIE(IN P_WAPI_INFO_ELEM_T prInfoElem, OUT P_WAPI_INFO_T prWapiInfo)
{
	...
	if (pucPairSuite != NULL) {
		...
		for (i = 0U; i < (UINT_32) u2PairSuiteCount; i++) {
			...
			(UCHAR)(0x000000FFU &
			(prWapiInfo->au4PairwiseKeyCipherSuite[i] >> 8U)),
			(UCHAR)(0x000000FFU &
			(prWapiInfo->au4PairwiseKeyCipherSuite[i] >> 16U)),
			(UCHAR)(0x000000FFU &
			(prWapiInfo->au4PairwiseKeyCipherSuite[i] >> 24U)));
		}
	} else {
		/*
		 ...
		 */
		prWapiInfo->u4PairwiseKeyCipherSuiteCount = 1U;
		...
		0x000000FFU),
		(UCHAR)((prWapiInfo->au4PairwiseKeyCipherSuite[0] >> 8U) &
		0x000000FFU),
		(UCHAR)((prWapiInfo->au4PairwiseKeyCipherSuite[0] >> 16U) &
		0x000000FFU),
		(UCHAR)((prWapiInfo->au4PairwiseKeyCipherSuite[0] >> 24U) &
		0x000000FFU));
	}
	...
	if (pucAuthSuite != NULL) {
		...
		for (i = 0U; i < (UINT_32) u2AuthSuiteCount; i++) {
			...
			0x000000FFU),
			(UCHAR)((prWapiInfo->au4AuthKeyMgtSuite[i] >> 8U) &
			0x000000FFU),
			(UCHAR)((prWapiInfo->au4AuthKeyMgtSuite[i] >> 16U) &
			0x000000FFU),
			(UCHAR)((prWapiInfo->au4AuthKeyMgtSuite[i] >> 24U) &
			0x000000FFU));
		}
	} else {
		/*
		 ...
		 */
		prWapiInfo->u4AuthKeyMgtSuiteCount = 1U;
		...
		DBGLOG(SEC, LOUD,
			...
			0x000000FFU),
			(UCHAR)((prWapiInfo->au4AuthKeyMgtSuite[0] >> 8U) &
			0x000000FFU),
			(UCHAR)((prWapiInfo->au4AuthKeyMgtSuite[0] >> 16U) &
			0x000000FFU),
			(UCHAR)((prWapiInfo->au4AuthKeyMgtSuite[0] >> 24U) &
			0x000000FFU));
	}
...
{
	UINT_32 i;
	UINT_32 u4PairwiseCipher = 0U;
	UINT_32 u4GroupCipher = 0U;
	UINT_32 u4AkmSuite = 0U;
	...
	for (i = 0U; i < prBssWapiInfo->u4PairwiseKeyCipherSuiteCount; i++) {
	...
	for (i = 0U; i < prBssWapiInfo->u4AuthKeyMgtSuiteCount; i++) {
