...
wmmCalculatePktUsedTime(P_BSS_INFO_T prBssInfo, P_STA_RECORD_T prStaRec, UINT_16 u2PktLen)
{
	...
	if (prStaRec->ucDesiredPhyTypeSet & PHY_TYPE_SET_802_11N) {
		...
		if (prStaRec->ucDesiredPhyTypeSet & PHY_TYPE_802_11G) {
			u2DataRate = RATE_54M;
			for (i = 13; i >= 4; i--) {
				if ((prStaRec->u2BSSBasicRateSet & BIT(i)) && aucDataRates[i] <= u2DataRate)
					break;
			}
			ucFlags |= FLAG_G_MODE;
		...
		u4TxTime = wmmAcmTxTimeCal(ucSecurityPadding, u2PktLen, u2DataRate, aucDataRates[i], ucFlags);
		DBGLOG(WMM, INFO,
			"DataRate %d, BasicRate %d, Tx %d bytes, SecExtra %d bytes, Flags %02x, Time %u us\n",
			u2DataRate, aucDataRates[i], u2PktLen, ucSecurityPadding, ucFlags, u4TxTime);
	}
	return u4TxTime;
}
