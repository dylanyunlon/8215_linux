...
VOID cmdBufInitialize(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < (UINT_32)CFG_TX_MAX_CMD_PKT_NUM; i++) {
...
VOID cmdBufDumpCmdQueue(P_QUE_T prQueue, CHAR *queName)
{
	...
	while (prCmdInfo != NULL) {
		P_CMD_INFO_T prCmdInfo1, prCmdInfo2, prCmdInfo3;

		prCmdInfo1 = (P_CMD_INFO_T)QUEUE_GET_NEXT_ENTRY
			(&prCmdInfo->rQueEntry);
		if (prCmdInfo1 == NULL) {
			DBGLOG(NIC, INFO, "CID:%d SEQ:%d\n", prCmdInfo->ucCID, prCmdInfo->ucCmdSeqNum);
			break;
		}
		prCmdInfo2 = (P_CMD_INFO_T)QUEUE_GET_NEXT_ENTRY
			(&prCmdInfo1->rQueEntry);
		if (prCmdInfo2 == NULL) {
			DBGLOG(NIC, INFO, "CID:%d, SEQ:%d; CID:%d, SEQ:%d\n", prCmdInfo->ucCID,
				prCmdInfo->ucCmdSeqNum, prCmdInfo1->ucCID, prCmdInfo1->ucCmdSeqNum);
			break;
		}
		prCmdInfo3 = (P_CMD_INFO_T)QUEUE_GET_NEXT_ENTRY
			(&prCmdInfo2->rQueEntry);
		if (prCmdInfo3 == NULL) {
		...
		prCmdInfo = (P_CMD_INFO_T)QUEUE_GET_NEXT_ENTRY
			(&prCmdInfo3->rQueEntry);
	}
}
...

{
	...
	//DBGLOG(MEM, ERROR, "Call Function: %s\n", fileAndLine);
