...
********************************************************************************
*/
static const UINT_8 aucPhyCfg2PhyTypeSet[PHY_CONFIG_NUM] = {
	(UINT_8)PHY_TYPE_SET_802_11ABG,	/* PHY_CONFIG_802_11ABG */
	(UINT_8)PHY_TYPE_SET_802_11BG,	/* PHY_CONFIG_802_11BG */
	(UINT_8)PHY_TYPE_SET_802_11G,	/* PHY_CONFIG_802_11G */
	(UINT_8)PHY_TYPE_SET_802_11A,	/* PHY_CONFIG_802_11A */
	(UINT_8)PHY_TYPE_SET_802_11B,	/* PHY_CONFIG_802_11B */
	(UINT_8)PHY_TYPE_SET_802_11ABGN,	/* PHY_CONFIG_802_11ABGN */
	(UINT_8)PHY_TYPE_SET_802_11BGN,	/* PHY_CONFIG_802_11BGN */
	(UINT_8)PHY_TYPE_SET_802_11AN,	/* PHY_CONFIG_802_11AN */
	(UINT_8)PHY_TYPE_SET_802_11GN,	/* PHY_CONFIG_802_11GN */
	(UINT_8)PHY_TYPE_SET_802_11AC,
	(UINT_8)PHY_TYPE_SET_802_11ANAC,
	(UINT_8)PHY_TYPE_SET_802_11ABGNAC
};

static const ECO_INFO_T g_arEcoVersionTable[] = {
...
static INT_EVENT_MAP_T arIntEventMapTable[] = {
	{(UINT_32)WHISR_ABNORMAL_INT, (UINT_32)INT_EVENT_ABNORMAL},
	{(UINT_32)WHISR_D2H_SW_INT, (UINT_32)INT_EVENT_SW_INT},
	{(UINT_32)WHISR_TX_DONE_INT, (UINT_32)INT_EVENT_TX},
	{(UINT_32)(WHISR_RX0_DONE_INT | WHISR_RX1_DONE_INT),
		(UINT_32)INT_EVENT_RX}
};

static const UINT_8 ucIntEventMapSize =
	((UINT_8)sizeof(arIntEventMapTable) / (UINT_8)sizeof(INT_EVENT_MAP_T));
...
#define LOCAL_NIC_ALLOCATE_MEMORY(pucMem, u4Size, eMemType, pucComment) \
	{ \
		...
		ASSERT_BOOLEAN(((ULONG)(pucMem) % 4U) == 0U); \
...
WLAN_STATUS nicAllocateAdapterMemory(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		...
		prAdapter->u4MgtBufCachedSize = (UINT_32)MGT_BUFFER_SIZE;
		...
		prRxCtrl->u4RxCachedSize = CFG_RX_MAX_PKT_NUM
			* ALIGN_4((UINT_32)sizeof(SW_RFB_T));
		...
		prTxCtrl->u4TxCachedSize = CFG_TX_MAX_PKT_NUM
			* ALIGN_4((UINT_32)sizeof(MSDU_INFO_T));
	...
	do {
		...
		if (wlanIsChipNoAck(prAdapter) != TRUE) {
			...
			if (prAdapter->u4MemFreeDynamicCount != prAdapter->u4MemAllocDynamicCount) {
				...
				LINK_FOR_EACH_ENTRY(prMemTrack, &prAdapter->rMemTrackLink,
					rLinkEntry, struct MEM_TRACK) {
					DBGLOG(MEM, ERROR,
					"file:line %s, command id: %u, where: %u\n",
			...
			ASSERT_BOOLEAN(prAdapter->u4MemFreeDynamicCount
...
VOID nicDisableInterrupt(IN P_ADAPTER_T prAdapter)
{
	ASSERT(prAdapter);
#if defined(MT6631)
	HAL_MCR_WR(prAdapter, MCR_WHLPCR, (UINT_8)WHLPCR_INT_EN_CLR);
#else
	HAL_BYTE_WR(prAdapter, MCR_WHLPCR, (UINT_8)WHLPCR_INT_EN_CLR);
#endif
...
VOID nicEnableInterrupt(IN P_ADAPTER_T prAdapter)
{
	...
	if (prAdapter->fgIsIntEnableWithLPOwnSet != FALSE) {
	...
	else if (fgIsIntEnableCache != TRUE) {
#if defined(MT6631)
		HAL_MCR_WR(prAdapter, MCR_WHLPCR, (UINT_8)WHLPCR_INT_EN_SET);
		__enable_irq();
#else
		HAL_BYTE_WR(prAdapter, MCR_WHLPCR, (UINT_8)WHLPCR_INT_EN_SET);
#endif
	} else {
	}
...
VOID nicSDIOInit(IN P_ADAPTER_T prAdapter)
{
	...
	ASSERT_BOOLEAN(IS_ALIGN_4((ULONG)(&prAdapter->prSDIOCtrl->u4WHISR))
	!= FALSE);
	...
	/* 4 <2> Setup STATUS count. */
	{
		...
		u4Value |= (UINT_32)WHCR_RX_ENHANCE_MODE_EN;
#else
		u4Value &= ~(UINT_32)WHCR_RX_ENHANCE_MODE_EN;
...
VOID nicSDIOReadIntStatus(IN P_ADAPTER_T prAdapter, OUT PUINT_32 pu4IntStatus)
{
	...
	/* workaround */
	if ((prSDIOCtrl->u4WHISR & WHISR_TX_DONE_INT) == 0U
		&& (prSDIOCtrl->rTxInfo.au4WTSR[0]
			...
			| prSDIOCtrl->rTxInfo.au4WTSR[7]) != 0U) {
		prSDIOCtrl->u4WHISR |= (UINT_32)WHISR_TX_DONE_INT;
	}

	if ((prSDIOCtrl->u4WHISR & (UINT_32)BIT(31U)) == 0U &&
	    HAL_GET_MAILBOX_READ_CLEAR(prAdapter) == TRUE &&
	    (prSDIOCtrl->u4RcvMailbox0 != 0U ||
	    prSDIOCtrl->u4RcvMailbox1 != 0U)) {
		prSDIOCtrl->u4WHISR |= (UINT_32)BIT(31U);
	}
...
WLAN_STATUS nicProcessIST(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < prAdapter->rWifiVar.u4HifIstLoopCount; i++) {
		...
		if ((u4IntStatus & ~(WHIER_DEFAULT
			| WHIER_FW_OWN_BACK_INT_EN)) != 0U) {
			DBGLOG(INTR, WARN, "Un-handled HISR %#lx, HISR = %#lx (HIER:0x%lx)\n",
					(u4IntStatus & ~(UINT_32)WHIER_DEFAULT),
					u4IntStatus, (UINT_32)WHIER_DEFAULT);
			u4IntStatus &= (UINT_32)WHIER_DEFAULT;
		}
...
WLAN_STATUS nicProcessIST_impl(IN P_ADAPTER_T prAdapter, IN UINT_32 u4IntStatus)
{
	...
	for (u4IntCount = 0; u4IntCount < ucIntEventMapSize; u4IntCount++) {
		if ((prIntEventMap->u4Int & prAdapter->u4IntStatus) != 0U) {
			if (prIntEventMap->u4Event == (UINT_32)INT_EVENT_RX
...
UINT_16 nicGetChipID(IN P_ADAPTER_T prAdapter)
{
	...
	if (prAdapter->fgIsReadRevID == FALSE) {
		...
		if (prAdapter->u2ChipID == 0x0279U)	/* Everest */
			prAdapter->u2ChipID = 0x6797U;
		if (prAdapter->u2ChipID == 0x0507U)	/* Alaska */
			prAdapter->u2ChipID = 0x6759U;
...
BOOL nicVerifyChipID(IN P_ADAPTER_T prAdapter)
{
...
#if defined(MT6631)
#ifdef CONFIG_OF
	if (prAdapter->prGlueInfo->rHifInfo.Dev != NULL) {
	...
	if ((u4CIR & WCIR_CHIP_ID) != u4HwCode) {
		DBGLOG(NIC, ERROR,
			"HW code mismatch from chip[%lu] and pre-defined[%lu]\n",
			u4CIR & WCIR_CHIP_ID, u4HwCode);
		return FALSE;
	}
#endif
...
VOID nicProcessSoftwareInterrupt(IN P_ADAPTER_T prAdapter)
{
	...
	u4IntrBits = prAdapter->u4IntStatus & (UINT_32)BITS(8U, 31U);

	if ((u4IntrBits & WHISR_D2H_SW_ASSERT_INFO_INT) != 0U) {
...
VOID nicSetSwIntr(IN P_ADAPTER_T prAdapter, IN UINT_32 u4SwIntrBitmap)
{
	 ...
	 */
	ASSERT_BOOLEAN((u4SwIntrBitmap & (UINT_32)BITS(0U, 15U)) == 0U);
...
nicMediaStateChange(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, IN P_EVENT_CONNECTION_STATUS prConnectionStatus)
{
	...
	case NETWORK_TYPE_AIS:
		if (prConnectionStatus->ucMediaStatus ==
			(UINT_8)PARAM_MEDIA_STATE_DISCONNECTED) {
			...
			prAdapter->fgIsLinkRateValid = FALSE;
					...
					(UINT_8)PARAM_MEDIA_STATE_CONNECTED) {
			prAdapter->rWlanInfo.u4SysTime = kalGetTimeTick();
...
UINT_8 nicGetVhtS1(UINT_8 ucPrimaryChannel)
{
	...
	if (ucPrimaryChannel >= 36U && ucPrimaryChannel <= 48U)
		return 42U;
	else if (ucPrimaryChannel >= 52U && ucPrimaryChannel <= 64U)
		return 58U;
	else if (ucPrimaryChannel >= 100U && ucPrimaryChannel <= 112U)
		return 106U;
	else if (ucPrimaryChannel >= 116U && ucPrimaryChannel <= 128U)
		return 122U;
	else if (ucPrimaryChannel >= 132U && ucPrimaryChannel <= 144U)
		return 138;
	else if (ucPrimaryChannel >= 149U && ucPrimaryChannel <= 161U)
		return 155U;
	else
		return 0U;
}
...
WLAN_STATUS nicActivateNetwork(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	ASSERT(prAdapter);
	ASSERT_BOOLEAN(IS_BSS_INDEX_VALID(ucBssIndex));
	...
	if (EQUAL_MAC_ADDR(&aucZeroMacAddr[0], &prBssInfo->aucBSSID[0])) {
		prBssInfo->ucBMCWlanIndex =
		    secPrivacySeekForBcEntry(prAdapter, prBssInfo->ucBssIndex,
					     prBssInfo->aucOwnMacAddr, 0xffU,
					     CIPHER_SUITE_NONE, 0xffU, 0x0U,
					     (UINT_8)BIT(0U));
	} else {
		prBssInfo->ucBMCWlanIndex =
			secPrivacySeekForBcEntry(prAdapter, prBssInfo->ucBssIndex, prBssInfo->aucBSSID,
					0xffU, CIPHER_SUITE_NONE,
					0xffU, 0x0U, (UINT_8)BIT(0U));
	}
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_BSS_ACTIVATE_CTRL,
				   TRUE,
				   FALSE,
				   FALSE,
				   NULL, NULL,
				   (UINT_32)sizeof(CMD_BSS_ACTIVATE_CTRL),
...
WLAN_STATUS nicUpdateBss(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	rCmdSetBssInfo.ucHiddenSsidMode = (UINT_8)prBssInfo->eHiddenSsidType;
#endif
	...
	if (rCmdSetBssInfo.ucBssIndex == prAdapter->prAisBssInfo->ucBssIndex) {
		...
		if (prAdapter->fgIsP2PRegistered != FALSE) {
			...
			rCmdSetBssInfo.ucIsApMode =
				(UINT_8)p2pFuncIsAPMode(
				prAdapter->rWifiVar.prP2PConnSettings);

			if (rCmdSetBssInfo.ucIsApMode != 0U)
...
#if CFG_ENABLE_WIFI_DIRECT
	else if ((prAdapter->fgIsP2PRegistered != FALSE) &&
	...
	u4Status = wlanSendSetQueryCmd(prAdapter,
				       (UINT_8)CMD_ID_SET_BSS_INFO,
				       TRUE,
				       FALSE,
				       FALSE,
				       NULL,
				       NULL,
				       (UINT_32)sizeof(CMD_SET_BSS_INFO),
...
WLAN_STATUS nicPmIndicateBssCreated(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_INDICATE_PM_BSS_CREATED,
				   TRUE,
				   FALSE,
				   FALSE,
				   NULL,
				   NULL,
				   (UINT_32)sizeof(CMD_INDICATE_PM_BSS_CREATED),
...
WLAN_STATUS nicPmIndicateBssConnected(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
		   (UINT_8)CMD_ID_INDICATE_PM_BSS_CONNECTED,
		   TRUE,
		   FALSE,
		   FALSE,
		   NULL,
		   NULL,
		   (UINT_32)sizeof(CMD_INDICATE_PM_BSS_CONNECTED),
...
WLAN_STATUS nicPmIndicateBssAbort(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_INDICATE_PM_BSS_ABORT,
				   TRUE,
				   FALSE,
				   FALSE,
				   NULL,
				   NULL,
				   (UINT_32)sizeof(CMD_INDICATE_PM_BSS_ABORT),
...
/* prAdapter->rWlanInfo.ePowerSaveMode.ucPsProfile = (UINT_8)ePwrMode; */
	...
	return wlanSendSetQueryCmd(prAdapter,
				   ...
				   TRUE,
				(fgEnCmdEvent != FALSE
					? nicCmdEventSetCommon : NULL),
				   (fgEnCmdEvent != FALSE
					? nicOidCmdTimeoutCommon : NULL),
				   (UINT_32)sizeof(CMD_PS_PROFILE_T),
				   (PUINT_8)&(prAdapter->rWlanInfo.arPowerSaveMode[ucBssIndex]),
				   NULL, (UINT_32)sizeof(PARAM_POWER_MODE)
	    );
...
WLAN_STATUS nicEnterCtiaMode(IN P_ADAPTER_T prAdapter, BOOLEAN fgEnterCtia, BOOLEAN fgEnCmdEvent)
{
	...
	if (fgEnterCtia != FALSE) {
		...
		rCmdSwCtrl.u4Id = 0xa0100003U;
		rCmdSwCtrl.u4Data = 0x0U;
			  ...
			  IN UINT_8 ucBssIndex, IN UINT_16 u2Capability, IN PUINT_8 aucIe, IN UINT_16 u2IELen)
{
	...
	if (eIeUpdMethod == IE_UPD_METHOD_UPDATE_RANDOM || eIeUpdMethod == IE_UPD_METHOD_UPDATE_ALL) {
		u2CmdBufLen =
			(UINT_16)OFFSET_OF(CMD_BEACON_TEMPLATE_UPDATE, aucIE)
			+ u2IELen;
	} else if (eIeUpdMethod == IE_UPD_METHOD_DELETE_ALL) {
		u2CmdBufLen =
			(UINT_16)OFFSET_OF(CMD_BEACON_TEMPLATE_UPDATE, u2IELen);
	} else {
		ASSERT(NULL);
	...
	prCmdInfo = cmdBufAllocateCmdInfo(prAdapter,
		((UINT_32)CMD_HDR_SIZE + (UINT_32)u2CmdBufLen));
	if (prCmdInfo == NULL) {
	...
	prCmdInfo->ucCID = (UINT_8)CMD_ID_UPDATE_BEACON_CONTENT;
	...
	prWifiCmd->ucSetQuery = (UINT_8)prCmdInfo->fgSetQuery;
...
WLAN_STATUS nicQmUpdateWmmParms(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	kalMemCopy(&rCmdUpdateWmmParms.arACQueParms[0],
		...
		(sizeof(AC_QUE_PARMS_T) * (UINT_32)AC_NUM));

	rCmdUpdateWmmParms.fgIsQBSS = (UINT_8)(prBssInfo->fgIsQBSS);

	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_UPDATE_WMM_PARMS,
				   TRUE,
				   FALSE,
				   FALSE,
				   NULL, NULL,
				   (UINT_32)sizeof(CMD_UPDATE_WMM_PARMS_T),
...
WLAN_STATUS nicUpdateTxPower(IN P_ADAPTER_T prAdapter, IN P_CMD_TX_PWR_T prTxPwrParam)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_SET_TX_PWR,
				   TRUE,
				   FALSE, FALSE, NULL, NULL,
				   (UINT_32)sizeof(CMD_TX_PWR_T),
				...
				IN P_CMD_AUTO_POWER_PARAM_T prAutoPwrParam)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_SET_AUTOPWR_CTRL,
				   TRUE,
				   FALSE,
				   FALSE,
				   NULL, NULL,
				   (UINT_32)sizeof(CMD_AUTO_POWER_PARAM_T),
...
/*----------------------------------------------------------------------------*/
#if 0
static WLAN_STATUS nicSetAutoTxPowerControl(IN P_ADAPTER_T prAdapter,
...
WLAN_STATUS nicUpdateDPD(IN P_ADAPTER_T prAdapter, IN P_CMD_PWR_PARAM_T prDpdCalResult)
{
	...
	return wlanSendSetQueryCmd(prAdapter,
				   (UINT_8)CMD_ID_SET_PWR_PARAM,
				   TRUE,
				   FALSE,
				   FALSE, NULL, NULL,
				   (UINT_32)sizeof(CMD_PWR_PARAM_T),
		 ...
		 IN PARAM_RATES_EX rSupportedRates, IN UINT_16 u2IELength, IN PUINT_8 pucIEBuf)
{
	...
	u4BufferSize = (UINT_32) ARRAY_SIZE(prAdapter->rWlanInfo.aucScanIEBuf);
	...
	for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
		...
		if (prAdapter->rWlanInfo.arScanResult[i].eOpMode == eOpMode &&
		    ...
		    &&
		    (EQUAL_SSID
		     (&(prAdapter->rWlanInfo.arScanResult[i].rSsid.aucSsid[0]),
		      prAdapter->rWlanInfo.arScanResult[i].rSsid.u4SsidLen,
		      &prSsid->aucSsid[0], prSsid->u4SsidLen)
		     || prAdapter->rWlanInfo.arScanResult[i].rSsid.u4SsidLen
		     == 0U)) {
			/* replace entry */
			bReplace = TRUE;
			...
			prAdapter->rWlanInfo.arScanResult[i].u4Length =
			  (UINT_32)OFFSET_OF(PARAM_BSSID_EX_T, aucIEs)
			  + (UINT_32)u2IELength;
			...
			prAdapter->rWlanInfo.arScanResult[i].u4Privacy =
				((u2CapInfo & (UINT_16)CAP_INFO_PRIVACY)
				!= 0U) ? 1U : 0U;
...
VOID nicFreeScanResultIE(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Idx)
{
	...
	ASSERT(prAdapter);
	ASSERT_BOOLEAN(u4Idx < CFG_MAX_NUM_BSS_LIST);

	if (prAdapter->rWlanInfo.arScanResult[u4Idx].u4IELength == 0U
	...
	u4ReserveSize = (UINT_32)(((ULONG) pucPivot)
...
{
	ASSERT(prAdapter);
	ASSERT_BOOLEAN(eRateSetting > FIXED_RATE_NONE
		&& eRateSetting < FIXED_RATE_NUM);

	switch (prAdapter->rWifiVar.eRateSetting) {
	case FIXED_RATE_1M:
		*pucDesiredPhyTypeSet = (UINT_8)PHY_TYPE_BIT_HR_DSSS;
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_1M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_1M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_2M:
		*pucDesiredPhyTypeSet = (UINT_8)PHY_TYPE_BIT_HR_DSSS;
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_2M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_2M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_5_5M:
		*pucDesiredPhyTypeSet = (UINT_8)PHY_TYPE_BIT_HR_DSSS;
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_5_5M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_5_5M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_11M:
		*pucDesiredPhyTypeSet = (UINT_8)PHY_TYPE_BIT_HR_DSSS;
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_11M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_11M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_6M:
		if (((*pucDesiredPhyTypeSet)
				| (UINT_8)PHY_TYPE_BIT_ERP) != 0U) {
			*pucDesiredPhyTypeSet = (UINT_8)PHY_TYPE_BIT_ERP;
		} else if (((*pucDesiredPhyTypeSet)
				| (UINT_8)PHY_TYPE_BIT_OFDM) != 0U) {
			*pucDesiredPhyTypeSet = (UINT_8)PHY_TYPE_BIT_OFDM;
		} else {
		}


		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_6M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_6M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_9M:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_9M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_9M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_12M:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_12M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_12M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_18M:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_18M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_18M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_24M:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_24M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_24M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_36M:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_36M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_36M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_48M:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_48M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_48M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_54M:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_54M;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_54M;
		*pucMcsSet = 0;
		*pucSupMcs32 = 0;
		*pu2HtCapInfo = 0;
		break;
	...
	case FIXED_RATE_MCS0_20M_800NS:
		*pucDesiredPhyTypeSet = (UINT_8)PHY_TYPE_BIT_HT;
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS0_INDEX;
		*pucSupMcs32 = 0U;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
	...
	case FIXED_RATE_MCS1_20M_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS1_INDEX;
		*pucSupMcs32 = 0U;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
	...
	case FIXED_RATE_MCS2_20M_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS2_INDEX;
		*pucSupMcs32 = 0;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
	...
	case FIXED_RATE_MCS3_20M_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS3_INDEX;
		*pucSupMcs32 = 0;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
	...
	case FIXED_RATE_MCS4_20M_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS4_INDEX;
		*pucSupMcs32 = 0;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
	...
	case FIXED_RATE_MCS5_20M_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS5_INDEX;
		*pucSupMcs32 = 0;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
	...
	case FIXED_RATE_MCS6_20M_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS6_INDEX;
		*pucSupMcs32 = 0;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
	...
	case FIXED_RATE_MCS7_20M_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS7_INDEX;
		*pucSupMcs32 = 0;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
	...
	case FIXED_RATE_MCS0_20M_400NS:
		...
		*pucSupMcs32 = 0;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SUP_CHNL_WIDTH
				| HT_CAP_INFO_SHORT_GI_40M
				| HT_CAP_INFO_HT_GF));
		(*pu2HtCapInfo) |= (UINT_16)HT_CAP_INFO_SHORT_GI_20M;
		break;
	...
	case FIXED_RATE_MCS32_800NS:
		...
		*pu2DesiredNonHTRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pu2BSSBasicRateSet = (UINT_16)RATE_SET_BIT_HT_PHY;
		*pucMcsSet = (UINT_8)HT_RATE_MCS32_INDEX;
		*pucSupMcs32 = 1;
		(*pu2HtCapInfo) &= (UINT_16)(~(HT_CAP_INFO_SHORT_GI_20M
				| HT_CAP_INFO_SHORT_GI_40M
				| HT_CAP_INFO_HT_GF));
		    ...
		    IN UINT_32 u4ArSysParam1, IN UINT_32 u4ArSysParam2, IN UINT_32 u4ArSysParam3)
{
	...
	ucArVer = (UINT_8) (u4ArSysParam0 & (UINT_32)BITS(0U, 3U));
	ucAbwVer = (UINT_8) ((u4ArSysParam0 & (UINT_32)BITS(4U, 5U)) >> 4U);
	ucAgiVer = (UINT_8) ((u4ArSysParam0 & (UINT_32)BITS(6U, 7U)) >> 6U);
	u2HtClrMask = (UINT_16) ((u4ArSysParam0
		& (UINT_32)BITS(8U, 15U)) >> 8U);
	u2LegacyClrMask = (UINT_16) ((u4ArSysParam0
		& (UINT_32)BITS(16U, 31U)) >> 16U);
	...
	ucArCheckWindow = (UINT_8) (u4ArSysParam1 & (UINT_32)BITS(0U, 7U));
	ucArPerForceRateDownPer = (UINT_8) (((u4ArSysParam1 >> 8U)
		& (UINT_32)BITS(0U, 7U)));
	ucArPerH = (UINT_8) (((u4ArSysParam1 >> 16U) & (UINT_32)BITS(0U, 7U)));
	ucArPerL = (UINT_8) (((u4ArSysParam1 >> 24U) & (UINT_32)BITS(0U, 7U)));
...
#define SWCR_DATA_ADDR(MOD, ADDR) (0x90000000U+((MOD)<<8U)+((ADDR)))
#define SWCR_DATA_CMD(CATE, WRITE, INDEX, OPT0, OPT1) \
	(((CATE)<<24U) | ((WRITE)<<23U) | \
	((INDEX)<<16U) | ((OPT0) << 8U) | (OPT1))
#define SWCR_DATA0 0x0U
#define SWCR_DATA1 0x4U
#define SWCR_DATA2 0x8U
#define SWCR_DATA3 0xCU
#define SWCR_DATA4 0x10U
#define SWCR_WRITE 1U
#define SWCR_READ 0U
	...
	if (ucArVer > 0U) {
		...
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA1), 1U);
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA0),
			SWCR_DATA_CMD(0U, SWCR_WRITE, 5U, 0U, 0U));
	} else {
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA1), 0U);
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA0),
			SWCR_DATA_CMD(0U, SWCR_WRITE, 5U, 0U, 0U));
	}
	...
	nicWriteMcr(prAdapter, SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA1), ucArVer);
	nicWriteMcr(prAdapter,
		SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA0),
		SWCR_DATA_CMD(0U, SWCR_WRITE, 7U, 0U, 0U));

	/* Candidate rate Ht mask */
	nicWriteMcr(prAdapter,
		SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA1), u2HtClrMask);
	nicWriteMcr(prAdapter,
		SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA0),
		SWCR_DATA_CMD(0U, SWCR_WRITE, 0x1cU, 0U, 0U));

	/* Candidate rate legacy mask */
	nicWriteMcr(prAdapter,
		SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA1), u2LegacyClrMask);
	nicWriteMcr(prAdapter,
		SWCR_DATA_ADDR(1U /*MOD*/, SWCR_DATA0),
		SWCR_DATA_CMD(0U, SWCR_WRITE, 0x1dU, 0U, 0U));
	...
	if (ucArPerH != 0) {
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1 /*MOD*/, SWCR_DATA1), ucArPerH);
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1 /*MOD*/, SWCR_DATA0),
	...
	if (ucArPerL != 0) {
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1 /*MOD*/, SWCR_DATA1), ucArPerL);
		nicWriteMcr(prAdapter,
			SWCR_DATA_ADDR(1 /*MOD*/, SWCR_DATA0),
...
#else
	nicGetMailbox(prAdapter, 0U, &u4MailBox0);
	nicGetMailbox(prAdapter, 1U, &u4MailBox1);
#endif

	line = u4MailBox0 & 0x0000FFFFU;

	u4MailBox0 = ((u4MailBox0 >> 16U) & 0x0000FFFFU);

	kalMemCopy(&aucAssertFile[0], &u4MailBox0, 2U);
	kalMemCopy(&aucAssertFile[2], &u4MailBox1, 4U);
	...
	case NETWORK_TYPE_AIS:
		if (GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
			...
			if ((prAdapter->fgIsLinkQualityValid == FALSE)
				...
				cRssi = (INT_8) (((INT_16) (cRssi)
					* (INT_16)u2AdjustRssi) / 10);
				DBGLOG(RLM, INFO,
...
static UINT_8 nicGetChipHwVer(VOID)
{
	...
	ret = (UINT_8)(mtk_wcn_wmt_ic_info_get(WMTCHIN_HWVER)
		& (UINT_32)BITS(0U, 7U));
	return ret;
}
...
static UINT_8 nicGetChipSwVer(VOID)
{
	...
	ret = (UINT_8)(mtk_wcn_wmt_ic_info_get(WMTCHIN_FWVER)
		& (UINT_32)BITS(0U, 7U));
	return ret;
}
...
UINT_8 nicGetChipEcoVer(VOID)
{
	...
	ucMaxEcoVer = (UINT_8)(sizeof(g_arEcoVersionTable)
		/ sizeof(ECO_INFO_T));
...
BOOLEAN nicIsEcoVerEqualTo(UINT_8 ucEcoVer)
{
	...
	/* Unknown ECO version */
	if ((ucEcoVer > ucMaxEcoVer) || (ucEcoVer == 0U))
		return FALSE;

	/* Get ECO info from table */
	prEcoInfo = (P_ECO_INFO_T) & (g_arEcoVersionTable[ucEcoVer - 1U]);
	...
	IN P_PARAM_CUSTOM_UAPSD_PARAM_STRUCT_T prUapsdParams, IN ENUM_NETWORK_TYPE_T eNetworkTypeIdx)
{
	...
	prPmProfSetupInfo->ucBmpDeliveryAC =
	    ((prUapsdParams->fgEnAPSD_AcBe << 0U) |
	     (prUapsdParams->fgEnAPSD_AcBk << 1U) |
	     (prUapsdParams->fgEnAPSD_AcVi << 2U) |
	     (prUapsdParams->fgEnAPSD_AcVo << 3U));
