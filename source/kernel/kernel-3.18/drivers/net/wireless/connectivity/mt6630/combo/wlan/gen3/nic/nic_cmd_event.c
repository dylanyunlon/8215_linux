...
VOID nicCmdEventQueryMcrRead(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		prGlueInfo = prAdapter->prGlueInfo;
		prCmdAccessReg = (P_CMD_ACCESS_REG) (pucEventBuf);

		u4QueryInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_MCR_RW_STRUCT_T);
...
VOID nicCmdEventQuerySwCtrlRead(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = (UINT_32)sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T);
...
VOID nicCmdEventQueryChipConfig(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen =
			(UINT_32)sizeof(PARAM_CUSTOM_CHIP_CONFIG_STRUCT_T);
...
VOID nicCmdEventSetIpAddress(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	(UINT_32)OFFSET_OF(CMD_SET_NETWORK_ADDRESS_LIST, arNetAddress))
	/ (UINT_32)sizeof(IPV4_NETWORK_ADDRESS);

	if (prCmdInfo->fgIsOid != FALSE) {
		/* Update Set Information Length */
		kalOidComplete(prAdapter->prGlueInfo,
			prCmdInfo->fgSetQuery,
			(UINT_32)OFFSET_OF(PARAM_NETWORK_ADDRESS_LIST,
				arAddress) + u4Count *
			(UINT_32)(OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress) +
...
VOID nicCmdEventQueryRfTestATInfo(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = (UINT_32)sizeof(EVENT_TEST_STATUS);
...
VOID nicCmdEventQueryLinkQuality(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = (UINT_32)sizeof(PARAM_RSSI);
...
VOID nicCmdEventQueryLinkSpeed(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		*pu4LinkSpeed = (UINT_32)(prLinkQuality->u2LinkSpeed) * 5000U;

		u4QueryInfoLen = (UINT_32)sizeof(UINT_32);
...
{
#define WAIT_FW_READY_RETRY_CNT 200U

	UINT_32 u4WHISR = 0, u4Value = 0;
	UINT_32 au4TxCount[8];
	...
	while (1 == 1) {
		...
		if ((u4Value & (UINT_32)WCIR_WLAN_READY) != 0U) {
			break;
		} else if (kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE
			|| fgIsBusAccessFailed == TRUE
			|| kalIsResetting() != FALSE
			|| u2RetryCnt >= WAIT_FW_READY_RETRY_CNT) {
			if (prCmdInfo->fgIsOid != FALSE) {
				/* Update Set Information Length */
				kalOidComplete(prAdapter->prGlueInfo,
					       prCmdInfo->fgSetQuery,
	...
	if (HAL_IS_TX_DONE_INTR(u4WHISR) != FALSE)
...
#define WAIT_FW_READY_RETRY_CNT 200U
	...
	HAL_READ_INTR_STATUS(prAdapter, 4U, (PUINT_8)&u4WHISR);
	if (HAL_IS_TX_DONE_INTR(u4WHISR) != FALSE)
		HAL_READ_TX_RELEASED_COUNT(prAdapter, au4TxCount);
	/* 4. Reset TX Counter */
	nicTxResetResource(prAdapter);
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		if (prCmdInfo->u4InformationBufferLength < u4QueryInfoLen) {
			...
			kalMemCopy(prCmdInfo->pvInformationBuffer,
				...
				(UINT_64)prEventMacMcastAddr->u4NumOfGroupAddr
				* MAC_ADDR_LEN);
...
VOID nicCmdEventSetMediaStreamMode(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	rParamMediaStreamIndication.eMediaStreamMode =
		prAdapter->rWlanInfo.eLinkAttr.ucMediaStreamMode ==
			0U ? ENUM_MEDIA_STREAM_OFF : ENUM_MEDIA_STREAM_ON;

	kalIndicateStatusAndComplete(prAdapter->prGlueInfo,
			WLAN_STATUS_MEDIA_SPECIFIC_INDICATION,
			(PVOID)&rParamMediaStreamIndication,
			(UINT_32)sizeof(PARAM_MEDIA_STREAMING_INDICATION));
}
...
VOID nicCmdEventSetStopSchedScan(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	 ...
	 */
	BOOLEAN fgDriverTriggerd;

	ASSERT(prAdapter);
	ASSERT(prCmdInfo);

	fgDriverTriggerd = (prCmdInfo->fgIsOid
		!= TRUE ? TRUE : FALSE);
	...
	kalSchedScanStopped(prAdapter->prGlueInfo, fgDriverTriggerd);
...
VOID nicCmdEventQueryXmitOk(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		if (prCmdInfo->u4InformationBufferLength == sizeof(UINT_32)) {
			...
			*pu4Data = (UINT_32) prEventStatistics
				->rTransmittedFragmentCount.QuadPart;
		} else {
			u4QueryInfoLen = (UINT_32)sizeof(UINT_64);

			pu8Data = (PUINT_64) prCmdInfo->pvInformationBuffer;
			*pu8Data = (UINT_64)prEventStatistics
				->rTransmittedFragmentCount.QuadPart;
		}
...
VOID nicEventQueryMemDump(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucEventBuf)
{
	...
	if (prEventDumpMem->ucFragNum == 1U) {
	...
	if (prEventDumpMem->u4RemainLength == 0U
		|| prEventDumpMem->u4Address == 0xFFFFFFFFU) {
...
VOID nicCmdEventQueryMemDump(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (1 == 1) {
		...
		u4QueryInfoLen =
			(UINT_32)sizeof(P_PARAM_CUSTOM_MEM_DUMP_STRUCT_T);
		...
		if (prEventDumpMem->ucFragNum == 1U) {
			...
			if (kalCheckPath(aucPath) == 1U) {
				kalMemSet(aucPath, 0x00, 256);
				sprintf(aucPath, "/data/dump_%05d.hex",
					g_u2DumpIndex);
				} else
					kalTrunkPath(aucPath);

				DBGLOG(INIT, INFO,
					"iCap Create Dump File dump_%05d.hex\n",
					g_u2DumpIndex);
#else
...
VOID nicCmdEventBatchScanResult(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = (UINT_32)sizeof(EVENT_BATCH_RESULT_T);
...
VOID nicCmdEventBuildDateCode(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = sizeof(UINT_8) * 16U;
...
VOID nicCmdEventQueryStaStatistics(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = (UINT_32)sizeof(PARAM_GET_STA_STA_STATISTICS);

		/* Statistics from FW is valid */
		if ((prEvent->u4Flags & (UINT_32)BIT(0U)) != 0U) {
...
VOID nicCmdEventQueryLteSafeChn(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = (UINT_32)sizeof(PARAM_GET_CHN_INFO);

		/* Statistics from FW is valid */
		if ((prEvent->u4Flags & (UINT_32)BIT(0U)) != 0U) {
			for (ucIdx = 0U; ucIdx < 5U; ucIdx++) {
...
VOID nicCmdEventQueryCfgRead(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid != FALSE) {
		...
		u4QueryInfoLen = (UINT_32)sizeof(struct _CMD_HEADER_T);
...
VOID nicEventUpdateFwInfo(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	for (i = 0; i < (HW_BSSID_NUM + 1U); i++) {
		if (prAdapter->aprBssInfo[i] != NULL) {
	...
	IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	struct GL_DETECT_REPLAY_INFO *pRpInfo = NULL;
	...
	prGlueInfo = prAdapter->prGlueInfo;
	pRpInfo = &prGlueInfo->prDetRplyInfo;
	if (pucEventBuf != NULL) {
		prWifiCmd = (P_WIFI_CMD_T) (pucEventBuf);
		prCmdKey = (P_CMD_802_11_KEY) (prWifiCmd->aucBuffer);
		if (prCmdKey->ucKeyType == 0U) {
			pRpInfo->ucCurKeyId = prCmdKey->ucKeyId;
			pRpInfo->ucKeyType = prCmdKey->ucKeyType;
			pRpInfo->arReplayPNInfo[prCmdKey->ucKeyId].fgRekey =
				TRUE;
			pRpInfo->arReplayPNInfo[prCmdKey->ucKeyId].fgFirstPkt =
				TRUE;
