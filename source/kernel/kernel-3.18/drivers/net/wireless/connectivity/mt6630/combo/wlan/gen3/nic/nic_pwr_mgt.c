...
VOID nicpmSetFWOwn(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgEnableGlobalInt)
{
	...
	&& (u4RegValue & WHLPCR_FW_OWN_REQ_SET) != 0U)
	|| ((prAdapter->fgIsFwOwn == FALSE)
	&& ((u4RegValue & WHLPCR_FW_OWN_REQ_SET) == 0U)))
		DBGLOG(NIC, ERROR, "FW OWN is %x, WHLPCR=%x\n",
			prAdapter->fgIsFwOwn, u4RegValue);

	if (prAdapter->fgIsFwOwn == TRUE)
		return;

	if ((u4RegValue & WHLPCR_FW_OWN_REQ_SET) == 0U)
		return;
...
{
#define LP_OWN_BACK_TOTAL_DELAY_MS      2048U	/* exponential of 2 */
#define LP_OWN_BACK_CLR_OWN_ITERATION   256U	/* exponential of 2 */
#define LP_OWN_BACK_FAILED_RETRY_CNT    5U
#define LP_OWN_BACK_FAILED_LOG_SKIP_MS  2000U
#define LP_OWN_BACK_FAILED_RESET_CNT    3U // 5U
#define LP_OWN_BACK_LOOP_DELAY_MIN_US   900U
#define LP_OWN_BACK_LOOP_DELAY_MAX_US   1000U

	BOOLEAN fgStatus = TRUE;
	UINT_32 u4CurrTick, u4RegValue = 0;
	BOOLEAN fgTimeout;
	...
	if (fgHifFwOwn != prAdapter->fgIsFwOwn) {
		DBGLOG(NIC, ERROR,"Driver own status mismatch! fgIsFwOwn=%d, WHLPCR=0x%x\n",
				prAdapter->fgIsFwOwn, u4RegValue);
		if (!fgHifFwOwn) {
			prAdapter->fgIsFwOwn = FALSE;
			return fgStatus;
	...
	while (1 == 1) {
		...
		if ((u4RegValue & WHLPCR_FW_OWN_REQ_SET) != 0U) {
			...
			if ((prAdapter->u4OwnFailedCount == 0U) ||
		...
		if ((i & (LP_OWN_BACK_CLR_OWN_ITERATION - 1U)) == 0U) {
	...
	if (i > 10)
		DBGLOG(NIC, WARN, "DRIVER OWN, status=%d count=%d\n", fgStatus, i);
	else
