...
const UINT_16 au2RateHTMixed[9] = {
...
const UINT_16 au2RateHTGreenField[9] = {
...
nicGetHwRateByPhyRate(
	IN UINT_8 ucIdx)
{
	return	(UINT_32)aucHwRate2PhyRate[ucIdx]; /* uint : 500 kbps */
}
	...
	)
{
	...
	if (ucRateSwIndex >= (UINT_8)RATE_6M_SW_INDEX) {
		*pucRateIndex = ucRateSwIndex - (UINT_8)RATE_6M_SW_INDEX;
		*pucPreambleOption = (UINT_8)PREAMBLE_OFDM_MODE;
	} else {
		*pucRateIndex = ucRateSwIndex;
		*pucPreambleOption = (UINT_8)PREAMBLE_DEFAULT_LONG_NONE;
	}
...
{
	BOOLEAN fgInvalidData = FALSE;

	switch (ucPreambleOption) {
	case (UINT_8)PREAMBLE_DEFAULT_LONG_NONE:
		if (ucRateIndex >= (UINT_8)CCK_RATE_NUM)
			fgInvalidData = TRUE;
		else
			*pu2RateCode = au2RateCCKLong[ucRateIndex];
		break;

	case (UINT_8)PREAMBLE_OPTION_SHORT:
		if (ucRateIndex >= (UINT_8)CCK_RATE_NUM)
			fgInvalidData = TRUE;
		else
	...
	case (UINT_8)PREAMBLE_OFDM_MODE:
		if (ucRateIndex >= (UINT_8)OFDM_RATE_NUM)
			fgInvalidData = TRUE;
		else
			*pu2RateCode = au2RateOFDM[ucRateIndex];
		break;

	case (UINT_8)PREAMBLE_HT_MIXED_MODE:
		if (ucRateIndex >= 9U)
			fgInvalidData = TRUE;
		else
	...
	case (UINT_8)PREAMBLE_HT_GREEN_FIELD:
		if (ucRateIndex >= 9U)
			fgInvalidData = TRUE;
		else
	...
	case (UINT_8)PREAMBLE_VHT_FIELD:
		if (ucRateIndex >= 10U)
			fgInvalidData = TRUE;
		else
	...
	IN UINT_8   ucRateNss)
{
	...
	ucPhyRate = (UINT_8)RATE_CODE_GET_PHY_RATE(u2RateCode);
	u2TxMode = u2RateCode & (UINT_16)RATE_TX_MODE_MASK;
	ucRateNss = ucRateNss + (UINT_8)AR_SS_1; /* change to be base=1 */
	if ((u2TxMode == TX_MODE_HT_GF) || (u2TxMode == TX_MODE_HT_MM)) {
		if (ucPhyRate > PHY_RATE_MCS7)
			u2RateCode = u2RateCode - (UINT_8)HT_RATE_MCS7_INDEX;
		else
			ucRateNss = (UINT_8)AR_SS_1;
	...
	IN UINT_8   ucGI)
{
	...
	if ((ucBandwidth == (UINT_8)FIX_BW_NO_FIXED)
		|| (ucBandwidth == (UINT_8)FIX_BW_20))
		ucBw = (UINT_8)MAC_BW_20;
	else if (ucBandwidth == (UINT_8)FIX_BW_40)
		ucBw = (UINT_8)MAC_BW_40;
	else if (ucBandwidth == (UINT_8)FIX_BW_80)
		ucBw = (UINT_8)MAC_BW_80;
	else if (ucBandwidth == (UINT_8)FIX_BW_160)
		ucBw = (UINT_8)MAC_BW_160;
	else {
		;
	}
	ucPhyRate = (UINT_8)RATE_CODE_GET_PHY_RATE(u2RateCode);
	...
	if ((u2TxMode == TX_MODE_HT_GF) ||
		...
		u4PhyRateIn100Kbps = (nicGetHwRateByPhyRate(
			ucPhyRate & (UINT_8)BITS(0U, 3U)))*5U;
	} else {
		ASSERT(NULL);
	}
	return u4PhyRateIn100Kbps;
...
{
	UINT_32 i;
	UINT_32 u4CurPhyRate, u4TarPhyRate;
	UINT_32 u4HighestPhyRate = 0, u4LowestPhyRate = 0;
	UINT_8 ucRateIndex, ucRatePreamble, ucTarRateSwIndex, ucHighestPhyRateSwIdx, ucLowestPhyRateSwIdx;
	UINT_16 u2CurRateCode;
	UINT_32 u4Status;

	/* Set init value */
	if (fgGetLowest != FALSE) {
		u4TarPhyRate = 0xFFFFFFFFU;
		u4HighestPhyRate = 0;
		ucHighestPhyRateSwIdx = (UINT_8)RATE_NUM_SW;
	} else {
		u4TarPhyRate = 0U;
		u4LowestPhyRate = 0xFFFFFFFFU;
		ucLowestPhyRateSwIdx = (UINT_8)RATE_NUM_SW;
	}

	ucTarRateSwIndex = (UINT_8)RATE_NUM_SW;

	/* Find SW rate index by limitation */
	for (i = (UINT_8)RATE_1M_SW_INDEX;
			i <= (UINT_8)RATE_54M_SW_INDEX; i++) {
		if ((u2RateSet & (UINT_16)BIT(i)) != 0U) {

			/* Convert SW rate index to phy rate in 100kbps */
			nicSwIndex2RateIndex((UINT_8)i,
				&ucRateIndex, &ucRatePreamble);
	...
	if (ucTarRateSwIndex < (UINT_8)RATE_NUM_SW) {
