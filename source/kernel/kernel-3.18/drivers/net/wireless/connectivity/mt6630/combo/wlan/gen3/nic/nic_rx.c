...
********************************************************************************
*/
#define RX_RESPONSE_TIMEOUT (1000U)
...
VOID nicRxInitialize(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = CFG_RX_MAX_PKT_NUM; i != 0U; i--) {
	...
	ASSERT_BOOLEAN((UINT_32) (pucMemHandle - prRxCtrl->pucRxCached)
...
#else
	HAL_CFG_MAX_HIF_RX_LEN_NUM(prAdapter, 1U);
#endif
...
VOID nicRxFillRFB(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	u2RxStatusOffset = (UINT_16)sizeof(HW_MAC_RX_DESC_T);
	prSwRfb->ucGroupVLD = (UINT_8)HAL_RX_STATUS_GET_GROUP_VLD(prRxStatus);
	if ((prSwRfb->ucGroupVLD & (UINT_8)BIT((UINT_8)RX_GROUP_VLD_4)) != 0U) {
		prSwRfb->prRxStatusGroup4 = (P_HW_MAC_RX_STS_GROUP_4_T) ((P_UINT_8) prRxStatus + u2RxStatusOffset);
		u2RxStatusOffset += (UINT_16)sizeof(HW_MAC_RX_STS_GROUP_4_T);
	...
	if ((prSwRfb->ucGroupVLD & (UINT_8)BIT((UINT_8)RX_GROUP_VLD_1)) != 0U) {
		...
		u2RxStatusOffset += (UINT_16)sizeof(HW_MAC_RX_STS_GROUP_1_T);

	}
	if ((prSwRfb->ucGroupVLD & (UINT_8)BIT((UINT_8)RX_GROUP_VLD_2)) != 0U) {
		prSwRfb->prRxStatusGroup2 = (P_HW_MAC_RX_STS_GROUP_2_T) ((P_UINT_8) prRxStatus + u2RxStatusOffset);
		u2RxStatusOffset += (UINT_16)sizeof(HW_MAC_RX_STS_GROUP_2_T);

	}
	if ((prSwRfb->ucGroupVLD & (UINT_8)BIT((UINT_8)RX_GROUP_VLD_3)) != 0U) {
		prSwRfb->prRxStatusGroup3 = (P_HW_MAC_RX_STS_GROUP_3_T) ((P_UINT_8) prRxStatus + u2RxStatusOffset);
		u2RxStatusOffset += (UINT_16)sizeof(HW_MAC_RX_STS_GROUP_3_T);
	}
...
VOID nicRxFillChksumStatus(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb, IN UINT_32 u4TcpUdpIpCksStatus)
{
	...
	if (prAdapter->u4CSUMFlags != CSUM_NOT_SUPPORTED) {
		...
		if ((u4TcpUdpIpCksStatus
				& (UINT_32)RX_CS_TYPE_IPv4) != 0U) {
			prSwRfb->aeCSUM[CSUM_TYPE_IPV6] = CSUM_RES_NONE;
			/* IP packet csum failed */
			if ((u4TcpUdpIpCksStatus
				& (UINT_32)RX_CS_STATUS_IP) != 0U) {
...
P_SW_RFB_T incRxDefragMPDU(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSWRfb, OUT P_QUE_T prReturnedQue)
{
	...
	if ((u2FrameCtrl & (UINT_16)MASK_FC_MORE_FRAG) == 0U) {
		...
		if (prFragInfo->pr1stFrag != NULL) {
			...
			if (CHECK_FOR_EXPIRATION(rCurrentTime,
				...
				QUEUE_INSERT_TAIL(prReturnedQue,
					&prFragInfo->pr1stFrag->rQueEntry);
	...
	for (i = 0; i < MAX_NUM_CONCURRENT_FRAGMENTED_MSDUS; i++) {
		...
		if (fgFirst != FALSE) {	/* looking for timed-out frag buffer */
			...
			if (RXM_IS_QOS_DATA_FRAME(u2FrameCtrl) != FALSE) {
	...
	if (fgFirst != FALSE) {
		...
		DBGLOG(RX, LOUD,
			   ...
			   u2SeqCtrl, prFragInfo->u2SeqNo,
				prFragInfo->ucNextFragNo, prFragInfo->u8NextPN);
...
BOOLEAN nicRxIsDuplicateFrame(IN OUT P_SW_RFB_T prSwRfb)
{
	...
	BOOLEAN fgIsDuplicate = FALSE;
	UINT_8 fgIsAmsduSubframe = 0U;
	P_WLAN_MAC_HEADER_T prWlanHeader = NULL;
	P_HW_MAC_RX_DESC_T prRxStatus = NULL;
	P_HW_MAC_RX_STS_GROUP_4_T prRxStatusGroup4 = NULL;
	...
	if (RXM_IS_QOS_DATA_FRAME(u2FrameCtrl) != FALSE) {
		if (prSwRfb->prStaRec
			->aprRxReorderParamRefTbl[prSwRfb->ucTid] != NULL) {
		...
		if ((prSwRfb->prStaRec->ucDesiredPhyTypeSet
			& (UINT_8)(PHY_TYPE_BIT_HT
			| PHY_TYPE_BIT_VHT)) != 0U) {
			u4SeqCtrlCacheIdx = prSwRfb->ucTid;
		} else {
			if (prSwRfb->ucTid < 8U) {	/* UP = 0~7 */
				u4SeqCtrlCacheIdx =
				(UINT_32)aucTid2ACI[prSwRfb->ucTid];
			} else {
				DBGLOG(RX, WARN,
	...
	if ((u2FrameCtrl & (UINT_16)MASK_FC_RETRY) != 0U) {
...
VOID nicRxProcessPktWithoutReorder(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prSwRfb->prStaRec != NULL
		&& (prAdapter->rWifiVar.rWfdConfigureSettings.ucWfdEnable > 0U))
...
VOID nicRxProcessGOBroadcastPkt(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	ASSERT_BOOLEAN(CFG_NUM_OF_QM_RX_PKT_NUM >= 16U);

	if (prRxCtrl->rFreeSwRfbList.u4NumElem
	    >= (CFG_RX_MAX_PKT_NUM
	    - (CFG_NUM_OF_QM_RX_PKT_NUM
	    - 16U /* Reserved for others */))) {
		...
		if (prSwRfbDuplicated != NULL) {
			kalMemCopy(prSwRfbDuplicated->pucRecvBuff,
				   prSwRfb->pucRecvBuff,
				   ALIGN_4((UINT_64)(prRxStatus->u2RxByteCount)
				   + HIF_RX_HW_APPENDED_LEN));

			prSwRfbDuplicated->ucPacketType =
				(UINT_8)RX_PKT_TYPE_RX_DATA;
...
VOID nicRxFillRadiotapMCS(IN OUT P_MONITOR_RADIOTAP_T prMonitorRadiotap, IN P_HW_MAC_RX_STS_GROUP_3_T prRxStatusGroup3)
{
	...
	ucFrMode = (UINT_8)(((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_FR_MODE_MASK) >> RX_VT_FR_MODE_OFFSET);
	/* VHTA1 B0-B1 */
	ucShortGI = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_SHORT_GI) != 0U ? 1U : 0U;
	ucRxMode = (UINT_8)(((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_RX_MODE_MASK) >> RX_VT_RX_MODE_OFFSET);
	ucLDPC = ((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_LDPC) != 0U ? 1U : 0U;
	ucSTBC = (UINT_8)(((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_STBC_MASK) >> RX_VT_STBC_OFFSET);
	ucNess = (UINT_8)(((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_NESS_MASK) >> RX_VT_NESS_OFFSET);

	prMonitorRadiotap->ucMcsKnown = ((UINT_8)BITS(0U, 6U)
		| (((ucNess & (UINT_8)BIT(1U)) >> 1U) << 7U));
	...
	prMonitorRadiotap->ucMcsMcs = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_RX_RATE_MASK);
}
...
VOID nicRxFillRadiotapVHT(IN OUT P_MONITOR_RADIOTAP_T prMonitorRadiotap, IN P_HW_MAC_RX_STS_GROUP_3_T prRxStatusGroup3)
{
	...
	prMonitorRadiotap->u2VhtKnown = (UINT_16)BITS(0U, 8U);

	ucSTBC = (UINT_8)(((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_STBC_MASK) >> RX_VT_STBC_OFFSET);
	ucTxopPsNotAllow = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_TXOP_PS_NOT_ALLOWED) != 0U ? 1U : 0U;
	ucShortGI = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_SHORT_GI) != 0U ? 1U : 0U;
	ucNsym = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_SHORT_GI_NSYM) != 0U ? 1U : 0U;
	ucLDPC = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_LDPC) != 0U ? 1U : 0U;
	ucBeamFormed = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_BEAMFORMED) != 0U ? 1U : 0U;
	...
	case RX_VT_FR_MODE_20:
		prMonitorRadiotap->ucVhtBandwidth = 0U;
		break;
	case RX_VT_FR_MODE_40:
		prMonitorRadiotap->ucVhtBandwidth = 1U;
		break;
	case RX_VT_FR_MODE_80:
		prMonitorRadiotap->ucVhtBandwidth = 4U;
		break;
	case RX_VT_FR_MODE_160:
		prMonitorRadiotap->ucVhtBandwidth = 11U;
		break;
	default:
		prMonitorRadiotap->ucVhtBandwidth = 0U;
		break;
	...
	ucNsts = (UINT_8)(((prRxStatusGroup3)->u4RxVector[1]
		& (UINT_32)RX_VT_NSTS_MASK)	>> RX_VT_NSTS_OFFSET) + 1U;
	/* VHTA1 B10-B12 */

	/* Bit[3:0] for 802.11ac, mcs0 ~ mcs9 */
	ucMcs = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
		& (UINT_32)RX_VT_RX_RATE_AC_MASK);

	/* STBC = Nsts - Nss */
	prMonitorRadiotap->aucVhtMcsNss[0] =
	((ucMcs << 4U) | (ucNsts - ucSTBC));

	/* VHTA2 B2-B3 */
	prMonitorRadiotap->ucVhtCoding =
				(UINT_8)(((prRxStatusGroup3)->u4RxVector[0] &
				(UINT_32)RX_VT_CODING_MASK) >>
				RX_VT_CODING_OFFSET);
	/* VHTA1 B4-B9 */
	prMonitorRadiotap->ucVhtGroupId =
				(UINT_8)(((((prRxStatusGroup3)->u4RxVector[1] &
				(UINT_32)RX_VT_GROUPID_1_MASK)
				>> RX_VT_GROUPID_1_OFFSET)
				<< 2U) | (((prRxStatusGroup3)->u4RxVector[0]
				& (UINT_32)RX_VT_GROUPID_0_MASK) >>
				RX_VT_GROUPID_0_OFFSET));
	/* VHTA1 B13-B21 */
	prMonitorRadiotap->u2VhtPartialAid =
				(UINT_8)((((prRxStatusGroup3)->u4RxVector[2] &
				(UINT_32)RX_VT_AID_1_MASK) << 4U) |
				(((prRxStatusGroup3)->u4RxVector[1] &
				(UINT_32)RX_VT_AID_0_MASK) >>
				RX_VT_AID_0_OFFSET));
...
VOID nicRxProcessMonitorPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (((prSwRfb->ucGroupVLD & (UINT_8)BIT((UINT_8)RX_GROUP_VLD_2)) == 0U)
		|| ((prSwRfb->ucGroupVLD
		& (UINT_8)BIT((UINT_8)RX_GROUP_VLD_3)) == 0U)) {
		nicRxReturnRFB(prAdapter, prSwRfb);
		return;
	}
	...
	u4VendorNsLen = (UINT_32)sizeof(RADIOTAP_FIELD_VENDOR_T);
	...
	rRadiotapFieldVendor.u2DataLen = (UINT_16)u4VendorNsLen - 6U;
	/* VHTA1 B0-B1 */
	...
	if (HAL_RX_STATUS_IS_FRAG(prRxStatus) == TRUE)
		rMonitorRadiotap.ucFlags |= (UINT_32)BIT(3U);

	if (HAL_RX_STATUS_IS_FCS_ERROR(prRxStatus) == TRUE)
		rMonitorRadiotap.ucFlags |= (UINT_32)BIT(6U);

	/* Bit Number 2 RATE */
	if ((ucRxMode == RX_VT_LEGACY_CCK) || (ucRxMode == RX_VT_LEGACY_OFDM)) {
		/* Bit[2:0] for Legacy CCK, Bit[3:0] for Legacy OFDM */
		ucRxRate = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
			& (UINT_32)BITS(0U, 3U));
		rMonitorRadiotap.ucRate =
			(UINT_8)nicGetHwRateByPhyRate(ucRxRate);
	} else {
		ucMcs = (UINT_8)((prRxStatusGroup3)->u4RxVector[0]
			& (UINT_32)RX_VT_RX_RATE_AC_MASK);
		/* VHTA1 B0-B1 */
		...
		if (u4PhyRate > 1275U)
			rMonitorRadiotap.ucRate = 0xFFU;
		else
			rMonitorRadiotap.ucRate = (UINT_8)(u4PhyRate / 5U);
	}

	/* Bit Number 3 CHANNEL */
	if (ucRxMode == RX_VT_LEGACY_CCK)
		rMonitorRadiotap.u2ChFlags |= (UINT_16)BIT(5U);
	else			/* OFDM */
		rMonitorRadiotap.u2ChFlags |= (UINT_16)BIT(6U);

	ucChanNum = HAL_RX_STATUS_GET_CHNL_NUM(prRxStatus);
	if (HAL_RX_STATUS_GET_RF_BAND(prRxStatus) == BAND_2G4) {
		rMonitorRadiotap.u2ChFlags |= (UINT_16)BIT(7U);
		rMonitorRadiotap.u2ChFrequency =
			((UINT_16)ucChanNum * 5U + 2407U);
	} else {		/* BAND_5G */
		rMonitorRadiotap.u2ChFlags |= (UINT_16)BIT(8U);
		rMonitorRadiotap.u2ChFrequency =
			((UINT_16)ucChanNum * 5U + 5000U);
	}

	/* Bit Number 5 ANT SIGNAL */
	rMonitorRadiotap.ucAntennaSignal =
		(UINT_8)(((prRxStatusGroup3)->u4RxVector[3]
		& (UINT_32)RX_VT_IB_RSSI_MASK));

	/* Bit Number 6 ANT NOISE */
	rMonitorRadiotap.ucAntennaNoise =
		(UINT_8)((((prRxStatusGroup3)->u4RxVector[5]
		& (UINT_32)RX_VT_NF0_MASK) >> 1U) + 128U);

	/* Bit Number 11 ANT */
	rMonitorRadiotap.ucAntenna =
		((prRxStatusGroup3)->u4RxVector[2]
		& (UINT_32)RX_VT_SEL_ANT) != 0U ? 1U : 0U;

	/* Bit Number 19 MCS */
	if ((u4ItPresent & (UINT_32)RADIOTAP_FIELD_MCS) != 0U)
		nicRxFillRadiotapMCS(&rMonitorRadiotap, prRxStatusGroup3);

	/* Bit Number 20 AMPDU */
	if (HAL_RX_STATUS_IS_AMPDU_SUB_FRAME(prRxStatus) != FALSE) {
		if (HAL_RX_STATUS_GET_RXV_SEQ_NO(prRxStatus) != 0U)
			++prRxCtrl->u4AmpduRefNum;
		rMonitorRadiotap.u4AmpduRefNum = prRxCtrl->u4AmpduRefNum;
	}

	/* Bit Number 21 VHT */
	if ((u4ItPresent & (UINT_32)RADIOTAP_FIELD_VHT) != 0U)
...
VOID nicRxProcessDataPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	/* non BA session */
		...
		fgDrop = TRUE;
		if (HAL_RX_STATUS_IS_ICV_ERROR(prRxStatus) != TRUE
		...
		} else if (HAL_RX_STATUS_IS_LLC_MIS(prRxStatus) != FALSE) {
	...
	if (fgDrop == FALSE) {
		...
		u4TcpUdpIpCksStatus = *(pu4Temp
			+ (ALIGN_4((UINT_32)prRxStatus->u2RxByteCount) >> 2U));
	...
	while (prRetSwRfb != NULL) {
		/* save next first */
		prNextSwRfb =
		(P_SW_RFB_T) QUEUE_GET_NEXT_ENTRY(&prRetSwRfb->rQueEntry);
		...
		case RX_PKT_DESTINATION_HOST:
			...
			do {
				...
				uEapolKeyType =
					(UINT_8)secGetEapolKeyType(
					(PUINT_8)prRetSwRfb->pvHeader);
				if (uEapolKeyType != (UINT_32)EAPOL_KEY_3_OF_4)
					break;
				...
				if (ucBssIndex <= HW_BSSID_NUM) {
					secSetKeyCmdAction(
						prAdapter
						->aprBssInfo[ucBssIndex],
						(UINT_8)EAPOL_KEY_3_OF_4,
						ucKeyCmdAction);
					break;
					...
					secSetKeyCmdAction(
					prAdapter
					->aprBssInfo[prStaRec->ucBssIndex],
					(UINT_8)EAPOL_KEY_3_OF_4,
					ucKeyCmdAction);
					break;
				}
...
UINT_8 nicRxProcessGSCNEvent(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	case (UINT_8)EVENT_ID_GSCAN_SCAN_AVAILABLE:
		{
	...
	case (UINT_8)EVENT_ID_GSCAN_RESULT:
		{
	...
	case (UINT_8)EVENT_ID_GSCAN_CAPABILITY:
		{
			...
			mtk_cfg80211_vendor_get_gscan_capabilities(wiphy,
				prGlueInfo->prDevHandler->ieee80211_ptr,
				prEventGscnCapbiblity,
				(int)sizeof(EVENT_GSCAN_CAPABILITY_T));
		}
		break;

	case (UINT_8)EVENT_ID_GSCAN_SCAN_COMPLETE:
		{
	...
	case (UINT_8)EVENT_ID_GSCAN_FULL_RESULT:
		{
			UINT_32 ie_len = 0;
			...
			mtk_cfg80211_vendor_event_full_scan_results(wiphy,
					...
					(UINT_32)offsetof(
					PARAM_WIFI_GSCAN_FULL_RESULT,
					ie_data)
					+ ie_len);
		}
		break;

	case (UINT_8)EVENT_ID_GSCAN_SIGNIFICANT_CHANGE:
		{
	...
	case (UINT_8)EVENT_ID_GSCAN_GEOFENCE_FOUND:
		{
...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (prEvent->ucEID != (UINT_8)EVENT_ID_DEBUG_MSG)
	...
	case (UINT_8)EVENT_ID_MIC_ERR_INFO:
		{
	...
	case (UINT_8)EVENT_ID_SCAN_DONE:
		fgIsNewVersion = FALSE;
		if (prEvent->u2PacketLength >
				((UINT_16)(EVENT_HDR_SIZE)
				+ (UINT_16)sizeof(EVENT_SCAN_DONE)
				- SCAN_DONE_DIFFERENCE))
			fgIsNewVersion = TRUE;
		scnEventScanDone(prAdapter, (P_EVENT_SCAN_DONE) (prEvent->aucBuffer), fgIsNewVersion);
		break;

	case (UINT_8)EVENT_ID_NLO_DONE:
		...
		if (prAdapter->prAisBssInfo->fgIsNetRequestInActive != FALSE
			&& prAdapter->prAisBssInfo->fgIsPNOEnable != FALSE) {
	...
	case (UINT_8)EVENT_ID_SLEEPY_INFO:
		{
...
#if CFG_SUPPORT_MULTITHREAD
			if (prEventSleepyNotify->ucSleepyState != 0U)
				kalSetFwOwnEvent2Hif(prGlueInfo);
#endif
		}
		break;
	case (UINT_8)EVENT_ID_BT_OVER_WIFI:
#if CFG_ENABLE_BT_OVER_WIFI
		{
			...
			if (prEventBtOverWifi->ucLinkStatus == 0U) {
				/* Connection */
				prBowEvent->rHeader.ucEventId =
					(UINT_8)BOW_EVENT_ID_LINK_CONNECTED;
				prBowEvent->rHeader.ucSeqNumber = 0;
				prBowEvent->rHeader.u2PayloadLength =
					(UINT_16)sizeof(BOW_LINK_CONNECTED);
				...
				kalMemZero((void *)(
					prBowLinkConnected->aucPeerAddress),
					MAC_ADDR_LEN);
				...
				/* Disconnection */
				prBowEvent->rHeader.ucEventId =
					(UINT_8)BOW_EVENT_ID_LINK_DISCONNECTED;
				prBowEvent->rHeader.ucSeqNumber = 0;
				prBowEvent->rHeader.u2PayloadLength =
					(UINT_16)sizeof(BOW_LINK_DISCONNECTED);
		...
		break;
#endif
	case (UINT_8)EVENT_ID_STATISTICS:
		/* buffer statistics for further query */
		prAdapter->fgIsStatValid = TRUE;
		prAdapter->rStatUpdateTime = kalGetTimeTick();
		kalMemCopy(&prAdapter->rStatStruct,
			&prEvent->aucBuffer[0], sizeof(EVENT_STATISTICS));
	...
	case (UINT_8)EVENT_ID_CH_PRIVILEGE:
		cnmChMngrHandleChEvent(prAdapter, prEvent);
		break;

	case (UINT_8)EVENT_ID_BSS_ABSENCE_PRESENCE:
		qmHandleEventBssAbsencePresence(prAdapter, prEvent);
		break;

	case (UINT_8)EVENT_ID_STA_CHANGE_PS_MODE:
		qmHandleEventStaChangePsMode(prAdapter, prEvent);
		break;
#if CFG_ENABLE_WIFI_DIRECT
	case (UINT_8)EVENT_ID_STA_UPDATE_FREE_QUOTA:
		qmHandleEventStaUpdateFreeQuota(prAdapter, prEvent);
		break;
#endif
	case (UINT_8)EVENT_ID_BSS_BEACON_TIMEOUT:
		...
		break;
	case (UINT_8)EVENT_ID_UPDATE_NOA_PARAMS:
#if CFG_ENABLE_WIFI_DIRECT
	...
	case (UINT_8)EVENT_ID_STA_AGING_TIMEOUT:
#if CFG_ENABLE_WIFI_DIRECT
		{
	...
	case (UINT_8)EVENT_ID_AP_OBSS_STATUS:
#if CFG_ENABLE_WIFI_DIRECT
		if (prAdapter->fgIsP2PRegistered != FALSE)
	...
	case (UINT_8)EVENT_ID_ROAMING_STATUS:
		...
		break;
	case (UINT_8)EVENT_ID_SEND_DEAUTH:
#if DBG
		{
	...
	case (UINT_8)EVENT_ID_UPDATE_RDD_STATUS:
		{
	...
	case (UINT_8)EVENT_ID_UPDATE_BWCS_STATUS:
		{
	...
	case (UINT_8)EVENT_ID_UPDATE_BCM_DEBUG:
		{
			P_PTA_IPC_T prEventBwcsStatus;
		...
		break;
#endif
	case (UINT_8)EVENT_ID_ADD_PKEY_DONE:
		{
		...
		break;
	case (UINT_8)EVENT_ID_ICAP_DONE:
		{
			...
			wlanoidQueryMemDump(prAdapter, &rMemDumpInfo,
				(UINT_32)sizeof(rMemDumpInfo), &u4QueryInfo);

		}

		break;
	case (UINT_8)EVENT_ID_DEBUG_MSG:
		{
	...
	case (UINT_8)EVENT_ID_BATCH_RESULT:
	...
	case (UINT_8)EVENT_ID_RSSI_MONITOR:
		{
			INT_32 rssi = 0;

			kalMemCopy(&rssi,
				&prEvent->aucBuffer[0], sizeof(INT_32));
...
#if CFG_SUPPORT_TDLS
	case (UINT_8)EVENT_ID_TDLS:

		TdlsexEventHandle(prAdapter->prGlueInfo,
				  (UINT_8 *) prEvent->aucBuffer,
				  ((UINT_32)(prEvent->u2PacketLength) - 8U));
		break;
#endif /* CFG_SUPPORT_TDLS */

	case (UINT_8)EVENT_ID_DUMP_MEM:
		...
		if (prCmdInfo != NULL) {
			if (prCmdInfo->pfCmdDoneHandler != NULL)
				...
				prCmdInfo->fgSetQuery, 0U, WLAN_STATUS_SUCCESS);
			/* return prCmdInfo */
			cmdBufFreeCmdInfo(prAdapter, prCmdInfo);
		} else {
			/* Burst mode */
		...
		break;
	case (UINT_8)EVENT_ID_RSP_CHNL_UTILIZATION:
		cnmHandleChannelUtilization(prAdapter, (struct EVENT_RSP_CHNL_UTILIZATION *)prEvent->aucBuffer);
		break;
	case (UINT_8)EVENT_ID_UPDATE_FW_INFO:
		nicEventUpdateFwInfo(prAdapter, prEvent);
		break;
	case (UINT_8)EVENT_ID_ACCESS_REG:
	case (UINT_8)EVENT_ID_NIC_CAPABILITY:
		/* case EVENT_ID_MAC_MCAST_ADDR: */
	case (UINT_8)EVENT_ID_ACCESS_EEPROM:
	case (UINT_8)EVENT_ID_TEST_STATUS:
	default:
		prCmdInfo = nicGetPendingCmdInfo(prAdapter, prEvent->ucSeqNum);
	...
	if (prGlueInfo->prAdapter->fgIsChipNoAck != FALSE) {
	...
	if (prSwRfb->prRxStatusGroup3 == NULL) {
		...
		if (prSwRfb->pvHeader != NULL)
			DBGLOG_MEM8(RX, WARN, (PUINT_8) prSwRfb->pvHeader,
				prSwRfb->u2PacketLen > 32U
				? 32U:prSwRfb->u2PacketLen);
		nicRxReturnRFB(prAdapter, prSwRfb);
...
//#endif
	...
	ucSubtype = (*(PUINT_8) (prSwRfb->pvHeader) &
		(UINT_8)MASK_FC_SUBTYPE) >> OFFSET_OF_FC_SUBTYPE;
...
#endif
#if CFG_SUPPORT_802_11W
	if (HAL_RX_STATUS_IS_ICV_ERROR(prSwRfb->prRxStatus) != FALSE) {
...
#if CFG_MGMT_FRAME_HANDLING
		if (apfnProcessRxMgtFrame[ucSubtype] != NULL) {
...
static VOID nicRxCheckWakeupReason(P_SW_RFB_T prSwRfb)
{
	...
	case (UINT_8)RX_PKT_TYPE_RX_DATA:
	{
		...
		if (prSwRfb->ucGroupVLD & (UINT_8)BIT(RX_GROUP_VLD_4) != 0U)
			u2Temp += sizeof(HW_MAC_RX_STS_GROUP_4_T);
		if (prSwRfb->ucGroupVLD & (UINT_8)BIT(RX_GROUP_VLD_1) != 0U)
			u2Temp += sizeof(HW_MAC_RX_STS_GROUP_1_T);
		if (prSwRfb->ucGroupVLD & (UINT_8)BIT(RX_GROUP_VLD_2) != 0U)
			u2Temp += sizeof(HW_MAC_RX_STS_GROUP_2_T);
		if (prSwRfb->ucGroupVLD & (UINT_8)BIT(RX_GROUP_VLD_3) != 0U)
	...
	case (UINT_8)RX_PKT_TYPE_SW_DEFINED:
		...
		/* case HIF_RX_PKT_TYPE_MANAGEMENT: */
			...
			u4HeaderOffset = (UINT_32) (HAL_RX_STATUS_GET_HEADER_OFFSET(prRxStatus));
			if (prSwRfb->ucGroupVLD
				& (UINT_8)BIT(RX_GROUP_VLD_4) != 0U)
				u2Temp += sizeof(HW_MAC_RX_STS_GROUP_4_T);
			if (prSwRfb->ucGroupVLD
				& (UINT_8)BIT(RX_GROUP_VLD_1) != 0U)
				u2Temp += sizeof(HW_MAC_RX_STS_GROUP_1_T);
			if (prSwRfb->ucGroupVLD
				& (UINT_8)BIT(RX_GROUP_VLD_2) != 0U)
				u2Temp += sizeof(HW_MAC_RX_STS_GROUP_2_T);
			if (prSwRfb->ucGroupVLD
				& (UINT_8)BIT(RX_GROUP_VLD_3) != 0U)
				u2Temp += sizeof(HW_MAC_RX_STS_GROUP_3_T);
			pvHeader = (PUINT_8)prRxStatus + u2Temp + u4HeaderOffset;
			if (pvHeader == NULL) {
		...
		while (QUEUE_IS_NOT_EMPTY(&prRxCtrl->rReceivedRfbList)) {
			...
			while (QUEUE_IS_NOT_EMPTY(prTempRfbList)) {
				...
				if (prSwRfb == NULL)
					break;
#if CFG_SUPPORT_WAKEUP_REASON_DEBUG
				if (kalIsWakeupByWlan(prAdapter) != FALSE)
				...
				case (UINT_8)RX_PKT_TYPE_SW_DEFINED:
					...
					/* case HIF_RX_PKT_TYPE_MANAGEMENT: */
				case (UINT_8)RX_PKT_TYPE_TX_STATUS:
				case (UINT_8)RX_PKT_TYPE_RX_VECTOR:
				case (UINT_8)RX_PKT_TYPE_TM_REPORT:
				default:
...
WLAN_STATUS nicRxReadBuffer(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		u4PktLen = u4RegValue & (UINT_32)BITS(0U, 15U);
		if (u4PktLen != 0U) {
			rxNum = 0;
		} else {
			rxNum = 1;
		...
		u4ReadBytes = ALIGN_4(u4PktLen) + 4U;
		       ...
		       IN UINT_32 u4DataPort, IN UINT_16 u2RxLength, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	DBGLOG_MEM8(RX, TRACE, pucBuf,
		ALIGN_4((UINT_32)u2RxLength + HIF_RX_HW_APPENDED_LEN));
	return u4Status;
}
...
{
	UINT_32 i, rxNum;
	UINT_16 u2RxPktNum, u2Tmp, u2RxLength = 0;
...
VOID nicRxSDIOAggReceiveRFBs(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		...
		u4MaxLoopCount--;
		if (u4MaxLoopCount == 0U)
			break;
#endif

		if (prEnhDataStr->rRxInfo.u.u2NumValidRx0Len == 0U
			&& prEnhDataStr->rRxInfo.u.u2NumValidRx1Len == 0U)
			break;
...
restart:
			...
			for (i = 0; i < u4RxAggCount; i++) {
				...
				pucSrcAddr += ALIGN_4((UINT_32)u2PktLength
					+ HIF_RX_HW_APPENDED_LEN);
...
#endif
	} while ((prEnhDataStr->rRxInfo.u.u2NumValidRx0Len != 0U
	|| prEnhDataStr->rRxInfo.u.u2NumValidRx1Len != 0U)
		 && fgIsRxEnhanceMode != FALSE);
		  ...
		  IN UINT_8 ucPortIdx, OUT PUINT_8 pucRspBuffer, IN UINT_32 u4MaxRespBufferLen, OUT PUINT_32 pu4Length)
{
	...
	do {
		...
		if (ucPortIdx == 0U)
			u4PktLen = u4Value & 0xFFFFU;
		else
			u4PktLen = (u4Value >> 16U) & 0xFFFFU;
		...
		if (u4PktLen == 0U) {
			...
			HAL_PORT_RD(prAdapter,
				    ucPortIdx == 0U ? MCR_WRDR0 : MCR_WRDR1,
				    ALIGN_4(u4PktLen) + 4U,
				    prRxCtrl->pucRxCoalescingBufPtr, CFG_RX_COALESCING_BUFFER_SIZE);
...
WLAN_STATUS nicRxFlush(IN P_ADAPTER_T prAdapter)
{
	...
	if (prSwRfb != NULL) {
		do {
			...
			/* save next first */
			prNextSwRfb =
				(P_SW_RFB_T) QUEUE_GET_NEXT_ENTRY(
				&prSwRfb->rQueEntry);
 ...
 */
	...
	if (fgRobustAction != FALSE
		&& prSwRfb->prStaRec != NULL
		...
		if (prAisSpecBssInfo->fgMgmtProtection != FALSE
			&& ((prActFrame->u2FrameCtrl
			& MASK_FC_PROTECTED_FRAME) == 0U
...
#if CFG_SOFTAP_WPA3_NEED_SOFTAP_PMF
				else if ((prBssInfo->eNetworkType == NETWORK_TYPE_P2P) &&
