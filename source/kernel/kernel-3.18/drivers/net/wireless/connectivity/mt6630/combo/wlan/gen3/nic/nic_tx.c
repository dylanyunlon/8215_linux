...
static const TX_RESOURCE_CONTROL_T arTcResourceControl[TC_NUM] = {
	...
	/* First HW queue */
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC0_INDEX,
			(UINT_8)HIF_TX_AC0_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC1_INDEX,
			(UINT_8)HIF_TX_AC1_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC2_INDEX,
			(UINT_8)HIF_TX_AC2_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC3_INDEX,
			(UINT_8)HIF_TX_AC3_INDEX},
	{PORT_INDEX_MCU, (UINT_8)MCU_Q1_INDEX,
			(UINT_8)HIF_TX_CPU_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC4_INDEX,
			(UINT_8)HIF_TX_AC4_INDEX},

	/* Second HW queue */
#if NIC_TX_ENABLE_SECOND_HW_QUEUE
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC10_INDEX,
			(UINT_8)HIF_TX_AC10_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC11_INDEX,
			(UINT_8)HIF_TX_AC11_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC12_INDEX,
			(UINT_8)HIF_TX_AC12_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC13_INDEX,
			(UINT_8)HIF_TX_AC13_INDEX},
	{PORT_INDEX_LMAC, (UINT_8)MAC_TXQ_AC14_INDEX,
			(UINT_8)HIF_TX_AC14_INDEX},
#endif
};
...
VOID nicTxInitialize(IN P_ADAPTER_T prAdapter)
{
	...
	ASSERT_BOOLEAN(prTxCtrl->rFreeMsduInfoList.u4NumElem
		== CFG_TX_MAX_PKT_NUM);
	/* Check if the memory allocation consist with this initialization function */
	ASSERT_BOOLEAN(((UINT_32)(pucMemHandle - prTxCtrl->pucTxCached))
...
static BOOLEAN nicTxSanityCheckResource(IN P_ADAPTER_T prAdapter)
{
	P_TX_CTRL_T prTxCtrl;
	UINT_8 ucTC;
	...
	if ((prAdapter->rWifiVar.ucTxDbg & BIT(0U)) != 0U) {
		prTxCtrl = &prAdapter->rTxCtrl;

		prTxCtrl->u4TotalPageNum = 546;

		for (ucTC = (UINT_8)TC0_INDEX; ucTC < (UINT_8)TC_NUM; ucTC++) {
	...
	return (fgError == TRUE ? FALSE:TRUE);
}
...
WLAN_STATUS nicTxPollingResource(IN P_ADAPTER_T prAdapter, IN UINT_8 ucTC)
{
	...
	UINT_16 au4WTSR[HIF_TX_NUM] = {0};

	ASSERT(prAdapter);
	prTxCtrl = &prAdapter->rTxCtrl;

	if (ucTC >= (UINT_8)TC_NUM)
		return WLAN_STATUS_FAILURE;

	if (prTxCtrl->rTc.au2FreeBufferCount[ucTC] > 0U)
	...
	while (i-- > 0) {
		...
		if (kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE || fgIsBusAccessFailed == TRUE) {
			...
			break;
		} else if (nicTxReleaseResource(prAdapter, au4WTSR)
			!= FALSE) {
			if (prTxCtrl->rTc.au2FreeBufferCount[ucTC] > 0U) {
				u4Status = WLAN_STATUS_SUCCESS;
				break;
...
*/
/*----------------------------------------------------------------------------*/
static BOOLEAN nicTxCalculateResource(IN P_ADAPTER_T prAdapter,
		IN UINT_16 * au2TxRlsCnt, OUT UINT_16 * au2FreeTcResource)
{
	...
	fgEnExtraTxDone = (BOOLEAN)(prAdapter->rWifiVar.ucExtraTxDone);

	/* Get used page count */
	if (fgEnExtraTxDone != FALSE) {
		KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_RESOURCE);
		for (ucTcIdx = (UINT_8)TC0_INDEX;
			ucTcIdx < (UINT_8)TC_NUM; ucTcIdx++) {
	...
	for (ucTcIdx = (UINT_8)TC0_INDEX; ucTcIdx < (UINT_8)TC_NUM; ucTcIdx++) {
	...
	if (u2AvaliableCnt != 0U && u2TotalTxDoneCnt != 0U) {
	...
	if ((u2AvaliableCnt != 0U)
		&& (u2TotalExtraTxDone != 0U)
		&& (fgEnExtraTxDone != FALSE)) {
...
BOOLEAN nicTxReleaseResource(IN P_ADAPTER_T prAdapter, IN UINT_16 *au2TxRlsCnt)
{
	...
	if (nicTxCalculateResource(prAdapter,
			au2TxRlsCnt, au2FreeTcResource) != FALSE) {

		/* Return free Tc page count */
		KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_RESOURCE);
		for (i = (UINT_8)TC0_INDEX; i < (UINT_8)TC_NUM; i++) {
			...
			prTcqStatus->au2FreeBufferCount[i] =
				(UINT_16)u4BufferCountToBeFreed;

			if (au2FreeTcResource[i] != 0U) {
				DBGLOG(TX, TRACE,
...
#if (CFG_SUPPORT_TRACE_TC4 == 1)
		if (au2TxRlsCnt[TC4_INDEX] != 0U)
			wlanTraceReleaseTcRes(prAdapter, au2TxRlsCnt, prTcqStatus->au2FreeBufferCount[TC4_INDEX]);
#endif
		for (i = (UINT_8)TC0_INDEX; i < (UINT_8)TC_NUM; i++)
			prQM->au4QmTcResourceBackCounter[i] +=
				au2FreeTcResource[i];
		KAL_RELEASE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_RESOURCE);
		bStatus = TRUE;
	}

	if (nicTxSanityCheckResource(prAdapter) != TRUE)
...
WLAN_STATUS nicTxResetResource(IN P_ADAPTER_T prAdapter)
{
	...
	prTxCtrl->rTc.ucNextTcIdx = (UINT_8)TC0_INDEX;
	...
	for (ucIdx = (UINT_8)TC0_INDEX; ucIdx < (UINT_8)TC_NUM; ucIdx++) {
		/* Page Count */
		prTxCtrl->rTc.au2MaxNumOfPage[ucIdx] =
			(UINT_16)prAdapter->rWifiVar.au4TcPageCount[ucIdx];
...
UINT_8 nicTxGetFrameResourceType(IN UINT_8 eFrameType, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	switch (eFrameType) {
	case (UINT_8)FRAME_TYPE_802_1X:
		ucTC = (UINT_8)TC4_INDEX;
		break;

	case (UINT_8)FRAME_TYPE_MMPDU:
		if (prMsduInfo != NULL)
...
UINT_8 nicTxGetCmdResourceType(IN P_CMD_INFO_T prCmdInfo)
{
	...
	case COMMAND_TYPE_SECURITY_FRAME:
		ucTC = nicTxGetFrameResourceType((UINT_8)FRAME_TYPE_802_1X,
			NULL);
		break;

	case COMMAND_TYPE_MANAGEMENT_FRAME:
		ucTC = nicTxGetFrameResourceType((UINT_8)FRAME_TYPE_MMPDU,
...
WLAN_STATUS nicTxMsduInfoList(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfoListHead)
{
	...
	while (prMsduInfo != NULL) {
		...
		case (UINT_8)TC0_INDEX:
		case (UINT_8)TC1_INDEX:
		case (UINT_8)TC2_INDEX:
		case (UINT_8)TC3_INDEX:
		case (UINT_8)TC5_INDEX:	/* Broadcast/multicast data packets */
			QUEUE_GET_NEXT_ENTRY(&(prMsduInfo->rQueEntry)) = NULL;
			...
			ASSERT_BOOLEAN(status == WLAN_STATUS_SUCCESS);

			break;

		case (UINT_8)TC4_INDEX:	/* Management packets */
			QUEUE_GET_NEXT_ENTRY(&(prMsduInfo->rQueEntry)) = NULL;
	...
	if (prDataPort0->u4NumElem > 0U)
		nicTxMsduQueue(prAdapter, 0, prDataPort0);

	if (prDataPort1->u4NumElem > 0U)
...
WLAN_STATUS nicTxMsduInfoListMthread(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfoListHead)
{
	...
	if (prDataPort0->u4NumElem > 0U || prDataPort1->u4NumElem > 0U) {
...
UINT_32 nicTxMsduQueueMthread(IN P_ADAPTER_T prAdapter)
{
	...
	while (u4TxLoopCount != 0U) {
		...
		while (QUEUE_IS_NOT_EMPTY((&(prAdapter->rTxP0Queue)))) {
			...
			nicTxMsduQueue(prAdapter, 0U, prDataPort0);
		}
		...
		while (QUEUE_IS_NOT_EMPTY((&(prAdapter->rTxP1Queue)))) {
			...
			nicTxMsduQueue(prAdapter, 1U, prDataPort1);
		}
	}
		 ...
		 IN UINT_8 ucTxDescLength, IN BOOLEAN fgIsTemplate, OUT PUINT_8 prTxDescBuffer)
{
	...
	if (prMsduInfo->fgIs802_11 != FALSE) {
		...
		HAL_MAC_TX_DESC_SET_802_11_HEADER_LENGTH(prTxDesc,
			(prMsduInfo->ucMacHeaderLength >> 1U));
	} else {
	...
	/* Protection */
	if (secIsProtectedFrame(prAdapter, prMsduInfo, prStaRec) != FALSE) {
	...
	if (prMsduInfo->fgIs802_11 != FALSE) {
		...
		HAL_MAC_TX_DESC_SET_TYPE(prTxDesc,
			(prWlanHeader->u2FrameCtrl & MASK_FC_TYPE) >> 2U);
	...
	/* Remaining TX time */
	if ((prMsduInfo->u4Option & (UINT_32)MSDU_OPT_MANUAL_LIFE_TIME) == 0U)
	...
	if ((prMsduInfo->u4Option
			& (UINT_32)MSDU_OPT_MANUAL_RETRY_LIMIT) == 0U) {
	...
	case (UINT_8)MSDU_RATE_MODE_MANUAL_DESC:
		HAL_MAC_TX_DESC_SET_DW(prTxDesc,
			6U, 1U, &prMsduInfo->u4FixedRateOption);
		HAL_MAC_TX_DESC_SET_FIXED_RATE_MODE_TO_DESC(prTxDesc);
		HAL_MAC_TX_DESC_SET_FIXED_RATE_ENABLE(prTxDesc);
		break;

	case (UINT_8)MSDU_RATE_MODE_MANUAL_CR:
		HAL_MAC_TX_DESC_SET_FIXED_RATE_MODE_TO_CR(prTxDesc);
		HAL_MAC_TX_DESC_SET_FIXED_RATE_ENABLE(prTxDesc);
		break;

	case (UINT_8)MSDU_RATE_MODE_AUTO:
	default:
		break;
	}
			      ...
			      IN P_CMD_INFO_T prCmdInfo, OUT PUINT_8 prTxDescBuffer, OUT PUINT_8 pucTxDescLength)
{
	...
	P_BSS_INFO_T prBssInfo;
	UINT_8 ucTid = 0;
	UINT_8 ucTempTC = (UINT_8)TC4_INDEX;
	...
	if (GLUE_TEST_PKT_FLAG(prNativePacket, (UINT_8)ENUM_PKT_802_3) == 0U) {
		/* Set EthernetII */
		HAL_MAC_TX_DESC_SET_ETHERNET_II(prTxDesc);
	}
	/* Header Padding */
	...
	if (pucTxDescLength != NULL)
		*pucTxDescLength = ucTxDescAndPaddingLength;
}

static BOOLEAN nicTxIsTXDTemplateAllowd(IN P_ADAPTER_T prAdapter,
			IN P_MSDU_INFO_T prMsduInfo, IN P_STA_RECORD_T prStaRec)
{

	if (prMsduInfo->fgIsTXDTemplateValid != FALSE
		&& (prMsduInfo->ucControlFlag == 0U)
		&& prStaRec != NULL
...
*/
	...
	if (nicTxIsTXDTemplateAllowd(prAdapter,
			prMsduInfo, prStaRec) != FALSE) {
		prTxDescTemplate = prStaRec->aprTxDescTemplate[prMsduInfo->ucUserPriority];

		if (HAL_MAC_TX_DESC_IS_LONG_FORMAT(prTxDescTemplate) != 0U)
...
*/
	...
	if (prMsduInfo->fgIs802_11 != TRUE) {
		if (prMsduInfo->fgIs802_3 != FALSE)
			HAL_MAC_TX_DESC_UNSET_ETHERNET_II(prTxDesc);
		if (prMsduInfo->fgIsVlanExists != FALSE)
...
nicTxCopyDesc(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucTarTxDesc, IN PUINT_8 pucSrcTxDesc, OUT PUINT_8 pucTxDescLength)
{
	...
	if (HAL_MAC_TX_DESC_IS_LONG_FORMAT(
		(P_HW_MAC_TX_DESC_T) pucSrcTxDesc) != 0U)
		ucTxDescLength = NIC_TX_DESC_LONG_FORMAT_LENGTH;
	else
		ucTxDescLength = NIC_TX_DESC_SHORT_FORMAT_LENGTH;
...
WLAN_STATUS nicTxGenerateDescTemplate(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	prMsduInfo->ucRateMode = (UINT_8)MSDU_RATE_MODE_AUTO;
	prMsduInfo->ucBssIndex = prStaRec->ucBssIndex;
	prMsduInfo->ucPacketType = (UINT_8)TX_PACKET_TYPE_DATA;
...
VOID nicTxFreeDescTemplate(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	for (ucTid = 0; ucTid < TX_DESC_TID_NUM; ucTid++) {
		...
		if (prTxDesc != NULL) {
			...
			if (HAL_MAC_TX_DESC_IS_LONG_FORMAT(prTxDesc) != 0U)
...
WLAN_STATUS nicTxMsduQueue(IN P_ADAPTER_T prAdapter, UINT_8 ucPortIdx, P_QUE_T prQue)
{
	...
	if (prQue->u4NumElem > 0U) {
		...
		while (prMsduInfo != NULL) {
			...
			u4TotalLength += ((UINT_32)ucTxDescSize
		...
		ASSERT_BOOLEAN(u4TotalLength <= u4ValidBufSize);
...
WLAN_STATUS nicTxCmd(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN UINT_8 ucTC)
{
	...
	if (prCmdInfo->eCmdType == COMMAND_TYPE_SECURITY_FRAME) {
		...
		u2OverallLength = (UINT_16)ALIGN_4((UINT_32)ucTxDescLength
			+ NIC_TX_DESC_PADDING_LENGTH + prCmdInfo->u2InfoBufLen);

		prNativePacket = prMsduInfo->prPacket;
		if (prNativePacket != NULL)
		...
		ASSERT_BOOLEAN(prMsduInfo->fgIs802_11 == TRUE);
		ASSERT_BOOLEAN(prMsduInfo->eSrc == TX_PACKET_MGMT);
		...
		u2OverallLength = (UINT_16)ALIGN_4(
			(UINT_32)prCmdInfo->u2InfoBufLen);
		...
		ASSERT_BOOLEAN(u2OverallLength
		<= prAdapter->u4CoalescingBufCachedSize);

		if ((prWifiCmd->ucCID == (UINT_8)CMD_ID_SCAN_REQ) ||
			(prWifiCmd->ucCID == (UINT_8)CMD_ID_SCAN_CANCEL) ||
			(prWifiCmd->ucCID == (UINT_8)CMD_ID_SCAN_REQ_V2)) {
...
static VOID nicTxInterruptSanityCheck(IN P_ADAPTER_T prAdapter,
		IN UINT_16 * au2TxRlsCnt)
{
	UINT_8 ucIdx;
	BOOLEAN fgError = FALSE;

	if ((prAdapter->rWifiVar.ucTxDbg & BIT(1U)) != 0U) {
		for (ucIdx = (UINT_8)HIF_TX_AC0_INDEX;
			ucIdx < (UINT_8)HIF_TX_NUM; ucIdx++) {
...
VOID nicProcessTxInterrupt(IN P_ADAPTER_T prAdapter)
{
	...
	if (kalGetTxPendingCmdCount(prAdapter->prGlueInfo) > 0U
			|| wlanGetTxPendingFrameCount(prAdapter) > 0U)
...
BOOLEAN nicTxFillMsduInfo(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo, IN P_NATIVE_PACKET prPacket)
{
	...
	if (GLUE_IS_PKT_FLAG_SET(prPacket) != 0U) {
		prMsduInfo->fgIs802_1x =
			(BOOLEAN)GLUE_TEST_PKT_FLAG(prPacket,
			(UINT_8)ENUM_PKT_1X);
		...
		if (GLUE_TEST_PKT_FLAG(prPacket,
				(UINT_8)ENUM_PKT_DHCP) != 0U
			&& prAdapter->rWifiVar.ucDhcpTxDone != 0U) {
			prMsduInfo->pfTxDoneHandler = wlanDhcpTxDone;
			prMsduInfo->ucTxSeqNum = GLUE_GET_PKT_SEQ_NO(prPacket);
		} else if (GLUE_TEST_PKT_FLAG(prPacket,
				(UINT_8)ENUM_PKT_ARP) != 0U
			&& prAdapter->rWifiVar.ucArpTxDone != 0U) {
			prMsduInfo->pfTxDoneHandler = wlanArpTxDone;
			prMsduInfo->ucTxSeqNum = GLUE_GET_PKT_SEQ_NO(prPacket);
		} else if (GLUE_TEST_PKT_FLAG(prPacket,
				(UINT_8)ENUM_PKT_ICMP) != 0U
			&& prAdapter->rWifiVar.ucIcmpTxDone != 0U) {
			prMsduInfo->pfTxDoneHandler = wlanIcmpTxDone;
			prMsduInfo->ucTxSeqNum = GLUE_GET_PKT_SEQ_NO(prPacket);
		} else if (GLUE_TEST_PKT_FLAG(prPacket,
				(UINT_8)ENUM_PKT_TDLS) != 0U) {
			prMsduInfo->pfTxDoneHandler = wlanTdlsTxDone;
			prMsduInfo->ucTxSeqNum = GLUE_GET_PKT_SEQ_NO(prPacket);
		} else if (GLUE_TEST_PKT_FLAG(prPacket,
				(UINT_8)ENUM_PKT_DNS) != 0U) {
...
WLAN_STATUS nicTxAdjustTcq(IN P_ADAPTER_T prAdapter)
{
	...
	if (qmAdjustTcQuotas(prAdapter, &rTcqAdjust, &prTxCtrl->rTc) != FALSE) {

		KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_RESOURCE);

		for (u4Num = 0U; u4Num < (UINT_8)TC_NUM; u4Num++) {
			/* Page count */
			...
			ASSERT(prTxCtrl->rTc.au2FreeBufferCount[u4Num] >= 0U);
			ASSERT(prTxCtrl->rTc.au2MaxNumOfBuffer[u4Num] >= 0U);
		}
...
WLAN_STATUS nicTxInitCmd(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo)
{
	...
	prTxCtrl = &prAdapter->rTxCtrl;
	pucOutputBuf = prTxCtrl->pucTxCoalescingBufPtr;
	u2OverallLength = (UINT_16)ALIGN_4((UINT_32)prCmdInfo->u2InfoBufLen);

	/* <1> Copy CMD Header to command buffer (by using pucCoalescingBufCached) */
	kalMemCopy(pucOutputBuf, prCmdInfo->pucInfoBuffer, prCmdInfo->u2InfoBufLen);

	ASSERT_BOOLEAN(u2OverallLength <= prAdapter->u4CoalescingBufCachedSize);
...
WLAN_STATUS nicTxInitResetResource(IN P_ADAPTER_T prAdapter)
{
	...
	kalMemZero(prTxCtrl->rTc.au2PreUsedPageCount, sizeof(prTxCtrl->rTc.au2PreUsedPageCount));
	prTxCtrl->rTc.ucNextTcIdx = (UINT_8)TC0_INDEX;
	prTxCtrl->rTc.u2AvaliablePageCount = 0U;
...
static BOOLEAN nicTxProcessMngPacket(IN P_ADAPTER_T prAdapter,
		IN P_MSDU_INFO_T prMsduInfo)
{
	...
	if (prMsduInfo->u2FrameLength == 0U)
		return FALSE;

	if (prMsduInfo->ucMacHeaderLength == 0U)
		return FALSE;


	/* MMPDU: force stick to TC4 */
	prMsduInfo->ucTC = (UINT_8)TC4_INDEX;

	/* No Tx descriptor template for MMPDU */
	prMsduInfo->fgIsTXDTemplateValid = FALSE;

	/* Fixed Rate */
	if (prMsduInfo->ucRateMode == (UINT_8)MSDU_RATE_MODE_AUTO)
...
WLAN_STATUS nicTxEnqueueMsdu(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	while (prMsduInfo != NULL) {
		...
		if (prMsduInfo->eSrc == TX_PACKET_MGMT) {
			if (nicTxProcessMngPacket(prAdapter,
					prMsduInfo) != FALSE) {
				/* Valid MGMT */
				QUEUE_INSERT_TAIL(prDataPort1,
					&(prMsduInfo->rQueEntry));
	...
	if (prTxCtrl->i4TxMgmtPendingNum > 0
		|| kalGetTxPendingFrameCount(prAdapter->prGlueInfo) > 0U)
		kalSetEvent(prAdapter->prGlueInfo);
...
{
	if (nicTxGetFreeCmdCount(prAdapter) > (CFG_TX_MAX_CMD_PKT_NUM / 2U))
		return TRUE;
	else
		return FALSE;
}
...
UINT_8 nicTxGetPageCount(IN UINT_32 u4FrameLength, IN BOOLEAN fgIncludeDesc)
{
	...
	if (NIC_TX_PAGE_SIZE_IS_POWER_OF_2 != FALSE)
		ucPageCount = (UINT_8)((u4RequiredBufferSize
			+ (NIC_TX_PAGE_SIZE - 1U)) >>
		  ...
		  UINT_16 u2FrameLength, PFN_TX_DONE_HANDLER pfTxDoneHandler, UINT_8 ucRateMode)
{
	...
	prMsduInfo->ucPID = NIC_TX_DESC_PID_RESERVED;
	prMsduInfo->ucPacketType = (UINT_8)TX_PACKET_TYPE_MGMT;
	prMsduInfo->ucUserPriority = 0;
...
VOID nicTxFillDescByPktOption(P_MSDU_INFO_T prMsduInfo, P_HW_MAC_TX_DESC_T prTxDesc)
{
	...
	if ((u4PktOption & (UINT_32)MSDU_OPT_NO_ACK) != 0U)
		HAL_MAC_TX_DESC_SET_NO_ACK(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_PROTECTED_FRAME) != 0U) {
	...
	case HEADER_FORMAT_802_11_ENHANCE_MODE:
		if ((u4PktOption & (UINT_32)MSDU_OPT_EOSP) != 0U)
			HAL_MAC_TX_DESC_SET_EOSP(prTxDesc);

		if ((u4PktOption & (UINT_32)MSDU_OPT_AMSDU) != 0U)
	...
	case HEADER_FORMAT_NON_802_11:
		...
		if ((u4PktOption & (UINT_32)MSDU_OPT_MORE_DATA) != 0U)
			HAL_MAC_TX_DESC_SET_MORE_DATA(prTxDesc);

		if ((u4PktOption & (UINT_32)MSDU_OPT_REMOVE_VLAN) != 0U)
			HAL_MAC_TX_DESC_SET_REMOVE_VLAN(prTxDesc);
		break;

	case HEADER_FORMAT_802_11_NORMAL_MODE:
		if (fgProtected != FALSE && prMsduInfo->prPacket != NULL) {
			P_WLAN_MAC_HEADER_T prWlanHeader =
			    (P_WLAN_MAC_HEADER_T) ((ULONG) (prMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD);

			prWlanHeader->u2FrameCtrl |=
				(UINT_16)MASK_FC_PROTECTED_FRAME;
		}
		break;
	...
	if (fgIsLongFormat != TRUE)
		return;

	/* Fields in DW2~6 (Long Format) */
	if ((u4PktOption & (UINT_32)MSDU_OPT_NO_AGGREGATE) != 0U)
		HAL_MAC_TX_DESC_SET_BA_DISABLE(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_TIMING_MEASURE) != 0U)
		HAL_MAC_TX_DESC_SET_TIMING_MEASUREMENT(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_NDP) != 0U)
		HAL_MAC_TX_DESC_SET_NDP(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_NDPA) != 0U)
		HAL_MAC_TX_DESC_SET_NDPA(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_SOUNDING) != 0U)
		HAL_MAC_TX_DESC_SET_SOUNDING_FRAME(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_FORCE_RTS) != 0U)
		HAL_MAC_TX_DESC_SET_FORCE_RTS_CTS(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_BIP) != 0U)
		HAL_MAC_TX_DESC_SET_BIP(prTxDesc);

	/* SW field */
	if ((u4PktOption & (UINT_32)MSDU_OPT_SW_DURATION) != 0U)
		HAL_MAC_TX_DESC_SET_DURATION_CONTROL_BY_SW(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_SW_PS_BIT) != 0U)
		HAL_MAC_TX_DESC_SET_SW_PM_CONTROL(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_SW_HTC) != 0U)
		HAL_MAC_TX_DESC_SET_HTC_EXIST(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_SW_BAR_SN) != 0U)
		HAL_MAC_TX_DESC_SET_SW_BAR_SSN(prTxDesc);

	if ((u4PktOption & (UINT_32)MSDU_OPT_MANUAL_SN) != 0U) {
...
VOID nicTxFillDescByPktControl(P_MSDU_INFO_T prMsduInfo, P_HW_MAC_TX_DESC_T prTxDesc)
{
	...
	if (HAL_MAC_TX_DESC_IS_LONG_FORMAT(prTxDesc) != 0U) {
		...
		if ((ucPktControl & (UINT_8)MSDU_CONTROL_FLAG_FORCE_TX) != 0U)
			ucSwReserved |= (UINT_8)MSDU_CONTROL_FLAG_FORCE_TX;
...
VOID nicTxSetPktMacTxQue(P_MSDU_INFO_T prMsduInfo, UINT_8 ucMacTxQue)
{
	...
	if (ucTcIdx < (UINT_8)TC_NUM) {
		prMsduInfo->ucTC = ucTcIdx;
		prMsduInfo->u4Option |= (UINT_32)MSDU_OPT_MANUAL_TX_QUE;
	}
}
			       ...
			       BOOLEAN fgItxBeamforming, UINT_8 ucAntennaIndex, UINT_8 ucAntennaPriority)
{
	...
	HAL_MAC_TX_DESC_GET_DW(prTxDesc,
		6U, 1U, &prMsduInfo->u4FixedRateOption);
...
VOID nicTxSetPktLowestFixedRate(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	prMsduInfo->fgIsTXDTemplateValid = FALSE;
	prMsduInfo->ucRateMode = (UINT_8)MSDU_RATE_MODE_MANUAL_DESC;
	if (prStaRec != NULL) {
	...
	if (prBssInfo->u4CoexPhyRateLimit != 0U) {
		u4CurrentPhyRate = nicRateCode2PhyRate(u2RateCode,
			(UINT_8)FIX_BW_NO_FIXED,
			(UINT_8)MAC_GI_NORMAL,
			(UINT_8)AR_SS_NULL);
	...
	nicTxSetPktFixedRateOption(prMsduInfo,
		u2RateCode,
		(UINT_8)FIX_BW_NO_FIXED, FALSE, FALSE);
}
...
VOID nicTxSetPktMoreData(P_MSDU_INFO_T prCurrentMsduInfo, BOOLEAN fgSetMoreDataBit)
{
	...
	if (prCurrentMsduInfo->fgIs802_11 != FALSE) {
		prWlanMacHeader =
		    (P_WLAN_MAC_HEADER_T) (((PUINT_8) (prCurrentMsduInfo->prPacket)) + MAC_TX_RESERVED_FIELD);
	}

	if (fgSetMoreDataBit != FALSE) {
		if (prCurrentMsduInfo->fgIs802_11 != TRUE)
			prCurrentMsduInfo->u4Option |=
				(UINT_32)MSDU_OPT_MORE_DATA;
		else
			prWlanMacHeader->u2FrameCtrl |=
				(UINT_16)MASK_FC_MORE_DATA;
	} else {
		if (prCurrentMsduInfo->fgIs802_11 != TRUE)
			prCurrentMsduInfo->u4Option &=
				~((UINT_32)MSDU_OPT_MORE_DATA);
...
VOID nicTxSetPktEOSP(P_MSDU_INFO_T prCurrentMsduInfo, BOOLEAN fgSetEOSPBit)
{
	...
	if (fgSetEOSPBit != FALSE) {
		if (fgWriteToDesc != FALSE)
			prCurrentMsduInfo->u4Option |= (UINT_32)MSDU_OPT_EOSP;
		else
			prWlanMacHeader->u2QosCtrl |= (UINT_16)MASK_QC_EOSP;
	} else {
		if (fgWriteToDesc != FALSE)
			prCurrentMsduInfo->u4Option &= ~(UINT_32)MSDU_OPT_EOSP;
		else
...
VOID nicTxUpdateBssDefaultRate(P_BSS_INFO_T prBssInfo)
{
	...
	if (rateGetLowestRateIndexFromRateSet(prBssInfo->u2BSSBasicRateSet,
		&ucLowestBasicRateIndex) != FALSE) {
		nicRateIndex2RateCode((UINT_8)PREAMBLE_DEFAULT_LONG_NONE,
			ucLowestBasicRateIndex,
			&prBssInfo->u2HwDefaultFixedRateCode);
	} else {
		switch (prBssInfo->ucNonHTBasicPhyType) {
		case (UINT_8)PHY_TYPE_ERP_INDEX:
		case (UINT_8)PHY_TYPE_OFDM_INDEX:
			prBssInfo->u2HwDefaultFixedRateCode = RATE_OFDM_6M;
			break;
