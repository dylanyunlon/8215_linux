		  ...
		  IN P_EVENT_P2P_DEV_DISCOVER_RESULT_T prP2pResult, IN PUINT_8 pucRxIEBuf, IN UINT_16 u2RxIELength)
{
	...
	for (u4Idx = 0; u4Idx < prP2pInfo->u4DeviceNum; u4Idx++) {
		...
		if (EQUAL_MAC_ADDR(&prTargetResult->aucDeviceAddr[0],
			&prP2pResult->aucDeviceAddr[0])) {
			bUpdate = TRUE;
			...
			/* then buffer */
			kalMemCopy(prTargetResult, prP2pResult,
				sizeof(EVENT_P2P_DEV_DISCOVER_RESULT_T));

			/* See if new IE length is longer or not. */
			if ((u2RxIELength > u2IELength) && (u2IELength != 0U)) {
				/* Buffer is not enough. */
				u2RxIELength = u2IELength;
			} else if ((u2IELength == 0U) && (u2RxIELength != 0U)) {
				/* RX new IE buf. */
				ASSERT(pucIeBuf);
	...
	if (bUpdate != TRUE) {
		 ...
		 */
		if ((u4Idx < (UINT_32) CFG_MAX_NUM_BSS_LIST)
			&& (UNEQUAL_MAC_ADDR(&zeroMac[0],
			&prP2pResult->aucDeviceAddr[0]))) {
			prTargetResult = &prP2pInfo->arP2pDiscoverResult[u4Idx];
			...
			if (u2RxIELength != 0U) {
				...
				kalMemCopy((PUINT_8) prTargetResult->pucIeBuf,
					(PUINT_8) pucRxIEBuf,
					(UINT_32) u2IELength);
				prTargetResult->u2IELength = u2IELength;
			} else {
