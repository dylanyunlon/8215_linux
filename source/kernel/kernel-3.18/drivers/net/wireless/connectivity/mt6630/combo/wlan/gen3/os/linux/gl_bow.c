...
BOOLEAN glRegisterAmpc(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if (prGlueInfo->rBowInfo.fgIsRegistered == TRUE)
...
#else
		prGlueInfo->rBowInfo.u4DeviceNumber = MKDEV(226UL, 0U);
#endif
		...
		if ((kfifo_alloc((struct kfifo *)&(prGlueInfo->rBowInfo.rKfifo),
			GLUE_BOW_KFIFO_DEPTH, GFP_KERNEL)) != 0)
		...
		if (cdev_add(&(prGlueInfo->rBowInfo.cdev),
			prGlueInfo->rBowInfo.u4DeviceNumber, 1) != 0)
			goto fail_cdev_add;
...
#if CFG_BOW_SEPARATE_DATA_PATH
	uninit_status = kalUninitBowDevice(prGlueInfo);
#endif
...
static ssize_t bow_ampc_read(IN struct file *filp, IN char __user *buf, IN size_t size, IN OUT loff_t *ppos)
{
	...
	if ((prGlueInfo->rBowInfo.fgIsRegistered == FALSE) ||
		(prGlueInfo->ulFlag & (ULONG)GLUE_FLAG_HALT) != 0U)
		return -EFAULT;
	/* size check */
/* if(kfifo_len(prGlueInfo->rBowInfo.prKfifo) >= size) */
	if (kfifo_len(&(prGlueInfo->rBowInfo.rKfifo)) >= size)
		...
		retval = (ssize_t)kfifo_len(&(prGlueInfo->rBowInfo.rKfifo));
...
static ssize_t bow_ampc_write(IN struct file *filp, OUT const char __user *buf, IN size_t size, IN OUT loff_t *ppos)
{
	...
	if (size > (size_t)MAX_BUFFER_SIZE)
		return -EINVAL;
	else if (copy_from_user(aucBuffer, buf, size) != 0U)
		return -EIO;
	else
		DBGLOG(BOW, EVENT, "AMP driver CMD buffer size : %zu.\n", size);

	for (i = 0; i < (UINT_8)MAX_BUFFER_SIZE; i++)
		DBGLOG(BOW, EVENT, "AMP write content : 0x%x.\n", aucBuffer[i]);

	DBGLOG(BOW, EVENT, "BoW CMD write.\n");

	prCmd = (P_AMPC_COMMAND) (&aucBuffer[0]);
...
static long bow_ampc_ioctl(IN struct file *filp, IN unsigned int cmd, IN OUT unsigned long arg)
{
	...
	if ((prGlueInfo->rBowInfo.fgIsRegistered == FALSE) ||
		(prGlueInfo->ulFlag & GLUE_FLAG_HALT) != 0U)
		return -EFAULT;
	/* permission check */
	if ((_IOC_DIR(cmd) & _IOC_READ) != 0U) {
...
static unsigned int bow_ampc_poll(IN struct file *filp, IN poll_table (*wait))
{
	...
	retval = ((UINT_8)POLLOUT | (UINT_8)POLLWRNORM);

/* DBGLOG(BOW, EVENT, ("bow_ampc_pol, POLLOUT | POLLWRNORM, %x\n", retval)); */

/* if(kfifo_len(prGlueInfo->rBowInfo.prKfifo) > 0) */
	if (kfifo_len(&(prGlueInfo->rBowInfo.rKfifo)) > 0U) {
		retval |= ((UINT_8)POLLIN | (UINT_8)POLLRDNORM);
...
VOID kalIndicateBOWEvent(IN P_GLUE_INFO_T prGlueInfo, IN P_AMPC_EVENT prEvent)
{
	...
	/* check device */
	if ((prGlueInfo->rBowInfo.fgIsRegistered == FALSE) ||
		(prGlueInfo->ulFlag & GLUE_FLAG_HALT) != 0UL)
		return;
	...
	u4AvailSize = (size_t)GLUE_BOW_KFIFO_DEPTH -
		kfifo_len(&(prGlueInfo->rBowInfo.rKfifo));

	u4EventSize = (size_t)prEvent->rHeader.u2PayloadLength +
		sizeof(AMPC_EVENT_HEADER_T);
...
ENUM_BOW_DEVICE_STATE kalGetBowState(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 aucPeerAddress[6])
{
	...
	for (i = 0; i < (UINT_8)CFG_BOW_PHYSICAL_LINK_NUM; i++) {
		if (EQUAL_MAC_ADDR(&prGlueInfo->rBowInfo.arPeerAddr[0],
			&aucPeerAddress[0])) {
			DBGLOG(BOW, EVENT,
...
/* Henry, can reduce this logic to indentify state change */
	...
	for (i = 0; i < (UINT_32)CFG_BOW_PHYSICAL_LINK_NUM; i++) {
...
UINT_8 kalGetBowRole(IN P_GLUE_INFO_T prGlueInfo, IN PARAM_MAC_ADDRESS rPeerAddr)
{
	...
	for (i = 0; i < (UINT_32)CFG_BOW_PHYSICAL_LINK_NUM; i++) {
		if (EQUAL_MAC_ADDR(&(prGlueInfo->rBowInfo.arPeerAddr[i][0]),
			&rPeerAddr[0]))
...
BOOLEAN kalBowFrameClassifier(IN P_GLUE_INFO_T prGlueInfo, IN P_NATIVE_PACKET prPacket, OUT PBOOLEAN pfgIs1X)
{
	...
	UINT_8 ucEthTypeLenOffset = (UINT_8)ETHER_HEADER_LEN -
		(UINT_8)ETHER_TYPE_LEN;
	PUINT_8 pucNextProtocol = NULL;
	UINT_8 aucLLC[] = ETH_LLC;
	UINT_8 aucSnapBtOui[] = ETH_SNAP_BT_SIG_OUI;
	UINT_8 ucMinLength = ucEthTypeLenOffset + (UINT_8)ETHER_TYPE_LEN +
		(UINT_8)ETH_LLC_LEN + (UINT_8)ETH_SNAP_LEN;

	DEBUGFUNC("kalQoSFrameClassifierAndPacketInfo");

	u4PacketLen = (UINT_32)prSkb->len;
	...
	aucLookAheadBuf = (PUINT_8)prSkb->data;
	...
	*pfgIs1X = FALSE;
	...
	if (u2EtherTypeLen > (UINT_16)ETH_802_3_MAX_LEN) {
		/* Not BoW frame */
		return FALSE;
	}
	...
	if (u4PacketLen >= ucMinLength &&
	    kalMemCmp(&pucNextProtocol[0], &aucLLC[0], ETH_LLC_LEN) == 0 &&
	    kalMemCmp(&pucNextProtocol[ETH_LLC_LEN], &aucSnapBtOui[0],
		ETH_SNAP_OUI_LEN) == 0) {
		UINT_16 u2LocalCode;

		WLAN_GET_FIELD_BE16(pucNextProtocol + ETH_LLC_LEN + ETH_SNAP_OUI_LEN, &u2LocalCode);

		if (u2LocalCode == (UINT_16)BOW_PROTOCOL_ID_SECURITY_FRAME)
			*pfgIs1X = TRUE;
...
static int bowHardStartXmit(IN struct sk_buff *prSkb, IN struct net_device *prDev)
{
	...
	if (kalBowFrameClassifier(prGlueInfo,
		(P_NATIVE_PACKET) prSkb, &fgIs1x) == FALSE) {
	...
	if (fgIs1x == TRUE)
		GLUE_SET_PKT_FLAG(prSkb, (UINT_16)ENUM_PKT_1X);
...
BOOLEAN kalInitBowDevice(IN P_GLUE_INFO_T prGlueInfo, IN const char *prDevName)
{
	...
	ASSERT(prGlueInfo);
	ASSERT_BOOLEAN(prGlueInfo->rBowInfo.fgIsRegistered == TRUE);
	...
	if (prGlueInfo->rBowInfo.fgIsNetRegistered == FALSE) {
		prGlueInfo->rBowInfo.prDevHandler =
		    alloc_netdev_mq((INT_32)sizeof(P_GLUE_INFO_T), prDevName,
		    NET_NAME_PREDICTABLE, ether_setup, CFG_MAX_TXQ_NUM);

		if (prGlueInfo->rBowInfo.prDevHandler == NULL)
			return FALSE;
		...
		COPY_MAC_ADDR(&rMacAddr[0], &prAdapter->rMyMacAddr[0]);
