...
WLAN_STATUS kalFirmwareOpen(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	UINT_8 ucMaxEcoVer = ((UINT_8)sizeof(appucFwNameTable) /
		(UINT_8)sizeof(PPUINT_8));
	...
	if (ucMaxEcoVer < ucCurEcoVer)
		...
		apucNameTable = appucFwNameTable[ucCurEcoVer - 1U];

	/* Try to open FW binary */
	for (ucPathIdx = 0U; apucFwPath[ucPathIdx] != NULL; ucPathIdx++) {
		for (ucNameIdx = 0; apucNameTable[ucNameIdx] != NULL;
			ucNameIdx++) {
			...
			filp_FW_dl = filp_open(aucFwName, O_RDONLY, 0);
			if (IS_ERR(filp_FW_dl)) {
				DBGLOG(INIT, TRACE, "Open FW image %s failed, filp[%p]\n",
				       aucFwName, filp_FW_dl);
				continue;
			} else {
...
WLAN_STATUS kalFirmwareClose(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if ((filp_FW_dl != NULL) && !IS_ERR(filp_FW_dl)) {
		/* close firmware file */
		if (filp_close(filp_FW_dl, NULL) < 0)
			DBGLOG(INIT, ERROR, "kalFirmwareClose Fail\n");
		/* restore */
		set_fs(orgfs);
		{
			struct cred *cred = (struct cred *)get_current_cred();
...
WLAN_STATUS kalFirmwareLoad(IN P_GLUE_INFO_T prGlueInfo, OUT PVOID prBuf, IN UINT_32 u4Offset, OUT PUINT_32 pu4Size)
{
	...
	if ((filp_FW_dl == NULL) || IS_ERR(filp_FW_dl) ||
		(filp_FW_dl->f_op == NULL)) {
		goto error_read;
	} else {
		filp_FW_dl->f_pos = (INT_64)u4Offset;
		*pu4Size = (UINT_32)__vfs_read(filp_FW_dl,
			(__force void __user *)prBuf,
			*pu4Size, &filp_FW_dl->f_pos);
	}
...
WLAN_STATUS kalFirmwareSize(IN P_GLUE_INFO_T prGlueInfo, OUT PUINT_32 pu4Size)
{
	...
	*pu4Size = (UINT_32)(filp_FW_dl->f_path.dentry->d_inode->i_size);
...
PVOID kalFirmwareImageMapping(IN P_GLUE_INFO_T prGlueInfo, OUT PPVOID ppvMapFileBuf, OUT PUINT_32 pu4FileLength)
{
	...
	do {
		...
		if (kalFirmwareSize(prGlueInfo, &u4FwSize) !=
			WLAN_STATUS_SUCCESS)
			DEBUGFUNC("kalFirmwareImageMapping Size Fail");
		/* <3> Use vmalloc for allocating large memory trunk */
		prFwBuffer = vmalloc(ALIGN_4((UINT_64)u4FwSize));
		/* <4> Load image binary into buffer */
		if (kalFirmwareLoad(prGlueInfo, prFwBuffer, 0, &u4FwSize) != WLAN_STATUS_SUCCESS) {
			vfree(prFwBuffer);
			if (kalFirmwareClose(prGlueInfo) !=
				WLAN_STATUS_SUCCESS)
				DEBUGFUNC("kalFirmwareImageMapping Close Fail");
			break;
		}
		/* <5> write back info */
...
VOID kalFirmwareImageUnmapping(IN P_GLUE_INFO_T prGlueInfo, IN PVOID prFwHandle, IN PVOID pvMapFileBuf)
{
	...
	if (kalFirmwareClose(prGlueInfo) != WLAN_STATUS_SUCCESS)
		DEBUGFUNC("UnMapping close Fail");
}
...
VOID kalAcquireMutex(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_MUTEX_CATEGORY_E rMutexCategory)
{
	...
	if (rMutexCategory < MUTEX_NUM) {
		...
		prGlueInfo->arMutexOwner[rMutexCategory] = current;
		prGlueInfo->arMutexCount[rMutexCategory] = 1;
		DBGLOG(INIT, TRACE, "MUTEX_LOCK[%u] Acquired\n", rMutexCategory);
	}

}				/* end of kalAcquireMutex() */

/* nested version for the same thread */
VOID kalAcquireMutex2(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_MUTEX_CATEGORY_E rMutexCategory)
{
	ASSERT(prGlueInfo);

	if (rMutexCategory < MUTEX_NUM) {
		DBGLOG(INIT, TRACE, "MUTEX_LOCK[%u] Try to acquire\n", rMutexCategory);
		if (mutex_is_locked(&prGlueInfo->arMutex[rMutexCategory])
				&& prGlueInfo->arMutexOwner[rMutexCategory] == current) {
			prGlueInfo->arMutexCount[rMutexCategory]++;
		} else {
			mutex_lock(&prGlueInfo->arMutex[rMutexCategory]);
			prGlueInfo->arMutexOwner[rMutexCategory] = current;
			prGlueInfo->arMutexCount[rMutexCategory] = 1;
...
VOID kalReleaseMutex(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_MUTEX_CATEGORY_E rMutexCategory)
{
	...
	if (rMutexCategory < MUTEX_NUM) {
		...
		prGlueInfo->arMutexOwner[rMutexCategory] = NULL;
		prGlueInfo->arMutexCount[rMutexCategory] = 0;
		DBGLOG(INIT, TRACE, "MUTEX_UNLOCK[%u]\n", rMutexCategory);
	}

}				/* end of kalReleaseMutex() */

VOID kalReleaseMutex2(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_MUTEX_CATEGORY_E rMutexCategory)
{
	ASSERT(prGlueInfo);

	if (rMutexCategory < MUTEX_NUM) {
		if (0 == --prGlueInfo->arMutexCount[rMutexCategory]) {
			prGlueInfo->arMutexOwner[rMutexCategory] = NULL;
			mutex_unlock(&prGlueInfo->arMutex[rMutexCategory]);
			DBGLOG(INIT, TRACE, "MUTEX_UNLOCK[%u]\n", rMutexCategory);
		}
...
VOID kalUpdateMACAddress(IN P_GLUE_INFO_T prGlueInfo, IN PUINT_8 pucMacAddr)
{
	...
	if (UNEQUAL_MAC_ADDR((PUINT_8)(&prGlueInfo->prDevHandler->dev_addr[0]),
		pucMacAddr))
		memcpy(&prGlueInfo->prDevHandler->dev_addr[0],
...
VOID kalQueryTxChksumOffloadParam(IN PVOID pvPacket, OUT PUINT_8 pucFlag)
{
	...
	if (skb->ip_summed == (UINT_8)CHECKSUM_PARTIAL) {
#if DBG
		/*
...
#endif
			ucFlag |= ((UINT_8)TX_CS_IP_GEN |
				(UINT_8)TX_CS_TCP_UDP_GEN);
	}
...
VOID kalUpdateRxCSUMOffloadParam(IN PVOID pvPacket, IN ENUM_CSUM_RESULT_T aeCSUM[])
{
	...
	if ((aeCSUM[(UINT_32)CSUM_TYPE_IPV4] == CSUM_RES_SUCCESS ||
		aeCSUM[(UINT_32)CSUM_TYPE_IPV6] == CSUM_RES_SUCCESS) &&
		((aeCSUM[(UINT_32)CSUM_TYPE_TCP] == CSUM_RES_SUCCESS)
		|| (aeCSUM[(UINT_32)CSUM_TYPE_UDP] == CSUM_RES_SUCCESS))) {
		skb->ip_summed = CHECKSUM_UNNECESSARY;
	} else {
		skb->ip_summed = CHECKSUM_NONE;
#if DBG
		if (aeCSUM[(UINT_32)CSUM_TYPE_IPV4] == CSUM_RES_NONE &&
			aeCSUM[(UINT_32)CSUM_TYPE_IPV6] == CSUM_RES_NONE)
			DBGLOG(RX, TRACE, "RX: \"non-IPv4/IPv6\" Packet\n");
		else if (aeCSUM[(UINT_32)CSUM_TYPE_IPV4] == CSUM_RES_FAILED)
			DBGLOG(RX, TRACE, "RX: \"bad IP Checksum\" Packet\n");
		else if (aeCSUM[(UINT_32)CSUM_TYPE_TCP] == CSUM_RES_FAILED)
			DBGLOG(RX, TRACE, "RX: \"bad TCP Checksum\" Packet\n");
		else if (aeCSUM[(UINT_32)CSUM_TYPE_UDP] == CSUM_RES_FAILED)
			DBGLOG(RX, TRACE, "RX: \"bad UDP Checksum\" Packet\n");
		else
			DBGLOG(RX, TRACE, "RX:Packet\n");
		   ...
		   IN BOOLEAN fgIsRetain, IN ENUM_CSUM_RESULT_T aerCSUM[])
{
	...
	/* Put data */
	if (skb_put(skb, u4PacketLen) == NULL)
		DBGLOG(RX, WARN, "skb_put fail\n");
...
WLAN_STATUS kalRxIndicatePkts(IN P_GLUE_INFO_T prGlueInfo, IN PVOID apvPkts[], IN UINT_8 ucPktNum)
{
	...
	for (ucIdx = 0U; ucIdx < ucPktNum; ucIdx++) {
		if (kalRxIndicateOnePkt(prGlueInfo, apvPkts[ucIdx])
			!= WLAN_STATUS_SUCCESS)
			DBGLOG(RX, WARN, "kalRxIndicateOnePkt:%d\n", ucIdx);
	}
	if (netif_carrier_ok(prNetDev)) {
		if (kalPerMonStart(prGlueInfo) != 0)
			DBGLOG(RX, WARN,
				"kalRxIndicatePkts kalPerMonStart Fail\n");
	}
...
WLAN_STATUS kalRxIndicateOnePkt(IN P_GLUE_INFO_T prGlueInfo, IN PVOID pvPkt)
{
	...
	if (prNetDev == NULL) {
		...
		if (bssIdx == (UINT_8)NET_DEV_P2P_IDX) {
	...
	if (in_interrupt() == 0U) {
		if (netif_rx_ni(prSkb) < 0)
			DBGLOG(RX, ERROR, "netif_rx_ni error\n");
	} else {
		if (netif_rx(prSkb) < 0)
			DBGLOG(RX, ERROR, "netif_rx error\n");
	}
...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		qustatus = wlanQueryInformation(prGlueInfo->prAdapter,
			wlanoidQueryBssid, &arBssid[0],
			(UINT_32)sizeof(arBssid), &bufLen);
		if (qustatus != WLAN_STATUS_SUCCESS) {
			DBGLOG(INIT, INFO, "wlanQueryInformation: %d\n",
				qustatus);
		}
		...
		do {
			...
			qustatus = wlanQueryInformation(prGlueInfo->prAdapter,
				...
				(UINT_32)sizeof(ssid), &bufLen);
			if (qustatus != WLAN_STATUS_SUCCESS) {
				DBGLOG(INIT, INFO,
					"wlanQueryInformation: %d\n",
					qustatus);
			}
			/*
			 *ssid.aucSsid[(ssid.u4SsidLen >=
			 *	(UINT_32)PARAM_MAX_LEN_SSID) ? ((UINT_32)
			 *	PARAM_MAX_LEN_SSID - 1U) : ssid.u4SsidLen]
			 *	= 0;
			 */
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			kalMemCopy(prGlueInfo->prDevHandler->ieee80211_ptr->ssid,
					ssid.aucSsid, ssid.u4SsidLen);
			...
			if (bss == NULL) {
				...
				if (!bss) {
					DBGLOG(SCN, WARN,
					"inform_bss failed, BSSID[%pM/%pM] SSID[%s] Chnl[%d/%d] \
						RCPI[%d] IELng[%d] Bcn[%d]\n",
	...
	case WLAN_STATUS_MEDIA_DISCONNECT_LOCALLY:
		...
		if (prGlueInfo->prAdapter->rWifiVar.rConnSettings.assocIeLen > 0) {
			kalMemFree(prGlueInfo->prAdapter->rWifiVar.rConnSettings.pucAssocIEs, VIR_MEM_TYPE,
				   prGlueInfo->prAdapter->rWifiVar.rConnSettings.assocIeLen);
			prGlueInfo->prAdapter->rWifiVar.rConnSettings.assocIeLen = 0;
		}
#endif
	...
	case WLAN_STATUS_MEDIA_SPECIFIC_INDICATION:
		if (pStatus != NULL) {
			...
			default:
				...
				/*
				 */
				DBGLOG(SCN, EVENT, "Indicate status: %d",
					pStatus->eStatusType);
				break;
			}
		}
	...
	case WLAN_STATUS_JOIN_TIMEOUT:
		...
		prGlueInfo->prAdapter->rWifiVar.rAisFsmInfo.prTargetBssDesc;
			...
			if (prBssDesc != NULL)
				COPY_MAC_ADDR(&arBssid[0],
					&prBssDesc->aucBSSID[0]);
			else
				COPY_MAC_ADDR(&arBssid[0],
		&prGlueInfo->prAdapter->rWifiVar.rConnSettings.aucBSSID[0]);
			cfg80211_connect_result(prGlueInfo->prDevHandler,
					...
					(UINT_16)WLAN_STATUS_AUTH_TIMEOUT,
					GFP_KERNEL);
			break;
		}
	...
	default:
		DBGLOG(SCN, WARN, "Indicate status: %d\n", eStatus);
		break;
	}
			...
			IN PUINT_8 pucFrameBody, IN UINT_32 u4FrameBodyLen, IN BOOLEAN fgReassocRequest)
{
	...
	if (fgReassocRequest == TRUE) {
		if (u4FrameBodyLen < 15U)
			return;
	} else {
		if (u4FrameBodyLen < 9U)
			return;
	}
	...
	if (u4FrameBodyLen <= (UINT_32)CFG_CFG80211_IE_BUF_LEN) {
		prGlueInfo->u4ReqIeLength = u4FrameBodyLen;
		kalMemCopy(&(prGlueInfo->aucReqIe[0]), cp, u4FrameBodyLen);
	}
...
VOID kalUpdateReAssocRspInfo(IN P_GLUE_INFO_T prGlueInfo, IN PUINT_8 pucFrameBody, IN UINT_32 u4FrameBodyLen)
{
	...
	/* reset */
	prGlueInfo->u4RspIeLength = 0U;

	if (u4IELength <= (UINT_32)CFG_CFG80211_IE_BUF_LEN) {
...
kalHardStartXmit(struct sk_buff *prSkb, IN struct net_device *prDev, P_GLUE_INFO_T prGlueInfo, UINT_8 ucBssIndex)
{
	...
	if ((prGlueInfo->ulFlag & GLUE_FLAG_HALT) != 0U) {
	...
	if ((UINT_32)GLUE_GET_REF_CNT(prGlueInfo->ai4TxPendingFrameNumPerQueue
...
VOID kalSendCompleteAndAwakeQueue(IN P_GLUE_INFO_T prGlueInfo, IN PVOID pvPacket)
{
	...
	u2QueueIdx = skb_get_queue_mapping(prSkb);
	ASSERT_BOOLEAN(u2QueueIdx < CFG_MAX_TXQ_NUM);
...
VOID kalQueryRegistryMacAddr(IN P_GLUE_INFO_T prGlueInfo, OUT PUINT_8 paucMacAddr)
{
	...
	kalMemCopy((PVOID) paucMacAddr, (PVOID)(&aucZeroMac[0]), MAC_ADDR_LEN);
...
UINT_32 kalReadExtCfg(IN P_GLUE_INFO_T prGlueInfo)
{
	 ...
	 * driver.
	 */
	if (prGlueInfo->u4ExtCfgLength != 0U)
		       ...
		       OUT P_TX_PACKET_INFO prTxPktInfo)
{
	...
	/* IPv4 version check */
	ucIpVersion = (pucIpHdr[0] & (UINT_8)IP_VERSION_MASK)
		>> (UINT_8)IP_VERSION_OFFSET;
	if (ucIpVersion != (UINT_8)IP_VERSION_4) {
	...
	if (ucIpProto == IP_PRO_UDP) {
		...
		/* BOOTP/DHCP protocol */
		if ((u2DstPort == (UINT_16)IP_PORT_BOOTP_SERVER) ||
			(u2DstPort == (UINT_16)IP_PORT_BOOTP_CLIENT)) {
			...
			prTxPktInfo->u2Flag |=
				(UINT_16)BIT((UINT_16)ENUM_PKT_DNS);
		} else {
			DBGLOG(TX, LOUD, "u2DstPort: %d\n", u2DstPort);
		}
		...
		if (ucIcmpType == 3U) /* don't log network unreachable packet */
			return FALSE;
		...
		prTxPktInfo->u2Flag |= (UINT_16)BIT((UINT_16)ENUM_PKT_ICMP);
	} else {
		//DBGLOG(TX, INFO, "ucIpProto: %d\n", ucIpProto);
	}
		       ...
		       IN P_NATIVE_PACKET prPacket, IN PUINT_8 pucIpHdr, OUT P_TX_PACKET_INFO prTxPktInfo)
{
	...
	prTxPktInfo->u2Flag |= (UINT_16)BIT((UINT_16)ENUM_PKT_ARP);
	return TRUE;
}
		       ...
		       IN P_NATIVE_PACKET prPacket, IN PUINT_8 pucIpHdr, OUT P_TX_PACKET_INFO prTxPktInfo)
{
	...
	prTxPktInfo->u2Flag |= (UINT_16)BIT((UINT_16)ENUM_PKT_TDLS);
}
		       ...
		       IN UINT_16 u2EthType, OUT P_TX_PACKET_INFO prTxPktInfo)
{
	...
	if (u2EthType == ETH_P_1X) {
		...
		case 0U: /* eap packet */
			...
			pucPos = pucEapol + 8U;
			if (*pucPos != (UINT_8)EAP_TYPE_EXPANDED)
				break;
			pucPos += 1U;
			WLAN_GET_FIELD_BE24(pucPos, &i4ExpVendor);
			pucPos += 3U;
			WLAN_GET_FIELD_BE32(pucPos, &u32ExpType);
			if (i4ExpVendor != (INT_32)EAP_VENDOR_WFA ||
				u32ExpType != (UINT_32)EAP_VENDOR_TYPE_WSC)
				break;
			pucPos += 4U;
			if (*pucPos != 5U)
				break;
		...
		DBGLOG(SW4, INFO, "u2EthType: %d\n", u2EthType);
	}
	prTxPktInfo->u2Flag |= (UINT_16)BIT((UINT_16)ENUM_PKT_1X);
				   ...
				   IN P_NATIVE_PACKET prPacket, OUT P_TX_PACKET_INFO prTxPktInfo)
{
	...
	if (u2EtherType == (UINT_16)ETH_P_VLAN) {
		prTxPktInfo->u2Flag |=
			(UINT_16)BIT((UINT_16)ENUM_PKT_VLAN_EXIST);
		ucEthTypeLenOffset += (UINT_8)ETH_802_1Q_HEADER_LEN;
		WLAN_GET_FIELD_BE16(&aucLookAheadBuf[ucEthTypeLenOffset], &u2EtherType);
	}
	/* 4 <2> Obtain next protocol pointer */
	pucNextProtocol = &aucLookAheadBuf[ucEthTypeLenOffset +
		(UINT_8)ETHER_TYPE_LEN];

	/* 4 <3> Handle ethernet format */
	switch (u2EtherType) {

	case (UINT_16)ETH_P_IPV4:
		/* IPv4 header length check */
		if (u4PacketLen < ((UINT_32)ucEthTypeLenOffset +
			(UINT_32)ETHER_TYPE_LEN + (UINT_32)IPV4_HDR_LEN)) {
			DBGLOG(INIT, WARN, "Invalid IPv4 packet length: %lu\n", u4PacketLen);
			break;
		}

		if (kalIPv4FrameClassifier(prGlueInfo, prPacket,
			pucNextProtocol, prTxPktInfo) == FALSE)
			DBGLOG(INIT, WARN, "kalIPv4FrameClassifier Fail: %d\n",
				u2EtherType);
		break;

	case ETH_P_ARP:

		if (kalArpFrameClassifier(prGlueInfo, prPacket,
			pucNextProtocol, prTxPktInfo) == FALSE)
			DBGLOG(INIT, WARN, "kalArpFrameClassifier Fail: %d\n",
				u2EtherType);
		break;
	...
	default:
		...
		if (u2EtherType <= (UINT_16)ETH_802_3_MAX_LEN)
			prTxPktInfo->u2Flag |=
				(UINT_16)BIT((UINT_16)ENUM_PKT_802_3);
		break;
	}
	...
	prTxPktInfo->ucPriorityParam = (UINT_8)prSkb->priority;

	/* 4 <6> Retrieve Packet Information - DA */
	/* Packet Length/ Destination Address */
	prTxPktInfo->u4PacketLen = u4PacketLen;

	kalMemCopy(&prTxPktInfo->aucEthDestAddr[0], aucLookAheadBuf,
		PARAM_MAC_ADDR_LEN);
...
BOOLEAN kalGetEthDestAddr(IN P_GLUE_INFO_T prGlueInfo, IN P_NATIVE_PACKET prPacket, OUT PUINT_8 pucEthDestAddr)
{
	...
	/* Sanity Check */
	if (prPacket == NULL || prGlueInfo == NULL)
		return FALSE;
	       ...
	       IN BOOLEAN fgSetQuery, IN UINT_32 u4SetQueryInfoLen, IN WLAN_STATUS rOidStatus)
{
	...
	prGlueInfo->u4OidCompleteFlag = 1U;
	 ...
	 IN UINT_32 u4InfoBufLen, IN BOOL fgRead, IN BOOL fgWaitResp, IN BOOL fgCmd, OUT PUINT_32 pu4QryInfoLen)
{
	 ...
	 */
	if (kalHaltLock(2 * WLAN_OID_TIMEOUT_THRESHOLD) != 0)
		return WLAN_STATUS_FAILURE;
	...
	prGlueInfo->u4OidCompleteFlag = 0U;
...
UINT_32 kalProcessTxPacket(P_GLUE_INFO_T prGlueInfo, struct sk_buff *prSkb)
{
	...
	if (GLUE_TEST_PKT_FLAG(prSkb, (UINT_16)ENUM_PKT_1X) != 0U) {
		if (wlanProcessSecurityFrame(prGlueInfo->prAdapter,
			(P_NATIVE_PACKET) prSkb) == TRUE) {
...
VOID kalProcessTxReq(P_GLUE_INFO_T prGlueInfo, PBOOLEAN pfgNeedHwAccess)
{
	...
	if (wlanProcessMboxMessage(prGlueInfo->prAdapter)
		!= WLAN_STATUS_SUCCESS)
		DBGLOG(INIT, WARN,
			"wlanProcessMboxMessage Fail\n");
	...
	GLUE_RELEASE_SPIN_LOCK(prGlueInfo, SPIN_LOCK_CMD_QUE);
	if (u4CmdCount > 0U) {
		if (wlanProcessCommandQueue(prGlueInfo->prAdapter,
			prCmdQue) != WLAN_STATUS_SUCCESS)
			DBGLOG(INIT, WARN,
				"wlanProcessCommandQueue1 Fail\n");
	}
#else
	if (prCmdQue->u4NumElem > 0U) {
		if (*pfgNeedHwAccess == FALSE) {
			*pfgNeedHwAccess = TRUE;

			wlanAcquirePowerControl(prGlueInfo->prAdapter);
		}
		if (wlanProcessCommandQueue(prGlueInfo->prAdapter, prCmdQue)
			!= WLAN_STATUS_SUCCESS)
			DBGLOG(INIT, WARN,
				"wlanProcessCommandQueue2 Fail\n");
	}
#endif


	while (u4TxLoopCount > 0U) {
		u4TxLoopCount--;
		while (QUEUE_IS_NOT_EMPTY(prTxQueue)) {
			...
			if (wlanGetTxPendingFrameCount(prGlueInfo->prAdapter)
				> 0U) {
				if (wlanTxPendingPackets(prGlueInfo->prAdapter,
					pfgNeedHwAccess) != WLAN_STATUS_SUCCESS)
					DBGLOG(INIT, WARN,
						"wlanTxPendingPackets2 Fail\n");
			}
		...
		if (wlanGetTxPendingFrameCount(prGlueInfo->prAdapter) > 0U) {
			if (wlanTxPendingPackets(prGlueInfo->prAdapter,
				pfgNeedHwAccess) != WLAN_STATUS_SUCCESS)
				DBGLOG(INIT, WARN,
					"wlanTxPendingPackets Fail\n");
		}
	}
...
int hif_thread(void *data)
{
	...
	prGlueInfo->u4HifThreadPid = (UINT_32)KAL_GET_CURRENT_THREAD_ID();
	...
	while (1 == 1) {
		...
		if ((prGlueInfo->ulFlag & GLUE_FLAG_HIF_PROCESS) == 0U)
		...
		if (KAL_WAKE_LOCK_ACTIVE(prGlueInfo->prAdapter,
			&rHifThreadWakeLock) == 0U)
			KAL_WAKE_LOCK(prGlueInfo->prAdapter, &rHifThreadWakeLock);

		if (wlanAcquirePowerControl(prGlueInfo->prAdapter)
			!= WLAN_STATUS_SUCCESS)
			DBGLOG(INIT, INFO, "wlanAcquirePowerControl Fail\n");
		...
		if (test_and_clear_bit(GLUE_FLAG_HIF_TX_CMD_BIT,
			&prGlueInfo->ulFlag) != 0) {
			if (wlanTxCmdMthread(prGlueInfo->prAdapter) !=
				WLAN_STATUS_SUCCESS)
				DBGLOG(INIT, INFO, "wlanTxCmdMthread Fail\n");
		}

		/* Process TX data packet to SDIO request */
		if (test_and_clear_bit(GLUE_FLAG_HIF_TX_BIT,
			&prGlueInfo->ulFlag) != 0) {
			if (nicTxMsduQueueMthread(prGlueInfo->prAdapter) != 0U)
				DBGLOG(INIT, INFO,
					"nicTxMsduQueueMthread Fail\n");
		}
		...
		if (test_and_clear_bit(GLUE_FLAG_HAL_MCR_RD_BIT,
			&prGlueInfo->ulFlag) != 0) {
		...
		if (test_and_clear_bit(GLUE_FLAG_HAL_MCR_WR_BIT,
			&prGlueInfo->ulFlag) != 0) {
		...
		/* Release to FW own */
		if (wlanReleasePowerControl(prGlueInfo->prAdapter)
			!= WLAN_STATUS_SUCCESS)
			DBGLOG(INIT, WARN, "wlanReleasePowerControl Fail\n");
#if defined(MT6631)
...
int rx_thread(void *data)
{
	...
	prGlueInfo->u4RxThreadPid = (UINT_32)KAL_GET_CURRENT_THREAD_ID();
	...
	while (1 == 1) {
		...
		if ((prGlueInfo->ulFlag & GLUE_FLAG_RX_PROCESS) == 0U)
		...
		do {
			ret = wait_event_interruptible(prGlueInfo->waitq_rx,
					((prGlueInfo->ulFlag &
					GLUE_FLAG_RX_PROCESS) != 0U));
		} while (ret != 0);

		if (KAL_WAKE_LOCK_ACTIVE(prGlueInfo->prAdapter,
			&rRxThreadWakeLock) == 0U)
			KAL_WAKE_LOCK(prGlueInfo->prAdapter, &rRxThreadWakeLock);
		...
		if (test_and_clear_bit(GLUE_FLAG_RX_TO_OS_BIT,
			...
			while (u4LoopCount > 0U) {
				...
				while (QUEUE_IS_NOT_EMPTY(prTempRxQue)) {
					...
					if (kalRxIndicateOnePkt(
						prGlueInfo,
				(PVOID) GLUE_GET_PKT_DESCRIPTOR(prQueueEntry))
						!= WLAN_STATUS_SUCCESS)
						DBGLOG(INIT, WARN,
						"kalRxIndicateOnePkt Fail\n");
					}
...
int tx_thread(void *data)
{
...
#if CFG_SUPPORT_MULTITHREAD
	prGlueInfo->u4TxThreadPid = (UINT_32)KAL_GET_CURRENT_THREAD_ID();
#endif

	current->flags |= (UINT_16)PF_NOFREEZE;
	ASSERT(prGlueInfo);
	...
	while (1 == 1) {
		...
		if ((prGlueInfo->ulFlag & GLUE_FLAG_TX_PROCESS) == 0U)
		...
		if (prGlueInfo->fgEnSdioTestPattern == TRUE) {
			...
			if (wlanAcquirePowerControl(prGlueInfo->prAdapter)
				!= WLAN_STATUS_SUCCESS)
				DBGLOG(INIT, WARN,
					"wlanAcquirePowerControl Fail\n");

			if (prGlueInfo->fgIsSdioTestInitialized == FALSE) {
				/* enable PRBS mode */
				if (kalDevRegWrite(prGlueInfo, MCR_WTMCR,
					0x00080002) == FALSE)
					DBGLOG(INIT, WARN,
						"kalDevRegWrite Fail\n");
			...
			if (prGlueInfo->fgSdioReadWriteMode == TRUE) {
				/* read test */
				if (kalDevPortRead(prGlueInfo,
					MCR_WTMDR,
					256U,
					prGlueInfo->aucSdioTestBuffer,
				(UINT_32)sizeof(prGlueInfo->aucSdioTestBuffer))
					== FALSE)
					DBGLOG(INIT, WARN,
						"kalDevPortRead Fail\n");
			} else {
				/* write test */
				if (kalDevPortWrite(prGlueInfo,
					MCR_WTMDR,
					172U,
					prGlueInfo->aucSdioTestBuffer,
				(UINT_32)sizeof(prGlueInfo->aucSdioTestBuffer))
		...
		do {
			if (test_and_clear_bit(GLUE_FLAG_OID_BIT,
				...
				if (prIoReq->rStatus != WLAN_STATUS_PENDING) {
					...
					if (wlanoidTimeoutCheck(
						prGlueInfo->prAdapter,
						prIoReq->pfnOidHandler)
						== FALSE)
						DBGLOG(INIT, WARN,
						"wlanoidTimeoutCheck Fail\n");
				}
			}
		...
		if (test_and_clear_bit(GLUE_FLAG_TX_CMD_DONE_BIT,
			&prGlueInfo->ulFlag) != 0) {
			if (wlanTxCmdDoneMthread(prGlueInfo->prAdapter)
				!= WLAN_STATUS_SUCCESS)
				DBGLOG(INIT, WARN,
					"wlanTxCmdDoneMthread Fail\n");
		}
#endif
		...
		if (test_and_clear_bit(GLUE_FLAG_TIMEOUT_BIT,
			&prGlueInfo->ulFlag) != 0) {
			if (wlanTimerTimeoutCheck(prGlueInfo->prAdapter)
				!= WLAN_STATUS_SUCCESS)
				DBGLOG(INIT, WARN,
					"wlanTimerTimeoutCheck Fail\n");
		}
#if CFG_SUPPORT_SDIO_READ_WRITE_PATTERN
		if (prGlueInfo->fgEnSdioTestPattern == TRUE)
			kalSetEvent(prGlueInfo);
	...
	if (GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum) > 0U)
		kalFlushPendingTxPackets(prGlueInfo);

	/* flush pending security frames */
	if (GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingSecurityFrameNum) > 0U)
...
BOOLEAN kalRetrieveNetworkAddress(IN P_GLUE_INFO_T prGlueInfo, IN OUT PARAM_MAC_ADDRESS *prMacAddr)
{
	...
	if (node != NULL) {
		...
		if (prop != NULL) {
			COPY_MAC_ADDR((PUINT_8)prMacAddr, prop);
			fgIsReadError = FALSE;
		...
		COPY_MAC_ADDR(prMacAddr, &prGlueInfo->rMacAddrOverride[0]);
...
VOID kalOidCmdClearance(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	while (prQueueEntry != NULL) {
		...
		if (((P_CMD_INFO_T) prQueueEntry)->fgIsOid == TRUE) {
			prCmdInfo = (P_CMD_INFO_T) prQueueEntry;
			break;
		}
	...
	GLUE_ACQUIRE_SPIN_LOCK(prGlueInfo, SPIN_LOCK_CMD_QUE);
	//QUEUE_CONCATENATE_QUEUES_HEAD(prCmdQue, prReturnCmdQue);
	QUEUE_CONCATENATE_QUEUES_HEAD(prTempCmdQue, prReturnCmdQue);
	QUEUE_MOVE_ALL(prCmdQue, prTempCmdQue);
	GLUE_RELEASE_SPIN_LOCK(prGlueInfo, SPIN_LOCK_CMD_QUE);
...
BOOLEAN kalSetTimer(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4Interval)
{
	...
	if (del_timer_sync(&(prGlueInfo->tickfn)) < 0)
		DBGLOG(TX, LOUD, "del_timer_sync fail\n");

	prGlueInfo->tickfn.expires = (UINT_64)jiffies +
	(UINT_64)u4Interval * div64_u64((UINT_64)HZ, (UINT_64)MSEC_PER_SEC);
...
VOID kalScanDone(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_KAL_NETWORK_TYPE_INDEX_T eNetTypeIdx, IN WLAN_STATUS status)
{
	...
	// Autochips delete
	//if (wlanCheckSystemConfiguration(prGlueInfo->prAdapter)
	//	!= WLAN_STATUS_SUCCESS)
	//	DBGLOG(TX, WARN,
	//		"kalScanDone wlanCheckSystemConfiguration Fail");
...
{
	if (pr->rx_thread == NULL)
		return;

	KAL_WAKE_LOCK_TIMEOUT(pr->prAdapter, &pr->rTimeoutWakeLock, MSEC_TO_JIFFIES(WAKE_LOCK_THREAD_WAKEUP_TIMEOUT));

	set_bit((INT_32)GLUE_FLAG_RX_TO_OS_BIT, &pr->ulFlag);
...
VOID kalSetTxCmdEvent2Hif(P_GLUE_INFO_T pr)
{
	...
	set_bit((INT_32)GLUE_FLAG_HIF_TX_CMD_BIT, &pr->ulFlag);
	wake_up_interruptible(&pr->waitq_hif);
}
#endif
/*----------------------------------------------------------------------------*/
			   ...
			   OUT PUINT_16 pu2Part2CfgOwnVersion, OUT PUINT_16 pu2Part2CfgPeerVersion)
{
	...
	if (node != NULL) {
		...
		if (prop != NULL) {
			memcpy((PUINT_8)pu2Part1CfgOwnVersion, prop,
				sizeof(UINT_16));
		...
		if (prop != NULL) {
			memcpy((PUINT_8)pu2Part1CfgPeerVersion, prop,
				sizeof(UINT_16));
		...
		if (prop != NULL) {
			memcpy((PUINT_8)pu2Part2CfgOwnVersion, prop,
				sizeof(UINT_16));
		...
		if (prop != NULL) {
			memcpy((PUINT_8)pu2Part2CfgPeerVersion, prop,
				sizeof(UINT_16));
...
#endif
	default:
		DBGLOG(INIT, INFO, "kalUpdateRSSI: %d\n", eNetTypeIdx);
		break;

	}

	if (pStats != NULL) {
		pStats->qual.qual = (UINT_8)cLinkQuality;
		pStats->qual.noise = 0U;
		pStats->qual.updated = (UINT_8)IW_QUAL_QUAL_UPDATED |
			(UINT_8)IW_QUAL_NOISE_UPDATED | (UINT_8)IW_QUAL_DBM;
		pStats->qual.level = (UINT_8)0x100 + (UINT_8)cRssi;
		pStats->qual.updated |= (UINT_8)IW_QUAL_LEVEL_UPDATED;
	}
...
BOOLEAN kalInitIOBuffer(VOID)
{
	...
	if (CFG_COALESCING_BUFFER_SIZE >= CFG_RX_COALESCING_BUFFER_SIZE)
		u4Size = CFG_COALESCING_BUFFER_SIZE +
			(UINT_32)sizeof(ENHANCE_MODE_DATA_STRUCT_T);
	else
...
struct file *kalFileOpen(const char *path, int flags, int rights)
{
	...
	oldfs = get_fs();
	set_fs(get_ds());
	filp = filp_open(path, flags, (UINT_16)rights);
	set_fs(oldfs);
	if (IS_ERR(filp)) {
...
VOID kalFileClose(struct file *file)
{
	if (filp_close(file, NULL) < 0)
		DBGLOG(INIT, TRACE, "kalFileClose fail\n");
}

UINT_32 kalFileRead(struct file *file, unsigned long long offset,
	unsigned char *data, unsigned int size)
{
	...
	ret = (int)vfs_read(file, data, size, &offset);
...
UINT_32 kalFileWrite(struct file *file, unsigned long long offset, unsigned char *data, unsigned int size)
{
	...
	ret = (int)vfs_write(file, data, size, &offset);
...
UINT_32 kalWriteToFile(const PUINT_8 pucPath, BOOLEAN fgDoAppend, PUINT_8 pucData, UINT_32 u4Size)
{
	...
	file = kalFileOpen(pucPath, (INT_32)((UINT_32)O_WRONLY |
		(UINT_32)O_CREAT | u4Flags), 0700);
...
INT_32 kalReadToFile(const char *pucPath, PUINT_8 pucData, UINT_32 u4Size,
		   ...
		   IN UINT_32 u4FrameLen, IN UINT_8 ucChannelNum, IN INT_32 i4SignalStrength)
{
	...
	if (ucChannelNum <= 14U) {
		prChannel = ieee80211_get_channel(wiphy,
			ieee80211_channel_to_frequency((INT_32)ucChannelNum,
			NL80211_BAND_2GHZ));
	} else {
		prChannel = ieee80211_get_channel(wiphy,
			ieee80211_channel_to_frequency((INT_32)ucChannelNum,
	...
	if (prGlueInfo->fgIsRegistered == TRUE) {
		if (ucChannelNum <= 14U) {
			prChannel = ieee80211_get_channel(
				priv_to_wiphy(prGlueInfo),
				ieee80211_channel_to_frequency(
				(INT_32)ucChannelNum, NL80211_BAND_2GHZ));
		} else {
			prChannel = ieee80211_get_channel(
				priv_to_wiphy(prGlueInfo),
				ieee80211_channel_to_frequency(
				(INT_32)ucChannelNum, NL80211_BAND_5GHZ));
		}
...
VOID kalIndicateRxMgmtFrame(IN P_GLUE_INFO_T prGlueInfo, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		i4Freq = (INT_32)nicChannelNum2Freq(ucChnlNum) / 1000;
		...
		if (!cfg80211_rx_mgmt(
			...
			GFP_ATOMIC))
			DBGLOG(AIS, TRACE, "cfg80211_rx_mgmt fail\n");
	} while (0 != 0);
...
BOOLEAN kalSetSdioTestPattern(IN P_GLUE_INFO_T prGlueInfo, IN BOOLEAN fgEn, IN BOOLEAN fgRead)
{
	...
	if (fgRead == FALSE) {
		...
		for (i = 0U; i < (UINT_32)sizeof(aucPattern); i++)
...
VOID kalSchedScanStopped(IN P_GLUE_INFO_T prGlueInfo, BOOLEAN fgDriverTriggerd)
{
	 ...
	 */
	if (fgDriverTriggerd == TRUE) {
		if (!schedule_delayed_work(&sched_workq, 0))
		  ...
		  IN UINT_32 u4MaxNumOfAddr, OUT PUINT_8 pucIpv4Addrs, OUT PUINT_32 pu4NumOfIpv4Addr)
{
	...
	if (prDev == NULL || (prDev->ip_ptr == NULL) ||
		((struct in_device *)(prDev->ip_ptr))->ifa_list == NULL) {
	...
	while ((u4NumIPv4 < u4MaxNumOfAddr) && prIfa != NULL) {
		kalMemCopy(&pucIpv4Addrs[u4NumIPv4 * u4AddrLen],
			(PUINT_8)(&prIfa->ifa_local), u4AddrLen);
		  ...
		  IN UINT_32 u4MaxNumOfAddr, OUT PUINT_8 pucIpv6Addrs, OUT PUINT_32 pu4NumOfIpv6Addr)
{
	...
	/* 4 <1> Sanity check of netDevice */
	if (prDev == NULL || (prDev->ip6_ptr) == NULL) {
	...
	list_for_each_entry(prIfa, prAddrList, if_list) {
		kalMemCopy(&pucIpv6Addrs[u4NumIPv6 * u4AddrLen],
			(PUINT_8)(&prIfa->addr), u4AddrLen);

		DBGLOG(INIT, INFO,
		       "IPv6 addr [%u][" IPV6STR "]\n", u4NumIPv6, IPV6TOSTR(&pucIpv6Addrs[u4NumIPv6 * u4AddrLen]));

		if ((u4NumIPv6 + 1U) >= u4MaxNumOfAddr)
			break;
		u4NumIPv6++;
	}
		 ...
		 IN PUINT_8 pucIPv4Addr, IN UINT_32 u4NumIPv4Addr, IN PUINT_8 pucIPv6Addr, IN UINT_32 u4NumIPv6Addr)
{
	...
	UINT_32 u4Len =
		(UINT_32)OFFSET_OF(PARAM_NETWORK_ADDRESS_LIST, arAddress);
	...
	for (i = 0U; i < u4NumIPv4Addr; i++) {
		prParamNetAddr->u2AddressType = PARAM_PROTOCOL_ID_TCP_IP;
		prParamNetAddr->u2AddressLength = (UINT_16)u4AddrLen;
		kalMemCopy(&prParamNetAddr->aucAddress[0],
	...
	u4AddrLen = (UINT_32)IPV6_ADDR_LEN;
...
VOID kalSetNetAddressFromInterface(IN P_GLUE_INFO_T prGlueInfo, IN struct net_device *prDev, IN BOOLEAN fgSet)
{
	...
	if (fgSet == TRUE) {
		if (kalGetIPv4Address(prDev, CFG_PF_ARP_NS_MAX_NUM,
			pucIPv4Addr, &u4NumIPv4) == FALSE)
			DBGLOG(REQ, WARN, "SetNetAddress get IPV4 fail");

		if (kalGetIPv6Address(prDev, CFG_PF_ARP_NS_MAX_NUM,
			pucIPv6Addr, &u4NumIPv6) == FALSE)
			DBGLOG(REQ, WARN, "SetNetAddress get IPV6 fail");
	...
	if (u4NumIPv4 + u4NumIPv6 > (UINT_32)CFG_PF_ARP_NS_MAX_NUM) {
		if (u4NumIPv4 >= (UINT_32)CFG_PF_ARP_NS_MAX_NUM) {
			u4NumIPv4 = (UINT_32)CFG_PF_ARP_NS_MAX_NUM;
			u4NumIPv6 = 0U;
		} else {
			u4NumIPv6 = (UINT_32)CFG_PF_ARP_NS_MAX_NUM - u4NumIPv4;
		}
	}
...
BOOLEAN kalMetCheckProfilingPacket(IN P_GLUE_INFO_T prGlueInfo, IN P_NATIVE_PACKET prPacket)
{
	...
	if (u4PacketLen < (UINT_32)ETHER_HEADER_LEN) {
		DBGLOG(INIT, WARN, "Invalid Ether packet length: %lu\n", u4PacketLen);
		return FALSE;
	}
	...
	/* 4 <1> Skip 802.1Q header (VLAN Tagging) */
	if (u2EtherTypeLen == (UINT_16)ETH_P_VLAN) {
		ucEthTypeLenOffset += (UINT_8)ETH_802_1Q_HEADER_LEN;
		WLAN_GET_FIELD_BE16(&aucLookAheadBuf[ucEthTypeLenOffset], &u2EtherTypeLen);
	}
	/* 4 <2> Obtain next protocol pointer */
	...
	case ETH_P_IPV4:
		{
			...
			/* IPv4 header length check */
			if (u4PacketLen < ((UINT_32)ucEthTypeLenOffset +
				(UINT_32)ETHER_TYPE_LEN +
				(UINT_32)IPV4_HDR_LEN)) {
				DBGLOG(INIT, WARN, "Invalid IPv4 packet length: %lu\n", u4PacketLen);
				result = FALSE;
				break;
			}
			...
			if (pucIpHdr[IPV4_HDR_IP_PROTOCOL_OFFSET] ==
				(UINT_8)IP_PROTOCOL_UDP) {
...
{
	if (unlikely(tracing_mark_write_addr == 0U))
		tracing_mark_write_addr = kallsyms_lookup_name("tracing_mark_write");
}

VOID kalMetTagPacket(IN P_GLUE_INFO_T prGlueInfo, IN P_NATIVE_PACKET prPacket, IN ENUM_TX_PROFILING_TAG_T eTag)
{
	if (prGlueInfo->fgMetProfilingEn == FALSE)
		return;

	switch (eTag) {
	case TX_PROF_TAG_OS_TO_DRV:
		if (kalMetCheckProfilingPacket(prGlueInfo, prPacket) == TRUE) {
			__mt_update_tracing_mark_write_addr();
#if 0		/* #if CFG_MET_PACKET_TRACE_SUPPORT */
	...
	case TX_PROF_TAG_DRV_TX_DONE:
		if (GLUE_GET_PKT_IS_PROF_MET(prPacket) != 0U) {
			__mt_update_tracing_mark_write_addr();
#if 0		/* #if CFG_MET_PACKET_TRACE_SUPPORT */
...
static ssize_t kalMetCtrlWriteProcfs(struct file *file, const char __user *buffer, size_t count, loff_t *off)
{
	...
	u4CopySize = (count < (sizeof(acBuf) - 1UL)) ?
		(UINT_32)count : (UINT_32)(sizeof(acBuf) - 1U);
	result = (ssize_t)copy_from_user(acBuf, buffer, u4CopySize);
	acBuf[u4CopySize] = '\0';
...
static ssize_t kalMetPortWriteProcfs(struct file *file, const char __user *buffer, size_t count, loff_t *off)
{
	...
	u4CopySize = (count < (sizeof(acBuf) - 1UL)) ? (UINT_32)count :
		(UINT_32)sizeof(acBuf) - 1U;
	result = (ssize_t)copy_from_user(acBuf, buffer, u4CopySize);
	acBuf[u4CopySize] = '\0';
...
int kalMetInitProcfs(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if (proc_create(PROC_MET_PROF_CTRL, 0000,
		pMetProcDir, &rMetProcCtrlFops) == NULL)
		DBGLOG(INIT, WARN, "kalMetInitProcfs proc_create1 fail\n");

	if (proc_create(PROC_MET_PROF_PORT, 0000, pMetProcDir,
		&rMetProcPortFops) == NULL)
		DBGLOG(INIT, WARN, "kalMetInitProcfs proc_create2 fail\n");
...
int kalMetRemoveProcfs(void)
{
	...
	if (remove_proc_subtree("wlan0", init_net.proc_net) < 0)
		DBGLOG(AIS, TRACE, "remove_proc_subtree failed\n");
	/* clear MetGlobalData */
	pMetGlobalData = NULL;
...
BOOLEAN kalIndicateAgpsNotify(P_ADAPTER_T prAdapter, UINT_8 cmd, PUINT_8 data, UINT_16 dataLen)
{
	...
	struct sk_buff *skb = cfg80211_testmode_alloc_event_skb(
		priv_to_wiphy(prGlueInfo),
		(INT_32)dataLen, GFP_KERNEL);
	if (skb == NULL) {
		DBGLOG(AIS, TRACE, "kalIndicateAgpsNotify: alloc skb failed\n");
		return FALSE;
	}
	/* DBGLOG(CCX, INFO, ("WLAN_STATUS_AGPS_NOTIFY, cmd=%d\n", cmd)); */
	if (unlikely(nla_put(skb, (INT_32)MTK_ATTR_AGPS_CMD,
		(INT_32)sizeof(cmd), &cmd) < 0))
		goto nla_put_failure;
	if (dataLen > 0U && data != NULL &&
		unlikely(nla_put(skb, (INT_32)MTK_ATTR_AGPS_DATA,
			(INT_32)dataLen, data) < 0))
		goto nla_put_failure;
	if (unlikely(nla_put(skb, (INT_32)MTK_ATTR_AGPS_IFINDEX,
		(INT_32)sizeof(UINT_32),
		&prGlueInfo->prDevHandler->ifindex) < 0))
		goto nla_put_failure;
	/* currently, the ifname maybe wlan0, p2p0, so the maximum name length will be 5 bytes */
	if (unlikely(nla_put(skb, (INT_32)MTK_ATTR_AGPS_IFNAME, 5,
...
UINT_64 kalGetBootTime(void)
{
	...
	bootTime = (UINT_64)ts.tv_sec;
	bootTime *= (UINT_64)USEC_PER_SEC;
	bootTime += div64_u64((UINT_64)ts.tv_nsec, (UINT_64)NSEC_PER_USEC);
	return bootTime;
}
...
INT_32 kalHaltLock(UINT_32 waitMs)
{
	...
	if (waitMs != 0U) {
		i4Ret = down_timeout(&rHaltCtrl.lock,
			(INT_64)MSEC_TO_JIFFIES(waitMs));
		if (i4Ret == 0)
			goto success;
		if (i4Ret != -ETIME)
			return i4Ret;
		if (rHaltCtrl.fgHeldByKalIoctl == TRUE) {
...
VOID kalHaltUnlock(VOID)
{
	if (kalGetTimeTick() - rHaltCtrl.u4HoldStart >
		(UINT_32)WLAN_OID_TIMEOUT_THRESHOLD * 2U &&
...
VOID kalDumpWTBL(P_ADAPTER_T prAdapter)
{
	...
	if (pucWtblBaseAddr == NULL)
		return;

	for (i = 0U; i < (UINT_8)WTBL_SIZE; i++)
		if (prWtbl[i].ucUsed != 0U) {
...
INT_32 kalPerMonStart(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if ((wlan_fb_power_down == TRUE ||
		prGlueInfo->fgIsInSuspendMode == TRUE) &&
		KAL_TEST_BIT(PERF_MON_DISABLE_BIT,
		prPerMonitor->ulPerfMonFlag) == 0) {
		/*
...
INT_32 kalPerMonStop(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if (KAL_TEST_BIT(PERF_MON_RUNNING_BIT, prPerMonitor->ulPerfMonFlag)
		...
		if (kalBoostCpu(0) < 0)
			DBGLOG(SW4, TRACE, "kalBoostCpu1 fail\n");
	}
	DBGLOG(SW4, TRACE, "exit %s\n", __func__);
	return 0;
}
...
{
	return kalPerMonDisable(prGlueInfo);
}
...
VOID kalPerMonHandler(IN P_ADAPTER_T prAdapter, ULONG ulParam)
{
	...
	for (u4Idx = 0U; u4Idx < (UINT_32)BSS_INFO_NUM; u4Idx++) {
	...
	latestTxBytes = (LONG)prGlueInfo->prDevHandler->stats.tx_bytes;
	latestRxBytes = (LONG)prGlueInfo->prDevHandler->stats.rx_bytes;
	p2pLatestTxBytes =
		(LONG)prGlueInfo->prP2PInfo->prDevHandler->stats.tx_bytes;
	p2pLatestRxBytes =
		(LONG)prGlueInfo->prP2PInfo->prDevHandler->stats.rx_bytes;
	if (prPerMonitor->ulLastRxBytes == 0U &&
		prPerMonitor->ulLastTxBytes == 0U &&
		prPerMonitor->ulP2PLastRxBytes == 0U &&
		prPerMonitor->ulP2PLastTxBytes == 0U) {
		prPerMonitor->ulThroughput = 0U;
	} else {
		txDiffBytes = latestTxBytes - (LONG)prPerMonitor->ulLastTxBytes;
		rxDiffBytes = latestRxBytes - (LONG)prPerMonitor->ulLastRxBytes;
		if (txDiffBytes < 0)
		...
		p2pTxDiffBytes = p2pLatestTxBytes -
			(LONG)prPerMonitor->ulP2PLastTxBytes;
		p2pRxDiffBytes = p2pLatestRxBytes -
			(LONG)prPerMonitor->ulP2PLastRxBytes;
		if (p2pTxDiffBytes < 0)
			p2pTxDiffBytes = -(p2pTxDiffBytes);
		if (p2pRxDiffBytes < 0)
			p2pRxDiffBytes = -(p2pRxDiffBytes);

		prPerMonitor->ulThroughput = ((ULONG)txDiffBytes +
			(ULONG)rxDiffBytes + (ULONG)p2pTxDiffBytes +
			(ULONG)p2pRxDiffBytes);
		prPerMonitor->ulThroughput *= 1000U;
		prPerMonitor->ulThroughput = div64_u64(prPerMonitor->ulThroughput,
					(ULONG)prPerMonitor->u4UpdatePeriod);
		prPerMonitor->ulThroughput <<= 3U;
	}

	prPerMonitor->ulLastTxBytes = (ULONG)latestTxBytes;
	prPerMonitor->ulLastRxBytes = (ULONG)latestRxBytes;
	prPerMonitor->ulP2PLastTxBytes = (ULONG)p2pLatestTxBytes;
	prPerMonitor->ulP2PLastRxBytes = (ULONG)p2pLatestRxBytes;

	if (prPerMonitor->ulThroughput < THROUGHPUT_L1_THRESHOLD)
		prPerMonitor->u4TarPerfLevel = 0U;
	else if (prPerMonitor->ulThroughput < THROUGHPUT_L2_THRESHOLD)
		prPerMonitor->u4TarPerfLevel = 1U;
	else if (prPerMonitor->ulThroughput < THROUGHPUT_L3_THRESHOLD)
		prPerMonitor->u4TarPerfLevel = 2U;
	else if (prPerMonitor->ulThroughput < THROUGHPUT_L4_THRESHOLD)
		prPerMonitor->u4TarPerfLevel = 3U;
	else
		prPerMonitor->u4TarPerfLevel = 9U;

	if (wlan_fb_power_down == TRUE ||
		prGlueInfo->fgIsInSuspendMode == TRUE ||
		!(netif_carrier_ok(prNetDev) ||
		(prP2pBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) ||
		(prP2pBssInfo->rStaRecOfClientList.u4NumElem > 0U))) {
		if (kalPerMonStop(prGlueInfo) < 0)
			DBGLOG(SW4, TRACE, "kalPerMonStop Fail\n");
	}
	else {
		...
		if (prPerMonitor->u4TarPerfLevel != prPerMonitor->u4CurrPerfLevel) {
			...
			if (prPerMonitor->u4TarPerfLevel != 0U) {
				if (kalBoostCpu(prPerMonitor->u4TarPerfLevel
					+ 1U) < 0)
					DBGLOG(SW4, TRACE,
						"kalBoostCpu2 fail\n");
			}
...
static int wlan_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (event != (unsigned long)FB_EVENT_BLANK || prGlueInfo == NULL)
		return 0;
	...
	case FB_BLANK_UNBLANK:
		if (kalPerMonEnable(prGlueInfo) != 0)
			DBGLOG(SW4, WARN, "kalPerMonEnable fail\n");
	...
	case FB_BLANK_POWERDOWN:
		...
		break;
	default:
		DBGLOG(SW4, WARN, "wlan_fb_notifier_callback:%d\n", blank);
		break;
	}
...
{
	if (fb_unregister_client(&wlan_fb_notifier) < 0)
		DBGLOG(SW4, WARN, "fb_unregister_client fail\n");
...
UINT_8 kalGetEapolKeyType(P_NATIVE_PACKET prPacket)
{
	...
	if (prSkb == NULL)
		return (UINT_8)EAPOL_KEY_NOT_KEY;
