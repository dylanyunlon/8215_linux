...
static const struct wiphy_vendor_command mtk_p2p_vendor_ops[] = {
	{
		...
		.flags = (UINT_32)WIPHY_VENDOR_CMD_NEED_WDEV |
			(UINT_32)WIPHY_VENDOR_CMD_NEED_NETDEV,
...
static const struct nl80211_vendor_cmd_info mtk_p2p_vendor_events[] = {
	{
		.vendor_id = OUI_QCA,
		.subcmd = (UINT_32)WIFI_EVENT_STA_DISCONNECT
	},
};
#endif
...
static const struct ieee80211_txrx_stypes
	...
	[NL80211_IFTYPE_ADHOC] = {
		.tx = 0xffff,
		.rx = (UINT_16)(BIT((UINT_16)IEEE80211_STYPE_ACTION >> 4U))
	},
	[NL80211_IFTYPE_STATION] = {
		.tx = 0xffff,
		.rx = (UINT_16)(BIT((UINT_16)IEEE80211_STYPE_ACTION >> 4U) |
			BIT((UINT_16)IEEE80211_STYPE_PROBE_REQ >> 4U))
	},
	[NL80211_IFTYPE_AP] = {
		.tx = 0xffff,
		.rx = (UINT_16)BIT((UINT_16)IEEE80211_STYPE_PROBE_REQ >> 4U) |
	...
	[NL80211_IFTYPE_AP_VLAN] = {
		/* copy AP */
		.tx = 0xffff,
		.rx = (UINT_16)(BIT((UINT_16)IEEE80211_STYPE_ASSOC_REQ >> 4U) |
		      BIT((UINT_16)IEEE80211_STYPE_REASSOC_REQ >> 4U) |
		      BIT((UINT_16)IEEE80211_STYPE_PROBE_REQ >> 4U) |
		      BIT((UINT_16)IEEE80211_STYPE_DISASSOC >> 4U) |
		      BIT((UINT_16)IEEE80211_STYPE_AUTH >> 4U) |
		      BIT((UINT_16)IEEE80211_STYPE_DEAUTH >> 4U) |
		      BIT((UINT_16)IEEE80211_STYPE_ACTION >> 4U))
	},
...
static const struct ieee80211_iface_limit mtk_p2p_iface_limits[] = {
	{
		.max = 1,
		.types = (UINT_16)(BIT((UINT_16)NL80211_IFTYPE_STATION) |
			BIT((UINT_16)NL80211_IFTYPE_AP))
	},
	{
		.max = 1,
		.types = (UINT_16)(BIT((UINT_16)NL80211_IFTYPE_P2P_CLIENT) |
		BIT((UINT_16)NL80211_IFTYPE_P2P_GO))
	},
};
...
static const struct ieee80211_iface_combination mtk_p2p_iface_combos[] = {
	{
		...
		.n_limits = (UINT_8)ARRAY_SIZE(mtk_p2p_iface_limits),
...
BOOLEAN p2PAllocInfo(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	do {
		if (prGlueInfo->prP2PInfo == NULL) {
			...
			ASSERT(prAdapter->prP2pInfo);
			ASSERT(prWifiVar->prP2PConnSettings);
			/* ASSERT(prWifiVar->prP2pFsmInfo != NULL); */
			ASSERT(prWifiVar->prP2pSpecificBssInfo);
		}
		...
		if (prWifiVar->prP2PConnSettings != NULL)
...
BOOLEAN p2pNetRegister(P_GLUE_INFO_T prGlueInfo, BOOLEAN fgIsRtnlLockAcquired)
{
	...
	if (fgDoRegister == FALSE)
		return TRUE;

	if (fgIsRtnlLockAcquired == TRUE && rtnl_is_locked() != 0) {
...
BOOLEAN glRegisterP2P(P_GLUE_INFO_T prGlueInfo, const char *prDevName, BOOLEAN fgIsApMode)
{
	...
	/*0. allocate p2pinfo */
	if (p2PAllocInfo(prGlueInfo) == FALSE) {
	...
	if (prGlueInfo->prAdapter->fgEnable5GBand == FALSE)
	...
	prGlueInfo->prP2PInfo->prDevHandler =
		alloc_netdev_mq((INT_32)sizeof(NETDEV_PRIVATE_GLUE_INFO),
		prDevName, NET_NAME_PREDICTABLE, ether_setup, CFG_MAX_TXQ_NUM);
...
/* prGlueInfo->prP2PInfo->prDevHandler->wireless_handlers    = &mtk_p2p_wext_handler_def; */
	...
	prGlueInfo->prP2PInfo->prDevHandler->features =
		(UINT_32)NETIF_F_IP_CSUM;
#endif /* CFG_TCP_IP_CHKSUM_OFFLOAD */

	status = kalResetStats(prGlueInfo->prP2PInfo->prDevHandler);
	if (status != WLAN_STATUS_SUCCESS)
		DBGLOG(INIT, WARN, "RESET Fail for p2p\n");
...
BOOLEAN glP2pCreateWirelessDevice(P_GLUE_INFO_T prGlueInfo)
{
	...
	/* 1. allocate WIPHY */
	prWiphy = wiphy_new(&mtk_p2p_ops, (INT_32)sizeof(P_GLUE_INFO_T));
	if (prWiphy == NULL) {
		DBGLOG(P2P, ERROR, "unable to allocate wiphy for p2p\n");
		goto free_wdev;
	}

	prWiphy->interface_modes = (UINT_16)(BIT((UINT_16)NL80211_IFTYPE_AP)
	    | BIT((UINT_16)NL80211_IFTYPE_P2P_CLIENT)
	    | BIT((UINT_16)NL80211_IFTYPE_P2P_GO)
	    | BIT((UINT_16)NL80211_IFTYPE_STATION));
	...
	prWiphy->flags = (UINT_32)WIPHY_FLAG_HAS_REMAIN_ON_CHANNEL |
		(UINT_32)WIPHY_FLAG_HAVE_AP_SME;
	...
	prWiphy->n_vendor_commands = ((INT_32)sizeof(mtk_p2p_vendor_ops) /
		(INT_32)sizeof(struct wiphy_vendor_command));
#if (CFG_NOTIFY_REASON_CODE == 1)
	prWiphy->vendor_events = mtk_p2p_vendor_events;
	prWiphy->n_vendor_events = (INT_32)sizeof(mtk_p2p_vendor_events) /
	(INT_32)sizeof(mtk_p2p_vendor_events[0]);
#endif
...
BOOLEAN glUnregisterP2P(P_GLUE_INFO_T prGlueInfo)
{
	...
	/* adjust glUnregisterP2P sequence to the revert of glRegisterP2P
	 *
	 * glRegisterP2P sequence:
	 *   p2PAllocInfo->alloc_netdev_mq->p2pDevFsmInit->p2pRoleFsmInit
	 *
	 * glUnregisterP2P sequence:
	 *   p2pRoleFsmUninit->p2pDevFsmUninit->free_netdev->p2PFreeInfo
	 */

	/* 4 <1> Uninit P2P role FSM */
	for (ucRoleIdx = 0; ucRoleIdx < BSS_P2P_NUM; ucRoleIdx++) {
		if (P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, ucRoleIdx) != NULL)
			p2pRoleFsmUninit(prGlueInfo->prAdapter, ucRoleIdx);
	}

	/* 4 <2> Uninit P2P dev FSM */
	/* Uninit P2P device FSM */
	p2pDevFsmUninit(prAdapter);

	/* 4 <3> Free Wiphy & netdev */
	if (prGlueInfo->prP2PInfo->prDevHandler){
		if (prGlueInfo->prP2PInfo->prDevHandler->reg_state == NETREG_REGISTERED) {
	...
	if (p2PFreeInfo(prGlueInfo) == FALSE) {
...
void mtk_p2p_wext_set_Multicastlist(P_GLUE_INFO_T prGlueInfo)
{
	...
	if ((prDev->flags & (UINT_32)IFF_PROMISC) != 0U)
		prGlueInfo->prP2PInfo->u4PacketFilter |=
			(UINT_32)PARAM_PACKET_FILTER_PROMISCUOUS;

	if ((prDev->flags & (UINT_32)IFF_BROADCAST) != 0U)
		prGlueInfo->prP2PInfo->u4PacketFilter |=
			(UINT_32)PARAM_PACKET_FILTER_BROADCAST;
	u4McCount = (UINT_32)netdev_mc_count(prDev);

	if ((prDev->flags & (UINT_32)IFF_MULTICAST) != 0U) {
		if ((prDev->flags & (UINT_32)IFF_ALLMULTI) != 0U ||
			(u4McCount > (UINT_32)MAX_NUM_GROUP_ADDR))
			prGlueInfo->prP2PInfo->u4PacketFilter |=
	...
	if ((prGlueInfo->prP2PInfo->u4PacketFilter &
		(UINT_32)PARAM_PACKET_FILTER_MULTICAST) != 0U) {
		...
		if (i >= (UINT_32)MAX_NUM_GROUP_ADDR)
			return;
		status = wlanoidSetP2PMulticastList(prGlueInfo->prAdapter,
				&(prGlueInfo->prP2PInfo->aucMCAddrList[0]),
				(i * (UINT_32)ETH_ALEN), &u4SetInfoLen);

		if (status != WLAN_STATUS_SUCCESS)
			DBGLOG(P2P, WARN, "SEt Multicast List ERROR\n");
...
int p2pHardStartXmit(IN struct sk_buff *prSkb, IN struct net_device *prDev)
{
	...
	XmitStatus = kalHardStartXmit(prSkb, prDev, prGlueInfo, ucBssIndex);
	if (XmitStatus != WLAN_STATUS_SUCCESS)
		DBGLOG(P2P, WARN, "p2pHardStartXmit xmit error\n");
	...
	if ((prP2pBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) ||
		...
		if (status < 0)
			DBGLOG(P2P, WARN, "p2pHardStartXmit PMonStart error\n");
	}
