			     ...
			     u8 key_index, bool pairwise, const u8 *mac_addr, struct key_params *params)
{
	...
	if (params->key != NULL)
		kalMemCopy(rKey.aucKeyMaterial,
		params->key, (size_t)params->key_len);
				...
				const u8 *mac, struct station_info *sinfo)
{
	...
	do {
		...
		/* Inactive time. */
		sinfo->filled |= BIT((UINT_8)NL80211_STA_INFO_INACTIVE_TIME);
...
int mtk_p2p_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	...
	do {
		...
		u4MsgSize = (UINT_32)sizeof(MSG_P2P_SCAN_REQUEST_T) +
			((UINT_32)request->n_channels *
			(UINT_32)sizeof(RF_CHANNEL_INFO_T)) +
			((UINT_32)request->n_ssids *
			(UINT_32)sizeof(PARAM_SSID_T)) +
			(UINT_32)request->ie_len;
		...
		for (u4Idx = 0; u4Idx < request->n_channels; u4Idx++) {
			...
			prRfChannelInfo->ucChannelNum =
				...
				(UINT_32)prChannel->center_freq * 1000U);
		...
		if (request->n_ssids != 0) {
			ASSERT_BOOLEAN((ULONG) prSsidStruct
			== (ULONG)&
			(prMsgScanRequest->arChannelListInfo[u4Idx]));
			prMsgScanRequest->prSSID = prSsidStruct;
		}

		for (u4Idx = 0; u4Idx < (UINT_32)request->n_ssids; u4Idx++) {
		...
		if (request->ie_len != 0U) {
			...
			prMsgScanRequest->u4IELen = (UINT_32)request->ie_len;
		} else {
...
int mtk_p2p_cfg80211_set_wiphy_params(struct wiphy *wiphy, u32 changed)
{
	...
	do {
		...
		if ((changed & (u32)WIPHY_PARAM_RETRY_SHORT) != 0U) {
			/* TODO: */
			DBGLOG(P2P, TRACE, "The RETRY short param is changed.\n");
		}

		if ((changed & (u32)WIPHY_PARAM_RETRY_LONG) != 0U) {
			/* TODO: */
			DBGLOG(P2P, TRACE, "The RETRY long param is changed.\n");
		}

		if ((changed & (u32)WIPHY_PARAM_FRAG_THRESHOLD) != 0U) {
			/* TODO: */
			DBGLOG(P2P, TRACE, "The RETRY fragmentation threshold is changed.\n");
		}

		if ((changed & (u32)WIPHY_PARAM_RTS_THRESHOLD) != 0U) {
			/* TODO: */
			DBGLOG(P2P, TRACE, "The RETRY RTS threshold is changed.\n");
		}

		if ((changed & (u32)WIPHY_PARAM_COVERAGE_CLASS) != 0U) {
			/* TODO: */
...
int mtk_p2p_cfg80211_set_power_mgmt(struct wiphy *wiphy, struct net_device *ndev, bool enabled, int timeout)
{
	...
	kalIoctl(prGlueInfo,
		 ...
		 &rPowerMode,
		 (UINT_32)sizeof(rPowerMode), FALSE, FALSE, TRUE, &u4Leng);
...
int mtk_p2p_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_ap_settings *settings)
{
	...
	do {
		...
		prP2pBcnUpdateMsg = (P_MSG_P2P_BEACON_UPDATE_T) cnmMemAlloc(prGlueInfo->prAdapter,
				RAM_TYPE_MSG,
				((UINT_32)sizeof(MSG_P2P_BEACON_UPDATE_T)+
				(UINT_32)settings->beacon.head_len +
				(UINT_32)settings->beacon.tail_len));
		...
		if (settings->beacon.head_len != 0U) {
		...
		if (settings->beacon.tail_len != 0U) {
			UINT_32 u4cLen = (UINT_32)settings->beacon.tail_len;
			...
			u4cLen += (UINT_32)IE_SIZE(pucBuffer);
			pucBuffer += (UINT_8)IE_SIZE(pucBuffer);

			kalMemCopy(pucBuffer, settings->beacon.tail, settings->beacon.tail_len);

			prP2pBcnUpdateMsg->u4BcnBodyLen = u4cLen;
		} else {
		...
		prP2pStartAPMsg = (P_MSG_P2P_START_AP_T) cnmMemAlloc(prGlueInfo->prAdapter,
			RAM_TYPE_MSG, (UINT_32)sizeof(MSG_P2P_START_AP_T));
		...
		prP2pStartAPMsg->u4BcnInterval =
			(UINT_32)settings->beacon_interval;

		prP2pStartAPMsg->u4DtimPeriod = (UINT_32)settings->dtim_period;

		prP2pStartAPMsg->eHiddenSsidType =
			(ENUM_HIDDEN_SSID_TYPE_T)settings->hidden_ssid;

		prP2pStartAPMsg->fgIsPrivacy = settings->privacy ? TRUE : FALSE;
		...
		COPY_SSID(prP2pStartAPMsg->aucSsid,
			...
			settings->ssid, (UINT_16)settings->ssid_len);
...
int mtk_p2p_cfg80211_change_beacon(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_beacon_data *info)
{
	...
	do {
		...
		if ((info->head_len != 0U) || (info->tail_len != 0U)) {
			prP2pBcnUpdateMsg =
				(P_MSG_P2P_BEACON_UPDATE_T)
				cnmMemAlloc(prGlueInfo->prAdapter,
					RAM_TYPE_MSG,
					((UINT_32)sizeof(
						MSG_P2P_BEACON_UPDATE_T)
					+ (UINT_32)info->head_len
					+ (UINT_32)info->tail_len));
			...
			if (info->tail_len != 0U) {
				UINT_32 u4cLen = (UINT_32)info->tail_len;
...
int mtk_p2p_cfg80211_stop_ap(struct wiphy *wiphy, struct net_device *dev)
{
	...
	do {
		...
		if (mtk_Netdev_To_RoleIdx(prGlueInfo->prP2PInfo, dev, &ucRoleIdx) < 0)
			break;
		/* Switch OP MOde. */
		prP2pSwitchMode = cnmMemAlloc(
			prGlueInfo->prAdapter, RAM_TYPE_MSG,
			(UINT_32)sizeof(MSG_P2P_SWITCH_OP_MODE_T));
				       ...
				       unsigned int duration, u64 *cookie)
{
	...
	do {
		...
		*cookie = prGlueP2pInfo->u8Cookie++;
		...
		prMsgChnlReq = cnmMemAlloc(
			prGlueInfo->prAdapter,
			RAM_TYPE_MSG,
			(UINT_32)sizeof(MSG_P2P_CHNL_REQUEST_T));
					      ...
					      u64 cookie)
{
	...
	do {
		...
		prMsgChnlAbort = cnmMemAlloc(
			prGlueInfo->prAdapter,
			RAM_TYPE_MSG,
			(UINT_32)sizeof(MSG_P2P_CHNL_ABORT_T));
				...
				u64 *cookie)
{
	...
	do {
		...
		if (mtk_Netdev_To_RoleIdx(prGlueP2pInfo, dev, &ucRoleIdx) < 0) {
			...
			ASSERT_BOOLEAN(ucRoleIdx < KAL_P2P_NUM);
			/* Role Interface. */
		...
		*cookie = prGlueP2pInfo->u8Cookie++;
		...
		prMsgTxReq = cnmMemAlloc(
			prGlueInfo->prAdapter,
			RAM_TYPE_MSG,
			(UINT_32)sizeof(MSG_P2P_MGMT_TX_REQUEST_T));
...
#endif
{
	...
	do {
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
		tmp_mac = mac ?: aucBcMac;
#else
		tmp_mac = params->mac ?: aucBcMac;
#endif
		...
		prDisconnectMsg =
			...
			(UINT_32)sizeof(MSG_P2P_CONNECTION_ABORT_T));
		...
		COPY_MAC_ADDR(prDisconnectMsg->aucTargetID, tmp_mac);
...
int mtk_p2p_cfg80211_connect(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	do {
		...
		prConnReqMsg =
			...
			cnmMemAlloc(prGlueInfo->prAdapter,
			RAM_TYPE_MSG,
			((UINT_32)sizeof(MSG_P2P_CONNECTION_REQUEST_T)
			+ (UINT_32)sme->ie_len));
		...
		prConnReqMsg->u4IELen = (UINT_32)sme->ie_len;
					   ...
					   IN P_RF_CHANNEL_INFO_T prRfChnlInfo, IN P_ENUM_CHNL_EXT_T prChnlSco)
{
	...
	do {
		...
		if (prRfChnlInfo != NULL) {
			prRfChnlInfo->ucChannelNum =
				(UINT_8)nicFreq2ChannelNum(
				(UINT_32)channel->center_freq * 1000U);
		...
		if (prChnlSco != NULL) {
			...
			switch (channel_type) {
			case (UINT_8)NL80211_CHAN_NO_HT:
				*prChnlSco = CHNL_EXT_SCN;
				break;
			case (UINT_8)NL80211_CHAN_HT20:
				*prChnlSco = CHNL_EXT_SCN;
				break;
			case (UINT_8)NL80211_CHAN_HT40MINUS:
				*prChnlSco = CHNL_EXT_SCA;
				break;
			case (UINT_8)NL80211_CHAN_HT40PLUS:
...
int mtk_p2p_cfg80211_testmode_cmd(IN struct wiphy *wiphy, IN struct wireless_dev *wdev, IN void *data, IN int len)
{
	...
	case 1:	/* P2P Simga */
#if CFG_SUPPORT_HOTSPOT_OPTIMIZATION
		{
			...
			if ((prParamsCmd->adr & 0xffff0000U) == 0xffff0000U) {
	...
	case 0x30:  /* Auto channel selection in LTE safe channels */
		...
		break;
#endif
// Autochips delete this item
#if 0
	case 1000: /* UPDATE STA PMKID */
...
int mtk_p2p_cfg80211_testmode_p2p_sigma_pre_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	if (data != NULL && len != 0)
		memcpy(&rParams, data, (size_t)len);
	...
	case 108:		/* p2p_set_power_save */
		{
			...
			rPowerMode.ePowerMode = (PARAM_POWER_MODE)value;
			rPowerMode.ucBssIdx = ucBssIdx;
...
int mtk_p2p_cfg80211_testmode_p2p_sigma_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	case 102:		/* Sigma P2p reset */
		...
		break;
	case 103:		/* WPS MODE */
		kalP2PSetWscMode(prGlueInfo, (UINT_8)value);
		break;
	case 104:		/* P2p send persence, duration */
		break;
	case 105:		/* P2p send persence, interval */
...
#if CFG_SUPPORT_HOTSPOT_WPS_MANAGER
		kalIoctl(prGlueInfo, wlanoidSetP2pWPSmode,
			&value, (UINT_32)sizeof(value),
			FALSE, FALSE, TRUE, &u4Leng);
#endif
		break;
	default:
...
{
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4U
	...
	skb = cfg80211_testmode_alloc_reply_skb(
		wiphy, (UINT_32)sizeof(u4AcsChnReport));
	if (skb == NULL) {
	...
	for (i = 0; i < ucNumOfChannel; i++) {
		ucIdx = aucChannelList[i].ucChannelNum - 1U;
		...
		if (ucIdx < 3U) {
			...
			u2UpThreshold = 3;
		} else if (ucIdx >= (ucNumOfChannel - 3U)) {
			u2LowThreshold = 3;
			u2UpThreshold =
				(UINT_16)ucNumOfChannel - ((UINT_16)ucIdx + 1U);
		} else {
		}
	...
	if (prQueryLteChn == NULL) {
		...
		rStatus = kalIoctl(prGlueInfo,
				   ...
				   prQueryLteChn,
				   (UINT_32)sizeof(PARAM_GET_CHN_INFO),
				   TRUE,
				   FALSE,
				   TRUE,
				   &u4BufLen);
		...
		u4LteSafeChnBitMask_2G =
			...
			[(UINT_8)NL80211_TM_CHAN_ATTR_2G_BASE_1 - 1U];
	...
	for (i = 0; i < ucNumOfChannel; i++) {
		if ((u4LteSafeChnBitMask_2G
			& (UINT_32)BIT(arChannelDirtyScore_2G[i].ucChannel))
				== 0U)
			continue;
	...
	u4AcsChnReport[(UINT_8)NL80211_TM_CHAN_ATTR_2G_BASE_1 - 1U]
		= (UINT_32)BIT(31U);
	if (rPreferChannel.ucChannel > 0U)
		u4AcsChnReport[(UINT_8)NL80211_TM_CHAN_ATTR_2G_BASE_1 - 1U]
		|= (UINT_32)BIT(rPreferChannel.ucChannel - 1U);

	/* ToDo: Support 5G Channel Selection */

acs_report:
	if (unlikely(nla_put_u32(skb, (UINT_32)NL80211_TM_CHAN_ATTR_2G_BASE_1,
		u4AcsChnReport[(UINT_8)NL80211_TM_CHAN_ATTR_2G_BASE_1 - 1U])
			< 0U))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, (UINT_32)NL80211_TM_CHAN_ATTR_5G_BASE_36,
		u4AcsChnReport[(UINT_8)NL80211_TM_CHAN_ATTR_5G_BASE_36 - 1U])
			< 0U))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, (UINT_32)NL80211_TM_CHAN_ATTR_5G_BASE_52,
		u4AcsChnReport[(UINT_8)NL80211_TM_CHAN_ATTR_5G_BASE_52 - 1U])
			< 0U))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, (UINT_32)NL80211_TM_CHAN_ATTR_5G_BASE_100,
		u4AcsChnReport[(UINT_8)NL80211_TM_CHAN_ATTR_5G_BASE_100 - 1U])
			< 0U))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, (UINT_32)NL80211_TM_CHAN_ATTR_5G_BASE_149,
		u4AcsChnReport[(UINT_8)NL80211_TM_CHAN_ATTR_5G_BASE_149 - 1U])
			< 0U))
		goto nla_put_failure;
