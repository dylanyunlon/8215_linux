...
/* unsigned char aucExtraInfoBuf[200]; */
	...
	if (wextSrchDesiredWPSIE(cp, (INT_32)u4FrameBodyLen,
		0xDD, &pucDesiredIE) == TRUE) {
		/* WPS IE found */
	} else if (wextSrchDesiredWPAIE(cp,
			(INT_32)u4FrameBodyLen, 0x30, &pucDesiredIE) == TRUE) {
		/* RSN IE found */
	} else if (wextSrchDesiredWPAIE(cp,
			(INT_32)u4FrameBodyLen, 0xDD, &pucDesiredIE) == TRUE) {
		/* WPA IE found */
	} else {
	       ...
	       IN ENUM_PARAM_MEDIA_STATE_T eState, IN PARAM_MAC_ADDRESS rPeerAddr, IN UINT_8 ucRole)
{
	...
	if (eState == PARAM_MEDIA_STATE_CONNECTED) {
		...
		evt.data.length = (UINT_16)strlen(aucBuffer);
...
BOOLEAN kalP2PGetCipher(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if (prGlueInfo->prP2PInfo->u4CipherPairwise
		== (UINT_32)IW_AUTH_CIPHER_CCMP)
		return TRUE;

	if (prGlueInfo->prP2PInfo->u4CipherPairwise
		== (UINT_32)IW_AUTH_CIPHER_TKIP)
		return TRUE;
				...
				IN INT_32 i4ConfigMethod, IN INT_32 i4ActiveConfigMethod)
{
	...
	kalMemCopy(prGlueInfo->prP2PInfo->aucConnReqDevName,
		pucDevName,
		(size_t)prGlueInfo->prP2PInfo->u4ConnReqNameLength);
			   ...
			   IN UINT_8 ucOperatingChnl, IN UINT_8 ucInvitationType, IN PUINT_8 pucGroupBssid)
{
	...
	prGlueInfo->prP2PInfo->u4ConnReqNameLength =
		(UINT_32)((prP2pDevDesc->u2NameLength > 32U)
			? 32U : prP2pDevDesc->u2NameLength);
			   ...
			   IN ENUM_BAND_T eBand, IN ENUM_CHNL_EXT_T eSco, IN UINT_32 u4Duration)
{
	...
	do {
		...
		rChannelInfo.ucChannelNum = (UINT_8)u4ChannelNum;
			     ...
			     IN UINT_32 u4ChannelNum, IN ENUM_BAND_T eBand, IN ENUM_CHNL_EXT_T eSco)
{
	...
	do {
		...
		rRfChannelInfo.ucChannelNum = (UINT_8)u4ChannelNum;
...
#define DBG_P2P_MGMT_FRAME_INDICATION 1
	...
	do {
		...
		ucChnlNum = (UINT_8)HAL_RX_STATUS_GET_CHNL_NUM(
							prSwRfb->prRxStatus);
...
VOID
atcKalP2PGOStationUpdate(IN P_GLUE_INFO_T prGlueInfo,
		      IN UINT_8 ucRoleIndex, IN P_STA_RECORD_T prCliStaRec, IN BOOLEAN fgIsNew,
		      IN UINT_16 ap_be_disconnected_reason)
{
	...
	do {
		...
		if (fgIsNew != FALSE) {
			...
			// Autochips modify for the reason code when the ap is disconnected
			DBGLOG(INIT, INFO, "deauth reason_code:%u\n", ap_be_disconnected_reason);
			if (ap_be_disconnected_reason != REASON_CODE_INVALID_FOR_ATC) {
				atc_cfg80211_del_sta(prGlueInfo->prP2PInfo->aprRoleHandler[ucRoleIndex],
						/* struct net_device * dev, */
						prCliStaRec->aucMacAddr, GFP_KERNEL, ap_be_disconnected_reason);
			} else {
...
kalP2PGOStationUpdate(IN P_GLUE_INFO_T prGlueInfo,
		      IN UINT_8 ucRoleIndex, IN P_STA_RECORD_T prCliStaRec, IN BOOLEAN fgIsNew)
{
	atcKalP2PGOStationUpdate(prGlueInfo,
		      ucRoleIndex, prCliStaRec, fgIsNew, REASON_CODE_INVALID_FOR_ATC);
}
...
struct ieee80211_channel *kalP2pFuncGetChannelEntry(IN P_GL_P2P_INFO_T prP2pInfo, IN P_RF_CHANNEL_INFO_T prChannelInfo)
{
	...
	do {
		...
		for (u4Idx = 0; u4Idx < u4TblSize; u4Idx++) {
...
BOOLEAN kalP2PSetBlackList(IN P_GLUE_INFO_T prGlueInfo, IN PARAM_MAC_ADDRESS rbssid, IN BOOLEAN fgIsblock)
{
	...
	if (fgIsblock != FALSE) {
		...
		for (i = 0; i < P2P_MAXIMUM_CLIENT_COUNT; i++) {
			if (EQUAL_MAC_ADDR(
				&(prGlueInfo->prP2PInfo->aucblackMACList[i][0]),
				rbssid)) {
...
VOID kalP2PSetMaxClients(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4MaxClient)
{
	...
	if (u4MaxClient == 0U
		|| prGlueInfo->prP2PInfo->ucMaxClients
...
BOOLEAN kalP2PMaxClients(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4NumClient)
{
	...
	if (prGlueInfo->prP2PInfo->ucMaxClients != 0U) {
 ...
 */
/*----------------------------------------------------------------------------*/
VOID kalStoreAPSetting(IN P_GLUE_INFO_T prGlueInfo,
	struct cfg80211_ap_settings *settings)
{
	struct AP_PARAMS *prApSetting = NULL;

	ASSERT(prGlueInfo);
	prApSetting = &prGlueInfo->prP2PInfo->apsetting;
	ASSERT(prApSetting);

	prApSetting->head_len = 0U;
	if (settings->beacon.head_len != 0U
		&& settings->beacon.head_len < 128U) {
		prApSetting->head_len = settings->beacon.head_len;
		kalMemCopy(prApSetting->head,
			settings->beacon.head, settings->beacon.head_len);
	} else
		DBGLOG(P2P, WARN,
			"invalid head ie size(%d)\n",
			settings->beacon.head_len);
	prApSetting->tail_len = 0U;
	if (settings->beacon.tail_len != 0U
		&& settings->beacon.tail_len < 128U) {
		prApSetting->tail_len = settings->beacon.tail_len;
		kalMemCopy(prApSetting->tail,
			settings->beacon.tail, settings->beacon.tail_len);
	} else
		DBGLOG(P2P, WARN,
			"invalid hetailad ie size(%zu)\n",
			settings->beacon.tail_len);
	prApSetting->chandef.chan = &prApSetting->chan;
	kalMemCopy(prApSetting->chandef.chan,
		settings->chandef.chan, sizeof(struct ieee80211_channel));
	...
	prApSetting->HiddenSsidType = 0U;
	COPY_SSID(prApSetting->Ssid,
		prApSetting->SsidLen,
