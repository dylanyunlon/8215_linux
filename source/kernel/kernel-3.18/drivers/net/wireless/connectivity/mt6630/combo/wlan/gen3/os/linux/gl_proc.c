...
#define PROC_MCR_ACCESS_MAX_USER_INPUT_LEN      20U
#define PROC_RX_STATISTICS_MAX_USER_INPUT_LEN   10U
#define PROC_TX_STATISTICS_MAX_USER_INPUT_LEN   10U
#define PROC_DBG_LEVEL_MAX_USER_INPUT_LEN       20U
#define PROC_DBG_LEVEL_MAX_DISPLAY_STR_LEN      30U
...
static ssize_t procDbgLevelRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	u2ModuleNum = ((UINT_16)sizeof(aucDbModuleName)
			/ PROC_DBG_LEVEL_MAX_DISPLAY_STR_LEN)
			& 0xfeU;
	for (i = 0; i < u2ModuleNum; i += 2U)
		SPRINTF(temp, ("DBG_%s_IDX\t(0x%02x):\t0x%02x\tDBG_%s_IDX\t(0x%02x):\t0x%02x\n",
				&aucDbModuleName[i][0], i, aucDebugModule[i],
				&aucDbModuleName[i+1U][0], i+1U,
				aucDebugModule[i+1U]));

	if (((sizeof(aucDbModuleName)
		/ PROC_DBG_LEVEL_MAX_DISPLAY_STR_LEN) & 0x1U) != 0U)
		SPRINTF(temp, ("DBG_%s_IDX\t(0x%02x):\t0x%02x\n",
				&aucDbModuleName[u2ModuleNum][0], u2ModuleNum, aucDebugModule[u2ModuleNum]));

	u4CopySize = (UINT_32)kalStrLen(aucProcBuf);
	if (u4CopySize > count)
		u4CopySize = (UINT_32)count;
	if (copy_to_user(buf, aucProcBuf, u4CopySize) != 0U) {
		DBGLOG(HAL, ERROR, "copy to user failed\n");
		return -EFAULT;
	}

	*f_pos += (loff_t)u4CopySize;
...
static ssize_t procDbgLevelWrite(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	UINT_8 *temp = &aucProcBuf[0];
	UINT_32 u4CopySize = (UINT_32)sizeof(aucProcBuf);
	...
	if (copy_from_user(aucProcBuf, buffer, u4CopySize) != 0U) {
		pr_err("error of copy from user\n");
		return -EFAULT;
	}
	aucProcBuf[u4CopySize] = (UINT_8)'\0';
	...
	while (temp != NULL) {
		...
		if (u4NewDbgModule == 0xFFU) {
			UINT_8 i = 0;

			for (; i < (UINT_8)DBG_MODULE_NUM; i++)
				aucDebugModule[i] = (UINT_8)(u4NewDbgLevel
							& DBG_CLASS_MASK);

			break;
		} else if (u4NewDbgModule >= (UINT_8)DBG_MODULE_NUM) {
			pr_info("debug module index should less than %d\n", DBG_MODULE_NUM);
			break;
		} else {
		}

		aucDebugModule[u4NewDbgModule]
			=  (UINT_8)(u4NewDbgLevel & (UINT_32)DBG_CLASS_MASK);
		temp = (UINT_8 *)kalStrChr(temp, (INT32)(','));
		if (temp == NULL)
			break;
		temp++; /* skip ',' */
	}
...
#define MODULE_RX 2
	...
	while (temp != NULL) {
		...
		if (sscanf(temp, "%s %x %hu %u",
			aucModule, &u4IpProto, &u2PortNum,
			&u4DelayThreshold) != 4)  {
...
static ssize_t procMCRRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			...
			(UINT_32)sizeof(rMcrInfo), TRUE, TRUE, TRUE, &u4BufLen);
	kalMemZero(aucProcBuf, sizeof(aucProcBuf));
	SPRINTF(temp, ("MCR (0x%08xh): 0x%08x\n", rMcrInfo.u4McrOffset, rMcrInfo.u4McrData));

	u4Count = (UINT_32)kalStrLen(aucProcBuf);
	if (copy_to_user(buf, aucProcBuf, u4Count) != 0U) {
		pr_err("copy to user failed\n");
		return -EFAULT;
	}

	*f_pos += (loff_t)u4Count;
										...
										size_t count, loff_t *data)
{
	...
	i4CopySize = (int)((count < (sizeof(acBuf) - 1U))
				? count : (sizeof(acBuf) - 1U));
	if (copy_from_user(acBuf, buffer, (UINT_64)i4CopySize) != 0U)
		return 0;
	acBuf[i4CopySize] = '\0';
...
static ssize_t cfgRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	cmdV1Header.cmdBufferLen = (UINT_16)sizeof(struct _CMD_FORMAT_V1_T);
	kalMemSet(cmdV1Header.buffer, 0, MAX_CMD_BUFFER_LENGTH);

	pr_cmd_v1->itemStringLength = (UINT_8)kalStrLen(aucCfgQueryKey);
	...
	u4CopySize = (UINT_32)kalStrLen(aucCfgOutputBuf);
	if (u4CopySize > count)
		u4CopySize = (UINT_32)count;

	if (copy_to_user(buf, aucCfgOutputBuf, u4CopySize) != 0U)
...
static ssize_t cfgWrite(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	UINT_8 i = 0;
	UINT_32 u4CopySize = (UINT_32)sizeof(aucCfgBuf);
	UINT_8 token_num = 1;
	...
	if (copy_from_user(aucCfgBuf, buf, u4CopySize) != 0U) {
		DBGLOG(INIT, ERROR, "copy from user failed\n");
		return -EFAULT;
	}

	for (; i < u4CopySize; i++) {
		if (aucCfgBuf[i] == (UINT_8)(' ')) {
			token_num++;
			break;
		}
	}
	...
	if (token_num == 1U) {
		...
		if (u4CopySize > sizeof(aucCfgQueryKey))
			return -EINVAL;
		/* remove the 0x0a */
		memcpy((PUINT_8)aucCfgQueryKey, (PUINT_8)aucCfgBuf, u4CopySize);
		if (aucCfgQueryKey[u4CopySize - 1U] == 0x0aU)
			aucCfgQueryKey[u4CopySize - 1U] = (UINT_8)'\0';
	} else {
		if (u4CopySize != 0U)
