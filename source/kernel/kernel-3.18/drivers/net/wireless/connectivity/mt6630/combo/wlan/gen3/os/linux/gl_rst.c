...
static void mtk_reset_wifi_only(struct work_struct *work);
...
VOID glResetInit(VOID)
{
	...
	if (mtk_wcn_wmt_msgcb_reg(WMTDRV_TYPE_WIFI,
		(PF_WMT_CB) glResetCallback) < 0)
		DBGLOG(INIT, WARN, "mtk_wcn_wmt_msgcb_reg fail!\n");

	/* 2. Initialize reset work */
	INIT_WORK(&(wifi_rst.rst_wifi_only_work), mtk_reset_wifi_only);
...
VOID glResetUninit(VOID)
{
	...
	if (mtk_wcn_wmt_msgcb_unreg(WMTDRV_TYPE_WIFI) < 0)
		DBGLOG(INIT, WARN, "mtk_wcn_wmt_msgcb_unreg fail!\n");
}
			     ...
			     ENUM_WMTMSG_TYPE_T eMsgType, void *prMsgBody, unsigned int u4MsgLength)
{
	...
	case WMTMSG_TYPE_RESET:
		if (u4MsgLength == sizeof(ENUM_WMTRSTMSG_TYPE_T)) {
			ENUM_WMTRSTMSG_TYPE_T *prRstMsg = (ENUM_WMTRSTMSG_TYPE_T *) prMsgBody;
			...
			case WMTRSTMSG_RESET_START:
				...
				if (wifi_reset_start() < 0)
					DBGLOG(INIT, WARN,
						"wifi_reset_start fail!\n");
				break;
			...
			case WMTRSTMSG_RESET_END:
				...
				if (!schedule_work(&(wifi_rst.rst_work)))
					DBGLOG(INIT, WARN,
						"schedule_work1 fail!\n");
				break;
			...
			default:
				DBGLOG(INIT, WARN, "glResetCallback2:%d\n",
					*prRstMsg);
				break;
			}
		}
	...
	default:
		DBGLOG(INIT, WARN, "glResetCallback:%d\n", eMsgType);
		break;
	}
...
static void mtk_wifi_reset(struct work_struct *work)
{
	...
	if (wifi_reset_end(rst->rst_data) < 0)
		DBGLOG(INIT, WARN, "wifi_reset_end fail!\n");
}
...
static void mtk_wifi_trigger_reset(struct work_struct *work)
{
	MTK_WCN_BOOL fgResult;

	fgResetTriggered = TRUE;
	fgResult = mtk_wcn_wmt_assert(WMTDRV_TYPE_WIFI, (UINT32)64U);
...
BOOLEAN glResetTrigger(P_ADAPTER_T prAdapter)
{
	...
	if (kalIsResetting() == TRUE || fgResetTriggered == TRUE) {
		DBGLOG(INIT, ERROR,
		...
		if (!schedule_work(&(wifi_rst.rst_trigger_work)))
			DBGLOG(INIT, WARN,
				"schedule_work3 fail!\n");
	}
...
UINT_64 u8ResetTime;
VOID glSetRstReason(ENUM_CHIP_RESET_REASON_TYPE_T eReason)
{
	u8ResetTime = sched_clock();
	eResetReason = eReason;
}

VOID glResetWifiOnly(VOID)
{
	if (kalIsResetting()) {
		DBGLOG(INIT, ERROR, "skip glResetWifiOnly during resetting\n");
		return;
	}
	DBGLOG(INIT, ERROR, "glResetWifiOnly\n");
	schedule_work(&wifi_rst.rst_wifi_only_work);
}
