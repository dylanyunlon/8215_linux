 ...
 ********************************************************************************
 */
static const long channel_freq[] = {
...
static const iw_handler rIwPrivHandler[IOCTL_GET_DRIVER -
	SIOCIWFIRSTPRIV + 1] = {
...
const struct iw_handler_def wext_handler_def = {
	.num_standard = 0,
	.num_private = (__u16)sizeof(rIwPrivHandler) /
		(__u16)sizeof(iw_handler),
	.num_private_args = (__u16)sizeof(rIwPrivTable) /
		(__u16)sizeof(struct iw_priv_args),
...
BOOLEAN wextSrchDesiredWAPIIE(IN PUINT_8 pucIEStart, IN INT_32 i4TotalIeLen, OUT PUINT_8 *ppucDesiredIE)
{
	...
	while (i4TotalIeLen >= 2) {
		...
		if (pucIEStart[0] == (UINT_8)ELEM_ID_WAPI &&
...
BOOLEAN wextIsDesiredHS20IE(IN PUINT_8 pucCurIE, IN INT_32 i4TotalIeLen)
{
	...
	if (pucCurIE[0] == (UINT_8)ELEM_ID_VENDOR &&
		i4InfoElemLen <= i4TotalIeLen) {
		if (pucCurIE[1] >= (UINT_8)ELEM_MIN_LEN_HS20_INDICATION) {
...
BOOLEAN wextIsDesiredInterworkingIE(IN PUINT_8 pucCurIE, IN INT_32 i4TotalIeLen)
{
	...
	if (pucCurIE[0] == (UINT_8)ELEM_ID_INTERWORKING &&
		i4InfoElemLen <= i4TotalIeLen) {
		switch (pucCurIE[1]) {
		case (UINT_8)IW_IE_LENGTH_ANO:
		case (UINT_8)IW_IE_LENGTH_ANO_HESSID:
		case (UINT_8)IW_IE_LENGTH_ANO_VENUE:
		case (UINT_8)IW_IE_LENGTH_ANO_VENUE_HESSID:
			result = TRUE;
			break;
		default:
			result = FALSE;
...
BOOLEAN wextIsDesiredAdvProtocolIE(IN PUINT_8 pucCurIE, IN INT_32 i4TotalIeLen)
{
	...
	if (pucCurIE[0] == (UINT_8)ELEM_ID_ADVERTISEMENT_PROTOCOL &&
		i4InfoElemLen <= i4TotalIeLen)
		return TRUE;
	/* check desired EID */
	return FALSE;
...
BOOLEAN wextIsDesiredRoamingConsortiumIE(IN PUINT_8 pucCurIE, IN INT_32 i4TotalIeLen)
{
	...
	if (pucCurIE[0] == (UINT_8)ELEM_ID_ROAMING_CONSORTIUM &&
		i4InfoElemLen <= i4TotalIeLen)
		return TRUE;
	/* check desired EID */
	return FALSE;
...
BOOLEAN wextSrchDesiredHS20IE(IN PUINT_8 pucIEStart, IN INT_32 i4TotalIeLen, OUT PUINT_8 *ppucDesiredIE)
{
	...
	while (i4TotalIeLen >= 2) {
		...
		if (pucIEStart[0] == (UINT_8)ELEM_ID_VENDOR &&
			i4InfoElemLen <= i4TotalIeLen) {
			if (pucIEStart[1] >=
				(UINT_8)ELEM_MIN_LEN_HS20_INDICATION) {
...
BOOLEAN wextSrchDesiredInterworkingIE(IN PUINT_8 pucIEStart, IN INT_32 i4TotalIeLen, OUT PUINT_8 *ppucDesiredIE)
{
	...
	while (i4TotalIeLen >= 2) {
		...
		if (pucIEStart[0] == (UINT_8)ELEM_ID_INTERWORKING &&
			i4InfoElemLen <= i4TotalIeLen) {
			*ppucDesiredIE = &pucIEStart[0];
			return TRUE;
		}
...
BOOLEAN wextSrchDesiredAdvProtocolIE(IN PUINT_8 pucIEStart, IN INT_32 i4TotalIeLen, OUT PUINT_8 *ppucDesiredIE)
{
	...
	while (i4TotalIeLen >= 2) {
		...
		if (pucIEStart[0] == (UINT_8)ELEM_ID_ADVERTISEMENT_PROTOCOL &&
			i4InfoElemLen <= i4TotalIeLen) {
			*ppucDesiredIE = &pucIEStart[0];
			return TRUE;
		}
...
BOOLEAN wextSrchDesiredRoamingConsortiumIE(IN PUINT_8 pucIEStart, IN INT_32 i4TotalIeLen, OUT PUINT_8 *ppucDesiredIE)
{
	...
	while (i4TotalIeLen >= 2) {
		...
		if (pucIEStart[0] == (UINT_8)ELEM_ID_ROAMING_CONSORTIUM &&
			i4InfoElemLen <= i4TotalIeLen) {
			*ppucDesiredIE = &pucIEStart[0];
			return TRUE;
		}
	      ...
	      IN struct iw_request_info *prIwrInfo, OUT struct iw_freq *prIwFreq, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			wlanoidQueryFrequency, &u4Channel,
			(UINT_32)sizeof(u4Channel),
			TRUE, FALSE, FALSE, &u4BufLen);
	      ...
	      IN struct iw_request_info *prIwReqInfo, OUT unsigned int *pu4Mode, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			...
			&eOpMode, (UINT_32)sizeof(eOpMode),
			TRUE, FALSE, FALSE, &u4BufLen);
	       ...
	       IN struct iw_request_info *prIwrInfo, IN struct iw_point *prData, OUT char *pcExtra)
{
	...
	prRange->event_capa[0] = ((UINT_32)IW_EVENT_CAPA_K_0 |
			(UINT_32)IW_EVENT_CAPA_MASK(SIOCGIWAP)
			| (UINT_32)IW_EVENT_CAPA_MASK(SIOCGIWSCAN)
	...
	for (i = 0; i < NUM_CHANNELS; i++) {
		...
		prRange->freq[i].i = (UINT_8)i + 1U;
		prRange->freq[i].m = (INT_32)channel_freq[i];
		prRange->freq[i].e = 6;	/* Values in table in MHz */
	}

	rStatus = kalIoctl(prGlueInfo,
			   wlanoidQuerySupportedRates,
			   &aucSuppRate, (UINT_32)sizeof(aucSuppRate),
			   TRUE, FALSE, FALSE, &u4BufLen);

	for (i = 0; i < IW_MAX_BITRATES && i < PARAM_MAX_LEN_RATES_EX; i++) {
		if (aucSuppRate[i] == 0U)
			break;
		prRange->bitrate[i] = (INT_32)(aucSuppRate[i] & 0x7FU) *
			500000;	/* 0.5Mbps */
	}
	prRange->num_bitrates = (UINT_8)i;
	...
	/* enc_capa */
#if WIRELESS_EXT > 17
	prRange->enc_capa = (UINT_32)IW_ENC_CAPA_WPA |
		(UINT_32)IW_ENC_CAPA_WPA2 |
		(UINT_32)IW_ENC_CAPA_CIPHER_TKIP |
		(UINT_32)IW_ENC_CAPA_CIPHER_CCMP;
#endif
	    ...
	    IN struct iw_request_info *prIwrInfo, OUT struct sockaddr *prAddr, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQueryBssid, prAddr->sa_data,
		(UINT_32)ETH_ALEN, TRUE, FALSE, FALSE, &u4BufLen);
	      ...
	      IN struct iw_request_info *prIwrInfo, IN struct iw_point *prData, IN char *pcExtra)
{
	...
	if (prMlme->cmd == (UINT_16)IW_MLME_DEAUTH ||
		prMlme->cmd == (UINT_16)IW_MLME_DISASSOC) {
		if (!netif_carrier_ok(prNetDev)) {
	      ...
	      IN struct iw_request_info *prIwrInfo, IN struct iw_scan_req *prIwScanReq, IN char *pcExtra)
{
	...
	/* retrieve SSID */
	if (prIwScanReq != NULL)
		essid_len = (int)(prIwScanReq->essid_len);
#endif
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetBssidListScan, pcExtra,
			(UINT_32)essid_len, FALSE, FALSE, FALSE, &u4BufLen);
...
static inline int snprintf_hex(char *buf, size_t buf_size, const u8 *data, size_t len)
{
	...
	for (i = 0; i < len; i++) {
		ret = snprintf(pos, (UINT_64)(end - pos), "%02x", data[i]);
	      ...
	      IN struct iw_request_info *prIwrInfo, IN OUT struct iw_point *prData, IN char *pcExtra)
{
	...
	if (prList->u4NumberOfItems > (UINT_32)CFG_MAX_NUM_BSS_LIST) {
	...
	for (i = 0; i < prList->u4NumberOfItems; ++i) {
		...
		iwEvent.len = (UINT_16)IW_EV_ADDR_LEN;
		...
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_ADDR_LEN);
		...
		iwEvent.len = (UINT_16)(IW_EV_POINT_LEN
...
#if WIRELESS_EXT <= 18
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, iwEvent.len);
#else
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_LCP_LEN);
		memcpy((PUCHAR)(pcCur + IW_EV_LCP_LEN),
			(PUCHAR)&iwEvent.u.data.length,
			sizeof(struct iw_point) - IW_EV_POINT_OFF);
#endif
		memcpy((PUCHAR)(pcCur + IW_EV_POINT_LEN),
			(PUCHAR)prBss->rSsid.aucSsid, iwEvent.u.essid.length);
		pcCur += iwEvent.len;
		/* Frequency */
		iwEvent.cmd = SIOCGIWFREQ;
		iwEvent.len = (UINT_16)IW_EV_FREQ_LEN;
		if ((pcCur + iwEvent.len) > pcEnd)
			break;
		iwEvent.u.freq.m = (INT_32)(prBss->rConfiguration.u4DSConfig);
		iwEvent.u.freq.e = 3;	/* (in KHz) */
		iwEvent.u.freq.i = 0;
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_FREQ_LEN);
		pcCur += IW_EV_FREQ_LEN;

		/* Operation Mode */
		iwEvent.cmd = SIOCGIWMODE;
		iwEvent.len = (UINT_16)IW_EV_UINT_LEN;
		if ((pcCur + iwEvent.len) > pcEnd)
			break;
		...
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_UINT_LEN);
		pcCur += IW_EV_UINT_LEN;

		/* Quality */
		iwEvent.cmd = IWEVQUAL;
		iwEvent.len = (UINT_16)IW_EV_QUAL_LEN;
		if ((pcCur + iwEvent.len) > pcEnd)
			break;
		iwEvent.u.qual.qual = 0;	/* Quality not available now */
		/* -100 < Rssi < -10, normalized by adding 0x100 */
		iwEvent.u.qual.level = (UINT_8)(0x100 + prBss->rRssi);
		iwEvent.u.qual.noise = 0;	/* Noise not available now */
		iwEvent.u.qual.updated = (UINT_8)IW_QUAL_QUAL_INVALID |
			(UINT_8)IW_QUAL_LEVEL_UPDATED |
			(UINT_8)IW_QUAL_NOISE_INVALID;
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_QUAL_LEN);
		...
		if (prBss->u4Privacy == 0U)
			iwEvent.u.data.flags |= (UINT_16)IW_ENCODE_DISABLED;
#if WIRELESS_EXT <= 18
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_POINT_LEN);
#else
		memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_LCP_LEN);
		memcpy((PUCHAR)(pcCur + IW_EV_LCP_LEN),
			(PUCHAR)&iwEvent.u.data.length,
		...
		u4BufIndex = (UINT_32)sprintf(aucRatesBuf, "Rates (Mb/s):");
		u4HighestRate = 0;
		for (j = 0; j < (UINT_32)PARAM_MAX_LEN_RATES_EX; ++j) {
			UINT_8 curRate = prBss->rSupportedRates[j] & 0x7FU;

			if (curRate == 0U)
				break;

			if ((UINT_32)curRate > u4HighestRate)
				u4HighestRate = curRate;

			if (curRate == (UINT_8)RATE_5_5M)
				...
				u4BufIndex += sprintf(&aucRatesBuf[u4BufIndex],
					" %d", curRate / 2U);
#if DBG
			if (u4BufIndex > (UINT_32)sizeof(aucRatesBuf))
				break;
#endif
		}
		/* Report Highest Rates */
		iwEvent.cmd = SIOCGIWRATE;
		iwEvent.len = (UINT_16)IW_EV_PARAM_LEN;
		if ((pcCur + iwEvent.len) > pcEnd)
			break;
		iwEvent.u.bitrate.value = (INT_32)u4HighestRate * 500000;
		...
		iwEvent.u.data.length = (UINT_16)u4BufIndex;
		iwEvent.len = (UINT_16)(IW_EV_POINT_LEN
		...
		if (wextSrchDesiredWPAIE(
...
#if WIRELESS_EXT <= 18
			memcpy((PUCHAR)pcCur,
				(PUCHAR)&iwEvent, IW_EV_POINT_LEN);
#else
			memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_LCP_LEN);
			memcpy((PUCHAR)(pcCur + IW_EV_LCP_LEN),
			       (PUCHAR)&iwEvent.u.data.length,
			...
			memcpy((PUCHAR)(pcCur + IW_EV_POINT_LEN),
				(PUCHAR)prDesiredIE,
				2U + (UINT_64)prDesiredIE->ucLength);
			pcCur += iwEvent.len;
		}
		...
		if (wextSrchDesiredWPSIE(
			...
			if ((pcCur + iwEvent.len) > pcEnd)
				break;
#if WIRELESS_EXT <= 18
			memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent,
				IW_EV_POINT_LEN);
#else
			memcpy((PUCHAR)pcCur, (PUCHAR)&iwEvent, IW_EV_LCP_LEN);
			memcpy((PUCHAR)(pcCur + IW_EV_LCP_LEN),
...
#endif
			memcpy((PUCHAR)(pcCur + IW_EV_POINT_LEN),
				(PUCHAR)prDesiredIE, 2U +
				(UINT_64)prDesiredIE->ucLength);
			pcCur += iwEvent.len;
		}
		...
		if (wextSrchDesiredWAPIIE(&prBss->aucIEs[sizeof(PARAM_FIXED_IEs)],
			...
			snprintf_hex(pcCur, (UINT_64)(pcEnd - pcCur),
				(UINT_8 *) prDesiredIE,
				(UINT_64)prDesiredIE->ucLength + 2U);

			pcCur += (2U + prDesiredIE->ucLength) *
				2U /* iwEvent.len */;
#endif
		}
#endif
	 ...
	 * applications.
	 */
	prData->length = (UINT_16)(pcValidEntryEnd - pcExtra);
	/* kalIndicateStatusAndComplete(prGlueInfo, WLAN_STATUS_SCAN_COMPLETE, NULL, 0); */
	       ...
	       IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEssid, IN char *pcExtra)
{
	...
	if (prEssid->length > (UINT_16)IW_ESSID_MAX_SIZE)
		return -E2BIG;
...
/* #endif */
	 ...
	 */
	rNewSsid.u4SsidLen = (prEssid->length != 0U) ?
		prEssid->length - 1U : 0U;
#else
	rNewSsid.u4SsidLen = prEssid->length;
#endif
	kalMemCopy(&rNewSsid.aucSsid[0], pcExtra, rNewSsid.u4SsidLen);
	...
	if (kalIoctl(prGlueInfo,
		wlanoidSetSsid,
		(PVOID)&rNewSsid, (UINT_32)sizeof(PARAM_SSID_T),
	       ...
	       IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEssid, OUT char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQuerySsid, prSsid,
			(UINT_32)sizeof(PARAM_SSID_T),
			TRUE, FALSE, FALSE, &u4BufLen);

	if ((rStatus == WLAN_STATUS_SUCCESS) &&
		(prSsid->u4SsidLen <= (UINT_32)MAX_SSID_LEN)) {
		kalMemCopy(pcExtra, &prSsid->aucSsid[0], prSsid->u4SsidLen);
		prEssid->length = (UINT_16)(prSsid->u4SsidLen);
		prEssid->flags = 1;
	}

	kalMemFree(prSsid, VIR_MEM_TYPE, sizeof(PARAM_SSID_T));

	return (INT_32)rStatus;
}				/* wext_get_essid */
	      ...
	      IN struct iw_request_info *prIwrInfo, OUT struct iw_param *prRate, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQueryLinkSpeed, &u4Rate,
				(UINT_32)sizeof(u4Rate),
				TRUE, FALSE, FALSE, &u4BufLen);

	if (rStatus != WLAN_STATUS_SUCCESS)
		return -EFAULT;

	prRate->value = (INT_32)u4Rate * 100;/* u4Rate is in unit of 100bps */
	prRate->fixed = 0;
	     ...
	     IN struct iw_request_info *prIwrInfo, IN struct iw_param *prRts, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			wlanoidSetRtsThreshold, &u4RtsThresh,
			(UINT_32)sizeof(u4RtsThresh),
			FALSE, FALSE, FALSE, &u4BufLen);

	prRts->value = (typeof(prRts->value)) u4RtsThresh;
	prRts->disabled = (UINT_8)((prRts->value > 2347) ? 1 : 0);
	prRts->fixed = 1;
	     ...
	     IN struct iw_request_info *prIwrInfo, OUT struct iw_param *prRts, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			wlanoidQueryRtsThreshold, &u4RtsThresh,
			(UINT_32)sizeof(u4RtsThresh),
			TRUE, FALSE, FALSE, &u4BufLen);

	prRts->value = (typeof(prRts->value)) u4RtsThresh;
	prRts->disabled = (UINT_8)((prRts->value > 2347 || prRts->value < 0)
					? 1 : 0);
	prRts->fixed = 1;
	       ...
	       IN struct iw_request_info *prIwrInfo, IN struct iw_param *prTxPow, IN char *pcExtra)
{
	...
	prGlueInfo->ePowerState = (PARAM_DEVICE_POWER_STATE)ePowerState;
		...
		IN struct iw_request_info *prIwrInfo, OUT struct iw_point *prEnc, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo,
			wlanoidQueryEncryptionStatus, &eEncMode,
			(UINT_32)sizeof(eEncMode),
			TRUE, FALSE, FALSE, &u4BufLen);
	...
	case ENUM_WEP_KEY_ABSENT:
		prEnc->flags = (UINT_16)IW_ENCODE_ENABLED |
			(UINT_16)IW_ENCODE_NOKEY;
		break;
	default:
		prEnc->flags = IW_ENCODE_ENABLED;
		break;
	...
	prEnc->flags |= (UINT_16)IW_ENCODE_NOKEY;
#endif
	return 0;
}				/* wext_get_encode */
		...
		IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	if ((prEnc->flags & (UINT_16)IW_ENCODE_MODE) ==
		(UINT_16)IW_ENCODE_DISABLED) {
		...
		rStatus = kalIoctl(prGlueInfo,
				...
				&eEncStatus, (UINT_32)sizeof(eEncStatus),
				FALSE, FALSE, FALSE, &u4BufLen);
	...
	if (prEnc->length == 5U || prEnc->length == 13U ||
		prEnc->length == 16U) {
		/* prepare PARAM_WEP key structure */
		prWepKey->u4KeyIndex
			= ((UINT_32)prEnc->flags & (UINT_32)IW_ENCODE_INDEX)
			!= 0U ? ((UINT_32)prEnc->flags
				& (UINT_32)IW_ENCODE_INDEX) - 1U : 0U;
		if (prWepKey->u4KeyIndex > 3U) {
			/* key id is out of range */
			return -EINVAL;
		}
		prWepKey->u4KeyIndex |= 0x80000000U;
		prWepKey->u4Length = 12U + (UINT_32)prEnc->length;
		prWepKey->u4KeyLength = prEnc->length;
		kalMemCopy(&prWepKey->aucKeyMaterial[0],
			pcExtra, prEnc->length);
		...
		rStatus = kalIoctl(prGlueInfo,
				wlanoidSetEncryptionStatus,
				&eEncStatus,
				(UINT_32)sizeof(ENUM_PARAM_ENCRYPTION_STATUS_T),
				FALSE, FALSE, FALSE, &u4BufLen);
	       ...
	       IN struct iw_request_info *prIwrInfo, OUT struct iw_param *prPower, IN char *pcExtra)
{
	...
	rStatus = wlanQueryInformation(prGlueInfo->prAdapter,
				wlanoidQuery802dot11PowerSaveProfile,
				&ePowerMode, (UINT_32)sizeof(ePowerMode),
				&u4BufLen);
#endif
	...
	if (Param_PowerModeMAX_PSP == ePowerMode) {
		prPower->value = 1;
		prPower->disabled = 0;
	}

	if (Param_PowerModeFast_PSP == ePowerMode) {
		prPower->value = 2;
		prPower->disabled = 0;
	}

	prPower->flags = (UINT_16)IW_POWER_PERIOD | (UINT_16)IW_POWER_RELATIVE;
#if WIRELESS_EXT < 21
	prPower->value *= 1000000U;
#endif
	      ...
	      IN struct iw_request_info *prIwrInfo, IN struct iw_param *prAuth, IN char *pcExtra)
{
	...
	switch (prAuth->flags & (UINT_16)IW_AUTH_INDEX) {
	case IW_AUTH_WPA_VERSION:
#if CFG_SUPPORT_WAPI
		if (wlanQueryWapiMode(prGlueInfo->prAdapter) == TRUE) {
			prGlueInfo->rWpaInfo.u4WpaVersion = IW_AUTH_WPA_VERSION_DISABLED;
			prGlueInfo->rWpaInfo.u4AuthAlg = IW_AUTH_ALG_OPEN_SYSTEM;
		} else {
			prGlueInfo->rWpaInfo.u4WpaVersion =
				(UINT_32)(prAuth->value);
		}
#else
	...
	case IW_AUTH_CIPHER_GROUP:
		prGlueInfo->rWpaInfo.u4CipherGroup = (UINT_32)(prAuth->value);
		break;

	case IW_AUTH_KEY_MGMT:
		prGlueInfo->rWpaInfo.u4KeyMgmt = (UINT_32)(prAuth->value);
#if CFG_SUPPORT_WAPI
		if (prGlueInfo->rWpaInfo.u4KeyMgmt ==
			(UINT_32)IW_AUTH_KEY_MGMT_WAPI_PSK ||
			prGlueInfo->rWpaInfo.u4KeyMgmt ==
			(UINT_32)IW_AUTH_KEY_MGMT_WAPI_CERT) {
			UINT_32 u4BufLen;
			WLAN_STATUS rStatus;
	...
	case IW_AUTH_80211_AUTH_ALG:
		prGlueInfo->rWpaInfo.u4AuthAlg = (UINT_32)(prAuth->value);
		break;

	case IW_AUTH_PRIVACY_INVOKED:
		prGlueInfo->rWpaInfo.fgPrivacyInvoke = (BOOLEAN)(prAuth->value);
		break;
#if CFG_SUPPORT_802_11W
	case IW_AUTH_MFP:
		prGlueInfo->rWpaInfo.u4Mfp = (UINT_32)(prAuth->value);
		break;
#endif
#if CFG_SUPPORT_WAPI
		    ...
		    IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEnc, IN char *pcExtra)
{
...
#if CFG_SUPPORT_WAPI
	if (prIWEncExt->alg == (UINT_16)IW_ENCODE_ALG_SMS4) {
		if ((prEnc->flags & (UINT_16)IW_ENCODE_DISABLED) != 0U)
			return 0;

		/* KeyID */
		prWpiKey->ucKeyID = ((UINT_8)prEnc->flags &
			(UINT_8)IW_ENCODE_INDEX);
		prWpiKey->ucKeyID--;
		...
		if ((prIWEncExt->ext_flags & (UINT_32)IW_ENCODE_EXT_GROUP_KEY)
			!= 0U) {
			...
			DBGLOG(INIT, INFO, "ext_flags:%d\n",
				prIWEncExt->ext_flags);

		/* PN */
		memcpy((PUCHAR)&prWpiKey->aucPN[0],
			(PUCHAR)&prIWEncExt->tx_seq[0],
			IW_ENCODE_SEQ_MAX_SIZE * 2);

		/* BSSID */
		memcpy(&prWpiKey->aucAddrIndex[0],
			(PUINT_8)&prIWEncExt->addr.sa_data[0], 6);

		memcpy((PUCHAR)prWpiKey->aucWPIEK, (PUCHAR)prIWEncExt->key, 16);
		prWpiKey->u4LenWPIEK = 16;

		memcpy((PUCHAR)prWpiKey->aucWPICK,
			(PUCHAR)&prIWEncExt->key[16], 16);
...
#endif
	{
		...
		if ((prEnc->flags & (UINT_16)IW_ENCODE_MODE) ==
			(UINT_16)IW_ENCODE_DISABLED) {
			prRemoveKey->u4Length = (UINT_32)sizeof(*prRemoveKey);
			memcpy(&prRemoveKey->arBSSID[0],
				(PUINT_8)&prIWEncExt->addr.sa_data[0], 6);
		...
		case IW_ENCODE_ALG_WEP:
			...
			/* iwconfig wlan0 key 01234567890123456789012345 [1] */
			/* check key size for WEP */
			if (prIWEncExt->key_len == 5U ||
				prIWEncExt->key_len == 13U ||
				prIWEncExt->key_len == 16U) {
				/* prepare PARAM_WEP key structure */
				prWepKey->u4KeyIndex = ((UINT_32)prEnc->flags
					& (UINT_32)IW_ENCODE_INDEX) != 0U ?
					((UINT_32)prEnc->flags
					& (UINT_32)IW_ENCODE_INDEX) - 1U : 0U;
				if (prWepKey->u4KeyIndex > 3U) {
					/* key id is out of range */
				...
				prWepKey->u4KeyIndex |= 0x80000000U;
				prWepKey->u4Length = 12U +
					(UINT_32)prIWEncExt->key_len;
		...
		case IW_ENCODE_ALG_AES_CMAC:
#endif
			/* KeyID */
			prKey->u4KeyIndex = ((UINT_32)prEnc->flags
				& (UINT_32)IW_ENCODE_INDEX) != 0U ?
					((UINT_32)prEnc->flags
					& (UINT_32)IW_ENCODE_INDEX) - 1U : 0U;
#if CFG_SUPPORT_802_11W
			if (prKey->u4KeyIndex > 5U) {
#else
			if (prKey->u4KeyIndex > 3U) {
#endif
			...
			if ((prIWEncExt->ext_flags &
				(UINT_32)IW_ENCODE_EXT_SET_TX_KEY) != 0U)
				prKey->u4KeyIndex |= (UINT_32)(0x1UL << 31U);
			...
			if ((prIWEncExt->ext_flags &
				...
				/* pairwise key */
				prKey->u4KeyIndex |= (UINT_32)(0x1UL << 30U);
			}

			/* Rx SC Bit(29) */
			if ((prIWEncExt->ext_flags &
				(UINT_32)IW_ENCODE_EXT_RX_SEQ_VALID) != 0U) {
				prKey->u4KeyIndex |= (UINT_32)(0x1UL << 29U);
				memcpy((PUCHAR)&prKey->rKeyRSC,
					(PUCHAR)&prIWEncExt->rx_seq[0],
					IW_ENCODE_SEQ_MAX_SIZE);
			}

			/* BSSID */
			memcpy(&prKey->arBSSID[0],
				(PUINT_8)&prIWEncExt->addr.sa_data[0], 6);

			/* switch tx/rx MIC key for sta */
			if (prIWEncExt->alg == (UINT_16)IW_ENCODE_ALG_TKIP &&
				prIWEncExt->key_len == 32U) {
				memcpy(&prKey->aucKeyMaterial[0],
					&prIWEncExt->key[0], 16);
				memcpy(&prKey->aucKeyMaterial[16],
					&prIWEncExt->key[24], 8);
			...
			break;
		default:
			DBGLOG(INIT, INFO, "Set encode ext:%d\n",
				prIWEncExt->alg);
			break;
		}
	}
...
static int wext_set_country(IN struct net_device *prNetDev, IN struct iw_point *prData)
{
	 ...
	 * and "COUNTRY JP"
	 */
	if (GLUE_CHK_PR2(prNetDev, prData) == FALSE || prData->pointer == NULL
		|| prData->length < (UINT_16)COUNTRY_CODE_LEN)
		return -EINVAL;

	prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(prNetDev));

	if (copy_from_user(aucCountry, prData->pointer, COUNTRY_CODE_LEN) != 0U)
		return -EFAULT;

	rStatus = kalIoctl(prGlueInfo,
			wlanoidSetCountryCode,
			&aucCountry[COUNTRY_CODE_LEN-2],
			2U, FALSE, FALSE, TRUE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS) {
...
int wext_support_ioctl(IN struct net_device *prDev, IN struct ifreq *prIfReq, IN int i4Cmd)
{
	...
	case SIOCGIWRANGE:	/* 0x8B0B, get range of parameters */
		if (iwr->u.data.pointer != NULL) {
			...
			if (iwr->u.data.length <
				(UINT_16)sizeof(struct iw_range)) {
				ret = -E2BIG;
				break;
			}
			...
			iwr->u.data.length = (UINT_16)sizeof(struct iw_range);

			ret = wext_get_range(prDev, NULL, &iwr->u.data, prExtraBuf);
			/* Push up to the caller */
			if (copy_to_user(iwr->u.data.pointer,
				prExtraBuf, iwr->u.data.length) != 0U)
				ret = -EFAULT;
	...
	case SIOCSIWMLME:	/* 0x8B16, request MLME operation */
		...
		if (iwr->u.data.length != (UINT_16)sizeof(struct iw_mlme)) {
	...
	case SIOCSIWSCAN:	/* 0x8B18, scan request */
		if (iwr->u.data.pointer == NULL)
...
#if WIRELESS_EXT > 17
		else if (iwr->u.data.length ==
			(UINT_16)sizeof(struct iw_scan_req)) {
			...
			if (copy_from_user(prIwScanReq, iwr->u.data.pointer,
				iwr->u.data.length) != 0U)
				ret = -EFAULT;
			else {
				if (prIwScanReq->essid_len >
					(UINT_8)IW_ESSID_MAX_SIZE)
	...
	case SIOCGIWSCAN:	/* 0x8B19, get scan results */
		...
		if (ret != 0) {
			...
			ASSERT_BOOLEAN(iwr->u.data.length <= u4ExtraSize);
			if (iwr->u.data.length > (UINT_16)u4ExtraSize) {
				DBGLOG(INIT, INFO,
				       "Updated result length is larger than allocated (%u > %u)\n",
					iwr->u.data.length, u4ExtraSize);
				iwr->u.data.length = (UINT_16)u4ExtraSize;
			}
	...
	case SIOCSIWESSID:	/* 0x8B1A, set SSID (network name) */
		...
		if (u4ExtraSize > (UINT_32)IW_ESSID_MAX_SIZE) {
			ret = -E2BIG;
			break;
		}
	...
	case SIOCGIWESSID:	/* 0x8B1B, get SSID */
		...
		if (u4ExtraSize != (UINT_32)IW_ESSID_MAX_SIZE &&
			u4ExtraSize != (UINT_32)IW_ESSID_MAX_SIZE + 1U) {
	...
	case SIOCSIWENCODE:	/* 0x8B2A, set encoding token & mode */
		...
		if (iwr->u.encoding.pointer != NULL) {
			if (u4ExtraSize > 16U) {
				ret = -E2BIG;
				break;
			}
		...
		if (iwr->u.encoding.pointer == NULL && u4ExtraSize != 0U) {
			ret = -EINVAL;
			break;
		}
	...
	case SIOCSIWENCODEEXT:	/* 0x8B34, set extended encoding token & mode */
		if (iwr->u.encoding.pointer != NULL) {
			u4ExtraSize = iwr->u.encoding.length;
			if (u4ExtraSize >
				(UINT_32)sizeof(struct iw_encode_ext)) {
				ret = -EINVAL;
				break;
			}
		...
		if (iwr->u.encoding.pointer == NULL &&
			iwr->u.encoding.length != 0U) {
			ret = -EINVAL;
			break;
		}


		if (ret == 0 && prExtraBuf != NULL)
	...
	case SIOCSIWPMKSA:	/* 0x8B36, pmksa cache operation */
		...
		if (iwr->u.data.pointer != NULL) {
			/* Fixed length structure */
			if (iwr->u.data.length !=
				(UINT_16)sizeof(struct iw_pmksa)) {
				ret = -EINVAL;
				break;
			}

			u4ExtraSize = (UINT_32)sizeof(struct iw_pmksa);
			prExtraBuf = kalMemAlloc(u4ExtraSize, VIR_MEM_TYPE);
			if (prExtraBuf == NULL) {
				ret = -ENOMEM;
				break;
		...
		if (iwr->u.data.pointer == NULL && iwr->u.data.length != 0U) {
			ret = -EINVAL;
			break;
		}
#endif
...
static void wext_support_ioctl_SIOCSIWGENIE(IN P_GLUE_INFO_T prGlueInfo, IN char *prExtraBuf, IN UINT_32 u4ExtraSize)
{
	...
	if (rStatus != WLAN_STATUS_SUCCESS) {
		...
		if (wextSrchDesiredWPSIE(prExtraBuf, (INT_32)u4ExtraSize, 0xDD,
			(PUINT_8 *) &prDesiredIE) == TRUE) {
			rStatus = kalIoctl(prGlueInfo,
					wlanoidSetWSCAssocInfo,
					prDesiredIE,
					IE_SIZE(prDesiredIE), FALSE,
					FALSE, TRUE, &u4BufLen);
		}
#endif
#endif
...
wext_support_ioctl_SIOCSIWPMKSA_Action(IN struct net_device *prDev, IN char *prExtraBuf, IN int ioMode, OUT int *ret)
{
	...
	case IW_PMKSA_ADD:
		pmkid.ucFlags = 0;
		pmkid.ucBssIdx = wlanGetBssIdx(prDev);
		kalMemCopy(pmkid.arBSSID, ((struct iw_pmksa *)prExtraBuf)->bssid.sa_data, 6);
		kalMemCopy(pmkid.arPMKID, ((struct iw_pmksa *)prExtraBuf)->pmkid, IW_PMKID_LEN);

		rStatus = kalIoctl(prGlueInfo,
				wlanoidSetPmkid, &pmkid, sizeof(PARAM_PMKID_T), FALSE, FALSE, TRUE, &u4BufLen);
		if (rStatus != WLAN_STATUS_SUCCESS)
	...
	case IW_PMKSA_FLUSH:
		...
		rStatus = kalIoctl(prGlueInfo,
				wlanoidFlushPmkid, &pmkid, sizeof(PARAM_PMKID_T), FALSE, FALSE, TRUE, &u4BufLen);
		if (rStatus != WLAN_STATUS_SUCCESS)
			DBGLOG(INIT, INFO, "flush pmkid error:%x\n", rStatus);

		break;
	default:
		DBGLOG(INIT, INFO, "SIOCSIWPMKSA_Action:%d\n", ioMode);
		break;
	}
}
			 ...
			 IN unsigned int u4Cmd, IN unsigned char *pucData, IN unsigned int u4dataLen)
{
	...
	switch (u4Cmd) {
	case SIOCGIWTXPOW:
		memcpy((PUCHAR)&wrqu.power, pucData, u4dataLen);
		break;
	case SIOCGIWSCAN:
	...
	case IWEVASSOCREQIE:
		...
		if (wextSrchDesiredWPAIE(pucData, (INT_32)u4dataLen, 0x30,
			&pucDesiredIE) == TRUE) {
			/* RSN IE found */
			/* RSN IE found */
		}
...
#endif
		else if (wextSrchDesiredWPAIE(pucData, (INT_32)u4dataLen, 0xDD,
				&pucDesiredIE) == TRUE) {
			/* WPA IE found */
			/* WPA IE found */
		}
#if CFG_SUPPORT_WAPI		/* Android+ */
		else if (wextSrchDesiredWAPIIE(pucData, (INT_32)u4dataLen,
				&pucDesiredIE) == TRUE) {
			/* WAPI IE found */
			/* WAPI IE found */
		}
		...
		for (i = 0; i < (int)(pucDesiredIE[1] + 2U); ++i)
			pucExtraInfo += sprintf(pucExtraInfo, "%02x", pucDesiredIE[i]);
		pucExtraInfo = aucExtraInfoBuf;
		wrqu.data.length = 17U + (pucDesiredIE[1] + 2U) * 2U;
#else
		/* IWEVASSOCREQIE, indicate binary string */
		pucExtraInfo = pucDesiredIE;
		wrqu.data.length = (UINT_16)pucDesiredIE[1] + 2U;
#endif
#endif /* WIRELESS_EXT < 15 */
		break;
	...
	case IWEVMICHAELMICFAILURE:
		...
		if (pucData != NULL) {
			...
			wrqu.data.length = (UINT_16)(pucExtraInfo -
				aucExtraInfoBuf);
			pucExtraInfo = aucExtraInfoBuf;
		}
#endif /* WIRELESS_EXT < 15 */
	...
	case IWEVPMKIDCAND:
		if (prGlueInfo->rWpaInfo.u4WpaVersion ==
			...
			prGlueInfo->rWpaInfo.u4KeyMgmt ==
			(UINT_32)IW_AUTH_KEY_MGMT_802_1X) {
			...
			kalMemCopy(&rPmkidCand.bssid.sa_data[0],
				&prPmkidCand->arBSSID[0], 6);

			kalMemCopy(pucExtraInfo, (PUINT_8) &rPmkidCand, sizeof(struct iw_pmkid_cand));
			wrqu.data.length =
				(UINT_16)sizeof(struct iw_pmkid_cand);
	...
	case IWEVCUSTOM:
		...
		wrqu.data.length = (UINT_16)sizeof(PTA_IPC_T);
		break;
...
struct iw_statistics *wext_get_wireless_stats(struct net_device *prDev)
{
	...
	if (prDev == NULL || !netif_carrier_ok(prDev)) {
		/* network not connected */
		goto stat_out;
	}

	rStatus = kalIoctl(prGlueInfo, wlanoidQueryRssi, &i4Rssi,
			(UINT_32)sizeof(i4Rssi), TRUE, TRUE, TRUE, &bufLen);
