...
priv_driver_get_chip_config_16(PUINT_8 pucStartAddr,
			UINT_32 u4Length, UINT_32 u4Line,
			int i4TotalLen, INT_32 i4BytesWritten,
			char *pcCommand);
...
static int
priv_driver_set_chip_config(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_set_cfg(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

#if CFG_SUPPORT_SNIFFER
static int
priv_driver_set_monitor(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);
#endif

static int
priv_driver_get_linkspeed(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_set_miracast(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_set_suspend_mode(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_get_cfg(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static UINT_32
batchChannelNum2Freq(UINT_32 u4ChannelNum);

static int
priv_driver_set_sw_ctrl(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_set_country(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_set_test_cmd(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_set_txpower(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);

static int
priv_driver_set_band(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen);
...
********************************************************************************
*/
static UINT_8 aucOidBuf[CMD_OID_BUF_LENGTH] = { 0U };
...
static WLAN_REQ_ENTRY arWlanOidReqTable[] = {
	...
	{OID_802_11_SUPPORTED_RATES,
	 ...
	 TRUE, FALSE, ENUM_OID_DRIVER_CORE, (UINT_32)sizeof(PARAM_RATES_EX),
	...
	{OID_PNP_SET_POWER,
	 ...
	 (UINT_32)sizeof(PARAM_DEVICE_POWER_STATE),
	 NULL,
	...
	{OID_CUSTOM_MCR_RW,
	 ...
	 (UINT_32)sizeof(PARAM_CUSTOM_MCR_RW_STRUCT_T),
	...
	{OID_CUSTOM_EEPROM_RW,
	 ...
	 TRUE, TRUE, ENUM_OID_DRIVER_CORE,
	 (UINT_32)sizeof(PARAM_CUSTOM_EEPROM_RW_STRUCT_T),
	...
	{OID_CUSTOM_SW_CTRL,
	 ...
	 TRUE, TRUE, ENUM_OID_DRIVER_CORE,
	 (UINT_32)sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T),
	...
	{OID_CUSTOM_MEM_DUMP,
	 ...
	 TRUE, TRUE, ENUM_OID_DRIVER_CORE,
	 (UINT_32)sizeof(PARAM_CUSTOM_MEM_DUMP_STRUCT_T),
	...
	{OID_CUSTOM_MTK_WIFI_TEST,
	 ...
	 (UINT_32)sizeof(PARAM_MTK_WIFI_TEST_STRUCT_T),
	...
	{OID_CUSTOM_MTK_NVRAM_RW,
	 ...
	 TRUE, TRUE, ENUM_OID_DRIVER_CORE,
	 (UINT_32)sizeof(PARAM_CUSTOM_NVRAM_RW_STRUCT_T),
	...
	{OID_CUSTOM_CFG_SRC_TYPE,
	 ...
	 FALSE, FALSE, ENUM_OID_DRIVER_CORE,
	 (UINT_32)sizeof(ENUM_CFG_SRC_TYPE_T),
	...
	{OID_CUSTOM_EEPROM_TYPE,
	 ...
	 FALSE, FALSE, ENUM_OID_DRIVER_CORE,
	 (UINT_32)sizeof(ENUM_EEPROM_TYPE_T),
	...
	{OID_802_11_SET_WAPI_KEY,
	 ...
	 FALSE, FALSE, ENUM_OID_DRIVER_CORE,
	 (UINT_32)sizeof(PARAM_WPI_KEY_T),
	 NULL,
...
int priv_support_ioctl(IN struct net_device *prNetDev, IN OUT struct ifreq *prIfReq, IN int i4Cmd)
{
	...
	case IOCTL_SET_INT:
		...
		result = priv_set_int(prNetDev, &rIwReqInfo, &(prIwReq->u),
			(char *)&(prIwReq->u));
		break;

	case IOCTL_GET_INT:
		/* NOTE(Kevin): 1/3 INT Type <= IFNAMSIZ, so we don't need copy_from/to_user() */
		result = priv_get_int(prNetDev, &rIwReqInfo, &(prIwReq->u),
			(char *)&(prIwReq->u));
		break;

	case IOCTL_SET_STRUCT:
	case IOCTL_SET_STRUCT_FOR_EM:
		result = priv_set_struct(prNetDev, &rIwReqInfo, &prIwReq->u,
			(char *)&(prIwReq->u));
		break;

	case IOCTL_GET_STRUCT:
		result = priv_get_struct(prNetDev, &rIwReqInfo, &prIwReq->u,
			(char *)&(prIwReq->u));
		break;
	default:
		result = -EOPNOTSUPP;
...
static EVENT_BATCH_RESULT_T g_rEventBatchResult[CFG_BATCH_MAX_MSCAN];

static UINT_32 batchChannelNum2Freq(UINT_32 u4ChannelNum)
{
	UINT_32 u4ChannelInMHz;

	if (u4ChannelNum >= 1U && u4ChannelNum <= 13U)
		u4ChannelInMHz = 2412U + (u4ChannelNum - 1U) * 5U;
	else if (u4ChannelNum == 14U)
		u4ChannelInMHz = 2484U;
	else if (u4ChannelNum == 133U)
		u4ChannelInMHz = 3665U;	/* 802.11y */
	else if (u4ChannelNum == 137U)
		u4ChannelInMHz = 3685U;	/* 802.11y */
	else if (u4ChannelNum >= 34U && u4ChannelNum <= 165U)
		u4ChannelInMHz = 5000U + u4ChannelNum * 5U;
	else if (u4ChannelNum >= 183U && u4ChannelNum <= 196U)
		u4ChannelInMHz = 4000U + u4ChannelNum * 5U;
	else
		u4ChannelInMHz = 0U;
		   ...
		   OUT PVOID pvBuffer, IN UINT_32 u4MaxBufferLen, OUT PUINT_32 pu4RetLen)
{
	...
	nsize = 0;
	nleft = (INT_32)u4MaxBufferLen - 5;	/* -5 for "----\n" */

	pBr = prEventBatchResult;
	scancount = 0;
	for (j = 0; j < CFG_BATCH_MAX_MSCAN; j++) {
		scancount += (INT_32)pBr->ucScanCount;
		pBr++;
	}
		...
		for (i = 0; i < (INT_32)pBr->ucScanCount; i++) {
			...
			nsize3 =
			    kalSnprintf(text3, TMP_TEXT_LEN_L,
					...
					"dist=%u\ndistSd=%u\n==\n",
					freq, prEntry->cRssi,
					prEntry->u4Dist, prEntry->u4Distsd);
	...
	*pu4RetLen = u4MaxBufferLen - (UINT_32)nleft;
...
short_buf:
	DBGLOG(SCN, TRACE,
	       "Short buffer issue! %u > %u, %p\n", (INT_32)u4MaxBufferLen +
	       (nsize - nleft), u4MaxBufferLen, pvBuffer);
	     ...
	     IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	case PRIV_CMD_TEST_MODE:
		...
		if (pu4IntBuf[1] == (UINT_32)PRIV_CMD_TEST_MAGIC_KEY) {
			prNdisReq->ndisOidCmd = (UINT_32)OID_CUSTOM_TEST_MODE;
		} else if (pu4IntBuf[1] == 0U) {
			prNdisReq->ndisOidCmd =
				(UINT_32)OID_CUSTOM_ABORT_TEST_MODE;
		} else if (pu4IntBuf[1] ==
			(UINT_32)PRIV_CMD_TEST_MAGIC_KEY_ICAP) {
			prNdisReq->ndisOidCmd =
				(UINT_32)OID_CUSTOM_TEST_ICAP_MODE;
		} else {
			status = 0;
			break;
		}
		prNdisReq->inNdisOidlength = 0U;
		prNdisReq->outNdisOidLength = 0U;
	...
	case PRIV_CMD_TEST_CMD:
		...
		kalMemCopy(&prNdisReq->ndisOidContent[0],
			(UINT_8 *)(&pu4IntBuf[1]), 8);

		prNdisReq->ndisOidCmd = (UINT_32)OID_CUSTOM_MTK_WIFI_TEST;
		prNdisReq->inNdisOidlength = 8U;
		prNdisReq->outNdisOidLength = 8U;
	...
	case PRIV_CMD_ACCESS_MCR:
		...
		if (pu4IntBuf[1] == (UINT_32)PRIV_CMD_TEST_MAGIC_KEY) {
			if (pu4IntBuf[2] == (UINT_32)PRIV_CMD_TEST_MAGIC_KEY)
				prGlueInfo->fgMcrAccessAllowed = TRUE;
			status = 0;
			break;
		}
		if (prGlueInfo->fgMcrAccessAllowed == TRUE) {
			kalMemCopy(&prNdisReq->ndisOidContent[0],
				(PUINT_8)(&pu4IntBuf[1]), 8);

			prNdisReq->ndisOidCmd = (UINT_32)OID_CUSTOM_MCR_RW;
			prNdisReq->inNdisOidlength = 8U;
			prNdisReq->outNdisOidLength = 8U;
	...
	case PRIV_CMD_SW_CTRL:
		...
		prNdisReq->ndisOidCmd = (UINT_32)OID_CUSTOM_SW_CTRL;
		prNdisReq->inNdisOidlength = 8U;
		prNdisReq->outNdisOidLength = 8U;
	...
	case PRIV_CMD_CSUM_OFFLOAD:
		{
			...
			if (pu4IntBuf[1] == 1U)
				u4CSUMFlags = (UINT_32)CSUM_OFFLOAD_EN_ALL;
			else if (pu4IntBuf[1] == 0U)
				u4CSUMFlags = 0U;
			else {
				status = -EINVAL;
	...
	case PRIV_CMD_POWER_MODE:
		{
			...
			rstatus = kalIoctl(prGlueInfo,
					...
					&rPowerMode,
					(UINT_32)sizeof(PARAM_POWER_MODE_T),
					FALSE, FALSE, TRUE, &u4BufLen);
			if (rstatus != WLAN_STATUS_SUCCESS)
				DBGLOG(REQ, WARN, "rstatus = %d\n", rstatus);
		}
		break;
	...
	case PRIV_CMD_WMM_PS:
		{
			...
			rWmmPsTest.reserved = 0U;

			rstatus = kalIoctl(prGlueInfo,
				wlanoidSetWiFiWmmPsTest,
				(PVOID)&rWmmPsTest,
				(UINT_32)sizeof(
				PARAM_CUSTOM_WMM_PS_TEST_STRUCT_T),
				FALSE, FALSE, TRUE, &u4BufLen);
			if (rstatus != WLAN_STATUS_SUCCESS)
				DBGLOG(REQ, WARN, "rstatus = %d\n", rstatus);
		}
	...
	case PRIV_CUSTOM_BWCS_CMD:
		...
		prPtaIpc->u.aucBTPParams[0] = (UINT_8) (pu4IntBuf[1] >> 24U);
		prPtaIpc->u.aucBTPParams[1] = (UINT_8) (pu4IntBuf[1] >> 16U);
		prPtaIpc->u.aucBTPParams[2] = (UINT_8) (pu4IntBuf[1] >> 8U);
		...
		status = (INT_32)wlanoidSetBT(prGlueInfo->prAdapter,
				(PVOID)&aucOidBuf[0],
				(UINT_32)sizeof(PARAM_PTA_IPC_T), &u4BufLen);
	...
	case PRIV_CMD_P2P_MODE:
#if 0
		{
			...
			if (rSetP2P.u4Enable == 0U) {
				reg_result = p2pNetUnregister(prGlueInfo, TRUE);
				if (reg_result == FALSE)
					DBGLOG(INIT, ERROR, "p2pNetUnregister Fail\n");
			}

			/* pu4IntBuf[0] is used as input SubCmd */
			rWlanStatus = kalIoctl(prGlueInfo, wlanoidSetP2pMode, (PVOID)&rSetP2P,
				(UINT_32)sizeof(PARAM_CUSTOM_P2P_SET_STRUCT_T), FALSE, FALSE, TRUE, &u4BufLen);

			if ((rSetP2P.u4Enable != 0U) && (rWlanStatus == WLAN_STATUS_SUCCESS)
					&& (fgIsResetting == FALSE)) {
				reg_result = p2pNetRegister(prGlueInfo, TRUE);
				if (reg_result == FALSE)
					DBGLOG(INIT, ERROR, "p2pNetRegister Fail\n");
			}
		}
#else
		/* Due to below dead lock contition, we deprecated this api
		 * and use /dev/wmtWifi instead.
		 *
		 * PRIV_CMD_P2P_MODE call stack:
		 *
		 * sock_ioctl
		 *   dev_ioctl
		 *     wext_handle_ioctl
		 ...
		 *           ioctl_process_call
		 *             ...
		 *               wlan_netdev_ops.ndo_do_ioctl();
		 *                 wlanDoIOCTL();
		 *                   priv_support_ioctl();
		 *                     priv_set_int();
		 *                       case PRIV_CMD_P2P_MODE:
		 *                         lock(MUTEX_P2P_HANDLE);
		 *                         ...
		 *                         unlock(MUTEX_P2P_HANDLE);
		 *         rtnl_unlock
		 *
		 * Dead Lock condition:
		 *
		 ...
		 *   priv_set_int();
		 *     case PRIV_CMD_P2P_MODE:
		 *       // thread B hold rtnl_lock and acquire p2p mutex here
		 *       lock(MUTEX_P2P_HANDLE);
		 *
		 *       if (!u4Enable)
		 *         p2pNetUnregister();
		 *           rtnl_unlock
		 *           unregister_netdev
		 *             // thread A hold p2p mutex and acquire rtnl_lock here
		 *             rtnl_lock
		 *             rtnl_unlock
		 *           rtnl_lock
		 *
		 *       kalIoctl(wlanoidSetP2pMode);
		 *
		 *       if (u4Enable)
		 *         p2pNetRegister();
		 *           rtnl_unlock
		 ...
		 *   rtnl_unlock
		 */
		DBGLOG(P2P, ERROR, "cmd%d not supported, "
				"use /dev/wmtWifi instead\n", PRIV_CMD_P2P_MODE);
		status = -EOPNOTSUPP;
#endif
		break;
#endif
	...
	case PRIV_CMD_MET_PROFILING:
		{
			...
			prGlueInfo->fgMetProfilingEn = (BOOLEAN) pu4IntBuf[1];
	     ...
	     IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN OUT char *pcExtra)
{
	...
	switch (u4SubCmd) {
	case (UINT_32)PRIV_CMD_TEST_CMD:
		prNdisReq = (P_NDIS_TRANSPORT_STRUCT) &aucOidBuf[0];
...
#if CFG_SUPPORT_PRIV_MCR_RW
	case (UINT_32)PRIV_CMD_ACCESS_MCR:
		prNdisReq = (P_NDIS_TRANSPORT_STRUCT) &aucOidBuf[0];

		if (prGlueInfo->fgMcrAccessAllowed == FALSE) {
			status = 0;
			return status;
		}
	...
	case (UINT_32)PRIV_CMD_DUMP_MEM:
		prNdisReq = (P_NDIS_TRANSPORT_STRUCT) &aucOidBuf[0];
		...
		prNdisReq->ndisOidCmd = (UINT_32)OID_CUSTOM_MEM_DUMP;
	...
	case (UINT_32)PRIV_CMD_BAND_CONFIG:
		DBGLOG(INIT, INFO, "CMD get_band=\n");
		prIwReqData->mode = 0U;
		break;

	default:
		DBGLOG(INIT, INFO, "Not valid cmd=%d\n", u4SubCmd);
		break;
	}

	if (u4SubCmd == (UINT_32)PRIV_CMD_TEST_CMD ||
		u4SubCmd == (UINT_32)PRIV_CMD_ACCESS_MCR ||
		u4SubCmd == (UINT_32)PRIV_CMD_DUMP_MEM ||
		u4SubCmd == (UINT_32)PRIV_CMD_SW_CTRL ||
		u4SubCmd == (UINT_32)PRIV_CMD_BAND_CONFIG)
		return status;

	u4SubCmd = (UINT_32) prIwReqData->data.flags;

	switch (u4SubCmd) {
	case (UINT_32)PRIV_CMD_GET_CH_LIST:
		{
			UINT_16 i, j = 0U;
			UINT_8 NumOfChannel = 50U;
			UINT_8 ucMaxChannelNum = 50U;
			...
			if (kalIsAPmode(prGlueInfo) == TRUE) {
				for (i = 0U; i < (UINT_16)NumOfChannel; i++) {
	      ...
	      IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	switch (u4SubCmd) {
	case (UINT_32)PRIV_CMD_SET_TX_POWER:
		{
			if (u4CmdLen > 4U) {
				status = -EINVAL;
				break;
			...
			if (setting[0] == 0 &&
				...
				prIwReqData->data.length == 2U) {
				prTxpwr->ucConcurrencePolicy =
					(UINT_8)setting[1];
			} else if (setting[0] == 2 &&
				prIwReqData->data.length == 3U) {
				if (setting[1] == 0) {
					for (i = 0U; i < 14U; i++)
	...
	case (UINT_32)PRIV_CMD_GET_CH_LIST:
		{
			...
			prIwReqData->data.length = (UINT_16)NumOfChannel;
			if (copy_to_user(prIwReqData->data.pointer, ch,
				(UINT_64)NumOfChannel *
				sizeof(INT_32)) != 0U)
				status = -EFAULT;
		...
		IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	case (UINT_32)PRIV_CUSTOM_BWCS_CMD:
		u4CmdLen =
			(UINT_32)prIwReqData->data.length *
			((UINT_32)sizeof(UINT_32));
		ASSERT_BOOLEAN(sizeof(PARAM_PTA_IPC_T) >= u4CmdLen);
		if ((UINT_32)sizeof(PARAM_PTA_IPC_T) < u4CmdLen) {
			status = -EFAULT;
			break;
		}
...
#if CFG_SUPPORT_WPS2
	case (UINT_32)PRIV_CMD_WSC_PROBE_REQ:
		{
			/* retrieve IE for Probe Request */
			u4CmdLen = (UINT_32)prIwReqData->data.length;
			if (u4CmdLen > (UINT_32)GLUE_INFO_WSCIE_LENGTH) {
				DBGLOG(REQ, ERROR, "Input data length is invalid %u\n", u4CmdLen);
				status = -EINVAL;
				break;
			}

			if (prIwReqData->data.length > 0u) {
				...
				prGlueInfo->u2WSCIELen = (UINT_16)u4CmdLen;
			} else {
				prGlueInfo->u2WSCIELen = 0U;
			}
		}
		break;
#endif
	case (UINT_32)PRIV_CMD_OID:
		u4CmdLen = prIwReqData->data.length;
		if (u4CmdLen > (UINT_32)CMD_OID_BUF_LENGTH) {
			DBGLOG(REQ, ERROR, "Input data length is invalid %u\n", u4CmdLen);
			status = -EINVAL;
			break;
		}
		...
		if (kalMemCmp(&aucOidBuf[0], (UINT_8 *)pcExtra,
			u4CmdLen) == 0) {
			/* ToDo:: DBGLOG */
	...
	case (UINT_32)PRIV_CMD_SW_CTRL:
		...
		if (u4CmdLen > (UINT_32)sizeof(prNdisReq->ndisOidContent)) {
			DBGLOG(REQ, ERROR, "Input data length is invalid %u\n", u4CmdLen);
			status = -EINVAL;
			break;
		}
		...
		IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN OUT char *pcExtra)
{
	...
	ASSERT(prNetDev);
	ASSERT(prIwReqData);
	if (prNetDev == NULL || prIwReqData == NULL) {
	...
	case (UINT_32)PRIV_CMD_OID:
		if (copy_from_user(&aucOidBuf[0], prIwReqData->data.pointer,
			sizeof(NDIS_TRANSPORT_STRUCT)) != 0U) {
	...
	case (UINT_32)PRIV_CMD_SW_CTRL:
		...
		if (prIwReqData->data.length > (UINT_32)(sizeof(aucOidBuf)
...
priv_set_ndis(IN struct net_device *prNetDev, IN NDIS_TRANSPORT_STRUCT * prNdisReq, OUT PUINT_32 pu4OutputLen)
{
	...
	if (prNetDev == NULL || prNdisReq == NULL || pu4OutputLen == NULL) {
	...
	if (prWlanReqEntry->fgSetBufLenChecking == TRUE) {
...
priv_get_ndis(IN struct net_device *prNetDev, IN NDIS_TRANSPORT_STRUCT * prNdisReq, OUT PUINT_32 pu4OutputLen)
{
	...
	if (prWlanReqEntry->fgQryBufLenChecking == TRUE) {
		if (prNdisReq->inNdisOidlength < prWlanReqEntry->u4InfoBufLen) {
...
static BOOLEAN reqSearchSupportedOidEntry(IN UINT_32 rOid, OUT P_WLAN_REQ_ENTRY *ppWlanReqEntry)
{
	...
	i = 0;
	j = (UINT_32)NUM_SUPPORTED_OIDS - 1U;
		...
		IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN OUT char *pcExtra)
{
	...
	ASSERT_BOOLEAN(IW_IS_GET(u2Cmd));
	if (prIwReqData->data.length != 0U) {
	...
	if (i4BytesWritten > 0) {
		if (i4BytesWritten > 2000)
			i4BytesWritten = 2000;
		/* the iwpriv will use the length */
		prIwReqData->data.length = (UINT_16)i4BytesWritten;
...
static int priv_driver_get_sw_ctrl(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	...
	DBGLOG(REQ, LOUD, "argc is %i, %d\n", i4Argc, rStatus);

	if (i4Argc >= 2) {
		/* rSwCtrlInfo.u4Id = kalStrtoul(apcArgv[1], NULL, 0); */
		rSwCtrlInfo.u4Data = 0U;
		...
		rStatus = kalIoctl(prGlueInfo,
				...
				&rSwCtrlInfo, (UINT_32)sizeof(rSwCtrlInfo),
		...
		i4BytesWritten = (INT_32)snprintf(pcCommand,
					(UINT_64)i4TotalLen, "0x%08x",
...
static int priv_driver_set_sw_ctrl(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
...
static int priv_driver_set_cfg(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	if (i4Argc >= 3) {
		...
		rStatus = kalIoctl(prGlueInfo,
				wlanoidSetKeyCfg, &rKeyCfgInfo,
				(UINT_32)sizeof(rKeyCfgInfo),
				FALSE, FALSE, TRUE, &u4BufLen);
...
static int priv_driver_get_cfg(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	if (i4Argc >= 2) {
		/* by wlanoid ? */
		if (wlanCfgGet(prAdapter, apcArgv[1], aucValue, Str_Null, 0U)
			== WLAN_STATUS_SUCCESS) {
			kalStrnCpy(pcCommand, aucValue, WLAN_CFG_VALUE_LEN_MAX);
			i4BytesWritten = (INT_32)kalStrnLen(pcCommand,
...
static int priv_driver_set_chip_config(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	u4CmdLen = (UINT_32)kalStrnLen(pcCommand, (UINT_64)i4TotalLen);
	u4PrefixLen = (UINT_32)(kalStrLen(CMD_SET_CHIP) + 1U) /*space */;
...
static int priv_driver_get_chip_config(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	u4CmdLen = (UINT_32)kalStrnLen(pcCommand, (UINT_64)i4TotalLen);
	u4PrefixLen = (UINT_32)kalStrLen(CMD_GET_CHIP) + 1U /*space */;

	/* if(i4Argc >= 2) { */
	if (u4CmdLen > u4PrefixLen) {
		rChipConfigInfo.ucRespType =
			(UINT_8)CHIP_CONFIG_TYPE_WO_RESPONSE;
		rChipConfigInfo.ucType = (UINT_8)CHIP_CONFIG_TYPE_ASCII;
		/* rChipConfigInfo.u2MsgSize = kalStrnLen(apcArgv[1],CHIP_CONFIG_RESP_SIZE); */
		rChipConfigInfo.u2MsgSize = (UINT_16)(u4CmdLen - u4PrefixLen);
		/* kalStrnCpy(rChipConfigInfo.aucCmd,apcArgv[1],CHIP_CONFIG_RESP_SIZE); */
		kalStrnCpy(rChipConfigInfo.aucCmd, pcCommand + u4PrefixLen,
		...
		/* Check respType */
		u2MsgSize = (UINT_32)rChipConfigInfo.u2MsgSize;
		...
		if (u2MsgSize > (UINT_32)sizeof(rChipConfigInfo.aucCmd)) {
		...
		if (u2MsgSize > 0U) {
			...
			if (rChipConfigInfo.ucRespType ==
				(UINT_8)CHIP_CONFIG_TYPE_ASCII) {
				i4BytesWritten =
				snprintf(pcCommand + i4BytesWritten,
					(UINT_64)i4TotalLen, "%s",
			       ...
			       INT_32 i4BytesWritten, char *pcCommand)
{
	...
	while (u4Length >= 16U) {
		if (i4TotalLen > i4BytesWritten) {
			i4BytesWritten +=
			    snprintf(pcCommand + i4BytesWritten,
				((UINT_64)i4TotalLen - (UINT_64)i4BytesWritten),
...
static int priv_driver_get_linkspeed(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQueryLinkSpeed, &u4Rate,
			(UINT_32)sizeof(u4Rate), TRUE, TRUE, TRUE, &u4BufLen);

	if (rStatus != WLAN_STATUS_SUCCESS)
		return -1;

	u4LinkSpeed = u4Rate * 100U;
	i4BytesWritten = (INT_32)snprintf(pcCommand, (UINT_64)i4TotalLen,
				"LinkSpeed %u", (unsigned int)u4LinkSpeed);
	DBGLOG(REQ, INFO, "%s: command result is %s\n", __func__, pcCommand);
	return i4BytesWritten;

}				/* priv_driver_get_linkspeed */

static int priv_driver_set_band(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen)
{
	P_ADAPTER_T prAdapter = NULL;
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	if (i4Argc >= 2) {
		...
		if (ucBssIndex >= (UINT_8)BSS_INFO_NUM)
			return -1;
		eBand = BAND_NULL;
		if (ucBand == (UINT_32)CMD_BAND_5G)
			eBand = BAND_5G;
		if (ucBand == (UINT_32)CMD_BAND_2G)
			eBand = BAND_2G4;
...
static int priv_driver_set_txpower(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	...
	if (ai4Setting[0] == 0 && (i4Argc - 1) == 4 /* argc num */) {
		...
		if (ai4Setting[1] == 3 || ai4Setting[1] == 0) {
			if (ai4Setting[2] == 0 || ai4Setting[2] == 1)
				prTxpwr->c2GP2pPwrOffset = (INT_8)ai4Setting[3];
			if (ai4Setting[2] == 0 || ai4Setting[2] == 2)
				prTxpwr->c5GP2pPwrOffset = (INT_8)ai4Setting[3];
		}
		if (ai4Setting[1] == 4 || ai4Setting[1] == 0) {
			if (ai4Setting[2] == 0 || ai4Setting[2] == 1)
				prTxpwr->c2GBowPwrOffset = (INT_8)ai4Setting[3];
			if (ai4Setting[2] == 0 || ai4Setting[2] == 2)
				prTxpwr->c5GBowPwrOffset = (INT_8)ai4Setting[3];
		}
	} else if (ai4Setting[0] == 1 && (i4Argc - 1) == 2) {
		/* ai4Setting[1] : 0 (highest power is used) (default), 1 (lowest power is used) */
		prTxpwr->ucConcurrencePolicy = (UINT_8)ai4Setting[1];
		...
		if (ai4Setting[1] <= 14 && ai4Setting[1] > 0) {
...
static int priv_driver_set_country(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 2) {
		...
		aucCountry[0] = (UINT_8)apcArgv[1][0];
		aucCountry[1] = (UINT_8)apcArgv[1][1];

		rStatus = kalIoctl(prGlueInfo, wlanoidSetCountryCode,
			&aucCountry[0], 2U, FALSE, FALSE, TRUE, &u4BufLen);
...
static int priv_driver_set_miracast(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 2) {
		...
		prMsgWfdCfgUpdate = cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
			(UINT_32)sizeof(MSG_WFD_CONFIG_SETTINGS_CHANGED_T));
		...
		if (prMsgWfdCfgUpdate != NULL) {
			...
			if (ucMode == (UINT_32)MIRACAST_MODE_OFF) {
				prWfdCfgSettings->ucWfdEnable = 0U;
				snprintf(pcCommand, (UINT_64)i4TotalLen,
					CMD_SET_CHIP " mira 0");
			} else if (ucMode == (UINT_32)MIRACAST_MODE_SOURCE) {
				prWfdCfgSettings->ucWfdEnable = 1U;
				snprintf(pcCommand, (UINT_64)i4TotalLen,
					CMD_SET_CHIP " mira 1");
			} else if (ucMode == (UINT_32)MIRACAST_MODE_SINK) {
				prWfdCfgSettings->ucWfdEnable = 2U;
				snprintf(pcCommand, (UINT_64)i4TotalLen,
					CMD_SET_CHIP " mira 2");
			} else {
				prWfdCfgSettings->ucWfdEnable = 0U;
			...
			cresult = priv_driver_set_chip_config(prNetDev,
				pcCommand, i4TotalLen);
			if (cresult < 0)
				DBGLOG(REQ, LOUD, "Set chip config fail\n");
		} /* prMsgWfdCfgUpdate */
		else {
			ASSERT(NULL);
			i4BytesWritten = -1;
...
#if CFG_SUPPORT_SNIFFER
static int priv_driver_set_monitor(IN struct net_device *prNetDev,
			IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	P_ADAPTER_T prAdapter = NULL;
	...
	if (i4Argc >= 5) {
		...
		eBand = (ucPriChannel <= 14U) ? BAND_2G4 : BAND_5G;
		...
		if (fgIsLegalChannel == FALSE) {
			i4BytesWritten = snprintf(pcCommand,
				(UINT_64)i4TotalLen,
				"Illegal primary channel %d", ucPriChannel);
			return i4BytesWritten;
		}
		...
		case 160U:
			...
			if (ucPriChannel >= 36U && ucPriChannel <= 64U)
				ucChannelS2 = 50U;
			else if (ucPriChannel >= 100U && ucPriChannel <= 128U)
				ucChannelS2 = 114U;
			else
				fgError = TRUE;
			break;
		...
		case 80U:
			...
			if (ucPriChannel >= 36U && ucPriChannel <= 48U)
				ucChannelS1 = 42U;
			else if (ucPriChannel >= 52U && ucPriChannel <= 64U)
				ucChannelS1 = 58U;
			else if (ucPriChannel >= 100U && ucPriChannel <= 112U)
				ucChannelS1 = 106U;
			else if (ucPriChannel >= 116U && ucPriChannel <= 128U)
				ucChannelS1 = 122U;
			else if (ucPriChannel >= 132U && ucPriChannel <= 144U)
				ucChannelS1 = 138U;
			else if (ucPriChannel >= 149U && ucPriChannel <= 161U)
				ucChannelS1 = 155U;
			else
				fgError = TRUE;
			break;
		...
		case 40U:
			...
			ucSco = (ucExt != 0U) ?
				(UINT_8) CHNL_EXT_SCA : (UINT_8) CHNL_EXT_SCB;
			break;
		...
		fgEnable = (ucEnable != 0U) ? TRUE : FALSE;
		...
		rStatus = kalIoctl(prGlueInfo,
				wlanoidSetMonitor,
				&rMonitorSetInfo,
				(UINT_32)sizeof(rMonitorSetInfo),
				FALSE, FALSE, TRUE, &u4BufLen);

		i4BytesWritten =
		snprintf(pcCommand, (UINT_64)i4TotalLen,
			"set monitor config %s",
			(rStatus == WLAN_STATUS_SUCCESS) ? "success" : "fail");

		return i4BytesWritten;
	}

	i4BytesWritten = snprintf(pcCommand, (UINT_64)i4TotalLen,
			...
			IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 3) {
		...
		u4Ret = (UINT_32)kalkStrtou32(apcArgv[1], 0,
				(PUINT_32)&(prNdisReq->ndisOidContent[0]));
		if (u4Ret != 0U)
			DBGLOG(REQ, LOUD, "parse rSwCtrlInfo error u4Ret=%d\n", u4Ret);
		u4Ret = (UINT_32)kalkStrtou32(apcArgv[2], 0,
		...
		prNdisReq->inNdisOidlength = 8U;
		prNdisReq->outNdisOidLength = 8U;
		rStatus = (WLAN_STATUS)priv_set_ndis(prNetDev,
						prNdisReq, &u4BufLen);

		if (rStatus != WLAN_STATUS_SUCCESS) {
			i4BytesWritten = snprintf(pcCommand,
				(UINT_64)i4TotalLen, "Exec result Fail");
			return -1;
		}

		i4BytesWritten = snprintf(pcCommand,
			(UINT_64)i4TotalLen, "Exec result OK");
		return i4BytesWritten;
	}
...
INT_32 priv_driver_cmds(IN struct net_device *prNetDev, IN PCHAR pcCommand, IN INT_32 i4TotalLen)
{
...

	{
		...
		/* Mediatek private command */
			...
			rstatus = kalIoctl(prGlueInfo,
				 ...
				 (PVOID) pcCommand, (UINT_32)i4TotalLen,
				 FALSE, FALSE, TRUE,
				 (UINT_32 *)(&i4BytesWritten));

			if (rstatus != WLAN_STATUS_SUCCESS)
				DBGLOG(REQ, WARN, "status:%d\n", rstatus);
			...
			for (i = 0; i < CFG_BATCH_MAX_MSCAN; i++) {
				g_rEventBatchResult[i].ucScanCount =
					(UINT_8)i + 1U;/* for get which mscan */
				rstatus = kalIoctl(prGlueInfo,
					 wlanoidQueryBatchScanResult,
					 (PVOID)&g_rEventBatchResult[i],
					 (UINT_32)sizeof(EVENT_BATCH_RESULT_T),
					 TRUE, TRUE, TRUE, &u4BufLen);
				if (rstatus != WLAN_STATUS_SUCCESS)
					DBGLOG(REQ, WARN,
						"status:%d\n", rstatus);
			...
			rstatus = batchConvertResult(&g_rEventBatchResult[0],
				pcCommand,
				(UINT_32)i4TotalLen,
				(PUINT_32)&i4BytesWritten);
	...
	if (i4BytesWritten >= 0) {
		...
		if (i4BytesWritten >= i4TotalLen) {
			...
			pcCommand[i4BytesWritten] = (INT_8)'\0';
			i4BytesWritten++;
		}
	}
