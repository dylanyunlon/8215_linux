...
#define KAL_BSS_NUM             (3U)
#define KAL_P2P_NUM             (1U)
...
#define RADIOTAP_FIELD_TSFT			BIT(0U)
#define RADIOTAP_FIELD_FLAGS		BIT(1U)
#define RADIOTAP_FIELD_RATE			BIT(2U)
#define RADIOTAP_FIELD_CHANNEL		BIT(3U)
#define RADIOTAP_FIELD_ANT_SIGNAL	BIT(5U)
#define RADIOTAP_FIELD_ANT_NOISE	BIT(6U)
#define RADIOTAP_FIELD_ANT			BIT(11U)
#define RADIOTAP_FIELD_MCS			BIT(19U)
#define RADIOTAP_FIELD_AMPDU		BIT(20U)
#define RADIOTAP_FIELD_VHT			BIT(21U)
#define RADIOTAP_FIELD_VENDOR       BIT(30U)

#define RADIOTAP_LEN_VHT			48U
...
#define RADIOTAP_LEN_LEGACY			26U
...
#define KAL_ACQUIRE_MUTEX2(_prAdapter, _rLockCategory)  \
		kalAcquireMutex2(((P_ADAPTER_T)_prAdapter)->prGlueInfo, _rLockCategory)

#define KAL_RELEASE_MUTEX2(_prAdapter, _rLockCategory)  \
		kalReleaseMutex2(((P_ADAPTER_T)_prAdapter)->prGlueInfo, _rLockCategory)
...
#define kalMemCopy(pvDst, pvSrc, u4Size) \
		memcpy((void *)(pvDst), (const void *)(pvSrc), (u4Size))
...
#define kalDevSetPowerState(_prGlueInfo, _ePowerMode) \
	glSetPowerState(_prGlueInfo, _ePowerMode)
#else
#define kalDevSetPowerState(_prGlueInfo, _ePowerMode)
#endif
...
*/
/*----------------------------------------------------------------------------*/
#define kalSendComplete(_prGlueInfo, _pvPacket, _status)   \
	    kalSendCompleteAndAwakeQueue(_prGlueInfo, _pvPacket)
...
*/
/*----------------------------------------------------------------------------*/
#define kalQueryBufferPointer(_prGlueInfo, _pvPacket)     \
	    ((PUINT_8)((struct sk_buff *)_pvPacket)->data)
...
*/
/*----------------------------------------------------------------------------*/
#define kalQueryValidBufferLength(_prGlueInfo, _pvPacket)     \
	    ((UINT_32)((struct sk_buff *)_pvPacket)->end -  \
	     (UINT_32)((struct sk_buff *)_pvPacket)->data)
...
*/
/*----------------------------------------------------------------------------*/
#define kalCopyFrame(_prGlueInfo, _pvPacket, pucDestBuffer)   \
	do { \
		struct sk_buff *skb = (struct sk_buff *)_pvPacket; \
...
WLAN_STATUS
kalProcessRxPacket(IN P_GLUE_INFO_T prGlueInfo, IN PVOID pvPacket,
...
VOID kalUpdateRxCSUMOffloadParam(IN PVOID pvPacket,
...
WLAN_STATUS kalFirmwareLoad(IN P_GLUE_INFO_T prGlueInfo, OUT PVOID prBuf,
	IN UINT_32 u4Offset, OUT PUINT_32 pu4Size);

WLAN_STATUS kalFirmwareSize(IN P_GLUE_INFO_T prGlueInfo,
...
ENUM_BOW_DEVICE_STATE kalGetBowState(IN P_GLUE_INFO_T prGlueInfo,
	IN UINT_8 aucPeerAddress[6]);

BOOLEAN kalSetBowState(IN P_GLUE_INFO_T prGlueInfo,
...
BOOLEAN kalBowFrameClassifier(IN P_GLUE_INFO_T prGlueInfo,
...
INT_32 kalReadToFile(const char *pucPath, PUINT_8 pucData,
	UINT_32 u4Size, PUINT_32 pu4ReadSize);
...
kalIndicateBssInfo(IN P_GLUE_INFO_T prGlueInfo,
		...
		IN UINT_32 u4FrameLen, IN UINT_8 ucChannelNum,
...
BOOLEAN kalMetCheckProfilingPacket(IN P_GLUE_INFO_T prGlueInfo,
...
UINT_32 kalFileWrite(struct file *file, unsigned long long offset,
	unsigned char *data, unsigned int size);
...
kalSetNetAddress(IN P_GLUE_INFO_T prGlueInfo,
		 IN UINT_8 ucBssIdx,
		 IN PUINT_8 pucIPv4Addr, IN UINT_32 u4NumIPv4Addr,
