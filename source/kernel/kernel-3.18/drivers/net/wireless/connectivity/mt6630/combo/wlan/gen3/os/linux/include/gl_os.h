...
/* number of tx queue for support multi-queue h/w  */
#define CFG_MAX_TXQ_NUM                     4U
...
#include <../drivers/misc/atc/include/msdc/aee.h>
#else
...
********************************************************************************
*/
#define GLUE_FLAG_HALT                  BIT(0U)
#define GLUE_FLAG_INT                   BIT(1U)
#define GLUE_FLAG_OID                   BIT(2U)
#define GLUE_FLAG_TIMEOUT               BIT(3U)
#define GLUE_FLAG_TXREQ                 BIT(4U)
#define GLUE_FLAG_SUB_MOD_MULTICAST     BIT(7U)
#define GLUE_FLAG_FRAME_FILTER          BIT(8U)
#define GLUE_FLAG_FRAME_FILTER_AIS      BIT(9U)
...
#define GLUE_FLAG_RX            BIT(10U)
#define GLUE_FLAG_TX_CMD_DONE   BIT(11U)
#define GLUE_FLAG_HIF_TX        BIT(12U)
#define GLUE_FLAG_HIF_TX_CMD    BIT(13U)
#define GLUE_FLAG_RX_TO_OS      BIT(14U)
#define GLUE_FLAG_HIF_FW_OWN    BIT(15U)
#define GLUE_FLAG_HAL_MCR_RD	BIT(16U)
#define GLUE_FLAG_HAL_MCR_WR	BIT(17U)
...
struct _GLUE_INFO_T {
	...
	struct task_struct *arMutexOwner[MUTEX_NUM];
	UINT_32 arMutexCount[MUTEX_NUM];
...
#define GLUE_ACQUIRE_SPIN_LOCK(_prGlueInfo, rLockCategory)   \
	    { \
		if ((rLockCategory) < SPIN_LOCK_NUM) \
			spin_lock_bh( \
			&((_prGlueInfo)->rSpinLock[rLockCategory])); \
	    }
#define GLUE_RELEASE_SPIN_LOCK(_prGlueInfo, rLockCategory)   \
	    { \
		if ((rLockCategory) < SPIN_LOCK_NUM) \
			spin_unlock_bh( \
			&((_prGlueInfo)->rSpinLock[rLockCategory])); \
...
#define GLUE_SET_PKT_FLAG(_p, _flag) \
	(GLUE_GET_PKT_PRIVATE_DATA(_p)->u2Flag |= ((UINT_16)BIT(_flag)))
...
#define GLUE_SET_PKT_FLAG_PROF_MET(_p) \
	(GLUE_GET_PKT_PRIVATE_DATA(_p)->ucProfilingFlag |= ((UINT_8)BIT(0U)))
...
#define GLUE_CHK_DEV(prDev) \
	(((prDev) != NULL && \
	*((P_GLUE_INFO_T *) netdev_priv(prDev)) != NULL) ? TRUE : FALSE)

#define GLUE_CHK_PR2(prDev, pr2) \
	((GLUE_CHK_DEV(prDev) == TRUE && (pr2) != NULL) ? TRUE : FALSE)

#define GLUE_CHK_PR3(prDev, pr2, pr3) \
	((GLUE_CHK_PR2(prDev, pr2) == TRUE && (pr3) != NULL) ? TRUE : FALSE)

#define GLUE_CHK_PR4(prDev, pr2, pr3, pr4) \
	((GLUE_CHK_PR3(prDev, pr2, pr3) == TRUE && \
	(pr4) != NULL) ? TRUE : FALSE)
...
#define GLUE_GET_INDEPENDENT_PKT(_p)    \
	(GLUE_GET_PKT_PRIVATE_DATA(_p)->fgIsIndependentPkt != FALSE)
