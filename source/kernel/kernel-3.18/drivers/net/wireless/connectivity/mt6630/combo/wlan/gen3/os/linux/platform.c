...
*/
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
#define WIFI_NVRAM_FILE_NAME   "/etc/nvram/WIFI"
#else
...
void wlanRegisterNotifier(void)
{
	...
	temp_rstatus = register_inetaddr_notifier(&inetaddr_notifier);
	if (temp_rstatus < 0) {
		DBGLOG(REQ, WARN,
			"wlanRegisterNotifier error: %d.\n", temp_rstatus);
	}
#if 0				/* CFG_SUPPORT_PASSPOINT */
...
void wlanUnregisterNotifier(void)
{
	...
	temp_rstatus = unregister_inetaddr_notifier(&inetaddr_notifier);
	if (temp_rstatus < 0) {
		DBGLOG(REQ, WARN,
			"wlanUnregisterNotifier error: %d.\n", temp_rstatus);
	}
#if 0				/* CFG_SUPPORT_PASSPOINT */
...
static int nvram_read(char *filename, char *buf, ssize_t len, int offset)
{
	...
	do {
		...
		retLen = (INT_32)__vfs_read(fd, p, (UINT_64)len, &pos);
		if (retLen < 0)
			DBGLOG(INIT, ERROR, "[nvram_read] : read failed!! Error code: %d\n", retLen);

	} while (FALSE);

	tmp_close_status = filp_close(fd, NULL);
	if (tmp_close_status < 0) {
		DBGLOG(INIT, ERROR,
			"[nvram_read] : filp_close failed: %d\n",
			tmp_close_status);
	}
...
static int nvram_write(char *filename, char *buf, ssize_t len, int offset)
{
	...
	fd = filp_open(filename, (UINT_32)O_WRONLY | (UINT_32)O_CREAT, 0644);
	...
	do {
		...
		retLen = (INT_32)__vfs_write(fd, p, (UINT_64)len, &pos);
		if (retLen < 0)
	...
	if (tmp_close_status < 0) {
		DBGLOG(INIT, ERROR,
			"[nvram_write] : filp_close failed: %d\n",
			tmp_close_status);
	}
...
{
	// Autochips: Our NVRAM file is "/etc/nvram/WIFI"
	//char filname[] = "/data/nvram/APCFG/APRDEB/WIFI";
	char filname[] = "/etc/nvram/WIFI";

	if (pu2Data == NULL)
		return FALSE;

	if (nvram_read(&filname[0],
		       (char *)pu2Data,
		       (ssize_t)sizeof(unsigned short),
		       (INT_32)u4Offset) != (INT_32)sizeof(unsigned short)) {
		return FALSE;
	} else {
		return TRUE;
	}
