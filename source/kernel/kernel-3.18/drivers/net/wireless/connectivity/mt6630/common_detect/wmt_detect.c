...
unsigned int gWmtDetectDbgLvl = WMT_DETECT_LOG_WARN;

#ifdef MTK_WCN_COMBO_CHIP_SUPPORT
inline unsigned int wmt_plat_get_soc_chipid(void)
{
	if (!strcmp(current->comm, "blkid")) {
		WMT_DETECT_INFO_FUNC("block blkid open\n");
		return -EPERM;
	}
...
static int wmt_detect_driver_init(void)
{
...
#if !(MTK_WCN_REMOVE_KO)
	ret = sdio_detect_init();
	if (ret) {
		WMT_DETECT_ERR_FUNC("sdio_detect_init fail ret:%d\n", ret);
		goto err2;
	}
#endif
	...
	ret = cdev_add(&gWmtDetectCdev, devID, WMT_DETECT_DEV_NUM);
	if (ret) {
...
err1:
#endif
	WMT_DETECT_ERR_FUNC("fail ret(%d)\n", ret);
