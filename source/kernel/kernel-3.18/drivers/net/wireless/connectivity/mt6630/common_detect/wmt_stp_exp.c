...
#undef DFT_TAG
#endif
#define DFT_TAG         "[CNN][WMT_STP_EXP-6630]"

#define WMT_STP_EXP_INFO_FUNC(fmt, arg...) \
	pr_info(DFT_TAG "[I]%s,%d: " fmt, __func__, __LINE__, ##arg)

#define WMT_STP_EXP_WARN_FUNC(fmt, arg...) \
	pr_warn(DFT_TAG "[W]%s,%d:WARN! " fmt, __func__, __LINE__, ##arg)

#define WMT_STP_EXP_ERR_FUNC(fmt, arg...)  \
...
MTK_WCN_STP_TRG_RESET mtk_wcn_stp_trg_reset_f = NULL;
...
UINT32 mtk_wcn_stp_exp_cb_reg(P_MTK_WCN_STP_EXP_CB_INFO pStpExpCb)
{
	...
	mtk_wcn_stp_trg_reset_f = pStpExpCb->stp_trg_reset_cb;
...
INT32 mtk_wcn_stp_trg_reset(VOID)
{
	...
	if (mtk_wcn_stp_trg_reset_f)
		ret = (*mtk_wcn_stp_trg_reset_f) ();
	else
		WMT_STP_EXP_ERR_FUNC("mtk_wcn_stp_trg_reset_f cb is null\n");
