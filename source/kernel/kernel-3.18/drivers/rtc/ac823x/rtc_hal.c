...
extern unsigned long rtc_reg_base1;
...
int RTCIsTimeRight(void)
{
	...
	value = RTCYearRead();
	pr_debug("check: year = %d\n", value);

	if (!(value >= 0 && value <= 127)) {
		pr_err("check: year ERROR\n");
		return -1;
	}

	value = RTCMonthRead();
	pr_debug("check: Month = %d\n", value);

	if (!(value >= 0 && value <= 12)) {
		pr_err("check: Month ERROR\n");
		return -1;
	}

	value = RTCDOMRead();
	pr_debug("check: Day = %d\n", value);

	if (!(value >= 0 && value <= 31)) {
		pr_err("check: Day ERROR\n");
		return -1;
	}

	value = RTCHourRead();
	pr_debug("check: Hour = %d\n", value);

	if (!(value >= 0 && value <= 23)) {
		pr_err("check: Hour ERROR\n");
		return -1;
	}

	value = RTCMinRead();
	pr_debug("check: Min = %d\n", value);

	if (!(value >= 0 && value <= 59)) {
		pr_err("check: Min ERROR\n");
		return -1;
	}

	value = RTCSecRead();
	pr_debug("check: Sec = %d\n", value);

	if (!(value >= 0 && value <= 59)) {
		pr_err("check: Sec ERROR\n");
		return -1;
	}

	pr_debug("check: time is RIGHT\n");
	return 0;
...
int RTCHWInit(void)
{
	...
	for (i = 0; i < RTCINTMAX; i++) {
		while (RTC_READ32(RTC_BBPU) & 0x80) {
			...
			if (count > 100) {
				count = 0;
				pr_err("can not read RTC reg, pls check rtc clock.\n");
				break;
		...
		if (0x43 != (uint16_t)(RTC_READ32(RTC_BBPU) >> 8)) {
			...
			CORETORTC();
			pr_debug("rtc is ok to do bus write.\n");
		}
		...
		if (!((0xa357 == (uint16_t)RTC_READ32(RTC_POWERKEY1))
			...
			pr_info("POWERKEY is not fit %d times.\n", (i + 1));
		} else {
			pr_debug("POWERKEY is fit %d times.\n", (i + 1));
			ret = 0;
			break;
		}
		...
		if ((i + 1) == (RTCINTMAX - 1)) {
			...
			if ((0xa357 != (uint16_t)RTC_READ32(RTC_POWERKEY1))
				...
				if (RTCIsTimeRight() != 0) {
					...
					RtcFirstRunChk();
					RTCProtect(1);
					pr_info("set RTC powerkey and value !\n");
				} else {
					RTCProtect(0);
					RtcFirstSetPowerkeys();
					RTCProtect(1);
					pr_info("set RTC powerkey !\n");
				}
