...
void start_monitor_thermal_zone(struct thermal_zone_device *tz, int flag)
{
	mutex_lock(&tz->lock);

	if (tz->passive && flag)
		thermal_zone_device_set_polling(tz, tz->passive_delay);
	else if (tz->polling_delay && flag)
		thermal_zone_device_set_polling(tz, tz->polling_delay);
	else
		thermal_zone_device_set_polling(tz, 0);
...
EXPORT_SYMBOL_GPL(start_monitor_thermal_zone);
				     ...
				     unsigned int weight)
{
	...
	/* fix bug: compare unsigned long to -1 problem */
	#if 0
