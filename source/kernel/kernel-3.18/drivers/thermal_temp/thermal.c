...
int thermal_clk_select(UINT32 therm_ck, UINT32 therm_slow_ck)
{
	...
	temp = DRV_Reg32(CKGEN_BASE + 0x18);
	temp &= (~(3 << 22));
	temp |= (therm_ck << 22);
	DRV_WriteReg32(CKGEN_BASE + 0x18, temp);
...
int thermal_clk_enable(UINT32 therm_ck_flag, UINT32 therm_slow_ck_flag)
{
	...
	if (therm_ck_flag) {
		temp = DRV_Reg32(CKGEN_BASE + 0xA0);
		temp |= (therm_ck_flag << 18);
		DRV_WriteReg32(CKGEN_BASE + 0xA0, temp);
	} else {
		temp = DRV_Reg32(CKGEN_BASE + 0xA0);
		temp &= (~(therm_ck_flag << 18));
		DRV_WriteReg32(CKGEN_BASE + 0xA0, temp);
...
int thermal_clk_reset(UINT32 therm_reset)
{
	...
	if (therm_reset) {
		temp = DRV_Reg32(CKGEN_BASE + 0xBC);
		temp |= (3 << 18);
		DRV_WriteReg32(CKGEN_BASE + 0xBC, temp);
	} else {
		temp = DRV_Reg32(CKGEN_BASE + 0xBC);
		temp &= (~(3 << 18));
		DRV_WriteReg32(CKGEN_BASE + 0xBC, temp);
...
int thermal_slt_test(void)
{
	...
	BIM_EnableIrq(VECTOR_PTP_THERM);  // zplee
	#endif
...
extern volatile int enable_output;
extern volatile int log_time;
...
int thermal_get_temp(void)
{
	unsigned int temp = 0;
    // 0xA0000
	thm_reg_base = ioremap(0x100A0000, 0x1000);
	// 0x24000
	pdwnc_reg_base = ioremap(0x10024000, 0x1000);
	// 0x00000
	ckgen_reg_base = ioremap(0x10000000, 0x1000);
	// 0x54000
	efuse_reg_base = ioremap(0x10054000, 0x1000);
