...
static phys_addr_t mem_pa;

phys_addr_t trusty_log_get_pa(void)
{
    return mem_pa;
}
EXPORT_SYMBOL(trusty_log_get_pa);
static int trusty_log_probe(struct platform_device *pdev)
{
    ...
    mem_pa = (u32)(pa) + 0x20000000;
#elif defined(CONFIG_ATC_PLATFORM_ac83xx)
    mem_pa = pa;
#else
#endif

    pr_info("memlog pa: 0x%p \n", mem_pa);
