...
extern phys_addr_t trusty_log_get_pa(void);
			  ...
			  pgprot_t pgprot)
{
    ...
    ////Before we step into TEE, flush the cache~
	...
	if (SMC_IS_FASTCALL(smcnr)) {
		return trusty_fast_call32(dev, smcnr,
					  (u32)pg_inf.attr+0x20000000,
					  (u32)(pg_inf.attr >> 32)&0xffffff00, size);
	} else {
...
#else
    printk("debug for ac8317 inf->attr : 0x%p \n", pg_inf.attr);
    if (SMC_IS_FASTCALL(smcnr)) {
    	return trusty_fast_call32(dev, smcnr,
    				  (u32)pg_inf.attr,
