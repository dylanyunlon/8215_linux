...
#define UART_NR                 (6)//(6)
struct clk* clk_ac8317_uart[UART_NR];
struct clk* clk_ac8317_uart_select[UART_NR];

struct clk* clk_ac8317_uart_parent[UART_NR];
...
static ulong uart_base_vaddr[UART_NR];
static u32 uart_irq[UART_NR];
...
#define UART_READ32(REG)            __raw_readl(__io(uart_base_vaddr[0] + REG))
#define UART_WRITE32(VAL, REG)      __raw_writel(VAL, __io(uart_base_vaddr[0]  + REG))
#define CKGEN_VIRT_READ32(REG)            __raw_readl(__io(uart_base_vaddr[0] + REG))
#define CKGEN_VIRT_WRITE32(VAL, REG)      __raw_writel(VAL, __io(uart_base_vaddr[0]  + REG))
...
static void _ac83xx_uart_hwinit(void)
{
	...
	for(i = 0; i < UART_NR; i++)  {
		//clk_ac8317_uart_parent[i] = clk_get(NULL,"syspll_d20"); clk27m_ck
		clk_ac8317_uart_parent[i] = clk_get(NULL,"clk27m_ck");
		...
		clk_prepare(clk_ac8317_uart_select[i]);
		//clk_enable(clk_ac8317_uart_select[i]);
	}
	#if defined (CONFIG_ATC_PRJ_ac823x_adas)
		for(i = 2; i < UART_NR; i++) {
			if(clk_ac8317_uart_select[i]) {
				//clk_disable(clk_ac8317_uart_select[i]);
			  	clk_unprepare(clk_ac8317_uart_select[i]);
			}
		}
	
	#endif
	UART_WRITE32(0x3E2, UART0_COMMCTRL);
...
static uint32_t  uart_soucce_clk = 27000000;
		...
		int datalen, int stop, int parity)
{
	...
	uart_err("uartx Set uCOMMCTRL =0x%x,baud=%d,\r\n", uCOMMCTRL, baud);
	if(baud <= 921600)
	{
		...
		uart_err("default baud:uartx Set regCOMMCTRL =0x%x\r\n", temp);
	}
	else
	{
		...
		UART_REG_BITCLR(U_BAUD_RATE_MASK, uCOMMCTRL);
		UART_REG_BITSET(SETBAUD(0X7), uCOMMCTRL);
		UART_REG_BITCLR(U_RATE_DIVISOR_MASK, uCOMMCTRL);

		UART_REG_BITSET(U_SET_RATE_DIVISOR(u4Rate_Div), uCOMMCTRL);
		UART_REG_BITCLR(0x000000FF, uCOMMCTRL);
		temp = UART_READ32(uCOMMCTRL);
		uart_err("test baud:uartx Set regCOMMCTRL =0x%x\r\n", temp);
	}
}
...
static int _ac83xx_uart_startup(struct uart_port *port)
{
	...
	if (_irq_allocated[mport->nport] == 1) 
	{
		if(mport->nport!=0) 
		{
			...
			restatus = UART_READ32(0x10);
			while ((restatus&0x80) == 0x80) {
				restatus = UART_READ32(0x10);
				tmp = UART_READ32(UART0_DATA_BYTE);
				UART_WRITE32(UART_READ32(0x10)&0xFFFFFFDF, 0x10);
				mt33xx_mask_ack_bim_irq(118);
			}
			retval = request_irq(118,_ac83xx_uart0_interrupt, IRQF_SHARED,"AC83xx Serial", port);
			if (retval) {
				_irq_allocated[mport->nport]--;
				return retval;
			}
...
static int _ac83xx_uart_request_port(struct uart_port *port)
{
	...
	//pv_region = request_mem_region(port->mapbase, AC83XX_UART_SIZE,"AC83XX Uart IO Mem");
 ...
 */
#if 1
void _ac83xx_uart_console_write(struct console *co, const char *s, unsigned int count)
{
	struct ac83xx_uart_port *mport;
	struct uart_port *port;
...
#define CONSOLE_RETRY  (5000)
	for(i=0; i< (int)count; i++)
	{
		...
		spin_lock_irqsave(&atc_console_lock, flags);
		mport->fn_write_byte(mport->nport, s[i]);
		spin_unlock_irqrestore(&atc_console_lock, flags);
...
static void set_uart_default_settings(int idx)
{
	...
	switch (idx) {
	case 0:
		node = of_find_compatible_node(NULL, NULL, "atc,mt33xx-serial0");
		break;

	case 1:
		node = of_find_compatible_node(NULL, NULL, "atc,mt33xx-serial01");
		break;

	case 2:
		node = of_find_compatible_node(NULL, NULL, "atc,mt33xx-serial02");
		break;
	...
	if (node) {
		uart_base_vaddr[idx] = (ulong)of_iomap(node, 0);
		uart_irq[idx] = irq_of_parse_and_map(node, 0);

		for(i=0; i<UART_NR; i++)
		{
			_ac83xx_uart_ports[i].port.mapbase = uart_base_vaddr[idx];
			_ac83xx_uart_ports[i].port.membase = (void *)uart_base_vaddr[idx];
			//_ac83xx_uart_ports[idx].port.irq = uart_irq[idx];
		}
...
static int __init _ac83xx_uart_console_init(void)
{
	...
	spin_lock_init(&atc_console_lock);
	set_uart_default_settings(0);
...
struct task_struct *uart_probe_task = NULL;

static s32 ac83xx_uart_probe_snd(void *arg)
{
	...
	do {
		...
		value = UART_READ32(UART0_COMMCTRL);
		if (!(value & 0x00000040)) {
			UART_WRITE32(0x3E2, UART0_COMMCTRL);
		}
...
static int _ac83xx_uart_probe(struct platform_device *pdev)
{
	...
	clk_ac8317_uart_select[0] = devm_clk_get(&pdev->dev, "uart-select1");
	clk_ac8317_uart_select[1] = devm_clk_get(&pdev->dev, "uart-select2");
	clk_ac8317_uart_select[2] = devm_clk_get(&pdev->dev, "uart-select3");
	clk_ac8317_uart_select[3] = devm_clk_get(&pdev->dev, "uart-select4");
	clk_ac8317_uart_select[4] = devm_clk_get(&pdev->dev, "uart-select5");
	clk_ac8317_uart_select[5] = devm_clk_get(&pdev->dev, "uart-select6");
	//clk_ac8317_uart[5] = devm_clk_get(&pdev->dev, "uart5");
	for(i = 0;i < UART_NR;i++)  {
		if((clk_ac8317_uart[i] == NULL) || (clk_ac8317_uart_select[i] == NULL))   {
			pr_err("get uart %d clk error \r\n ",i);
			return -1;
		}
	}

	#if 1
	uart_probe_task = kthread_create(ac83xx_uart_probe_snd, (void *)NULL, "uart2ndprobe");
	wake_up_process(uart_probe_task);
	uart_err("start thread to check uart mode! \n");
	#endif
...
static const struct of_device_id ac83xx_uart_of_ids[] = {
		{ .compatible = "Autochips,ac823x-serial", },
		{ .compatible = "atc,mt33xx-serial0", },
		{}
};
