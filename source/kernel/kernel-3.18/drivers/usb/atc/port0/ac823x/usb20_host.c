...
#else
/*add by zwx 
/**	GPIO0 used for USB OTG ID
/**  When ID pin is High,OTG should work as b-device;
/**  When ID pin is Low,OTG should work as A-device.
**/
	iddig_state = __gpio_get_value(0);
	if(iddig_state == 1)
		usb_is_host = false;
	else if(iddig_state == 0)
		usb_is_host = true;
#endif//end #ifndef CONFIG_USB_OTG
	DBG(0, "usb_is_host = %d\n", usb_is_host);
	return usb_is_host;
}
...
void otg_a_device_init(void)
{
	...
	//wake_lock(&atc_musb->usb_lock);//zwx
	//musb_platform_set_vbus(atc_musb, 1);
	musb_platform_set_power(atc_musb, 1);
	...
	USBPHY_SET32(0x10, 0x50000);
	USBPHY_SET32(0x18, 0xe0000);
	/**zwx OTG mode should not force iddig(bit 1 in phy reg offset 0x6d)*/
	USBPHY_CLR8(0x6d, 0x02);
	DBG(0, "force PHY to host mode, 0x6d=%x, 0x6c=%x\n", USBPHY_READ8(0x6d), USBPHY_READ8(0x6c));
	atc_musb->xceiv->state = OTG_STATE_A_HOST;
	musb_start(atc_musb);
	MUSB_HST_MODE(atc_musb);
...
void otg_b_device_init(void)
{
	...
	/**work as b-device,vbus should be turned off*/
	musb_platform_set_power(atc_musb, 0);
	if(musb_resume_flag == false)
		msleep(100);
	...
	USBPHY_CLR8(0x6c, 0x2e);
	USBPHY_SET8(0x6d, 0x3f);
	DBG(0, "force PHY to idle, 0x6d=%x, 0x6c=0x%04x\n", USBPHY_READ8(0x6d), USBPHY_READ32(0x6c));
	mdelay(5);
#endif
	/* VBUSVALID=0, AVALID=1, BVALID=1, SESSEND=0, IDDIG=X, IDPULLUP=1 */
	
	USBPHY_CLR8(0x6c, 0x11);
	USBPHY_SET8(0x6c, 0x0c);
	USBPHY_CLR8(0x6d, 0x3f);
	USBPHY_SET8(0x6d, 0x0c);
	DBG(0, "force PHY to idle, 0x6d=%x, 0x6c=0x%04x\n", USBPHY_READ8(0x6d), USBPHY_READ32(0x6c));
	musb_stop(atc_musb);
	/* stop dma controller */
	if (is_dma_capable() && atc_musb->dma_controller) {
...
static void musb_otg_work(struct work_struct *data)
{
	unsigned long flags;
...
#ifdef	CONFIG_USB_OTG
	INIT_DELAYED_WORK(&musb->id_pin_work, musb_otg_work);
#else
