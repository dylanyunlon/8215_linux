...
#endif
	if (atc_musb->is_host) {
			DBG(0, "now is host return false\n");
			return false;
		}
...
static int ac_usb_set_power(struct musb *musb, int on)
{
	int err;
	...
	if(GPIO_MultiFun_Get(PIN_1_GPIO1) != PINMUX_LEVEL_GPIO_END_FLAG)
...
static void ac83xx_usb_suspend(struct musb* musb)
{
...
/***	add by zwx for certification of CTS feature 
***	system can't suspend when  system working for CTS.
**		USB as device mode will hold PM lock in order not to suspend system
**/
#ifdef ATC_AOSP_ENHANCEMENT_CTS
...
static void ac83xx_usb_resume(struct musb* musb)
{
	//init PHY REG
	usb_clock_enable();
	usb_phy_recover();
...
{
	printk("[USB0]-- ac83xx m usb suspend --\n");
	//usb_hcd_resume_root_hub(musb_to_hcd(musb));
	//musb_root_disconnect(musb);
	ac83xx_usb_suspend(musb);
	printk("[USB0]-- ac83xx m usb suspend end--\n");
}
...
{
	unsigned long	flags;
	DBG(0,"[USB0]++ac83xx m usb resume++\n");
	spin_unlock_irqrestore(&musb->lock, flags);
	ac83xx_usb_resume(musb);
	spin_lock_irqsave(&musb->lock, flags);
	DBG(0,"[USB0]++ac83xx m usb resume end++\n");
}
