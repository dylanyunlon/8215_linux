...
static void prepare_rx_gpd(u8 *pBuf, u32 data_len, u8 ep_num, u8 ioc)
{
	TGPD *gpd;
...
static void prepare_tx_gpd(u8 *pBuf, u32 data_len, u8 ep_num, u8 zlp, u8 ioc)
{
	TGPD *gpd;
...
void atc_qmu_resume(u8 ep_num, u8 isRx)
{
...
bool atc_is_qmu_enabled(u8 ep_num, u8 isRx)
{
	void __iomem *base = qmu_base;
...
void atc_qmu_enable(struct musb *musb, u8 ep_num, u8 isRx)
{
	...
	if (isRx) {
		...
		/* set rx ioc function enable */
		MGC_WriteQIRQ32(base, MGC_O_QIRQ_IOCDISR, MGC_ReadQIRQ32(base, MGC_O_QIRQ_IOCDISR)& ~(DQMU_M_RQ_DIS_IOC(ep_num)));
		...
		/* set tx ioc function enable */
		MGC_WriteQIRQ32(base, MGC_O_QIRQ_IOCDISR, MGC_ReadQIRQ32(base, MGC_O_QIRQ_IOCDISR)& ~(1<<(ep_num-1)));
...
void atc_qmu_stop(u8 ep_num, u8 isRx)
{
	void __iomem *base = qmu_base;
...
static void atc_qmu_disable(u8 ep_num, u8 isRx)
{
	u32 QCR;
	void __iomem *base = qmu_base;
...
void atc_qmu_insert_task(u8 ep_num, u8 isRx, u8 *buf, u32 length, u8 zlp, u8 ioc)
{
	QMU_INFO("ep_num: %d, isRx: %d, buf: %p, length: %d\n", ep_num, isRx, buf, length);
	if (isRx) {
		/* rx don't care zlp input */
		prepare_rx_gpd(buf, length, ep_num, ioc);
	} else {
		prepare_tx_gpd(buf, length, ep_num, zlp, ioc);
	}
}

void qmu_done_host_rx(struct musb *musb, u8 ep_num)
{
	void __iomem *base = qmu_base;
	...
	QMU_INFO("[RXH]" "%s EP%d, Last=%p, Current=%p, End=%p\n",
	...
	if (gpd == gpd_current) {
		...
		QMU_ERR("[RXH][ERROR] gpd(%p) == gpd_current(%p)\n", gpd, gpd_current);

		QMU_ERR("[RXH][ERROR]EP%d RQCSR=%x, RQSAR=%x, RQCPR=%x, RQLDPR=%x\n",
			ep_num,
			MGC_ReadQMU32(base, MGC_O_QMU_RQCSR(ep_num)),
		...
		QMU_ERR("[RXH][ERROR]QCR0=%x, QCR2=%x, QCR3=%x, QGCSR=%x\n",
		...
		QMU_ERR("[RXH][ERROR]HWO=%d, Next_GPD=%p ,DataBufLen=%d, DataBuf=%p, RecvLen=%d, Endpoint=%d\n",
	...
	if (!gpd || !gpd_current) {
		...
		QMU_ERR
		    ("[RXH][ERROR] EP%d, gpd=%p, gpd_current=%p, ishwo=%d, rx_gpd_last=%p,	RQCPR=0x%x\n",
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		if (rcv_len > buf_len)
			QMU_ERR("[RXH][ERROR] rcv(%d) > buf(%d) AUK!?\n", rcv_len, buf_len);

		QMU_INFO("[RXH]" "gpd=%p ->HWO=%d, Next_GPD=%p, RcvLen=%d, BufLen=%d, pBuf=%p\n",
		...
		urb = next_urb(qh);
		if (unlikely(!urb)) {
			QMU_INFO("[RXH] BOGUS RX%d ready,qh%p\n", ep_num, hw_ep->in_qh);
			atc_qmu_stop(ep_num, 1);
			return;
		}
		if (usb_pipeisoc(urb->pipe)) {
			...
			if (++qh->iso_idx >= urb->number_of_packets) {
				musb_g_advance_schedule(musb, urb, hw_ep, USB_DIR_IN);
				qh->iso_idx = 0;
			}
		}

		if (!TGPD_GET_NEXT(gpd) || !TGPD_GET_DATA(gpd)) {
			QMU_ERR("[RXH][ERROR] EP%d ,gpd=%p\n", ep_num, gpd);
			/* BUG_ON(1); */
			break;
		}
		...
		if (!gpd) {
			QMU_ERR("[RXH][ERROR] !gpd, EP%d ,gpd=%p\n", ep_num, gpd);
			/* BUG_ON(1); */
			break;
		}
	...
	QMU_INFO("[RXH]" "%s EP%d, Last=%p, End=%p, complete\n", __func__,
		 ep_num, Rx_gpd_last[ep_num], Rx_gpd_end[ep_num]);

}

void qmu_done_host_tx(struct musb *musb, u8 ep_num)
{
	void __iomem *base = qmu_base;
	...
	QMU_INFO("[TXH]" "%s EP%d, Last=%p, Current=%p, End=%p\n",
		 __func__, ep_num, gpd, gpd_current, Tx_gpd_end[ep_num]);

	/*gpd_current should at least point to the next GPD to the previous last one. */
	if (gpd == gpd_current) {
		QMU_ERR("[TXH] gpd(%p) == gpd_current(%p)\n", gpd, gpd_current);
		return;
	}

	if (TGPD_IS_FLAGS_HWO(gpd)) {
		QMU_ERR("[TXH] HWO=1, CPR=%x\n", MGC_ReadQMU32(base, MGC_O_QMU_TQCPR(ep_num)));
		/* BUG_ON(1); */
		return;
	}
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		QMU_INFO("[TXH]" "gpd=%p ->HWO=%d, BPD=%d, Next_GPD=%p, DataBuffer=%p, BufferLen=%d\n",
		...
		if (!TGPD_GET_NEXT(gpd)) {
			QMU_ERR("[TXH][ERROR]" "Next GPD is null!!\n");
			/* BUG_ON(1); */
			break;
		}
		...
		urb = next_urb(qh);
		if (unlikely(!urb)) {
			QMU_INFO("[TXH] BOGUS RX%d ready,qh%p\n", ep_num, hw_ep->in_qh);
			atc_qmu_stop(ep_num, 0);
			return;
		}
		if (usb_pipeisoc(urb->pipe)) {
			if (++qh->iso_idx >= urb->number_of_packets) {
				musb_g_advance_schedule(musb, urb, hw_ep, USB_DIR_OUT);
				qh->iso_idx = 0;
			}
		}
	...
	if (gpd != gpd_current && TGPD_IS_FLAGS_HWO(gpd)) {
		...
		QMU_ERR("[TXH][ERROR]" "EP%d TQCSR=%x, TQSAR=%x, TQCPR=%x\n",
			ep_num,
			MGC_ReadQMU32(base, MGC_O_QMU_TQCSR(ep_num)),
			MGC_ReadQMU32(base, MGC_O_QMU_TQSAR(ep_num)),
			MGC_ReadQMU32(base, MGC_O_QMU_TQCPR(ep_num)));

		QMU_ERR("[TXH][ERROR]" "QCR0=%x, QCR2=%x, QCR3=%x, QGCSR=%x\n",
			MGC_ReadQMU32(base, MGC_O_QMU_QCR0),
			MGC_ReadQMU32(base, MGC_O_QMU_QCR2),
			MGC_ReadQMU32(base, MGC_O_QMU_QCR3),
			MGC_ReadQUCS32(base, MGC_O_QUCS_USBGCSR));

		QMU_ERR("[TXH][ERROR]" "HWO=%d, BPD=%d, Next_GPD=%p, DataBuffer=%p, BufferLen=%d, Endpoint=%d\n",
			(u32) TGPD_GET_FLAG(gpd), (u32) TGPD_GET_FORMAT(gpd),
			TGPD_GET_NEXT(gpd), TGPD_GET_DATA(gpd),
			(u32) TGPD_GET_BUF_LEN(gpd), (u32) TGPD_GET_EPaddr(gpd));
	}

	QMU_INFO("[TXH]" "%s EP%d, Last=%p, End=%p, complete\n", __func__,
		 ep_num, Tx_gpd_last[ep_num], Tx_gpd_end[ep_num]);
}

void qmu_done_dev_rx(struct musb *musb, u8 ep_num)
{
	void __iomem *base = qmu_base;
...
void qmu_done_dev_tx(struct musb *musb, u8 ep_num)
{
	void __iomem *base = qmu_base;
	TGPD *gpd = Tx_gpd_last[ep_num];
	TGPD *gpd_current = (TGPD *) (unsigned long)MGC_ReadQMU32(base, MGC_O_QMU_TQCPR(ep_num));
...
void atc_disable_q(struct musb *musb, u8 ep_num, u8 isRx)
{
	void __iomem *mbase = musb->mregs;
	struct musb_hw_ep *hw_ep = musb->endpoints + ep_num;
	void __iomem *epio = hw_ep->regs;
...
void atc_qmu_err_recover(struct musb *musb, u8 ep_num, u8 isRx, bool is_len_err)
{
	struct musb_ep *musb_ep;
	...
	flush_ep_csr(musb, ep_num, isRx);
	atc_qmu_enable(musb, ep_num, isRx);
		...
		if (request->request.dma != DMA_ADDR_INVALID) {
			if (request->tx) {
				...
				if (request->request.length > 0) {
					...
					atc_qmu_insert_task(request->epnum,
							    isRx,
	...
	QMU_ERR("RESUME QMU\n");
	/* RESUME QMU */
	atc_qmu_resume(ep_num, isRx);
}

void atc_qmu_irq_err(struct musb *musb, u32 qisar)
{
	u8 i;
	u32 wQmuVal;
	u32 wRetVal;
	...
	if (err_ep_num)
		atc_qmu_err_recover(musb, err_ep_num, isRx, is_len_err);
}
#endif
