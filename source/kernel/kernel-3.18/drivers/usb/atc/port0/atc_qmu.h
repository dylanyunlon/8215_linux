...
extern int atc_qmu_dbg_level;	/* refer to musb_core.c */
static inline int atc_dbg_level(unsigned level)
{
	return atc_qmu_dbg_level >= level;
}
...
//#define QMU_DBG_ON
#ifdef QMU_DBG_ON
#define QMU_ERR(format, args...) do {if (atc_dbg_level(LOG_ERR)) \
	pr_warn("QMU_ERR,<%s %d>, " format , __func__, __LINE__ , ## args);  } \
	while (0)
#define QMU_WARN(format, args...) do {if (atc_dbg_level(LOG_WARN)) \
	pr_warn("QMU_WARN,<%s %d>, " format , __func__, __LINE__ , ## args);  } \
	while (0)
#define QMU_INFO(format, args...) do {if (atc_dbg_level(LOG_INFO)) \
	pr_warn("QMU_INFO,<%s %d>, " format , __func__, __LINE__ , ## args);  } \
	while (0)
#define QMU_DBG(format, args...) do {if (atc_dbg_level(LOG_DBG)) \
...
extern bool atc_is_qmu_enabled(u8 EP_Num, u8 isRx);
extern void atc_qmu_enable(struct musb *musb, u8 EP_Num, u8 isRx);
extern void atc_qmu_insert_task(u8 EP_Num, u8 isRx, u8 *buf, u32 length, u8 zlp, u8 ioc);
extern void atc_qmu_resume(u8 EP_Num, u8 isRx);
extern void qmu_done_dev_rx(struct musb *musb, u8 ep_num);
extern void qmu_done_dev_tx(struct musb *musb, u8 ep_num);
extern void qmu_done_host_rx(struct musb *musb, u8 ep_num);
extern void qmu_done_host_tx(struct musb *musb, u8 ep_num);
extern void atc_disable_q(struct musb *musb, u8 ep_num, u8 isRx);
extern void atc_qmu_irq_err(struct musb *musb, u32 qisar);
extern void flush_ep_csr(struct musb *musb, u8 ep_num, u8 isRx);
extern void atc_qmu_stop(u8 ep_num, u8 isRx);
