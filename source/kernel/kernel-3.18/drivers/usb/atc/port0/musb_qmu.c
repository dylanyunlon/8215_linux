...
#ifdef CONFIG_OF
	qmu_base = (void __iomem *)(atc_musb->mregs + MUSB_QMUBASE);
	/* debug variable to check qmu_base issue */
	qmu_base_2 = (void __iomem *)(atc_musb->mregs + MUSB_QMUBASE);
#else
...
void musb_disable_q_all(struct musb *musb)
{
	...
	for (ep_num = 1; ep_num <= RXQ_NUM; ep_num++) {
		if (atc_is_qmu_enabled(ep_num, RXQ))
			atc_disable_q(musb, ep_num, 1);
	}
	for (ep_num = 1; ep_num <= TXQ_NUM; ep_num++) {
		if (atc_is_qmu_enabled(ep_num, TXQ))
			atc_disable_q(musb, ep_num, 0);
	}
}
...
void musb_kick_D_CmdQ(struct musb *musb, struct musb_request *request)
{
...
#if 0
	if (!atc_is_qmu_enabled(request->epnum, isRx)) {
		/* enable qmu */
		atc_qmu_enable(musb, request->epnum, isRx);
	}
#endif
	...
	atc_qmu_resume(request->epnum, isRx);
}

void musb_kick_H_CmdQ(struct musb *musb, int is_in, struct musb_qh *qh)
{
	...
	if (!atc_is_qmu_enabled(hw_ep->epnum, is_in)) {
		atc_qmu_enable(musb, hw_ep->epnum, is_in);
	}
	...
	if (qh->type == USB_ENDPOINT_XFER_ISOC)
	{
		...
		for(i=0; i<urb->number_of_packets; i++)
		{
			...
			atc_qmu_insert_task(hw_ep->epnum, is_in, pBuffer+offset, dwLength, 0, bIsIoc);
			atc_qmu_resume(hw_ep->epnum, is_in);
		}
	}
}
...
irqreturn_t musb_q_irq(struct musb *musb)
{
	...
	for (i = 1; i <= MAX_QMU_EP; i++) {
		...
		{
			if (isHost)
				qmu_done_host_rx(musb, i);
			else
				qmu_done_dev_rx(musb, i);
		}
		if (wQmuVal & DQMU_M_TX_DONE(i))
		{
			if (isHost)
				qmu_done_host_tx(musb, i);
			else
				qmu_done_dev_tx(musb, i);
		}
	}
#endif
	atc_qmu_irq_err(musb, wQmuVal);
