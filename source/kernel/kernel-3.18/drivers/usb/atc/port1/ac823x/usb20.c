...
{
    	u32 val;
	h_g_base =  (u64)ioremap(0x10000000,0x1000);
	/*0x284 ?*/
	val = musb_readl((void __iomem *)h_g_base,0x284);
	val &= ~0x1;
	musb_writel((void __iomem *)h_g_base,0x284,val);
	/*ac8237 A0 ret BIT 16 for USB0,BIT 17 for USB1,SET high to enable USB PLL*/
#if  1
	val = musb_readl((void __iomem *)h_g_base,0xA0);
	val |= (1<<16);
	musb_writel((void __iomem *)h_g_base,0xA0,val);
	...
	val = musb_readl((void __iomem *)h_g_base,0x5C4);
	val &= ~(1 << 16);	/* RG_PLLGP_BIAS_PWD = 0 */
	musb_writel((void __iomem *)h_g_base,0x5C4,val);
	val = musb_readl((void __iomem *)h_g_base,0x580);
	val &= ~(1 << 31);	/* RG_SYSPLL1_PWD = 0 */
	musb_writel((void __iomem *)h_g_base,0x580,val);//PLL
	msleep(1);
#endif
...
/*********	gpio_6        	USB2_OCOP2 				********/
/*****************		end			*********************/
static void h_usb_power_on(void)
{
	uint32_t u4Reg = 0;
	/* Disable PORT1 to drive VBUS since it is 
	 * used as host. It is controlled via
	 * GPIO_2
	 */
	/* GPIO_2 Direction */
	u4Reg = musb_readl((void __iomem *)(h_g_base),0x74);
	u4Reg |= 1 << 2; /* 1 stands for OUTPUT. GPIO_1 */
	musb_writel((void __iomem *)h_g_base,0x74,u4Reg);

	/* GPIO_1 Output Value */
	u4Reg = musb_readl((void __iomem *)(h_g_base),0xE0);
	u4Reg |= (1 << 2);
	musb_writel((void __iomem *)h_g_base,0xE0,u4Reg);
...
static void ac823x_h_usb_suspend(struct musb* musb)
{
...
static void ac823x_h_usb_resume(struct musb* musb)
{
	//init PHY REG
	if(musb->is_host != 1){
		musb->is_host = 1;
...
{
	printk("[USB1]-- ac823x m usb suspend --\n");
	//usb_hcd_resume_root_hub(musb_to_hcd(musb));
	//h_musb_root_disconnect(musb);
	ac823x_h_usb_suspend(musb);
	printk("[USB1]-- ac823x m usb suspend end--\n");
}

void h_platform_musb_resume(struct musb* musb)
{
	printk("[USB1]++ac823x m usb resume++\n");
	ac823x_h_usb_resume(musb);
	printk("[USB1]++ac823x m usb resume end++\n");
}
