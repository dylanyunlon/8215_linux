...
#define ACM_FUNC_ENABLE
#define ACCESSORY_FUNC_ENABLE
...
#define MANUFACTURER_STRING "Autochips"

#ifdef CONFIG_ATC_PLATFORM_ac83xx
#define PRODUCT_STRING 	"AC83xx"
#else ifdef CONFIG_ATC_PLATFORM_ac823x
#define PRODUCT_STRING 	"AC823x"
#endif
...
static void android_enable(struct android_dev *dev)
{
	...
	if (--dev->disable_depth == 0) {
		cdev->next_string_id = dev->next_string_id; // restore next string id
			    ...
			    const char *buff, size_t size)
{
	...
	pr_notice("enable_store enable=%d\n", enabled);
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	device_desc.iSerialNumber = id;
 	 // save next string id for enable/disable functions
    	dev->next_string_id = id;
...
android_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *c)
{
	...
	/**zwx the log will be always output untile adb function  init  OK
	**/
	//pr_notice("[USB]%s\n", __func__);
