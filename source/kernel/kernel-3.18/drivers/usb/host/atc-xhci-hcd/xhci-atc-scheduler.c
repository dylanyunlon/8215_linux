...
int mtk_xhci_scheduler_add_ep(struct usb_hcd *hcd, struct usb_device *udev, struct usb_host_endpoint *ep){
	...
	printk(KERN_DEBUG "add_ep parameters: rh_port_num %d, dev_speed %d, is_in %d, isTT %d, ep_type %d, maxp %d, interval %d, burst %d, mult %d, ep@0x%p, ep_ctx@0x%p\n", rh_port_num, dev_speed, is_in, isTT, ep_type, maxp
		, interval, burst, mult, ep, ep_ctx);
