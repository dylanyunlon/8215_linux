...
extern int sd_probe_done;
extern bool bad_device;

#include "boot_state.h"
/**add by zwx
**get_bs_event to listen to system event(boot start,boot end,suspend start,suspend end etc).
** when we get event of suspending,we should stop transport for not block suspend.
**/
struct us_data *g_us = NULL;
			 ...
			 unsigned long bs_event, void *unused)
{
	...
	case STATUS_BOOT_START:
		pr_info("[transport] STATUS_BOOT_START\n");
		break;
	case STATUS_BOOT_END:
		pr_info("[transport] STATUS_BOOT_END\n");
		break;
	case STATUS_SUSPEND_START:
		if(i_am_in_suspend == 0)
			i_am_in_suspend = 1;
		if((sd_probe_done >= 1)&&(sd_probe_done < 4))
			bad_device = 1;
		else
			bad_device = 0;
		pr_info("[transport] STATUS_SUSPEND_START,bad_device is %d,g_us is:%p\n",bad_device,g_us);
		if((bad_device)&&(g_us)){
			pr_info("[transport] usb stor stop transport start\n");
			usb_stor_stop_transport(g_us);
			pr_info("[transport] usb_stor_stop_transport end \n");
		}
		break;
	case STATUS_SUSPEND_END:
		pr_info("[transport]  STATUS_SUSPEND_END\n");
		break;
	case STATUS_RESUME_START:
		pr_info("[transport]  STATUS_RESUME_START\n");
		break;
	case STATUS_RESUME_END:
		pr_info("[transport]  STATUS_RESUME_END\n");
		break;
	}
...
struct notifier_block atc_transport_bs_notifier_block = {
...
void usb_stor_invoke_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	/**add by zwx**/
	/**not submit sici cmd when system in suspend mode
	**/
	if(i_am_in_suspend&&bad_device)
	{
		printk("usb_stor_invoke_transport suspend\n");
...
Retry_Sense:
  ...
  Handle_Errors:
	...
	/**change by zwx
	** reset operation will block suspen 
	**not do reset opetarion when system in suspend mode
	 **/
	if(!i_am_in_suspend){
