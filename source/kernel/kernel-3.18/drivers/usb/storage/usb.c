...
#ifdef CONFIG_ATC_MEDIA_STORAGE_HELPER                                                           
	/* add by zwx
	 * if mass storage device has been disconnected on USB port,
	 * we should send message
	 */
	const char *devpath = NULL;            
	const char *head_info = "EJECT_INFO=Eject@";
        char insert_info[1024] = {'\0'};
	char path_len = 0;
        struct usb_device *udev = interface_to_usbdev(intf);
        devpath = kobject_get_path(&udev->dev.kobj, GFP_KERNEL);
        strcat(insert_info, head_info);  
        if (devpath) {
		if ((strlen(devpath) + strlen(head_info)) < sizeof(insert_info)) {
        		dev_info(&udev->dev, "devpath is %s\n", devpath);
                	strcat(insert_info, devpath);
         		strcat(insert_info, "$");       
         		atc_storage_sendMountInfo2Usrspace(insert_info);
			kfree(devpath);
         		dev_info(&udev->dev, "mass storage device disconnect\n");
		}
		else {
			kfree(devpath);
			dev_err(&udev->dev, "%s: devpath:%d is too long\n", __func__, strlen(devpath));
		}
        }                              
        else {                          
                dev_err(&udev->dev, "mass storage devpath is NULL\n");
         }                              
#endif              
...
#ifdef CONFIG_ATC_MEDIA_STORAGE_HELPER      
         /* add by zwx
          * if mass storage device has been connected on USB port,
          * we should send message
	  */
		...
		if ((strlen(devpath) + strlen(head_info)) < sizeof(insert_info)) {
			dev_info(&udev->dev, "devpath is %s\n", devpath);
         		...
         		strcat(insert_info, "$");       
   		      	atc_storage_sendMountInfo2Usrspace(insert_info);
			kfree(devpath);
         		dev_info(&udev->dev, "mass storage device found\n");
		}
		else {
			kfree(devpath);
			dev_err(&udev->dev, "%s: devpath:%d is too long\n", __func__, strlen(devpath));
