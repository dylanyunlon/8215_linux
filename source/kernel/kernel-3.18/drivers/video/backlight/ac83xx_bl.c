...
static void __iomem *tmp_gpio_addr;
struct pinctrl *pinctrl_bkl = NULL;
struct pinctrl_state *pins_bkl_set_gpio125 = NULL;
struct gpio_desc *bklGPIO = NULL;
int bkl_enable_flag = 0;
...
static long atc_bkl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case DISPLAY_SET_BKL_INTENSITY:
		if (!bkl_enable_flag) {
			bkl_enable_flag = 1;
			if (IS_ERR(bklGPIO)) {
				pr_err("[bkl]:can not get gpio %s, %s, %s, %s\r\n",
					 bklgpiofct, FILE_ONLY, __func__, __LINE__);
				return -1;
			}
			err = gpiod_direction_output(bklGPIO, 1);
			if (err) {
				pr_err("[bkl]can not look state backlight-gpios, %s, %s, %s\r\n",
	...
	case DISPLAY_GET_BKL_INTENSITY:
		{
			...
			if (copy_to_user((void __user *)arg, &tconvalue, sizeof(__s32))) {
				err  = -EINVAL;
				pr_err("[bkl]copy_to_user return %d iocode %d, %s, %s, %s\n",
					 err, cmd, FILE_ONLY, __func__, __LINE__);
			}
			break;
		}
	...
	case DISPLAY_SET_BKL_SHUTDOWN:
		if (arg == 1) {
			...
			err = gpiod_direction_output(bklGPIO, 0);
			if (err) {
				pr_err("[bkl]can not look state backlight-gpios, %s, %s, %s\r\n",
					FILE_ONLY, __func__, __LINE__);
				return -1;
	...
	default:
		err  = -EINVAL;
		pr_err("[bkl]atc_bkl_ioctl do not support this cmd = 0x%x, %s, %s, %s\n",
			 cmd, FILE_ONLY, __func__, __LINE__);
		break;
	}
...
static int bkl_probe(struct platform_device *pdev)
{
	...
	if (result == 0)
		...
		pr_err("[bkl]misc device register error, %s, %s, %s\n", 
			FILE_ONLY, __func__, __LINE__);
		kfree(bkldev);
		return result;
	}
	...
	ret = MetaZone_Read(MZ_DISPLAY_BACKLIGHT, &value);
	if (ret) {
		dev_err(&pdev->dev, "MetaZone_Read MZ_DISPLAY_BACKLIGHT failed\n");
	} else {
		pr_info("MetaZone_Read MZ_DISPLAY_BACKLIGHT success: %d\n", value);
	}

	bl->props.brightness = value;//default for linux
	backlight_update_status(bl);

	platform_set_drvdata(pdev, bl);

//
	pinctrl_bkl = devm_pinctrl_get(bkldev->dev);
	if (IS_ERR(pinctrl_bkl))
		/*dev_err(&pdev->dev, "wch get pinctrl error!\n");*/
		return -1;

	pins_bkl_set_gpio125 = pinctrl_lookup_state(pinctrl_bkl, "bkl_set_gpio125_out");
	if (IS_ERR(pins_bkl_set_gpio125))
		pr_err("[bkl]pin state gpio125 err %p, %s, %s, %s\r\n",
			pins_bkl_set_gpio125, FILE_ONLY, __func__, __LINE__);
	ret = pinctrl_select_state(pinctrl_bkl, pins_bkl_set_gpio125);
	if (ret)
		pr_err("[bkl]pin select gpio125 err ret=%d, %s, %s, %s\r\n",
			ret, FILE_ONLY, __func__, __LINE__);

	bklGPIO = __gpiod_get(bkldev->dev, bklgpiofct, GPIOD_ASIS);
	if (IS_ERR(bklGPIO)) {
		pr_err("[bkl]__gpiod_get err\r\n");
	}
...
static int bkl_remove(struct platform_device *pdev)
{
	...
	if (bkldev == NULL) {
		pr_err("[bkl]No device when bkl remove! %s, %s, %s\n",
			FILE_ONLY, __func__, __LINE__);
		return -ENODEV;
	}
...
static int atc_bkl_resume(struct device *dev)
{
	...
	pr_info("[bkl]set PIN_124_GPIO124 low!\r\n");
	pr_info("[bkl]: %s\n", __func__);
	return 0;
}
...
void vSetPWM(u32 dty_cyc)
{
	...
	************************************************************/
	...
	/* pwm_bl en setting */
	pwm_bl[0].clk_id = CLKSRC_27M, pwm_bl[0].pin_id = 0;	/* GPIO 125 output en */
	pwm_bl[0].pwm_en = 0;
	...
	pr_info("[bkl][wts] set bk light %d\n", (int)dty_cyc);
}
...
void vSetBacklightIntensity(__u32 u4Value)
{
		...
		if (0 != u4Value) {
			ret = MetaZone_Write(MZ_DISPLAY_BACKLIGHT, u4Value);
			if (ret) {
				pr_err("MetaZone_Write MZ_DISPLAY_BACKLIGHT failed %d\n", u4Value);
			} else {
				pr_info("MetaZone_Write MZ_DISPLAY_BACKLIGHT successed %d\n", u4Value);
			}
		}
...
static int __init bkl_init(void)
{
	...
	pr_info("[bkl]bkl_init--->\n");
	ret = platform_driver_register(&bkl_plt_drv);
	if (ret)
		pr_err("[bkl]register driver failed, %s, %s, %s\n",
			FILE_ONLY,  __func__, __LINE__);
	return ret;
}
