...
void DisplaySclInit(bool fgHwReset)
{
	...
	if (fgHwReset) {
		...
		SCL_Config(_u4LCDWidth, _u4LCDHeight);
	}
}

/*static __u32 rgn_list[2], rgn[2], plane;*/
static __u32 rgn_list, rgn, plane;
static unsigned long fb_addr1, fb_addr2;
...
bool subtitle_osd_init(unsigned int addr, OSD_DATA_T *prData)
{
	...
	if (1) {
		...
		FB_PRINT(FB_LOG_LVL_DBG, "[MTK] using private dest rect\n");
	}

	if (u4BytesPerPixel == 0) {
		FB_PRINT(FB_LOG_LVL_DBG, "[MTK] FAILED to get bytes per pixel %d\n", (int)prData->u4PixelFormat);
	...
	FB_PRINT(FB_LOG_LVL_DBG, "[MTK] subtitle_osd_init plane id %d  rect  %d %d %d %d\n",
...
bool subtitle_osd_palette_init(unsigned int addr, OSD_DATA_T *prData)
{
	...
	st_rgn_list = st_plane;
	FB_PRINT(FB_LOG_LVL_DBG, "[MTK] subtitle_osd_palette_init %d %d %d %d\n",
...
void subtitle_osd_off(unsigned int u4Plane)
{
	...
	if (_afgOSDPlaneState[u4Plane].fgEnable) {
		FB_PRINT(FB_LOG_LVL_DBG, "[MTK] disable osd   %d.......\n", (int)layerId);
		i4OsdPlaneEnble(layerId, FALSE);
		u4Rgn  = GetPlaneRgn(layerId);

		if (u4Rgn < OSD_MAX_NUM_RGN) {
			FB_PRINT(FB_LOG_LVL_DBG, "[MTK] rgn delete  .......\n");
...
static bool osd_init(unsigned long addr1, unsigned long addr2, int width, int height)
{
#ifdef FB_DEBUG
...
void display_init(unsigned long addr1, unsigned long addr2, unsigned int width, unsigned int height)
{
	__u32 *pu4OSDReset;
...
/*
	...
	if (RESET_HW_ENGINE) {
		...
		FB_PRINT(FB_LOG_LVL_DBG, "[DDI]display_init: master mode %d\n", RESET_HW_ENGINE);
		vTconTimingInput(TRUE, u4LCDHStart, u4LCDHStart + width, u4LCDVStart, u4LCDVStart + height);
		vPmxHalSetMasterMode(TRUE);
#else
		FB_PRINT(FB_LOG_LVL_DBG, "[DDI]display_init: slave mode %d\n", RESET_HW_ENGINE);
...
/*
	if (RESET_HW_ENGINE) {
		...
		*pu4OSDReset &= ~0xff0;
		msleep(300);
		//vSetBacklightIntensity(76);//for 8237
	}
*/
	/*i4ImgResz_Drv_Init();*/
}
