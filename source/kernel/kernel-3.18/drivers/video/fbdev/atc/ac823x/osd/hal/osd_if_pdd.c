...
__s32   i4OSDRestoreHwReg(void)
{
	...
	*((__u32 *)(IO_BASE_BRINGUP + 0x10)) |= 0x1 << 28;
	*((__u32 *)(IO_BASE_BRINGUP + 0xd4)) |= 0xffff << 16;
	return TRUE;
}
...
__s32 i4OsdPlaneFlipTo(__u32 u4Plane, __u32 u4RgnList)
{
	...
	__u32 i4Ret = OSD_RET_OK;
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "i4OsdPlaneFlipTo begin\n");
#endif
	OSD_PLA_FlipTo(u4Plane, u4RgnList);
	_OSD_PLA_UpdateHwReg(u4Plane);
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "plane flip to %d \r\n", u4Plane);
#endif
...
__u32 u4PixelFormatToOSDColorMode(__u32 u4PixelFormat)
{
	...
	default:
		//FB_PRINT(FB_LOG_LVL_DBG, "OSD does not support the pre-defined pixel format \r\n");
		break;
