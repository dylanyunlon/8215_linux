...
#define OSD_REGION_SETGET_ADDRESS_TMPL(NAME, FIELD)  \
	...
	{ \
		...
		OSD_VERIFY_REGION(u4Region); \
		prRgn = OSD_RGN_GetAdd(u4Region); \
		prRgn[u4Region].rField.FIELD = (u4Value == 0) ? 0 : VIRT_TO_BUS(u4Value) >> 4; \
		OSD_REGION_FLUSH(u4Region); \
		return (__s32)OSD_RET_OK; \
	} \
	__s32 _OSD_RGN_Get##NAME(__u32 u4Region, __u32 *pu4Value) \
...
#define OSD_REGION_SETGET_ADDRESS_TMPL(NAME, FIELD)  \
	...
	{ \
		...
		OSD_VERIFY_REGION(u4Region); \
		prRgn = OSD_RGN_GetAdd(u4Region); \
		prRgn[u4Region].rField.FIELD = (u4Value == 0) ? 0 : VA_TO_PA(u4Value) >> 4; \
		OSD_REGION_FLUSH(u4Region); \
		return (__s32)OSD_RET_OK; \
	} \
	__s32 _OSD_RGN_Get##NAME(__u32 u4Region, __u32 *pu4Value) \
	{ \
		...
		*pu4Value = (__u32)PA_TO_VA(prRgn[u4Region].rField.FIELD << 4); \
		return (__s32)OSD_RET_OK; \
	} \
...
static unsigned int _au4OsdRegionStatus[OSD_MAX_NUM_RGN + 1];
...
unsigned int  _memset(char *addr,unsigned int value, unsigned int size)
{
	unsigned int i =0;
	char *  ptr =addr;
...
{
	unsigned long i4Unused;
	unsigned long u4PhyAddr;
	...
	if (_fgRgnMemNeedInit) {
#ifndef __ARM2__
		u4PhyAddr = fbm_base + fbm_size - 0x8000;
...
#endif
		AddPaVatoMapTable(u4PhyAddr, (__u32)_prOsdRegionReg1, sizeof(OSD_RGN_UNION_T) * OSD_MAX_NUM_RGN);

		/* _prOsdRegionReg1 = (OSD_RGN_UNION_T*)((__u32)(_au1OsdRegion + 0xf) & ~0xf);*/
		i4Unused = (unsigned long)_memset(_prOsdRegionReg1, 0, sizeof(OSD_RGN_UNION_T) * OSD_MAX_NUM_RGN);
		i4Unused = (unsigned long)_memset(_au4OsdRegionStatus, 0, sizeof(unsigned int) * (OSD_MAX_NUM_RGN + 1));
		...
		if (fgOsdBitmapMemPoolNeedInit) {
			...
			if (i4Ret != OSR_OK) {
				FB_PRINT(FB_LOG_LVL_ERR, "%s %d  x_mem_part_create failed : %d\r\n", __func__
					, __LINE__, i4Ret);
		...
		/*if (_pu1OsdRgnMem2Raw == NULL) {
			FB_PRINT(FB_LOG_LVL_ERR, "%s %d  x_mem_part_alloc failed\r\n", __func__, __LINE__);
			return;
		}
 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 _OSD_RGN_SetDataAddr(__u32 u4Region, unsigned long u4Value)
{
	OSD_RGN_UNION_T *prRgn;

	OSD_VERIFY_REGION(u4Region);
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "_OSD_RGN_SetDataAddr begin\n");
#endif
	prRgn = OSD_RGN_GetAdd(u4Region);
#ifndef __ARM2__
	prRgn[u4Region].rField.u4DataAddr   = (unsigned int)(VIRT_TO_BUS(__va(u4Value)) >> 4);
	prRgn[u4Region].rField.u4DataAddrHI = (unsigned int)(VIRT_TO_BUS(__va(u4Value)) >> 28);
	prRgn[u4Region].rField.fgDeCompLineBased = (unsigned int)(VIRT_TO_BUS(__va(u4Value)) >> 30);
	prRgn[u4Region].rField.fgWTEn = (unsigned int)(VIRT_TO_BUS(__va(u4Value)) >> 31);
#else
	prRgn[u4Region].rField.u4DataAddr	= (unsigned int)((u4Value) >> 4);
	prRgn[u4Region].rField.u4DataAddrHI = (unsigned int)((u4Value) >> 28);
	prRgn[u4Region].rField.fgDeCompLineBased = (unsigned int)((u4Value) >> 30);
	prRgn[u4Region].rField.fgWTEn = (unsigned int)((u4Value) >> 31);
#endif
	OSD_REGION_FLUSH(u4Region);
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "_OSD_RGN_SetDataAddr end:u4DataAddr=%x\n",  prRgn[u4Region].rField.u4DataAddr);
#endif
	return (__s32)OSD_RET_OK;
}
 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 _OSD_RGN_GetHandle(unsigned long u4Addr, __u32 *pu4Region)
{
	...
	if ((u4Addr < (unsigned long)prRgn1) ||
	    (u4Addr >= (unsigned long)((OSD_MAX_NUM_RGN * sizeof(OSD_RGN_UNION_T)) + (unsigned long)prRgn1))) {
		if ((u4Addr < (unsigned long)prRgn2) ||
		    (u4Addr >= (unsigned long)((OSD_MAX_NUM_RGN * sizeof(OSD_RGN_UNION_T)) + (__u32)prRgn2))) {
			return -(unsigned long)OSD_RET_UNDEF_ERR;
		}
		*pu4Region = (u4Addr - (unsigned long)prRgn2) / sizeof(OSD_RGN_UNION_T);
		*pu4Region += OSD_RGN_INDARMCHNANNEL2_START;
	} else {
		*pu4Region = (u4Addr - (unsigned long)prRgn1) / sizeof(OSD_RGN_UNION_T);
 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 _OSD_RGN_GetAddress(__u32 u4Region , unsigned long *pu4Addr)
{
	OSD_RGN_UNION_T *prRgn;
	...
	*pu4Addr = (unsigned long)&prRgn[u4Region];
...
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "_au4OsdRegionStatus[0]=%d, _au4OsdRegionStatus[1]=%d, _au4OsdRegionStatus[2]=%d\n",
...
#ifdef FB_DEBUG
			FB_PRINT(FB_LOG_LVL_DBG, "_OSD_RGN_Alloc:*pu4Region=%d\n", *pu4Region);
#endif
			return (__s32)OSD_RET_OK;
		}
	}
