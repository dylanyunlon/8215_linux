 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 OSD_SC_Scale(__u32 u4Scaler, __u32 u4Enable, __u32 u4SrcWidth,
		   __u32 u4SrcHeight, __u32 u4DstWidth, __u32 u4DstHeight)
{
	...
	if ((u4MaxWidth == 0) || (u4MaxHeight) == 0) {
		...
		FB_PRINT(FB_LOG_LVL_ERR, "[osd_scaler_if]fastlogo didn't run first.\n");
	}
	...
	if (u4SrcHeight == u4DstHeight) {
		...
		if (u4SrcHeight <= u4DstHeight) {
			...
			if (OSD_SC_CheckCapability(u4SrcWidth, u4SrcHeight, u4DstWidth, u4DstHeight) !=
			    (__s32)OSD_RET_OK) {
				FB_PRINT(FB_LOG_LVL_ERR, "OSD_SC_CheckCapability: fail\n");
			}
