...
__s32 OSD_SAVE_Cur_Rgn(__u32 u4Plane)
{
	...
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "[OSD_SAVE_Cur_Rgn]: u4FirstRgn is %d, i4FlipedList is %d\n",
	...
	if (OSD_Is_PLA_Enabled(u4Plane)) {
		...
		FB_PRINT(FB_LOG_LVL_INFO, "OSD", "[OSD_SAVE_Cur_Rgn]: u4FirstRgn is %d\n", _rOsdCurRgn[u4Plane]);
	} else {
...
__s32 OSD_Restore_Rgn(__u32 u4Plane)
{
	...
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "[OSD_Restore_Rgn]: u4FirstRgn is %d, i4FlipedList is %d\n",
		 _rOsdCurRgn[u4Plane], _arOsdPlaneInfo[u4Plane].i4FlipedList);
...
static void _Osd_PustReset_Plane(__u32 u4Plane)
{
	...
	case OSD_PLANE_7:
		u4ValueR &= 0xFFFF00c0;//osdr2
		break;

	case OSD_PLANE_8:
		u4ValueR &= 0xFFFF0300;//osdr3
		break;
	...
	if (u4Plane <= OSD_PLANE_5){
		...
		IO_WRITE32(osdr_reg, 4, u4ValueR);
	}
}
...
__s32 OSD_PLA_FlipTo(__u32 u4Plane, __u32 u4List)
{
	...
	__u32 u4FirstRegionAddr;
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_PLA_FlipTo begin\n");
#endif
	OSD_VERIFY_PLANE(u4Plane);
...
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "i4FirstRegion=%d\n", i4FirstRegion);
#endif
	...
	VERIFY((__s32)OSD_RET_OK ==
	       _OSD_RGN_GetAddress((__u32)i4FirstRegion, &u4FirstRegionAddr));
...
#ifdef FB_dEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "u4FirstRegionAddr=%x, u4List=%d\n", u4FirstRegionAddr, u4List);
#endif
	return (__s32)OSD_RET_OK;
}
...
__s32 OsdFlipPlaneRightNow(__u32 u4Plane, bool fgValidReg, __u32 u4List)
{
	...
	if (eScrnState_Enabled == _rOsdScrnFlag.eOsdScrnState) {
		if (OSD_PLANE_UI != u4Plane && OSD_PLANE_5 != u4Plane) {
			FB_PRINT(FB_LOG_LVL_DBG, "OSD", "In ScrnSaver, Flip plane %d\n", u4Plane);
			return OSD_RET_OK;
		}
	}
