...
#define vWriteScale(dAddr, dVal)  (*(volatile __u32*)(IO_BASE_ADDRESS + 0x3000 + dAddr) = dVal)

#ifdef HWGETGAMMA
static __u8 g_pbGammaDefault[64] = {
	0, 1, 3, 5, 7, 9, 11, 13,
...
{
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "fgHwReset\r\n");
#endif

	if (fgHwReset) {
#ifdef FB_DEBUG
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "fgHwReset1\r\n");
#endif
		__s32 i;
...
void vTconTimingInput(__s32 i4En, __s32 i4HStart, __s32 i4HEnd, __s32 i4VStart, __s32 i4VEnd)
{
	...
	if (i4En % 2) {
		...
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "TCON Timing Input Enable&ActiveArea(%d,%d,%d,%d)\r\n", (int)i4HStart
			 , (int)i4HEnd, (int)i4VStart, (int)i4VEnd);
	} else {
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "TCON Timing Input Disable\r\n");
	}

#else
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "Feature not support!\r\n");
#endif
}
...
void vPanelSetGamma(__u8 *pu4GammaData)
{
	...
	for (bIndex = 0; bIndex < 64; bIndex++) {
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "[Gamma Read]Index %d : %d \r\n", bIndex, *(pu4GammaData + bIndex));
...
void vTConPatternGnrtr(__u8 bPattenType)
{
	switch (bPattenType) {
	case 21:
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "Tcon  Pattern : Index  0 \r\n");
		vWritePCLRP(0x00, 0x100000);
		vWritePCLRP(0x18, 0x4100000);
		/*vWritePCLRP(0x60, 0x12d20);*/
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "Tcon  Pattern : End \r\n");
		break;

	case 22:
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "Tcon  Pattern : Index  1 \r\n");
	...
	case 23:
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "Tcon  Pattern : Index  2 \r\n");
...
void vTConGeneralPro(__u8 bType)
{
	...
	/*	vSetDefaultYCPrco();*/
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "Tcon  vTConGeneralPro : %d \r\n", bType);
...
bool IsLVDS8Bit(void)
{
	...
	FB_PRINT(FB_LOG_LVL_INFO, "FPD", "IsLVDS8Bit %d\r\n", (int)fgLVDS8Bit);
...
{
	WriteREG(0xa4700,0x03180700);
	WriteREG(0xa4704,0x31ffffff);
	WriteREG(0xa4724,0x2b000000);
	WriteRegMsk(0xa4780,0x1<<31,0x1<<31);
	WriteRegMsk(0xa4780,0x1<<10,0x1<<10);
	WriteREG(0xa4804,0x0104017e);
	WriteREG(0xa4820,0x00000001);
	WriteREG(0xa4824,0x001f71f5);
	WriteREG(0xa4830,0x0006800e);
	WriteREG(0xa4834,0x00000001);
	WriteREG(0xa48e0,0x0000247d); //[9..8] = 0->Pll(30MHz); 2->27MHz
	WriteREG(0xa48e4,0x0005f51a);
	WriteREG(0xa48f0,0x00000002);

	WriteREG(0x2d4,0x00000701);
	WriteREG(0x2d8,0x00000701);

	WriteREGMsk(0x52c00,0x1<<30,0x1<<30);//modify 7-26
	WriteRegMsk(0x005c8, 0x7<<5,(0x7<<5)|(0x1<<8));
	WriteRegMsk(0x005cc, 0x7<<5,(0x7<<5)|(0x1<<8));
	WriteREG(0xd8,0x00080000);
	WriteREG(0xdc,0x00000001);

	if(_u4LCDType == LCD_TYPE_TTL)
	{
		FB_PRINT(FB_LOG_LVL_OFF, "FPD", "[@vFpdTconInit]  ttl panel init <<< \r\n");
		...
		WriteREG(0xa6000,0x000207a0);
		WriteREG(0xa6004,0x0001f1f1);
		WriteREG(0xa6020,0x010dd100);

		WriteREG(0xa6020,0x0009d100);
		WriteREG(0xa6024,0x00039003);
		WriteREG(0xa602c,0x00000140);
		WriteREG(0xa6020,0x00095100);

		WriteREG(0xa6000,0x000007a0);
		WriteREG(0xa600c,0x03000000);
		WriteREG(0xa6000,0x1f0107a9);

		WriteRegMsk(0x14,0x3<<21,0x3<<21); //TTL clk devider select
	}
	else
	{
		FB_PRINT(FB_LOG_LVL_OFF, "FPD", "[@vFpdTconInit] lvds DDDS init <<< \r\n");
		// @pmx_vfy_dat.c config DDDS
		WriteREG(0x52c0c,0x8000061f);
		WriteREG(0x52c04,0x020d00c3);
		WriteREG(0x52c14,0x313e013e);
		WriteREG(0x52c14,0x713e013e);  //reset DDDS
		WriteREG(0xa504c,0x0000000f);
		WriteREG(0xa504c,0x00000000);//add 7-26
		WriteREG(0xa5018,0x00000000);
		WriteREG(0xa501C,0x0000241B);// lvds chanel swap

		//LVDS VPLL Power On Sequence Start
		WriteREG(0xa6000,0x000207a0);
		WriteREG(0xa6004,0x00000001);
		WriteREG(0xa6020,0x010dd100);
		if((_u4LCDWidth*_u4LCDHeight) <= (640*240))
		{
			WriteREG(0xa6020,0x011bd100);
			WriteREG(0xa6024,0x0007a003);
			WriteREG(0xa602c,0x00000140);
			WriteREG(0xa6020,0x010d5100);
		}
		else if((_u4LCDWidth*_u4LCDHeight) <= (1280*480))
		{
			WriteREG(0xa6020,0x011bd100);
			WriteREG(0xa6024,0x0005a003);
			WriteREG(0xa602c,0x00000140);
			WriteREG(0xa6020,0x011b5100);
		}
		else
		{
			WriteREG(0xa6020,0x0106d100);
			WriteREG(0xa6024,0x0000a003);
			WriteREG(0xa602c,0x00000140);
			WriteREG(0xa6020,0x01065100);
		}
		WriteREG(0xa6000,0x000007a0);
		WriteREG(0xa600c,0x03000000);
		WriteREG(0xa6000,0x01f107a9);

		//@pmx_vfy_hal.c
		WriteREGMsk(0x14,0x1<<22,0x1<<22);
		WriteRegMsk(0x34,0x1<<3,0x1<<3);
		WriteREG(0xa5018,0x00000002);
		WriteREG(0xa504c,0x0000000f);
		WriteREG(0xa504c,0x00000000);

		//+++++tcon.c++++++++++++++++
		FB_PRINT(FB_LOG_LVL_OFF, "FPD", "LVDS Module Init,(_u4LCDW*_u4LCDH)=[%d*%d=%d] \n",
			_u4LCDWidth, _u4LCDHeight, _u4LCDWidth*_u4LCDHeight);  //no delete
		...
		if ( IsLVDS8Bit() == FALSE)
		{
			WriteREG(0xa500c,0x002D2492);
			WriteREG(0xa5010,0x002D2492);
			WriteREG(0xa5014,0x002D2492);
		}
		else
		{
			WriteREG(0xa500c,0x00000000);
			WriteREG(0xa5010,0x00000000);
			WriteREG(0xa5014,0x00000000);
		}
		//LVDS VPLL Power On Sequence End
		WriteREG(0xa5018,0x00000002);//lds en
		WriteREG(0xa5090,0x01000000);// lvds swap
	}

	WriteRegMsk(0x94,0x1<<6,0x1<<6); //vb0~vb3  Enable(Pad Mux)  //add for ttl from LVDS
	WriteRegMsk(0x34,0,0x1<<4);      // twds_sel
	WriteRegMsk(0xDC,0,0x1FFFFF<<0); // fpd_hck_dly_sel
	vWritePGMAMsk(RW_PGMA_CTRL0, FULL_RANGE_IN, FULL_RANGE_IN);
	vWritePGMAMsk(RW_PGMA_CTRL4, FULL_RANGE_OUT, FULL_RANGE_OUT);

}

void LvdsSscConfig(unsigned int dir, unsigned int freq, unsigned int range)
{
	unsigned long long freq_cw = 0;
	unsigned long long spread_step = 0;
	unsigned long long spread_period = 0;
	unsigned int reg_value = 0;

	FB_PRINT(FB_LOG_LVL_INFO, "FPD", "LvdsSscConfig dir:%d, freq:%d, range:%d\n", dir, freq, range);

	//freq_cw = 648 / 72 * 16777216;
	freq_cw = ReadREG(0x52c00) & 0xFFFFFFF;
	spread_step = (48 * range * freq * freq_cw) / 64800000000;
	spread_period = 648000000 / 48 / freq - 1;

	FB_PRINT(FB_LOG_LVL_INFO, "FPD", "LvdsSscConfig freq_cw: 0x%llx, spread_step: 0x%llx, spread_period: 0x%llx \r\n", freq_cw, spread_step, spread_period);
	WriteREG(0x52c0c, 0x8000061f);
	WriteREG(0x52c04, 0x020d00c3);
	reg_value = ((0x1 << 24) | spread_step);
	WriteREG(0x52c10, reg_value);
	FB_PRINT(FB_LOG_LVL_INFO, "FPD", "LvdsSscConfig 0x52c10: 0x%x \r\n", reg_value);
	reg_value = ((1 << 30) | (dir << 28) | (spread_period << 16) | spread_period);
	WriteREG(0x52c14, reg_value);
	FB_PRINT(FB_LOG_LVL_INFO, "FPD", "LvdsSscConfig 0x52c14: 0x%x \r\n", reg_value);
}
...
void vFpdInit(void)
{
	...
	switch (_u4DispMode) {
	case RES_600P_1024: { /* lvds_1024_600 panel*/
		/*               Step0:  Config DDDS Clock to 72MHz                 */
		FB_PRINT(FB_LOG_LVL_INFO, "FPD", "[wts] 1024x600 lvds panel init <<< \r\n");
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "[vFpdInit] tcon register base %x, %x, %x\r\n", (unsigned int)tlcp_reg
			, (unsigned int)togc_reg, (unsigned int)tcon_reg);
		WriteREG(0x52c0c, 0x8000061f);
		WriteREG(0x52c00, 0x46540000);
		WriteREG(0x52c04, 0x020d00c3);
		WriteREG(0x52c14, 0x313e013e);
		WriteREG(0x52c14, 0x713e013e);
		WriteRegMsk(0x005c8, 0x7 << 5, (0x7 << 5) | (0x1 << 8)); /* ddds1 clk div2*/
		WriteRegMsk(0x005cc, 0x7 << 5, (0x7 << 5) | (0x1 << 8)); /* ddds2 clk div2*/
		/*                   Step1:  Configure VOPLL for PCLK (DDDS * 10 /4)        */
		/*WriteRegMsk(0xB0,1<<0,1<<0);//LVDS Clock En & Reset*/
		/*WriteRegMsk(0xCC,1<<0,1<<0);//LVDS Clock En & Reset*/
		...
		if (IsLVDS8Bit() == FALSE) {
			...
			WriteREG(0xa5010, 0x00000000);
			WriteREG(0xa5014, 0x00000000);
			vWritePGMAMsk(0x80, 0xe20, 0xff0); /*output fmt set to 8 bit*/
			WriteRegMsk(0xa4780, 0x1 << 31, 0x1 << 31);
		}
		...
		/*WriteRegMsk(0x14,0x0<<21,0x3FF<<21);//CLK27M*/
		/*WriteRegMsk(0x34,1<<3,0x1<<3);      // lvds_sel*/
	}
	break;

	case RES_480P_800: { /* ttl_800_480*/
		if (_u4LCDType == LCD_TYPE_TTL) {
			FB_PRINT(FB_LOG_LVL_DBG, "FPD", "[wts] 800 x480 ttl panel init <<< \r\n");
			/*               Step0:  Config DDDS Clock to 72MHz                 */
			WriteREG(0x52c0c, 0x8000061f);
			WriteREG(0x52c00, 0x44800000);
			WriteREG(0x52c04, 0x020d00c3);
			WriteREG(0x52c14, 0x313e013e);
			WriteREG(0x52c14, 0x713e013e);
			WriteRegMsk(0x005c8, 0x7 << 5, (0x7 << 5) | (0x1 << 8));
			...
			/*WriteRegMsk(0x14,0x3<<21,0x3FF<<21);//VPLL_TTL_CK_D6*/
			/*WriteRegMsk(0x34,0,0x1<<3);      // lvds_sel*/
		} else {
			FB_PRINT(FB_LOG_LVL_DBG, "FPD", "[wts] 800 x480 lvds panel init <<< \r\n");
			WriteREG(0x52c0c, 0x8000061f);
			WriteREG(0x52c00, 0x48A3D710);
			WriteREG(0x52c04, 0x020d00c3);
			WriteREG(0x52c14, 0x313e013e);
			WriteREG(0x52c14, 0x713e013e);
			WriteRegMsk(0x005c8, 0x7 << 5, (0x7 << 5) | (0x1 << 8)); /* ddds1 clk div2*/
	...
	case RES_768P_1024: {
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "[wts] 1024 x768 lvds panel init <<< \r\n");
		WriteREG(0x52c0c, 0x8000061f);
		WriteREG(0x52c00, 0x433D70A3);
		WriteREG(0x52c04, 0x020d00c3);
		WriteREG(0x52c14, 0x313e013e);
		WriteREG(0x52c14, 0x713e013e);
		WriteRegMsk(0x005c8, 0x7 << 5, (0x7 << 5) | (0x1 << 8)); /* ddds1 clk div2*/
    ...
    case RES_800P_1280:
        {
		printk("[wts] 1280 x800 lvds panel init <<< \n");
		WriteREG(0x52c0c,0x8000061f);
		WriteREG(0x52c00,0x433D70A3);
		WriteREG(0x52c04,0x020d00c3); 
		WriteREG(0x52c14,0x313e013e); 
		...
		************************************************************/
		...
		WriteREG(0xa5090,0x01000000);/* lvds swap*/
		/* LVDS Config End*/
		vWritePGMAMsk(0x80, 0xe10, 0xff0);/* output fmt set to 8 bit*/
		WriteRegMsk(0xa4780,0x1<<31,0x1<<31);
		/*WriteRegMsk(0x14,0x0<<21,0x3FF<<21);//CLK27M*/
		/*WriteRegMsk(0x34,1<<3,0x1<<3);      // lvds_sel*/
        }
	...
	WriteRegMsk(0x34, 0, 0x1 << 4);  /* twds_sel*/
	WriteRegMsk(0xA48E0, 0, 0x3 << 8); /* fpd_ck_sel*/

	WriteRegMsk(0xD8, 1 << 19, 1 << 19); /* fpd_on_global*/
	WriteRegMsk(0xA48E0, 1, 1 << 0); /* fpd_on_aux*/

	WriteRegMsk(0xDC, 0, 0x1 << 22); /* fpd_hck_inv_en*/
	WriteRegMsk(0xDC, 0, 0x1 << 21); /* fpd_hck_dly_en*/
	WriteRegMsk(0xDC, 0, 0x1FFFFF << 0); /* fpd_hck_dly_sel*/

#if 0
	/*                   Step4:  Configure Panel Pad Preplace                           */
	...
	/*Cofig FPD full range input and output*/
	vWritePGMAMsk(RW_PGMA_CTRL0, FULL_RANGE_IN, FULL_RANGE_IN);
	vWritePGMAMsk(RW_PGMA_CTRL4, FULL_RANGE_OUT, FULL_RANGE_OUT);
	WriteRegMsk(0xa4780, 0x1 << 10, 0x1 << 10);/* modify dither DE select */
}
...
void  vTconSetBrightness(__u32 u4Value)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set  tcon  brightness  %d \r\n", (int)u4Value);
...
void  vTconSetContrast(__u32 u4Value)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set tcon contrast  %d \r\n", (int)u4Value);
...
void vTconSetSaturation(__u32 u4Value)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set tcon saturationi   %d \r\n", (int)u4Value);
...
void vTconSetHue(__u32 u4Value)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set tcon HUE %d \r\n", (int)u4Value);
...
void vTconSetYGain(__u32 u4Value)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set tcon YGain %d \r\n", (int)u4Value);
...
void vTconSetUGain(__u32 u4Value)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set tcon UGain %d \r\n", (int)u4Value);
...
void vTconSetVGain(__u32 u4Value)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set tcon VGain %d \r\n", (int)u4Value);
...
void vTconSetGainYUV(__u32 gain_Y, __u32 gain_U, __u32 gain_V)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "set YUV gain (0x%x, 0x%x, 0x%x) \r\n", (unsigned int)gain_Y
...
int TconGetContrast(void)
{
	...
	value = (((value * 1000) / 0xff) + 5) / 10;

	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "get tcon contrast %d\r\n", (int)value);
	return (int)value;
}
...
int TconGetBrightness(void)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "get tcon brightness %d\r\n", (int)value);
	return (int)value;
}
...
int TconGetHue(void)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "get tcon hue %d\r\n", (int)value);
	return (int)value;
}
...
int TconGetSaturation(void)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "get tcon saturation  %d \r\n",  (int)value);
	return (int)value;
}
...
int TconGetYGain(void)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "get tcon YGain  %d \r\n",  (int)value);
	return (int)value;
}
...
int TconGetUGain(void)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "get tcon UGain  %d \r\n",  (int)value);
	return (int)value;
}
...
int TconGetVGain(void)
{
	...
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "get tcon VGain  %d \r\n",  (int)value);
	return (int)value;
}
...
void vTconSetDDDSClk(UINT16 u2LCDType, __u32 fClkFrq)
{
	...
	if (1 == u2LCDType) { /*LVDS*/
		...
		*(volatile unsigned int *)0xFD052C00 |= u4DDDSClk;
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "[wts] set clk %x \r\n", (__u32)u4DDDSClk);
...
{
	if ((timing.u4Clock >= 300) && (timing.u4Clock <= 1000)) {
		/*Set DDDS & FPD Clk*/
		...
		*(volatile unsigned int *)0xFD0A468c |= timing.u4VTotal << 0;     /*vtotal*/
		...
		if (0 == u2LCDType) {
			...
			FB_PRINT(FB_LOG_LVL_DBG, "FPD", "[wts] set tcon timing \r\n");
		}

	} else {
		FB_PRINT(FB_LOG_LVL_ERR, "FPD", "Set CLK fail! please set clk in range of [30,52.5]! \r\n");
	}
	...
	for (u4Idx = 0; u4Idx < TCON_GAMMA_INDEX_NUM; u4Idx++) {
		...
		if (NULL != pbReadTable) {
			...
			*(pbReadTable + u4Idx) = (__u8)((u4Data & GAMMA_READ_DAT_R) >> GAMMA_READ_DAT_R_SHF);
			...
			FB_PRINT(FB_LOG_LVL_DBG, "FPD", "TconGetGamma :pbReadTable is NULL\r\n");
...
void vTconSupend(void)
{
	...
	__u32 u4Idx  = 0;
#endif
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "vTconSupend: backup tcon 0x%x, 0x%x, 0x%x\r\n", (unsigned int)tlcp_reg
		, (unsigned int)togc_reg, (unsigned int)tcon_reg);
	memcpy(u4TconReg1, (__u32 *)(tlcp_reg), 0x100);
	memcpy(u4TconReg2, (__u32 *)(togc_reg), 0x100);
	memcpy(u4TconReg3, (__u32 *)(tcon_reg), 0x100);
	memcpy(&u4DClk, (__u32 *)(IO_BASE_ADDRESS + 0x52c00), 0x4);
	memcpy(&u4HVTotal, (__u32 *)(IO_BASE_ADDRESS + 0xa468c), 0x4);
#ifdef HWGETGAMMA

	if ((dReadPGMA(RW_PGMA_CTRL0) & GAMMA_ON) && gb_first) {
		gb_first = FALSE;
		TconGetGamma(g_pbPanelGamma);

		for (u4Idx = 0; u4Idx < TCON_GAMMA_INDEX_NUM - 1; u4Idx++) {
			if (g_pbPanelGamma[u4Idx + 1] < g_pbPanelGamma[u4Idx]) {
				FB_PRINT(FB_LOG_LVL_DBG, "FPD", "gamma is illegal \r\n");
				gb_defaultgamma = TRUE;
				break;
			}
		}
...
{
	FB_PRINT(FB_LOG_LVL_DBG, "FPD", "vTconResume: backup tcon 0x%x, 0x%x, 0x%x\r\n", (unsigned int)tlcp_reg
		, (unsigned int)togc_reg, (unsigned int)tcon_reg);
	memcpy((__u32 *)(tlcp_reg), u4TconReg1, 0x100);
	memcpy((__u32 *)(togc_reg), u4TconReg2, 0x100);
	memcpy((__u32 *)(tcon_reg), u4TconReg3, 0x100);
	memcpy((__u32 *)(IO_BASE_ADDRESS + 0x52c00), &u4DClk, 0x4);
	memcpy((__u32 *)(IO_BASE_ADDRESS + 0xa468c), &u4HVTotal, 0x4);

	if (dReadPGMA(RW_PGMA_CTRL0) & GAMMA_ON) {
#ifdef HWGETGAMMA

		if (gb_defaultgamma) {
			vPanelSetGamma(g_pbGammaDefault);
		} else {
...
void TconSetDither(bool enable, __u32 u4Input, __u32 u4Output)
{
	...
	FB_PRINT(FB_LOG_LVL_INFO, "FPD", "dither enable= %d \r\n", enable);
	...
	FB_PRINT(FB_LOG_LVL_INFO, "FPD", "dither input= %d,output=%d \r\n", (int)u4Input, (int)u4Output);
}
 ...
 int TconGetDither(void)
 {
	 ...
	 value = (dReadPGMA(RW_PGMA_DITHER) & (0x30)) >> (OUT_FMT_SHF);
	 FB_PRINT(FB_LOG_LVL_INFO, "FPD", "get tcon dither %d, dReadPTCON(RW_PGMA_DITHER): 0x%x\r\n", (int)value, dReadPGMA(RW_PGMA_DITHER));
	 return (int)value;
 }


 //level1:0~0xf,  default=0x9;     level2:0~0x7,  default=0x0
 // vSetLVDSTX(0x5,0x0);
/*  level1   level  LVDS_TX   note
    0x1      0x0    100mV     min
    0x9      0x0    300mV     default
    0xf      0x0    500mV     --
    0xf      0x7    600mV     max  */
void vSetLVDSTX(__u32  level1, __u32 level2)
{
     if((level1 > 0) && (level1 <= 0xf))
     {
         vWritePLVDSMsk(RW_LVDSA_CTL1, level1 << 0 , RG_LVDSA_TVO);
     }
     else
     {
         FB_PRINT(FB_LOG_LVL_ERR, "FPD", "Set LVDS TX fail! please set level1 in range of [0~0xf]! \r\n");
     }

     if((level2 > 0) && (level2 <= 0x7))
     {
          vWritePLVDSMsk(RW_LVDSA_CTL2, 0x1 << 29, RG_LVDSA_SEL_MERGE);
          vWritePLVDSMsk(RW_LVDSA_CTL1, level2 << 28, RG_LVDSA_TVO_PRE);

     }
     else
     {
         FB_PRINT(FB_LOG_LVL_ERR, "FPD", "Set LVDS TX fail! please set level2 in range of [0~0x7]! \r\n");
     }

     FB_PRINT(FB_LOG_LVL_ERR, "FPD", "Set LVDS TX,[level1,level2]=[0x%x,0x%x] \r\n",level1,level2);
...
void DisplaySetDrvAbility(__u32 curt)
{
	...
	if (curt == 2) { /* 2mA*/
		...
		*(volatile unsigned int *)0xFD000514 = 0xFFFFFFFF;
		...
		FB_PRINT(FB_LOG_LVL_DBG, "FPD", "current=%d \r\n", (int)curt);
	}     /*none*/
}
