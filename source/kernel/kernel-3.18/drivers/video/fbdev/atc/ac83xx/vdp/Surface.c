...
static __u32 m_u4PaOfInterPool;
static __u32 m_u4VaOfInterPool;
...
__u32 _PhysicalAddress(MTKSurface *pSurf)
{
	__u32 ret = pSurf ? pSurf->u4Pa : 0;

	return ret;
}

__u32 _GetSurfaceCbCrPa(MTKSurface *pSurf)
{
	__u32 u4Pa;
...
__u32 _VirtualAddress(MTKSurface *pSurf)
{
	__u32 ret = pSurf ? pSurf->u4Va : 0;
...
PMTKSurface AllocRoateSurface(UINT32 u4Width, UINT32 u4Height){    
	...
	if(u4Size > 0x400000)       
		FB_PRINT(FB_LOG_LVL_ERR, "VDP", "AllocRoateSurface:size error,%d > 0x10000\r\n", u4Size);       
	u4Pa = m_u4PaOfRotateBuf;   
	u4Va = m_u4VaOfRotateBuf;       
	FB_PRINT(FB_LOG_LVL_DBG, "VDP", "AllocRoateSurface :pa %x va %x w,h %d %d size %d\r\n", (unsigned int)u4Pa, (unsigned int)u4Va, (int)u4Width, (int)u4Height, (int)u4Size);
	pSurf = &_rRoateSurf;    
	pSurf->u4Width = u4Width;   
	pSurf->u4Height = u4Height;    
	pSurf->u4Pa = u4Pa;    
...
PMTKSurface AllocSurface(__u32 u4Width, __u32 u4Height)
{
	...
	if (m_u4FreeOffsetOfInterPool + u4Size > m_u4InterPoolSize) {
		FB_PRINT(FB_LOG_LVL_ERR, "VDP", "AllocSurfaceInternal couldn't allocate physical memory for surface %x\r\n"
			, m_u4InterPoolSize);
		goto clearUp;
	}
	...
	FB_PRINT(FB_LOG_LVL_DBG, "VDP", "AllocSurface :pa %x va %x w,h %d %d size %d\r\n", (unsigned int)u4Pa
		, (unsigned int)u4Va, (int)u4Width, (int)u4Height, (int)u4Size);

	m_u4FreeOffsetOfInterPool += u4Size;

	if (m_u4SurfaceCnt >= MAX_SURFACE) {
		FB_PRINT(FB_LOG_LVL_ERR, "VDP", "AllocSurface : Can not get surface pointer %d\r\n", (int)m_u4SurfaceCnt);
		goto clearUp;
	}
...
void set_pool_param(__u32 base, __u32 va, __u32 size)
{
