...
{
	FB_PRINT(FB_LOG_LVL_DBG, "VDP", "FBM_Init: %d %d\n", (int)u4GroupID, (int)i4Size);
...
void* FBM_Lock(__u32 u4GroupID)
{
	...
	if (!_pTable[u4GroupID]) {
		FB_PRINT(FB_LOG_LVL_ERR, "VDP", "FBM_Lock : Failed ..GRP %d\r\n", u4GroupID);
		return NULL;
	}
	...
	FB_PRINT(FB_LOG_LVL_DBG, "VDP", "FBM_Lock u4Cur = %d pbuffer = %x\n", u4Cur, (unsigned int)pBuffer);
...
void* FBM_Flip(__u32 u4GroupID)
{
	...
	if (!_pTable[u4GroupID]) {
		FB_PRINT(FB_LOG_LVL_ERR, "VDP", "FBM_Flip: Failed u4GroupID %d >>>\n", (int)u4GroupID);
		return NULL;
	}
	...
	FB_PRINT(FB_LOG_LVL_DBG, "VDP", "FBM_Flip grp %d to %d\r\n", (int)u4GroupID, (int)pi4Cur[0]);
	(*pi4Cur)++;
...
void* FBM_GetOnScreen(__u32 u4GroupID)
{
	...
	if (!_pTable[u4GroupID]) {
		FB_PRINT(FB_LOG_LVL_ERR, "VDP", "FBM_GetOnScreen: Failed grp %d\r\n", (int)u4GroupID);
		return NULL;
	}
	...
	FB_PRINT(FB_LOG_LVL_DBG, "VDP", "FBM_GetOnScreen grp %d to %d\r\n", (int)u4GroupID, (int)pi4On[0]);
