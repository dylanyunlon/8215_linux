...
__s32 i4OSD_UninitDAL(void)
{
	if (_fgOsdInit == 1) {
		...
		for (i = OSD_SCALER_1; i < OSD_SCALER_MAX_NUM; i++) {
			...
			IGNORE_RET(OSD_SC_Scale_DAL(i, FALSE, 0, 0, 0, 0));
...
__s32 OSD_Reset(bool fgHwReset)
{
	...
	if (fgHwReset) {
		...
		IO_WRITE32(IO_BASE_VA, 0xB4, (0x10 | IO_READ32(IO_BASE_VA, 0xB4))); /*osd_r_clk_en[4]     add*/
		IO_WRITE32(IO_BASE_VA, 0xD0, (0x10 | IO_READ32(IO_BASE_VA, 0xD0))); /*osd_r_rst_en[4]*/
		IO_WRITE32(IO_BASE_VA, 0xB4, (0x08 | IO_READ32(IO_BASE_VA, 0xB4))); /*osd_clk_en[3]*/
		IO_WRITE32(IO_BASE_VA, 0xD0, (0x08 | IO_READ32(IO_BASE_VA, 0xD0))); /*osd_rst_en[3]*/
		IO_WRITE32(IO_BASE_VA, 0xB4, (0xffff0000 | IO_READ32(IO_BASE_VA, 0xB4))); /*osd_clk_en[3]*/
		IO_WRITE32(IO_BASE_VA, 0xD0, (0xffff0000 | IO_READ32(IO_BASE_VA, 0xD0))); /*osd_rst_en[3]*/
		IO_WRITE32(IO_BASE_VA, 0xD4, (0xffff0000 | IO_READ32(IO_BASE_VA, 0xD4))); /* osd pclk select panel*/
		if((_u4LCDWidth*_u4LCDHeight) > (1024*600)) { /*osd oclk select 147 > pixel clock*/
			IO_WRITE32(IO_BASE_VA, 0x10, (0x50000000 | IO_READ32(IO_BASE_VA, 0x10)));
		} else { /*osd oclk select 64.8*/
			IO_WRITE32(IO_BASE_VA, 0x10, (0x10000000 | IO_READ32(IO_BASE_VA, 0x10)));
		}
...
__s32 OSD_BASE_SetDisplayMode_InVsync(__u32 u4DisplayMode)
{
	...
	__u32 u4OsdHtotal,u4OsdVtotal;
	ACTIVE_OFFSET_VALUE rOsdStartPos;

	u4OsdHtotal = g_rPanelSetting.u4OsdHTotal;
	u4OsdVtotal = g_rPanelSetting.u4OsdVTotal;
	rOsdStartPos.u4OffsetX = g_rPanelSetting.rOsdStart.u4OffsetX;
	rOsdStartPos.u4OffsetY = g_rPanelSetting.rOsdStart.u4OffsetY;

	if((g_rPanelSetting.u4PanelWidth != _u4LCDWidth) || (g_rPanelSetting.u4PanelHeight != _u4LCDHeight))
	{
		//not suport this (i4Width*i4Height) solution now , need add setting @g_rPanelSetting; or other error case
		FB_PRINT(FB_LOG_LVL_ERR, "OSD", "not suport this solution now,Width&height=[%d,%d]\r\n",
			g_rPanelSetting.u4PanelWidth,g_rPanelSetting.u4PanelHeight);

	}

	// FB_PRINT(FB_LOG_LVL_ERR, "OSD", "[osd_base_if] _u4DispMode=%d,u4OsdH&Vtotal=%d,%d,rOsdStartPos=[0x%x,0x%x]\r\n",
	// _u4DispMode,u4OsdHtotal,u4OsdVtotal,rOsdStartPos);
	// 0x20010
	IGNORE_RET(_OSD_BASE_SetScrnHStartOsd1(rOsdStartPos.u4OffsetX));//rOffset.rOSD1.i4XOffset));  // 0x74 = 116
	IGNORE_RET(_OSD_BASE_SetScrnHStartOsd2(rOsdStartPos.u4OffsetX));
	// 0x20014
	IGNORE_RET(_OSD_BASE_SetScrnHStartCsr(rOsdStartPos.u4OffsetX));
	IGNORE_RET(_OSD_BASE_SetScrnHStartOsd3(rOsdStartPos.u4OffsetX));
	// 0x20040
	IGNORE_RET(_OSD_BASE_SetScrnHStartOsd4(rOsdStartPos.u4OffsetX));
	IGNORE_RET(_OSD_BASE_SetScrnHStartOsd5(rOsdStartPos.u4OffsetX));
	...
	// 0x2000C
	IGNORE_RET(_OSD_BASE_SetOvtMain(u4OsdVtotal));  // 0x1E0 = 480
	IGNORE_RET(_OSD_BASE_SetVsWidthMain(0x01));

	IGNORE_RET(_OSD_BASE_SetHsWidthMain(1));   // 1 = 0x01
	IGNORE_RET(_OSD_BASE_SetHsWidthAux(1));   // 1 = 0x01

	IGNORE_RET(_OSD_BASE_SetOhtMain(u4OsdHtotal));
	IGNORE_RET(_OSD_BASE_SetOhtAux(u4OsdHtotal));

	IGNORE_RET(_OSD_BASE_SetOvtAux(u4OsdVtotal));
	IGNORE_RET(_OSD_BASE_SetVsWidthAux(6));

	//0x20018
	IGNORE_RET(_OSD_BASE_SetScrnVStartBotMain(rOsdStartPos.u4OffsetY));  // 0x11C = 284->0x10f
	IGNORE_RET(_OSD_BASE_SetScrnVStartTopMain(rOsdStartPos.u4OffsetY));   // 0x2A
	IGNORE_RET(_OSD_BASE_SetScrnVStartBotAux(rOsdStartPos.u4OffsetY));  // 0x11C = 284
	IGNORE_RET(_OSD_BASE_SetScrnVStartTopAux(rOsdStartPos.u4OffsetY));   // 0x2A

	//0x2001C
	IGNORE_RET(_OSD_BASE_SetScrnHSizeMain(_u4LCDWidth));
	IGNORE_RET(_OSD_BASE_SetScrnVSizeMain(_u4LCDHeight));
	IGNORE_RET(_OSD_BASE_SetScrnHSizeAux(_u4LCDWidth));
	IGNORE_RET(_OSD_BASE_SetScrnVSizeAux(_u4LCDHeight));
...
*/
/*-----------------------------------------------------------------------------*/
__s32 OSD_BASE_SetOsdPosition_DAL(__u32 u4Plane, __u32 u4X, __u32 u4Y)
{
...
EXPORT_SYMBOL(OSD_BASE_SetOsdPosition_DAL);
...
__s32 OSD_R_BASE_SetDisplayMode_InVsync(__u32 u4DisplayMode)
{
	...
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "[osd_base_if] rDspMode =%d.\n", rDspMode);
