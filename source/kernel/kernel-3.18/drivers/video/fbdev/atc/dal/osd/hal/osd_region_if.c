...
static __s32 _OsdRegionCompare(__u32 u4Rgn1, __u32 u4Rgn2)
{
	...
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "[OSD driver] multi region collision, insert region failed\n");
...
*/
__s32 OSD_RGN_LIST_Create_DAL(__u32 *pu4RgnList)
{
#if 1/*(!CONFIG_DRV_VERIFY_SUPPORT)*/
	__s32 i4Index;
#endif
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_LIST_Create begin\n");
#endif
...
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "_rAllRgnList[0].u4ListStatus=%d\n", _rAllRgnList[0].u4ListStatus);
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "_rAllRgnList[1].u4ListStatus=%d\n", _rAllRgnList[1].u4ListStatus);
#endif
...
#ifdef FB_DEBUG
			FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_LIST_Create end\n");
#endif
			return (__s32)OSD_RET_OK;
		}
	}
...
EXPORT_SYMBOL(OSD_RGN_LIST_Create_DAL);
 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 OSD_RGN_LIST_DetachAll_DAL(__u32 u4RgnList)
{
#if 1/*(!CONFIG_DRV_VERIFY_SUPPORT)*/
...
EXPORT_SYMBOL(OSD_RGN_LIST_DetachAll_DAL);
 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 OSD_RGN_Set_DAL(__u32 u4Region, __s32 i4Cmd, __u32 u4Value)
{
	OSD_VERIFY_REGION(u4Region);
 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 OSD_RGN_Create_DAL(__u32 *pu4Region, __u32 u4BmpWidth, __u32 u4BmpHeight,
...
EXPORT_SYMBOL(OSD_RGN_Create_DAL);
			...
			__u32 u4DispW, __u32 u4DispH, __u32 ui4Plane)
{
	...
	if ((pu4Region == NULL) || (pvBitmap == NULL) ||
	    (((__u32)pvBitmap & 0xf) != 0) || (u4BmpWidth == 0) ||
...
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Create_EX begin:u4BmpPitch=%d\n", u4BmpPitch);
#endif
	i4MemChn = OSD_DRV_GetMemChannelEx(ui4Plane);
	i4Ret = _OSD_RGN_Alloc(&u4Region,  i4MemChn);
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Create_EX:i4Ret=%d\n", i4Ret);
#endif
	...
	i4Ret = _OSD_RGN_SetDataAddr(u4Region, ((__u32)pvBitmap));
	...
	i4Ret = _OSD_RGN_SetColorKey(u4Region, 0xFFFF);
#else
	...
	i4Ret = OSD_RGN_Set_DAL(u4Region, (__s32)OSD_RGN_BMP_PITCH, u4BmpPitch);
	...
	/* palette mode*/
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Create_EX:eColorMode=%d\n", eColorMode);
#endif
	...
	if (((eColorMode <= (__u32)OSD_CM_YCBCR_CLUT8) ||
		...
		if ((eColorMode == (__u32)OSD_CM_YCBCR_CLUT2) ||
		    (eColorMode == (__u32)OSD_CM_RGB_CLUT2)) {
			i4Ret = OSD_RGN_Set_DAL(u4Region, (__s32)OSD_RGN_PAL_LEN, 0);
		} else if ((eColorMode == (__u32)OSD_CM_YCBCR_CLUT4) ||
			   (eColorMode == (__u32)OSD_CM_RGB_CLUT4)) {
			i4Ret = OSD_RGN_Set_DAL(u4Region, (__s32)OSD_RGN_PAL_LEN, 1);
		} else if ((eColorMode == (__u32)OSD_CM_YCBCR_CLUT8) ||
			   (eColorMode == (__u32)OSD_CM_RGB_CLUT8)) {
			i4Ret = OSD_RGN_Set_DAL(u4Region, (__s32)OSD_RGN_PAL_LEN, 2);
		}
...
#if 0
		i4Ret = OSD_RGN_Set_DAL(u4Region, (__s32)OSD_RGN_PAL_ADDR,
				    (((__u32)pvBitmap +
				      (u4BmpPitch * u4BmpHeight) + 0x2000) & ~0x1fff));
#endif
		i4Ret = OSD_RGN_Set_DAL(u4Region, (__s32)OSD_RGN_PAL_LOAD, 1);
	}
	...
	*pu4Region = u4Region;
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Create_EX:*pu4Region=%d, _rAllRgnNode[u4Region].u4NodeStatus=%d\n",
...
__s32 OSD_RGN_AttachTail(__u32 u4Region, __u32 u4RgnList)
{
	...
	if (((void *)prRgnList->i4Head == NULL) && (prRgnList->i4Count == 0)) {
		return OSD_RGN_Insert_DAL(u4Region, u4RgnList);
	}
 ...
 */
/*-----------------------------------------------------------------------------*/
__s32 OSD_RGN_Insert_DAL(__u32 u4Region, __u32 u4RgnList)
{
#if 1/*(!CONFIG_DRV_VERIFY_SUPPORT)*/
	__s32 i4RgnCurr, i4RgnNext;
	OSD_REGION_LIST_T *prRgnList;
	__u32 u4ColorMode, u4Address;
	__s32 i4Ignore = 0;
#endif
	...
	OSD_VERIFY_RGNLIST(u4RgnList);
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Insert begin\n");
#endif
...
#ifdef FB_DEBUG
	FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Insert:i4RgnCurr=%d, i4Count=%d\n", i4RgnCurr, prRgnList->i4Count);
#endif
	...
	if (i4RgnCurr == OSD_INVALID_REGION_HANDLE) {
		if (prRgnList->i4Count == 0) {
...
#ifdef FB_DEBUG
			FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Insert:i4Head=%d\n", _rAllRgnList[u4RgnList].i4Head);
#endif
			...
			i4Ignore = _OSD_RGN_GetColorMode(u4Region, &u4ColorMode);
#ifdef FB_DEBUG
			FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Insert:u4ColorMode=%d\n", u4ColorMode);
#endif
...
#ifdef FB_DEBUG
			FB_PRINT(FB_LOG_LVL_DBG, "OSD", "OSD_RGN_Insert end\n");
#endif
			return (__s32)OSD_RET_OK;
		} else {
			return -(__s32)OSD_RET_INV_LIST;
	...
	case -1:
		...
		i4Ignore = _OSD_RGN_GetAddress((__u32)i4RgnCurr, &u4Address);
	...
	while ((i4RgnNext = _rAllRgnNode[i4RgnCurr].i4Next) !=
		...
		case -1:
			...
			i4Ignore = _OSD_RGN_GetAddress((__u32)i4RgnNext, &u4Address);
			/* region --> next*/
			i4Ignore = _OSD_RGN_SetNextRegion(u4Region, u4Address);
...
EXPORT_SYMBOL(OSD_RGN_Insert_DAL);
...
__s32 OSD_RNG_List_Set_Xvycc_En(__u32 u4RgnList)
{
	...
	while (i4Region != OSD_INVALID_REGION_HANDLE) {
		...
		IGNORE_RET(OSD_RGN_Set_DAL(i4Region, (__s32)OSD_RNG_XVYCC_EN, _fgXvYccEn));
...
__s32 OSD_RGN_Dump(__u32 u4Region)
{
	...
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "REGION(%d)\n", u4Region);
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "\tp[%d] n[%d] fg[%d] pos[%d,%d] src_wh[%d,%d] dst_wh[%d,%d]\n",
	    u4Prev, u4Next, u4Flag, u4PosX, u4PosY, u4BmpW, u4BmpH, u4DispW,
	    u4DispH);
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "\tcm[%d] afa[%d] bmp[0x%08x] pitch[0x%x] cliph[%d] clipv[%d]\n",
	    u4ColorMode, u4Alpha, u4BmpAddr, u4BmpPitch, u4ClipH, u4ClipV);
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "\tvflip[%d] hmirror[%d]\n", u4VFlip, u4HMirror);
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "\tpal_load[%d] pal_addr[0x%08x] pal_len[%d] steph[0x%04x]\n",
	    u4PalLoad, u4PalAddr, u4PalLen, u4StepH);
	FB_PRINT(FB_LOG_LVL_INFO, "OSD", "\tstepv[0x%04x] key[0x%08x] key_en[%d] blend_mode[%d]\n",
