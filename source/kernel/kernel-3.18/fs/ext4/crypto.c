...

{
	...
	if (!req) {
		printk_ratelimited(KERN_ERR
				   "%s %s: crypto_request_alloc() failed\n",
				   EXT4_LOG_TAG, __func__);
		return -ENOMEM;
	}
	...
	if (res) {
		printk_ratelimited(
			KERN_ERR
			"%s %s: crypto_ablkcipher_encrypt() returned %d\n",
			EXT4_LOG_TAG, __func__, res);
		return res;
	}
	return 0;
}
...
int ext4_encrypted_zeroout(struct inode *inode, struct ext4_extent *ex)
{
	...
	ext4_msg(inode->i_sb, KERN_CRIT,
		 "%s ext4_encrypted_zeroout ino %lu lblk %u len %u",
		 EXT4_LOG_TAG, (unsigned long) inode->i_ino, lblk, len);
#endif
	...
	while (len--) {
		...
		if (ret != inode->i_sb->s_blocksize) {
			...
			ext4_msg(inode->i_sb, KERN_ERR,
				 "%s bio_add_page failed: %d", EXT4_LOG_TAG, ret);
			WARN_ON(1);
			bio_put(bio);
			err = -EIO;
			goto errout;
