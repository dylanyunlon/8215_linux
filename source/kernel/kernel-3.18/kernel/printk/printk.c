...
static size_t print_time(u64 ts, char *buf)
{
    ...
    tspec.tv_sec = tspec.tv_sec - (sys_tz.tz_minuteswest * 60);
    rtc_time_to_tm(tspec.tv_sec, &tm);

    if (!buf)
        //return snprintf(NULL, 0, "[%5lu.000000] ", (unsigned long)ts);
        return snprintf(NULL, 0, "[%02d-%02d %02d:%02d:%02d.000000] ",
                        tm.tm_mon+1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);

    //return sprintf(buf, "[%5lu.%6lu] ", (unsigned long)ts, rem_nsec / 1000);
    return sprintf(buf, "[%02d-%02d %02d:%02d:%02d.%06d] ",
                    tm.tm_mon+1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, rem_nsec / 1000);
}
