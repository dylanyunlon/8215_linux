...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		if (atc_combo_get_chip_type() == ATC_WIFI_CHIP_TYPE_AIC8800_SDIO) {
			// nop
		} else if (!(rdev->wiphy.features & NL80211_FEATURE_SAE)
				&& auth_type == NL80211_AUTHTYPE_SAE) {
			return false;
		}
#else
...
static int nl80211_send_station(struct sk_buff *msg,
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
		u32 cmd,
#endif
...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
	...
	else if (!(request->flags & NL80211_SCAN_FLAG_RANDOM_ADDR) && info->attrs[NL80211_ATTR_MAC])
		memcpy(request->bssid, nla_data(info->attrs[NL80211_ATTR_MAC]), ETH_ALEN);
	else
...
static int nl80211_join_ibss(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (err)
		kzfree(connkeys);
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
	else if (atc_combo_get_chip_type() == ATC_WIFI_CHIP_TYPE_AIC8800_SDIO
		      ...
		      struct station_info *sinfo, gfp_t gfp)
{
	...
	if (nl80211_send_station(msg,
				...
				NL80211_CMD_NEW_STATION,
#endif
				0, 0, 0, rdev, dev, mac_addr, sinfo) < 0) {
		nlmsg_free(msg);
		return;
	}
...
#ifdef CONFIG_ATC_AOSP_ENHANCEMENT
void atc_cfg80211_del_sta(struct net_device *dev,
			  const u8 *mac_addr, gfp_t gfp, u32 reason)
{
	struct station_info atc_sinfo;
	memset(&atc_sinfo, 0, sizeof(struct station_info));
	/* Autochips:: When disconnected, the connected_time field is not
	 * used, we use this field to characterize the reason code.
	 */
	atc_sinfo.filled = (1ULL << NL80211_STA_INFO_CONNECTED_TIME);
	atc_sinfo.connected_time = reason;
	cfg80211_del_sta_sinfo(dev, mac_addr, &atc_sinfo, gfp);
}
EXPORT_SYMBOL(atc_cfg80211_del_sta);
#endif
