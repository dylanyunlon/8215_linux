...
sub do_nm
{
	...
	if (-e "$source.c" || -e "$source.S") {
		$source = "$objtree$File: //CNB/AC8317/DEV_BR/linux/atc_linux/kernel/kernel-3.18/scripts/namespace.pl $source";
	} else {
		$source = "$srctree$File: //CNB/AC8317/DEV_BR/linux/atc_linux/kernel/kernel-3.18/scripts/namespace.pl $source";
	}
	if (! -e "$source.c" && ! -e "$source.S") {
