...
def lcd_devices_is_egl(lcd_devices):
  if lcd_devices =='egl_for_fsl' or lcd_devices =='egl_for_x11' or lcd_devices =='egl_for_gbm' or lcd_devices =='egl_for_ac8xxx' :
    return True
  return False
...
os.environ["awtk_linux_fb_dir"] = TK_LINUX_FB_ROOT
...
COMMON_CCFLAGS=COMMON_CCFLAGS+' -DLOAD_ASSET_WITH_MMAP=1 -DWITH_SOCKET=1 -DWITH_LCD_CLEAR_ALPHA=1'
...
COMMON_CCFLAGS=COMMON_CCFLAGS+' -DSTATIC_LOAD_RES '

ATC_SHOW_VBA = os.environ.get('ATC_SHOW_VBA', 'false')
if ATC_SHOW_VBA == "false":
	COMMON_CCFLAGS=COMMON_CCFLAGS+' -DATC_SHOW_LOGO_BOOT '
...
#atc revise
AC83XX_TOP_DIR = os.environ.get('TOPDIR')
TOOLS_PREFIX = joinPath(AC83XX_TOP_DIR, '../out/host/bin/arm-buildroot-linux-gnueabi-')
print("TOOLS_PREFIX=",TOOLS_PREFIX)
#for prebuild tslib
TSLIB_LIB_DIR=''
TSLIB_INC_DIR=''
#TOOLS_PREFIX='/data/atc0266/ac8317/out/host/bin/arm-buildroot-linux-gnueabi-'
...
OS_LIBS = ['stdc++', 'pthread', 'rt', 'm', 'dl', 'settings_atc']
print("LD=",LD)
...
#OS_LINKFLAGS= OS_LINKFLAGS + ' -Wl,-rpath=/proj/atc0266/atc0266/ac8317_ttttt/out/target/ac83xx/lib/ '
...
#LIBPATH=[LIB_DIR, BIN_DIR] + OS_LIBPATH + [ATC_LIBPATH]
CCFLAGS=OS_FLAGS + COMMON_CCFLAGS 

STATIC_LIBS =['awtk_global', 'fscript_ext_widgets', 'extwidgets', 'widgets', 'awtk_linux_fb', 'base', 'gpinyin', 'linebreak', 'fribidi','egl_devices']
STATIC_LIBS += TKC_STATIC_LIBS
if TSLIB_LIB_DIR != '':
  SHARED_LIBS=['awtk', 'ts', 'atcbootanicom', 'egl_devices'] + OS_LIBS;
else:
  SHARED_LIBS=['awtk', 'atcbootanicom', 'egl_devices'] + OS_LIBS;
...
CPPPATH=[TK_ROOT, 
  ...
  #atc revise
  joinPath(AC83XX_TOP_DIR, '../source/kernel/kernel-3.18/drivers/misc/atc/bootanimation'), 
  ] + OS_CPPPATH
