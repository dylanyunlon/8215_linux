...
struct {
	...
	.native_visual_id = EGL_DONT_CARE,
};

EGLConfig choose_config(EGLDisplay egl_disp, const char* str, int format)
{
	...
	if (str != NULL) {
		...
		while (*tok != '\0') {
			if (strncmp(tok, "rgba8888", strlen("rgba8888")) == 0 ||
				...
				//egl_conf_attr.native_visual_id = SCREEN_FORMAT_RGBA8888;
				tok += strlen("rgba8888");
			} else if (strncmp(tok, "bgra8888", strlen("bgra8888")) == 0 ||
				strncmp(tok, "bgrx8888", strlen("bgrx8888")) == 0) {
				egl_conf_attr.red_size = 8;
				egl_conf_attr.green_size = 8;
				egl_conf_attr.blue_size = 8;
				egl_conf_attr.alpha_size = 8;
				//egl_conf_attr.native_visual_id = SCREEN_FORMAT_BGRA8888;
				tok += strlen("bgra8888");
			} else if (strncmp(tok, "rgba1010102", strlen("rgba1010102")) == 0 ||
				strncmp(tok, "rgbx1010102", strlen("rgbx1010102")) == 0) {
				egl_conf_attr.red_size = 10;
				egl_conf_attr.green_size = 10;
				egl_conf_attr.blue_size = 10;
				egl_conf_attr.alpha_size = 2;
				//egl_conf_attr.native_visual_id = SCREEN_FORMAT_RGBA1010102;
				tok += strlen("rgba1010102");
			} else if (strncmp(tok, "bgra1010102", strlen("bgra1010102")) == 0 ||
				strncmp(tok, "bgrx1010102", strlen("bgrx1010102")) == 0) {
				egl_conf_attr.red_size = 10;
				egl_conf_attr.green_size = 10;
				egl_conf_attr.blue_size = 10;
				egl_conf_attr.alpha_size = 2;
				//egl_conf_attr.native_visual_id = SCREEN_FORMAT_BGRA1010102;
				tok += strlen("bgra1010102");
				...
				//egl_conf_attr.native_visual_id = SCREEN_FORMAT_RGBA5551;
				tok += strlen("rgba5551");
				...
				//egl_conf_attr.native_visual_id = SCREEN_FORMAT_RGBA4444;
				tok += strlen("rgba4444");
				...
				//egl_conf_attr.native_visual_id = SCREEN_FORMAT_RGB565;
				tok += strlen("rgb565");
			} else if (isdigit(*tok)) {
				val = atoi(tok);
				while (isdigit(*(++tok)));
		...
		egl_conf_attr.native_visual_id = format;
#if 0
		switch (format) {
			...
			case SCREEN_FORMAT_BGRX1010102:
				...
				break;
			default:
				/* If something went wrong, pick any format */
				egl_conf_attr.native_visual_id = EGL_DONT_CARE;
				break;
		}
		#endif
	}
	...
	for (i = 0; i < egl_num_configs; i++) {
		...
		/* Store first suitable format */
		if (!egl_conf) {
			egl_conf = egl_configs[i];
		}

		/* Now try to tune up format with correct native visual type if available, it */
		/* is required for proper RGBA/BGRA selection. */
		if (egl_conf_attr.native_visual_id != EGL_DONT_CARE) {
			eglGetConfigAttrib(egl_disp, egl_configs[i], EGL_NATIVE_VISUAL_ID, &val);
			if ((val != 0) && (val == egl_conf_attr.native_visual_id)) {
				/* We have found detailed format, no need to continue */
				egl_conf = egl_configs[i];
				break;
			}
		}
...
{
	EGLint buffer_bit_depth, alpha_bit_depth, nativeid = 0;

	/* Replace nativeid with real native visual type*/
	eglGetConfigAttrib(egl_disp, egl_conf, EGL_NATIVE_VISUAL_ID, &nativeid);
	if (nativeid != 0) {
		return nativeid;
	}

	/* Lets start our guess the pixel format game */
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++) {
		...
		} else if (strncmp(argv[i], "-nbuffers=", strlen("-nbuffers=")) == 0) {
			tok = argv[i] + strlen("-nbuffers=");
			nbuffers = atoi(tok);
...
#if 0
	/* Create an empty window before choosing pixel format */
	...
	if (rc) {
		perror("screen_get_window_property_iv(SCREEN_PROPERTY_FORMAT)");
		goto fail2;
	}

	egl_conf = choose_config(egl_disp, conf_str, format);
	if (egl_conf == (EGLConfig)0) {
		goto fail2;
	}
#endif
	...
	if (verbose) {
		...
		i = 0;
		eglGetConfigAttrib(egl_disp, egl_conf, EGL_NATIVE_VISUAL_ID, &i);
	...
	//if (format == 0) {
	//	fprintf(stderr, "Can't detect default pixel format for the window!\n");
	//	goto fail3;
	//}
	#if 0
...
#endif
	int screen = DefaultScreen(egl_disp);
	Window window;
	egl_surf = eglCreateWindowSurface(egl_disp, egl_conf,
		screen, (EGLint*)&egl_surf_attr);
	if (egl_surf == EGL_NO_SURFACE) {
		egl_perror("eglCreateWindowSurface");
		//goto fail3;
	}
window = XCreateSimpleWindow(egl_disp, RootWindow(egl_disp, screen), 10, 10, 800, 600, 1,
                                 BlackPixel(egl_disp, screen), WhitePixel(egl_disp, screen));
	...
	gears_init(size[0], size[1]);
	  ...
	  hmi_init();
    hmi_set_eglDisplay(egl_disp);
    hmi_set_eglContext(egl_ctx);
 	hmi_set_eglSurface(egl_surf);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/release-3.2.0/apps/screen/demos/gles2-gears/gles2-gears.c $ $Rev: 942148 $")
#endif
