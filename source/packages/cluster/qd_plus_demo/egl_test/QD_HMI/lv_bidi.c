 ...
 */
lv_base_dir_t _lv_bidi_process(const char * str_in, char * str_out, lv_base_dir_t base_dir)
{
                                ...
                                uint16_t * pos_conv_out, uint16_t pos_conv_len)
{
    ...
    while(rd < len) {
        letter = lv_txt_utf8_next(str_in, &rd);
        pos_conv_rd++;
        dir = lv_bidi_get_letter_dir(letter);
...
static bool lv_bidi_letter_is_weak(uint32_t letter)
{
    ...
    do 
	{
        x = lv_txt_utf8_next(weaks, &i);
        if(letter == x) 
		{
            return true;
        }
    } while(weaks[i] != '\0');
	if((letter >= HMI_FARSI_NUMBER_BEGIN)&&(letter <= HMI_FARSI_NUMBER_END))
	{
		return true;
	}
    return false;
                                  ...
                                  uint16_t  * pos_conv_len)
{
	...
	lv_base_dir_t run_dir = 0;

    uint32_t i_prev = 0;
    uint32_t i_last_strong = 0;
    uint16_t pos_conv_i_prev = 0;
    uint16_t pos_conv_i_last_strong = 0;
	lv_base_dir_t next_dir =0;
	lv_base_dir_t dir =0;

    letter = lv_txt_utf8_next(txt, NULL);
    dir = lv_bidi_get_letter_dir(letter);
                        ...
                        uint16_t pos_conv_len)
{
	...
	uint32_t letter = 0;
	uint16_t pos_conv_letter =0;
	uint32_t last_weak = 0;
	uint32_t first_weak = 0;
	uint16_t pos_conv_last_weak = 0;
	uint16_t pos_conv_first_weak = 0;
	uint32_t letter_size =0;
	uint32_t new_letter = 0;
    while(i) 
	{
         letter = lv_txt_utf8_prev(src, &i);
        ...
        if(lv_bidi_letter_is_weak(letter)) 
		{
            ...
            while(i) 
			{
				...
				{
                    lv_txt_utf8_next(src, &i);   /*Rewind one letter*/
                    pos_conv_i++;
                    first_weak = i;
                    pos_conv_first_weak = pos_conv_i;
                    break;
		...
		{
             letter_size = lv_txt_utf8_size((const char *)&src[i]);
            /*Swap arithmetical symbols*/
            if(letter_size == 1) 
			{
                 new_letter = letter = char_change_to_pair(letter);
                                     ...
                                     lv_base_dir_t base_dir)
{
    ...
    for(i = 0; bracket_left[i] != '\0'; i++) 
	{
        if(bracket_left[i] == letter) 
		{
            ...
            while(txt_i < len) 
			{
                 letter_next = lv_txt_utf8_next(txt, &txt_i);
            ...
            if(txt_i) 
            {
				lv_txt_utf8_prev(txt, &txt_i);
            }
            while(txt_i > 0) 
			{
                 letter_next = lv_txt_utf8_prev(txt, &txt_i);
                 letter_dir = lv_bidi_get_letter_dir(letter_next);
...
#define UTF8_LEN 500
char str_bi[UTF8_LEN]	= {0};
char str_lh[UTF8_LEN]	= {0};

BOOLEAN hmi_bidi(U16 length,U16 * unicode,U16 *pstr_u16)
{
	UINT8	high = 0;
	UINT8	low = 0;
	UINT8	h_high = 0;
	UINT8	h_low = 0;
	UINT8	len_true = 0;
	//char* str_bi	= (char *)(hmi_malloc_vram(UTF8_LEN));
	//char* str_lh	= (char*)(hmi_malloc_vram(UTF8_LEN));
	UINT32 utf8		= 0;
	lv_base_dir_t string_dir = LV_BASE_DIR_AUTO;
	BOOLEAN		 dir_is_rtl = FALSE;
	int i = 0;
	int j = 0;
	memset(str_bi,0,UTF8_LEN);
	memset(str_lh,0,UTF8_LEN);
	for( i = 0,j = 0; i < length;i++)
	{
		utf8 = qd_txt_unicode_to_utf8(unicode[i]);
		high = (utf8 >> 8) & 0xff;
		low = utf8 & 0xff;
		h_high = (utf8 >> 24) & 0xff;
		h_low = (utf8 >> 16) & 0xff;
	...
	str_lh[len_true]=0;
	len_true++;
	string_dir =_lv_bidi_process(str_lh, str_bi, LV_BASE_DIR_AUTO);
	//str_bi[len_true] = 0;
	#if 0
	for(int j = 0; str_bi[j] != 0;j++)
	{
		u8_un[j] = str_bi[j];
	}
	#endif
	MultiMedia_bUtfToUnic(str_bi,len_true,pstr_u16, len_true);
	//hmi_dealloc_vram(str_bi);
	//hmi_dealloc_vram(str_lh);
	if(string_dir == LV_BASE_DIR_RTL)
	{
		dir_is_rtl = TRUE;
	}
	else
