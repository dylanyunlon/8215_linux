                                     ...
                                     lv_text_flag_t flag, uint32_t * word_w_ptr, lv_text_cmd_state_t * cmd_state, bool force)
{
    ...
    while(txt[i] != '\0') {
        ...
        letter_w = 0;/* lq lv_font_get_glyph_width(font, letter, letter_next)*/;
        cur_w += letter_w;
                               ...
                               lv_coord_t letter_space, lv_coord_t max_width, lv_text_flag_t flag)
{
    ...
    while(txt[i] != '\0' && max_width > 0) {
        ...
        if(advance == 0) {
            if(i == 0) lv_txt_utf8_next(txt, &i); // prevent inf loops
            break;
        }
                            ...
                            lv_text_flag_t flag)
{
    ...
    if(length != 0) {
        while(i < length) {
            ...
            lv_coord_t char_width = 0;/* lqlv_font_get_glyph_width(font, letter, letter_next);*/
            if(char_width > 0) {
                width += char_width;
                width += letter_space;
            }
...
#endif
 void * lv_memcpy(void * dst, const void * src, uint32_t len)
{
 ...
 */
 uint8_t lv_txt_utf8_size(const char * str)
{
 ...
 */
 uint32_t lv_txt_utf8_conv_wc(uint32_t c)
{
 ...
 uint32_t lv_txt_utf8_prev(const char * txt, uint32_t * i)
{
    ...
    do {
     ...
     i_tmp  = *i;
     letter = lv_txt_utf8_next(txt, &i_tmp); /*Character found, get it*/
 ...
 */
 uint32_t lv_txt_utf8_get_byte_id(const char * txt, uint32_t utf8_id)
{
    uint32_t i;
    uint32_t byte_cnt = 0;
    for(i = 0; i < utf8_id && txt[byte_cnt] != '\0'; i++) {
        uint8_t c_size = lv_txt_utf8_size(&txt[byte_cnt]);
